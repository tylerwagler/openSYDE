 ...
 * ---------------------------------------------------------------------------------------------------------
 */
class C_TblTreDataElementModel : public C_TblTreModel {
  Q_OBJECT

public:
  enum E_Mode { eNVM_LIST, eDATAPOOLS, eDATAPOOL_ELEMENT, eBUS_SIGNAL };
...
private:
  ...
  static C_PuiSvDbNodeDataPoolListElementId
  mh_Translate(const stw::opensyde_core::C_OscCanMessageIdentificationIndices
                   &orc_Indices,
               const uint32_t ou32_SignalIndex,
               const C_PuiSvDbNodeDataPoolListElementId::E_Type oe_IdType);
  void m_InitBusSignal(
  ...
  static void mh_InitDatapoolElementsHalc(
  ...
  static void mh_InitDatapoolElementsHalcConfig(
      ...
      const stw::opensyde_core::C_OscNodeDataPool &orc_Dp,
      const uint32_t ou32_NodeIndex, const uint32_t ou32_DpIndex,
      const uint32_t ou32_ChannelIndex, const uint32_t ou32_ChannelArrayIndex,
      uint32_t &oru32_CounterParam, uint32_t &oru32_CounterInput,
      uint32_t &oru32_CounterOutput, uint32_t &oru32_CounterStatus,
