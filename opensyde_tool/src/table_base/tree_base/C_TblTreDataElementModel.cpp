 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_TblTreDataElementModel::mhc_ICON_NODE =
    ":/images/system_definition/IconNode.svg";
const QString C_TblTreDataElementModel::mhc_ICON_DATAPOOL =
    ":/images/system_definition/IconDataPoolSmall.svg";
const QString C_TblTreDataElementModel::mhc_ICON_LIST =
    ":/images/system_definition/IconDatapoolList.svg";
const QString C_TblTreDataElementModel::mhc_ICON_VARIABLE =
    ":/images/system_definition/IconVariable.svg";
const QString C_TblTreDataElementModel::mhc_ICON_PARAMETER =
    ":/images/system_definition/IconParameter.svg";
const QString C_TblTreDataElementModel::mhc_ICON_SIGNAL =
    ":/images/system_definition/IconSignal.svg";
const QString C_TblTreDataElementModel::mhc_ICON_ETHERNET =
    ":/images/system_definition/IconBus.svg";
const QString C_TblTreDataElementModel::mhc_ICON_CAN =
    ":/images/system_definition/IconBus.svg";
const QString C_TblTreDataElementModel::mhc_ICON_MESSAGE =
    ":/images/system_definition/IconMessage.svg";
...
const QString C_TblTreDataElementModel::mhc_ADDITIONAL_DATA_POOL_INFO =
    " (Already assigned)";
...
C_TblTreDataElementModel::C_TblTreDataElementModel(QObject *const opc_Parent)
    : C_TblTreModel(opc_Parent), me_Mode(eDATAPOOL_ELEMENT) {}
...
/*! \brief  Set the active node index
   ...
   \param[in]  os32_SkipApplicationIndex     Application index to not display as
   used
   \param[in]  orc_UsedDataPoolIndicesIndex  Data pools to always display as
   used
*/
//----------------------------------------------------------------------------------------------------------------------
  ...
  if (pc_Node != NULL) {
    ...
    for (uint32_t u32_ItDataPool = 0;
      ...
      if (rc_DataPool.e_Type == C_OscNodeDataPool::eCOM) {
        ...
        pc_DataPoolItem->c_Name += C_PuiSdUtil::h_ConvertProtocolTypeToString(
            C_PuiSdUtil::h_GetRelatedCanProtocolType(ou32_NodeIndex,
                                                     u32_ItDataPool));
      ...
      if ((rc_DataPool.s32_RelatedDataBlockIndex < 0) ||
          ((os32_SkipApplicationIndex >= 0) &&
           (rc_DataPool.s32_RelatedDataBlockIndex ==
            os32_SkipApplicationIndex))) {
        bool q_Found = false;
        for (uint32_t u32_It = 0UL;
             u32_It < orc_UsedDataPoolIndicesIndex.size(); ++u32_It) {
        ...
        if (q_Found == false) {
          ...
          pc_DataPoolItem->c_Name +=
              C_TblTreDataElementModel::mhc_ADDITIONAL_DATA_POOL_INFO;
        }
      } else {
        pc_DataPoolItem->q_Enabled = false;
        pc_DataPoolItem->q_Selectable = false;
...
/*! \brief  Initialize tree structure
   ...
   \param[in]  oq_ShowOnlyWriteElements   Optional flag to show only writable
   elements
   \param[in]  oq_ShowArrayElements       Optional flag to hide all array
   elements (if false)
   \param[in]  oq_ShowArrayIndexElements  Optional flag to hide all array index
   elements (if false)
   \param[in]  oq_Show64BitValues         Optional flag to hide all 64 bit
   elements (if false)
   \param[in]  opc_AlreasyUsedElements    Optional pointer to vector with
   already used elements. All added elements will be marked as used an will be
   disabled
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::InitSv(
  ...
  case eDATAPOOL_ELEMENT:
    ...
    if (c_It != C_TblTreDataElementModel::mhc_ViewSetupsDe.end()) {
      ...
      C_TblTreDataElementModel::mh_UpdateDatapoolElement(
          oq_ShowOnlyWriteElements, oq_ShowArrayElements,
          oq_ShowArrayIndexElements, oq_Show64BitValues,
          opc_AlreasyUsedElements, this->mpc_InvisibleRootItem);
    } else {
      this->mpc_InvisibleRootItem = new C_TblTreItem();
      m_InitDatapoolElements(ou32_ViewIndex, oq_ShowOnlyWriteElements,
      ...
      C_TblTreDataElementModel::mhc_ViewSetupsDe.insert(
          c_Hashes,
          static_cast<C_TblTreDataElementModel::C_TblTreDataElementModelState>(
  ...
  case eBUS_SIGNAL:
    ...
    if (c_It != C_TblTreDataElementModel::mhc_ViewSetupsBs.end()) {
      ...
      this->mpc_InvisibleRootItem = new C_TblTreItem();
      m_InitBusSignal(ou32_ViewIndex, oq_ShowOnlyWriteElements,
                      oq_ShowArrayElements, oq_ShowArrayIndexElements,
                      oq_Show64BitValues, opc_AlreasyUsedElements);
      // Clean up (old values probably not necessary in future);
  ...
  case eNVM_LIST:
    ...
    if (c_It != C_TblTreDataElementModel::mhc_ViewSetupsNl.end()) {
      ...
      C_TblTreDataElementModel::mh_UpdateDatapoolElement(
          oq_ShowOnlyWriteElements, oq_ShowArrayElements,
          oq_ShowArrayIndexElements, oq_Show64BitValues, NULL,
          this->mpc_InvisibleRootItem);
    } else {
      // Clean up (old values probably not necessary in future);
...
/*! \brief  Initialize tree structure
   ...
   \param[in]  ou32_SdDataLoggerUseCaseNodeIndex   System definition data logger
   use case: node index
   \param[in]  oq_ShowOnlyWriteElements            Optional flag to show only
   writable elements
   \param[in]  oq_ShowArrayElements                Optional flag to hide all
   array elements (if false)
   \param[in]  oq_ShowArrayIndexElements           Optional flag to hide all
   array index elements (if false)
   \param[in]  oq_Show64BitValues                  Optional flag to hide all 64
   bit elements (if false)
   \param[in]  opc_AlreasyUsedElements             Optional pointer to vector
   with already used elements. All added elements will be marked as used an will
   be disabled
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::InitSdDatapoolElements(
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_TblTreDataElementModel::columnCount(const QModelIndex &orc_Parent) const {
  Q_UNUSED(orc_Parent)
  return 1;
}
...
QModelIndex C_TblTreDataElementModel::GetIndexForItem(
  ...
  if (orc_ItemIndices.size() > 0UL) {
    ...
    do {
      ...
      for (int32_t s32_ItChild = 0;
           s32_ItChild < this->rowCount(c_PreviousParent); ++s32_ItChild) {
        const int32_t s32_COLUMN = 0;
        const QModelIndex c_Tmp =
            this->index(s32_ItChild, s32_COLUMN, c_PreviousParent);
        // lint -e{9079}  Result of Qt interface restrictions, set by index
        // function
        ...
        if (pc_TreeItem != NULL) {
          if (pc_TreeItem->u32_Index == orc_ItemIndices[u32_Counter]) {
            ...
            // Stop otherwise the counter is invalid and we already have found
            // the correct element
            break;
          }
        }
      }
    } while ((u32_Counter < orc_ItemIndices.size()) &&
             (q_FoundSomething == true));
  }
  return c_Retval;
}
...
C_TblTreDataElementModel::GetGenericRepresentationForIndex(
  ...
  while (c_CurItem.isValid() == true) {
    ...
    const C_TblTreItem *const pc_TreeItem =
        static_cast<const C_TblTreItem *>(c_CurItem.internalPointer());
    if (pc_TreeItem != NULL) {
...
void C_TblTreDataElementModel::CleanUpLastModel(void) {
  if (this->mpc_InvisibleRootItem != NULL) {
    ...
    if (mh_Contains(C_TblTreDataElementModel::mhc_ViewSetupsBs,
                    this->mpc_InvisibleRootItem) == false) {
      if (mh_Contains(C_TblTreDataElementModel::mhc_ViewSetupsDe,
                      this->mpc_InvisibleRootItem) == false) {
        if (mh_Contains(C_TblTreDataElementModel::mhc_ViewSetupsNl,
                        this->mpc_InvisibleRootItem) == false) {
          delete (this->mpc_InvisibleRootItem);
        }
      }
    }
    // Setting always to NULL. The saved items for each case will be assigned
    // again when necessary. But it could be that the pointer will get invalid,
    // if a new dialog will replace the saved item and delete the old one.
...
C_PuiSvDbNodeDataPoolListElementId C_TblTreDataElementModel::mh_Translate(
    const C_OscCanMessageIdentificationIndices &orc_Indices,
  ...
  C_PuiSvDbNodeDataPoolListElementId c_Retval(
      0, 0, 0, 0, C_PuiSvDbNodeDataPoolListElementId::eDATAPOOL_ELEMENT, false,
      0UL, false);
  const C_OscNodeDataPool *const pc_DataPool =
      C_PuiSdHandler::h_GetInstance()->GetOscCanDataPool(
  ...
  if (pc_DataPool != NULL) {
    ...
    if (C_OscCanProtocol::h_GetComListIndex(
            *pc_DataPool, orc_Indices.u32_InterfaceIndex,
            orc_Indices.q_MessageIsTx, u32_ListIndex) == C_NO_ERR) {
      const C_OscCanMessageContainer *const pc_Container =
          C_PuiSdHandler::h_GetInstance()->GetCanProtocolMessageContainer(
              orc_Indices.u32_NodeIndex, orc_Indices.e_ComProtocol,
              orc_Indices.u32_InterfaceIndex, orc_Indices.u32_DatapoolIndex);

      const C_OscCanProtocol *const pc_Protocol =
          C_PuiSdHandler::h_GetInstance()->GetCanProtocol(
              orc_Indices.u32_NodeIndex, orc_Indices.e_ComProtocol,
              orc_Indices.u32_DatapoolIndex);

      if ((pc_Container != NULL) && (pc_Protocol != NULL)) {
        const uint32_t u32_SignalDataStartIndex =
            pc_Container->GetMessageSignalDataStartIndex(
                orc_Indices.q_MessageIsTx, orc_Indices.u32_MessageIndex);
        c_Retval = C_PuiSvDbNodeDataPoolListElementId(
            orc_Indices.u32_NodeIndex, pc_Protocol->u32_DataPoolIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::m_InitBusSignal(
  ...
  if ((pc_View != NULL) && (pc_View->GetOscPcData().GetConnected() == true)) {
    // Init sync managers
    // Busses
    if (C_PuiSvHandler::h_GetInstance()->CheckBusDisabled(
            ou32_ViewIndex, pc_View->GetOscPcData().GetBusIndex()) == false) {
      ...
      if (pc_Bus != NULL) {
        ...
        case C_OscSystemBus::eETHERNET:
          pc_BusItem->c_Icon =
              QIcon(C_TblTreDataElementModel::mhc_ICON_ETHERNET);
          break;
        default:
          Q_ASSERT(false);
          break;
        ...
        // Protocols
        pc_BusItem->ReserveChildrenSpace(
            static_cast<uint32_t>(C_OscCanProtocol::hc_ALL_PROTOCOLS.size()));

        for (uint8_t u8_ItProtocol = 0U;
             u8_ItProtocol <
             static_cast<uint8_t>(C_OscCanProtocol::hc_ALL_PROTOCOLS.size());
             ++u8_ItProtocol) {
          ...
          pc_ProtocolItem->c_Name =
              C_PuiSdUtil::h_ConvertProtocolTypeToString(e_Type);
          ...
          // Messages
          if (mh_AddCommMessageItems(
                  pc_ProtocolItem, c_UniqueMessages,
...
/*! \brief  Initialize tree structure for data pool elements
   ...
   with already used elements. All added elements will be marked as used an will
   be disabled
   \param[in]  oq_IsModelUsedInSysViews            True if the model is used for
   a view in system views, false otherwise
   \param[in]  ou32_SdDataLoggerUseCaseNodeIndex   System definition data logger
   use case: node index
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::m_InitDatapoolElements(
    const uint32_t ou32_ViewIndex, const bool oq_ShowOnlyWriteElements,
    const bool oq_ShowArrayElements, const bool oq_ShowArrayIndexElements,
    const bool oq_Show64BitValues,
    const std::vector<C_PuiSvDbNodeDataPoolListElementId> *const
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    for (uint32_t u32_ItNode = 0; u32_ItNode < u32_NodeSize; ++u32_ItNode) {
      ...
      // Model used in a different context than system views e.g. system
      // definition
      if (oq_IsModelUsedInSysViews == false) {
        q_Check = C_PuiSdUtil::h_CheckXappNodeReachable(
            ou32_SdDataLoggerUseCaseNodeIndex, u32_ItNode);
      } else {
      ...
      if ((c_NodeActiveFlags[u32_ItNode] == 1U) && (q_Check == true)) {
        C_TblTreItem *const pc_NodeItem = new C_TblTreItem();
        const C_OscNode *const pc_Node =
            C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(u32_ItNode);
        q_NodeValid = false;
        // Static
        pc_NodeItem->c_Icon = QIcon(C_TblTreDataElementModel::mhc_ICON_NODE);
        // Node
        if ((pc_Node != NULL) && (pc_Node->c_Properties.e_DiagnosticServer ==
          ...
          for (uint32_t u32_ItDataPool = 0;
            ...
            if (rc_DataPool.e_Type == C_OscNodeDataPool::eCOM) {
              ...
              pc_DataPoolItem->c_Name +=
                  C_PuiSdUtil::h_ConvertProtocolTypeToString(
                      C_PuiSdUtil::h_GetRelatedCanProtocolType(u32_ItNode,
                                                               u32_ItDataPool));
              pc_DataPoolItem->c_Name += ")";
              pc_DataPoolItem->c_ToolTipHeading = pc_DataPoolItem->c_Name;
            }
            ...
            C_TblTreDataElementModel::mh_InitDatapoolElementsHalc(
                pc_DataPoolItem, q_DataPoolValid, *pc_Node, rc_DataPool,
                u32_ItNode, u32_ItDataPool, oq_ShowOnlyWriteElements,
                oq_ShowArrayElements, oq_ShowArrayIndexElements,
                oq_Show64BitValues, opc_AlreasyUsedElements);
            if (q_DataPoolValid == true) {
              ...
              C_TblTreDataElementModel::mh_InitDatapoolElementsComm(
                  pc_DataPoolItem, q_DataPoolValid, *pc_Node, rc_DataPool,
                  u32_ItNode, u32_ItDataPool, oq_ShowOnlyWriteElements,
                  oq_ShowArrayElements, oq_ShowArrayIndexElements,
                  oq_Show64BitValues, opc_AlreasyUsedElements);
              if (q_DataPoolValid == true) {
                ...
                if ((rc_DataPool.e_Type == C_OscNodeDataPool::eDIAG) ||
                  ...
                  pc_DataPoolItem->ReserveChildrenSpace(
                      rc_DataPool.c_Lists.size());
                  for (uint32_t u32_ItList = 0;
                       u32_ItList < rc_DataPool.c_Lists.size(); ++u32_ItList) {
                    const C_OscNodeDataPoolList &rc_List =
                        rc_DataPool.c_Lists[u32_ItList];
                    ...
                    q_ListValid = false;
                    // Elements
                    pc_ListItem->ReserveChildrenSpace(
                        rc_List.c_Elements.size());
                    for (uint32_t u32_ItElement = 0;
                         u32_ItElement < rc_List.c_Elements.size();
                         ++u32_ItElement) {
                      ...
                      q_ListValid = true;
                      // Type
                      switch (rc_DataPool.e_Type) // lint !e788 //check for diag
                                                  // or nvm was already done
                      {
                      case C_OscNodeDataPool::eDIAG:
                        q_DataPoolDiagValid = true;
                        pc_ElementItem->c_Icon =
                            QIcon(C_TblTreDataElementModel::mhc_ICON_VARIABLE);
                        break;
                      case C_OscNodeDataPool::eNVM:
                        q_DataPoolNvmValid = true;
                        pc_ElementItem->c_Icon =
                            QIcon(C_TblTreDataElementModel::mhc_ICON_PARAMETER);
                        break;
                      default:
                        break;
                      }
                      ...
                      pc_ElementItem->ConfigureDynamicName(
                          oq_ShowOnlyWriteElements, oq_ShowArrayElements,
                          oq_ShowArrayIndexElements, oq_Show64BitValues,
                          opc_AlreasyUsedElements);
                      ...
                      C_TblTreDataElementModel::mh_CreateArrayElementNodes(
                          oq_ShowOnlyWriteElements, oq_ShowArrayElements,
                          oq_ShowArrayIndexElements, oq_Show64BitValues,
                          rc_Element, oq_IsString, pc_ElementItem, c_Id,
                          opc_AlreasyUsedElements);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::mh_InitDatapoolElementsHalc(
  ...
  if ((opc_DpItem != NULL) &&
      ((orc_Dp.e_Type == C_OscNodeDataPool::eHALC) ||
       (orc_Dp.e_Type == C_OscNodeDataPool::eHALC_NVM))) {
    const C_OscHalcMagicianDatapoolListHandler c_DpHandler(
        orc_Node.c_HalcConfig, C_OscHalcDefDomain::eVA_PARAM,
        orc_Dp.q_IsSafety);
    ...
    pc_UnusedChannelsItem->c_ToolTipContent =
        "Channels which are not used by this datapool";
    pc_UnusedChannelsItem->q_Selectable = false;
    for (uint32_t u32_ItDomain = 0UL;
         u32_ItDomain < orc_Node.c_HalcConfig.GetDomainSize(); ++u32_ItDomain) {
      const C_OscHalcDefDomain *const pc_DomainDef =
          orc_Node.c_HalcConfig.GetDomainDefDataConst(u32_ItDomain);
      const C_OscHalcConfigDomain *const pc_DomainConfig =
      ...
      if ((pc_DomainConfig != NULL) && (pc_DomainDef != NULL)) {
        ...
        if (pc_DomainConfig->c_ChannelConfigs.size() > 0UL) {
          ...
          for (uint32_t u32_ItConfig = 0UL;
               u32_ItConfig < pc_DomainConfig->c_ChannelConfigs.size();
               ++u32_ItConfig) {
            const C_OscHalcConfigChannel &rc_Config =
                pc_DomainConfig->c_ChannelConfigs[u32_ItConfig];
            if (c_DpHandler.CheckChanPresent(rc_Config)) {
              // Move section to appropriate node
              C_TblTreItem *const pc_BaseItem =
                  c_DpHandler.CheckChanRelevant(rc_Config)
                      ? opc_DpItem
                      : pc_UnusedChannelsItem;
              bool *const pq_Valid = c_DpHandler.CheckChanRelevant(rc_Config)
                                         ? &orq_HalcValid
                                         : &q_UnusedNodeValid;
              ...
              C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfig(
                  pc_BaseItem, *pq_Valid, *pc_DomainDef, *pc_DomainConfig,
                  rc_Config, pc_DomainDef->c_ChannelValues, orc_Dp,
                  ou32_NodeIndex, ou32_DpIndex, u32_ItConfig,
                  u32_RelevantChannelIndex, u32_CurCounterParam,
                  u32_CurCounterInput, u32_CurCounterOutput,
                  u32_CurCounterStatus, u32_RelevantChannels,
                  oq_ShowOnlyWriteElements, oq_ShowArrayElements,
                  oq_ShowArrayIndexElements, oq_Show64BitValues, c_DpHandler,
                  opc_AlreasyUsedElements, u32_DatapoolUniqueChannelCounter);
              // Iterate channel
          ...
          if (c_DpHandler.CheckChanPresent(pc_DomainConfig->c_DomainConfig)) {
            // Move section to appropriate node
            C_TblTreItem *const pc_BaseItem =
                c_DpHandler.CheckChanRelevant(pc_DomainConfig->c_DomainConfig)
                    ? opc_DpItem
                    : pc_UnusedChannelsItem;
            bool *const pq_Valid =
                c_DpHandler.CheckChanRelevant(pc_DomainConfig->c_DomainConfig)
            ...
            C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfig(
                pc_BaseItem, *pq_Valid, *pc_DomainDef, *pc_DomainConfig,
                pc_DomainConfig->c_DomainConfig, pc_DomainDef->c_DomainValues,
                orc_Dp, ou32_NodeIndex, ou32_DpIndex, 0UL, 0UL,
                u32_CounterParam, u32_CounterInput, u32_CounterOutput,
                u32_CounterStatus, u32_RelevantChannels,
                oq_ShowOnlyWriteElements, oq_ShowArrayElements,
                oq_ShowArrayIndexElements, oq_Show64BitValues, c_DpHandler,
                opc_AlreasyUsedElements, u32_DatapoolUniqueChannelCounter);
            // Iterate channel
...
/*! \brief  Init datapool elements HALC config
   ...
   with already used elements. All added elements will be marked as used an will
   be disabled
   \param[in]      ou32_DatapoolUniqueChannelCounter  Datapool unique channel
   counter
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfig(
    C_TblTreItem *const opc_DpItem, bool &orq_HalcValid,
    ...
    const C_OscNodeDataPool &orc_Dp, const uint32_t ou32_NodeIndex,
    const uint32_t ou32_DpIndex, const uint32_t ou32_ChannelIndex,
    ...
    uint32_t &oru32_CounterStatus, const uint32_t ou32_RelevantChannelNumber,
    const bool oq_ShowOnlyWriteElements, const bool oq_ShowArrayElements,
    const bool oq_ShowArrayIndexElements, const bool oq_Show64BitValues,
  ...
  const bool q_ChanNumVarNecessary =
      orc_DpHandler.CheckChanNumVariableNecessary(orc_DomainConfig);
  const bool q_UseCaseVarNecessary =
      orc_DpHandler.CheckUseCaseVariableNecessary(orc_DomainConfig);
  const bool q_SafetyFlagVarNecessary =
  ...
  case C_OscHalcDefDomain::eCA_INPUT:
    pc_ChannelItem->c_Icon =
        QIcon(C_TblTreDataElementModel::mhc_ICON_HALC_INPUT);
    break;
  case C_OscHalcDefDomain::eCA_OUTPUT:
    pc_ChannelItem->c_Icon =
        QIcon(C_TblTreDataElementModel::mhc_ICON_HALC_OUTPUT);
    break;
  case C_OscHalcDefDomain::eCA_OTHER:
    pc_ChannelItem->c_Icon =
        QIcon(C_TblTreDataElementModel::mhc_ICON_HALC_MISC);
    break;
  default:
    Q_ASSERT(false);
    break;
  ...
  // Fill
  for (uint32_t u32_ItList = 0UL; u32_ItList < orc_Dp.c_Lists.size();
       ++u32_ItList) {
    const C_OscNodeDataPoolList &rc_List = orc_Dp.c_Lists[u32_ItList];
    if (rc_List.c_Name ==
        C_OscHalcMagicianUtil::h_GetListName(C_OscHalcDefDomain::eVA_PARAM)) {
      C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfigList(
          pc_ChannelItem, q_ChannelValid, orc_Values.c_Parameters, rc_List,
          ou32_ChannelArrayIndex, ou32_NodeIndex, ou32_DpIndex, u32_ItList,
    ...
    } else if (rc_List.c_Name == C_OscHalcMagicianUtil::h_GetListName(
                                     C_OscHalcDefDomain::eVA_INPUT)) {
      C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfigList(
          pc_ChannelItem, q_ChannelValid, orc_Values.c_InputValues, rc_List,
          ou32_ChannelArrayIndex, ou32_NodeIndex, ou32_DpIndex, u32_ItList,
      ...
      C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfigList(
          pc_ChannelItem, q_ChannelValid, orc_Values.c_OutputValues, rc_List,
          ou32_ChannelArrayIndex, ou32_NodeIndex, ou32_DpIndex, u32_ItList,
      ...
      C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfigList(
          pc_ChannelItem, q_ChannelValid, orc_Values.c_StatusValues, rc_List,
          ou32_ChannelArrayIndex, ou32_NodeIndex, ou32_DpIndex, u32_ItList,
...
void C_TblTreDataElementModel::mh_InitDatapoolElementsHalcConfigList(
    ...
    const uint32_t ou32_UseCase, uint32_t &oru32_Counter,
    const uint32_t ou32_RelevantChannelNumber,
    const bool oq_ChanNumVarNecessary, const bool oq_UseCaseVarNecessary,
    const bool oq_SafetyFlagVarNecessary, const bool oq_IsParam,
  ...
  for (uint32_t u32_ItStruct = 0UL; u32_ItStruct < orc_Values.size();
       ++u32_ItStruct) {
    ...
    if (rc_Struct.c_UseCaseAvailabilities.size() > 0UL) {
      for (uint32_t u32_ItUseCase = 0UL;
           u32_ItUseCase < rc_Struct.c_UseCaseAvailabilities.size();
           ++u32_ItUseCase) {
    ...
    if (rc_Struct.c_StructElements.size() > 0UL) {
      for (uint32_t u32_ItEl = 0UL;
           u32_ItEl < rc_Struct.c_StructElements.size(); ++u32_ItEl) {
        if (q_IsRelevant) {
          C_TblTreDataElementModel::mh_AddHalcItem(
              pc_ListItem, rc_Struct.c_StructElements[u32_ItEl], ou32_NodeIndex,
              ou32_DpIndex, ou32_ListIndex, oru32_Counter,
              ou32_RelevantChannelNumber, ou32_ChannelArrayIndex,
              oq_ShowOnlyWriteElements, oq_ShowArrayElements,
              oq_ShowArrayIndexElements, oq_Show64BitValues,
              opc_AlreasyUsedElements, orc_HalChannelOrDomainName);
        } else {
          C_TblTreDataElementModel::mh_AddHalcItem(
              pc_OtherItem, rc_Struct.c_StructElements[u32_ItEl],
              ou32_NodeIndex, ou32_DpIndex, ou32_ListIndex, oru32_Counter,
              ou32_RelevantChannelNumber, ou32_ChannelArrayIndex,
              oq_ShowOnlyWriteElements, oq_ShowArrayElements,
              oq_ShowArrayIndexElements, oq_Show64BitValues,
      ...
      if (q_IsRelevant) {
        C_TblTreDataElementModel::mh_AddHalcItem(
            pc_ListItem, rc_Struct, ou32_NodeIndex, ou32_DpIndex,
            ou32_ListIndex, oru32_Counter, ou32_RelevantChannelNumber,
            ou32_ChannelArrayIndex, oq_ShowOnlyWriteElements,
            oq_ShowArrayElements, oq_ShowArrayIndexElements, oq_Show64BitValues,
            opc_AlreasyUsedElements, orc_HalChannelOrDomainName);
      } else {
        C_TblTreDataElementModel::mh_AddHalcItem(
            pc_OtherItem, rc_Struct, ou32_NodeIndex, ou32_DpIndex,
            ou32_ListIndex, oru32_Counter, ou32_RelevantChannelNumber,
            ou32_ChannelArrayIndex, oq_ShowOnlyWriteElements,
            oq_ShowArrayElements, oq_ShowArrayIndexElements, oq_Show64BitValues,
            opc_AlreasyUsedElements, orc_HalChannelOrDomainName);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::mh_AddHalcItem(
    C_TblTreItem *const opc_BaseItem, const C_OscHalcDefElement &orc_HalcItem,
    const uint32_t ou32_NodeIndex, const uint32_t ou32_DpIndex,
    const uint32_t ou32_ListIndex, uint32_t &oru32_Counter,
    const uint32_t ou32_RelevantChannelNumber,
  ...
  C_TblTreDataElementModel::mh_AddHalcTreeItem(
      ...
      (orc_HalcItem.GetComplexType() == C_OscHalcDefContent::eCT_STRING) ||
          (ou32_RelevantChannelNumber != 1UL),
      orc_HalcItem.GetComplexType() == C_OscHalcDefContent::eCT_STRING,
      oq_ShowOnlyWriteElements, oq_ShowArrayElements, oq_ShowArrayIndexElements,
      oq_Show64BitValues, opc_AlreasyUsedElements);
  // Iterate
  oru32_Counter += C_OscHalcMagicianDatapoolListHandler::h_CountElements(
      orc_HalcItem, ou32_RelevantChannelNumber);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::mh_AddHalcTreeItem(
    C_TblTreItem *const opc_ListItem, const QString &orc_Name,
  ...
  if (pc_OscElement != NULL) {
    C_TblTreDataElementItem *const pc_ElementItem = new C_TblTreDataElementItem(
        oq_IsArray && (!oq_IsString), oq_IsArray, orc_Name,
    ...
    pc_ElementItem->c_ToolTipContent =
        C_SdUtil::h_GetToolTipContentDpListElement(orc_Id);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::mh_InitDatapoolElementsComm(
  ...
  if ((opc_DpItem != NULL) && (orc_Dp.e_Type == C_OscNodeDataPool::eCOM)) {
    ...
    Q_ASSERT(C_PuiSdHandler::h_GetInstance()->GetCanProtocolType(
                 ou32_NodeIndex, ou32_DpIndex, e_Protocol) == C_NO_ERR);

    // get all message ids
    for (uint32_t u32_ItInterface = 0;
         u32_ItInterface < orc_Node.c_Properties.c_ComInterfaces.size();
         u32_ItInterface++) {
      ...
      if (rc_CurInterface.e_InterfaceType == C_OscSystemBus::eCAN) {
        ...
        for (std::vector<const C_OscCanMessageContainer *>::const_iterator
          ...
          if (pc_Container != NULL) {
            ...
            for (uint32_t u32_ItTxMessage = 0;
                 u32_ItTxMessage < pc_Container->c_TxMessages.size();
                 u32_ItTxMessage++) {
              const C_OscCanMessageIdentificationIndices c_MessageId(
                  ou32_NodeIndex, e_Protocol, u32_ItInterface, ou32_DpIndex,
                  true, u32_ItTxMessage);
              c_MessageIds.push_back(c_MessageId);
            }

            // Add Rx message identification indices
            for (uint32_t u32_ItRxMessage = 0;
                 u32_ItRxMessage < pc_Container->c_RxMessages.size();
                 u32_ItRxMessage++) {
              const C_OscCanMessageIdentificationIndices c_MessageId(
                  ou32_NodeIndex, e_Protocol, u32_ItInterface, ou32_DpIndex,
                  false, u32_ItRxMessage);
              c_MessageIds.push_back(c_MessageId);
            }
          }
        }
    ...
    if (C_TblTreDataElementModel::mh_AddCommMessageItems(
            opc_DpItem, c_MessageIds,
            C_PuiSvDbNodeDataPoolListElementId::eDATAPOOL_ELEMENT,
            oq_ShowOnlyWriteElements, oq_ShowArrayElements,
            oq_ShowArrayIndexElements, oq_Show64BitValues,
            opc_AlreasyUsedElements) == true) {
...
/*! \brief  Add COMM message item
   ...
   \retval  true     At least one message with at least one signal was added to
   opc_BaseItem
...
bool C_TblTreDataElementModel::mh_AddCommMessageItems(
  ...
  if (opc_BaseItem != NULL) {
    ...
    for (uint32_t u32_ItMessage = 0U; u32_ItMessage < orc_MessageIds.size();
         ++u32_ItMessage) {
      C_TblTreItem *const pc_MessageItem = new C_TblTreItem();
      const C_OscCanMessage *const pc_Message =
          C_PuiSdHandler::h_GetInstance()->GetCanMessage(
      ...
      if (pc_Message != NULL) {
        ...
        pc_MessageItem->c_Icon =
            QIcon(C_TblTreDataElementModel::mhc_ICON_MESSAGE);
          ...
          if (pc_Element != NULL) {
            ...
            pc_ElementItem->c_ToolTipContent =
                C_SdUtil::h_GetToolTipContentSignal(
                    orc_MessageIds[u32_ItMessage], u32_ItSignal);
            pc_ElementItem->c_Icon =
                QIcon(C_TblTreDataElementModel::mhc_ICON_SIGNAL);
            pc_ElementItem->ConfigureDynamicName(
                oq_ShowOnlyWriteElements, oq_ShowArrayElements,
                oq_ShowArrayIndexElements, oq_Show64BitValues,
                opc_AlreasyUsedElements);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Adapts the properties of already existing tree structure for data
   pool elements
...
void C_TblTreDataElementModel::mh_UpdateDatapoolElement(
  ...
  if (opc_Tree != NULL) {
    C_TblTreDataElementItem *const pc_AdaptableItem =
        dynamic_cast<C_TblTreDataElementItem *const>(opc_Tree);
    if (pc_AdaptableItem != NULL) {
      pc_AdaptableItem->ConfigureDynamicName(
          oq_ShowOnlyWriteElements, oq_ShowArrayElements,
          oq_ShowArrayIndexElements, oq_Show64BitValues,
          opc_AlreasyUsedElements);
    }
    for (uint32_t u32_It = 0UL; u32_It < opc_Tree->c_Children.size();
         ++u32_It) {
      C_TblTreDataElementModel::mh_UpdateDatapoolElement(
          oq_ShowOnlyWriteElements, oq_ShowArrayElements,
          oq_ShowArrayIndexElements, oq_Show64BitValues,
          opc_AlreasyUsedElements, opc_Tree->c_Children[u32_It]);
    }
  }
}
...
/*! \brief  Handle array element section
   ...
   \param[in]  oq_IsStringElement         Flag if current item is an string
   element
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblTreDataElementModel::mh_CreateArrayElementNodes(
    const bool oq_ShowOnlyWriteElements, const bool oq_ShowArrayElements,
    const bool oq_ShowArrayIndexElements, const bool oq_Show64BitValues,
  ...
  if (opc_ElementItem != NULL) {
    if ((orc_Element.GetArray()) && (oq_IsStringElement == false)) {
      // Create new
      for (uint32_t u32_ArrayIndex = 0U;
           u32_ArrayIndex < orc_Element.GetArraySize(); ++u32_ArrayIndex) {
        const C_PuiSvDbNodeDataPoolListElementId c_Id(
            orc_ParentId.u32_NodeIndex, orc_ParentId.u32_DataPoolIndex,
            orc_ParentId.u32_ListIndex, orc_ParentId.u32_ElementIndex,
            C_PuiSvDbNodeDataPoolListElementId::eDATAPOOL_ELEMENT, true,
            u32_ArrayIndex, true);
        C_TblTreDataElementItem *const pc_ArrayItem =
            new C_TblTreDataElementItem(
                true, false, static_cast<QString>("[%1]").arg(u32_ArrayIndex),
...
void C_TblTreDataElementModel::m_InitNvmList(const uint32_t ou32_ViewIndex) {
  ...
  if ((pc_View != NULL) && (s32_Retval == C_NO_ERR)) {
    ...
    Q_ASSERT(c_NodeActiveFlags.size() == u32_NodeSize);
    for (uint32_t u32_ItNode = 0; u32_ItNode < u32_NodeSize; ++u32_ItNode) {
      if ((c_NodeActiveFlags[u32_ItNode] == 1U) &&
          (mh_SvCheckNodeDiagnostic(ou32_ViewIndex, u32_ItNode) == true)) {
        C_TblTreItem *const pc_NodeItem = new C_TblTreItem();
        const C_OscNode *const pc_Node =
            C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(u32_ItNode);
        q_NodeValid = false;
            ...
            for (uint32_t u32_ItList = 0;
              ...
              if (rc_List.c_Elements.size() > 0) {
                ...
                if ((rc_DataPool.e_Type == C_OscNodeDataPool::eNVM) ||
                  ...
                  // Check usage
                  if (pc_View->CheckNvmParamListUsage(C_OscNodeDataPoolListId(
                          u32_ItNode, u32_ItDataPool, u32_ItList)) == true) {
...
C_TblTreDataElementModel::m_GetDatapools(const QModelIndex &orc_Index) const {
  ...
  if (pc_TreeItem != NULL) {
    const C_TblTreItem *const pc_FirstParent =
        dynamic_cast<const C_TblTreItem *const>(pc_TreeItem->pc_Parent);
    if ((pc_FirstParent != NULL) && (pc_FirstParent->pc_Parent != NULL)) {
      ...
      if ((pc_SecondParent != NULL) && (pc_SecondParent->pc_Parent != NULL)) {
        ...
        if ((pc_ThirdParent != NULL) && (pc_ThirdParent->pc_Parent != NULL)) {
          ...
          c_Retval.emplace_back(C_PuiSvDbNodeDataPoolListElementId(
              pc_SecondParent->u32_Index, pc_TreeItem->u32_Index, 0UL, 0UL,
      ...
      if ((pc_SecondParent != NULL) && (pc_SecondParent->pc_Parent != NULL)) {
        ...
        if ((pc_ThirdParent != NULL) && (pc_ThirdParent->pc_Parent != NULL)) {
          const C_TblTreItem *const pc_FourthParent =
              dynamic_cast<const C_TblTreItem *const>(
                  pc_ThirdParent->pc_Parent);
          if ((pc_FourthParent != NULL) &&
              (pc_FourthParent->pc_Parent != NULL)) {
            // Should not happen
            Q_ASSERT(false);
          } else {
            // List
            ...
            if (pc_OscList != NULL) {
              for (uint32_t u32_ItElement = 0;
                   u32_ItElement < pc_OscList->c_Elements.size();
                   ++u32_ItElement) {
                c_Retval.emplace_back(C_PuiSvDbNodeDataPoolListElementId(
                    u32_NodeIndex, u32_DataPoolIndex, u32_ListIndex,
                    u32_ItElement,
...
C_TblTreDataElementModel::mh_GetViewSdHash(const uint32_t ou32_ViewIndex) {
  ...
  if ((pc_View != NULL) && (s32_Retval == C_NO_ERR)) {
    ...
    // Active flags
    for (uint32_t u32_ItNode = 0; u32_ItNode < c_NodeActiveFlags.size();
         ++u32_ItNode) {
    ...
    // The parameter widget has an unique configuration which has side effects
    // to the selection model too and must be added to the view hash
    for (u32_DashboardCounter = 0U; u32_DashboardCounter < rc_Dashboards.size();
      ...
      for (u32_ParamCounter = 0U; u32_ParamCounter < rc_Params.size();
        ...
        for (u32_ElementConfigCounter = 0U;
             ...
             rc_Params[u32_ParamCounter].c_DataPoolElementsConfig.size();
             ++u32_ElementConfigCounter) {
          // Only the element configuration of the parameter widget is relevant
          // for this hash calculation
          rc_Params[u32_ParamCounter]
              .c_DataPoolElementsConfig[u32_ElementConfigCounter]
              .CalcHash(u32_Hash);
        }
      }
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_TblTreDataElementModel::mh_SvCheckNodeDiagnostic(
    const uint32_t ou32_ViewIndex, const uint32_t ou32_NodeIndex) {
  bool q_Retval = false;
  const C_SyvRoRouteCalculation c_RouteCalculation(
      ou32_ViewIndex, ou32_NodeIndex, C_OscRoutingCalculation::eDIAGNOSTIC);
...
/*! \brief  Get current node active flags
   ...
   \param[in]      ou32_ViewIndex            View index
   \param[in]      oq_IsModelUsedInSysViews  True if the model is used for a
   view in system views, false otherwise
...
int32_t C_TblTreDataElementModel::mh_GetCurrentNodeActiveFlags(
  ...
  if (oq_IsModelUsedInSysViews == false) {
    ...
    s32_Retval =
        C_PuiSvHandler::h_GetInstance()->GetNodeActiveFlagsWithSquadAdaptions(
            ou32_ViewIndex, orc_NodeActiveFlags);
  }
  return s32_Retval;
}
