...
C_TblTreSimpleModel::C_TblTreSimpleModel(QObject *const opc_Parent)
    : QAbstractItemModel(opc_Parent), mpc_InvisibleRootItem(NULL) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
QModelIndex C_TblTreSimpleModel::index(const int32_t os32_Row,
  ...
  if (hasIndex(os32_Row, os32_Column, orc_Parent) == true) {
    if (orc_Parent.isValid() == true) {
      ...
      if (pc_TreeItem != NULL) {
        c_Retval = this->createIndex(
            os32_Row, os32_Column,
            pc_TreeItem->c_Children.at(static_cast<uint32_t>(os32_Row)));
      }
    } else {
      if (this->mpc_InvisibleRootItem != NULL) {
        // Top level
        c_Retval = this->createIndex(os32_Row, os32_Column,
                                     this->mpc_InvisibleRootItem->c_Children.at(
                                         static_cast<uint32_t>(os32_Row)));
      }
    }
  }
  return c_Retval;
...
/*! \brief  Get parent index
   ...
   Warning: currently this implementation works,
            but if a tree requires more layers this implementation has to be
   adapted
  ...
  if (orc_Index.isValid() == true) {
    ...
    if (pc_TreeItem != NULL) {
      if (pc_TreeItem->pc_Parent != NULL) {
        const int32_t s32_Index =
            pc_TreeItem->pc_Parent->GetIndexInParentNumber();
        if (s32_Index >= 0) {
...
/*! \brief  Count layers
   ...
   \param[in]   orc_Index           Index
   \param[out]  oru32_ValidLayers   Number of valid layers, starts with 1 for
   the top layer
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_TblTreSimpleModel::m_CountLayers(const QModelIndex &orc_Index,
                                           uint32_t &oru32_ValidLayers) const {
