...
/*!
   \file
   \brief       Table model base class providing an interface for common actions
   (as far as possible) (implementation)

   Table model base class providing an interface for common actions (as far as
   possible)
   ...
   ** Move selected items (one direction only!)
   ...
   This interface is currently only intended to be used with the rowCount
   reflecting the current item count (may be split if required, don't forget the
   actions! e.g. beginMoveRow endMoveRow)
...
uint32_t C_TblModelAction::DeleteSelectedItems(
  ...
  for (uint32_t u32_ItSection = c_ContiguousSections.size();
    ...
    if (rc_Section.size() > 0UL) {
      ...
      // to keep next index valid)
      for (uint32_t u32_ItItem = rc_Section.size(); u32_ItItem > 0UL;
           --u32_ItItem) {
  ...
  if (this->m_GetSizeItems() > 0UL) {
    // Get best index to select after this action -> ideal: one item after last
    // deleted item
    //  (or the one before that if it was the end)
    u32_Retval = std::min(
        this->m_GetSizeItems() - 1U,
...
void C_TblModelAction::MoveSelectedItems(
  ...
  // Step 1 sort (so the next step can assume the contiguous selection has the
  // same order!
  C_Uti::h_SortIndicesAscendingAndSync(c_SelectedIndicesCopy, c_TargetIndices);

  // Step 2: get contiguous selection
  c_ContiguousSections =
      C_Uti::h_GetContiguousSectionsAscending(orc_SelectedIndices);

  // Step 3: move each contiguous section
  for (uint32_t u32_ItSection = 0UL;
       u32_ItSection < c_ContiguousSections.size(); ++u32_ItSection) {
    bool q_Continue;
    ...
    if (q_InBetween) {
      ...
      if (oq_Up) {
        ...
        if (u32_ItSection ==
            (static_cast<uint32_t>(c_ContiguousSections.size()) - 1UL)) {
          q_Continue = false;
        } else {
          q_Continue = true;
        }
    ...
    if (q_Continue) {
      ...
      if (rc_Section.size() > 0UL) {
        ...
        // Special adaptation of target index for moving "down" (our & qt
        // interface)
        if (u32_TargetIndex > rc_Section[0UL]) {
          // Qt interface seems to insert the items before removing anything so
          // the "new position" has to have an
          //  bigger offset
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check if all indices are inside of item size boundary (can be used
   for move operation target index check)
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Reload table content (after change of data outside this classes
   responsibility)

   Warning: should only be used if type of change has unknown effect on complete
   table content
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblModelAction::m_BeginRemoveRows(const uint32_t ou32_FirstIndex,
                                         const uint32_t ou32_LastIndex) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblModelAction::m_EndRemoveRows(const uint32_t ou32_FirstIndex,
                                       const uint32_t ou32_LastIndex) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_TblModelAction::mh_HandleHexValue(const uint64_t ou64_Value,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Utility function to transform bool value into Qt::CheckState stored
   as QVariant
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Utility function to transform Qt::CheckState stored as QVariant into
   bool
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_TblModelAction::mh_GetCheckStateVariantAsBool(
...
void C_TblModelAction::m_MoveItems(
  ...
  if (orc_ContiguousIndices.size() > 0UL) {
    ...
    for (uint32_t u32_ItItem = 0UL; u32_ItItem < orc_ContiguousIndices.size();
         ++u32_ItItem) {
      if (q_Forward == true) {
        // There will always be a "new item" at the same position (if the move
        // is implemented correctly!)
        this->m_MoveItem(orc_ContiguousIndices[0UL], ou32_TargetIndex);
      } else {
        // The item position will not change but the target will (if the move is
        // implemented correctly!)
        this->m_MoveItem(orc_ContiguousIndices[u32_ItItem],
                         ou32_TargetIndex + u32_ItItem);
      }
    }
  }
}
