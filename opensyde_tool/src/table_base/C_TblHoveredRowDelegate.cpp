...
/*!
   \file
   \brief       Table delegate drawing an icon and a hovered-row border (implementation)

   Paints the item normally, then the index's icon, then -- for the row currently under the
   mouse -- a border in the highlight colour. The left and right edges are only drawn on the
   first and last column, so the border reads as one rectangle around the whole row rather
   than a box around each cell; that is why the delegate needs the model, purely to ask how
   many columns there are.

   This replaced five byte-identical copies (bus-edit message/signal table, HALC overview,
   CANopen overview, CANopen PDO table, data-logger job overview). Two of them also carried
   an mc_CheckMark pixmap member that was constructed from a resource and never used.
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_TblHoveredRowDelegate::C_TblHoveredRowDelegate(QObject * const opc_Parent) :
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblHoveredRowDelegate::paint(QPainter * const opc_Painter, const QStyleOptionViewItem & orc_Option,
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_TblHoveredRowDelegate::SetHoveredRow(const int32_t & ors32_Value)
{
   bool q_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblHoveredRowDelegate::SetModel(const QAbstractTableModel * const opc_Value)
{
