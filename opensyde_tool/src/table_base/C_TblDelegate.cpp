...
/*!
   \file
   \brief       Delegate (interaction) component base for any table
   (implementation)
...
C_TblDelegate::C_TblDelegate(QObject *const opc_Parent)
    : QStyledItemDelegate(opc_Parent), mq_InitialSelection(true) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
QWidget *C_TblDelegate::createEditor(QWidget *const opc_Parent,
  ...
  if (orc_Index.data(ms32_USER_ROLE_INTERACTION_IS_LINK).toBool() == false) {
    ...
    case eURIEL_COMBO_BOX:
      ...
      c_StringListValues =
          orc_Index.data(ms32_USER_ROLE_INTERACTION_COMBO_BOX_VALUES_LIST)
              .toStringList();
      c_StringListStrings =
          orc_Index.data(ms32_USER_ROLE_INTERACTION_COMBO_BOX_STRINGS_LIST)
              .toStringList();
      pc_ComboBox->InitFromStringList(c_StringListStrings, c_StringListValues);
      if (orc_Index.data(ms32_USER_ROLE_INTERACTION_COMBO_BOX_EDITABLE)
              .toBool() == true) {
        // make editable
        ...
        if (c_StringList.size() == 4) {
          ...
          Q_ASSERT(C_SdNdeDpContentUtil::h_SetAllContentFromString(
                       c_StringList.at(0), c_Min) == C_NO_ERR);
          Q_ASSERT(C_SdNdeDpContentUtil::h_SetAllContentFromString(
                       c_StringList.at(1), c_Max) == C_NO_ERR);
          f64_Factor = rc_Factor.toDouble(&q_Ok);
          Q_ASSERT(q_Ok == true);
          f64_Offset = rc_Offset.toDouble(&q_Ok);
          Q_ASSERT(q_Ok == true);
          pc_ComboBox->InitMinMaxAndScaling(c_Min, c_Max, f64_Factor,
                                            f64_Offset);
        }
      }
      // connect combo box with model (commit on index change; includes commit
      // on text change)
      connect(pc_ComboBox,
    ...
    case eURIEL_MULTI_SELECT_COMBO_BOX:
      ...
      connect(pc_MultiSelectComboBox, &C_OgeCbxMultiSelect::SigValueChanged,
              this, &C_TblDelegate::m_CommitData);
    ...
    case eURIEL_GENERIC_SPIN_BOX:
      c_StringList =
          orc_Index
              .data(ms32_USER_ROLE_INTERACTION_GENERIC_SPIN_BOX_PARAMETERS_LIST)
              .toStringList();
      if (c_StringList.size() == 4) {
        C_OscNodeDataPoolContent c_Min;
        C_OscNodeDataPoolContent c_Max;
        ...
        f64_Offset = rc_Offset.toDouble(&q_Ok);
        Q_ASSERT(q_Ok == true);
        pc_Retval = m_CreateGenericEditor(opc_Parent, orc_Index, c_Min, c_Max,
                                          f64_Factor, f64_Offset, 0UL);
      }
      break;
    case eURIEL_NONE:
      pc_Retval =
          QStyledItemDelegate::createEditor(opc_Parent, orc_Option, orc_Index);
      break;
    default:
      Q_ASSERT(false);
      break;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblDelegate::setEditorData(QWidget *const opc_Editor,
                                  const QModelIndex &orc_Index) const {
  ...
  case eURIEL_LINE_EDIT:
    ...
    if (pc_LineEdit != NULL) {
      pc_LineEdit->SetFromVariant(
          orc_Index.data(static_cast<int32_t>(Qt::DisplayRole)),
          orc_Index.data(static_cast<int32_t>(Qt::EditRole)));
      if (orc_Index.data(ms32_USER_ROLE_INTERACTION_USE_MIN_VALUE).toBool()) {
        pc_LineEdit->SetMinFromVariant(
            orc_Index.data(ms32_USER_ROLE_INTERACTION_MINIMUM_VALUE));
      }
      if (orc_Index.data(ms32_USER_ROLE_INTERACTION_USE_MAX_VALUE).toBool()) {
        pc_LineEdit->SetMaxFromVariant(
            orc_Index.data(ms32_USER_ROLE_INTERACTION_MAXIMUM_VALUE));
      }
      if (orc_Index.data(ms32_USER_ROLE_INTERACTION_USE_STR_LENGTH).toBool()) {
        pc_LineEdit->setMaxLength(
            orc_Index.data(ms32_USER_ROLE_INTERACTION_STR_LENGTH).toUInt());
      }
    }
    break;
  case eURIEL_COMBO_BOX:
    ...
    if (pc_ComboBox != NULL) {
      bool q_Ok;
      const int64_t s64_Val =
          orc_Index.data(static_cast<int32_t>(Qt::EditRole)).toLongLong(&q_Ok);
      if (q_Ok) {
  ...
  case eURIEL_MULTI_SELECT_COMBO_BOX:
    ...
    if (pc_MultiSelectComboBox != NULL) {
      const QBitArray c_BitArrayValues =
          orc_Index.data(static_cast<int32_t>(Qt::EditRole)).toBitArray();
      const QStringList c_StringListStrings =
          orc_Index
              .data(
                  ms32_USER_ROLE_INTERACTION_MULTI_SELECT_COMBO_BOX_STRINGS_LIST)
              .toStringList();
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblDelegate::setModelData(QWidget *const opc_Editor,
  ...
  case eURIEL_LINE_EDIT:
    ...
    if (pc_LineEdit != NULL) {
      ...
      QVariant c_Value;
      if (pc_LineEdit->GetValueAsVariant(c_Value, c_ErrorDescription) ==
          C_NO_ERR) {
        opc_Model->setData(orc_Index, c_Value,
                           static_cast<int32_t>(Qt::EditRole));
      } else {
        C_OgeWiCustomMessage c_Message(opc_Editor,
                                       C_OgeWiCustomMessage::eWARNING);
  ...
  case eURIEL_COMBO_BOX:
    ...
    if (pc_ComboBox != NULL) {
      ...
      if (pc_ComboBox->GetValue(s64_Value, c_ErrorDescription) == C_NO_ERR) {
        opc_Model->setData(orc_Index, static_cast<qlonglong>(s64_Value),
                           static_cast<int32_t>(Qt::EditRole));
      } else {
        C_OgeWiCustomMessage c_Message(opc_Editor,
                                       C_OgeWiCustomMessage::eWARNING);
  ...
  case eURIEL_MULTI_SELECT_COMBO_BOX:
    ...
    if (pc_MultiSelectComboBox != NULL) {
      const QBitArray c_BitArrayValues =
          pc_MultiSelectComboBox->GetValuesAsBitArray();
      opc_Model->setData(orc_Index, c_BitArrayValues,
                         static_cast<int32_t>(Qt::EditRole));
    }
    break;
  case eURIEL_GENERIC_SPIN_BOX:
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblDelegate::paint(QPainter *const opc_Painter,
                          const QStyleOptionViewItem &orc_Option,
                          const QModelIndex &orc_Index) const {
  const QStringList c_IconPaths =
      orc_Index.data(ms32_USER_ROLE_ICON).toStringList();

  QStyledItemDelegate::paint(opc_Painter, orc_Option, orc_Index);
  if (c_IconPaths.count() == 2) {
    const QSize c_ICON_SIZE(16, 16);
    const QRect c_Rect(
        orc_Option.rect.topLeft() +
            QPoint((orc_Option.rect.width() - c_ICON_SIZE.width()) / 2,
...
*/
//----------------------------------------------------------------------------------------------------------------------
E_UserRoleInteractionElementValue
C_TblDelegate::mh_GetInteractionElementValue(const QModelIndex &orc_Index) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QWidget *C_TblDelegate::m_CreateGenericEditor(
    QWidget *const opc_Parent, const QModelIndex &orc_Index,
  ...
  if (((orc_Index.isValid() == true) &&
       (orc_Min.GetType() == orc_Max.GetType())) &&
      (orc_Min.GetArray() == orc_Max.GetArray())) {
    // Create
    C_OgeWiSpinBoxGroup *const pc_SpinBoxGroup = m_CreateSpinBox(opc_Parent);
    // Init type
    pc_SpinBoxGroup->Init(orc_Min, orc_Max, of64_Factor, of64_Offset,
                          oru32_ArrayIndex);
    pc_Retval = pc_SpinBoxGroup;
    // connect spin box with model (commit on value change)
    connect(pc_SpinBoxGroup, &C_OgeWiSpinBoxGroup::SigValueChanged, this,
            &C_TblDelegate::m_CommitData);
  }
  return pc_Retval;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblDelegate::m_SetGenericEditorDataVariable(
    QWidget *const opc_Editor, const QModelIndex &orc_Index) const {
  C_OgeWiSpinBoxGroup *const pc_SpinBox =
      dynamic_cast<C_OgeWiSpinBoxGroup *const>(opc_Editor);
...
/*! \brief  Slot to emit commitData signal from sender widget.
   ...
   The signal commitData must be emitted when the editor widget has completed
   editing the data, and wants to write it back into the model. Adopted from
   Star Delegate Example of Qt Documentation.
*/
//----------------------------------------------------------------------------------------------------------------------
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_TblDelegate::mh_SetModelGenericDataVariable(
    QWidget *const opc_Editor, QAbstractItemModel *const opc_Model,
