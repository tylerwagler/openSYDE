...
/*!
   \file
   \brief       Utility class for sequences required for device configuration.
   (header)
...
public:
  ...
  C_SyvDcDeviceConfiguation &
  operator=(const C_SyvDcDeviceConfiguation &orc_Source) &;

  // Identifier of the device
  stw::opensyde_core::C_OscProtocolSerialNumber c_SerialNumber;
  uint8_t u8_SubNodeId;

  // Communication configuration of node which was detected by getting serial
  // number
...
class C_SyvDcSequences : public QObject,
...
public:
  ...
  // Helper function
  int32_t FillDeviceConfig(
      C_SyvDcDeviceConfiguation &orc_Config,
      stw::opensyde_core::C_OscNodeProperties::E_FlashLoaderProtocol
          &ore_Flashloader) const;

  int32_t ScanCanEnterFlashloader(const uint32_t ou32_UsedBitrate);
  int32_t ScanCanSendFlashloaderRequest(const uint32_t ou32_ScanTime,
                                        const bool oq_ScanEndless);
...
protected:
  ...
  void m_RunConfCanStwFlashloaderDevicesState(
      const uint32_t ou32_Step, const int32_t os32_Result,
      const stw::opensyde_core::C_OscProtocolDriverOsyNode &orc_Server) const;
  void m_RunConfOpenSydeDevicesState(
      const uint32_t ou32_Step, const int32_t os32_Result,
...
private:
  ...
  int32_t
  m_RunScanCanGetInfoFromStwFlashloaderDevice(const uint8_t ou8_LocalId);
  ...
  int32_t m_SetEthOpenSydeIpAddress(
      const stw::opensyde_core::C_OscProtocolDriverOsyNode &orc_ServerId,
      const C_SyvDcDeviceConfiguation &orc_DeviceConfig) const;
  int32_t m_SetOpenSydeNodeIds(
      const stw::opensyde_core::C_OscProtocolDriverOsyNode &orc_ServerId,
      const C_SyvDcDeviceConfiguation &orc_DeviceConfig) const;
