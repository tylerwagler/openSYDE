 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_SyvDcConnectedNodeList::mhc_MIME_DATA =
    "stw_opensyde_connected_node";
const QString C_SyvDcConnectedNodeList::mhc_MIME_DATA_EXT_FORMAT =
    "stw_opensyde_connected_node_ext_format";
const QString C_SyvDcConnectedNodeList::mhc_MIME_DATA_MANUFACTURER_FORMAT =
    "stw_opensyde_connected_node_manu_format";
const QString C_SyvDcConnectedNodeList::mhc_MIME_DATA_DEVICE =
    "stw_opensyde_connected_node_device";
const QString C_SyvDcConnectedNodeList::mhc_MIME_DATA_DEVICE_VALID =
    "stw_opensyde_connected_node_device_valid";
const QString
    C_SyvDcConnectedNodeList::mhc_MIME_DATA_SUB_NODE_IDS_TO_OLD_NODE_IDS =
...
C_SyvDcConnectedNodeList::C_SyvDcConnectedNodeList(QWidget *const opc_Parent)
    : QListWidget(opc_Parent), mq_GridSizeSet(false) {
  // UI Settings
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvDcConnectedNodeList::EnableSerialNumber(
  ...
  for (int32_t s32_It = 0; s32_It < this->count(); ++s32_It) {
    C_SyvDcConnectedNodeWidget *const pc_Widget =
        dynamic_cast<C_SyvDcConnectedNodeWidget *const>(
            this->itemWidget(this->item(s32_It)));
    if ((pc_Widget != NULL) &&
...
/*! \brief  Sets width of the list item
   ...
   Note: This function should be called after "SetData" because in "SetData" an
   initial width is set to the list item. (SetData -> m_Init -> AppendNode ->
   C_SyvDcConnectedNodeWidget::m_Init)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvDcConnectedNodeList::SetListItemWidth(
    const int32_t os32_WidgetWidth) {
  for (int32_t s32_It = 0; s32_It < this->count(); ++s32_It) {
    C_SyvDcConnectedNodeWidget *const pc_Widget =
        dynamic_cast<C_SyvDcConnectedNodeWidget *const>(
            this->itemWidget(this->item(s32_It)));
...
void C_SyvDcConnectedNodeList::SelectRow(const int32_t os32_RowIndex) {
  if ((os32_RowIndex >= 0) && (this->count() > 0) &&
      (os32_RowIndex < this->count())) {
...
void C_SyvDcConnectedNodeList::startDrag(const Qt::DropActions oc_Actions) {
  ...
  if ((pc_Widget != NULL) && (pc_Widget->isEnabled() == true)) {
    ...
    Q_EMIT(this->SigStartDrag(pc_Widget->GetDeviceName(),
    ...
    // pc_Drag->exec will start its own event loop. It will return when the
    // mouse button is released. No Qt event or signal will detect this on drop
    // exit on one common position That is the only solution to handle a drag
    // exit always.
    Q_EMIT(this->SigStopDrag());
  } // lint !e429  //no memory leak because of the parent of pc_Drag and the Qt
    // memory management
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
QMimeData *C_SyvDcConnectedNodeList::mimeData(
    const QList<QListWidgetItem *> &orc_Items) const {
    ...
    if (orc_Items.size() > 0) {
      ...
      if (pc_Widget != NULL) {
        ...
        pc_Retval->setData(
            ...
            pc_Widget->GetPlainSerialNumberString().toUtf8());
        pc_Retval->setData(
        ...
        pc_Retval->setData(C_SyvDcConnectedNodeList::mhc_MIME_DATA_DEVICE,
                           pc_Widget->GetDeviceName().toUtf8());
        ...
        for (c_ItIds = c_SubNodeIdsToOldNodeIds.begin();
             c_ItIds != c_SubNodeIdsToOldNodeIds.end(); ++c_ItIds) {
        ...
        pc_Retval->setData(
            ...
            c_StringSubNodeIdsToOldNodeIds.toUtf8());
      }
    }
  }
  return pc_Retval;
...
void C_SyvDcConnectedNodeList::m_Init(void) {
  ...
  for (u32_ItData = 0U; u32_ItData < this->mc_Data.size(); ++u32_ItData) {
    ...
    for (u32_ItDataUnique = 0U; u32_ItDataUnique < c_DataUnique.size();
         ++u32_ItDataUnique) {
      const C_OscDcDeviceInformation &rc_DataUnique =
          c_DataUnique[u32_ItDataUnique];
      ...
      if (rc_DataUnique.IsSerialNumberIdentical(rc_Data) == true) {
        ...
        c_DataUniqueSubNodeIdsToOldNodeIds[u32_ItDataUnique]
                                          [rc_Data.u8_SubNodeId] =
                                              c_OldComConfig;
  ...
  for (u32_ItData = 0U; u32_ItData < c_DataUnique.size(); ++u32_ItData) {
    m_AppendNode(c_DataUnique[u32_ItData],
                 c_DataUniqueSubNodeIdsToOldNodeIds[u32_ItData]);
  }
}
...
/*! \brief   Append node to list
   ...
   \param[in] orc_SubNodeIdsToOldNodeIds  Detected sub node ids and the
   associated used node ids with same serial number
                                          - In case of a normal node, exact one
   sub node id which should be 0
                                          - In case of a multiple CPU, at least
   two sub node ids
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvDcConnectedNodeList::m_AppendNode(
  ...
  pc_Widget =
      new C_SyvDcConnectedNodeWidget(this->item(this->count() - 1), orc_Info,
                                     orc_SubNodeIdsToOldNodeIds, this);

  this->setItemWidget(this->item(this->count() - 1), pc_Widget);

  // Style first
  C_OgeWiUtil::h_ApplyStylesheetProperty(pc_Widget, "First",
                                         this->count() == 1);

  if (this->mq_GridSizeSet == false) {
    // Set the grid size for one time. The size hint seems not to be enough
    // since Qt 5.15.2
    this->mq_GridSizeSet = true;
    this->setGridSize(QSize(pc_Widget->width(), pc_Widget->height()));
  }
} // lint !e429  //no memory leak because of the parent of pc_Widget and the Qt
  // memory management

//----------------------------------------------------------------------------------------------------------------------
void C_SyvDcConnectedNodeList::m_ScrollBarRangeChangedVer(
