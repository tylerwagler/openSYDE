...
/*!
   ...
   The function call just starts the process and returns. The caller is
   responsible to keep the specified "C_SyvComDriverFlash" stable till the
   thread was finished.

   Intermediate results are signaled to the GUI via Qt signals. A signal is also
   triggered when the functions finish. The results are passed along with the
   signals.
...
C_SyvUpSequences::C_SyvUpSequences(void)
    : QObject(), C_OscSuSequences(), mpc_CanDllDispatcher(NULL),
      mpc_EthernetDispatcher(NULL), mq_AbortFlag(false), mu32_ViewIndex(0U),
      me_Sequence(eNOT_ACTIVE), ms32_Result(C_NOACT) {
...
/*! \brief   Initialize all members
   ...
   least one node C_BUSY        System view error detected C_COM         CAN
   initialization failed or no route found for at least one node C_CHECKSUM
   Internal buffer overflow detected C_RANGE       Routing configuration failed
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpSequences::InitUpSequences(const uint32_t ou32_ViewIndex) {
  ...
  s32_Return = C_SyvComDriverUtil::h_GetOscComDriverParamFromView(
      ou32_ViewIndex, u32_ActiveBusIndex, c_ActiveNodes,
      &this->mpc_CanDllDispatcher, &this->mpc_EthernetDispatcher, true, false,
      NULL);
...
/*! \brief   Reinitialize the CAN dispatcher
   ...
   The function can initialize the dispatcher again after calling
   CloseDispatcher.
...
void C_SyvUpSequences::CloseDispatcher(void) {
  if ((this->IsInitialized() == true) && (this->mpc_CanDllDispatcher != NULL)) {
    this->mpc_CanDllDispatcher->CAN_Exit();
  }
}
...
/*! \brief   Creating temporary folder
   ...
   \param[in,out] orc_ApplicationsToWrite Vector with all file paths which will
   be adapted as output
   \param[out]    orc_ErrorPath           Optional pointer to store path error
   ...
   orc_ApplicationsToWrite[node] is higher than the number of applications of
   the node in orc_Nodes (for an active and address based node) C_RANGE     file
   referenced by orc_ApplicationsToWrite does not exist orc_TargetPath does not
   end in "\" or "/" C_BUSY      could not erase pre-existing target path (note:
   can result in partially erased target path) C_RD_WR     could not copy file
...
int32_t C_SyvUpSequences::SyvUpCreateTemporaryFolder(
  ...
  if ((this->IsInitialized() == true) && (pc_View != NULL)) {
    QString c_ErrorPath;
...
/*! \brief   Get result of previously started service execution
   ...
   \param[out]  ors32_Result       result code of executed service function
                                   for possible values see the DataDealer's
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_SyvUpSequences::GetLastUpdatePosition(uint32_t &oru32_NodeIndex,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns the reported results of StartReadDeviceInformation for
   openSYDE nodes
   ...
   \param[out]    orc_OsyDeviceInformation   All device information associated
   to the node index in the same order
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpSequences::GetOsyDeviceInformation(
  ...
  Q_ASSERT(this->mc_ReportOsyDeviceInformationNodeIndex.size() ==
           this->mc_ReportOsyDeviceInformation.size());
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns the reported results of StartReadDeviceInformation for STW
   Flashloader nodes
   ...
   \param[out]    orc_XflDeviceInformation   All device information associated
   to the node index in the same order
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpSequences::GetXflDeviceInformation(
  ...
  Q_ASSERT(this->mc_ReportXflDeviceInformationNodeIndex.size() ==
           this->mc_ReportXflDeviceInformation.size());
...
QString C_SyvUpSequences::GetStepName(const E_ProgressStep oe_Step) const {
  ...
  case eACTIVATE_FLASHLOADER_ROUTING_AVAILABLE_FEATURE_ERROR:
    c_Text = "Activate Flashloader: Error on start routing due to available "
             "features";
    break;
  ...
  case eREAD_DEVICE_INFO_OSY_FLASHLOADER_CHECK_SECURITY_ACTIVATION_START:
    c_Text =
        "Read Device Information: Flashloader security activation state start";
    break;
  case eREAD_DEVICE_INFO_OSY_FLASHLOADER_CHECK_SECURITY_ACTIVATION_ERROR:
    c_Text =
        "Read Device Information: Flashloader security activation state error";
    break;
  ...
  case eREAD_DEVICE_INFO_XFL_WAKEUP_ERROR:
    c_Text = "Read Device Information: Connection with STW Flashloader device "
             "failed";
    break;
  ...
  case eREAD_DEVICE_INFO_XFL_READING_INFORMATION_ERROR:
    c_Text = "Read Device Information: Could not read information from STW "
             "Flashloader device";
    break;
  ...
  case eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_PREPARE_START:
    c_Text = "Update System: Preparation of Node flash area of file system "
             "file start";
    break;
  ...
  case eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_TRANSFER_START:
    c_Text =
        "Update System: Node flash area of file system file transfer start";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_TRANSFER_ERROR:
    c_Text =
        "Update System: Node flash area of file system file transfer error";
    break;
  ...
  case eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_MAX_SIZE_ERROR:
    c_Text =
        "Update System: Node NVM write read back of maximum block length error";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_OPEN_FILE_START:
    c_Text = "Update System: Node NVM write read back of parameter set image "
             "file start";
    break;
  ...
  case eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_WRITE_FILE_START:
    c_Text =
        "Update System: Node NVM write write of parameter set image file start";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_WRITE_FILE_ERROR:
    c_Text =
        "Update System: Node NVM write write of parameter set image file error";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_FILE_FINISHED:
    c_Text =
        "Update System: Node NVM write of parameter set image file finished";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_FINISHED:
    c_Text =
        "Update System: Node NVM write of parameter set image files finished";
    break;
  ...
  case eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_AVAILABLE_FEATURE_ERROR:
    c_Text = "Update System: Node write of security activation state available "
             "feature error";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_SESSION_ERROR:
    c_Text =
        "Update System: Node write of security activation state session error";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_SEND_ERROR:
    c_Text =
        "Update System: Node write of security activation state send error";
    break;
  ...
  case eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_AVAILABLE_FEATURE_ERROR:
    c_Text = "Update System: Node write of debugger activation state available "
             "feature error";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_SESSION_ERROR:
    c_Text =
        "Update System: Node write of debugger activation state session error";
    break;
  case eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_SEND_ERROR:
    c_Text =
        "Update System: Node write of debugger activation state send error";
    break;
  ...
  case eUPDATE_SYSTEM_XFL_NODE_FLASH_HEX_ERROR:
    ...
    break;
  case eXFL_PROGRESS: // wrapped progress information from STW flashloader
                      // driver
...
/*! \brief   Starts thread to update system
   ...
   \param[in]  orc_NodesOrder     Vector with node update order (index is update
   position, value is node index)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpSequences::AbortCurrentProgress(void) { this->mq_AbortFlag = true; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpSequences::m_ReportProgress(const E_ProgressStep oe_Step,
  ...
  if ((oe_Step != C_SyvUpSequences::eXFL_PROGRESS) ||
      (!orc_Information.contains(stw::opensyde_core::C_OscUtils::h_LoadString(
          STR_FDL_TXT_WR_FLASH_RQ)))) {
    c_Text = "Step: " + this->GetStepName(oe_Step);
    c_Text += " Progress: " + QString::number(ou8_Progress);
    c_Text += " Result: " + QString::number(os32_Result);
  ...
  Q_EMIT this->SigReportProgress(static_cast<uint32_t>(oe_Step), os32_Result,
                                 ou8_Progress);
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpSequences::m_ReportProgress(
  ...
  if ((oe_Step != C_SyvUpSequences::eXFL_PROGRESS) ||
    ...
    c_Text += " Progress: " + QString::number(ou8_Progress);
    c_Text += " Result: " + QString::number(os32_Result);
    c_Text += " Bus ID: " + QString::number(orc_Server.u8_BusIdentifier);
    c_Text += " Node ID: " + QString::number(orc_Server.u8_NodeIdentifier);
    c_Text += " Info: " + orc_Information;
  ...
  // Enum make problems with signal slot mechanism
  Q_EMIT this->SigReportProgressForServer(
      static_cast<uint32_t>(oe_Step), os32_Result, ou8_Progress,
      orc_Server.u8_BusIdentifier, orc_Server.u8_NodeIdentifier);
...
/*! \brief   Reports information read from STW Flashloader server node
   ...
   Flashloader node. Here: save device information

   \param[in]     orc_Info          Information read from node
   \param[in]     ou32_NodeIndex   Index of node within mpc_SystemDefinition
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpSequences::m_ReportStwFlashloaderInformationRead(
...
void C_SyvUpSequences::mh_ThreadFunc(void *const opv_Instance) {
  ...
  C_SyvUpSequences *const pc_Sequences =
      reinterpret_cast<C_SyvUpSequences *>(opv_Instance);
...
void C_SyvUpSequences::m_ThreadFunc(void) {
  if (this->mpc_ComDriver != NULL) {
    ...
    case eACTIVATE_FLASHLOADER:
      ...
      // Ignore error (Same error should be reported with read device
      // information)
    ...
    case eUPDATE_SYSTEM:
      this->ms32_Result =
          this->UpdateSystem(this->mc_NodesToFlash, this->mc_NodesOrder);
      break;
    case eRESET_SYSTEM:
...
void C_SyvUpSequences::mh_WriteLog(
    const C_OscSuSequences::E_ProgressStep oe_Step, const QString &orc_Text) {
