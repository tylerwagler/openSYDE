...
C_SyvUpPieChart::C_SyvUpPieChart(QWidget *const opc_Parent)
    : QWidget(opc_Parent), mu16_ValueInPercent(0),
      mc_ActiveColor(mc_STYLE_GUIDE_COLOR_13), mq_FinishedAnimation(false) {
  ...
  if (c_FileInfo.exists() == true) {
    ...
    this->mpc_Movie =
        new QMovie("://images/system_views/UpdateFinishedAnimation.gif");
  }
  connect(this->mpc_Movie, &QMovie::frameChanged, this,
          &C_SyvUpPieChart::m_OnFrameChanged);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPieChart::SetProgress100(const uint16_t ou16_Progress100,
...
void C_SyvUpPieChart::paintEvent(QPaintEvent *const opc_Event) {
  ...
  const int32_t s32_Height =
      std::min(c_CompleteRect.width(), c_CompleteRect.height());
  const int32_t s32_X =
      static_cast<int32_t>(c_CompleteRect.x()) +
      ((static_cast<int32_t>(c_CompleteRect.width()) - s32_Height) / 2);
  const int32_t s32_Y =
      static_cast<int32_t>(c_CompleteRect.y()) +
      ((static_cast<int32_t>(c_CompleteRect.height()) - s32_Height) / 2);
  QPainter c_Painter(this);

  if ((this->mq_FinishedAnimation == true) && (this->mpc_Movie != NULL)) {
    c_Painter.drawPixmap(QRect(s32_X, s32_Y, s32_Height, s32_Height),
                         this->mpc_Movie->currentPixmap().scaled(
                             s32_Height, s32_Height, Qt::KeepAspectRatio,
    ...
    const float32_t f32_InnerCircleOffset =
        ((f32_Height * 12.0F) / 180.0F) / 2.0F;
    const int32_t s32_InnerCircleOffset =
        static_cast<int32_t>(f32_InnerCircleOffset);
    QString c_String;
    ...
    c_Font.setPointSize(
        static_cast<int32_t>((s32_Height - (6 * s32_InnerCircleOffset)) / 3));
    ...
    f32_SpanAngle = (-1.0F * (360.0F / 100.0F) *
                     static_cast<float32_t>(mu16_ValueInPercent) * 16.0F);

    c_Painter.setBrush(c_PieBrush);
    c_Painter.drawPie(c_Rect, s16_StartAngle,
                      static_cast<int32_t>(f32_SpanAngle));

    // foreground cycle
    c_Painter.setBrush(c_ForegroundBrush);
    c_Painter.drawEllipse(
        static_cast<int32_t>(s32_X + s32_InnerCircleOffset),
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPieChart::m_OnFrameChanged(void) { this->update(); }
