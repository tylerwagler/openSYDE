...
#include "widget/C_OgeWiCustomMessage.hpp"
#include <QString>
#include <QStringList>
 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_SyvUpNodePropertiesDialog::mhc_HTML_CELL_TAG_START_MAIN =
    "<td style=\"padding: 0px 40px 10px 0px;\">";
const QString C_SyvUpNodePropertiesDialog::mhc_HTML_CELL_TAG_START_CONTENT =
    "<td valign=\"middle\" style=\"padding: 0px 30px 0px 0px;\">";
const QString C_SyvUpNodePropertiesDialog::mhc_HTML_CELL_TAG_START_STATUS =
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpNodePropertiesDialog::C_SyvUpNodePropertiesDialog(
    stw::opensyde_gui_elements::C_OgePopUpDialog &orc_Parent,
    const uint32_t ou32_NodeIndex, const C_GiSvNodeData &orc_NodeData)
    : QWidget(&orc_Parent), mpc_Ui(new Ui::C_SyvUpNodePropertiesDialog),
      mrc_ParentDialog(orc_Parent), mu32_NodeIndex(ou32_NodeIndex),
  ...
  connect(this->mpc_Ui->pc_PushButtonOk, &QPushButton::clicked, this,
          &C_SyvUpNodePropertiesDialog::m_OkClicked);
  connect(this->mpc_Ui->pc_PushButtonDiscard, &QPushButton::clicked, this,
          &C_SyvUpNodePropertiesDialog::m_OnDiscard);
}
...
void C_SyvUpNodePropertiesDialog::InitStaticNames(void) const {
  ...
  this->mpc_Ui->pc_PushButtonDiscard->SetToolTipInformation(
      "Force Update", "Option to force flashing of already up to date nodes.",
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::SetStatus(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::CopyInitialStatus(
    C_GiSvNodeData &orc_NodeData) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::CopyUpdateStatus(
    C_GiSvNodeData &orc_NodeData) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::CopyStwDeviceInfo(
    C_GiSvNodeData &orc_NodeData) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::CopyOpenSydeDeviceInfo(
    C_GiSvNodeData &orc_NodeData) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::CopyDiscardedStatus(
    C_GiSvNodeData &orc_NodeData) const {
...
void C_SyvUpNodePropertiesDialog::m_InitStatus(void) const {
  ...
  if (this->mc_NodeData.AreAllNodesValid()) {
    ...
    if (this->mc_NodeData.GetConnectInProgressStatus() == true) {
      ...
      this->mpc_Ui->pc_LabelDescription->setText(
          "Enter update mode to get the node status");
    }
  }
...
void C_SyvUpNodePropertiesDialog::m_InitDataBlockTable(void) const {
  ...
  if (c_NodeIndices.size() == this->mc_NodeData.GetSubNodeCount()) {
    for (uint32_t u32_ItDevice = 0UL; u32_ItDevice < c_NodeIndices.size();
         ++u32_ItDevice) {
      const C_OscNode *const pc_Node =
          C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(
              c_NodeIndices[u32_ItDevice]);
      ...
      if ((pc_Node != NULL) && (pc_SubDevice != NULL)) {
        C_SyvUpNodePropertiesDialog::mh_InitDataBlockTableForNode(
            *pc_SubDevice, c_Content, *pc_Node, c_NodeIndices.size() > 1UL);
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_InitDataBlockTableForNode(
    const C_GiSvSubNodeData &orc_NodeInfo, QString &orc_Content,
  ...
  if (orc_NodeInfo.GetValidStatus()) {
    ...
    if (orc_NodeInfo.GetDataBlockFoundStatus()) {
      Q_ASSERT(orc_Node.u32_SubDeviceIndex <
               orc_Node.pc_DeviceDefinition->c_SubDevices.size());
      if (orc_Node.pc_DeviceDefinition
              ->c_SubDevices[orc_Node.u32_SubDeviceIndex]
        ...
        if (c_HexApps.size() > 0) {
          if (c_HexApps.size() == orc_NodeInfo.GetHexFileInfosCount()) {
            for (uint32_t u32_ItFile = 0;
                 u32_ItFile < orc_NodeInfo.GetHexFileInfosCount();
                 ++u32_ItFile) {
              QString c_FileProjectName;
              QString c_FileVersion;
              QString c_FileBuild;
              const QString c_ApplicationName = c_HexApps[u32_ItFile].c_Name;
              QString c_Icon;
              QString c_State;
              ...
              C_SyvUpNodePropertiesDialog::mh_GetApplicationDataForNode(
                  orc_NodeInfo, u32_ItFile, c_Icon, c_State, c_DeviceValid,
                  c_FileProjectName, c_DeviceProjectName, c_FileVersion,
                  c_DeviceVersion, c_FileBuild, c_DeviceBuild);
              C_SyvUpNodePropertiesDialog::mh_ExtractDetailsPartFromDataForNode(
                  orc_NodeInfo.GetHexAppInfoAmbiguous(u32_ItFile),
                  c_FileProjectName, c_DeviceProjectName, c_FileVersion,
                  c_DeviceVersion, c_FileBuild, c_DeviceBuild, c_DetailsPart);
  ...
  C_SyvUpNodePropertiesDialog::mh_InitDataBlockTableOtherSectionForNode(
      orc_NodeInfo, c_NewContent);
  C_SyvUpNodePropertiesDialog::mh_HandleSectionAppend(
      c_NewContent, orc_Content, orc_Node, oq_IsMultiDevice);
}
...
/*! \brief  Get application data for node
   ...
   \param[in]      orc_NodeInfo              Node info
   \param[in]      ou32_ApplicationIndex     Application index in list of all
   apps that are not HALC NVM Data Blocks
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_GetApplicationDataForNode(
    const C_GiSvSubNodeData &orc_NodeInfo, const uint32_t ou32_ApplicationIndex,
  ...
  const C_XFLECUInformation *const pc_FileInfo =
      orc_NodeInfo.GetHexFileInfo(ou32_ApplicationIndex);
  bool q_Missing = true;
  bool q_Match = true;
  ...
  if (pc_FileInfo != NULL) {
    orc_FileProjectName = pc_FileInfo->GetProjectName();
    orc_FileVersion = pc_FileInfo->GetProjectVersion();
    orc_FileBuildDate = (pc_FileInfo->GetDate() + " " + pc_FileInfo->GetTime());
  }
  ...
  // If no node information is available skip this as this probably means:
  // discarded information If error happened also skip this
  if (((((orc_NodeInfo.GetInitialStatus() == C_SyvUtil::eI_APPLICATION_MATCH) ||
    ...
    q_Missing = false;
              ...
              (ou32_ApplicationIndex <
               orc_NodeInfo.GetFailedApplicationIndex())) &&
             (orc_NodeInfo.IsDeviceInfoDiscarded() == false)) {
    // Assuming known application status
    orc_DeviceProjectName = orc_FileProjectName;
    orc_DeviceFileVersion = orc_FileVersion;
  ...
  // Another exception: HEX files with multiple application blocks need to be
  // flashed anyway
    ...
    if (c_DeviceInfo.pc_StwDevice != NULL) {
      C_SyvUpNodePropertiesDialog::mh_ExtractStwDeviceInformation(
          orc_NodeInfo, orc_FileProjectName, orc_DeviceProjectName,
          orc_FileVersion, orc_DeviceFileVersion, orc_FileBuildDate,
          orc_DeviceBuildDate, orc_DeviceValidStatus, q_Missing, q_Match);
    } else if (c_DeviceInfo.pc_OpenSydeDevice != NULL) {
      C_SyvUpNodePropertiesDialog::mh_ExtractOpenSydeDeviceInformation(
          orc_NodeInfo, orc_FileProjectName, orc_DeviceProjectName,
          orc_FileVersion, orc_DeviceFileVersion, orc_FileBuildDate,
          orc_DeviceBuildDate, orc_DeviceValidStatus, q_Missing, q_Match);
    } else {
  ...
  if (q_Missing == true) {
    orc_ApplicationStateIcon +=
        "<img src=\"://images/system_views/IconUpdateWaiting.svg\"";
    orc_ApplicationState = "Update required";
  } else {
    if (q_Match == true) {
      orc_ApplicationStateIcon +=
          "<img src=\"://images/system_views/IconUpdateSuccess.svg\"";
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_ExtractStwDeviceInformation(
    const C_GiSvSubNodeData &orc_NodeInfo, const QString &orc_FileProjectName,
    QString &orc_DeviceProjectName, const QString &orc_FileVersion,
    QString &orc_DeviceFileVersion, const QString &orc_FileBuildDate,
    QString &orc_DeviceBuildDate, QString &orc_DeviceValidStatus,
    bool &orq_MissingStatus, bool &orq_MatchStatus) {
  const C_SyvUpDeviceInfo c_DeviceInfo = orc_NodeInfo.GetDeviceInfo();

  for (int32_t s32_ItDeviceInfoBlock = 0;
       s32_ItDeviceInfoBlock <
       c_DeviceInfo.pc_StwDevice->c_BasicInformation.c_DeviceInfoBlocks.size();
       ++s32_ItDeviceInfoBlock) {
    const C_XFLECUInformation &rc_StwDeviceInfo =
        c_DeviceInfo.pc_StwDevice->c_BasicInformation
            .c_DeviceInfoBlocks[s32_ItDeviceInfoBlock];
    // Search for name match
    if (orc_FileProjectName.compare(
            rc_StwDeviceInfo.GetProjectName().trimmed()) == 0) {
      // Signal found
      orq_MissingStatus = false;
      // Apply
      orc_DeviceProjectName = rc_StwDeviceInfo.GetProjectName();
      orc_DeviceFileVersion = rc_StwDeviceInfo.GetProjectVersion();
      orc_DeviceBuildDate =
          (rc_StwDeviceInfo.GetDate() + " " + rc_StwDeviceInfo.GetTime());
      // Highlighting
      orq_MatchStatus = C_SyvUpNodePropertiesDialog::mh_HandleHighlighting(
          orc_FileProjectName, orc_DeviceProjectName, orc_FileVersion,
          orc_DeviceFileVersion, orc_FileBuildDate, orc_DeviceBuildDate);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_ExtractOpenSydeDeviceInformation(
    const C_GiSvSubNodeData &orc_NodeInfo, const QString &orc_FileProjectName,
    QString &orc_DeviceProjectName, const QString &orc_FileVersion,
    QString &orc_DeviceFileVersion, const QString &orc_FileBuildDate,
    QString &orc_DeviceBuildDate, QString &orc_DeviceValidStatus,
  ...
  for (uint32_t u32_ItOsyApplication = 1;
    ...
    // Search for name match
    if (orc_FileProjectName.trimmed().compare(
            rc_OsyDeviceInfo.c_ApplicationName.trimmed()) == 0) {
      // Signal found
      orq_MissingStatus = false;
      // Apply
      orc_DeviceProjectName = rc_OsyDeviceInfo.c_ApplicationName;
      orc_DeviceFileVersion = rc_OsyDeviceInfo.c_ApplicationVersion;
      orc_DeviceBuildDate =
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpNodePropertiesDialog::mh_HandleHighlighting(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_ExtractDetailsPartFromDataForNode(
    const bool oq_HexAppInfoAmbiguous, const QString &orc_FileProjectName,
    const QString &orc_DeviceProjectName, const QString &orc_FileVersion,
    const QString &orc_DeviceFileVersion, const QString &orc_FileBuildDate,
  ...
  if (oq_HexAppInfoAmbiguous == true) {
    ...
    c_NewContent += "<tr>";
    c_NewContent +=
        C_SyvUpNodePropertiesDialog::mhc_HTML_CELL_TAG_START_CONTENT;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_AppendApplicationForNode(
    const uint32_t ou32_ApplicationIndex, const QString &orc_ApplicationName,
  ...
  c_NewContent += static_cast<QString>("#%1 - %2 ")
                      .arg(ou32_ApplicationIndex + 1)
                      .arg(orc_ApplicationName);
  c_NewContent += "</h4>";
  ...
  c_NewContent +=
      orc_ApplicationStateIcon + "</td><td>" + orc_ApplicationState + "</td>";
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_InitDataBlockTableOtherSectionForNode(
    const C_GiSvSubNodeData &orc_NodeInfo, QString &orc_Content) {
  QString c_NewContent;
  ...
  if ((orc_NodeInfo.GetParamFileInfosCount() > 0UL) ||
    ...
    c_NewContent +=
        "<img "
        "src=\"://images/system_views/"
        "IconUpdateWaiting.svg\"style=\"vertical-align: middle;\"/></td><td>";
    c_NewContent += "No status request supported. Update required.";
    c_NewContent += "</td>";
    c_NewContent += "</tr></table></td>";
    c_NewContent += "</tr>";
...
void C_SyvUpNodePropertiesDialog::m_InitFlashloaderTable(void) const {
  ...
  if (c_NodeIndices.size() == this->mc_NodeData.GetSubNodeCount()) {
    for (uint32_t u32_ItDevice = 0UL; u32_ItDevice < c_NodeIndices.size();
      ...
      Q_ASSERT((pc_Node != NULL) && (pc_SubDevice != NULL));
      if ((pc_Node != NULL) && (pc_SubDevice != NULL)) {
        C_SyvUpNodePropertiesDialog::mh_InitFlashloaderTableForNode(
            *pc_SubDevice, c_Content, *pc_Node, c_NodeIndices.size() > 1UL);
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_InitFlashloaderTableForNode(
    const C_GiSvSubNodeData &orc_NodeInfo, QString &orc_Content,
    const C_OscNode &orc_Node, const bool oq_IsMultiDevice) {
  QString c_NewContent = "";
  const C_SyvUpDeviceInfo c_DeviceInfo = orc_NodeInfo.GetDeviceInfo();
  ...
  if (c_DeviceInfo.pc_OpenSydeDevice != NULL) {
    C_OscSuSequences::h_OpenSydeFlashloaderInformationToText(
        *c_DeviceInfo.pc_OpenSydeDevice, c_List);
  } else if (c_DeviceInfo.pc_StwDevice != NULL) {
    C_OscSuSequences::h_StwFlashloaderInformationToText(
        *c_DeviceInfo.pc_StwDevice, c_List);
  } else {
    // Unexpected
  }
  if (c_List.size() > 1) {
    c_NewContent += static_cast<QString>(
        "Note: Flashloader data is read during \"Enter Update Mode\"");
    c_NewContent += "<p>";
    for (int32_t s32_ItString = 0; s32_ItString < c_List.size();
         ++s32_ItString) {
      c_NewContent += c_List.at(s32_ItString) + "<br/>";
    }
    c_NewContent += "</p>";
  }
  C_SyvUpNodePropertiesDialog::mh_HandleSectionAppend(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpNodePropertiesDialog::mh_HandleSectionAppend(
    const QString &orc_NewPart, QString &orc_CompleteSection,
    const C_OscNode &orc_Node, const bool oq_IsMultiDevice) {
  if ((!orc_NewPart.isEmpty()) && (oq_IsMultiDevice)) {
    orc_CompleteSection += "<h3>";
    orc_CompleteSection += orc_Node.c_Properties.c_Name;
    orc_CompleteSection += "</h3>";
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Updates the second tab with the progress log based on the connect
 * and update states
 */
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_SyvUpNodePropertiesDialog::m_GetProgressLogConnectStatesString(void) const {
  QString c_Text = "";
  ...
  if (this->mc_NodeData.IsNodeConnectStatesSet() == true) {
    ...
    for (u32_SubNodeCounter = 0U; u32_SubNodeCounter < u32_SubNodeCount;
         ++u32_SubNodeCounter) {
      ...
      if (this->mc_NodeData.IsSubNodeActive(u32_SubNodeCounter) == true) {
        ...
        if (pc_SubNodeData != NULL) {
          if (u32_SubNodeCount > 1) {
            ...
            const C_OscNode *const opc_OscNode =
                C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(u32_NodeIndex);
            if (opc_OscNode != NULL) {
              c_Text += static_cast<QString>("Node: ") +
                        static_cast<QString>(
                            opc_OscNode->c_Properties.c_Name);
            }
          }

          // Printing the states
          c_Text +=
              mh_GetProgressLogConnectStatesStringForSubNode(pc_SubNodeData) +
              "<br>";
          if ((u32_SubNodeCounter + 1U) < u32_SubNodeCount) {
            // Not the last sub node, add an additional space between sub nodes
    ...
    c_Text += static_cast<QString>(
                  "Enter update mode to get the progress information.") +
              "<br>";
  }
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_SyvUpNodePropertiesDialog::mh_GetProgressLogConnectStatesStringForSubNode(
  ...
  if (opc_SubNodeData != NULL) {
    ...
    const C_OscSuSequencesNodeConnectStates &rc_States =
        opc_SubNodeData->GetNodeConnectStates();
    const C_GiSvSubNodeData::C_GiSvSubNodeDataPreconditionErrors
        &rc_PreconditionErrors =
            opc_SubNodeData->GetNodeConnectPreconditionErrors();

    c_Text += mh_GetTableLineForBrowser(
        "Request connection",
        ((rc_States.q_Timeout == false) ? c_Ok : c_Failed), 1);

    c_Text += mh_GetTableLineForBrowser(
        "Node information read",
        mh_GetSuSequenceNodeStateString(rc_States.e_InformationRead), 1);

    c_Text += mh_GetTableLineForBrowser(
        "Client authentication required?",
        ((rc_States.q_AuthenticationNecessary == true) ? c_Yes : c_No), 1);

    if (rc_States.q_AuthenticationNecessary == true) {
      c_Text += mh_GetTableLineForBrowser(
          "Client authentication",
          ((rc_States.q_AuthenticationError == false) ? c_Ok : c_Failed), 2);
    } else {
      // No security active
      c_Text +=
          mh_GetTableLineForBrowser("Running authentication", c_Skipped, 2);
    }

    c_Text += mh_GetTableLineForBrowser(
        static_cast<QString>("Parameter Set Image file (syde_psi)"), "", 1);
    c_Text += mh_GetTableLineForBrowser(
        "Writing to NVM supported?",
    ...
    c_Text +=
        mh_GetTableLineForBrowser(static_cast<QString>("PEM file"), "", 1);
    c_Text += mh_GetTableLineForBrowser(
        "Security supported?",
        mh_GetSuSequenceNodePreconditionString(
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Updates the second tab with the progress log based on the update
 * states
 */
//----------------------------------------------------------------------------------------------------------------------
QString
C_SyvUpNodePropertiesDialog::m_GetProgressLogUpdateStatesString(void) const {
  QString c_Text = "";
      ...
      for (u32_SubNodeCounter = 0U; u32_SubNodeCounter < u32_SubNodeCount;
        ...
        if (this->mc_NodeData.IsSubNodeActive(u32_SubNodeCounter) == true) {
          ...
          if (pc_SubNodeData != NULL) {
            ...
            // Printing the states
            c_Text +=
                mh_GetProgressLogUpdateStatesStringForSubNode(pc_SubNodeData) +
                "<br>";
            if ((u32_SubNodeCounter + 1U) < u32_SubNodeCount) {
              // Not the last sub node, add an additional space between sub
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_SyvUpNodePropertiesDialog::mh_GetProgressLogUpdateStatesStringForSubNode(
    const C_GiSvSubNodeData *const opc_SubNodeData) {
  QString c_Text = "";

  if (opc_SubNodeData != NULL) {
    const C_OscSuSequencesNodeUpdateStates &rc_States =
        opc_SubNodeData->GetNodeUpdateStates();
    uint32_t u32_FileCounter;
    bool q_AtLeastOneFile = false;

    // Hex files
    for (u32_FileCounter = 0U;
         u32_FileCounter < rc_States.c_StateHexFiles.size();
         ++u32_FileCounter) {
      const C_OscSuSequencesNodeHexFileStates &rc_FileState =
          rc_States.c_StateHexFiles[u32_FileCounter];

      q_AtLeastOneFile = true;

      c_Text += mh_GetTableLineForBrowser(
          "File " + static_cast<QString>(rc_FileState.c_FileName) + ":",
          "", 1);

      c_Text += mh_GetTableLineForBrowser(
          "Loading file",
          mh_GetSuSequenceNodeFileLoadStateString(rc_FileState.e_FileExists,
                                                  rc_FileState.e_FileLoaded),
          2);

      // Checking preconditions
      c_Text += mh_GetTableLineForBrowser(
          "Reading file signature block address",
          mh_GetSuSequenceNodeStateString(
              rc_FileState.e_SignatureBlockAddressFromFileRead),
          2);

      c_Text += mh_GetTableLineForBrowser(
          "Comparing node name",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_NodeNameCompared), 2);
      ...
      c_Text += mh_GetTableLineForBrowser(
          ...
          mh_GetSuSequenceNodeStateString(
              rc_FileState.e_AvailableFlashMemoryChecked),
          2);
      ...
      c_Text += mh_GetTableLineForBrowser(
          ...
          mh_GetSuSequenceNodeStateString(
              rc_FileState.e_RequestTransferAddressExitSent),
          2);
    }

    // STW flashloader files
    for (u32_FileCounter = 0U;
         u32_FileCounter < rc_States.c_StateStwFlHexFiles.size();
         ++u32_FileCounter) {
      const C_OscSuSequencesNodeStwFlHexFileStates &rc_FileState =
          rc_States.c_StateStwFlHexFiles[u32_FileCounter];
      ...
      // STW flashloader file states have no information about the loading of
      // the file
      c_Text += mh_GetTableLineForBrowser(
          "Checking file",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_FileExists), 2);

      // Transfers
      c_Text += mh_GetTableLineForBrowser(
          "Sending file",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_FileSent), 2);
    }

    // Other files
    for (u32_FileCounter = 0U;
         u32_FileCounter < rc_States.c_StateOtherFiles.size();
         ++u32_FileCounter) {
      const C_OscSuSequencesNodeOtherFileStates &rc_FileState =
          rc_States.c_StateOtherFiles[u32_FileCounter];
      ...
      // Transfers
      c_Text +=
          mh_GetTableLineForBrowser("Sending transfer request",
                                    mh_GetSuSequenceNodeStateString(
                                        rc_FileState.e_RequestFileTransferSent),
                                    2);
      ...
      c_Text += mh_GetTableLineForBrowser(
          "Sending transfer exit request",
          mh_GetSuSequenceNodeStateString(
              rc_FileState.e_RequestTransferFileExitSent),
          2);

      c_Text += mh_GetTableLineForBrowser(
          "Sending transfer exit result request",
          mh_GetSuSequenceNodeStateString(
              rc_FileState.e_RequestTransferFileExitResultSent),
          2);
    }

    // PSI files
    for (u32_FileCounter = 0U;
         u32_FileCounter < rc_States.c_StatePsiFiles.size();
         ++u32_FileCounter) {
      const C_OscSuSequencesNodePsiFileStates &rc_FileState =
          rc_States.c_StatePsiFiles[u32_FileCounter];
      ...
      c_Text += mh_GetTableLineForBrowser(
          "Writing file",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_PsiFileWrote), 2);
    }

    // PEM file
    if (rc_States.c_StatePemFile.c_FileName != "") {
      const C_OscSuSequencesNodePemFileStates &rc_FileState =
          rc_States.c_StatePemFile;
      ...
      c_Text += mh_GetTableLineForBrowser(
          "Extracting file",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_PemFileExtracted), 2);

      c_Text += mh_GetTableLineForBrowser(
          "Sending public key of file",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_SecurityKeySent), 2);

      // PEM file states
      c_Text += mh_GetTableLineForBrowser(
          "Sending security enable flag",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_SecurityFlagSent), 2);

      c_Text += mh_GetTableLineForBrowser(
          "Sending debugger enable flag",
          mh_GetSuSequenceNodeStateString(rc_FileState.e_DebuggerFlagSent), 2);
    }

    if (q_AtLeastOneFile == false) {
      c_Text += mh_GetTableLineForBrowser("No files for updating", "", 1);
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns the two input strings in HTML table format separated in two
   column for one row
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SyvUpNodePropertiesDialog::mh_GetTableLineForBrowser(
    const QString &orc_FirstRow, const QString &orc_SecondRow,
  ...
  c_Text += "<table width=\"100%\" style =\" margin-left:" +
            QString::number(s32_UsedMargin) + "px\">";
  c_Text += "<tr>";
  c_Text += "<td width=\"" + QString::number(s32_UsedWidthCol1) + "%\">" +
            orc_FirstRow + "</td>";
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SyvUpNodePropertiesDialog::mh_GetSuSequenceNodeStateString(
    const E_OscSuSequencesNodeState oe_State) {
  QString c_Text;
  const QString c_Ok = static_cast<QString>("Ok");
  const QString c_Failed = static_cast<QString>("Failed");
  const QString c_Skipped = static_cast<QString>("Skipped");
...
QString C_SyvUpNodePropertiesDialog::mh_GetSuSequenceNodeFileLoadStateString(
  ...
  if ((oe_StateExists == eSUSEQ_STATE_NO_ERR) &&
    ...
    c_Text = c_Ok;
  } else if (((oe_StateExists == eSUSEQ_STATE_NOT_EXECUTED) ||
              (oe_StateExists == eSUSEQ_STATE_NO_ERR)) &&
...
/*! \brief   Returns the precondition state value in text form
   ...
   \param[in]       oq_FeatureError         Flag if the feature is not supported
   but needed
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SyvUpNodePropertiesDialog::mh_GetSuSequenceNodePreconditionString(
    const bool oq_FeatureSupported, const bool oq_FeatureError) {
  QString c_Text;
  const QString c_Yes = static_cast<QString>("Yes");
  const QString c_No = static_cast<QString>("No");
