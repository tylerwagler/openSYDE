...
/*! \brief   Default constructor
   ...
   \param[in]      opc_ProgressLogParent  Optional pointer to progress log
   parent
   \param[in,out]  opc_Parent             Optional pointer to parent
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpUpdateWidget::C_SyvUpUpdateWidget(const uint32_t ou32_ViewIndex,
                                         QWidget *const opc_ProgressLogParent,
                                         QWidget *const opc_Parent)
    : QWidget(opc_Parent), mpc_Ui(new Ui::C_SyvUpUpdateWidget),
      mu32_ViewIndex(ou32_ViewIndex), mu32_ApplicationIndex(0U),
      mpc_Scene(NULL), mpc_ProgressLog(NULL), mpc_FixMinimizedProgressLog(NULL),
      mpc_ProgressLogParent(opc_ProgressLogParent),
      mpc_ProgressLogContent(NULL), mpc_UpSequences(NULL),
      me_Step(C_SyvUpSequences::eNOT_ACTIVE), mq_Connected(false),
      mq_ClearProgressLog(true), mq_StartUpdateAfterConnect(false),
      mq_ConnectFailed(false), mq_ErrorDetected(false),
      mq_NodesPreconditionError(false), ms64_DisconnectTime(0),
      ms64_UpdateTime(0) {
  ...
  // configure scene
  this->mpc_Scene->setSceneRect(
      0.0, 0.0, static_cast<float64_t>(this->mpc_Ui->pc_GraphicsView->width()),
  ...
  this->mpc_Ui->pc_GraphicsView->SetZoomValue(C_UsHandler::h_GetInstance()
                                                  ->GetProjSvSetupView(c_Name)
                                                  .s32_UpdateViewZoom,
                                              true);
  this->mpc_Ui->pc_GraphicsView->SetViewPos(
      C_UsHandler::h_GetInstance()->GetProjSvSetupView(c_Name).c_UpdateViewPos);
  ...
  connect(this->mpc_Scene, &C_SebScene::SigShowToolTip,
          this->mpc_Ui->pc_GraphicsView, &C_SebGraphicsView::ShowToolTip);
  connect(this->mpc_Scene, &C_SebScene::SigHideToolTip,
          this->mpc_Ui->pc_GraphicsView, &C_SebGraphicsView::HideToolTip);
  connect(this->mpc_Ui->pc_GraphicsView, &C_SebGraphicsView::SigShowToolTip,
          this->mpc_Scene, &C_SebScene::DisplayToolTip);
  // Connects
  connect(this->mpc_Ui->pc_PbConnect,
          &stw::opensyde_gui_elements::C_OgePubUpdate::clicked, this,
          &C_SyvUpUpdateWidget::m_Connect);
  connect(this->mpc_Ui->pc_PbUpdate,
          &stw::opensyde_gui_elements::C_OgePubUpdate::clicked, this,
          &C_SyvUpUpdateWidget::m_Update);
  connect(this->mpc_Ui->pc_PbDisconnnect,
          &stw::opensyde_gui_elements::C_OgePubUpdate::clicked, this,
          &C_SyvUpUpdateWidget::m_Disconnect);
  connect(this->mpc_Ui->pc_PbCancel,
          &stw::opensyde_gui_elements::C_OgePubUpdate::clicked, this,
          &C_SyvUpUpdateWidget::m_Cancel);
  connect(this->mpc_Scene, &C_SyvUpScene::SigDiscardInfo, this,
          &C_SyvUpUpdateWidget::m_DiscardInfo);
  connect(this, &C_SyvUpUpdateWidget::SigNodeConnectStates, this->mpc_Scene,
          &C_SyvUpScene::SetNodeConnectStates);
  connect(this, &C_SyvUpUpdateWidget::SigNodeUpdateStates, this->mpc_Scene,
          &C_SyvUpScene::SetNodeUpdateStates);
  connect(this, &C_SyvUpUpdateWidget::SigInvalidView,
          this->mpc_Ui->pc_WiUpdateInformation,
          &C_SyvUpInformationWidget::SigInvalidView);

  connect(&this->mc_Timer, &QTimer::timeout, this,
          &C_SyvUpUpdateWidget::m_Timer);
  ...
  connect(this->mpc_Ui->pc_WiUpdateInformation,
          &C_SyvUpInformationWidget::SigUpdatePackageState, this,
          &C_SyvUpUpdateWidget::m_UpdatePackageState);
...
*/
//----------------------------------------------------------------------------------------------------------------------
// lint -e{1540}  no memory leak because of the parent of mpc_Scene and the Qt
// memory management
C_SyvUpUpdateWidget::~C_SyvUpUpdateWidget() {
  ...
  if (pc_View != NULL) {
    ...
    if (c_Sizes.count() > 1) {
      C_UsHandler::h_GetInstance()->SetProjSvUpdateHorizontalSplitterVertical(
          pc_View->GetName(), c_Sizes.at(1));
    }
...
void C_SyvUpUpdateWidget::InitText(void) const {
  ...
  this->mpc_Ui->pc_PbConnect->setText(
      static_cast<QString>("Enter Update Mode"));
  this->mpc_Ui->pc_PbDisconnnect->setText(
      static_cast<QString>("Exit Update Mode"));
  this->mpc_Ui->pc_PbCancel->setText(
      static_cast<QString>("Abort System Update"));
  ...
  this->mpc_Ui->pc_PbConnect->SetToolTipInformation(
      "Enter Update Mode",
      "Puts all devices into flashloader mode and gets current "
      "versions of each device.");
  this->mpc_Ui->pc_PbDisconnnect->SetToolTipInformation(
      "Exit Update Mode", "Puts all devices back into application mode.");
  this->mpc_Ui->pc_PbCancel->SetToolTipInformation(
      "Abort System Update", "Forces the update to abort.");
}
  ...
  if (this->mpc_UpSequences != NULL) {
    ...
    if (this->me_Step == C_SyvUpSequences::eNOT_ACTIVE) {
      ...
      C_OgeWiCustomMessage c_MessageBox(
          this, C_OgeWiCustomMessage::E_Type::eQUESTION);
      C_OgeWiCustomMessage::E_Outputs e_ReturnMessageBox;

      c_MessageBox.SetHeading("System Update abort");
      c_MessageBox.SetDescription("System Update is running. Do you really "
                                  "want to abort the current System Update?");
      ...
      if (e_ReturnMessageBox == C_OgeWiCustomMessage::eNO) {
        ...
        // Wait till the thread has finished and process the signals from the
        // thread
        do {
...
void C_SyvUpUpdateWidget::resizeEvent(QResizeEvent *const opc_Event) {
  if (this->mpc_ProgressLog != NULL) {
    ...
    // only resize if scene is active to avoid bad toolbox geometry in case of
    // "open tool with *.syde double click"
    //      if ((this->mpc_Scene != NULL) && (this->mpc_Scene->isActive() ==
    //      true))
    //      {
    ...
    if ((this->mpc_ProgressLog->x() + this->mpc_ProgressLog->width() +
         mhs32_WIDGET_BORDER) > pc_Widget->width()) {
      // is the toolbox to big?
      if ((this->mpc_ProgressLog->width() + (2 * mhs32_WIDGET_BORDER)) >
          pc_Widget->width()) {
        c_Size.setWidth(pc_Widget->width() - (2 * mhs32_WIDGET_BORDER));
      } else {
        // adapt position of toolbox
        c_Point.setX((pc_Widget->width() - this->mpc_ProgressLog->width()) -
                     mhs32_WIDGET_BORDER);
      }
    }

    // would the toolbox be outside of the widget in y direction
    if ((this->mpc_ProgressLog->y() + this->mpc_ProgressLog->height() +
         mhs32_WIDGET_BORDER) > pc_Widget->height()) {
      // is the toolbox to big?
      if ((this->mpc_ProgressLog->height() + (2 * mhs32_WIDGET_BORDER)) >
          pc_Widget->height()) {
        c_Size.setHeight(pc_Widget->height() - (2 * mhs32_WIDGET_BORDER));
      } else {
        // adapt position of toolbox
        c_Point.setY((pc_Widget->height() - this->mpc_ProgressLog->height()) -
                     mhs32_WIDGET_BORDER);
      }
    }

    // adapt position of fix minimized toolbox
    c_PointFixMiniProgressLog.setX(
        (pc_Widget->width() - this->mpc_FixMinimizedProgressLog->width()) -
        mhs32_WIDGET_BORDER);

    this->mpc_ProgressLog->setGeometry(QRect(c_Point, c_Size));
    this->mpc_FixMinimizedProgressLog->setGeometry(QRect(
        c_PointFixMiniProgressLog, this->mpc_FixMinimizedProgressLog->size()));
    //      }
  }
}
...
void C_SyvUpUpdateWidget::m_CheckError(void) {
  ...
  this->mq_ErrorDetected = C_SyvUtil::h_GetViewStatusLabelInfo(
      this->mu32_ViewIndex, ms32_SUBMODE_SYSVIEW_UPDATE, c_ErrorTextHeading,
      c_ErrorText, c_ErrorTextTooltip, e_ToolTipType, c_IconPath, s32_ColorId);
  ...
  if (this->mq_ErrorDetected == true) {
    ...
    this->mpc_Ui->pc_ErrorLabelIcon->SetToolTipInformation(
        "Invalid", c_ErrorTextTooltip, e_ToolTipType);
    this->mpc_Ui->pc_ErrorLabelTitle->SetForegroundColor(s32_ColorId);
    this->mpc_Ui->pc_ErrorLabelTitle->setText(c_ErrorTextHeading);
    this->mpc_Ui->pc_ErrorLabelTitle->SetToolTipInformation(
        "Invalid", c_ErrorTextTooltip, e_ToolTipType);
    this->mpc_Ui->pc_ErrorLabel->SetForegroundColor(s32_ColorId);
    this->mpc_Ui->pc_ErrorLabel->SetCompleteText(
        c_ErrorText, c_ErrorTextTooltip, e_ToolTipType);
...
int32_t C_SyvUpUpdateWidget::m_InitSequence(void) {
  ...
  if (pc_View != NULL) {
    if (pc_View->GetOscPcData().GetConnected()) {
      const C_OscSystemBus *const pc_Bus =
          C_PuiSdHandler::h_GetInstance()->GetOscBus(
  ...
  if (this->mpc_UpSequences == NULL) {
    ...
    connect(this->mpc_UpSequences,
            &C_SyvUpSequences::SigReportProgressForServer, this,
            &C_SyvUpUpdateWidget::m_ReportProgressForServer,
            Qt::QueuedConnection);
    connect(this->mpc_UpSequences,
            &C_SyvUpSequences::SigReportOpenSydeFlashloaderInformationRead,
            this,
            &C_SyvUpUpdateWidget::m_ReportOpenSydeFlashloaderInformationRead);
    connect(this->mpc_UpSequences,
            &C_SyvUpSequences::SigReportStwFlashloaderInformationRead, this,
  ...
  case C_CONFIG:
    c_Message =
        static_cast<QString>("Invalid SYSTEM DEFINITION/View configuration.");
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  case C_RD_WR:
    c_Message = static_cast<QString>("Configured communication DLL does not "
                                     "exist or DLL could not be opened.");
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  case C_OVERFLOW:
    c_Message =
        static_cast<QString>("Unknown transport protocol or unknown diagnostic "
                             "server for at least one node.");
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  case C_NOACT:
    c_Message = static_cast<QString>(
        "System View is invalid. Action cannot be performed.");
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  case C_COM:
    if (q_IsEthernet) {
      c_Message = static_cast<QString>(
          "Ethernet initialization failed. Check your Ethernet adapter "
          "settings in Windows system network configuration.");
      c_MessageBox.SetCustomMinHeight(180, 180);
    } else {
      const uint32_t u32_BITNESS = 8 * sizeof(size_t);
      c_Message =
          static_cast<QString>(
              "CAN initialization failed. Check your PC CAN interface "
              "configuration (System View "
              "setup - double-click on PC). Make sure to use a %1-bit DLL.")
              .arg(u32_BITNESS);
      c_MessageBox.SetCustomMinHeight(180, 180);
    }
    break;
  ...
  case C_UNKNOWN_ERR:
    c_Message =
        static_cast<QString>("Wrapped error of internal function call.");
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  case C_WARN:
    ...
    break;
  case C_BUSY:
    c_Message = static_cast<QString>("System View is invalid. Action cannot be "
                                     "performed. Fix the issues and retry.");
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  default:
    c_Message = static_cast<QString>("Unknown error: %1")
                    .arg(C_Uti::h_StwError(s32_Retval));
    c_MessageBox.SetCustomMinHeight(180, 180);
    break;
  }
...
void C_SyvUpUpdateWidget::m_CleanUpSequence(void) {
  if (this->mpc_UpSequences != NULL) {
    ...
    disconnect(this->mpc_UpSequences, &C_SyvUpSequences::SigReportProgress,
               this, &C_SyvUpUpdateWidget::m_ReportProgress);
    disconnect(this->mpc_UpSequences,
               &C_SyvUpSequences::SigReportProgressForServer, this,
               &C_SyvUpUpdateWidget::m_ReportProgressForServer);
    disconnect(
        this->mpc_UpSequences,
        &C_SyvUpSequences::SigReportOpenSydeFlashloaderInformationRead, this,
        &C_SyvUpUpdateWidget::m_ReportOpenSydeFlashloaderInformationRead);
    disconnect(this->mpc_UpSequences,
               &C_SyvUpSequences::SigReportStwFlashloaderInformationRead, this,
               &C_SyvUpUpdateWidget::m_ReportStwFlashloaderInformationRead);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Adapts the state of the buttons depending on the update package and
   the view error state

   \param[in]  os32_State  State of update package
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_UpdatePackageState(const int32_t os32_State) {
  if ((os32_State == C_NO_ERR) && (this->mq_ErrorDetected == false)) {
    ...
    // when Connect and Update is enabled we also want to lock the Update
    // Package creation
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_ReportProgress(const uint32_t ou32_Step,
                                           const int32_t os32_Result,
                                           const uint8_t ou8_Progress) {
  const C_OscSuSequences::E_ProgressStep e_Step =
      static_cast<C_OscSuSequences::E_ProgressStep>(ou32_Step);
  ...
  if (e_Step == C_OscSuSequences::eUPDATE_SYSTEM_FINISHED) {
    ...
    this->mpc_Ui->pc_WiUpdateInformation->SetHeading(
        "://images/system_views/UpdateSummaryOk.svg",
...
/*! \brief   Reporting slot for system update for specific server
   ...
   \param[in]  ou32_Step            Step of node configuration
   \param[in]  os32_Result          Result of service
   \param[in]  ou8_Progress         Progress of sequence in percentage (goes
   from 0..100 for each function) Progress invalid: 255
   \param[in]  ou8_BusIdentifier    Affected sub net
   \param[in]  ou8_NodeIdentifier   Node Id of affected node
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_ReportProgressForServer(
    const uint32_t ou32_Step, const int32_t os32_Result,
  ...
  const int32_t s32_Result = C_SyvUtil::h_GetIndicesFromBusId(
      ou8_BusIdentifier, ou8_NodeIdentifier, u32_NodeIndex, u32_BusIndex);
  ...
  if (s32_Result == C_NO_ERR) {
    ...
    if (C_SyvUpUpdateWidget::mh_IsConnectionStart(e_Step) == true) {
    ...
    } else if (C_SyvUpUpdateWidget::mh_IsUpdateAppStart(e_Step, q_IsParam,
                                                        q_IsPemFile) == true) {
      this->mpc_Ui->pc_WiUpdateInformation->SetUpdateApplicationStarted(
          u32_NodeIndex, q_IsParam, q_IsPemFile);
      ...
      // Scroll scene
      if (pc_UiNode != NULL) {
        this->mpc_Ui->pc_GraphicsView->ScrollTo(
            pc_UiNode->c_UiPosition,
      ...
      if (this->mpc_ProgressLogContent != NULL) {
        ...
        if (pc_Node != NULL) {
          const QString c_Text = static_cast<QString>("Update %1...")
      ...
      // Signal progress log
      if (this->mpc_ProgressLogContent != NULL) {
        this->mpc_ProgressLogContent->UpdateStatus(u32_NodeIndex, "FAIL",
                                                   false);
      }
      // Signal scene
      if (this->mpc_Scene != NULL) {
      ...
      this->mpc_Ui->pc_WiUpdateInformation->SetUpdateApplicationError(
          u32_NodeIndex);
      // Signal progress log
      if (this->mpc_ProgressLogContent != NULL) {
      ...
      if (this->mpc_Scene != NULL) {
        this->mpc_Scene->SetNodeError(u32_NodeIndex);
        this->mpc_Scene->StopProgressAnimation(
            true, this->mu32_ApplicationIndex, true);
      }
      ...
      // Signal progress log
      if (this->mpc_ProgressLogContent != NULL) {
        this->mpc_ProgressLogContent->UpdateStatus(u32_NodeIndex,
                                                   "USER CANCELED", true);
      }
      // Signal scene
      if (this->mpc_Scene != NULL) {
      ...
      // Signal summary
      this->mpc_Ui->pc_WiUpdateInformation->SetHeading(
          "://images/system_views/UpdateSummaryFailure.svg",
          "System Update failed!", true, false);
      this->mpc_Ui->pc_WiUpdateInformation->StopElapsedTimer();
    } else if (C_SyvUpUpdateWidget::mh_IsUpdateAppSuccess(
                   e_Step, q_IsParam, q_IsPemFile) == true) {
      ++this->mu32_ApplicationIndex;
      this->mpc_Ui->pc_WiUpdateInformation->SetUpdateApplicationFinished(
          u32_NodeIndex, q_IsParam, q_IsPemFile);
      ...
      {
      case C_OscSuSequences::
          eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_AREA_TRANSFER_START: // The normal
                                                                 // flash
                                                                 // progress
      case C_OscSuSequences::
          eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_TRANSFER_START: // The normal flash
                                                             // progress
      case C_OscSuSequences::eXFL_PROGRESS: // The normal flash progress
        if (ou8_Progress != 0xFF) {
          this->mpc_Ui->pc_WiUpdateInformation->SetNodeProgress(u32_NodeIndex,
                                                                ou8_Progress);
          // Signal summary
          this->mpc_Ui->pc_WiUpdateInformation->UpdateProgress(
              static_cast<uint16_t>(ou8_Progress), false);
        }
  ...
  if (this->mpc_UpSequences != NULL) {
    ...
    for (u32_Counter = 0U; u32_Counter < c_NodeIndexes.size(); ++u32_Counter) {
      const C_OscSuSequences::C_OsyDeviceInformation &rc_Info =
          c_DeviceInformation[u32_Counter];
      QString c_Temp;
      ...
      for (uint8_t u8_Application = 0U;
           u8_Application < rc_Info.c_Applications.size(); u8_Application++) {
        this->m_UpdateReportText(
        ...
        this->m_UpdateReportText(
            static_cast<QString>(" Block start address: 0x%1")
                .arg(QString::number(rc_Info.c_Applications[u8_Application]
                                         .u32_BlockStartAddress,
                                     16)
                         .rightJustified(8, '0')));
        this->m_UpdateReportText(
            static_cast<QString>(" Block end address: 0x%1")
                .arg(QString::number(rc_Info.c_Applications[u8_Application]
                                         .u32_BlockEndAddress,
                                     16)
                         .rightJustified(8, '0')));
        this->m_UpdateReportText(
            static_cast<QString>(" Signature valid: %1")
                .arg(
                    (rc_Info.c_Applications[u8_Application].u8_SignatureValid ==
                     0)
                        ? "yes"
                        : "no"));
        this->m_UpdateReportText(
            static_cast<QString>(" Additional information: %1")
                .arg(rc_Info.c_Applications[u8_Application]
                         .c_AdditionalInformation));
      }

      this->m_UpdateReportText(
          static_cast<QString>("Flashloader software version: V%1.%2r%3")
              .arg(QString::number(rc_Info.c_MoreInformation
                                       .au8_FlashloaderSoftwareVersion[0])
                       .rightJustified(2, '0'),
                   QString::number(rc_Info.c_MoreInformation
                   ...
                   QString::number(rc_Info.c_MoreInformation
                       ...
                       .rightJustified(2, '0')));
      this->m_UpdateReportText(
          static_cast<QString>("Flashloader protocol version: V%1.%2r%3")
              .arg(QString::number(rc_Info.c_MoreInformation
                   ...
                   QString::number(rc_Info.c_MoreInformation
                       ...
                       .rightJustified(2, '0')));
      this->m_UpdateReportText(
          static_cast<QString>("Protocol version: V%1.%2r%3")
              .arg(QString::number(
                       rc_Info.c_MoreInformation.au8_ProtocolVersion[0])
                       .rightJustified(2, '0'),
                   QString::number(
                       rc_Info.c_MoreInformation.au8_ProtocolVersion[1])
                       .rightJustified(2, '0'),
                   QString::number(
                       rc_Info.c_MoreInformation.au8_ProtocolVersion[2])
                       .rightJustified(2, '0')));
      this->m_UpdateReportText(
          static_cast<QString>("Flash count: %1")
              .arg(rc_Info.c_MoreInformation.u32_FlashCount));
      if (rc_Info.c_MoreInformation.c_AvailableFeatures
              .q_ExtendedSerialNumberModeImplemented == false) {
        c_Temp = "(Format: Standard)";
      } else {
        c_Temp = static_cast<QString>(
                     "(Format: Extended with Manufacturer Format %1)")
                     .arg(QString::number(static_cast<uint32_t>(
                         rc_Info.c_MoreInformation.c_SerialNumber
      ...
      this->m_UpdateReportText(
          static_cast<QString>("Flash fingerprint date: %1-%2-%3 (yy-mm-dd)")
              .arg(QString::number(
                       static_cast<uint32_t>(rc_Info.c_MoreInformation
                   ...
                   QString::number(
                       ...
                       .rightJustified(2, '0')));
      this->m_UpdateReportText(
          static_cast<QString>("Flash fingerprint time: %1:%2:%3")
              .arg(QString::number(
                       static_cast<uint32_t>(rc_Info.c_MoreInformation
      ...
      if (rc_Info.c_MoreInformation.c_AvailableFeatures
        ...
        this->m_UpdateReportText(
            static_cast<QString>(
                " Maximum block size information available: 1 (%1 Bytes)")
                .arg(QString::number(
                    rc_Info.c_MoreInformation.u16_MaxNumberOfBlockLength)));
      } else {
        this->m_UpdateReportText(static_cast<QString>(
            " Maximum block size information available: 0"));
      }
      this->m_UpdateReportText(
          static_cast<QString>(" NVM writing available: %1")
              .arg(QString::number(static_cast<uint8_t>(
                  rc_Info.c_MoreInformation.c_AvailableFeatures
                      .q_FlashloaderCanWriteToNvm))));
      this->m_UpdateReportText(
          static_cast<QString>(" Security supported: %1")
              .arg(QString::number(static_cast<uint8_t>(
                  rc_Info.c_MoreInformation.c_AvailableFeatures
                      .q_SupportsSecurity))));
      this->m_UpdateReportText(
          static_cast<QString>(" Disabling debugger supported: %1")
              .arg(QString::number(static_cast<uint8_t>(
                  rc_Info.c_MoreInformation.c_AvailableFeatures
                      .q_SupportsDebuggerOff))));
      this->m_UpdateReportText(
          static_cast<QString>(" Enabling debugger supported: %1")
              .arg(QString::number(static_cast<uint8_t>(
                  rc_Info.c_MoreInformation.c_AvailableFeatures
                      .q_SupportsDebuggerOn))));
      this->m_UpdateReportText(
          static_cast<QString>(" Ethernet2Ethernet routing supported: %1")
              .arg(QString::number(static_cast<uint8_t>(
                  rc_Info.c_MoreInformation.c_AvailableFeatures
    ...
    for (uint32_t u32_ItDevice = 0; u32_ItDevice < c_DeviceInformation.size();
         ++u32_ItDevice) {
      C_SyvUpDeviceInfo c_NewDevice;
      const C_OscSuSequences::C_OsyDeviceInformation &rc_Info =
          c_DeviceInformation[u32_ItDevice];
      c_NewDevice.pc_OpenSydeDevice = &rc_Info;
      c_Devices.push_back(c_NewDevice);
    }

    // Check for problems
    this->m_CheckOpenSydeFlashloaderInformation(c_NodeIndexes,
                                                c_DeviceInformation);
    ...
    this->mpc_Ui->pc_WiUpdateInformation->UpdateDeviceInformation(c_NodeIndexes,
                                                                  c_Devices);
  }
}
...
void C_SyvUpUpdateWidget::m_ReportStwFlashloaderInformationRead(void) {
  if (this->mpc_UpSequences != NULL) {
    ...
    for (u32_Counter = 0U; u32_Counter < c_NodeIndexes.size(); ++u32_Counter) {
      ...
      this->m_UpdateReportText(
          static_cast<QString>(
              "STW Flashloader device information found for node with index %1")
              .arg(c_NodeIndexes[u32_Counter]));

      C_OscSuSequences::h_StwFlashloaderInformationToText(
          c_DeviceInformation[u32_Counter], c_Strings);

      for (uint32_t u32_StringIndex = 0U;
           u32_StringIndex < (uint32_t)c_Strings.count(); u32_StringIndex++) {
        this->m_UpdateReportText(c_Strings.at(u32_StringIndex));
      }
    }
    ...
    for (uint32_t u32_ItDevice = 0; u32_ItDevice < c_DeviceInformation.size();
         ++u32_ItDevice) {
      C_SyvUpDeviceInfo c_NewDevice;
      const C_OscSuSequences::C_XflDeviceInformation &rc_Info =
          c_DeviceInformation[u32_ItDevice];
...
/*! \brief  Check the read openSYDE Flashloader information for problems
   ...
   \param[in]  orc_OsyDeviceInformation   All device information associated to
   the node index in the same order
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_CheckOpenSydeFlashloaderInformation(
  ...
  for (u32_NodesInformationCounter = 0U;
       u32_NodesInformationCounter < orc_OsyNodeIndexes.size();
    ...
    for (u32_NodesPackageCounter = 0U;
         u32_NodesPackageCounter < this->mc_NodesToFlash.size();
         ++u32_NodesPackageCounter) {
      if (orc_OsyNodeIndexes[u32_NodesInformationCounter] ==
          u32_NodesPackageCounter) {
        // Same node index
        const C_OscSuSequences::C_OsyDeviceInformation &rc_Info =
        ...
        if (this->mc_NodesToFlash[u32_NodesPackageCounter]
          ...
          if ((rc_Info.c_MoreInformation.c_AvailableFeatures
            ...
            this->mc_NodePreconditionErrors.c_NvmWriteError.push_back(
                u32_NodesPackageCounter);

            osc_write_log_error("Update Node",
                                "Node " + c_NodeName +
                                    " has not the Flashloader features to "
                                    "write a Parameter Set Image file.");
          }
        }
        ...
        if (this->mc_NodesToFlash[u32_NodesPackageCounter].c_PemFile != "") {
          ...
          if (rc_Info.c_MoreInformation.c_AvailableFeatures
            ...
            // Node does not have the ability
            this->mq_NodesPreconditionError = true;
            this->mc_NodePreconditionErrors.c_PemWriteError.push_back(
                u32_NodesPackageCounter);

            osc_write_log_error("Update Node",
                                "Node " + c_NodeName +
                                    " has not the Flashloader features to "
                                    "support security and to write PEM files.");
          }

          if (this->mc_NodesToFlash[u32_NodesPackageCounter]
                  .q_SendDebuggerEnabledState == true) {
            if ((rc_Info.c_MoreInformation.c_AvailableFeatures
                     .q_SupportsDebuggerOn == false) &&
                (this->mc_NodesToFlash[u32_NodesPackageCounter]
                     .q_DebuggerEnabled == true)) {
              // Node does not have the ability
              this->mq_NodesPreconditionError = true;
              this->mc_NodePreconditionErrors.c_DebuggerEnableError.push_back(
                  u32_NodesPackageCounter);

              osc_write_log_error("Update Node",
                                  "Node " + c_NodeName +
                                      " has not the Flashloader features to "
                                      "support enabling the debugger.");
            }
            ...
            if ((rc_Info.c_MoreInformation.c_AvailableFeatures
              ...
              // Node does not have the ability
              this->mq_NodesPreconditionError = true;
              this->mc_NodePreconditionErrors.c_DebuggerDisableError.push_back(
                  u32_NodesPackageCounter);

              osc_write_log_error("Update Node",
                                  "Node " + c_NodeName +
                                      " has not the Flashloader features to "
                                      "support disabling the debugger.");
            }
          }
        }
        ...
        if ((rc_Info.c_MoreInformation.c_AvailableFeatures
            ...
            (this->mpc_UpSequences->IsEthToEthRoutingNecessary(
                 u32_NodesPackageCounter) == true)) {
          // Node does not support Ethernet to Ethernet routing but it is
          // necessary for at least one route
          this->mq_NodesPreconditionError = true;
          this->mc_NodePreconditionErrors.c_EthToEthError.push_back(
              u32_NodesPackageCounter);

          osc_write_log_error("Update Node",
                              "Node " + c_NodeName +
                                  " has not the Flashloader features for "
...
void C_SyvUpUpdateWidget::m_Connect(void) {
  ...
  if (this->mpc_ProgressLogContent != NULL) {
    ...
    // Let the user know, when the tool is occupied with itself before
  ...
  while ((this->ms64_DisconnectTime + mhu32_WAIT_TIME) >
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    // Signal scene
    if (this->mpc_Scene != NULL) {
      // Reset connected status (required if in system disconnected state due to
      // no node response)
    ...
    // Due to the timer of the update package and the signal used, timing could
    // be a problem. Let the update package timer and its signal finish in any
    // case and deactivate the buttons again just in case
    QApplication::processEvents();
    this->mpc_Ui->pc_PbConnect->setEnabled(false);
    this->mpc_Ui->pc_PbUpdate->setEnabled(false);
    ...
    s32_Return = this->mpc_Ui->pc_WiUpdateInformation->GetUpdatePackage(
        this->mc_NodesToFlash, this->mc_NodesOrder,
        &this->mc_NodesWithAllApplications);

    if (s32_Return == C_NO_ERR) {
      if (C_SyvUpUpdateWidget::mh_IsSecurityWarningNecessary(
              this->mc_NodesToFlash)) {
        C_OgeWiCustomMessage c_MessageBox(
            this, C_OgeWiCustomMessage::E_Type::eWARNING);
        C_OgeWiCustomMessage::E_Outputs e_Output;
        c_MessageBox.SetHeading("Transferring PEM file");
        c_MessageBox.SetDescription(
            "The Update Package includes PEM file(s). After transferring the "
            "public certificate and activating the security flag on the "
            "target,\n"
    ...
    // Saving both, the original and temporary, paths will help to map changed
    // relevant applications between connect and update by discarding
    // application information blocks
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      s32_Return = this->mpc_UpSequences->SyvUpCreateTemporaryFolder(
          c_TemporaryPath, this->mc_NodesWithAllApplicationsAndTempPath,
          c_ErrorPath);
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        case C_CONFIG:
          ...
          break;
        case C_OVERFLOW:
          c_MessageText = "Number of applications to write or number of active "
                          "nodes is invalid.";
          break;
        case C_NOACT:
          c_MessageText = "Update package contains at least one file for an "
                          "inactive node or number of "
      ...
      this->m_UpdateReportText(
          static_cast<QString>("Activate Flashloader: Cannot start. Error on "
                               "creation temporary files: %1")
              .arg(C_Uti::h_StwError(s32_Return)));
    }
...
void C_SyvUpUpdateWidget::m_Update(void) {
  if ((this->mq_Connected == true) && (this->mq_ConnectFailed == true)) {
    // A connect was done and it failed
    C_OgeWiCustomMessage c_Message(this, C_OgeWiCustomMessage::E_Type::eERROR);
    c_Message.SetHeading("System Update");
    c_Message.SetDescription("Update process cannot be started. There are "
                             "active nodes, which are not responding. \n"
    ...
    if (this->mq_Connected == true) {
      ...
      if (this->mpc_UpSequences != NULL) {
        ...
        if (pc_View != NULL) {
          ...
          while ((this->ms64_UpdateTime + mhu32_WAIT_TIME) >
                 QDateTime::currentMSecsSinceEpoch()) {
            // Wait till it is possible
          }
          ...
          if (s32_Return == C_NOACT) {
            ...
            c_Message.SetHeading("System Update");
            c_Message.SetDescription(
                "No System Update needed. All applications for the devices "
                "match the ones in the "
                "Update Package. To update anyway see details.");
            c_Message.SetDetails("For updating a device nevertheless, there is "
                                 "an option to trigger a \"Force Update\" "
                                 "in the \"Device Status Information\" dialog "
          ...
          if (s32_Return == C_NO_ERR) {
            ...
            s32_Return = this->mpc_UpSequences->StartUpdateSystem(
                this->mc_NodesToFlash, this->mc_NodesOrder);
      ...
      // Connect in the first step and start the update after the successful
      // connecting sequence
...
void C_SyvUpUpdateWidget::m_Cancel(void) {
  if (this->mpc_UpSequences != NULL) {
    ...
    c_MessageBox.SetDescription("System Update is running. Do you really want "
                                "to abort the current System Update?");
    c_MessageBox.SetOkButtonText("Continue Update");
    c_MessageBox.SetNoButtonText("Abort Update");
    c_MessageBox.SetCustomMinWidth(650);
    c_MessageBox.SetCustomMinHeight(180, 180);
  ...
  if (s32_Result == C_NO_ERR) {
    ...
    case C_SyvUpSequences::eACTIVATE_FLASHLOADER:
      if (s32_SequenceResult == C_NO_ERR) {
        ...
        Q_ASSERT(this->mpc_UpSequences->GetConnectStates(c_NodeStates) ==
                 C_NO_ERR);
        Q_EMIT(this->SigNodeConnectStates(c_NodeStates,
                                          this->mc_NodePreconditionErrors));
    ...
    case C_SyvUpSequences::eREAD_DEVICE_INFORMATION:
      ...
      if ((s32_SequenceResult == C_NO_ERR) &&
        ...
        if (this->mq_StartUpdateAfterConnect == true) {
          // In the event queue could be some emitted signals from thread left.
          // We need all information for showing the result before starting the
          // update
          QApplication::processEvents();
          ...
          // Only necessary if not directly updating because updating will do
          // this step automatically Update the state of the update package (now
          // includes the device status) Prepare the update package
          this->mc_NodesToFlash.clear();
          this->mc_NodesOrder.clear();
          // Get the update package with the update configuration
          this->mpc_Ui->pc_WiUpdateInformation->GetUpdatePackage(
              this->mc_NodesToFlash, this->mc_NodesOrder);
          this->mpc_Ui->pc_WiUpdateInformation->InitUpdatePackage(
              this->mc_NodesToFlash, this->mc_NodesOrder,
              m_GetIsFileBasedFlagForEach(), true);
        ...
        m_HandleConnectionFailure((s32_SequenceResult == C_NO_ERR)
                                      ? this->mq_NodesPreconditionError
                                      : false);
        ...
        if (this->mq_NodesPreconditionError == true) {
          ...
          if (this->mc_NodePreconditionErrors.c_NvmWriteError.size() > 0) {
            this->m_HandlePreconditionErrorType(
                ...
                "Following node(s) do not support writing parameter set image "
                "files:\n");
          }
          ...
          if (this->mc_NodePreconditionErrors.c_PemWriteError.size() > 0) {
            this->m_HandlePreconditionErrorType(
                ...
                "Following node(s) do not support security and writing PEM "
                "files:\n");
          }

          // Error for debugger enabling
          if (this->mc_NodePreconditionErrors.c_DebuggerEnableError.size() >
              0) {
            this->m_HandlePreconditionErrorType(
          ...
          if (this->mc_NodePreconditionErrors.c_DebuggerDisableError.size() >
              0) {
            this->m_HandlePreconditionErrorType(
          ...
          if (this->mc_NodePreconditionErrors.c_EthToEthError.size() > 0) {
            this->m_HandlePreconditionErrorType(
                ...
                " to Ethernet routing.",
                "Following node(s) do not support Ethernet to Ethernet "
                "routing:\n");
          }
        }
      }
    ...
    case C_SyvUpSequences::eUPDATE_SYSTEM:
      ...
      case C_NO_ERR:
        ...
        break;
      case C_CONFIG:
        c_Message = "Initialization of update mechanism invalid or no "
      ...
      case C_DEFAULT:
        c_Message =
            "At least one of the *.syde_psi file has an invalid checksum.";
        break;
      case C_RANGE:
        c_Message = "At least one necessary feature of the openSYDE "
                    "Flashloader is not available for NVM writing.";
        break;
      case C_CHECKSUM:
        c_Message = "Authentication between openSYDE Tool and device(s) has "
      ...
      if (s32_SequenceResult != C_NO_ERR) {
        ...
        // Adapt the update configuration. Remove all applications which are
        // updated successfully in case of a retry without new connect
        if (this->mpc_UpSequences->GetLastUpdatePosition(
                u32_ErrorNodeIndex, u32_ErrorFileIndex) == C_NO_ERR) {
          uint32_t u32_Position;
          // Remove all applications of the finished nodes
          for (u32_Position = 0U; u32_Position < this->mc_NodesOrder.size();
               u32_Position++) {
          ...
          this->mc_NodesOrder.erase(this->mc_NodesOrder.begin(),
                                    this->mc_NodesOrder.begin() + u32_Position);
          ...
          if ((u32_ErrorFileIndex > 0U) &&
              ...
              (u32_ErrorFileIndex < this->mc_NodesToFlash[u32_ErrorNodeIndex]
                                        .c_FilesToFlash.size())) {
            this->mc_NodesToFlash[u32_ErrorNodeIndex].c_FilesToFlash.erase(
                this->mc_NodesToFlash[u32_ErrorNodeIndex]
                    .c_FilesToFlash.begin(),
                this->mc_NodesToFlash[u32_ErrorNodeIndex]
                        .c_FilesToFlash.begin() +
                    u32_ErrorFileIndex);
          }
        ...
        // Signal progress log
        if (this->mpc_ProgressLogContent != NULL) {
          this->mpc_ProgressLogContent->UpdateStatus(u32_ErrorNodeIndex, "FAIL",
                                                     true);
        }
        // Signal scene
        if (this->mpc_Scene != NULL) {
        ...
        // Need to be written before m_HandleUpdateFailure due to the link in
        // the message box
        this->m_UpdateReportText(
            static_cast<QString>("Update System: %1").arg(c_Message));
    ...
    case C_SyvUpSequences::eRESET_SYSTEM:
      ...
      // Signal summary
      this->mpc_Ui->pc_WiUpdateInformation->ResetSummary();
      // This one will implicitly enable the buttons therefore it cannot be
      // informed at an earlier time
      //  And update package
      this->mpc_Ui->pc_WiUpdateInformation->SetDisconnected();
    ...
    if (s32_SequenceResult == C_CHECKSUM) {
      C_OgeWiCustomMessage c_MessageAuth(this,
                                         C_OgeWiCustomMessage::E_Type::eERROR);

      C_OscLoggingHandler::h_Flush();
      c_MessageAuth.SetHeading("System Update");
      c_MessageAuth.SetDescription("Authentication between openSYDE Tool and "
                                   "device(s) has failed. Access denied.");
      c_MessageAuth.SetDetails(
          "Possible reasons:<br/>"
          "- Associated private key (*.pem) not found in /certificates folder "
          "(most common)<br/>"
...
/*! \brief   Handles a precondition error type
   ...
   \param[in]       orc_ErrorNodeIndexes     Node indexes in system definition
   of all nodes with error of the same type
   \param[out]      orc_Description          Description text for the message
   box
   \param[in,out]   orc_DetailsStart         Beginning of the details text of
   the message box
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_HandlePreconditionErrorType(
  ...
  for (u32_NodeCounter = 0U; u32_NodeCounter < orc_ErrorNodeIndexes.size();
       ++u32_NodeCounter) {
    this->m_HandleNodePreconditionError(c_Details,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Extends the error text with the node name and informs the progress
   log and scene about the specific node

   \param[in,out]   orc_ErrorText           Error text for extending by node
   name
   \param[in]       ou32_ErrorNodeIndex     Node index in system definition of
   node with error
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_HandleNodePreconditionError(
    QString &orc_ErrorText, const uint32_t ou32_ErrorNodeIndex) {
  const C_OscNode *const pc_Node =
      C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(ou32_ErrorNodeIndex);

  if (pc_Node != NULL) {
    orc_ErrorText += "- ";
    orc_ErrorText += pc_Node->c_Properties.c_Name;
    orc_ErrorText += "\n";
  }

  // Signal progress log
  if (this->mpc_ProgressLogContent != NULL) {
    this->mpc_ProgressLogContent->UpdateStatus(ou32_ErrorNodeIndex, "FAIL",
                                               false);
  }
  // Signal scene
  if (this->mpc_Scene != NULL) {
...
void C_SyvUpUpdateWidget::m_UpdateReportText(
  ...
  osc_write_log_info("Update Node", orc_NewTextPart);
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Handle connection failure

   \param[in]  oq_SuppressMessageBox   Flag if message box shall be suppressed
   and not be showed
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_HandleConnectionFailure(
    const bool oq_SuppressMessageBox) {
  // Signal summary
  ...
  if ((this->mpc_Scene != NULL) && (this->mpc_ProgressLogContent != NULL)) {
    ...
    c_NodeIndicesWhichRequireResponse.reserve(
        c_ActiveNoneThirdPartyNodeIndices.size() *
        c_ActiveNoneThirdPartyNodeIndices.size());
    for (uint32_t u32_ItNoneThirdPartyNode = 0;
         u32_ItNoneThirdPartyNode < c_ActiveNoneThirdPartyNodeIndices.size();
         ++u32_ItNoneThirdPartyNode) {
      const C_SyvRoRouteCalculation c_Calc(
          this->mu32_ViewIndex,
      ...
      // Directly required
      c_NodeIndicesWhichRequireResponse.push_back(
          c_ActiveNoneThirdPartyNodeIndices[u32_ItNoneThirdPartyNode]);
      if (pc_Route != NULL) {
        for (uint32_t u32_ItRoute = 0;
             u32_ItRoute < pc_Route->c_VecRoutePoints.size(); ++u32_ItRoute) {
          const C_OscRoutingRoutePoint &rc_RountingPoint =
              pc_Route->c_VecRoutePoints[u32_ItRoute];
          // Indirectly required
          c_NodeIndicesWhichRequireResponse.push_back(
              rc_RountingPoint.u32_NodeIndex);
        }
      }
    }
    ...
    for (uint32_t u32_ItActiveNode = 0;
         u32_ItActiveNode < c_NodeIndicesWhichRequireResponse.size();
         ++u32_ItActiveNode) {
      const uint32_t &ru32_CurActiveNodeIndex =
          c_NodeIndicesWhichRequireResponse[u32_ItActiveNode];
      // Check if active
      bool q_AlreadyAdded = false;
      // Check if already added
      for (uint32_t u32_AddedNode = 0; u32_AddedNode < c_RespondedNodes.size();
           ++u32_AddedNode) {
      ...
      if (q_AlreadyAdded == false) {
        ...
        this->mpc_ProgressLogContent->UpdateStatus(ru32_CurActiveNodeIndex,
                                                   "FAIL", false);
    ...
    if (oq_SuppressMessageBox == false) {
      ...
      c_Message.SetDetails(
          "For more information see " +
          C_Uti::h_GetLink("log file", mc_STYLESHEET_GUIDE_COLOR_LINK, c_Log) +
          ".");
...
void C_SyvUpUpdateWidget::m_InitProgressLog(void) {
  ...
  if (this->mpc_ProgressLog == NULL) {
    ...
    mpc_ProgressLog = new C_OgeWiHover(
        *mpc_ProgressLogContent, "PROGRESS LOG",
        ":images/system_views/IconProgressLog.svg", false,
        this->mpc_ProgressLogParent, this->mpc_ProgressLogParent);
  }

  // create fix minimized progress log
  if (this->mpc_FixMinimizedProgressLog == NULL) {
    this->mpc_FixMinimizedProgressLog = new C_OgeWiFixPosition(
        "PROGRESS LOG", ":images/system_views/IconProgressLog.svg",
  ...
  if (c_ViewSettings.GetUpdateProgressLogPos().x() < 0) {
    if (this->mpc_ProgressLogParent == NULL) {
      ...
      this->mpc_ProgressLog->move(mhs32_WIDGET_BORDER,
                                  mhs32_TOOLBOX_INIT_POS_Y);
    } else {
      // default value
      this->mpc_ProgressLog->setGeometry(
          QRect(QPoint((((this->mpc_ProgressLogParent->width() -
    ...
    this->mpc_ProgressLog->setGeometry(
        QRect(c_ViewSettings.GetUpdateProgressLogPos(),
              c_ViewSettings.GetUpdateProgressLogSize()));
    this->mpc_ProgressLog->SetMaximizedHeight(
        c_ViewSettings.GetUpdateProgressLogSize().height());
  }
  ...
  connect(this->mpc_ProgressLog, &C_OgeWiHover::SigWiHoverMinBtnClicked, this,
          &C_SyvUpUpdateWidget::m_WiHoverMinBtnClicked);
  connect(this->mpc_FixMinimizedProgressLog,
          &C_OgeWiFixPosition::SigWiFixPosMaxBtnClicked, this,
          &C_SyvUpUpdateWidget::m_WiFixPosMaxBtnClicked);
}
...
void C_SyvUpUpdateWidget::m_CleanUpProgressLog(void) {
  if (this->mpc_ProgressLog != NULL) {
    ...
    if (pc_View != NULL) {
      ...
      C_UsHandler::h_GetInstance()->SetProjSvUpdateProgressLog(
          pc_View->GetName(), this->mpc_ProgressLog->pos(), c_Size,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpUpdateWidget::m_AddProgressLogConnectEntry(
    const uint32_t ou32_NodeIndex) {
  if (this->mpc_ProgressLogContent != NULL) {
    const C_OscNode *const pc_Node =
        C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(ou32_NodeIndex);
    if (pc_Node != NULL) {
      const QString c_Text = static_cast<QString>("Get status from %1...")
                                 .arg(pc_Node->c_Properties.c_Name);
...
/*! \brief   Adapt paths of mc_NodesToFlash to the temporary folder
   ...
   In case of an update of mc_NodesToFlash, the paths must be adapted again to
   the temporary path with the copied files. This function will adapt the paths
   by using the saved paths in mc_NodesWithAllApplications and
...
void C_SyvUpUpdateWidget::m_ReplaceOriginalWithTempPaths(void) {
  ...
  Q_ASSERT(this->mc_NodesToFlash.size() ==
           this->mc_NodesWithAllApplications.size());
  Q_ASSERT(this->mc_NodesToFlash.size() ==
           this->mc_NodesWithAllApplicationsAndTempPath.size());

  for (u32_NodeCounter = 0U; u32_NodeCounter < this->mc_NodesToFlash.size();
       ++u32_NodeCounter) {
    ...
    for (u32_FileCounter = 0U;
         u32_FileCounter <
         this->mc_NodesToFlash[u32_NodeCounter].c_FilesToFlash.size();
         ++u32_FileCounter) {
      uint32_t u32_AllApplCounter;

      for (u32_AllApplCounter = 0U;
           u32_AllApplCounter <
           this->mc_NodesWithAllApplications[u32_NodeCounter]
               .c_FilesToFlash.size();
           ++u32_AllApplCounter) {
        // mc_NodesWithAllApplications has a 1:1 mapping to
        // mc_NodesWithAllApplicationsAndTempPath Searching of the correct temp
        // path in mc_NodesWithAllApplicationsAndTempPath for mc_NodesToFlash
        if (this->mc_NodesToFlash[u32_NodeCounter]
                .c_FilesToFlash[u32_FileCounter] ==
            this->mc_NodesWithAllApplications[u32_NodeCounter]
                .c_FilesToFlash[u32_AllApplCounter]) {
          // Same path, replace the original by the temporary path
          this->mc_NodesToFlash[u32_NodeCounter]
              .c_FilesToFlash[u32_FileCounter] =
    ...
    for (u32_FileCounter = 0U;
         u32_FileCounter <
         this->mc_NodesToFlash[u32_NodeCounter].c_FilesToWriteToNvm.size();
         ++u32_FileCounter) {
      ...
      for (u32_AllParamFileCounter = 0U;
           ...
           this->mc_NodesWithAllApplications[u32_NodeCounter]
               .c_FilesToWriteToNvm.size();
           ++u32_AllParamFileCounter) {
        // mc_NodesWithAllApplications has a 1:1 mapping to
        // mc_NodesWithAllApplicationsAndTempPath Searching of the correct temp
        // path in mc_NodesWithAllApplicationsAndTempPath for mc_NodesToFlash
        if (this->mc_NodesToFlash[u32_NodeCounter]
                .c_FilesToWriteToNvm[u32_FileCounter] ==
            this->mc_NodesWithAllApplications[u32_NodeCounter]
...
void C_SyvUpUpdateWidget::m_DiscardInfo(const uint32_t ou32_NodeIndex) {
  this->mpc_Ui->pc_WiUpdateInformation->DiscardApplicationStatus(
      ou32_NodeIndex);
...
std::vector<bool> C_SyvUpUpdateWidget::m_GetIsFileBasedFlagForEach(void) const {
  ...
  if (s32_FuncRetval == C_NO_ERR) {
    for (uint32_t u32_ItActiveNode = 0UL;
         u32_ItActiveNode < c_NodeActiveFlags.size(); ++u32_ItActiveNode) {
      if (c_NodeActiveFlags[u32_ItActiveNode] == 1U) {
        const C_OscNode *const pc_Node =
            C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(u32_ItActiveNode);
        if ((pc_Node != NULL) && (pc_Node->pc_DeviceDefinition != NULL) &&
            (pc_Node->u32_SubDeviceIndex <
             pc_Node->pc_DeviceDefinition->c_SubDevices.size())) {
          if (pc_Node->pc_DeviceDefinition
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsConnectionStart(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;

  if (oe_Step ==
      C_OscSuSequences::eACTIVATE_FLASHLOADER_OSY_XFL_BC_PING_START) {
    q_Retval = true;
  } else {
    q_Retval = false;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsConnectionSuccess(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsConnectionFailure(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;
...
/*! \brief   Check if current step update application start
   ...
   \param[in]   oe_Step       Current step
   \param[out]  orq_IsParam   Flag if current step was a parameter file step
   (only valid if return is true!)
   \param[out]  orq_IsPemFile Flag if current step was a PEM file step (only
   valid if return is true!)
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsUpdateAppStart(
    const C_OscSuSequences::E_ProgressStep oe_Step, bool &orq_IsParam,
    bool &orq_IsPemFile) {
  bool q_Retval;

  switch (oe_Step) // lint !e788 //we do not handle all steps here
  {
  case C_OscSuSequences::eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_START: // Application
                                                                  // start state
  case C_OscSuSequences::
      eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_START: // Application start state
  case C_OscSuSequences::eUPDATE_SYSTEM_XFL_NODE_FLASH_HEX_START: // Application
                                                                  // start state
    q_Retval = true;
    orq_IsParam = false;
    orq_IsPemFile = false;
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsUpdateAppSuccess(
    const C_OscSuSequences::E_ProgressStep oe_Step, bool &orq_IsParam,
    bool &orq_IsPemFile) {
  bool q_Retval;
  ...
  {
  case C_OscSuSequences::
      eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_FINISHED: // Finished application state
  case C_OscSuSequences::
      eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_FINISHED: // Finished application state
  case C_OscSuSequences::
      eUPDATE_SYSTEM_XFL_NODE_FLASH_HEX_FINISHED: // Finished application state
    orq_IsParam = false;
    q_Retval = true;
    orq_IsPemFile = false;
    break;
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsUpdateNodeStart(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;

  switch (oe_Step) // lint !e788 //we do not handle all steps here
  {
  case C_OscSuSequences::eUPDATE_SYSTEM_OSY_NODE_START: // Application start
                                                        // state
  case C_OscSuSequences::eUPDATE_SYSTEM_XFL_NODE_START: // Application start
                                                        // state
    q_Retval = true;
    break;
  default:
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsUpdateNodeSuccess(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;

  switch (oe_Step) // lint !e788 //we do not handle all steps here
  {
  case C_OscSuSequences::eUPDATE_SYSTEM_OSY_NODE_FINISHED: // Finished
                                                           // application state
  case C_OscSuSequences::eUPDATE_SYSTEM_XFL_NODE_FINISHED: // Finished
                                                           // application state
    q_Retval = true;
    break;
  default:
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsUpdateFailure(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;
  ...
  case C_OscSuSequences::
      eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_AVAILABLE_FEATURE_ERROR:
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpUpdateWidget::mh_IsUpdateAbort(
    const C_OscSuSequences::E_ProgressStep oe_Step) {
  bool q_Retval;
...
void C_SyvUpUpdateWidget::m_UpdateUpdatePackageStatus() {
  ...
  this->mpc_Ui->pc_WiUpdateInformation->InitUpdatePackage(
      this->mc_NodesToFlash, this->mc_NodesOrder, m_GetIsFileBasedFlagForEach(),
      true);
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Slot function of fix minimized toolbox widget for button maximizing
 * click
 */
//----------------------------------------------------------------------------------------------------------------------
...
void C_SyvUpUpdateWidget::InitIcon(void) const {
  this->mpc_Ui->pc_PbUpdate->InitCustomIcon(
      "://images//system_views/IconUpdate.svg",
      "://images//system_views/IconUpdateDisabled.svg");
  this->mpc_Ui->pc_PbDisconnnect->InitCustomIcon(
      ":/images/system_views/IconDisconnect.svg",
      "://images/system_views/IconDisconnectDisabled.svg");
  this->mpc_Ui->pc_PbConnect->InitCustomIcon(
      "://images/system_views/IconConnectWhite.svg",
      "://images/system_views/IconConnectGrey.svg");
  this->mpc_Ui->pc_PbCancel->InitCustomIcon(
      "://images/system_views/IconCancelBright.svg",
