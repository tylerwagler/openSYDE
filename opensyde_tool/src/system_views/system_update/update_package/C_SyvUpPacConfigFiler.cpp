...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacConfigFiler::h_LoadConfig(const QString &orc_FilePath,
                                            C_SyvUpPacConfig &orc_Config) {
  ...
  if (QFileInfo(orc_FilePath).exists() && QFileInfo(orc_FilePath).isFile()) {
    ...
    s32_Return = c_XmlParser.LoadFromFile(orc_FilePath);
      ...
      if (c_XmlParser.SelectRoot() == "opensyde-update-package-configuration") {
        if (c_XmlParser.SelectNodeChild("file-version") == "file-version") {
          ...
          try {
            ...
            osc_write_log_error(
                "Loading Update Package Configuration",
          ...
          if (s32_Return == C_NO_ERR) {
            osc_write_log_info("Loading Update Package Configuration",
          ...
          // Return
          Q_ASSERT(c_XmlParser.SelectNodeParent() ==
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_SyvUpPacConfigFiler::h_SaveConfig(const QString &orc_FilePath,
  ...
  if (QFileInfo(orc_FilePath).exists() && QFileInfo(orc_FilePath).isFile()) {
    ...
    // Main node
    q_Success = (c_XmlParser.CreateAndSelectNodeChild(
                     "opensyde-update-package-configuration") ==
                 "opensyde-update-package-configuration");

    if (q_Success == true) {
      // File Version
      q_Success = (c_XmlParser.CreateAndSelectNodeChild("file-version") ==
                   "file-version");

      if (q_Success == true) {
        c_XmlParser.SetNodeContent(QString::number(hu16_FILE_VERSION_LATEST));
        // Return
        Q_ASSERT(c_XmlParser.SelectNodeParent() ==
                 "opensyde-update-package-configuration");
      }
    ...
    if (q_Success == true) {
      ...
      if (q_Success == true) {
        ...
        for (u32_NodeCounter = 0U;
             u32_NodeCounter < orc_Config.c_NodeConfigs.size();
             ++u32_NodeCounter) {
          q_Success = mh_SaveNode(c_XmlParser,
                                  orc_Config.c_NodeConfigs[u32_NodeCounter]);
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_SaveNode(
    C_OscXmlParser &orc_XmlParser, const C_SyvUpPacConfigNode &orc_NodeConfig) {
  bool q_Success;
  ...
  if (q_Success == true) {
    ...
    orc_XmlParser.CreateNodeChild("name", orc_NodeConfig.c_Name);
    orc_XmlParser.CreateNodeChild("type", orc_NodeConfig.c_DeviceType);

    q_Success = (orc_XmlParser.CreateAndSelectNodeChild("applications") ==
                 "applications");

    if (q_Success == true) {
      // Save all application configurations
      for (u32_Counter = 0U; u32_Counter < orc_NodeConfig.c_AppConfigs.size();
           ++u32_Counter) {
        q_Success =
            mh_SaveApp(orc_XmlParser, orc_NodeConfig.c_AppConfigs[u32_Counter]);
    ...
    if (q_Success == true) {
      q_Success = (orc_XmlParser.CreateAndSelectNodeChild(
                       "parameter-set-images") == "parameter-set-images");
    }
    if (q_Success == true) {
      // Save all parameter set configurations
      for (u32_Counter = 0U;
           u32_Counter < orc_NodeConfig.c_ParamSetConfigs.size();
           ++u32_Counter) {
        q_Success = mh_SaveParamSet(
            orc_XmlParser, orc_NodeConfig.c_ParamSetConfigs[u32_Counter]);
    ...
    if (q_Success == true) {
      ...
      for (u32_Counter = 0U; u32_Counter < orc_NodeConfig.c_FileConfigs.size();
           ++u32_Counter) {
        q_Success = mh_SaveFile(orc_XmlParser,
                                orc_NodeConfig.c_FileConfigs[u32_Counter]);
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_SaveApp(
    C_OscXmlParser &orc_XmlParser,
    const C_SyvUpPacConfigNodeApp &orc_AppConfig) {
  bool q_Success;

  q_Success =
      (orc_XmlParser.CreateAndSelectNodeChild("application") == "application");

  if (q_Success == true) {
    orc_XmlParser.CreateNodeChild(
        "type",
        C_OscNodeApplication::h_ApplicationToString(orc_AppConfig.e_Type));
    orc_XmlParser.CreateNodeChild("name", orc_AppConfig.c_Name);
    orc_XmlParser.CreateNodeChild("path", orc_AppConfig.c_Path);
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_SaveParamSet(C_OscXmlParser &orc_XmlParser,
                                            const QString &orc_Path) {
  bool q_Success;

  q_Success = (orc_XmlParser.CreateAndSelectNodeChild("parameter-set-image") ==
               "parameter-set-image");

  if (q_Success == true) {
    orc_XmlParser.CreateNodeChild("path", orc_Path);
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_SaveFile(C_OscXmlParser &orc_XmlParser,
                                        const QString &orc_Path) {
  bool q_Success;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacConfigFiler::mh_SaveNodeUpdateInformationPem(
    const C_SyvUpPacConfigNode &orc_NodeConfig,
    C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateAndSelectNodeChild("pem-file");
  orc_XmlParser.CreateNodeChild("path", orc_NodeConfig.c_PemFilePath);
  orc_XmlParser.CreateAndSelectNodeChild("states");
  orc_XmlParser.CreateNodeChild("security",
                                C_OscViewFiler::h_PemFileStateSecurityToString(
                                    orc_NodeConfig.e_StateSecurity));
  orc_XmlParser.CreateNodeChild("debugger",
                                C_OscViewFiler::h_PemFileStateDebuggerToString(
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_LoadNodes(C_OscXmlParser &orc_XmlParser,
                                         C_SyvUpPacConfig &orc_Config) {
  bool q_Success = true;
    ...
    if (orc_XmlParser.SelectNodeChild("node") == "node") {
      do {
        ...
        if (orc_XmlParser.SelectNodeChild("name") == "name") {
          c_NodeConfig.c_Name = orc_XmlParser.GetNodeContent();
          // Return
          Q_ASSERT(orc_XmlParser.SelectNodeParent() == "node");
        } else {
          q_Success = false;
        ...
        if (orc_XmlParser.SelectNodeChild("type") == "type") {
          c_NodeConfig.c_DeviceType = orc_XmlParser.GetNodeContent();
          // Return
          Q_ASSERT(orc_XmlParser.SelectNodeParent() == "node");
        } else {
          q_Success = false;
        ...
        if (q_Success == true) {
          if (mh_LoadNodeUpdateInformationPem(c_NodeConfig, orc_XmlParser) !=
              C_NO_ERR) {
            q_Success = false;
          }
        }
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_LoadApps(C_OscXmlParser &orc_XmlParser,
      ...
      do {
        ...
        if (orc_XmlParser.SelectNodeChild("type") == "type") {
          C_OscNodeApplication::h_StringToApplication(
              orc_XmlParser.GetNodeContent(), c_AppConfig.e_Type);
        ...
        // Name of application
        if (orc_XmlParser.SelectNodeChild("name") == "name") {
          c_AppConfig.c_Name = orc_XmlParser.GetNodeContent();
          // Return
          Q_ASSERT(orc_XmlParser.SelectNodeParent() == "application");
        } else {
          q_Success = false;
        ...
        if (orc_XmlParser.SelectNodeChild("path") == "path") {
          c_AppConfig.c_Path = orc_XmlParser.GetNodeContent();
          // Return
          Q_ASSERT(orc_XmlParser.SelectNodeParent() == "application");
        } else {
          q_Success = false;
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_LoadParamSets(
    C_OscXmlParser &orc_XmlParser, C_SyvUpPacConfigNode &orc_NodeConfig) {
  bool q_Success = true;

  if (orc_XmlParser.SelectNodeChild("parameter-set-images") ==
      "parameter-set-images") {
    if (orc_XmlParser.SelectNodeChild("parameter-set-image") ==
        "parameter-set-image") {
      do {
        QString c_Path;
      ...
      } while (orc_XmlParser.SelectNodeNext("parameter-set-image") ==
               "parameter-set-image");
...
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacConfigFiler::mh_LoadFiles(C_OscXmlParser &orc_XmlParser,
                                         C_SyvUpPacConfigNode &orc_NodeConfig) {
  bool q_Success = true;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacConfigFiler::mh_LoadNodeUpdateInformationPem(
    C_SyvUpPacConfigNode &orc_NodeConfig, C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  if (orc_XmlParser.SelectNodeChild("pem-file") == "pem-file") {
    s32_Retval = orc_XmlParser.SelectNodeChildError("path");
    if (s32_Retval == C_NO_ERR) {
      orc_NodeConfig.c_PemFilePath = orc_XmlParser.GetNodeContent();
      // Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "pem-file");
      s32_Retval = C_SyvUpPacConfigFiler::mh_LoadNodeUpdateInformationPemStates(
          orc_NodeConfig, orc_XmlParser);
    }
    if (s32_Retval == C_NO_ERR) {
      // Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "node");
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacConfigFiler::mh_LoadNodeUpdateInformationPemStates(
    C_SyvUpPacConfigNode &orc_NodeConfig, C_OscXmlParserBase &orc_XmlParser) {
