...
class C_SyvUpPacNodeWidget : public QWidget {
  Q_OBJECT

public:
  explicit C_SyvUpPacNodeWidget(const uint32_t ou32_ViewIndex,
  ...
  void UpdateDeviceInformation(const stw::opensyde_gui_logic::C_SyvUpDeviceInfo
  ...
  void AdaptFile(const QString &orc_File,
  ...
  void PrepareExportConfig(
      stw::opensyde_gui_logic::C_SyvUpPacConfigNode &orc_NodeConfig) const;
  void LoadImportConfig(
      const stw::opensyde_gui_logic::C_SyvUpPacConfig &orc_Config) const;

  int32_t CheckAllFiles(uint32_t &oru32_CountFiles,
  ...
  GetAndSelectApplication(const QPoint &orc_Pos) const;
  C_SyvUpPacListNodeItemWidget *
  GetApplication(const QPoint &orc_Pos,
                 uint32_t *const opu32_Number = NULL) const;
  ...
  int32_t GetUpdatePackage(
      stw::opensyde_core::C_OscSuSequences::C_DoFlash &orc_ApplicationsToWrite,
      stw::opensyde_core::C_OscSuSequences::C_DoFlash *const
          opc_AllApplications);
...
private:
  ...
  void m_SetApplicationsUnselected(
      const C_SyvUpPacSectionNodeWidget *const opc_List) const;
  C_SyvUpPacSectionNodeWidget *
  m_GetAppParentList(C_SyvUpPacListNodeItemWidget *const opc_App) const;
  ...
  void m_OnCreatePackage(const QString &orc_PublicKeyPath,
