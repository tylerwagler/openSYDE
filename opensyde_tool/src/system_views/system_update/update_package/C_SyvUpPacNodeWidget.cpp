 ...
 * ---------------------------------------------------------------------------------------
 */
const int32_t C_SyvUpPacNodeWidget::mhs32_LAYOUT_THRESHOLD =
    100; // Keep synced with update
// summary widget
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacNodeWidget::C_SyvUpPacNodeWidget(const uint32_t ou32_ViewIndex,
                                           const uint32_t ou32_PositionNumber,
                                           const uint32_t ou32_NodeIndex,
                                           const QString &orc_NodeName,
                                           QWidget *const opc_Parent)
    : QWidget(opc_Parent), mpc_Ui(new Ui::C_SyvUpPacNodeWidget),
      mu32_ViewIndex(ou32_ViewIndex), mu32_NodeIndex(ou32_NodeIndex),
      mc_NodeName(orc_NodeName), mq_FileBased(false), mq_NvmHalcBased(false),
      mq_StwFlashloader(false), mq_Connected(false),
      mq_EmptyOptionalSectionsVisible(true), mq_ShowAddSecurityPack(false),
      mu32_PositionNumber(ou32_PositionNumber), mu32_FilesUpdated(0U),
      mpc_FilesWidget(NULL), mpc_FilesWidgetSeparator(NULL) {
  this->mpc_Ui->setupUi(this);

  stw::opensyde_gui_logic::C_OgeWiUtil::h_ApplyStylesheetProperty(
      this->mpc_Ui->pc_FrameSepTop, "HasColor9Background", true);
  ...
  this->mpc_Ui->pc_PbAddSecurityPackage->SetCustomIcons(
  ...
  this->mpc_Ui->pc_LabIconWarning->SetSvg(
      "://images/system_views/IconWarning.svg");
  ...
  this->mpc_Ui->pc_LabIconLock->SetSvg(
      "://images/system_views/IconLockedGrey.svg");
  ...
  if (pc_View != NULL) {
    ...
    QVector<bool> c_Flags = C_UsHandler::h_GetInstance()
                                ->GetProjSvSetupView(pc_View->GetName())
                                .GetSvNode(orc_NodeName)
                                .GetSectionsExpanded();
    ...
    if (c_Flags.size() == s32_ExpectedSections) {
      ...
      for (s32_DatablockCounter = 0;
           s32_DatablockCounter < this->mc_DatablockWidgets.size();
           ++s32_DatablockCounter) {
        this->mc_DatablockWidgets[s32_DatablockCounter]->Expand(
            c_Flags[s32_DatablockCounter]);
      }
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
// lint -e{1540}  no memory leak because of the parent of mpc_FilesWidget &
// mpc_FilesWidgetSeparator and the Qt memory
// management
C_SyvUpPacNodeWidget::~C_SyvUpPacNodeWidget() {
  ...
  if (pc_View != NULL) {
    ...
    for (s32_DatablockCounter = 0;
         ...
         ++s32_DatablockCounter) {
      c_Flags[s32_DatablockCounter] =
          this->mc_DatablockWidgets[s32_DatablockCounter]->IsExpanded();
    }
    ...
    C_UsHandler::h_GetInstance()->SetProjSvUpdateSectionsExpandedFlags(
        pc_View->GetName(), this->mc_NodeName, c_Flags);
  }
...
void C_SyvUpPacNodeWidget::SetUpdateStarted(void) const {
  ...
  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    // lint -e{514}  Using operator with a bool value was intended and is no
    // accident
    q_ApplicationUpdateNecessary |=
        this->mc_DatablockWidgets[s32_DatablockCounter]->IsUpdateNecessary();
  }
...
void C_SyvUpPacNodeWidget::SetUpdateApplicationStarted(void) const {
  C_SyvUpPacSectionNodeWidget *const pc_List =
      this->m_GetNextListInUpdateOrder();
...
void C_SyvUpPacNodeWidget::SetUpdateApplicationFinished(void) {
  ...
  if (u32_FileCount > 0) {
    this->mpc_Ui->pc_WidgetTitle->SetProgress((this->mu32_FilesUpdated * 100U) /
                                              u32_FileCount);
  } else {
    this->mpc_Ui->pc_WidgetTitle->SetProgress(100U);
  }
...
void C_SyvUpPacNodeWidget::SetDisconnected(void) {
  ...
  this->mpc_Ui->pc_LabIconLock->setVisible(false);
  this->mpc_Ui->pc_PbAddSecurityPackage->setVisible(
      this->mq_ShowAddSecurityPack);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::UpdateDeviceInformation(
    const C_SyvUpDeviceInfo &orc_DeviceInformation) const {
  int32_t s32_DatablockCounter;

  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    this->mc_DatablockWidgets[s32_DatablockCounter]->UpdateDeviceInformation(
...
void C_SyvUpPacNodeWidget::SetProgress(const uint32_t ou32_Percentage) const {
  ...
  if (u32_FileCount > 0U) {
    const uint32_t u32_FinishedNodesPercentage =
        (this->mu32_FilesUpdated * 100U) / u32_FileCount;
    this->mpc_Ui->pc_WidgetTitle->SetProgress(
        u32_FinishedNodesPercentage + (ou32_Percentage / u32_FileCount));
  } else {
    this->mpc_Ui->pc_WidgetTitle->SetProgress(0U);
  }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::UpdatePositionNumber(
    const uint32_t ou32_PositionNumber) {
  int32_t s32_DatablockCounter;
  ...
  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    this->mc_DatablockWidgets[s32_DatablockCounter]->UpdatePositionNumber(
        this->mu32_PositionNumber);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Adds a new file for the file based file list, a parameter set file
   or a PEM file
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::AddNewFile(const QString &orc_File,
                                      const bool oq_Paramset,
                                      const bool oq_PemFile) {
  // compare all data block and file based files
  if ((this->mpc_FilesWidget != NULL) && (this->mq_StwFlashloader == false) &&
  ...
  // special case security certificate package: warn user if there is more than
  // one
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::AdaptFile(
    const QString &orc_File, C_SyvUpPacListNodeItemWidget *const opc_App) {
  C_SyvUpPacSectionNodeWidget *const pc_Parent = m_GetAppParentList(opc_App);

  if (pc_Parent != NULL) {
    if (this->mq_FileBased == true) {
      // for file based nodes it is not allowed two add a file twice,
      // independent of section where it gets added
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Revert the current application to the system definition default
   path
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::RevertFile(
    C_SyvUpPacListNodeItemWidget *const opc_App) const {
  C_SyvUpPacSectionNodeWidget *const pc_Parent = m_GetAppParentList(opc_App);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::RemoveFile(
    C_SyvUpPacListNodeItemWidget *const opc_App) const {
  C_SyvUpPacSectionNodeWidget *const pc_Parent = m_GetAppParentList(opc_App);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::PrepareExportConfig(
    C_SyvUpPacConfigNode &orc_NodeConfig) const {
  int32_t s32_DatablockCounter;
  ...
  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    this->mc_DatablockWidgets[s32_DatablockCounter]->PrepareExportConfig(
        orc_NodeConfig);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Loads all matching configurations for this node of the import
   configuration

   \param[out]  orc_Config    Import configuration
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::LoadImportConfig(
    const C_SyvUpPacConfig &orc_Config) const {
  int32_t s32_DatablockCounter;

  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    this->mc_DatablockWidgets[s32_DatablockCounter]->LoadImportConfig(
        orc_Config);
  }
...
/*! \brief   Checks all paths for existence
   ...
   \param[out]  opc_FlashwareWarningsApps    App names of apps with flashware
   warnings
   \param[out]  opc_MissingApps              App names of apps with missing data
   block files
   \param[out]  opc_MissingParamFiles        App names of apps with missing
   parameter set image files
   \param[out]  opc_MissingFiles             App names of apps with missing
   files
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacNodeWidget::CheckAllFiles(
    uint32_t &oru32_CountFiles, QStringList *const opc_FlashwareWarningsApps,
  ...
  for (s32_DatablockCounter = 0U;
       ...
       ++s32_DatablockCounter) {
    s32_Return +=
        this->mc_DatablockWidgets[s32_DatablockCounter]->CheckAllFiles(
            oru32_CountFiles, opc_MissingApps, opc_MissingParamFiles,
            opc_FlashwareWarningsApps);
  }

  if (this->mpc_FilesWidget != NULL) {
    s32_Return += this->mpc_FilesWidget->CheckAllFiles(
        oru32_CountFiles, opc_MissingFiles, opc_MissingParamFiles,
        opc_FlashwareWarningsApps);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSectionNodeWidget *
C_SyvUpPacNodeWidget::GetSectionList(const QPoint &orc_Pos) const {
  C_SyvUpPacSectionNodeWidget *pc_Return = NULL;
  const QPoint c_AdaptedPosScrollArea =
      this->mpc_Ui->pc_ScrollAreaWidget->mapFrom(this->parentWidget(), orc_Pos);
  ...
  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    C_SyvUpPacSectionNodeDatablockWidget *const pc_DbWidget =
        this->mc_DatablockWidgets[s32_DatablockCounter];
    const QPoint c_AdaptedPosList = pc_DbWidget->mapFrom(
        this->mpc_Ui->pc_ScrollAreaWidget, c_AdaptedPosScrollArea);
  ...
  if ((pc_Return == NULL) && (this->mpc_FilesWidget != NULL)) {
    const QPoint c_AdaptedPosList = this->mpc_FilesWidget->mapFrom(
        this->mpc_Ui->pc_ScrollAreaWidget, c_AdaptedPosScrollArea);
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacListNodeItemWidget *
C_SyvUpPacNodeWidget::GetAndSelectApplication(const QPoint &orc_Pos) const {
  C_SyvUpPacListNodeItemWidget *pc_App = NULL;
  const QPoint c_AdaptedPos =
      this->mpc_Ui->pc_ScrollAreaWidget->mapFrom(this->parentWidget(), orc_Pos);
  ...
  for (s32_DatablockCounter = 0U;
       s32_DatablockCounter < this->mc_DatablockWidgets.size();
       ++s32_DatablockCounter) {
    const C_SyvUpPacSectionNodeDatablockWidget *const pc_DbWidget =
        this->mc_DatablockWidgets[s32_DatablockCounter];
  ...
  if ((pc_App == NULL) && (this->mpc_FilesWidget != NULL)) {
...
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacListNodeItemWidget *
C_SyvUpPacNodeWidget::GetApplication(const QPoint &orc_Pos,
                                     uint32_t *const opu32_Number) const {
  C_SyvUpPacListNodeItemWidget *pc_App = NULL;
  const QPoint c_AdaptedPos =
      this->mpc_Ui->pc_ScrollAreaWidget->mapFrom(this->parentWidget(), orc_Pos);
  ...
  if (pc_App == NULL) {
    if (this->mpc_FilesWidget != NULL) {
      pc_App =
          this->mpc_FilesWidget->GetApplication(c_AdaptedPos, opu32_Number);
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Creates and returns the update package with all information for the
   system update for the concrete node

   \param[out]  orc_ApplicationsToWrite   Vector with node update configuration
   \param[out]  opc_AllApplications       Optional vector with all node
   applications
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacNodeWidget::GetUpdatePackage(
  ...
  for (s32_DatablockCounter = 0U;
    ...
    const C_SyvUpPacSectionNodeDatablockWidget *const pc_DbWidget =
        this->mc_DatablockWidgets[s32_DatablockCounter];
    s32_Return = pc_DbWidget->GetUpdatePackage(
        orc_ApplicationsToWrite, opc_AllApplications, this->mu32_FilesUpdated);
    if (s32_Return == C_NO_ERR) {
      q_NoErr = true;
  ...
  if (((s32_Return == C_NO_ERR) || (s32_Return == C_NOACT)) &&
    ...
    s32_Return = this->mpc_FilesWidget->GetUpdatePackage(
        orc_ApplicationsToWrite, opc_AllApplications, this->mu32_FilesUpdated);
  }

  if ((q_NoErr == true) && (s32_Return == C_NOACT)) {
    // C_NOACT is no error if at least one GetUpdatePackage call returned with
    // C_NO_ERR
    s32_Return = C_NO_ERR;
  }
...
void C_SyvUpPacNodeWidget::CollapseAll(void) const {
  ...
  this->mpc_Ui->pc_ScrollArea->setVerticalScrollBarPolicy(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::SetEmptyOptionalSectionsVisible(
    const bool oq_Visible) {
  // remember
...
/*! \brief Update visibility of sections
   ...
   Show or hide sections "Parameter Set Images" and "Files" if there are no
   files in this sections. Do not touch sections that contain files.
*/
//----------------------------------------------------------------------------------------------------------------------
  ...
  if (this->mpc_FilesWidget != NULL) // never visible for non-file-based node
  {
    if (this->mpc_FilesWidget->GetFileCount() == 0) {
      ...
      if (this->mpc_FilesWidgetSeparator != NULL) {
        // Hide the separator. Could be NULL when mpc_FilesWidget is not NULL if
        // no the node has no datablocks
        this->mpc_FilesWidgetSeparator->setVisible(
...
void C_SyvUpPacNodeWidget::AddSecurityCertificatePackage() {
  const QPointer<C_OgePopUpDialog> c_PopUpDialog =
      new C_OgePopUpDialog(this, this);
  C_SyvUpPacSecurityCertificatePackageDialog *const
      pc_SecurityCertificatePackageDialog =
  ...
  if (c_PopUpDialog->exec() == static_cast<int32_t>(QDialog::Accepted)) {
    const bool q_OptionAddPemFiles =
        pc_SecurityCertificatePackageDialog->GetOptionAddPemFiles();
    const bool q_OptionAddSecureAuthentification =
        pc_SecurityCertificatePackageDialog
            ->GetOptionAddSecureAuthentification();
    const QString c_Password =
        pc_SecurityCertificatePackageDialog->GetPassword();
    const QString c_PublicKeyPath =
        pc_SecurityCertificatePackageDialog->GetPublicKeyPath();
    const QStringList c_PemFiles =
        pc_SecurityCertificatePackageDialog->GetPemFiles();
    this->m_OnCreatePackage(c_PublicKeyPath, c_Password, c_PemFiles,
                            q_OptionAddPemFiles,
...
} // lint !e429  //no memory leak because of the parent of c_PopUpDialog and the
...
void C_SyvUpPacNodeWidget::dragEnterEvent(QDragEnterEvent *const opc_Event) {
  if (this->mq_Connected == false) {
    ...
    // Accept always to get the dragLeaveEvent to reset the size of the lock
    // icon
    opc_Event->accept();
...
void C_SyvUpPacNodeWidget::dragMoveEvent(QDragMoveEvent *const opc_Event) {
  if (this->mq_Connected == false) {
    ...
    if (pc_MimeData->hasUrls() == true) {
      const QPoint c_AdaptedPos =
          this->mpc_Ui->pc_ScrollAreaWidget->mapFrom(this, opc_Event->pos());
...
void C_SyvUpPacNodeWidget::dropEvent(QDropEvent *const opc_Event) {
  if (this->mq_Connected == false) {
    ...
    if (this->m_CheckMime(pc_MimeData, c_AdaptedPos, &c_FilePathsDatablocks,
                          &c_FilePathsParamsetFiles, &c_FilePathsFileBased,
                          &c_FilePathsPemFiles, &pc_App) == true) {
      const uint8_t u8_TIMER_THRESHOLD_IN_MSEC = 100;

      QTimer::singleShot(
          u8_TIMER_THRESHOLD_IN_MSEC, this,
          [this, c_FilePathsDatablocks, pc_App, c_FilePathsParamsetFiles,
           c_FilePathsFileBased, c_FilePathsPemFiles, opc_Event]() {
            ...
            if ((pc_App != NULL) && (c_FilePathsDatablocks.size() == 1)) {
              // Datablock only

              // Check if relative paths are possible and appreciated
              c_TimerFilePathsDatablocks[0] =
                  C_ImpUtil::h_AskUserToSaveRelativePath(
                      this, c_TimerFilePathsDatablocks[0], c_Folder);
              ...
              QStringList c_TimerFilePathsParamsetFiles =
              ...
              // Check if relative paths are possible and appreciated (for all
              // files ask once)
              ...
              c_PathsParamAndPemAndFile =
                  C_ImpUtil::h_AskUserToSaveRelativePath(
                      this, c_PathsParamAndPemAndFile, c_Folder);

              if (c_PathsParamAndPemAndFile.size() > 0) {
                // Re-split paths
                Q_ASSERT(c_PathsParamAndPemAndFile.size() ==
                         (c_TimerFilePathsParamsetFiles.size() +
                          c_TimerFilePathsFileBased.size() +
                          c_TimerFilePathsPemFiles.size()));

                c_TimerFilePathsParamsetFiles = c_PathsParamAndPemAndFile.mid(
                    0, c_TimerFilePathsParamsetFiles.size());
                ...
                if ((pc_TimerApp != NULL) &&
                  ...
                  this->AdaptFile(c_TimerFilePathsParamsetFiles[0],
                                  pc_TimerApp);
                } else if ((pc_TimerApp != NULL) &&
                           (c_TimerFilePathsFileBased.size() == 0) &&
                           (c_TimerFilePathsParamsetFiles.size() == 0) &&
                  ...
                  while (s32_TimerFileCounter <
                         c_TimerFilePathsFileBased.size()) {
                    // Add new file. If list does not support adding new files,
                    // nothing will happen
                    this->AddNewFile(
                        c_TimerFilePathsFileBased[s32_TimerFileCounter], false,
                        false);
                  ...
                  while (s32_TimerFileCounter <
                    ...
                    // nothing will happen
                    this->AddNewFile(
                        c_TimerFilePathsParamsetFiles[s32_TimerFileCounter],
                        true, false);
                    s32_TimerFileCounter = s32_TimerFileCounter + 1;
                  }
                  ...
                  while (s32_TimerFileCounter <
                    ...
                    // nothing will happen
                    this->AddNewFile(
                        c_TimerFilePathsPemFiles[s32_TimerFileCounter], false,
                        true);
                    s32_TimerFileCounter = s32_TimerFileCounter + 1;
                  }
                }
...
void C_SyvUpPacNodeWidget::m_Init(void) {
  ...
  QSpacerItem *const pc_Spacer =
      new QSpacerItem(0, 3, QSizePolicy::Minimum, QSizePolicy::Expanding);
  ...
  if (pc_Node != NULL) {
    ...
    Q_ASSERT(pc_Node->u32_SubDeviceIndex <
    ...
    this->mq_StwFlashloader =
        (pc_Node->c_Properties.e_FlashLoader == C_OscNodeProperties::eFL_STW);
    this->mc_DeviceType = pc_Node->c_DeviceType;

    // Only relevant if the node has datablocks
    for (u32_DatablockCounter = 0U;
         u32_DatablockCounter < pc_Node->c_Applications.size();
         ++u32_DatablockCounter) {
      C_SyvUpPacSectionNodeDatablockWidget *const pc_Datablock =
          new C_SyvUpPacSectionNodeDatablockWidget(this);
      this->mpc_Ui->pc_ScrollAreaLayout->addWidget(pc_Datablock);
      this->mc_DatablockWidgets.push_back(pc_Datablock);

      pc_Datablock->InitWidget(
          this->mu32_ViewIndex, this->mu32_PositionNumber, this->mu32_NodeIndex,
          this->mc_NodeName, u32_DatablockCounter, u32_ViewDataBlockPathNumber);

      // Special case, parameter set image files in a datablock. Necessary to
      // know the reserved saved paths of the parameter set image file for the
      // files widget
      ...
      if ((static_cast<uint32_t>(u32_DatablockCounter) <
           (pc_Node->c_Applications.size() - 1U)) ||
          ((this->mq_StwFlashloader == false) &&
           (this->mq_NvmHalcBased == false))) {
        this->m_AddSeparatorToScrollArea(
            static_cast<uint32_t>(u32_DatablockCounter) ==
            (pc_Node->c_Applications.size() - 1U));
      }
    } // lint !e429  //no memory leak because of the parent of pc_Datablock and
      // the Qt memory management

    // Check if an "Other files" section is necessary
    // It is only necessary for file based devices and can get psi files without
    // restrictions
    if (this->mq_StwFlashloader == false) {
      ...
      this->mpc_FilesWidget->SetCountSkippedParamSetFiles(
          u32_DatablockParamSetFiles);
      // last parameter is "0", since this case has no Data Block output files
      this->mpc_FilesWidget->InitWidget(
          this->mu32_ViewIndex, this->mu32_PositionNumber, this->mu32_NodeIndex,
          this->mc_NodeName, pc_Node->c_Applications.size(), 0);
    }

    // Show "Add security package"-Button only if file based and X.App support
    // is enabled:
    this->mq_ShowAddSecurityPack =
        pc_Node->c_Properties.q_XappSupport && this->mq_FileBased;
    this->mpc_Ui->pc_PbAddSecurityPackage->setVisible(
        this->mq_ShowAddSecurityPack);
  }

  this->mpc_Ui->pc_ScrollAreaLayout->addSpacerItem(pc_Spacer);
  this->mpc_Ui->pc_ScrollAreaLayout->setStretch(
      this->mpc_Ui->pc_ScrollAreaLayout->count() - 1, 1);
} // lint !e429  //no memory leak because of the parent of pc_Spacer and the Qt
  // memory management
...
void C_SyvUpPacNodeWidget::m_AddSeparatorToScrollArea(
  ...
  c_SizePolicy.setHeightForWidth(
      pc_Separator->sizePolicy().hasHeightForWidth());

  stw::opensyde_gui_logic::C_OgeWiUtil::h_ApplyStylesheetProperty(
      pc_Separator, "HasColor9Background", true);
...
void C_SyvUpPacNodeWidget::m_UpdateTitle(void) const {
  this->mpc_Ui->pc_Title->setText(
      static_cast<QString>("#%1 - %2")
          .arg(QString::number(this->mu32_PositionNumber + 1U),
               this->mc_NodeName));
}
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Returns the current relevant list for the current
  updating/transferring file/application

  This function use the value mu32_FilesUpdated as reference. The current file
  shall not be calculated into mu32_FilesUpdated before the call of this
  function.

   \retval   Valid pointer    Pointer to the list, which has the current updated
  file/application
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSectionNodeWidget *
C_SyvUpPacNodeWidget::m_GetNextListInUpdateOrder(void) const {
  ...
  for (s32_DatablockCounter = 0;
    ...
    if (pc_List->GetPrimaryFileCount() > 0U) {
      // No more files updated than current datablock. Do not check next
      // section.
      if (pc_List->GetSectionState() !=
          C_SyvUpPacListNodeItemWidget::hu32_STATE_FINISHED) {
        q_ListFound = true;
        break;
      }
    }
  ...
  if (q_ListFound == false) {
    if (this->mpc_FilesWidget != NULL) {
      // Do not check for finished state here. If no list was already found, a
      // state configuration error for a PEM file was probably detected, so the
      // error belongs to the "other files", but the finished state does not
      // match
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::m_SetApplicationsUnselected(
    const C_SyvUpPacSectionNodeWidget *const opc_List) const {
  uint32_t u32_AppCounter;

  for (u32_AppCounter = 0U; u32_AppCounter < opc_List->GetFileCount();
       ++u32_AppCounter) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSectionNodeWidget *C_SyvUpPacNodeWidget::m_GetAppParentList(
    C_SyvUpPacListNodeItemWidget *const opc_App) const {
  C_SyvUpPacSectionNodeWidget *pc_Parent = NULL;

  if (opc_App != NULL) {
    pc_Parent =
        dynamic_cast<C_SyvUpPacSectionNodeWidget *>(opc_App->GetListParent());
...
/*! \brief  Compare all files in sections data blocks and files with input file.
   ...
   If file is already contained in update package, an information message is
   shown.
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacNodeWidget::m_CheckFileAlreadyContained(
  ...
  if ((pc_Node != NULL) && (pc_View != NULL)) {
    ...
    if (q_Retval == false) {
      ...
      for (std::vector<QString>::const_iterator c_It = c_Paths.begin();
        ...
        if ((c_AbsoluteFile ==
             C_PuiUtil::h_GetAbsolutePathFromProject(*c_It)) ||
            (static_cast<QFileInfo>(c_AbsoluteFile).fileName() ==
             static_cast<QFileInfo>(
                 C_PuiUtil::h_GetAbsolutePathFromProject(*c_It))
                 .fileName())) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check for multiple security certificate packages and warn the user.
   No further action.

   \param[in]  orc_File    File path of new file
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::m_CheckForMultipleSecurityCertificatePackages(
    const QString &orc_File) {
  if (("." + static_cast<QFileInfo>(orc_File).suffix().toLower()) ==
    ...
    if (u32_NumberOfSecurityPacks > 1) {
      ...
      c_Message.SetDescription(
          static_cast<QString>(
              "There are multiple security certificate packages contained "
              "in the update package of node %1.")
              .arg(pc_Node->c_Properties.c_Name));
      c_Message.SetDetails(
          static_cast<QString>("The node can probably only handle one security "
                               "certificate package "
                               "and thus the rest will be ignored. You can "
                               "remove unused packages.\n\n"
                               "The following security certificate packages "
                               "are contained in the update package:\n"
                               "%1")
              .arg(c_FoundPaths));
...
/*! \brief   Check if mime valid
   ...
   \param[out]  opc_FilePathsDatablocks   File paths which could be used for
   datablock list
   \param[out]  opc_FilePathsParamsets    File paths which could be used for
   parameter set image list
   \param[out]  opc_FilePathsFileBased    File paths which could be used for
   file based list
   \param[out]  opc_RelevantPemFilePaths  PEM file paths which could be used for
   file based list
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacNodeWidget::m_CheckMime(
    const QMimeData *const opc_Mime, const QPoint &orc_Pos,
  ...
  if ((opc_Mime != NULL) && (opc_Mime->hasUrls() == true)) {
    ...
    // Check all paths
    for (s32_FileCounter = 0U; s32_FileCounter < c_PathList.size();
         ++s32_FileCounter) {
    ...
    if (q_FilesValid == true) {
      ...
      for (s32_DatablockCounter = 0;
           s32_DatablockCounter < this->mc_DatablockWidgets.size();
           ++s32_DatablockCounter) {
        q_Retval = this->mc_DatablockWidgets[s32_DatablockCounter]->CheckMime(
            c_PathList, orc_Pos, opc_FilePathsDatablocks,
            opc_FilePathsParamsets, NULL, oppc_App);
      ...
      if ((q_Retval == false) && (this->mpc_FilesWidget != NULL)) {
        // Check if no file/parameter set image operation is possible or the
        // caller wants to know all possibilities In this case both checks are
        // necessary
        q_Retval = this->mpc_FilesWidget->CheckMime(
            c_PathList, orc_Pos, opc_FilePathsFileBased, opc_FilePathsParamsets,
            opc_RelevantPemFilePaths, oppc_App);
      }
    }
  }
...
/*! \brief   Creates the security certificate package
   ...
   \param[in]       oq_OptionAddSecureAuthentification   Option add secure
   authentification
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacNodeWidget::m_OnCreatePackage(
    const QString &orc_PublicKeyPath, const QString &orc_Password,
    const QStringList &orc_CertificatesPath, const bool oq_OptionAddPemFiles,
  ...
  c_FilterName += C_OscXceBase::h_GetPackageExtension();
  c_FilterName += ")";
  c_DefaultFilename += "_SecurityCertificatePackage";
  c_DefaultFilename +=
      static_cast<QString>(C_OscXceBase::h_GetPackageExtension());

  c_Folder =
      C_UsHandler::h_GetInstance()->GetLastKnownSecureCertificatePackagePath();

  // use project path if there is no existing last known path
  if (c_Folder.isEmpty() ||
      (static_cast<QFileInfo>(c_Folder).dir().exists() == false)) {
    c_Folder = C_PuiProject::h_GetInstance()->GetFolderPath();
  }

  c_FullPackagePath = C_OgeWiUtil::h_GetSaveFileName(
      this, "Select Directory for Secure Certificate Package", c_Folder,
      c_FilterName, c_DefaultFilename, QFileDialog::Options(),
  ...
  if (!c_FullPackagePath.isEmpty()) {
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      c_MessageResult.SetDescription(
          "Security certificate package successfully created. "
          "Do you want to add it to the update package now?");
      c_MessageResult.SetDetails("Package saved at " +
                                 C_Uti::h_GetLink(c_FullPackagePath,
                                                  mc_STYLE_GUIDE_COLOR_LINK,
                                                  c_FullPackagePath));
      ...
      if (c_MessageResult.Execute() == C_OgeWiCustomMessage::eOK) {
        const QString c_AddPath = C_ImpUtil::h_AskUserToSaveRelativePath(
            this, c_FullPackagePath,
            C_PuiProject::h_GetInstance()->GetFolderPath());
      ...
      c_MessageResult.SetHeading("Create Security Certificate Package");
      c_MessageResult.SetDescription(
          "Could not create security certificate package!");
