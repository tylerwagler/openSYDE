...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSecurityCertificatePackageDialog::
    C_SyvUpPacSecurityCertificatePackageDialog(
  ...
  this->mpc_Ui->pc_PushButtonAdd->SetCustomIcons(
      "://images/IconAddEnabled.svg", "", "", "://images/IconAddDisabled.svg");
  this->mpc_Ui->pc_PushButtonClearall->SetCustomIcons(
      "://images/system_views/IconClearAllEnabled.svg", "", "",
  ...
  connect(
      this->mpc_Ui->pc_PushButtonClearall, &QPushButton::clicked, this,
      &C_SyvUpPacSecurityCertificatePackageDialog::m_HandleClearAllPemFiles);
  connect(this->mpc_Ui->pc_PEMFilesWidget,
          &C_SyvUpPacPemFilesWidget::SigUpdateFileCounter, this,
          &C_SyvUpPacSecurityCertificatePackageDialog::
              m_UpdateLabelTitleWithFileCounter);
...
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSecurityCertificatePackageDialog::
    ~C_SyvUpPacSecurityCertificatePackageDialog(void) {
  delete this->mpc_Ui;
}
...
/*! \brief  Get pem files
   ...
   \return
    QStringList   returns list of Pem Files
*/
//----------------------------------------------------------------------------------------------------------------------
QStringList C_SyvUpPacSecurityCertificatePackageDialog::GetPemFiles() {
  return this->mpc_Ui->pc_PEMFilesWidget->GetPemFilePaths();
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacSecurityCertificatePackageDialog::
    GetOptionAddSecureAuthentification() const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSecurityCertificatePackageDialog::keyPressEvent(
    QKeyEvent *const opc_KeyEvent) {
  bool q_CallOrg = true;
...
void C_SyvUpPacSecurityCertificatePackageDialog::m_InitStaticNames(void) const {
  ...
  this->mpc_Ui->pc_ChxBoxAddPEMFiles->setText(
      "Add new authentication PEM files");
  this->mpc_Ui->pc_ChxBoxAddPEMFiles->SetToolTipInformation(
      "Add new authentication PEM files",
  ...
  this->mpc_Ui->pc_LabelWorkflowInfo->setText(
      "When the Security Certificate Package is installed on X.App Node, "
      "following steps are done:\n"
      "- delete existing configuration for the security features \"Secure "
      "Authentication\" (private PEM files) and \"Secure Update\" (public PEM "
      "files).\n"
      "- Secure Authentication: if \"Add new authentication PEM files\" is "
      "enabled, new authentication PEM files are added.\n"
      "- Secure Update: if \"Apply new Secure Update configuration\" is "
      "enabled, new Secure Update config is added.");
  if (this->mpc_Ui->pc_PEMFilesWidget != NULL) {
    this->mpc_Ui->pc_LabelNewAuthenticationPEMFile->setText(
        static_cast<QString>("New authentication PEM files (%1)")
            .arg(QString::number(
                this->mpc_Ui->pc_PEMFilesWidget->GetFilesCount())));
  }

  // Secure Update
  this->mpc_Ui->pc_LabelSecureUpdate->setText("Secure Update");
  this->mpc_Ui->pc_ChxBoxApplyUpdateConfiguration->setText(
      "Apply new Secure Update configuration");
  this->mpc_Ui->pc_ChxBoxApplyUpdateConfiguration->SetToolTipInformation(
      "Apply new Secure Update configuration",
      "Secure Update: if "
      "\"Apply new Secure Update configuration\" "
  ...
  // Tooltips
  this->mpc_Ui->pc_PushButtonClearall->SetToolTipInformation(
      "Clear", "Remove all PEM files.");
  this->mpc_Ui->pc_PushButtonAdd->SetToolTipInformation("Add",
                                                        "Add new PEM file.");
  this->mpc_Ui->pc_LabelPublicPEMFile->SetToolTipInformation(
      "Signature Public Key", "Full path to public key file (*.pem).");
  this->mpc_Ui->pc_LabelPassword->SetToolTipInformation(
      "Encryption Password (optional)",
      "Flashware will be encrypted if password is given.");
  this->mpc_Ui->pc_PubPublicKeyPath->SetToolTipInformation(
      "Browse", "Browse for path to public key file (PEM file).");
  this->mpc_Ui->pc_PubTogglePassword->SetToolTipInformation(
      "Show/Hide Password", "Shows/hides password when pressed down.");
...
void C_SyvUpPacSecurityCertificatePackageDialog::m_OkClicked(void) {
  ...
  C_UsHandler::h_GetInstance()->SetLastKnownPublicPemFilePath(
      this->mpc_Ui->pc_LineEditPublicPEMFile->text());
  C_UsHandler::h_GetInstance()->SetLastKnownPemFilePassword(
      this->mpc_Ui->pc_LineEditPassword->text());
  C_UsHandler::h_GetInstance()->SetLastKnownAddPemFileState(
      this->mpc_Ui->pc_ChxBoxAddPEMFiles->isChecked());
...
void C_SyvUpPacSecurityCertificatePackageDialog::m_ReloadLastSavedData() {
  this->mpc_Ui->pc_ChxBoxAddPEMFiles->setChecked(
      C_UsHandler::h_GetInstance()->GetLastKnownAddPemFileState());
  this->mpc_Ui->pc_ChxBoxApplyUpdateConfiguration->setChecked(
      C_UsHandler::h_GetInstance()->GetLastKnownSecureUpdateConfigState());
  this->mpc_Ui->pc_LineEditPublicPEMFile->SetPath(
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSecurityCertificatePackageDialog::
    m_UpdateLabelTitleWithFileCounter() {
  if (this->mpc_Ui->pc_PEMFilesWidget != NULL) {
    const int32_t s32_FileCount = mpc_Ui->pc_PEMFilesWidget->GetFilesCount();
    this->mpc_Ui->pc_LabelNewAuthenticationPEMFile->setText(
        static_cast<QString>("New authentication PEM files (%1)")
            .arg(QString::number(s32_FileCount)));
...
void C_SyvUpPacSecurityCertificatePackageDialog::m_ReloadLastSavedPassword() {
  const QString c_Password =
      C_UsHandler::h_GetInstance()->GetLastKnownPemFilePassword();
  ...
  C_OgeWiUtil::h_ApplyStylesheetProperty(this->mpc_Ui->pc_LineEditPassword,
                                         "NoRightBorder", true);
  this->mpc_Ui->pc_PubTogglePassword->setIcon(
      QIcon("://images/main_page_and_navi_bar/Icon_password_show.svg"));
...
void C_SyvUpPacSecurityCertificatePackageDialog::m_PublicKeyPathClicked() {
  ...
  const QString c_FilterName =
      static_cast<QString>("PEM file (*.pem);;Others (*.*)");

  // Do not use QFileDialog::getOpenFileName because it does not support default
  // suffix
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_SyvUpPacSecurityCertificatePackageDialog::m_OnValidateSecureUpdateConfig() {
  ...
  if (s32_Result != C_NO_ERR) {
    ...
    switch (s32_Result) {
    case C_NOACT:
      c_ErrorPath.SetDescription("You need to enter a public PEM file to apply "
                                 "new Secure Update configuration.");
      break;
    case C_RANGE:
      c_ErrorPath.SetDescription(
          "Invalid file format of public PEM file for Secure Update. The "
          "format (*.pem) is required.");
      break;
    case C_RD_WR:
      c_ErrorPath.SetDescription(
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Validating Secure authentication configuration after create button
   click
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacSecurityCertificatePackageDialog::
    m_OnValidateSecureAuthenticationConfig() {
  ...
  if (s32_Result != C_NO_ERR) {
    ...
    case C_RD_WR:
      ...
      break;
    case C_CONFIG:
      c_ErrorPath.SetDescription("At least one file for Secure Authentication "
                                 "has invalid key configuration.");
      c_ErrorPath.SetDetails(
          c_PathDetails.replace("\n", "<br>") + "<br>" +
          static_cast<QString>("For details see ") +
          C_Uti::h_GetLink(
              "log file.", mc_STYLESHEET_GUIDE_COLOR_LINK,
              C_OscLoggingHandler::h_GetCompleteLogFileLocation()));
...
int32_t C_SyvUpPacSecurityCertificatePackageDialog::m_CheckUpdatePath() {
  ...
  if (this->mpc_Ui->pc_ChxBoxApplyUpdateConfiguration->isChecked()) {
    ...
    if (this->GetPublicKeyPath() == "") {
      ...
      s32_Return = C_RANGE;
    } else if (C_OscUtils::h_CheckValidFilePath(this->GetPublicKeyPath()) ==
               false) {
      s32_Return = C_RD_WR;
...
/*! \brief  Validates the secure authentication PEM files
   ...
   \param[out]  orc_ErrorPath    PEM file path that led to error path (only
   first fail gets reported)

   \retval   C_NOACT     PEM files list is empty after enabling "AddNewPemFiles"
   checkbox
   \retval   C_RANGE     File suffix *.pem missing
   \retval   C_RD_WR     Invalid path (e.g. contains invalid characters)
   \retval   C_CONFIG    Incorrect key usage or no private keys included
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvUpPacSecurityCertificatePackageDialog::m_CheckAuthPemFiles(
    QString &orc_ErrorPath) const {
  int32_t s32_Return = C_NO_ERR;

  if (this->mpc_Ui->pc_ChxBoxAddPEMFiles->isChecked()) {
    const QStringList &rc_Files =
        this->mpc_Ui->pc_PEMFilesWidget->GetPemFilePaths();
    if (rc_Files.isEmpty()) {
      s32_Return = C_NOACT;
    }

    // check all files (stop on first failure, we do not need to parse all if
    // one is invalid)
    for (QStringList::const_iterator c_It = rc_Files.begin();
         (c_It < rc_Files.end()) && (s32_Return == C_NO_ERR); ++c_It) {
      ...
      if ((s32_Return == C_NO_ERR) && (c_FileInfo.exists() == true)) {
        ...
        if (s32_Result != C_NO_ERR) {
          ...
          const C_OscSecurityPemKeyInfo::C_CertificateKeyUsageInformation
              &rc_Usage = rc_KeyInfo.GetKeyUsageInformation();
          if (((rc_Usage.q_KeyUsageDefined == true) &&
               (rc_Usage.q_KeyUsageDigitalSignature == false)) ||
              ((rc_Usage.q_ExtendedKeyUsageDefined == true) &&
               (rc_Usage.q_ExtendedKeyUsageClientAuth == false))) {
            // error: key usage is defined, but not the right one
            osc_write_log_error("PEM file keys",
                                "Certificate: Key usage flags are defined in "
            ...
            s32_Return = C_CONFIG;
                     ...
                     (rc_Usage.q_ExtendedKeyUsageClientAuth == false)) {
            // no error: we need to stay compatible and allow to load "old" PEM
            // files that had no key usage
            osc_write_log_warning("PEM file keys",
                                  "Certificate: Key usage flags not as "
                                  "expected in PEM file used for secure "
