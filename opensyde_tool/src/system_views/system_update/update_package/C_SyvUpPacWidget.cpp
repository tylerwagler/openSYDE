...
C_SyvUpPacWidget::C_SyvUpPacWidget(QWidget *const opc_Parent)
    : QWidget(opc_Parent), mpc_Ui(new Ui::C_SyvUpPacWidget) {
  const bool q_ServiceModeActive =
  ...
  this->mpc_Ui->pc_PushButtonScrollLeft->setIcon(
      QIcon("://images/IconScrollLeftEnabled.svg"));
  this->mpc_Ui->pc_PushButtonScrollRight->setIcon(
      QIcon("://images/IconScrollRightEnabled.svg"));
  ...
  this->m_SetUpdatePackageStatusNotLocked(QStringList(), QStringList(),
  ...
  connect(&this->mc_Timer, &QTimer::timeout, this,
          &C_SyvUpPacWidget::m_FileCheckTimer);
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacWidget::~C_SyvUpPacWidget() { delete mpc_Ui; }
...
void C_SyvUpPacWidget::InitText(void) const {
  this->mpc_Ui->pc_LabelTitle->setText(
      static_cast<QString>("UPDATE PACKAGE (%1)")
          .arg(QString::number(this->mpc_Ui->pc_ListWidget->count())));

  this->mpc_Ui->pc_PushButtonClearAll->SetToolTipInformation(
      "Clear Update Package", "Clear content of complete Update Package.");
  this->mpc_Ui->pc_PushButtonImport->SetToolTipInformation(
      "Import Update Package",
      "Import Update Package configuration from file.");

  this->mpc_Ui->pc_PushButtonExport->SetToolTipInformation(
      "Export Update Package to File",
      "Export Update Package configuration to file.");

  this->mpc_Ui->pc_PushButtonCreatePackage->SetToolTipInformation(
      "Create Service Update Package", "All required files are packed together "
                                       "(SYSTEM DEFINITION, flashware, ... )");
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacWidget::SetUpdateApplicationStarted(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacWidget::SetUpdateApplicationFinished(
    const uint32_t ou32_NodeIndex) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacWidget::SetUpdateApplicationError(
    const uint32_t ou32_NodeIndex) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacWidget::DiscardApplicationStatus(
    const uint32_t ou32_NodeIndex) const {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Disables the Update Package creation when the selected View is
 * invalid.
 */
//----------------------------------------------------------------------------------------------------------------------
...
/*! \brief   Sets the progress for a specific node
   ...
   \param[in]     ou32_NodeIndex         Index of node
   \param[in]     ou8_Progress           Entire progress of node of update
   process
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacWidget::SetNodeProgress(const uint32_t ou32_NodeIndex,
...
void C_SyvUpPacWidget::m_ButtonRightClicked(void) const {
  this->mpc_Ui->pc_ListWidget->SetActualLine(
      this->mpc_Ui->pc_ListWidget->GetActualLine() + 1);
}
...
void C_SyvUpPacWidget::m_ButtonCreatePackage(void) {
  ...
  C_SyvUpPacServiceUpdatePackageDialog::E_PackageType e_Type =
      C_SyvUpPacServiceUpdatePackageDialog::eARCHIVE;
  ...
  if (c_PopUpDialog->exec() == static_cast<int32_t>(QDialog::Accepted)) {
    e_Type = pc_ServiceUpdatePackageDialog->GetSelectedOption();
    q_Continue = true;
    switch (e_Type) {
    ...
    if (e_Type == C_SyvUpPacServiceUpdatePackageDialog::eARCHIVE) {
      c_CurrentSelectedVersion =
          pc_ServiceUpdatePackageDialog->GetSelectedVersion();
    }
  } else // "Cancel" clicked
  {
    q_Continue = false;
  ...
  if (q_Continue) {
    ...
    if (q_SecureFile == true) {
      ...
      if ((this->m_ShowSecureArchiveFileDialog(c_Password, c_PrivateKeyPath)) ==
          true) {
        ...
        this->mpc_Ui->pc_ListWidget->CreateServiceUpdatePackage(
            q_SaveAsFile, q_SecureFile, c_EncryptNodes, c_EncryptNodesPassword,
            c_EncryptNodes, c_PemFilePath);
      }
    }
  ...
  // pc_ServiceUpdatePackageDialog and the Qt memory management
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacWidget::m_ShowSecureArchiveFileDialog(QString &orc_Password,
                                                     QString &orc_Path) {
  bool q_Return = false;
  const QPointer<C_OgePopUpDialog> c_PopUpDialog =
      new C_OgePopUpDialog(this, this);
...
void C_SyvUpPacWidget::m_UpdateWidget(void) {
  if (this->mpc_Ui->pc_ListWidget->GetCountLines() > 1) {
    ...
    } else if (this->mpc_Ui->pc_ListWidget->GetActualLine() >=
               (this->mpc_Ui->pc_ListWidget->GetCountLines() - 1)) {
    ...
    this->mpc_Ui->pc_IndexViewWidget->SetColor(mc_STYLE_GUIDE_COLOR_4,
                                               mc_STYLE_GUIDE_COLOR_10);

    this->mpc_Ui->pc_IndexViewWidget->SetCountIndex(
        this->mpc_Ui->pc_ListWidget->GetCountLines());
    this->mpc_Ui->pc_IndexViewWidget->SetCurrentIndex(
        this->mpc_Ui->pc_ListWidget->GetActualLine());
  } else {
...
void C_SyvUpPacWidget::m_FileCheckTimer(void) {
  ...
  int32_t s32_Return = this->mpc_Ui->pc_ListWidget->CheckAllPaths(
      u32_CountFiles, &c_FlashwareWarningsApps, &c_MissingDataBlocks,
  ...
  if (s32_Return == C_NO_ERR) {
    if ((this->mpc_Ui->pc_ListWidget->count() == 0) || (u32_CountFiles == 0)) {
      ...
      this->m_SetUpdatePackageStatusNotLocked(
          QStringList(), QStringList(), QStringList(), c_FlashwareWarningsApps);
    }
  } else {
    this->mpc_Ui->pc_PushButtonImport->setEnabled(true);
    this->mpc_Ui->pc_PushButtonExport->setEnabled(true);
    ...
    this->mpc_Ui->pc_LabelStateIcon->setVisible(true);
    this->m_SetUpdatePackageStatusNotLocked(c_MissingDataBlocks,
                                            c_MissingParamFiles, c_MissingFiles,
                                            c_FlashwareWarningsApps);
  }

  this->mpc_Ui->pc_PushButtonCreatePackage->setEnabled(
      (s32_Return == C_NO_ERR) && (q_ServiceModeActive == false));
...
/*! \brief   Set update package status to: warning or OK
   ...
   \param[in]     orc_MissingDataBlocks      List of missing data block files
   (including applications)
   \param[in]     orc_MissingParamFiles      List of missing parameter set image
   files (including applications)
   \param[in]     orc_MissingFiles           List of missing files (including
   applications)
   \param[in]     orc_FlashwareWarningsApps  List of apps with flashware
   warnings
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacWidget::m_SetUpdatePackageStatusNotLocked(
    const QStringList &orc_MissingDataBlocks,
  ...
  if ((this->mc_MissingDataBlocks != orc_MissingDataBlocks) ||
    ...
    if ((orc_MissingDataBlocks.length() > 0) ||
        (orc_MissingParamFiles.length() > 0) ||
        (orc_MissingFiles.length() > 0)) {
      const int32_t s32_TotalMissingFiles =
          static_cast<int32_t>(orc_MissingDataBlocks.length()) +
          orc_MissingParamFiles.length() + orc_MissingFiles.length();
      ...
      if (s32_TotalMissingFiles == 1) {
        ...
        c_LabelText =
            QString::number(s32_TotalMissingFiles) + " files are missing!";
      }
      this->mpc_Ui->pc_LabelStateIcon->SetSvg(
      ...
      this->mpc_Ui->pc_LabelStateIcon->SetSvg(
          "://images/system_views/IconOk.svg");
    }
    ...
    this->mpc_Ui->pc_LabelState->SetToolTipInformation(c_TooltipHeading,
                                                       c_TooltipContent);
    this->mpc_Ui->pc_LabelStateIcon->SetToolTipInformation(c_TooltipHeading,
                                                           c_TooltipContent);
  }
}
