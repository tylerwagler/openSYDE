...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSectionNodeFilesWidget::C_SyvUpPacSectionNodeFilesWidget(
...
void C_SyvUpPacSectionNodeFilesWidget::AddFile(const QString &orc_File) {
  ...
  if ((q_ParamSet == false) && (q_PemFile == false)) {
    ...
    if ((pc_Node != NULL) && (pc_View != NULL)) {
      ...
      C_SyvUpPacListNodeItemFileWidget *const pc_FileWidget =
          new C_SyvUpPacListNodeItemFileWidget(
              this->mu32_ViewIndex, this->mu32_NodeIndex, this->mc_DeviceType,
              this->mq_FileBased, this->mq_StwFlashloader, this);

      // Pre initialization of the size. If this is not set, the eliding of the
      // path label will cause a visible resizing
      ...
      this->mpc_Ui->pc_FileVerticalLayout->insertWidget(
          this->mpc_Ui->pc_FileVerticalLayout->count(), pc_FileWidget);
      ...
      ++this->mu32_FileCount;
    } // lint !e429  //no memory leak because of the parent of pc_AppWidget and
    ...
    if (this->mu32_PemFileCount == 0) {
      C_SyvUpPacListNodeItemPemFileWidget *const pc_PemWidget =
          new C_SyvUpPacListNodeItemPemFileWidget(
              this->mu32_ViewIndex, this->mu32_NodeIndex, this->mc_DeviceType,
              this->mq_FileBased, this->mq_StwFlashloader, this);
      // Set default values
      const C_OscViewNodeUpdate::E_StateSecurity e_STATE_SECURITY =
          C_OscViewNodeUpdate::eST_SEC_ACTIVATE;
      const C_OscViewNodeUpdate::E_StateDebugger e_STATE_DEBUGGER =
      ...
      // Save the new file as application path
      Q_ASSERT(
          C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationPemFilePath(
              this->mu32_ViewIndex, this->mu32_NodeIndex, orc_File) ==
          C_NO_ERR);
      // Set the new default states as well
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationStates(
                   this->mu32_ViewIndex, this->mu32_NodeIndex, e_STATE_SECURITY,
                   e_STATE_DEBUGGER) == C_NO_ERR);
      ...
      const QString c_Message =
          static_cast<QString>("A PEM file was already added. "
                               "Only one PEM file for a node is allowed. The "
                               "file %1 was not added.")
              .arg(orc_File);
    ...
    if (this->m_GetParamsetFileInfo(orc_File, c_ParamFileInfo) == C_NO_ERR) {
      ...
      pc_ParamWidget->SetAppNumber(this->mu32_CountSkippedParamSetFiles +
      ...
      this->mpc_Ui->pc_FileVerticalLayout->insertWidget(
          static_cast<uint32_t>(this->mu32_PemFileCount +
                                this->mu32_ParamSetFileCount),
          pc_ParamWidget);

      // Save the new file as parameter path
      Q_ASSERT(
          C_PuiSvHandler::h_GetInstance()->AddNodeUpdateInformationParamInfo(
              this->mu32_ViewIndex, this->mu32_NodeIndex, c_ParamFileInfo) ==
          C_NO_ERR);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::AdaptFile(
  ...
  if (opc_App != NULL) {
    if (((opc_App->GetType() == mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_FILE) &&
      ...
      if ((q_NewFileParamSet == false) && (q_NewFilePem == false)) {
        ...
        Q_ASSERT(C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationPath(
                     ...
                     opc_App->GetAppNumber(), orc_File,
                     C_OscViewNodeUpdate::eFTP_FILE_BASED) == C_NO_ERR);
      } else if (q_NewFilePem == true) {
        C_SyvUpPacSectionNodeWidget::AdaptFile(orc_File, opc_App);
        Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                     ->SetNodeUpdateInformationPemFilePath(
                         this->mu32_ViewIndex, this->mu32_NodeIndex,
                         orc_File) == C_NO_ERR);
      } else {
      ...
      if ((q_NewFilePem == false) || (this->mu32_PemFileCount == 0U)) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::SetSkipOfUpdateFile(
    ...
    if (opc_App->GetType() == mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_FILE) {
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                   ->SetNodeUpdateInformationSkipUpdateOfPath(
                       ...
                       C_OscViewNodeUpdate::eFTP_FILE_BASED) == C_NO_ERR);
    } else if (opc_App->GetType() ==
               mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_PEM) {
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                   ->SetNodeUpdateInformationSkipUpdateOfPemFile(
                       this->mu32_ViewIndex, this->mu32_NodeIndex, oq_Skip) ==
               C_NO_ERR);
    } else {
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::RemoveFile(
    C_SyvUpPacListNodeItemWidget *const opc_App) {
  if (opc_App != NULL) {
    const uint32_t u32_Number = opc_App->GetAppNumber();
    if (opc_App->GetType() == mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_FILE) {
      // Remove the application file path
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()->RemoveNodeUpdateInformationPath(
                   this->mu32_ViewIndex, this->mu32_NodeIndex, u32_Number,
                   C_OscViewNodeUpdate::eFTP_FILE_BASED) == C_NO_ERR);
    } else if (opc_App->GetType() ==
               mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_PEM) {
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                   ->RemoveNodeUpdateInformationPemFilePath(
                       this->mu32_ViewIndex, this->mu32_NodeIndex) == C_NO_ERR);
    } else {
      Q_ASSERT(
          C_PuiSvHandler::h_GetInstance()->RemoveNodeUpdateInformationParamInfo(
              this->mu32_ViewIndex, this->mu32_NodeIndex, u32_Number) ==
          C_NO_ERR);
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::OpenPemFileSettings(
    C_SyvUpPacListNodeItemWidget *const opc_App) {
  ...
  if (pc_PemApp != NULL) {
    ...
    // Adapt the states
    {
      ...
      const C_SyvUpPacPemFileOptionsPopUp *const pc_InfoDialog =
          new C_SyvUpPacPemFileOptionsPopUp(*c_New, e_StateSecurity,
                                            e_StateDebugger);
    ...
    } // lint !e429  //no memory leak because of the parent of pc_InfoDialog and
      // the Qt memory management

    pc_PemApp->SetPemStates(e_StateSecurity, e_StateDebugger);
    Q_ASSERT(C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationStates(
                 this->mu32_ViewIndex, this->mu32_NodeIndex, e_StateSecurity,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::PrepareExportConfig(
        ...
        if (pc_App != NULL) {
          if (pc_App->GetType() == mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_PEM) {
            ...
            if (pc_PemApp != NULL) {
              ...
              pc_PemApp->GetPemStates(orc_NodeConfig.e_StateSecurity,
                                      orc_NodeConfig.e_StateDebugger);
            }
          } else if (pc_App->GetType() !=
                     mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_PARAMSET) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::LoadImportConfig(
    const C_SyvUpPacConfig &orc_Config) {
  uint32_t u32_ConfigCounter;

  // Search the matching configuration
  for (u32_ConfigCounter = 0;
       u32_ConfigCounter < orc_Config.c_NodeConfigs.size();
       ++u32_ConfigCounter) {
    if ((orc_Config.c_NodeConfigs[u32_ConfigCounter].c_Name ==
         this->mc_NodeName) &&
        (orc_Config.c_NodeConfigs[u32_ConfigCounter].c_DeviceType ==
         this->mc_DeviceType)) {
      ...
      for (u32_FileCounter = this->mu32_CountSkippedParamSetFiles;
           u32_FileCounter <
           orc_Config.c_NodeConfigs[u32_ConfigCounter].c_ParamSetConfigs.size();
           ++u32_FileCounter) {
        const QString c_Path = orc_Config.c_NodeConfigs[u32_ConfigCounter]
                                   .c_ParamSetConfigs[u32_FileCounter];
      ...
      if (this->mq_FileBased == true) {
        for (u32_FileCounter = 0U;
             u32_FileCounter <
             orc_Config.c_NodeConfigs[u32_ConfigCounter].c_FileConfigs.size();
             ++u32_FileCounter) {
          // Update UI and update view data handling
          this->AddFile(orc_Config.c_NodeConfigs[u32_ConfigCounter]
                            .c_FileConfigs[u32_FileCounter]);
        }
      }

      if (!orc_Config.c_NodeConfigs[u32_ConfigCounter]
               .c_PemFilePath.isEmpty()) {
        this->AddFile(
            orc_Config.c_NodeConfigs[u32_ConfigCounter].c_PemFilePath);
        // Configure PEM states
        QLayoutItem *const pc_Item =
            this->mpc_Ui->pc_FileVerticalLayout->itemAt(0);

        if (pc_Item != NULL) {
          C_SyvUpPacListNodeItemPemFileWidget *const pc_PemApp =
              dynamic_cast<C_SyvUpPacListNodeItemPemFileWidget *>(
                  pc_Item->widget());

          if (pc_PemApp != NULL) {
            pc_PemApp->SetPemStates(
                orc_Config.c_NodeConfigs[u32_ConfigCounter].e_StateSecurity,
...
/*! \brief   Check if mime valid
   ...
   \param[in,out]  orc_PathList                    File paths to analyze,
   relevant paths in opc_RelevantFilePaths in will be removed here
   \param[in]      orc_Pos                         Mouse position
   \param[out]     opc_RelevantFilePaths           File paths which could be
   used for this list
   \param[out]     opc_RelevantParamSetImagePaths  Parameter set image paths
   which could be used for this list
   \param[out]     opc_RelevantPemFilePaths        PEM file paths which could be
   used for this list
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacSectionNodeFilesWidget::CheckMime(
  ...
  if (orc_PathList.size() > 0) {
    ...
    if (q_Retval == true) {
      ...
      // Assign all file paths to the parameter set files if it is a file with
      // the correct extension or add it to the "other files"
      do {
...
/*! \brief   Sets the number of skipped parameter set image files
   ...
   \param[in]  ou32_CountSkippedFiles  Number of parameter set image files which
   are used by the datablock sections
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeFilesWidget::SetCountSkippedParamSetFiles(
    const uint32_t ou32_CountSkippedFiles) {
...
void C_SyvUpPacSectionNodeFilesWidget::m_InitSpecificItem(
  ...
  // PSI files second
  for (u32_Counter = this->mu32_CountSkippedParamSetFiles;
       u32_Counter < c_ViewParamsetPaths.size(); ++u32_Counter) {
    C_OscViewNodeUpdateParamInfo c_ParamInfo;
    const C_OscViewNodeUpdateParamInfo &rc_FileInfo =
        c_ViewParamsetPaths[u32_Counter];
    C_SyvUpPacListNodeItemParamSetWidget *const pc_ParamWidget =
        new C_SyvUpPacListNodeItemParamSetWidget(
            this->mu32_ViewIndex, this->mu32_NodeIndex, this->mc_DeviceType,
            this->mq_FileBased, this->mq_StwFlashloader, this);
    ...
    c_ParamInfo.SetContent(rc_FileInfo.GetPath(),
                           rc_FileInfo.GetLastKnownCrc());
...
/*! \brief   Adapts the number of a param set file if necessary
   ...
   In case of param set files in the files widget, the skipped count must be
   used for the real number of the param set file
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_SyvUpPacSectionNodeFilesWidget::m_AdaptParamSetNumber(
    const uint32_t ou32_Number) {
