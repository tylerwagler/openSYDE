...
void C_SyvUpPacPemFilesListWidget::AddFileAction(void) {
  ...
  this->m_AddNewPemFile("Select PEM file",
                        static_cast<QString>("PEM file") + " (*.pem)",
                        c_Folder);
}
...
void C_SyvUpPacPemFilesListWidget::SaveUserSettings() {
  C_UsHandler::h_GetInstance()->SetLastKnownUpdatePemFilePaths(
      this->GetPemFilePaths());
}
...
void C_SyvUpPacPemFilesListWidget::DeleteAllItems() {
  ...
  c_MessageBox.SetDescription(
      "Do you really want to remove all files from the list?");
  ...
  for (int32_t s32_It = 0; s32_It < this->count(); ++s32_It) {
    ...
    if (pc_CurrentItem != NULL) {
      C_SyvUpPacPemFileEntry *const pc_ItemWidget =
          dynamic_cast<C_SyvUpPacPemFileEntry *>(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::DragEnterEvent(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::DragMoveEvent(
    QDragMoveEvent *const opc_Event) {
  const QMimeData *const pc_MimeData = opc_Event->mimeData();
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::m_AddNewPemFile(
    const QString &orc_DialogCaption, const QString &orc_DialogFilter,
    const QString &orc_Folder) {
  QStringList c_InvalidFiles;
  const QStringList c_Files = QFileDialog::getOpenFileNames(
      this, orc_DialogCaption, orc_Folder, orc_DialogFilter);
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SyvUpPacPemFilesListWidget::m_LastSuffixOfFileName(
    const QString &orc_File) const {
  QString c_Result = "";
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::m_AddFile(
    const QString &orc_File, const int32_t os32_CurrentPemFileIndex) {
  ...
  for (int32_t s32_It = 0; s32_It < this->count(); ++s32_It) {
    ...
    if (pc_CurrentItem != NULL) {
      ...
      if ((pc_ItemWidget != NULL) &&
          (pc_ItemWidget->c_FilePath.isEmpty() == false)) {
        const QFileInfo c_ExistingFileInfo(pc_ItemWidget->c_FilePath);
        const QString c_AbsoluteExistingFilePath =
            c_ExistingFileInfo.absoluteFilePath();
  ...
  if (q_FileAlreadyExists == false) {
    ...
    pc_Item->setSizeHint(
        QSize(pc_ItemWidget->width(), pc_ItemWidget->height()));
  ...
  } // lint !e429  //no memory leak of pc_ItemWidget, pc_Item because of
    // handling in
  // "this->m_DeleteItem()"
  else {
...
void C_SyvUpPacPemFilesListWidget::m_LoadLastKnownPemFilePaths() {
  QStringList c_PemFilePaths =
      C_UsHandler::h_GetInstance()->GetLastKnownUpdatePemFilePaths();

  for (int32_t s32_It = 0; s32_It < static_cast<int32_t>(c_PemFilePaths.size());
       s32_It++) {
...
/*! \brief  Delete item on delete button click
   ...
   \param[in]      os32_CurrentPemFileIndex           Current pem file widget
   index
   \param[in]          oq_NeedToUpdateListIndex           Need to update list
   indexes
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::m_DeleteItem(
  ...
  if (pc_CurrentItem != NULL) {
    ...
    if (pc_ItemWidget != NULL) {
      if (oq_NeedToUpdateListIndex) {
        ...
        if (c_MessageBox.Execute() == C_OgeWiCustomMessage::eYES) {
          pc_ItemWidget = dynamic_cast<C_SyvUpPacPemFileEntry *>(
              this->takeItem(os32_CurrentPemFileIndex));
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::m_UpdatePemFileWidgetIndex(
    const int32_t os32_CurrentPemFileIndex) const {
  for (int32_t s32_It = os32_CurrentPemFileIndex; s32_It < this->count();
       ++s32_It) {
    QListWidgetItem *const pc_CurrentItem = this->item(s32_It);
    if (pc_CurrentItem != NULL) {
      C_SyvUpPacPemFileEntry *const pc_ItemWidget =
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacPemFilesListWidget::m_ShowHideVerticalScrollBar(
