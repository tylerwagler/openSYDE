 ...
 */
const QString
    C_SyvUpPacSectionNodeDatablockWidget::mhc_REMOVE_PATH_TEXT("<Add File>");
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvUpPacSectionNodeDatablockWidget::C_SyvUpPacSectionNodeDatablockWidget(
...
/*! \brief   Sets a new file for the application
   ...
   \param[in]  orc_File    New path (already contains Db Project path and might
   be relative to openSYDE project)
   \param[in]  opc_App     Application widget
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::AdaptFile(
  ...
  if (orc_File == mhc_REMOVE_PATH_TEXT) {
    ...
    if (this->me_Type == C_OscNodeApplication::ePARAMETER_SET_HALC) {
      ...
      if (pc_ParamSetWidget != NULL) {
        ...
        Q_ASSERT(
            C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationParamInfo(
    ...
    // Check the file
    // note: Data Block project path is already prefixed, so we only make
    // absolute to openSYDE project
    const QString c_AbsoluteFilePath =
      ...
      if (this->me_Type == C_OscNodeApplication::ePARAMETER_SET_HALC) {
        ...
        if (pc_HexFile->LoadFromFile(c_AbsoluteFilePath.toUtf8().constData()) ==
          ...
          const int32_t s32_Result =
              pc_HexFile->ScanApplicationInformationBlockFromHexFile(
          ...
          if ((s32_Result == C_NO_ERR) || (s32_Result == C_WARN)) {
            ...
            if ((this->mq_StwFlashloader == true) ||
                (QString::compare(this->mc_DeviceType.trimmed(),
                                  c_AppDeviceType, Qt::CaseInsensitive) == 0)) {
              q_FileIsOk = true;
            } else {
              ...
              if (q_FileIsOk == false) {
                ...
                c_Message.SetDescription(
                    "Device type of selected HEX file does not "
                    "match the node type.");
                c_Message.SetDetails(
                    static_cast<QString>(
                        "Device type of %1 does not match node type %2.")
                        .arg(c_AppDeviceType.trimmed(),
                             this->mc_DeviceType.trimmed()));
            ...
            if (s32_Result == C_OVERFLOW) {
              c_Message.SetDescription(
                  "HEX file has multiple application information "
                  "blocks with non-equal device names!");
              c_Message.SetCustomMinHeight(180, 180);
            } else {
              c_Message.SetDescription(
                  "HEX file has no application information block!");
              c_Message.SetCustomMinHeight(180, 180);
            }
            c_Message.Execute();
          }
          ...
          const QString c_Details =
              static_cast<QString>("File path: %1").arg(c_AbsoluteFilePath);
          // Invalid file
  ...
  if ((q_FileIsOk == true) && (opc_App != NULL)) {
    C_SyvUpPacSectionNodeWidget::AdaptFile(orc_File, opc_App);

    Q_ASSERT(C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationPath(
                 this->mu32_ViewIndex, this->mu32_NodeIndex,
                 this->mu32_DataBlockPathNumber, orc_File,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::SetSkipOfUpdateFile(
    const bool oq_Skip, C_SyvUpPacListNodeItemWidget *const opc_App) {
  if (opc_App != NULL) {
    ...
    if (opc_App->GetType() == mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_DATABLOCK) {
      ...
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                   ->SetNodeUpdateInformationSkipUpdateOfPath(
                       this->mu32_ViewIndex, this->mu32_NodeIndex,
                       this->mu32_DataBlockPathNumber, oq_Skip,
                       C_OscViewNodeUpdate::eFTP_DATA_BLOCK) == C_NO_ERR);
    } else {
      // In this case only one NVM HALC Datablock can exist, so the app number
      // equals the param set file number
      Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                   ->SetNodeUpdateInformationSkipUpdateOfParamInfo(
                       this->mu32_ViewIndex, this->mu32_NodeIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::RevertFile(
  ...
  if (opc_App != NULL) {
    ...
    if (pc_Node != NULL) {
      ...
      Q_ASSERT(u32_AppNumber < pc_Node->c_Applications[this->mu32_SectionNumber]
                                   .c_ResultPaths.size());
      if (pc_Node->c_Applications[this->mu32_SectionNumber]
        ...
        if ((this->mu32_SectionNumber < pc_Node->c_Applications.size())) {
          ...
          if (this->me_Type != C_OscNodeApplication::ePARAMETER_SET_HALC) {
            ...
            Q_ASSERT(
                C_PuiSvHandler::h_GetInstance()->SetNodeUpdateInformationPath(
                    this->mu32_ViewIndex, this->mu32_NodeIndex,
            ...
            // Remove the view specific path
            Q_ASSERT(C_PuiSvHandler::h_GetInstance()
                         ->SetNodeUpdateInformationParamInfo(
                             this->mu32_ViewIndex, this->mu32_NodeIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::RemoveFile(
    C_SyvUpPacListNodeItemWidget *const opc_App) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::PrepareExportConfig(
    C_SyvUpPacConfigNode &orc_NodeConfig) const {
  if (this->mpc_Ui->pc_FileVerticalLayout->count() > 0) {
    int32_t s32_Counter;

    orc_NodeConfig.c_AppConfigs.reserve(orc_NodeConfig.c_AppConfigs.size() +
                                        this->mu32_PrimaryFileCount);
    orc_NodeConfig.c_ParamSetConfigs.reserve(
        orc_NodeConfig.c_ParamSetConfigs.size() + this->mu32_ParamSetFileCount);

    for (s32_Counter = 0;
         s32_Counter < this->mpc_Ui->pc_FileVerticalLayout->count();
         ++s32_Counter) {
      QLayoutItem *const pc_Item =
          this->mpc_Ui->pc_FileVerticalLayout->itemAt(s32_Counter);
      ...
      if (pc_Item != NULL) {
        ...
        if (pc_App != NULL) {
          if (pc_App->GetType() !=
              mu32_UPDATE_PACKAGE_NODE_SECTION_TYPE_PARAMSET) {
            ...
            orc_NodeConfig.c_ParamSetConfigs.push_back(
                pc_App->GetAppFilePath());
          }
        }
      }
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::LoadImportConfig(
    const C_SyvUpPacConfig &orc_Config) {
  uint32_t u32_NodeConfigCounter;

  // Search the matching configuration
  for (u32_NodeConfigCounter = 0;
       u32_NodeConfigCounter < orc_Config.c_NodeConfigs.size();
       ++u32_NodeConfigCounter) {
    const C_SyvUpPacConfigNode &rc_NodeConfig =
        orc_Config.c_NodeConfigs[u32_NodeConfigCounter];
    ...
    if ((rc_NodeConfig.c_Name == this->mc_NodeName) &&
      ...
      if (this->me_Type != C_OscNodeApplication::ePARAMETER_SET_HALC) {
        ...
        for (u32_AppConfigCounter = 0;
             u32_AppConfigCounter < rc_NodeConfig.c_AppConfigs.size();
          ...
          if ((rc_AppConfig.c_Name == this->mc_SectionName) &&
            ...
            // Matching configuration found. Load the configuration for the
            // applications
            for (s32_FileCounter = 0;
                 s32_FileCounter < this->mpc_Ui->pc_FileVerticalLayout->count();
                 ++s32_FileCounter) {
              QLayoutItem *const pc_Item =
                  this->mpc_Ui->pc_FileVerticalLayout->itemAt(s32_FileCounter);

              if (pc_Item != NULL) {
                C_SyvUpPacListNodeItemWidget *const pc_App =
                    dynamic_cast<C_SyvUpPacListNodeItemWidget *>(
                        pc_Item->widget());
        ...
        // Search for a special case NVM HALC Datablock when parameter set image
        // files exists
        if ((rc_NodeConfig.c_ParamSetConfigs.size() > 0) &&
            (rc_NodeConfig.c_ParamSetConfigs.size() >=
            ...
            if (pc_Item != NULL) {
              ...
              if ((pc_App != NULL) &&
                ...
                const QString c_Path =
                    rc_NodeConfig.c_ParamSetConfigs[s32_FileCounter];
...
/*! \brief   Update device information
   ...
   Check and update of information which application is already finished and
   must not be updated

   \param[in]  orc_DeviceInformation   Device info
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpPacSectionNodeDatablockWidget::UpdateDeviceInformation(
    const C_SyvUpDeviceInfo &orc_DeviceInformation) {
  ...
  if (orc_DeviceInformation.pc_OpenSydeDevice != NULL) {
    u32_AppCountOnTarget =
        orc_DeviceInformation.pc_OpenSydeDevice->c_Applications.size();
  } else if (orc_DeviceInformation.pc_StwDevice != NULL) {
    u32_AppCountOnTarget = orc_DeviceInformation.pc_StwDevice
                               ->c_BasicInformation.c_DeviceInfoBlocks.size();
  } else {
    // Nothing to do
    u32_AppCountOnTarget = 0U;
  }
  // go through all files in the UI
  for (s32_WidgetCounter = 0;
       s32_WidgetCounter < this->mpc_Ui->pc_FileVerticalLayout->count();
       ++s32_WidgetCounter) {
    QLayoutItem *const pc_Item =
        this->mpc_Ui->pc_FileVerticalLayout->itemAt(s32_WidgetCounter);
    ...
    if (pc_Item != NULL) {
      ...
      if (pc_File != NULL) {
        ...
        for (uint32_t u32_AppCounter = 0U;
             u32_AppCounter < u32_AppCountOnTarget; ++u32_AppCounter) {
          QString c_AppName = "";
          ...
          if (orc_DeviceInformation.pc_OpenSydeDevice != NULL) {
            const C_OscProtocolDriverOsy::C_FlashBlockInfo &rc_FlashBlockInfo =
                orc_DeviceInformation.pc_OpenSydeDevice
                    ->c_Applications[u32_AppCounter];

            c_AppName = rc_FlashBlockInfo.c_ApplicationName;
            c_AppVersion = rc_FlashBlockInfo.c_ApplicationVersion;
            c_AppBuildTime = rc_FlashBlockInfo.c_BuildTime;
            c_AppBuildDate = rc_FlashBlockInfo.c_BuildDate;
            ...
            const stw::diag_lib::C_XFLECUInformation &rc_DeviceInfoBlock =
                orc_DeviceInformation.pc_StwDevice->c_BasicInformation
                    .c_DeviceInfoBlocks[u32_AppCounter];
            c_AppName = rc_DeviceInfoBlock.GetProjectName();
            c_AppVersion = rc_DeviceInfoBlock.GetProjectVersion();
            c_AppBuildTime = rc_DeviceInfoBlock.GetTime();
            c_AppBuildDate = rc_DeviceInfoBlock.GetDate();

            // No application valid flag
            q_Valid = true;
          }
          if ((q_Valid == true) &&
              (pc_File->IsFileIdentical(c_AppName, c_AppVersion, c_AppBuildTime,
                                        c_AppBuildDate) == true)) {
...
/*! \brief   Check if mime valid
   ...
   Datablocks accept only one file for a specific datablock. No multiple files.
   No adding of new files.

   \param[in]   orc_PathList                    File paths to analyze
   \param[in]   orc_Pos                         Mouse position
   \param[out]  opc_RelevantFilePaths           File paths which could be used
   for this list
   \param[out]  opc_RelevantParamSetImagePaths  Parameter set image paths which
   could be used for this list
   \param[out]  opc_RelevantPemFilePaths        PEM file paths (Not relevant and
   possible for datablocks)
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvUpPacSectionNodeDatablockWidget::CheckMime(
    QStringList &orc_PathList, const QPoint &orc_Pos,
  ...
  if (orc_PathList.size() == 1) {
    ...
    q_Retval = C_SyvUpPacSectionNodeWidget::CheckMime(
        orc_PathList, orc_Pos, opc_RelevantFilePaths,
        opc_RelevantParamSetImagePaths, opc_RelevantPemFilePaths, &pc_App);

    // Dropping on datablocks must have a specific app as target
    if ((q_Retval == true) && (pc_App != NULL)) {
      if (this->me_Type != C_OscNodeApplication::ePARAMETER_SET_HALC) {
      ...
      if ((q_Retval == true) && (oppc_App != NULL)) {
        *oppc_App = pc_App;
      }
    } else {
      q_Retval = false;
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Get type of subsection (data block/parameter set file/file based
   node file)
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Initialization of the applications in case of datablock
   configuration
  ...
  if (this->mu32_SectionNumber < orc_Node.c_Applications.size()) {
    ...
    if (rc_Datablock.e_Type != C_OscNodeApplication::ePARAMETER_SET_HALC) {
      ...
      C_SyvUpPacListNodeItemDatablockWidget *const pc_FileWidget =
          new C_SyvUpPacListNodeItemDatablockWidget(
              this->mu32_ViewIndex, this->mu32_NodeIndex, this->mc_DeviceType,
      ...
      this->mc_SectionName = rc_Datablock.c_Name;
      ...
      if ((c_ViewDatablockPaths[this->mu32_DataBlockPathNumber] != "") &&
          (c_ViewDatablockPaths[this->mu32_DataBlockPathNumber] != c_Path)) {
        pc_FileWidget->SetAppFile(
            c_ViewDatablockPaths[this->mu32_DataBlockPathNumber], false);
      } else {
        pc_FileWidget->SetAppFile(c_Path, true);
      }
      pc_FileWidget->SetSkipOfUpdateFile(
          c_ViewDatablockSkipFlags[this->mu32_DataBlockPathNumber]);
    ...
    } // lint !e429  //no memory leak because of the parent of pc_FileWidget and
      ...
      for (u32_ParamSetFileCounter = 0U;
           u32_ParamSetFileCounter < rc_Datablock.c_ResultPaths.size();
           ++u32_ParamSetFileCounter) {
        C_OscViewNodeUpdateParamInfo c_ParamInfo;
        C_SyvUpPacListNodeItemParamSetWidget *const pc_ParamWidget =
            new C_SyvUpPacListNodeItemParamSetWidget(
                this->mu32_ViewIndex, this->mu32_NodeIndex, this->mc_DeviceType,
                this->mq_FileBased, this->mq_StwFlashloader, this);
        const QString c_DefaultPath =
            C_PuiUtil::h_MakeIndependentOfDbProjectPath(
                rc_Datablock.c_ProjectPath,
                rc_Datablock.c_ResultPaths[u32_ParamSetFileCounter]);
        ...
        if (c_FileInfo.GetPath() != "") {
          ...
          c_ParamInfo.SetContent(c_FileInfo.GetPath(),
        ...
        pc_ParamWidget->SetSkipOfUpdateFile(
            c_ViewParamSetSkipFlags[u32_ParamSetFileCounter]);
      ...
      } // lint !e429  //no memory leak because of the parent of pc_ParamWidget
