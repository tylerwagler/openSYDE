...
/*!
   ...
   Widget to put together update package and update summary widgets and handle
...
C_SyvUpInformationWidget::C_SyvUpInformationWidget(QWidget *const opc_Parent)
    : QWidget(opc_Parent), mpc_Ui(new Ui::C_SyvUpInformationWidget),
      mu32_ItFile(0), mu32_ItParamFile(0U), mu32_ItPemFile(0U),
      mu32_ViewIndex(0U), mu64_NodeStartTimeMs(0ULL),
      mu64_LastKnownDataRateSeconds(0ULL), mu64_CurrentFlashedBytes(0ULL),
  ...
  connect(&this->mc_Timer, &QTimer::timeout, this,
          &C_SyvUpInformationWidget::m_UpdateTime);
  connect(&this->mc_Timer, &QTimer::timeout, this,
          &C_SyvUpInformationWidget::m_UpdateDataRate);
  connect(this->mpc_Ui->pc_WidgetUpdatePackage,
          &C_SyvUpPacWidget::SigUpdatePackageState, this,
          &C_SyvUpInformationWidget::SigUpdatePackageState);

  connect(this->mpc_Ui->pc_WidgetUpdateSummary,
          &C_SyvUpSummaryWidget::SigHideBigSummaryWidget, this,
          &C_SyvUpInformationWidget::m_HideBigUpdateSummary);
  connect(this->mpc_Ui->pc_WidgetUpdateSummarySmall,
          &C_SyvUpSummaryWidgetSmall::SigHideSmallSummaryWidget, this,
          &C_SyvUpInformationWidget::m_HideSmallUpdateSummary);
  connect(this, &C_SyvUpInformationWidget::SigInvalidView,
          this->mpc_Ui->pc_WidgetUpdatePackage,
          &C_SyvUpPacWidget::DisableUpdatePackage);
} // lint !e429  //no memory leak because of the parent of pc_Button and the Qt
  // memory management
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetUpdateNodeStarted(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetUpdateNodeSuccess(
    const uint32_t ou32_NodeIndex) {
  const uint64_t u64_NodeEndTimeMs = this->mc_ElapsedTimer.elapsed();

  this->mc_FileSizeInformation.SetElapsedTimeForNode(
      ou32_NodeIndex, u64_NodeEndTimeMs - this->mu64_NodeStartTimeMs);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetUpdateApplicationStarted(
    const uint32_t ou32_NodeIndex, const bool oq_IsParam,
    const bool oq_IsPemFile) const {
  Q_UNUSED(oq_IsParam)
  Q_UNUSED(oq_IsPemFile)
  this->mpc_Ui->pc_WidgetUpdatePackage->SetUpdateApplicationStarted(
      ou32_NodeIndex);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetUpdateApplicationFinished(
    const uint32_t ou32_NodeIndex, const bool oq_IsParam,
    const bool oq_IsPemFile) {
  this->mpc_Ui->pc_WidgetUpdatePackage->SetUpdateApplicationFinished(
      ou32_NodeIndex);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetUpdateApplicationError(
    const uint32_t ou32_NodeIndex) const {
  this->mpc_Ui->pc_WidgetUpdatePackage->SetUpdateApplicationError(
      ou32_NodeIndex);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::DiscardApplicationStatus(
    const uint32_t ou32_NodeIndex) const {
  this->mpc_Ui->pc_WidgetUpdatePackage->DiscardApplicationStatus(
      ou32_NodeIndex);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetNodeProgress(
    const uint32_t ou32_NodeIndex, const uint8_t ou8_Progress) const {
  this->mpc_Ui->pc_WidgetUpdatePackage->SetNodeProgress(ou32_NodeIndex,
                                                        ou8_Progress);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::SetHeading(const QString &orc_Icon,
                                          const QString &orc_Text,
                                          const bool oq_Failure,
                                          const bool oq_Success) const {
  this->mpc_Ui->pc_WidgetUpdateSummary->SetHeading(orc_Icon, orc_Text,
                                                   oq_Failure, oq_Success);
  this->mpc_Ui->pc_WidgetUpdateSummarySmall->SetHeading(orc_Icon, orc_Text,
                                                        oq_Failure, oq_Success);
}
...
/*! \brief   Set known update configuration for flash progress assumptions
   ...
   \param[in]  orc_IsFileBased               File based flag for all included
   devices
   \param[in]  oq_IncludesCurrentNodeStatus  Flag if the current call includes
...
void C_SyvUpInformationWidget::InitUpdatePackage(
  ...
  for (uint32_t u32_ItOrder = 0; u32_ItOrder < orc_Order.size();
       ++u32_ItOrder) {
    const uint32_t u32_NodeIndex = orc_Order[u32_ItOrder];
    if (u32_NodeIndex < orc_Flashpackage.size()) {
      // Check if there are any files
      const C_OscSuSequences::C_DoFlash &rc_Device =
      ...
      if (rc_Device.c_FilesToFlash.size() > 0) {
        ...
        for (uint32_t u32_ItFile = 0;
          ...
          const auto &rc_File = rc_Device.c_FilesToFlash[u32_ItFile];
          bool q_UseFileSize = true;
          // Check which case it is
          if ((u32_NodeIndex < orc_IsFileBased.size()) &&
          ...
          if (q_UseFileSize == true) {
            const QFileInfo c_FileInfo(rc_File);
        ...
        for (uint32_t u32_ItFile = 0;
             u32_ItFile < rc_Device.c_FilesToWriteToNvm.size(); ++u32_ItFile) {
          const auto &rc_File = rc_Device.c_FilesToWriteToNvm[u32_ItFile];
          C_OscParamSetHandler c_FileHandler;
          if (c_FileHandler.ReadFile(rc_File, false, true, NULL) == C_NO_ERR) {
            const C_OscParamSetInterpretedData &rc_InterpretedData =
                c_FileHandler.GetInterpretedData();
            if (rc_InterpretedData.c_InterpretedNodes.size() >= 1UL) {
              const C_OscParamSetInterpretedNode &rc_Node =
                  rc_InterpretedData.c_InterpretedNodes[0UL];
              uint64_t u64_Size = 0ULL;
              for (uint32_t u32_ItDp = 0UL;
                   u32_ItDp < rc_Node.c_DataPools.size(); ++u32_ItDp) {
                const C_OscParamSetInterpretedDataPool &rc_Dp =
                    rc_Node.c_DataPools[u32_ItDp];
                for (uint32_t u32_ItLi = 0UL; u32_ItLi < rc_Dp.c_Lists.size();
                     ++u32_ItLi) {
                  const C_OscParamSetInterpretedList &rc_Li =
                      rc_Dp.c_Lists[u32_ItLi];
                  for (uint32_t u32_ItEl = 0UL;
                       u32_ItEl < rc_Li.c_Elements.size(); ++u32_ItEl) {
                    const C_OscParamSetInterpretedElement &rc_El =
                        rc_Li.c_Elements[u32_ItEl];
        ...
        this->mc_FileSizeInformation.AppendFiles(u32_NodeIndex, c_Files,
                                                 c_ParamFiles);
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::UpdateProgress(const uint16_t ou16_Progress100,
                                              const bool oq_Finished) {
  ...
  if (oq_Finished == false) {
    ...
    for (uint32_t u32_ItDevice = 0;
         (u32_ItDevice < this->mc_FileSizeInformation.GetNumOtherFiles()) &&
         (q_Stop == false);
         ++u32_ItDevice) {
      const uint32_t u32_NumFiles =
          this->mc_FileSizeInformation.GetNumOtherFilesForDevice(u32_ItDevice);
      for (uint32_t u32_ItFile = 0;
           (u32_ItFile < u32_NumFiles) && (q_Stop == false); ++u32_ItFile) {
        const uint64_t u64_CurFileSize =
            this->mc_FileSizeInformation.GetOtherFileSizeForDevice(u32_ItDevice,
                                                                   u32_ItFile);
        if (u32_Counter == this->mu32_ItFile) {
          const uint64_t u64_OverallFilesSize =
              this->mc_FileSizeInformation.GetOverallFilesSize();
          // Match
          // Adapt to progress
          u64_FlashedBytes +=
              (u64_CurFileSize * static_cast<uint64_t>(ou16_Progress100)) /
              100ULL;
          // Calculate overall progress
          if (u64_OverallFilesSize != 0U) {
            u16_ActualProgress = static_cast<uint16_t>(
                (u64_FlashedBytes * 100ULL) / u64_OverallFilesSize);
          } else {
    ...
    u32_Counter = 0UL;
    for (uint32_t u32_ItDevice = 0;
         (u32_ItDevice < this->mc_FileSizeInformation.GetNumParamFiles()) &&
         (q_Stop == false);
         ++u32_ItDevice) {
      const uint32_t u32_NumFiles =
          this->mc_FileSizeInformation.GetNumParamFilesForDevice(u32_ItDevice);
      for (uint32_t u32_ItFile = 0;
           (u32_ItFile < u32_NumFiles) && (q_Stop == false); ++u32_ItFile) {
        const uint64_t u64_CurFileSize =
            this->mc_FileSizeInformation.GetParamFileSizeForDevice(u32_ItDevice,
                                                                   u32_ItFile);
    ...
    if ((static_cast<uint64_t>(this->mc_ElapsedTimer.elapsed()) !=
         this->mu64_FlashedBytesTimestampPrev) &&
        (this->mu64_FlashedBytesHistoryPrev !=
         this->mu64_CurrentFlashedBytes)) {
      this->mu64_FlashedBytesHistoryPrevPrev =
          this->mu64_FlashedBytesHistoryPrev;
      this->mu64_FlashedBytesTimestampPrevPrev =
          this->mu64_FlashedBytesTimestampPrev;
    ...
    // Update data transfer
    m_UpdateDataTransfer(this->mc_FileSizeInformation.GetOverallFilesSize(),
                         true);
    // Calculate data rate base on current elapsed time and flashed byte count
    if (this->mc_ElapsedTimer.elapsed() > 0) {
      u64_AverageDataRate =
          (this->mu64_CurrentFlashedBytes * 1000ULL) /
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::StopElapsedTimer(void) { this->mc_Timer.stop(); }
...
void C_SyvUpInformationWidget::m_UpdateTime(void) const {
  ...
  const QString c_Time = static_cast<QString>("%1:%2:%3")
                             .arg(u64_ElapsedHours, 2, 10, QChar('0'))
                             .arg(u64_ElapsedMin, 2, 10, QChar('0'))
                             .arg(u64_ElapsedSeconds, 2, 10, QChar('0'));
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::m_UpdateDataTransfer(
    const uint64_t &oru64_OverallFlashedBytes, const bool oq_ShowDataRate) {
  QString c_Value;
  QString c_Progress;
  QString c_Complete;
  QString c_Unit;
  const float64_t f64_OverallFlashedBytes =
      static_cast<float64_t>(oru64_OverallFlashedBytes);
  const float64_t f64_OverallFilesSize = static_cast<float64_t>(
      this->mc_FileSizeInformation.GetOverallFilesSize());

  this->mu64_CurrentFlashedBytes = oru64_OverallFlashedBytes;
  this->mq_ShowDataRateBytesPerSecond = oq_ShowDataRate;

  if (this->mc_FileSizeInformation.GetOverallFilesSize() >
      (1024ULL * 1024ULL * 1024ULL)) {
    // GB
    c_Unit = "GB";
    c_Progress = QString::number(
        f64_OverallFlashedBytes / (1024.0 * 1024.0 * 1024.0), 'f', 1);
    c_Complete = QString::number(
        f64_OverallFilesSize / (1024.0 * 1024.0 * 1024.0), 'f', 1);
  } else if (this->mc_FileSizeInformation.GetOverallFilesSize() >
             (1024ULL * 1024ULL)) {
    // MB
    c_Unit = "MB";
    c_Progress =
        QString::number(f64_OverallFlashedBytes / (1024.0 * 1024.0), 'f', 1);
    c_Complete =
        QString::number(f64_OverallFilesSize / (1024.0 * 1024.0), 'f', 1);
    ...
    c_Complete = QString::number(
        f64_OverallFilesSize / static_cast<float32_t>(1024ULL), 'f', 1);
  } else {
    // Bytes
    c_Unit = "B";
  ...
  c_Value = static_cast<QString>("%1 %3/%2 %3")
                .arg(c_Progress)
                .arg(c_Complete)
                .arg(c_Unit);

  // Translation: 1:File progress
  this->mc_DataRateAbsoluteBytes =
      static_cast<QString>("Data transfer %1").arg(c_Value);
...
void C_SyvUpInformationWidget::m_UpdateLabel(void) const {
  ...
  if (this->mc_EstimatedTime.compare("") == 0) {
    ...
    this->mpc_Ui->pc_WidgetUpdateSummary->SetDataTransfer(
        static_cast<QString>("%1\n%2")
            .arg(this->mc_EstimatedTime)
            .arg(c_DataRateCombined));
  }

  this->mpc_Ui->pc_WidgetUpdateSummarySmall->SetDataTransfer(
      c_DataRateCombined);
  this->mpc_Ui->pc_WidgetUpdateSummarySmall->SetEstimatedTime(
      this->mc_EstimatedTime);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::m_UpdateEstimatedWaitTime(
    const bool oq_IncludesCurrentNodeStatus) {
  ...
  if (oq_IncludesCurrentNodeStatus == true) {
    if (this->mc_FileSizeInformation.GetOverallFilesSize() > 0ULL) {
      bool q_Worked;
      const uint64_t u64_WaitingTimeSeconds =
          this->mc_FileSizeInformation.GetEstimatedTimeSeconds(&q_Worked);
      ...
      if (q_Worked == true) {
        ...
        if (u64_WaitingTimeMin == 0ULL) {
          this->mc_EstimatedTime = static_cast<QString>(
              "Estimated waiting time less than one minute.");
        } else {
          // One min for rounding errors has to be added before
          //-> we should round ceil (include the last seconds) but we are
          //rounding floor (integer)
          const uint64_t u64_WaitingTimeHours =
              (u64_WaitingTimeMin + 1ULL) / 60ULL;
          // lint -e{774,831,948} False positive
          if (u64_WaitingTimeHours == 0ULL) {
            this->mc_EstimatedTime =
                static_cast<QString>("Estimated waiting time %1 min.")
                    .arg(u64_WaitingTimeMin + 1ULL);
          } else {
            this->mc_EstimatedTime =
                static_cast<QString>("Estimated waiting time %1 h %2 min.")
                    .arg(u64_WaitingTimeHours)
                    .arg((u64_WaitingTimeMin + 1ULL) % 60ULL);
          }
        }
      } else {
...
void C_SyvUpInformationWidget::m_UpdateDataRate() {
  if (this->mu64_FlashedBytesHistoryPrev > 0ULL) {
    ...
    QString c_Unit;
    const uint64_t u64_Timespan = this->mu64_FlashedBytesTimestampPrev -
                                  this->mu64_FlashedBytesTimestampPrevPrev;
    const float64_t f64_Timespan = static_cast<float64_t>(u64_Timespan);
    const uint64_t u64_NumberOfFlashedBytesInTimeSpan =
        this->mu64_FlashedBytesHistoryPrev -
        this->mu64_FlashedBytesHistoryPrevPrev;
    const float64_t f64_NumberOfFlashedBytesInTimeSpan =
        static_cast<float64_t>(u64_NumberOfFlashedBytesInTimeSpan);
    const float64_t f64_NumberOfFlashedBytesThisSecond =
        f64_NumberOfFlashedBytesInTimeSpan / (f64_Timespan * 0.001);
    const uint64_t u64_NumberOfFlashedBytesThisSecond =
        static_cast<uint64_t>(f64_NumberOfFlashedBytesThisSecond);
    ...
    if (u64_NumberOfFlashedBytesThisSecond > (1024ULL * 1024ULL * 1024ULL)) {
      // GB
      c_Unit = "GB";
      c_DataTransferRate = QString::number(f64_NumberOfFlashedBytesThisSecond /
                                               (1024.0 * 1024.0 * 1024.0),
                                           'f', 0);
    } else if (u64_NumberOfFlashedBytesThisSecond > (1024ULL * 1024ULL)) {
      // MB
      c_Unit = "MB";
      c_DataTransferRate = QString::number(
          f64_NumberOfFlashedBytesThisSecond / (1024.0 * 1024.0), 'f', 0);
    } else if (u64_NumberOfFlashedBytesThisSecond > 1024ULL) {
      // kB
      c_Unit = "kB";
      c_DataTransferRate =
          QString::number(f64_NumberOfFlashedBytesThisSecond / 1024.0, 'f', 0);
    } else {
      // Bytes
      c_Unit = "B";
      c_DataTransferRate =
          QString::number(f64_NumberOfFlashedBytesThisSecond, 'f', 0);
    }
    this->mc_DataRateBytesPerSecond =
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvUpInformationWidget::m_UpdateProgressVisualization(
    const uint16_t ou16_Progress100, const bool oq_Finished) const {
  this->mpc_Ui->pc_WidgetUpdateSummary->SetProgress(ou16_Progress100,
                                                    oq_Finished);
  this->mpc_Ui->pc_WidgetUpdateSummarySmall->SetProgress(ou16_Progress100,
                                                         oq_Finished);
}
...
void C_SyvUpInformationWidget::m_HideBigUpdateSummary(void) const {
  ...
  this->mpc_Ui->pc_LayoutNoSplitter->addWidget(
      this->mpc_Ui->pc_WidgetUpdatePackage);
...
void C_SyvUpInformationWidget::m_HideSmallUpdateSummary(void) const {
  ...
  this->mpc_Ui->pc_LayoutSplitter->insertWidget(
      0, this->mpc_Ui->pc_WidgetUpdatePackage);
...
void C_SyvUpInformationWidget::m_SaveUserSettings() const {
  ...
  if (pc_View != NULL) {
    ...
    if (c_Sizes.count() > 1) {
      C_UsHandler::h_GetInstance()->SetProjSvUpdateSplitterHorizontal(
          pc_View->GetName(), c_Sizes.at(1));
    }

    // summary widget style type
    C_UsHandler::h_GetInstance()->SetProjSvUpdateSummaryBig(
        pc_View->GetName(),
