...
public:
  ...
  int32_t StartAddOsySysDef(const QString &orc_PathSystemDefinition);
  int32_t StartAddOsySysDef(const QString &orc_PathSystemDefinition,
                            const uint32_t ou32_BusIndex);

  int32_t SetOsySysDefBus(const QString &orc_PathSystemDefinition,
                          const uint32_t ou32_BusIndex) override;
  int32_t GetOsySysDef(const QString &orc_PathSystemDefinition,
                       stw::opensyde_core::C_OscComMessageLoggerOsySysDefConfig
                           &orc_SystemDefinition) override;

  // DBC handling
  int32_t StartAddDbcFile(const QString &orc_PathDbc);
  int32_t GetDbcFile(const QString &orc_PathDbc,
                     C_CieConverter::C_CieCommDefinition &orc_DbcDefinition);

  // Generic database handling
  int32_t RemoveDatabase(const QString &orc_Path) override;
  int32_t ActivateDatabase(const QString &orc_Path,
  ...
  // Filter handling
  void AddFilter(const stw::opensyde_core::C_OscComMessageLoggerFilter
                     &orc_Filter) override;
  void RemoveFilter(const stw::opensyde_core::C_OscComMessageLoggerFilter
                        &orc_Filter) override;
  void RemoveAllFilter(void) override;
  uint32_t GetFilteredMessages(void) const override;

  // CAN bus handling
  int32_t HandleCanMessage(const stw::can::T_STWCAN_Msg_RX &orc_Msg,
                           const bool oq_IsTx) override;
...
protected:
  int32_t
  m_GetCanMessage(stw::opensyde_core::C_OscComMessageLoggerData &orc_Message);

  void m_UpdateProtocolString(
  ...
  bool m_InterpretSysDef(stw::opensyde_core::C_OscComMessageLoggerData
                             &orc_MessageData) const override;

  bool m_CheckInterpretation(
      stw::opensyde_core::C_OscComMessageLoggerData &orc_MessageData) override;
  QString m_GetProtocolStringHexHook(void) const override;
  QString m_GetProtocolStringDecHook(void) const override;
...
private:
  ...
  int32_t m_AddDbcFile(const QString &orc_PathDbc);
  const C_CieConverter::C_CieCanMessage *
  m_CheckDbcFile(const stw::can::T_STWCAN_Msg_RX &orc_Msg);
  bool m_InterpretDbcFile(
      const C_CieConverter::C_CieCanMessage *const opc_DbcMessage,
      stw::opensyde_core::C_OscComMessageLoggerData &orc_MessageData) const;
  static void mh_InterpretDbcFileCanSignal(
