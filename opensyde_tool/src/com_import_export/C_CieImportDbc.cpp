 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_CieImportDbc::mhc_SEND_TYPE = "GenMsgSendType";
const QString C_CieImportDbc::mhc_CYCLE_TIME = "GenMsgCycleTime";
const QString C_CieImportDbc::mhc_INITIAL_VALUE = "GenSigStartValue";
...
QString C_CieImportDbc::mhc_DefaultSendTypeValue;    // get value from network
                                                     // attribute default values
float32_t C_CieImportDbc::mhf32_DefaultInitialValue; // default initial value of
                                                     // DBC file or openSYDE
bool C_CieImportDbc::mhq_DefaultValueDefined;        // in DBC file
QStringList C_CieImportDbc::mhc_WarningMessages;     // empty list
QString C_CieImportDbc::mhc_ErrorMessage = "";       // empty string
...
/*! \brief    Import network configuration from .dbc file
   ...
   The caller is responsible to provided valid pointers for the function
   parameters.

   \param[in]   orc_File                  path and name of .dbc file
   \param[out]  orc_Definition            communication stack definition filled
   with data read from .dbc file
   \param[out]  orc_WarningMessages       list of global warnings e.g. why some
   messages could not be imported
   \param[out]  orc_ErrorMessage          error message with reason for failed
   import
   \param[in]   oq_AddUnmappedMessages    Optional flag to include unmapped
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportDbc::h_ImportNetwork(
  ...
  if ((s32_Return == C_NO_ERR) || (s32_Return == C_WARN)) {
    ...
    for (const auto &rc_DbcNode : c_DbcNetwork.nodes) {
      ...
      osc_write_log_info("DBC file import",
                         "Reading messages with signals for node \"" +
                             rc_Node.c_Properties.c_Name + "\" ...");

      for (const auto &rc_DbcMessage : c_DbcNetwork.messages) {
        const int32_t s32_Tmp =
            mh_GetMessage(c_DbcNetwork, rc_DbcMessage.second, rc_Node);
        ...
        if (oq_AddUnmappedMessages == true) {
          ...
          if (c_DbcMessage != c_DbcNetwork.messages.end()) {
            ...
            if (mh_ConvertAndAddMessage(c_DbcNetwork, c_DbcMessage->second,
                                        orc_Definition.c_UnmappedMessages) !=
                C_NO_ERR) {
              s32_Return = C_WARN;
            }
          }
          ...
          osc_write_log_warning(
              "DBC file import",
              "message \"" + c_String +
                  "\" is not assigned to a node and ignored.");
          mhc_WarningMessages.append(
              "Message \"" + c_String +
              "\" is not assigned to a node and ignored.");
        }
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportDbc::mh_ReadFile(const QString &orc_File,
                                    Vector::DBC::Network &orc_Network) {
  int32_t s32_Return = C_NO_ERR;

  if (orc_File == "") {
    s32_Return = C_RANGE;
  } else if (QFileInfo(orc_File).exists() == false ||
             QFileInfo(orc_File).isFile() == false) {
    ...
    // try/catch the load function due to the dbc library may throw exceptions
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieImportDbc::mh_GetNode(const Vector::DBC::Node &orc_DbcNode,
                                C_CieConverter::C_CieNode &orc_Node) {
  orc_Node.c_Properties.c_Name = QString::fromStdString(orc_DbcNode.name);
  orc_Node.c_Properties.c_Comment =
      mh_ReEscapeCriticalSymbols(QString::fromStdString(orc_DbcNode.comment));
}
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CieImportDbc::mh_GetMessage(const Vector::DBC::Network &orc_DbcNetwork,
  ...
  if (q_IsTxMessage == true) {
    ...
    if (mh_ConvertAndAddMessage(orc_DbcNetwork, orc_DbcMessage,
                                orc_Node.c_TxMessages) != C_NO_ERR) {
      s32_Return = C_WARN;
    }
  } else if (q_IsRxMessage == true) {
    // add message if valid
    if (mh_ConvertAndAddMessage(orc_DbcNetwork, orc_DbcMessage,
                                orc_Node.c_RxMessages) != C_NO_ERR) {
      s32_Return = C_WARN;
    }
  } else {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportDbc::mh_PrepareMessage(
  ...
  orc_Message.c_CanMessage.u32_CanId =
      static_cast<uint32_t>(orc_DbcMessage.id) & 0x7FFFFFFFU;
  ...
  if (orc_Message.c_CanMessage.u32_CanId > 0x1FFFFFFFU) {
    s32_Return = C_CONFIG;
    osc_write_log_error("DBC file import", "CAN message \"" +
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    osc_write_log_info("DBC file import",
                       "Importing signals for CAN message \"" +
                           orc_Message.c_CanMessage.c_Name + "\" ...");

    // get all signal definitions for this message
    for (const auto &rc_DbcSignal : orc_DbcMessage.signals) {
      // If a multiplexer signal exists the message is a multiplexed message
      if (mh_GetSignal(orc_DbcNetwork, rc_DbcSignal.second, q_MessageAdapted,
                       orc_Message) != C_NO_ERR) {
        s32_Return = C_WARN;
      }
    }
    ...
    if (q_MessageAdapted == true) {
      ...
      orc_Message.c_Warnings.append("The multiplexer signal properties were "
                                    "adapted due to a multiplexed message.\n"
...
/*! \brief  Add valid messages to list of import messages.
   ...
   \param[in,out]  orc_Messages     Messages where imported message should be
   added if valid
...
int32_t C_CieImportDbc::mh_ConvertAndAddMessage(
  ...
  const int32_t s32_RetPrepMessage = C_CieImportDbc::mh_PrepareMessage(
      orc_DbcNetwork, orc_DbcMessage, c_Message);
  ...
  if ((s32_RetPrepMessage == C_NO_ERR) || (s32_RetPrepMessage == C_WARN)) {
    ...
    osc_write_log_warning("DBC file import", "Invalid CAN message \"" +
                                                 c_Message.c_CanMessage.c_Name +
                                                 "\" was not imported.");
  }
...
/*! \brief    Get signal definition
   ...
   \param[in,out]  orq_SignalAdapted   flag if signal was adapted, will not be
   reset to false
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CieImportDbc::mh_GetSignal(const Vector::DBC::Network &orc_DbcNetwork,
  ...
  case Vector::DBC::Signal::Multiplexor::MultiplexedSignal:
    ...
    c_Signal.u16_MultiplexValue =
        static_cast<uint16_t>(orc_DbcSignal.multiplexerSwitchValue);
    break;
  ...
  if ((q_MultiplexerSignal == true) && (orc_DbcSignal.bitSize > 16U)) {
  ...
  if (c_Signal.u16_ComBitLength == 0U) {
    const QString c_String = QString::fromStdString(orc_DbcSignal.name);

    c_Signal.u16_ComBitLength = 1U;
    osc_write_log_warning("DBC file import",
                          "Invalid bit length of signal \"" + c_String +
                              "\". Bit length set to \"1\".");
    orc_Message.c_Warnings.append("Invalid bit length of signal \"" + c_String +
                                  "\". Bit length set to \"1\".");
    s32_Return = C_WARN;
  }
  ...
  if ((orc_DbcSignal.byteOrder == Vector::DBC::ByteOrder::Motorola) ||
    ...
    c_Signal.e_ComByteOrder =
        C_OscCanSignal::E_ByteOrderType::eBYTE_ORDER_MOTOROLA;
  } else if ((orc_DbcSignal.byteOrder == Vector::DBC::ByteOrder::Intel) ||
             (orc_DbcSignal.byteOrder ==
              Vector::DBC::ByteOrder::LittleEndian)) {
    c_Signal.e_ComByteOrder =
        C_OscCanSignal::E_ByteOrderType::eBYTE_ORDER_INTEL;
  } else {
    const QString c_String = QString::fromStdString(orc_DbcSignal.name);
    ...
    c_Signal.e_ComByteOrder =
        C_OscCanSignal::E_ByteOrderType::eBYTE_ORDER_INTEL;
    osc_write_log_warning("DBC file import",
                          "\"" + c_String + "\" signal byte order type error");
    orc_Message.c_Warnings.append("\"" + c_String +
                                  "\" signal byte order type error");
    s32_Return = C_WARN;
  }
  ...
  c_Signal.c_Element.c_Comment =
      mh_ReEscapeCriticalSymbols(QString::fromStdString(orc_DbcSignal.comment));
  if (q_MultiplexerSignal == false) {
    ...
    c_Signal.c_Element.c_Unit = QString::fromStdString(orc_DbcSignal.unit);
  } else {
    // Multiplexer signal restriction
    if ((C_OscUtils::h_IsFloat64NearlyEqual(orc_DbcSignal.factor, 1.0) ==
         false) ||
        (C_OscUtils::h_IsFloat64NearlyEqual(orc_DbcSignal.offset, 0.0) ==
         false) ||
        (orc_DbcSignal.unit != "")) {
      orq_SignalAdapted = true;
    }
  ...
  if (mh_GetSignalValues(orc_DbcNetwork, orc_DbcSignal, q_MultiplexerSignal,
                         orq_SignalAdapted, c_Signal.c_Element,
                         orc_Message.c_Warnings) != C_NO_ERR) {
    s32_Return = C_WARN;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieImportDbc::mh_GetSignalSpnInfo(
    const Vector::DBC::Network &orc_DbcNetwork,
    const Vector::DBC::Signal &orc_DbcSignal,
  ...
  if (c_IterSpn != orc_DbcSignal.attributeValues.end()) {
    const std::map<std::string,
        ...
        c_IterSpnType = orc_DbcNetwork.attributeDefinitions.find("SPN");
    if (c_IterSpnType != orc_DbcNetwork.attributeDefinitions.end()) {
      if (c_IterSpnType->second.valueType.type ==
          Vector::DBC::AttributeValueType::Type::Int) {
        orc_Signal.u32_J1939Spn =
            static_cast<uint32_t>(c_IterSpn->second.integerValue);
      } else if (c_IterSpnType->second.valueType.type ==
                 Vector::DBC::AttributeValueType::Type::Float) {
        orc_Signal.u32_J1939Spn =
            static_cast<uint32_t>(c_IterSpn->second.floatValue);
      } else if (c_IterSpnType->second.valueType.type ==
                 Vector::DBC::AttributeValueType::Type::Hex) {
        orc_Signal.u32_J1939Spn =
            static_cast<uint32_t>(c_IterSpn->second.hexValue);
      } else if (c_IterSpnType->second.valueType.type ==
                 Vector::DBC::AttributeValueType::Type::Enum) {
        orc_Signal.u32_J1939Spn =
            static_cast<uint32_t>(c_IterSpn->second.enumValue);
      } else {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Verify if all signal values are inside of the signal value range, if
   not: remove

   \param[in,out]  orc_DbcSignal    Signal for which to clean up the value
   tables
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieImportDbc::mh_VerifySignalValueTable(
    C_CieConverter::C_CieCanSignal &orc_DbcSignal) {
...
/*! \brief    Get signal values and type
   ...
   \param[in,out]  orq_SignalAdapted      flag if signal was adapted, will not
   be reset to false
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportDbc::mh_GetSignalValues(
    const Vector::DBC::Network &orc_DbcNetwork,
  ...
  Vector::DBC::Signal c_DbcSignal =
      orc_DbcSignal; // remove const binding because of minimumRawValue() and
                     // maximumRawValue()

  C_OscNodeDataPoolContent::E_Type
      e_CurrentType; // guarantees the same type for min, max and initial value
  ...
  float64_t f64_MinValuePhy; // used to display warning messages with physical
                             // values to user
  ...
  if (oq_MultiplexerSignal == true) {
    // Multiplexer signal restrictions
    if ((c_DbcSignal.extendedValueType !=
         Vector::DBC::Signal::ExtendedValueType::Integer) &&
        (c_DbcSignal.extendedValueType !=
         Vector::DBC::Signal::ExtendedValueType::Undefined)) {
      c_DbcSignal.extendedValueType =
          Vector::DBC::Signal::ExtendedValueType::Integer;
      orq_SignalAdapted = true;
    }
  ...
  // extendedValueType is currently set only in case of variable type float or
  // double
  if ((c_DbcSignal.extendedValueType ==
       Vector::DBC::Signal::ExtendedValueType::Integer) ||
      (c_DbcSignal.extendedValueType ==
    ...
    if (c_DbcSignal.bitSize <= 64U) {
      ...
      f64_MinValuePhy =
          (f64_MinValue * c_DbcSignal.factor) + c_DbcSignal.offset;
      f64_MaxValuePhy =
          (f64_MaxValue * c_DbcSignal.factor) + c_DbcSignal.offset;

      osc_write_log_warning("DBC file import",
                            "Unknown value type error for signal \"" +
                                c_String + "\".");
      orc_WarningMessages.append("Unknown value type error for signal \"" +
                                 c_String + "\".");
      s32_Return = C_WARN;
    }
  } else if (c_DbcSignal.extendedValueType ==
    ...
    osc_write_log_warning("DBC file import",
                          "Unknown value type error for signal \"" + c_String +
                              "\".");
    orc_WarningMessages.append("Unknown value type error for signal \"" +
                               c_String + "\".");
    s32_Return = C_WARN;
  ...
  if (mh_CheckRange(f64_MinValue, e_CurrentType) != C_NO_ERR) {
    const QString c_Message =
        ...
        "\". Correct datatype with \"Start Bit\" and \"Length\" has to be set "
        "manually.";
  ...
  C_OscNodeDataPoolContentUtil::h_SetValueInContent(f64_MinValue,
                                                    orc_Element.c_MinValue);
  C_OscNodeDataPoolContentUtil::h_SetValueInContent(f64_MaxValue,
                                                    orc_Element.c_MaxValue);

  if (oq_MultiplexerSignal == true) {
    C_SdNdeDpContentUtil::h_InitMinForSignal(
        orc_Element.c_MinValue, static_cast<uint16_t>(c_DbcSignal.bitSize));
    C_SdNdeDpContentUtil::h_InitMaxForSignal(
        orc_Element.c_MaxValue, static_cast<uint16_t>(c_DbcSignal.bitSize));
  }
  ...
  if (c_Iter != c_DbcSignal.attributeValues.end()) {
    ...
    const std::map<std::string,
                   Vector::DBC::AttributeDefinition>::const_iterator
        c_IterType =
            orc_DbcNetwork.attributeDefinitions.find(c_StartValue.name);
    if (c_IterType != orc_DbcNetwork.attributeDefinitions.end()) {
      ...
      if (c_IterType->second.valueType.type ==
          Vector::DBC::AttributeValueType::Type::Int) {
        ...
        f64_InitialValue = c_StartValue.floatValue;
                 ...
                 Vector::DBC::AttributeValueType::Type::Hex) // has the same
                                                             // representation
      // as int
      {
        ...
        const float64_t f64_DefaultPhy =
            (f64_DEFAULT * c_DbcSignal.factor) + c_DbcSignal.offset;
        const QString c_Message =
            "Signal \"" + c_String +
            "\": Type for initial value unknown. Initial value set to default "
            "value \"" +
      ...
      if ((oq_MultiplexerSignal == true) &&
          (C_OscUtils::h_IsFloat64NearlyEqual(f64_InitialValue, 0.0) ==
           false)) {
        // Multiplexer signal restrictions
      ...
      f64_InitialValuePhy =
          (f64_InitialValue * c_DbcSignal.factor) + c_DbcSignal.offset;
      ...
      if (mh_CheckRange(f64_InitialValue, e_CurrentType) != C_NO_ERR) {
        const QString c_Message =
            ...
            "\"" +
            QString::number(f64_InitialValue) +
      ...
      C_OscNodeDataPoolContentUtil::h_SetValueInContent(f64_InitialValue,
                                                        c_InitialValue);
      // set value in range, leave initial value if in range
      C_OscNodeDataPoolContentUtil::E_ValueChangedTo e_ValueChangedTo;
      const int32_t s32_Tmp =
          C_OscNodeDataPoolContentUtil::h_SetValueInMinMaxRange(
              orc_Element.c_MinValue, orc_Element.c_MaxValue, c_InitialValue,
              e_ValueChangedTo, C_OscNodeDataPoolContentUtil::eLEAVE_VALUE);
      if (s32_Tmp == C_RANGE) {
        // min and max values are interchanged
        // rare case with no practical scenario yet
        // do not know how to handle this better than leave all values as they
    ...
    // Hint: No range check for global or default initial raw value datatype of
    // DBC signal
    //       because range conflicts can happen very often in dependence of DBC
    //       file.

    if (oq_MultiplexerSignal == false) {
      // set initial value to default
      C_OscNodeDataPoolContentUtil::h_SetValueInContent(f64_DEFAULT,
                                                        c_InitialValue);
    } else {
      // Multiplexer signal -> restriction to zero but no warning due to no real
      // change set initial value to zero
    ...
    if (s32_Tmp == C_RANGE) {
      ...
      if (e_ValueChangedTo == C_OscNodeDataPoolContentUtil::eNO_CHANGE) {
        f64_InitialValuePhy =
            (f64_InitialValue * c_DbcSignal.factor) + c_DbcSignal.offset;
        if (mhq_DefaultValueDefined ==
            false) // true is standard case; if false, then openSYDE default
                   // value is used
        {
          const QString c_Message =
        ...
        // initial value was not in min and max range, took closer min or max
        // value
        QString c_PlaceholderMinMax;
        ...
        if (mhq_DefaultValueDefined == true) {
          ...
          const QString c_Message =
              "No valid initial value for signal \"" + c_String +
              "\" available. Initial value set to " + c_PlaceholderMinMax +
              " value \"" + c_PlaceholderValue + "\".";
          osc_write_log_warning("DBC file import", c_Message);
          orc_WarningMessages.append(c_Message);
        }
  ...
  // check if raw value of DBC signal are in range for data type after it was
  // set automatically
  if (mh_CheckRange(f64_InitialValue, e_CurrentType) != C_NO_ERR) {
    const QString c_Message =
        "Signal \"" + c_String +
        "\": violation of datatype range in DBC file for initial raw value \"" +
        QString::number(f64_InitialValue) +
        "\" after it was set automatically. Correct datatype with \"Start "
        "Bit\" and \"Length\""
        " has to be set manually.";
    osc_write_log_warning("DBC file import", c_Message);
    orc_WarningMessages.append(c_Message);
  ...
  orc_Element.c_DataSetValues.push_back(
      c_InitialValue); // init value is the first and only element of
                       // c_DataSetValues
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportDbc::mh_GetAttributeDefinitions(
    const Vector::DBC::Network &orc_DbcNetwork) {
  int32_t s32_Return = C_NO_ERR;
  ...
  if (c_DbcAttributeDefinition != orc_DbcNetwork.attributeDefinitions.end()) {
    ...
    s32_Return = C_WARN;
    mhc_WarningMessages.append("Missing attribute definition for send type \"" +
                               mhc_SEND_TYPE + "\".");
    osc_write_log_warning("DBC file import",
                          "Missing attribute definition for send type \"" +
                              mhc_SEND_TYPE + "\".");
  }
  ...
  if (c_DbcAttributeDefaults != orc_DbcNetwork.attributeDefaults.end()) {
    ...
    mhc_WarningMessages.append(
        "Missing default attribute definition for send type \"" +
        mhc_SEND_TYPE +
        "\"."
        " Set to \"" +
        mhc_DefaultSendTypeValue + "\".");
  ...
  if (c_DbcAttributeDefaultsInit != orc_DbcNetwork.attributeDefaults.end()) {
    ...
    if (c_IterType != orc_DbcNetwork.attributeDefinitions.end()) {
      if (c_IterType->second.valueType.type ==
          Vector::DBC::AttributeValueType::Type::Int) {
        mhf32_DefaultInitialValue = static_cast<float32_t>(
            (c_DbcAttributeDefaultsInit->second).integerValue);
        mhq_DefaultValueDefined = true;
      } else if (c_IterType->second.valueType.type ==
                 Vector::DBC::AttributeValueType::Type::Hex) {
        mhf32_DefaultInitialValue = static_cast<float32_t>(
            (c_DbcAttributeDefaultsInit->second).hexValue);
        mhq_DefaultValueDefined = true;
      } else if (c_IterType->second.valueType.type ==
                 Vector::DBC::AttributeValueType::Type::Float) {
        mhf32_DefaultInitialValue = static_cast<float32_t>(
            (c_DbcAttributeDefaultsInit->second).floatValue);
        mhq_DefaultValueDefined = true;
      } else if (c_IterType->second.valueType.type ==
                 Vector::DBC::AttributeValueType::Type::Enum) {
        mhf32_DefaultInitialValue = static_cast<float32_t>(
            (c_DbcAttributeDefaultsInit->second).enumValue);
        mhq_DefaultValueDefined = true;
      } else if (c_IterType->second.valueType.type ==
        ...
        // strange, print a warning to user
        s32_Return = C_WARN;
        mhc_WarningMessages.append("Default type value for signals \"" +
                                   mhc_SEND_TYPE +
                                   "\". Default value set to \"0\".");
        osc_write_log_warning("DBC file import",
                              "Default type value for signals \"" +
                                  mhc_SEND_TYPE +
                                  "\". Default value set to \"0\".");
      }
    } else {
      ...
      s32_Return = C_WARN;
      mhc_WarningMessages.append(
          "Default definition for signals \"" +
          QString::fromStdString(c_DbcAttributeDefaultsInit->second.name) +
...
/*! \brief    Get transmission type and time
   ...
   structure.
   Hint: If the default value in DBC shall used, may be the respective entry in
   .dbc file is missing. Thus the default value is used. (Option: don't accept
   default values but set the parameters for each message explicitly)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieImportDbc::mh_GetTransmission(
    const Vector::DBC::Network &orc_DbcNetwork,
    const Vector::DBC::Message &orc_DbcMessage,
    C_CieConverter::C_CieNodeMessage &orc_Message) {
  QString c_MessageType;
  ...
  if (mhc_DefaultSendTypeValue.toLower().contains("cyclic")) {
    orc_Message.c_CanMessage.e_TxMethod =
        C_OscCanMessage::E_TxMethodType::eTX_METHOD_CYCLIC;
  } else {
    orc_Message.c_CanMessage.e_TxMethod =
        C_OscCanMessage::E_TxMethodType::eTX_METHOD_ON_EVENT;
  }

  // search for attribute send type (keyword 'cyclic' shall always indicate
  // openSYDE method eTX_METHOD_CYCLIC, otherwise it shall be of type
  // eTX_METHOD_ON_EVENT)
  for (const auto &rc_DbcAttributeValue : orc_DbcMessage.attributeValues) {
  ...
  if ((orc_Message.c_CanMessage.e_TxMethod ==
       C_OscCanMessage::E_TxMethodType::eTX_METHOD_CYCLIC) &&
      (q_CycleTimeFound == false)) {
    const QString c_Message =
        "Message cycle time was 0ms or not valid, default set to " +
        QString::number(orc_Message.c_CanMessage.u32_CycleTimeMs) + "ms.";

    osc_write_log_warning("DBC file import", c_Message);
    orc_Message.c_Warnings.append(c_Message);
  } else if (orc_Message.c_CanMessage.e_TxMethod ==
             C_OscCanMessage::E_TxMethodType::eTX_METHOD_ON_EVENT) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportDbc::mh_CheckRange(
  ...
  if ((oe_Datatype == C_OscNodeDataPoolContent::eUINT8) &&
    ...
    s32_Return = C_RANGE;
             ...
             ((of64_Value < -std::pow(2.0, 63.0)) ||
              (of64_Value >= std::pow(2.0, 63.0)))) {
    s32_Return = C_RANGE;
  } else {
    // OK, we do not check for float range because there should be no case in
    // practice
    s32_Return = C_NO_ERR;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CieImportDbc::mh_ReEscapeCriticalSymbols(const QString &orc_String) {
  QString c_Temp(orc_String);
