...
public:
  ...
  static void h_ReportEdsImportError(QWidget *const opc_ParentWidget,
                                     const int32_t os32_ImportResult,
                                     const QString &orc_ParsingError,
                                     const uint8_t ou8_NodeId);
  static QString h_GetMessageName(
      const stw::opensyde_core::C_OscNode &orc_Node, const bool oq_IsTx,
...
private:
  static int32_t mh_ImportDbcFile(
      const uint32_t ou32_BusIndex,
      const stw::opensyde_core::C_OscCanProtocol::E_Type oe_ProtocolType,
