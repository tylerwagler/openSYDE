 ...
 * ---------------------------------------------------------------------------------------
 */
static const QString mc_MessageLineBreak =
...
/*! \brief   Convert DBC import data structure to openSYDE data structure.
   ...
   Assumptions:
   * consistent data (guaranteed by DBC import mechanism, this is no customer
   API function)
...
C_CieDataPoolListAdapter::h_GetStructureFromDbcFileImport(
  ...
  mh_FillUpCoreStructureByDbcValues(
      orc_CieNode.c_TxMessages, c_DataStructure.c_Core.c_OscTxMessageData,
      c_DataStructure.c_Core.c_OscTxSignalData,
      c_DataStructure.c_Core.c_WarningMessagesPerTxMessage);

  // get Rx core data
  mh_FillUpCoreStructureByDbcValues(
      orc_CieNode.c_RxMessages, c_DataStructure.c_Core.c_OscRxMessageData,
...
C_CieDataPoolListAdapter::h_GetStructureFromDcfAndEdsFileImport(
  ...
  c_Retval.c_Core.c_EdsOscRxMessageIsSrdo =
      orc_OscRxMessageData.c_MessageIsSrdo;
  c_Retval.c_Core.c_EdsOscTxMessageIsSrdo =
      orc_OscTxMessageData.c_MessageIsSrdo;
  ...
  Q_ASSERT(orc_InfoMessagesPerMessage.size() ==
           (orc_OscRxMessageData.c_OscMessageData.size() +
            orc_OscTxMessageData.c_OscMessageData.size()));
  if (orc_InfoMessagesPerMessage.size() ==
      (orc_OscRxMessageData.c_OscMessageData.size() +
       orc_OscTxMessageData.c_OscMessageData.size())) {
    c_Retval.c_Core.c_WarningMessagesPerRxMessage.reserve(
        orc_OscRxMessageData.c_OscMessageData.size());
    c_Retval.c_Core.c_WarningMessagesPerTxMessage.reserve(
        orc_OscTxMessageData.c_OscMessageData.size());
    for (uint32_t u32_ItInfoMessage = 0;
      ...
      if (u32_ItInfoMessage < orc_OscRxMessageData.c_OscMessageData.size()) {
        ...
        c_Retval.c_Core.c_WarningMessagesPerTxMessage.push_back(
            c_CombinedMessages);
      }
    }
  }
  ...
  mh_FillUpUiStructure(c_Retval, true,
                       &orc_OscRxMessageData.c_SignalDefaultMinMaxValuesUsed,
...
void C_CieDataPoolListAdapter::mh_FillUpCoreStructureByDbcValues(
  ...
  for (c_CanMessageIter = orc_CieNodeMessages.begin();
    ...
    // restore warnings
    if (c_CanMessageIter->c_Warnings.count() > 0) {
      QString c_Tmp;
      for (uint32_t u32_Pos = 0; u32_Pos < c_CanMessageIter->c_Warnings.count();
           u32_Pos++) {
    ...
    c_CanMessage.u32_CycleTimeMs =
        c_CanMessageIter->c_CanMessage.u32_CycleTimeMs;

    if (c_CanMessage.e_TxMethod != C_OscCanMessage::eTX_METHOD_ON_EVENT) {
      c_CanMessage.u32_TimeoutMs = C_PuiSdUtil::h_GetMessageAutoTimeoutTime(
          c_CanMessage.u32_CycleTimeMs);
    } else {
    ...
    for (c_CanMessageSignalsIter =
         ...
         c_CanMessageSignalsIter !=
         c_CanMessageIter->c_CanMessage.c_Signals.end();
         ++c_CanMessageSignalsIter) {
      ...
      c_CanSignal.e_MultiplexerType =
          c_CanMessageSignalsIter->e_MultiplexerType;
      c_CanSignal.u16_MultiplexValue =
          c_CanMessageSignalsIter->u16_MultiplexValue;
      c_CanSignal.u32_J1939SuspectParameterNumber =
          c_CanMessageSignalsIter->u32_J1939Spn;
      u32_SignalIndex++;
      // store signal in CAN message structure
      c_CanMessage.c_Signals.push_back(c_CanSignal);

      // store further signal data for data pool list elements in separate
      // signal container
      const C_CieConverter::C_CieDataPoolElement *const pc_Element =
          &(c_CanMessageSignalsIter->c_Element);
      ...
      // set initial value which is the first and only element of
      // c_DataSetValues
      c_DataPoolListElement.c_DataSetValues.push_back(
          pc_Element->c_DataSetValues.at(0));

      // placeholder variables, we must set the type and array
      c_DataPoolListElement.c_Value.SetType(
          pc_Element->c_MinValue.GetType()); // all elements have the same type
      c_DataPoolListElement.c_Value.SetArray(false);
      c_DataPoolListElement.c_NvmValue.SetType(
          pc_Element->c_MinValue.GetType()); // all elements have the same type
...
/*! \brief   Internal adapter function to fill up data for Ui functionality.
   ...
   This function handles TX methods for messages and calls
   mh_FillUpUiStructureForSignal for signals.

   \param[in,out]  orc_DataPoolListStructure           openSYDE data structure
   for data pool lists
   \param[in,out]  oq_ActivateAutoMinMaxForSignals     Flag to automatically set
   the auto min max flag if necessary
   \param[in]      opc_RxSignalDefaultMinMaxValuesUsed Optional flags if
   imported core Rx signal data uses the default min max values or or specific
   set values
   \param[in]      opc_TxSignalDefaultMinMaxValuesUsed Optional flags if
   imported core Tx signal data uses the default min max values or or specific
   set values
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieDataPoolListAdapter::mh_FillUpUiStructure(
  ...
  for (c_MessageIter =
       ...
       c_MessageIter !=
       orc_DataPoolListStructure.c_Core.c_OscRxMessageData.end();
       ++c_MessageIter) {
    ...
    if (c_MessageIter->u32_TimeoutMs == 0U) {
      if ((c_MessageIter->e_TxMethod == C_OscCanMessage::eTX_METHOD_ON_EVENT) ||
        ...
        c_UiMessage.e_ReceiveTimeoutMode =
            C_PuiSdNodeCanMessage::eRX_TIMEOUT_MODE_DISABLED;
      } else {
      ...
      if ((c_MessageIter->e_TxMethod == C_OscCanMessage::eTX_METHOD_ON_EVENT) ||
          (c_MessageIter->u32_TimeoutMs !=
           C_PuiSdUtil::h_GetMessageAutoTimeoutTime(
               c_MessageIter->u32_CycleTimeMs))) {
        // Using the specific value as custom value
        c_UiMessage.e_ReceiveTimeoutMode =
            C_PuiSdNodeCanMessage::eRX_TIMEOUT_MODE_CUSTOM;
      } else {
  ...
  orc_DataPoolListStructure.c_Ui.c_UiTxMessageData.reserve(
      orc_DataPoolListStructure.c_Core.c_OscTxMessageData.size());
  for (c_MessageIter =
           orc_DataPoolListStructure.c_Core.c_OscTxMessageData.begin();
       c_MessageIter !=
       orc_DataPoolListStructure.c_Core.c_OscTxMessageData.end();
       ++c_MessageIter) {
    C_PuiSdNodeCanMessage c_UiMessage;
    c_UiMessage.c_Signals.resize(c_MessageIter->c_Signals.size());

    // Adaption of Ui specific timeout mode
    if ((c_MessageIter->e_TxMethod ==
         C_OscCanMessage::eTX_METHOD_CAN_OPEN_TYPE_254) ||
        (c_MessageIter->e_TxMethod ==
         C_OscCanMessage::eTX_METHOD_CAN_OPEN_TYPE_255)) {
      if (c_MessageIter->u32_TimeoutMs == 0U) {
        // Special case CANopen: 0 means disabled timeout check for on event and
        // the Tx message has the information about receiving timeout too
        c_UiMessage.e_ReceiveTimeoutMode =
            C_PuiSdNodeCanMessage::eRX_TIMEOUT_MODE_DISABLED;
      } else {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Internal adapter function to fill up data for Ui functionality for
   signals.

   Handles adaption of Ui specific auto min/max setting (multiplexed signal
   only)

   \param[in]      orc_Message                        Message that holds the
   signals
   ...
   \param[in]      opu8_SignalDefaultMinMaxValuesUsed Optional flags if imported
   core signal data uses the default min max values or or specific set values
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieDataPoolListAdapter::mh_FillUpUiStructureForSignals(
    const C_OscCanMessage &orc_Message, const bool oq_TxMessage,
  ...
  for (u32_SignalCounter = 0U; u32_SignalCounter < orc_Message.c_Signals.size();
       ++u32_SignalCounter) {
    c_AdaptedUiSignal = orc_DefaultUiSig;
    if (orc_Message.c_Signals[u32_SignalCounter].e_MultiplexerType ==
      ...
      c_AdaptedUiSignal.q_AutoMinMaxActive =
          (opu8_SignalDefaultMinMaxValuesUsed[u32_SignalCounter] == 1U) ? true
                                                                        : false;
    } else {
      // Nothing to do
    }
    ...
    if (oq_TxMessage == true) {
      orc_DataPoolListStructure.c_Ui.c_UiTxSignalData.push_back(
          c_AdaptedUiSignal);
    } else {
      orc_DataPoolListStructure.c_Ui.c_UiRxSignalData.push_back(
          c_AdaptedUiSignal);
    }
  }
}
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Convert openSYDE CAN message data structure to DBC data structure
   ...
   \return
   C_NO_ERR    successful
   C_WARN      message(s) without (correct) signals, see message container with
   warnings
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieDataPoolListAdapter::h_ConvertToDbcImportMessage(
    const uint32_t ou32_BusIndex, const C_OscCanProtocol::E_Type oe_Type,
  ...
  orc_CieNodeMessage.c_CanMessage.u32_CycleTimeMs =
      orc_OscCanMessage.u32_CycleTimeMs;

  // to get the data pool list element for each signal we need the message sync
  // manager
  ...
  if (c_MessageSyncManager.GetMessageIdForMessageName(
    ...
    for (uint32_t u32_PosSignal = 0;
      ...
      const C_OscCanSignal &rc_CurrentOscCanSignal =
          orc_OscCanMessage.c_Signals[u32_PosSignal];

      // fill up data of signal
      // (u32_ComDataElementIndex is not used in DBC files)
      c_CurrentCieSignal.u16_ComBitLength =
          rc_CurrentOscCanSignal.u16_ComBitLength;
      c_CurrentCieSignal.u16_ComBitStart =
          rc_CurrentOscCanSignal.u16_ComBitStart;
      c_CurrentCieSignal.e_ComByteOrder = rc_CurrentOscCanSignal.e_ComByteOrder;
      c_CurrentCieSignal.e_MultiplexerType =
          rc_CurrentOscCanSignal.e_MultiplexerType;
      c_CurrentCieSignal.u16_MultiplexValue =
          rc_CurrentOscCanSignal.u16_MultiplexValue;
      c_CurrentCieSignal.u32_J1939Spn =
          rc_CurrentOscCanSignal.u32_J1939SuspectParameterNumber;
      ...
      const C_OscNodeDataPoolListElement *const pc_OscElement =
          C_PuiSdHandler::h_GetInstance()->GetOscCanDataPoolListElement(
              c_MessageId, u32_PosSignal);
      if (pc_OscElement != NULL) {
        C_CieConverter::C_CieDataPoolElement &rc_CieElement =
            c_CurrentCieSignal.c_Element;
        ...
        // set initial value which is the first and only element of
        // c_DataSetValues
        rc_CieElement.c_DataSetValues.push_back(
            pc_OscElement->c_DataSetValues.at(0));

        // store signal data
        orc_CieNodeMessage.c_CanMessage.c_Signals.push_back(c_CurrentCieSignal);
      } else {
        const QString c_Message =
            "Can't find data elements for signal with position \"" +
            QString::number(u32_PosSignal) + "\" in message \"" +
            orc_OscCanMessage.c_Name + "\" in bus \"" +
            QString::number(ou32_BusIndex) + "\".";
    ...
    const QString c_Message = "Can't find valid signal for message \"" +
                              orc_OscCanMessage.c_Name + "\" in bus \"" +
                              QString::number(ou32_BusIndex) + "\".";
    orc_Warnings.append(c_Message);
    osc_write_log_warning("DBC Export", c_Message);
