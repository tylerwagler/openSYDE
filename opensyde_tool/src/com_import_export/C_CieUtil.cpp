...
/*!
   \file
   \brief       Utility class with functions for comm import export
   (implementation)
...
int32_t C_CieUtil::h_ImportFile(const uint32_t ou32_BusIndex,
  ...
  const QString c_Filter =
      static_cast<QString>("COMM Messages Files") + " (*.dbc *.eds *.dcf)";
  ...
  const QString c_FullFilePath = C_OgeWiUtil::h_GetOpenFileName(
      opc_Parent, "Select COMM Messages File", c_Folder, c_Filter, "*.dbc");
  ...
  if (c_FullFilePath != "") {
    ...
    C_UsHandler::h_GetInstance()->SetProjSdTopologyLastKnownImportPath(
        c_FileInfo.absoluteDir().absolutePath());
    if (c_Extension.contains("dbc") == true) {
      s32_Return = C_CieUtil::mh_ImportDbcFile(
          ou32_BusIndex, oe_ProtocolType, c_FullFilePath, opc_Parent,
          orc_NodeIndexes, orc_InterfaceIndexes);
    } else if ((c_Extension.contains("eds") == true) ||
               (c_Extension.contains("dcf") == true)) {
      s32_Return = C_CieUtil::mh_ImportDcfEdsFile(
          ou32_BusIndex, oe_ProtocolType, c_FullFilePath, opc_Parent,
          orc_NodeIndexes, orc_InterfaceIndexes);
    } else {
      // strange
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieUtil::h_ExportFile(
  ...
  // Load user settings value
  QString c_Folder =
      C_UsHandler::h_GetInstance()->GetProjSdTopologyLastKnownExportPath();
  ...
  // Get file name
  c_FullFilePath = C_OgeWiUtil::h_GetSaveFileName(
      opc_Parent, "Select File for openSYDE DBC Export", c_Folder, c_FilterName,
      c_DefaultFilename);
  ...
  if (c_FullFilePath.compare("") != 0) {
    ...
    const QString c_Extension = c_FileInfo.completeSuffix().toLower();
    // Store new user settings value
    C_UsHandler::h_GetInstance()->SetProjSdTopologyLastKnownExportPath(
        c_FileInfo.absoluteDir().absolutePath());
    if (c_Extension == "dbc") {
      QString c_Error;
      ...
      if ((s32_Return == C_NO_ERR) || (s32_Return == C_WARN)) {
        ...
        if (s32_Tmp == C_NO_ERR) {
          const uint32_t u32_NumOfOutputNodes =
              static_cast<uint32_t>(c_NodeMapping.size());
          ...
          if (s32_Tmp == C_NO_ERR) {
            const uint32_t u32_NumOfOutputMessages =
                c_ExportStatistic.u32_NumOfMessages;
            const uint32_t u32_NumOfOutputSignals =
                c_ExportStatistic.u32_NumOfSignals;
          ...
          C_CieExportReportWidget *const pc_DialogExportReport =
              new C_CieExportReportWidget(*c_PopUpDialogReportDialog,
                                          c_FullFilePath);

          pc_DialogExportReport->SetMessageData(c_NodeMapping,
                                                c_ExportStatistic, c_Warnings);

          // resize
          c_PopUpDialogReportDialog->SetSize(mc_POPUP_REPORT_SIZE);

          // display message report
          if (c_PopUpDialogReportDialog->exec() ==
        ...
        } // lint !e429  //no memory leak because of the parent of pc_Dialog and
          // the Qt memory management
      } else {
        // display error
        C_OgeWiCustomMessage c_ExportError(
            opc_Parent, C_OgeWiCustomMessage::E_Type::eERROR);
      ...
      c_Message.SetDescription(
          "The specified file has the wrong extension, use: \".dbc\".");
      c_Message.SetDetails(static_cast<QString>("Invalid extension: \"%1\"")
                               .arg("." + c_Extension));
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieUtil::h_AdaptName(QString &orc_Name, QString &orc_Comment,
  ...
  if (c_NewName.length() >
    ...
    c_NewName.resize(C_PuiSdHandler::h_GetInstance()->GetNameMaxCharLimit());
  }
  ...
  if ((orc_Name == c_NewName) && (oq_AlwaysAppendNameInComment == false)) {
    ...
    // Translation: 1=Original name from file,2=Optional line break if there was
    // some more content
    orc_Comment +=
...
void C_CieUtil::h_InsertMessages(
  ...
  for (uint32_t u32_ItNodes = 0; u32_ItNodes < orc_ImportDataAssignment.size();
       u32_ItNodes++) {
    const C_CieImportDataAssignment &rc_CurData =
        orc_ImportDataAssignment[u32_ItNodes];

    Q_ASSERT(rc_CurData.s32_DatapoolIndexForNew >= 0);
    if (rc_CurData.s32_DatapoolIndexForNew >= 0) {
      // Insert Tx messages
      int32_t s32_Retval = mh_InsertMessages(
          rc_CurData.u32_OsyNodeIndex, oe_ProtocolType,
          rc_CurData.u32_OsyInterfaceIndex, rc_CurData.s32_DatapoolIndexForNew,
          true, rc_CurData.c_ImportData.c_Core.c_OscTxMessageData,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CieUtil::h_GetDeviceInfo(const uint32_t ou32_DeviceNodeIndex,
                           const uint32_t ou32_DeviceNodeInterfaceIndex,
                           const uint32_t ou32_ManagerNodeIndex,
                           const uint8_t ou8_ManagerNodeInterfaceNumber,
                           const QString &orc_EdsPath,
                           C_CieImportDataAssignment &orc_NodeAssignment,
                           C_CieImportDataAssignment &orc_InvalidNodeAssignment,
                           QString &orc_ParsingError) {
  int32_t s32_Retval = C_RANGE;
  const C_OscNode *const pc_Node =
      C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(ou32_DeviceNodeIndex);

  Q_ASSERT(pc_Node != NULL);
  if (pc_Node != NULL) {
    Q_ASSERT(ou32_DeviceNodeInterfaceIndex <
             pc_Node->c_Properties.c_ComInterfaces.size());
    if (ou32_DeviceNodeInterfaceIndex <
        pc_Node->c_Properties.c_ComInterfaces.size()) {
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if (C_PuiSdHandler::h_GetInstance()->TranslateCanInterfaceNumberToIndex(
                ou32_ManagerNodeIndex, ou8_ManagerNodeInterfaceNumber,
        ...
        orc_NodeAssignment.c_ImportData =
            C_CieDataPoolListAdapter::h_GetStructureFromDcfAndEdsFileImport(
                c_OscRxMessageData, c_OscTxMessageData,
                c_ImportMessagesPerMessage);
        orc_InvalidNodeAssignment.c_ImportData =
            C_CieDataPoolListAdapter::h_GetStructureFromDcfAndEdsFileImport(
                c_InvalidOscRxMessageData, c_InvalidOscTxMessageData,
                c_InvalidImportMessagesPerMessage);
        if (C_PuiSdHandler::h_GetInstance()->TranslateCanInterfaceIndexToId(
                ou32_DeviceNodeIndex, ou32_DeviceNodeInterfaceIndex,
                u8_InterfaceId) == C_NO_ERR) {
          const C_OscCanInterfaceId c_Id(ou32_DeviceNodeIndex, u8_InterfaceId);
          C_CieDataPoolListAdapter::h_AssignNode(
              c_Id, orc_NodeAssignment.c_ImportData.c_Core.c_OscRxMessageData);
          C_CieDataPoolListAdapter::h_AssignNode(
              c_Id, orc_NodeAssignment.c_ImportData.c_Core.c_OscTxMessageData);
          C_CieDataPoolListAdapter::h_AssignNode(
              c_Id,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieUtil::h_ReportEdsImportError(QWidget *const opc_ParentWidget,
                                       const int32_t os32_ImportResult,
                                       const QString &orc_ParsingError,
                                       const uint8_t ou8_NodeId) {
  C_OgeWiCustomMessage c_Message(opc_ParentWidget,
                                 C_OgeWiCustomMessage::E_Type::eERROR);
  ...
  case C_NOACT:
    c_Message.SetDescription(
        static_cast<QString>("EDS file import failed. Node ID %1 is invalid.")
            .arg(ou8_NodeId));
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_CieUtil::h_GetMessageName(const C_OscNode &orc_Node, const bool oq_IsTx,
                            const uint32_t ou32_MessageIndex,
                            const C_OscCanProtocol::E_Type oe_ProtocolType,
                            const QString *const opc_NodeNameReplacement,
                            const bool oq_IsSrdo, uint32_t &oru32_SrdoIndex) {
  QString c_Retval;

  QString c_Nodename;
  QString c_TpdoRpdo = "RPDO";
  if (opc_NodeNameReplacement != NULL) {
    c_Nodename = *opc_NodeNameReplacement;
  } else {
    c_Nodename = orc_Node.c_Properties.c_Name;
  }
  if (c_Nodename.length() >
      (C_PuiSdHandler::h_GetInstance()->GetNameMaxCharLimit() - 7UL)) {
    c_Nodename = c_Nodename.mid(
        1, C_PuiSdHandler::h_GetInstance()->GetNameMaxCharLimit() - 7UL);
  }
  ...
  if (oq_IsSrdo) {
    c_TpdoRpdo = "SRDO";
    c_Retval = C_CieUtil::mh_ConstructMessageName(c_Nodename, c_TpdoRpdo,
                                                  oru32_SrdoIndex);
    ++oru32_SrdoIndex;
  } else {
    c_Retval = C_CieUtil::mh_ConstructMessageName(c_Nodename, c_TpdoRpdo,
...
bool C_CieUtil::h_GetIsSrdoFromMessageIsSrdoVector(
  ...
  if ((ou32_MessageIndex < orc_MessageIsSrdo.size()) &&
      (orc_MessageIsSrdo[ou32_MessageIndex] == 1U)) {
    q_IsSrdo = true;
  } else {
    q_IsSrdo = false;
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CieUtil::mh_ImportDbcFile(const uint32_t ou32_BusIndex,
                            const C_OscCanProtocol::E_Type oe_ProtocolType,
  ...
  QApplication::setOverrideCursor(
      Qt::WaitCursor);           // big DBC file can take some time to load
  QApplication::processEvents(); // update cursor
  s32_ImportReturn = C_CieImportDbc::h_ImportNetwork(
      orc_FullFilePath, c_CommDef, c_WarningMessages, c_ErrorMessage, true);
  QApplication::restoreOverrideCursor(); // get old cursor again
  QApplication::processEvents();         // update cursor

  if ((s32_ImportReturn == stw::errors::C_NO_ERR) ||
      (s32_ImportReturn == stw::errors::C_WARN)) {
    if ((c_CommDef.c_Nodes.size() == 0UL) &&
        (c_CommDef.c_UnmappedMessages.size() == 0UL))

    {
      C_OgeWiCustomMessage c_NodesError(opc_Parent,
                                        C_OgeWiCustomMessage::eERROR);
      c_NodesError.SetHeading("DBC file import");
      c_NodesError.SetDescription(
          "No nodes or messages found in selected DBC file.");
      c_NodesError.SetDetails(
          static_cast<QString>(
              "%1<a href=\"file:%2\"><span style=\"color: %3;\">%4</span></a>.")
      ...
      if ((s32_ImportReturn == stw::errors::C_WARN) &&
          (c_WarningMessages.count() > 0)) {
        ...
        C_OgeWiCustomMessage c_ImportWarnings(
            opc_Parent, C_OgeWiCustomMessage::E_Type::eWARNING);
        c_ImportWarnings.SetHeading("DBC file import");
        c_ImportWarnings.SetDescription(
            "Warnings occurred during DBC file import.");
      ...
      C_CieDbcImportNodeAssignmentWidget *const pc_DialogNodeSelection =
          new C_CieDbcImportNodeAssignmentWidget(*c_PopUpDialogNodeAssignment,
                                                 orc_FullFilePath,
                                                 ou32_BusIndex, c_CommDef);
      ...
      if (c_PopUpDialogNodeAssignment->exec() ==
        ...
        for (c_It = c_DbcNodeAssignment.begin();
          ...
          c_NodeAssignmentConverted.c_ImportData =
              C_CieDataPoolListAdapter::h_GetStructureFromDbcFileImport(
                  rc_CurrentAssignment.c_CieNode);
          if ((rc_CurrentAssignment.s32_AssignedOsyNodeIndex >= 0) &&
              (rc_CurrentAssignment.s32_AssignedOsyInterfaceIndex >= 0)) {
            c_NodeAssignmentConverted.u32_OsyNodeIndex = static_cast<uint32_t>(
                rc_CurrentAssignment.s32_AssignedOsyNodeIndex);
            c_NodeAssignmentConverted.u32_OsyInterfaceIndex =
                static_cast<uint32_t>(
                    rc_CurrentAssignment.s32_AssignedOsyInterfaceIndex);

            // add to output to forward information about which nodes should get
            // active
            orc_NodeIndexes.push_back(
                c_NodeAssignmentConverted.u32_OsyNodeIndex);
            orc_InterfaceIndexes.push_back(
                c_NodeAssignmentConverted.u32_OsyInterfaceIndex);
          } else {
            Q_ASSERT(false);
          }
        ...
        C_CieImportReportWidget *const pc_DialogImportReport =
            new C_CieImportReportWidget(
                *c_PopUpDialogReportDialog, orc_FullFilePath, ou32_BusIndex,
                oe_ProtocolType, c_NodeAssignmentsConverted,
                c_SkippedImportDataAssigned, NULL);
    ...
    const QString c_ErrorMsg = c_ErrorMessage;
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CieUtil::mh_ImportDcfEdsFile(const uint32_t ou32_BusIndex,
                               const C_OscCanProtocol::E_Type oe_ProtocolType,
                               const QString &orc_FullFilePath,
                               QWidget *const opc_Parent,
                               std::vector<uint32_t> &orc_NodeIndexes,
  ...
  C_CieDcfEdsImportNodeSelectWidget *const pc_DialogNodeSelection =
      new C_CieDcfEdsImportNodeSelectWidget(*c_PopUpDialog, orc_FullFilePath,
                                            ou32_BusIndex);
  ...
  if (c_PopUpDialog->exec() == static_cast<int32_t>(QDialog::Accepted)) {
    ...
    Q_ASSERT(pc_DialogNodeSelection->GetNodeSelection(
                 u32_NodeIndex, u32_InterfaceIndex) == C_NO_ERR);
    pc_DialogNodeSelection->NodeIdToBeChanged(u32_NodeIndex,
                                              u32_InterfaceIndex);
    const C_OscNode *const pc_Node =
    ...
    if (pc_Node != NULL) {
      ...
      if (u32_InterfaceIndex < pc_Node->c_Properties.c_ComInterfaces.size()) {
        ...
        const int32_t s32_ImportResult = C_OscImportEdsDcf::h_Import(
            ...
            c_ParsingError, oe_ProtocolType, c_InvalidOscRxMessageData,
            c_InvalidOscTxMessageData, c_InvalidImportMessagesPerMessage);
        if (s32_ImportResult == C_NO_ERR) {
          ...
          c_NodeAssignment.c_ImportData =
              C_CieDataPoolListAdapter::h_GetStructureFromDcfAndEdsFileImport(
                  c_OscRxMessageData, c_OscTxMessageData,
                  c_ImportMessagesPerMessage);
          if ((c_NodeAssignment.c_ImportData.c_Core.c_OscRxMessageData.size() >
               0UL) ||
            ...
            C_CieImportReportWidget *const pc_Dialog =
                new C_CieImportReportWidget(
                    *c_New, orc_FullFilePath, ou32_BusIndex, oe_ProtocolType,
          ...
          } // lint !e429  //no memory leak because of the parent of pc_Dialog
            // and the Qt memory management
          else {
            C_OgeWiCustomMessage c_Message(opc_Parent);
            c_Message.SetHeading("Nothing found");
            c_Message.SetDescription("No messages found in file.");
            c_Message.SetDetails(
                static_cast<QString>("%1<a href=\"file:%2\"><span "
          ...
          case C_RANGE:
            c_Message.SetDescription("Invalid parameter.");
            c_Message.SetDetails(c_ParsingError);
            c_Message.SetCustomMinHeight(180, 300);
            break;
          case C_NOACT:
            c_Message.SetDescription(
                static_cast<QString>(
                    "EDS file import failed. Node ID %1 is invalid.")
                    .arg(rc_CurInterface.u8_NodeId));
            c_Message.SetDetails(
                "CANopen standard only supports node IDs in the range "
                "of 1 to 127.\nThe node ID can be changed in node "
                "properties.");
            c_Message.SetCustomMinHeight(180, 270);
            break;
...
void C_CieUtil::mh_AdaptMessageNames(
  ...
  for (uint32_t u32_ItNodes = 0; u32_ItNodes < orc_ImportDataAssignment.size();
       u32_ItNodes++) {
    uint32_t u32_Count;
    C_CieImportDataAssignment &rc_CurData =
        orc_ImportDataAssignment[u32_ItNodes];

    // Each message
    for (u32_Count = 0;
         u32_Count < rc_CurData.c_ImportData.c_Core.c_OscRxMessageData.size();
         ++u32_Count) {
      C_OscCanMessage &rc_CurMessage =
          rc_CurData.c_ImportData.c_Core.c_OscRxMessageData[u32_Count];
      C_CieUtil::h_AdaptName(rc_CurMessage.c_Name, rc_CurMessage.c_Comment,
                             oq_AlwaysAppendNameInComment);
    }
    for (u32_Count = 0;
         u32_Count < rc_CurData.c_ImportData.c_Core.c_OscTxMessageData.size();
         ++u32_Count) {
      C_OscCanMessage &rc_CurMessage =
          rc_CurData.c_ImportData.c_Core.c_OscTxMessageData[u32_Count];
      C_CieUtil::h_AdaptName(rc_CurMessage.c_Name, rc_CurMessage.c_Comment,
                             oq_AlwaysAppendNameInComment);
    }
    // Each signal
    for (u32_Count = 0;
         u32_Count < rc_CurData.c_ImportData.c_Core.c_OscRxSignalData.size();
         ++u32_Count) {
      C_OscNodeDataPoolListElement &rc_CurSignal =
          rc_CurData.c_ImportData.c_Core.c_OscRxSignalData[u32_Count];
      C_CieUtil::h_AdaptName(rc_CurSignal.c_Name, rc_CurSignal.c_Comment,
                             oq_AlwaysAppendNameInComment);
    }
    for (u32_Count = 0;
         u32_Count < rc_CurData.c_ImportData.c_Core.c_OscTxSignalData.size();
         ++u32_Count) {
      C_OscNodeDataPoolListElement &rc_CurSignal =
          rc_CurData.c_ImportData.c_Core.c_OscTxSignalData[u32_Count];
      C_CieUtil::h_AdaptName(rc_CurSignal.c_Name, rc_CurSignal.c_Comment,
                             oq_AlwaysAppendNameInComment);
    }
  }
...
void C_CieUtil::mh_AdaptMessagesToProtocolType(
  ...
  for (uint32_t u32_ItNodes = 0; u32_ItNodes < orc_ImportDataAssignment.size();
    ...
    for (uint32_t u32_ItMessage = 0;
         u32_ItMessage <
         rc_CurData.c_ImportData.c_Core.c_OscRxMessageData.size();
         ++u32_ItMessage) {
      QStringList c_Infos;
      QString &rc_CurInfoMessage =
          rc_CurData.c_ImportData.c_Core
              .c_WarningMessagesPerRxMessage[u32_ItMessage];
      C_OscCanMessage &rc_CurMessage =
      ...
      C_SdUtil::h_AdaptMessageToProtocolType(
          ...
          rc_CurData.c_ImportData.c_Ui.c_UiRxSignalData, oe_ProtocolType,
          &c_Infos);
      // Add info
    ...
    for (uint32_t u32_ItMessage = 0;
         u32_ItMessage <
         rc_CurData.c_ImportData.c_Core.c_OscTxMessageData.size();
         ++u32_ItMessage) {
      QStringList c_Infos;
      QString &rc_CurInfoMessage =
          rc_CurData.c_ImportData.c_Core
              .c_WarningMessagesPerTxMessage[u32_ItMessage];
      C_OscCanMessage &rc_CurMessage =
...
/*! \brief   Insert all specified messages into the specified message container
   ...
   \param[in]  orc_MessageOverrideIndices       Indices indicating if message
   will get replaced or will be added
   \param[in]  oq_UniqueAddRequested            Flag to add messages unique
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieUtil::mh_InsertMessages(
  ...
  if (((orc_OscMessageData.size() == orc_MessageOverrideIndices.size()) &&
    ...
    if (pc_Node != NULL) {
      for (uint32_t u32_ItProtocol = 0;
           u32_ItProtocol < pc_Node->c_ComProtocols.size(); ++u32_ItProtocol) {
        const C_OscCanProtocol &rc_ComProtocol =
            pc_Node->c_ComProtocols[u32_ItProtocol];
        if (rc_ComProtocol.u32_DataPoolIndex < pc_Node->c_DataPools.size()) {
          const C_OscNodeDataPool &rc_CurDataPool =
              pc_Node->c_DataPools[rc_ComProtocol.u32_DataPoolIndex];
          pc_CurList = C_OscCanProtocol::h_GetComListConst(
              rc_CurDataPool, ou32_InterfaceIndex, oq_MessagesAreTx);
        }
      }
    }
    for (uint32_t u32_ItMessage = 0; u32_ItMessage < orc_OscMessageData.size();
         ++u32_ItMessage) {
      ...
      for (uint32_t u32_ItSignal = 0;
           u32_ItSignal < rc_CurMessage.c_Signals.size(); ++u32_ItSignal) {
        const C_OscCanSignal &rc_CurSignal =
            rc_CurMessage.c_Signals[u32_ItSignal];
        if (rc_CurSignal.u32_ComDataElementIndex < orc_OscSignalData.size()) {
          C_OscNodeDataPoolListElement c_Copy =
              orc_OscSignalData[rc_CurSignal.u32_ComDataElementIndex];
          if (pc_CurList != NULL) {
            ...
            c_Copy.c_DataSetValues.resize(pc_CurList->c_DataSets.size(),
                                          c_DataSet);
          }
          c_CurOscSignalCommons.push_back(c_Copy);
          c_CurUiSignalCommons.push_back(
              orc_UiSignalData[rc_CurSignal.u32_ComDataElementIndex]);
        }
      }
      if (orc_MessageOverrideIndices[u32_ItMessage].second < 0L) {
        // Insert new messages
        if (C_PuiSdHandler::h_GetInstance()->AddCanMessage(
                ou32_NodeIndex, oe_Type, ou32_InterfaceIndex,
                ou32_DatapoolIndexForNewContent, oq_MessagesAreTx,
                rc_CurMessage, c_CurOscSignalCommons, c_CurUiSignalCommons,
        ...
        if (pc_Node != NULL) {
          ...
          const uint32_t u32_PresentDatapoolIndex =
              orc_MessageOverrideIndices[u32_ItMessage].first;

          const stw::opensyde_core::C_OscCanMessageContainer *const
              pc_Container =
          ...
          if ((pc_Container != NULL) &&
            ...
            const C_OscCanMessageIdentificationIndices c_Id(
                ou32_NodeIndex, oe_Type, ou32_InterfaceIndex,
                u32_PresentDatapoolIndex, oq_MessagesAreTx,
                static_cast<uint32_t>(
                    orc_MessageOverrideIndices[u32_ItMessage].second));
            ...
            if ((rc_Interface.GetBusConnected() == true) &&
              ...
              const C_OscCanMessage *const pc_Message =
                  C_PuiSdHandler::h_GetInstance()->GetCanMessage(c_Id);
              if (pc_Message != NULL) {
                ...
                for (uint32_t u32_ItSignal = pc_Message->c_Signals.size();
                     u32_ItSignal > 0UL; --u32_ItSignal) {
                  if (c_SyncManager.DeleteCanSignal(c_Id, u32_ItSignal - 1UL) !=
                      C_NO_ERR) {
                    s32_Retval = C_RANGE;
                  }
                }
                ...
                for (uint32_t u32_ItSignal = 0;
                     u32_ItSignal < rc_CurMessage.c_Signals.size();
                     ++u32_ItSignal) {
                  if (c_SyncManager.AddCanSignal(
                          c_Id, rc_CurMessage.c_Signals[u32_ItSignal],
              ...
              // Replace message
              if (C_PuiSdHandler::h_GetInstance()->DeleteCanMessage(c_Id) !=
                  C_NO_ERR) {
                s32_Retval = C_RANGE;
              } else {
                if (C_PuiSdHandler::h_GetInstance()->InsertCanMessage(
                        c_Id, rc_CurMessage, c_CurOscSignalCommons,
                        c_CurUiSignalCommons, rc_UiMessage,
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CieUtil::mh_ConstructMessageName(const QString &orc_Nodename,
                                           const QString &orc_TpdoRpdo,
                                           const uint32_t ou32_MessageIndex) {
  const QString c_Retval =
      static_cast<QString>((orc_Nodename + "_" + orc_TpdoRpdo + "%1"))
