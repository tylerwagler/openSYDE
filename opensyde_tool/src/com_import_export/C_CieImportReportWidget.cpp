...
/*!
   \file
   \brief       Dialog for system node data pool list import report
   (implementation)
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_CieImportReportWidget::C_CieImportReportWidget(
    C_OgePopUpDialog &orc_Parent, const QString &orc_FilePath,
    ...
    const QList<C_CieImportDataAssignment> &orc_SkippedImportDataAssigned,
    const QString *const opc_NodeNameReplacement, const bool oq_IsCanOpen,
    const bool oq_UniqueAddRequested)
    : QWidget(&orc_Parent), mpc_Ui(new Ui::C_CieImportReportWidget),
      mrc_ParentDialog(orc_Parent), mc_FilePath(orc_FilePath),
      mu32_BusIndex(ou32_BusIndex), me_ProtocolType(oe_ProtocolType),
  ...
  const bool q_IsEdsOrDcfImport = C_CieImportReportWidget::mh_IsEdsOrDcfImport(
      c_FileInfo.completeSuffix().toUpper());
  ...
  this->mrc_ParentDialog.SetTitle(
      static_cast<QString>("%1 File Import")
          .arg(c_FileInfo.completeSuffix().toUpper()));
  this->mrc_ParentDialog.SetSubTitle("Report");

  // Trigger report
  C_CieUtil::h_AdaptImportMessages(this->mc_ImportedAssignedData,
                                   this->me_ProtocolType, q_IsEdsOrDcfImport);
  C_CieUtil::h_AdaptImportMessages(this->mc_SkippedImportedAssignedData,
                                   this->me_ProtocolType, q_IsEdsOrDcfImport);
  Q_ASSERT(m_ShowReport(c_FileInfo.completeSuffix().toUpper(), oq_IsCanOpen) ==
           C_NO_ERR);
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_CieImportReportWidget::~C_CieImportReportWidget(void) { delete mpc_Ui; }
...
QString C_CieImportReportWidget::h_GetInitialReportPart(
  ...
  // Source File Info
  c_CompleteLog += C_ImpUtil::h_FormatSourceFileInfoForReport(
      orc_ImportedFilePath, c_ReadContent);
  ...
  if (oq_AddMessageAdaptationInfo) {
    ...
    c_CompleteLog += "- ";
    c_CompleteLog +=
        static_cast<QString>("Cut to %1 (= project setting) characters")
            .arg(C_PuiSdHandler::h_GetInstance()->GetNameMaxCharLimit());
    c_CompleteLog += "</p>";
    if ((oq_IsCanOpenCase == true) &&
        (c_FileInfo.completeSuffix().toLower() == "dcf")) {
      c_CompleteLog += "<p>";
      c_CompleteLog +=
          "Node ID and device name will not be adopted from the DCF - File.";
      c_CompleteLog += "</p>";
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportReportWidget::h_GetMessageTableContent(
  ...
  mh_GetTableWithMessageEntries(s32_Retval, c_RawTable, u32_MessageCount,
                                orc_Suffix, orc_ImportDataAssignment,
                                oe_ProtocolType, opc_NodeNameReplacement,
  ...
  if ((s32_Retval == C_NO_ERR) && (c_RawTable.isEmpty() == false)) {
    ...
    orc_ImportTable +=
        static_cast<QString>("%1 (%2)").arg(orc_Heading).arg(u32_MessageCount);
    orc_ImportTable += "</b></p>";
...
void C_CieImportReportWidget::m_OkClicked(void) {
  ...
  for (uint32_t u32_ItNodes = 0;
       u32_ItNodes < this->mc_ImportedAssignedData.size(); u32_ItNodes++) {
    C_CieImportDataAssignment &rc_CurData =
        this->mc_ImportedAssignedData[u32_ItNodes];
    uint32_t u32_NumberCommDps =
        C_PuiSdHandler::h_GetInstance()
            ->GetOscCanDataPools(rc_CurData.u32_OsyNodeIndex,
    ...
    if (rc_CurData.u32_NewMessageCount > 0) {
      ...
      if (u32_NumberCommDps == 0) {
        ...
        if (C_SdUtil::h_CheckDatapoolNumber(rc_CurData.u32_OsyNodeIndex, false,
                                            this) == true) {
          Q_ASSERT(C_PuiSdHandler::h_GetInstance()->AddAutoGenCommDataPool(
                       rc_CurData.u32_OsyNodeIndex, this->me_ProtocolType) ==
                   C_NO_ERR);
      ...
      if (q_Continue == true) {
        if (u32_NumberCommDps == 1) {
          const C_OscNode *const pc_Node =
              C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(
                  rc_CurData.u32_OsyNodeIndex);

          // get datapool index of the corresponding datapool
          if (pc_Node != NULL) {
            for (uint32_t u32_ItDatapool = 0;
                 u32_ItDatapool < pc_Node->c_DataPools.size();
                 ++u32_ItDatapool) {
              if (pc_Node->c_DataPools[u32_ItDatapool].e_Type ==
                  C_OscNodeDataPool::eCOM) {
                if (C_PuiSdUtil::h_GetRelatedCanProtocolType(
                        rc_CurData.u32_OsyNodeIndex, u32_ItDatapool) ==
                    this->me_ProtocolType) {
                  rc_CurData.s32_DatapoolIndexForNew =
                      static_cast<int32_t>(u32_ItDatapool);
                  break;
                }
              }
            }
          ...
          // ask user which COMM Datapool the new messages should get inserted
          // at

          const QPointer<C_OgePopUpDialog> c_Popup =
              new C_OgePopUpDialog(this, this);
          C_CieImportDatapoolSelectWidget *const pc_DialogImportReport =
              new C_CieImportDatapoolSelectWidget(
                  *c_Popup, rc_CurData.u32_OsyNodeIndex, this->me_ProtocolType);
          ...
          if (c_Popup->exec() == static_cast<int32_t>(QDialog::Accepted)) {
            rc_CurData.s32_DatapoolIndexForNew =
                pc_DialogImportReport->GetSelectedDatapoolIndex();
          } else {
            q_Continue = false;
          }
        ...
        } // lint !e429  //no memory leak because of the parent of
          // pc_DialogImportReport and the Qt memory
      }
    } else {
  ...
  if (q_Continue == true) {
    // only accept if user did not cancel datapool selection (else caller does
    // not know that user canceled)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportReportWidget::m_ShowReport(const QString &orc_Suffix,
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval =
        m_CheckMessageMatches(orc_Suffix, this->mc_ImportedAssignedData);
  }
  // Get table with message entries
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_CieImportReportWidget::h_GetMessageTableContent(
        c_ImportTable, orc_Suffix, this->mc_ImportedAssignedData,
        this->me_ProtocolType, this->mpc_NodeNameReplacement, "Import Messages",
        true);
  }
  ...
  if (s32_SkippedRetval == C_NO_ERR) {
    s32_SkippedRetval =
        m_CheckMessageMatches(orc_Suffix, this->mc_SkippedImportedAssignedData);
  }
  // Get skipped table with message entries
  if (s32_SkippedRetval == C_NO_ERR) {
    s32_SkippedRetval = C_CieImportReportWidget::h_GetMessageTableContent(
        c_SkippedTable, orc_Suffix, this->mc_SkippedImportedAssignedData,
        this->me_ProtocolType, this->mpc_NodeNameReplacement,
        "Skipped Messages", true);
  }

  if (s32_Retval == C_NO_ERR) {
    QString c_CompleteLog = C_CieImportReportWidget::h_GetInitialReportPart(
        this->mc_ImportedAssignedData, this->mc_FilePath, oq_IsCanOpen,
  ...
  if (s32_SkippedRetval != C_NO_ERR) {
    osc_write_log_warning("Skipped Messages while importing Messages",
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportReportWidget::m_CheckMessages(
    const QList<C_CieImportDataAssignment> &orc_ImportDataAssignment)
    const {
  int32_t s32_Retval = C_NO_ERR;
  ...
  // Each imported node
  for (uint32_t u32_ItNodes = 0;
       (u32_ItNodes < orc_ImportDataAssignment.size()) &&
       (s32_Retval == C_NO_ERR);
       u32_ItNodes++) {
    const C_CieImportDataAssignment &rc_CurData =
        orc_ImportDataAssignment[u32_ItNodes];
    ...
    if (rc_CurData.c_ImportData.c_Core.c_OscRxMessageData.size() !=
      ...
      for (uint32_t u32_ItMessage = 0;
           ...
           (s32_Retval == C_NO_ERR);
           ++u32_ItMessage) {
        const C_OscCanMessage &rc_OscCurMessage =
        ...
        if (rc_OscCurMessage.c_Signals.size() !=
            rc_UiCurMessage.c_Signals.size()) {
          s32_Retval = C_CONFIG;
        }
        for (uint32_t u32_ItSignal = 0;
             (u32_ItSignal < rc_OscCurMessage.c_Signals.size()) &&
             (s32_Retval == C_NO_ERR);
             ++u32_ItSignal) {
          const C_OscCanSignal &rc_CurSignal =
              rc_OscCurMessage.c_Signals[u32_ItSignal];
          if (rc_CurSignal.u32_ComDataElementIndex >=
              rc_CurData.c_ImportData.c_Core.c_OscRxSignalData.size()) {
            s32_Retval = C_CONFIG;
          }
        }
      ...
      for (uint32_t u32_ItMessage = 0;
        ...
        for (uint32_t u32_ItSignal = 0;
          ...
          const C_OscCanSignal &rc_CurSignal =
              rc_OscCurMessage.c_Signals[u32_ItSignal];
          if (rc_CurSignal.u32_ComDataElementIndex >=
              rc_CurData.c_ImportData.c_Core.c_OscTxSignalData.size()) {
            s32_Retval = C_CONFIG;
          }
        }
        u32_SignalSum += rc_OscCurMessage.c_Signals.size();
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportReportWidget::m_CheckMessageMatches(
  ...
  for (uint32_t u32_ItNodes = 0;
    ...
    QList<stw::opensyde_core::C_OscCanMessage>::iterator
        c_ImportMessageIt;
    ...
    for (c_ImportMessageIt =
             rc_CurData.c_ImportData.c_Core.c_OscTxMessageData.begin();
         (c_ImportMessageIt !=
          rc_CurData.c_ImportData.c_Core.c_OscTxMessageData.end()) &&
         (s32_Retval == C_NO_ERR);
         ++c_ImportMessageIt) {
      const bool q_IsSrdo = C_CieUtil::h_GetIsSrdoFromMessageIsSrdoVector(
          rc_CurData.c_ImportData.c_Core.c_EdsOscTxMessageIsSrdo,
          u32_TxMessageCount);
      s32_Retval = m_GetMessageOverrideInfo(
          *c_ImportMessageIt, true, q_IsSrdo, rc_CurData.u32_OsyNodeIndex,
    ...
    for (c_ImportMessageIt =
             rc_CurData.c_ImportData.c_Core.c_OscRxMessageData.begin();
         (c_ImportMessageIt !=
          rc_CurData.c_ImportData.c_Core.c_OscRxMessageData.end()) &&
         (s32_Retval == C_NO_ERR);
         ++c_ImportMessageIt) {
      const bool q_IsSrdo = C_CieUtil::h_GetIsSrdoFromMessageIsSrdoVector(
          ...
          u32_RxMessageCount);
      s32_Retval = m_GetMessageOverrideInfo(
          *c_ImportMessageIt, false, q_IsSrdo, rc_CurData.u32_OsyNodeIndex,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Compare a import message with existing messages to find out if the
   message is new or just an update.

   \param[in]      orc_ImportMessageToFind   message from import file that
   should be checked for existing message
   ...
   \param[out]     orc_MessageOverrideInfo   pair of datapool index and index of
   message in corresponding protocol
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportReportWidget::m_GetMessageOverrideInfo(
    C_OscCanMessage &orc_ImportMessageToFind, const bool oq_Tx,
    const bool oq_IsSrdo, const uint32_t ou32_OsyNodeIndex,
    const uint32_t ou32_OsyInterfaceIndex,
    std::pair<int32_t, int32_t> &orc_MessageOverrideInfo,
  ...
  if (this->mq_UniqueAddRequested == false) {
    const C_OscNode *const pc_Node =
        C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(ou32_OsyNodeIndex);
    ...
    if (pc_Node != NULL) {
      ...
      if (this->me_ProtocolType != C_OscCanProtocol::eCAN_OPEN) {
        QList<const C_OscCanProtocol *> c_Protocols =
            pc_Node->GetCanProtocolsConst(this->me_ProtocolType);
        QList<const C_OscCanProtocol *>::const_iterator c_ProtocolIt;

        // Iterate Protocols (resp. Datapools)
        for (c_ProtocolIt = c_Protocols.begin();
             c_ProtocolIt != c_Protocols.end(); ++c_ProtocolIt) {
          ...
          if (pc_Protocol != NULL) {
            if (ou32_OsyInterfaceIndex < pc_Protocol->c_ComMessages.size()) {
              ...
              const QList<C_OscCanMessage> &rc_Messages =
                  rc_MessageContainer.GetMessagesConst(oq_Tx);

              // Check all existing messages for a match (CAN Id and Name)
              for (uint32_t u32_MessageIt = 0;
                   u32_MessageIt < rc_Messages.size(); u32_MessageIt++) {
                const C_OscCanMessage &rc_ExistingMessage =
                    rc_Messages[u32_MessageIt];
                if (((rc_ExistingMessage.u32_CanId ==
                      orc_ImportMessageToFind.u32_CanId) &&
                     (rc_ExistingMessage.q_IsExtended ==
                      orc_ImportMessageToFind.q_IsExtended)) &&
                    (rc_ExistingMessage.c_Name ==
                     orc_ImportMessageToFind.c_Name)) {
                  orc_MessageOverrideInfo = std::make_pair(
                      static_cast<int32_t>(pc_Protocol->u32_DataPoolIndex),
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CieImportReportWidget::mh_GetTableWithMessageEntries(
  ...
  QList<C_OscCanMessage>
      c_MessagesList; // for listing every message only once

  // Each imported node
  for (uint32_t u32_ItNodes = 0;
       (u32_ItNodes < orc_ImportDataAssignment.size()) &&
       (ors32_Retval == C_NO_ERR);
       u32_ItNodes++) {
    C_CieImportDataAssignment &rc_CurData =
        orc_ImportDataAssignment[u32_ItNodes];
    ...
    // Tx
    ors32_Retval = mh_GetMessageEntries(
        oru32_MessageCount, rc_CurData.u32_NewMessageCount, orc_ImportTable,
        c_MessagesList, rc_CurData.c_ImportData.c_Core.c_OscTxMessageData,
        rc_CurData.c_ImportData.c_Core.c_OscTxSignalData,
        rc_CurData.c_ImportData.c_Core.c_EdsOscTxMessageIsSrdo,
        rc_CurData.c_ImportData.c_Core.c_WarningMessagesPerTxMessage,
        rc_CurData.c_TxMessageOverrideIndices, rc_CurData.u32_OsyNodeIndex,
        true, orc_Suffix, oe_ProtocolType, opc_NodeNameReplacement,
        oq_ReplaceMessageNames, u32_NewSrdoMessageCount);
    if (ors32_Retval == C_NO_ERR) {
      // Rx
      ors32_Retval = mh_GetMessageEntries(
          oru32_MessageCount, rc_CurData.u32_NewMessageCount, orc_ImportTable,
          c_MessagesList, rc_CurData.c_ImportData.c_Core.c_OscRxMessageData,
...
void C_CieImportReportWidget::m_InsertMessages(void) const {
  C_CieUtil::h_InsertMessages(this->mc_ImportedAssignedData,
                              this->me_ProtocolType,
                              this->mq_UniqueAddRequested);
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Format multiple message entries in import report

   \param[in,out]  oru32_EntryCount             Number of entries in messages
   table
   \param[out]     oru32_NewEntryCount          New entry count
   \param[in,out]  orc_TableEntries             Current content of messages
   table
   \param[in,out]  orc_MessageList              All already listed messages
   \param[in]      orc_OscMessageData           Imported core message data for
   this message type (Tx or Rx)
   \param[in]      orc_OscAllSignalData         All signal data for this message
   type (Tx or Rx)
   \param[in]      orc_MessageIsSrdo            Flag if message is SRDO
   \param[in]      orc_InfoMessagesPerMessage   Info messages for imported CAN
   messages for this message type (Tx or Rx)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CieImportReportWidget::mh_GetMessageEntries(
    uint32_t &oru32_EntryCount, uint32_t &oru32_NewEntryCount,
    QString &orc_TableEntries, QList<C_OscCanMessage> &orc_MessageList,
    const QList<C_OscCanMessage> &orc_OscMessageData,
    const QList<C_OscNodeDataPoolListElement> &orc_OscAllSignalData,
    ...
    const QString *const opc_NodeNameReplacement,
    const bool oq_ReplaceMessageNames, uint32_t &oru32_SrdoIndex) {
  int32_t s32_Retval = C_NO_ERR;

  // Should not happen at this point
  Q_ASSERT(
      (orc_MessageOverrideIndices.size() == orc_OscMessageData.size()) &&
  ...
  if ((orc_MessageOverrideIndices.size() == orc_OscMessageData.size()) &&
      (orc_MessageOverrideIndices.size() ==
       orc_InfoMessagesPerMessage.size())) {
    for (uint32_t u32_ItMessage = 0;
         u32_ItMessage < orc_MessageOverrideIndices.size(); ++u32_ItMessage) {
      bool q_AddToTable = true;
      // check if message is already in table
      for (QList<C_OscCanMessage>::const_iterator c_MessIt =
               orc_MessageList.begin();
      ...
      if (q_AddToTable == true) {
        const bool q_IsSrdo = C_CieUtil::h_GetIsSrdoFromMessageIsSrdoVector(
            orc_MessageIsSrdo, u32_ItMessage);
        orc_TableEntries += "<tr>";
        orc_TableEntries += mh_GetMessageEntry(
            oru32_EntryCount, orc_OscMessageData[u32_ItMessage],
            orc_OscAllSignalData, orc_InfoMessagesPerMessage[u32_ItMessage],
            ou32_NodeIndex, oq_IsTx, q_IsSrdo, u32_ItMessage, orc_Suffix,
            oe_ProtocolType, opc_NodeNameReplacement, oq_ReplaceMessageNames,
            oru32_SrdoIndex);
        orc_TableEntries += "</tr>";
        ++oru32_EntryCount;
      ...
      // count if message is new for this node (needed later to know if datapool
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CieImportReportWidget::mh_GetMessageEntry(
    const uint32_t ou32_Index, const C_OscCanMessage &orc_CurMessage,
    const QList<C_OscNodeDataPoolListElement> &orc_OscAllSignalData,
    const QString &orc_InfoMessages, const uint32_t ou32_NodeIndex,
    const bool oq_IsTx, const bool oq_IsSrdo, const uint32_t ou32_MessageIndex,
    const QString &orc_Suffix, const C_OscCanProtocol::E_Type oe_ProtocolType,
    const QString *const opc_NodeNameReplacement,
    const bool oq_ReplaceMessageNames, uint32_t &oru32_SrdoIndex) {
  ...
  c_Retval += C_CieImportReportWidget::hc_HTML_TABLE_DATA_START +
              QString::number(ou32_Index + 1U) + c_TableEntryEnd;
  // Message name
  if (C_CieImportReportWidget::mh_IsEdsOrDcfImport(orc_Suffix)) {
    ...
    if (oq_ReplaceMessageNames) {
      ...
      if (pc_Node != NULL) {
        c_MessageName = C_CieUtil::h_GetMessageName(
            *pc_Node, oq_IsTx, ou32_MessageIndex, oe_ProtocolType,
  ...
  if (orc_CurMessage.c_Signals.size() == 0UL) {
    ...
    c_Tmp = "";
    for (uint32_t u32_ItSignal = 0;
         u32_ItSignal < orc_CurMessage.c_Signals.size(); ++u32_ItSignal) {
      const C_OscCanSignal &rc_CurSignal =
          orc_CurMessage.c_Signals[u32_ItSignal];
  ...
  c_Retval += C_CieImportReportWidget::hc_HTML_TABLE_DATA_START + c_Tmp +
              c_TableEntryEnd;
  // Warnings
