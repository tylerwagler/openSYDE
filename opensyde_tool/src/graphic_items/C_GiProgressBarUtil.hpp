...
public:
   ...
   static void h_DefineType1Margins(const QRect & orc_Rect, QHBoxLayout * const opc_Layout, const bool & orq_ShowMinMax,
                                    ...
                                    const float & orf32_MinimumFontSize, const float & orf32_ArrowOffset);
   static float h_GetType1FontSizeValue(const QRect & orc_Rect, const float & orf32_MinimumFontSize);
   ...
   static float h_GetType2FontSizeValue(const QRect & orc_Rect, const float & orf32_MinimumFontSize);
   ...
   static void h_DefineType3Margins(const QRect & orc_Rect, QHBoxLayout * const opc_Layout, const bool & orq_ShowMinMax,
                                    const stw::opensyde_gui_logic::C_PuiSvDbProgressBar::E_Alignment & ore_Alignment,
                                    const float & orf32_MinimumFontSize);
   static float h_GetType3FontSizeValue(const QRect & orc_Rect, const float & orf32_MinimumFontSize);
   static float h_GetType3FontSizeMaxMin(const QRect & orc_Rect, const float & orf32_MinimumFontSize);
   static void h_SetType3Value(const stw::opensyde_gui_elements::C_OgeDashboardProgressBar * const opc_ProgressBar,
                               const QString & orc_CurrentValue, const QString & orc_Unit,
                               const stw::opensyde_gui_logic::C_PuiSvDbProgressBar::E_Alignment & ore_Alignment,
                               const float orf32_FontSize, QPainter & orc_Painter);
...
private:
   ...
   static QRectF mh_GetCircle(const QPointF & orc_P1, const double of64_Width);
   ...
   static double mh_GetArc(const QPointF & orc_V1, const QPointF & orc_V2);
   ...
   static void mh_GetMarginType1TopBottom(float * const opf32_MarginTop, float * const opf32_MarginBottom,
   ...
   static void mh_GetMarginType1LeftRight(float * const opf32_MarginLeft, float * const opf32_MarginRight,
                                          const QString orc_MaxValue, const QString orc_MinValue,
                                          const QString orc_Unit, const bool & orq_ShowMinMax,
                                          const QFontMetrics & orc_TextFontMetric);

   // Progress bar type 2 calculation
   static void mh_GetMarginType23LeftRight(float * const opf32_MarginLeft, float * const opf32_MarginRight,
                                           const QRect & orc_Rect,
                                           const stw::opensyde_gui_logic::C_PuiSvDbProgressBar::E_Alignment & ore_Alignment, const float & orf32_ReservedCurrentValueSpace);

   static void mh_GetMarginTyp23TopBottom(float * const opf32_MarginTop, float * const opf32_MarginBottom,
                                          const bool & orq_ShowMinMax, const QFontMetrics & orc_TextFontMetric);

   // Calculate height position
   static double mh_GetCurrentValueHeightPosition(const QRect & orc_Rect,
   ...
   static double mh_GetCurrentValueWidthPosition(
      const stw::opensyde_gui_elements::C_OgeDashboardProgressBar * const opc_ProgressBar,
      const QString & orc_MaxValue, const QString & orc_MinValue, const QString & orc_CurrentValue);
   static float mh_GetValueInPercentageDecimal(const QString & orc_MaxValue, const QString & orc_MinValueString,
   ...
   // General
   static float mh_ShrinkCornerPointsDistance(const float & orf32_ProgressBarPointDistance);
   static QFontMetrics mh_GetType1FontMetrics(const QRect & orc_Rect, const float & orf32_MinimumFontSize);
   static QFontMetrics mh_GetType3FontMetrics(const QRect & orc_Rect, const float & orf32_MinimumFontSize);

   //-------Constants---------------------------------------------------------------------------------------------------
   // General constants
   static constexpr float mhf32_DEFAULT_MARGIN = 0.0F;
   static constexpr float mhf32_HALF_MODIFIER = 2.0F;
   static constexpr uint32_t mhu32_PAINTER_FLAG = 0; // No alignment flag
   static constexpr float mhf32_ARROW_BAR_SPACER = 1.0F;

   // Arrow constants
   static constexpr float mhf32_ARROW_SIZE_H = 3.0F;
   static constexpr float mhf32_ARROW_SIZE_V = 3.0F;
   static constexpr float mhf32_ARROW_MODIFIER = 2.0F;
};
