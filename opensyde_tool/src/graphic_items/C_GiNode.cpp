...
/*!
   \file
   \brief       Offers visualization and functionality of a node
   (implementation)
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_GiNode::C_GiNode(const int32_t &ors32_Index, const uint64_t &oru64_Id,
                   const float64_t &orf64_Width, const float64_t &orf64_Height,
                   QGraphicsItem *const opc_Parent)
    : // lint -e{1938}  static const is guaranteed preinitialized before main
      C_GiRectBaseGroup(ors32_Index, C_PuiSdDataElement::eNODE, oru64_Id,
                        mhf64_MIN_WIDTH_NODE, mhf64_MIN_HEIGHT_NODE,
                        mf64_ACTION_POINT_OFFSET_NODE, false, opc_Parent),
      C_GiBiCustomToolTip(), mq_ErrorIconHovered(false), mq_Valid(true),
      mq_DrawWhiteFilter(false), ms32_IconSize(24) {
  QString c_Name = "Node";
  uint32_t u32_SubNodesCount = 0;
  const C_OscNode *const pc_Node =
      C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(this->ms32_Index);
  ...
  if (pc_Node != NULL) {
    ...
    if (C_PuiSdHandler::h_GetInstance()->GetNodeSquadIndexWithNodeIndex(
            this->ms32_Index, u32_NodeSquadIndex) == C_NO_ERR) {
      // squad node
      ...
      if (pc_NodeSquad != NULL) {
        u32_SubNodesCount =
            static_cast<uint32_t>(pc_NodeSquad->c_SubNodeIndexes.size());
      }
    } else {
      // no squad node
      u32_SubNodesCount = 0;
  ...
  // lint -e{1938}  static const is guaranteed preinitialized before main
  this->mpc_Boundary = new C_GiNodeBoundary(
      c_Name, std::max(mhf64_MIN_WIDTH_NODE, orf64_Width),
      std::max(mhf64_MIN_HEIGHT_NODE, orf64_Height), u32_SubNodesCount);
  this->m_DetectIconSize();

  // Notify the base class about the boundary as biggest item as orientation.
  // Very important!
  ...
  // lint -e{1938}  static const is guaranteed preinitialized before main
  this->m_UpdateItems(
      (std::max(mhf64_MIN_WIDTH_NODE, orf64_Width) - mhf64_MIN_WIDTH_NODE),
...

  {
    ...
    this->mc_Ports[ms32_INDEX_ABOVE]->setY(
        (this->mc_Ports[ms32_INDEX_ABOVE]->y() -
         this->mc_Ports[ms32_INDEX_ABOVE]->boundingRect().height()) +
        2.0);
  }

  {
    ...
    this->mc_Ports[ms32_INDEX_RIGHT]->setX(
        (this->mc_Ports[ms32_INDEX_RIGHT]->x() +
         ...
         this->mc_Ports[ms32_INDEX_RIGHT]->boundingRect().height()) -
        2.0);
...

  {
    ...
    this->mc_Ports[ms32_INDEX_LEFT]->setX(
        (this->mc_Ports[ms32_INDEX_LEFT]->x() -
         this->mc_Ports[ms32_INDEX_LEFT]->boundingRect().height()) +
        2.0);
...
void C_GiNode::m_InitConflictIcon(void) {
  const float64_t f64_PosHorizontal =
      (this->mpc_Boundary->boundingRect().width() - 33.0) - // static offset for
                                                            // correct
                                                            // position
      (static_cast<float64_t>(this->ms32_IconSize) -
       24.0); // offset of scaled icon

  // create the conflict icon
  this->mpc_ConflictIcon = new C_GiRectPixmap(QRectF(
      f64_PosHorizontal, 9.0, static_cast<float64_t>(this->ms32_IconSize),
      static_cast<float64_t>(this->ms32_IconSize)));
...
//----------------------------------------------------------------------------------------------------------------------
void C_GiNode::m_UpdateItems(const float64_t of64_DiffWidth,
                             const float64_t of64_DiffHeight,
  ...
  if (oq_Initial == false) {
    ...
    this->mpc_ConflictIcon->moveBy(
        of64_DiffWidth - static_cast<float64_t>(s32_IconSizeDiff), 0.0);
    this->mpc_ConflictIcon->SetNewSize(
        QSizeF(static_cast<float64_t>(this->ms32_IconSize),
               static_cast<float64_t>(this->ms32_IconSize)));
    this->mpc_ConflictIcon->update();

    // move only by mouse movements
    this->mc_Ports[ms32_INDEX_ABOVE]->moveBy((of64_DiffWidth / 2.0), 0.0);
    this->mc_Ports[ms32_INDEX_BOTTOM]->moveBy((of64_DiffWidth / 2.0),
                                              of64_DiffHeight);
    ...
    this->mc_Ports[ms32_INDEX_LEFT]->moveBy(0.0,
                                            (of64_DiffHeight / 2.0) * -1.0);
  }
  ...
  if (this->mpc_Boundary->f64_Height >= mhf64_MIN_WIDTH_NODE) {
    ...
    // The aim is to get the ports left and right to the same proportions like
    // the port above and bottom
...
//----------------------------------------------------------------------------------------------------------------------
C_GiNode::~C_GiNode() {
  for (int32_t s32_ItConn = 0; s32_ItConn < this->mc_Connections.size();
       ++s32_ItConn) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_GiNode::FindClosestPoint(const QPointF &orc_ScenePoint,
                                QPointF &orc_Closest) const {
  //
  QPointF c_CurClosest;
  float64_t f64_CurDist;
  float64_t f64_Best = std::numeric_limits<float64_t>::max();

  for (int32_t s32_ItPort = 0; s32_ItPort < this->mc_Ports.size();
       ++s32_ItPort) {
    this->mc_Ports[s32_ItPort]->FindClosestPoint(orc_ScenePoint, c_CurClosest);
    C_GiBiConnectableItem::h_DistToPoint(orc_ScenePoint, c_CurClosest,
                                         f64_CurDist);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_GiNode::FindClosestPort(const QPointF &orc_ScenePoint,
                               C_GiPort *(&orpc_Closest)) const {
  QPointF c_CurClosest;
  float64_t f64_CurDist;
  float64_t f64_Best = std::numeric_limits<float64_t>::max();
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_GiNode::type() const { return ms32_GRAPHICS_ITEM_NODE; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_GiNode::CheckConnectionAvailable(
    const C_OscSystemBus::E_Type &ore_Type) const {
  bool q_Retval = false;
  ...
  if (pc_Node != NULL) {
    q_Retval = C_SdUtil::h_CheckNodeInterfaceAvailable(
        pc_Node->c_Properties.c_ComInterfaces, ore_Type);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_GiNode::CheckInterfaceAvailable(const C_OscSystemBus::E_Type &ore_Type,
  ...
  if (q_Retval == true) {
    ...
    if (pc_Node != NULL) {
      const C_OscNodeComInterfaceSettings *const pc_ComInterface =
          pc_Node->c_Properties.GetComInterface(ore_Type, oru8_Interface);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_GiNode::GetIndexOfConnector(
...
void C_GiNode::GetOscNodeConst(const C_OscNode *(&orpc_Node)) const {
  orpc_Node = C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(
    ...
    if (pc_Node != NULL) {
      ...
      if (pc_Device != NULL) {
        ...
        if (pc_Device->u8_NumCanBusses > 0) {
          for (uint8_t u8_ItInterface = 0;
               u8_ItInterface < pc_Device->u8_NumCanBusses; ++u8_ItInterface) {
            q_Found = false;
                  ...
                  if (rc_Conn.GetBusConnectedRawValue() == true) {
                    ...
                    if (pc_Bus != NULL) {
                      ...
                      if (pc_Bus->e_Type == C_OscSystemBus::eCAN) {
                        c_BusName.append(
                            static_cast<QString>(" (Bitrate: %1 kbit/s)")
                                .arg(QString::number(pc_Bus->u64_BitRate /
                                                     1000ULL)));
                      }
                      q_Found = true;
                    }
            ...
            c_Entry = static_cast<QString>("%1: %2").arg(
        ...
        if (pc_Device->u8_NumEthernetBusses > 0) {
          for (uint8_t u8_ItInterface = 0;
               u8_ItInterface < pc_Device->u8_NumEthernetBusses;
               ++u8_ItInterface) {
            q_Found = false;
            for (uint32_t u32_ItBusConnection = 0;
                 (u32_ItBusConnection <
              ...
              const C_OscNodeComInterfaceSettings &rc_Conn =
                  pc_Node->c_Properties.c_ComInterfaces[u32_ItBusConnection];
              if (rc_Conn.e_InterfaceType ==
                  C_OscSystemBus::E_Type::eETHERNET) {
                if (rc_Conn.u8_InterfaceNumber == u8_ItInterface) {
                  if (rc_Conn.GetBusConnectedRawValue() == true) {
                    const C_OscSystemBus *const pc_Bus =
                        C_PuiSdHandler::h_GetInstance()->GetOscBus(
        ...
        if (q_FileExists == true) {
          ...
          this->SetDefaultToolTipImagePath(
              "://images/system_definition/Image_Grey.svg");
        }
...
void C_GiNode::ReevaluatePortState(void) {
  ...
  for (QVector<C_GiPort *>::iterator pc_ItPort = mc_Ports.begin();
       pc_ItPort != mc_Ports.end(); ++pc_ItPort) {
  ...
  for (QVector<C_GiLiBusConnector *>::iterator pc_ItConn =
           mc_Connections.begin();
       pc_ItConn != mc_Connections.end(); ++pc_ItConn) {
...
void C_GiNode::hoverMoveEvent(QGraphicsSceneHoverEvent *const opc_Event) {
  ...
  if (this->mpc_ConflictIcon != NULL) {
    if (this->mpc_ConflictIcon->isVisible() == true) {
      if (this->mpc_ConflictIcon->sceneBoundingRect().contains(
              opc_Event->scenePos()) == true) {
...
bool C_GiNode::m_UpdateError(void) {
  ...
  C_SdUtil::h_GetErrorToolTipNode(static_cast<uint32_t>(this->ms32_Index),
                                  this->mc_ErrorText, q_ErrorDetected, true);
...
//----------------------------------------------------------------------------------------------------------------------
void C_GiNode::m_ResizeUpdateItems(const float64_t of64_DiffWidth,
                                   const float64_t of64_DiffHeight) {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_GiNode::m_GetIconSize(void) const { return this->ms32_IconSize; }
...
uint32_t C_GiNode::m_GetScaleCategory(void) const {
  ...
  // check first scaling
  if ((c_ActSize.width() >=
       mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_1]) &&
      (c_ActSize.height() >=
       mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_1])) {
    // check second scaling
    if ((c_ActSize.width() >=
         mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_2]) &&
        (c_ActSize.height() >=
         mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_2])) {
      // check third scaling
      if ((c_ActSize.width() >=
           mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_3]) &&
          (c_ActSize.height() >=
           mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_3])) {
        // check fourth scaling
        if ((c_ActSize.width() >=
             mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_4]) &&
            (c_ActSize.height() >=
             mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_4])) {
          // check fifth scaling
          if ((c_ActSize.width() >=
               mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_5]) &&
              (c_ActSize.height() >=
               mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_5])) {
            // check sixth scaling
            if ((c_ActSize.width() >=
                 mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_6]) &&
                (c_ActSize.height() >=
                 mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_6])) {
              // check seventh scaling
              if ((c_ActSize.width() >=
                   mhaf64_SCALE_MIN_WIDTH_NODE[mhu32_SCALE_CATEGORY_7]) &&
                  (c_ActSize.height() >=
                   mhaf64_SCALE_MIN_HEIGHT_NODE[mhu32_SCALE_CATEGORY_7])) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_GiNode::GetValid(void) const { return mq_Valid; }
...
void C_GiNode::LoadData(void) {
  const C_PuiSdNode *const pc_Node =
      C_PuiSdHandler::h_GetInstance()->GetUiNode(ms32_Index);
...
void C_GiNode::UpdateData(void) {
  ...
  C_PuiSdHandler::h_GetInstance()->SetUiNodeConnections(ms32_Index,
                                                        c_BusConnections);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_GiNode::RemoveConnector(
    const C_GiLiBusConnector *const opc_BusConnectorGraphicsItem) {
  int32_t s32_ItConn;

  const C_PuiSdNodeConnectionId *const pc_ConnId =
      this->GetNodeConnectionId(opc_BusConnectorGraphicsItem);

  if (pc_ConnId != NULL) {
    // We need a copy due to multi CPU nodes. RemoveConnection removes the
    // connection and the first sub node remove can cause a change the value
    // pc_ConnId points at. All following deletions of the other sub nodes in
    // RemoveConnection could have wrong values where pc_ConnId is pointing at
    const C_PuiSdNodeConnectionId c_ConnIdCopy = *pc_ConnId;
    C_PuiSdHandler::h_GetInstance()->RemoveConnection(
        static_cast<uint32_t>(this->ms32_Index), c_ConnIdCopy);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_GiNode::CheckIndexRelevantForThisNode(
    const uint32_t ou32_NodeIndex) const {
  bool q_Retval = false;

  if (this->ms32_Index >= 0) {
    q_Retval = C_PuiSdHandler::h_GetInstance()
                   ->CheckNodeIndexAssociatedWithAnotherNodeIndex(
...
void C_GiNode::AddConnectionAndData(
  ...
  if (opc_Connection != NULL) {
    ...
    C_PuiSdHandler::h_GetInstance()->AddConnection(
        static_cast<uint32_t>(this->GetIndex()),
        orc_NodeConnection.u8_InterfaceNumber, orc_Properties, oru32_BusIndex);
  }
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Update internal connection data identified by specified
   C_GiLiBusConnector instance
...
void C_GiNode::UpdateConnection(
  ...
  if (pc_PrevConn != NULL) {
    C_PuiSdHandler::h_GetInstance()->ChangeCompleteConnection(
        static_cast<uint32_t>(this->ms32_Index), *pc_PrevConn,
        orc_NodeConnection, orc_Properties, oru32_BusIndex);
  }
}
...
void C_GiNode::ChangeInterface(
  ...
  if (opc_Connection != NULL) {
    ...
    if (pc_Bus != NULL) {
      const C_PuiSdNodeConnectionId *const pc_ConnectionId =
          this->GetNodeConnectionId(opc_Connection);
      if (pc_ConnectionId != NULL) {
        C_PuiSdHandler::h_GetInstance()->ChangeConnection(
            static_cast<uint32_t>(this->ms32_Index), *pc_ConnectionId,
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_PuiSdNodeConnectionId *C_GiNode::GetNodeConnectionId(
    const C_GiLiBusConnector *const opc_Connection) const {
  const C_PuiSdNodeConnectionId *pc_Retval = NULL;

  if (opc_Connection != NULL) {
    const C_PuiSdNode *const pc_Node =
        C_PuiSdHandler::h_GetInstance()->GetUiNode(
            static_cast<uint32_t>(this->ms32_Index));
    if (pc_Node != NULL) {
