...
/*!
   ...
   Note: For openSYDE_Test.pro there exists a simply copied dummy class of this
   class!
...
/*! \brief   Save project
   ...
   \param[in]  oq_ForceSaveAll               Optional flag if all files should
   be saved
   \param[in]  oq_UseDeprecatedFileFormatV2  Flag to enable saving using the
   deprecated V2 file format

   \return
   C_NO_ERR   data saved
   C_RD_WR    problems accessing file system (could not erase pre-existing file
   before saving; no write access to file) C_RANGE    Path is empty C_NOACT
   Could not create project directory
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::Save(const bool oq_ForceSaveAll,
                           const bool oq_UseDeprecatedFileFormatV2) {
  int32_t s32_Retval;
  ...
  if (this->m_IsServiceModeProject() == false) {
    ...
    s32_Retval =
        this->m_SaveServiceModeProject(this->GetPath(), this->mc_Password);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Save the current project encrypted with activated service to a
   specific path

   This function does not change the current opened project

   \param[in]       orc_FilePath     Path to save the service mode project
   \param[in]       orc_Password     Used password for service project
   encryption
   \param[in]       orc_ViewConfigs  Flags for view configuration which view
   function will be available in service project
   ...
   \retval   C_RD_WR    problems accessing file system (could not erase
   pre-existing file before saving; no write access to file; could not open
   input file)
   \retval   C_NOACT    Could not create temporary project directory
                        could not add data to zip file (does the path to the
   file exist ?) output file already exists or could not be written
   ...
   \retval   C_CONFIG   at least one expected input file for the project
...
int32_t C_PuiProject::SaveCurrentProjectForServiceMode(
  ...
  if ((orc_FilePath != "") &&
    ...
    // Adapt the system view configuration for the creation. At the end it will
    // be reverted.
    C_PuiSvHandler::h_GetInstance()->SetServiceModeActive(true);
    for (u32_ViewCounter = 0U; u32_ViewCounter < u32_ViewCount;
         ++u32_ViewCounter) {
      const std::array<bool, 3> &rc_ViewConfig =
          orc_ViewConfigs[u32_ViewCounter];

      if ((rc_ViewConfig[0] == true) || (rc_ViewConfig[1] == true) ||
          (rc_ViewConfig[2] == true)) {
        // At least one screen of the view is active
        C_PuiSvHandler::h_GetInstance()->SetViewServiceModeActive(
            u32_ViewCounter, true);

        C_PuiSvHandler::h_GetInstance()->SetViewServiceModeSetupActive(
            u32_ViewCounter, rc_ViewConfig[0]);
        C_PuiSvHandler::h_GetInstance()->SetViewServiceModeUpdateActive(
            u32_ViewCounter, rc_ViewConfig[1]);
        C_PuiSvHandler::h_GetInstance()->SetViewServiceModeDashboardActive(
            u32_ViewCounter, rc_ViewConfig[2]);
    ...
    for (u32_ViewCounter = 0U; u32_ViewCounter < u32_ViewCount;
         ++u32_ViewCounter) {
      C_PuiSvHandler::h_GetInstance()->SetViewServiceModeActive(u32_ViewCounter,
                                                                false);
      C_PuiSvHandler::h_GetInstance()->SetViewServiceModeSetupActive(
          u32_ViewCounter, false);
      C_PuiSvHandler::h_GetInstance()->SetViewServiceModeUpdateActive(
          u32_ViewCounter, false);
      C_PuiSvHandler::h_GetInstance()->SetViewServiceModeDashboardActive(
          u32_ViewCounter, false);
...
/*! \brief   Load project
   ...
   \param[in]      opu16_FileVersion               Optional storage for system
   ...
   C_NOACT     A project file is present but structure is invalid (e.g. invalid
   XML file) C_CONFIG    Content of a project file is invalid or incomplete
   C_COM       Device definition not found for project
   C_OVERFLOW  node in system definition references a device not part of the
   device definitions C_CHECKSUM  password is incorrect C_BUSY      Problems
   with removing temporary folders
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::Load(
  ...
  if (this->m_IsServiceModeProject() == false) {
    s32_Retval =
        this->m_LoadProject(opu16_FileVersion, opc_ErrorDetailsMissingDevices);
  } else {
    s32_Retval = this->m_LoadServiceModeProject(
        this->mc_Password, opu16_FileVersion, opc_ErrorDetailsMissingDevices);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_PuiProject::GetPath(void) const { return this->mc_Path; }
...
/*! \brief   Get information about project load switch.
   ...
   \return
   true     successfully loaded a provided project (switch to last known use
   case) false    loaded a project from recent project list (stay at main page)
*/
//----------------------------------------------------------------------------------------------------------------------
...
uint64_t C_PuiProject::h_GetProjectSize(const QString &orc_ProjectPath) {
  ...
  if (c_ProjectFileInfo.exists() == true) {
    ...
    C_PuiProject::h_AdaptProjectPathToSystemDefinition(
        orc_ProjectPath, c_SystemDefinitionFilePath);
...
C_PuiProject::C_PuiProject(void)
    : C_OscProject(), mu32_CalculatedProjectHash(),
...
/*! \brief   Prepare loading initial project
   ...
   Call before LoadInitialProject. If the return value equals C_CHECKSUM, set a
  password by calling SetPassword before calling LoadInitialProject.
...
int32_t C_PuiProject::PrepareLoadInitialProject(void) {
  ...
  // Search in recent projects for existing *.syde file
    ...
    const QStringList c_RecentProjects =
        C_UsHandler::h_GetInstance()->GetRecentProjects();
    this->mq_SwitchToLastKnownUseCase = false;
    SetPath("");
    for (int32_t s32_ItRecentProject = 0;
         s32_ItRecentProject < c_RecentProjects.count();
         ++s32_ItRecentProject) {
      QFileInfo c_File;
...
/*! \brief   Load initial project
   ...
   \param[out]     orc_LoadedProject               project that was tried to
  load (if load fails, this->mc_Path gets overwritten)
   \param[in,out]  opc_ErrorDetailsMissingDevices  (Optional parameter) if
  C_OVERFLOW contains types of all missing devices
  ...
  \return
   ...
   C_RANGE     A project file does not exist
   C_NOACT     A project file is present but structure is invalid (e.g. invalid
  XML file) C_CONFIG    Content of a project file is invalid or incomplete C_COM
  Device definition not found for project C_OVERFLOW  node in system definition
  references a device not part of the device definitions C_CHECKSUM  password is
  incorrect C_BUSY      Problems with removing temporary folders
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::LoadInitialProject(
  ...
  if (s32_Error == C_NO_ERR) {
    ...
    // Load empty project on fail (User feedback for error and next try with
    // recent projects is done by main window)
    this->LoadEmpty();
...
void C_PuiProject::h_HandlePendingEvents(void) {
  ...
  if (c_PreviousFocusWidget != NULL) {
    // Force focus change so any on focus change events (e.g. edit finished)
    // will be processed before the CRC check
    c_PreviousFocusWidget->clearFocus();
    // Process focus change and handle all pending events which may cause
    // project changes before checking the project
    //  for changes
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_PuiProject::h_AdaptProjectPathToSystemDefinition(
    const QString &orc_ProjectPath, QString &orc_SystemDefintionPath) {
  const QFileInfo c_File(orc_ProjectPath);

  orc_SystemDefintionPath = c_File.path() + "/system_definition/" +
                            c_File.completeBaseName() + ".syde_sysdef";
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::SaveAsWithoutInternalChange(
    const QString &orc_FilePath, const bool oq_UseDeprecatedFileFormatV2) {
  return this->m_SaveAs(orc_FilePath, true, oq_UseDeprecatedFileFormatV2,
                        false);
}
...
int32_t C_PuiProject::m_Save(const bool oq_ForceSaveAll,
                             const bool oq_UseDeprecatedFileFormatV2) {
  return this->m_SaveAs(this->mc_Path, oq_ForceSaveAll,
                        oq_UseDeprecatedFileFormatV2, true);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::m_SaveServiceModeProject(const QString &orc_FilePath,
                                               const QString &orc_Password) {
  int32_t s32_Retval;
  ...
  const QString c_TemporaryProjectPath =
      c_TemporaryPath + c_TargetFileName + "/" + c_TargetFileName + ".syde";
  ...
  if (s32_Retval == C_NO_ERR) {
    QStringList c_AllFilesAbsolute;
...

    {
      ...
      s32_Retval = C_OscAesFile::h_CreateEncryptedZipFile(
          c_TemporaryPath, c_AllFilesRelative,
          orc_FilePath,
          orc_Password, &c_ErrorString);
    }
  }
...
/*! \brief   Load project
   ...
   C_RD_WR  Problems accessing file system (e.g. no read access to file)
   C_RANGE  A project file does not exist
   C_NOACT  A project file is present but structure is invalid (e.g. invalid XML
   file) C_CONFIG Content of a project file is invalid or incomplete C_COM
   Device definition not found for project C_OVERFLOW  node in system definition
   references a device not part of the device definitions
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::m_LoadProject(
  ...
  if (this->mc_Path.compare("") == 0) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      // Try newest path
      h_AdaptProjectPathToSystemDefinition(this->mc_Path,
                                           c_SystemDefintionPath);
      {
        ...
        if (c_FileInfoSysDef.exists() == false) {
          mh_AdaptProjectPathToSystemDefinitionV2(this->mc_Path,
                                                  c_SystemDefintionPath);
        }
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          mh_AdaptProjectPathToSystemViews(this->mc_Path, c_SystemViewsPath);
          {
            ...
            if (c_FileInfoSysView.exists() == false) {
              mh_AdaptProjectPathToSystemViewsV1(this->mc_Path,
                                                 c_SystemViewsPath);
            }
            // Load system views
...
/*! \brief   Load project
   ...
   file) C_CONFIG Content of a project file is invalid or incomplete C_COM
   Device definition not found for project C_OVERFLOW  node in system definition
   references a device not part of the device definitions C_CHECKSUM  password
   is incorrect C_BUSY   Problems with removing temporary folders
...
int32_t C_PuiProject::m_LoadServiceModeProject(
  ...
  const QString c_TemporaryPath = c_ExePath + "/" + mhc_TEMP_FOLDER + "/";
  const QString c_TemporaryProjectPath =
      c_TemporaryPath + c_ProjectName + "/" + c_ProjectName + ".syde";
  QString c_ErrorString;

  // Decrypt the encrypted zip file
  s32_Retval = C_OscAesFile::h_UnpackEncryptedZipFile(
      c_OriginalPath,
      c_TemporaryPath, orc_Password,
      &c_ErrorString);
...
/*! \brief   Save project
   ...
   \param[in]  oq_UseDeprecatedFileFormatV2  Flag to enable saving using the
   deprecated V2 file format
   \param[in]  oq_UpdateInternalState        Allow update of internal state
   (only if no export or similar action)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_PuiProject::m_SaveAs(const QString &orc_FilePath,
                               const bool oq_ForceSaveAll,
                               const bool oq_UseDeprecatedFileFormatV2,
  ...
  if (c_Directory.mkpath(".") == true) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if ((C_PuiSdHandler::h_GetInstance()->HasHashChanged() == true) ||
          (oq_ForceSaveAll == true)) {
        QString c_SystemDefintionPath;
        // Each format uses different folders!
        if (oq_UseDeprecatedFileFormatV2) {
          mh_AdaptProjectPathToSystemDefinitionV2(orc_FilePath,
                                                  c_SystemDefintionPath);
        } else {
          h_AdaptProjectPathToSystemDefinition(orc_FilePath,
                                               c_SystemDefintionPath);
        }
        {
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if ((C_PuiSvHandler::h_GetInstance()->HasHashChanged() == true) ||
            (oq_ForceSaveAll == true)) {
          QString c_SystemViewsPath;
          // Each format uses different folders!
          if (oq_UseDeprecatedFileFormatV2) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_PuiProject::mh_AdaptProjectPathToSystemViews(
    const QString &orc_ProjectPath, QString &orc_SystemViewsPath) {
  const QFileInfo c_File(orc_ProjectPath);

  orc_SystemViewsPath = c_File.path() + "/system_views/" +
                        c_File.completeBaseName() + ".syde_sysviews";
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_PuiProject::mh_AdaptProjectPathToSystemViewsV1(
    const QString &orc_ProjectPath, QString &orc_SystemViewsPath) {
  const QFileInfo c_File(orc_ProjectPath);

  orc_SystemViewsPath =
      c_File.path() + "/" + c_File.completeBaseName() + ".syde_sysviews";
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_PuiProject::mh_AdaptProjectPathToSystemDefinitionV2(
    const QString &orc_ProjectPath, QString &orc_SystemDefintionPath) {
  const QFileInfo c_File(orc_ProjectPath);

  orc_SystemDefintionPath =
      c_File.path() + "/" + c_File.completeBaseName() + ".syde_sysdef";
}
