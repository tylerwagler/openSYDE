...
void C_OgeSpxInt64AutoFix::m_Init(void)
{
   ...
   if (this->mc_UnscaledMin.GetType() == this->mc_UnscaledMax.GetType())
   {
      ...
      if (C_OscUtils::h_IsScalingActive(this->mf64_Factor, this->mf64_Offset) == true)
      {
         ...
         if (this->GetIsUnsigned() == true)
         {
            u64_ScaledMin = static_cast<quint64>(f64_Tmp);
            c_ScaledMin = QVariant(static_cast<quint64>(u64_ScaledMin));
         }
         else
         {
            s64_ScaledMin = static_cast<qint64>(f64_Tmp);
         ...
         if (this->GetIsUnsigned() == true)
         {
            u64_ScaledMax = static_cast<quint64>(f64_Tmp);
            c_ScaledMax = QVariant(static_cast<quint64>(u64_ScaledMax));
         }
         else
         {
            s64_ScaledMax = static_cast<qint64>(f64_Tmp);
