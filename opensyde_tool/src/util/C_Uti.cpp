...
/*! \brief   Get number of decimals for floating point value
   ...
   Else Detected number of decimals (Restricted to maximum
   ms32_DOUBLE_SPIN_BOX_DECIMAL_COUNT)
*/
//----------------------------------------------------------------------------------------------------------------------
...
QString C_Uti::h_GetStringFromDouble(const float64_t of64_Value) {
  ...
  // -1 because of:
  // at maximum precision the last decimal digit may be rounded because of first
  // meaningless digit this leads to cases like 2.00000000000001 which we do not
  // want

  // first get a string with very much decimals
  c_StringFromValue =
      QString::number(of64_Value, 'f', ms32_DOUBLE_SPIN_BOX_DECIMAL_COUNT);
  ...
  if (c_SplitString.size() == 2) {
    ...
    // more than one or one that is not zero --> substract from decimal
    // precision
    if ((s32_DigitsWithoutDecimals > 0) && (c_SplitString.at(0) != "0")) {
      s32_Precision -= s32_DigitsWithoutDecimals;
    }
    // one that is zero --> count zeros right of decimal point and add to
    // decimal precision
    else if (c_SplitString.at(0) == "0") {
      // example:
      // 0,0000321 --> 0000321 --> 321 --> 3 --> index of 3 in 0000321 is 4 and
      // this is exactly what should be added
    ...
    for (int32_t s32_Pos = c_StringFromValue.length() - 1; s32_Pos > 0;
         s32_Pos--) {
...
/*! \brief   Scales the color
   ...
   The functions adapts the color by calculating the percentage of the
   difference to the maximum 255 of all three colors.
...
*/
//----------------------------------------------------------------------------------------------------------------------
QColor C_Uti::h_ScaleColor(const QColor &orc_Color,
  ...
  if (ou32_Percentage != 100U) {
    ...
    if (ou32_Percentage > 0) {
      s32_DeltaColor = (s32_DeltaColor * 100) /
                       (10000 / static_cast<int32_t>(ou32_Percentage));
    } else {
      s32_DeltaColor = 0;
    }
...
QColor C_Uti::h_GetDisabledColorLook(const QColor &orc_DefaultColorLook) {
  const int32_t s32_Red =
      orc_DefaultColorLook.red() + ((255 - orc_DefaultColorLook.red()) / 2);
  const int32_t s32_Green =
      orc_DefaultColorLook.green() + ((255 - orc_DefaultColorLook.green()) / 2);
  const int32_t s32_Blue =
      orc_DefaultColorLook.blue() + ((255 - orc_DefaultColorLook.blue()) / 2);
  const QColor c_Retval(s32_Red, s32_Green, s32_Blue,
                        orc_DefaultColorLook.alpha());

  return c_Retval;
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Adapts the string with three dots at the end if not enough space is
   available
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_Uti::h_AdaptStringToSize(const QString &orc_String,
                                   const QFontMetrics &orc_FontMetrics,
...
QString C_Uti::h_AdaptStringToSize(const QString &orc_String,
  ...
  return C_Uti::h_AdaptStringToSize(orc_String, orc_FontMetrics,
                                    static_cast<int32_t>(of64_Width));
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_Uti::h_CheckKeyModifier(
    const Qt::KeyboardModifiers &orc_ActiveKeyModifiers,
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_Uti::h_CheckStyleState(const QStyle::State &orc_ActiveState,
                              const QStyle::StateFlag &ore_CheckState) {
...
/*! \brief   Get current executable path (No slash at end)
   ...
   Note: Needs a QApplication object to have been instanced (otherwise the
   DirPath is empty)
...
/*! \brief   Set current directory to exe directory
   ...
   Depending on how the binary is executed those might be different when
   starting resulting in unexpected behavior. This function can be used to make
   sure both folders are the same.
*/
//----------------------------------------------------------------------------------------------------------------------
...
/*! \brief   Get resource version number of a file as an QString
   ...
   This function is Windows specific and needs to be replaced by another
   solution when porting to a non-Windows system
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_Uti::h_CheckAndReplaceWithExePathIfNecessary(const QString &orc_Path) {
  QString c_Retval;
...
/*! \brief   Convert text to styled link
   ...
   symbol, especially in text browsers. Just use "<br>" instead to make it work
   fine.
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_Uti::h_GetLink(const QString &orc_DisplayedText,
                         const QColor &orc_Color,
                         const QString &orc_HyperLink) {
  const QString c_Retval =
      static_cast<QString>("<a href=\"%5\">"
                           "<span style=\"color: rgba(%1,%2,%3,%4);\">" +
                           orc_DisplayedText + "</span></a>")
          .arg(QString::number(orc_Color.red()),
               QString::number(orc_Color.green()),
               QString::number(orc_Color.blue()),
               QString::number(orc_Color.alpha()), orc_HyperLink);
...
QString C_Uti::h_GetCompleteLogFileLocation(const QString &orc_Extension) {
  ...
  c_Retval = static_cast<QString>("%1/Logs/%2")
                 .arg(C_Uti::h_GetExePath())
                 .arg(c_FileBaseName);
...
QString C_Uti::h_StwError(const int32_t os32_Error) {
  return C_OscLoggingHandler::h_StwError(os32_Error);
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Minimize full paths to fit in GUI edit fields.

   The given width of an edit field for example is optimally used, so that a
   user can expect a full file path. The drive name, the file name and as many
   subfolders as possible can be seen for the given width in points. The number
   of characters seen is dependent of the used font type and size. The
   subfolders are recursively tried to fit for the given width from the end of
   the path and completely showed:
   ...
   * do not convert the result of this function to std::string because the
   ellipsis "..." has an Unicode character representation (U+2026).

   \param[in]  orc_Path       full file path
   \param[in]  orc_Font       font to calculate number of maximal possible
   characters
   \param[in]  ou32_Width     size in pixels for minimized file path to fit e.g.
   in line edit field
   \param[in]  ou32_Buffer    left and right margin for text e.g. in line edit
   field, if this parameter is not needed then set value to 0.

   \return
   minimized file path
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_Uti::h_MinimizePath(const QString &orc_Path, const QFont &orc_Font,
                              const uint32_t ou32_Width,
  ...
  if (orc_Path != "") {
    ...
    const QString c_Path =
        c_FileInfo.filePath(); // in case we have windows-style path
    // get drive name
    const QStorageInfo c_StorageInfo(c_FileInfo.absoluteDir());
    QString c_DriveName = c_StorageInfo.rootPath();

    // try to extract drive name from string in case of not existing files or
    // relative path
    if ((c_DriveName == "") || // not existing path: drive name is empty
        (c_FileInfo.isRelative() ==
         true)) // relative path: use characters before first "/" (e.g. "./" or
                // "Dir")
    {
      const QStringList c_Temp = c_Path.split(
          "/"); // c_Path is Qt style path, i.e. uses "/" as separators
      if (c_Temp.size() > 1) // if no "/" occurs in path, split returns a list
    ...
    const QString c_Placeholder =
        static_cast<QString>(static_cast<QChar>(0x2026)) +
        "/"; // with horizontal ellipsis
             // "..."
    ...
    if (s32_CurrentWidth > static_cast<int32_t>(u32_AvailableWidth)) {
      ...
      if (s32_CurrentWidth < static_cast<int32_t>(u32_AvailableWidth)) {
        ...
        c_MinimizedPath = c_Path;
        int32_t s32_DelimPos =
            c_MinimizedPath.indexOf("/", 0); // first delimiter
        do {
          // get next folder
          s32_DelimPos =
              c_MinimizedPath.indexOf("/", s32_DelimPos + 1); // next delimiter

          c_MinimizedPath = c_DriveName + c_Placeholder +
                            c_MinimizedPath.right(
                                (c_MinimizedPath.length() - s32_DelimPos) - 1);

          s32_DelimPos =
              static_cast<QString>(c_DriveName + c_Placeholder).length();
          s32_CurrentWidth = c_FontMetrics.horizontalAdvance(c_MinimizedPath);
        }
        // second condition should not occur and is only defensive
        while (
            (s32_CurrentWidth > (static_cast<int32_t>(u32_AvailableWidth))) ||
            (s32_DelimPos == -1));
      } else {
        // drive and filename does not fit for minimizing therefore we do a hard
        // cut of the full path
        c_MinimizedPath = c_FontMetrics.elidedText(
            c_Path, Qt::ElideMiddle, static_cast<int32_t>(u32_AvailableWidth),
...
/*! \brief   Convert version in format 1.00.0 to STW format V1.00r0
   ...
   \param[in]  orc_Version    version in format 1.00.0 (e.g.
   APPLICATION_VERSION)
...
/*! \brief   Utility function to convert relative path to absolute path if
   ...
   Consolidation (cleaning of redundant "." and "..") will be performed unless
   the path contains a "%" sign.
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_Uti::h_ConcatPathIfNecessary(const QString &orc_BaseDir,
                               const QString &orc_RelativeOrAbsolutePath) {
  QString c_Result = C_OscUtils::h_ConcatPathIfNecessary(
      orc_BaseDir, orc_RelativeOrAbsolutePath);
...
/*! \brief   Create ascending sorted index map for input vector
   ...
   -> set the 7. output element to be 0 (we said this is the first found
   element)
   -> and so on

   Once the function is done you can iterate over the output
   and the first non negative value you find is the index of the "lowest" value
   element Also as you might notice this will directly make the output unique
   ...
   otherwise: index was part of input vector and the value shows at which
   position this index was standing
...
/*! \brief   Handle paths after file save dialog.
   ...
   Note: If one of the input paths is empty this simply returns the given path
   and its concatenation with the directory. If the reference path is a file
   path (ending on File.txt), behavior is undefined (handling directory AND
   files AND existing AND non-existing did not work well). If the reference path
   is not absolute, behavior is undefined (Qt then defaults to calling path,
   which is often but not always the path of the executable).

   \param[in]   orc_PathIn                relative or absolute path of file or
   directory
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_Uti::h_IsPathRelativeToDir(const QString &orc_PathIn,
                                  const QString &orc_AbsoluteReferenceDir,
                                  QString &orc_PathAbsolute,
                                  QString &orc_PathRelative) {
  ...
  if ((orc_PathIn.isEmpty() == false) &&
      (orc_AbsoluteReferenceDir.isEmpty() == false)) {
    ...
    if (oq_Return == true) {
      ...
      orc_PathAbsolute =
          C_Uti::h_ConcatPathIfNecessary(orc_AbsoluteReferenceDir, orc_PathIn);
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_Uti::h_GetValueAsHex(const uint64_t ou64_Value,
                               const uint8_t ou8_FieldWidth) {
  return "0x" + static_cast<QString>("%1")
                    .arg(ou64_Value, ou8_FieldWidth, 16, QChar('0'))
                    .toUpper();
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_Uti::h_GetValueAsHex(const uint32_t ou32_Value,
                               const uint8_t ou8_FieldWidth) {
  return C_Uti::h_GetValueAsHex(static_cast<uint64_t>(ou32_Value),
                                ou8_FieldWidth);
}
...
/*! \brief   Parses a specific folder for files
   ...
   All files will be added to orc_FilePaths and all found folders will be
   checked recursively. orc_FilePaths will not be cleared
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_Uti::h_GetAllFilePathsInFolder(const QString &orc_FolderPath,
                                      QStringList &orc_FilePaths) {
  const QDir c_Dir(orc_FolderPath);
  const QStringList c_AllFiles =
      c_Dir.entryList(QDir::Files | QDir::AllDirs | QDir::NoDotAndDotDot);
  int32_t s32_Counter;
