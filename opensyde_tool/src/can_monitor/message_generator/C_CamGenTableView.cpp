...
C_CamGenTableView::C_CamGenTableView(QWidget *const opc_Parent)
    : C_TblViewInteraction(opc_Parent), mq_CommunicationActive(false),
  ...
  connect(&this->mc_Model, &C_CamGenTableModel::SigItemCountChanged, this,
          &C_CamGenTableView::SigItemCountChanged);
  connect(&this->mc_Model, &C_CamGenTableModel::SigUpdateMessageDlc, this,
          &C_CamGenTableView::SigUpdateMessageDlc);
  connect(&this->mc_Model, &C_CamGenTableModel::SigRegisterCyclicMessage, this,
          &C_CamGenTableView::m_ModelRegisterCyclicMessage);
  connect(this, &C_CamGenTableView::SigLinkClicked, this,
          &C_CamGenTableView::m_HandleLinkClicked);
  connect(&this->mc_Model, &C_CamGenTableModel::SigItemCheck, this,
          &C_CamGenTableView::m_HandleCheckChange);
  connect(&this->mc_Model, &C_CamGenTableModel::SigReport, this,
          &C_CamGenTableView::m_Report);
...
void C_CamGenTableView::AddMessageFromDatabase(void) {
  ...
  C_CamMosDatabaseSelectionPopup *const pc_Dialog =
      new C_CamMosDatabaseSelectionPopup(*c_New, true);
...
/*! \brief  Restore column widths
   ...
   \param[in]  orc_ColumnWidths  Stored column widths (Restores default values
   if empty)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableView::SetCurrentColumnWidths(
  ...
  if (orc_ColumnWidths.size() > 0) {
    for (uint32_t u32_ItCol = 0; u32_ItCol < orc_ColumnWidths.size();
         ++u32_ItCol) {
      this->setColumnWidth(static_cast<int32_t>(u32_ItCol),
                           orc_ColumnWidths[u32_ItCol]);
    }
  } else {
    // Default
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eNAME), 230);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eID), 60);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eXTD), 52);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eRTR), 51);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eDLC), 51);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eDATA),
        mhs32_COL_WIDTH_DATA);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eCYCLIC_TRIGGER),
        63);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eCYCLIC_TIME),
        94);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eKEY), 60);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eMANUAL_TRIGGER),
        120);
    this->setColumnWidth(
        C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eAUTO_SUPPORT),
        110);
  }
}
...
void C_CamGenTableView::SaveUserSettings(void) const {
  C_UsHandler::h_GetInstance()->SetMessageColWidths(
      this->GetCurrentColumnWidths());
  C_UsHandler::h_GetInstance()->SetSelectedMessages(
      C_CamGenTableView::mh_ConvertVector(this->m_GetSelectedRows()));
}
...
void C_CamGenTableView::LoadUserSettings(void) {
  this->SetCurrentColumnWidths(
      C_UsHandler::h_GetInstance()->GetMessageColWidths());
  this->m_SetSelectedMessages(
      C_UsHandler::h_GetInstance()->GetSelectedMessages());
}
...
/*! \brief  Remove all messages for the specified file
   ...
   \param[in]  opc_Indices    Optional indices to specify which messages should
   be deleted instead of all
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableView::RemoveMessagesForFile(
  ...
  if (opc_Indices != nullptr) {
    ...
    C_CamProHandler::h_GetInstance()->GetAllMessagesFromDatabase(orc_File, nullptr,
                                                                 &c_Indices);
  }
...
/*! \brief  Request trigger of model function for update cyclic message
   ...
   Reason: have one central point for each check which has to be done when
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableView::TriggerModelUpdateCyclicMessage(
...
bool C_CamGenTableView::CheckAndHandleKey(const QString &orc_Input) {
  ...
  if (this->mq_CommunicationActive) {
    // Check all messages
    for (uint32_t u32_ItMessage = 0UL;
         u32_ItMessage < C_CamProHandler::h_GetInstance()->GetMessages().size();
         ++u32_ItMessage) {
...
void C_CamGenTableView::keyPressEvent(QKeyEvent *const opc_Event) {
  ...
  case Qt::Key_Delete:
    ...
    opc_Event->accept();
    break;
  case Qt::Key_BracketRight: // Qt::Key_BracketRight matches the
                             // "Not-Num-Plus"-Key
  case Qt::Key_Plus:
    if (C_Uti::h_CheckKeyModifier(opc_Event->modifiers(),
                                  Qt::ControlModifier) == true) {
  ...
  if (q_CallOrig == true) {
    ...
    if (this->selectedIndexes().size() > 0L) {
      ...
      const QSpinBox *const pc_SpinBox =
          dynamic_cast<const QSpinBox *const>(this->focusWidget());
      const QLineEdit *const pc_LineEdit =
          dynamic_cast<const QLineEdit *const>(this->focusWidget());
      const QComboBox *const pc_ComboBox =
          dynamic_cast<const QComboBox *const>(this->focusWidget());
      if (((pc_SpinBox == nullptr) && (pc_LineEdit == nullptr)) &&
          (pc_ComboBox == nullptr)) {
        // If no editor check current index
        bool q_Ignore = false;
        const QModelIndex c_Current =
            this->mc_SortProxyModel.mapToSource(this->currentIndex());
        const C_CamGenTableModel::E_Columns e_Col =
            C_CamGenTableModel::h_ColumnToEnum(c_Current.column());
        ...
        if (q_Ignore) {
          // ignore if not relevant cell -> cell can not be edited via single
          // key press (exception: space ignored)
          opc_Event->ignore();
        } else {
        ...
        // Don't ignore event
        // Check first if the key is relevant for message triggering (so the
        // event might have to be ignored or not)
        if (opc_Event->text().isEmpty() == false) {
          // Explicitly accept the event if entering edit mode -> does not
          // natively accept
          opc_Event->accept();
        }
      }
...
/*! \brief  Returns the hint for size column
   ...
   resizeColumnToContents is used by double click on the separator in the header
   too.
...
QList<uint32_t> C_CamGenTableView::m_GetSelectedRows(void) const {
  ...
  for (QModelIndexList::const_iterator c_It = c_Indices.begin();
       c_It != c_Indices.end(); ++c_It) {
...
void C_CamGenTableView::m_SetupContextMenu(void) {
  ...
  this->mpc_ActionAddFromDatabase = this->mpc_ContextMenu->addAction(
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Utility function to select the specified row (clear other selection)
   and scroll to this row (first column)
...
void C_CamGenTableView::m_SelectAndScrollToRow(const uint32_t ou32_Row) {
  const QModelIndex c_Index =
      this->mc_SortProxyModel.mapFromSource(this->mc_Model.index(ou32_Row, 0));
...
void C_CamGenTableView::m_RepositionButtons(void) {
  ...
  // Position
  this->mpc_PushButtonScrollTop->setGeometry(
      QRect(QPoint(0, 0), c_BUTTON_SIZE_UP));
}
...
void C_CamGenTableView::m_HandleLinkClicked(const QModelIndex &orc_Index) {
  ...
  if (s32_Row >= 0) {
    if (C_CamGenTableModel::h_ColumnToEnum(
            this->mc_SortProxyModel.mapToSource(orc_Index).column()) ==
        C_CamGenTableModel::eMANUAL_TRIGGER) {
      if (this->mq_CommunicationActive) {
        Q_EMIT(this->SigSendMessage(
            static_cast<uint32_t>(s32_Row),
            static_cast<uint32_t>(QDateTime::currentMSecsSinceEpoch())));
      }

      else {
        C_OgeWiCustomMessage c_Message(this,
                                       C_OgeWiCustomMessage::eINFORMATION);
        c_Message.SetHeading("Measurement not started");
        c_Message.SetDescription(
            "The transmission of messages is only allowed as long "
            "as the measurement is running. \nClick the play button "
            "in trace to start measurement.");
        c_Message.Execute();
      }
    } else if (C_CamGenTableModel::h_ColumnToEnum(
                   this->mc_SortProxyModel.mapToSource(orc_Index).column()) ==
    ...
    } // lint !e429  no memory leak because of the parent pc_Dialog and the Qt
      // memory management
    else {
      // Unexpected
    }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Stop cyclic communication for these items if necessary (if currently
   is registered)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableView::m_ModelRegisterCyclicMessage(
    const uint32_t ou32_MessageIndex, const bool oq_Active) {
...
void C_CamGenTableView::m_AddMessageFromDatabase(
  ...
  for (uint32_t u32_ItNewItem = 0UL; u32_ItNewItem < orc_NewItems.size();
    ...
    c_NewMessage.c_Name = orc_NewItems[u32_ItNewItem][1UL];
    c_NewMessage.SetMessageBoolValue(C_CamProMessageData::eGBODS_RTR, false);
    c_NewMessage.SetMessageBoolValue(C_CamProMessageData::eGBODS_DO_CYCLIC,
                                     false);
    c_NewMessage.SetMessageBoolValue(C_CamProMessageData::eGBODS_AUTO_SUPPORT,
                                     false);

    if (c_FileInfo.suffix().compare("dbc", Qt::CaseInsensitive) == 0) {
      // First prepare C_CamDbHandler
      if (C_CamDbHandler::h_GetInstance()->FindDbcMessage(
      ...
      if (C_CamDbHandler::h_GetInstance()->FindOsyMessage(
        ...
        const C_OscNodeDataPoolList *const pc_List =
            C_CamDbHandler::h_GetInstance()->GetOscList(
                orc_NewItems[u32_ItNewItem][0UL],
                orc_NewItems[u32_ItNewItem][1UL], false, 0UL);
  ...
  // Add complete message
  c_NewItems = this->mc_Model.AddSpecificNewItems(this->m_GetSelectedRows(),
                                                  c_NewMessages);
...
void C_CamGenTableView::m_HandleNewItemScrollingAndSelection(
  ...
  if (c_Items.size() > 0UL) {
    ...
    if (c_Items.size() > 1) {
      const QModelIndex c_Index = this->mc_SortProxyModel.mapFromSource(
          this->mc_Model.index(u32_HighestItem, 0));
      this->scrollTo(c_Index);
    } else {
  ...
  if ((this->mq_CommunicationActive == true) &&
    ...
    for (uint32_t u32_It = 0UL; u32_It < c_Items.size(); ++u32_It) {
      const C_CamProMessageData *const pc_Message =
          C_CamProHandler::h_GetInstance()->GetMessageConst(c_Items[u32_It]);
      if (pc_Message != nullptr) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableView::m_SelectRow(
    const int32_t os32_Row,
    const QItemSelectionModel::SelectionFlags &orc_Flags) {
  const QModelIndex c_StartIndex =
      this->mc_SortProxyModel.mapFromSource(this->mc_Model.index(os32_Row, 0));
  const QModelIndex c_EndIndex = this->mc_SortProxyModel.mapFromSource(
      this->mc_Model.index(os32_Row, this->mc_Model.columnCount() - 1));
...
void C_CamGenTableView::m_RegisterAllCyclicMessages(void) {
  if ((this->mq_CyclicTransmissionActive == true) &&
  ...
  // remove of all current cyclic messages is handled by main window
  // (start/stop) resp. by messages widget (toggle)
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableView::m_Report(const C_OgeWiCustomMessage::E_Type oe_Type,
                                 const QString &orc_Heading,
