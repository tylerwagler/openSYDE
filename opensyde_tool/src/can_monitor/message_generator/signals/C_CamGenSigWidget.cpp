...
C_CamGenSigWidget::C_CamGenSigWidget(QWidget *const opc_Parent)
    : C_OgeWiOnlyBackground(opc_Parent), mpc_Ui(new Ui::C_CamGenSigWidget),
      mu32_NumSelectedItems(0UL), mu32_Row(0UL) {
  ...
  this->mpc_Ui->pc_LabelHeadingDynamic->SetFontPixel(
      14, true, false); // also used in m_SetDynamicNameElided()
  this->mpc_Ui->pc_LabelNoDatabase->SetForegroundColor(8);
  this->mpc_Ui->pc_LabelNoDatabase->SetFontPixel(12, false, false);

  connect(this->mpc_Ui->pc_TableView,
          &C_CamGenSigTableView::SigUpdateMessageData, this,
          &C_CamGenSigWidget::SigUpdateMessageData);
  connect(this->mpc_Ui->pc_TableView,
          &C_CamGenSigTableView::SigTriggerModelUpdateCyclicMessage, this,
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_CamGenSigWidget::~C_CamGenSigWidget(void) { delete this->mpc_Ui; }
...
void C_CamGenSigWidget::TriggerSignalReload(void) {
  ...
  // Restore last known selection -> also handles check if database is still
  // available
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenSigWidget::UpdateMessageDlc(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenSigWidget::UpdateSelection(const uint32_t ou32_NumSelectedItems,
                                        const uint32_t ou32_Row) {
  QString c_Name = "-";
  ...
  if (ou32_NumSelectedItems == 0UL) {
    ...
    // Set the index first (should locate the message in the database, if not
    // done already)
    ...
    if (ou32_Row < C_CamProHandler::h_GetInstance()->GetMessages().size()) {
      const C_CamProMessageData &rc_Message =
          C_CamProHandler::h_GetInstance()->GetMessages()[ou32_Row];
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenSigWidget::m_Reset(void) { this->UpdateSelection(0UL, 0UL); }
