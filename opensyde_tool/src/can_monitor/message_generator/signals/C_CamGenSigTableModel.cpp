...
C_CamGenSigTableModel::C_CamGenSigTableModel(QObject *const opc_Parent)
    : QAbstractTableModel(opc_Parent), mu32_MessageIndex(0UL) {}
...
void C_CamGenSigTableModel::SetMessage(const uint32_t ou32_Message) {
  ...
  if (pc_Message != NULL) {
    ...
    C_CamDbHandler::h_GetInstance()->FindOsyMessage(
        pc_Message->c_DataBaseFilePath, pc_Message->c_Name);
  }
  this->endResetModel();
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamGenSigTableModel::headerData(const int32_t os32_Section,
                                           const Qt::Orientation oe_Orientation,
                                           const int32_t os32_Role) const {
  QVariant c_Retval =
      QAbstractTableModel::headerData(os32_Section, oe_Orientation, os32_Role);
  ...
  if (oe_Orientation == Qt::Orientation::Horizontal) {
    ...
    if (os32_Role == static_cast<int32_t>(Qt::DisplayRole)) {
      ...
      case ePHYSICAL:
        c_Retval =
            "CAN message signal physical value (=interpreted/scaled value)";
        break;
      case eUNIT:
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CamGenSigTableModel::columnCount(const QModelIndex &orc_Parent) const {
...
int32_t C_CamGenSigTableModel::rowCount(const QModelIndex &orc_Parent) const {
  ...
  if (orc_Parent.isValid() == false) {
        ...
        if (q_IsMultiplexed) {
          ...
          if (pc_OsyMessage != NULL) {
            ...
            if ((C_CamProHandler::h_GetInstance()
                     ->GetMessageConst(this->mu32_MessageIndex)
                     ->q_SetAutoSupportMode == true) &&
                (e_ProtocolType == C_OscCanProtocol::eECES)) {
              s32_Retval =
                  static_cast<int32_t>(pc_OsyMessage->c_Signals.size()) -
                  2; // Not showing Message
    ...
    // Qt documentation: When implementing a table based model, rowCount()
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamGenSigTableModel::data(const QModelIndex &orc_Index,
                                     const int32_t os32_Role) const {
  QVariant c_Retval;

  if (orc_Index.isValid() == true) {
    const C_CamGenSigTableModel::E_Columns e_Col =
        h_ColumnToEnum(orc_Index.column());
    if (os32_Role == static_cast<int32_t>(Qt::ForegroundRole)) {
      switch (e_Col) // lint !e788  //not all constants explicitly handled on
                     // purpose
      {
      case eRAW:
      case ePHYSICAL:
      ...
      if (this->m_CheckInterpretedMode()) {
        if (e_Col == eRAW) {
          ...
          if ((os32_Role == static_cast<int32_t>(Qt::DisplayRole)) ||
              ...
              if ((pc_OsySignalCommon != NULL) && (pc_OsySignal != NULL)) {
                ...
                if (pc_DbcSignal != NULL) {
                  ...
                  if (pc_DbcSignal->e_MultiplexerType ==
                      C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) {
                    c_Name =
                        static_cast<QString>("%1 (Multiplexer)").arg(c_Name);
                  }
                  c_Retval = c_Name;
                } else {
                  // Default
            ...
            if (e_Col == eNAME) {
              // Return signal name
              c_Retval =
                  this->data(orc_Index, static_cast<int32_t>(Qt::DisplayRole));
            } else {
              // Default: no tool tip
            }
          } else if (os32_Role == ms32_USER_ROLE_TOOL_TIP_CONTENT) {
            if (e_Col == eNAME) {
              ...
              // Tool tip
              c_Retval = C_SdTooltipUtil::h_GetToolTipContentSignal(
                  c_OsySignal, c_OsyMessage, c_OsySignalCommon, e_ProtocolType,
                  "", "");
            } else {
              // Default: no tool tip
            }
        ...
        if ((os32_Role == static_cast<int32_t>(Qt::DisplayRole)) ||
          ...
          switch (e_Col) // lint !e788  //not all constants explicitly handled
                         // on purpose
          {
          case eNAME:
          ...
          case eRAW:
            ...
            if ((pc_Message != NULL) && (u32_Index < 8UL)) {
              if (os32_Role == static_cast<int32_t>(Qt::DisplayRole)) {
                ...
                c_Retval =
                    static_cast<qulonglong>(pc_Message->c_Bytes[u32_Index]);
              }
            }
            break;
          default:
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamGenSigTableModel::setData(const QModelIndex &orc_Index,
  ...
  if (data(orc_Index, os32_Role) != orc_Value) {
    if (orc_Index.isValid() == true) {
      ...
      Q_EMIT(this->SigTriggerModelUpdateCyclicMessage(this->mu32_MessageIndex,
                                                      false));
      if (this->m_CheckInterpretedMode()) {
        ...
        C_OscNodeDataPoolContent c_Value =
            mh_GetInitialValue(pc_OsySignalCommon, pc_DbcSignal);
        // Handle check box
        if (this->flags(orc_Index).testFlag(Qt::ItemIsUserCheckable)) {
          bool q_Val;
          const Qt::CheckState e_CheckedState =
              static_cast<Qt::CheckState>(orc_Value.toInt());
          ...
          C_SdNdeDpContentUtil::h_SetDataVariableFromGenericWithScaling(
              orc_Value, c_Value, f64_Factor, f64_Offset, 0UL);
        }
        ...
        if (this->m_SetSignalFromOsyValue(c_Signal, c_Value) == C_NO_ERR) {
          ...
          if (c_Signal.e_MultiplexerType !=
            ...
            if (e_Col == C_CamGenSigTableModel::ePHYSICAL) {
              const QModelIndex c_Index = this->index(
                  orc_Index.row(), C_CamGenSigTableModel::h_EnumToColumn(
        ...
        if (e_Col == C_CamGenSigTableModel::eRAW) {
          ...
          if (C_CamProHandler::h_GetInstance()->SetMessageUint32Value(
                  this->mu32_MessageIndex, e_Selector,
                  static_cast<uint32_t>(orc_Value.toULongLong(&q_Ok))) ==
              C_NO_ERR) {
            if (q_Ok == true) {
              q_Retval = true;
            }
...
Qt::ItemFlags C_CamGenSigTableModel::flags(const QModelIndex &orc_Index) const {
  ...
  if (orc_Index.isValid() == true) {
    ...
    {
    case eRAW:
      c_Retval = c_Retval | Qt::ItemIsEnabled | Qt::ItemIsSelectable |
                 Qt::ItemIsEditable;
        ...
        if ((pc_OsySignal != NULL) && (pc_OsySignalCommon != NULL)) {
          const C_OscNodeDataPoolContent c_Min =
              C_CamGenSigTableModel::mh_GetBorderValue(
                  ...
                  pc_OsySignal->u16_ComBitLength, true);
          const C_OscNodeDataPoolContent c_Max =
              C_CamGenSigTableModel::mh_GetBorderValue(
                  pc_OsySignalCommon->c_MinValue,
                  pc_OsySignal->u16_ComBitLength, false);
          ...
          if (pc_DbcSignal != NULL) {
            const C_OscNodeDataPoolContent c_Min =
                C_CamGenSigTableModel::mh_GetBorderValue(
                    ...
                    pc_DbcSignal->u16_ComBitLength, true);
            const C_OscNodeDataPoolContent c_Max =
                C_CamGenSigTableModel::mh_GetBorderValue(
                    pc_DbcSignal->c_Element.c_MinValue,
                    pc_DbcSignal->u16_ComBitLength, false);
            if (m_CheckUseCheckBox(c_Min, c_Max)) {
              c_Retval = c_Retval | Qt::ItemIsUserCheckable;
            }
          }
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_CamGenSigTableModel::E_Columns
C_CamGenSigTableModel::h_ColumnToEnum(const int32_t os32_Column) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamGenSigTableModel::h_EnumToColumn(
    const C_CamGenSigTableModel::E_Columns oe_Value) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
const stw::opensyde_core::C_OscCanSignal *
...
C_OscNodeDataPoolContent C_CamGenSigTableModel::mh_DecodeRawToContentDbc(
  ...
  c_Retval = C_CamGenSigUtil::h_DecodeRawToContentSignal(
      orc_Raw, c_Signal, orc_Signal.c_Element.c_MinValue);
  return c_Retval;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamGenSigTableModel::m_CheckUseCheckBox(
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamGenSigTableModel::m_HandleColRawInterpreted(
    const uint32_t ou32_Index, const int32_t os32_Role) const {
  QVariant c_Retval;
  ...
  if ((os32_Role == static_cast<int32_t>(Qt::DisplayRole)) ||
          ...
          if (pc_Message != NULL) {
            ...
            if (os32_Role == static_cast<int32_t>(Qt::DisplayRole)) {
              ...
              c_Retval = C_SdNdeDpContentUtil::h_ConvertScaledContentToGeneric(
                  c_ConvertedContent, 1.0, 0.0, 0UL);
            }
          }
        }
      }
    ...
    c_Retval = static_cast<int32_t>(eURIEL_GENERIC_SPIN_BOX);
             ...
             ms32_USER_ROLE_INTERACTION_GENERIC_SPIN_BOX_PARAMETERS_LIST) {
    const C_OscNodeDataPoolListElement *const pc_OsySignalCommon =
        m_GetSignalInterpretedOsyCommon(ou32_Index);
    const C_OscCanSignal *const pc_OsySignal =
        m_GetSignalInterpretedOsy(ou32_Index);
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamGenSigTableModel::m_HandleColPhysicalInterpreted(
    const uint32_t ou32_Index, const int32_t os32_Role) const {
  QVariant c_Retval;
  ...
  if ((os32_Role == static_cast<int32_t>(Qt::DisplayRole)) ||
      ...
      if (pc_OsySignalCommon != NULL) {
        ...
        if (pc_Message != NULL) {
          ...
          if (os32_Role == static_cast<int32_t>(Qt::DisplayRole)) {
            QString c_DisplayValue;
            C_SdNdeDpContentUtil::h_GetValueAsScaledString(
                c_ConvertedContent, pc_OsySignalCommon->f64_Factor,
                pc_OsySignalCommon->f64_Offset, c_DisplayValue, 0UL);
            c_Retval = c_DisplayValue;
          } else {
            c_Retval = C_SdNdeDpContentUtil::h_ConvertScaledContentToGeneric(
                c_ConvertedContent, pc_OsySignalCommon->f64_Factor,
                pc_OsySignalCommon->f64_Offset, 0UL);
          }
        }
      }
    } else {
      ...
      if (pc_DbcSignal != NULL) {
        ...
        if (pc_Message != NULL) {
          ...
          if (os32_Role == static_cast<int32_t>(Qt::DisplayRole)) {
            QString c_DisplayValue;
            C_SdNdeDpContentUtil::h_GetValueAsScaledString(
                c_ConvertedContent, pc_DbcSignal->c_Element.f64_Factor,
                pc_DbcSignal->c_Element.f64_Offset, c_DisplayValue, 0UL);
            c_Retval = c_DisplayValue;
          } else {
            // Value table needs raw value
            if (pc_DbcSignal->c_ValueDescription.size() > 0UL) {
              ...
              c_Retval = C_SdNdeDpContentUtil::h_ConvertScaledContentToGeneric(
                  c_ConvertedContent, pc_DbcSignal->c_Element.f64_Factor,
                  pc_DbcSignal->c_Element.f64_Offset, 0UL);
            }
          }
        }
      }
    ...
    if (pc_DbcSignal != NULL) {
      ...
      const uint64_t u64_AllowedNumValuesSigned =
          C_CamGenSigTableModel::mh_GetMax(pc_DbcSignal->u16_ComBitLength -
                                           1UL);
      const uint64_t u64_AllowedNumValues =
          C_CamGenSigTableModel::mh_GetMax(pc_DbcSignal->u16_ComBitLength);
      bool q_IsSigned;
      QStringList c_Strings;
      // Check for length and number of bits
      switch (pc_DbcSignal->c_Element.c_MinValue
                  .GetType()) // lint !e788  //not all types handled explicitly
      {
    ...
    // Interaction type -> combo box or spin box (no check box for physical
    // value)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenSigTableModel::mh_SetBoolInContent(
    C_OscNodeDataPoolContent &orc_Value, const bool oq_Value) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamGenSigTableModel::m_SetSignalFromOsyValue(
    const C_OscCanSignal &orc_OsySignal,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Function to get border (min/max) values depending on current bit
   length
   ...
   \return
   Border value with the init value type and the value set as specified by the
   bit length and the border flag
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolContent C_CamGenSigTableModel::mh_GetBorderValue(
    const C_OscNodeDataPoolContent &orc_InitValue,
  ...
  if (ou16_BitLength > 64U) {
    ...
    // Error handling
    osc_write_log_warning("Message signal edit",
                          "Signals over 64 bit are not supported");
  } else {
    float64_t f64_Base;
    uint64_t u64_Base;
    int64_t s64_Base;
    ...
    case C_OscNodeDataPoolContent::eSINT64:
      ...
      if (oq_IsMin) {
        ...
        if (ou16_BitLength >= 64) {
          ...
          // Use one bit less for signed
          const uint64_t u64_Max =
              C_CamGenSigTableModel::mh_GetMax(ou16_BitLength - 1UL);
          s64_Base = static_cast<int64_t>(u64_Max);
          // Invert to negative
          s64_Base *= -1LL;
          // Subtract one more as negative range is one bigger than positive
          // range
          s64_Base -= 1LL;
        }
      } else {
    ...
    case C_OscNodeDataPoolContent::eFLOAT64:
      ...
      if (oq_IsMin) {
        ...
        if (ou16_BitLength >= 64) {
          ...
          f64_Base =
              static_cast<float64_t>(std::numeric_limits<float32_t>::lowest());
        }
      } else {
        // Special handling for 64 bit
        if (ou16_BitLength >= 64) {
      ...
      // Apply value
      C_OscNodeDataPoolContentUtil::h_SetValueInContent(f64_Base, c_Retval,
                                                        0UL);
      break;
    default:
      Q_ASSERT(false);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Get maximum value for an unsigned variable with the specified number
   of bits

   \param[in]  ou16_Bit    Number of bits for variable (only supported up to 64
   bit)
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamGenSigTableModel::mh_IsMultiplexed(
  ...
  if (opc_OsyMessage != NULL) {
    ...
    for (uint32_t u32_ItSig = 0UL; u32_ItSig < opc_DbcMessage->c_Signals.size();
      ...
      const C_CieConverter::C_CieCanSignal &rc_Sig =
          opc_DbcMessage->c_Signals[u32_ItSig];
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint16_t C_CamGenSigTableModel::m_GetMultiplexerValue(
    const C_OscCanMessage *const opc_OsyMessage,
      ...
      for (uint32_t u32_ItSig = 0UL;
        ...
        if (rc_Sig.e_MultiplexerType ==
          ...
          if (pc_OsySignalCommon != NULL) {
            ...
            C_SdNdeDpContentUtil::h_GetAnyValueAsSint64(c_ConvertedContent,
                                                        s64_Tmp, 0UL);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamGenSigTableModel::mh_GetNumRowsForMuxValue(
    const C_OscCanMessage *const opc_OsyMessage,
    const C_CieConverter::C_CieCanMessage *const opc_DbcMessage,
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_CamGenSigTableModel::m_TranslateRowToIndex(const int32_t os32_Row) const {
  // Default
  ...
  if ((pc_OsyMessage != NULL) || (pc_DbcMessage != NULL)) {
    ...
    if (q_IsMultiplexed) {
      ...
      if (pc_OsyMessage != NULL) {
        ...
        for (uint32_t u32_ItSig = 0UL;
             u32_ItSig < pc_OsyMessage->c_Signals.size(); ++u32_ItSig) {
        ...
        const uint16_t u16_SearchedStartBit =
            c_StartBits[static_cast<uint32_t>(os32_Row)];
        for (uint32_t u32_ItSig = 0UL;
             u32_ItSig < pc_DbcMessage->c_Signals.size(); ++u32_ItSig) {
          const C_CieConverter::C_CieCanSignal &rc_Sig =
              pc_DbcMessage->c_Signals[u32_ItSig];
          if (rc_Sig.u16_ComBitStart == u16_SearchedStartBit) {
            // Also check if it is actually part of the current message
            if ((rc_Sig.e_MultiplexerType !=
                 C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) ||
...
/*! \brief  Update multiplexed signal
   ...
   Either reset to zero or reset to initial value, depending on flag
   oq_SetToZero.
   ...
   \param[in]  oq_SetToZero      true: set to zero; false: reset to initial
   value
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenSigTableModel::m_UpdateMultiplexedSignal(
    const C_OscCanMessage *const opc_OsyMessage,
    const C_CieConverter::C_CieCanMessage *const opc_DbcMessage,
        ...
        if (oq_SetToZero == true) {
          if (rc_Sig.u16_MultiplexValue != ou16_MuxValue) {
            ...
            const C_OscCanSignal &rc_OsySig =
                C_CamGenSigUtil::h_ConvertDbcToOsy(*pc_DbcSignal);
            C_OscNodeDataPoolContentUtil::h_ZeroContent(c_InitialValue);
            Q_ASSERT(this->m_SetSignalFromOsyValue(rc_OsySig, c_InitialValue) ==
                     C_NO_ERR);
          }
        } else {
          if (rc_Sig.u16_MultiplexValue == ou16_MuxValue) {
            ...
            Q_ASSERT(this->m_SetSignalFromOsyValue(
                         rc_OsySig, rc_InitialValue) == C_NO_ERR);
          }
        }
      }
    }
