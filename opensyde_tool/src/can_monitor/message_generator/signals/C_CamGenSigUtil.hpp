...
{
public:
   static QByteArray h_ConvertRawDataFormat(const C_CamProMessageData & orc_Message);
   static stw::opensyde_core::C_OscNodeDataPoolContent h_DecodeRawToContentSignal(const QByteArray & orc_Raw,
                                                                                  const stw::opensyde_core::C_OscCanSignal & orc_Signal, const stw::opensyde_core::C_OscNodeDataPoolContent & orc_SignalMin);
   static void h_DecodeSignalValueToRaw(QByteArray & orc_Raw,
   ...
   static bool h_GetBit(const uint32_t ou32_BitPos, const QByteArray & orc_Bytes);
   static void h_SetBit(const uint32_t ou32_BitPos, QByteArray & orc_Bytes, const bool oq_Value);
   static uint32_t h_CalcMessageHash(const stw::opensyde_core::C_OscCanMessage & orc_Message,
                                     const QList<stw::opensyde_core::C_OscNodeDataPoolListElement> & orc_DatapoolPart);
   static uint32_t h_CalcMessageHash(const stw::opensyde_core::C_OscCanMessage & orc_Message,
