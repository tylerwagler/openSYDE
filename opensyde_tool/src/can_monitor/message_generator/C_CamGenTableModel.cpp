...
void C_CamGenTableModel::UpdateMessageKey(const uint32_t ou32_MessageIndex) {
  ...
  const int32_t s32_Col =
      C_CamGenTableModel::h_EnumToColumn(C_CamGenTableModel::eKEY);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableModel::UpdateAutoProtocolCellData(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableModel::TriggerModelUpdateCyclicMessage(
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamGenTableModel::headerData(const int32_t os32_Section,
  ...
  if (oe_Orientation == Qt::Orientation::Horizontal) {
    ...
    if (os32_Role == static_cast<int32_t>(Qt::DisplayRole)) {
      ...
      case eKEY:
        ...
        break;
      case eAUTO_SUPPORT:
        c_Header = "ECeS: When sending automatic calculation of the CRC and "
                   "incrementing of "
                   "the block counter is applied.\n"
                   "ECoS: When sending the inverted second frame will be sent "
                   "automatically "
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamGenTableModel::data(const QModelIndex &orc_Index,
                                  const int32_t os32_Role) const {
  QVariant c_Retval;
      ...
      if (((os32_Role == static_cast<int32_t>(Qt::DisplayRole)) ||
        ...
        case eID:
          if (os32_Role == ms32_USER_ROLE_SORT) {
            ...
            c_Retval = mh_HandleHexValue(
                static_cast<uint64_t>(pc_Message->u32_Id), os32_Role);
          }
          break;
        case eDLC:
        ...
        case eDATA:
          ...
          if (os32_Role == ms32_USER_ROLE_SORT) {
            ...
            for (uint16_t u16_ItByte = 0U; u16_ItByte < pc_Message->u16_Dlc;
                 ++u16_ItByte) {
              u64_Result +=
                  static_cast<uint64_t>(
                      pc_Message->c_Bytes[static_cast<uint32_t>(u16_ItByte)])
                  << (56U - (u16_ItByte * 8U));
            }
        ...
        case eKEY:
          if (pc_Message->c_Key.isEmpty()) {
            ...
            if (pc_Message->u32_KeyPressOffset > 0UL) {
              c_Offset = static_cast<QString>(" (+%1ms)")
                             .arg(pc_Message->u32_KeyPressOffset);
            }
            c_Retval = static_cast<QString>("%1%2")
                           .arg(pc_Message->c_Key)
                           .arg(c_Offset);
          }
          break;
        case eRTR:
        ...
        if (this->data(orc_Index, ms32_USER_ROLE_INTERACTION_IS_LINK)
                .toBool() == true) {
        ...
        if (e_Col == eDATA) {
          if ((pc_Message->q_SetAutoSupportMode != true) ||
              (C_CamGenTableModel::m_GetCurrentMessageProtocolType(u32_Index) !=
            ...
            for (uint16_t u16_It = 0U; u16_It < pc_Message->u16_Dlc; ++u16_It) {
              ...
              c_Tmp += static_cast<QString>("%1")
                           .arg(pc_Message->c_Bytes[u16_It], 2, 16, QChar('0'))
                           .toUpper();
            }
            c_Retval = c_Tmp;
          } else {
        ...
        default:
          ...
          break;
                  ...
                  ms32_USER_ROLE_INTERACTION_COMBO_BOX_STRINGS_LIST) ||
                 (os32_Role ==
                  ms32_USER_ROLE_INTERACTION_COMBO_BOX_VALUES_LIST)) {
        QStringList c_Tmp;
        if (e_Col == eDLC) {
          c_Tmp.append("0");
          c_Tmp.append("1");
        ...
        {
        case eID:
          c_Retval = C_Uti::h_GetValueAsHex(static_cast<uint64_t>(0ULL));
          break;
        case eDLC:
          c_Retval = 0;
          break;
        ...
        case eID:
          if (pc_Message->GetExtended()) {
            ...
            c_Retval = C_Uti::h_GetValueAsHex(static_cast<uint64_t>(0x7FFULL));
          }
          break;
        case eDLC:
          c_Retval = 8;
        ...
        case eCYCLIC_TRIGGER:
          c_Retval =
              mh_GetBoolAsCheckStateVariant(pc_Message->q_DoCyclicTrigger);
          break;
        case eXTD:
        ...
        case eAUTO_SUPPORT:
          c_Retval =
              mh_GetBoolAsCheckStateVariant(pc_Message->q_SetAutoSupportMode);
          break;
        default:
          Q_ASSERT(false);
          break;
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamGenTableModel::setData(const QModelIndex &orc_Index,
  ...
  if (data(orc_Index, os32_Role) != orc_Value) {
    if (orc_Index.isValid() == true) {
      ...
      if (os32_Role == static_cast<int32_t>(Qt::EditRole)) {
        ...
        switch (e_Col) {
        case eNAME:
          Q_ASSERT(C_CamProHandler::h_GetInstance()->SetMessageName(
                       u32_Index, orc_Value.toString()) == C_NO_ERR);
          q_Retval = true;
          break;
        case eID:
        ...
        if (q_Continue == true) {
          const uint32_t u32_Value =
              static_cast<uint32_t>(orc_Value.toULongLong());
          ...
          Q_ASSERT(C_CamProHandler::h_GetInstance()->SetMessageUint32Value(
                       u32_Index, e_Selector, u32_Value) == C_NO_ERR);
          q_Retval = true;
          ...
          if (e_Col == eDLC) {
            ...
            const QModelIndex c_Temp =
                orc_Index.sibling(orc_Index.row(), s32_Col);

            Q_EMIT(this->SigUpdateMessageDlc(u32_Index));
            // lint -e{1793} Qt example
            Q_EMIT(this->dataChanged(c_Temp, c_Temp,
        ...
        if (orc_Value.type() == QVariant::Int) {
          ...
          if (q_Continue == true) {
            ...
            // lint -e644 that's what the q_Continue is for
            Q_ASSERT(C_CamProHandler::h_GetInstance()->SetMessageBoolValue(
                         u32_Index, e_Selector, q_Val) == C_NO_ERR);
            // Special handling for XTD
            if ((e_Col == eXTD) && (q_Val == false)) {
              m_SpecialXtdFlagSetHandling(orc_Index.row(), u32_Index,
                                          os32_Role);
            }
            // Trigger sending of updated message
            if (q_UpdateCyclicMessage == true) {
              ...
              // Special for cyclic trigger: change the current message as
              // necessary (no two signals necessary)
              if ((e_Col == eCYCLIC_TRIGGER) &&
                  (C_CamProHandler::h_GetInstance()
                       ->GetCyclicMessageTransmitActive() == true)) {
  ...
  if (orc_Index.isValid() == true) {
    ...
    case eDLC:
      if (orc_Index.row() >= 0) {
        ...
        if ((pc_Message != NULL) &&
            (pc_Message->c_DataBaseFilePath.isEmpty() == true)) {
    ...
    case eDATA:
      ...
      break;
    case eAUTO_SUPPORT:
      if ((C_CamGenTableModel::m_GetCurrentMessageProtocolType(
               static_cast<uint32_t>(orc_Index.row())) ==
           C_OscCanProtocol::eECES) ||
          (C_CamGenTableModel::m_GetCurrentMessageProtocolType(
               static_cast<uint32_t>(orc_Index.row())) ==
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_CamGenTableModel::E_Columns
C_CamGenTableModel::h_ColumnToEnum(const int32_t os32_Column) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamGenTableModel::h_EnumToColumn(
    const C_CamGenTableModel::E_Columns oe_Value) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CamGenTableModel::UpdateDataForAutoProtocol(
...
void C_CamGenTableModel::m_DeleteItem(const uint32_t ou32_Index) {
  Q_ASSERT(C_CamProHandler::h_GetInstance()->DeleteMessage(ou32_Index) ==
           C_NO_ERR);
}
...
/*! \brief  Move specific item
   ...
   For implementation: First delete ou32_SourceIndex then insert
   ou32_TargetIndex Warning: not expected to fail

   \param[in]  ou32_SourceIndex  Source index
   \param[in]  ou32_TargetIndex  Target index
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableModel::m_MoveItem(const uint32_t ou32_SourceIndex,
                                    const uint32_t ou32_TargetIndex) {
  Q_ASSERT(C_CamProHandler::h_GetInstance()->MoveMessage(
               ou32_SourceIndex, ou32_TargetIndex) == C_NO_ERR);
}
...
QList<uint32_t> C_CamGenTableModel::m_AddNewMessages(
  ...
  if (orc_Data.size() > 0UL) {
    this->beginInsertRows(
        QModelIndex(), u32_Index,
        static_cast<int32_t>((u32_Index + orc_Data.size()) - 1UL));
    c_Retval.reserve(orc_Data.size());
    for (uint32_t u32_It = 0UL; u32_It < orc_Data.size(); ++u32_It) {
      C_CamProHandler::h_GetInstance()->InsertMessage(u32_Index + u32_It,
                                                      orc_Data[u32_It], true);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableModel::m_CheckAndHandleRegisterCyclicMessage(
    const uint32_t ou32_MessageIndex, const bool oq_Active) {
  const C_CamProMessageData *const pc_Message =
  ...
  if ((pc_Message != NULL) && (pc_Message->q_DoCyclicTrigger == true) &&
      ((C_CamProHandler::h_GetInstance()->GetCyclicMessageTransmitActive() ==
        true))) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamGenTableModel::m_SpecialXtdFlagSetHandling(const int32_t os32_Row,
                                                     const uint32_t ou32_Index,
                                                     const int32_t os32_Role) {
  const int32_t s32_Col = C_CamGenTableModel::h_EnumToColumn(eID);
  const uint64_t u64_MessageId =
      this->data(this->index(os32_Row, s32_Col), ms32_USER_ROLE_SORT)
          .toULongLong();
  ...
  if (u64_MessageId > 0x7FFULL) {
    ...
    Q_ASSERT(C_CamProHandler::h_GetInstance()->SetMessageUint32Value(
                 ou32_Index, e_SELECTOR_ID, u32_NEW_VAL) == C_NO_ERR);
    // Message
    Q_EMIT(this->SigReport(
        stw::opensyde_gui_elements::C_OgeWiCustomMessage::eWARNING,
        "Check Message ID",
...
/*! \brief  getting message protocol type
   ...
   \retval   eCAN_OPEN_SAFETY,  Data pool communication protocol CAN open safety
   (safety protocol)
   \retval   eECES,             Data pool communication protocol ECeS (safety
   protocol)
   \retval   eJ1939,            Data pool communication protocol J1939
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscCanProtocol::E_Type C_CamGenTableModel::m_GetCurrentMessageProtocolType(
  ...
  if (pc_Message != NULL) {
    if (C_CamDbHandler::h_GetInstance()->FindOsyMessage(
            pc_Message->c_DataBaseFilePath, pc_Message->c_Name) == C_NO_ERR) {
      if (C_CamDbHandler::h_GetInstance()->GetOscMessage(
        ...
        // It is totally fine to be failed at this point, as still the DB file
        // is in loading process
      }
    }
  }
