...
C_CamMainWindow::C_CamMainWindow(QWidget *const opc_Parent)
    : QMainWindow(opc_Parent), mpc_Ui(new Ui::C_CamMainWindow),
      mpc_CanDllDispatcher(nullptr), mq_LoggingStarted(false) {
  this->mpc_Ui->setupUi(this);

  // set stretch factor to have the splitter at real user settings position on
  // tool start
  ...
  // (real position is set on first move, so we avoid calculation and use 0 for
  // simplicity)
  ...
  stw::opensyde_gui_logic::C_HeHandler::h_GetInstance().SetHelpFileRelPath(
      "/../Help/openSYDE PC Tool.chm");

  // Connections
  // State handling
  connect(this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::SigStartLogging, this,
          &C_CamMainWindow::m_StartLogging);
  connect(this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::SigPauseLogging, this,
          &C_CamMainWindow::m_PauseLogging);
  connect(this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::SigContinueLogging,
          this, &C_CamMainWindow::m_ContinueLogging);
  connect(this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::SigStopLogging, this,
          &C_CamMainWindow::m_StopLogging);
  ...
  connect(this->mpc_Ui->pc_SplitterSettings, &C_CamOgeSpi::splitterMoved, this,
          &C_CamMainWindow::m_OnSettingsSplitterMoved);
  connect(this->mpc_Ui->pc_SplitterSettings, &C_CamOgeSpi::SigDoubleClick, this,
          &C_CamMainWindow::m_OnSettingsSplitterHandleDoubleClick);
  // Message generator splitter: individual collapse behavior
  connect(this->mpc_Ui->pc_GeneratorWidget,
          &C_CamGenWidget::SigExpandMessageGen, this,
          &C_CamMainWindow::m_OnExpandMessageGen);
  ...
  connect(this->mpc_Ui->pc_TitleBarWidget,
          &C_CamTitleBarWidget::windowTitleChanged, this,
          &C_CamMainWindow::setWindowTitle);
  // Filter configuration
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigFilterNumberChanged, this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SetActiveFilters);
  ...
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigRemoveFilterItems, this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::RemoveFilterItems);
  ...
  // Connect before database handling -> needs to be called first before next
  // load is started
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigAddDatabaseOsySysDef, this,
          &C_CamMainWindow::m_OnDatabaseLoadStarted);
  connect(this->mpc_Ui->pc_SettingsWidget, &C_CamMosWidget::SigAddDatabaseDbc,
          this, &C_CamMainWindow::m_OnDatabaseLoadStarted);
  connect(this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SigDatabaseLoadResultDbc, this,
          &C_CamMainWindow::m_OnDatabaseLoadFinished);
  connect(this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SigDatabaseLoadResultOsySysDef, this,
          &C_CamMainWindow::m_OnDatabaseLoadFinished);
  // Database handling
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigAddDatabaseOsySysDef,
  ...
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigNotifyMissingDataBase, this,
          &C_CamMainWindow::m_CheckMessagesForLoadedDatabase);
  ...
  connect(this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SigDatabaseLoadResultDbc,
          this->mpc_Ui->pc_SettingsWidget, &C_CamMosWidget::OnLoadFinishedDbc);
  connect(this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SigDatabaseLoadResultOsySysDef,
          this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::OnLoadFinishedOsySysDef);
  connect(this->mpc_Ui->pc_SettingsWidget, &C_CamMosWidget::SigRemoveDatabase,
          this, &C_CamMainWindow::m_OnDatabaseRemove);
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigSetDatabaseOsySysDefBus, this,
          &C_CamMainWindow::m_OnOsyChangeBus);
  connect(this->mpc_Ui->pc_SettingsWidget, &C_CamMosWidget::SigActivateDatabase,
          this, &C_CamMainWindow::m_OnActivateDatabase);
  connect(C_CamProHandler::h_GetInstance(),
          &C_CamProHandler::SigNewConfiguration, this,
          &C_CamMainWindow::m_OnNewConfiguration);
  connect(C_CamProHandler::h_GetInstance(),
          &C_CamProHandler::SigClearOldConfiguration, this,
          &C_CamMainWindow::m_OnClearOldConfiguration);
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigSetDatabaseOsySysDefBus,
          this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SetDatabaseOsySysDefBus);
  connect(this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SigDatabaseSetOsySysDefBusResult,
          this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::OnSigOsySysDefBusResult);
  ...
  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigRemoveAllLogFiles, this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::RemoveAllLogFiles);
  connect(this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::SigLogFileAddResult,
          this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::OnSigLogFileAddResult);
  // Saving user settings
  connect(this->mpc_Ui->pc_TitleBarWidget,
          &C_CamTitleBarWidget::SigPrepareForSave, this,
          &C_CamMainWindow::m_SaveUserSettings);

  mpc_CanThread = new stw::opensyde_gui_logic::C_SyvComDriverThread(
      &C_CamMainWindow::mh_ThreadFunc, this);
  ...
  this->mc_ComDriver.RegisterLogger(
  ...
  // Message generator
  connect(this->mpc_Ui->pc_GeneratorWidget, &C_CamGenWidget::SigSendMessage,
          this, &C_CamMainWindow::m_SendMessage);
  connect(this->mpc_Ui->pc_GeneratorWidget,
          &C_CamGenWidget::SigRegisterCyclicMessage, this,
          &C_CamMainWindow::m_RegisterCyclicMessage);
  connect(this->mpc_Ui->pc_GeneratorWidget,
          &C_CamGenWidget::SigRemoveAllCyclicMessages, this,
          &C_CamMainWindow::m_RemoveAllCyclicMessages);

  connect(this->mpc_Ui->pc_TraceWidget,
          &C_CamMetWidget::SigEmitAddFilterToParentWidget, this,
          &C_CamMainWindow::m_AddFilterData);
  connect(this, &C_CamMainWindow::SigEmitAddFilterToChildWidget,
          this->mpc_Ui->pc_SettingsWidget, &C_CamMosWidget::AddFilterData);

  connect(this->mpc_Ui->pc_SettingsWidget,
          &C_CamMosWidget::SigSendCanMsgDroppedToParentWidget, this,
          &C_CamMainWindow::CanFilterMsgDropped);
  connect(this, &C_CamMainWindow::SigSendCanMsgDroppedToChildrenWidget,
          this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::CanFilterMsgDropped);

  connect(this->mpc_Ui->pc_TraceWidget, &C_CamMetWidget::SigClearData, this,
          &C_CamMainWindow::m_ClearData);
  connect(this->mpc_Ui->pc_GeneratorWidget,
          &C_CamGenWidget::SigAutoProtocolSupport, this,
...
C_CamMainWindow::~C_CamMainWindow() {
  ...
  // Disconnect database remove (otherwise the signal will be handled by a
  // destroyed main window
  disconnect(this->mpc_Ui->pc_SettingsWidget,
             &C_CamMosWidget::SigRemoveDatabase, this,
             &C_CamMainWindow::m_OnDatabaseRemove);
  disconnect(this->mpc_Ui->pc_SettingsWidget,
             &C_CamMosWidget::SigNotifyMissingDataBase, this,
             &C_CamMainWindow::m_CheckMessagesForLoadedDatabase);
  ...
  if (mpc_CanDllDispatcher != nullptr) {
    this->m_CloseCan();

    delete mpc_CanDllDispatcher;
    mpc_CanDllDispatcher = nullptr;
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Function call of connect mechanism from child widget constructor.
   Used to pass CanMsgDropped Information to child widget
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Function call of connect mechanism from constructor. Received QList
 ...
 *   Can Id's  and IsCanMsgExtended data from child widget
   ...
   \param[in]       oc_CanMsgXtd     List of selected CanMessage has extended
 format
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_AddFilterData(const QList<int32_t> oc_CanMsgId,
                                      const QList<uint8_t> oc_CanMsgXtd) {
  Q_EMIT C_CamMainWindow::SigEmitAddFilterToChildWidget(oc_CanMsgId,
                                                        oc_CanMsgXtd);
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  On Update of AutoSupportProtocol to send information for stop/start
   Message counter
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_UpdateAutoSupportProtocol(
    const uint32_t ou32_MessageIndex, const bool oq_Active) {
  const C_CamProMessageData *const pc_Message =
      C_CamProHandler::h_GetInstance()->GetMessageConst(ou32_MessageIndex);

  if (pc_Message != nullptr) {
    C_OscCanProtocol::E_Type e_ProtocolType = C_OscCanProtocol::eCAN_OPEN;
    C_CamDbHandler::h_GetInstance()->GetOscMessage(
        pc_Message->c_DataBaseFilePath, pc_Message->c_Name,
        pc_Message->q_ContainsValidHash, pc_Message->u32_Hash, &e_ProtocolType);

    this->mc_ComDriver.UpdateAutoSupportProtocol(
        pc_Message->ToCanMessage().u32_ID, oq_Active, e_ProtocolType);
  }
}
...
void C_CamMainWindow::keyPressEvent(QKeyEvent *const opc_KeyEvent) {
  if (stw::opensyde_gui_logic::C_HeHandler::h_CheckHelpKey(opc_KeyEvent) ==
      true) {
    stw::opensyde_gui_logic::C_HeHandler::h_GetInstance().CallSpecificHelpPage(
        this->metaObject()->className());
  } else if (this->mpc_Ui->pc_GeneratorWidget->CheckAndHandleKey(
                 opc_KeyEvent->text()) == true) {
    // Accepted
  }
  // Save on Ctrl+S
  ...
  if (mh_CheckMime(pc_MimeData, &c_FilePath) == true) {
    ...
    if (C_OscUtils::h_CheckValidFilePath(c_FilePath) == false) {
      ...
      if (c_FileInfo.suffix().compare("syde_cam", Qt::CaseInsensitive) == 0) {
        if (this->mpc_Ui->pc_TitleBarWidget->HandleProjectComparison() ==
            true) {
          // Let all modules save their specific user settings before saving to
          // file
          this->m_SaveUserSettings();
...
void C_CamMainWindow::m_StartLogging(void) {
  ...
  if ((s32_Return == C_NO_ERR) || (s32_Return == C_WARN)) {
    ...
    // Use the current configured bitrate of the CAN DLL for the bus load
    // calculation
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_ClearData() { this->mc_ComDriver.ClearData(); }
...
int32_t C_CamMainWindow::m_InitCan(int32_t &ors32_Bitrate) {
  ...
  // Safety check
  if (this->mpc_CanDllDispatcher == nullptr) {
    return C_CONFIG;
  }

  // Get absolute DLL path (resolve variables and make absolute if it is
  // relative ant not empty)
...
void C_CamMainWindow::m_LoadInitialProject(void) {
  const QStringList c_Projects =
      C_UsHandler::h_GetInstance()->GetRecentProjects();
  ...
  if (QApplication::arguments().size() > 1) {
    ...
    if (this->mpc_Ui->pc_TitleBarWidget->LoadConfig(
            QApplication::arguments().at(1)) == C_NO_ERR) {
  ...
  if (q_ProjectLoadSuccess == false) {
    ...
    for (s32_Pos = 0; s32_Pos < c_Projects.size(); ++s32_Pos) {
      // directly use project handler because we do not want an error message
      // for every fail
      if (C_CamProHandler::h_GetInstance()->LoadFromFile(
              c_Projects.at(s32_Pos)) == C_NO_ERR) {
        q_ProjectLoadSuccess = true;
        break;
      }
    }
    ...
    // update recent projects menu to remove files that could not be loaded
    // (already got called in cmd case)
...
void C_CamMainWindow::m_LoadUserSettings(void) {
  ...
  C_OgeWiUtil::h_CheckAndFixDialogPositionAndSize(
      c_Position, c_Size, C_UsHandler::h_GetInstance()->GetAppScreenIndex(),
      QSize(1000, 700));
  this->setGeometry(c_Position.x(), c_Position.y(), c_Size.width(),
                    c_Size.height());

  if (C_UsHandler::h_GetInstance()->GetAppMaximized() == true) {
    // move to correct screen on multi screen setups
    const uint32_t u32_ScreenIndex =
        C_UsHandler::h_GetInstance()->GetAppScreenIndex();
    const QScreen *const pc_Screen =
        (static_cast<int32_t>(u32_ScreenIndex) <
         QGuiApplication::screens().size())
            ? QGuiApplication::screens().at(u32_ScreenIndex)
  ...
  this->m_OnExpandMessageGen(
      C_UsHandler::h_GetInstance()->GetMessageGenIsExpanded());
}
...
void C_CamMainWindow::m_SaveUserSettings(void) {
  ...
  if (C_UsHandler::h_GetInstance()->GetSettingsAreExpanded() == true) {
    ...
    if (c_Sizes.size() > 1) {
      C_UsHandler::h_GetInstance()->SetSplitterSettingsHorizontal(
          c_Sizes.at(1));
    }
  }
  ...
  if (C_UsHandler::h_GetInstance()->GetMessageGenIsExpanded() == true) {
    ...
    if (c_Sizes.size() > 1) {
      C_UsHandler::h_GetInstance()->SetSplitterMessageGenVertical(
          c_Sizes.at(1));
    }
  }
}
...
/*! \brief  Check if mime valid
   ...
   \param[in,out]  opc_FilePath  Optional parameter for file path output if
   valid
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamMainWindow::mh_CheckMime(const QMimeData *const opc_Mime,
                                   QString *const opc_FilePath) {
  bool q_Retval = false;
      ...
      if (c_PathList.size() == 1) {
        ...
        if (c_File.exists() == true) {
          if ((c_File.suffix().compare("syde_cam", Qt::CaseInsensitive) == 0) ||
              (c_File.suffix().compare("syde_sysdef", Qt::CaseInsensitive) ==
               0) ||
...
void C_CamMainWindow::mh_ThreadFunc(void *const opv_Instance) {
  ...
  C_CamMainWindow *const pc_Instance =
      reinterpret_cast<C_CamMainWindow *>(opv_Instance);
...
void C_CamMainWindow::m_OnExpandSettings(const bool oq_Expand) {
  if (oq_Expand == true) {
    // expand
    this->mpc_Ui->pc_SplitterSettings->SetSecondSegment(std::max(
        mhs32_SETTINGS_SPLITTER_MAX,
        C_UsHandler::h_GetInstance()->GetSplitterSettingsHorizontal()));
  } else {
    // remember segment size (only when they were expanded, i.e. do not
    // overwrite on tool start)
    if (C_UsHandler::h_GetInstance()->GetSettingsAreExpanded() == true) {
      const QList<int32_t> c_Sizes = this->mpc_Ui->pc_SplitterSettings->sizes();
    ...
    // collapse (because childrenCollapsible == false the following results in
    // minimum size)
...
void C_CamMainWindow::m_OnSettingsSplitterHandleDoubleClick(void) const {
  ...
  this->mpc_Ui->pc_SettingsWidget->ExpandSettings(
      !C_UsHandler::h_GetInstance()->GetSettingsAreExpanded());
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_OnSettingsSplitterMoved(const int32_t &ors32_Pos,
  ...
  if (ors32_Index == 1) {
    if (rc_Sizes.size() > 1) {
      ...
      if ((rc_Sizes.at(1) <= C_CamMainWindow::mhs32_SETTINGS_SPLITTER_MAX) &&
          (rc_Sizes.at(1) > 35)) {
    ...
    if (C_UsHandler::h_GetInstance()->GetSettingsAreExpanded() == false) {
      ...
      this->mpc_Ui->pc_SplitterSettings->SetSecondSegment(std::max(
          mhs32_SETTINGS_SPLITTER_MAX,
          static_cast<int32_t>(this->mpc_Ui->pc_SplitterSettings->width()) -
              ors32_Pos));
    }
  }
}
...
void C_CamMainWindow::m_OnExpandMessageGen(const bool oq_Expand) {
  if (oq_Expand == true) {
    // expand
    this->mpc_Ui->pc_SplitterMessageGen->SetSecondSegment(std::max(
        mhs32_MESSAGE_GEN_SPLITTER_MAX,
    ...
    if (C_UsHandler::h_GetInstance()->GetMessageGenIsExpanded() == true) {
      {
        const QList<int32_t> c_Sizes =
            this->mpc_Ui->pc_SplitterMessageGen->sizes();
...
void C_CamMainWindow::m_OnMessageGenSplitterHandleDoubleClick(void) const {
  // toggle splitter expanded state
  this->mpc_Ui->pc_GeneratorWidget->ExpandMessageGen(
      !C_UsHandler::h_GetInstance()->GetMessageGenIsExpanded());
}
...
/*! \brief  Slot function for splitter move.
   ...
   If message generator section gets collapsed, do not really collapse but
   minimize it.

   \param[in]  ors32_Pos      New position of splitter (distance to left border
   of widget)
   \param[in]  ors32_Index    Index of splitter widget
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_OnMessageGenSplitterMoved(const int32_t &ors32_Pos,
                                                  const int32_t &ors32_Index) {
  ...
  if (ors32_Index == 1) {
    if (rc_Sizes.size() > 1) {
      // is actual size between boundaries?
      if ((rc_Sizes.at(1) <= mhs32_MESSAGE_GEN_SPLITTER_MAX) &&
          (rc_Sizes.at(1) > 18)) {
        if (ors32_Pos > this->ms32_MessageGenSplitterPosition) {
          ...
          if (C_UsHandler::h_GetInstance()->GetMessageGenIsExpanded() ==
              false) {
    ...
    if (C_UsHandler::h_GetInstance()->GetMessageGenIsExpanded() == false) {
      ...
      this->mpc_Ui->pc_SplitterMessageGen->SetSecondSegment(std::max(
          mhs32_MESSAGE_GEN_SPLITTER_MAX,
          static_cast<int32_t>(this->mpc_Ui->pc_SplitterMessageGen->height()) -
              ors32_Pos));
    }
  }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_RegisterCyclicMessage(const uint32_t ou32_MessageIndex,
  ...
  if (pc_Message != nullptr) {
    ...
    // Send
    if (oq_Active) {
      this->mc_ComDriver.AddCyclicCanMessage(
          c_Message, pc_Message->q_SetAutoSupportMode, e_ProtocolType);
    } else {
      this->mc_ComDriver.RemoveCyclicCanMessage(
          c_Message, pc_Message->q_SetAutoSupportMode, e_ProtocolType);
    }
  }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_SendMessage(const uint32_t ou32_MessageIndex,
                                    const uint32_t ou32_TimeToSend) {
  const C_CamProMessageData *const pc_Message =
      C_CamProHandler::h_GetInstance()->GetMessageConst(ou32_MessageIndex);
  ...
  if (pc_Message != nullptr) {
    ...
    // Send
    this->mc_ComDriver.SendCanMessage(
        c_Message, pc_Message->q_SetAutoSupportMode, e_ProtocolType);
  }
}
...
void C_CamMainWindow::m_OnNewConfiguration(void) {
  ...
  // Must be after constructor calls because of connect dependencies on the
  // other modules For example: The push buttons with states must send their
  // toggle signal on setting the loaded user settings
...
void C_CamMainWindow::m_OnDatabaseLoadFinished(const int32_t os32_Result) {
  if ((this->mc_CurrentLoadedFile.isEmpty() == false) &&
      (this->mc_CurrentLoadedFileOrg.isEmpty() == false)) {
    ...
    if ((os32_Result == C_NO_ERR) || (os32_Result == C_WARN)) {
      ...
      if (pc_MessageMonitor != nullptr) {
        ...
        if (c_FileInfo.suffix().compare("dbc", Qt::CaseInsensitive) == 0) {
          ...
          if (C_CamDbHandler::h_GetInstance()->GetContainsDbc(
                  this->mc_CurrentLoadedFileOrg) == false) {
            C_CieConverter::C_CieCommDefinition c_DbcDefinition;
            if (pc_MessageMonitor->GetDbcFile(this->mc_CurrentLoadedFile,
                                              c_DbcDefinition) == C_NO_ERR) {
              // Add to currently loaded files
              C_CamDbHandler::h_GetInstance()->AddDbcFile(
                  this->mc_CurrentLoadedFileOrg, c_DbcDefinition);
            } else {
              Q_ASSERT(false);
            }
          }
        } else if (c_FileInfo.suffix().compare("syde_sysdef",
                                               Qt::CaseInsensitive) == 0) {
          // Check if already known
          if (C_CamDbHandler::h_GetInstance()->GetContainsOsy(
                  this->mc_CurrentLoadedFileOrg) == false) {
            const stw::opensyde_core::C_OscSystemDefinition c_Tmp;
            stw::opensyde_core::C_OscComMessageLoggerOsySysDefConfig
                c_SystemDefinition(c_Tmp, 0UL);
            if (pc_MessageMonitor->GetOsySysDef(this->mc_CurrentLoadedFile,
                                                c_SystemDefinition) ==
                C_NO_ERR) {
              // Add to currently loaded files
              C_CamDbHandler::h_GetInstance()->AddOsyFile(
                  this->mc_CurrentLoadedFileOrg, c_SystemDefinition);
            } else {
              Q_ASSERT(false);
            }
          }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_OnDatabaseRemove(const QString &orc_File,
                                         const QString &orc_OrgPath,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_OnActivateDatabase(const QString &orc_File,
                                           const QString &orc_OrgPath,
                                           const bool oq_Active) {
  Q_UNUSED(orc_File)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMainWindow::m_OnOsyChangeBus(const QString &orc_File,
                                       const QString &orc_OrgPath,
  ...
  C_CamDbHandler::h_GetInstance()->ReplaceOsyBusIndex(
      orc_OrgPath, ou32_BusIndex, q_AnyChange);
...
void C_CamMainWindow::m_DisplayCheckMessagesDialog(
  ...
  if (orc_Indices.size() > 0UL) {
    ...
    c_Message.SetDescription(
        "Inconsistent messages in message generator found. "
    ...
    for (uint32_t u32_It = 0UL; u32_It < orc_Indices.size(); ++u32_It) {
      ...
      const C_CamProMessageData *const pc_Message =
          C_CamProHandler::h_GetInstance()->GetMessageConst(
              orc_Indices[u32_It]);
      if (pc_Message != nullptr) {
        const QString c_MessageName =
            C_CamProHandler::h_GetCompleteMessageName(*pc_Message);
        c_Entry = static_cast<QString>("%1 (%2)\n")
                      .arg(c_MessageName)
                      .arg(C_Uti::h_GetValueAsHex(pc_Message->u32_Id));
      }
    ...
    c_Message.Execute();
    // Handle messages
    this->mpc_Ui->pc_GeneratorWidget->RemoveMessagesForFile(orc_DatabasePath,
                                                            &orc_Indices);
    // Notify signal widgets
    this->mpc_Ui->pc_GeneratorWidget->TriggerSignalReload();
  }
