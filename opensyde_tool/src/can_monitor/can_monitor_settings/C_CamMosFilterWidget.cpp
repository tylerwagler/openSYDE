...
C_CamMosFilterWidget::C_CamMosFilterWidget(QWidget *const opc_Parent)
    : C_OgeWiOnlyBackground(opc_Parent), mpc_Ui(new Ui::C_CamMosFilterWidget),
  ...
  connect(C_CamProHandler::h_GetInstance(),
          ...
          &C_CamMosFilterWidget::m_LoadConfig);
...
void C_CamMosFilterWidget::LoadUserSettings(void) const {
  this->mpc_Ui->pc_WiHeader->SetExpanded(
      C_UsHandler::h_GetInstance()->GetWiFilterExpanded());
  this->m_OnExpand(C_UsHandler::h_GetInstance()->GetWiFilterExpanded());
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Prepare widget for menu view in collapsed state resp. expanded
   view.

   \param[in]     oq_Expand       true: prepare for expanded, false: prepare for
   collapsed menu
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::PrepareForExpanded(const bool oq_Expand) const {
  if (oq_Expand == false) {
    ...
    this->mpc_Ui->pc_WiContent->setVisible(
        C_UsHandler::h_GetInstance()->GetWiFilterExpanded());
  }
...
void C_CamMosFilterWidget::Clear(void) {
  ...
  for (uint32_t u32_ItEntry = 0; u32_ItEntry < this->mc_Entries.size();
       ++u32_ItEntry) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Set function for calling m_OnAddFilterFromContextmenu or
   m_SetAddFilterToExistingFilter
...
void C_CamMosFilterWidget::SetAddFilter(const QList<int32_t> oc_CanMsgId,
  ...
  const int32_t s32_MessageFilterItemDropMaxPosLimit =
      52 + (static_cast<int32_t>(c_Filters.size()) * 26);

  if (((ms32_LastMessageFilterItemDropPosition >= 62) &&
       (ms32_LastMessageFilterItemDropPosition <=
        s32_MessageFilterItemDropMaxPosLimit)) &&
      (s32_MessageFilterItemDroppedOnIndex <=
       static_cast<int32_t>(c_Filters.size())) &&
      (c_Filters.size() >= 1)) {
    ...
    if (e_ReturnMessageBox == C_OgeWiCustomMessage::eOK) {
      C_CamMosFilterWidget::m_SetAddFilterToExistingFilter(
          oc_CanMsgId, oc_CanMsgXtd,
          static_cast<uint32_t>(s32_MessageFilterItemDroppedOnIndex));
    } else if (e_ReturnMessageBox == C_OgeWiCustomMessage::eNO) {
      C_CamMosFilterWidget::m_OnAddFilterFromContextmenu(oc_CanMsgId,
                                                         oc_CanMsgXtd);
    } else {
...
/*! \brief  Emit signal on filter message dropped on existing item
   ...
   \param[in]       oc_CanMsgId                  List of selected message
   CanId's
   \param[in]       oc_CanMsgXtd                 List of selected CanMessage has
   extended format
   \param[in]       ou32_ExistingFilterIndex     Index on which the filter
   message got dropped
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::m_SetAddFilterToExistingFilter(
  ...
  Q_EMIT C_CamMosFilterWidget::SigAddFilterToExistingFilter(oc_CanMsgId,
                                                            oc_CanMsgXtd);
...
/*! \brief  Overridden drag enter event slot
   ...
   Here accepts (MimeData Format) qabstractitemmodeldatalist item from
   C_CamMetTreeView class
...
void C_CamMosFilterWidget::m_InitUi(void) {
  ...
  C_OgeWiUtil::h_ApplyStylesheetProperty(this->mpc_Ui->pc_FrameBottom,
                                         "HasColor8Background", true);
  C_OgeWiUtil::h_ApplyStylesheetProperty(this->mpc_Ui->pc_FrameTop,
                                         "HasColor8Background", true);
  ...
  this->mpc_Ui->pc_BtnAdd->SetToolTipInformation(
      "Add New Filter", "Add new receive filter and configure filter items.");
  ...
  this->mpc_Ui->pc_LabNoFilter->setText(
      "No receive filters are declared.\nadd any via \"Add Filter\".");

  // connects
  connect(this->mpc_Ui->pc_WiHeader,
          &C_CamOgeWiSettingSubSection::SigExpandSection, this,
          &C_CamMosFilterWidget::m_OnExpand);
  ...
  connect(this->mpc_Ui->pc_BtnAdd, &C_CamOgePubSettingsAdd::clicked, this,
          &C_CamMosFilterWidget::m_OnAddClicked);
}
...
void C_CamMosFilterWidget::m_LoadConfig(void) {
  ...
  // initialize enabled state (setChecked also emits toggled signal, so
  // m_EnableFilters gets called automatically)
  this->mpc_Ui->pc_WiHeader->SetToggleState(
      C_CamProHandler::h_GetInstance()->GetFilterWidgetEnabled());
...
void C_CamMosFilterWidget::m_EnableFilters(const bool &orq_Enabled) {
  ...
  if (orq_Enabled == true) {
    ...
    for (uint32_t u32_PosFilter = 0; u32_PosFilter < c_Filters.size();
         u32_PosFilter++) {
      if (c_Filters[u32_PosFilter].q_Enabled == true) {
        // check all filter items
        QList<C_CamProFilterItemData> c_Temp =
            c_Filters[u32_PosFilter].c_FilterItems;
...
void C_CamMosFilterWidget::m_OnAddClicked() {
  ...
  for (uint32_t u32_ItFilter = 0UL; u32_ItFilter < c_Filters.size();
       ++u32_ItFilter) {
    c_Names[c_Filters[u32_ItFilter].c_Name] = true;
  }
  c_FilterData.c_Name = C_Uti::h_GetUniqueNameQt(c_Names, c_ProposedName);

  // Pop up dialog (use scroll area widget as parent to make sure scroll bars
  // are styled correctly)
  const QPointer<C_OgePopUpDialog> c_New = new C_OgePopUpDialog(
      this->mpc_Ui->pc_ScrollAreaContents, this->mpc_Ui->pc_ScrollAreaContents);
  C_CamMosFilterPopup *const pc_Dialog =
      new C_CamMosFilterPopup(c_FilterData, *c_New);
  ...
  if (c_New->exec() == static_cast<int32_t>(QDialog::Accepted)) {
    ...
    // c_FilterData.q_Enabled is not set from dialog but we want the default
    // anyway
    ...
    if (C_CamProHandler::h_GetInstance()->GetFilterWidgetEnabled() == false) {
      ...
      c_Message.SetDescription(
          "Filters are not applied as long as receive filter setting is "
      ...
      if (c_Message.Execute() == C_OgeWiCustomMessage::eOK) {
        ...
        // this also emits toggled signal, so filter items get enabled
        // automatically
      }
    } else {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Calling an existing function
 (C_CamMosFilterWidget::m_OnAddClicked()) with modificatios
 *  Sending QList of selected messages Can Id's  and IsCanMsgExtended to child
 widget "C_CamMosFilterPopup"
 *
   \param[in]       oc_CanMsgId     List of selected message CanId's
   \param[in]       oc_CanMsgXtd     List of selected CanMessage has extended
 format
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::m_AddFilterWidget(
    const C_CamProFilterData &orc_FilterData) {
  C_CamMosFilterItemWidget *const pc_NewItem =
      new C_CamMosFilterItemWidget(orc_FilterData, this);

  // remember item
  this->mc_Entries.push_back(pc_NewItem);

  // insert new item before spacer -> "-1"
  this->mpc_Ui->pc_LayoutContents->insertWidget(
      this->mpc_Ui->pc_LayoutContents->count() - 1, pc_NewItem);
  ...
  // connect
  connect(pc_NewItem, &C_CamMosFilterItemWidget::SigRemoveFilter, this,
          &C_CamMosFilterWidget::m_RemoveFilter);
  connect(pc_NewItem, &C_CamMosFilterItemWidget::SigEnableFilter, this,
          &C_CamMosFilterWidget::m_ActivateFilter);
  connect(pc_NewItem, &C_CamMosFilterItemWidget::SigUpdateFilter, this,
          &C_CamMosFilterWidget::m_UpdateFilterConfiguration);
} // lint !e429  no memory leak because of the parent of pc_NewItem and the Qt
  // memory management
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::m_RemoveFilter(
    C_CamMosFilterItemWidget *const opc_ItemWidget) {
  // ask user
  ...
  if (c_Message.Execute() == C_OgeWiCustomMessage::eOK) {
    if (opc_ItemWidget != NULL) {
      ...
      // update data handling
      Q_ASSERT(C_CamProHandler::h_GetInstance()->DeleteFilter(
                   s32_IndexToRemove) == stw::errors::C_NO_ERR);
      ...
      if (C_CamProHandler::h_GetInstance()->GetFilterWidgetEnabled() == true) {
        ...
        QList<C_CamProFilterItemData> c_FilterItems =
            C_CamProHandler::h_GetInstance()
                ->GetFilters()[s32_IndexToRemove]
                .c_FilterItems;
        ...
        Q_EMIT(
            this->SigFilterNumberChanged(this->m_CountActiveFilterPackages()));
      }
    }

    // number of filter widgets should now equal number of filters in data
    // handling
    Q_ASSERT(C_CamProHandler::h_GetInstance()->GetFilters().size() ==
             mc_Entries.size());
...
/*! \brief   Activate or deactivate given filter.
   ...
   \param[in]     orq_Enable        True: activate filter, false: deactivate
   filter
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::m_ActivateFilter(
  ...
  if (s32_IndexToActivate >= 0) {
    // update data handling
    C_CamProHandler::h_GetInstance()->SetFilterEnabled(s32_IndexToActivate,
                                                       orq_Enable);
    ...
    if (C_CamProHandler::h_GetInstance()->GetFilterWidgetEnabled() == true) {
      ...
      QList<C_CamProFilterItemData> c_FilterItems =
          C_CamProHandler::h_GetInstance()
              ->GetFilters()[s32_IndexToActivate]
              .c_FilterItems;
      this->m_GetActiveFilterItems(c_FilterItems);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::m_UpdateFilterConfiguration(
    const C_CamMosFilterItemWidget *const opc_ItemWidget,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamMosFilterWidget::m_GetIndexFromWidget(
    const C_CamMosFilterItemWidget *const opc_ItemWidget) {
  int32_t s32_Return = 0;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosFilterWidget::m_GetActiveFilterItems(
    QList<C_CamProFilterItemData> &orc_ActiveFilterItems) const {
  QList<C_CamProFilterItemData> c_TempItems;
  for (int32_t s32_PosItem = 0; s32_PosItem < orc_ActiveFilterItems.size();
       s32_PosItem++) {
