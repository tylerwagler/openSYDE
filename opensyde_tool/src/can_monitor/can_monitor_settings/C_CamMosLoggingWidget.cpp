...
C_CamMosLoggingWidget::C_CamMosLoggingWidget(QWidget *const opc_Parent)
    : C_OgeWiOnlyBackground(opc_Parent), mpc_Ui(new Ui::C_CamMosLoggingWidget),
      mq_Online(false) {
  ...
  this->mpc_Ui->pc_LabFile->SetToolTipInformation(
      "File", "Name of log file without extension.");
  this->mpc_Ui->pc_LabFolder->setText("Directory");
  this->mpc_Ui->pc_LabFolder->SetToolTipInformation(
  ...
  this->mpc_Ui->pc_LabOverwrite->SetToolTipInformation(
      ...
      "  - Make log file name unique by adding a timestamp suffix. Format: "
      "<LogFileName>_<Date>_<Time>\n"
  ...
  connect(this->mpc_Ui->pc_WiHeader,
          ...
          &C_CamMosLoggingWidget::m_OnExpand);
  ...
  connect(this->mpc_Ui->pc_WiHeader, &C_CamOgeWiSettingSubSection::SigToggled,
          this, &C_CamMosLoggingWidget::m_OnToggled);
  ...
  connect(this->mpc_Ui->pc_LeFile, &C_OgeLeUnified::editingFinished, this,
  ...
  connect(this->mpc_Ui->pc_CbxFormat,
          static_cast<void (QComboBox::*)(int32_t)>(
              &QComboBox::currentIndexChanged),
  ...
  connect(this->mpc_Ui->pc_PubVariables,
          ...
          &C_CamMosLoggingWidget::m_InsertPathVar);
}
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_CamMosLoggingWidget::~C_CamMosLoggingWidget(void) { delete this->mpc_Ui; }
...
void C_CamMosLoggingWidget::LoadUserSettings(void) const {
  this->mpc_Ui->pc_WiHeader->SetExpanded(
      C_UsHandler::h_GetInstance()->GetWiLoggingExpanded());
...
/*! \brief   Slot for communication change signal.
   ...
   \param[in]  oq_Online    true: communication started; false: communication
   stopped.
*/
//----------------------------------------------------------------------------------------------------------------------
...
void C_CamMosLoggingWidget::OnSigLogFileAddResult(const int32_t os32_Result) {
  if (os32_Result != stw::errors::C_NO_ERR) {
    ...
    c_Message.SetDescription(
        "Could not start logging. Possible reason: Missing write access.");
    c_Message.Execute();
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosLoggingWidget::OnSigSavedAsNew(void) { this->m_OnFolderEdited(); }
...
void C_CamMosLoggingWidget::m_LoadConfig(void) const {
  const C_CamProLoggingData &rc_LoggingData =
      C_CamProHandler::h_GetInstance()->GetLoggingData();

  this->m_LoadFolderConfig();

  // remaining values
  this->mpc_Ui->pc_LeFile->setText(rc_LoggingData.c_FileName);
  this->mpc_Ui->pc_CbxOverwrite->setCurrentIndex(
      static_cast<int32_t>(rc_LoggingData.e_OverwriteMode));
  this->mpc_Ui->pc_CbxFormat->setCurrentIndex(
      static_cast<int32_t>(rc_LoggingData.e_FileFormat));
...
void C_CamMosLoggingWidget::m_LoadFolderConfig(void) const {
  ...
  if (rc_LoggingData.c_Directory == "") {
    ...
    this->mpc_Ui->pc_LeFolder->SetPath(
        c_DefaultDir, C_CamProHandler::h_GetInstance()->GetCurrentProjDir());
  } else {
    this->mpc_Ui->pc_LeFolder->SetPath(
        rc_LoggingData.c_Directory,
...
void C_CamMosLoggingWidget::m_OnFolderEdited(void) {
  // update data handling
  C_CamProHandler::h_GetInstance()->SetLoggingDirectory(
      this->mpc_Ui->pc_LeFolder->GetPath());
}
...
void C_CamMosLoggingWidget::m_OnFileNameEdited(void) {
  // update data handling
  C_CamProHandler::h_GetInstance()->SetLoggingFileName(
      this->mpc_Ui->pc_LeFile->text());
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosLoggingWidget::m_OnOverwriteModeSelected(
    const int32_t os32_Index) const {
  // update data handling
...
void C_CamMosLoggingWidget::m_OnFormatSelected(const int32_t os32_Index) const {
  // update data handling
  C_CamProHandler::h_GetInstance()->SetLoggingFormat(
      static_cast<C_CamProLoggingData::E_Format>(os32_Index));
}
...
void C_CamMosLoggingWidget::m_OnBrowse(void) {
  QString c_Path;
  QString c_Folder = C_CamUti::h_GetResolvedAbsolutePathFromProj(
      C_CamProHandler::h_GetInstance()->GetLoggingData().c_Directory);
  ...
  c_Path = QFileDialog::getExistingDirectory(
      this, "Select Log File Location", c_Folder, QFileDialog::ShowDirsOnly);
...
void C_CamMosLoggingWidget::m_SetLoggingPath(const QString &orc_New) {
  const QString c_Path = C_CamUtiGeneric::h_AskUserToSaveRelativePath(
      this, orc_New, C_CamProHandler::h_GetInstance()->GetCurrentProjDir());

  // if path contains invalid characters this returned empty
  if (c_Path.isEmpty() == false) {
    this->mpc_Ui->pc_LeFolder->SetPath(
        c_Path, C_CamProHandler::h_GetInstance()->GetCurrentProjDir());
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Use online flag and enabled flag to decide if to start or stop
 * logging.
 */
//----------------------------------------------------------------------------------------------------------------------
void C_CamMosLoggingWidget::m_DecideStartStop() {
  if ((this->mq_Online == true) &&
      (C_CamProHandler::h_GetInstance()->GetLoggingData().q_Enabled == true)) {
    ...
    // stop logging
    // GUI only supports one log file at once so this is also used for removing
    // a log file.
...
void C_CamMosLoggingWidget::m_CheckAndStartLogging() {
  ...
  // check directory
  q_ValidDir =
      stw::opensyde_core::C_OscUtils::h_CheckValidFilePath(c_ResolvedPath);

  // check file name
  q_ValidName = stw::opensyde_core::C_OscUtils::h_CheckValidFileName(
      rc_LoggingData.c_FileName);

  if ((q_ValidDir == false) || (q_ValidName == false)) {
    // inform user
    C_OgeWiCustomMessage c_Message(this, C_OgeWiCustomMessage::eERROR);
    const QString c_Heading = "Logging Directory";
    const QString c_MessageText = "Could not start logging because of invalid "
  ...
  if (q_Continue == true) {
    ...
    c_FilePath = C_CamUti::h_GetResolvedAbsolutePathFromProj(
                     rc_LoggingData.c_Directory) +
                 "/" + rc_LoggingData.c_FileName;
    ...
    // add suffix
    c_FilePath += "." + C_CamProLoggingData::h_FormatEnumToString(
                            rc_LoggingData.e_FileFormat);

    // check existence if flag is warn-before-overwrite (in the "fast-click"
    // case that a file with timestamp suffix already exists we also just
    // overwrite because the first file would not hold much information)
    c_FileInfo.setFile(c_FilePath);
    if (rc_LoggingData.e_OverwriteMode ==
        C_CamProLoggingData::eWARN_BEFORE_OVERWRITE) {
      if (c_FileInfo.exists() == true) {
        ...
        c_Message.SetDescription(
            static_cast<QString>("The log file %1 already exists. "
