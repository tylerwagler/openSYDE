 ...
 * ---------------------------------------------------------------------------------------------------------
 */
class C_CamMetTreeModel : public C_TblTreSimpleModel {
public:
  enum E_Columns {
    eTIME_STAMP,
    eCAN_ID,
  ...
  QModelIndex
  index(const int32_t os32_Row, const int32_t os32_Column,
        const QModelIndex &orc_Parent = QModelIndex()) const override;
  QModelIndex parent(const QModelIndex &orc_Index)
      const override; // lint !e1511  we want to hide the base
  int32_t
  columnCount(const QModelIndex &orc_Parent = QModelIndex()) const override;
  int32_t
  rowCount(const QModelIndex &orc_Parent = QModelIndex()) const override;
  QVariant data(const QModelIndex &orc_Index,
                const int32_t os32_Role =
                    static_cast<int32_t>(Qt::DisplayRole)) const override;
  ...
  int32_t SearchMessageData(const QString &orc_SearchString, const bool oq_Next,
                            int32_t &ors32_SignalRow,
                            int32_t &ors32_MultiplexedSignalRow);
  uint32_t
  TranslateTreeRowsToSignalIndex(const QModelIndex &orc_CurrentIndex) const;
  uint32_t
  TranslateTreeRowsToSignalIndex(const int32_t os32_MessageIndex,
...
private:
  ...
  uint32_t mu32_TraceBufferSizeConfig; // The set buffer size by user. Will be
                                       // set after restart the
  // communication
  ...
  void m_HandleNewUniqueMessage(const C_CamMetTreeLoggerData &orc_Message,
                                const int32_t os32_MultiplexerValue);
  int32_t
  m_GetPosIndexForUniqueMessage(const QString &orc_AbsoluteTimeStamp) const;
  ...
  void m_UpdateTreeItemBasedOnMessage(C_TblTreSimpleItem *const opc_Item,
                                      const C_CamMetTreeLoggerData &orc_Message,
                                      const bool oq_SignalInsert,
                                      const int32_t os32_MessageRow);
  bool m_CheckDataForSearch(const C_CamMetTreeLoggerData &orc_Data,
                            const QString &orc_SearchString, const bool oq_Next,
                            int32_t &ors32_SignalIndex,
                            bool &orq_LastSignalChecked) const;
  bool m_CheckMessageDataForSearch(const C_CamMetTreeLoggerData &orc_Data,
                                   const QString &orc_SearchString) const;
  bool m_CheckSignalDataForSearch(
  ...
  static C_CamMetTreeLoggerDataGreyOutInformation
  mh_ExtractPreviousGreyOutInformation(
  ...
  static void mh_ApplyPreviousGreyOutInformation(
