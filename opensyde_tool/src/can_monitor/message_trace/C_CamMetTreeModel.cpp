 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_CamMetTreeModel::mhc_ICON_MESSAGE =
    ":images/IconMessageInactive.svg";
const QString C_CamMetTreeModel::mhc_ICON_MESSAGE_SELECTED =
    "://images/IconMessageSelected.svg";
const QString C_CamMetTreeModel::mhc_ICON_SIGNAL =
    ":images/IconSignalInactive.svg";
const QString C_CamMetTreeModel::mhc_ICON_SIGNAL_SELECTED =
    "://images/IconSignalSelected.svg";
...
C_CamMetTreeModel::C_CamMetTreeModel(QObject *const opc_Parent)
    : C_TblTreSimpleModel(opc_Parent), mq_DisplayTree(false),
      mq_UniqueMessageMode(false), mq_DisplayAsHex(false),
      mq_DisplayTimestampRelative(false),
      mq_DisplayTimestampAbsoluteTimeOfDay(false),
      mu32_TraceBufferSizeUsed(1000U), mu32_TraceBufferSizeConfig(1000U),
      mq_DataUnlocked(false), mu32_OldestItemIndex(0),
      ms32_SelectedParentRow(-1), mq_GrayOutPause(false),
      mu32_GrayOutPauseTimeStamp(0U),
      mpc_RootItemContinuous(new C_TblTreSimpleItem()),
      mpc_RootItemStatic(new C_TblTreSimpleItem()),
      ms32_LastSearchedMessageRow(-1), ms32_LastSearchedSignalRow(-1),
  ...
  this->mc_FontTransparcencyColors.resize(
      (ms32_TRACE_TRANSPARENCY_START - ms32_TRACE_TRANSPARENCY_END) + 1,
      mc_STYLE_GUIDE_COLOR_1);

  // Value on position 0 equals no transparency and at the end equals maximum
  // transparency
  for (s32_Counter = 0U;
       s32_Counter <
       static_cast<int32_t>(this->mc_FontTransparcencyColors.size());
       ++s32_Counter) {
    this->mc_FontTransparcencyColors[s32_Counter].setAlpha(
        ms32_TRACE_TRANSPARENCY_START - s32_Counter);
  }

  // Default
  this->mpc_InvisibleRootItem = this->mpc_RootItemContinuous;

  connect(&this->mc_GrayOutTimer, &QTimer::timeout, this,
          &C_CamMetTreeModel::m_GrayOutTimer);
...
/*! \brief  Continues the gray out engine
   ...
   Calculation of an offset to all saved unique messages timestamps to
   compensate the pause time
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::Continue(void) {
  // ms resolution is enough
  // Using US as base because we want to compare with the US value of the CAN
  // message. QDateTime can have a difference of 1 to 2 seconds because of a
  // different rounding
  ...
  const uint32_t u32_GrayOutPauseOffset =
      u32_CurrentTime - this->mu32_GrayOutPauseTimeStamp;

  QMap<QString, C_CamMetTreeLoggerData>::iterator c_It;

  // Adapt all timestamps used by the gray out engine
  for (c_It = this->mc_UniqueMessages.begin();
    ...
    // The original timestamp of the message can be adapted, it used only for
    // the gray out engine For showing the timestamp, the prepared and already
    // calculated timestamps will be used
    rc_Data.c_CanMsg.u64_TimeStamp +=
        static_cast<uint64_t>(u32_GrayOutPauseOffset) * 1000ULL;

    // Timestamp of all data bytes
    rc_Data.c_GreyOutInformation.u32_DataChangedTimeStamp +=
        u32_GrayOutPauseOffset;

    // Timestamp of each data byte
    for (s32_CounterDataByte = 0; s32_CounterDataByte < rc_Data.c_CanMsg.u8_DLC;
         ++s32_CounterDataByte) {
      rc_Data.c_GreyOutInformation
          .c_DataBytesChangedTimeStamps[s32_CounterDataByte] +=
          u32_GrayOutPauseOffset;
    }
...
void C_CamMetTreeModel::Pause(void) {
  ...
  // ms resolution is enough
  // Using US as base because we want to compare with the US value of the CAN
  // message. The TglGetTickCountUs can have a difference of 1 to 2 seconds
  // because of a different rounding
  this->mu32_GrayOutPauseTimeStamp =
      static_cast<uint32_t>(QDateTime::currentMSecsSinceEpoch());
}
...
/*! \brief  Pauses the gray out engine
   ...
   Calculation of the offset is not necessary. Stop can only be followed by
   start and start resets the offset in any case.
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::Stop(void) { this->mq_GrayOutPause = true; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::Start(void) { this->mq_GrayOutPause = false; }

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Function to lock up all data requests (performance)
 */
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::LockData(void) { this->mq_DataUnlocked = false; }

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Function to unlock up all data requests (performance)
 */
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::UnlockData(void) { this->mq_DataUnlocked = true; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::SetSelection(const int32_t os32_SelectedParentRow,
  ...
  // Update data row
  this->dataChanged(this->index(0, s32_ColIdData),
                    this->index(this->rowCount() - 1, s32_ColIdData));
}
...
void C_CamMetTreeModel::SetDisplayAsHex(const bool oq_Value) {
  ...
  // Update UI
  Q_EMIT this->dataChanged(this->index(0, s32_ColCanId),
                           this->index(this->rowCount() - 1, s32_ColCanId));
  Q_EMIT this->dataChanged(this->index(0, s32_ColDataId),
                           this->index(this->rowCount() - 1, s32_ColDataId));
}
...
void C_CamMetTreeModel::SetDisplayTimestampRelative(const bool oq_Value) {
  ...
  // Update UI
  Q_EMIT(this->dataChanged(this->index(0, s32_ColId),
                           this->index(this->rowCount() - 1, s32_ColId)));
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::SetDisplayTimestampAbsoluteTimeOfDay(
    const bool oq_Value) {
  // Update value
...
C_CamMetTreeModel::GetAllMessagesForProtocolChange(void) {
  ...
  // Append vector
  for (uint32_t u32_ItVec = 0; u32_ItVec < this->mc_DataBase.size();
       ++u32_ItVec) {
    c_Retval.push_back(&this->mc_DataBase[u32_ItVec]);
  }
  // Append map
  for (QMap<QString, C_CamMetTreeLoggerData>::iterator c_It =
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamMetTreeModel::headerData(const int32_t os32_Section,
                                       const Qt::Orientation oe_Orientation,
                                       const int32_t os32_Role) const {
  QVariant c_Retval =
      QAbstractItemModel::headerData(os32_Section, oe_Orientation, os32_Role);
...

    {
      ...
      case eTIME_STAMP:
        if (this->mq_DisplayTimestampAbsoluteTimeOfDay == false) {
          c_Retval = "Absolute time (hh:mm:ss.ms.us) from the measurement "
                     "start, or relative "
                     "to the previous event.";
        } else {
          c_Retval =
              "Absolute time (hh:mm:ss.ms.us) with time of day, or relative "
              "to the previous event.";
        }
        break;
      case eCAN_ID:
        ...
        break;
      case eCAN_NAME:
        c_Retval = "Symbolic name of the CAN message. Empty if not defined in "
                   "one of the "
                   "active database(s).";
        break;
      case eCAN_DIR:
...
*/
//----------------------------------------------------------------------------------------------------------------------
QModelIndex C_CamMetTreeModel::index(const int32_t os32_Row,
...
*/
//----------------------------------------------------------------------------------------------------------------------
QVariant C_CamMetTreeModel::data(const QModelIndex &orc_Index,
                                 const int32_t os32_Role) const {
  QVariant c_Retval;
    ...
    if (((os32_Role == static_cast<int32_t>(Qt::DisplayRole)) ||
        ...
        if (pc_CurMessage != NULL) {
          ...
          case eTIME_STAMP:
            if (this->mq_DisplayTimestampRelative == true) {
              c_Retval = pc_CurMessage->c_TimeStampRelative;
            } else if (this->mq_DisplayTimestampAbsoluteTimeOfDay == false) {
              c_Retval = pc_CurMessage->c_TimeStampAbsoluteStart;
            } else {
              c_Retval = pc_CurMessage->c_TimeStampAbsoluteTimeOfDay;
            }
            break;
          case eCAN_ID:
            if (os32_Role == ms32_USER_ROLE_SORT) {
              ...
              if (this->mq_DisplayAsHex == true) {
                c_Retval = pc_CurMessage->c_CanIdHex;
              } else {
                c_Retval = pc_CurMessage->c_CanIdDec;
              }
            }
            break;
          case eCAN_NAME:
            c_Retval = pc_CurMessage->c_Name;
            break;
          case eCAN_DIR:
            ...
            break;
          case eCAN_DLC:
            c_Retval = pc_CurMessage->c_CanDlc;
            break;
          case eCAN_DATA:
            if (pc_CurMessage->c_ProtocolTextDec == "") {
              if ((pc_CurMessage->c_CanMsg.u8_RTR == 0U) &&
                  ((orc_Index.data(ms32_USER_ROLE_MARKER)
                        .toBitArray()
                        .isEmpty() == true) ||
                   (os32_Role == ms32_USER_ROLE_MARKER_TEXT))) {
                if (this->mq_DisplayAsHex == true) {
                  c_Retval = pc_CurMessage->c_CanDataHex;
                } else {
                  c_Retval = pc_CurMessage->c_CanDataDec;
                }
              }
            } else {
              if (this->mq_DisplayAsHex == true) {
                c_Retval = pc_CurMessage->c_ProtocolTextHex;
              } else {
                c_Retval = pc_CurMessage->c_ProtocolTextDec;
              }
            }
            break;
          case eCAN_COUNTER:
            if (os32_Role == ms32_USER_ROLE_SORT) {
              c_Retval =
                  static_cast<qlonglong>(pc_CurMessage->c_Counter.toLongLong());
            } else {
              c_Retval = pc_CurMessage->c_Counter;
            }
            break;
          case eCAN_STATUS:
            c_Retval = pc_CurMessage->c_Status;
            break;
          default:
            break;
          }
      ...
      if (orc_Index.parent().isValid() == true) {
        ...
        if (pc_CurMessage != NULL) {
          if ((os32_Role == ms32_USER_ROLE_TABLE_ALL_STRINGS_DYNAMIC_COL_1) ||
            ...
            const C_OscComMessageLoggerDataSignal &rc_CurSignal =
                pc_CurMessage->c_Signals[this->TranslateTreeRowsToSignalIndex(
                    orc_Index)];
            QString c_Comment = rc_CurSignal.c_Comment;
            QString c_Value;
            c_List.reserve(5);
            c_Value = rc_CurSignal.c_Name;
            c_List.push_back(c_Value);
            c_Value = rc_CurSignal.c_Value;
            c_List.push_back(c_Value);
            c_Value = rc_CurSignal.c_Unit;
            c_List.push_back(c_Value);
            if (this->mq_DisplayAsHex == true) {
              c_Value = rc_CurSignal.c_RawValueHex;
            } else {
              c_Value = rc_CurSignal.c_RawValueDec;
            }
            c_List.push_back(c_Value);
      ...
      if (e_Col == eCAN_DATA) {
        ...
        if ((pc_CurMessage != NULL) &&
          ...
          if ((((this->ms32_SelectedParentRow >= 0) &&
                (this->mc_SelectedChildBytes.size() > 0UL)) &&
          ...
          if (e_Col == eCAN_DLC) {
            if (pc_CurMessage->q_CanDlcError == true) {
              ...
              if (pc_CurMessage->c_GreyOutInformation.s32_GrayOutValueMsg <
                  static_cast<int32_t>(
                      this->mc_FontTransparcencyColors.size())) {
                c_Retval =
                    this->mc_FontTransparcencyColors[pc_CurMessage
                                                         ->c_GreyOutInformation
                                                         .s32_GrayOutValueMsg];
              }
            }
            ...
            if (pc_CurMessage->c_GreyOutInformation.s32_GrayOutValueMsg <
                static_cast<int32_t>(this->mc_FontTransparcencyColors.size())) {
              c_Retval =
                  this->mc_FontTransparcencyColors
                      [pc_CurMessage->c_GreyOutInformation.s32_GrayOutValueMsg];
            }
          }
        }
      } else {
        const int32_t s32_MessageRow =
            static_cast<int32_t>(orc_Index.parent().parent().isValid()
                                     ? orc_Index.parent().parent().row()
                                     : orc_Index.parent().row());
        const C_CamMetTreeLoggerData *const pc_CurMessage =
        ...
        if (pc_CurMessage != NULL) {
          ...
          if (u32_SignalIndex < pc_CurMessage->c_Signals.size()) {
            const C_OscComMessageLoggerDataSignal &rc_Signal =
                pc_CurMessage->c_Signals[u32_SignalIndex];
            if (rc_Signal.c_OscSignal.e_MultiplexerType ==
              ...
              if (rc_Signal.c_OscSignal.e_MultiplexerType ==
                ...
                const uint32_t u32_MultiplexerSignalIndex =
                    this->TranslateTreeRowsToSignalIndex(orc_Index.parent());
      ...
      // Although you should never see this icon we still need to return
      // something so the default drawing engine
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_CamMetTreeModel::E_Columns
C_CamMetTreeModel::h_ColumnToEnum(const int32_t os32_Column) {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CamMetTreeModel::h_EnumToColumn(const C_CamMetTreeModel::E_Columns oe_Value) {
  int32_t s32_Retval;
...
C_CamMetTreeModel::GetMessageData(const int32_t os32_Row) const {
  ...
  if (this->mq_UniqueMessageMode == false) {
    ...
    const uint32_t u32_Index =
        (this->mu32_OldestItemIndex + static_cast<uint32_t>(os32_Row)) %
        this->mu32_TraceBufferSizeUsed;
    pc_Retval = &this->mc_DataBase[u32_Index];
  } else {
    // If unique messages: look in map
    const QMap<QString, QString>::const_iterator c_ItOrder =
        this->mc_UniqueMessagesOrdering.begin() + os32_Row;
    const QMap<QString, C_CamMetTreeLoggerData>::const_iterator c_It =
...
/*! \brief   Search a matching item
   ...
   \param[in]       oq_Next                     Flag for search direction
                                                 true:  Search the next entry,
   forward false: Search the previous entry, backward
   \param[out]      ors32_SignalRow               < 0: No signal of the message
   as search result
                                                >= 0: If
   ors32_MultiplexedSignalRow >= 0: Row of Multiplexer signal If
   ors32_MultiplexedSignalRow < 0: Row of checked signal
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamMetTreeModel::SearchMessageData(
    const QString &orc_SearchString, const bool oq_Next,
  ...
  if (u32_NumberEntries > 0U) {
    ...
    if ((this->mc_LastSearchedString == orc_SearchString) &&
      ...
      // In case of the same search string, continue search with next or
      // previous index
      if (oq_Next == true) {
        // Forward
        if (this->mq_IsLastSearchedSignalLastOfMessage == true) {
          ...
          u32_StartIndex =
              (static_cast<uint32_t>(this->ms32_LastSearchedMessageRow) + 1) %
              u32_NumberEntries;
        } else {
          // Start search with next available signal of last message
          u32_StartIndex =
              static_cast<uint32_t>(this->ms32_LastSearchedMessageRow);
          s32_CurrentSignalIndex = this->ms32_LastSearchedSignalRow + 1;
          s32_CurrentSignalIndexMultiplexed =
              this->ms32_LastSearchedSignalRowMultiplexed + 1;
          if (s32_CurrentSignalIndex >= 0) {
            // In case of a wrap around search, the signals before the start
            // signal must be checked at the end too No further abort condition
            // for this scenario: Assuming the start condition is the found
            // signal from the last search trigger, so in case of no further
            // result, the same signal is found again which is no problem
        ...
        if (this->ms32_LastSearchedSignalRow < 0) {
          ...
          // but at this point the number of signals of the messages is here not
          // available
          s32_CurrentSignalIndex = s32_START_AT_LAST_SIGNAL;
          s32_CurrentSignalIndexMultiplexed = s32_START_AT_LAST_SIGNAL;
        } else {
          // Start search with previous available signal of last message or the
          // message itself (-1)
          u32_StartIndex =
              static_cast<uint32_t>(this->ms32_LastSearchedMessageRow);
          s32_CurrentSignalIndex = this->ms32_LastSearchedSignalRow - 1;
          ...
          // In case of a wrap around search, the signals after the start signal
          // must be checked at the end too No further abort condition for this
          // scenario: Assuming the start condition is the found signal from the
          // last search trigger, so in case of no further result, the same
          // signal is found again which is no problem
          ++u32_NumberEntriesToCheck;
        }
      }
    ...
    for (u32_Counter = 0U; u32_Counter < u32_NumberEntriesToCheck;
         ++u32_Counter) {
      uint32_t u32_Row;
      ...
      if (oq_Next == true) {
        ...
        if (u32_Counter > 0U) {
          // special case: starting at the last signal of the message due to
          // backward search the decision of the first message of the loop is
          // set at the initial check of the start indexs
          s32_CurrentSignalIndex = s32_START_AT_LAST_SIGNAL;
          s32_CurrentSignalIndexMultiplexed = s32_START_AT_LAST_SIGNAL;
        }
      }
      ...
      if (pc_Data != NULL) {
        ...
        if (s32_CurrentSignalIndex <= s32_START_AT_LAST_SIGNAL) {
          ...
          s32_CurrentSignalIndex =
              static_cast<int32_t>(pc_Data->c_Signals.size()) - 1;
          s32_CurrentSignalIndexMultiplexed = s32_CurrentSignalIndex;
        }

        // Search the message and all of its signals till something was found or
        // no signals are left to check
        q_SearchMessageResult = this->m_CheckDataForSearch(
            *pc_Data, orc_SearchString, oq_Next, s32_CurrentSignalIndex,
        ...
        if (q_SearchMessageResult == true) {
          ...
          this->ms32_LastSearchedSignalRowMultiplexed =
              this->ms32_LastSearchedSignalRow;
          ...
          if (s32_CurrentSignalIndex >= 0) {
            ...
            if (C_CamMetTreeModel::mh_IsMessageMultiplexed(*pc_Data) == true) {
              ...
              C_CamMetTreeModel::mh_SortMultiplexedSignals(
                  c_CopyData.c_Signals);

              // Search again
              q_SearchMessageResult = this->m_CheckDataForSearch(
                  c_CopyData, orc_SearchString, oq_Next,
                  s32_CurrentSignalIndexMultiplexed, q_LastMessageDummy);
              ...
              if (q_SearchMessageResult == true) {
                ...
                this->m_GetMultiplexedMsgSignalRow(
                    c_CopyData.c_Signals,
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_CamMetTreeModel::TranslateTreeRowsToSignalIndex(
    const QModelIndex &orc_CurrentIndex) const {
  uint32_t u32_Retval;

  if (orc_CurrentIndex.parent().isValid()) {
    if (orc_CurrentIndex.parent().parent().isValid()) {
      u32_Retval = TranslateTreeRowsToSignalIndex(
          orc_CurrentIndex.parent().parent().row(),
          orc_CurrentIndex.parent().row(), orc_CurrentIndex.row());
    } else {
      u32_Retval = TranslateTreeRowsToSignalIndex(
          orc_CurrentIndex.parent().row(), orc_CurrentIndex.row(), -1);
    }
  } else {
    // Unexpected
    u32_Retval = 0UL;
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_CamMetTreeModel::TranslateTreeRowsToSignalIndex(
    const int32_t os32_MessageIndex, const int32_t os32_SignalIndex,
...
/*! \brief  Drag functionality using flags set from base classes
   ...
   \retval   return OR operation of Qt::ItemIsDragEnabled and
...
Qt::ItemFlags C_CamMetTreeModel::flags(const QModelIndex &orc_Index) const {
  ...
  if (!orc_Index.isValid()) {
    ...
    c_ItemFlagsResult =
        Qt::ItemIsDragEnabled | QAbstractItemModel::flags(orc_Index);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Reserves new size by the necessary vectors and sets the configured
 * buffer size as used value
 */
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::m_AdaptTraceBufferSize(void) {
  ...
  // lint -e{1938}  static const is guaranteed preinitialized before main
  this->mpc_RootItemContinuous->ReserveChildrenSpace(
      this->mu32_TraceBufferSizeUsed);
}
...
std::vector<int32_t> C_CamMetTreeModel::m_AddRowsContinuousMode(
  ...
  if (orc_Data.empty() == false) {
    ...
    const uint32_t u32_CompleteSize =
        static_cast<uint32_t>(this->mc_DataBase.size()) +
    ...
    // Check if we need to append any items to reach the max storage size
      ...
      // Simple model reset notification (if replaced: look at two segments:
      // adding new and queuing mechanism)
      if (this->mq_UniqueMessageMode == false) {
        this->beginResetModel();
      }
      ...
      for (std::list<C_CamMetTreeLoggerData>::const_iterator c_ItData =
        ...
        // 2. start replacing oldest ones
          ...
          this->mpc_RootItemContinuous->c_Children.erase(
              this->mpc_RootItemContinuous->c_Children.begin());
          this->mpc_RootItemContinuous->AddChild(pc_Item);
          // Iterate
      ...
      // Begin queue shift
      if (this->mq_UniqueMessageMode == false) {
        // Use reset model to avoid console output, warning: does have negative
        // side effects, but also faster
        this->beginResetModel();
      ...
      if (this->mq_UniqueMessageMode == false) {
        // Every reset will clear the selection
        this->SetSelection(-1, -1);
        // Use reset model to avoid console output, warning: reset model does
  ...
  if (orc_Data.empty() == false) {
    ...
    for (std::list<C_CamMetTreeLoggerData>::const_iterator c_ItData =
      ...
      const QMap<QString, C_CamMetTreeLoggerData>::const_iterator
          c_ItMessage = this->mc_UniqueMessages.find(c_ItData->c_CanIdDec);
      const int32_t s32_MuxValue =
          C_CamMetUtil::h_GetMultiplexerValue(c_ItData->c_Signals);

      // Check if there is a new row
      if (c_ItMessage != this->mc_UniqueMessages.end()) {
        this->m_HandleNewUniqueMessageForExistingUniqueMessage(
            *c_ItData, c_ItMessage.key(), s32_MuxValue);
      } else {
...
/*! \brief  Handle new unique message for existing unique message entry
   ...
   \param[in]  orc_ExistingMessageKey  Key to use to find the existing message
   position
   \param[in]  os32_MultiplexerValue   Current multiplexer value
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::m_HandleNewUniqueMessageForExistingUniqueMessage(
  ...
  C_CamMetTreeLoggerData &rc_Message =
      this->mc_UniqueMessages[orc_Message.c_CanIdDec];
  const uint64_t u64_PreviousAbsoluteTimestampValue =
      rc_Message.u64_TimeStampAbsoluteStart;
  const uint64_t u64_NewAbsoluteTimestampValue =
      orc_Message.u64_TimeStampAbsoluteStart;
  uint64_t u64_RelativeTimestamp;

  const uint32_t u32_PrevMsgTimeStamp =
      static_cast<uint32_t>(rc_Message.c_CanMsg.u64_TimeStamp / 1000ULL);
  const uint32_t u32_NewMsgTimeStamp =
      static_cast<uint32_t>(orc_Message.c_CanMsg.u64_TimeStamp / 1000ULL);
  ...
  const C_CamMetTreeLoggerDataGreyOutInformation c_PreviousInfo =
      C_CamMetTreeModel::mh_ExtractPreviousGreyOutInformation(
          rc_Message, orc_Message, u32_PrevMsgTimeStamp, u32_NewMsgTimeStamp,
          os32_MultiplexerValue, q_UpdateDataTimeStamp);

  // Keep not redundant signal information (multiplexer + multiplexed signal)
  C_CamMetTreeModel::mh_CopyMessageWhileKeepingUniqueSignals(
      rc_Message, orc_Message, os32_MultiplexerValue);

  C_CamMetTreeModel::mh_ApplyPreviousGreyOutInformation(
      rc_Message, c_PreviousInfo, q_UpdateDataTimeStamp, u32_PrevMsgTimeStamp);

  // Handle relative timestamp value
  if (u64_NewAbsoluteTimestampValue > u64_PreviousAbsoluteTimestampValue) {
    u64_RelativeTimestamp =
        u64_NewAbsoluteTimestampValue - u64_PreviousAbsoluteTimestampValue;
  } else {
    u64_RelativeTimestamp = 0U;
  }
  rc_Message.c_TimeStampRelative =
      C_OscComMessageLoggerData::h_GetTimestampAsString(u64_RelativeTimestamp);

  // Search existing item index
  for (QMap<QString, QString>::const_iterator c_ItMsg =
           this->mc_UniqueMessagesOrdering.begin();
  ...
  m_UpdateTreeItemBasedOnMessage(
      this->mpc_RootItemStatic
          ->c_Children[static_cast<uint32_t>(s32_MessageCounter)],
      rc_Message, true, s32_MessageCounter);
  // Notify the model of data changes for all replaced items (current: all
  // items)
  if (this->mq_UniqueMessageMode == true) {
    ...
    const int32_t s32_ColIdData = C_CamMetTreeModel::h_EnumToColumn(eCAN_DATA);
    const int32_t s32_ColIdTime =
        C_CamMetTreeModel::h_EnumToColumn(eTIME_STAMP);
    const int32_t s32_ColIdCounter =
        C_CamMetTreeModel::h_EnumToColumn(eCAN_COUNTER);
    const int32_t s32_ColIdStatus =
        C_CamMetTreeModel::h_EnumToColumn(eCAN_STATUS);
    // Notification for possible new child
    this->dataChanged(this->index(s32_MessageCounter, s32_ColIdTime),
                      this->index(s32_MessageCounter, s32_ColIdTime));
    this->dataChanged(this->index(s32_MessageCounter, s32_ColIdCounter),
                      this->index(s32_MessageCounter, s32_ColIdCounter));
    // Possible changes
    this->dataChanged(this->index(s32_MessageCounter, s32_ColIdName),
                      this->index(s32_MessageCounter, s32_ColIdName));
    this->dataChanged(this->index(s32_MessageCounter, s32_ColIdDlc),
                      this->index(s32_MessageCounter, s32_ColIdDlc));
    this->dataChanged(this->index(s32_MessageCounter, s32_ColIdData),
                      this->index(s32_MessageCounter, s32_ColIdData));
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::m_HandleNewUniqueMessage(
    const C_CamMetTreeLoggerData &orc_Message,
    const int32_t os32_MultiplexerValue) {
  bool q_NewRow = false;
  ...
  QMap<QString, C_CamMetTreeLoggerData>::iterator c_NewPos;
  // Handle begin!
  C_TblTreSimpleItem *const pc_NewItem = new C_TblTreSimpleItem();
  const int32_t s32_EstimatedPosIndex =
      this->m_GetPosIndexForUniqueMessage(orc_Message.c_TimeStampAbsoluteStart);
  // Update tree with known index
  m_UpdateTreeItemBasedOnMessage(pc_NewItem, orc_Message, false,
  ...
  if (this->mq_UniqueMessageMode == true) {
    ...
    this->beginInsertRows(QModelIndex(), s32_EstimatedPosIndex,
                          s32_EstimatedPosIndex);
  }

  // Insert new item
  this->mc_UniqueMessagesOrdering.insert(orc_Message.c_TimeStampAbsoluteStart,
                                         orc_Message.c_CanIdDec);
  c_NewPos =
      this->mc_UniqueMessages.insert(orc_Message.c_CanIdDec, orc_Message);
  // New message, new data. Update the timestamp of the CAN message data and its
  // bytes
  c_NewPos->c_GreyOutInformation.u32_DataChangedTimeStamp =
      static_cast<uint32_t>(c_NewPos->c_CanMsg.u64_TimeStamp / 1000ULL);
  for (u8_DbCounter = 0; u8_DbCounter < c_NewPos->c_CanMsg.u8_DLC;
       ++u8_DbCounter) {
  ...
  if (os32_MultiplexerValue >= 0) {
    c_NewPos->c_GreyOutInformation
        .c_MapMultiplexerValueToChangedTimeStamps[os32_MultiplexerValue] =
        c_NewPos->c_GreyOutInformation.u32_DataChangedTimeStamp;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamMetTreeModel::m_GetPosIndexForUniqueMessage(
    const QString &orc_AbsoluteTimeStamp) const {
  int32_t s32_EstimatedPosIndex = 0;
  // Get first valid item after the position it should be at
  const QMap<QString, QString>::const_iterator c_EstimatedPos =
      this->mc_UniqueMessagesOrdering.upperBound(orc_AbsoluteTimeStamp);

  // Calc estimated new position index
  for (QMap<QString, QString>::const_iterator c_ItUniqueMessages =
           this->mc_UniqueMessagesOrdering.begin();
       (c_ItUniqueMessages != this->mc_UniqueMessagesOrdering.end()) &&
       (c_ItUniqueMessages != c_EstimatedPos);
       ++c_ItUniqueMessages) {
    ++s32_EstimatedPosIndex;
  }
...
void C_CamMetTreeModel::m_GrayOutTimer(void) {
  if ((this->mq_GrayOutPause == false) &&
    ...
    for (c_It = this->mc_UniqueMessages.begin();
         c_It != this->mc_UniqueMessages.end(); ++c_It) {
      C_CamMetTreeLoggerData &rc_Data = c_It.value();
      const uint32_t u32_DiffMsg =
          u32_CurrentTime -
          static_cast<uint32_t>(rc_Data.c_CanMsg.u64_TimeStamp / 1000ULL);
      // TimeStamp for data is set by AddRows when new CAN messages are added to
      // the model
      ...
      for (std::map<int32_t, uint32_t>::const_iterator c_ItValue =
        ...
        s32_TransparencyStepDataByte =
            C_CamMetTreeModel::mh_GetTransparencyStep(u32_DiffDataByte);
        ...
        if (s32_TransparencyStepDataByte !=
          ...
          // Send the change only if the value changed really and let update the
          // foreground only
          c_Roles.push_back(static_cast<int32_t>(Qt::ForegroundRole));
          this->dataChanged(
              this->index(s32_Row, 0, this->index(s32_RowCounter, 0)),
              this->index(s32_Row, 0, this->index(s32_RowCounter, 0)), c_Roles);
        }
      }

      // Check data byte gray out adaption
      for (s32_CounterDataByte = 0;
           s32_CounterDataByte < rc_Data.c_CanMsg.u8_DLC;
           ++s32_CounterDataByte) {
        const uint32_t u32_DiffDataByte =
            u32_CurrentTime -
            rc_Data.c_GreyOutInformation
                .c_DataBytesChangedTimeStamps[s32_CounterDataByte];
        int32_t s32_TransparencyStepDataByte;
        ...
        if (s32_TransparencyStepDataByte !=
            rc_Data.c_GreyOutInformation
                .c_GrayOutValueDataBytes[s32_CounterDataByte]) {
      ...
      if (s32_TransparencyStepMsg !=
          rc_Data.c_GreyOutInformation.s32_GrayOutValueMsg) {
        // Update the gray out value. The gray out value for data will be
        // adapted too
        const int32_t s32_ColIdTime =
            C_CamMetTreeModel::h_EnumToColumn(eTIME_STAMP);
        const int32_t s32_ColIdCounter =
        ...
        // foreground only
        c_Roles.push_back(static_cast<int32_t>(Qt::ForegroundRole));
        this->dataChanged(this->index(s32_RowCounter, s32_ColIdTime),
                          this->index(s32_RowCounter, s32_ColIdCounter),
                          c_Roles);
      } else if (q_DataByteChangedTransparency == true) {
        // Update the gray out value of CAN message data only.
        const int32_t s32_ColData =
            C_CamMetTreeModel::h_EnumToColumn(eCAN_DATA);
        QVector<int32_t> c_Roles;

        // Send the change only if the value changed really and let update the
        // foreground only
        c_Roles.push_back(static_cast<int32_t>(Qt::ForegroundRole));
        this->dataChanged(this->index(s32_RowCounter, s32_ColData),
                          this->index(s32_RowCounter, s32_ColData), c_Roles);
      } else {
        // Nothing to do
      }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Detects the necessary transparency step dependent of the difference
   time
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_CamMetTreeModel::mh_GetTransparencyStep(const uint32_t ou32_DiffTime) {
  ...
  if (ou32_DiffTime > mu32_TRACE_TRANSPARENCY_TIME_START) {
    if (ou32_DiffTime >= mu32_TRACE_TRANSPARENCY_TIME_END) {
      ...
      // Interpolation
      const uint32_t u32_DiffPart =
          ou32_DiffTime - mu32_TRACE_TRANSPARENCY_TIME_START;
      s32_TransparencyStep = static_cast<int32_t>(
          (u32_DiffPart *
           static_cast<uint32_t>(ms32_TRACE_TRANSPARENCY_STEPS)) /
...
C_CamMetTreeModel::mh_GetCount(const C_CamMetTreeLoggerData &orc_Message) {
  ...
  // Append words
  for (uint32_t u32_ItSig = 0UL; u32_ItSig < orc_Message.c_Signals.size();
       ++u32_ItSig) {
    const C_OscComMessageLoggerDataSignal &rc_Signal =
        orc_Message.c_Signals[u32_ItSig];
    c_W1.push_back(rc_Signal.c_Name);
    c_W2.push_back(rc_Signal.c_Unit);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Resize string to the specified maximum length if the string exceeds
   this limit

   \param[in,out]  orc_Str          String that will get resized if necessary
   \param[in]      os32_MaxLength   Maximum length the string may not exceed
   after calling this function
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::mh_ResizeIfNecessary(QString &orc_Str,
                                             const int32_t os32_MaxLength) {
  orc_Str.resize(std::min(static_cast<int32_t>(orc_Str.count()),
                          static_cast<int32_t>(os32_MaxLength)));
}
...
/*! \brief  Update tree item based on message
   ...
   \param[in]      oq_SignalInsert  Flag if begin and end insert should be
   called
   \param[in]      os32_MessageRow  Message row index
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::m_UpdateTreeItemBasedOnMessage(
  ...
  if (c_Order.size() == 0UL) {
    ...
    if (opc_Item->c_Children.size() < orc_Message.c_Signals.size()) {
      if ((oq_SignalInsert) && (os32_MessageRow >= 0)) {
        this->beginInsertRows(
            this->index(os32_MessageRow, 0),
            static_cast<int32_t>(opc_Item->c_Children.size()),
            static_cast<int32_t>(opc_Item->c_Children.size() +
                                 orc_Message.c_Signals.size()));
      }
      // Add
      for (uint32_t u32_ItSig = opc_Item->c_Children.size();
    ...
    for (uint32_t u32_ItOr = 0UL; u32_ItOr < c_Order.size(); ++u32_ItOr) {
      ...
      for (uint32_t u32_ItSig = 0UL; u32_ItSig < orc_Message.c_Signals.size();
           ++u32_ItSig) {
        const C_OscComMessageLoggerDataSignal &rc_SignalData =
            orc_Message.c_Signals[u32_ItSig];
        if (rc_SignalData.c_OscSignal.e_MultiplexerType ==
            C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) {
          if (static_cast<int32_t>(
                  rc_SignalData.c_OscSignal.u16_MultiplexValue) ==
              c_Order[u32_ItOr]) {
            ++u32_Counter;
          }
        }
    ...
    for (uint32_t u32_ItChild = 0UL; u32_ItChild < opc_Item->c_Children.size();
         ++u32_ItChild) {
    ...
    if (c_Expected != c_Current) {
      ...
      const bool q_FixByInsertingNewChild =
          C_CamMetTreeModel::mh_CheckForFixByInsertingNewChild(
              c_Expected, c_Current, u32_InsertAt, u32_InsertNum);
      if (q_FixByInsertingNewChild) {
        if ((oq_SignalInsert) && (os32_MessageRow >= 0)) {
          this->beginInsertRows(this->index(os32_MessageRow, 0), u32_InsertAt,
                                u32_InsertAt);
        }
        C_TblTreSimpleItem *const pc_Parent = new C_TblTreSimpleItem();
        for (uint32_t u32_ItChild = 0UL; u32_ItChild < u32_InsertNum;
             ++u32_ItChild) {
      ...
      } // lint !e429  no memory leak because of the parent and the Qt memory
        // management
      else {
        if ((oq_SignalInsert) && (os32_MessageRow >= 0)) {
          this->beginResetModel();
        ...
        opc_Item->ClearChildren();
        // Rebuild as expected
        for (uint32_t u32_ItExpected = 0UL; u32_ItExpected < c_Expected.size();
             ++u32_ItExpected) {
          C_TblTreSimpleItem *const pc_Parent = new C_TblTreSimpleItem();
          for (uint32_t u32_ItChild = 0UL;
               u32_ItChild < c_Expected[u32_ItExpected]; ++u32_ItChild) {
            pc_Parent->AddChild(new C_TblTreSimpleItem());
          }
...
/*! \brief   Compares one data entry with the search string
   ...
   \param[in]     oq_Next               Flag for search direction of signals
                                          ...
                                          false: Search the previous entry,
   backward
   \param[in,out] ors32_SignalIndex      Input: >= 0: signal index to start
   search; -1: start with message Output: >= 0: signal matches; -1: no signal
   matched or the message matches
   \param[out]    orq_LastSignalChecked true: all signals were checked of the
   message, not necessary to check in next round false: something did match to
   the search string, but not all signal were checked. At least one further
   check is necessary for this message
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamMetTreeModel::m_CheckDataForSearch(
    const C_CamMetTreeLoggerData &orc_Data, const QString &orc_SearchString,
  ...
  if (orc_SearchString != "") {
    ...
    if (os32_StartSignalRow < 0) {
      ...
      if (oq_Next == false) {
        // Searching backward means in this case no signals of the message will
        // be checked at this run
        q_SkipSignals = true;
      }
    }
    ...
    if (q_SkipSignals == false) {
      ...
      if (orc_Data.c_Signals.size() > 0) {
        ...
        if (q_Return == false) {
          ...
          do {
            // Check each signal
            q_Return = this->m_CheckSignalDataForSearch(
                orc_Data.c_Signals[u32_Counter], orc_SearchString);
          ...
          // Check if the last signal was reached. Not relevant if it matched or
          // not
          if (u32_Counter ==
              (static_cast<uint32_t>(orc_Data.c_Signals.size()) - 1UL)) {
            orq_LastSignalChecked = true;
          }

          // Special case: Searching backward, started with a signal and nothing
          // found yet -> Search message now
          if ((oq_Next == false) && (q_Return == false)) {
            q_Return =
                this->m_CheckMessageDataForSearch(orc_Data, orc_SearchString);
          }
        }
      } else {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamMetTreeModel::m_CheckMessageDataForSearch(
    const C_CamMetTreeLoggerData &orc_Data,
    const QString &orc_SearchString) const {
  bool q_Return = false;

  // Compare all in upper case to be case insensitive
  const QString c_SearchString = orc_SearchString.toUpper();
  ...
  if (orc_SearchString != "") {
    ...
    if ((orc_Data.c_Name.toUpper().contains(c_SearchString)) ||
        (orc_Data.c_CanDlc.contains(c_SearchString)) ||
        ((this->mq_DisplayTimestampRelative == true) &&
         (orc_Data.c_TimeStampRelative.contains(c_SearchString))) ||
        ((this->mq_DisplayTimestampAbsoluteTimeOfDay == true) &&
         (orc_Data.c_TimeStampAbsoluteTimeOfDay.contains(c_SearchString))) ||
        ((this->mq_DisplayTimestampAbsoluteTimeOfDay == false) &&
         (orc_Data.c_TimeStampAbsoluteStart.contains(c_SearchString)))) {
      q_Return = true;
    } else {
      if (this->mq_DisplayAsHex == true) {
        // Check hex display specific strings
        if ((orc_Data.c_CanIdHex.toUpper().contains(c_SearchString)) ||
            (orc_Data.c_CanDataHex.toUpper().contains(c_SearchString)) ||
            (orc_Data.c_ProtocolTextHex.toUpper().contains(c_SearchString))) {
          q_Return = true;
        }
      } else {
        // Check decimal display specific strings
        if ((orc_Data.c_CanIdDec.contains(c_SearchString)) ||
            (orc_Data.c_CanDataDec.contains(c_SearchString)) ||
            (orc_Data.c_ProtocolTextDec.toUpper().contains(c_SearchString))) {
          q_Return = true;
        }
      }
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamMetTreeModel::m_CheckSignalDataForSearch(
    const C_OscComMessageLoggerDataSignal &orc_SignalData,
    const QString &orc_SearchString) const {
  bool q_Return = false;
  ...
  if (orc_SearchString != "") {
    ...
    if ((orc_SignalData.c_Name.toUpper().contains(c_SearchString)) ||
        (orc_SignalData.c_Comment.toUpper().contains(c_SearchString)) ||
        (orc_SignalData.c_Unit.toUpper().contains(c_SearchString)) ||
        (orc_SignalData.c_Value.toUpper().contains(c_SearchString))) {
      q_Return = true;
    } else {
      if (this->mq_DisplayAsHex == true) {
        // Check hex display specific strings
        if (orc_SignalData.c_RawValueHex.toUpper().contains(c_SearchString)) {
          q_Return = true;
        }
      } else {
        // Check decimal display specific strings
        if (orc_SignalData.c_RawValueDec.contains(c_SearchString)) {
          q_Return = true;
        }
      }
    }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Checks a signal for its multiplexer configuration and the resulting
   row position and tree layer position

   If ors32_MultiplexedSignalRow is >= 0, the checked  signal is a multiplexed
   signal and the signal is on the third level of the model. Then os32_SignalRow
   equals the row of the multiplexer signal.

   If ors32_MultiplexedSignalRow is < 0, the checked signal can be a normal
   signal or the multiplexer signal and is on the second level of the model.

   Condition: The vector of signals must be sorted by function
   mh_SortMultiplexedSignals or by default

   \param[in]       orc_Signals                All signals to compare and check
   with
   \param[in]       ou32_SignalIndexToCheck    The signal to check
   \param[out]      ors32_SignalRow              >= 0: If
   ors32_MultiplexedSignalRow >= 0: Row of Multiplexer signal If
   ors32_MultiplexedSignalRow < 0: Row of checked signal < 0:
   ou32_SignalIndexToCheck not valid
   \param[out]      ors32_MultiplexedSignalRow   >= 0: Row of Multiplexed signal
                                                 < 0: Signal is not a
   multiplexed signal
*/
...
void C_CamMetTreeModel::m_GetMultiplexedMsgSignalRow(
  ...
  if (ou32_SignalIndexToCheck < orc_Signals.size()) {
    uint32_t u32_Counter;
    const C_OscCanSignal &rc_OscSginalToCheck =
        orc_Signals[ou32_SignalIndexToCheck].c_OscSignal;

    // Multiplexer signals come first
    // Multiplexed signals has the matching multiplexer signal as parent
    // Default signals come after the multiplexer signals with its multiplexed
    // signals
    if (rc_OscSginalToCheck.e_MultiplexerType == C_OscCanSignal::eMUX_DEFAULT) {
      ...
      for (u32_Counter = 0U; u32_Counter < orc_Signals.size(); ++u32_Counter) {
        const C_OscCanSignal &rc_OscSginal =
            orc_Signals[u32_Counter].c_OscSignal;

        // Count all multiplexer signals which are always at the beginning and
        // all default signals which have a lower index Multiplexed signals are
        // not on the same level
        if ((rc_OscSginal.e_MultiplexerType ==
             C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) ||
    ...
    } else if (rc_OscSginalToCheck.e_MultiplexerType ==
               C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) {
      ...
      for (u32_Counter = 0U; u32_Counter < orc_Signals.size(); ++u32_Counter) {
        const C_OscComMessageLoggerDataSignal &rc_Signal =
            orc_Signals[u32_Counter];
        const C_OscCanSignal &rc_OscSginal = rc_Signal.c_OscSignal;

        if ((u32_Counter < ou32_SignalIndexToCheck) &&
            (rc_OscSginal.e_MultiplexerType ==
             C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) &&
            (rc_OscSginal.u16_MultiplexValue ==
             rc_OscSginalToCheck.u16_MultiplexValue)) {
          // Search the multiplexed signals with the same multiplexer value with
          // a lower index to get the row of the checked signal below the
          // multiplexer signal
          ++ors32_MultiplexedSignalRow;
        } else if (rc_OscSginal.e_MultiplexerType ==
                   C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) {
          bool ok_2505 = false;
          const int32_t s32_ValueTmp = rc_Signal.c_RawValueDec.toInt(&ok_2505);
          const int32_t s32_Value = ok_2505 ? s32_ValueTmp : -1;
          ...
          if (s32_Value >= 0) {
            ...
            if (u16_Value <= rc_OscSginalToCheck.u16_MultiplexValue) {
              ...
              // Starting with -1 all multiplexer signals whit a concrete value
              // lower or equal to the expected value count for the row
              ++ors32_SignalRow;
            }
          }
        } else {
      ...
      // Multiplexer signal
      bool ok_2525 = false;
      const int32_t s32_CheckValueTmp = orc_Signals[ou32_SignalIndexToCheck].c_RawValueDec.toInt(&ok_2525);
      const int32_t s32_CheckValue = ok_2525 ? s32_CheckValueTmp : -1;

      if (s32_CheckValue >= 0) {
        for (u32_Counter = 0U; u32_Counter < orc_Signals.size();
             ++u32_Counter) {
          const C_OscComMessageLoggerDataSignal &rc_Signal =
              orc_Signals[u32_Counter];
          const C_OscCanSignal &rc_OscSginal = rc_Signal.c_OscSignal;
          bool ok_2533 = false;
          const int32_t s32_ValueTmp = rc_Signal.c_RawValueDec.toInt(&ok_2533);
          const int32_t s32_Value = ok_2533 ? s32_ValueTmp : -1;
...
/*! \brief  Extract previous gray out value information to handle new message
   ...
   \param[out]  orq_UpdateDataTimeStamp   Flag if data timestamp should be
   updated
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_CamMetTreeLoggerDataGreyOutInformation
C_CamMetTreeModel::mh_ExtractPreviousGreyOutInformation(
    const C_CamMetTreeLoggerData &orc_MessagePrev,
    const C_CamMetTreeLoggerData &orc_MessageNew,
  ...
  c_Retval.s32_GrayOutValueMsg =
      orc_MessagePrev.c_GreyOutInformation.s32_GrayOutValueMsg;
  ...
  c_Retval.c_MapMultiplexerValueToChangedTimeStamps =
      orc_MessagePrev.c_GreyOutInformation
          .c_MapMultiplexerValueToChangedTimeStamps;

  // Check if the CAN message data changed
  if (orc_MessagePrev.c_CanDataHex != orc_MessageNew.c_CanDataHex) {
    // Old message with changed data. Update the timestamp of the CAN message
    // data
    orq_UpdateDataTimeStamp = true;
  } else {
    // Using the previous timestamp again. It may not be overwritten by the new
    // message with an unset timestamp
    c_Retval.u32_DataChangedTimeStamp =
        orc_MessagePrev.c_GreyOutInformation.u32_DataChangedTimeStamp;
  }
  ...
  if (orc_MessagePrev.c_CanMsg.u8_DLC < orc_MessageNew.c_CanMsg.u8_DLC) {
    ...
    for (uint8_t u8_DbCounter = orc_MessagePrev.c_CanMsg.u8_DLC;
         u8_DbCounter < orc_MessageNew.c_CanMsg.u8_DLC; ++u8_DbCounter) {
      // Set the current timestamp for the changed byte
      c_Retval.c_DataBytesChangedTimeStamps[u8_DbCounter] =
          ou32_PrevMsgTimeStamp;
    }
  }

  // reset the gap between old and new dlc
  for (uint8_t u8_DbCounter = 0; u8_DbCounter < orc_MessagePrev.c_CanMsg.u8_DLC;
       ++u8_DbCounter) {
    // replace timestamp when databyte has changed
    if (orc_MessagePrev.c_CanMsg.au8_Data[u8_DbCounter] !=
        orc_MessageNew.c_CanMsg.au8_Data[u8_DbCounter]) {
      // Set the current timestamp for the changed byte
      c_Retval.c_DataBytesChangedTimeStamps[u8_DbCounter] =
          ou32_PrevMsgTimeStamp;
    } else {
      ...
      c_Retval.c_DataBytesChangedTimeStamps[u8_DbCounter] =
          orc_MessagePrev.c_GreyOutInformation
              .c_DataBytesChangedTimeStamps[u8_DbCounter];
    }
  }
  // Update specific timestamp of multiplexer map
  if (os32_MultiplexerValue >= 0) {
    c_Retval.c_MapMultiplexerValueToChangedTimeStamps[os32_MultiplexerValue] =
        ou32_NewMsgTimeStamp;
  }
  return c_Retval;
}
...
/*! \brief  Apply previous gray out value to new message
   ...
   \param[in]      oq_UpdateDataTimeStamp          Flag to update the data
   timestamps
   \param[in]      ou32_PrevMsgTimeStamp           Previous message timestamp
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::mh_ApplyPreviousGreyOutInformation(
    C_CamMetTreeLoggerData &orc_Message,
    const C_CamMetTreeLoggerDataGreyOutInformation
  ...
  if (oq_UpdateDataTimeStamp == true) {
    // Update the timestamp. Update it after the assignment to avoid further
    // temporary copies of the instance
    orc_Message.c_GreyOutInformation.u32_DataChangedTimeStamp =
        ou32_PrevMsgTimeStamp;
  } else {
    // Restore the previous timestamp
    orc_Message.c_GreyOutInformation.u32_DataChangedTimeStamp =
        orc_StoredGrayOutInformation.u32_DataChangedTimeStamp;
  }

  // Reassign the data byte timestamps
  for (uint8_t u8_DbCounter = 0; u8_DbCounter < 8U; ++u8_DbCounter) {
    orc_Message.c_GreyOutInformation
        .c_DataBytesChangedTimeStamps[u8_DbCounter] =
        orc_StoredGrayOutInformation.c_DataBytesChangedTimeStamps[u8_DbCounter];
  }

  // Restore previous values after the assignment to avoid further temporary
  // copies of the instance
  orc_Message.c_GreyOutInformation.s32_GrayOutValueMsg =
      orc_StoredGrayOutInformation.s32_GrayOutValueMsg;
  ...
  orc_Message.c_GreyOutInformation.c_GrayOutValueDataBytes =
      orc_StoredGrayOutInformation.c_GrayOutValueDataBytes;
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check if the difference between the two vectors can be fixed by
...
bool C_CamMetTreeModel::mh_CheckForFixByInsertingNewChild(
  ...
  if (orc_ExpectedVec.size() == (orc_CurrentVec.size() + 1UL)) {
    ...
    for (uint32_t u32_ItCurrent = 0;
         (u32_ItCurrent < orc_CurrentVec.size()) && q_FixByInsertingNewChild;
         ++u32_ItCurrent) {
      if (u32_Counter < orc_ExpectedVec.size()) {
        if (orc_ExpectedVec[u32_Counter] == orc_CurrentVec[u32_ItCurrent]) {
          ...
          if (q_FoundMismatch) {
            ...
            if (u32_Counter < orc_ExpectedVec.size()) {
              if (orc_ExpectedVec[u32_Counter] ==
                  orc_CurrentVec[u32_ItCurrent]) {
                // next element matches again so continue
                // pass
                ++u32_Counter;
              } else {
                // The current element and the next expected element also don't
                // match, so at least two operations
                //  are necessary
                // No match possible
                q_FixByInsertingNewChild = false;
              }
            } else {
              // The current vector contains one more element than the expected
              // and also has a mismatch, so at
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Copy the new message content while keeping relevant signals from the
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::mh_CopyMessageWhileKeepingUniqueSignals(
...

{
  ...
  if (os32_MuxValue >= 0) {
    ...
    for (std::vector<C_OscComMessageLoggerDataSignal>::iterator c_ItSig =
      ...
      if (rc_Sig.c_OscSignal.e_MultiplexerType ==
          C_OscCanSignal::eMUX_DEFAULT) {
        ...
        if (rc_Sig.c_OscSignal.e_MultiplexerType ==
            C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) {
          // Multiplexed
          if (rc_Sig.c_OscSignal.u16_MultiplexValue ==
              static_cast<uint16_t>(os32_MuxValue)) {
            // Will be replaced
          } else {
            // Keep information
          ...
          // Multiplexer
          //  In case of a DLC error the signal will be replaced in any case and
          //  avoid getting the invalid value as integer
          if ((rc_Sig.q_DlcError == true) ||
              (rc_Sig.c_RawValueDec.toInt() ==
               static_cast<uint16_t>(os32_MuxValue))) {
            // Will be replaced
          } else {
            // Keep information
...
uint32_t C_CamMetTreeModel::mh_TranslateTreeRowsToSignalIndex(
  ...
  if (static_cast<uint32_t>(os32_SignalIndex) < c_Order.size()) {
    // Part of multiplexer
    for (uint32_t u32_ItSigL1 = 0UL; u32_ItSigL1 < orc_Signals.size();
         ++u32_ItSigL1) {
      const C_OscComMessageLoggerDataSignal &rc_SignalData =
          orc_Signals[u32_ItSigL1];
      if (os32_SignalIndexL2 >= 0) {
        if (rc_SignalData.c_OscSignal.e_MultiplexerType ==
          ...
          if (static_cast<int32_t>(
            ...
            if (u32_Counter == static_cast<uint32_t>(os32_SignalIndexL2)) {
              ...
              // Only count children (= multiplexed signals with this
              // multiplexer value)
              ++u32_Counter;
            }
          }
        ...
        if ((rc_SignalData.c_OscSignal.e_MultiplexerType ==
            ...
            (rc_SignalData.c_RawValueDec.toInt() ==
             c_Order[static_cast<uint32_t>(os32_SignalIndex)])) {
    ...
    // Others
    for (uint32_t u32_ItSig = 0UL; u32_ItSig < orc_Signals.size();
         ++u32_ItSig) {
      const C_OscComMessageLoggerDataSignal &rc_SignalData =
          orc_Signals[u32_ItSig];
      if (rc_SignalData.c_OscSignal.e_MultiplexerType ==
          C_OscCanSignal::eMUX_DEFAULT) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamMetTreeModel::mh_IsMessageMultiplexed(
    const C_CamMetTreeLoggerData &orc_Data) {
  bool q_Return = false;
  uint32_t u32_Counter;

  for (u32_Counter = 0U; u32_Counter < orc_Data.c_Signals.size();
       ++u32_Counter) {
    if (orc_Data.c_Signals[u32_Counter].c_OscSignal.e_MultiplexerType ==
        C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) {
      q_Return = true;
      break;
    }
...
/*! \brief   Sorts the signals in order of the tree
  ...
  Example:
   ...
   \param[in,out]   orc_Signals   Vector with signals to sort (input sorted by
  start bit)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamMetTreeModel::mh_SortMultiplexedSignals(
  ...
  std::sort(orc_Signals.begin(), orc_Signals.end(),
            C_LoggerDataComparatorForMultiplexer());

  // Search all multiplexed signal for each the multiplexer. Iterate over the
  // multiplexer signals
  while (u32_MultiplexerCounter < orc_Signals.size()) {
    const C_OscComMessageLoggerDataSignal &rc_MultiplexerSig =
        orc_Signals[u32_MultiplexerCounter];
    if (rc_MultiplexerSig.c_OscSignal.e_MultiplexerType ==
        C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) {
      bool ok_2986 = false;
      const int32_t s32_MultiplexerValueTmp = rc_MultiplexerSig.c_RawValueDec.toInt(&ok_2986);
      const int32_t s32_MultiplexerValue = ok_2986 ? s32_MultiplexerValueTmp : -1;

      if (s32_MultiplexerValue >= 0) {
        // Compare the multiplexer value of this multiplexer signal with all
        // multiplexed signals
        for (u32_SignalCounter = u32_MultiplexerCounter + 1U;
             u32_SignalCounter < orc_Signals.size(); ++u32_SignalCounter) {
          if ((orc_Signals[u32_SignalCounter].c_OscSignal.e_MultiplexerType ==
            ...
            const C_OscComMessageLoggerDataSignal c_SigCopy =
                orc_Signals[u32_SignalCounter];
            ...
            orc_Signals.insert(orc_Signals.begin() + u32_MultiplexerCounter,
                               c_SigCopy);
          }
        }
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamMetTreeModel::m_IsStatusValid(
    const C_CamMetTreeLoggerData &orc_CurrentMessage) const {
  bool q_StatusValid = true;

  // If status is "invalid" (Invalid counter or CRC for ECeS message)
  const QString c_Status(orc_CurrentMessage.c_Status);

  if ((c_Status.contains("invalid", Qt::CaseInsensitive)) ||
      (c_Status.contains("incorrect", Qt::CaseInsensitive))) {
    q_StatusValid = false;
  }
...
/*! \brief   Custom operator for sorting C_OscComMessageLoggerDataSignal
   ...
   All multiplexer signals will be sorted to the beginning (less than) in order
   of the set multiplexer value
...
bool C_CamMetTreeModel::C_LoggerDataComparatorForMultiplexer::operator()(
    ...
    const stw::opensyde_core::C_OscComMessageLoggerDataSignal &orc_Data2)
    const {
  bool q_Return;

  if ((orc_Data1.c_OscSignal.e_MultiplexerType ==
       C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) &&
      (orc_Data2.c_OscSignal.e_MultiplexerType ==
       C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL)) {
    // Both are multiplexer signals, compare the values
    bool ok_3073 = false;
    const int32_t s32_Value1Tmp = orc_Data1.c_RawValueDec.toInt(&ok_3073);
    const int32_t s32_Value1 = ok_3073 ? s32_Value1Tmp : -1;
    bool ok_3074 = false;
    const int32_t s32_Value2Tmp = orc_Data2.c_RawValueDec.toInt(&ok_3074);
    const int32_t s32_Value2 = ok_3074 ? s32_Value2Tmp : -1;

    q_Return = s32_Value1 < s32_Value2;
  } else if (orc_Data1.c_OscSignal.e_MultiplexerType ==
             C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) {
    // orc_Data2 is not a multiplexer signal
    q_Return = true;
  } else if (orc_Data2.c_OscSignal.e_MultiplexerType ==
             C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL) {
