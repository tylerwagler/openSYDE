...
C_CamDbOsy::C_CamDbOsy()
    : mq_Active(true), mq_FoundAll(false),
...
void C_CamDbOsy::FindAllMessages(void) {
  if (this->mq_FoundAll == false) {
    ...
    if (c_NodeIndexes.size() == c_InterfaceIndexes.size()) {
      ...
      for (uint32_t u32_ItFoundItem = 0UL;
           u32_ItFoundItem < c_NodeIndexes.size(); ++u32_ItFoundItem) {
        if (c_NodeIndexes[u32_ItFoundItem] <
            this->mc_Data.c_OsySysDef.c_Nodes.size()) {
          // Found node
          const C_OscNode &rc_Node =
              this->mc_Data.c_OsySysDef.c_Nodes[c_NodeIndexes[u32_ItFoundItem]];
          // Id
          c_CurListId.c_Id.u32_NodeIndex = c_NodeIndexes[u32_ItFoundItem];
          c_CurId.c_Id.u32_NodeIndex = c_NodeIndexes[u32_ItFoundItem];
          // All protocols
          for (uint8_t u8_ItProt = 0U;
               u8_ItProt < rc_Node.c_ComProtocols.size(); ++u8_ItProt) {
            const C_OscCanProtocol &rc_Protocol =
                rc_Node.c_ComProtocols[u8_ItProt];
            // Id
            ...
            if ((rc_Protocol.u32_DataPoolIndex < rc_Node.c_DataPools.size()) &&
                (c_InterfaceIndexes[u32_ItFoundItem] <
                 rc_Protocol.c_ComMessages.size())) {
              const C_OscNodeDataPool &rc_Datapool =
                  rc_Node.c_DataPools[rc_Protocol.u32_DataPoolIndex];
              // Found container
              const C_OscCanMessageContainer &rc_Container =
                  rc_Protocol
                      .c_ComMessages[c_InterfaceIndexes[u32_ItFoundItem]];
              // Id
              c_CurId.c_Id.u32_InterfaceIndex =
                  c_InterfaceIndexes[u32_ItFoundItem];
              if ((C_OscCanProtocol::h_GetComListIndex(
                       rc_Datapool, c_InterfaceIndexes[u32_ItFoundItem], false,
                       c_CurListId.c_Id.u32_ListIndex) == C_NO_ERR) &&
                  (c_CurListId.c_Id.u32_ListIndex <
                   rc_Datapool.c_Lists.size())) {
                // Each Rx message
                for (uint32_t u32_ItMsg = 0UL;
                     u32_ItMsg < rc_Container.c_RxMessages.size();
                     ++u32_ItMsg) {
                  const C_OscCanMessage &rc_Message =
                      rc_Container.c_RxMessages[u32_ItMsg];
                  // Use first occurance
                  if (!this->mc_FoundMessagesId.contains(rc_Message.c_Name)) {
                    const uint32_t u32_Hash =
                        C_CamGenSigUtil::h_CalcMessageHash(
                            rc_Message,
                            rc_Datapool
                                .c_Lists[c_CurListId.c_Id.u32_ListIndex]);
                    // Id
                    ...
                    this->mc_FoundMessagesId.insert(rc_Message.c_Name, c_CurId);
                    this->mc_FoundMessagesListId.insert(rc_Message.c_Name,
                                                        c_CurListId);
                  }
                }
              }
              if ((C_OscCanProtocol::h_GetComListIndex(
                       rc_Datapool, c_InterfaceIndexes[u32_ItFoundItem], true,
                       c_CurListId.c_Id.u32_ListIndex) == C_NO_ERR) &&
                  (c_CurListId.c_Id.u32_ListIndex <
                   rc_Datapool.c_Lists.size())) {
                // Each Tx message
                for (uint32_t u32_ItMsg = 0UL;
                     u32_ItMsg < rc_Container.c_TxMessages.size();
                     ++u32_ItMsg) {
                  const C_OscCanMessage &rc_Message =
                      rc_Container.c_TxMessages[u32_ItMsg];
                  // Use first occurance
                  if (!this->mc_FoundMessagesId.contains(rc_Message.c_Name)) {
                    const uint32_t u32_Hash =
                        C_CamGenSigUtil::h_CalcMessageHash(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamDbOsy::FindMessageById(const uint32_t ou32_Id,
  ...
  if (c_NodeIndexes.size() == c_InterfaceIndexes.size()) {
    // All found nodes & interfaces
    for (uint32_t u32_ItFoundItem = 0UL;
         (u32_ItFoundItem < c_NodeIndexes.size()) && (s32_Retval == C_NOACT);
         ++u32_ItFoundItem) {
      if (c_NodeIndexes[u32_ItFoundItem] <
          this->mc_Data.c_OsySysDef.c_Nodes.size()) {
        // Found node
        ...
        // All protocols
        for (uint8_t u8_ItProt = 0U;
             (u8_ItProt < rc_Node.c_ComProtocols.size()) &&
             (s32_Retval == C_NOACT);
             ++u8_ItProt) {
          const C_OscCanProtocol &rc_Protocol =
              rc_Node.c_ComProtocols[u8_ItProt];
          if (c_InterfaceIndexes[u32_ItFoundItem] <
              rc_Protocol.c_ComMessages.size()) {
            // Found container
            const C_OscCanMessageContainer &rc_Container =
            ...
            for (uint32_t u32_ItMsg = 0UL;
              ...
              if ((rc_Message.u32_CanId == ou32_Id) &&
                  (rc_Message.q_IsExtended == oq_IsExtended)) {
                // Found match
                orc_Message = rc_Message.c_Name;
                s32_Retval = C_NO_ERR;
              }
            }
            // Each Tx message
        ...
        if (c_NodeIndexes[u32_ItFoundItem] <
          ...
          for (uint8_t u8_ItProt = 0U;
            ...
            if ((rc_Protocol.u32_DataPoolIndex < rc_Node.c_DataPools.size()) &&
              ...
              // Each Rx message
              for (uint32_t u32_ItMsg = 0UL;
                   u32_ItMsg < rc_Container.c_RxMessages.size(); ++u32_ItMsg) {
                const C_OscCanMessage &rc_Message =
                    rc_Container.c_RxMessages[u32_ItMsg];
                if (orc_Message.compare(rc_Message.c_Name) == 0) {
                  // Found match
                  // Id
                  c_CurId.c_Id.q_MessageIsTx = false;
                  c_CurId.c_Id.u32_MessageIndex = u32_ItMsg;
                  if ((C_OscCanProtocol::h_GetComListIndex(
                           rc_Datapool, c_InterfaceIndexes[u32_ItFoundItem],
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamDbOsy::GetActive(void) const { return this->mq_Active; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
const QMap<QString, C_CamDbOsyMessageId> &
C_CamDbOsy::GetFoundMessages(void) const {
...
/*! \brief   Get OSC CAN message
   ...
   Requirement: this function can only return a valid index if the function
   FindMessage was at least called once
...
const stw::opensyde_core::C_OscCanMessage *C_CamDbOsy::GetOscMessage(
  ...
  if (this->mq_Active) {
    const QMap<QString, C_CamDbOsyMessageId>::const_iterator c_It =
        this->mc_FoundMessagesId.find(orc_Message);

    if (c_It != this->mc_FoundMessagesId.end()) {
      if ((oq_UseHash == false) || (ou32_Hash == c_It->u32_Hash)) {
        if (c_It->c_Id.u32_NodeIndex <
            this->mc_Data.c_OsySysDef.c_Nodes.size()) {
          const C_OscNode &rc_Node =
              this->mc_Data.c_OsySysDef.c_Nodes[c_It->c_Id.u32_NodeIndex];
...
C_CamDbOsy::GetOscList(const QString &orc_Message, const bool oq_UseHash,
  ...
  // Don't allow access to inactive file
  if (this->mq_Active) {
    const QMap<QString, C_CamDbOsyListId>::const_iterator c_It =
        this->mc_FoundMessagesListId.find(orc_Message);
      ...
      if ((oq_UseHash == false) || (ou32_Hash == c_It->u32_Hash)) {
        if (c_It->c_Id.u32_NodeIndex <
          ...
          if (c_It->c_Id.u32_DataPoolIndex < rc_Node.c_DataPools.size()) {
            const C_OscNodeDataPool &rc_DataPool =
                rc_Node.c_DataPools[c_It->c_Id.u32_DataPoolIndex];
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_CamDbOsy::CheckHashForMessage(const QString &orc_Message,
                                     const uint32_t ou32_Hash) const {
  bool q_Retval = false;
