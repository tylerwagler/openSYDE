...
void C_CamProClipBoardHelper::mh_GetNewParser(C_OscXmlParser &orc_XmlParser) {
  orc_XmlParser.CreateAndSelectNodeChild(
      "opensyde-can-monitor-configuration-clip-board");
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_CamProClipBoardHelper::mh_StoreParserInClipBoard(
    const C_OscXmlParser &orc_XmlParser) {
  QString c_String;
  orc_XmlParser.SaveToString(c_String);
  C_CamProClipBoardHelper::mh_SetClipBoard(c_String);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CamProClipBoardHelper::mh_GetParserFromClipBoard(
    C_OscXmlParser &orc_XmlParser) {
  int32_t s32_Retval = C_CONFIG;
  const QString c_Content = C_UtiClipBoardHelper::mh_GetClipBoard();

  if (orc_XmlParser.LoadFromString(c_Content.toStdString()) ==
      C_NO_ERR) {
    if (orc_XmlParser.SelectRoot() ==
        "opensyde-can-monitor-configuration-clip-board") {
      s32_Retval = C_NO_ERR;
    }
  }
