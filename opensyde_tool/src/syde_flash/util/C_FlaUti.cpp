...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check if path contains variables (e.g. %{CAN_MONITOR_PROJECT}) and
   resolve them.
...
QString C_FlaUti::h_ResolvePlaceholderVariables(const QString &orc_Path) {
  ...
  if (c_Return.contains("%") == true) {
    ...
    if (c_Return.contains(mc_PATH_VARIABLE_SYDE_FLASH_BIN) == true) {
      c_Return.replace(mc_PATH_VARIABLE_SYDE_FLASH_BIN,
                       C_Uti::h_GetExePath() + '/');
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Resolve variables and interpret relative paths as relative to
   executable

   Resolve all path variables (e.g. %{CAN_MONITOR_PROJ}). If the resulting path
   is relative it is meant as relative to CANMonitor.exe, so we make it absolute
   by concatenating with executable path.

   \param[in]  orc_Path    path that probably contains path variables and if
   relative is relative to executable
...
QString C_FlaUti::h_GetResolvedAbsolutePathFromExe(const QString &orc_Path) {
  ...
  // first resolve, then make absolute (resolving might change from relative to
  // absolute!)
