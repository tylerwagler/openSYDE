...
/*!
   ...
   \copyright   Copyright 2026 Sensor-Technik Wiedemann GmbH. All rights
   reserved.
*/
//----------------------------------------------------------------------------------------------------------------------
...
C_FlaBitrateWidget::C_FlaBitrateWidget(QWidget *const opc_Parent)
    : C_OgeWiOnlyBackground(opc_Parent), mpc_Ui(new Ui::C_FlaBitrateWidget) {
  ...
  connect(
      this->mpc_Ui->pc_ComboBoxBitrate,
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_FlaBitrateWidget::~C_FlaBitrateWidget() { delete this->mpc_Ui; }
...
void C_FlaBitrateWidget::LoadUserSettings(void) {
  ...
  const bool q_Expanded =
      C_UsHandler::h_GetInstance()->GetWiDllConfigExpanded();
...
int32_t C_FlaBitrateWidget::GetSelectedBitrate(void) const {
  ...
  static const int32_t has32_Bitrates[] = {10,  20,  50,  100, 125,
                                           250, 500, 800, 1000};
  const int32_t s32_Index = this->mpc_Ui->pc_ComboBoxBitrate->currentIndex();

  if ((s32_Index >= 0) &&
      (s32_Index <
       static_cast<int32_t>(sizeof(has32_Bitrates) / sizeof(int32_t)))) {
