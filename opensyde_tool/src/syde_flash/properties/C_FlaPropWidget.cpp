...
C_FlaPropWidget::C_FlaPropWidget(QWidget *const opc_Parent)
    : QWidget(opc_Parent), mpc_Ui(new Ui::C_FlaPropWidget) {
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_FlaPropWidget::~C_FlaPropWidget() { delete this->mpc_Ui; }
...
void C_FlaPropWidget::LoadUserSettings() const {
  const uint8_t u8_NodeId =
      static_cast<uint8_t>(C_UsHandler::h_GetInstance()->GetPropNodeId());
...
void C_FlaPropWidget::m_InitStaticNames(void) const {
  ...
  this->mpc_Ui->pc_WiTitle->SetIconType(
      C_CamOgeWiSectionHeader::E_ButtonType::eNOBUTTON);
  ...
  this->mpc_Ui->pc_LabelNodeId->SetToolTipInformation(
      "Node ID", "Current node ID of the node to be configured/updated.");

  this->mpc_Ui->pc_LabelBitrate->SetToolTipInformation(
      "Bitrate", "Current bitrate of the node to be configured/updated.");
