...
C_FlaMainWindow::C_FlaMainWindow(QWidget *const opc_Parent)
  ...
  stw::opensyde_gui_logic::C_HeHandler::h_GetInstance().SetHelpFileRelPath(
      "/../../tool/Help/openSYDE PC Tool.chm");
  ...
  connect(this->mpc_Ui->pc_SplitterSettings, &C_CamOgeSpi::splitterMoved, this,
          &C_FlaMainWindow::m_OnSettingsSplitterMoved);
  connect(this->mpc_Ui->pc_SplitterSettings, &C_CamOgeSpi::SigDoubleClick, this,
          &C_FlaMainWindow::m_OnSettingsSplitterHandleDoubleClick);
  // Title bar
  connect(this->mpc_Ui->pc_TitleBarWidget, &C_FlaTitleBarWidget::SigUpdateNode,
          this, &C_FlaMainWindow::m_OnUpdateNode);
  connect(this->mpc_Ui->pc_TitleBarWidget, &C_FlaTitleBarWidget::SigSearchNode,
          this, &C_FlaMainWindow::m_OnSearchNode);
  connect(this->mpc_Ui->pc_TitleBarWidget,
          &C_FlaTitleBarWidget::SigConfigureNode, this,
          &C_FlaMainWindow::m_OnConfigureNode);

  // Timers
  connect(&this->mc_TimerUpdate, &QTimer::timeout, this,
          &C_FlaMainWindow::m_TimerUpdate);
  this->mc_TimerUpdate.setInterval(25);

  connect(&this->mc_SecTimer, &QTimer::timeout, this,
          &C_FlaMainWindow::m_CountTime);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaMainWindow::m_OnSettingsSplitterMoved(const int32_t &ors32_Pos,
  ...
  if (ors32_Index == 1) {
    if (rc_Sizes.size() > 1) {
      ...
      if ((rc_Sizes.at(1) <= C_FlaMainWindow::mhs32_SETTINGS_SPLITTER_MAX) &&
          (rc_Sizes.at(1) > 35)) {
...
void C_FlaMainWindow::m_OnUpdateNode() {
  ...
  if (this->mpc_Ui->pc_UpdateWidget->GetHexFilePaths().isEmpty() == true) {
    ...
    c_Message.SetDescription("No HEX file(s) found for flashing.");
    c_Message.Execute();
  ...
  if (q_ProgressError || q_ProgressUpdate) {
    uint8_t u8_ProgressState =
        static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_DEFAULT);
    if (q_ProgressError) {
      u8_ProgressState =
          static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_ERROR);
    }
...
void C_FlaMainWindow::m_OnSearchNode() {
  ...
  C_FlaSenSearchNodePopup *const pc_Dialog =
      new C_FlaSenSearchNodePopup(*c_New);
  ...
  connect(pc_Dialog, &C_FlaSenSearchNodePopup::SigReportProgress, this,
          &C_FlaMainWindow::m_ShowProgress);

  if (pc_Dialog->StartSearch(
          this->mpc_Ui->pc_SettingsWidget->GetCanDllPath(),
...
void C_FlaMainWindow::m_OnConfigureNode() {
  ...
  const uint32_t u32_Bitrate =
      this->mpc_Ui->pc_GeneralPropertiesWidget->GetBitrate();
  const QString &rc_CanDllPath =
      this->mpc_Ui->pc_SettingsWidget->GetCanDllPath();

  const QPointer<C_OgePopUpDialog> c_New = new C_OgePopUpDialog(this, this);

  C_FlaConNodeConfigPopup *const pc_Dialog = new C_FlaConNodeConfigPopup(
      *c_New, u8_NodeId, u32_Bitrate,
      this->mpc_Ui->pc_SettingsWidget->GetFlashloaderResetWaitTime(),
      rc_CanDllPath);
  ...
  connect(pc_Dialog, &C_FlaConNodeConfigPopup::SigReportProgress, this,
          &C_FlaMainWindow::m_ShowProgress);
...
/*! \brief  Show progress bar text
   ...
   \param[in]  u8_ProgressState     State of the progress (hu32_STATE_DEFAULT -
   black, hu32_STATE_FINISHED - green, hu32_STATE_ERROR - red)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaMainWindow::m_SetHeadingText(
    const QString &orc_Text, const uint8_t &oru8_ProgressState) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t stw::opensyde_gui::C_FlaMainWindow::m_CalculateFileBytesFlashed(
    const uint8_t &oru8_ProgressInPercentage) {
  const uint8_t u8_MAX_PERCENTAGE = 100;

  const uint32_t u32_FlashableBytesPerFile =
      this->mpc_Ui->pc_UpdateWidget->GetHexFileSize(mu32_FlashedFilesCounter);
  const float32_t f32_PercentageDecimal =
      static_cast<float32_t>(oru8_ProgressInPercentage) /
      static_cast<float32_t>(u8_MAX_PERCENTAGE);
  const float32_t f32_FlashedBytes =
      f32_PercentageDecimal * static_cast<float32_t>(u32_FlashableBytesPerFile);
...
/*! \brief  Error has been detected
   ...
   Reset progress bar, taskbar progress, necessary values and show a popup
   window if wanted
...
bool C_FlaMainWindow::m_ErrorDetected(const bool &orq_ShowErrorMessage) {
  ...
  const uint8_t u8_STATE =
      static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_ERROR);
  const QString c_Text = "Update failed!";
  const uint8_t u8_PROGRESS_STATE =
      static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_ERROR);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaMainWindow::m_UpdateBottomBar(
    const uint8_t &oru8_ProgressInPercentage) {
  const uint8_t u8_MAX_PERCENTAGE = 100;

  if ((oru8_ProgressInPercentage == 0) && mq_NewFile) {
    const uint8_t u8_STATE_PROGRESS =
        static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_IN_PROGRESS);
  ...
  const uint32_t u32_CalculateFileBytesFlashed =
      m_CalculateFileBytesFlashed(oru8_ProgressInPercentage);
  if ((u32_CalculateFileBytesFlashed > 0) &&
      (u32_CalculateFileBytesFlashed > mu32_FlashedBytesTmp)) {
    uint8_t u8_TotalProgressInPercentage;

    const uint32_t u32_Differences =
        u32_CalculateFileBytesFlashed - mu32_FlashedBytesTmp;
    ...
    this->mpc_Ui->pc_UpdateWidget->UpdateProgressBar(
        u8_TotalProgressInPercentage);
  ...
  if (oru8_ProgressInPercentage == u8_MAX_PERCENTAGE) {
    const uint8_t u8_STATE_FINISHED =
        static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_FINISHED);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaMainWindow::m_UpdateFileIcon(
    const uint32_t &oru32_FileIndex, const uint8_t &oru8_ProgressState) const {
  this->mpc_Ui->pc_UpdateWidget->SetStatusIcon(oru32_FileIndex,
                                               oru8_ProgressState);
}
...
uint8_t C_FlaMainWindow::m_CalcualteTotalProgressInPercentage(void) const {
  const uint8_t u8_MINIMUM_PROGRESS_PERCENTAGE =
      2; // defined in C_OscBuSequences

  const uint8_t u8_MAX_PERCENTAGE = 100;
  const uint32_t u32_TotalNumberOfFlashableSize =
      this->mpc_Ui->pc_UpdateWidget->GetHexFileSize();
  uint8_t u8_ProgressInPercentage = static_cast<uint8_t>(
      (u8_MAX_PERCENTAGE * mu64_FlashedBytes) / u32_TotalNumberOfFlashableSize);

  if (u8_ProgressInPercentage <
      (u8_MAX_PERCENTAGE - u8_MINIMUM_PROGRESS_PERCENTAGE)) {
...
int32_t C_FlaMainWindow::m_InitUpdateSequence(void) {
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        osc_write_log_error("Initialization",
                            "Could not initialize the CAN interface!");
      }
    } else {
      const QString c_Bitness = QString::number(8 * sizeof(size_t));
      osc_write_log_error("Initialization",
                          "Could not load the CAN DLL! Make sure to use a " +
                              c_Bitness + "-bit DLL.");
    }
  }
  ...
  if (this->mpc_UpSequences == nullptr) {
    ...
    // connect signals
    connect(mpc_UpSequences, &C_FlaUpSequences::SigReportProgress, this,
            &C_FlaMainWindow::m_ShowProgress);
    connect(mpc_UpSequences, &C_FlaUpSequences::SigReportFlashingProgress, this,
            &C_FlaMainWindow::m_UpdateBottomBar);
    connect(mpc_UpSequences,
            &C_FlaUpSequences::SigReportFlashloaderInformationText, this,
            &C_FlaMainWindow::m_ShowProgress);
    connect(mpc_UpSequences, &C_FlaUpSequences::SigReportDeviceName, this,
            &C_FlaMainWindow::m_CheckDeviceName);
  }
  ...
  if (s32_Return == C_NO_ERR) {
    // initialize sequence
    s32_Return = this->mpc_UpSequences->Init(
...
void C_FlaMainWindow::m_CleanupUpdateSequence(void) {
  if (this->mpc_UpSequences != nullptr) {
    ...
    disconnect(mpc_UpSequences,
               &C_FlaUpSequences::SigReportFlashloaderInformationText, this,
               &C_FlaMainWindow::m_ShowProgress);
    disconnect(mpc_UpSequences, &C_FlaUpSequences::SigReportDeviceName, this,
               &C_FlaMainWindow::m_CheckDeviceName);
  }
...
void C_FlaMainWindow::m_TimerUpdate(void) {
  ...
  if (s32_ThreadResult == C_NO_ERR) {
    ...
    if (s32_SequenceResult != C_NO_ERR) {
      ...
      // note: me_UpdateStep is the current step that is already finished -> so
      // we always do the "next step" stuff
      ...
      case C_FlaUpSequences::eREADDEVICEINFO:
        // nothing to do here, we go on after device name check that is
        // triggered by signal
        break;
      case C_FlaUpSequences::eUPDATENODE:
        // next step: update node with next hex file if available (and if device
        // name check is ok)
        if (mq_ContinueUpdate == true) {
          const QStringList &rc_HexFiles =
              this->mpc_Ui->pc_UpdateWidget->GetHexFilePaths();
          if (ms32_NextHexFile < rc_HexFiles.size()) {
            s32_ThreadResult = this->mpc_UpSequences->StartUpdateNode(
                rc_HexFiles.at(ms32_NextHexFile),
            ...
            const uint8_t u8_PROGRESS_STATE = static_cast<uint8_t>(
                C_FlaUpListItemWidget::hu32_STATE_FINISHED);
            ...
            c_Message.SetDescription(
                "Successfully updated node with ID " +
                QString::number(
                    this->mpc_Ui->pc_GeneralPropertiesWidget->GetNodeId()) +
                ".");
            c_Message.SetDetails(
            ...
            if (c_Message.Execute() == C_OgeWiCustomMessage::eOK) {
              const uint8_t u8_STATE = static_cast<uint8_t>(
                  C_FlaUpListItemWidget::hu32_STATE_FINISHED);
              const uint32_t u32_FlashedFileIndex =
                  mu32_FlashedFilesCounter - 1;
...
void C_FlaMainWindow::m_CheckDeviceName(const QString &orc_ReadDeviceName) {
  ...
  QStringList c_DeviceNames =
      this->mpc_Ui->pc_UpdateWidget->GetHexFileDeviceNames();
  ...
  for (int32_t s32_Index = 0; s32_Index < c_DeviceNames.size(); s32_Index++) {
    ...
    // Compare device name
    if (orc_ReadDeviceName.compare(c_DeviceNames[s32_Index],
  ...
  if (q_DeviceNameCheck == false) {
    ...
    const QString c_Details =
        static_cast<QString>("Device: %1 \nHEX file(s): %2")
            .arg(orc_ReadDeviceName)
            .arg(c_DeviceNamesList);
    ...
    c_Message.SetDescription(
        ...
        "device name reported. Do you want to continue with the update "
        "anyway?");
...
void C_FlaMainWindow::m_CountTime(void) const {
  ...
  const uint64_t u64_ElapsedSeconds =
      (u64_ELAPSED_MS / u64_MS_TO_SEC) % u64_MODULOR_OPERATION_VALUE;
  const uint64_t u64_ElapsedMin =
      (u64_ELAPSED_MS / u64_MS_TO_MIN) % u64_MODULOR_OPERATION_VALUE;
...
bool stw::opensyde_gui::C_FlaMainWindow::m_ShowErrorMessage(void) {
  ...
  c_Message.SetDescription(
      "Failure when updating node with ID " +
