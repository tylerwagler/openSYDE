...
C_FlaTitleBarWidget::C_FlaTitleBarWidget(QWidget *const opc_Parent)
    : C_OgeWiOnlyBackground(opc_Parent), mpc_Ui(new Ui::C_FlaTitleBarWidget) {
  QPixmap c_ImgLogo;

  this->mpc_Ui->setupUi(this);
  this->SetBackgroundColor(10);

  C_OgeWiUtil::h_ApplyStylesheetProperty(this->mpc_Ui->pc_Frame,
  ...
  c_ImgLogo =
      c_ImgLogo.scaled((c_ImgLogo.width() / 18), (c_ImgLogo.height() / 18),
  ...
  this->mpc_Ui->pc_PushButtonUpdate->SetSvg("://images/IconUpdate.svg",
                                            "://images/IconUpdateDisabled.svg");
  this->mpc_Ui->pc_PushButtonSearch->SetSvg("://images/IconSearch.svg",
                                            "://images/IconSearchDisable.svg");
  this->mpc_Ui->pc_PushButtonConfigure->SetSvg(
      "://images/IconConfigure.svg", "://images/IconConfigureDisabled.svg");
  ...
  connect(this->mpc_Ui->pc_PushButtonUpdate, &QPushButton::clicked, this,
          &C_FlaTitleBarWidget::SigUpdateNode);
  connect(this->mpc_Ui->pc_PushButtonSearch, &QPushButton::clicked, this,
          &C_FlaTitleBarWidget::SigSearchNode);
  connect(this->mpc_Ui->pc_PushButtonConfigure, &QPushButton::clicked, this,
          &C_FlaTitleBarWidget::SigConfigureNode);
  connect(this->mpc_Ui->pc_PushButtonAbout, &QPushButton::clicked, this,
          &C_FlaTitleBarWidget::m_ShowAbout);
  connect(this->mpc_Ui->pc_PushButtonHelp, &QPushButton::clicked, this,
          &C_FlaTitleBarWidget::m_TriggerHelp);
}
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_FlaTitleBarWidget::~C_FlaTitleBarWidget() { delete this->mpc_Ui; }
...
void C_FlaTitleBarWidget::InitStaticNames(void) const {
  ...
  this->mpc_Ui->pc_PushButtonAbout->setText(
      "About"); // no icon therefore no icon-only mode
  this->m_SetButtonsText(false);

  // tooltips
  this->mpc_Ui->pc_PushButtonHelp->SetToolTipInformation(
      "Help", "Open user manual for help.");
  this->mpc_Ui->pc_PushButtonAbout->SetToolTipInformation(
      "About", "Show information about SYDEflash.");

  this->mpc_Ui->pc_PushButtonUpdate->SetToolTipInformation(
      "Update Node", "Flash selected HEX files on connected node.");
  this->mpc_Ui->pc_PushButtonSearch->SetToolTipInformation(
      "Search Node", "Scan network for connected openSYDE nodes.");
  this->mpc_Ui->pc_PushButtonConfigure->SetToolTipInformation(
      "Configure Node", "Apply new node ID and bitrate "
                        "to connected node.");
}
...
/*! \brief  Set buttons text or remove them if only icon should be visible (in
   small width mode)
...
void C_FlaTitleBarWidget::m_SetButtonsText(const bool oq_IconOnly) const {
  ...
  if (oq_IconOnly == true) {
    ...
    this->mpc_Ui->pc_PushButtonConfigure->setMaximumWidth(
        u32_SMALL_MAXIMUM_SIZE);
  } else {
    ...
    // we cannot change Qt constant but it is still better than using the hard
    // coded magic number 16777215
    this->mpc_Ui->pc_PushButtonUpdate->setMaximumWidth(
        QWIDGETSIZE_MAX); // lint !e893 !e9130 !e9136
    this->mpc_Ui->pc_PushButtonSearch->setMaximumWidth(
        QWIDGETSIZE_MAX); // lint !e893 !e9130 !e9136
    this->mpc_Ui->pc_PushButtonConfigure->setMaximumWidth(
        QWIDGETSIZE_MAX); // lint !e893 !e9130 !e9136
  }
