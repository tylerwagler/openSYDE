...
/*!
   ...
   certain Bitrate. The dialog holds logic that monitors the threads for the
   necessary sequences (enter flashloader, configure node, reset system) and
   starts/stops them in order. The report output of all sequences is forwarded
   to the MainWindow where the Report Widget is filled with information.
...
/*! \brief   Default constructor
   ...
   \param[in]      ou8_NodeId                      Node ID to be set in this
   dialog as default
   \param[in]      ou32_Bitrate                    Bitrate to be set in this
   dialog as default
   \param[in]      ou32_FlashloaderResetWaitTime   Wait time for flashloader
   reset from settings widget
   \param[in]      orc_CanDllPath                  CAN DLL file path
*/
//----------------------------------------------------------------------------------------------------------------------
C_FlaConNodeConfigPopup::C_FlaConNodeConfigPopup(
  ...
  const QString c_CbxText =
      "CAN " +
      QString::number(C_UsHandler::h_GetInstance()->GetNodeCfgInterfaceIndex() +
                      1);
  ...
  this->mpc_Ui->pc_CbxBitrate->setCurrentText(
      QString::number(mu32_CurrentBitrate) + " kbit/s");
  ...
  connect(this->mpc_Ui->pc_PushButtonCancel, &QPushButton::clicked, this,
          &C_FlaConNodeConfigPopup::m_OnCancel);

  // Timer
  connect(&this->mc_Timer, &QTimer::timeout, this,
          &C_FlaConNodeConfigPopup::m_MonitorSequence);
...
uint8_t C_FlaConNodeConfigPopup::GetNodeId() const {
  const uint8_t u8_Retval =
      static_cast<uint8_t>(this->mpc_Ui->pc_SpxNodeId->value());
...
uint32_t C_FlaConNodeConfigPopup::GetBitrate() const {
  ...
  const uint32_t u32_Retval =
      static_cast<uint32_t>(c_Text.remove(" kbit/s").toInt());
...
void C_FlaConNodeConfigPopup::m_InitStaticNames(void) const {
  ...
  this->mpc_Ui->pc_LabelInterface->SetToolTipInformation(
      "Interface",
      "Interface the configuration shall be applied for. "
      "\nThis is not"
      " necessarily the interface the Node is currently connected to.");
  this->mpc_Ui->pc_LabelNodeId->SetToolTipInformation(
      "New Node ID", "Node ID which shall be applied to the selected interface "
  ...
  // fill interface combobox. We just want to show 6 CAN interfaces, since there
  // probably no ECU with more
  for (uint8_t u8_IfCount = 1; u8_IfCount < 7; ++u8_IfCount) {
    this->mpc_Ui->pc_CbxInterface->addItem("CAN " +
                                           QString::number(u8_IfCount));
  }
}
...
int32_t C_FlaConNodeConfigPopup::m_InitDcSequence(void) {
  ...
  s32_Return = this->mpc_DcSequences->InitDcSequences(
      this->mc_CanDllPath, this->mu32_CurrentBitrate);
...
void C_FlaConNodeConfigPopup::m_OkClicked(void) {
  ...
  if (s32_Result == C_NO_ERR) {
    ...
    this->mpc_DcSequences->StartScanEnterFlashloader(
        mu32_FlashloaderResetWaitTime);
    this->mc_Timer.start();
  } else {
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_FlaConNodeConfigPopup::m_OnCancel() { this->mrc_ParentDialog.reject(); }

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Monitors the sequence that is currently running (triggered by
   timeout of mc_Timer).
  ...
  if (s32_ThreadResult == C_NO_ERR) {
    ...
    case C_FlaSenDcBasicSequences::eSCAN_GET_INFO:
      ...
      if (ms32_SequenceResult == C_NO_ERR) {
        const uint8_t u8_InterfaceIndex =
            static_cast<uint8_t>(this->mpc_Ui->pc_CbxInterface->currentIndex());

        // start the actual configuration sequence
        this->mpc_DcSequences->StartConfigureDevice(
            mu8_CurrentNodeId, this->GetNodeId(), this->GetBitrate(),
            u8_InterfaceIndex);
        this->mc_Timer.start();
      } else {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Gives user feedback when Node Configuration is finished (fail or
   success) and initiates a system reset either way

   \param[in]   os32_SequenceResult   Error code from Node Configuration
   sequence
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaConNodeConfigPopup::m_FinishConfig(
    const int32_t os32_SequenceResult) {
  ...
  if (os32_SequenceResult != C_NO_ERR) {
    ...
    if (c_Message.exec() ==
        static_cast<int32_t>(C_OgeWiCustomMessage::Accepted)) {
    ...
    c_Message.SetDescription(
        "Successfully configured node interface \"" +
        this->mpc_Ui->pc_CbxInterface->currentText() +
        "\":\nNew Node ID = " + QString::number(u8_NodeId) +
        "\nNew Bitrate = " + QString::number(u32_Bitrate) + " kbit/s");

    C_UsHandler::h_GetInstance()->SetNodeCfgInterfaceIndex(
        this->mpc_Ui->pc_CbxInterface->currentIndex());
...
/*! \brief  Connected to signal in openSYDE Core that sends all the information.
   ...
   Looks how many devices answer with the Node ID about to be configured and
   logs an error if > 1.

   \param[in]       orc_DeviceInfoResult    device information for every found
   node
...
void C_FlaConNodeConfigPopup::m_DeviceInfoRead(
  ...
  //  has Security activated. Configure Node will run into an error that is
  //  handled gracefully and reported to log file
...
/*! \brief  Helper function to en/disable the UI when process is running or not.
   ...
   \param[in]       oq_Enabled   true: UI will be enabled, false: will be
   disabled
*/
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_FlaConNodeConfigPopup::m_GetErrorAsString(const int32_t os32_SequenceResult) {
  QString c_Retval;

  if (os32_SequenceResult == C_RANGE) {
    c_Retval = "More than one node answered with the current Node ID " +
               QString::number(mu8_CurrentNodeId);
  } else if (os32_SequenceResult == C_WARN) {
    c_Retval = "The requested CAN Interface does not exist or is out of range "
               "for this Node.\n";
  } else if (os32_SequenceResult == C_TIMEOUT) {
    c_Retval =
        "No node found for Node ID " + QString::number(mu8_CurrentNodeId);
  } else {
    c_Retval = "Failure when configuring node. Sequence result: " +
               QString::number(os32_SequenceResult);
  }
