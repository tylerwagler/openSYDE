 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_FlaUpListWidget::~C_FlaUpListWidget() { delete this->mpc_ContextMenu; }
//----------------------------------------------------------------------------------------------------------------------
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::AddFileAction(const bool oq_IsActionForExistingFile,
  ...
  if (!oq_IsActionForExistingFile) {
    c_Folder =
        C_UsHandler::h_GetInstance()->GetLastKnownUpdateHexFileLocation();
  ...
  this->m_AddNewHexFile("Select HEX file",
                        static_cast<QString>("Hex file") + " (*.hex)", c_Folder,
                        oq_IsActionForExistingFile, os32_CurrentHexFileIndex,
                        oq_IsFileToReplace);
}
...
void C_FlaUpListWidget::SetLastKnownHexFilePaths() {
  C_UsHandler::h_GetInstance()->SetLastKnownUpdateHexFilePaths(
      this->GetHexFilePaths());
  C_UsHandler::h_GetInstance()->SetHexFilePathsAsRelativeOrAbsolute(
      this->m_GetHexFilePathsAsRelativeOrAbsolute());
}
    ...
    if (pc_CurrentItem != nullptr) {
      ...
      if ((pc_ItemWidget != nullptr) && (pc_ItemWidget->pc_HexFileInfo != nullptr) &&
        ...
        c_DeviceNames.append(
            pc_ItemWidget->pc_HexFileInfo
                ->c_BlockInfo[pc_ItemWidget->GetApplicationBlockIndex()]
                .c_DeviceId);
      }
    }
  }
            ...
            if ((pc_ItemWidget != nullptr) &&
                                         ...
                                         //  files on single file
              {
                C_OgeWiCustomMessage c_MessageBox(
                    this, C_OgeWiCustomMessage::eWARNING);
                ...
                C_OgeWiCustomMessage c_MessageBox(
                    this, C_OgeWiCustomMessage::eQUESTION);
                ...
                if (c_MessageBox.Execute() == C_OgeWiCustomMessage::eYES) {
                  this->m_AddFile(c_LocalFilePath, true, s32_DroppedItemAtIndex,
                                  this->m_AskUserToSaveRelativePath(
                                      c_LocalFilePath, C_Uti::h_GetExePath()));
                }
              } else {
                // Do nothing here
              }
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_FlaUpListWidget::GetHexFileSize(const uint32_t &oru32_HexFileIndex) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::SetStatusIcon(const int32_t &ors32_FileIndex,
...
void C_FlaUpListWidget::ResetStatusIcons(void) {
  const uint8_t u8_DISABLE_ICON =
      static_cast<uint8_t>(C_FlaUpListItemWidget::hu32_STATE_DISABLED);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::m_AddNewFile(const QString &orc_File,
                                     const bool oq_HexFile,
                                     const bool oq_IsActionForExistingFile,
                                     const int32_t os32_CurrentHexFileIndex) {
  if ((oq_HexFile == true) && (!orc_File.isEmpty())) {
    this->m_AddFile(
        orc_File, oq_IsActionForExistingFile, os32_CurrentHexFileIndex,
...
/*! \brief  Adds a new file for the file container
   ...
   \param[in]  os32_CurrentHexFileIndex      Current hex file index
   \param[in]  oq_IsRelativePath             Information whether file path is
   relative or absolute
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::m_AddFile(const QString &orc_File,
                                  const bool oq_IsActionForExistingFile,
                                  const int32_t os32_CurrentHexFileIndex,
  ...
  if (q_HexFile) {
    ...
    if (q_FileAlreadyExists == false) {
      C_FlaUpHexFileInfo *const pc_HexFileInfo =
          new C_FlaUpHexFileInfo(orc_File);
      ...
      pc_FileWidget =
          new C_FlaUpListItemWidget(this, pc_HexFileInfo, oq_IsRelativePath);
      pc_Item->setSizeHint(
          QSize(pc_FileWidget->width(), pc_FileWidget->height()));
      if (!oq_IsActionForExistingFile) {
      ...
      connect(pc_FileWidget, &C_FlaUpListItemWidget::SigDeleteItemWidget, this,
              &C_FlaUpListWidget::m_DeleteItem);

      connect(pc_FileWidget, &C_FlaUpListItemWidget::SigSelectFile, this,
              &C_FlaUpListWidget::m_SelectFile);

      connect(pc_FileWidget, &C_FlaUpListItemWidget::SigDeleteAllItems, this,
              &C_FlaUpListWidget::m_DeleteAllItem);
    ...
    } // lint !e429  //no memory leak of pc_HexFileInfo, pc_FileWidget, pc_Item
      // because of handling in
      //  "this->DeleteItem()"
    else {
      ...
      c_Message.SetDetails(static_cast<QString>("%1").arg(orc_File));
      c_Message.Execute();
    }
  }
}
...
void C_FlaUpListWidget::m_LoadLastKnownHexFilePaths() {
  QStringList c_HexFilePaths =
      C_UsHandler::h_GetInstance()->GetLastKnownUpdateHexFilePaths();
  QStringList c_HexFilePathsAsRelativeOrAbsolute =
      C_UsHandler::h_GetInstance()->GetHexFilePathsAsRelativeOrAbsolute();

  for (int32_t s32_It = 0; s32_It < static_cast<int32_t>(c_HexFilePaths.size());
       s32_It++) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::m_UpdateHexFileWidgetIndex(
    const int32_t os32_CurrentHexFileIndex) const {
...
void C_FlaUpListWidget::m_SetupContextMenu() {
  ...
  this->mpc_ContextMenu->addAction("Remove all Files", this,
                                   &C_FlaUpListWidget::m_DeleteAllItem);
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_FlaUpListWidget::m_AskUserToSaveRelativePath(
    const QString &orc_Path, const QString &orc_AbsoluteReferenceDir) {
  bool c_Return;
  QString c_PathRelative;
  QString c_PathAbsolute;
  C_OgeWiCustomMessage c_Message(this, C_OgeWiCustomMessage::eQUESTION);

  C_Uti::h_IsPathRelativeToDir(orc_Path, orc_AbsoluteReferenceDir,
                               c_PathAbsolute, c_PathRelative);

  c_Message.SetHeading("Relative Path");
  c_Message.SetDescription(
      static_cast<QString>(
          "Do you want to save the selected path (%1) relative or absolute?")
          .arg(c_PathAbsolute));
...
/*! \brief  Adding a new hex file
   ...
   \param[in]      orc_DialogFilter                   Filter configuration for
   file dialog
   \param[in]      oc_Folder                          Folder of hex file
   \param[in]      oq_IsActionForExistingFile         Is Action for existing
   files
   \param[in,out]  os32_CurrentHexFileIndex           Current hex file widget
   index
   \param[in]      oq_IsFileToReplace                 Is file to replace
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::m_AddNewHexFile(const QString &orc_DialogCaption,
  ...
  if (!oq_IsFileToReplace) {
    c_Files = QFileDialog::getOpenFileNames(this, orc_DialogCaption, orc_Folder,
                                            orc_DialogFilter);
  } else {
    const QString c_File = C_OgeWiUtil::h_GetOpenFileName(
        this, orc_DialogCaption, orc_Folder, orc_DialogFilter, "*.hex");
    if (!c_File.isEmpty()) {
  ...
  for (const QString &rc_File : c_Files) {
    ...
    if (!q_HexFile) {
      const QString c_Details =
          static_cast<QString>("File path: %1").arg(rc_File);
      C_OgeWiCustomMessage c_Message(this,
      ...
      // add file
      this->m_AddNewFile(rc_File, q_HexFile, oq_IsActionForExistingFile,
                         os32_CurrentHexFileIndex);
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_FlaUpListWidget::m_LastSuffixOfFileName(const QString &orc_File) const {
  QString c_Result = "";
...
/*! \brief  Delete item on delete button click
   ...
   \param[in,out]      os32_CurrentHexFileIndex           Current hex file
   widget index
   \param[in]          oq_NeedToUpdateListIndex           Need to update list
   indexes
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::m_DeleteItem(const int32_t os32_CurrentHexFileIndex,
  ...
  if (pc_CurrentItem != nullptr) {
    ...
    if ((pc_ItemWidget != nullptr) && (pc_ItemWidget->pc_HexFileInfo != nullptr)) {
      if (oq_NeedToUpdateListIndex) {
        ...
        c_MessageBox.SetCustomMinHeight(180, 180);
        if (c_MessageBox.Execute() == C_OgeWiCustomMessage::eYES) {
          pc_ItemWidget = dynamic_cast<C_FlaUpListItemWidget *>(
...
void C_FlaUpListWidget::m_DeleteAllItem() {
  ...
  c_MessageBox.SetDescription(
      "Do you really want to remove all files from the Hex-Files list?");
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListWidget::m_ReloadCurrentFile(
    const QString &orc_File, C_FlaUpHexFileInfo *const opc_HexFileInfo) {
  bool q_HexFile = false;
