...
C_FlaUpSequences::C_FlaUpSequences(void)
    : QObject(), C_OscBuSequences(), me_Sequence(eNOT_ACTIVE),
      mu32_FlashloaderResetWaitTime(1000), mc_HexFilePath(""),
      mu32_RequestDownloadTimeout(40000), mu32_TransferDataTimeout(1000),
      ms32_Result(C_NOACT) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_FlaUpSequences::StartActivateFlashLoader(
...
/*! \brief  Start thread for node update
   ...
   erase one continuous area in flash
   \param[in]  ou32_TransferDataTimeout      Maximum time in ms it can take to
   write up to 4kB of data to flash
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_FlaUpSequences::StartUpdateNode(const QString &orc_HexFilePath,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpSequences::m_ReportProgressPercentage(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpSequences::m_ReportProgress(const int32_t os32_Result,
...
/*! \brief  Reports information read from openSYDE flashloader of node
   ...
   Here: Report read information to progress log via
   SigReportFlashloaderInformationRead.

   \param[in]  orc_DeviceName    Device name
   \param[in]  orc_Information   Flashloader information
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpSequences::m_ReportFlashloaderInformationRead(
...
void C_FlaUpSequences::m_ThreadFunc(void) {
  ...
  case eACTIVATEFLASHLOADER:
    this->ms32_Result =
        this->ActivateFlashLoader(mu32_FlashloaderResetWaitTime);
    break;
  ...
  case eUPDATENODE:
    this->ms32_Result = this->UpdateNode(
        this->mc_HexFilePath.toStdString().c_str(),
