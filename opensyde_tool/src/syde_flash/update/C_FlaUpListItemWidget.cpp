...
C_FlaUpListItemWidget::C_FlaUpListItemWidget(
    QListWidget *const opc_Parent, C_FlaUpHexFileInfo *const opc_HexFileInfo,
    const bool oq_IsRelativePath)
    : C_OgeWiOnlyBackground(opc_Parent), pc_HexFileInfo(opc_HexFileInfo),
      q_IsRelativePathToAdd(oq_IsRelativePath),
      mpc_Ui(new Ui::C_FlaUpListItemWidget), mpc_ContextMenu(nullptr),
      ms32_ApplicationBlockIndex(0), mpc_FileStatusChecktimer(nullptr),
      mpc_Movie(new QMovie(":/images/UpdateAnimationInProgress.gif",
                           QByteArray(), this)) {
  ...
  stw::opensyde_gui_logic::C_OgeWiUtil::h_ApplyStylesheetProperty(
      this->mpc_Ui->pc_Separator, "HasColor9Background", true);

  this->m_GetHexFileInformation();
  this->m_SetupContextMenu();

  this->m_GetAbsoluteAndRelativePaths(
      this->pc_HexFileInfo->c_HexFileInfo.c_FilePath, C_Uti::h_GetExePath());

  connect(this->mpc_Ui->pc_DeleteButton, &QPushButton::clicked, this,
          &C_FlaUpListItemWidget::m_DeleteItem);
  connect(this->mpc_Ui->pc_BrowseButton, &QPushButton::clicked, this,
          &C_FlaUpListItemWidget::m_SelectFile);

  connect(mpc_FileStatusChecktimer, &QTimer::timeout, this,
          &C_FlaUpListItemWidget::m_FileStatusCheck);
  mpc_FileStatusChecktimer->start(10);

  m_SetSizePolicy();

  this->mpc_Movie->setScaledSize(c_DEFAULT_SIZE);
  this->mpc_Ui->pc_StatusIcon->setPixmap(
      QIcon(c_StatusIconPath).pixmap(c_DEFAULT_SIZE));
} // lint !e818
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
// lint -e{1540}  no memory leak because of the parent of mpc_Movie and the Qt
// memory management
C_FlaUpListItemWidget::~C_FlaUpListItemWidget() {
  ...
  if (c_FileInfo.exists() == true) {
    if (this->q_IsRelativePathToAdd) {
      this->mpc_Ui->pc_FilePathLabel->setText(C_Uti::h_MinimizePath(
          ...
          static_cast<uint32_t>(this->width()) - mhs32_TITLE_OFFSET, 0U));
    } else {
      this->mpc_Ui->pc_FilePathLabel->setText(C_Uti::h_MinimizePath(
          this->mc_CurrentAbsolutePath, this->mpc_Ui->pc_FilePathLabel->font(),
          static_cast<uint32_t>(this->width()) - mhs32_TITLE_OFFSET, 0U));
    }
    if (pc_HexFileInfo->c_HexFileInfo.c_TimeStamp !=
        "No information available") {
      for (int32_t s32_It = 0;
           s32_It < static_cast<int32_t>(pc_HexFileInfo->c_BlockInfo.size());
           s32_It++) {
        if (pc_HexFileInfo->c_BlockInfo.at(s32_It).c_BlockType ==
            "Application") {
          QString c_Content;

          this->ms32_ApplicationBlockIndex = s32_It;
          c_Content =
              this->m_UpdateToolTipContent(this->ms32_ApplicationBlockIndex);

          this->mpc_Ui->pc_DeviceIDLabel->setText(
              pc_HexFileInfo->c_BlockInfo.at(this->ms32_ApplicationBlockIndex)
                  .c_DeviceId);
          this->mpc_Ui->pc_ProjectNameLabel->setText(
              pc_HexFileInfo->c_BlockInfo.at(this->ms32_ApplicationBlockIndex)
                  .c_ProjectName);
          this->mpc_Ui->pc_ProjectVersionLabel->setText(
              pc_HexFileInfo->c_BlockInfo.at(this->ms32_ApplicationBlockIndex)
                  .c_ProjectVersion);
    ...
    this->mpc_Ui->pc_DeviceIdWidget->SetToolTipInformation(
        "Missing", "Cannot able to find file");
  }
}
...
void C_FlaUpListItemWidget::SetState(const uint32_t ou32_State) {
  ...
  // Stop the GIF
  disconnect(this->mpc_Movie, &QMovie::finished, this,
             &C_FlaUpListItemWidget::m_RestartMovie);
  this->mpc_Movie->stop();

  switch (ou32_State) {
  case C_FlaUpListItemWidget::hu32_STATE_DEFAULT:
    this->mpc_Ui->pc_StatusIcon->setPixmap(
        QIcon(":/images/IconUpdateWaiting.svg").pixmap(c_Size));
    break;
  case C_FlaUpListItemWidget::hu32_STATE_ERROR:
    this->mpc_Ui->pc_StatusIcon->setPixmap(
        QIcon(":/images/IconUpdateError.svg").pixmap(c_Size));
    break;
  case C_FlaUpListItemWidget::hu32_STATE_FINISHED:
    this->mpc_Ui->pc_StatusIcon->setPixmap(
        QIcon(":/images/IconUpdateSuccess.svg").pixmap(c_Size));
    break;
  case C_FlaUpListItemWidget::hu32_STATE_IN_PROGRESS:
    this->mpc_Ui->pc_StatusIcon->setMovie(this->mpc_Movie);
    connect(this->mpc_Movie, &QMovie::finished, this,
            &C_FlaUpListItemWidget::m_RestartMovie);
...
void C_FlaUpListItemWidget::m_SetupContextMenu() {
  ...
  this->mpc_ContextMenu->addAction("Select File", this,
                                   &C_FlaUpListItemWidget::m_SelectFile);
  this->mpc_ContextMenu->addSeparator();
  this->mpc_ContextMenu->addAction("View File Information", this,
                                   &C_FlaUpListItemWidget::m_ViewFileInfo);
  this->mpc_ContextMenu->addAction("Show in Explorer", this,
                                   &C_FlaUpListItemWidget::m_ShowInExplorer);
  this->mpc_ContextMenu->addSeparator();
  this->mpc_ContextMenu->addAction("Remove File", this,
                                   &C_FlaUpListItemWidget::m_DeleteItem);
  this->mpc_ContextMenu->addAction("Remove all Files", this,
                                   &C_FlaUpListItemWidget::m_DeleteAllItems);
...
void C_FlaUpListItemWidget::m_ViewFileInfo() {
  if (this->pc_HexFileInfo != NULL) {
    ...
    const C_FlaUpHexFileView *const pc_HexViewFileDialog =
        new C_FlaUpHexFileView(*c_New, *pc_HexFileInfo);
...
C_FlaUpListItemWidget::m_UpdateToolTipContent(const int32_t os32_Index) {
  ...
  if (this->pc_HexFileInfo != NULL) {
    ...
    const float64_t f64_FileSize =
        std::ceil(static_cast<float64_t>(u32_FileSize) / 1024.0);

    c_Content += "Project Name: ";
    c_Content += this->pc_HexFileInfo->c_BlockInfo.at(os32_Index).c_ProjectName;

    c_Content += "\nTimestamp: ";
    c_Content += this->pc_HexFileInfo->c_BlockInfo.at(os32_Index).c_Date + " " +
                 this->pc_HexFileInfo->c_BlockInfo.at(os32_Index).c_Time;

    c_Content += "\nVersion: ";
    c_Content +=
        this->pc_HexFileInfo->c_BlockInfo.at(os32_Index).c_ProjectVersion;
...
void C_FlaUpListItemWidget::m_DeleteItem() {
  Q_EMIT this->SigDeleteItemWidget(pc_HexFileInfo->s32_CurrentHexFileIndex,
                                   true);
}
    ...
    if (c_FileInfo.exists() == false) {
      if (this->mpc_Ui->pc_DeviceIDLabel->text() != "Missing") {
        Q_EMIT this->SigReloadCurrentFile(
            this->pc_HexFileInfo->c_HexFileInfo.c_FilePath,
            this->pc_HexFileInfo);
        this->mpc_Ui->pc_DeviceIDLabel->SetBackgroundColor(18);
        this->mpc_Ui->pc_DeviceIDLabel->setText("Missing");
        this->mpc_Ui->pc_ProjectNameLabel->setVisible(false);
      ...
      if (this->pc_HexFileInfo->c_HexFileInfo.s32_NumberOfBlocks > 0) {
        if (this->mpc_Ui->pc_DeviceIDLabel->text() !=
            this->pc_HexFileInfo->c_BlockInfo
                .at(this->ms32_ApplicationBlockIndex)
                .c_DeviceId) {
          Q_EMIT this->SigReloadCurrentFile(
              this->pc_HexFileInfo->c_HexFileInfo.c_FilePath,
              this->pc_HexFileInfo);
...
void C_FlaUpListItemWidget::resizeEvent(QResizeEvent *const opc_Event) {
  ...
  if (this->pc_HexFileInfo != NULL) {
    if (this->q_IsRelativePathToAdd) {
      this->mpc_Ui->pc_FilePathLabel->setText(C_Uti::h_MinimizePath(
          this->mc_CurrentRelativePath, this->mpc_Ui->pc_FilePathLabel->font(),
          static_cast<uint32_t>(s32_Width) - mhs32_TITLE_OFFSET, 0U));
    } else {
      this->mpc_Ui->pc_FilePathLabel->setText(C_Uti::h_MinimizePath(
          this->mc_CurrentAbsolutePath, this->mpc_Ui->pc_FilePathLabel->font(),
          static_cast<uint32_t>(s32_Width) - mhs32_TITLE_OFFSET, 0U));
  ...
  if (this->pc_HexFileInfo != NULL) {
    if ((this->pc_HexFileInfo->c_HexFileInfo.s32_NumberOfBlocks > 0) &&
      ...
      this->mpc_Ui->pc_ProjectNameLabel->setText(C_Uti::h_MinimizePath(
          this->pc_HexFileInfo->c_BlockInfo.at(this->ms32_ApplicationBlockIndex)
              .c_ProjectName,
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListItemWidget::m_RestartMovie(void) { this->mpc_Movie->start(); }

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Get absolute or relative paths for hex files

   \param[in]       orc_Path                  file path
   \param[in]      orc_AbsoluteReferenceDir   Detailed output parameter
   description
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaUpListItemWidget::m_GetAbsoluteAndRelativePaths(
