...
/*!
   \file
   \brief       Integration class for sequence of device configuration,
   containing search nodes and configure node.
...
C_FlaSenDcBasicSequences::C_FlaSenDcBasicSequences(void)
    : QObject(), C_OscDcBasicSequences(), me_Sequence(eNOT_ACTIVE),
      mu32_FlashloaderResetWaitTime(1000), mu8_CurrentNodeId(0),
      mu8_NewNodeId(0), mu32_CanBitrate(125), mu8_InterfaceIndex(0),
      ms32_Result(C_NOACT) {
  mpc_Thread =
      new C_SyvComDriverThread(&C_FlaSenDcBasicSequences::mh_ThreadFunc, this);
}
    ...
    if (this->mpc_Thread->isRunning() == true) {
      ...
      if (this->mpc_Thread->wait(2000U) == false) {
        ...
        osc_write_log_warning(
            "Closing device configuration sequences",
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Initialize transport protocol, openSYDE protocol driver and CAN with
   given parameters.
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_FlaSenDcBasicSequences::InitDcSequences(const QString &orc_CanDllPath,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_FlaSenDcBasicSequences::StartScanEnterFlashloader(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_FlaSenDcBasicSequences::StartConfigureDevice(
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_FlaSenDcBasicSequences::E_Sequence
C_FlaSenDcBasicSequences::GetCurrentSequence() const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_FlaSenDcBasicSequences::m_ReportProgress(
...
void C_FlaSenDcBasicSequences::m_ReportDevicesInfoRead(
  ...
  const QString c_Message = C_OscDcBasicSequences::h_DevicesInfoToString(
      orc_DeviceInfoResult, oq_SecurityFeatureUsed);

  if (c_Message == "Found 0 device(s):") {
    osc_write_log_warning("Report", c_Message);
  } else {
    osc_write_log_info("Report", c_Message);
  }

  Q_EMIT(this->SigReportDevicesInfoRead(orc_DeviceInfoResult,
...
void C_FlaSenDcBasicSequences::m_ThreadFunc(void) {
  ...
  case eSCAN_ENTER_FLASHLOADER:
    this->ms32_Result =
        this->ScanEnterFlashloader(mu32_FlashloaderResetWaitTime);
    break;
  case eSCAN_GET_INFO:
  ...
  case eRESET_SYSTEM:
    ...
    break;
  case eCONF_DEVICES:
    this->ms32_Result = this->ConfigureDevice(
        mu8_CurrentNodeId, mu8_NewNodeId, mu32_CanBitrate, mu8_InterfaceIndex);
    break;
  default:
    Q_ASSERT(false);
    break;
