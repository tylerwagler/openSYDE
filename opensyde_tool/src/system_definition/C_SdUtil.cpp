...
/*! \brief   Generate node and interface names
   ...
   \param[in]   orc_NodeIndices              Node indices (match with interface
   indices expected)
   \param[in]   orc_InterfaceIndices         Interface indices (match with node
   indices expected)
   \param[out]  orc_Names                    Generate node & interface names
   \param[in]   oq_NameWithInterfaceAlways   Flag if the interface name will be
   added always, or only in case of multiple interfaces of node used
   \param[in]   opc_DatapoolIndices          Optional pointer to Datapool
   indices (match with node indices expected)
   \param[out]  opc_DatapoolNames            Optional pointer to found Datapool
   names
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_SdUtil::h_GetNames(const QList<uint32_t> &orc_NodeIndices,
                     const QList<uint32_t> &orc_InterfaceIndices,
                     QStringList &orc_Names,
                     const bool oq_NameWithInterfaceAlways,
                     const QList<uint32_t> *const opc_DatapoolIndices,
  ...
  int32_t s32_Retval = C_NO_ERR;
   ...
   if (orc_NodeIndices.size() == orc_InterfaceIndices.size()) {
    ...
    if ((oq_NameWithInterfaceAlways == false) && (orc_NodeIndices.size() > 1)) {
      for (uint32_t u32_NodeCounter = 0U;
           u32_NodeCounter <
           static_cast<uint32_t>(orc_NodeIndices.size() - 1UL);
           ++u32_NodeCounter) {
        for (uint32_t u32_NextNodeCounter = u32_NodeCounter + 1U;
          ...
          // The Datapool index is not relevant for this comparison, but can
          // cause multiple entries with same node index and interface index, so
          // iterating through all entries
          if ((orc_NodeIndices[u32_NodeCounter] ==
               orc_NodeIndices[u32_NextNodeCounter]) &&
              (orc_InterfaceIndices[u32_NodeCounter] !=
               orc_InterfaceIndices[u32_NextNodeCounter])) {
    ...
    for (uint32_t u32_NodeCounter = 0U;
         (u32_NodeCounter < orc_NodeIndices.size()) && (s32_Retval == C_NO_ERR);
         ++u32_NodeCounter) {
      if ((oq_NameWithInterfaceAlways == true) ||
          (c_SetNodesWithMultipleItf.find(orc_NodeIndices[u32_NodeCounter]) !=
           c_SetNodesWithMultipleItf.end())) {
        s32_Retval =
            h_GetName(orc_NodeIndices[u32_NodeCounter],
                      orc_InterfaceIndices[u32_NodeCounter], c_TmpName);
      } else {
      ...
      if (((opc_DatapoolIndices != NULL) && (opc_DatapoolNames != NULL)) &&
        ...
        const C_OscNodeDataPool *const pc_Dp =
            C_PuiSdHandler::h_GetInstance()->GetOscDataPool(
                orc_NodeIndices[u32_NodeCounter],
                (*opc_DatapoolIndices)[u32_NodeCounter]);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SdUtil::h_GetName(const uint32_t &oru32_NodeIndex,
                            QString &orc_Name) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QIcon C_SdUtil::h_InitStaticIconSvg(const QString &orc_File,
                                    const QSize &orc_Size) {
  QIcon c_Retval;

  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Normal,
                     QIcon::On);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Normal,
                     QIcon::Off);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Disabled,
                     QIcon::On);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Disabled,
                     QIcon::Off);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Active,
                     QIcon::On);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Active,
                     QIcon::Off);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Selected,
                     QIcon::On);
  c_Retval.addPixmap(QIcon(orc_File).pixmap(orc_Size), QIcon::Selected,
                     QIcon::Off);
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_ConvertTxMethodToName(
    const C_OscCanMessage::E_TxMethodType &ore_Type) {
  QString c_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_ConvertByteOrderToName(
    const C_OscCanSignal::E_ByteOrderType &ore_Type) {
  QString c_Retval;
...
/*! \brief   Get error tooltip suggestion
   ...
   \param[in]   oq_NvmOverlapDetected  Flag if NVM overlap of at least two
   Datapools detected
   \param[out]  orc_Heading            Heading suggestion
   \param[out]  orc_Content            Content suggestion
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_GetErrorToolTipDataPools(
    const uint32_t ou32_NodeIndex, const QList<uint32_t> &orc_Indices,
  ...
  for (uint32_t u32_ItDataPool = 0;
       (u32_ItDataPool < orc_Indices.size()) &&
       (u32_ItDataPool < mu32_TOOL_TIP_MAXIMUM_ITEMS);
       ++u32_ItDataPool) {
  ...
  if (mu32_TOOL_TIP_MAXIMUM_ITEMS < orc_Indices.size()) {
    orc_Content += static_cast<QString>("+%1\n").arg(
        orc_Indices.size() -
        static_cast<uint32_t>(mu32_TOOL_TIP_MAXIMUM_ITEMS));
  }
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sort source vector descending ( Sorting steps are done for the
   target vector in sync)

   \param[in,out]  orc_Source    Unsorted source indices
   \param[in,out]  orc_Target    Unsorted target indices
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_SortSourceDescending(QList<uint32_t> &orc_Source,
                                      QList<uint32_t> &orc_Target) {
  if (h_CheckSortedDescending(orc_Source) == false) {
    QList<uint32_t> c_NewSource;
    QList<uint32_t> c_NewTarget;
    const QList<int32_t> c_IndexMap =
        C_Uti::h_CreateAscendingIndexMap(orc_Source);
    ...
    for (uint32_t u32_It = c_IndexMap.size(); u32_It > 0; --u32_It) {
      ...
      if (c_IndexMap[u32_PreviousIndex] >= 0) {
        const uint32_t u32_CurIndex =
            static_cast<uint32_t>(c_IndexMap[u32_PreviousIndex]);
        if ((u32_CurIndex < orc_Source.size()) &&
            (u32_CurIndex < orc_Target.size())) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdUtil::h_CheckSortedDescending(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_SortIndicesAscending(QList<uint32_t> &orc_IndicesTmp) {
  if (C_Uti::h_CheckSortedAscending(orc_IndicesTmp) == false) {
    ...
    for (uint32_t u32_ItIndex = 0; u32_ItIndex < c_IndexMap.size();
         ++u32_ItIndex) {
      if (c_IndexMap[u32_ItIndex] >= 0) {
        const uint32_t u32_CurIndex =
            static_cast<uint32_t>(c_IndexMap[u32_ItIndex]);
...
uint8_t C_SdUtil::h_GetNodeIdMaximum(const uint32_t &oru32_NodeIndex) {
  ...
  if (pc_Node != NULL) {
    ...
    if ((rc_NodeProp.e_DiagnosticServer ==
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_SdUtil::h_GetActiveNodeInterface(const QComboBox &orc_ComboBox,
    ...
    for (u32_InterfaceCounter = 0;
      ...
      if (rc_ComInterface.e_InterfaceType == ore_Type) {
        const QString c_Name = C_PuiSdUtil::h_GetInterfaceName(
            ore_Type, rc_ComInterface.u8_InterfaceNumber);
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdUtil::h_InitNodeInterfaceComboBox(
    const C_OscNode &orc_Node, const C_OscSystemBus::E_Type &ore_BusType,
  ...
  const QList<C_OscNodeComInterfaceSettings> c_EthernetInterfaces =
      c_NodeProperties.GetEthernetInterfaces();
  const QList<C_OscNodeComInterfaceSettings> c_CanInterfaces =
      c_NodeProperties.GetCanInterfaces();
  const QList<C_OscNodeComInterfaceSettings> &rc_Interfaces =
      (ore_BusType == C_OscSystemBus::E_Type::eETHERNET) ? c_EthernetInterfaces
                                                         : c_CanInterfaces;

  const uint32_t u32_NumBusses =
      (ore_BusType == C_OscSystemBus::E_Type::eETHERNET)
          ? static_cast<uint32_t>(pc_DevDef->u8_NumEthernetBusses)
          : static_cast<uint32_t>(pc_DevDef->u8_NumCanBusses);

  // original(when the project has been saved) devdef size of the specific
  // element
  uint32_t u32_InterfaceSize =
      (ore_BusType == C_OscSystemBus::E_Type::eETHERNET)
          ? c_EthernetInterfaces.size()
          : c_CanInterfaces.size();

  u32_InterfaceSize =
      (u32_NumBusses <= u32_InterfaceSize) ? u32_NumBusses : u32_InterfaceSize;

  // Fill the combo box
  for (u32_Counter = 0U; u32_Counter < u32_InterfaceSize; ++u32_Counter) {
    const C_OscNodeComInterfaceSettings &rc_Interface =
        rc_Interfaces[u32_Counter];
    ...
    if (rc_Interface.GetBusConnected()) {
      if (ors32_SpecialInterface >= 0) {
        if (static_cast<uint8_t>(ors32_SpecialInterface) ==
            rc_Interface.u8_InterfaceNumber) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QList<uint32_t> C_SdUtil::h_GetUsedNodeIdsForBusUniqueAndSortedAscending(
    const uint32_t &oru32_BusIndex, const uint32_t &oru32_SpecialNodeIndex,
    const int32_t &ors32_SpecialInterface) {
  QList<uint32_t> c_Retval;
  const C_OscSystemBus *const pc_Bus =
      C_PuiSdHandler::h_GetInstance()->GetOscBus(oru32_BusIndex);
  if (pc_Bus != NULL) {
    QList<uint32_t> c_NodeIndexes;
    QList<uint32_t> c_InterfaceIndexes;

    C_PuiSdHandler::h_GetInstance()
        ->GetOscSystemDefinitionConst()
        .GetNodeIndexesOfBus(oru32_BusIndex, c_NodeIndexes, c_InterfaceIndexes);
    c_Retval.reserve(c_NodeIndexes.size());
    if (c_NodeIndexes.size() == c_InterfaceIndexes.size()) {
      for (uint32_t u32_ItNode = 0; u32_ItNode < c_NodeIndexes.size();
           ++u32_ItNode) {
        const C_OscNode *const pc_CurNode =
            C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(
                c_NodeIndexes[u32_ItNode]);
        if (pc_CurNode != NULL) {
          if (c_InterfaceIndexes[u32_ItNode] <
              pc_CurNode->c_Properties.c_ComInterfaces.size()) {
            const C_OscNodeComInterfaceSettings &rc_CurComInterface =
                pc_CurNode->c_Properties
                    .c_ComInterfaces[c_InterfaceIndexes[u32_ItNode]];
...
//----------------------------------------------------------------------------------------------------------------------
QList<uint32_t>
C_SdUtil::h_GetUsedProcessIdsForApplicationUniqueAndSortedAscending(
    const uint32_t ou32_NodeIndex, const int32_t os32_SpecialApplicationIndex) {
  QList<uint32_t> c_Retval;
  const C_OscNode *const pc_Node =
      C_PuiSdHandler::h_GetInstance()->GetOscNodeConst(ou32_NodeIndex);
  if (pc_Node != NULL) {
    for (uint32_t u32_ItApp = 0; u32_ItApp < pc_Node->c_Applications.size();
         ++u32_ItApp) {
      if ((os32_SpecialApplicationIndex < 0) ||
          (u32_ItApp != static_cast<uint32_t>(os32_SpecialApplicationIndex))) {
        const C_OscNodeApplication &rc_CurApplication =
            pc_Node->c_Applications[u32_ItApp];
        if (rc_CurApplication.e_Type ==
            C_OscNodeApplication::ePROGRAMMABLE_APPLICATION) {
          c_Retval.push_back(
              static_cast<uint32_t>(rc_CurApplication.u8_ProcessId));
        }
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QList<uint32_t> C_SdUtil::h_GetUsedBusIdsUniqueAndSortedAscending(
    const int32_t os32_SpecialBusIndex) {
  QList<uint32_t> c_Retval;
  for (uint32_t u32_ItBus = 0;
       u32_ItBus < C_PuiSdHandler::h_GetInstance()->GetOscBusesSize();
       ++u32_ItBus) {
    if ((os32_SpecialBusIndex < 0) ||
        (u32_ItBus != static_cast<uint32_t>(os32_SpecialBusIndex))) {
      const C_OscSystemBus *const pc_Bus =
          C_PuiSdHandler::h_GetInstance()->GetOscBus(u32_ItBus);
      if (pc_Bus != NULL) {
...
/*! \brief   Get the last bytes of all IP Addresses used on the given bus.
             Only STW default IP addresses (starting with 192.168.0.X) are
   considered.
        ...
        if (pc_CurNode != NULL) {
          if (c_InterfaceIndexes[u32_ItNode] <
            ...
            if (((static_cast<uint8_t>(ors32_SpecialInterface) !=
              ...
              if ((rc_CurComInterface.GetBusConnected() == true) &&
                  (rc_CurComInterface.u32_BusIndex == oru32_BusIndex)) {
                // only consider addresses which match the STW default in their
                // first 3 bytes
                if ((rc_CurComInterface.c_Ip.au8_IpAddress[0] ==
                     C_OscNodeComInterfaceSettings::C_IpAddress::
                         hu8_IP_FIRST_BYTE) &&
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Gets all IP Addresses used on the given bus. (Complete addresses,
   not just last bytes)
...
//----------------------------------------------------------------------------------------------------------------------
QList<QByteArray>
C_SdUtil::h_GetAllUsedIpAddressesForBus(const uint32_t &oru32_BusIndex,
                                        const uint32_t &oru32_SpecialNodeIndex,
                                        const int32_t &ors32_SpecialInterface) {
  QList<QByteArray> c_Retval;
          ...
          if (c_InterfaceIndexes[u32_ItNode] <
            ...
            if (((static_cast<uint8_t>(ors32_SpecialInterface) !=
              ...
              if ((rc_CurComInterface.GetBusConnected() == true) &&
                  (rc_CurComInterface.u32_BusIndex == oru32_BusIndex)) {
                QByteArray c_Tmp;
                c_Tmp.append(static_cast<char>(rc_CurComInterface.c_Ip.au8_IpAddress[0]));
                c_Tmp.append(static_cast<char>(rc_CurComInterface.c_Ip.au8_IpAddress[1]));
                c_Tmp.append(static_cast<char>(rc_CurComInterface.c_Ip.au8_IpAddress[2]));
                c_Tmp.append(static_cast<char>(rc_CurComInterface.c_Ip.au8_IpAddress[3]));
...
/*! \brief   Adapt message to protocol restrictions
   ...
   \param[in,out]  orc_OscSignalListElements    Core datapool list elements for
   signals
   \param[in]      orc_UiSignalListElements     Ui datapool list elements for
   signals
   \param[in]      oe_Type                      Protocol type
   \param[in,out]  opc_AdaptationInfos          Optional report about
   adaptations
   \param[in]      oq_IncludeSignalUpdate       Flag to control signal update
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_AdaptMessageToProtocolType(
    C_OscCanMessage &orc_Message, C_PuiSdNodeCanMessage &orc_UiMessage,
    QList<C_OscNodeDataPoolListElement> &orc_OscSignalListElements,
    const QList<C_PuiSdNodeDataPoolListElement> &orc_UiSignalListElements,
    ...
    if (orc_Message.e_TxMethod != C_OscCanMessage::eTX_METHOD_CYCLIC) {
      c_Info.push_back(
          static_cast<QString>(
              ...
              .arg(C_SdUtil::h_ConvertTxMethodToName(
                  C_OscCanMessage::eTX_METHOD_CYCLIC)));
      ...
      if ((orc_Message.u32_CycleTimeMs == 0U) ||
        ...
        orc_Message.u32_CycleTimeMs =
            C_PuiSdUtil::h_GetDefaultMessageCycleTime();
        orc_Message.u32_TimeoutMs = C_PuiSdUtil::h_GetMessageAutoTimeoutTime(
            orc_Message.u32_CycleTimeMs);

        orc_UiMessage.e_ReceiveTimeoutMode =
            C_PuiSdNodeCanMessage::eRX_TIMEOUT_MODE_AUTO;
      }
    }
  }
  ...
  if ((oe_Type == C_OscCanProtocol::eECES) ||
    ...
    case C_OscCanProtocol::eECES:
      ...
      if (orc_Message.u16_Dlc != 8U) {
        c_Info.push_back(
            static_cast<QString>(
                "Message DLC changed from %1 to 8 due to ECeS/ECoS "
    ...
    case C_OscCanProtocol::eCAN_OPEN_SAFETY:
      ...
      if ((orc_Message.u32_CanId < mu32_PROTOCOL_ECOS_MESSAGE_ID_MIN) ||
        ...
        c_Info.push_back(
            static_cast<QString>(
                "Message ID changed from 0x%1 to 0x%2 due to ECoS protocol "
    ...
    case C_OscCanProtocol::eJ1939:
      // DLC fix 8
      if (orc_Message.u16_Dlc != 8U) {
        c_Info.push_back(static_cast<QString>(
                             "Message DLC changed from %1 to 8 due to J1939 "
                             "protocol restrictions.")
                             .arg(orc_Message.u16_Dlc));
        orc_Message.u16_Dlc = 8U;
      }
  ...
  if (oq_IncludeSignalUpdate) {
    // Adapt signals
    for (QList<C_OscCanSignal>::iterator c_SignalIt =
             orc_Message.c_Signals.begin();
         c_SignalIt != orc_Message.c_Signals.end(); ++c_SignalIt) {
      Q_ASSERT(c_SignalIt->u32_ComDataElementIndex <
               orc_OscSignalListElements.size());
      if (c_SignalIt->u32_ComDataElementIndex <
          orc_OscSignalListElements.size()) {
        Q_ASSERT(c_SignalIt->u32_ComDataElementIndex <
                 orc_UiSignalListElements.size());
        if (c_SignalIt->u32_ComDataElementIndex <
            orc_UiSignalListElements.size()) {
          h_AdaptSignalToProtocolType(
              *c_SignalIt,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_AdaptSignalToProtocolType(
    C_OscCanSignal &orc_Signal,
  ...
  if (oe_Type == C_OscCanProtocol::eJ1939) {
    const QString c_PrevType = C_SdNdeDpUtil::h_ConvertContentTypeToString(
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_InitUsedIdsString(const QList<uint32_t> &orc_UsedIds,
                                      const QString &orc_ItemName,
                                      const QString &orc_ItemType,
                                      const bool oq_SkipItem) {
  QString c_Retval;

  if (orc_UsedIds.size() > 0UL) {
    QString c_BusIds;

    for (uint32_t u32_ItBusId = 0;
         u32_ItBusId < static_cast<uint32_t>(orc_UsedIds.size() - 1UL);
         ++u32_ItBusId) {
    ...
    if (oq_SkipItem == true) {
      ...
      c_Retval = static_cast<QString>("Already used IDs on %3 %1: %2")
                     .arg(orc_ItemName)
                     .arg(c_BusIds)
                     .arg(orc_ItemType);
    }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Builds a string which contains all used IP addresses separated with
   new line.
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_InitUsedIpsString(
    const QList<QByteArray> &orc_UsedIps,
  ...
  if (orc_UsedIps.size() > 0UL) {
    QString c_BusIps;
    for (uint16_t u16_ItIp = 0; u16_ItIp < (orc_UsedIps.size() - 1UL);
         ++u16_ItIp) {
      c_BusIps += static_cast<QString>("%1,\n").arg(
          C_SdUtil::h_IpAddressAsString(orc_UsedIps[u16_ItIp]));
    }
    // append last one without comma & new line
    c_BusIps +=
        C_SdUtil::h_IpAddressAsString(orc_UsedIps[orc_UsedIps.size() - 1]);

    if (oq_SkiptItem == true) {
      c_Retval = static_cast<QString>("Already used IPs: %1").arg(c_BusIps);
    } else {
      c_Retval = static_cast<QString>("Already used IPs on %3 %1:\n%2")
                     .arg(orc_ItemName)
                     .arg(c_BusIps)
                     .arg(orc_ItemType);
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_IpAddressAsString(const QByteArray &orc_Ip) {
  QString c_Retval;
  ...
  if (orc_Ip.size() == 4) {
    for (uint8_t u8_It = 0; u8_It < (orc_Ip.size() - 1UL); ++u8_It) {
      c_Retval +=
          static_cast<QString>("%1.").arg(static_cast<uint32_t>(static_cast<uint8_t>(orc_Ip[u8_It])));
    }
    // append last one without point
    c_Retval +=
        QString::number(static_cast<uint32_t>(static_cast<uint8_t>(orc_Ip[orc_Ip.size() - 1])));
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Function generates node id or last byte for IP address depending on
   oq_GenerateId

   \param[in]  orc_Interfaces          Node interfaces
   \param[in]  orc_UsedNodeProperties  Already used node properties (id/ip:
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_SdUtil::h_GetNextFreeNodeProperty(
    const QList<C_OscNodeComInterfaceSettings> &orc_Interfaces,
    const QList<uint32_t> &orc_UsedNodeProperties,
    const int32_t &ors32_SpecialInterface, const bool oq_GenerateId) {
  uint32_t u32_Retval = 0;

  if (ors32_SpecialInterface >= 0) {
    for (uint32_t u32_ItInterface = 0; u32_ItInterface < orc_Interfaces.size();
         ++u32_ItInterface) {
      const C_OscNodeComInterfaceSettings &rc_CurComIf =
          orc_Interfaces[u32_ItInterface];
      if (rc_CurComIf.u8_InterfaceNumber ==
          static_cast<uint8_t>(ors32_SpecialInterface)) {
        if (oq_GenerateId) {
    ...
    for (uint32_t u32_It = 0; u32_It < orc_UsedNodeProperties.size();
         ++u32_It) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdUtil::h_HasConnectionType(const C_OscNode &orc_Node,
  ...
  for (u32_IntfCounter = 0U;
       u32_IntfCounter < orc_Node.c_Properties.c_ComInterfaces.size();
       ++u32_IntfCounter) {
    if (orc_Node.c_Properties.c_ComInterfaces[u32_IntfCounter]
            .e_InterfaceType == oe_Type) {
      q_Return = true;
      break;
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Check if any com interfaces for the specified bus type are
   available and free
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdUtil::h_CheckNodeInterfaceAvailable(
    const QList<C_OscNodeComInterfaceSettings> &orc_ComInterfaces,
    const C_OscSystemBus::E_Type &ore_BusType) {
  bool q_Retval = false;

  for (uint32_t u32_ItInterface = 0; u32_ItInterface < orc_ComInterfaces.size();
       ++u32_ItInterface) {
    const C_OscNodeComInterfaceSettings &rc_Interface =
        orc_ComInterfaces[u32_ItInterface];
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Check if at least one com interfaces for the specified bus type is
   connected
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdUtil::h_CheckNodeInterfaceConnected(
    const QList<C_OscNodeComInterfaceSettings> &orc_ComInterfaces,
    const C_OscSystemBus::E_Type &ore_BusType) {
  bool q_Retval = false;
...
/*! \brief   Check the total number of datapool
   ...
   The total number of datapools is bounded above by
   C_OscNode::hu32_MAX_NUMBER_OF_DATA_POOLS_PER_NODE.

   \param[in]  oru32_NodeIndex      node index
   \param[in]  orq_AlreadyChecked   for pre-checking cases (true: we know that
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdUtil::h_CheckDatapoolNumber(const uint32_t &oru32_NodeIndex,
                                     const bool &orq_AlreadyChecked,
  ...
  if (q_Return == false) {
    C_OgeWiCustomMessage c_MessageBox(opc_Parent,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SdUtil::h_GetErrorToolTipNode(const uint32_t &oru32_NodeIndex,
                                        QString &orc_Text,
                                        bool &orq_ErrorDetected,
                                        const bool oq_CheckAll) {
  bool q_IsMulti = C_OscNodeSquad::h_CheckIsMultiDevice(
      oru32_NodeIndex, C_PuiSdHandler::h_GetInstance()
                           ->GetOscSystemDefinitionConst()
                           .c_NodeSquads);
  ...
  for (uint32_t u32_ItNode = 0UL;
       (u32_ItNode < c_NodeIndices.size()) && (s32_Retval == C_NO_ERR);
       ++u32_ItNode) {
    bool q_NameConflict;
    bool q_NameEmpty;
    ...
    const bool q_DataPoolNvmConflict =
        C_PuiSdHandler::h_GetInstance()->CheckNodeNvmDataPoolsSizeConflict(
            c_NodeIndices[u32_ItNode], &q_DatapoolNvmSizeConflict,
    ...
    QList<uint32_t> c_InvalidInterfaceIndices;
    QList<uint32_t> c_InvalidDataPoolIndices;
    QList<uint32_t> c_InvalidApplicationIndices;
    QList<uint32_t> c_InvalidDomainIndices;
    QList<C_OscCanProtocol::E_Type> c_InvalidProtocolTypes;
    s32_Retval =
        C_PuiSdHandler::h_GetInstance()
            ->GetOscSystemDefinitionConst()
            .CheckErrorNode(
                c_NodeIndices[u32_ItNode], &q_NameConflict, &q_NameEmpty,
                &q_NodeIdInvalid, &q_IpInvalid, &q_DataPoolsInvalid,
                &q_ApplicationsInvalid, &q_DomainsInvalid,
                &q_CommMinSignalCountInvalid, &q_CommMaxSignalCountInvalid,
                &q_CoPdoCountInvalid, &q_CoNodeIdInvalid, &q_CoHeartbeatInvalid,
                true, &c_InvalidInterfaceIndices, &c_InvalidDataPoolIndices,
                &c_InvalidApplicationIndices, &c_InvalidDomainIndices,
                &c_InvalidProtocolTypes);

    if (s32_Retval == C_NO_ERR) {
      if ((q_NameConflict == true) || (q_NodeIdInvalid == true) ||
          (q_IpInvalid == true) || (q_DataPoolsInvalid == true) ||
          (q_ApplicationsInvalid == true) || (q_DomainsInvalid == true) ||
          (q_DataPoolNvmConflict == true) || (q_NameEmpty == true) ||
          (q_CommMinSignalCountInvalid == true) ||
          (q_CommMaxSignalCountInvalid == true) ||
          (q_CoPdoCountInvalid == true) || (q_CoNodeIdInvalid == true) ||
        ...
        if (((q_NameConflict == true) || (q_NodeIdInvalid == true) ||
          ...
          if (q_NodeIdInvalid == true) {
            if (c_InvalidInterfaceIndices.size() == 1UL) {
              ...
              orc_Text +=
                  static_cast<QString>("%1 duplicate node IDs detected.\n")
                      .arg(c_InvalidInterfaceIndices.size());
            }
          }
          if (q_IpInvalid == true) {
            if (c_InvalidInterfaceIndices.size() == 1UL) {
        ...
        if (q_ApplicationsInvalid == true) {
          ...
          if (mu32_TOOL_TIP_MAXIMUM_ITEMS <
              c_InvalidApplicationIndices.size()) {
        ...
        if ((q_DataPoolsInvalid == true) || (q_DataPoolNvmConflict == true)) {
          QString c_Heading;
          QString c_Content;
          C_SdUtil::h_GetErrorToolTipDataPools(
              c_NodeIndices[u32_ItNode], c_InvalidDataPoolIndices,
              q_DatapoolNvmSizeConflict, q_DatapoolNvmOverlapConflict,
              c_Heading, c_Content);
          orc_Text += c_Heading + "\n";
          orc_Text += c_Content;
          orc_Text += "\n";
        ...
        if (q_DomainsInvalid == true) {
          ...
          for (uint32_t u32_ItDomains = 0;
               (u32_ItDomains < c_InvalidDomainIndices.size()) &&
               (u32_ItDomains < mu32_TOOL_TIP_MAXIMUM_ITEMS);
               ++u32_ItDomains) {
            const C_OscHalcConfigDomain *const pc_Domain =
                C_PuiSdHandler::h_GetInstance()->GetHalcDomainConfigDataConst(
                    c_NodeIndices[u32_ItNode],
        ...
        if ((q_CommMinSignalCountInvalid == true) ||
          ...
          orc_Text += "Invalid COMM protocol configuration with invalid signal "
                      "count:\n";
          for (u32_InvalidProtCounter = 0U;
               u32_InvalidProtCounter < c_InvalidProtocolTypes.size();
               ++u32_InvalidProtCounter) {
            orc_Text += C_PuiSdUtil::h_ConvertProtocolTypeToString(
                c_InvalidProtocolTypes[u32_InvalidProtCounter]);
          }
        }

        if ((q_CoNodeIdInvalid == true) || (q_CoHeartbeatInvalid == true) ||
          ...
          if (q_CoHeartbeatInvalid == true) {
            orc_Text +=
                "Invalid CANopen Device Heartbeat consumer time detected.\n";
          }

          if (q_CoPdoCountInvalid == true) {
            orc_Text +=
                "Too many PDOs in a CANopen Manager configuration detected.\n";
          }
        }
      }
    } else {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_GetErrorToolTipBus(const uint32_t &oru32_BusIndex,
                                    QString &orc_Text) {
  bool q_NameConflict;
  bool q_NameEmpty;
  bool q_IdInvalid;
  ...
  QList<stw::opensyde_core::C_OscCanProtocol::E_Type> c_InvalidProtocols;
  ...
  if ((((q_NameConflict == true) || (q_NameEmpty == true)) ||
       (q_IdInvalid == true)) ||
    ...
    if (c_InvalidNodesForBitRate.size() > 0UL) {
      ...
      for (uint32_t u32_ItNode = 0UL;
           (u32_ItNode < c_InvalidNodesForBitRate.size()) &&
           (u32_ItNode < mu32_TOOL_TIP_MAXIMUM_ITEMS);
           ++u32_ItNode) {
        orc_Text += "- " + c_InvalidNodesForBitRate[u32_ItNode] + "\n";
      }
      if (mu32_TOOL_TIP_MAXIMUM_ITEMS < c_InvalidNodesForBitRate.size()) {
        orc_Text += static_cast<QString>("+%1\n").arg(
            c_InvalidNodesForBitRate.size() -
            static_cast<int32_t>(mu32_TOOL_TIP_MAXIMUM_ITEMS));
      }
    }
    orc_Text += "\n";
  ...
  if (c_InvalidProtocols.size() > 0) {
    ...
    for (uint32_t u32_ItProtocol = 0;
         u32_ItProtocol < c_InvalidProtocols.size(); ++u32_ItProtocol) {
      orc_Text += static_cast<QString>("%1\n").arg(
          C_PuiSdUtil::h_ConvertProtocolTypeToString(
              c_InvalidProtocols[u32_ItProtocol]));
    }
    orc_Text += "\n";
  }
}
...
/*! \brief   Get string for tooltip content for datapool list.
   ...
   \param[in]  oru32_ListIndex      index of list for which the tooltip is
   requested
...
QString C_SdUtil::h_GetToolTipContentDpList(const uint32_t &oru32_NodeIndex,
  ...
  const C_OscNodeDataPoolList *const pc_DpList =
      C_PuiSdHandler::h_GetInstance()->GetOscDataPoolList(
          oru32_NodeIndex, oru32_DatapoolIndex, oru32_ListIndex);
...
/*! \brief   Get string for tooltip content for datapool list element.
   ...
   \param[in]  orc_NodeDatapoolListElementId    indexes of element for which the
   tooltip is requested
   \param[in]  orc_AdditionalInformation        Optional additional information
   string which will get inserted after comment (no "\n" at end necessary)

   \return
      content as string
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_GetToolTipContentDpListElement(
  ...
  if ((pc_Node != NULL) && (pc_Datapool != NULL) && (pc_DpList != NULL) &&
      (pc_DpListElement != NULL) && (pc_UiElement != NULL)) {
    if (pc_Datapool->e_Type == C_OscNodeDataPool::eCOM) {
      // get message id and signal index and use special tool tip content
      // generation
      C_OscCanMessageIdentificationIndices c_MessageId;
      uint32_t u32_SignalIndex;
      C_PuiSdHandler::h_GetInstance()->ConvertElementIndexToSignalIndex(
          orc_NodeDatapoolListElementId, c_MessageId, u32_SignalIndex);
      ...
      // Properties:
      c_ToolTipContent.append(
          C_PuiSdHandlerNodeLogic::h_GetElementTypeName(pc_Datapool->e_Type) +
          " Properties: ");
      c_ToolTipContent.append("\n");
      ...
      if (pc_DpListElement->GetArray() == false) {
        ...
        c_ToolTipContent.append(
            QString::number(pc_DpListElement->GetArraySize()));
      }
      c_ToolTipContent.append("\n");
      ...
      if (pc_DpListElement->q_InterpretAsString == false) {
        ...
        C_SdNdeDpContentUtil::h_GetValuesAsScaledString(
            pc_DpListElement->c_MinValue, pc_DpListElement->f64_Factor,
        ...
        for (uint32_t u32_Pos = 0; u32_Pos < c_HelpVector.size(); u32_Pos++) {
          ...
          if ((pc_DpListElement->GetArray() == false) ||
              (pc_UiElement->q_AutoMinMaxActive == true)) {
            break;
          }
        }
        ...
        c_HelpVector.clear();
        C_SdNdeDpContentUtil::h_GetValuesAsScaledString(
            pc_DpListElement->c_MaxValue, pc_DpListElement->f64_Factor,
            pc_DpListElement->f64_Offset, c_HelpVector);
        c_HelpString = "";
        for (uint32_t u32_Pos = 0; u32_Pos < c_HelpVector.size(); u32_Pos++) {
          c_HelpString.append(c_HelpVector[u32_Pos]);
      ...
      if (pc_DpList->c_DataSets.size() > 0) {
        for (uint32_t u32_PosDataset = 0;
          ...
          if (pc_DpListElement->q_InterpretAsString) {
            c_HelpString = C_SdNdeDpContentUtil::h_ConvertToString(
                pc_DpListElement->c_DataSetValues[u32_PosDataset]);
          } else {
            C_SdNdeDpContentUtil::h_GetValuesAsScaledString(
                pc_DpListElement->c_DataSetValues[u32_PosDataset],
                pc_DpListElement->f64_Factor, pc_DpListElement->f64_Offset,
                c_HelpVector);
            for (uint32_t u32_PosArray = 0; u32_PosArray < c_HelpVector.size();
                 u32_PosArray++) {
      ...
      c_ToolTipContent.append(C_SdNdeDpUtil::h_ConvertElementAccessToString(
          pc_DpListElement->e_Access));
      c_ToolTipContent.append("\n");
      ...
      if ((pc_Datapool->e_Type == C_OscNodeDataPool::eNVM) ||
        ...
        c_ToolTipContent.append(
            QString::number(pc_DpListElement->GetSizeByte()));
        ...
        c_ToolTipContent.append(
            QString::number(pc_DpListElement->u32_NvmStartAddress -
...
/*! \brief   Get string for tooltip content for message.
   ...
   \param[in]  orc_MessageId  Message id belonging to the signal for which the
   tooltip is requested
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_GetToolTipContentMessage(
...
/*! \brief   Get string for tooltip content for signal.
   ...
   \param[in]  orc_MessageId              message id belonging to the signal for
   which the tooltip is requested
   \param[in]  oru32_SignalIndex          index of signal for which the tooltip
   is requested
   \param[in]  orc_AdditionalInformation  Optional additional information string
   which will get inserted after comment (no "\n" at end necessary)

   \return
      content as string
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_GetToolTipContentSignal(
    const C_OscCanMessageIdentificationIndices &orc_MessageId,
  ...
  const C_OscCanSignal *const pc_Signal =
      C_PuiSdHandler::h_GetInstance()->GetCanSignal(orc_MessageId,
                                                    oru32_SignalIndex);
  const C_OscCanMessage *const pc_Message =
      C_PuiSdHandler::h_GetInstance()->GetCanMessage(orc_MessageId);
  const stw::opensyde_core::C_OscNodeDataPoolListElement *const
      pc_DpListElement =
          C_PuiSdHandler::h_GetInstance()->GetOscCanDataPoolListElement(
              orc_MessageId, oru32_SignalIndex);
  const C_PuiSdNodeDataPoolListElement *const pc_DpListElementUi =
      C_PuiSdHandler::h_GetInstance()->GetUiCanDataPoolListElement(
          orc_MessageId, oru32_SignalIndex);
  ...
  if ((pc_Message != NULL) && (pc_Signal != NULL) &&
    ...
    c_ToolTipContent = C_SdTooltipUtil::h_GetToolTipContentSignal(
        *pc_Signal, *pc_Message, *pc_DpListElement, orc_MessageId.e_ComProtocol,
        c_AutoMinMaxInfo, orc_AdditionalInformation);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sort indices descending ( Sorting steps are done for the content
   vectors in sync)
...
void C_SdUtil::h_SortIndicesDescendingAndSync(
    QList<uint32_t> &orc_IndicesTmp, QList<T> &orc_OscContentTmp,
  ...
  if (h_CheckSortedDescending(orc_IndicesTmp) == false) {
    ...
    // convention checker
    QList<T> c_OscContentTmp;
    // lint -e{8080} //template naming not correctly handled by naming
    // convention checker
    QList<U> c_UiContentTmp;
    // Step 1: Fill new vector in sorted order with which element should be
    // copied to which position
    const QList<int32_t> c_IndexMap =
        C_Uti::h_CreateAscendingIndexMap(orc_IndicesTmp);
    ...
    for (uint32_t u32_ItIndex = c_IndexMap.size(); u32_ItIndex > 0;
         --u32_ItIndex) {
      if (c_IndexMap[static_cast<uint32_t>(u32_ItIndex - 1U)] >= 0) {
        const uint32_t u32_CurIndex = static_cast<uint32_t>(
            c_IndexMap[static_cast<uint32_t>(u32_ItIndex - 1U)]);
        if (((u32_CurIndex < orc_IndicesTmp.size()) &&
...
// Explicit declaration of every type usage is necessary for templates to allow
// split of declaration and implementation lint
// -esym(754,stw::opensyde_gui_logic::C_SdUtil::h_SortIndicesDescendingAndSync*)
template void
C_SdUtil::h_SortIndicesDescendingAndSync<C_OscCanMessageIdentificationIndices,
                                         uint32_t>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscCanMessageIdentificationIndices> &orc_OscContentTmp,
    QList<uint32_t> &orc_UiContentTmp);
template void C_SdUtil::h_SortIndicesDescendingAndSync<C_OscNodeDataPoolList,
                                                       C_PuiSdNodeDataPoolList>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscNodeDataPoolList> &orc_OscContentTmp,
    QList<C_PuiSdNodeDataPoolList> &orc_UiContentTmp);
template void
C_SdUtil::h_SortIndicesDescendingAndSync<C_OscNodeDataPoolDataSet,
                                         QList<C_OscNodeDataPoolContent>>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscNodeDataPoolDataSet> &orc_OscContentTmp,
    QList<QList<C_OscNodeDataPoolContent>> &orc_UiContentTmp);
template void
C_SdUtil::h_SortIndicesDescendingAndSync<C_OscNodeDataPoolListElement,
                                         C_PuiSdNodeDataPoolListElement>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscNodeDataPoolListElement> &orc_OscContentTmp,
    QList<C_PuiSdNodeDataPoolListElement> &orc_UiContentTmp);

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sort indices ascending ( Sorting steps are done for the content
   vectors in sync)
...
//----------------------------------------------------------------------------------------------------------------------
template <typename T, typename U>
void C_SdUtil::h_SortIndicesAscendingAndSync(
    QList<uint32_t> &orc_IndicesTmp, QList<T> &orc_OscContentTmp,
    QList<U> &orc_UiContentTmp) {
  if (C_Uti::h_CheckSortedAscending(orc_IndicesTmp) == false) {
    QList<uint32_t> c_IndicesTmp;
...
// Explicit declaration of every type usage is necessary for templates to allow
// split of declaration and implementation lint
// -esym(754,stw::opensyde_gui_logic::C_SdUtil::h_SortIndicesAscendingAndSync*)
template void
C_SdUtil::h_SortIndicesAscendingAndSync<C_OscCanMessageIdentificationIndices,
                                        uint32_t>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscCanMessageIdentificationIndices> &orc_OscContentTmp,
    QList<uint32_t> &orc_UiContentTmp);
template void C_SdUtil::h_SortIndicesAscendingAndSync<C_OscNodeDataPoolList,
                                                      C_PuiSdNodeDataPoolList>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscNodeDataPoolList> &orc_OscContentTmp,
    QList<C_PuiSdNodeDataPoolList> &orc_UiContentTmp);
template void
C_SdUtil::h_SortIndicesAscendingAndSync<C_OscNodeDataPoolDataSet,
                                        QList<C_OscNodeDataPoolContent>>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscNodeDataPoolDataSet> &orc_OscContentTmp,
    QList<QList<C_OscNodeDataPoolContent>> &orc_UiContentTmp);
template void
C_SdUtil::h_SortIndicesAscendingAndSync<C_OscNodeDataPoolListElement,
                                        C_PuiSdNodeDataPoolListElement>(
    QList<uint32_t> &orc_IndicesTmp,
    QList<C_OscNodeDataPoolListElement> &orc_OscContentTmp,
    QList<C_PuiSdNodeDataPoolListElement> &orc_UiContentTmp);
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdUtil::h_GetEdsFileDetails(
    const C_OscCanOpenObjectDictionary oc_CanOpenObjDictionary) {
  QString c_String;
  QString c_TrueOrFalse;
  const C_OscCanOpenEdsInfoBlock c_InfoBlock =
      oc_CanOpenObjDictionary.c_InfoBlock;
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Counting messages and signals of a specific node for a specific
   protocol

   \param[in]       ou32_NodeIndex           Node index for counting the
   messages. In case of CANopen it must be always the index of the CANopen
   Manager
   \param[in]       ou32_InterfaceIndex      Used interface index
   \param[in]       oe_Protocol              Current CAN protocol
   \param[in]       opc_CoDeviceInterfaceId  Optional: In case of protocol
   CANopen if the count of messages of a specific device is needed.
   ou32_NodeIndex must be the CANopen Manager which the device is assigned
   \param[out]      oru32_RxMessageCount     Counted Rx messages of the specific
   node
   \param[out]      oru32_TxMessageCount     Counted Tx messages of the specific
   node
   \param[out]      opu32_SignalCount        Optional: Signal count of all Rx
   and Tx messages
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SdUtil::h_GetMessageCountOfNode(
  ...
  if ((pc_Node != NULL) &&
    ...
    for (u32_DatapoolCounter = 0U;
         u32_DatapoolCounter < pc_Node->c_DataPools.size();
         ++u32_DatapoolCounter) {
      const C_OscNodeDataPool *const pc_Datapool =
          &pc_Node->c_DataPools[u32_DatapoolCounter];

      if ((pc_Datapool != NULL) &&
          (pc_Datapool->e_Type == C_OscNodeDataPool::eCOM)) {
        const C_OscCanProtocol *const pc_Protocol =
            pc_Node->GetRelatedCanProtocolConst(u32_DatapoolCounter);
        if ((pc_Protocol != NULL) &&
          ...
          const C_OscNodeDataPoolList *const pc_DataPoolList1 =
              C_PuiSdHandler::h_GetInstance()->GetOscDataPoolList(
                  ou32_NodeIndex, u32_DatapoolCounter, u32_ListIndex);
          const C_OscNodeDataPoolList *const pc_DataPoolList2 =
              C_PuiSdHandler::h_GetInstance()->GetOscDataPoolList(
                  ou32_NodeIndex, u32_DatapoolCounter, u32_ListIndex + 1);
          ...
          if ((pc_DataPoolList1 != NULL) && (pc_DataPoolList2 != NULL)) {
            ...
            if (oe_Protocol != C_OscCanProtocol::eCAN_OPEN) {
              const uint32_t u32_SignalSize1 =
                  pc_DataPoolList1->c_Elements.size();
              const uint32_t u32_SignalSize2 =
                  pc_DataPoolList2->c_Elements.size();

              oru32_RxMessageCount += static_cast<uint32_t>(
                  rc_MessageContainer.c_RxMessages.size());
              oru32_TxMessageCount += static_cast<uint32_t>(
                  rc_MessageContainer.c_TxMessages.size());
              if (opu32_SignalCount != NULL) {
                *opu32_SignalCount += u32_SignalSize1 + u32_SignalSize2;
              }
            } else {
              // In case of CANopen the active flag of each message must be
              // checked
              uint32_t u32_MessageCounter;
              // RX messages
              for (u32_MessageCounter = 0U;
                   u32_MessageCounter < rc_MessageContainer.c_RxMessages.size();
                   ++u32_MessageCounter) {
                const C_OscCanMessage &rc_Msg =
                    rc_MessageContainer.c_RxMessages[u32_MessageCounter];
                // The message must be active and in case of the device, the
                // device must be the matching communication partner for this
                // PDO
                if ((rc_Msg.q_CanOpenManagerMessageActive == true) &&
                    ((opc_CoDeviceInterfaceId ==
                      NULL) || // No CANopen device, or it must match
                     (rc_Msg.c_CanOpenManagerOwnerNodeIndex ==
                      *opc_CoDeviceInterfaceId))) {
              ...
              // TX messages
              for (u32_MessageCounter = 0U;
                   u32_MessageCounter < rc_MessageContainer.c_TxMessages.size();
                   ++u32_MessageCounter) {
                const C_OscCanMessage &rc_Msg =
                    rc_MessageContainer.c_TxMessages[u32_MessageCounter];
                // The message must be active and in case of the device, the
                // device must be the matching communication partner for this
                // PDO
                if ((rc_Msg.q_CanOpenManagerMessageActive == true) &&
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_SdUtil::h_GetCanOpenSignalObjectIndex(const uint32_t ou32_ObjectIndex,
                                        const uint32_t ou32_ObjectSubIndex) {
  const QString c_ObjectIndexString =
      QString::number(ou32_ObjectIndex, 16).toUpper();
  const QString c_ObjectSubIndexString =
      QString::number(ou32_ObjectSubIndex, 16).toUpper();
  const QString c_Retval = static_cast<QString>("%1sub%2")
                               .arg(c_ObjectIndexString)
                               .arg(c_ObjectSubIndexString);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::h_NodeIdToBeChanged(const uint32_t ou32_NodeIndex,
                                   const uint32_t ou32_InterfaceIndex,
                                   QWidget *const opc_Parent) {
  C_OscNode *const pc_CurrentNode =
      C_PuiSdHandler::h_GetInstance()->GetOscNode(ou32_NodeIndex);

  Q_ASSERT(pc_CurrentNode != NULL);

  if (pc_CurrentNode != NULL) {
    const C_OscNodeComInterfaceSettings *const pc_ComInterface =
        pc_CurrentNode->c_Properties.GetComInterface(
            C_OscSystemBus::E_Type::eCAN,
    ...
    if (pc_ComInterface != NULL) {
      if (pc_ComInterface->u8_NodeId == 0) {
        const QList<uint32_t> c_UsedProperties =
        ...
        const uint32_t u32_NewNodeId = C_SdUtil::h_GetNextFreeNodeProperty(
            pc_CurrentNode->c_Properties.c_ComInterfaces, c_UsedProperties, -1,
            true);
        QString c_Details;
        c_Details.append(
        ...
        if (c_MessageBox.Execute() == C_OgeWiCustomMessage::eYES) {
          C_OscNodeProperties c_CurrentNodeProperties =
              pc_CurrentNode->c_Properties;
          c_CurrentNodeProperties.c_ComInterfaces[ou32_InterfaceIndex]
              .u8_NodeId = static_cast<uint8_t>(u32_NewNodeId);
          C_PuiSdHandler::h_GetInstance()->SetOscNodeProperties(
              ou32_NodeIndex, c_CurrentNodeProperties);
        }
      }
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Write ethernet and can messages in the log file if the number of
 interfaces differs from the original
 *           stored value

   \param[in]      opc_NodeProperties        Stored interfaces after storing the
 program
   \param[in]      opc_DevDef                Node properties stored in Device
 definition file
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::mh_WriteEtherCanLogMessage(
    const C_OscNodeProperties *const opc_NodeProperties,
  ...
  if (static_cast<uint32_t>(opc_DevDef->u8_NumEthernetBusses) !=
    ...
    const QString c_Activity = "Device definition deviation";

    const QString c_Message =
        "Device definition deviation between original number of ethernet "
        "interfaces and new stored ethernet interfaces: " +
        QString::number(opc_DevDef->u8_NumEthernetBusses) + "; " +
        QString::number(opc_NodeProperties->GetEthernetInterfaces().size());

    osc_write_log_warning(c_Activity,
                          c_Message);
  }
  if (static_cast<uint32_t>(opc_DevDef->u8_NumCanBusses) !=
    ...
    const QString c_Message =
        "Device definition deviation between original number of can interfaces "
        "and new stored can interfaces: " +
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::mh_AdaptSignalToUnsignedType(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::mh_AdaptSignalMaxToUnsignedType(
    C_OscNodeDataPoolContent &orc_Content, QStringList &orc_AdaptationInfos,
    const uint16_t ou16_BitLength, const QString &orc_SignalName) {
  ...
  if (orc_Content != c_Max) {
    ...
    Q_ASSERT(C_SdNdeDpContentUtil::h_GetValueAsFloat64(
                 orc_Content, f64_MaxValPrev, 0UL) == C_NO_ERR);

    Q_ASSERT(C_SdNdeDpContentUtil::h_GetValueAsFloat64(c_Max, f64_MaxValNew,
                                                       0UL) == C_NO_ERR);
    orc_Content = c_Max;
    orc_AdaptationInfos.append(
        static_cast<QString>(
            "Max value of signal \"%1\" set from %2 to %3 because "
            "of change to unsigned type and auto min max setting.")
            .arg(orc_SignalName)
            .arg(f64_MaxValPrev)
            .arg(f64_MaxValNew));
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdUtil::mh_AdaptDataElementToUnsignedType(
    C_OscNodeDataPoolContent &orc_Content, QStringList &orc_AdaptationInfos,
    const QString &orc_ContentType, const QString &orc_SignalName) {
  ...
  if (f64_MinVal < 0.0) {
    ...
    orc_AdaptationInfos.append(
        static_cast<QString>("%1 of signal \"%2\" set from %3 to 0 because "
                             "of change to unsigned type.")
            .arg(orc_ContentType)
            .arg(orc_SignalName)
            .arg(f64_MinVal));
  }
