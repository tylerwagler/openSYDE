...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdTooltipUtil::h_GetToolTipContentMessage(
  ...
  if (orc_Message.c_Comment.isEmpty() == false) {
    c_ToolTipContent = orc_Message.c_Comment;
  ...
  c_ToolTipContent +=
      static_cast<QString>(static_cast<QString>("   ") + "Type: %1\n")
          .arg(c_Tmp);
  c_ToolTipContent +=
      static_cast<QString>(static_cast<QString>("   ") + "CAN ID: 0x%1\n")
          .arg(static_cast<QString>("%1")
  ...
  case C_OscCanMessage::eTX_METHOD_ON_EVENT:
    c_ToolTipContent += static_cast<QString>(static_cast<QString>("   ") +
                                             "Tx-Method: On Event\n");
    break;
  case C_OscCanMessage::eTX_METHOD_CAN_OPEN_TYPE_0:
    c_ToolTipContent +=
        static_cast<QString>(static_cast<QString>("   ") +
                             "Tx-Method: Type 0 - synchronous transmission "
                             "after next SYNC and change\n");
    break;
  case C_OscCanMessage::eTX_METHOD_CAN_OPEN_TYPE_1_TO_240:
    c_ToolTipContent += static_cast<QString>(
        static_cast<QString>("   ") + "Tx-Method: Type 1 to 240 - synchronous "
                                      "transmission after 1st to 240th SYNC\n");
    break;
  ...
  case C_OscCanMessage::eTX_METHOD_ON_CHANGE:
    ...
    c_ToolTipContent += static_cast<QString>(static_cast<QString>("   ") +
                                             "Not earlier than: %1\n")
                            .arg(orc_Message.u16_DelayTimeMs);
    c_ToolTipContent += static_cast<QString>(static_cast<QString>("   ") +
...
/*! \brief   Get string for tooltip content for signal.
   ...
   \param[in]  orc_Signal                 Signal information for which the
   ...
   \param[in]  orc_AutoMinMaxInformation  Optional auto min max information
   string which will get inserted after the signal properties heading (no "\n"
   at end necessary)
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdTooltipUtil::h_GetToolTipContentSignal(
    const C_OscCanSignal &orc_Signal, const C_OscCanMessage &orc_Message,
  ...
  if (oe_ProtocolType == C_OscCanProtocol::eCAN_OPEN) {
    ...
    c_ToolTipContent.append(
        QString::number(orc_Signal.u16_CanOpenManagerObjectDictionaryIndex,
                        16) +
  ...
  if (orc_DpListElement.c_Comment.isEmpty() == false) {
    c_ToolTipContent.append(orc_DpListElement.c_Comment);
    c_ToolTipContent.append("\n\n");
  }
  ...
  if (C_SdNdeDpContentUtil::h_GetValueAsFloat64(orc_DpListElement.c_MinValue,
  ...
  if (C_SdNdeDpContentUtil::h_GetValueAsFloat64(orc_DpListElement.c_MaxValue,
                                                f64_Value, 0UL) == C_NO_ERR) {
    c_ToolTipContent.append(QString::number(f64_Value));
  }
  c_ToolTipContent.append("\n");
  ...
  if (orc_DpListElement.c_DataSetValues.size() > 0UL) {
    ...
    if (C_SdNdeDpContentUtil::h_GetValueAsFloat64(
            orc_DpListElement.c_DataSetValues[0UL], f64_Value, 0UL) ==
        C_NO_ERR) {
      c_ToolTipContent.append(QString::number(f64_Value));
    }
  }
  ...
  if (orc_DpListElement.c_Unit.isEmpty() == false) {
    ...
    c_ToolTipContent.append(orc_DpListElement.c_Unit);
  }
  c_ToolTipContent.append("\n");
  ...
  c_ToolTipContent.append(C_SdTooltipUtil::h_ConvertTypeToNameSimplified(
      orc_DpListElement.c_MinValue.GetType()));
  c_ToolTipContent.append("\n");
  ...
  c_ToolTipContent.append(orc_Message.c_Name);
  c_ToolTipContent.append("\n");
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SdTooltipUtil::h_ConvertTypeToNameSimplified(
    const C_OscNodeDataPoolContent::E_Type &ore_Type) {
  QString c_Retval;
