            ...
            if (s32_Retval == C_NO_ERR)
            {
               ...
               if (c_StringXml.SelectNodeChild("core") == "core")
               {
                  if (c_StringXml.SelectNodeChild("data-pool") == "data-pool")
                  {
                     ...
                     s32_Retval = C_OscNodeDataPoolFiler::h_LoadDataPool(orc_OscContent, c_StringXml).value();
                  }
                  else
                  {
                     s32_Retval = C_CONFIG;
               ...
               if (c_StringXml.SelectNodeChild("lists") == "lists")
               {
                  ...
                  if (s32_Retval == C_NO_ERR)
                  {
                     ...
                     if (c_StringXml.SelectNodeChild("core") == "core")
                     {
                        if (c_StringXml.SelectNodeChild("lists") == "lists")
                        {
                           s32_Retval = C_OscNodeDataPoolFiler::h_LoadDataPoolLists(orc_OscContent,
                                                                                    c_StringXml).value();
                        }
                        else
                        {
         ...
         if (c_StringXml.SelectNodeChild("data-elements") == "data-elements")
         {
            ...
            if (s32_Retval == C_NO_ERR)
            {
               ...
               if (c_StringXml.SelectNodeChild("core") == "core")
               {
                  ...
                  {
                     //the project filers report std::error_code now; this class keeps the STW int32_t convention
                     s32_Retval = C_OscNodeDataPoolFiler::h_LoadDataPoolListElements(orc_OscContent,
                                                                                     c_StringXml).value();
                  }
                  else
                  {
...
int32_t C_SdClipBoardHelper::h_LoadIndicesFromString(std::vector<uint32_t> & orc_Indices, const QString & orc_Input)
{
   ...
   if (c_StringXml.SelectRoot() == "clip-board")
   {
      ...
      {
         std::string c_Node = c_StringXml.SelectNodeChild("index");
         while (c_Node == "index")
         {
   ...
   std::vector<std::vector<C_OscNodeDataPoolContent> > & orc_OscDataSetValues, const QString & orc_Input)
{
   ...
   if (c_StringXml.SelectRoot() == "clip-board")
   {
      if (c_StringXml.SelectNodeChild("core") == "core")
      {
         ...
         {
            //the project filers report std::error_code now; this class keeps the STW int32_t convention
            s32_Retval = C_OscNodeDataPoolFiler::h_LoadDataPoolListDataSets(orc_OscNames, c_StringXml).value();
            if (s32_Retval == C_NO_ERR)
            {
               //Return
               tgl_assert(c_StringXml.SelectNodeParent() == "core");
               ...
               {
                  std::string c_CurNodeDataSetValue = c_StringXml.SelectNodeChild("data-set-values");
                  ...
                  if (c_CurNodeDataSetValue == "data-set-values")
                  {
                     do
                     {
                        ...
                        if (s32_Retval == C_NO_ERR)
                        {
                           std::string c_CurNodeDataSetValueValue;
                                            ...
                                            std::vector<std::vector<bool> > & orc_OwnerIsTxFlag)
{
   ...
   if (c_StringXml.SelectRoot() == "clip-board")
   {
      if (c_StringXml.SelectNodeChild("core") == "core")
      {
         ...
         {
            //the project filers report std::error_code now; this class keeps the STW int32_t convention
            s32_Retval = C_OscNodeCommFiler::h_LoadNodeComMessages(orc_Messages, c_StringXml).value();
            if (s32_Retval == C_NO_ERR)
            {
               //Return
               tgl_assert(c_StringXml.SelectNodeParent() == "core");
               ...
               {
                  std::string c_CurrentNode = c_StringXml.SelectNodeChild("data-elements");
                              ...
                              if (c_StringXml.SelectNodeChild("parents") == "parents")
                              {
                                 ...
                                 if (c_CurrentNode == "message-parents")
                                 {
                                    do
                                    {
                                       ...
                                       std::string c_CurrentNode2 = c_StringXml.SelectNodeChild(
                                          "message-parent");
                                                          ...
                                                          std::vector<C_OscCanMessageIdentificationIndices> & orc_MessageIds)
{
   ...
   if (c_StringXml.SelectRoot() == "clip-board")
   {
      if (c_StringXml.SelectNodeChild("core") == "core")
      {
         ...
         {
            std::string c_CurrentNode = c_StringXml.SelectNodeChild("message-id");
            while (c_CurrentNode == "message-id")
            {
               ...
               {
                  //the project filers report std::error_code now; this class keeps the STW int32_t convention
                  if (C_OscNodeCommFiler::h_StringToCommunicationProtocol(
                         c_StringXml.GetNodeContent(), c_MessageId.e_ComProtocol).value() != C_NO_ERR)
                  {
                     s32_Retval = C_RANGE;
                  }
                  //Return
   ...
   if (c_StringXml.SelectRoot() == "opensyde-system-definition")
   {
      ...
      if (c_StringXml.SelectNodeChild("nodes-core") == "nodes-core")
      {
         ...
         s32_Retval = C_OscSystemDefinitionFiler::h_LoadNodes(orc_Data.c_OscNodes, c_StringXml,
                                                              C_OscSystemDefinition::hc_Devices, "", true,
                                                              false).value();
         if (s32_Retval == C_NO_ERR)
         {
            //Return
      ...
      {
         s32_Retval = C_OscSystemDefinitionFiler::h_LoadBuses(orc_Data.c_OscBuses, c_StringXml).value();
         if (s32_Retval == C_NO_ERR)
         {
            //Return
            tgl_assert(c_StringXml.SelectNodeParent() == "opensyde-system-definition");
                                                      ...
                                                      std::vector<C_PuiSdNodeCanSignal> & orc_UiSignals)
{
   ...
   if (c_StringXml.SelectRoot() == "clip-board")
   {
      if (c_StringXml.SelectNodeChild("core") == "core")
      {
         ...
         {
            //the project filers report std::error_code now; this class keeps the STW int32_t convention
            s32_Retval = C_OscNodeCommFiler::h_LoadNodeComSignals(orc_Signals, c_StringXml).value();
            if (s32_Retval == C_NO_ERR)
            {
               //Return
               tgl_assert(c_StringXml.SelectNodeParent() == "core");
