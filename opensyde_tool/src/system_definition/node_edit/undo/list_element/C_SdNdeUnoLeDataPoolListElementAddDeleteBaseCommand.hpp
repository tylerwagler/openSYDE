...
protected:
   ...
   void m_SetInitialData(const QList<stw::opensyde_core::C_OscNodeDataPoolListElement> & orc_OscContent,
                         const QList<C_PuiSdNodeDataPoolListElement> & orc_UiContent);
   void m_ReSelect(const QList<QList<uint32_t> > & orc_Items, const bool oq_ScrollToLast);

private:
   QList<uint32_t> mc_Indices;
   QList<stw::opensyde_core::C_OscNodeDataPoolListElement> mc_OscContent;
   QList<C_PuiSdNodeDataPoolListElement> mc_UiContent;
