...
{
   std::vector<stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token> c_Tokens;
...
bool C_SdNdeDalTriggerCheckHelper::mh_ParseTokens(const std::string & orc_Expression,
                                                  std::vector<stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token> & orc_Tokens,
                                                  std::string * const opc_ErrorDetails)
{
   return stw::opensyde_core::C_OscDataLoggerTriggerParser::h_Tokenize(orc_Expression, orc_Tokens, opc_ErrorDetails);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdNdeDalTriggerCheckHelper::mh_ReplaceChannelTokens(std::vector<stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token> & orc_Tokens,
                                                           std::string * const opc_ErrorDetails)
{
   bool q_IsValid = true;
   ...
   try
   {
      ...
      {
         const stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token & rc_Token = orc_Tokens[u32_ItToken];
         if (rc_Token.e_Type == stw::opensyde_core::C_OscDataLoggerTriggerParser::eCONSTANT_VALUE)
         {
            //Verify the literal is a number the rest of openSYDE can convert
            static_cast<void>(ToDoubleCompat(rc_Token.c_Text));
         }
      }
   }
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SdNdeDalTriggerCheckHelper::mh_CheckSyntax(const std::vector<stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token> & orc_Tokens,
                                                  std::string * const opc_ErrorDetails)
{
   return stw::opensyde_core::C_OscDataLoggerTriggerParser::h_CheckSyntax(orc_Tokens, opc_ErrorDetails);
}
...
bool C_SdNdeDalTriggerCheckHelper::mh_CheckTokens(const uint32_t ou32_NodeIndex,
                                                  const std::vector<stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token> & orc_Tokens,
                                                  std::string * const opc_ErrorDetails)
{
   bool q_IsValid = true;
   ...
   {
      const stw::opensyde_core::C_OscDataLoggerTriggerParser::C_Token & rc_Token = orc_Tokens[u32_ItToken];
      if (rc_Token.e_Type == stw::opensyde_core::C_OscDataLoggerTriggerParser::eCHANNEL)
      {
         const std::vector<std::string> c_Components = rc_Token.c_Components;
   ...
   const std::string & orc_Condition)
{
   ...
   for (std::string::const_iterator c_ItChar = c_Copy.cbegin(); c_ItChar != c_Copy.cend(); ++c_ItChar)
   {
      const bool q_IsPartOfVariable = C_OscUtils::h_CheckValidCeName(std::string(1, *c_ItChar), true) ||
                                      (*c_ItChar == '.');
