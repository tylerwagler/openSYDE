...
/*!
   ...
   Create a .c and .h file providing initialization structures for OSS CANopen
   init functions
...
/*! \brief  Create source files
   ...
   \param[in]       orc_IfWithCanOpenMan      vector with all interface indices,
   which contain a CANopen manager \param[in]       orc_ExportToolInfo
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportCanOpenInit::h_CreateSourceCode(
    const QString &orc_FilePath, const C_OscNode &orc_Node,
    const QByteArray &orc_IfWithCanOpenMan, const QString &orc_ExportToolInfo) {
  int32_t s32_Return;
  ...
  c_Data.append("/*!");
  c_Data.append("   \\file");
  c_Data.append("   \\brief       Application specific openSYDE initialization "
                "(Header file with interface)");
  c_Data.append("");
  ...
  c_Data.append("");
  c_Data.append("//Header files exporting application specific CANopen Manager "
                "interface configurations:");

  for (uint32_t u32_IfIt = 0; u32_IfIt < orc_IfWithCanOpenMan.size();
       ++u32_IfIt) {
    const uint32_t u32_InterfaceIndex =
        static_cast<uint32_t>(orc_IfWithCanOpenMan[u32_IfIt]);
    c_Data.append("#include \"osco_man_config_can" +
                  QString::number(u32_InterfaceIndex + 1U) + ".h\"");
  }
  ...
  for (uint32_t u32_IfIt = 0; u32_IfIt < orc_IfWithCanOpenMan.size();
    ...
    const uint32_t u32_InterfaceIndex =
        static_cast<uint32_t>(orc_IfWithCanOpenMan[u32_IfIt]);
    c_Data.append("#define OSCO_MAN_CONFIG_INIT_CONFIG_INDEX_CAN" +
                  QString::number(u32_InterfaceIndex + 1U) + " (" +
                  QString::number(u32_IfIt) + "U)");
  }
  ...
  mh_ComposeDefineNumTotal(c_DefineValue, orc_IfWithCanOpenMan, true,
                           c_Subject);
  // add comment to line
  ...
  mh_ComposeDefineNumTotal(c_DefineValue, orc_IfWithCanOpenMan, false,
                           c_Subject);
  // add comment to line
  ...
  c_Data.append(
      "#define OSCO_MAN_CONFIG_INIT_NUM_PDOS       "
      "(OSCO_MAN_CONFIG_INIT_NUM_TX_PDOS + OSCO_MAN_CONFIG_INIT_NUM_RX_PDOS)");
  ...
  c_Data.append("#define OSCO_MAN_CONFIG_INIT_NUM_SIGNALS    "
                "(OSCO_MAN_CONFIG_INIT_NUM_TX_SIGNALS + "
                "OSCO_MAN_CONFIG_INIT_NUM_RX_SIGNALS)");
  ...
  c_Data.append("extern const T_osco_man_manager_init_configuration "
                "gt_osco_man_InitConfiguration;");
  c_Data.append("");
  ...
  s32_Return =
      C_OscExportUti::h_SaveToFile(c_Data, orc_FilePath, h_GetFileName(), true);
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    c_Data.append("/*!");
    c_Data.append("   \\file");
    c_Data.append("   \\brief       Application specific openSYDE CANopen "
                  "Manager initialization (Source file with implementation)");
    c_Data.append("");
    c_Data.append(C_OscExportUti::h_GetCreationToolInfo(orc_ExportToolInfo));
    c_Data.append("*/");
    c_Data.append(C_OscExportUti::h_GetHeaderSeparator());
    ...
    c_Data.append("OSCO_MAN_CREATE_STATIC_DEFINITION_INSTANCE_DATA(mt_"
                  "OscoManInstanceData)");
    c_Data.append("");

    c_Data.append(
        "static const T_osco_man_manager_configuration * const "
        "mapt_OscoManConfigs[OSCO_MAN_CONFIG_INIT_NUM_CONFIGURATIONS] =");
    c_Data.append("{");
    // generate string with ProtocolConfiguration struct for each interface
    for (uint8_t u8_IfIt = 0; u8_IfIt < orc_IfWithCanOpenMan.size();
         ++u8_IfIt) {
      const QString c_InterfaceIndex = QString::number(
          static_cast<uint32_t>(orc_IfWithCanOpenMan[u8_IfIt]) + 1U);
      c_ProtocolConfig =
          "   &gt_osco_man_can" + c_InterfaceIndex + "_ProtocolConfiguration";
    ...
    c_Data.append("const T_osco_man_manager_init_configuration "
                  "gt_osco_man_InitConfiguration =");
    c_Data.append("{");
    ...
    s32_Return = C_OscExportUti::h_SaveToFile(c_Data, orc_FilePath,
                                              h_GetFileName(), false);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Composes a string which represents the value for "#define
 OSCO_MAN_CONFIG_INIT_NUM_T/RX_PDOS" or
 *          for "#define OSCO_MAN_CONFIG_INIT_NUM_TX_SIGNALS" depending on last
 parameter
   ...
   \param[in]       orc_IfWithCanOpenMan   vector with all interface indices,
 which contain a CANopen manager \param[in]       oq_IsTx                True:
 compose string for TX_PDOS \param[in]       orc_Subject            "subject"
 the string is composed for (can be "PDOS" or "SIGNALS")
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCanOpenInit::mh_ComposeDefineNumTotal(
    QString &orc_DefineValue, const QByteArray &orc_IfWithCanOpenMan,
  ...
  for (uint32_t u32_IfIt = 0; u32_IfIt < orc_IfWithCanOpenMan.size();
    ...
    orc_DefineValue += "OSCO_MAN_CAN" + c_InterfaceIndex + "_NUMBER_OF_" +
                       c_PdoType + "_" + orc_Subject;
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Composes a string which represents the value for "#define
   OSCO_MAN_CONFIG_INIT_NUM_DEVICES"
   ...
   \param[in,out]   orc_DefineValue        storage for string
   \param[in]       orc_IfWithCanOpenMan   vector with all interface indices,
   which contain a CANopen manager
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCanOpenInit::mh_ComposeDefineNumDevices(
    QString &orc_DefineValue, const QByteArray &orc_IfWithCanOpenMan) {
  for (uint32_t u32_IfIt = 0; u32_IfIt < orc_IfWithCanOpenMan.size();
       ++u32_IfIt) {
    const QString c_InterfaceIndex = QString::number(
