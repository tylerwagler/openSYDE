...
public:
  enum E_Linkage ///< Flag for Datapool-application-relation when generating
                 ///< source code files
  {
    eLOCAL = 0,   ///< Generate code for Datapool owner
    eREMOTE,      ///< Generate code for DPD owner
    eREMOTEPUBLIC ///< Generate code for public Datapool that is owned by
                  ///< another application
  };
  ...
  static int32_t h_CreateSourceCode(
...
protected:
  ...
  static int32_t mh_CreateHeaderFile(
      const QString &orc_ExportToolInfo, const QString &orc_Path,
      const C_OscNodeDataPool &orc_DataPool, const uint8_t ou8_DataPoolIndex,
  ...
  static void mh_AddIncludes(QStringList &orc_Data,
                             const C_OscNodeDataPool &orc_DataPool,
                             const bool oq_FileType);
  static int32_t mh_AddDefinesHeader(
      ...
      const C_OscNodeCodeExportSettings::E_Scaling oe_ScalingSupport);
  static void mh_AddDefinesImpl(QStringList &orc_Data,
                                const C_OscNodeDataPool &orc_DataPool,
                                const QString &orc_ProjectId,
                                const uint16_t ou16_GenCodeVersion);
  static void mh_AddTypes(QStringList &orc_Data,
                          const C_OscNodeDataPool &orc_DataPool,
  ...
  static void mh_AddGlobalVariables(QStringList &orc_Data,
                                    ...
                                    const bool oq_FileType,
                                    const E_Linkage oe_Linkage);
  static void mh_AddModuleGlobal(QStringList &orc_Data,
                                 const C_OscNodeDataPool &orc_DataPool,
                                 const uint16_t ou16_GenCodeVersion,
                                 const uint8_t ou8_ProcessId,
                                 const uint8_t ou8_DataPoolIndexRemote,
                                 const E_Linkage oe_Linkage);
  static void mh_AddImplementation(QStringList &orc_Data,
                                   const bool oq_FileType);
  static QString mh_GetType(const C_OscNodeDataPoolContent::E_Type oe_Type);
  static QString
  mh_GetElementValueString(const C_OscNodeDataPoolContent &orc_Value,
                           const C_OscNodeDataPoolContent::E_Type oe_Type,
                           const bool oq_IsArray);
  static QString
  mh_GetElementSize(const C_OscNodeDataPoolContent::E_Type oe_Type,
