...
//----------------------------------------------------------------------------------------------------------------------
QString
C_OscExportDataPool::h_GetFileName(const C_OscNodeDataPool &orc_DataPool) {
  // assemble filename: Datapool name + 'data_pool'
  return orc_DataPool.c_Name.toLower() + "_data_pool";
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Convert overall code structure version to parametrization code
   subversion.
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint16_t C_OscExportDataPool::h_ConvertOverallCodeVersion(
  ...
  case 5:
    u16_Return =
        ou16_GenCodeVersion -
        1; // -1 because version is always one lower than overall version
    break;
  case 6:
...
/*! \brief   Create source files
   ...
   specified for this version) \param[in] oe_ScalingSupport        Flag for data
   type and existence of factor, offset and scaling macros \param[in]
   ou8_DataPoolIndex        index of Datapool within local process \param[in]
   oe_Linkage               flag for linkage context \param[in]
   ou8_DataPoolIndexRemote  index of Datapool within remote process \param[in]
   ou8_ProcessId            ID of process owning this Datapool \param[in]
   orc_ExportToolInfo       information about calling executable (name +
   version)

   \return
   C_NO_ERR  Operation success
   C_RD_WR   Operation failure: cannot store files
   C_NOACT   Application has unknown code structure version or invalid linkage
   configuration C_CONFIG  Input data not suitable for code generation. Details
   will be written to OSC Log. Possible reasons:
             * Datapool list element factor negative or would be generated as
   zero
             * A list contains more than 255 Datasets
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportDataPool::h_CreateSourceCode(
    const QString &orc_Path, const uint16_t ou16_GenCodeVersion,
    ...
    const uint8_t ou8_ProcessId, const QString &orc_ExportToolInfo) {
  ...
  if (ou16_GenCodeVersion >
      C_OscNodeApplication::hu16_HIGHEST_KNOWN_CODE_VERSION) {
    s32_Retval = C_NOACT;
    osc_write_log_error("Creating source code",
                        "Did not generate code for Datapool \"" +
                            orc_DataPool.c_Name +
                            "\" because code format version is unknown.");
  }

  // make sure linkage is not eREMOTEPUBLIC if code format does not support this
  // feature
  if ((s32_Retval == C_NO_ERR) && (ou16_GenCodeVersion < 4U) &&
      (oe_Linkage == eREMOTEPUBLIC)) {
    s32_Retval = C_NOACT;
    osc_write_log_error("Creating source code",
                        "Did not generate code for Datapool \"" +
                            orc_DataPool.c_Name +
                            "\" because code format version does not support "
                            "public Datapools.");
  }
  ...
  if (s32_Retval == C_NO_ERR) {
    for (uint32_t u32_ListIndex = 0U;
         u32_ListIndex < orc_DataPool.c_Lists.size(); u32_ListIndex++) {
      if (orc_DataPool.c_Lists[u32_ListIndex].c_DataSets.size() > 255U) {
        s32_Retval = C_CONFIG;
        osc_write_log_error("Creating source code",
                            "Did not generate code for Datapool \"" +
                                orc_DataPool.c_Name + "\" because list \"" +
                                orc_DataPool.c_Lists[u32_ListIndex].c_Name +
  ...
  if (s32_Retval == C_NO_ERR) {
    // create header file
    s32_Retval = mh_CreateHeaderFile(
        orc_ExportToolInfo, orc_Path, orc_DataPool, ou8_DataPoolIndex,
        c_ProjectId, ou16_GenCodeVersion, oe_Linkage, oe_ScalingSupport);
  }

  // create implementation file
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = mh_CreateImplementationFile(
        orc_ExportToolInfo, orc_Path, orc_DataPool, c_ProjectId,
        ou16_GenCodeVersion, ou8_DataPoolIndexRemote, ou8_ProcessId,
        oe_Linkage);
  }
...
/*! \brief   Create header file
   ...
   (name + version) \param[in] orc_Path                 storage path for created
   file \param[in] orc_DataPool             Datapool configuration \param[in]
   ou8_DataPoolIndex        index of Datapool \param[in] orc_ProjectId project
   id for consistency check \param[in] ou16_GenCodeVersion      version of
   structure (generate code as specified for this version) \param[in] oe_Linkage
   flag for linkage context \param[in] oe_ScalingSupport        Flag for data
   type and existence of factor, offset and scaling macros
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportDataPool::mh_CreateHeaderFile(
  ...
  // add defines
  s32_Retval = mh_AddDefinesHeader(c_Data, orc_DataPool, ou8_DataPoolIndex,
                                   orc_ProjectId, ou16_GenCodeVersion,
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    mh_AddGlobalVariables(c_Data, orc_DataPool, ou16_GenCodeVersion,
                          mhq_IS_HEADER_FILE, oe_Linkage);

    // add function prototypes
    C_OscExportUti::h_AddProjIdFunctionPrototype(
        c_Data, mh_GetMagicName(orc_ProjectId, orc_DataPool));

    // add implementation
    mh_AddImplementation(c_Data, mhq_IS_HEADER_FILE);

    // finally save all stuff into the file
    s32_Retval = C_OscExportUti::h_SaveToFile(
        c_Data, orc_Path, h_GetFileName(orc_DataPool), true);
  }
...
/*! \brief   Create implementation file
   ...
   (name + version) \param[in] orc_Path                 storage path for created
   file \param[in] orc_DataPool             Datapool configuration \param[in]
   orc_ProjectId            project id for consistency check \param[in]
   ou16_GenCodeVersion      version of structure (generate code as specified for
   this version) \param[in] ou8_DataPoolIndexRemote  index of Datapool within
   remote process \param[in] ou8_ProcessId            ID of process owning this
   Datapool \param[in] oe_Linkage               flag for linkage context
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportDataPool::mh_CreateImplementationFile(
    const QString &orc_ExportToolInfo, const QString &orc_Path,
    const C_OscNodeDataPool &orc_DataPool, const QString &orc_ProjectId,
  ...
  // add header
  mh_AddHeader(orc_ExportToolInfo, c_Data, orc_DataPool,
  ...
  mh_AddModuleGlobal(c_Data, orc_DataPool, ou16_GenCodeVersion, ou8_ProcessId,
                     ou8_DataPoolIndexRemote, oe_Linkage);
  ...
  s32_Retval = C_OscExportUti::h_SaveToFile(c_Data, orc_Path,
                                            h_GetFileName(orc_DataPool), false);
...
/*! \brief   Add file header
   ...
   \param[in]  orc_ExportToolInfo       information about calling executable
   (name + version) \param[out] orc_Data                 converted data to
   string list \param[in]  orc_DataPool             Datapool configuration
   \param[in]  oq_FileType              .c or .h file selected
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddHeader(const QString &orc_ExportToolInfo,
  ...
  if (oq_FileType == mhq_IS_IMPLEMENTATION_FILE) {
    orc_Data.append("   \\brief       openSYDE Datapool definition (Source "
                    "file with constant definitions)");
    if (orc_DataPool.c_Comment.isEmpty() == false) {
      orc_Data.append("");
      orc_Data.append("   " + C_OscUtils::h_NiceifyStringForCeComment(
                                  orc_DataPool.c_Comment));
    }
    orc_Data.append("");
    orc_Data.append(C_OscExportUti::h_GetCreationToolInfo(orc_ExportToolInfo));
  } else {
    orc_Data.append("   \\brief       openSYDE Datapool definition (Header "
                    "file with constant and global definitions)");
    orc_Data.append("");
    orc_Data.append(C_OscExportUti::h_GetCreationToolInfo(orc_ExportToolInfo));
  }
  orc_Data.append("*/");
  ...
  if (oq_FileType == mhq_IS_HEADER_FILE) {
    const QString c_HeaderGuard = h_GetFileName(orc_DataPool).toUpper() + "H";
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddIncludes(QStringList &orc_Data,
                                         const C_OscNodeDataPool &orc_DataPool,
                                         const bool oq_FileType) {
  orc_Data.append("");
...
/*! \brief   Add defines for header file
   ...
   \param[in]  ou16_GenCodeVersion      version of structure (generate code as
   specified for this version) \param[in]  oe_Linkage               flag for
   linkage context \param[in]  oe_ScalingSupport        Flag for data type and
   existence of factor, offset and scaling macros
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportDataPool::mh_AddDefinesHeader(
  ...
  const QString c_DataPoolName = orc_DataPool.c_Name.toUpper();
  const QString c_MagicName = mh_GetMagicName(orc_ProjectId, orc_DataPool);
  ...
  orc_Data.append("#define " + c_DataPoolName + "_DATA_POOL_INDEX (" +
                  QString::number(ou8_DataPoolIndex) + "U)");
  orc_Data.append("");

  if ((oe_Linkage == eLOCAL) ||
      ((ou16_GenCodeVersion >= 4U) && (oe_Linkage == eREMOTEPUBLIC))) {
    uint16_t u16_ListIndex;
    orc_Data.append("///Index of lists");
    for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
         u16_ListIndex++) {
      const QString c_ListName =
          orc_DataPool.c_Lists[u16_ListIndex].c_Name.toUpper();
      orc_Data.append("#define " + c_DataPoolName + "_LIST_INDEX_" +
                      c_ListName + " (" + QString::number(u16_ListIndex) +
                      "U)");
    }
    orc_Data.append("#define " + c_DataPoolName + "_NUMBER_OF_LISTS " + "(" +
    ...
    for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
      ...
      const C_OscNodeDataPoolList &rc_List =
          orc_DataPool.c_Lists[u16_ListIndex];
      const QString c_ListName = rc_List.c_Name.toUpper();
      for (uint16_t u16_ElementIndex = 0U;
           u16_ElementIndex < rc_List.c_Elements.size(); u16_ElementIndex++) {
        const QString c_ElementName =
            rc_List.c_Elements[u16_ElementIndex].c_Name.toUpper();
        orc_Data.append("#define " + c_DataPoolName + "_ELEM_INDEX_" +
                        c_ListName + "_" + c_ElementName + " (" +
                        QString::number(u16_ElementIndex) + "U)");
      }
      orc_Data.append("#define " + c_DataPoolName + "_" + c_ListName +
                      "_NUMBER_OF_ELEMENTS " + "(" +
    ...
    for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
      ...
      if (rc_List.c_Elements.size() > 0) {
        for (uint16_t u16_DataSetIndex = 0U;
             u16_DataSetIndex < rc_List.c_DataSets.size(); u16_DataSetIndex++) {
          const QString c_DataSetName =
              rc_List.c_DataSets[u16_DataSetIndex].c_Name.toUpper();
          orc_Data.append("#define " + c_DataPoolName + "_DATA_SET_INDEX_" +
                          c_ListName + "_" + c_DataSetName + " (" +
    ...
    if (oe_ScalingSupport != C_OscNodeCodeExportSettings::eNONE) {
      ...
      for (u16_ListIndex = 0U; (u16_ListIndex < orc_DataPool.c_Lists.size()) &&
        ...
        const C_OscNodeDataPoolList &rc_List =
            orc_DataPool.c_Lists[u16_ListIndex];
        const QString c_ListName = rc_List.c_Name.toUpper();
        const QString c_FloatType =
            (oe_ScalingSupport == C_OscNodeCodeExportSettings::eFLOAT64) ? ""
                                                                         : "F";
        for (uint16_t u16_ElementIndex = 0U;
             u16_ElementIndex < rc_List.c_Elements.size(); u16_ElementIndex++) {
          const QString c_ElementName =
          ...
          if (oe_ScalingSupport == C_OscNodeCodeExportSettings::eFLOAT64) {
            c_Factor = C_OscExportUti::h_FloatToStrGe(
                rc_List.c_Elements[u16_ElementIndex].f64_Factor,
                &q_InfOrNanFactor);
            c_Offset = C_OscExportUti::h_FloatToStrGe(
                rc_List.c_Elements[u16_ElementIndex].f64_Offset,
                &q_InfOrNanOffset);
          } else {
          ...
          if (((rc_List.c_Elements[u16_ElementIndex].f64_Factor <= 0.0) ||
               (c_Factor == "0.0")) ||
            ...
            osc_write_log_error(
                ...
                "Did not generate code because factor of element \"" +
                    orc_DataPool.c_Name + "::" + rc_List.c_Name +
                    "::" + rc_List.c_Elements[u16_ElementIndex].c_Name +
          ...
          if (q_InfOrNanOffset == true) {
            osc_write_log_error(
                "Creating source code",
                "Did not generate code because offset of element \"" +
                    orc_DataPool.c_Name + "::" + rc_List.c_Name +
                    "::" + rc_List.c_Elements[u16_ElementIndex].c_Name +
                    "\" would be generated as 'inf' or 'nan'.");
            s32_Retval = C_CONFIG;
          ...
          orc_Data.append("#define " +
                          mh_GetElementScaleDefine(c_DataPoolName, c_ListName,
                                                   c_ElementName, true) +
      ...
      for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
        ...
        for (uint16_t u16_ElementIndex = 0U;
          ...
          const C_OscNodeDataPoolListElement &rc_Element =
              rc_List.c_Elements[u16_ElementIndex];
          const QString c_ElementName = rc_Element.c_Name.toUpper();
          QString c_ElementStruct;
          QString c_Makro;

          if ((ou16_GenCodeVersion >= 4U) && (oe_Linkage == eREMOTEPUBLIC)) {
            c_ElementStruct =
                "gpt_" + orc_DataPool.c_Name + "_DataPoolValues->";
          } else {
            c_ElementStruct = "gt_" + orc_DataPool.c_Name + "_DataPoolValues.";
          }

          c_ElementStruct += "t_" + rc_List.c_Name + "Values." +
                             C_OscExportUti::h_GetTypePrefix(
          ...
          // setter
          c_Makro = "#define " + c_DataPoolName + "_SET_VALUE_FROM_SCALED_" +
                    c_ListName + "_" + c_ElementName + "(";
          if (rc_Element.GetArray() == true) {
            c_Makro += "INDEX, ";
          }
          c_Makro += "SCALED_VALUE) (" + c_ElementStruct +
                     " = (SCALED_VALUE - " +
                     mh_GetElementScaleDefine(c_DataPoolName, c_ListName,
                                              c_ElementName, false) +
                     ") / " +
          ...
          // getter
          c_Makro = "#define " + c_DataPoolName + "_GET_SCALED_VALUE_" +
                    c_ListName + "_" + c_ElementName + "(";
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddDefinesImpl(
    QStringList &orc_Data, const C_OscNodeDataPool &orc_DataPool,
    const QString &orc_ProjectId, const uint16_t ou16_GenCodeVersion) {
  const QString c_MagicName = mh_GetMagicName(orc_ProjectId, orc_DataPool);
  ...
  if (ou16_GenCodeVersion >= 3U) {
    ...
    orc_Data.append(
        "#if OSY_DPA_DATA_POOL_DEFINITION_VERSION != 0x" +
        QString::number(C_OscExportDataPool::h_ConvertOverallCodeVersion(
                            ou16_GenCodeVersion),
                        16)
            .toUpper() +
        "U");
    orc_Data.append("///if compilation fails here the openSYDE library version "
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddTypes(QStringList &orc_Data,
                                      const C_OscNodeDataPool &orc_DataPool,
                                      const bool oq_FileType,
                                      const E_Linkage oe_Linkage) {
  ...
  if ((oq_FileType == mhq_IS_HEADER_FILE) &&
    ...
    bool q_AtLeastOneElement =
        false; // defensive (actually no file generation for empty datapools)

    for (uint16_t u16_ListIndex = 0U;
         u16_ListIndex < orc_DataPool.c_Lists.size(); u16_ListIndex++) {
      const C_OscNodeDataPoolList &rc_List =
          orc_DataPool.c_Lists[u16_ListIndex];
      if (rc_List.c_Elements.size() != 0) {
        ...
        for (uint16_t u16_ElementIndex = 0U;
          ...
          const C_OscNodeDataPoolListElement &rc_Element =
              rc_List.c_Elements[u16_ElementIndex];
          const QString c_TypePrefix = C_OscExportUti::h_GetTypePrefix(
              rc_Element.GetType(), rc_Element.GetArray());
          const QString c_ElementType = mh_GetType(rc_Element.GetType());
          if (rc_Element.GetArray() == true) {
            const QString c_String = QString::number(rc_Element.GetArraySize());
            orc_Data.append(
                "   " + c_ElementType + " " + c_TypePrefix + "_" +
                rc_Element.c_Name + "[" + c_String + "]; /* " +
        ...
        orc_Data.append("} T_" + orc_DataPool.c_Name + "_" + rc_List.c_Name +
                        "_Values;");
        orc_Data.append("");
      }
    }
    ...
    if (q_AtLeastOneElement == true) {
      ...
      for (uint16_t u16_ListIndex = 0U;
        ...
        if (orc_DataPool.c_Lists[u16_ListIndex].c_Elements.size() != 0) {
          const QString c_ListName = orc_DataPool.c_Lists[u16_ListIndex].c_Name;
          orc_Data.append("   T_" + orc_DataPool.c_Name + "_" + c_ListName +
                          "_Values t_" + c_ListName + "Values;");
        }
      }
...
/*! \brief   Add global variables
   ...
   \param[in]  orc_DataPool             Datapool configuration
   \param[in]  ou16_GenCodeVersion      version of structure (generate code as
   specified for this version) \param[in]  oq_FileType              .c or .h
   file selected \param[in]  oe_Linkage               flag for linkage context
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddGlobalVariables(
    QStringList &orc_Data, const C_OscNodeDataPool &orc_DataPool,
  ...
  for (uint16_t u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
       u16_ListIndex++) {
    if (orc_DataPool.c_Lists[u16_ListIndex].c_Elements.size() != 0) {
      q_AtLeastOneElement = true;
      break;
  ...
  if (oq_FileType == mhq_IS_HEADER_FILE) {
    if (q_AtLeastOneElement == true) {
      if (oe_Linkage == eLOCAL) {
        ...
        orc_Data.append("extern T_" + orc_DataPool.c_Name +
                        "_DataPoolValues gt_" + orc_DataPool.c_Name +
                        "_DataPoolValues;");
        orc_Data.append("");
      }

      if (oe_Linkage == eREMOTEPUBLIC) {
        orc_Data.append("///Pointer to global Datapool RAM variables in other "
                        "application:");
        orc_Data.append("extern T_" + orc_DataPool.c_Name +
                        "_DataPoolValues * gpt_" + orc_DataPool.c_Name +
                        "_DataPoolValues;");
        orc_Data.append("");
      }
    }
    ...
    orc_Data.append("extern const T_osy_dpa_data_pool gt_" +
                    orc_DataPool.c_Name + "_DataPool;");
  } else {
    if (q_AtLeastOneElement == true) {
      if (oe_Linkage == eLOCAL) {
        ...
        orc_Data.append(c_SafeRamData + "T_" + orc_DataPool.c_Name +
                        "_DataPoolValues gt_" + orc_DataPool.c_Name +
                        "_DataPoolValues;");
      }
      ...
      if (oe_Linkage == eREMOTEPUBLIC) {
        orc_Data.append("///Pointer to global Datapool RAM variables in other "
                        "application:");
        orc_Data.append("OSY_DPA_SAFE_RAM_DATA_PRIVATE_ZERO T_" +
                        orc_DataPool.c_Name + "_DataPoolValues * gpt_" +
...
/*! \brief   Add module global variables and function prototypes
   ...
   \param[in]   orc_DataPool              Datapool configuration
   \param[in]   ou16_GenCodeVersion       version of structure (generate code as
   specified for this version) \param[in]   ou8_ProcessId             ID of
   process owning this Datapool \param[in]   ou8_DataPoolIndexRemote   index of
   Datapool within remote process (only used for remote Datapools) \param[in]
   oe_Linkage                flag for linkage context
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddModuleGlobal(
    QStringList &orc_Data, const C_OscNodeDataPool &orc_DataPool,
  ...
  const QString c_DataPoolName = orc_DataPool.c_Name;
  QString c_String;
  uint32_t u32_HashValue = 0U;
  ...
  if (oe_Linkage == eLOCAL) {
    ...
    orc_Data.append("OSY_DPA_CREATE_STATIC_DP_DEFINITION_INSTANCE_DATA(mt_"
                    "DpDefinitionInstanceData)");
    orc_Data.append("");
    ...
    for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
      ...
      if (c_List.c_Elements.size() != 0) {
        ...
        orc_Data.append("static const T_" + orc_DataPool.c_Name + "_" +
                        c_List.c_Name + "_Values mt_" + c_List.c_Name +
                        "MinValues =");
        orc_Data.append("{");
        for (u16_ElementIndex = 0U; u16_ElementIndex < c_List.c_Elements.size();
             u16_ElementIndex++) {
          const C_OscNodeDataPoolListElement &rc_Element =
              c_List.c_Elements[u16_ElementIndex];
        ...
        if (c_List.c_DataSets.size() > 0) {
          ...
          orc_Data.append("static const T_" + c_DataPoolName + "_" +
                          c_List.c_Name + "_Values mat_" + c_List.c_Name +
                          "DataSetValues [" + c_DataPoolName.toUpper() + "_" +
                          c_ListName.toUpper() + "_NUMBER_OF_DATA_SETS] =");
          orc_Data.append("{");

          for (u8_DataSetIndex = 0U; u8_DataSetIndex < c_List.c_DataSets.size();
               u8_DataSetIndex++) {
            orc_Data.append("   {");
            for (u16_ElementIndex = 0U;
                 u16_ElementIndex < c_List.c_Elements.size();
                 u16_ElementIndex++) {
              C_OscNodeDataPoolListElement c_Element =
                  c_List.c_Elements[u16_ElementIndex];
              c_String = "      ";
              c_String += mh_GetElementValueString(
                  c_Element.c_DataSetValues[u8_DataSetIndex],
                  c_Element.GetType(), c_Element.GetArray());
              if (u16_ElementIndex == (c_List.c_Elements.size() - 1U)) {
                c_String += " ";
              } else {
                c_String += ",";
              ...
              c_String += ("   ///< " + c_Element.c_Name + " (" +
                           C_OscUtils::h_NiceifyStringForCeComment(
                               c_Element.c_Comment) +
                           ")");
              orc_Data.append(c_String);
            }
          ...
          c_ListName = c_List.c_Name;
          orc_Data.append("static const T_osy_dpa_data_set mat_" +
                          c_List.c_Name + "DataSetTable[" +
                          c_DataPoolName.toUpper() + "_" +
                          c_ListName.toUpper() + "_NUMBER_OF_DATA_SETS] =");
          orc_Data.append("{");

          for (u8_DataSetIndex = 0U; u8_DataSetIndex < c_List.c_DataSets.size();
               u8_DataSetIndex++) {
            c_String = "   { &mat_" + c_List.c_Name + "DataSetValues[" +
                       QString::number(u8_DataSetIndex) + "] }";
    ...
    for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
      ...
      if (rc_List.c_Elements.size() != 0) {
        ...
        orc_Data.append(
            "static const T_osy_dpa_element_definition mat_DataPool" +
            rc_List.c_Name + "Elements[" + c_DataPoolName.toUpper() + "_" +
            c_ListName.toUpper() + "_NUMBER_OF_ELEMENTS] =");
        orc_Data.append("{");
        ...
        for (u16_ElementIndex = 0U;
          ...
          const C_OscNodeDataPoolListElement &rc_Element =
              rc_List.c_Elements[u16_ElementIndex];
          const QString c_ElementSize =
              mh_GetElementSize(rc_Element.GetType(), rc_Element.GetArraySize(),
                                rc_Element.GetArray());
          const QString c_TriggerEvent =
              (rc_Element.q_DiagEventCall == true) ? "1U" : "0U";
          const QString c_ElementName = C_OscExportUti::h_GetElementCeName(
              rc_Element.c_Name, rc_Element.GetArray(), rc_Element.GetType());
          QString c_ElementType = mh_GetType(rc_Element.GetType());
          if (rc_Element.GetArray() == true) {
            c_ElementType = "a" + c_ElementType;
          }
          c_String = "   { OSY_DPA_ELEMENT_TYPE_" + c_ElementType.toUpper() +
                     ", " + c_TriggerEvent + ", " + c_ElementSize + "U, &gt_" +
                     c_DataPoolName + "_DataPoolValues.t_" + c_ListName +
                     "Values." + c_ElementName + ", &mt_" + c_ListName +
                     "MinValues." + c_ElementName + ", &mt_" + c_ListName +
                     "MaxValues." + c_ElementName + " }";
    ...
    orc_Data.append(
        "static const T_osy_dpa_list_definition mat_DataPoolLists[" +
        c_DataPoolName.toUpper() + "_NUMBER_OF_LISTS] =");
    orc_Data.append("{");
    ...
    for (u16_ListIndex = 0U; u16_ListIndex < orc_DataPool.c_Lists.size();
      ...
      c_ListName = rc_List.c_Name;
      c_String = "   { ";
      c_String += c_DataPoolName.toUpper() + "_" + c_ListName.toUpper() +
                  "_NUMBER_OF_ELEMENTS, ";
      c_String += c_DataPoolName.toUpper() + "_" + c_ListName.toUpper() +
                  "_NUMBER_OF_DATA_SETS, ";
      ...
      if (rc_List.c_Elements.size() == 0) {
        ...
        if ((orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) ||
          ...
          c_String += "0x" +
                      QString::number(
                          static_cast<int64_t>(rc_List.u32_NvmStartAddress), 16)
                          .toUpper() +
                      "U, ";
        } else {
        ...
        c_String +=
            "sizeof(T_" + c_DataPoolName + "_" + c_ListName + "_Values), ";
        c_String += "&mat_DataPool" + c_ListName + "Elements[0], ";
        c_String += "&gt_" + c_DataPoolName + "_DataPoolValues.t_" +
                    c_ListName + "Values, ";
    ...
    if ((orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) ||
      ...
      // we have either a HALC or DIAG DP: technically this results in a
      // DIAGNOSIS DP
      orc_Data.append("   OSY_DPA_DATA_POOL_TYPE_DIAGNOSIS,");
    }
    orc_Data.append(
        "   { 0x" + QString::number(orc_DataPool.au8_Version[0], 16).toUpper() +
        "U, 0x" + QString::number(orc_DataPool.au8_Version[1], 16).toUpper() +
        "U, 0x" + QString::number(orc_DataPool.au8_Version[2], 16).toUpper() +
        "U }," + " ///< Datapool definition version V" +
    ...
    orc_Data.append(
        "   0x" +
        QString::number(static_cast<int64_t>(u32_HashValue), 16).toUpper() +
    ...
    if ((orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) ||
      ...
      orc_Data.append(
          "   0x" +
          QString::number(
              static_cast<int64_t>(orc_DataPool.u32_NvmStartAddress), 16)
              .toUpper() +
          "U,  ///< NVM start address");
  ...
  if (ou16_GenCodeVersion < 4U) {
    if (oe_Linkage == eLOCAL) {
      ...
      c_String += "Can be used to request information about remote Datapool "
                  "from another process:";
      orc_Data.append(c_String);
      orc_Data.append("static const T_osy_dpa_remote_data_pool_info "
                      "mt_RemoteDataPoolInfo =");
      orc_Data.append("{");
      orc_Data.append("   " + QString::number(ou8_ProcessId) +
                      "U, ///< Identification ID of remote server providing "
                      "information about the Datapool");
      orc_Data.append(
          "   " + QString::number(ou8_DataPoolIndexRemote) +
    ...
    if (oe_Linkage !=
        eLOCAL) // structure was removed for local Datapools since version 4
    {
      orc_Data.append("///Information about process and Datapool:");
      orc_Data.append("static const T_osy_dpa_remote_data_pool_info "
                      "mt_RemoteDataPoolInfo =");
      ...
      case eLOCAL:
        ...
        break;
      case eREMOTE:
        orc_Data.append("   NULL,  ///< Pointer only relevant for Datapool "
                        "with public scope");
        break;
      case eREMOTEPUBLIC:
        orc_Data.append("   (void**)&gpt_" + orc_DataPool.c_Name +
                        "_DataPoolValues,  ///< Pointer to pointer to struct "
                        "containing all Datapool values");
        break;
      default:
        Q_ASSERT(false);
        break;
  ...
  if ((ou16_GenCodeVersion >= 5U) && (oe_Linkage == eLOCAL) &&
    ...
    c_String = "OSY_DPA_CREATE_STATIC_DP_INSTANCE_DATA_WITH_BUFFER(mt_"
               "DpInstanceData, &mt_DataPoolDefinition, " +
  ...
  orc_Data.append("const T_osy_dpa_data_pool gt_" + c_DataPoolName +
                  "_DataPool =");
  orc_Data.append("{");
  orc_Data.append("   OSY_DPA_DATA_POOL_MAGIC,  ///< identification of valid "
                  "DP definition");
  ...
  if (ou16_GenCodeVersion >= 4U) {
    ...
    c_String += "OSY_DPA_DATA_POOL_LINKAGE_" +
                C_OscExportDataPool::mh_ConvertLinkageToString(oe_Linkage) +
                ",  ///< Datapool linkage";
  } else {
    if (oe_Linkage == eLOCAL) {
  ...
  case eREMOTEPUBLIC:
    orc_Data.append(
        "   &mt_RemoteDataPoolInfo,  ///< info about remote Datapool");
    break;
  default:
    Q_ASSERT(false);
    break;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportDataPool::mh_AddImplementation(QStringList &orc_Data,
                                               const bool oq_FileType) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportDataPool::mh_GetType(
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportDataPool::mh_GetElementValueString(
  ...
  if (oq_IsArray == false) {
    ...
    case C_OscNodeDataPoolContent::eUINT8: ///< Data type unsigned 8 bit integer
      ...
      break;
    case C_OscNodeDataPoolContent::eUINT16: ///< Data type unsigned 16 bit
                                            ///< integer
      c_String += QString::number(orc_Value.GetValueU16()) + "U";
      break;
    case C_OscNodeDataPoolContent::eUINT32: ///< Data type unsigned 32 bit
                                            ///< integer
      c_String += QString::number(orc_Value.GetValueU32()) + "UL";
      break;
    case C_OscNodeDataPoolContent::eUINT64: ///< Data type unsigned 64 bit
                                            ///< integer
    ...
    case C_OscNodeDataPoolContent::eSINT32: ///< Data type signed 32 bit integer
      ...
      if (s32_Value == std::numeric_limits<int32_t>::min()) {
        ...
        c_String +=
            QString::number(std::numeric_limits<int32_t>::min() + 1) + "L - 1";
      } else {
    ...
    case C_OscNodeDataPoolContent::eSINT64: ///< Data type signed 64 bit integer
      ...
      if (s64_Value == std::numeric_limits<int64_t>::min()) {
        // workaround for GCC warning when reaching lowest possible value
        c_String +=
            QString::number(std::numeric_limits<int64_t>::min() + 1) + "LL - 1";
      } else {
        c_String += QString::number(orc_Value.GetValueS64()) + "LL";
      }
      break;
    case C_OscNodeDataPoolContent::eFLOAT32: ///< Data type 32 bit floating
                                             ///< point
      c_String += C_OscExportUti::h_FloatToStrGe(orc_Value.GetValueF32()) + "F";
      break;
    case C_OscNodeDataPoolContent::eFLOAT64: ///< Data type 64 bit floating
                                             ///< point
    ...
    c_String = "{ ";
    for (uint32_t u32_ArrayIndex = 0U; u32_ArrayIndex < u32_ArraySize;
         u32_ArrayIndex++) {
      switch (oe_Type) {
      case C_OscNodeDataPoolContent::eUINT8: ///< Data type unsigned 8 bit
                                             ///< integer
        c_String +=
            QString::number(orc_Value.GetValueArrU8Element(u32_ArrayIndex)) +
            "U";
        break;
      case C_OscNodeDataPoolContent::eUINT16: ///< Data type unsigned 16 bit
                                              ///< integer
        c_String +=
            QString::number(orc_Value.GetValueArrU16Element(u32_ArrayIndex)) +
            "U";
        break;
      case C_OscNodeDataPoolContent::eUINT32: ///< Data type unsigned 32 bit
                                              ///< integer
        c_String +=
            QString::number(orc_Value.GetValueArrU32Element(u32_ArrayIndex)) +
            "UL";
        break;
      case C_OscNodeDataPoolContent::eUINT64: ///< Data type unsigned 64 bit
                                              ///< integer
        c_String +=
            QString::number(orc_Value.GetValueArrU64Element(u32_ArrayIndex)) +
            "ULL";
        break;
      case C_OscNodeDataPoolContent::eSINT8: ///< Data type signed 8 bit integer
        c_String +=
            QString::number(orc_Value.GetValueArrS8Element(u32_ArrayIndex));
        break;
      case C_OscNodeDataPoolContent::eSINT16: ///< Data type signed 16 bit
                                              ///< integer
        c_String +=
            QString::number(orc_Value.GetValueArrS16Element(u32_ArrayIndex));
        break;
      case C_OscNodeDataPoolContent::eSINT32: ///< Data type signed 32 bit
                                              ///< integer
        s32_Value = orc_Value.GetValueArrS32Element(u32_ArrayIndex);
        if (s32_Value == std::numeric_limits<int32_t>::min()) {
          // workaround for GCC warning when reaching lowest possible value
          c_String += QString::number(std::numeric_limits<int32_t>::min() + 1) +
                      "L - 1";
        } else {
          c_String +=
              QString::number(orc_Value.GetValueArrS32Element(u32_ArrayIndex)) +
              "L";
        }
        break;
      case C_OscNodeDataPoolContent::eSINT64: ///< Data type signed 64 bit
                                              ///< integer
        s64_Value = orc_Value.GetValueArrS64Element(u32_ArrayIndex);
        if (s64_Value == std::numeric_limits<int64_t>::min()) {
          // workaround for GCC warning when reaching lowest possible value
          c_String += QString::number(std::numeric_limits<int64_t>::min() + 1) +
                      "LL - 1";
        } else {
          c_String +=
              QString::number(orc_Value.GetValueArrS64Element(u32_ArrayIndex)) +
              "LL";
        }
        break;
      case C_OscNodeDataPoolContent::eFLOAT32: ///< Data type 32 bit floating
                                               ///< point
        c_String += C_OscExportUti::h_FloatToStrGe(
                        orc_Value.GetValueArrF32Element(u32_ArrayIndex)) +
                    "F";
        break;
      case C_OscNodeDataPoolContent::eFLOAT64: ///< Data type 64 bit floating
                                               ///< point
        c_String += C_OscExportUti::h_FloatToStrGe(
            orc_Value.GetValueArrF64Element(u32_ArrayIndex));
        break;
      default:
        Q_ASSERT(false);
        break;
...
/*! \brief   Get element size according to data type
   ...
   \param[in] oe_Type                 data type (uint8, sint8, ...)
   \param[in] ou32_ArraySize          size of array (if ou8_DataType means an
   array) \param[in] oq_IsArray              data type is an array

   \return
   element size as string
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportDataPool::mh_GetElementSize(
    const C_OscNodeDataPoolContent::E_Type oe_Type,
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportDataPool::mh_ConvertLinkageToString(
    const C_OscExportDataPool::E_Linkage oe_Linkage) {
  QString c_Return;
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_OscExportDataPool::mh_GetMagicName(const QString &orc_ProjectId,
                                     const C_OscNodeDataPool &orc_DataPool) {
  const QString c_MagicName =
      orc_DataPool.c_Name.toUpper() + "_PROJECT_ID_" + orc_ProjectId;
...
/*! \brief  Utility for getting define string of factor value constant.
   ...
   \param[in]       oq_Factor          true: get string for factor; else: get
   string for offset
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportDataPool::mh_GetElementScaleDefine(
    const QString &orc_DataPoolName, const QString &orc_ListName,
    const QString &orc_ElementName, const bool oq_Factor) {
  const QString c_Type = (oq_Factor == true) ? "FACTOR" : "OFFSET";
  const QString c_Return = orc_DataPoolName + "_SCALING_" + c_Type + "_" +
                           orc_ListName + "_" + orc_ElementName;
