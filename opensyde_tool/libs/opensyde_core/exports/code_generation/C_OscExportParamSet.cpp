...
int32_t C_OscExportParamSet::h_CreateParameterSetImage(
  ...
  if (rc_Application.u16_GenCodeVersion >
    ...
    osc_write_log_error("Creating PSI file",
                        ...
                        "file format version is unknown.");
  }

  // plausibility check if number of Datablock output paths matches number of
  // files we want to write
  if ((orc_Node.c_HalcConfig.e_SafetyMode ==
       C_OscHalcDefBase::eTWO_LEVELS_WITH_DROPPING) ||
      (orc_Node.c_HalcConfig.e_SafetyMode ==
  ...
  if (s32_Retval != C_NO_ERR) {
    osc_write_log_error("Creating PSI file",
                        "Could not generate files because number of output "
                        "files is not correctly configured.");
  }
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_Node.c_HalcConfig.e_SafetyMode !=
      ...
      s32_Retval =
          mh_FillPsiStructure(orc_Node, q_IS_SAFE, ou16_ApplicationIndex,
                              c_RawNodeSafe, c_IntNodeSafe);

      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscExportParamSet::mh_WriteParameterSetImage(
            c_RawNodeSafe, c_IntNodeSafe, q_IS_SAFE, rc_Application, orc_Path,
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_Node.c_HalcConfig.e_SafetyMode !=
      ...
      s32_Retval =
          mh_FillPsiStructure(orc_Node, q_IS_SAFE, ou16_ApplicationIndex,
                              c_RawNodeNonSafe, c_IntNodeNonSafe);

      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscExportParamSet::mh_WriteParameterSetImage(
            c_RawNodeNonSafe, c_IntNodeNonSafe, q_IS_SAFE, rc_Application,
            orc_Path, orc_Files, orc_ExportToolName, orc_ExportToolVersion);
      }
    }
  }
...
/*! \brief  Fill PSI data structure with data from safe or non-safe Datapools
   ...
   \param[in]   orc_Node               Node as information source for exported
   code
   \param[in]   oq_IsSafe              Flag if to generate safe or non safe
   structure
   \param[in]   ou16_ApplicationIndex  Index of Data Block within
   orc_Node.c_Applications
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportParamSet::mh_FillPsiStructure(
    const C_OscNode &orc_Node, const bool oq_IsSafe,
  ...
  const C_OscNodeDataPool *const pc_FirstDataPool =
      orc_Node.GetHalDataPoolConst(oq_IsSafe);
  ...
  if (pc_FirstDataPool != NULL) {
    ...
    if (s32_Retval != C_NO_ERR) {
      osc_write_log_error("Creating PSI file",
                          "Could not generate files because of unexpected "
                          "number of lists or datasets or list "
                          "elements in Datapool " +
                              orc_Node.c_Properties.c_Name +
                              "::" + pc_FirstDataPool->c_Name);
    }

    // add data of all Datapools of specified safety
    for (uint32_t u32_ItDataPool = 0U;
         (u32_ItDataPool < orc_Node.c_DataPools.size()) &&
         (s32_Retval == C_NO_ERR);
         ++u32_ItDataPool) {
      const C_OscNodeDataPool &rc_SdDataPool =
          orc_Node.c_DataPools[u32_ItDataPool];
      ...
      if ((rc_SdDataPool.s32_RelatedDataBlockIndex == ou16_ApplicationIndex) &&
        ...
        // fill raw entries
        s32_Retval = mh_FillRawEntries(rc_SdDataPool, c_ConfigRawBytes,
                                       orc_RawNode.c_Entries);
      }

      if (s32_Retval != C_NO_ERR) {
        osc_write_log_error("Creating PSI file",
                            "Could not generate files because of unexpected "
                            "number of lists or unexpected NVM "
                            "size in Datapool " +
                                orc_Node.c_Properties.c_Name +
                                "::" + rc_SdDataPool.c_Name);
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportParamSet::mh_FillInterpretedDatapool(
  ...
  orc_SdDataPool.CalcGeneratedDefinitionHash(
      orc_IntDataPool.c_DataPoolInfo.u32_DataPoolCrc);
  orc_IntDataPool.c_DataPoolInfo.u32_NvmStartAddress =
      orc_SdDataPool.u32_NvmStartAddress;
  orc_IntDataPool.c_DataPoolInfo.u32_NvmSize = orc_SdDataPool.u32_NvmSize;
  (void)memcpy(&orc_IntDataPool.c_DataPoolInfo.au8_Version[0],
               &orc_SdDataPool.au8_Version[0], 3);

  // Add all lists
  for (uint16_t u16_ListIndex = 0U;
       u16_ListIndex < orc_SdDataPool.c_Lists.size(); u16_ListIndex++) {
    const C_OscNodeDataPoolList &rc_SdList =
        orc_SdDataPool.c_Lists[u16_ListIndex];
    ...
    for (uint16_t u16_ElementIndex = 0U;
         u16_ElementIndex < rc_SdList.c_Elements.size(); u16_ElementIndex++) {
      const C_OscNodeDataPoolListElement rc_SdElement =
          rc_SdList.c_Elements[u16_ElementIndex];
...
/*! \brief  Fill raw entries with data
   ...
   Add 4 raw entries:
      1. CRC and HALC NVM configuration values from the only Dataset of list
   "configuration" 2.-4. CRC and zeroes

   \param[in]      orc_SdDataPool      Datapool definition from system
   definition (might be a "HAL Datapool copy")
   \param[in]      orc_ConfigRawBytes  Raw bytes containing HALC NVM
   configuration data
   \param[in,out]  orc_Entries         Raw entries to be filled with data

   \return
   C_NO_ERR    Success
   C_CONFIG    number of Datapool lists or datasets or list elements not as
   expected
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportParamSet::mh_FillRawEntries(
  ...
  if (orc_SdDataPool.c_Lists.size() ==
      4) // exactly 4 expected: configuration, inputs, outputs, statuses
  {
    for (uint16_t u16_ListIndex = 0U;
         (u16_ListIndex < orc_SdDataPool.c_Lists.size()) &&
         (s32_Retval == C_NO_ERR);
         u16_ListIndex++) {
      C_OscParamSetRawEntry c_RawEntry;

      c_RawEntry.u32_StartAddress =
          orc_SdDataPool.c_Lists[u16_ListIndex].u32_NvmStartAddress;
      ...
      if (u16_ListIndex == 0) {
        ...
        c_RawEntry.c_Bytes.resize(
            orc_SdDataPool.c_Lists[u16_ListIndex].u32_NvmSize, 0);
      }
...
C_OscExportParamSet::mh_GetFileInfo(const QString &orc_ExportToolName,
  ...
  c_Info.c_ProjectName =
      "N/A"; // info not available as we only load system definition
  c_Info.c_ProjectVersion =
      "N/A"; // info not available as we only load system definition
...
/*! \brief  Write parameter set image
   ...
   \param[in]      oq_IsSafe              Flag for safe vs. non-safe
   configuration
   \param[in]      orc_DataBlock          Data Block definition
   \param[in]      orc_Path               Storage path for created file
   \param[in,out]  orc_Files              List of all exported file names (with
   absolute or relative path)
   \param[in]      orc_ExportToolName     Name of calling executable
   \param[in]      orc_ExportToolVersion  Version of calling executable
   ...
   C_CONFIG    Internal data invalid (e.g. incorrect number of lists or datasets
   in HALC NVM Datapool) C_RD_WR     Problems accessing file system
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportParamSet::mh_WriteParameterSetImage(
  ...
  if (s32_Retval != C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      s32_Retval = C_RD_WR;
      osc_write_log_error("Writing PSI file",
                          "Could not write to file \"" + c_Path + "\".");
    }
  }
...
int32_t C_OscExportParamSet::mh_InsertCrc16(std::vector<uint8_t> &orc_Bytes) {
  ...
  if (orc_Bytes.size() > 2) {
    ...
    stw::scl::C_SclChecksums::CalcCRC16(
        &orc_Bytes[2], static_cast<uint32_t>(orc_Bytes.size()) - 2, u16_Crc);
...
int32_t C_OscExportParamSet::mh_GetConfigurationRawBytes(
  ...
  if (orc_SdDataPool.c_Lists.size() > 0) {
    ...
    for (uint32_t u32_ItElement = 0U;
         (u32_ItElement < orc_SdDataPool.c_Lists[0].c_Elements.size()) &&
         (s32_Retval == C_NO_ERR);
         ++u32_ItElement) {
    ...
    // make sure that number of bytes equals specified size of list and fill
    // eventually remaining space with zeroes
