...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportCommunicationStack::h_GetFileName(
  ...
  const QString c_Text =
      "comm_" + mh_GetProtocolNameByType(ore_ProtocolType).toLower() + "_can" +
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportCommunicationStack::h_GetConfigurationName(
    const uint8_t ou8_InterfaceIndex,
    const C_OscCanProtocol::E_Type &ore_ProtocolType) {
  const QString c_Name =
      "gt_comm_" + mh_GetProtocolNameByType(ore_ProtocolType).toLower() +
      "_can" + QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint16_t C_OscExportCommunicationStack::h_ConvertOverallCodeVersion(
...
/*! \brief   Create source files
   ...
   \param[in]  ou16_ApplicationIndex   index of programmable application within
   orc_Node.c_Applications \param[in]  ou8_InterfaceIndex      index of CAN
   interface \param[in]  ou32_DatapoolIndex      index of Datapool \param[in]
   ore_Protocol            protocol type (CL2, ECeS, ECoS) \param[in]
   orc_ExportToolInfo      information about calling executable (name + version)

   \return
   C_NO_ERR    Operation success
   C_NOACT     Application is not of type ePROGRAMMABLE_APPLICATION or has
   unknown code structure version C_RD_WR     Operation failure: cannot store
   files C_CONFIG    Protocol or Datapool not available in node for interface or
   application index out of range Message definition does not contain at least
   one message with signals C_RANGE     Application index out of range
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportCommunicationStack::h_CreateSourceCode(
  ...
  if (s32_Retval == C_NO_ERR) {
    const C_OscCanProtocol *const pc_ComProtocol =
        orc_Node.GetCanProtocolConst(ore_Protocol, ou32_DatapoolIndex);
    const C_OscNodeDataPool *const pc_DataPool =
        orc_Node.GetComDataPoolConst(ore_Protocol, ou32_DatapoolIndex);

    if ((pc_ComProtocol != NULL) && (pc_DataPool != NULL)) {
      // check whether there is at least one message with signals defined;
      // otherwise fail; KFXTCSWRSCC_474
      bool q_NoSignals = true;
      const C_OscCanMessageContainer &rc_Messages =
          pc_ComProtocol->c_ComMessages[ou8_InterfaceIndex];

      for (uint16_t u16_MessageIndex = 0U;
           u16_MessageIndex < rc_Messages.c_TxMessages.size();
           u16_MessageIndex++) {
      ...
      if (q_NoSignals == true) {
        for (uint16_t u16_MessageIndex = 0U;
             u16_MessageIndex < rc_Messages.c_RxMessages.size();
             u16_MessageIndex++) {
      ...
      if (q_NoSignals == false) {
        ...
        // create header file
        s32_Retval = mh_CreateHeaderFile(orc_ExportToolInfo, orc_Path,
                                         c_Application, *pc_ComProtocol,
                                         ou8_InterfaceIndex, c_ProjectId);

        // create implementation file
        if (s32_Retval == C_NO_ERR) {
          s32_Retval = mh_CreateImplementationFile(
              orc_ExportToolInfo, orc_Path, c_Application, *pc_ComProtocol,
              *pc_DataPool, ou8_InterfaceIndex, c_ProjectId);
        }
      } else {
        osc_write_log_error("Creating source code",
                            "No messages with signals exist for specified "
                            "communication protocol " +
                                mh_GetProtocolNameByType(ore_Protocol) +
                                " for interface index " +
                                QString::number(ou8_InterfaceIndex));
        s32_Retval = C_CONFIG;
      }
    } else {
      osc_write_log_error("Creating source code",
                          "Protocol definition or Datapool does not exist for "
...
/*! \brief   Create header file
   ...
   \param[in]  orc_ExportToolInfo   information about calling executable (name +
   version) \param[in]  orc_Path             storage path for created file
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportCommunicationStack::mh_CreateHeaderFile(
  ...
  // add header
  mh_AddHeader(orc_ExportToolInfo, c_Data, ou8_InterfaceIndex,
               orc_ComProtocol.e_Type, mhq_IS_HEADER_FILE);
  ...
  // add defines
  mh_AddDefines(c_Data, orc_ComProtocol.c_ComMessages[ou8_InterfaceIndex],
                ou8_InterfaceIndex, orc_ComProtocol.e_Type, orc_ProjectId,
                orc_Applicaton.u16_GenCodeVersion, mhq_IS_HEADER_FILE);
  ...
  c_Data.append(
      "extern const T_osy_com_protocol_configuration " +
  ...
  s32_Retval = C_OscExportUti::h_SaveToFile(
      c_Data, orc_Path,
      h_GetFileName(ou8_InterfaceIndex, orc_ComProtocol.e_Type), true);
  return s32_Retval;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportCommunicationStack::mh_CreateImplementationFile(
    const QString &orc_ExportToolInfo, const QString &orc_Path,
    const C_OscNodeApplication &orc_Applicaton,
  ...
  if ((C_OscCanProtocol::h_GetComListIndex(orc_DataPool, ou8_InterfaceIndex,
    ...
    const bool q_TxMessagesPresent =
        (orc_ComProtocol.c_ComMessages[ou8_InterfaceIndex].c_TxMessages.size() >
         0)
            ? true
            : false;
    const bool q_RxMessagesPresent =
        (orc_ComProtocol.c_ComMessages[ou8_InterfaceIndex].c_RxMessages.size() >
         0)
            ? true
            : false;
    const C_OscCanMessageContainer &rc_Messages =
        orc_ComProtocol.c_ComMessages[ou8_InterfaceIndex];
    bool q_MessageWithoutSignalsPresent = false;

    for (uint16_t u16_Message = 0U;
         u16_Message < rc_Messages.c_TxMessages.size(); u16_Message++) {
    ...
    if (q_MessageWithoutSignalsPresent == false) {
      for (uint16_t u16_Message = 0U;
           u16_Message < rc_Messages.c_RxMessages.size(); u16_Message++) {
    ...
    // add header
    mh_AddHeader(orc_ExportToolInfo, c_Data, ou8_InterfaceIndex,
                 orc_ComProtocol.e_Type, mhq_IS_IMPLEMENTATION_FILE);

    // add includes
    mh_AddCeIncludes(
        c_Data, orc_DataPool, ou8_InterfaceIndex, orc_ComProtocol.e_Type,
    ...
    mh_AddDefines(c_Data, orc_ComProtocol.c_ComMessages[ou8_InterfaceIndex],
                  ou8_InterfaceIndex, orc_ComProtocol.e_Type, orc_ProjectId,
                  orc_Applicaton.u16_GenCodeVersion,
    ...
    mh_AddCeModuleGlobal(c_Data, orc_DataPool.q_IsSafety,
                         orc_ComProtocol.c_ComMessages[ou8_InterfaceIndex],
                         ou8_InterfaceIndex, orc_ComProtocol.e_Type,
                         orc_Applicaton.u16_GenCodeVersion, u32_TxListIndex,
                         u32_RxListIndex);

    // add global variables
    mh_AddCeGlobalVariables(c_Data, orc_DataPool.c_Name, ou8_InterfaceIndex,
                            orc_ComProtocol.e_Type, q_TxMessagesPresent,
                            q_RxMessagesPresent);
    ...
    osc_write_log_error(
        ...
        "Datapool does not exist for specified communication protocol" +
            mh_GetProtocolNameByType(orc_ComProtocol.e_Type) +
            " with interface index " + QString::number(ou8_InterfaceIndex));
    s32_Retval = C_CONFIG;
  }
...
/*! \brief   Add file header
   ...
   (name + version) \param[in,out]  orc_Data            converted data to string
   list \param[in]      ou8_InterfaceIndex  index of interface \param[in]
   ore_Protocol        protocol type (CL2, ECeS, ECoS) \param[in] oq_FileType .c
   or .h file selected
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddHeader(
    const QString &orc_ExportToolInfo, QStringList &orc_Data,
    const uint8_t ou8_InterfaceIndex,
  ...
  if (oq_FileType == mhq_IS_IMPLEMENTATION_FILE) {
    orc_Data.append("   \\brief       openSYDE Communication stack data "
                    "definition (Source file with constant definitions)");
    orc_Data.append("");
    orc_Data.append(
        "   Defines the communication configuration for protocol type " +
    ...
    orc_Data.append(
        "   \\brief       openSYDE Communication stack data definition (Header "
  ...
  if (oq_FileType == mhq_IS_HEADER_FILE) {
    const QString c_HeaderGuard =
        h_GetFileName(ou8_InterfaceIndex, ore_Protocol).toUpper() + "H";
...
/*! \brief   Add includes into C file
   ...
   \param[in]      oq_NullRequired     true: definition of NULL required in .c
   file
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddCeIncludes(
    QStringList &orc_Data, const C_OscNodeDataPool &orc_DataPool,
    const uint8_t ou8_InterfaceIndex,
    const C_OscCanProtocol::E_Type &ore_Protocol, const bool oq_NullRequired) {
  const QString c_ProtocolName =
      mh_GetProtocolNameByType(ore_Protocol).toLower();
  ...
  orc_Data.append("#include \"" +
                  h_GetFileName(ou8_InterfaceIndex, ore_Protocol) + ".h\"");
  orc_Data.append("");
...
/*! \brief   Add defines
   ...
   \param[in,out]  orc_Data               converted data to string list
   \param[in]      orc_ComMessage         communication protocol message
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddDefines(
    QStringList &orc_Data, const C_OscCanMessageContainer &orc_ComMessage,
    const uint8_t ou8_InterfaceIndex,
    const C_OscCanProtocol::E_Type &ore_Protocol, const QString &orc_ProjectId,
    const uint16_t ou16_GenCodeVersion, const bool oq_FileType) {
  const QString c_ProtocolName =
      mh_GetProtocolNameByType(ore_Protocol).toUpper();
  const QString c_MagicName =
      mh_GetMagicName(orc_ProjectId, ou8_InterfaceIndex, ore_Protocol);
  ...
  if (oq_FileType == mhq_IS_HEADER_FILE) {
    ...
    for (uint16_t u16_MessageIndex = 0U;
         u16_MessageIndex < orc_ComMessage.c_TxMessages.size();
         u16_MessageIndex++) {
      const QString c_Name =
          orc_ComMessage.c_TxMessages[u16_MessageIndex].c_Name.toUpper();
      orc_Data.append(
          "#define COMM_" + c_ProtocolName + "_CAN" +
          QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
          "_TX_MSG_INDEX_" + c_Name + " (" + QString::number(u16_MessageIndex) +
          "U)");
    }
    orc_Data.append(
        "#define COMM_" + c_ProtocolName + "_CAN" +
    ...
    for (uint16_t u16_MessageIndex = 0U;
         u16_MessageIndex < orc_ComMessage.c_RxMessages.size();
         u16_MessageIndex++) {
      const QString c_Name =
          orc_ComMessage.c_RxMessages[u16_MessageIndex].c_Name.toUpper();
      orc_Data.append(
          "#define COMM_" + c_ProtocolName + "_CAN" +
          QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
          "_RX_MSG_INDEX_" + c_Name + " (" + QString::number(u16_MessageIndex) +
          "U)");
    }
    orc_Data.append(
        "#define COMM_" + c_ProtocolName + "_CAN" +
    ...
    if (ou16_GenCodeVersion >= 2U) {
      ...
      for (uint16_t u16_MessageIndex = 0U;
        ...
        const C_OscCanMessage &rc_Message =
            orc_ComMessage.c_TxMessages[u16_MessageIndex];
        ...
        if (rc_Message.IsMultiplexed() == true) {
          ...
          // Sort for deterministic code generation output
          QList<uint16_t> c_SortedValues = c_MultiplexerValues.values();
          std::sort(c_SortedValues.begin(), c_SortedValues.end());
          for (const uint16_t u16_Value : c_SortedValues) {
            orc_Data.append(
                "#define COMM_" + c_ProtocolName + "_CAN" +
                QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) +
                                1U) +
                "_TX_MSG_MUX_INDEX_" + c_Name + "_VALUE_" +
                QString::number(u16_Value) + " (" +
                QString::number(u16_ValueIndex) + "U)");
            u16_ValueIndex++;
          }
      ...
      for (uint16_t u16_MessageIndex = 0U;
           ...
           u16_MessageIndex++) {
        const C_OscCanMessage &rc_Message =
            orc_ComMessage.c_RxMessages[u16_MessageIndex];
        ...
        if (rc_Message.IsMultiplexed() == true) {
          ...
          for (const uint16_t u16_Value : c_SortedValues) {
            orc_Data.append(
                ...
                QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) +
                                1U) +
                "_RX_MSG_MUX_INDEX_" + c_Name + "_VALUE_" +
                QString::number(u16_Value) + " (" +
                QString::number(u16_ValueIndex) + "U)");
            u16_ValueIndex++;
          }
    ...
    if (ou16_GenCodeVersion >= 2U) {
      ...
      orc_Data.append(
          ...
          QString::number(
              C_OscExportCommunicationStack::h_ConvertOverallCodeVersion(
                  ou16_GenCodeVersion),
              16)
              .toUpper() +
          "U");
...
/*! \brief   Add module global variables and function prototypes
   ...
   \param[in]      ore_Protocol           protocol type (CL2, ECeS, ECoS)
   \param[in]      ou16_GenCodeVersion    version of structure (generate code as
   specified for this version) \param[in]      ou32_TxListIndex       Datapool
   list index for Tx messages \param[in]      ou32_RxListIndex       Datapool
   list index for Rx messages
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddCeModuleGlobal(
    QStringList &orc_Data, const bool oq_SafeData,
    const C_OscCanMessageContainer &orc_ComMessage,
    const uint8_t ou8_InterfaceIndex,
    const C_OscCanProtocol::E_Type &ore_Protocol,
    const uint16_t ou16_GenCodeVersion, const uint32_t ou32_TxListIndex,
  ...
  if (oq_SafeData == true) {
    // status data needs to be placed in safe memory as the driver deals with
  ...
  if (orc_ComMessage.c_TxMessages.size() > 0) {
    if (ou16_GenCodeVersion >= 2U) {
      orc_Data.append(
          "static " + c_SafeRamData +
          "T_osy_com_message_status mat_StatusTx[COMM_" + c_ProtocolName +
          "_CAN" +
          QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
          "_NUMBER_OF_TX_MSGS];");
    } else {
      orc_Data.append(
          "static " + c_SafeRamData +
          "T_osy_com_message_status_private mat_StatusTx[COMM_" +
          c_ProtocolName + "_CAN" +
          QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
          "_NUMBER_OF_TX_MSGS];");
    }
  ...
  if (orc_ComMessage.c_RxMessages.size() > 0) {
    if (ou16_GenCodeVersion >= 2U) {
      orc_Data.append(
          "static " + c_SafeRamData +
          "T_osy_com_message_status mat_StatusRx[COMM_" + c_ProtocolName +
          "_CAN" +
          QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
          "_NUMBER_OF_RX_MSGS];");
    } else {
      orc_Data.append(
          "static " + c_SafeRamData +
          "T_osy_com_message_status_private mat_StatusRx[COMM_" +
          c_ProtocolName + "_CAN" +
          QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
          "_NUMBER_OF_RX_MSGS];");
    }
  ...
  if (ou16_GenCodeVersion >= 2U) {
    ...
    if (orc_ComMessage.c_TxMessages.size() > 0) {
      orc_Data.append(
          "static " + c_SafeRamData +
          "T_osy_com_message_mux_status mat_StatusMuxTxMessages[" +
    ...
    if (orc_ComMessage.c_RxMessages.size() > 0) {
      orc_Data.append(
          "static " + c_SafeRamData +
          "T_osy_com_message_mux_status mat_StatusMuxRxMessages[" +
  ...
  if (orc_ComMessage.c_TxMessages.size() > 0) {
    ...
    mh_AddSignalDefinitions(orc_Data, ou32_TxListIndex,
                            orc_ComMessage.c_TxMessages, ou16_GenCodeVersion);
  }

  if (orc_ComMessage.c_RxMessages.size() > 0) {
    orc_Data.append("///Rx signal definitions");
    mh_AddSignalDefinitions(orc_Data, ou32_RxListIndex,
                            orc_ComMessage.c_RxMessages, ou16_GenCodeVersion);
  }
  ...
  if (orc_ComMessage.c_TxMessages.size() > 0) {
    ...
    mh_AddMessageDefinitions(orc_Data, ou8_InterfaceIndex, ore_Protocol,
                             orc_ComMessage.c_TxMessages, ou16_GenCodeVersion,
                             true);
  }
  ...
  orc_Data.append(C_OscExportUti::h_GetSectionSeparator(
      "Module Global Function Prototypes"));
  orc_Data.append("");
}
...
/*! \brief   Add global variables
   ...
   \param[in]      ou8_InterfaceIndex     index of interface
   \param[in]      ore_Protocol           protocol type (CL2, ECeS, ECoS)
   \param[in]      oq_TxMessagesPresent   true: There is at least one Tx message
   defined \param[in]      oq_RxMessagesPresent   true: There is at least one Rx
   message defined
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddCeGlobalVariables(
  ...
  orc_Data.append("const T_osy_com_protocol_configuration " +
                  ...
                  " =");
  orc_Data.append("{");
  orc_Data.append("   " + QString::number(ou8_InterfaceIndex) +
                  "U,  ///< selected CAN channel");
  orc_Data.append("   OSY_COM_PROTOCOL_TYPE_" + c_ProtocolName +
                  ",  ///< protocol type for this stack");
  orc_Data.append("   {");
  orc_Data.append("      OSY_COM_CONFIG_MAGIC,  ///< identification of valid "
                  "comm configuration");
  orc_Data.append(
      "      OSY_COM_CONFIG_DEFINITION_VERSION,  ///< configuration version");
  orc_Data.append(
      "      COMM_" + c_ProtocolName + "_CAN" +
      QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
  ...
  if (oq_TxMessagesPresent == true) {
    ...
    orc_Data.append(
        "      NULL,  ///< definition of Tx messages (none defined)");
  }
  if (oq_RxMessagesPresent == true) {
    ...
    orc_Data.append(
        "      NULL,  ///< definition of Rx messages (none defined)");
  }
  if (oq_TxMessagesPresent == true) {
  ...
  orc_Data.append("   },");
  orc_Data.append("   OSY_COM_" + c_ProtocolName +
                  "_DRIVER  ///< pointer to protocol specific driver");
  orc_Data.append("};");
  orc_Data.append("");
}
...
/*! \brief   Add signal definition section to C file
   ...
   signals \param[in]      orc_Messages           definition of messages
   \param[in]      ou16_GenCodeVersion    version of structure (generate code as
   specified for this version)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddSignalDefinitions(
    QStringList &orc_Data, const uint32_t ou32_SignalListIndex,
    const QList<C_OscCanMessage> &orc_Messages,
    const uint16_t ou16_GenCodeVersion) {
  for (uint16_t u16_MessageIndex = 0U; u16_MessageIndex < orc_Messages.size();
       u16_MessageIndex++) {
    const C_OscCanMessage &rc_Message = orc_Messages[u16_MessageIndex];

    // Only create of signal table(s) for messages with signals.
    // For multiplexed messages we have at least one signal by definition;
    // otherwise it could not be multiplexed. So the check will cover both types
    // of messages.
    if (rc_Message.c_Signals.size() > 0) {
      ...
      if (ou16_GenCodeVersion >= 2U) {
        ...
        if (rc_Message.IsMultiplexed(&u32_MultiplexerIndex) == true) {
          QMap<int32_t, QList<C_OscCanSignal>> c_MuxedSignalsPerValue;
          QList<C_OscCanSignal> c_NonMuxedSignals;

          // get signals grouped by mux value
          mh_GroupSignalsByMuxValue(rc_Message, u32_MultiplexerIndex,
                                    c_MuxedSignalsPerValue, c_NonMuxedSignals);

          // Special case: Multiplexed message without any multiplexed signals.
          if (c_MuxedSignalsPerValue.size() == 0) // no muxed signals ...
          {
            const C_OscCanSignal &rc_MultiplexerSignal =
                rc_Message.c_Signals[u32_MultiplexerIndex];
            ...
            orc_Data.append("static const T_osy_com_signal_definition mat_" +
                            rc_Message.c_Name + "[" +
                            QString::number(c_Signals.size()) + "] =");
            orc_Data.append("{");
            // add signals
            mh_ConvertSignalsToStrings(orc_Data, c_Signals,
                                       ou32_SignalListIndex, true);
            orc_Data.append("};");
            orc_Data.append("");
          } else {
            QMap<int32_t, QList<C_OscCanSignal>>::const_iterator c_ItValue;

            // add signal definitions to data string list
            for (c_ItValue = c_MuxedSignalsPerValue.begin();
                 c_ItValue != c_MuxedSignalsPerValue.end(); ++c_ItValue) {
              orc_Data.append("static const T_osy_com_signal_definition mat_" +
                              rc_Message.c_Name + "Value" +
                              QString::number(c_ItValue.key()) + "[" +
                              QString::number(c_ItValue.value().size() +
                                              c_NonMuxedSignals.size()) +
                              "] =");
              orc_Data.append("{");
              // add multiplexer+multiplexed signals
              mh_ConvertSignalsToStrings(
                  orc_Data, c_MuxedSignalsPerValue[c_ItValue.key()],
                  ou32_SignalListIndex, (c_NonMuxedSignals.size() == 0));
              // add non-muxed signals
              mh_ConvertSignalsToStrings(orc_Data, c_NonMuxedSignals,
                                         ou32_SignalListIndex, true);
              orc_Data.append("};");
              orc_Data.append("");
            }
      ...
      if (q_AddNonMux == true) {
        ...
        orc_Data.append("{");
        mh_ConvertSignalsToStrings(orc_Data, rc_Message.c_Signals,
                                   ou32_SignalListIndex, true);
        orc_Data.append("};");
        orc_Data.append("");
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddMessageMuxDefinitions(
  ...
  orc_Data.append(
      "static const T_osy_com_message_mux_definition mat_MessagesMux" +
      orc_TxRxString + "[" +
      QString::number(mh_CountMuxMessages(orc_Messages)) + "] =");

  orc_Data.append("{");

  for (uint16_t u16_MessageIndex = 0U; u16_MessageIndex < u16_MessageCount;
       u16_MessageIndex++) {
    QString c_Text;
    const C_OscCanMessage &rc_Message = orc_Messages[u16_MessageIndex];
    uint32_t u32_MultiplexerIndex;
    ...
    if (rc_Message.IsMultiplexed(&u32_MultiplexerIndex) == true) {
      ...
      // Special case: Multiplexed message without any multiplexed signals. Add
      // dummy-mux value "0".
      if (c_MuxedSignalsPerValue.size() == 0) // no muxed signals ...
      {
        c_Text = "   { 0U, " + QString::number(1U + c_NonMuxedSignals.size()) +
                 "U, " + "&mat_" + rc_Message.c_Name + "[0] }";
        ...
        for (c_ItValue = c_MuxedSignalsPerValue.begin();
             c_ItValue != c_MuxedSignalsPerValue.end(); ++c_ItValue) {
          c_Text = "   { " + QString::number(c_ItValue.key()) + "U, " +
                   QString::number(c_ItValue.value().size() +
                                   c_NonMuxedSignals.size()) +
                   "U, " + "&mat_" + rc_Message.c_Name + "Value" +
                   QString::number(c_ItValue.key()) + "[0] }";
          if ((u16_MessageIndex != (u16_MessageCount - 1U)) ||
              (u16_MuxCount !=
               static_cast<uint16_t>((c_MuxedSignalsPerValue.size() - 1)))) {
            c_Text += ",";
          }
          orc_Data.append(c_Text);
          u16_MuxCount++;
      ...
      // if number of signals is zero then add NULL instead of pointer to the
      // signal table muxed message cannot have zero signals so this only
      // affects non-muxed messages
      const uint16_t u16_NumberOfSignals =
          static_cast<uint16_t>(rc_Message.c_Signals.size());
      ...
      c_Text = "   { 0U, " + QString::number(u16_NumberOfSignals) + "U, " +
               c_SignalTable + " }";
      if (u16_MessageIndex != (u16_MessageCount - 1U)) {
        c_Text += ",";
      }
...
/*! \brief   Add message definition section to C file
   ...
   \param[in]      orc_Messages           definition of messages
   \param[in]      ou16_GenCodeVersion    version of structure (generate code as
   specified for this version) \param[in]      oq_Tx                  true:
   messages are Tx, false: messages are Rx
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_AddMessageDefinitions(
  ...
  uint16_t u16_MessageCountWithMuxOffset =
      0; // necessary for V2 mux message count
  ...
  orc_Data.append(
      "static const T_osy_com_message_definition mat_Messages" + c_TxRxString +
      "[COMM_" + c_ProtocolName + "_CAN" +
      QString::number(static_cast<uint32_t>(ou8_InterfaceIndex) + 1U) +
  ...
  for (uint16_t u16_MessageIndex = 0U; u16_MessageIndex < u16_MessageCount;
       u16_MessageIndex++) {
    const C_OscCanMessage &rc_Message = orc_Messages[u16_MessageIndex];
    const QString c_Trigger =
        mh_GetTransmissionTriggerNameByType(rc_Message.e_TxMethod);
    ...
    if (oq_Tx == true) {
      // delay time not used for Rx messages and only applicable for change
      // triggered transmissions
      u16_DelayTime =
          (rc_Message.e_TxMethod == C_OscCanMessage::eTX_METHOD_ON_CHANGE)
              ? rc_Message.u16_DelayTimeMs
              : 0U;
    } else {
      // message counter gap is not relevant for Tx messages
      if (ore_Protocol == C_OscCanProtocol::eECES) {
        if (rc_Message.u32_CycleTimeMs != 0) {
          u32_MessageCounterGap =
              ((rc_Message.u32_TimeoutMs / rc_Message.u32_CycleTimeMs) - 1U);
        } else {
    ...
    c_Text = "   { 0x" +
             QString::number(static_cast<uint32_t>(rc_Message.u32_CanId), 16)
                 .toUpper() +
             "U, " +                                            // CAN ID
    ...
    if (ou16_GenCodeVersion >= 2U) {
      c_Text +=
          QString::number(u32_MessageCounterGap) + "U, "; // message counter gap
    } else {
      c_Text +=
          QString::number(rc_Message.c_Signals.size()) + "U, "; // signal number
    }

    c_Text += QString::number(u16_DelayTime) + "U, "; // minimum interval

    // maximum interval
    if (oq_Tx == true) {
      c_Text += QString::number(rc_Message.u32_CycleTimeMs) + "U, ";
    } else {
      c_Text += QString::number(rc_Message.u32_TimeoutMs) + "U, ";
    }
    ...
    if (ou16_GenCodeVersion >= 2U) {
      QSet<uint16_t> c_MultiplexerValues;
      rc_Message.GetMultiplexerValues(c_MultiplexerValues);
      c_Text += QString::number(c_MultiplexerValues.size()) +
                "U, " + // number of mux messages resp. values
                "&mat_StatusMux" + c_TxRxString +
                "Messages[" + // pointer to first element of mux status
                QString::number(u16_MessageCountWithMuxOffset) + "], " +
                "&mat_MessagesMux" + c_TxRxString +
                "[" + // pointer to first element of mux definition
                QString::number(u16_MessageCountWithMuxOffset) + "] }";

      if (c_MultiplexerValues.isEmpty() == false) {
        u16_MessageCountWithMuxOffset +=
            static_cast<uint16_t>(c_MultiplexerValues.size());
      } else {
        u16_MessageCountWithMuxOffset++;
      }
    } else {
      ...
      c_Text += QString::number(u32_MessageCounterGap) +
                "U, " +               // message counter gap
    ...
    c_Text +=
        ("  /// " + C_OscUtils::h_NiceifyStringForCeComment(rc_Message.c_Name));
    if (rc_Message.c_Comment.isEmpty() == false) {
      c_Text +=
          (" (" +
           C_OscUtils::h_NiceifyStringForCeComment(rc_Message.c_Comment) + ")");
    }
    orc_Data.append(c_Text);
  }
...
/*! \brief  Count multiplex messages.
   ...
   If a message is not multiplexed, count it as one. If it is multiplexed count
   number of multiplexed messages corresponding to this message, i.e. number of
   multiplex values. If the message has a multiplexer signal and therefore is
   multiplexed, but has no multiplexed signal at all, also count with one. See
   KFXTCSWRSCC_380.
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscExportCommunicationStack::mh_CountMuxMessages(
    const QList<C_OscCanMessage> &orc_Messages) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportCommunicationStack::mh_GetProtocolNameByType(
    const C_OscCanProtocol::E_Type &ore_Protocol) {
  QString c_Name;
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscExportCommunicationStack::mh_GetMagicName(
    const QString &orc_ProjectId, const uint8_t ou8_InterfaceIndex,
    const C_OscCanProtocol::E_Type &ore_Protocol) {
  const QString c_ProtocolName =
      mh_GetProtocolNameByType(ore_Protocol).toUpper();
  const QString c_MagicName =
...
/*! \brief  Utility function for creating signal declarations
   ...
   \param[in,out]  orc_Data               converted data to string list
   \param[in]      orc_Signals            list of signals for creating signal
   declarations for \param[in]      ou32_SignalListIndex   index of Datapool
   list containing signals \param[in]      oq_RemoveLastComma     flag to handle
   comma (very last signals string should not end on comma)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_ConvertSignalsToStrings(
  ...
  for (uint8_t u8_SignalIndex = 0U; u8_SignalIndex < u8_SignalCount;
       u8_SignalIndex++) {
    ...
    c_Text = "   { " + c_String + ", " +
             QString::number(c_Signal.u16_ComBitStart) + "U, " +
             QString::number(c_Signal.u16_ComBitLength) + "U, " +
             QString::number(ou32_SignalListIndex) + "U, " +
             QString::number(c_Signal.u32_ComDataElementIndex) + "U }";
    if ((oq_RemoveLastComma == false) ||
        (u8_SignalIndex != (u8_SignalCount - 1U))) {
      c_Text += ",";
    }
    orc_Data.append(c_Text);
  }
...
/*! \brief  Group signals by multiplex value
   ...
   Sort all signals of the given message into vectors. There is one vector for
   each multiplex value and one separate vector holding all non-mux signals. The
   first entry in mux case is always the multiplexer signal itself.

   \param[in]   orc_Message              CAN message data for signal information
   \param[in]   ou32_MultiplexerIndex    Index of multiplexer signal in
   orc_Message \param[out]  orc_MuxedSignalsPerValue Signals sorted by
   multiplexer values (see description) \param[out]  orc_NonMuxedSignals
   Non-multiplexed signals in the message
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscExportCommunicationStack::mh_GroupSignalsByMuxValue(
  ...
  if (ou32_MultiplexerIndex < orc_Message.c_Signals.size()) {
    const C_OscCanSignal &rc_MultiplexerSignal =
        orc_Message.c_Signals[ou32_MultiplexerIndex];
    QSet<uint16_t> c_MultiplexValues;

    orc_Message.GetMultiplexerValues(c_MultiplexValues);
    for (const uint16_t u16_Value : c_MultiplexValues) {
      // add multiplexer signal of every multiplexer-value-group; must be first
      // entry!
      orc_MuxedSignalsPerValue[u16_Value].push_back(rc_MultiplexerSignal);
    }

    for (uint8_t u8_SignalIndex = 0U;
         u8_SignalIndex < static_cast<uint8_t>(orc_Message.c_Signals.size());
         u8_SignalIndex++) {
      ...
      case C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL:
        ...
        Q_ASSERT(orc_MuxedSignalsPerValue.count(rc_Signal.u16_MultiplexValue) ==
                 1);
        orc_MuxedSignalsPerValue[rc_Signal.u16_MultiplexValue].push_back(
            rc_Signal);
        break;
      default:
        Q_ASSERT(false);
