...
/*! \brief   Create files for one application
   ...
   If the target path orc_Path does not exist the function can create it (also
   recursively). The function will store all files in the folder specified by
   orc_Path.
   ...
   * hardware configuration
   ...
   \param[in]   orc_Node               Node as information source for exported
   code
   \param[in]   ou16_ApplicationIndex  Index of Data Block within
   orc_Node.c_Applications
   \param[in]   orc_Path               Storage path for created files
   \param[out]  orc_Files              List of all exported file names (with
   ...
   C_NOACT   Application is not of file generation type or has unknown/invalid
   code structure version C_RANGE   Information which application runs the DPD
   is invalid or refers to an invalid application Datapool does not provide
   information about owning application or refers to an invalid application
             ApplicationIndex references invalid application
   C_CONFIG  Protocol or Datapool not available in node for interface or
             Input data not suitable for file generation. Details will be
   written to OSC Log.
*/
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
...
int32_t C_OscExportNode::h_CreateSourceCode(
  ...
  if (s32_Retval == C_NO_ERR) {
    const C_OscNodeApplication &rc_Application =
        orc_Node.c_Applications[ou16_ApplicationIndex];

    // we only need to create code for programmable applications and PSI Data
    // Blocks
    if ((rc_Application.e_Type !=
      ...
      osc_write_log_error("Creating files", "File generation for \"" +
                                                rc_Application.c_Name +
                                                "\" is disabled.");
      s32_Retval = C_NOACT;
    }
    // check if the code structure version is unknown
    else if (rc_Application.u16_GenCodeVersion >
             C_OscNodeApplication::hu16_HIGHEST_KNOWN_CODE_VERSION) {
      osc_write_log_error("Creating source code",
                          "Structure version of application \"" +
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval != C_NO_ERR) {
      osc_write_log_error("Creating source code",
                          "Could not create target directory \"" + orc_Path +
                              "\".");
      s32_Retval = C_RD_WR;
    }
  }
  ...
  if (orc_Node.c_Applications[ou16_ApplicationIndex].e_Type ==
    ...
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = mh_CreateOsyInitCode(orc_Node, ou16_ApplicationIndex,
                                        orc_Path, orc_Files, c_ExportToolInfo);
    }

    // export Datapool
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = mh_CreateDatapoolCode(orc_Node, ou16_ApplicationIndex,
                                         orc_Path, orc_Files, c_ExportToolInfo);
    }
    ...
    if (s32_Retval == C_NO_ERR) {
      s32_Retval =
          mh_CreateCommStackCode(orc_Node, ou16_ApplicationIndex, orc_Path,
                                 orc_Files, c_ExportToolInfo);
    }

    // export HAL configuration
    if (s32_Retval == C_NO_ERR) {
      s32_Retval =
          mh_CreateHalConfigCode(orc_Node, ou16_ApplicationIndex, orc_Path,
                                 orc_Files, c_ExportToolInfo);
    }
  } else {
    // export HAL NVM information as parameter set image
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = mh_CreateHalNvmData(orc_Node, ou16_ApplicationIndex,
                                       orc_Path, orc_Files, orc_ExportToolName,
...
int32_t C_OscExportNode::mh_CreateOsyInitCode(
  ...
  s32_Retval = C_OscExportOsyInit::h_CreateSourceCode(
      orc_Path, orc_Node, q_CreateDpdInit, ou16_ApplicationIndex,
      orc_ExportToolInfo);

  // Handle file names
  if (s32_Retval == C_NO_ERR) {
    h_CollectFilePathsWrapper(orc_Files, orc_Path,
                              C_OscExportOsyInit::h_GetFileName(), true);
  }
...
/*! \brief  Create Datapool C code
   ...
   C_NO_ERR Operation success
   C_RD_WR  Cannot store files
   C_CONFIG Input data not suitable for code generation. Details will be written
   to OSC Log.
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportNode::mh_CreateDatapoolCode(
  ...
  for (uint32_t u32_ItDataPool = 0U;
       u32_ItDataPool < orc_Node.c_DataPools.size(); ++u32_ItDataPool) {
    bool q_Create = false;
    ...
    else if ((orc_Node.c_Applications[ou16_ApplicationIndex]
                  .u16_GenCodeVersion >= 4U) &&
             (rc_DataPool.q_ScopeIsPrivate == false)) {
      e_Relation = C_OscExportDataPool::eREMOTEPUBLIC;
      q_Create = true;
    }
    // if the Datapool is not owned by this application and does not have public
    // scope, but this
    //  application runs the protocol driver, then make the Datapool known as
    //  remote Datapool
    else if (ou16_ApplicationIndex ==
             orc_Node.c_Properties.c_OpenSydeServerSettings
                 .s16_DpdDataBlockIndex) {
    ...
    if (q_Create == true) {
      ...
      for (uint16_t u16_ListIndex = 0U;
        ...
        const C_OscNodeDataPoolList &rc_List =
            rc_DataPool.c_Lists[u16_ListIndex];
    ...
    if (q_Create == true) {
      ...
      if (rc_DataPool.e_Type == C_OscNodeDataPool::eCOM) {
        ...
        s32_Retval = C_OscExportNode::mh_GetAdaptedComDataPool(
            orc_Node, u32_ItDataPool, c_DataPool);
        Q_ASSERT(s32_Retval == C_NO_ERR);

        // Export
        s32_Retval = C_OscExportDataPool::h_CreateSourceCode(
            orc_Path, u16_GenCodeVersion,
            orc_Node.c_Properties.c_CodeExportSettings.e_ScalingSupport,
            c_DataPool, u8_DataPoolIndexWithinApplication, e_Relation,
            u8_DataPoolIndexRemote, u8_ProcessId, orc_ExportToolInfo);
      } else {
        // Export
        s32_Retval = C_OscExportDataPool::h_CreateSourceCode(
            orc_Path, u16_GenCodeVersion,
            orc_Node.c_Properties.c_CodeExportSettings.e_ScalingSupport,
            rc_DataPool, u8_DataPoolIndexWithinApplication, e_Relation,
            u8_DataPoolIndexRemote, u8_ProcessId, orc_ExportToolInfo);
      }
      // Handle file names
      if (s32_Retval == C_NO_ERR) {
        h_CollectFilePathsWrapper(
            orc_Files, orc_Path,
...
/*! \brief  Create COMM stack C code
   ...
   unknown code structure version C_RD_WR     Cannot store files C_CONFIG
   Protocol or Datapool not available in node for interface or application index
   out of range C_RANGE     Application index out of range
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportNode::mh_CreateCommStackCode(
  ...
  for (uint32_t u32_ItProtocol = 0U;
    ...
    // logic: C_OscCanProtocol refers to a Datapool; if that Datapool is owned
    ...
    if (orc_Node.c_DataPools[rc_Protocol.u32_DataPoolIndex]
      ...
      for (uint32_t u32_ItInterface = 0U;
        ...
        // For CANopen we allow to have configuration with just the manager but
        // no assigned devices
        //  (and therefore no messages). "c_ComMessages" lists all interfaces.
        // So we create code for all of those interfaces where a manager is
        // active:
        if ((rc_Protocol.e_Type == C_OscCanProtocol::eCAN_OPEN) &&
            (orc_Node.c_CanOpenManagers.count(
                 static_cast<uint8_t>(u32_ItInterface)) != 0)) {
          s32_Retval = C_OscExportCanOpenConfig::h_CreateSourceCode(
              orc_Path, orc_Node, ou16_ApplicationIndex,
              static_cast<uint8_t>(u32_ItInterface),
              rc_Protocol.u32_DataPoolIndex, orc_ExportToolInfo);
          // handle file names
          if (s32_Retval == C_NO_ERR) {
            h_CollectFilePathsWrapper(
                orc_Files, orc_Path,
                C_OscExportCanOpenConfig::h_GetFileName(
                    static_cast<uint8_t>(u32_ItInterface)),
                true);
          }

          c_IfWithCanOpenManager.push_back(
              static_cast<uint8_t>(u32_ItInterface));
        }
        // all other COMM protocol types remain as COMMStack export
          ...
          if ((rc_ComMessageContainer.c_TxMessages.size() > 0) ||
            ...
            s32_Retval = C_OscExportCommunicationStack::h_CreateSourceCode(
                orc_Path, orc_Node, ou16_ApplicationIndex,
                static_cast<uint8_t>(u32_ItInterface),
                rc_Protocol.u32_DataPoolIndex, rc_Protocol.e_Type,
                orc_ExportToolInfo);
            // Handle file names
            if (s32_Retval == C_NO_ERR) {
              h_CollectFilePathsWrapper(
      ...
      if (c_IfWithCanOpenManager.size() > 0) {
        s32_Retval = C_OscExportCanOpenInit::h_CreateSourceCode(
            orc_Path, orc_Node, c_IfWithCanOpenManager, orc_ExportToolInfo);
        // handle file names
        if (s32_Retval == C_NO_ERR) {
          h_CollectFilePathsWrapper(orc_Files, orc_Path,
                                    C_OscExportCanOpenInit::h_GetFileName(),
                                    true);
        }
      }
      if (s32_Retval != C_NO_ERR) {
      ...
      for (c_ItHalDataPools = c_HalDataPools.begin();
            ...
            0) // no safe resp. non-safe Datapool -> ok, no HALC file generation
        {
          ...
          if (rc_HalDataPool.s32_RelatedDataBlockIndex ==
            ...
            s32_Retval = C_OscExportHalc::h_CreateSourceCode(
                orc_Path,
                orc_Node.c_Applications[ou16_ApplicationIndex]
                    .u16_GenCodeVersion,
...
/*! \brief  Create HALC NVM configuration as parameter set image.
   ...
   C_RD_WR     Problems accessing file system
   C_CONFIG    Internal data invalid (e.g. incorrect number of lists or datasets
   in HALC NVM Datapool)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportNode::mh_CreateHalNvmData(
  ...
  if (orc_Node.c_HalcConfig.IsClear() == false) {
    s32_Retval = C_OscExportParamSet::h_CreateParameterSetImage(
        orc_Path, orc_Node, ou16_ApplicationIndex, orc_Files,
        orc_ExportToolName, orc_ExportToolVersion);
  } else {
    s32_Retval = C_NOACT;
  }
...
/*! \brief   Check for correct preconditions for creating source code
   ...
   Note: As we use the system definition without device definition here, we can
   not check for programmable flag and need to use NVM-based-HALC flag instead.

   \param[in]  orc_Node    Node as information source for exported code

   \return
   C_NO_ERR  Node configuration is OK
   C_RANGE   Information which application runs the DPD is invalid or refers to
   an invalid application Datapool does not provide information about owning
   application or refers to an invalid application
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportNode::mh_CheckPrerequisites(const C_OscNode &orc_Node) {
  ...
  if (orc_Node.c_HalcConfig.q_NvmBasedConfig == false) {
    ...
    if ((orc_Node.c_Properties.c_OpenSydeServerSettings.s16_DpdDataBlockIndex ==
         -1) ||
        (orc_Node.c_Properties.c_OpenSydeServerSettings.s16_DpdDataBlockIndex >=
         static_cast<int16_t>(orc_Node.c_Applications.size()))) {
      osc_write_log_error("Creating source code",
                          "Invalid definition of application running the "
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    for (uint8_t u8_DataPool = 0U; u8_DataPool < orc_Node.c_DataPools.size();
      ...
      const C_OscNodeDataPool &rc_CurDataPool =
          orc_Node.c_DataPools[u8_DataPool];
      if ((rc_CurDataPool.s32_RelatedDataBlockIndex == -1) ||
          (rc_CurDataPool.s32_RelatedDataBlockIndex >=
           static_cast<int32_t>(orc_Node.c_Applications.size()))) {
        if ((orc_Node.c_HalcConfig.q_NvmBasedConfig == false) ||
...
/*! \brief   Adapt com Datapool elements
   ...
   \param[in]   ou32_DataPoolIndex  Datapool index within Datapools owned by
   node
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscExportNode::mh_GetAdaptedComDataPool(const C_OscNode &orc_Node,
  ...
  if (ou32_DataPoolIndex < orc_Node.c_DataPools.size()) {
    ...
    s32_Retval = C_OscExportNode::mh_AdaptComDataPool(
        orc_Node, ou32_DataPoolIndex, orc_DataPool);
  } else {
    s32_Retval = C_RANGE;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportNode::mh_AdaptComDataPool(const C_OscNode &orc_Node,
                                             const uint32_t ou32_DataPoolIndex,
                                             C_OscNodeDataPool &orc_DataPool) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if ((ou32_DataPoolIndex < orc_Node.c_DataPools.size()) &&
    ...
    for (uint32_t u32_ItProtocol = 0;
      ...
      if (rc_Protocol.u32_DataPoolIndex == ou32_DataPoolIndex) {
        q_Found = true;
        // For each interface
        for (uint32_t u32_ItMessageContainer = 0;
             u32_ItMessageContainer < rc_Protocol.c_ComMessages.size();
          ...
          for (uint8_t u8_Toggle = 0; u8_Toggle < 2; ++u8_Toggle) {
            ...
            for (uint32_t u32_ItMessage = 0; u32_ItMessage < rc_Messages.size();
              ...
              for (uint32_t u32_ItSignal = 0;
                   u32_ItSignal < rc_Message.c_Signals.size(); ++u32_ItSignal) {
                const C_OscCanSignal &rc_Signal =
                    rc_Message.c_Signals[u32_ItSignal];
                if (rc_Signal.u32_ComDataElementIndex <
                  ...
                  rc_Element.c_Name =
                      rc_Message.c_Name + '_' + rc_Element.c_Name;
                }
              }
            }
          }
...
/*! \brief  Get indices of first safe and non-safe HAL Datapool
   ...
   \param[out]  orc_HalcDataPools   Corresponding data pools
                                    true = safe; false = non-safe; integer is
   corresponding Datapool index

   \return
   C_NO_ERR Operation success
   C_CONFIG Safety mode of HALC and existence of safe/non-safe Datapool do not
   match
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscExportNode::mh_GetHalDataPoolIndices(
  ...
  for (uint32_t u32_ItDataPool = 0U;
    ...
    if ((rc_DataPool.e_Type == C_OscNodeDataPool::eHALC) ||
      ...
      if (orc_HalcDataPools[rc_DataPool.q_IsSafety] == -1) {
        orc_HalcDataPools[rc_DataPool.q_IsSafety] =
  ...
  if (orc_Node.c_HalcConfig.e_SafetyMode ==
      C_OscHalcDefBase::eONE_LEVEL_ALL_NON_SAFE) {
    if (orc_HalcDataPools[true] >= 0) {
      s32_Retval = C_CONFIG;
    }
  } else if (orc_Node.c_HalcConfig.e_SafetyMode ==
  ...
  if (s32_Retval != C_NO_ERR) {
    osc_write_log_error("Creating source code",
                        "Safety mode of HALC definition does not match safety "
                        "settings of HALC Datapools.");
  }
