 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_OscXcoLoad::mhc_USE_CASE = "Unpacking X-Config Package";
...
/*! \brief   Unpack an X-Config package
   ...
   * write permission of target path for unzipped package
   ...
   \param[out]     orc_SystemDefinitionPath  path to unpacked System Definition
   (.syde_sysdef file)
   \param[out]     orc_DeviceDefinitionPath  path to unpacked Device Definitions
   (devices.ini file)
   \param[out]     orc_WarningMessages       warning messages for imported
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoLoad::h_ProcessPackage(const QString &orc_PackagePath,
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return =
        C_OscXcoManifestFiler_New::h_LoadXcoManifestFile(orc_Manifest, c_ManifestPath, c_TargetUnzipPath);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoLoad::mh_CheckParamsToProcessPackage(
...
/*! \brief  This function shall check if certain files are existent in the
   X-Config package. (internal function)

   The update package will be a created directory after unpacking a ".syde_xcfg"
   file, In order to successfully import we need to make sure that whether the
   "syde_xcfg" got "corrupted". There are some defensive checks for files which
   are necessary and if those are missing, we want to handle this gracefully.
...
int32_t C_OscXcoLoad::mh_CheckXcfgFiles(const QString &orc_PackagePath) {
  ...
  if (s32_Return == C_NO_ERR) {
    s32_Return = C_OscSpaServicePackageLoadUtil::h_SearchFilesInPath(
        orc_PackagePath + hc_XCFG_SYSDEF_FOLDER, c_NecessaryFilesSysDef);
  }
  if (s32_Return == C_NO_ERR) {
    s32_Return = C_OscSpaServicePackageLoadUtil::h_SearchFilesInPath(
        orc_PackagePath + hc_INI_DEV_FOLDER, c_NecessaryFilesDevDef);
  }
