 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_OscXcoManifestFiler::hc_FILE_NAME = "manifest.syde_pkg";
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoManifestFiler::h_LoadFile(C_OscXcoManifest &orc_Config,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoManifestFiler::h_SaveFile(const C_OscXcoManifest &orc_Config,
                                          const QString &orc_Path) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoManifestFiler::h_LoadData(C_OscXcoManifest &orc_Config,
    ...
    if (s32_Retval == C_NO_ERR) {
      if (c_Types != "x-app-config") {
        orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
            "types", "expecting content to be \"x-app-config\"");
        s32_Retval = C_CONFIG;
      }
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      s32_Retval =
          orc_XmlParser.GetAttributeStringError("name", orc_Config.c_NodeName);
      orc_XmlParser.SelectNodeParent();
    }
    orc_XmlParser.SelectNodeParent();
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscXcoManifestFiler::h_SaveData(const C_OscXcoManifest &orc_Config,
                                       C_OscXmlParserBase &orc_XmlParser) {
  // File version
  ...
  orc_XmlParser.SelectNodeParent();
  // Config
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("x-app-config") ==
           "x-app-config");
  orc_XmlParser.CreateNodeChild("package-version",
                                QString::number(mhu16_PACKAGE_VERSION_1));
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("x-app-node") ==
           "x-app-node");
