 ...
 * ---------------------------------------------------------------------------------------
 */
const QString C_OscXcoCreate::mhc_USE_CASE = "Creating X-Config Package";
...
/*! \brief   Create an X-Config package
   ...
   * write permission to target folder
   ...
   instance - no file!) \param[in]   orc_Manifest              Manifest
   \param[out]  orc_WarningMessages       warning messages for created package
   ...
   already exists C_NOACT     active node index is not in system definition
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoCreate::h_CreatePackage(
    const QString &orc_PackagePath,
    const C_OscSystemDefinition &orc_SystemDefinition,
    const C_OscXcoManifest &orc_Manifest, QStringList &orc_WarningMessages,
    QString &orc_ErrorMessage, const QString &orc_TemporaryDirectory) {
  int32_t s32_Return;
  ...
  const QString c_TargetZipArchive = orc_PackagePath; // complete path of target
                                                      // zip archive
  QSet<QString> c_XcfgFiles; // unique container with
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return =
        C_OscSpaServicePackageCreateUtil::h_CreateTempFolderAndSubFolders(
            ...
            c_PackagePathTmp, mhc_ErrorMessage);
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return =
        C_OscXcoManifestFiler_New::h_SaveFile(orc_Manifest, c_ManifestPath);
    if (s32_Return != C_NO_ERR) {
  ...
  if (s32_Return == C_NO_ERR) {
    const QString c_DevDefPath =
        stw::opensyde_core::C_OscUtils::h_IncludeTrailingDelimiter(
            c_PackagePathTmp + hc_INI_DEV_FOLDER);

    s32_Return =
        C_OscSpaServicePackageCreateUtil::h_SaveDeviceDefinitionsAndIni(
            orc_SystemDefinition, mhc_USE_CASE, c_DevDefPath,
            stw::opensyde_core::C_OscUtils::h_IncludeTrailingDelimiter(
                hc_INI_DEV_FOLDER),
            c_XcfgFiles, mhc_ErrorMessage);
  }
  ...
  if ((s32_Return == C_NO_ERR) || (s32_Return == C_WARN)) {
    s32_Return = C_OscSpaServicePackageCreateUtil::h_CreateZip(
        mhc_USE_CASE, c_PackagePathTmp, c_TargetZipArchive, c_XcfgFiles,
        mhc_ErrorMessage);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXcoCreate::mh_CheckParamsToCreatePackage(
    const QString &orc_PackagePath,
    const C_OscSystemDefinition &orc_SystemDefinition,
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    for (uint32_t u32_It = 0UL;
         (u32_It <= orc_SystemDefinition.c_Nodes.size()) &&
         (q_Contained == false);
         ++u32_It) {
    ...
    if (q_Contained == false) {
      mhc_ErrorMessage = "Node \"" + orc_Manifest.c_NodeName +
