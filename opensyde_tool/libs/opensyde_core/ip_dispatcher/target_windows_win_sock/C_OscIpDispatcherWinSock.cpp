...
// Purpose: the definitions in the Windows headers violate a lot of coding
// rules. So we can relocate the required suppressions here at a central spot
// ... The violations do not cause problems in the application as long as we are
...
bool C_OscIpDispatcherWinSock::C_BufferIdentifier::operator<(
  ...
  if (this->u8_ServerBusIdentifier < orc_Cmp.u8_ServerBusIdentifier) {
    ...
    if (this->u8_ServerNodeIdentifier < orc_Cmp.u8_ServerNodeIdentifier) {
      q_Return = true;
    } else if (this->u8_ServerNodeIdentifier ==
               orc_Cmp.u8_ServerNodeIdentifier) {
      if (this->u8_ClientBusIdentifier < orc_Cmp.u8_ClientBusIdentifier) {
        q_Return = true;
      } else if (this->u8_ClientBusIdentifier ==
                 orc_Cmp.u8_ClientBusIdentifier) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscIpDispatcherWinSock::C_OscIpDispatcherWinSock(void) : C_OscIpDispatcher() {
  WSADATA c_Data;
  const int
      x_Result = // lint !e8080 !e970 //using type to match library interface
...
C_OscIpDispatcherWinSock::~C_OscIpDispatcherWinSock(void) {
  ...
  for (uint16_t u16_Index = 0U; u16_Index < this->mc_SocketsTcp.size();
       u16_Index++) {
    (void)this->C_OscIpDispatcherWinSock::CloseTcp(u16_Index);
  }
  (void)this->C_OscIpDispatcherWinSock::CloseUdp();

  (void)WSACleanup(); // spec: one call of cleanup per call of startup; final
                      // call performs actual cleanup
}
...
/*! \brief   Initialize TCP communication
   ...
   * closing connection closes the handle bus does not reduce the list of
   handles
   * so we don't have any trouble with existing handles getting invalid

   \param[in]     orau8_Ip      IP address of server to connect to
   \param[out]    oru32_Handle  handle to new TCP connection (to be used in
   subsequent calls of TCP functions)

   \return
   C_NO_ERR   Connection created
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscIpDispatcherWinSock::InitTcp(const uint8_t (&orau8_Ip)[4],
...
/*! \brief   Utility: get IP of all local interfaces
   ...
   If mc_PreferredInterfaceName is not an empty string, only IPs of an adapter
...
int32_t C_OscIpDispatcherWinSock::m_GetAllInstalledInterfaceIps(void) {
  ...
  u32_RetVal =
      GetAdaptersAddresses(AF_INET, 0U, NULL, pc_Addresses, &x_AddressesBufLen);

  if (u32_RetVal ==
      NO_ERROR) // lint !e620 !e9106 //constant defined by API; no problem
  {
    ...
    while (pc_Adapter != NULL) {
      ...
      if (pc_Adapter->OperStatus == IfOperStatusUp) {
        IP_ADAPTER_UNICAST_ADDRESS *pc_Address =
            pc_Adapter->FirstUnicastAddress;

        while (pc_Address != NULL) {
          if ((this->mc_PreferredInterfaceNames.count() == 0) ||
              (this->mc_PreferredInterfaceNames.indexOf(
                   QString::fromWCharArray(pc_Adapter->FriendlyName)) != -1)) {
            // sockaddr is the generic descriptor and sockaddr_in is IPV4
            // specific
            // https://stackoverflow.com/questions/21099041/why-do-we-cast-sockaddr-in-to-sockaddr-when-calling-bind/21099196
            // lint -e{929,740,9176} // Both struct have the same size and is a
            // kind of C style polymorphism
            const in_addr c_IpAddr =
                reinterpret_cast<sockaddr_in *>(pc_Address->Address.lpSockaddr)
                    ->sin_addr;

            // ignore localhost; otherwise "locally" running servers will get
            // requests via the localhost and
            //  a physical local interface
            if (ntohl(c_IpAddr.S_un.S_addr) != 0x7F000001U) {
              QString c_Info;

              mc_LocalInterfaceIps.push_back(ntohl(
                  c_IpAddr.S_un.S_addr)); // add to list of known interfaces

              c_Info = "Local IP interface used with IP: " +
                       QString(inet_ntoa(c_IpAddr)) + ", name of adapter: \"" +
                       QString::fromWCharArray(pc_Adapter->FriendlyName) + "\"";
    ...
    osc_write_log_error(
        "openSYDE IP-TP",
        "UDP init failed. Could not get IP addresses with error: " +
...
/*! \brief   Utility: connect TCP socket to server
   ...
   Jobs to perform:
   - connect already created client socket to port 13400 of the specified server
   node
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscIpDispatcherWinSock::m_ConnectTcp(C_TcpConnection &orc_Connection) const {
  bool q_Error = false;
  ...
  if (q_Error == false) {
    ...
    if (ioctlsocket(orc_Connection.x_Socket, m_WsFionBio(), &x_Mode) ==
        SOCKET_ERROR) {
      osc_write_log_error(
          "openSYDE IP-TP",
          "TCP socket ioctlsocket() failed. Error: " +
  ...
  if (q_Error == false) {
    ...
    // lint -e{929,740,9176}  Side-effect of the POSIX-style API. Match is
    // guaranteed by the API.
    x_Return = connect(orc_Connection.x_Socket,
                       reinterpret_cast<const sockaddr *>(&c_TcpService),
                       sizeof(c_TcpService));
    // for this non-blocking TCP socket the function should immediately return
    // with WSAEWOULDBLOCK:
    if ((x_Return == SOCKET_ERROR) && (WSAGetLastError() != WSAEWOULDBLOCK)) {
      ...
      c_SocketWriteSet.fd_array[0] =
          orc_Connection.x_Socket;   // watch this socket ...
      c_SocketWriteSet.fd_count = 1; // only one socket
      c_SocketErrorSet.fd_array[0] =
          orc_Connection.x_Socket;   // watch this socket ...
      ...
      x_Return =
          select(0, NULL, &c_SocketWriteSet, &c_SocketErrorSet, &c_TimeOut);
      switch (x_Return) {
      case SOCKET_ERROR:
        osc_write_log_error(
            "openSYDE IP-TP",
      ...
      case 0:
        ...
        // No error. The connection to the concrete target can be established
        // later
        q_Error = false;
        break;
      case 1:
        ...
        if (FD_ISSET(orc_Connection.x_Socket, &c_SocketWriteSet)) {
          // Get port of client for logging (the byte order must be changed of
          // the read port by ntohs)
          sockaddr_in c_SocketInfo;
          int x_Size = // lint !e8080 !e970 //using type to match library
                       // interface
              sizeof(c_SocketInfo);

          // lint -e{929,740,9176}  Side-effect of the POSIX-style API. Match is
          // guaranteed by the API.
          getsockname(orc_Connection.x_Socket,
                      reinterpret_cast<sockaddr *>(&c_SocketInfo), &x_Size);
          ...
          osc_write_log_info("openSYDE IP-TP",
                             "TCP connect select() OK. IP-Address: " +
                                 mh_IpToText(orc_Connection.au8_IpAddress) +
                                 " on client port: " +
                                 QString::number(ntohs(c_SocketInfo.sin_port)));
        } else {
      ...
      default:
        osc_write_log_error("openSYDE IP-TP",
                            "TCP connect select() failed. Unknown problem: " +
                                QString::number(x_Return) + " IP-Address: " +
                                mh_IpToText(orc_Connection.au8_IpAddress));
        q_Error = true;
        break;
      }
...
int32_t
C_OscIpDispatcherWinSock::m_ConfigureUdpSocket(const bool oq_ServerPort,
  ...
  if ((q_Error == false) && (oq_ServerPort == false)) {
    ...
    x_Return = setsockopt(orx_Socket, SOL_SOCKET, SO_BROADCAST, &cn_BROADCAST,
                          sizeof(cn_BROADCAST));
    if (x_Return == SOCKET_ERROR) {
      osc_write_log_error("openSYDE IP-TP",
                          "UDP set broadcast permission failed. Error: " +
                              QString::number(WSAGetLastError()));
  ...
  if (q_Error == false) {
    ...
    // guaranteed by the API.
    x_Return =
        bind(orx_Socket, reinterpret_cast<const sockaddr *>(&c_UdpService),
             sizeof(c_UdpService));
    if (x_Return == SOCKET_ERROR) {
      osc_write_log_error("openSYDE IP-TP",
                          "UDP bind() failed. Error: " +
                              QString::number(WSAGetLastError()));
      q_Error = true;
    } else {
...
QString C_OscIpDispatcherWinSock::mh_IpToText(const uint8_t (&orau8_Ip)[4]) {
  ...
  c_Text = QString::asprintf("%d.%d.%d.%d", orau8_Ip[0], orau8_Ip[1],
...
int32_t C_OscIpDispatcherWinSock::InitUdp(void) {
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    osc_write_log_warning("openSYDE IP-TP",
                          "UDP init: could not find IP(s) of local interfaces. "
                          "Trying \"any\" ...");
  ...
  for (uint32_t u32_Interface = 0U; u32_Interface < mc_LocalInterfaceIps.size();
       u32_Interface++) {
  ...
  for (uint32_t u32_Interface = 0U; u32_Interface < mc_LocalInterfaceIps.size();
    ...
    // create UDP socket:
    s32_Return =
        m_ConfigureUdpSocket(false, mc_LocalInterfaceIps[u32_Interface],
                             mc_SocketsUdpClient[u32_Interface]);
    if (s32_Return == C_NO_ERR) {
      s32_Return =
          m_ConfigureUdpSocket(true, mc_LocalInterfaceIps[u32_Interface],
                               mc_SocketsUdpServer[u32_Interface]);
    }
    if (s32_Return != C_NO_ERR) {
      q_Error = true;
      break;
...
int32_t C_OscIpDispatcherWinSock::IsTcpConnected(const uint32_t ou32_Handle) {
  ...
  if (ou32_Handle >= this->mc_SocketsTcp.size()) {
    osc_write_log_error("openSYDE IP-TP",
                        "IsTcpConnected called with invalid handle.");
    s32_Return = C_RANGE;
    ...
    // The parameter MSG_PEEK avoid that received data will removed from the
    // queue when read
    const int x_BytesRead = // lint !e8080 !e970 //using type to match library
                            // interface
        recv(this->mc_SocketsTcp[ou32_Handle].x_Socket, &cn_Byte, 1, MSG_PEEK);
    ...
    if (x_BytesRead > 0) {
      ...
      const int x_SocketError = // lint !e8080 !e970 //using type to match
                                // library interface
          WSAGetLastError();

      // Resource temporarily unavailable. It means that no bytes could be read,
      // because of an empty queue. But the connection is already established.
...
/*! \brief   Reconnect TCP socket
   ...
   This function can be used by a client to reconnect after the connection was
   lost (e.g. dropped by server). If there is still an active connection the
   function shall close it before reconnecting. Function shall return when
   connected or after timeout.
...
int32_t C_OscIpDispatcherWinSock::ReConnectTcp(const uint32_t ou32_Handle) {
  ...
  if (ou32_Handle >= this->mc_SocketsTcp.size()) {
    osc_write_log_error("openSYDE IP-TP",
                        "ReConnectTcp called with invalid handle.");
    s32_Return = C_RANGE;
  } else {
    // still connected ?
...
int32_t C_OscIpDispatcherWinSock::CloseTcp(const uint32_t ou32_Handle) {
  ...
  if ((ou32_Handle >= this->mc_SocketsTcp.size()) ||
      (this->mc_SocketsTcp[ou32_Handle].x_Socket == m_WsInvalidSocket())) {
    osc_write_log_error("openSYDE IP-TP",
                        "CloseTcp called with invalid handle.");
    s32_Return = C_RANGE;
  } else {
    sockaddr_in c_SocketInfo;
    ...
    // lint -e{929,740,9176}  Side-effect of the POSIX-style API. Match is
    // guaranteed by the API.
    getsockname(this->mc_SocketsTcp[ou32_Handle].x_Socket,
                reinterpret_cast<sockaddr *>(&c_SocketInfo), &x_Size);
...
int32_t C_OscIpDispatcherWinSock::CloseUdp(void) {
  for (uint32_t u32_Interface = 0U; u32_Interface < mc_SocketsUdpClient.size();
       u32_Interface++) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscIpDispatcherWinSock::SendTcp(const uint32_t ou32_Handle,
                                          const QByteArray &orc_Data) {
  int32_t s32_Return;

  if (ou32_Handle >= this->mc_SocketsTcp.size()) {
    osc_write_log_error("openSYDE IP-TP",
                        "SendTcp called with invalid handle.");
    s32_Return = C_RANGE;
  } else {
    s32_Return = C_NO_ERR;
    ...
    if (this->mc_SocketsTcp[ou32_Handle].x_Socket == m_WsInvalidSocket()) {
      osc_write_log_error("openSYDE IP-TP",
                          "SendTcp called with invalid socket.");
      s32_Return = C_CONFIG;
    } else {
      // No side effects as long as we are on the Windows platform.
      const int x_BytesToSend = static_cast<int>(
          orc_Data.size()); // lint !e8080 !e970 //using type to match library
                            //  interface
      // lint -e{9176}  //Side-effect of the "char"-based API.
      const int x_BytesSent = // lint !e8080 !e970 //using type to match library
                              // interface
      ...
      if (x_BytesSent != x_BytesToSend) {
        if (x_BytesSent == SOCKET_ERROR) {
          const int x_Error = // lint !e8080 !e970 //using type to match library
                              // interface
              WSAGetLastError();
          ...
          osc_write_log_error(
              ...
              "SendTcp: Could not send TCP service. Data lost. Error: " +
                  QString::number(x_Error) + " IP-Address: " +
                  mh_IpToText(this->mc_SocketsTcp[ou32_Handle].au8_IpAddress));
...
int32_t C_OscIpDispatcherWinSock::ReadTcp(const uint32_t ou32_Handle,
  ...
  if (ou32_Handle >= this->mc_SocketsTcp.size()) {
    osc_write_log_error("openSYDE IP-TP",
                        "ReadTcp called with invalid handle.");
    s32_Return = C_RANGE;
  } else {
    s32_Return = C_NOACT;
    ...
    if (this->mc_SocketsTcp[ou32_Handle].x_Socket == m_WsInvalidSocket()) {
      osc_write_log_error("openSYDE IP-TP",
                          "ReadTcp called with invalid socket.");
      s32_Return = C_CONFIG;
    } else {
      int x_Return; // lint !e8080 !e970 //using type to match library interface
      // lint -e{8080} //using type provided by the library for compatibility
      ...
      x_Return = ioctlsocket(this->mc_SocketsTcp[ou32_Handle].x_Socket,
                             m_WsFionRead(), &x_SizeInBuffer);
      if ((x_Return != SOCKET_ERROR) && (x_SizeInBuffer >= orc_Data.size())) {
        // enough bytes: read
        const int x_BytesToRead = static_cast<int>(
            orc_Data.size()); // lint !e8080 !e970 //using type to match
                              //  library interface
        // lint -e{9176}  Side-effect of the "char"-based API. No side effects
        // as long as we are on the Windows
        //  platform.
        const int x_BytesRead = // lint !e8080 !e970 //using type to match
                                // library interface
...
/*! \brief   Read data from TCP socket
   ...
   Jobs to perform:
   - check whether TCP receive buffer contains required number of data bytes
   - Check if the identifier are matching (starting at byte 0). The header must
   be already read separately. If the identifier are not matching, the data will
   be stored in the buffer of the dispatcher.
   - read those bytes
   ...
   set by the caller (orc_Data.size()); out: received data
   ...
   C_RD_WR    error reading data
   C_RANGE    handle invalid
   C_WARN     data is not for the server with the node identifier and bus
   identifier
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscIpDispatcherWinSock::ReadTcp(
    const uint32_t ou32_Handle, const uint8_t ou8_ClientBusIdentifier,
  ...
  if (s32_Return == C_NO_ERR) {
    if (orc_Data.size() > 4) {
      ...
      if ((u8_SourceNodeId != ou8_ServerNodeIdentifier) ||
        ...
        // Save the message for other client server connections over the same IP
        // address
        QMap<C_BufferIdentifier, QList<QByteArray>>::iterator
            c_ItBuffer;
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscIpDispatcherWinSock::ReadTcpBuffer(const uint8_t ou8_ClientBusIdentifier,
                                        const uint8_t ou8_ClientNodeIdentifier,
                                        const uint8_t ou8_ServerBusIdentifier,
  ...
  const C_BufferIdentifier c_Id(
      ou8_ClientBusIdentifier, ou8_ClientNodeIdentifier,
...
int32_t C_OscIpDispatcherWinSock::SendUdp(const QByteArray &orc_Data) {
  ...
  if (mc_SocketsUdpClient.size() == 0) {
    ...
    for (uint32_t u32_Interface = 0U;
         u32_Interface < mc_SocketsUdpClient.size(); u32_Interface++) {
      if (mc_SocketsUdpClient[u32_Interface] != m_WsInvalidSocket()) {
        sockaddr_in c_TargetAddress;
        c_TargetAddress.sin_family = AF_INET;
        c_TargetAddress.sin_port =
            htons(mhu16_UDP_TCP_PORT); // target port [REQ DoIp-011]
        c_TargetAddress.sin_addr.s_addr =
            htonl(INADDR_BROADCAST); // lint !e9105 //constant defined by API;
        // no problem
        const int x_NumToSend = static_cast<int>(
            orc_Data.size()); // lint !e8080 !e970 //using type to match library
                              //  interface
        // lint -e{9176}  //Side-effect of the POSIX-style API. Match is
        // guaranteed by the API.
        const int x_Return = // lint !e8080 !e970 //using type to match library
                             // interface
            sendto(mc_SocketsUdpClient[u32_Interface],
                   reinterpret_cast<const char_t *>(orc_Data.constData()),
                   x_NumToSend, 0,
                   reinterpret_cast<const sockaddr *>(&c_TargetAddress),
        ...
        osc_write_log_error("openSYDE IP-TP",
                            "SendUdp called with invalid socket.");
      }
    }
  }
...
/*! \brief   Read package from UDP socket
   ...
   from UDP socket. Report datagrams sent from:
   * any IP
   * port 13400 (to ignore reception of our own broadcasts and unexpected
   traffic) Return the read data.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscIpDispatcherWinSock::ReadUdp(QByteArray &orc_Data,
                                          uint8_t (&orau8_Ip)[4]) {
  int32_t s32_Return = C_NOACT;
  ...
  if (mc_SocketsUdpServer.size() == 0) {
    ...
    for (uint32_t u32_Interface = 0U;
         u32_Interface < mc_SocketsUdpServer.size(); u32_Interface++) {
      if (mc_SocketsUdpClient[u32_Interface] != m_WsInvalidSocket()) {
        // do we have a package in Rx buffer ?
        u_long x_SizeInBuffer; // lint !e8080 //using type to match library
                               // interface
        int x_Return = // lint !e8080 !e970 //using type to match library
                       // interface
            ioctlsocket(mc_SocketsUdpServer[u32_Interface], m_WsFionRead(),
                        &x_SizeInBuffer);
        if ((x_Return != SOCKET_ERROR) && (x_SizeInBuffer >= 1U)) {
          sockaddr_in c_Sender;
          int x_AddressSize =
              sizeof(c_Sender); // lint !e8080 !e970 //using type to match
                                // library interface

          // enough bytes: read
          orc_Data.resize(x_SizeInBuffer);

          // lint -e{926,929}  //Side-effect of the "char"-based API. No
          // problems as long as we are on Windows. lint -e{740,9176}
          // //Side-effect of the POSIX-style API. Match is guaranteed by the
          // API.
          x_Return = recvfrom(
              mc_SocketsUdpServer[u32_Interface],
              reinterpret_cast<char_t *>(&orc_Data[0]),
              static_cast<int>(
                  orc_Data.size()), // lint !e970 //using expected API type
              0, reinterpret_cast<sockaddr *>(&c_Sender), &x_AddressSize);

          // there might be more than one package in the buffer; recvfrom only
          // reads one so we check whether we have more than zero bytes:
          if (x_Return > 0) {
            ...
            if (x_Return !=
                static_cast<int>(orc_Data.size())) // lint !e970 //using type to
                                                   // match library
                                                   //  interface
            {
              orc_Data.resize(
                  x_Return); // we only need the data we really received
            }
            ...
            osc_write_log_error(
                ...
                "reading failed. Reported size: " +
                    QString::number(orc_Data.size()) +
                    " Read size: " + QString::number(x_Return));
            s32_Return = C_RD_WR;
          }
        }
      } else {
        // Write error to log, then ignore the error and continue
        osc_write_log_error("openSYDE IP-TP",
                            "ReadUdp called with invalid socket.");
      }
    }
  }
...
/*! \brief   Loads optional configuration file
   ...
   If the file does not exist, no specific configuration will be used for
   Ethernet.
