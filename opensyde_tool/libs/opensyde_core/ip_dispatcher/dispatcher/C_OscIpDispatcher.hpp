...
/*!
   ...
   Defines an interface for client applications for IP based communication with
   openSYDE protocol server nodes.
   ...
   Typically each client protocol driver uses one IpDispatcher to communicate
   with one server node. Specific implementations can use e.g. the WinSock or
   POSIX APIs.
...
public:
  ...
  C_OscIpDispatcher(void) { mu32_ConnectionTimeoutSeconds = 2U; }
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t InitTcp(const uint8_t (&orau8_Ip)[4],
                          uint32_t &oru32_Handle) = 0;
  ...
  */
  ...
  /*!
     ...
     \param[in]      ou32_Handle   handle obtained by InitTcp()
     \param[in,out]  orc_Data      in: the expected number of bytes is set by
     the caller (orc_Data.size()); out: received data
  ...
  */
  ...
  /*!
     ...
     Jobs to perform:
     - check whether TCP receive buffer contains required number of data bytes
     - Check if the identifier are matching (starting at byte zero). The header
     must be already read separately.
     - read those bytes
     ...
     \param[in,out]  orc_Data                  in: the expected number of bytes
     is set by the caller (orc_Data.size()); out: received data
     ...
     C_RD_WR    error reading data
     C_RANGE    invalid handle
     C_WARN     data is not for the server and/or client with the node
     identifier and bus identifier
  */
  //-----------------------------------------------------------------------------
  ...
  /*!
     ...
     Jobs to perform:
     - Check if the identifier are matching (starting at byte 0). The header
     must be already read separately.
     - read those bytes
  ...
  */
  ...
  /*!
     ...
     Jobs to perform:
     Go through all set up UDP "client" sockets and send specified data as
     broadcast.
  ...
  */
  ...
  /*!
     ...
     Go through all set up UDP "server" sockets and try to read incoming
     datagram from UDP socket. Report datagrams sent from:
     * any IP
