...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscParamSetFilerBase::h_CheckFileVersion(C_OscXmlParserBase &orc_XmlParser) {
  ...
  if (orc_XmlParser.SelectNodeChild("file-version") == "file-version") {
    ...
    if (s32_Retval == C_NO_ERR) {
      osc_write_log_info("Loading Dataset data",
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetFilerBase::h_SaveFileVersion(
  ...
  orc_XmlParser.SetNodeContent(
      "0x" + QString::number(C_OscParamSetFilerBase::mhu16_FileVersion, 16)
                 .rightJustified(4, '0'));
  // Return
  Q_ASSERT(orc_XmlParser.SelectNodeParent() == "opensyde-parameter-sets");
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetFilerBase::h_SaveFileInfo(
    C_OscXmlParserBase &orc_XmlParser,
  ...
  orc_XmlParser.CreateNodeChild("projectversion",
                                orc_FileInfo.c_ProjectVersion);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetFilerBase::h_LoadFileInfo(
    C_OscXmlParserBase &orc_XmlParser,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscParamSetFilerBase::mh_LoadNodeName(QString &orc_Name,
                                        C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetFilerBase::mh_SaveNodeName(
    const QString &orc_Name, C_OscXmlParserBase &orc_XmlParser) {
  if (orc_XmlParser.SelectNodeChild("name") == "name") {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetFilerBase::mh_LoadDataPoolInfos(
    QList<C_OscParamSetDataPoolInfo> &orc_DataPoolInfos,
  ...
  if (orc_XmlParser.SelectNodeChild("datapools") == "datapools") {
    ...
    if (c_SelectedNode == "datapool") {
      do {
        ...
        s32_Retval = C_OscParamSetFilerBase::mh_LoadDataPoolInfo(
            c_Item, orc_XmlParser, orq_MissingOptionalContent);
        if (s32_Retval == C_NO_ERR) {
      ...
      osc_write_log_error("Loading Dataset data",
                          "Could not find \"datapools\".\"datapool\" node.");
      s32_Retval = C_CONFIG;
    }
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "node");
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetFilerBase::mh_SaveDataPoolInfos(
    const QList<C_OscParamSetDataPoolInfo> &orc_DataPoolInfos,
    C_OscXmlParserBase &orc_XmlParser) {
  // Avoid adding the same datapools twice
  if (orc_XmlParser.SelectNodeChild("datapools") != "datapools") {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("datapools") ==
             "datapools");
    for (uint32_t u32_ItDataPool = 0; u32_ItDataPool < orc_DataPoolInfos.size();
         ++u32_ItDataPool) {
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("datapool") ==
               "datapool");
      C_OscParamSetFilerBase::mh_SaveDataPoolInfo(
          orc_DataPoolInfos[u32_ItDataPool], orc_XmlParser);
      // Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "datapools");
    }
    // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetFilerBase::mh_LoadDataPoolInfo(
    C_OscParamSetDataPoolInfo &orc_DataPoolInfo,
    C_OscXmlParserBase &orc_XmlParser, bool &orq_MissingOptionalContent) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.AttributeExists("crc") == true) {
    ...
    osc_write_log_error(
        "Loading Dataset data",
        "Could not find \"node\".\"datapool\".\"crc\" attribute.");
    s32_Retval = C_CONFIG;
  }
  ...
  if (orc_XmlParser.AttributeExists("nvm-start-address") == true) {
    orc_DataPoolInfo.u32_NvmStartAddress =
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("name") == "name") {
      ...
      osc_write_log_error(
          "Loading Dataset data",
          "Could not find \"node\".\"datapool\".\"name\" node.");
      s32_Retval = C_CONFIG;
    }
  }
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("version") == "version") {
      if (orc_XmlParser.AttributeExists("major") == true) {
        orc_DataPoolInfo.au8_Version[0] =
            static_cast<uint8_t>(orc_XmlParser.GetAttributeUint32("major"));
      } else {
        osc_write_log_error(
            "Loading Dataset data",
            "Could not find \"node\".\"datapool\".\"version\".\"major\" "
            "attribute.");
        s32_Retval = C_CONFIG;
      }
      if ((orc_XmlParser.AttributeExists("minor") == true) &&
          (s32_Retval == C_NO_ERR)) {
        orc_DataPoolInfo.au8_Version[1] =
            static_cast<uint8_t>(orc_XmlParser.GetAttributeUint32("minor"));
      } else {
        osc_write_log_error(
            "Loading Dataset data",
            "Could not find \"node\".\"datapool\".\"version\".\"minor\" "
            "attribute.");
        s32_Retval = C_CONFIG;
      }
      if ((orc_XmlParser.AttributeExists("release") == true) &&
          (s32_Retval == C_NO_ERR)) {
        orc_DataPoolInfo.au8_Version[2] =
            static_cast<uint8_t>(orc_XmlParser.GetAttributeUint32("release"));
      } else {
        osc_write_log_error(
            "Loading Dataset data",
            "Could not find \"node\".\"datapool\".\"version\".\"release\" "
            "attribute.");
        s32_Retval = C_CONFIG;
      }
      // Return
      ...
      osc_write_log_error(
          "Loading Dataset data",
          "Could not find \"node\".\"datapool\".\"version\" node.");
      s32_Retval = C_CONFIG;
    }
  }
  return s32_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetFilerBase::mh_SaveDataPoolInfo(
    const C_OscParamSetDataPoolInfo &orc_DataPoolInfo,
    C_OscXmlParserBase &orc_XmlParser) {
