...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscParamSetRawNodeFiler::h_LoadRawNode(C_OscParamSetRawNode &orc_Node,
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = C_OscParamSetRawNodeFiler::mh_LoadEntries(orc_Node.c_Entries,
                                                             orc_XmlParser);
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetRawNodeFiler::h_SaveRawNode(
    const C_OscParamSetRawNode &orc_Node, C_OscXmlParserBase &orc_XmlParser) {
  C_OscParamSetFilerBase::mh_SaveNodeName(orc_Node.c_Name, orc_XmlParser);
  C_OscParamSetFilerBase::mh_SaveDataPoolInfos(orc_Node.c_DataPools,
                                               orc_XmlParser);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetRawNodeFiler::mh_LoadEntries(
  ...
  if (orc_XmlParser.SelectNodeChild("raw") == "raw") {
    ...
    if (c_SelectedNode == "raw-entry") {
      do {
        ...
        s32_Retval =
            C_OscParamSetRawNodeFiler::mh_LoadEntry(c_Item, orc_XmlParser);
        if (s32_Retval == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetRawNodeFiler::mh_SaveEntries(
    const QList<C_OscParamSetRawEntry> &orc_Entries,
    C_OscXmlParserBase &orc_XmlParser) {
  // Clean start
  if (orc_XmlParser.SelectNodeChild("raw") == "raw") {
  ...
  for (uint32_t u32_Index = 0U; u32_Index < orc_Entries.size(); u32_Index++) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("raw-entry") ==
             "raw-entry");
    C_OscParamSetRawNodeFiler::mh_SaveEntry(orc_Entries[u32_Index],
                                            orc_XmlParser);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "raw");
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscParamSetRawNodeFiler::mh_LoadEntry(C_OscParamSetRawEntry &orc_Entry,
                                        C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetRawNodeFiler::mh_SaveEntry(
    const C_OscParamSetRawEntry &orc_Entry, C_OscXmlParserBase &orc_XmlParser) {
  QString c_Bytes;

  orc_XmlParser.CreateNodeChild("address",
                                QString::number(orc_Entry.u32_StartAddress));
  orc_XmlParser.CreateNodeChild("size",
                                QString::number(orc_Entry.c_Bytes.size()));
