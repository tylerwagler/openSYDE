...
int32_t C_OscParamSetHandler::CreateCleanFileWithoutCrc(
  ...
  if ((c_FileInfo.exists() && c_FileInfo.isFile()) == false) {
    if ((oq_InterpretedDataOnly == true) ||
      ...
      Q_ASSERT(c_XmlParser.CreateAndSelectNodeChild(
                   "opensyde-parameter-sets") == "opensyde-parameter-sets");
      C_OscParamSetFilerBase::h_SaveFileVersion(c_XmlParser);
      C_OscParamSetFilerBase::h_SaveFileInfo(c_XmlParser,
                                             this->mc_Data.c_FileInfo);
      ...
      for (uint32_t u32_ItNode = 0;
        ...
        if (oq_InterpretedDataOnly == false) {
          C_OscParamSetRawNodeFiler::h_SaveRawNode(
              this->mc_RawNodes[u32_ItNode], c_XmlParser);
        }
        C_OscParamSetInterpretedNodeFiler::h_SaveInterpretedNode(
            this->mc_Data.c_InterpretedNodes[u32_ItNode], c_XmlParser);
        // Return
    ...
    if (s32_Retval == C_NO_ERR) {
      if (pc_Parser->SelectRoot() == "opensyde-parameter-sets") {
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          C_OscParamSetRawNodeFiler::h_LoadFileInfo(
              *pc_Parser, this->mc_Data.c_FileInfo, q_MissingOptionalContent);
          ...
          if ((opu16_FileCrc != NULL) && (s32_Retval == C_NO_ERR)) {
            *opu16_FileCrc = static_cast<uint16_t>(
                pc_Parser->GetAttributeUint32("file_crc"));
          }
        ...
        osc_write_log_error(
            ...
            "Could not find root element \"opensyde-parameter-sets\".");
        s32_Retval = C_RD_WR;
      }
    } else {
...
/*! \brief   Load node data
   ...
   \param[in]     oq_InterpretedDataOnly     Flag to load only the interpreted
   data and not the raw data \param[in,out] orq_MissingOptionalContent Flag for
   indication of optional content missing Warning: flag is never set to false if
   optional content is present
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetHandler::m_LoadNodes(C_OscXmlParser &orc_XmlParser,
                                          const bool oq_InterpretedDataOnly,
  ...
  if (orc_XmlParser.SelectNodeChild("nodes") == "nodes") {
    ...
    if (c_SelectedNode == "node") {
      do {
        ...
        // Content
        if (oq_InterpretedDataOnly == false) {
          s32_Retval = C_OscParamSetRawNodeFiler::h_LoadRawNode(
              c_RawItem, orc_XmlParser, orq_MissingOptionalContent);
          if (s32_Retval == C_NO_ERR) {
        ...
        if (s32_Retval == C_NO_ERR) {
          s32_Retval = C_OscParamSetInterpretedNodeFiler::h_LoadInterpretedNode(
              c_InterpretedItem, orc_XmlParser, orq_MissingOptionalContent);
          if (s32_Retval == C_NO_ERR) {
      ...
      osc_write_log_error("Loading Dataset data",
                          "Could not find \"nodes\".\"node\" node.");
      s32_Retval = C_CONFIG;
    }
    // Return
