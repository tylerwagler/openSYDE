...
/*! \brief   Load parameter set node
   ...
   "opensyde-parameter-sets" post-condition: the passed XML parser has the
   active node set to the same "opensyde-parameter-sets"

   \param[out]     orc_Node                     data storage
   \param[in,out]  orc_XmlParser                XML with specified node active
   \param[in,out]  orq_MissingOptionalContent   Flag for indication of optional
   content missing Warning: flag is never set to false if optional content is
   present
...
int32_t C_OscParamSetInterpretedNodeFiler::h_LoadInterpretedNode(
  ...
  int32_t s32_Retval =
      C_OscParamSetFilerBase::mh_LoadNodeName(orc_Node.c_Name, orc_XmlParser);

  if (s32_Retval == C_NO_ERR) {
    QList<C_OscParamSetDataPoolInfo> c_DataPoolInfos;
    s32_Retval = C_OscParamSetFilerBase::mh_LoadDataPoolInfos(
        c_DataPoolInfos, orc_XmlParser, orq_MissingOptionalContent);
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = C_OscParamSetInterpretedNodeFiler::mh_LoadDataPools(
          orc_Node.c_DataPools, c_DataPoolInfos, orc_XmlParser);
    }
  }
  return s32_Retval;
}
...
void C_OscParamSetInterpretedNodeFiler::h_SaveInterpretedNode(
    const C_OscParamSetInterpretedNode &orc_Node,
  ...
  for (uint32_t u32_ItDataPool = 0;
    ...
    const C_OscParamSetInterpretedDataPool &rc_DataPool =
  ...
  C_OscParamSetInterpretedNodeFiler::mh_SaveDataPools(orc_Node.c_DataPools,
                                                      orc_XmlParser);
}
...
int32_t C_OscParamSetInterpretedNodeFiler::mh_LoadDataPools(
  ...
  if (orc_XmlParser.SelectNodeChild("interpreted") == "interpreted") {
    ...
    if (c_SelectedNode == "datapool") {
      ...
      do {
        if (u32_ItDataPoolInfo < orc_DataPoolInfos.size()) {
          ...
          ++u32_ItDataPoolInfo;
          // Content
          s32_Retval = C_OscParamSetInterpretedNodeFiler::mh_LoadDataPool(
              c_Item, orc_XmlParser);
          if (s32_Retval == C_NO_ERR) {
            orc_DataPools.push_back(c_Item);
          }
        } else {
          osc_write_log_error("Loading Dataset data",
                              "Information for too many data pools present.");
          s32_Retval = C_CONFIG;
        }
      ...
      osc_write_log_error(
          ...
          "Could not find \"node\".\"interpreted\".\"datapool\" node.");
      s32_Retval = C_CONFIG;
    }
    // Check  number of found data pools
    if ((s32_Retval == C_NO_ERR) &&
        (orc_DataPools.size() != orc_DataPoolInfos.size())) {
      osc_write_log_error(
          "Loading Dataset data",
          "Number of Datapools described in file does not match number of "
          "defined Datapools.");
      s32_Retval = C_CONFIG;
    }
    // Return
    ...
    osc_write_log_error("Loading Dataset data",
                        "Could not find \"node\".\"interpreted\" node.");
    s32_Retval = C_CONFIG;
  }
  return s32_Retval;
}
...
void C_OscParamSetInterpretedNodeFiler::mh_SaveDataPools(
    const QList<C_OscParamSetInterpretedDataPool> &orc_DataPools,
    C_OscXmlParserBase &orc_XmlParser) {
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("interpreted") ==
           "interpreted");
  for (uint32_t u32_Index = 0U; u32_Index < orc_DataPools.size(); u32_Index++) {
    ...
    C_OscParamSetInterpretedNodeFiler::mh_SaveDataPool(orc_DataPools[u32_Index],
                                                       orc_XmlParser);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "interpreted");
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetInterpretedNodeFiler::mh_LoadDataPool(
    C_OscParamSetInterpretedDataPool &orc_DataPool,
    C_OscXmlParserBase &orc_XmlParser) {
  const int32_t s32_Retval = C_OscParamSetInterpretedNodeFiler::mh_LoadLists(
      orc_DataPool.c_Lists, orc_XmlParser);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetInterpretedNodeFiler::mh_SaveDataPool(
    const C_OscParamSetInterpretedDataPool &orc_DataPool,
    C_OscXmlParserBase &orc_XmlParser) {
  C_OscParamSetInterpretedNodeFiler::mh_SaveLists(orc_DataPool.c_Lists,
                                                  orc_XmlParser);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetInterpretedNodeFiler::mh_LoadLists(
    QList<C_OscParamSetInterpretedList> &orc_Lists,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("lists") == "lists") {
    ...
    if (c_SelectedNode == "list") {
      do {
        ...
        s32_Retval = C_OscParamSetInterpretedNodeFiler::mh_LoadList(
            c_Item, orc_XmlParser);
        if (s32_Retval == C_NO_ERR) {
      ...
      osc_write_log_error(
          "Loading Dataset data",
          "Could not find "
          "\"node\".\"interpreted\".\"datapool\".\"lists\".\"list\" node.");
      s32_Retval = C_CONFIG;
    }
    // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetInterpretedNodeFiler::mh_SaveLists(
    const QList<C_OscParamSetInterpretedList> &orc_Lists,
    C_OscXmlParserBase &orc_XmlParser) {
  ...
  for (uint32_t u32_Index = 0U; u32_Index < orc_Lists.size(); u32_Index++) {
    ...
    C_OscParamSetInterpretedNodeFiler::mh_SaveList(orc_Lists[u32_Index],
                                                   orc_XmlParser);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "lists");
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetInterpretedNodeFiler::mh_LoadList(
    C_OscParamSetInterpretedList &orc_List, C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("name") == "name") {
    ...
    osc_write_log_error("Loading Dataset data",
                        "Could not find \"list\".\"name\" node.");
    s32_Retval = C_CONFIG;
  }
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscParamSetInterpretedNodeFiler::mh_LoadElements(
        orc_List.c_Elements, orc_XmlParser);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetInterpretedNodeFiler::mh_SaveList(
    const C_OscParamSetInterpretedList &orc_List,
    C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateNodeChild("name", orc_List.c_Name);
  C_OscParamSetInterpretedNodeFiler::mh_SaveElements(orc_List.c_Elements,
                                                     orc_XmlParser);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetInterpretedNodeFiler::mh_LoadElements(
    QList<C_OscParamSetInterpretedElement> &orc_Elements,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("elements") == "elements") {
    ...
    if (c_SelectedNode == "element") {
      do {
        ...
        s32_Retval = C_OscParamSetInterpretedNodeFiler::mh_LoadElement(
            c_Item, orc_XmlParser);
        if (s32_Retval == C_NO_ERR) {
      ...
      osc_write_log_error("Loading Dataset data",
                          "Could not find \"elements\".\"element\" node.");
      s32_Retval = C_CONFIG;
    }
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "list");
    ...
    osc_write_log_error("Loading Dataset data",
                        "Could not find \"list\".\"elements\" node.");
    s32_Retval = C_CONFIG;
  }
  return s32_Retval;
}
...
void C_OscParamSetInterpretedNodeFiler::mh_SaveElements(
    const QList<C_OscParamSetInterpretedElement> &orc_Elements,
    C_OscXmlParserBase &orc_XmlParser) {
  ...
  for (uint32_t u32_Index = 0U; u32_Index < orc_Elements.size(); u32_Index++) {
    ...
    C_OscParamSetInterpretedNodeFiler::mh_SaveElement(orc_Elements[u32_Index],
                                                      orc_XmlParser);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "elements");
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscParamSetInterpretedNodeFiler::mh_LoadElement(
    C_OscParamSetInterpretedElement &orc_Element,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("name") == "name") {
    ...
    osc_write_log_error("Loading Dataset data",
                        "Could not find \"element\".\"name\" node.");
    s32_Retval = C_CONFIG;
  }

  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscNodeDataPoolFiler::h_LoadDataPoolElementType(
        orc_Element.c_NvmValue, orc_XmlParser);
    if ((s32_Retval == C_NO_ERR) &&
        (orc_XmlParser.SelectNodeChild("value") == "value")) {
      QString c_Error;
      s32_Retval = C_OscNodeDataPoolFiler::h_LoadDataPoolElementValue(
          orc_Element.c_NvmValue, orc_XmlParser, true, &c_Error);
      if (s32_Retval != C_NO_ERR) {
        osc_write_log_error("Loading Dataset data",
      ...
      osc_write_log_error("Loading Dataset data",
                          "Could not find \"element\".\"value\" node.");
      s32_Retval = C_CONFIG;
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscParamSetInterpretedNodeFiler::mh_SaveElement(
    const C_OscParamSetInterpretedElement &orc_Element,
    C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateNodeChild("name", orc_Element.c_Name);
  C_OscNodeDataPoolFiler::h_SaveDataPoolElementType(orc_Element.c_NvmValue,
                                                    orc_XmlParser);
  C_OscNodeDataPoolFiler::h_SaveDataPoolElementValue(
      "value", orc_Element.c_NvmValue, orc_XmlParser);
}
