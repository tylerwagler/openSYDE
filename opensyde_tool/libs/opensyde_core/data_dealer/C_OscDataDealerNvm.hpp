...
class C_OscDataDealerNvm : public C_OscDataDealer {
public:
  ...
  virtual int32_t NvmReadList(const uint32_t ou32_DataPoolIndex,
                              const uint32_t ou32_ListIndex,
                              uint8_t *const opu8_NrCode);
  int32_t NvmNotifyOfChanges(const uint8_t ou8_DataPoolIndex,
                             const uint8_t ou8_ListIndex,
                             bool &orq_ApplicationAcknowledge,
                             uint8_t *const opu8_NrCode);

protected:
  int32_t m_NvmReadListRaw(const C_OscNodeDataPoolList &orc_List,
                           QByteArray &orc_Values, uint8_t *const opu8_NrCode);
  int32_t m_SaveDumpToList(const QByteArray &orc_Values,
                           C_OscNodeDataPoolList &orc_List) const;
  int32_t m_SaveDumpValuesToListValues(const QByteArray &orc_Values,
                                       C_OscNodeDataPoolList &orc_List) const;
  static int32_t
  mh_AdaptProtocolReturnValue(const int32_t os32_ProtReturnValue);
};
