...
/*!
   ...
   The application is responsible to not "::Cycle" and poll at the same time
...
C_OscDataDealer::C_OscDataDealer(void)
    : mpc_Node(NULL), mu32_NodeIndex(0U), mpc_DiagProtocol(NULL) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscDataDealer::C_OscDataDealer(C_OscNode *const opc_Node,
                                 const uint32_t ou32_NodeIndex,
                                 C_OscDiagProtocolBase *const opc_DiagProtocol)
    : mpc_Node(opc_Node), mu32_NodeIndex(ou32_NodeIndex),
...
/*! \brief   Initializing the data dealer if the default constructor was used
   ...
   \param[in]     opc_Node                      Pointer to node of data dealer
   \param[in]     ou32_NodeIndex                Index of node of data dealer
   \param[in]     opc_DiagProtocol              Pointer to used diagnostic
   protocol
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealer::Initialize(
...
/*! \brief   Read data from server's data pool
   ...
   * places the read data in the associated data pool element (in correct
   endianness)
   ...
   protocol driver properly initialized ?) C_RANGE     specified data pool,
   list, element does not exist in data pools of configured node protocol driver
   reported parameter out of range (does the protocol support the index range ?)
   ...
   C_OVERFLOW  size of data received from server does not match size of
   specified data pool element
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealer::DataPoolRead(const uint8_t ou8_DataPoolIndex,
  ...
  if ((mpc_Node == NULL) || (mpc_DiagProtocol == NULL)) {
    ...
    C_OscNodeDataPoolListElement *const pc_Element =
        this->mpc_Node->GetDataPoolListElement(
    ...
    if (pc_Element == NULL) {
      ...
      if (pc_Element->GetArray() == false) {
        s32_Return = mpc_DiagProtocol->DataPoolReadNumeric(
            ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, c_Data,
            opu8_NrCode);
      } else {
        s32_Return = mpc_DiagProtocol->DataPoolReadArray(
            ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, c_Data,
            opu8_NrCode);
      }
      if (s32_Return == C_NO_ERR) {
        ...
        if (c_Data.size() != pc_Element->GetSizeByte()) {
          ...
          if (mpc_DiagProtocol->GetEndianness() ==
...
/*! \brief   Write data to server's data pool
   ...
   * write the data to the server's data pool using the installed diagnostic
   protocol
   ...
   list, element does not exist in data pools of configured node protocol driver
   reported parameter out of range (does the protocol support the index range ?)
   C_OVERFLOW  value in data pool does not lie within defined min/max range
   (checked on client side) C_TIMEOUT   expected response not received within
   timeout C_NOACT     could not send request (e.g. Tx buffer full) C_RD_WR
   protocol driver reported protocol violation C_WARN      error response
   received C_COM       communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealer::DataPoolWrite(const uint8_t ou8_DataPoolIndex,
                                       const uint16_t ou16_ListIndex,
                                       const uint16_t ou16_ElementIndex,
                                       uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  if ((mpc_Node == NULL) || (mpc_DiagProtocol == NULL)) {
    ...
    if (pc_Element == NULL) {
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        // use communication function matching the element type
        if (pc_Element->GetArray() == false) {
          s32_Return = mpc_DiagProtocol->DataPoolWriteNumeric(
              ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, c_Data,
              opu8_NrCode);
        } else {
          s32_Return = mpc_DiagProtocol->DataPoolWriteArray(
              ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, c_Data,
              opu8_NrCode);
        }
      }
...
/*! \brief   Read data from server's NVM
   ...
   * places the read data in the associated data pool element (in correct
   ...
   The function does not verify whether the specified data pool is an NVM data
   pool.
   ...
   protocol driver properly initialized ?) C_RANGE     specified data pool,
   list, element does not exist in data pools of configured node protocol driver
   reported parameter out of range (does the protocol support the address range
   ?) C_TIMEOUT   expected response not received within timeout C_NOACT could
   not send request (e.g. Tx buffer full) C_RD_WR     protocol driver reported
   protocol violation C_WARN      error response received C_COM       expected
   server response not received because of communication error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealer::NvmRead(const uint8_t ou8_DataPoolIndex,
                                 const uint16_t ou16_ListIndex,
                                 const uint16_t ou16_ElementIndex,
                                 uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  if ((mpc_Node == NULL) || (mpc_DiagProtocol == NULL)) {
    ...
    if (pc_Element == NULL) {
      ...
      s32_Return = mpc_DiagProtocol->NvmRead(pc_Element->u32_NvmStartAddress,
                                             c_Data, opu8_NrCode);
      if (s32_Return == C_NO_ERR) {
        // we have data
        // convert to native endianness depending on the type ...
...
/*! \brief   Write data to server's NVM
   ...
   * write the data to the server's NVM using the installed diagnostic protocol
   ...
   If the protocol requires a "start" -> "write" -> "finalize" sequence for
   writing to the NVM the application is responsible to handle it. This function
   can be called multiple times within such a transfer bracket.
   ...
   timeout C_NOACT     could not send request (e.g. Tx buffer full) C_RD_WR
   protocol driver reported protocol violation C_WARN      error response
   received C_COM       expected server response not received because of
   communication error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealer::NvmWrite(const uint8_t ou8_DataPoolIndex,
                                  const uint16_t ou16_ListIndex,
                                  const uint16_t ou16_ElementIndex,
                                  uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  if ((mpc_Node == NULL) || (mpc_DiagProtocol == NULL)) {
    ...
    if (pc_Element == NULL) {
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        s32_Return = mpc_DiagProtocol->NvmWrite(pc_Element->u32_NvmStartAddress,
                                                c_Data, opu8_NrCode);
      }
    }
  }
...
/*! \brief   Handler for reception of async response
   ...
   Called by the DataDealer after it has placed the data in the data pool
   content structure associated with the installed node.
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealer::m_OnReadDataPoolEventReceived(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex) {
  osc_write_log_warning(
      "Asynchronous communication",
      QString::asprintf("C_OscDataDealer: ReadDataPool event received but no "
                        "notification handler implemented. "
                        "Element (dp: %d list: %d element: %d) !",
                        ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex));
}
...
/*! \brief   Handler for reception of async error response
   ...
   Called by the DataDealer after it has received an error response for a cyclic
   transmission.
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealer::m_OnReadDataPoolEventErrorReceived(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_ErrorCode) {
  osc_write_log_warning(
      "Asynchronous communication",
      QString::asprintf(
          "C_OscDataDealer: ReadDataPool error event received but no "
          "notification handler "
          "implemented. Element (dp: %d list: %d element: %d) Code: %d !",
          ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, ou8_ErrorCode));
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealer::m_OnReadDataPoolNvmEventReceived(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex) {
  osc_write_log_warning(
      "Asynchronous communication",
      QString::asprintf("C_OscDataDealer: NvmRead event received but no "
                        "notification handler implemented. "
                        "Element (dp: %d list: %d element: %d) !",
                        ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex));
}
...
void C_OscDataDealer::m_Init(void) {
  if (this->mpc_DiagProtocol != NULL) {
    this->mpc_DiagProtocol->Initialize(
        &C_OscDataDealer::mh_ReadDataPoolDataEventReceived,
...
void C_OscDataDealer::mh_ReadDataPoolDataEventReceived(
  ...
  C_OscDataDealer *const pc_Dealer =
      reinterpret_cast<C_OscDataDealer *>(opv_Instance);

  Q_ASSERT(pc_Dealer != NULL);
  if (pc_Dealer != NULL) {
    pc_Dealer->m_ReadDataPoolDataEventReceived(
        ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, orc_Value);
  }
}
...
void C_OscDataDealer::m_ReadDataPoolDataEventReceived(
  ...
  if ((this->mpc_Node == NULL) || (this->mpc_DiagProtocol == NULL)) {
    osc_write_log_warning("Asynchronous communication",
                          "C_OscDataDealer: ReadDataPool event received but no "
                          "node or diagnostic protocol known!");
  } else {
    C_OscNodeDataPoolListElement *const pc_Element =
        this->mpc_Node->GetDataPoolListElement(
            ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex);
    if (pc_Element == NULL) {
      osc_write_log_warning(
          "Asynchronous communication",
          QString::asprintf("C_OscDataDealer: ReadDataPool event received but "
                            "referenced Datapool element ("
                            "dp: %d list: %d element: %d) not known!",
                            ou8_DataPoolIndex, ou16_ListIndex,
                            ou16_ElementIndex));
    } else {
      // we have data: is the size as expected ?
      // Not all protocols are able to report the exact size of incoming data.
      // Early openSYDE CAN server protocol implementations always would send 4
      // bytes even if not all of them were
      //  used.
      // So we do not check for an exact match but for "do we have enough ?"
      if (orc_Value.size() < pc_Element->GetSizeByte()) {
        osc_write_log_warning(
            "Asynchronous communication",
            QString::asprintf("C_OscDataDealer: ReadDataPool event received "
                              "but size does not match Datapool "
                              "element (dp: %d list: %d element: %d)!",
                              ou8_DataPoolIndex, ou16_ListIndex,
                              ou16_ElementIndex));
      } else {
        ...
        // inform application
        m_OnReadDataPoolEventReceived(ou8_DataPoolIndex, ou16_ListIndex,
                                      ou16_ElementIndex);
      }
    }
  }
...
/*! \brief   Handling an async received negative read datapool data event
   ...
   Called by specific protocol implementation when a negative message response
   to a event driven transmission was received.
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealer::mh_ReadDataPoolDataEventErrorReceived(
    void *const opv_Instance, const uint8_t ou8_DataPoolIndex,
    const uint16_t ou16_ListIndex, const uint16_t ou16_ElementIndex,
  ...
  Q_ASSERT(pc_Dealer != NULL);
  if (pc_Dealer != NULL) {
    pc_Dealer->m_ReadDataPoolDataEventErrorReceived(
        ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, ou8_NrCode);
  }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealer::m_ReadDataPoolDataEventErrorReceived(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_NrCode) {
  // check for weird misconfiguration:
  if ((this->mpc_Node == NULL) || (this->mpc_DiagProtocol == NULL)) {
    osc_write_log_warning("Asynchronous communication",
                          "C_OscDataDealer: ReadDataPool error event received "
                          "but no node or diagnostic protocol known!");
  } else {
    const C_OscNodeDataPoolListElement *const pc_Element =
        this->mpc_Node->GetDataPoolListElement(
            ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex);
    if (pc_Element == NULL) {
      osc_write_log_warning(
          "Asynchronous communication",
          QString::asprintf("C_OscDataDealer: ReadDataPool error event "
                            "received but referenced Datapool element "
                            "(dp: %d list: %d element: %d) not known!",
                            ou8_DataPoolIndex, ou16_ListIndex,
                            ou16_ElementIndex));
    } else {
      // report error:
      m_OnReadDataPoolEventErrorReceived(ou8_DataPoolIndex, ou16_ListIndex,
                                         ou16_ElementIndex, ou8_NrCode);
    }
  }
}
