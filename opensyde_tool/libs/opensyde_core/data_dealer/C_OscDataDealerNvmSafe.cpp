...
/*!
   \file
   \brief       Encapsulates safety relevant sequences for NVM access.
   (implementation)

   As the "NVM" classes inherit from the data dealer we can only handle the safe
   data of one node; as a result the application will need to handle parameters
   for multiple nodes (e.g. multiple files with parameter set values; separate
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscDataDealerNvmSafe::C_OscDataDealerNvmSafe(
    C_OscNode *const opc_Node, const uint32_t ou32_NodeIndex,
...
/*! \brief   Checking all CRCs of "NVM" datapool lists of one node
   ...
   The function calculates and checks the CRCs over all datapool lists with the
   flag q_NvmCrcActive is set to true of all datapools of type "NVM".
   ...
   C_CONFIG    No diagnostic protocol are known (was this class properly
   Initialize()d ?)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvmSafe::NvmSafeCheckCrcs(const C_OscNode &orc_Node) const {
  ...
  if (this->mpc_DiagProtocol == NULL) {
    ...
    for (u32_DataPoolCounter = 0U;
         u32_DataPoolCounter < orc_Node.c_DataPools.size();
         ++u32_DataPoolCounter) {
      if ((orc_Node.c_DataPools[u32_DataPoolCounter].e_Type ==
           C_OscNodeDataPool::eNVM) ||
          (orc_Node.c_DataPools[u32_DataPoolCounter].e_Type ==
           C_OscNodeDataPool::eHALC_NVM)) {
        uint32_t u32_ListCounter;

        for (u32_ListCounter = 0U;
             u32_ListCounter <
             orc_Node.c_DataPools[u32_DataPoolCounter].c_Lists.size();
             ++u32_ListCounter) {
          const C_OscNodeDataPoolList &rc_List =
              orc_Node.c_DataPools[u32_DataPoolCounter]
                  .c_Lists[u32_ListCounter];
          ...
          } else if (orc_Node.c_DataPools[u32_DataPoolCounter].q_IsSafety ==
                     true) {
...
/*! \brief   Writing of changed NVM values
   ...
   The function writes all values of all datapool elements of all lists of all
   datapools of the original node of the "C_OscDataDealer" marked as "changed"
   to the ECU NVM without updating the CRC(s). Only lists in datapools of type
   "NVM" will be considered.

   \param[out]  orc_ChangedElements         Container with all detected changed
   elements. The container will not be cleared.
   \param[in]   opc_AdditionalListsToUpdate Container with additional lists to
   update via this process even in the case they don't have any changed elements
                                            The specified node indices will be
   ignored.
   ...
   C_CONFIG    no node or diagnostic protocol are known (was this class properly
   Initialized ?) protocol driver reported configuration error (was the protocol
   driver properly initialized ?) C_OVERFLOW  At least one value lies outside of
   the defined minimum and maximum range. (checked by client side) C_BUSY No
   changed value found and no additional lists specified (checked by client
   side) C_TIMEOUT   Expected server response not received within timeout
...
int32_t C_OscDataDealerNvmSafe::NvmSafeWriteChangedValues(
  ...
  if ((this->mpc_Node == NULL) || (this->mpc_DiagProtocol == NULL)) {
    ...
    // Check the additional parameter first to not write anything if this one
    // does contain invalid indices
    if ((opc_AdditionalListsToUpdate != NULL) &&
        (opc_AdditionalListsToUpdate->size() > 0UL)) {
      s32_Return = C_NO_ERR;
      for (uint32_t u32_ItAdditionalIndex = 0;
           (u32_ItAdditionalIndex < opc_AdditionalListsToUpdate->size()) &&
           (s32_Return == C_NO_ERR);
        ...
        if (rc_CurAdditionalIndex.u32_DataPoolIndex <
            this->mpc_Node->c_DataPools.size()) {
          const C_OscNodeDataPool &rc_CurDataPool =
              this->mpc_Node
                  ->c_DataPools[rc_CurAdditionalIndex.u32_DataPoolIndex];
          if (rc_CurAdditionalIndex.u32_ListIndex <
              rc_CurDataPool.c_Lists.size()) {
            // No problem found
    ...
    if (s32_Return != C_CONFIG) {
      ...
      for (u32_DataPoolCounter = 0U;
           u32_DataPoolCounter < this->mpc_Node->c_DataPools.size();
           ++u32_DataPoolCounter) {
        c_ElementId.u32_DataPoolIndex = u32_DataPoolCounter;

        C_OscNodeDataPool *const pc_DataPool =
            &this->mpc_Node->c_DataPools[u32_DataPoolCounter];
        if ((pc_DataPool->e_Type == C_OscNodeDataPool::eNVM) ||
          ...
          for (u32_ListCounter = 0U;
               u32_ListCounter < pc_DataPool->c_Lists.size();
               ++u32_ListCounter) {
            for (u32_Elementcounter = 0U;
                 u32_Elementcounter <
                 pc_DataPool->c_Lists[u32_ListCounter].c_Elements.size();
                 ++u32_Elementcounter) {
              if (pc_DataPool->c_Lists[u32_ListCounter]
                      .c_Elements[u32_Elementcounter]
                      .q_NvmValueChanged == true) {
          ...
          if (u16_AccessCount > 0U) {
            ...
            s32_Return = this->mpc_DiagProtocol->NvmWriteStartTransaction(
                static_cast<uint8_t>(u32_DataPoolCounter), u16_AccessCount);
            if (s32_Return == C_NO_ERR) {
              ...
              for (u32_ListCounter = 0U;
                ...
                C_OscNodeDataPoolList *const pc_List =
                    &pc_DataPool->c_Lists[u32_ListCounter];
                ...
                for (u32_Elementcounter = 0U;
                     ...
                     ++u32_Elementcounter) {
                  C_OscNodeDataPoolListElement *const pc_Element =
                      &pc_List->c_Elements[u32_Elementcounter];
                  ...
                  if (pc_Element->q_NvmValueChanged == true) {
                    if (pc_Element->CheckNvmValueRange() == C_NO_ERR) {
                      ...
                      if (this->mpc_DiagProtocol->GetEndianness() ==
                        ...
                        pc_Element->c_NvmValue.GetValueAsBigEndianBlob(
                            c_ElementData);
                      } else {
                        pc_Element->c_NvmValue.GetValueAsLittleEndianBlob(
                            c_ElementData);
                      }

                      // Element value was changed and is valid
                      s32_Return = this->mpc_DiagProtocol->NvmWrite(
                          pc_Element->u32_NvmStartAddress, c_ElementData,
                          opu8_NrCode);
                      // Adapt errorcode
                      s32_Return =
                          C_OscDataDealerNvm::mh_AdaptProtocolReturnValue(
                              s32_Return);
                      ...
                      if (s32_Return == C_NO_ERR) {
                        ...
                        // Save the list index as changed list. Will be used by
                        // NvmSafeReadValues.
              ...
              s32_Return =
                  C_OscDataDealerNvm::mh_AdaptProtocolReturnValue(s32_Return);
            }
          ...
          // Stop if service failure, continue with C_BUSY (don't stop if no
          // elements found in first datapool)
...
/*! \brief   Reads values of all changed lists of ECU
   ...
   The function reads the values of all datapool elements of all datapool lists
   that contain datapool elements that were written by the preceding call to
   "NvmSafeWriteChangedValues". The function stores the read values in the copy
   of the original "C_OscNode" instance of "C_OscDataDealer". Read values are
   marked by setting the "Valid" flag to true.
   ...
   C_CONFIG    No node or diagnostic protocol are known (was this class properly
   Initialize()d ?) protocol driver reported configuration error (was the
   protocol driver properly initialized ?) C_OVERFLOW  At least one list has no
   elements. C_RD_WR     No list contains datapool elements that were written by
   the preceding call to "NvmSafeWriteChangedValues" C_RANGE     At least one
   index of a datapool or a list of changed lists is invalid or datapool element
   size configuration does not match with count of read bytes C_TIMEOUT Expected
   server response not received within timeout C_NOACT     Could not send
   request (e.g. Tx buffer full) C_WARN      Server sent error response C_COM
   expected server response not received because of communication error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvmSafe::NvmSafeReadValues(const C_OscNode *(&orpc_NodeCopy),
                                          uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  if ((this->mpc_Node == NULL) || (this->mpc_DiagProtocol == NULL)) {
    ...
    if (this->mc_ChangedLists.size() > 0) {
      ...
      for (c_ItChangedList = this->mc_ChangedLists.begin();
           c_ItChangedList != this->mc_ChangedLists.end(); ++c_ItChangedList) {
        if (((*c_ItChangedList).u32_DataPoolIndex <
             this->mc_NodeCopy.c_DataPools.size()) &&
            ((*c_ItChangedList).u32_ListIndex <
             this->mc_NodeCopy.c_DataPools[(*c_ItChangedList).u32_DataPoolIndex]
                 .c_Lists.size())) {
          ...
          for (u32_ElementCounter = 0U;
               u32_ElementCounter < rc_List.c_Elements.size();
               ++u32_ElementCounter) {
          ...
          if (s32_Return == C_NO_ERR) {
            // Store the read values into the copy without checking and updating
            // the CRC. The valid flag will be set to true if the element was
            // read
...
/*! \brief   Calculates the CRCs of changed lists and writes them to the ECU
   ...
   The function calculates the CRCs over all datapool lists read by the
   preceding call to "NvmSafeReadValues" and writes them to the ECU NVM.
   ...
   C_CONFIG    No node or diagnostic protocol are known (was this class properly
   Initialized ?) protocol driver reported configuration error (was the protocol
   driver properly initialized ?) C_CHECKSUM  At least one datapool of type
   "NVM" has the flag q_IsSafety set to true and at least one of its lists has
   the flag q_NvmCrcActive set to false. C_RANGE     The size of at least one
   list of NVM datapools of the copy of C_OscNode differs to the original
   instance. The count of lists of NVM datapools of the copy of C_OscNode
   differs to the original instance. The count of the datapools of the copy of
   C_OscNode differs to the original instance. C_BUSY      No list contains
   datapool elements that were written by the preceding call to
   "NvmSafeWriteChangedValues" C_OVERFLOW  At least one changed list has no
   elements. C_DEFAULT   At least one element of the changed lists has the flag
   q_IsValid set to false. C_TIMEOUT   Expected server response not received
   within timeout C_NOACT     Could not send request (e.g. Tx buffer full)
   C_WARN      Server sent error response
   C_RD_WR     unexpected content in server response
   C_COM       expected server response not received because of communication
   error
...
int32_t C_OscDataDealerNvmSafe::NvmSafeWriteCrcs(uint8_t *const opu8_NrCode) {
  ...
  if ((this->mpc_Node == NULL) || (this->mpc_DiagProtocol == NULL)) {
    ...
    if (this->mc_NodeCopy.c_DataPools.size() ==
        this->mpc_Node->c_DataPools.size()) {
      ...
      for (u32_DataPoolCounter = 0U;
        ...
        if (this->mc_NodeCopy.c_DataPools[u32_DataPoolCounter].c_Lists.size() ==
          ...
          for (u32_ListCounter = 0U;
               u32_ListCounter <
               this->mc_NodeCopy.c_DataPools[u32_DataPoolCounter]
                   .c_Lists.size();
               ++u32_ListCounter) {
            if (this->mc_NodeCopy.c_DataPools[u32_DataPoolCounter]
                    .c_Lists[u32_ListCounter]
                    .u32_NvmSize !=
                this->mpc_Node->c_DataPools[u32_DataPoolCounter]
                    .c_Lists[u32_ListCounter]
      ...
      for (c_ItChangedList = this->mc_ChangedLists.begin();
        ...
        if ((u32_DataPoolCounter < this->mc_NodeCopy.c_DataPools.size()) &&
          ...
          if (this->mc_NodeCopy.c_DataPools[u32_DataPoolCounter]
            ...
            // No elements in list
            s32_Return = C_OVERFLOW;
          } else if ((this->mc_NodeCopy.c_DataPools[u32_DataPoolCounter]
                          .q_IsSafety == true) &&
                     (this->mc_NodeCopy.c_DataPools[u32_DataPoolCounter]
                          .c_Lists[u32_ListCounter]
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (c_ItChangedList = this->mc_ChangedLists.begin();
        ...
        if (rc_List.q_NvmCrcActive == true) {
          ...
          s32_Return = this->mpc_DiagProtocol->NvmWrite(
              rc_List.u32_NvmStartAddress, c_CrcData, opu8_NrCode);
...
/*! \brief   Create parameter set file based on current ECU values
   ...
   C_CHECKSUM CRC over the values of a parameter list read from the ECU does not
   match those values C_CONFIG   No valid diagnostic protocol is set or no valid
   pointer to the original instance of "C_OscNode" is set in "C_OscDataDealer"
...
int32_t C_OscDataDealerNvmSafe::NvmSafeReadParameterValues(
  ...
  // Reset state
  this->me_CreateParameterSetWorkflowState =
      C_OscDataDealerNvmSafe::eCPSFS_IDLE;
  if (C_OscDataDealerNvmSafe::me_ParameterSetFileState ==
    ...
    if (orc_ListIds.size() > 0) {
      if ((this->mpc_Node != NULL) && (this->mpc_DiagProtocol != NULL)) {
        ...
        for (uint32_t u32_ItList = 0;
             (u32_ItList < orc_ListIds.size()) && (s32_Retval == C_NO_ERR);
             ++u32_ItList) {
          const C_OscNodeDataPoolListId &rc_DataPoolListId =
              orc_ListIds[u32_ItList];
          if ((rc_DataPoolListId.u32_NodeIndex == this->mu32_NodeIndex) &&
              (rc_DataPoolListId.u32_DataPoolIndex <
               this->mpc_Node->c_DataPools.size())) {
            C_OscNodeDataPool &rc_DataPool =
                this->mpc_Node
                    ->c_DataPools[rc_DataPoolListId.u32_DataPoolIndex];
            if (rc_DataPoolListId.u32_ListIndex < rc_DataPool.c_Lists.size()) {
              const C_OscNodeDataPoolList &rc_List =
                  rc_DataPool.c_Lists[rc_DataPoolListId.u32_ListIndex];
              if (rc_List.q_NvmCrcActive == true) {
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          for (uint32_t u32_ItList = 0;
            ...
            if ((rc_DataPoolListId.u32_NodeIndex == this->mu32_NodeIndex) &&
              ...
              C_OscNodeDataPool &rc_DataPool =
                  this->mpc_Node
                      ->c_DataPools[rc_DataPoolListId.u32_DataPoolIndex];
              if (rc_DataPoolListId.u32_ListIndex <
                  rc_DataPool.c_Lists.size()) {
                C_OscParamSetRawEntry c_RawEntry;
                C_OscNodeDataPoolList &rc_List =
                    rc_DataPool.c_Lists[rc_DataPoolListId.u32_ListIndex];
                s32_Retval = m_CreateRawEntryAndPrepareInterpretedData(
                    rc_List, c_RawEntry, opu8_NrCode);
                c_RawNode.c_Entries.push_back(c_RawEntry);
              } else {
                s32_Retval = C_RANGE;
              }
          ...
          if (s32_Retval == C_NO_ERR) {
            ...
            // Data pools
            for (uint32_t u32_ItCurListId = 0;
                 (u32_ItCurListId < orc_ListIds.size()) &&
                 (s32_Retval == C_NO_ERR);
                 ++u32_ItCurListId) {
              const C_OscNodeDataPoolListId &rc_DataPoolListId =
                  orc_ListIds[u32_ItCurListId];
              if (rc_DataPoolListId.u32_DataPoolIndex <
                  this->mpc_Node->c_DataPools.size()) {
                C_OscNodeDataPool &rc_DataPool =
                    this->mpc_Node
                        ->c_DataPools[rc_DataPoolListId.u32_DataPoolIndex];
                if (rc_DataPoolListId.u32_ListIndex <
                  ...
                  //------------------------
                  Q_ASSERT(c_AlreadyUsedDataPoolIndices.size() ==
                           c_InterpretedNode.c_DataPools.size());
                  for (uint32_t u32_ItCurrentDataPool = 0;
                    ...
                    if (c_AlreadyUsedDataPoolIndices[u32_ItCurrentDataPool] ==
                      ...
                      // Interpreted
                      for (uint32_t u32_ItListId = 0;
                           u32_ItListId < orc_ListIds.size(); ++u32_ItListId) {
                        const C_OscNodeDataPoolListId &rc_ListId =
                            orc_ListIds[u32_ItListId];
                        // If not same index but otherwise same ID abort
                        if ((u32_ItCurListId != u32_ItListId) &&
                            ((rc_ListId.u32_DataPoolIndex ==
                              rc_DataPoolListId.u32_DataPoolIndex) &&
                      ...
                      if (q_ListFound == false) {
                        // List
                        C_OscParamSetInterpretedDataPool
                            &rc_InterpretedDataPool =
                                c_InterpretedNode
                                    .c_DataPools[u32_ItCurrentDataPool];
                  ...
                  if (q_DataPoolFound == false) {
                    ...
                    rc_DataPool.CalcGeneratedDefinitionHash(
                        c_DataPoolInfo.u32_DataPoolCrc);
                    c_DataPoolInfo.c_Name = rc_DataPool.c_Name;
                    c_DataPoolInfo.u32_NvmSize = rc_DataPool.u32_NvmSize;
                    c_DataPoolInfo.u32_NvmStartAddress =
                        rc_DataPool.u32_NvmStartAddress;
                    ...
                    c_InterpretedNode.c_DataPools.push_back(
                        c_NewInterpretedDataPool);
                    // Update indices
                    c_AlreadyUsedDataPoolIndices.push_back(
                        rc_DataPoolListId.u32_DataPoolIndex);
                  }
                } else {
                  s32_Retval = C_RANGE;
            ...
            if (s32_Retval == C_NO_ERR) {
              if (this->mc_ImageFileHandler.AddRawDataForNode(c_RawNode) ==
                  C_NO_ERR) {
                if (this->mc_ImageFileHandler.AddInterpretedDataForNode(
                        c_InterpretedNode) == C_NO_ERR) {
                  // Finished
                  // Update state
                  this->me_CreateParameterSetWorkflowState =
                      C_OscDataDealerNvmSafe::eCPSFS_FILE_CREATED;
                } else {
                  s32_Retval = C_RANGE;
                }
              } else {
...
/*! \brief   Create clean file from internally stored content without adding a
   ...
   Note: this function handles a file step and there is only one parameter set
   file for each parametrization process, so this function needs to only be
   called once, so all participating data dealers can continue with the next
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvmSafe::NvmSafeCreateCleanFileWithoutCrc(
  ...
  if (!(QFileInfo(QString::fromStdString(orc_Path.toUtf8().constData())).exists() &&
    ...
    if (this->me_ParameterSetFileState ==
      ...
      s32_Retval = this->mc_ImageFileHandler.CreateCleanFileWithoutCrc(
          QString::fromStdString(orc_Path.toUtf8().constData()));
      if (s32_Retval == C_NO_ERR) {
        // Update internal variable
        this->mc_ParameterSetFilePath = orc_Path;
        // Update state
        this->me_ParameterSetFileState =
            C_OscDataDealerNvmSafe::ePSFS_FILE_CREATED;
      }
    } else {
      s32_Retval = C_OVERFLOW;
    }
...
/*! \brief   Read file and update internally stored content (cleared at start)
   ...
   XML file) C_CONFIG   file does not contain essential information
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvmSafe::NvmSafeReadFileWithoutCrc(const QString &orc_Path) {
  int32_t s32_Retval;

  if (this->me_ParameterSetFileState ==
      C_OscDataDealerNvmSafe::ePSFS_FILE_CREATED) {
    if (this->mc_ParameterSetFilePath == orc_Path) {
      s32_Retval = this->mc_ImageFileHandler.ReadFile(
          QString::fromStdString(orc_Path.toUtf8().constData()), true);
      if (s32_Retval == C_NO_ERR) {
        // Update state
        this->me_ParameterSetFileState =
            C_OscDataDealerNvmSafe::ePSFS_FILE_READ_WITHOUT_CRC;
      }
    } else {
      s32_Retval = C_RANGE;
    }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   The function reads the contents of a parameter set file without
   checking the file CRC

   \param[in]  orc_Path          File path
   \param[out] orc_DataPoolLists Loaded data pool lists (Always cleared at
   start)
   ...
   C_CONFIG   Mismatch of data with current node
               or no valid pointer to the original instance of "C_OscNode" is
   set in "C_OscDataDealer"
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvmSafe::NvmSafeCheckParameterFileContents(
  ...
  if (this->me_CreateParameterSetWorkflowState ==
      C_OscDataDealerNvmSafe::eCPSFS_FILE_CREATED) {
    if (C_OscDataDealerNvmSafe::mc_ParameterSetFilePath == orc_Path) {
      if (this->mpc_Node != NULL) {
        const C_OscParamSetRawNode *const pc_Node =
            this->mc_ImageFileHandler.GetRawDataForNode(
                this->mpc_Node->c_Properties.c_Name);
        if (pc_Node != NULL) {
          if (m_CheckParameterFileContent(*pc_Node) == C_NO_ERR) {
            // For each raw data pool
            // For each raw entry
            for (uint32_t u32_ItRawEntry = 0;
                 (u32_ItRawEntry < pc_Node->c_Entries.size()) &&
                 (s32_Retval == C_NO_ERR);
                 ++u32_ItRawEntry) {
              bool q_Found = false;
              const C_OscParamSetRawEntry &rc_CurRawEntry =
                  pc_Node->c_Entries[u32_ItRawEntry];
              // Find data pool list
              for (uint32_t u32_ItDataPool = 0;
                   (u32_ItDataPool < this->mpc_Node->c_DataPools.size()) &&
                   (s32_Retval == C_NO_ERR);
                   ++u32_ItDataPool) {
                const C_OscNodeDataPool &rc_DataPool =
                    this->mpc_Node->c_DataPools[u32_ItDataPool];
                ...
                if (((rc_DataPool.e_Type == C_OscNodeDataPool::eNVM) ||
                    ...
                    ((rc_DataPool.u32_NvmStartAddress +
                      rc_DataPool.u32_NvmSize) >
                     rc_CurRawEntry.u32_StartAddress)) {
                  for (uint32_t u32_ItList = 0;
                       (u32_ItList < rc_DataPool.c_Lists.size()) &&
                       (s32_Retval == C_NO_ERR);
                       ++u32_ItList) {
                    const C_OscNodeDataPoolList &rc_List =
                        rc_DataPool.c_Lists[u32_ItList];
                    // Check list address range
                    if ((rc_List.u32_NvmStartAddress <=
                         rc_CurRawEntry.u32_StartAddress) &&
                        ((rc_List.u32_NvmStartAddress + rc_List.u32_NvmSize) >
                         rc_CurRawEntry.u32_StartAddress)) {
                      if ((rc_CurRawEntry.c_Bytes.size() ==
                           rc_List.GetNumBytesUsed()) ||
                          (rc_CurRawEntry.c_Bytes.size() == 2)) {
                        // Data pool list found
                        const C_OscNodeDataPoolListId c_Value(
                            this->mu32_NodeIndex, u32_ItDataPool, u32_ItList);
                        q_Found = true;
            ...
            if (s32_Retval == C_NO_ERR) {
              ...
              for (uint32_t u32_ItCopy = 0; u32_ItCopy < c_Copy.size();
                ...
                for (uint32_t u32_ItExisting = 0;
                     u32_ItExisting < orc_DataPoolLists.size();
                     ++u32_ItExisting) {
                  const C_OscNodeDataPoolListId &rc_ExistingId =
                      orc_DataPoolLists[u32_ItExisting];
                  if (((rc_ExistingId.u32_NodeIndex ==
                        rc_CopyId.u32_NodeIndex) &&
                       (rc_ExistingId.u32_DataPoolIndex ==
                        rc_CopyId.u32_DataPoolIndex)) &&
                      (rc_ExistingId.u32_ListIndex ==
                       rc_CopyId.u32_ListIndex)) {
                    q_Found = true;
                  }
                }
                if (q_Found == false) {
              ...
              // As not every list might require a CRC the new list size cannot
              // be exactly matched
              if ((orc_DataPoolLists.size() >= (c_Copy.size() / 2)) &&
                  (orc_DataPoolLists.size() <= c_Copy.size())) {
                // Finished
                this->me_CreateParameterSetWorkflowState =
                    C_OscDataDealerNvmSafe::eCPSFS_FILE_CHECKED;
              } else {
                s32_Retval = C_CONFIG;
              }
            }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvmSafe::NvmSafeUpdateCrcForFile(const QString &orc_Path) {
  int32_t s32_Retval;

  if (this->me_ParameterSetFileState ==
      C_OscDataDealerNvmSafe::ePSFS_FILE_READ_WITHOUT_CRC) {
    if (this->mc_ParameterSetFilePath == orc_Path) {
      s32_Retval = C_OscParamSetHandler::h_UpdateCrcForFile(
          QString::fromStdString(orc_Path.toUtf8().constData()));
      if (s32_Retval == C_NO_ERR) {
        // Update state
...
/*! \brief   Read file and update internally stored content (cleared at start)
   ...
   After loading this functions checks whether the file contains data for
   exactly one node. Otherwise it will fail.
   ...
   C_RD_WR    specified file does not exist
              specified file is present but structure is invalid (e.g. invalid
   XML file) C_CONFIG   file does not contain data for exactly one node (zero or
   more than one) C_CHECKSUM specified file is present but checksum is invalid
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvmSafe::NvmSafeReadFileWithCrc(const QString &orc_Path) {
  int32_t s32_Retval = this->mc_ImageFileHandler.ReadFile(
      QString::fromStdString(orc_Path.toUtf8().constData()), false);
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (this->mc_ImageFileHandler.GetNumberOfNodes() != 1U) {
      ...
      // Update state
      this->me_ParameterSetFileState =
          C_OscDataDealerNvmSafe::ePSFS_FILE_READ_WITH_CRC;
      this->mc_ParameterSetFilePath = orc_Path;
    }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Read a parameter set file and writes the contained values and CRCs
   to the ECU's NVM
   ...
   * write to NVM
   ...
   C_CONFIG      1            No valid diagnostic protocol is set in
   "C_OscDataDealer" 2            No valid pointer to the original instance of
   "C_OscNode" is set in "C_OscDataDealer" C_NOACT       1 Communication
   protocol service could not be requested C_TIMEOUT     1 Communication
   protocol service has timed out C_WARN        1            Communication
   protocol service error response was received C_UNKNOWN_ERR <undefined>
   Communication protocol failed with non-specified error code
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvmSafe::NvmSafeWriteParameterSetFile(
    const QString &orc_Path, int32_t &ors32_ResultDetail) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (this->mpc_Node != NULL) {
    if (this->mpc_DiagProtocol != NULL) {
      if (C_OscDataDealerNvmSafe::me_ParameterSetFileState ==
          C_OscDataDealerNvmSafe::ePSFS_FILE_READ_WITH_CRC) {
        if (C_OscDataDealerNvmSafe::mc_ParameterSetFilePath == orc_Path) {
          // when we get here we can be sure there was data for exactly one node
          // contained in the file
          //  as this is checked by NvmSafeReadFileWithCRC
          const C_OscParamSetRawNode *const pc_Node =
              this->mc_ImageFileHandler.GetRawDataForNode(0U);
          Q_ASSERT(pc_Node != NULL);
          if (pc_Node != NULL) {
            // write the actual values:
            for (uint32_t u32_ItEntry = 0;
                 u32_ItEntry < pc_Node->c_Entries.size(); u32_ItEntry++) {
              const C_OscParamSetRawEntry &rc_Entry =
                  pc_Node->c_Entries[u32_ItEntry];
              s32_Retval = this->mpc_DiagProtocol->NvmWrite(
                  rc_Entry.u32_StartAddress, rc_Entry.c_Bytes, NULL);
              // Map error codes
              switch (s32_Retval) {
              case C_TIMEOUT:
                ors32_ResultDetail = 1;
              ...
              default:
                ...
                osc_write_log_info("Parametrization",
                                   "Not documented error code " +
                                       QString::number(s32_Retval) +
                                       " was returned by NvmWrite");
                break;
              }
              if (s32_Retval != C_NO_ERR) {
                break;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDataDealerNvmSafe::mh_CreateInterpretedList(
  ...
  for (uint32_t u32_ItElement = 0; u32_ItElement < orc_List.c_Elements.size();
       ++u32_ItElement) {
    C_OscParamSetInterpretedElement c_NewElement;
    const C_OscNodeDataPoolListElement &rc_Element =
        orc_List.c_Elements[u32_ItElement];
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvmSafe::m_CheckParameterFileContent(
    const C_OscParamSetRawNode &orc_Node) const {
  int32_t s32_Retval = C_NO_ERR;

  if (this->mpc_Node != NULL) {
    if ((this->mpc_Node->c_Properties.c_Name == orc_Node.c_Name) &&
        (orc_Node.c_DataPools.size() > 0)) {
      // Check all data pool info segments
      for (uint32_t u32_ItReadDataPool = 0U;
           (u32_ItReadDataPool < orc_Node.c_DataPools.size()) &&
           (s32_Retval == C_NO_ERR);
           ++u32_ItReadDataPool) {
        const C_OscParamSetDataPoolInfo &rc_DataPoolInfo =
            orc_Node.c_DataPools[u32_ItReadDataPool];
        bool q_Found = false;
        // Find matching data pool
        for (uint32_t u32_ItNodeDataPool = 0U;
             (u32_ItNodeDataPool < this->mpc_Node->c_DataPools.size()) &&
             (s32_Retval == C_NO_ERR);
             ++u32_ItNodeDataPool) {
          const C_OscNodeDataPool &rc_NodeDataPool =
              this->mpc_Node->c_DataPools[u32_ItNodeDataPool];
          if (rc_NodeDataPool.c_Name == rc_DataPoolInfo.c_Name) {
            ...
            } else if ((((rc_DataPoolInfo.au8_Version[0] !=
                          rc_NodeDataPool.au8_Version[0])) ||
                        (rc_DataPoolInfo.au8_Version[1] !=
                         rc_NodeDataPool.au8_Version[1])) ||
                       (rc_DataPoolInfo.au8_Version[2] !=
                        rc_NodeDataPool.au8_Version[2])) {
              s32_Retval = C_RD_WR;
            } else {
              // Match
            }
...
/*! \brief   Create raw entry for list and write bytes to interpreted data
   ...
   C_RANGE    List has no elements. Nothing to read.
              Datapool element size configuration does not match with count of
   read bytes C_TIMEOUT  Expected response not received within timeout C_NOACT
   Could not send request (e.g. Tx buffer full) C_WARN     Error response or
   malformed protocol response C_CONFIG   Pre-requisites not correct; e.g.
   driver not initialized or parameter out of range (checked by client side)
   C_RD_WR    Datapool element size configuration does not match with count of
   read bytes C_CHECKSUM Checksum of read datapool list is invalid C_COM
   expected server response not received because of communication error
*/
//----------------------------------------------------------------------------------------------------------------------
