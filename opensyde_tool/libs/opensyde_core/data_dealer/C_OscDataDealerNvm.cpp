...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscDataDealerNvm::C_OscDataDealerNvm(void) : C_OscDataDealer() {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscDataDealerNvm::C_OscDataDealerNvm(
...
//----------------------------------------------------------------------------------------------------------------------
uint16_t
C_OscDataDealerNvm::NvmCalcCrc(const C_OscNodeDataPoolList &orc_List) const {
  ...
  if (this->mpc_DiagProtocol != NULL) {
    ...
    for (u32_Counter = 0U; u32_Counter < orc_List.c_Elements.size();
         ++u32_Counter) {
      const C_OscNodeDataPoolListElement *const pc_Element =
          &orc_List.c_Elements[u32_Counter];
      QByteArray c_Data;
      ...
      stw::scl::C_SclChecksums::CalcCRC16(
          ...
          static_cast<uint32_t>(c_Data.size()), u16_Crc);
    }
  }
...
/*! \brief   Reads a specific NVM list of ECU
   ...
   protocol driver properly initialized ?) C_RANGE     Input parameter invalid
   C_OVERFLOW  List has no elements. Nothing to read
   C_RD_WR     Datapool element size configuration does not match with count of
   read bytes C_CHECKSUM  Checksum of read datapool list is invalid C_TIMEOUT
   Expected response not received within timeout C_NOACT     Could not send
   request (e.g. Tx buffer full) C_WARN      Error response or malformed
   protocol response C_COM       Pre-requisites not correct; e.g. driver not
   initialized or parameter out of range (checked by client side)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvm::NvmReadList(const uint32_t ou32_DataPoolIndex,
  ...
  if ((mpc_Node == NULL) || (mpc_DiagProtocol == NULL)) {
    s32_Return = C_CONFIG;
             ...
             (this->mpc_Node->c_DataPools[ou32_DataPoolIndex].c_Lists.size() >
              ou32_ListIndex)) {
    QByteArray c_Values;
    C_OscNodeDataPoolList &rc_List =
        this->mpc_Node->c_DataPools[ou32_DataPoolIndex].c_Lists[ou32_ListIndex];
...
/*! \brief   Notify server application about NVM data changes
   ...
   C_CONFIG   no node or diagnostic protocol are known (was this class properly
   Initialize()d ?) protocol driver reported configuration error (was the
   protocol driver properly initialized ?) C_TIMEOUT  Expected server response
   not received within timeout C_NOACT    Could not send request (e.g. Tx buffer
   full) C_WARN     Server sent error response C_RD_WR    unexpected content in
   server response (here: wrong data pool index) C_COM      communication driver
   reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvm::NvmNotifyOfChanges(const uint8_t ou8_DataPoolIndex,
  ...
  if ((mpc_Node == NULL) || (mpc_DiagProtocol == NULL)) {
    ...
    s32_Return = this->mpc_DiagProtocol->NvmNotifyOfChanges(
        ou8_DataPoolIndex, ou8_ListIndex, orq_ApplicationAcknowledge,
        opu8_NrCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvm::m_NvmReadListRaw(const C_OscNodeDataPoolList &orc_List,
                                     QByteArray &orc_Values,
                                     uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  if ((orc_List.u32_NvmSize > 2U) ||
    ...
    // TODO: KEFEX Protocol dependent count of calls necessary and problems with
    // fragmented lists im NVM memory
    // TODO: KEFEX Protocol dependent position of CRC!
    s32_Return = this->mpc_DiagProtocol->NvmRead(orc_List.u32_NvmStartAddress,
                                                 orc_Values, opu8_NrCode);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Interprets the value and if necessary the CRC of the container
   orc_Values into the datapool list
   ...
   C_RD_WR     Datapool element size configuration does not match with count of
   read bytes C_CHECKSUM  Checksum of read datapool list is invalid
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDataDealerNvm::m_SaveDumpToList(const QByteArray &orc_Values,
  ...
  if ((orc_List.q_NvmCrcActive == true) && (s32_Return == C_NO_ERR)) {
    if (orc_Values.size() >= 2) {
      ...
      (void)std::memcpy(reinterpret_cast<uint8_t *>(c_CrcData.data()),
                        orc_Values.data(), 2);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Interprets the value of the container orc_Values into the NVM
   datapool list
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvm::m_SaveDumpValuesToListValues(
    const QByteArray &orc_Values, C_OscNodeDataPoolList &orc_List) const {
  ...
  for (u32_Counter = 0U; u32_Counter < orc_List.c_Elements.size();
    ...
    const uint32_t u32_Index =
        pc_Element->u32_NvmStartAddress - orc_List.u32_NvmStartAddress;

    if (orc_Values.size() >=
        (static_cast<size_t>(u32_Index) + u32_SizeElement)) {
      QByteArray c_ElementData;

      // Get the relevant data for this element
      c_ElementData.resize(u32_SizeElement);
      (void)std::memcpy(reinterpret_cast<uint8_t *>(c_ElementData.data()),
                        orc_Values.data() + u32_Index, u32_SizeElement);
      ...
      if (this->mpc_DiagProtocol->GetEndianness() ==
        ...
        (void)pc_Element->c_NvmValue.SetValueFromLittleEndianBlob(
            c_ElementData);
      }
...
/*! \brief   Adapts the return value of a diag protocol base function
   ...
   \param[in]     os32_ProtReturnValue   Return value of diag protocol base
   function
   ...
   C_COM      Expected server response not received because of communication
   error C_CONFIG   Pre-requisites not correct; e.g. driver not initialized or
              parameter out of range (checked by client side)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDataDealerNvm::mh_AdaptProtocolReturnValue(
    const int32_t os32_ProtReturnValue) {
  int32_t s32_Return;
