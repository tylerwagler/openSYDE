...
// using namespace stw::scl;  // REMOVED
 ...
 */
const uint32_t
    C_OscHalcDefStructFiler::hu32_MAX_ALLOWED_COMBINED_VARIABLE_LENGTH = 31UL;
const QString C_OscHalcDefStructFiler::mhc_FALSE = "FALSE";
const QString C_OscHalcDefStructFiler::mhc_TRUE = "TRUE";
...
/*! \brief  Load IO parameters
   ...
   \param[in]      orc_UseCases           All available channel use cases for
   this domain \param[in]      orc_CurrentNodeName    Current XML node name
   \param[in]      orc_SectionNodeName    XML node name of section to parse
   \param[in]      orc_GroupNodeName      XML node name of section elements to
   parse \param[in]      orc_SingleNodeName     XML node name of group elements
   to parse \param[in]      oq_RequireId           Flag if ID is required in
   this section \param[in]      oq_RequireSection      Flag if this entire
   section is required \param[in]      ou32_DomainNameLength  Domain name length
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_LoadStructs(
  ...
  orc_Structs.clear();
  if (orc_XmlParser.SelectNodeChild(orc_SectionNodeName) ==
      orc_SectionNodeName) {
    QString c_NodeChannel = orc_XmlParser.SelectNodeChild(orc_GroupNodeName);
    if (c_NodeChannel == orc_GroupNodeName) {
      do {
        C_OscHalcDefStruct c_Struct;
        s32_Retval = mh_LoadStruct(c_Struct, orc_XmlParser, orc_UseCases,
                                   orc_GroupNodeName, orc_SingleNodeName,
      ...
      } while ((c_NodeChannel == orc_GroupNodeName) &&
               (s32_Retval == C_NO_ERR));
      if (s32_Retval == C_NO_ERR) {
        // Return
...
/*! \brief  Save generic struct data
   ...
   \param[in]      orc_UseCases           All available channel use cases for
   this domain \param[in,out]  orc_XmlParser          XML parser \param[in]
   orc_CurrentNodeName    Current node name \param[in]      orc_SectionNodeName
   Section node name \param[in]      orc_GroupNodeName      Group node name
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_SaveStructs(
  ...
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(orc_SectionNodeName) ==
           orc_SectionNodeName);
  for (uint32_t u32_ItStruct = 0UL;
       (u32_ItStruct < orc_Structs.size()) && (s32_Retval == C_NO_ERR);
       ++u32_ItStruct) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(orc_GroupNodeName) ==
             orc_GroupNodeName);
    s32_Retval = C_OscHalcDefStructFiler::mh_SaveStruct(
        orc_Structs[u32_ItStruct], orc_UseCases, orc_XmlParser,
        orc_GroupNodeName, orc_SingleNodeName);
    if (s32_Retval == C_NO_ERR) {
      // Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == orc_SectionNodeName);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_ParseSimplestTypeValue(
    const QString &orc_TypeStr, C_OscNodeDataPoolContent &orc_Content,
  ...
  if (s32_Retval == C_NO_ERR) {
    if (c_ItemStr == "") {
      orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
          orc_AttributeName, "is empty");
      s32_Retval = C_CONFIG;
    } else {
      switch (orc_Content.GetType()) {
      case C_OscNodeDataPoolContent::eUINT8:
        if (orc_TypeStr == "bool") {
          if (c_ItemStr.toLower() == mhc_FALSE.toLower()) {
            orc_Content.SetValueU8(static_cast<uint8_t>(false));
          } else if (c_ItemStr.toLower() == mhc_TRUE.toLower()) {
            orc_Content.SetValueU8(static_cast<uint8_t>(true));
          } else {
            if (mh_CheckValidUint(c_ItemStr) == C_NO_ERR) {
              orc_Content.SetValueU8(static_cast<uint8_t>(
                  orc_XmlParser.GetAttributeUint32(orc_AttributeName)));
            } else {
              s32_Retval = C_CONFIG;
            }
          }
      ...
      case C_OscNodeDataPoolContent::eUINT16:
        if (mh_CheckValidUint(c_ItemStr) == C_NO_ERR) {
          orc_Content.SetValueU16(static_cast<uint16_t>(
              orc_XmlParser.GetAttributeUint32(orc_AttributeName)));
        } else {
          s32_Retval = C_CONFIG;
        }
      ...
      case C_OscNodeDataPoolContent::eUINT64:
        if (mh_CheckValidUint(c_ItemStr) == C_NO_ERR) {
          orc_Content.SetValueU64(
              orc_XmlParser.GetAttributeSint64(orc_AttributeName));
        } else {
          s32_Retval = C_CONFIG;
        }
        break;
      case C_OscNodeDataPoolContent::eSINT8:
        if (mh_CheckValidSint(c_ItemStr) == C_NO_ERR) {
          orc_Content.SetValueS8(static_cast<int8_t>(
              orc_XmlParser.GetAttributeSint32(orc_AttributeName)));
        } else {
          s32_Retval = C_CONFIG;
        }
        break;
      case C_OscNodeDataPoolContent::eSINT16:
        if (mh_CheckValidSint(c_ItemStr) == C_NO_ERR) {
          orc_Content.SetValueS16(static_cast<int16_t>(
              orc_XmlParser.GetAttributeSint32(orc_AttributeName)));
        } else {
          s32_Retval = C_CONFIG;
        }
      ...
      case C_OscNodeDataPoolContent::eFLOAT32:
        if (mh_CheckValidDouble(c_ItemStr) == C_NO_ERR) {
          orc_Content.SetValueF32(
              orc_XmlParser.GetAttributeFloat32(orc_AttributeName));
        } else {
          s32_Retval = C_CONFIG;
        }
        break;
      case C_OscNodeDataPoolContent::eFLOAT64:
        if (mh_CheckValidDouble(c_ItemStr) == C_NO_ERR) {
          orc_Content.SetValueF64(
              orc_XmlParser.GetAttributeFloat64(orc_AttributeName));
        } else {
          s32_Retval = C_CONFIG;
        }
        break;
      ...
      if (s32_Retval == C_CONFIG) {
        orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
            orc_AttributeName,
            "Could not parse attribute content into type \"" + orc_TypeStr +
                "\"");
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_GetTypeForSimplestTypeString(
    const QString &orc_TypeStr, C_OscNodeDataPoolContent::E_Type &ore_Type,
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscHalcDefStructFiler::h_GetTypeString(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_SaveSimpleValueAsAttribute(
    const QString &orc_Node, C_OscXmlParserBase &orc_XmlParser,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_SetType(C_OscXmlParserBase &orc_XmlParser,
                                           C_OscHalcDefContent &orc_Content,
                                           QString &orc_TypeStr,
                                           QString &orc_BaseTypeStr,
                                           const QString &orc_CurrentNodeName) {
  int32_t s32_Retval =
      orc_XmlParser.GetAttributeStringError("type", orc_TypeStr);
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (orc_TypeStr == "float64") {
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          if ((e_Type == C_OscNodeDataPoolContent::eFLOAT32) ||
            ...
            orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
                "base-type", "Float type not supported");
            s32_Retval = C_CONFIG;
          }
        }
      }
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          if ((e_Type == C_OscNodeDataPoolContent::eFLOAT32) ||
            ...
            orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
                "base-type", "Float type and signed type not supported");
            s32_Retval = C_CONFIG;
          }
        }
      }
      ...
      s32_Retval =
          orc_XmlParser.GetAttributeUint32Error("strlen", u32_StringLength);
      if (s32_Retval == C_NO_ERR) {
        orc_Content.SetType(C_OscNodeDataPoolContent::eSINT8);
  ...
  // Parse enum
  if (s32_Retval == C_NO_ERR) {
    QString c_EnumItemNode = orc_XmlParser.SelectNodeChild("enum-item");
    if (c_EnumItemNode == "enum-item") {
      if (orc_TypeStr != "enum") {
        ...
        do {
          ...
          QString c_Display;
          s32_Retval =
              orc_XmlParser.GetAttributeStringError("display", c_Display);
          if (s32_Retval == C_NO_ERR) {
            s32_Retval = h_ParseSimplestTypeValue(orc_BaseTypeStr, c_Content,
                                                  orc_XmlParser, "value");
          }
          if (s32_Retval == C_NO_ERR) {
            if (orc_Content.AddEnumItem(c_Display, c_Content) != C_NO_ERR) {
              orc_XmlParser.ReportErrorForNodeContentAppendXmlContext(
                  "\"enum-item\" display name \"" + c_Display +
                  "\" is duplicate");
              s32_Retval = C_CONFIG;
            }
          }
  ...
  // Parse bitmask
  if (s32_Retval == C_NO_ERR) {
    QString c_BitmaskItemNode =
        orc_XmlParser.SelectNodeChild("bitmask-selection");
      ...
      if (orc_TypeStr != "bitmask") {
        ...
        do {
          ...
          s32_Retval = orc_XmlParser.GetAttributeStringError(
              "display", c_BitmaskItem.c_Display);
          if (s32_Retval == C_NO_ERR) {
            QString c_Content;
            s32_Retval = orc_XmlParser.GetAttributeStringError(
                "initial-apply-value-setting", c_Content);
            if (s32_Retval == C_NO_ERR) {
              if (c_Content.toLower() == mhc_FALSE.toLower()) {
                c_BitmaskItem.q_ApplyValueSetting = false;
              } else if (c_Content.toLower() == mhc_TRUE.toLower()) {
                c_BitmaskItem.q_ApplyValueSetting = true;
              } else {
                c_BitmaskItem.q_ApplyValueSetting =
                    orc_XmlParser.GetAttributeBool(
          ...
          if (s32_Retval == C_NO_ERR) {
            QString c_Content;
            s32_Retval =
                orc_XmlParser.GetAttributeStringError("value", c_Content);
            if (s32_Retval == C_NO_ERR) {
              if (c_BitmaskItem.SetValueByString(c_Content) != C_NO_ERR) {
                orc_XmlParser
                    .ReportErrorForAttributeContentStartingWithXmlContext(
                        "value",
                        "content \"" + c_Content + "\" cannot be parsed");
                s32_Retval = C_CONFIG;
              }
            }
          }
          if (s32_Retval == C_NO_ERR) {
            ...
            c_BitmaskItemNode =
                orc_XmlParser.SelectNodeNext("bitmask-selection");
          }
        } while ((c_BitmaskItemNode == "bitmask-selection") &&
                 (s32_Retval == C_NO_ERR));
      }
      if (s32_Retval == C_NO_ERR) {
        // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::h_ParseAttributeIntoContent(
  ...
  if (orc_XmlParser.AttributeExists(orc_AttributeName)) {
    if (orc_Type == "float64") {
      ...
      const QString c_ItemStr =
          orc_XmlParser.GetAttributeString(orc_AttributeName);
      if (c_ItemStr.toLower() == mhc_FALSE.toLower()) {
        orc_Content.SetValueU8(static_cast<uint8_t>(false));
      } else if (c_ItemStr.toLower() == mhc_TRUE.toLower()) {
        orc_Content.SetValueU8(static_cast<uint8_t>(true));
      ...
      if (orc_Content.SetEnumValue(c_ItemStr) != C_NO_ERR) {
        // Try simple value
        s32_Retval = h_ParseSimplestTypeValue(orc_BaseType, orc_Content,
      ...
      const QString c_ItemStr =
          orc_XmlParser.GetAttributeString(orc_AttributeName);
      if (orc_Content.SetStringValue(c_ItemStr) != C_NO_ERR) {
        orc_XmlParser.ReportErrorForNodeContentAppendXmlContext(
            "Unexpected value \"" + orc_Type + "\" for string");
        s32_Retval = C_CONFIG;
      }
    } else {
      orc_XmlParser.ReportErrorForNodeContentAppendXmlContext(
          "Unexpected type \"" + orc_Type + "\"");
      s32_Retval = C_CONFIG;
    }
  } else {
    if (oq_RequireAttribute) {
      orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
          orc_AttributeName, "is missing");
      s32_Retval = C_CONFIG;
    }
  }
  return s32_Retval;
...
/*! \brief  Load IO parameter struct
   ...
   \param[in,out]  orc_XmlParser          XML with default state
   \param[in]      orc_UseCases           All available channel use cases for
   this domain \param[in]      orc_GroupNodeName      XML node name of section
   elements to parse \param[in]      orc_SingleNodeName     XML node name of
   group elements to parse \param[in]      oq_RequireId           Flag if ID is
   required in this section \param[in]      ou32_DomainNameLength  Domain name
   length
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_LoadStruct(
  ...
  if (s32_Retval == C_NO_ERR) {
    if (c_Type == "struct") {
      ...
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = orc_XmlParser.GetAttributeStringError(
            "display", orc_Struct.c_Display);
        if (s32_Retval == C_NO_ERR) {
          if (static_cast<uint32_t>(orc_Struct.c_Display.length() +
            ...
            orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
                ...
                "content \"" + orc_Struct.c_Display +
                    "\" is too long to be combined with domain "
                    "\"singular-name\" (or \"name\" if not existing) node, "
                    "maximum allowed characters of combined value: " +
                    QString::number(
                        C_OscHalcDefStructFiler::
                            hu32_MAX_ALLOWED_COMBINED_VARIABLE_LENGTH) +
                    " (Current: " +
                    QString::number(orc_Struct.c_Display.length() +
                                    ou32_DomainNameLength) +
                    ").");
            s32_Retval = C_CONFIG;
          }
      ...
      if (s32_Retval == C_NO_ERR) {
        QString c_Availability;
        s32_Retval = orc_XmlParser.GetAttributeStringError("availability",
                                                           c_Availability);
        if (s32_Retval == C_NO_ERR) {
          s32_Retval = C_OscHalcDefStructFiler::mh_ParseAttributeAvailability(
              orc_Struct.c_UseCaseAvailabilities, c_Availability, orc_UseCases,
              orc_XmlParser);
        }
      }
      if (s32_Retval == C_NO_ERR) {
        QString c_NodeParameter;
        if (orc_XmlParser.SelectNodeChild("comment") == "comment") {
        ...
        if (c_NodeParameter == orc_SingleNodeName) {
          do {
            ...
            s32_Retval = mh_LoadDataElement(
                c_Element, orc_XmlParser, orc_UseCases,
                orc_Struct.c_UseCaseAvailabilities, orc_SingleNodeName,
                oq_RequireId, ou32_DomainNameLength);
            if (s32_Retval == C_NO_ERR) {
              orc_Struct.c_StructElements.push_back(c_Element);
              c_NodeParameter =
                  orc_XmlParser.SelectNodeNext(orc_SingleNodeName);
            }
          } while ((c_NodeParameter == orc_SingleNodeName) &&
                   (s32_Retval == C_NO_ERR));
          if (s32_Retval == C_NO_ERR) {
            // Return
            Q_ASSERT(orc_XmlParser.SelectNodeParent() == orc_GroupNodeName);
      ...
      s32_Retval = mh_LoadDataElement(orc_Struct, orc_XmlParser, orc_UseCases,
...
/*! \brief  Save generic struct data
   ...
   \param[in]      orc_UseCases        All available channel use cases for this
   domain \param[in,out]  orc_XmlParser       XML parser \param[in]
   orc_GroupNodeName   Group node name \param[in]      orc_SingleNodeName Single
   node name
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_SaveStruct(
    const C_OscHalcDefStruct &orc_Struct,
    const QList<C_OscHalcDefChannelUseCase> &orc_UseCases,
  ...
  if (orc_Struct.c_StructElements.size() > 0UL) {
    ...
    s32_Retval = mh_GetAvailabilityString(orc_Struct.c_UseCaseAvailabilities,
                                          orc_UseCases, c_Availability);
    if (s32_Retval == C_NO_ERR) {
      ...
      for (uint32_t u32_ItElem = 0UL;
           (u32_ItElem < orc_Struct.c_StructElements.size()) &&
           (s32_Retval == C_NO_ERR);
           ++u32_ItElem) {
        Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(orc_SingleNodeName) ==
                 orc_SingleNodeName);
        s32_Retval =
            mh_SaveDataElement(orc_Struct.c_StructElements[u32_ItElem],
                               orc_UseCases, orc_XmlParser, orc_SingleNodeName);
        Q_ASSERT(orc_XmlParser.SelectNodeParent() == orc_GroupNodeName);
      }
    }
  } else {
    s32_Retval = mh_SaveDataElement(orc_Struct, orc_UseCases, orc_XmlParser,
                                    orc_GroupNodeName);
  }
  return s32_Retval;
}
...
/*! \brief  Load IO parameter
   ...
   \param[in,out]  orc_XmlParser          XML with default state
   \param[in]      orc_UseCases           All available channel use cases for
   this domain \param[in]      orc_BaseAvailability   Availability of base node
   \param[in]      orc_SingleNodeName     XML node name of group elements to
   parse \param[in]      oq_RequireId           Flag if ID is required in this
   section \param[in]      ou32_DomainNameLength  Domain name length
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_LoadDataElement(
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval =
        orc_XmlParser.GetAttributeStringError("display", orc_Element.c_Display);
    if (s32_Retval == C_NO_ERR) {
      if (static_cast<uint32_t>(orc_Element.c_Display.length() +
                                ou32_DomainNameLength) >
          C_OscHalcDefStructFiler::hu32_MAX_ALLOWED_COMBINED_VARIABLE_LENGTH) {
        orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
            "display",
            "content \"" + orc_Element.c_Display +
                "\" is too long to be combined with domain \"singular-name\" "
                "(or \"name\" if not existing) node, maximum allowed "
                "characters of combined value: " +
                QString::number(C_OscHalcDefStructFiler::
                                    hu32_MAX_ALLOWED_COMBINED_VARIABLE_LENGTH) +
                " (Current: " +
                QString::number(orc_Element.c_Display.length() +
                                ou32_DomainNameLength) +
                ").");
        s32_Retval = C_CONFIG;
      }
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.AttributeExists("availability")) {
      const QString c_Availability =
          orc_XmlParser.GetAttributeString("availability");
      s32_Retval = C_OscHalcDefStructFiler::mh_ParseAttributeAvailability(
          orc_Element.c_UseCaseAvailabilities, c_Availability, orc_UseCases,
          orc_XmlParser);
    } else {
      if (orc_BaseAvailability.size() > 0UL) {
        ...
        // Assuming all
        s32_Retval = C_OscHalcDefStructFiler::mh_ParseAttributeAvailability(
            orc_Element.c_UseCaseAvailabilities, "all", orc_UseCases,
            orc_XmlParser);
      }
    }
  }
  // Parse content first
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = h_SetType(orc_XmlParser, orc_Element.c_InitialValue, c_TypeStr,
                           c_BaseTypeStr, orc_SingleNodeName);
    if (s32_Retval == C_NO_ERR) {
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.AttributeExists("min-value")) {
      s32_Retval = h_ParseAttributeIntoContent(orc_Element.c_MinValue,
                                               orc_XmlParser, "min-value",
                                               c_TypeStr, c_BaseTypeStr, true);
    } else {
      // Default
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.AttributeExists("max-value")) {
      s32_Retval = h_ParseAttributeIntoContent(orc_Element.c_MaxValue,
                                               orc_XmlParser, "max-value",
                                               c_TypeStr, c_BaseTypeStr, true);
    } else {
      // Default
      if ((c_TypeStr == "enum") || (c_TypeStr == "bitmask")) {
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    s32_Retval = h_ParseAttributeIntoContent(
        orc_Element.c_InitialValue, orc_XmlParser, "initial-value", c_TypeStr,
        c_BaseTypeStr,
        orc_Element.GetComplexType() != C_OscHalcDefContent::eCT_BIT_MASK);

    // Check bitmask content
    if (C_OscHalcDefStructFiler::mh_CheckInitialBitmaskContentValid(
            orc_Element.c_InitialValue, orc_XmlParser) == false) {
      s32_Retval = C_CONFIG;
    }
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      s32_Retval = C_OscNodeDataPoolContentUtil::h_SetValueInMinMaxRange(
          orc_Element.c_MinValue, orc_Element.c_MaxValue,
          orc_Element.c_InitialValue, e_Tmp,
          C_OscNodeDataPoolContentUtil::eLEAVE_VALUE);
      if (s32_Retval != C_NO_ERR) {
        orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
            "initial-value", "value could not be set in min max range");
        s32_Retval = C_CONFIG;
      }
    }
  }
...
/*! \brief  Save data element
   ...
   \param[in]      orc_UseCases        All available channel use cases for this
   domain \param[in,out]  orc_XmlParser       XML parser \param[in]
   orc_SingleNodeName  Single node name
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_SaveDataElement(
    const C_OscHalcDefElement &orc_Element,
    const QList<C_OscHalcDefChannelUseCase> &orc_UseCases,
    C_OscXmlParserBase &orc_XmlParser, const QString &orc_SingleNodeName) {
  int32_t s32_Retval = C_NO_ERR;
  bool q_SpecialValueHandling = false;
  const QString c_BaseType = h_GetTypeString(orc_Element.GetType());
  ...
  case C_OscHalcDefContent::eCT_BIT_MASK: {
    const QList<C_OscHalcDefContentBitmaskItem> &rc_BitmaskItems =
        orc_Element.GetBitmaskItems();
    ...
    for (uint32_t u32_It = 0UL;
         (u32_It < rc_BitmaskItems.size()) && (s32_Retval == C_NO_ERR);
         ++u32_It) {
      const C_OscHalcDefContentBitmaskItem &rc_Bitmask =
          rc_BitmaskItems[u32_It];
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("bitmask-selection") ==
               "bitmask-selection");
      orc_XmlParser.SetAttributeString("display", rc_Bitmask.c_Display);
      orc_XmlParser.SetAttributeBool("initial-apply-value-setting",
                                     rc_Bitmask.q_ApplyValueSetting);
      orc_XmlParser.SetAttributeString(
          "value",
          C_OscHalcDefStructFiler::mh_ConvertToHex(rc_Bitmask.u64_Value));
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("comment") == "comment");
  ...
  case C_OscHalcDefContent::eCT_ENUM: {
    ...
    for (QList<std::pair<QString, C_OscNodeDataPoolContent>>::const_iterator
      ...
      s32_Retval =
          h_SaveSimpleValueAsAttribute("value", orc_XmlParser, c_It->second);
      if (s32_Retval == C_NO_ERR) {
        Q_ASSERT(orc_XmlParser.SelectNodeParent() == orc_SingleNodeName);
      }
    }
  ...
  case C_OscHalcDefContent::eCT_STRING: {
    ...
    orc_XmlParser.SetAttributeUint32(
        "strlen", orc_Element.c_InitialValue.GetArraySize() - 1UL);
  ...
  if (q_SpecialValueHandling == false) {
    // Handle values
    s32_Retval = h_SaveSimpleValueAsAttribute("initial-value", orc_XmlParser,
                                              orc_Element.c_InitialValue);
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = h_SaveSimpleValueAsAttribute("max-value", orc_XmlParser,
                                                orc_Element.c_MaxValue);
    }
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = h_SaveSimpleValueAsAttribute("min-value", orc_XmlParser,
                                                orc_Element.c_MinValue);
    }
  }
  if (s32_Retval == C_NO_ERR) {
    QString c_Availability;
    s32_Retval = mh_GetAvailabilityString(orc_Element.c_UseCaseAvailabilities,
                                          orc_UseCases, c_Availability);
    if (s32_Retval == C_NO_ERR) {
      orc_XmlParser.SetAttributeString("availability", c_Availability);
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_ParseAttributeAvailability(
    QList<uint32_t> &orc_Availability, const QString &orc_AttributeContent,
    const QList<C_OscHalcDefChannelUseCase> &orc_UseCases,
    const C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  if (orc_AttributeContent.toLower() == "all") {
    for (uint32_t u32_ItUseCase = 0UL; u32_ItUseCase < orc_UseCases.size();
         ++u32_ItUseCase) {
      orc_Availability.push_back(u32_ItUseCase);
    }
  } else {
    QStringList c_SplittedString = orc_AttributeContent.split(",");
    for (int32_t s32_ItSplit = 0L; s32_ItSplit < c_SplittedString.size();
         ++s32_ItSplit) {
      const QString c_CurSplit = c_SplittedString[s32_ItSplit].trimmed();
      bool q_Found = false;
      for (uint32_t u32_ItUseCase = 0UL; u32_ItUseCase < orc_UseCases.size();
           ++u32_ItUseCase) {
        const C_OscHalcDefChannelUseCase &rc_CurUseCase =
            orc_UseCases[u32_ItUseCase];
      ...
      if (q_Found == false) {
        orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
            "availability", "Attribute content \"" + c_CurSplit +
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_GetAvailabilityString(
  ...
  for (uint32_t u32_It = 0UL; u32_It < orc_Availability.size(); ++u32_It) {
    if (orc_Availability[u32_It] < orc_UseCases.size()) {
      const C_OscHalcDefChannelUseCase &rc_Availability =
          orc_UseCases[orc_Availability[u32_It]];
      ...
      osc_write_log_error("Saving HALC definition",
                          "Availability index " +
                              QString::number(orc_Availability[u32_It]) +
                              " no longer in range of use-case count " +
                              QString::number(orc_UseCases.size()));
      s32_Retval = C_CONFIG;
    }
  }
  return s32_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcDefStructFiler::mh_SetMaxValForType(
    const QString &orc_TypeStr, C_OscNodeDataPoolContent &orc_Content) {
  if (orc_TypeStr == "float64") {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcDefStructFiler::mh_SetMinValForType(
    const QString &orc_TypeStr, C_OscNodeDataPoolContent &orc_Content) {
  if (orc_TypeStr == "float64") {
    orc_Content.SetValueF64(-std::numeric_limits<float64_t>::max());
  } else if (orc_TypeStr == "float32") {
    ...
    for (uint32_t u32_It = 0UL; u32_It < orc_Content.GetArraySize(); ++u32_It) {
      orc_Content.SetValueArrS8Element(std::numeric_limits<int8_t>::min(),
                                       u32_It);
    }
  } else {
    // Unexpected
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscHalcDefStructFiler::mh_CheckInitialBitmaskContentValid(
    const C_OscHalcDefContent &orc_Content,
    const C_OscXmlParserBase &orc_XmlParser) {
  bool q_Retval = true;
  ...
  if (orc_Content.GetComplexType() == C_OscHalcDefContent::eCT_BIT_MASK) {
    ...
    const QList<C_OscHalcDefContentBitmaskItem> &rc_Bitmasks =
        c_Copy.GetBitmaskItems();
    for (QList<C_OscHalcDefContentBitmaskItem>::const_iterator c_ItBitmask =
             rc_Bitmasks.begin();
         c_ItBitmask != rc_Bitmasks.end(); ++c_ItBitmask) {
      c_Copy.SetBitmask(c_ItBitmask->c_Display,
                        c_ItBitmask->q_ApplyValueSetting);
    }
    switch (orc_Content.GetType()) // lint !e788 not all enum constants used;
                                   // this is unsigned only
    {
    case C_OscNodeDataPoolContent::eUINT8:
      if (orc_Content.GetValueU8() != c_Copy.GetValueU8()) {
        q_Retval = false;
        c_Is = C_OscHalcDefStructFiler::mh_ConvertToHex(
            static_cast<uint64_t>(orc_Content.GetValueU8()));
        c_Should = C_OscHalcDefStructFiler::mh_ConvertToHex(
            static_cast<uint64_t>(c_Copy.GetValueU8()));
      }
      break;
    case C_OscNodeDataPoolContent::eUINT16:
      if (orc_Content.GetValueU16() != c_Copy.GetValueU16()) {
        q_Retval = false;
        c_Is = C_OscHalcDefStructFiler::mh_ConvertToHex(
            static_cast<uint64_t>(orc_Content.GetValueU16()));
        c_Should = C_OscHalcDefStructFiler::mh_ConvertToHex(
            static_cast<uint64_t>(c_Copy.GetValueU16()));
      }
      break;
    case C_OscNodeDataPoolContent::eUINT32:
      if (orc_Content.GetValueU32() != c_Copy.GetValueU32()) {
        q_Retval = false;
        c_Is = C_OscHalcDefStructFiler::mh_ConvertToHex(
            static_cast<uint64_t>(orc_Content.GetValueU32()));
        c_Should = C_OscHalcDefStructFiler::mh_ConvertToHex(
            static_cast<uint64_t>(c_Copy.GetValueU32()));
      }
      break;
    case C_OscNodeDataPoolContent::eUINT64:
      if (orc_Content.GetValueU64() != c_Copy.GetValueU64()) {
        q_Retval = false;
        c_Is =
            C_OscHalcDefStructFiler::mh_ConvertToHex(orc_Content.GetValueU64());
        c_Should =
            C_OscHalcDefStructFiler::mh_ConvertToHex(c_Copy.GetValueU64());
      }
      break;
    default:
      // invalid
      orc_XmlParser.ReportErrorForNodeContentAppendXmlContext(
          "invalid type for bitmask detected");
      break;
    }
    if (q_Retval == false) {
      orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
          "initial-value", "Bitmask has invalid initial value \"" + c_Is +
                               "\" which should be \"" + c_Should + "\"");
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscHalcDefStructFiler::mh_ConvertToHex(const uint64_t ou64_Value) {
  return QString("0x%1").arg(ou64_Value, 0, 16);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_CheckValidUint(const QString &orc_Item) {
  bool q_Ok = false;
  orc_Item.toULongLong(&q_Ok, 0);
  if (q_Ok == false) {
    return C_RANGE;
  }
  return C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_CheckValidSint(const QString &orc_Item) {
  bool q_Ok = false;
  orc_Item.toLongLong(&q_Ok, 0);
  if (q_Ok == false) {
    return C_RANGE;
  }
  return C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefStructFiler::mh_CheckValidDouble(const QString &orc_Item) {
  bool q_Ok = false;
  orc_Item.toDouble(&q_Ok);
  if (q_Ok == false) {
    return C_RANGE;
  }
  return C_NO_ERR;
