...
C_OscHalcDefBase::C_OscHalcDefBase(void)
    : u32_ContentVersion(0UL), e_SafetyMode(eTWO_LEVELS_WITH_DROPPING),
      u8_NumConfigCopies(1U), q_NvmBasedConfig(false),
...
bool C_OscHalcDefBase::CheckIdsUnique(QStringList &orc_DuplicateIds) const {
  ...
  // Find all valid IDs
  for (uint32_t u32_ItDomain = 0UL; u32_ItDomain < this->GetDomainSize();
       ++u32_ItDomain) {
    const C_OscHalcDefDomain *const pc_Domain =
        this->GetDomainDefDataConst(u32_ItDomain);
    Q_ASSERT(pc_Domain != NULL);
    if (pc_Domain != NULL) {
      c_AllIds.push_back(pc_Domain->c_Id);
      for (uint32_t u32_ItChannel = 0UL;
           u32_ItChannel < pc_Domain->c_ChannelUseCases.size();
           ++u32_ItChannel) {
        const C_OscHalcDefChannelUseCase &rc_Channel =
            pc_Domain->c_ChannelUseCases[u32_ItChannel];
        c_AllIds.push_back(rc_Channel.c_Id);
      }
      C_OscHalcDefBase::mh_AggregateIds(pc_Domain->c_ChannelValues.c_Parameters,
                                        c_AllIds);
      C_OscHalcDefBase::mh_AggregateIds(pc_Domain->c_DomainValues.c_Parameters,
                                        c_AllIds);
    }
  }

  // Check all IDs
  for (uint32_t u32_ItCompare1 = 0UL; u32_ItCompare1 < c_AllIds.size();
       ++u32_ItCompare1) {
    for (uint32_t u32_ItCompare2 = u32_ItCompare1;
         u32_ItCompare2 < c_AllIds.size(); ++u32_ItCompare2) {
      // Check every string, but don't compare with itself
      const QString &rc_Comp1 = c_AllIds[u32_ItCompare1];
      if ((u32_ItCompare1 != u32_ItCompare2) &&
          (rc_Comp1 == c_AllIds[u32_ItCompare2])) {
        bool q_AlreadyExists = false;
        for (uint32_t u32_ItOut = 0UL; u32_ItOut < orc_DuplicateIds.size();
             ++u32_ItOut) {
...
bool C_OscHalcDefBase::IsClear(void) const {
  ...
  if ((this->u32_ContentVersion == 0UL) && (this->c_DeviceName == "") &&
      (this->c_FileString == "") && (this->c_OriginalFileName == "") &&
      (this->e_SafetyMode == eTWO_LEVELS_WITH_DROPPING) &&
      (this->u8_NumConfigCopies == 1U) && (this->q_NvmBasedConfig == false) &&
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcDefBase::mh_AggregateIds(
    const QList<C_OscHalcDefStruct> &orc_Items, QStringList &orc_DuplicateIds) {
  for (uint32_t u32_ItItem = 0UL; u32_ItItem < orc_Items.size(); ++u32_ItItem) {
    ...
    for (uint32_t u32_ItItemElem = 0UL;
         u32_ItItemElem < rc_Item.c_StructElements.size(); ++u32_ItItemElem) {
      const C_OscHalcDefElement &rc_ItemElem =
          rc_Item.c_StructElements[u32_ItItemElem];
