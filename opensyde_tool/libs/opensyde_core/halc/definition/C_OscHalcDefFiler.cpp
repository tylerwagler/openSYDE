...
/*! \brief  Load HALC definition
   ...
   XML file) C_CONFIG    IO file content is invalid or incomplete IO file could
   not be loaded
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_LoadFile(C_OscHalcDefBase &orc_IoData,
  ...
  if (QFileInfo(orc_Path).exists() && QFileInfo(orc_Path).isFile()) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        // Remember name
        orc_IoData.c_OriginalFileName = QFileInfo(orc_Path).fileName();
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_SaveFile(const C_OscHalcDefBase &orc_IoData,
                                      const QString &orc_Path) {
  const int32_t s32_Retval = C_OscSystemFilerUtil::h_SaveStringToFile(
      orc_IoData.c_FileString, orc_Path, "Saving HALC definition");
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_LoadData(C_OscHalcDefBase &orc_IoData,
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      osc_write_log_info("Loading HALC definition",
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("safety-mode") == "safety-mode") {
      const QString c_NodeContent = orc_XmlParser.GetNodeContent();
      if (C_OscHalcDefFiler::mh_StringToSafetyMode(
              c_NodeContent, orc_IoData.e_SafetyMode) != C_NO_ERR) {
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("number-of-configuration-copies") ==
        "number-of-configuration-copies") {
      const QString c_NodeContent = orc_XmlParser.GetNodeContent();
      try {
        orc_IoData.u8_NumConfigCopies =
            static_cast<uint8_t>(c_NodeContent.toInt());
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_SaveData(const C_OscHalcDefBase &orc_IoData,
                                      C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  orc_XmlParser.SelectNodeParent();
  // Content version
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("content-version") ==
           "content-version");
  orc_XmlParser.SetNodeContent(QString::number(orc_IoData.u32_ContentVersion));
  // Return
  orc_XmlParser.SelectNodeParent();
  // Device
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("device-name") ==
           "device-name");
  orc_XmlParser.SetNodeContent(orc_IoData.c_DeviceName);
  // Return
  orc_XmlParser.SelectNodeParent();
  // Safety mode
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("safety-mode") ==
           "safety-mode");
  ...
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(
               "number-of-configuration-copies") ==
           "number-of-configuration-copies");
  orc_XmlParser.SetNodeContent(QString::number(orc_IoData.u8_NumConfigCopies));
  // Return
  orc_XmlParser.SelectNodeParent();
  // NVM config
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("nvm-config") ==
           "nvm-config");
  orc_XmlParser.SetAttributeBool("active", orc_IoData.q_NvmBasedConfig);
  if (orc_IoData.q_NvmBasedConfig) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(
                 "datapools-start-address-offset") ==
             "datapools-start-address-offset");
    for (uint32_t u32_It = 0UL;
         u32_It < orc_IoData.c_NvmNonSafeAddressOffset.size(); ++u32_It) {
      const QString c_AttributeName = "address-offset-datapool-" +
                                      QString::number(u32_It + 1U) +
                                      "-non-safe";
      orc_XmlParser.SetAttributeUint32(
          c_AttributeName, orc_IoData.c_NvmNonSafeAddressOffset[u32_It]);
    }
    for (uint32_t u32_It = 0UL;
         u32_It < orc_IoData.c_NvmSafeAddressOffset.size(); ++u32_It) {
      const QString c_AttributeName =
          "address-offset-datapool-" + QString::number(u32_It + 1U) + "-safe";
      orc_XmlParser.SetAttributeUint32(
          c_AttributeName, orc_IoData.c_NvmSafeAddressOffset[u32_It]);
    }
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "nvm-config");
  }
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("lists-reserved-size") ==
           "lists-reserved-size");
  orc_XmlParser.SetAttributeUint32(
      "parameters-list-size", orc_IoData.u32_NvmReservedListSizeParameters);
  orc_XmlParser.SetAttributeUint32(
      "input-values-list-size", orc_IoData.u32_NvmReservedListSizeInputValues);
  orc_XmlParser.SetAttributeUint32(
  ...
  for (uint32_t u32_ItDomain = 0UL;
       (u32_ItDomain < orc_IoData.GetDomainSize()) && (s32_Retval == C_NO_ERR);
       ++u32_ItDomain) {
    const C_OscHalcDefDomain *const pc_Domain =
        orc_IoData.GetDomainDefDataConst(u32_ItDomain);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_LoadAvailability(
  ...
  int32_t s32_Retval = orc_XmlParser.GetAttributeStringError(
      orc_AttributeName, c_AvailabilityString);

  if (s32_Retval == C_NO_ERR) {
    QStringList c_SubElements;
    s32_Retval = C_OscHalcDefFiler::mh_SplitAvailabilityString(
        c_AvailabilityString, c_SubElements, orc_XmlParser, orc_AttributeName);

    orc_Availability.clear();
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = C_OscHalcDefFiler::mh_ParseAvailabilityStringSubElements(
          c_SubElements, orc_Availability, ou32_NumChannels, orc_XmlParser,
          orc_AttributeName);
    }
  }
  return s32_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_CheckUseCaseValue(
    const C_OscHalcDefDomain &orc_IoDataDomain) {
  int32_t s32_Retval = C_NO_ERR;

  for (uint32_t u32_ItUseCase = 0UL;
       (u32_ItUseCase < orc_IoDataDomain.c_ChannelUseCases.size()) &&
       (s32_Retval == C_NO_ERR);
       ++u32_ItUseCase) {
    for (uint32_t u32_ItOtherUseCase = 0UL;
         (u32_ItOtherUseCase < orc_IoDataDomain.c_ChannelUseCases.size()) &&
         (s32_Retval == C_NO_ERR);
         ++u32_ItOtherUseCase) {
      if (u32_ItUseCase != u32_ItOtherUseCase) {
        const C_OscHalcDefChannelUseCase &rc_UseCase =
            orc_IoDataDomain.c_ChannelUseCases[u32_ItUseCase];
        const C_OscHalcDefChannelUseCase &rc_OtherUseCase =
            orc_IoDataDomain.c_ChannelUseCases[u32_ItOtherUseCase];
        if ((rc_UseCase.c_Value.GetArray() ==
             rc_OtherUseCase.c_Value.GetArray()) &&
            (rc_UseCase.c_Value.GetType() ==
             rc_OtherUseCase.c_Value.GetType())) {
          if (rc_UseCase.c_Value == rc_OtherUseCase.c_Value) {
            osc_write_log_error("Loading HALC definition",
                                "Duplicate value found in \"value\" attribute "
                                "of \"channel-use-case\" section in domain \"" +
                                    orc_IoDataDomain.c_SingularName + "\".");
            s32_Retval = C_CONFIG;
          }
        } else {
          osc_write_log_error("Loading HALC definition",
                              "\"value\" attribute of \"channel-use-case\" "
                              "section is of different types in domain \"" +
                                  orc_IoDataDomain.c_SingularName + "\".");
          s32_Retval = C_CONFIG;
        }
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::h_CheckDomainDisplayNames(
    const C_OscHalcDefDomain &orc_IoDataDomain) {
  int32_t s32_Retval;

  QStringList c_Names;

  C_OscHalcDefFiler::mh_GetAllNames(
      orc_IoDataDomain.c_ChannelValues.c_Parameters, c_Names);
  C_OscHalcDefFiler::mh_GetAllNames(
      orc_IoDataDomain.c_DomainValues.c_Parameters, c_Names);

  s32_Retval = C_OscHalcDefFiler::mh_CheckDuplicateNames(
      "parameters", orc_IoDataDomain.c_SingularName, c_Names);
  if (s32_Retval == C_NO_ERR) {
    c_Names.clear();
    C_OscHalcDefFiler::mh_GetAllNames(
        orc_IoDataDomain.c_ChannelValues.c_InputValues, c_Names);
    C_OscHalcDefFiler::mh_GetAllNames(
        orc_IoDataDomain.c_DomainValues.c_InputValues, c_Names);
    s32_Retval = C_OscHalcDefFiler::mh_CheckDuplicateNames(
        "inputs", orc_IoDataDomain.c_SingularName, c_Names);
  }
  if (s32_Retval == C_NO_ERR) {
    c_Names.clear();
    C_OscHalcDefFiler::mh_GetAllNames(
        orc_IoDataDomain.c_ChannelValues.c_OutputValues, c_Names);
    C_OscHalcDefFiler::mh_GetAllNames(
        orc_IoDataDomain.c_DomainValues.c_OutputValues, c_Names);
    s32_Retval = C_OscHalcDefFiler::mh_CheckDuplicateNames(
        "outputs", orc_IoDataDomain.c_SingularName, c_Names);
  }
  if (s32_Retval == C_NO_ERR) {
    c_Names.clear();
    C_OscHalcDefFiler::mh_GetAllNames(
        orc_IoDataDomain.c_ChannelValues.c_StatusValues, c_Names);
    C_OscHalcDefFiler::mh_GetAllNames(
        orc_IoDataDomain.c_DomainValues.c_StatusValues, c_Names);
    s32_Retval = C_OscHalcDefFiler::mh_CheckDuplicateNames(
        "status", orc_IoDataDomain.c_SingularName, c_Names);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_LoadNvmData(C_OscHalcDefBase &orc_IoData,
                                          C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("nvm-config") == "nvm-config") {
    ...
    if (s32_Retval == C_NO_ERR) {
      if (orc_XmlParser.SelectNodeChild("datapools-start-address-offset") ==
          "datapools-start-address-offset") {
        s32_Retval = mh_LoadNvmAddressOffsetData(
            orc_IoData.c_NvmNonSafeAddressOffset, orc_XmlParser,
            orc_IoData.e_SafetyMode, orc_IoData.u8_NumConfigCopies, false);
        if (s32_Retval == C_NO_ERR) {
          s32_Retval = mh_LoadNvmAddressOffsetData(
              orc_IoData.c_NvmSafeAddressOffset, orc_XmlParser,
      ...
      if (orc_XmlParser.SelectNodeChild("lists-reserved-size") ==
          "lists-reserved-size") {
        if (orc_XmlParser.AttributeExists("parameters-list-size")) {
          orc_IoData.u32_NvmReservedListSizeParameters =
              orc_XmlParser.GetAttributeUint32("parameters-list-size");
        } else {
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              "parameters-list-size", "is missing, but nvm-config is active");
          s32_Retval = C_CONFIG;
        }
        if (orc_XmlParser.AttributeExists("input-values-list-size")) {
          orc_IoData.u32_NvmReservedListSizeInputValues =
              orc_XmlParser.GetAttributeUint32("input-values-list-size");
        } else {
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              "input-values-list-size", "is missing, but nvm-config is active");
          s32_Retval = C_CONFIG;
        }
        if (orc_XmlParser.AttributeExists("output-values-list-size")) {
          orc_IoData.u32_NvmReservedListSizeOutputValues =
              orc_XmlParser.GetAttributeUint32("output-values-list-size");
        } else {
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              "output-values-list-size",
              "is missing, but nvm-config is active");
        ...
        if (orc_XmlParser.AttributeExists("status-values-list-size")) {
          orc_IoData.u32_NvmReservedListSizeStatusValues =
              orc_XmlParser.GetAttributeUint32("status-values-list-size");
        } else {
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              "status-values-nvm-list-size",
              "is missing, but nvm-config is active");
          s32_Retval = C_CONFIG;
        }
        // Return
        ...
        if (orc_IoData.q_NvmBasedConfig) {
          orc_XmlParser.ReportErrorForNodeContentStartingWithXmlContext(
              "node \"lists-reserved-size\" is missing, but nvm-config is "
              "active");
          s32_Retval = C_CONFIG;
        }
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_LoadNvmAddressOffsetData(
  ...
  int32_t s32_Retval = C_NO_ERR;
  const QString c_StartingPart = "address-offset-datapool-";
  const QString c_ClosingPart = oq_IsSafeVector ? "-safe" : "-non-safe";

  for (uint8_t u8_It = 0U; u8_It < ou8_NumConfigCopies; ++u8_It) {
    // lint -e{9114} kept for readability
    const QString c_CompleteAttributeName =
        c_StartingPart + QString::number(u8_It + 1U) + c_ClosingPart;
    if (orc_XmlParser.AttributeExists(c_CompleteAttributeName)) {
      orc_Vector.push_back(
          orc_XmlParser.GetAttributeUint32(c_CompleteAttributeName));
    } else {
      if ((((oe_SafetyMode == C_OscHalcDefBase::eTWO_LEVELS_WITH_DROPPING) ||
            (oe_SafetyMode ==
             C_OscHalcDefBase::eTWO_LEVELS_WITHOUT_DROPPING)) ||
        ...
        orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
            c_CompleteAttributeName, "is missing, but nvm-config is active");
        s32_Retval = C_CONFIG;
      }
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcDefFiler::mh_SaveIoDomain(const C_OscHalcDefDomain &orc_IoDataDomain,
                                   C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  // Singular name
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("singular-name") ==
           "singular-name");
  ...
  orc_XmlParser.SetNodeContent(C_OscHalcDefFiler::mh_DomainCategoryEnumToString(
  ...
  orc_XmlParser.SetAttributeUint32(
      "count", static_cast<uint32_t>(orc_IoDataDomain.c_Channels.size()));
  for (uint32_t u32_ItChannel = 0UL;
       u32_ItChannel < orc_IoDataDomain.c_Channels.size(); ++u32_ItChannel) {
    const C_OscHalcDefChannelDef &rc_Channel =
        orc_IoDataDomain.c_Channels[u32_ItChannel];
  ...
  if (orc_IoDataDomain.c_ChannelUseCases.size() > 0UL) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("channel-use-cases") ==
             "channel-use-cases");
    for (uint32_t u32_ItCu = 0UL;
         (u32_ItCu < orc_IoDataDomain.c_ChannelUseCases.size()) &&
         (s32_Retval == C_NO_ERR);
         ++u32_ItCu) {
      const C_OscHalcDefChannelUseCase &rc_UseCase =
          orc_IoDataDomain.c_ChannelUseCases[u32_ItCu];
      if (u32_ItCu == 0UL) {
        // First saves the type
        orc_XmlParser.SetAttributeString(
            "type", C_OscHalcDefStructFiler::h_GetTypeString(
                        rc_UseCase.c_Value.GetType()));
      }
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("channel-use-case") ==
               "channel-use-case");
      s32_Retval = mh_SaveUseCase(orc_IoDataDomain.c_ChannelUseCases[u32_ItCu],
                                  orc_XmlParser);
      if (s32_Retval == C_NO_ERR) {
        // Return
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
        orc_IoDataDomain.c_DomainValues.c_Parameters,
        orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "domain",
        "domain-parameters", "parameter", "parameter-element");
  }
  if (s32_Retval == C_NO_ERR) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("domain-values") ==
             "domain-values");
    s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
        orc_IoDataDomain.c_DomainValues.c_InputValues,
        orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "domain-values",
        "input-values", "value", "value-element");
  }
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
        orc_IoDataDomain.c_DomainValues.c_OutputValues,
        orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "domain-values",
        "output-values", "value", "value-element");
  }
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
        orc_IoDataDomain.c_DomainValues.c_StatusValues,
        orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "domain-values",
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    {
      s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
          orc_IoDataDomain.c_ChannelValues.c_Parameters,
          orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "domain",
          "channel-parameters", "parameter", "parameter-element");

      if (s32_Retval == C_NO_ERR) {
        Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("channel-values") ==
                 "channel-values");
        s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
            orc_IoDataDomain.c_ChannelValues.c_InputValues,
            orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "channel-values",
            "input-values", "value", "value-element");
      }
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
            orc_IoDataDomain.c_ChannelValues.c_OutputValues,
            orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "channel-values",
            "output-values", "value", "value-element");
      }
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefStructFiler::h_SaveStructs(
            orc_IoDataDomain.c_ChannelValues.c_StatusValues,
            orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser, "channel-values",
            "status-values", "value", "value-element");
      }
      if (s32_Retval == C_NO_ERR) {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcDefFiler::mh_LoadIoDataDomain(C_OscHalcDefDomain &orc_IoDataDomain,
                                       C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval =
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()) >
          (C_OscHalcDefStructFiler::hu32_MAX_ALLOWED_COMBINED_VARIABLE_LENGTH -
           u32_LONGEST_CONST_VAR_NAME_OFFSET)) {
        orc_XmlParser.ReportErrorForNodeContentStartingWithXmlContext(
            "content of domain \"singular-name\" (or \"name\" if not existing) "
            "node is too long, maximum allowed characters: " +
            QString::number(C_OscHalcDefStructFiler::
                                hu32_MAX_ALLOWED_COMBINED_VARIABLE_LENGTH -
                            u32_LONGEST_CONST_VAR_NAME_OFFSET) +
            " (Current: " +
            QString::number(orc_IoDataDomain.c_SingularName.length()) + ")");
        s32_Retval = C_CONFIG;
      }
    }
  }
  if (s32_Retval == C_NO_ERR) {
    ...
    if (orc_XmlParser.SelectNodeChild("category") == "category") {
      if (C_OscHalcDefFiler::mh_DomainCategoryStringToEnum(
              orc_XmlParser.GetNodeContent(), orc_IoDataDomain.e_Category) !=
          C_NO_ERR) {
        s32_Retval = C_CONFIG;
        orc_XmlParser.ReportErrorForNodeContentAppendXmlContext(
            "Unknown value");
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = mh_LoadChannelUseCases(
        orc_IoDataDomain.c_ChannelUseCases, orc_XmlParser,
        static_cast<uint32_t>(orc_IoDataDomain.c_Channels.size()));
  }
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
        orc_IoDataDomain.c_DomainValues.c_Parameters, orc_XmlParser,
        orc_IoDataDomain.c_ChannelUseCases, "domain", "domain-parameters",
        "parameter", "parameter-element", true, false,
        static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
  }
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("domain-values") == "domain-values") {
      s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
          orc_IoDataDomain.c_DomainValues.c_InputValues, orc_XmlParser,
          orc_IoDataDomain.c_ChannelUseCases, "domain-values", "input-values",
          "value", "value-element", false, false,
          static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
            orc_IoDataDomain.c_DomainValues.c_OutputValues, orc_XmlParser,
            orc_IoDataDomain.c_ChannelUseCases, "domain-values",
            "output-values", "value", "value-element", false, false,
            static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
      }
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
            orc_IoDataDomain.c_DomainValues.c_StatusValues, orc_XmlParser,
            orc_IoDataDomain.c_ChannelUseCases, "domain-values",
            "status-values", "value", "value-element", false, false,
            static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
      }
      if (s32_Retval == C_NO_ERR) {
        // Return
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
        orc_IoDataDomain.c_ChannelValues.c_Parameters, orc_XmlParser,
        orc_IoDataDomain.c_ChannelUseCases, "domain", "channel-parameters",
        "parameter", "parameter-element", true, false,
        static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
  }
  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("channel-values") == "channel-values") {
      s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
          orc_IoDataDomain.c_ChannelValues.c_InputValues, orc_XmlParser,
          orc_IoDataDomain.c_ChannelUseCases, "channel-values", "input-values",
          "value", "value-element", false, false,
          static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
            orc_IoDataDomain.c_ChannelValues.c_OutputValues, orc_XmlParser,
            orc_IoDataDomain.c_ChannelUseCases, "channel-values",
            "output-values", "value", "value-element", false, false,
            static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
      }
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefStructFiler::h_LoadStructs(
            orc_IoDataDomain.c_ChannelValues.c_StatusValues, orc_XmlParser,
            orc_IoDataDomain.c_ChannelUseCases, "channel-values",
            "status-values", "value", "value-element", false, false,
            static_cast<uint32_t>(orc_IoDataDomain.c_SingularName.length()));
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_CheckIoDataDomain(
    const C_OscHalcDefDomain &orc_IoDataDomain) {
  int32_t s32_Retval =
      C_OscHalcDefFiler::mh_CheckDefaultUseCase(orc_IoDataDomain);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_CheckDefaultUseCase(
    const C_OscHalcDefDomain &orc_IoDataDomain) {
  int32_t s32_Retval = C_NO_ERR;

  // check all channels if a default use case exists
  for (uint32_t u32_ItChannel = 0UL;
       (u32_ItChannel < orc_IoDataDomain.c_Channels.size()) &&
       (s32_Retval == C_NO_ERR);
       ++u32_ItChannel) {
    ...
    for (uint32_t u32_ItUseCase = 0UL;
      ...
      for (uint32_t u32_ItDefault = 0UL;
           (u32_ItDefault < rc_UseCase.c_DefaultChannels.size()) &&
           (s32_Retval == C_NO_ERR);
           ++u32_ItDefault) {
        if (u32_ItChannel == rc_UseCase.c_DefaultChannels[u32_ItDefault]) {
          if (q_FoundOne == false) {
            ...
            osc_write_log_error("Loading HALC definition",
                                "Ambiguous default in attribute "
                                "\"is-default-for\" for use-case index " +
                                    QString::number(u32_ItUseCase) +
                                    " in domain \"" +
                                    orc_IoDataDomain.c_SingularName + "\".");
            s32_Retval = C_CONFIG;
            break;
    ...
    if (q_FoundOne == false) {
      osc_write_log_error("Loading HALC definition",
                          "Could not find default in attribute "
                          "\"is-default-for\" for channel index " +
                              QString::number(u32_ItChannel) + " in domain \"" +
                              orc_IoDataDomain.c_SingularName + "\".");
      s32_Retval = C_CONFIG;
      break;
    }
  ...
  for (uint32_t u32_ItUseCase = 0UL;
    ...
    for (uint32_t u32_ItDefault = 0UL;
      ...
      for (uint32_t u32_ItAvail = 0UL;
           (u32_ItAvail < rc_UseCase.c_Availability.size()) &&
           (s32_Retval == C_NO_ERR);
           ++u32_ItAvail) {
        if (rc_UseCase.c_DefaultChannels[u32_ItDefault] ==
            rc_UseCase.c_Availability[u32_ItAvail].u32_ValueIndex) {
      ...
      if (q_DefaultAvailable == false) {
        osc_write_log_error(
            ...
            "Default in attribute \"is-default-for\" of use-case index " +
                QString::number(u32_ItUseCase) + " (\"" + rc_UseCase.c_Display +
                "\") is not available for this use-case in domain \"" +
                orc_IoDataDomain.c_SingularName + "\".");
        s32_Retval = C_CONFIG;
      }
    }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcDefFiler::mh_LoadChannels(QList<C_OscHalcDefChannelDef> &orc_Channels,
                                   C_OscXmlParserBase &orc_XmlParser) {
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    s32_Retval =
        orc_XmlParser.GetAttributeUint32Error("count", u32_ExpectedCount);
    ...
    if ((u32_ExpectedCount > 0UL) && (s32_Retval == C_NO_ERR)) {
      ...
      if (s32_Retval == C_NO_ERR) {
        QString c_NodeChannel;
        do {
          C_OscHalcDefChannelDef c_Channel;
          s32_Retval =
              orc_XmlParser.GetAttributeStringError("name", c_Channel.c_Name);
          if (s32_Retval == C_NO_ERR) {
      ...
      if (u32_ExpectedCount != orc_Channels.size()) {
        const QString c_Error =
            "Expected " + QString::number(u32_ExpectedCount) +
            " channels, got " + QString::number(orc_Channels.size()) +
            " channels";
...
/*! \brief  Load IO channel use-cases
   ...
   \param[in,out]  orc_XmlParser          XML with default state
   \param[in]      ou32_NumChannels       Number of available channels for this
   domain
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_LoadChannelUseCases(
    QList<C_OscHalcDefChannelUseCase> &orc_ChannelUsecases,
    C_OscXmlParserBase &orc_XmlParser, const uint32_t ou32_NumChannels) {
  int32_t s32_Retval = C_NO_ERR;

  orc_ChannelUsecases.clear();
  if (orc_XmlParser.SelectNodeChild("channel-use-cases") ==
      "channel-use-cases") {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      s32_Retval = C_OscHalcDefStructFiler::h_GetTypeForSimplestTypeString(
          c_TypeStr, e_Type, orc_XmlParser);
      if (s32_Retval == C_NO_ERR) {
        c_Value.SetType(e_Type);
      }
    }
    if (s32_Retval == C_NO_ERR) {
      QString c_NodeChannel = orc_XmlParser.SelectNodeChild("channel-use-case");
      if (c_NodeChannel == "channel-use-case") {
        do {
          C_OscHalcDefChannelUseCase c_UseCase;
          s32_Retval =
              orc_XmlParser.GetAttributeStringError("id", c_UseCase.c_Id);
          if (s32_Retval == C_NO_ERR) {
            s32_Retval = orc_XmlParser.GetAttributeStringError(
                "display", c_UseCase.c_Display);
          }
          if (s32_Retval == C_NO_ERR) {
            c_UseCase.c_Value = c_Value;
            s32_Retval = C_OscHalcDefStructFiler::h_ParseSimplestTypeValue(
                c_TypeStr, c_UseCase.c_Value, orc_XmlParser, "value");
          }
          if (s32_Retval == C_NO_ERR) {
            s32_Retval =
                h_LoadAvailability("availability", c_UseCase.c_Availability,
                                   ou32_NumChannels, orc_XmlParser);
            if (s32_Retval == C_NO_ERR) {
              C_OscHalcDefFiler::mh_CheckAvailability(c_UseCase.c_Availability,
                                                      orc_XmlParser);
            }
          }
          if (s32_Retval == C_NO_ERR) {
            ...
            if (orc_XmlParser.AttributeExists("is-default-for")) {
              s32_Retval =
                  h_LoadAvailability("is-default-for", c_TmpAvailability,
                                     ou32_NumChannels, orc_XmlParser);
            }
            c_UseCase.c_DefaultChannels.clear();
            for (uint32_t u32_It = 0UL; (u32_It < c_TmpAvailability.size()) &&
                                        (s32_Retval == C_NO_ERR);
                 ++u32_It) {
              const C_OscHalcDefChannelAvailability &rc_Availability =
                  c_TmpAvailability[u32_It];
              c_UseCase.c_DefaultChannels.push_back(
                  rc_Availability.u32_ValueIndex);
              if (rc_Availability.c_DependentValues.size() > 0UL) {
                orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
                    "is-default-for", "Found invalid grouped section");
                s32_Retval = C_CONFIG;
              }
            }
          }
        ...
        } while ((c_NodeChannel == "channel-use-case") &&
                 (s32_Retval == C_NO_ERR));
        if (s32_Retval == C_NO_ERR) {
          // Return
          Q_ASSERT(orc_XmlParser.SelectNodeParent() == "channel-use-cases");
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Parse IO channel use-case availability string into its string sub
   elements
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_SplitAvailabilityString(
    const QString &orc_AvailabilityString, QStringList &orc_SubElements,
    const C_OscXmlParserBase &orc_XmlParser, const QString &orc_AttributeName) {
  int32_t s32_Retval = C_NO_ERR;
  bool q_IgnoreComma = false;

  QString c_CurSubElement;

  orc_SubElements.clear();

  // Parse subelements
  for (int32_t s32_ItChar = 0; (s32_ItChar < orc_AvailabilityString.length()) &&
                               (s32_Retval == C_NO_ERR);
       ++s32_ItChar) {
    const QChar cn_CurChar = orc_AvailabilityString[s32_ItChar];
    if (cn_CurChar == ',') {
      // Char:comma
      if (q_IgnoreComma) {
        ...
        if (c_CurSubElement.isEmpty() == false) {
          ...
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              orc_AttributeName, "contains empty section");
          s32_Retval = C_CONFIG;
        }
      }
    } else {
      ...
      if (cn_CurChar == '{') {
        ...
        if (q_IgnoreComma) {
          ...
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              orc_AttributeName, "contains unexpected '}' character");
          s32_Retval = C_CONFIG;
        }
      } else {
        // No special handling
...
/*! \brief  Parse IO channel use-case availability string into its string sub
   ...
   \param[in]      ou32_NumChannels    Number of available channels for this
   domain \param[in]      orc_XmlParser       XML parser \param[in]
   orc_AttributeName   Attribute name
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_ParseAvailabilityStringSubElements(
  ...
  for (uint32_t u32_ItSubStr = 0UL;
       (u32_ItSubStr < orc_SubElements.size()) && (s32_Retval == C_NO_ERR);
       ++u32_ItSubStr) {
    const QString &rc_CurSubStr = orc_SubElements[u32_ItSubStr];
    if (rc_CurSubStr == "all") {
      for (uint32_t u32_ItChannel = 0UL; u32_ItChannel < ou32_NumChannels;
           ++u32_ItChannel) {
      ...
      for (int32_t s32_ItChar = 0;
           (s32_ItChar < rc_CurSubStr.length()) && (s32_Retval == C_NO_ERR);
           ++s32_ItChar) {
        const QChar cn_CurChar = rc_CurSubStr[s32_ItChar];
        if (cn_CurChar.isDigit()) {
          // Number
          c_Number += cn_CurChar;
        } else {
          ...
          if (cn_CurChar == '-') {
            if (q_LastNumDeclaredSection) {
              orc_XmlParser
                  .ReportErrorForAttributeContentStartingWithXmlContext(
                      orc_AttributeName,
                      "contains unexpected character sequence with multiple "
                      "'-' characters:\"" +
                          rc_CurSubStr + "\"");
              s32_Retval = C_CONFIG;
            } else {
              ...
              s32_Retval = C_OscHalcDefFiler::mh_ConvertStringToNumber(
                  c_Number, s32_NumberTmp, orc_XmlParser, orc_AttributeName);
              c_Number = "";
            ...
            // Handle vector
            s32_Retval = C_OscHalcDefFiler::mh_HandleNumberSection(
                c_Number, c_FoundNumbers, q_LastNumDeclaredSection,
                rc_CurSubStr, orc_XmlParser, orc_AttributeName);
          } else {
            orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
                orc_AttributeName,
                "contains unexpected character '" + QString(cn_CurChar) + "'");
            s32_Retval = C_CONFIG;
          }
        }
      }
      ...
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefFiler::mh_HandleNumberSection(
            c_Number, c_FoundNumbers, q_LastNumDeclaredSection, rc_CurSubStr,
            orc_XmlParser, orc_AttributeName);
      }
      // Handle availability
      if (s32_Retval == C_NO_ERR) {
        s32_Retval = C_OscHalcDefFiler::mh_HandleNumberSectionEnd(
            c_FoundNumbers, q_IsGroupSection, orc_Availability,
            ou32_NumChannels, orc_XmlParser, orc_AttributeName);
      }
    }
  }
  return s32_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_CheckAvailability(
    const QList<C_OscHalcDefChannelAvailability> &orc_Availability,
    const C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  // Compare each with everyone
  for (uint32_t u32_ItAvailability = 0UL;
       u32_ItAvailability < orc_Availability.size(); ++u32_ItAvailability) {
    const C_OscHalcDefChannelAvailability &rc_Availability =
        orc_Availability[u32_ItAvailability];
    for (uint32_t u32_ItOtherAvailability = 0UL;
         u32_ItOtherAvailability < orc_Availability.size();
         ++u32_ItOtherAvailability) {
      if (u32_ItAvailability != u32_ItOtherAvailability) {
        const C_OscHalcDefChannelAvailability &rc_OtherAvailability =
            orc_Availability[u32_ItOtherAvailability];
        ...
        if (rc_Availability.u32_ValueIndex ==
            rc_OtherAvailability.u32_ValueIndex) {
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              "availability",
              "contains duplicate usage of channel index " +
                  QString::number(rc_OtherAvailability.u32_ValueIndex));
          s32_Retval = C_CONFIG;
        }
      }
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_ConvertStringToNumber(
    const QString &orc_Number, int32_t &ors32_Number,
    const C_OscXmlParserBase &orc_XmlParser, const QString &orc_AttributeName) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  try {
    ...
    orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
        orc_AttributeName,
        "contains unexpected character sequence \"" + orc_Number + "\"");
    s32_Retval = C_CONFIG;
  }
  return s32_Retval;
}
...
/*! \brief  Handle number section segment
   ...
   \param[in,out]  orq_LastNumDeclaredSection   Flag if last element of section
   should be connected to this number \param[in]      orc_Section Section string
   for user information \param[in]      orc_XmlParser                XML parser
   \param[in]      orc_AttributeName            Attribute name
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_HandleNumberSection(
    QString &orc_Number, QList<int32_t> &orc_FoundNumbers,
  ...
  // Handle any left over number
  s32_Retval = C_OscHalcDefFiler::mh_ConvertStringToNumber(
      orc_Number, s32_NumberTmp, orc_XmlParser, orc_AttributeName);
  orc_Number = "";
  if (s32_Retval == C_NO_ERR) {
    if (orq_LastNumDeclaredSection) {
      ...
      if (orc_FoundNumbers.size() > 0UL) {
        const int32_t s32_NumberStart =
            orc_FoundNumbers[static_cast<QList<int32_t>::size_type>(
                orc_FoundNumbers.size() - 1UL)];
        orc_FoundNumbers.pop_back();
        if (s32_NumberStart < s32_NumberTmp) {
          for (int32_t s32_It = s32_NumberStart; s32_It <= s32_NumberTmp;
               ++s32_It) {
          ...
          orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
              orc_AttributeName,
              "contains invalid usage of '-' character in \"" + orc_Section +
                  "\"");
          s32_Retval = C_CONFIG;
        }
      } else {
        orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
            orc_AttributeName,
            "contains confusing usage of '-' character in \"" + orc_Section +
                "\"");
        s32_Retval = C_CONFIG;
      }
    } else {
...
/*! \brief  Handle number section segment end
   ...
   \param[in]      oq_IsGroupSection   Flag if current section is group of
   elements \param[in,out]  orc_Availability    Availability \param[in]
   ou32_NumChannels    Number of available channels for this domain \param[in]
   orc_XmlParser       XML parser \param[in]      orc_AttributeName   Attribute
   name
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcDefFiler::mh_HandleNumberSectionEnd(
  ...
  // Check
  for (uint32_t u32_ItNum = 0UL; u32_ItNum < orc_FoundNumbers.size();
       ++u32_ItNum) {
    if (orc_FoundNumbers[u32_ItNum] >= static_cast<int32_t>(ou32_NumChannels)) {
      orc_XmlParser.ReportErrorForAttributeContentStartingWithXmlContext(
          orc_AttributeName,
          "has number " + QString::number(orc_FoundNumbers[u32_ItNum]) +
              " which is out of range of number of channels: " +
              QString::number(ou32_NumChannels));
      s32_Retval = C_CONFIG;
    }
  }
  // Assign
    ...
    if (orc_FoundNumbers.size() > 0UL) {
      {
        ...
        for (uint32_t u32_ItChannel = 1UL;
             u32_ItChannel < orc_FoundNumbers.size(); ++u32_ItChannel) {
          c_Availability.c_DependentValues.push_back(
              orc_FoundNumbers[u32_ItChannel]);
        }
        orc_Availability.push_back(c_Availability);
      }
      // handle other sorting
      for (uint32_t u32_ItOthers = 1UL; u32_ItOthers < orc_FoundNumbers.size();
           ++u32_ItOthers) {
        C_OscHalcDefChannelAvailability c_Availability;
        c_Availability.u32_ValueIndex = orc_FoundNumbers[u32_ItOthers];
        for (uint32_t u32_ItDepend = 0UL;
             u32_ItDepend < orc_FoundNumbers.size(); ++u32_ItDepend) {
          if (orc_FoundNumbers[u32_ItDepend] !=
              orc_FoundNumbers[u32_ItOthers]) {
            c_Availability.c_DependentValues.push_back(
                orc_FoundNumbers[u32_ItDepend]);
          }
        }
        orc_Availability.push_back(c_Availability);
      }
...
QString C_OscHalcDefFiler::mh_GetAvailabilityString(
  ...
  for (uint32_t u32_ItAva = 0UL; u32_ItAva < orc_Availability.size();
       ++u32_ItAva) {
    const C_OscHalcDefChannelAvailability &rc_Avail =
        orc_Availability[u32_ItAva];
    if (rc_Avail.c_DependentValues.size() == 0UL) {
      if (c_Retval.isEmpty() == false) {
        c_Retval += ",";
      }
      c_Retval += QString::number(rc_Avail.u32_ValueIndex);
    } else {
      bool q_Continue = true;
      if (oq_OnlySaveOnce) {
        // Check sorted ascending (only save each pairing exactly once)
        uint32_t u32_RefVal = rc_Avail.u32_ValueIndex;
        for (uint32_t u32_ItDep = 0UL;
             u32_ItDep < rc_Avail.c_DependentValues.size(); ++u32_ItDep) {
      ...
      if (q_Continue) {
        ...
        for (uint32_t u32_ItDep = 0UL;
             u32_ItDep < rc_Avail.c_DependentValues.size(); ++u32_ItDep) {
          c_Retval += ",";
          c_Retval += QString::number(rc_Avail.c_DependentValues[u32_ItDep]);
        }
        c_Retval += "}";
      }
    }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcDefFiler::mh_SaveUseCase(const C_OscHalcDefChannelUseCase &orc_UseCase,
                                  C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval;

  orc_XmlParser.SetAttributeString("id", orc_UseCase.c_Id);
  orc_XmlParser.SetAttributeString("display", orc_UseCase.c_Display);
  s32_Retval = C_OscHalcDefStructFiler::h_SaveSimpleValueAsAttribute(
      "value", orc_XmlParser, orc_UseCase.c_Value);
  orc_XmlParser.SetAttributeString("availability",
                                   C_OscHalcDefFiler::mh_GetAvailabilityString(
                                       orc_UseCase.c_Availability, true));
  {
    QList<C_OscHalcDefChannelAvailability> c_TmpAvailabilities;
    for (uint32_t u32_It = 0UL; u32_It < orc_UseCase.c_DefaultChannels.size();
         ++u32_It) {
    ...
    if (c_TmpAvailabilities.size() > 0UL) {
      orc_XmlParser.SetAttributeString(
          "is-default-for", C_OscHalcDefFiler::mh_GetAvailabilityString(
                                c_TmpAvailabilities, false));
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcDefFiler::mh_GetAllNames(
    const QList<C_OscHalcDefStruct> &orc_Values, QStringList &orc_Names) {
  for (uint32_t u32_ItVal = 0UL; u32_ItVal < orc_Values.size(); ++u32_ItVal) {
    ...
    for (uint32_t u32_ItValElem = 0UL;
         u32_ItValElem < rc_Struct.c_StructElements.size(); ++u32_ItValElem) {
      const C_OscHalcDefElement &rc_Elem =
          rc_Struct.c_StructElements[u32_ItValElem];
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcDefFiler::mh_CheckDuplicateNames(const QString &orc_Section,
                                          const QString &orc_DomainSingularName,
                                          const QStringList &orc_Names) {
  int32_t s32_Retval = C_NO_ERR;

  for (uint32_t u32_ItName1 = 0UL;
       (u32_ItName1 < orc_Names.size()) && (s32_Retval == C_NO_ERR);
       ++u32_ItName1) {
    for (uint32_t u32_ItName2 = u32_ItName1;
         (u32_ItName2 < orc_Names.size()) && (s32_Retval == C_NO_ERR);
         ++u32_ItName2) {
      if (u32_ItName1 != u32_ItName2) {
        const QString &rc_Name = orc_Names[u32_ItName2];
        if (orc_Names[u32_ItName1] == rc_Name) {
          s32_Retval = C_CONFIG;
          osc_write_log_error("Loading HALC definition",
                              "Duplicate display name found in domain \"" +
                                  orc_DomainSingularName + "\", section \"" +
                                  orc_Section + "\": \"" +
