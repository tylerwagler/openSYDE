...
/*! \brief  Load IO standalone description
   ...
   XML file) C_CONFIG    HALC configuration content is invalid or incomplete
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigStandaloneFiler::h_LoadFileStandalone(
    C_OscHalcConfigStandalone &orc_IoData, const QString &orc_Path) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigStandaloneFiler::h_SaveFileStandalone(
    const C_OscHalcConfigStandalone &orc_IoData, const QString &orc_Path) {
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (s32_Retval != C_NO_ERR) {
        osc_write_log_error("Saving IO standalone data",
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigStandaloneFiler::h_LoadDataStandalone(
    C_OscHalcConfigStandalone &orc_IoData, C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  // Device Type
  if (orc_XmlParser.SelectNodeChild("definition-content-version") ==
      "definition-content-version") {
    orc_IoData.u32_DefinitionContentVersion = 0UL;
    try {
      orc_IoData.u32_DefinitionContentVersion =
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (orc_XmlParser.SelectNodeChild("domains") == "domains") {
      ...
      while ((c_NodeDomain == "domain") && (s32_Retval == C_NO_ERR)) {
        ...
        s32_Retval =
            C_OscHalcConfigFiler::h_LoadIoDomain(c_Domain, orc_XmlParser);
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          // Channel Names
          if (orc_XmlParser.SelectNodeChild("channel-names") ==
              "channel-names") {
            QString c_NodeChannelId =
                orc_XmlParser.SelectNodeChild("channel-name");
            bool q_AtLeastOneChannelId = false;

            while ((c_NodeChannelId == "channel-name") &&
                   (s32_Retval == C_NO_ERR)) {
              ...
              // Parameter Ids
              if (s32_Retval == C_NO_ERR) {
                if (orc_XmlParser.SelectNodeChild("parameter-ids") ==
                    "parameter-ids") {
                  QString c_NodeParameterId =
                      orc_XmlParser.SelectNodeChild("parameter-id");
                  bool q_AtLeastOneParamterId = false;

                  while (c_NodeParameterId == "parameter-id") {
                    c_ChannelId.c_ParameterIds.push_back(
                    ...
                    c_NodeParameterId =
                        orc_XmlParser.SelectNodeNext("parameter-id");
                  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigStandaloneFiler::h_SaveDataStandalone(
    const C_OscHalcConfigStandalone &orc_IoData,
  ...
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(
               "definition-content-version") == "definition-content-version");
  orc_XmlParser.SetNodeContent(
      QString::number(orc_IoData.u32_DefinitionContentVersion));
  // Return
  orc_XmlParser.SelectNodeParent();

  // Definition content version
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("device-type") ==
           "device-type");
  ...
  for (u32_DomainCounter = 0U; u32_DomainCounter < orc_IoData.c_Domains.size();
       ++u32_DomainCounter) {
    const C_OscHalcConfigStandaloneDomain &rc_Domain =
        orc_IoData.c_Domains[u32_DomainCounter];

    // Vector size must be identical
    Q_ASSERT(rc_Domain.c_ChannelConfigs.size() ==
             rc_Domain.c_StandaloneChannels.size());
    Q_ASSERT(rc_Domain.c_Channels.size() ==
             rc_Domain.c_StandaloneChannels.size());
    ...
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("domain-id") ==
             "domain-id");
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("channel-names") ==
               "channel-names");

      for (u32_ChannelCounter = 0U;
           u32_ChannelCounter < rc_Domain.c_StandaloneChannels.size();
           ++u32_ChannelCounter) {
        const C_OscHalcConfigStandaloneChannel &rc_ChannelId =
            rc_Domain.c_StandaloneChannels[u32_ChannelCounter];
        uint32_t u32_ParameterCounter;

        Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("channel-name") ==
                 "channel-name");
        orc_XmlParser.CreateNodeChild(
            "name", rc_Domain.c_Channels[u32_ChannelCounter].c_Name);

        // Parameter Ids of channel
        Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("parameter-ids") ==
                 "parameter-ids");

        for (u32_ParameterCounter = 0U;
             u32_ParameterCounter < rc_ChannelId.c_ParameterIds.size();
             ++u32_ParameterCounter) {
          Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("parameter-id") ==
                   "parameter-id");
          orc_XmlParser.SetNodeContent(
              rc_ChannelId.c_ParameterIds[u32_ParameterCounter]);
          // Return
          orc_XmlParser.SelectNodeParent();
        }
