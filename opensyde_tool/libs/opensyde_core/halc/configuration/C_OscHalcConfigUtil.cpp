...
/*! \brief  Prepares a stand alone configuration of the HALC
   ...
   \param[out]      orc_ConfigStandalone  Standalone HALC configuration as
   result with the configuration of orc_Config
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfigUtil::h_GetConfigStandalone(
  ...
  orc_ConfigStandalone.u32_DefinitionContentVersion =
      orc_Config.u32_ContentVersion;

  for (u32_DomainCounter = 0U; u32_DomainCounter < orc_Config.GetDomainSize();
       ++u32_DomainCounter) {
    const C_OscHalcConfigDomain *const pc_Domain =
        orc_Config.GetDomainConfigDataConst(u32_DomainCounter);
    ...
    if (pc_Domain != NULL) {
      ...
      for (u32_Channels = 0UL; u32_Channels < c_ChannelIds.size();
           ++u32_Channels) {
        uint32_t u32_Parameter;

        c_ChannelIds[u32_Channels].c_ParameterIds.resize(
            pc_Domain->c_ChannelValues.c_Parameters.size());

        // Get the Id of the parameters
        for (u32_Parameter = 0UL;
             u32_Parameter < pc_Domain->c_ChannelValues.c_Parameters.size();
             ++u32_Parameter) {
...

      {
        ...
        const C_OscHalcConfigStandaloneDomain c_StandaloneDomain(*pc_Domain,
                                                                 c_ChannelIds);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Prepares a stand alone configuration of the HALC with a specific
   channel

   \param[in]       orc_Config            HALC configuration as source
   \param[in]       ou32_DomainIndex      Index of domain in orc_Config
   \param[in]       ou32_ChannelIndex     Index of channel in domain with index
   ou32_DomainIndex \param[in]       oq_DomainOnly         Flag if the domain
   should be returned, not the concrete channel When true, ou32_ChannelIndex is
   not relevant \param[out]      orc_ConfigStandalone  Standalone HALC
   configuration as result with the configuration of orc_Config

   \retval   C_NO_ERR   Stand alone configuration prepared
   \retval   C_RANGE    ou32_DomainIndex or ou32_ChannelIndex is invalid
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigUtil::h_GetConfigStandaloneChannel(
    const C_OscHalcConfig &orc_Config, const uint32_t ou32_DomainIndex,
    ...
    if (pc_Domain != NULL) {
      if (((oq_DomainOnly == true) ||
        ...
        if (oq_DomainOnly == false) {
          ...
          c_ChannelIds[0].c_ParameterIds.resize(
              pc_Domain->c_ChannelValues.c_Parameters.size());
        ...
        {
          // Add the standalone domain
          C_OscHalcConfigStandaloneDomain c_StandaloneDomain(*pc_Domain,
                                                             c_ChannelIds);
          ...
          if (oq_DomainOnly == false) {
            ...
            if ((c_StandaloneDomain.c_ChannelConfigs.size() > 1) &&
                (ou32_ChannelIndex <
                 static_cast<uint32_t>(
                     c_StandaloneDomain.c_ChannelConfigs.size() - 1UL))) {
            ...
            if ((c_StandaloneDomain.c_ChannelConfigs.size() > 1) &&
              ...
              c_StandaloneDomain.c_Channels.erase(
                  c_StandaloneDomain.c_Channels.begin(),
                  c_StandaloneDomain.c_Channels.begin() + ou32_ChannelIndex);
            }
          } else {
