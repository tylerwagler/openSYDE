    ...
    if (s32_Retval == C_NO_ERR) {
      if (c_XmlParser.SelectRoot() == "opensyde-node-io-config") {
        ...
        osc_write_log_error("Loading IO data",
                            "Could not find \"opensyde-node-io-config\" node.");
        s32_Retval = C_CONFIG;
      }
    } else {
      osc_write_log_error("Loading IO data",
...
int32_t C_OscHalcConfigFiler::h_SaveFile(const C_OscHalcConfig &orc_IoData,
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    s32_Retval =
        h_SaveData(orc_IoData, c_XmlParser, orc_BasePath, opc_CreatedFiles);
    if (s32_Retval == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::h_LoadData(C_OscHalcConfig &orc_IoData,
                                         C_OscXmlParserBase &orc_XmlParser,
                                         const QString &orc_BasePath) {
  int32_t s32_Retval =
      mh_LoadIoDataBase(orc_IoData, orc_XmlParser, orc_BasePath);
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (s32_Retval == C_NO_ERR) {
        osc_write_log_info("Loading IO description",
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (s32_Retval == C_NO_ERR) {
        if (u32_RefId != orc_IoData.u32_ContentVersion) {
          orc_XmlParser.ReportErrorForNodeContentStartingWithXmlContext(
              "mismatch between expected " + QString::number(u32_RefId) +
              " and current " + QString::number(orc_IoData.u32_ContentVersion));
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::h_SaveData(const C_OscHalcConfig &orc_IoData,
                                         C_OscXmlParserBase &orc_XmlParser,
                                         const QString &orc_BasePath,
                                         QStringList *const opc_CreatedFiles) {
  int32_t s32_Retval;
  ...
  orc_XmlParser.SelectNodeParent();
  // Content version
  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("ref-content-version") ==
           "ref-content-version");
  orc_XmlParser.SetNodeContent(QString::number(orc_IoData.u32_ContentVersion));
  // Return
  orc_XmlParser.SelectNodeParent();
  // IO data
  s32_Retval = mh_SaveIoDataBase(orc_IoData, orc_XmlParser, orc_BasePath,
                                 opc_CreatedFiles);
  if (s32_Retval == C_NO_ERR) {
    {
      // General
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("general") == "general");
      // Datablock assignment is now only in Datapools. Fill structure with
      // dummies to be able to open HALC projects always in older tool versions
      // too.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::h_PrepareForFile(const QString &orc_Path) {
  int32_t s32_Retval = C_NO_ERR;
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigFiler::h_SaveIoDomain(const C_OscHalcConfigDomain &orc_IoDomain,
                                     C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval;

  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("domain") == "domain");
  // Config
  s32_Retval = mh_SaveIoChannel(orc_IoDomain.c_DomainConfig, orc_XmlParser,
                                "config", "domain");
  // Channels
  if (s32_Retval == C_NO_ERR) {
    s32_Retval =
        mh_SaveIoChannels(orc_IoDomain.c_ChannelConfigs, orc_XmlParser);
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigFiler::h_LoadIoDomain(C_OscHalcConfigDomain &orc_IoDomain,
                                     C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = orc_XmlParser.SelectNodeChildError("config");

  if (s32_Retval == C_NO_ERR) {
    s32_Retval =
        mh_LoadIoChannel(orc_IoDomain.c_DomainConfig, orc_XmlParser, "config");
    if (s32_Retval == C_NO_ERR) {
      // Return
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      s32_Retval =
          orc_XmlParser.GetAttributeUint32Error("length", u32_ExpectedCount);
      if (s32_Retval == C_NO_ERR) {
        uint32_t u32_ActualCount = 0UL;
        QString c_NodeChannel = orc_XmlParser.SelectNodeChild("channel");
        ...
        if (u32_ExpectedCount != u32_ActualCount) {
          const QString c_Tmp =
              QString("Unexpected channel count, expected: %1, got %2")
                  .arg(u32_ExpectedCount)
                  .arg(u32_ActualCount);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_SaveIoDataBase(
    const C_OscHalcDefBase &orc_IoData, C_OscXmlParserBase &orc_XmlParser,
    const QString &orc_BasePath, QStringList *const opc_CreatedFiles) {
  int32_t s32_Retval;

  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("io-base-file") ==
           "io-base-file");
  if (orc_BasePath.isEmpty()) {
    // To string
    s32_Retval = C_OscHalcDefFiler::h_SaveData(orc_IoData, orc_XmlParser);
    // Remember file name
    orc_XmlParser.SetAttributeString("original_file_name",
                                     orc_IoData.c_OriginalFileName);
    // Remember file content
    orc_XmlParser.SetAttributeString("original_file_content",
                                     orc_IoData.c_FileString);
  } else {
    const QString c_IoDataFileName =
        (orc_IoData.c_OriginalFileName.isEmpty() == false)
            ? orc_IoData.c_OriginalFileName
            : "halc_definition_file.xml";
    const QString c_CombinedFileName =
        C_OscSystemFilerUtil::h_CombinePaths(orc_BasePath, c_IoDataFileName);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigFiler::mh_SaveIoDomains(const C_OscHalcConfig &orc_IoData,
                                       C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  for (uint32_t u32_ItDomain = 0UL;
    ...
    const C_OscHalcConfigDomain *const pc_Domain =
        orc_IoData.GetDomainConfigDataConst(u32_ItDomain);
    if (pc_Domain != NULL) {
      s32_Retval =
          C_OscHalcConfigFiler::h_SaveIoDomain(*pc_Domain, orc_XmlParser);
      ...
      s32_Retval = C_CONFIG;
      osc_write_log_error("Saving IO data", "Could not find domain " +
                                                QString::number(u32_ItDomain) +
                                                ".");
    }
  }
  if (s32_Retval == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_SaveIoChannels(
    const QList<C_OscHalcConfigChannel> &orc_Channels,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("channels") == "channels");
  orc_XmlParser.SetAttributeUint32("length",
                                   static_cast<uint32_t>(orc_Channels.size()));
  for (uint32_t u32_ItChannel = 0UL;
       (u32_ItChannel < orc_Channels.size()) && (s32_Retval == C_NO_ERR);
       ++u32_ItChannel) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_SaveIoChannel(
    const C_OscHalcConfigChannel &orc_Channel,
    C_OscXmlParserBase &orc_XmlParser, const QString &orc_NodeName,
    const QString &orc_NodeParentName) {
  int32_t s32_Retval;

  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(orc_NodeName) ==
           orc_NodeName);
  orc_XmlParser.SetAttributeBool("safety-relevant",
                                 orc_Channel.q_SafetyRelevant);
  orc_XmlParser.SetAttributeUint32("use-case-index",
                                   orc_Channel.u32_UseCaseIndex);
  orc_XmlParser.CreateNodeChild("name", orc_Channel.c_Name);
  orc_XmlParser.CreateNodeChild("comment", orc_Channel.c_Comment);
  s32_Retval = mh_SaveIoParameterStructs(orc_Channel.c_Parameters,
                                         orc_XmlParser, orc_NodeName);
  if (s32_Retval == C_NO_ERR) {
    // Return
...
int32_t C_OscHalcConfigFiler::mh_SaveIoParameterStructs(
    const QList<C_OscHalcConfigParameterStruct> &orc_ParameterStructs,
    C_OscXmlParserBase &orc_XmlParser, const QString &orc_NodeName) {
  int32_t s32_Retval = C_NO_ERR;

  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("parameter-structs") ==
           "parameter-structs");
  orc_XmlParser.SetAttributeUint32(
      "length", static_cast<uint32_t>(orc_ParameterStructs.size()));
  for (uint32_t u32_ItParameter = 0UL;
       (u32_ItParameter < static_cast<uint32_t>(orc_ParameterStructs.size())) &&
       (s32_Retval == C_NO_ERR);
       ++u32_ItParameter) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("parameter-struct") ==
             "parameter-struct");
    s32_Retval = mh_SaveIoParameterStruct(orc_ParameterStructs[u32_ItParameter],
                                          orc_XmlParser);
    if (s32_Retval == C_NO_ERR) {
      // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_SaveIoParameterStruct(
    const C_OscHalcConfigParameterStruct &orc_ParameterStruct,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval;

  if (orc_ParameterStruct.c_ParameterElements.size() > 0UL) {
    s32_Retval = mh_SaveIoParameters(orc_ParameterStruct.c_ParameterElements,
                                     orc_XmlParser);
  } else {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("single-value") ==
             "single-value");
    s32_Retval =
        mh_SaveIoParameter(orc_ParameterStruct, orc_XmlParser, "single-value");
    if (s32_Retval == C_NO_ERR) {
      // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_SaveIoParameters(
    const QList<C_OscHalcConfigParameter> &orc_Parameters,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("parameters") ==
           "parameters");
  orc_XmlParser.SetAttributeUint32(
      "length", static_cast<uint32_t>(orc_Parameters.size()));
  for (uint32_t u32_ItParameter = 0UL;
       (u32_ItParameter < orc_Parameters.size()) && (s32_Retval == C_NO_ERR);
       ++u32_ItParameter) {
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild("parameter") ==
             "parameter");
    s32_Retval = mh_SaveIoParameter(orc_Parameters[u32_ItParameter],
                                    orc_XmlParser, "parameter");
    if (s32_Retval == C_NO_ERR) {
      // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_SaveIoParameter(
    const C_OscHalcConfigParameter &orc_Parameter,
    C_OscXmlParserBase &orc_XmlParser, const QString &orc_BaseNode) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  case C_OscHalcDefContent::eCT_PLAIN:
    orc_XmlParser.SetAttributeString("type",
                                     C_OscHalcDefStructFiler::h_GetTypeString(
                                         orc_Parameter.c_Value.GetType()));
    break;
  ...
  case C_OscHalcDefContent::eCT_STRING: {
    ...
    orc_XmlParser.SetAttributeUint32(
        "strlen", orc_Parameter.c_Value.GetArraySize() - 1UL);
    q_ValueHandled = true;
  } break;
  default:
    break;
  ...
  orc_XmlParser.SetAttributeUint32("array-size",
                                   orc_Parameter.c_Value.GetArraySize());
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    for (QList<QPair<QString, C_OscNodeDataPoolContent>>::const_iterator
      ...
      s32_Retval = C_OscHalcDefStructFiler::h_SaveSimpleValueAsAttribute(
          "value", orc_XmlParser, c_It->second);
      if (s32_Retval == C_NO_ERR) {
        // Return
  ...
  if (s32_Retval == C_NO_ERR) {
    const QList<C_OscHalcDefContentBitmaskItem> &rc_BitmaskItems =
        orc_Parameter.c_Value.GetBitmaskItems();
    for (uint32_t u32_ItBitMask = 0UL; u32_ItBitMask < rc_BitmaskItems.size();
         ++u32_ItBitMask) {
      const C_OscHalcDefContentBitmaskItem &rc_BitmaskItem =
          rc_BitmaskItems[u32_ItBitMask];
      ...
      orc_XmlParser.SetAttributeBool("initial-apply-value-setting",
                                     rc_BitmaskItem.q_ApplyValueSetting);
      const QString c_Mask = "0x" + QString::number(rc_BitmaskItem.u64_Value, 16).toUpper();
      orc_XmlParser.SetAttributeString("value", c_Mask);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigFiler::mh_LoadIoDataBase(C_OscHalcDefBase &orc_IoData,
                                        C_OscXmlParserBase &orc_XmlParser,
                                        const QString &orc_BasePath) {
  ...
  if (s32_Retval == C_NO_ERR) {
    if (orc_BasePath.isEmpty()) {
      ...
      // Retrieve file name
      Q_ASSERT(orc_XmlParser.GetAttributeStringError(
                   "original_file_name", orc_IoData.c_OriginalFileName) ==
               C_NO_ERR);
      // Remember file content
      ...
      s32_Retval = C_OscHalcDefFiler::h_LoadFile(
          orc_IoData, C_OscSystemFilerUtil::h_CombinePaths(
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigFiler::mh_LoadIoDomains(C_OscHalcConfig &orc_IoData,
                                       C_OscXmlParserBase &orc_XmlParser) {
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    if (s32_Retval == C_NO_ERR) {
      uint32_t u32_ActualCount = 0UL;
      QString c_NodeDomain = orc_XmlParser.SelectNodeChild("domain");
      if (c_NodeDomain == "domain") {
        do {
          const C_OscHalcConfigDomain *const pc_Domain =
              orc_IoData.GetDomainConfigDataConst(u32_ActualCount);
          if (pc_Domain != NULL) {
            C_OscHalcConfigDomain c_DomainCopy = *pc_Domain;
            s32_Retval = h_LoadIoDomain(c_DomainCopy, orc_XmlParser);
            if (s32_Retval == C_NO_ERR) {
              Q_ASSERT(orc_IoData.SetDomainConfig(u32_ActualCount,
                                                  c_DomainCopy) == C_NO_ERR);
              // Count
              ++u32_ActualCount;
              // Iterate
            ...
            orc_XmlParser.ReportErrorForNodeContentAppendXmlContext(
                "Domain count higher than in io spec file");
            s32_Retval = C_CONFIG;
          }
      ...
      if (u32_ExpectedCount != u32_ActualCount) {
        const QString c_Tmp =
            QString("Unexpected domain count, expected: %1, got %2")
                .arg(u32_ExpectedCount)
                .arg(u32_ActualCount);
        orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext("length",
                                                                     c_Tmp);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigFiler::mh_LoadIoChannel(C_OscHalcConfigChannel &orc_IoChannel,
                                       C_OscXmlParserBase &orc_XmlParser,
                                       const QString &orc_NodeName) {
  int32_t s32_Retval = orc_XmlParser.GetAttributeBoolError(
      "safety-relevant", orc_IoChannel.q_SafetyRelevant);

  if (s32_Retval == C_NO_ERR) {
    s32_Retval = orc_XmlParser.GetAttributeUint32Error(
        "use-case-index", orc_IoChannel.u32_UseCaseIndex);
  }
  if (s32_Retval == C_NO_ERR) {
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = mh_LoadIoParameterStructs(orc_IoChannel.c_Parameters,
                                           orc_XmlParser, orc_NodeName);
  }
  return s32_Retval;
}
...
int32_t C_OscHalcConfigFiler::mh_LoadIoParameterStructs(
    QList<C_OscHalcConfigParameterStruct> &orc_ParameterStructs,
    C_OscXmlParserBase &orc_XmlParser, const QString &orc_NodeName) {
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    s32_Retval =
        orc_XmlParser.GetAttributeUint32Error("length", u32_ExpectedLength);
    if (s32_Retval == C_NO_ERR) {
      QString c_CurrentParameterNode =
          orc_XmlParser.SelectNodeChild("parameter-struct");
      // Reserve
      ...
      if (c_CurrentParameterNode == "parameter-struct") {
        do {
          ...
          if (s32_Retval == C_NO_ERR) {
            ...
            // Iterate
            c_CurrentParameterNode =
                orc_XmlParser.SelectNodeNext("parameter-struct");
          }
        } while ((c_CurrentParameterNode == "parameter-struct") &&
                 (s32_Retval == C_NO_ERR));
        if (s32_Retval == C_NO_ERR) {
          // Return
          Q_ASSERT(orc_XmlParser.SelectNodeParent() == "parameter-structs");
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if (u32_ExpectedLength != orc_ParameterStructs.size()) {
          orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
              "length",
              "Content of attribute (" + QString::number(u32_ExpectedLength) +
                  ") mismatches with the number of found parameters (" +
                  QString::number(orc_ParameterStructs.size()) + ").");
          s32_Retval = C_CONFIG;
        }
      }
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_LoadIoParameterStruct(
    C_OscHalcConfigParameterStruct &orc_ParameterStruct,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = mh_LoadIoParameters(
      orc_ParameterStruct.c_ParameterElements, orc_XmlParser);

  if (s32_Retval == C_NO_ERR) {
    if (orc_XmlParser.SelectNodeChild("single-value") == "single-value") {
      s32_Retval = mh_LoadIoParameter(orc_ParameterStruct, orc_XmlParser,
                                      "single-value");
      if (s32_Retval == C_NO_ERR) {
        // Return
        Q_ASSERT(orc_XmlParser.SelectNodeParent() == "parameter-struct");
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_LoadIoParameters(
    QList<C_OscHalcConfigParameter> &orc_Parameters,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("parameters") == "parameters") {
    ...
    if (s32_Retval == C_NO_ERR) {
      QString c_CurrentParameterNode =
          orc_XmlParser.SelectNodeChild("parameter");
      // Reserve
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if (u32_ExpectedLength != orc_Parameters.size()) {
          orc_XmlParser.ReportErrorForAttributeContentAppendXmlContext(
              ...
              "Content of attribute (" + QString::number(u32_ExpectedLength) +
                  ") mismatches with the number of found parameters (" +
                  QString::number(orc_Parameters.size()) + ").");
          s32_Retval = C_CONFIG;
        }
      }
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigFiler::mh_LoadIoParameter(
    C_OscHalcConfigParameter &orc_Parameter, C_OscXmlParserBase &orc_XmlParser,
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval =
        C_OscHalcDefStructFiler::h_SetType(orc_XmlParser, orc_Parameter.c_Value,
                                           c_TypeStr, c_BaseTypeStr, "value");
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = C_OscHalcDefStructFiler::h_ParseAttributeIntoContent(
          orc_Parameter.c_Value, orc_XmlParser, "value", c_TypeStr,
          c_BaseTypeStr, true);
    }
    if (s32_Retval == C_NO_ERR) {
      // Return
