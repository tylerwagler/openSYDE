 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscHalcConfigDomain::C_OscHalcConfigDomain(void) : C_OscHalcDefDomain() {}
...
C_OscHalcConfigDomain::C_OscHalcConfigDomain(const C_OscHalcDefDomain &orc_Base)
  ...
  this->c_ChannelValues.c_OutputValues =
      orc_Base.c_ChannelValues.c_OutputValues;
  ...
  this->c_ChannelValues.c_StatusValues =
      orc_Base.c_ChannelValues.c_StatusValues;
  this->e_Category = orc_Base.e_Category;

  // Initialize from base (after definition copy-over!):

  // Channels
  for (uint32_t u32_ItChannel = 0UL; u32_ItChannel < orc_Base.c_Channels.size();
       ++u32_ItChannel) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfigDomain::CheckChannelNameUnique(
    const uint32_t ou32_ChannelIndex, bool *const opq_NameConflict) const {
  if ((opq_NameConflict != NULL) &&
      (ou32_ChannelIndex < this->c_ChannelConfigs.size())) {
    const C_OscHalcConfigChannel &rc_CheckedChannel =
        this->c_ChannelConfigs[ou32_ChannelIndex];

    *opq_NameConflict = false;

    // check if any other channel has the same name
    for (uint32_t u32_ItCompChannels = 0;
         u32_ItCompChannels < this->c_ChannelConfigs.size();
         ++u32_ItCompChannels) {
      if (u32_ItCompChannels !=
          ou32_ChannelIndex) // skip current channel to avoid comparison "with
                             // itself"
      {
        const C_OscHalcConfigChannel &rc_ComparedChannel =
            this->c_ChannelConfigs[u32_ItCompChannels];
        if (rc_CheckedChannel.c_Name.toLower() ==
            rc_ComparedChannel.c_Name.toLower()) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigDomain::CheckChannelLinked(
  ...
  if (oq_UseChannelIndex == true) {
    if (ou32_ChannelIndex < this->c_ChannelConfigs.size()) {
      ...
      if (this->c_ChannelUseCases.size() > 0UL) {
        const uint32_t u32_UseCaseIndex = (opu32_UseCaseIndex == NULL)
                                              ? rc_Config.u32_UseCaseIndex
                                              : *opu32_UseCaseIndex;
        if (u32_UseCaseIndex < this->c_ChannelUseCases.size()) {
          bool q_Found = false;
          const C_OscHalcDefChannelUseCase &rc_UseCase =
              this->c_ChannelUseCases[u32_UseCaseIndex];
          for (uint32_t u32_ItAv = 0UL;
               u32_ItAv < rc_UseCase.c_Availability.size(); ++u32_ItAv) {
            const C_OscHalcDefChannelAvailability &rc_Avail =
                rc_UseCase.c_Availability[u32_ItAv];
            if (rc_Avail.u32_ValueIndex == ou32_ChannelIndex) {
              ...
              if (rc_Avail.c_DependentValues.size() > 0UL) {
                ...
                if (opc_LinkedChannelNames != NULL) {
                  ...
                  for (uint32_t u32_ItDe = 0UL;
                       u32_ItDe < rc_Avail.c_DependentValues.size();
                       ++u32_ItDe) {
                    if (rc_Avail.c_DependentValues[u32_ItDe] <
                        this->c_ChannelConfigs.size()) {
                      const C_OscHalcConfigChannel &rc_LinkedChannelConfig =
                          this->c_ChannelConfigs
                              [rc_Avail.c_DependentValues[u32_ItDe]];
                      opc_LinkedChannelNames->push_back(
                          rc_LinkedChannelConfig.c_Name);
                    } else {
                      s32_Retval = C_RANGE;
                    }
                  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigDomain::ResetChannelToDefault(const uint32_t ou32_ChannelIndex) {
  int32_t s32_Return = C_NO_ERR;

  if ((ou32_ChannelIndex < this->c_ChannelConfigs.size()) &&
      (ou32_ChannelIndex < this->c_Channels.size())) {
    this->c_ChannelConfigs[ou32_ChannelIndex] =
        this->m_InitChannelConfig(ou32_ChannelIndex);
  } else {
    s32_Return = C_RANGE;
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfigDomain::ResetChannelUseCase(const uint32_t ou32_ChannelIndex) {
  int32_t s32_Return = C_NO_ERR;

  if ((ou32_ChannelIndex < this->c_ChannelConfigs.size()) &&
      (ou32_ChannelIndex < this->c_Channels.size())) {
    C_OscHalcConfigChannel &rc_Config =
        this->c_ChannelConfigs[ou32_ChannelIndex];
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfigDomain::GetRelevantIndicesForSelectedUseCase(
    const uint32_t ou32_ChannelIndex, const bool oq_UseChannelIndex,
  ...
  if (oq_UseChannelIndex) {
    if (ou32_ChannelIndex < this->c_ChannelConfigs.size()) {
      ...
      if (opc_ParameterIndices != NULL) {
        for (uint32_t u32_It = 0UL;
             u32_It < this->c_ChannelValues.c_Parameters.size(); ++u32_It) {
          const C_OscHalcDefStruct &rc_Struct =
              this->c_ChannelValues.c_Parameters[u32_It];
          for (uint32_t u32_ItAvail = 0UL;
               u32_ItAvail < rc_Struct.c_UseCaseAvailabilities.size();
               ++u32_ItAvail) {
            if (rc_Struct.c_UseCaseAvailabilities[u32_ItAvail] ==
      ...
      if (opc_InputIndices != NULL) {
        for (uint32_t u32_It = 0UL;
             u32_It < this->c_ChannelValues.c_InputValues.size(); ++u32_It) {
          const C_OscHalcDefStruct &rc_Struct =
              this->c_ChannelValues.c_InputValues[u32_It];
          for (uint32_t u32_ItAvail = 0UL;
               u32_ItAvail < rc_Struct.c_UseCaseAvailabilities.size();
               ++u32_ItAvail) {
            if (rc_Struct.c_UseCaseAvailabilities[u32_ItAvail] ==
      ...
      if (opc_OutputIndices != NULL) {
        for (uint32_t u32_It = 0UL;
             u32_It < this->c_ChannelValues.c_OutputValues.size(); ++u32_It) {
          const C_OscHalcDefStruct &rc_Struct =
              this->c_ChannelValues.c_OutputValues[u32_It];
          for (uint32_t u32_ItAvail = 0UL;
               u32_ItAvail < rc_Struct.c_UseCaseAvailabilities.size();
               ++u32_ItAvail) {
            if (rc_Struct.c_UseCaseAvailabilities[u32_ItAvail] ==
      ...
      if (opc_StatusIndices != NULL) {
        for (uint32_t u32_It = 0UL;
             u32_It < this->c_ChannelValues.c_StatusValues.size(); ++u32_It) {
          const C_OscHalcDefStruct &rc_Struct =
              this->c_ChannelValues.c_StatusValues[u32_It];
          for (uint32_t u32_ItAvail = 0UL;
               u32_ItAvail < rc_Struct.c_UseCaseAvailabilities.size();
               ++u32_ItAvail) {
            if (rc_Struct.c_UseCaseAvailabilities[u32_ItAvail] ==
    ...
    if (opc_ParameterIndices != NULL) {
      ...
      for (uint32_t u32_It = 0UL;
           u32_It < this->c_DomainValues.c_Parameters.size(); ++u32_It) {
        opc_ParameterIndices->push_back(u32_It);
      }
    }
    if (opc_InputIndices != NULL) {
      ...
      for (uint32_t u32_It = 0UL;
           u32_It < this->c_DomainValues.c_InputValues.size(); ++u32_It) {
        opc_InputIndices->push_back(u32_It);
      }
    }
    if (opc_OutputIndices != NULL) {
      ...
      for (uint32_t u32_It = 0UL;
           u32_It < this->c_DomainValues.c_OutputValues.size(); ++u32_It) {
        opc_OutputIndices->push_back(u32_It);
      }
    }
    if (opc_StatusIndices != NULL) {
      ...
      for (uint32_t u32_It = 0UL;
           u32_It < this->c_DomainValues.c_StatusValues.size(); ++u32_It) {
        opc_StatusIndices->push_back(u32_It);
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfigDomain::HandleFileLoadPostProcessing(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfigDomain::HandleNameMaxCharLimit(
    const uint32_t ou32_NameMaxCharLimit,
    QList<C_OscSystemNameMaxCharLimitChangeReportItem>
        *const opc_ChangedItems) {
  for (uint32_t u32_It = 0UL; u32_It < this->c_ChannelConfigs.size();
       ++u32_It) {
    this->c_ChannelConfigs[u32_It].HandleNameMaxCharLimit(
        ou32_NameMaxCharLimit, "hal-channel-name", opc_ChangedItems);
  }
  this->c_DomainConfig.HandleNameMaxCharLimit(
      ou32_NameMaxCharLimit, "hal-domain-name", opc_ChangedItems);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfigDomain::mh_AddParameters(
    const QList<C_OscHalcDefStruct> &orc_Parameters,
    QList<C_OscHalcConfigParameterStruct> &orc_ParamConfig) {
  for (uint32_t u32_ItParam = 0UL; u32_ItParam < orc_Parameters.size();
       ++u32_ItParam) {
    ...
    for (uint32_t u32_ItElem = 0UL;
         u32_ItElem < rc_Struct.c_StructElements.size(); ++u32_ItElem) {
      const C_OscHalcDefElement &rc_Elem =
          rc_Struct.c_StructElements[u32_ItElem];
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscHalcConfigChannel
C_OscHalcConfigDomain::mh_InitConfigFromName(const QString &orc_Name) {
...
/*! \brief  Initialize channel configuration
   ...
   \param[in]  ou32_ChannelIndex    Channel index (undefined behavior if out of
   range)
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscHalcConfigChannel C_OscHalcConfigDomain::m_InitChannelConfig(
    const uint32_t ou32_ChannelIndex) const {
  C_OscHalcConfigChannel c_NewChannel;

  if (ou32_ChannelIndex < this->c_Channels.size()) {
    // Default Name
    c_NewChannel = C_OscHalcConfigDomain::mh_InitConfigFromName(
        this->c_SingularName + "_" + QString::number(ou32_ChannelIndex + 1));

    // Default use case
    c_NewChannel.u32_UseCaseIndex =
        this->m_InitChannelUseCase(ou32_ChannelIndex);

    // Add parameters
    C_OscHalcConfigDomain::mh_AddParameters(this->c_ChannelValues.c_Parameters,
                                            c_NewChannel.c_Parameters);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscHalcConfigDomain::m_InitChannelUseCase(
    const uint32_t ou32_ChannelIndex) const {
  uint32_t u32_UseCaseIndex = 0;

  for (uint32_t u32_ItUseCase = 0UL;
       u32_ItUseCase < this->c_ChannelUseCases.size(); ++u32_ItUseCase) {
    const C_OscHalcDefChannelUseCase &rc_UseCase =
        this->c_ChannelUseCases[u32_ItUseCase];
    for (uint32_t u32_ItDefault = 0UL;
         u32_ItDefault < rc_UseCase.c_DefaultChannels.size(); ++u32_ItDefault) {
...
void C_OscHalcConfigDomain::m_InitDomainConfig(void) {
  // General
  this->c_DomainConfig =
      C_OscHalcConfigDomain::mh_InitConfigFromName(this->c_Name);

  // Add parameters
  C_OscHalcConfigDomain::mh_AddParameters(this->c_DomainValues.c_Parameters,
                                          this->c_DomainConfig.c_Parameters);
}
