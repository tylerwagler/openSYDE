 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscHalcConfig::C_OscHalcConfig(void) : C_OscHalcDefBase() {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscHalcConfigDomain *
C_OscHalcConfig::GetDomainConfigDataConst(const uint32_t ou32_Index) const {
...
C_OscHalcConfig::GetDomainDefDataConst(const uint32_t ou32_Index) const {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Checks if the HALC configuration is clear or if it has a loaded
   definition/configuration

   \retval   true    Definition is cleared. No definition is loaded.
   \retval   false   The definition is not cleared. A concrete definition is
   loaded.
*/
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfig::SetDomainConfig(const uint32_t ou32_Index,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelConfig(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
  ...
  if (ou32_DomainIndex < this->mc_Domains.size()) {
    ...
    if (oq_UseChannelIndex) {
      if (ou32_ChannelIndex < rc_Domain.c_ChannelConfigs.size()) {
        C_OscHalcConfigChannel &rc_Channel =
            rc_Domain.c_ChannelConfigs[ou32_ChannelIndex];
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfig::ResetDomainChannelConfig(const uint32_t ou32_DomainIndex,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscHalcConfig::ResetDomainChannelUseCase(const uint32_t ou32_DomainIndex,
                                           const uint32_t ou32_ChannelIndex,
                                           const bool oq_UseChannelIndex) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelConfigName(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    const bool oq_UseChannelIndex, const QString &orc_Name) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelConfigComment(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelConfigSafety(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelConfigUseCase(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelParameterConfig(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelParameterConfigElement(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    ...
    if (oq_UseChannelIndex) {
      if (ou32_ChannelIndex < rc_Domain.c_ChannelConfigs.size()) {
        C_OscHalcConfigChannel &rc_Channel =
            rc_Domain.c_ChannelConfigs[ou32_ChannelIndex];
        if (ou32_ParameterIndex < rc_Channel.c_Parameters.size()) {
          C_OscHalcConfigParameterStruct &rc_Struct =
              rc_Channel.c_Parameters[ou32_ParameterIndex];
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelParameterConfigElementPlain(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    const uint32_t ou32_ParameterIndex, const uint32_t ou32_ElementIndex,
    const bool oq_UseChannelIndex, const C_OscHalcDefContent &orc_Value) {
  int32_t s32_Retval = C_NO_ERR;
      ...
      if (ou32_ChannelIndex < rc_Domain.c_ChannelConfigs.size()) {
        ...
        if (ou32_ParameterIndex < rc_Channel.c_Parameters.size()) {
          ...
          if (rc_Struct.c_ParameterElements.size() > 0UL) {
            if (ou32_ElementIndex < rc_Struct.c_ParameterElements.size()) {
              C_OscHalcConfigParameter &rc_Element =
                  rc_Struct.c_ParameterElements[ou32_ElementIndex];
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelParameterConfigElementEnum(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    const uint32_t ou32_ParameterIndex, const uint32_t ou32_ElementIndex,
    const bool oq_UseChannelIndex, const QString &orc_DisplayName) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelParameterConfigElementBitmask(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    const uint32_t ou32_ParameterIndex, const uint32_t ou32_ElementIndex,
    const bool oq_UseChannelIndex, const QString &orc_DisplayName,
    const bool oq_Value) {
          ...
          if (rc_Struct.c_ParameterElements.size() > 0UL) {
            if (ou32_ElementIndex < rc_Struct.c_ParameterElements.size()) {
              C_OscHalcConfigParameter &rc_Element =
                  rc_Struct.c_ParameterElements[ou32_ElementIndex];
              s32_Retval =
                  rc_Element.c_Value.SetBitmask(orc_DisplayName, oq_Value);
            } else {
              s32_Retval = C_RANGE;
            }
          } else {
            s32_Retval =
                rc_Struct.c_Value.SetBitmask(orc_DisplayName, oq_Value);
          }
        } else {
          s32_Retval = C_RANGE;
        }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::SetDomainChannelParameterConfigElementString(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    const uint32_t ou32_ParameterIndex, const uint32_t ou32_ElementIndex,
    const bool oq_UseChannelIndex, const QString &orc_Value) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcConfig::GetRelevantIndicesForSelectedUseCase(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ChannelIndex,
    const bool oq_UseChannelIndex, QList<uint32_t> *const opc_ParameterIndices,
  ...
  if (ou32_DomainIndex < this->mc_Domains.size()) {
    ...
    rc_Domain.GetRelevantIndicesForSelectedUseCase(
        ou32_ChannelIndex, oq_UseChannelIndex, opc_ParameterIndices,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfig::CheckConfigValid(
  ...
  if (opq_DomainsInvalid != NULL) {
    *opq_DomainsInvalid = false;
    ...
    for (uint32_t u32_ItDomain = 0; u32_ItDomain < this->mc_Domains.size();
         u32_ItDomain++) {
      bool q_DomainInvalid;
      bool q_ChannelsInvalid;

      this->CheckDomainConfigValid(u32_ItDomain, &q_DomainInvalid,
                                   &q_ChannelsInvalid, NULL);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfig::CheckDomainConfigValid(
    const uint32_t ou32_DomainIndex, bool *const opq_DomainInvalid,
    bool *const opq_ChannelsInvalid,
    QList<uint32_t> *const opc_InvalidChannelIndices) const {
  if (ou32_DomainIndex < this->mc_Domains.size()) {
    const C_OscHalcConfigDomain &rc_CheckedDomain =
        this->mc_Domains[ou32_DomainIndex];
    ...
    if (opq_ChannelsInvalid != NULL) {
      *opq_ChannelsInvalid = false;
      ...
      for (uint32_t u32_ItChannel = 0;
           u32_ItChannel < rc_CheckedDomain.c_ChannelConfigs.size();
           u32_ItChannel++) {
        bool q_NameConflict;
        bool q_NameInvalid;

        const C_OscHalcConfigChannel &rc_CheckedChannel =
            rc_CheckedDomain.c_ChannelConfigs[u32_ItChannel];
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcConfig::HandleNameMaxCharLimit(
    const uint32_t ou32_NameMaxCharLimit,
    QList<C_OscSystemNameMaxCharLimitChangeReportItem>
        *const opc_ChangedItems) {
  for (uint32_t u32_ItDomain = 0UL; u32_ItDomain < this->mc_Domains.size();
       ++u32_ItDomain) {
    this->mc_Domains[u32_ItDomain].HandleNameMaxCharLimit(ou32_NameMaxCharLimit,
                                                          opc_ChangedItems);
  }
}
