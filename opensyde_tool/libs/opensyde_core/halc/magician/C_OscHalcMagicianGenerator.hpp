...
private:
  ...
  static int32_t
  mh_FillHalcElement(C_OscNodeDataPoolListElement *const opc_Element,
                     const C_OscHalcDefContent &orc_Value,
                     const uint32_t ou32_ChannelIndex);
  static int32_t mh_GenerateVariablesForDomain(
  ...
  static int32_t mh_GenerateVariablesForVector(
      ...
      C_OscNodeDataPoolList &orc_List);
  static int32_t mh_GenerateVariablesForVectorElement(
      const QList<C_OscHalcDefStruct> &orc_Definition,
      const uint32_t ou32_DefinitionElementIndex,
      const QString &orc_DomainSingularName, const bool oq_IsSafe,
      const QList<uint32_t> &orc_RelevantChannels, const bool oq_AddDataset,
      C_OscNodeDataPoolList &orc_List);
  static int32_t mh_AddVariableToList(
      ...
      C_OscNodeDataPoolList &orc_List);
  static int32_t
  mh_ConvertToDatapoolWithoutArray(const C_OscHalcDefContent &orc_HalcContent,
                                   C_OscNodeDataPoolContent &orc_DpContent);
  static int32_t
  mh_ConvertToDatapoolAndAssign(const C_OscHalcDefContent &orc_HalcContent,
                                C_OscNodeDataPoolContent &orc_DpContent,
                                const uint32_t ou32_Index);
  static int32_t
  mh_ConvertToDatapoolAndResize(const C_OscHalcDefContent &orc_HalcContent,
                                const uint32_t ou32_NumChannels,
                                C_OscNodeDataPoolContent &orc_DpContent);
  static void mh_InitList(C_OscNodeDataPoolList &orc_List,
  ...
  void
  m_FillEmptySpaceHalcDatapools(QList<C_OscNodeDataPool> &orc_Datapools) const;
