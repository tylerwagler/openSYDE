...
public:
  C_OscHalcMagicianDatapoolListHandler(
  ...
  const C_OscNodeDataPoolListElement *GetListElementConst(
      ...
      uint32_t *const opu32_ArrayIndex = NULL) const;
  C_OscNodeDataPoolListElement *
  GetUseCaseListElement(const uint32_t ou32_DomainIndex,
                        C_OscNodeDataPoolList &orc_List) const;
  const C_OscNodeDataPoolListElement *
  GetUseCaseListElementConst(const uint32_t ou32_DomainIndex,
                             const C_OscNodeDataPoolList &orc_List) const;
  C_OscNodeDataPoolListElement *
  GetChanNumListElement(const uint32_t ou32_DomainIndex,
                        C_OscNodeDataPoolList &orc_List) const;
  const C_OscNodeDataPoolListElement *
  GetChanNumListElementConst(const uint32_t ou32_DomainIndex,
                             const C_OscNodeDataPoolList &orc_List) const;
  C_OscNodeDataPoolListElement *
  GetSafetyFlagListElement(const uint32_t ou32_DomainIndex,
                           C_OscNodeDataPoolList &orc_List) const;
  const C_OscNodeDataPoolListElement *
  GetSafetyFlagListElementConst(const uint32_t ou32_DomainIndex,
                                const C_OscNodeDataPoolList &orc_List) const;

  uint32_t
  CountRelevantItems(const QList<C_OscHalcConfigChannel> &orc_Channels,
                     const C_OscHalcConfigChannel &orc_DomainConfig) const;
  QList<uint32_t>
  GetRelevantChannels(const QList<C_OscHalcConfigChannel> &orc_Channels,
                      const C_OscHalcConfigChannel &orc_DomainConfig) const;
  ...
  static uint32_t h_CountElements(const C_OscHalcDefStruct &orc_Struct,
                                  const uint32_t ou32_NumChannelsInDatapool);
  static uint32_t h_CountElements(const C_OscHalcDefElement &orc_Element,
                                  const uint32_t ou32_NumChannelsInDatapool);
  ...
  bool CheckChanNumVariableNecessary(
      const C_OscHalcConfigDomain &orc_ChannelConfig) const;
  bool CheckSafetyFlagVariableNecessary(void) const;
  bool
  CheckUseCaseVariableNecessary(const C_OscHalcConfigDomain &orc_Domain) const;
...
private:
  ...
  int32_t m_GetListIndex(const uint32_t ou32_DomainIndex,
                         const uint32_t ou32_ParameterStructIndex,
                         const uint32_t ou32_ParameterStructElementIndex,
  ...
  static int32_t mh_GetSubElementIndex(
  ...
  static void mh_GetSubDefElementIndex(
      const C_OscHalcDefElement &orc_DefElement, const uint32_t ou32_CurChannel,
      const QList<uint32_t> &orc_RelevantChannels, uint32_t &oru32_ListIndex);
  int32_t
  m_GetArrayIndexOfChannel(const uint32_t ou32_DomainIndex,
  ...
  // Avoid call
  C_OscHalcMagicianDatapoolListHandler(
      const C_OscHalcMagicianDatapoolListHandler &);
  C_OscHalcMagicianDatapoolListHandler &
  operator=(const C_OscHalcMagicianDatapoolListHandler &);
};
