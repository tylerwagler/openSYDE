...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscHalcMagicianDatapoolListHandler::C_OscHalcMagicianDatapoolListHandler(
    const C_OscHalcConfig &orc_HalcConfig,
    const C_OscHalcDefDomain::E_VariableSelector oe_Selector,
    const bool oq_IsSafe)
    : mrc_HalcConfig(orc_HalcConfig), me_Selector(oe_Selector),
      mq_IsSafe(oq_IsSafe) {}
...
/*! \brief  Get list element
   ...
   \param[in]      ou32_ParameterStructElementIndex   Parameter struct element
   index \param[in,out]  orc_List                           List \param[in]
   ou32_CurChannel                    Current channel \param[out] opq_IsArray
   Optional: Flag if Datapool Element is an array \param[out] opu32_ArrayIndex
   Optional: If opq_IsArray is true, the array index of the returned list
   element which is assigned to the channel
...
C_OscHalcMagicianDatapoolListHandler::GetListElement(
    ...
    bool *const opq_IsArray, uint32_t *const opu32_ArrayIndex) const {
  C_OscNodeDataPoolListElement *pc_Retval = NULL;
  uint32_t u32_Index;
  const int32_t s32_Result =
      this->m_GetListIndex(ou32_DomainIndex, ou32_ParameterStructIndex,
                           ou32_ParameterStructElementIndex, u32_Index, false,
                           false, false, ou32_CurChannel);

  if ((s32_Result == C_NO_ERR) && (u32_Index < orc_List.c_Elements.size())) {
    pc_Retval = &orc_List.c_Elements[u32_Index];
  }

  if ((s32_Result == C_NO_ERR) && (pc_Retval != NULL) &&
      (opq_IsArray != NULL) && (opu32_ArrayIndex != NULL)) {
    m_GetArrayIndexOfChannel(ou32_DomainIndex, *pc_Retval, ou32_CurChannel,
                             *opq_IsArray, *opu32_ArrayIndex);
  }
...
C_OscHalcMagicianDatapoolListHandler::GetListElementConst(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ParameterStructIndex,
    const uint32_t ou32_ParameterStructElementIndex,
    const C_OscNodeDataPoolList &orc_List, const uint32_t ou32_CurChannel,
    bool *const opq_IsArray, uint32_t *const opu32_ArrayIndex) const {
  ...
  if ((s32_Result == C_NO_ERR) && (u32_Index < orc_List.c_Elements.size())) {
    pc_Retval = &orc_List.c_Elements[static_cast<
        QList<C_OscNodeDataPoolListElement>::size_type>(u32_Index)];
  }
...
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolListElement *
C_OscHalcMagicianDatapoolListHandler::GetUseCaseListElement(
    const uint32_t ou32_DomainIndex, C_OscNodeDataPoolList &orc_List) const {
  C_OscNodeDataPoolListElement *pc_Retval = NULL;
  uint32_t u32_Index;
  const int32_t s32_Result = this->m_GetListIndex(
      ou32_DomainIndex, 0UL, 0UL, u32_Index, true, false, false, 0UL);
...
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPoolListElement *
C_OscHalcMagicianDatapoolListHandler::GetUseCaseListElementConst(
...
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolListElement *
C_OscHalcMagicianDatapoolListHandler::GetChanNumListElement(
    const uint32_t ou32_DomainIndex, C_OscNodeDataPoolList &orc_List) const {
  C_OscNodeDataPoolListElement *pc_Retval = NULL;
  uint32_t u32_Index;
  const int32_t s32_Result = this->m_GetListIndex(
      ou32_DomainIndex, 0UL, 0UL, u32_Index, false, true, false, 0UL);
...
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPoolListElement *
C_OscHalcMagicianDatapoolListHandler::GetChanNumListElementConst(
    const uint32_t ou32_DomainIndex,
    const C_OscNodeDataPoolList &orc_List) const {
  const C_OscNodeDataPoolListElement *pc_Retval = NULL;
  uint32_t u32_Index;
...
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolListElement *
C_OscHalcMagicianDatapoolListHandler::GetSafetyFlagListElement(
    const uint32_t ou32_DomainIndex, C_OscNodeDataPoolList &orc_List) const {
  C_OscNodeDataPoolListElement *pc_Retval = NULL;
  uint32_t u32_Index;
  const int32_t s32_Result = this->m_GetListIndex(
      ou32_DomainIndex, 0UL, 0UL, u32_Index, false, false, true, 0UL);
...
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPoolListElement *
C_OscHalcMagicianDatapoolListHandler::GetSafetyFlagListElementConst(
    const uint32_t ou32_DomainIndex,
    const C_OscNodeDataPoolList &orc_List) const {
  const C_OscNodeDataPoolListElement *pc_Retval = NULL;
  uint32_t u32_Index;
...
uint32_t C_OscHalcMagicianDatapoolListHandler::CountRelevantItems(
  ...
  if (orc_Channels.size() == 0UL) {
    if (this->mrc_HalcConfig.e_SafetyMode ==
    ...
    default:
      for (uint32_t u32_ItChannel = 0UL; u32_ItChannel < orc_Channels.size();
           ++u32_ItChannel) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscHalcMagicianDatapoolListHandler::h_CountElements(
    const QList<C_OscHalcDefStruct> &orc_Structs,
    const uint32_t ou32_NumChannelsInDatapool) {
  uint32_t u32_Retval = 0UL;

  for (uint32_t u32_ItStruct = 0UL; u32_ItStruct < orc_Structs.size();
       ++u32_ItStruct) {
    u32_Retval += C_OscHalcMagicianDatapoolListHandler::h_CountElements(
        orc_Structs[u32_ItStruct], ou32_NumChannelsInDatapool);
  }
...
uint32_t C_OscHalcMagicianDatapoolListHandler::h_CountElements(
  ...
  if (orc_Struct.c_StructElements.size() > 0UL) {
    for (uint32_t u32_It = 0UL;
         u32_It < static_cast<uint32_t>(orc_Struct.c_StructElements.size());
         ++u32_It) {
      u32_Retval += h_CountElements(orc_Struct.c_StructElements[u32_It],
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscHalcMagicianDatapoolListHandler::CheckChanPresent(
    const C_OscHalcConfigChannel &orc_ChannelConfig) const {
  return this->CheckChanRelevant(orc_ChannelConfig) ||
         this->m_CheckIgnoreFlag();
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscHalcMagicianDatapoolListHandler::CheckChanRelevant(
    const C_OscHalcConfigChannel &orc_ChannelConfig) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscHalcMagicianDatapoolListHandler::CheckChanNumVariableNecessary(
    const C_OscHalcConfigDomain &orc_ChannelConfig) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscHalcMagicianDatapoolListHandler::CheckSafetyFlagVariableNecessary(
    void) const {
  return (this->me_Selector == C_OscHalcDefDomain::eVA_PARAM) &&
         (this->mrc_HalcConfig.e_SafetyMode ==
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscHalcMagicianDatapoolListHandler::CheckUseCaseVariableNecessary(
    const C_OscHalcConfigDomain &orc_Domain) const {
  return (this->me_Selector == C_OscHalcDefDomain::eVA_PARAM) &&
         (orc_Domain.c_ChannelUseCases.size() > 0UL);
}
...
/*! \brief  Get list index
   ...
   \param[in]   ou32_ParameterStructIndex          Parameter struct index
   \param[in]   ou32_ParameterStructElementIndex   Parameter struct element
   index \param[out]  oru32_ListIndex                    List index \param[in]
   oq_GetUseCaseIndex                 Get use case index \param[in]
   oq_GetChanNumIndex                 Get channel number index \param[in]
   oq_GetSafetyFlagIndex              Get safety flag index \param[in]
   ou32_CurChannel                    Current channel
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcMagicianDatapoolListHandler::m_GetListIndex(
    const uint32_t ou32_DomainIndex, const uint32_t ou32_ParameterStructIndex,
  ...
  if (ou32_DomainIndex < this->mrc_HalcConfig.GetDomainSize()) {
    ...
    for (uint32_t u32_ItDomain = 0UL;
         (u32_ItDomain < ou32_DomainIndex) && (s32_Retval == C_NO_ERR);
         ++u32_ItDomain) {
      const C_OscHalcDefDomain *const pc_DomainDef =
          this->mrc_HalcConfig.GetDomainDefDataConst(u32_ItDomain);
      const C_OscHalcConfigDomain *const pc_DomainConfig =
          this->mrc_HalcConfig.GetDomainConfigDataConst(u32_ItDomain);
    ...
    if (((oq_GetUseCaseIndex == false) && (oq_GetChanNumIndex == false)) &&
      ...
      const C_OscHalcConfigDomain *const pc_CurrentDomainConfig =
          this->mrc_HalcConfig.GetDomainConfigDataConst(ou32_DomainIndex);
      // Current domain
      if ((pc_CurrentDomainDef != NULL) && (pc_CurrentDomainConfig != NULL)) {
        const QList<uint32_t> c_RelevantChannels =
            this->GetRelevantChannels(pc_CurrentDomainConfig->c_ChannelConfigs,
                                      pc_CurrentDomainConfig->c_DomainConfig);
        if (c_RelevantChannels.size() > 0UL) {
          // channels
          if (C_OscHalcMagicianDatapoolListHandler::
                  CheckChanNumVariableNecessary(*pc_CurrentDomainConfig)) {
            oru32_ListIndex += 1UL;
          }
          ...
          case C_OscHalcDefDomain::eVA_PARAM:
            if (C_OscHalcMagicianDatapoolListHandler::mh_GetSubElementIndex(
                    ou32_ParameterStructIndex, ou32_ParameterStructElementIndex,
                    ou32_CurChannel,
                    pc_CurrentDomainDef->c_DomainValues.c_Parameters,
                    c_RelevantChannels, oru32_ListIndex) == C_RANGE) {
              s32_Retval =
                  C_OscHalcMagicianDatapoolListHandler::mh_GetSubElementIndex(
    ...
    } else if (((oq_GetUseCaseIndex == true) &&
                (oq_GetChanNumIndex == false)) &&
               (oq_GetSafetyFlagIndex == false)) {
      const C_OscHalcConfigDomain *const pc_DomainConfig =
          this->mrc_HalcConfig.GetDomainConfigDataConst(ou32_DomainIndex);
      if ((pc_DomainConfig != NULL) &&
          (C_OscHalcMagicianDatapoolListHandler::CheckChanNumVariableNecessary(
              *pc_DomainConfig))) {
        // channels
        oru32_ListIndex += 1UL;
      }
    ...
    } else if (((oq_GetUseCaseIndex == false) &&
                (oq_GetChanNumIndex == false)) &&
               (oq_GetSafetyFlagIndex == true)) {
      const C_OscHalcConfigDomain *const pc_DomainConfig =
          this->mrc_HalcConfig.GetDomainConfigDataConst(ou32_DomainIndex);
      if ((pc_DomainConfig != NULL) &&
          (C_OscHalcMagicianDatapoolListHandler::CheckChanNumVariableNecessary(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcMagicianDatapoolListHandler::mh_GetSubElementIndex(
    const uint32_t ou32_Index, const uint32_t ou32_ElementIndex,
    const uint32_t ou32_CurChannel, const QList<C_OscHalcDefStruct> &orc_Values,
    const QList<uint32_t> &orc_RelevantChannels, uint32_t &oru32_ListIndex) {
  int32_t s32_Retval = C_NO_ERR;

  if (ou32_Index < orc_Values.size()) {
    // Passed structs
    for (uint32_t u32_ItStruct = 0UL; u32_ItStruct < ou32_Index;
         ++u32_ItStruct) {
    ...
    for (uint32_t u32_ItEl = 0UL; u32_ItEl < ou32_ElementIndex; ++u32_ItEl) {
      ...
      if (u32_ItEl < orc_Values[ou32_Index].c_StructElements.size()) {
        const C_OscHalcDefElement &rc_CurEl =
            orc_Values[ou32_Index].c_StructElements[u32_ItEl];
        oru32_ListIndex +=
            C_OscHalcMagicianDatapoolListHandler::h_CountElements(
                rc_CurEl, static_cast<uint32_t>(orc_RelevantChannels.size()));
      }
    }
    // Current element
    if (orc_Values[ou32_Index].c_StructElements.size() > 0UL) {
      Q_ASSERT(ou32_ElementIndex <
               orc_Values[ou32_Index].c_StructElements.size());
      if (ou32_ElementIndex < orc_Values[ou32_Index].c_StructElements.size()) {
        C_OscHalcMagicianDatapoolListHandler::mh_GetSubDefElementIndex(
            orc_Values[ou32_Index].c_StructElements[ou32_ElementIndex],
            ou32_CurChannel, orc_RelevantChannels, oru32_ListIndex);
      }
    } else {
      C_OscHalcMagicianDatapoolListHandler::mh_GetSubDefElementIndex(
          orc_Values[ou32_Index], ou32_CurChannel, orc_RelevantChannels,
          oru32_ListIndex);
    }
  } else {
    s32_Retval = C_RANGE;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscHalcMagicianDatapoolListHandler::mh_GetSubDefElementIndex(
    const C_OscHalcDefElement &orc_DefElement, const uint32_t ou32_CurChannel,
    const QList<uint32_t> &orc_RelevantChannels, uint32_t &oru32_ListIndex) {
  if (orc_DefElement.GetComplexType() == C_OscHalcDefContent::eCT_STRING) {
    for (uint32_t u32_ItCh = 0UL; u32_ItCh < orc_RelevantChannels.size();
         ++u32_ItCh) {
...
/*! \brief  Get list element
   ...
   \param[in]      ou32_CurChannel                    Current channel
   \param[out]     orq_IsArray                        Flag if Datapool Element
   is an array \param[out]     oru32_ArrayIndex                   If opq_IsArray
   is true, the array index of the returned list element which is assigned to
   the channel
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHalcMagicianDatapoolListHandler::m_GetArrayIndexOfChannel(
    const uint32_t ou32_DomainIndex,
  ...
  if (ou32_DomainIndex < this->mrc_HalcConfig.GetDomainSize()) {
    ...
    if (orq_IsArray == false) {
      ...
      case C_OscHalcDefBase::eONE_LEVEL_ALL_NON_SAFE:
      case C_OscHalcDefBase::eONE_LEVEL_ALL_SAFE:
        // All channels are assigned to safe or non safe. Due to an existing
        // pc_Retval No differentiation necessary. It is a 1:1 mapping from
        // channel to array index
        oru32_ArrayIndex = ou32_CurChannel;
        break;
      case C_OscHalcDefBase::eTWO_LEVELS_WITH_DROPPING:
      default:
        const C_OscHalcConfigDomain *const pc_DomainCfg =
            this->mrc_HalcConfig.GetDomainConfigDataConst(ou32_DomainIndex);
        oru32_ArrayIndex = 0U;
        if (pc_DomainCfg != NULL) {
          // Counting all channel with the same safety flag configuration
          for (uint32_t u32_ItChannel = 0UL;
               u32_ItChannel < pc_DomainCfg->c_ChannelConfigs.size();
               ++u32_ItChannel) {
            const C_OscHalcConfigChannel &rc_Channel =
                pc_DomainCfg->c_ChannelConfigs[u32_ItChannel];
            if (rc_Channel.q_SafetyRelevant == this->mq_IsSafe) {
...
bool C_OscHalcMagicianDatapoolListHandler::m_CheckIgnoreFlag(void) const {
  return ((this->mrc_HalcConfig.e_SafetyMode ==
           C_OscHalcDefBase::eTWO_LEVELS_WITHOUT_DROPPING) ||
          (this->mrc_HalcConfig.e_SafetyMode ==
