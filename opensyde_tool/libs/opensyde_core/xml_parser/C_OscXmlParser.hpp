...
private:
  C_OscXmlParserBase(const C_OscXmlParserBase
                         &orc_Source); ///< not implemented: prevent copying
  C_OscXmlParserBase &
  operator=(const C_OscXmlParserBase
...
public:
  ...
  void CreateNodeChild(const QString &orc_Name,
                       const QString &orc_Content = "");
  // append new child node under the current node and select it; returns new
  // child node name
  ...
  int32_t GetAttributeSint32(const QString &orc_Name,
                             const int32_t os32_Default = 0L) const;
  uint32_t GetAttributeUint32(const QString &orc_Name,
                              const uint32_t ou32_Default = 0UL) const;
  int64_t GetAttributeSint64(const QString &orc_Name,
                             const int64_t os64_Default = 0LL) const;
  uint64_t GetAttributeUint64(const QString &orc_Name,
                              const uint64_t ou64_Default = 0ULL) const;
  bool GetAttributeBool(const QString &orc_Name,
                        const bool oq_Default = false) const;
  float32_t GetAttributeFloat32(const QString &orc_Name,
                                const float32_t of32_Default = 0.0F) const;
  float64_t GetAttributeFloat64(const QString &orc_Name,
                                const float64_t of64_Default = 0.0) const;
  ...
  virtual int32_t GetAttributeSint32Error(const QString &orc_Name,
                                          int32_t &ors32_Value) const;
  virtual int32_t GetAttributeUint32Error(const QString &orc_Name,
                                          uint32_t &oru32_Value) const;
  virtual int32_t GetAttributeSint64Error(const QString &orc_Name,
                                          int64_t &ors64_Value) const;
  virtual int32_t GetAttributeUint64Error(const QString &orc_Name,
                                          uint64_t &oru64_Value) const;
  virtual int32_t GetAttributeBoolError(const QString &orc_Name,
                                        bool &orq_Value) const;
  virtual int32_t GetAttributeFloat32Error(const QString &orc_Name,
                                           float32_t &orf32_Value) const;
  virtual int32_t GetAttributeFloat64Error(const QString &orc_Name,
                                           float64_t &orf64_Value) const;

  // Base reporting functions
  virtual void ReportErrorForNodeContentAppendXmlContext(
      const QString &orc_ErrorMessage) const;
  virtual void ReportErrorForAttributeContentAppendXmlContext(
      const QString &orc_Attribute, const QString &orc_ErrorMessage) const;
  virtual void ReportErrorForNodeContentStartingWithXmlContext(
      const QString &orc_ErrorMessage) const;
  virtual void ReportErrorForAttributeContentStartingWithXmlContext(
      const QString &orc_Attribute, const QString &orc_ErrorMessage) const;
  virtual void
  ReportErrorForNodeMissing(const QString &orc_MissingNodeName) const;
...
/// file based parsing
class C_OscXmlParser : public C_OscXmlParserBase {
private:
  // not implemented -> prevent copying
  C_OscXmlParser(const C_OscXmlParser &orc_Source);
  // not implemented -> prevent assignment
  C_OscXmlParser &
  operator=(const C_OscXmlParser
                &orc_Source); // lint !e1511 //we want to hide the base function
