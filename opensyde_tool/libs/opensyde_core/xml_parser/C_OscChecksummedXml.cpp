 ...
 * ---------------------------------------------------------------------------------------
 */
static const QString mc_NAME_CRC_ATTRIBUTE = "file_crc";
...
C_OscChecksummedXml::C_OscChecksummedXml(void)
    : C_OscXmlParser(), mu16_CrcDepth(0U) {}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Open XML data from file

   Open XML file.
   If the file could not be opened the function will return an error and prepare
   an empty XML structure. After the data was opened the function will check the
   CRC

   The checksum value is expected in the attribute "c_NAME_CRC_ATTRIBUTE" of the
   root name.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscChecksummedXml::LoadFromFile(const QString &orc_FileName) {
  int32_t s32_Return;

  s32_Return = C_OscXmlParser::LoadFromFile(orc_FileName);
  if (s32_Return == C_NO_ERR) {
    const QString c_Text = this->SelectRoot();
    if (c_Text == "") {
      s32_Return = C_RD_WR;
    } else {
      if (this->AttributeExists(mc_NAME_CRC_ATTRIBUTE) == false) {
        ...
        const uint16_t u16_CrcFromFile = static_cast<uint16_t>(
            this->GetAttributeUint32(mc_NAME_CRC_ATTRIBUTE));
...
/*! \brief  Write XML data to file
   ...
   After calculation the checksum value is written to the attribute
   "c_NAME_CRC_ATTRIBUTE" of the root name. A pre-existing file will be
   replaced.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscChecksummedXml::SaveToFile(const QString &orc_FileName) {
  int32_t s32_Return;
  const uint16_t u16_CrcCalc = this->m_CalcXmlCrc();
  ...
  if (this->SelectRoot() == "") {
    ...
    this->SetAttributeString(
        mc_NAME_CRC_ATTRIBUTE,
        "0x" + QString::number(u16_CrcCalc, 16).rightJustified(4, '0'));
...
void C_OscChecksummedXml::m_CalcXmlCrcNode(uint16_t &oru16_Crc) {
  ...
  while (c_NodeLv1 != "") {
    ...
    for (uint32_t u32_Index = 0U; u32_Index < c_Attributes.size();
         u32_Index++) {
      C_SclChecksums::CalcCRC16(
          c_Attributes[u32_Index].c_Name.toLocal8Bit().constData(),
          c_Attributes[u32_Index].c_Name.length(), oru16_Crc);
      C_SclChecksums::CalcCRC16(
          c_Attributes[u32_Index].c_Value.toLocal8Bit().constData(),
          c_Attributes[u32_Index].c_Value.length(), oru16_Crc);
    }
    c_Text = this->GetNodeContent();
    C_SclChecksums::CalcCRC16(c_Text.toLocal8Bit().constData(), c_Text.length(),
                              oru16_Crc);
...
uint16_t C_OscChecksummedXml::m_CalcXmlCrc(void) {
  ...
  for (uint32_t u32_Index = 0U; u32_Index < c_Attributes.size(); u32_Index++) {
    if (c_Attributes[u32_Index].c_Name !=
        mc_NAME_CRC_ATTRIBUTE) // skip CRC value
    {
      C_SclChecksums::CalcCRC16(
          c_Attributes[u32_Index].c_Name.toLocal8Bit().constData(),
