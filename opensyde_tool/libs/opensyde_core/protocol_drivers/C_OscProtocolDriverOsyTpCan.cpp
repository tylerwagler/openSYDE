...
C_OscProtocolDriverOsyTpCan::C_ServiceState::C_ServiceState(void)
    : u16_TransmissionIndex(0U), u8_SequenceNumber(0U), e_Status(eIDLE),
...
/*! \brief   constructor
   ...
   \param[in]  ou16_MaxServiceQueueSize   maximum number of entries in Tx/Rx
   queue
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscProtocolDriverOsyTpCan::C_OscProtocolDriverOsyTpCan(
    const uint16_t ou16_MaxServiceQueueSize)
    : C_OscProtocolDriverOsyTpBase(ou16_MaxServiceQueueSize),
      mpc_CanDispatcher(NULL), mu16_DispatcherClientHandle(0U) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_HandleIncomingSingleFrame(
  ...
  if (orc_CanMessage.u8_DLC >= (c_Service.c_Data.size() + 1)) {
    ...
    if (c_Service.c_Data.size() > 0) {
      (void)std::memcpy(&c_Service.c_Data[0], &orc_CanMessage.au8_Data[1],
                        c_Service.c_Data.size());
    }
    // add to queue:
    s32_Return = m_AddToRxQueue(c_Service);
    if (s32_Return != C_NO_ERR) {
      m_LogWarningWithHeader("Rx Queue overflow. Incoming single frame dumped.",
                             TGL_UTIL_FUNC_ID);
      s32_Return = C_OVERFLOW;
    }
  } else {
    // malformed service -> ignore
    m_LogWarningWithHeader("Single frame with invalid DLC received. Ignoring.",
                           TGL_UTIL_FUNC_ID);
    s32_Return = C_CONFIG;
  }
  return s32_Return;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_HandleIncomingOsySpecificSingleFrame(
    const T_STWCAN_Msg_RX &orc_CanMessage) {
  C_OscProtocolDriverOsyService c_Service;
  int32_t s32_Return = C_NO_ERR;
  ...
  if ((orc_CanMessage.au8_Data[0] & 0x0FU) ==
      mhu8_OSY_OSF_TYPE_EVENT_DRIVEN_DP_SINGLE_FRAME) {
    ...
    if ((u8_Size > 0) && (u8_Size <= 8U)) {
      (void)std::memcpy(&c_Service.c_Data[1], &orc_CanMessage.au8_Data[1],
                        static_cast<size_t>(u8_Size) - 1);
    }
    ...
    m_LogWarningWithHeader(
        "Unexpected openSYDE single frame message type received. Ignoring.",
        TGL_UTIL_FUNC_ID);
  }
...
/*! \brief   Handle incoming first frame
   ...
   This is unspecified behavior: ongoing segmented Rx transfers must not be
   interrupted through other segmented Rx transfers as it is not possible to
   match consecutive frames to the transfer.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_HandleIncomingFirstFrame(
    const T_STWCAN_Msg_RX &orc_CanMessage) {
  int32_t s32_Return = C_NO_ERR;

  if (mc_RxService.e_Status != C_ServiceState::eIDLE) {
    m_LogWarningWithHeader(
        "Unexpected first frame received. Incomplete transfer dumped.",
        TGL_UTIL_FUNC_ID);
  ...
  if (orc_CanMessage.u8_DLC == 8U) {
    ...
    // get total size:
    const uint8_t u8_Size = orc_CanMessage.au8_Data[0] & 0x0FU;
    const uint16_t u16_ServiceSize =
        static_cast<uint16_t>(static_cast<uint16_t>(u8_Size) << 8U) +
        orc_CanMessage.au8_Data[1];
    mc_RxService.c_ServiceData.c_Data.resize(u16_ServiceSize);
    (void)std::memcpy(&mc_RxService.c_ServiceData.c_Data[0],
                      &orc_CanMessage.au8_Data[2], 6U);
    ...
    s32_Return = mpc_CanDispatcher->CAN_Send_Msg(
        c_TxMsg); // lint !e613  //caller is responsible for valid dispatcher
    if (s32_Return != C_NO_ERR) {
      m_LogWarningWithHeader("Could not send flow control CAN message.",
                             TGL_UTIL_FUNC_ID);
      s32_Return = C_COM;
    } else {
    ...
    // malformed service -> ignore
    m_LogWarningWithHeader("First frame with invalid DLC received. Ignoring.",
                           TGL_UTIL_FUNC_ID);
    s32_Return = C_CONFIG;
  }
  return s32_Return;
...
/*! \brief   Send next consecutive frames
   ...
   Only to be called when the state machine is in state
   eMORE_CONSECUTIVE_FRAMES_TO_SEND

   \return
   C_NO_ERR     no problems: all pending CFs sent; Tx state machine set to eIDLE
   again C_COM        could not send out consecutive frame (one/some might have
   been sent, however) Tx state machine still at
   eMORE_CONSECUTIVE_FRAMES_TO_SEND
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_SendNextConsecutiveFrames(void) {
  ...
  for (; mc_TxService.u16_TransmissionIndex <
         mc_TxService.c_ServiceData.c_Data.size();
       mc_TxService.u16_TransmissionIndex += 7) {
    T_STWCAN_Msg_TX c_TxMsg;
    const uint8_t u8_NumBytesToSend =
        static_cast<uint8_t>(((mc_TxService.c_ServiceData.c_Data.size() -
                               mc_TxService.u16_TransmissionIndex) > 7U)
                                 ? 7U
                                 : (mc_TxService.c_ServiceData.c_Data.size() -
    ...
    c_TxMsg.au8_Data[0] =
        mhu8_ISO15765_N_PCI_CF + mc_TxService.u8_SequenceNumber;
    // lint -e{670} //std::vector reference returned by [] is guaranteed to have
    ...
    if (s32_Return != C_NO_ERR) {
      // most likely Tx buffer is full; but we cannot be 100% sure, so write a
      // log entry
      m_LogWarningWithHeader(
          "Could not send consecutive frame CAN message (Tx buffer full ?).",
          TGL_UTIL_FUNC_ID);
...
/*! \brief   Handle incoming flow control
   ...
   Continue sending ongoing Tx transfer consecutive frames when waiting for flow
   control. Invalid and unexpected frames will be ignored.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_HandleIncomingFlowControl(
    const T_STWCAN_Msg_RX &orc_CanMessage) {
  int32_t s32_Return = C_NO_ERR;
  ...
  if (mc_TxService.e_Status == C_ServiceState::eWAITING_FOR_FLOW_CONTROL) {
    ...
    if (orc_CanMessage.u8_DLC == 3U) {
      if ((orc_CanMessage.au8_Data[1] == 0U) &&
          (orc_CanMessage.au8_Data[2] == 0U)) {
        // set total timeout value for sending all CFs (kicks in if the Tx
        // buffer of the CAN dispatcher is full) N_As is ISO 15765-2 is per CAN
        // frame (and quite high at 1000ms ...) For the total transfer we use
        // the total number of CAN frames, assume the lowest supported bitrate
        //  in openSYDE (100 kbit/s) and an alien busload of 50%
        // So we'll have around 2 ms/message
        // But we'll add a lower limit of 100ms to compensate for client side
        // timing constraints
        mc_TxService.u32_SendCfTimeoutDuration = static_cast<uint32_t>(
            (mc_TxService.c_ServiceData.c_Data.size() / 7U) * 2U);
        ...
        mc_TxService.e_Status =
            C_ServiceState::eMORE_CONSECUTIVE_FRAMES_TO_SEND;

        s32_Return = m_SendNextConsecutiveFrames();
      } else {
        m_LogWarningWithHeader(
            "Flow control with unsupported STmin or BS received. Ignoring.",
            TGL_UTIL_FUNC_ID);
        s32_Return = C_OVERFLOW;
      }
    } else {
      m_LogWarningWithHeader(
          "Flow control with incorrect DLC received. Ignoring.",
          TGL_UTIL_FUNC_ID);
      s32_Return = C_CONFIG;
    }
  } else {
    // this was unexpected -> ignore
    m_LogWarningWithHeader("Unexpected flow control received. Ignoring.",
                           TGL_UTIL_FUNC_ID);
    s32_Return = C_NOACT;
  }
  return s32_Return;
...
/*! \brief   Handle incoming consecutive frame
   ...
   After full service data has been received add full service to Rx service
   queue and the Rx state machine will be reset.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_HandleIncomingConsecutiveFrame(
    const T_STWCAN_Msg_RX &orc_CanMessage) {
  int32_t s32_Return = C_NO_ERR;

  if (mc_RxService.e_Status == C_ServiceState::eWAITING_FOR_CONSECUTIVE_FRAME) {
    if (mc_RxService.u8_SequenceNumber ==
        (orc_CanMessage.au8_Data[0] & 0x0FU)) {
      // correct sequence; copy data.
      const uint8_t u8_NumBytes = (orc_CanMessage.u8_DLC - 1);
      if (u8_NumBytes >= (mc_RxService.c_ServiceData.c_Data.size() -
                          mc_RxService.u16_TransmissionIndex)) {
        // enough data received to fill our buffer:
        (void)std::memcpy(&mc_RxService.c_ServiceData
                               .c_Data[mc_RxService.u16_TransmissionIndex],
                          &orc_CanMessage.au8_Data[1],
                          mc_RxService.c_ServiceData.c_Data.size() -
                              mc_RxService.u16_TransmissionIndex);
        // add to Rx queue:
        s32_Return = m_AddToRxQueue(mc_RxService.c_ServiceData);
        if (s32_Return != C_NO_ERR) {
          m_LogWarningWithHeader(
              "Rx Queue overflow. Incoming consecutive frame dumped.",
              TGL_UTIL_FUNC_ID);
          s32_Return = C_OVERFLOW;
        }
        mc_RxService.e_Status = C_ServiceState::eIDLE;
        ...
        // no need to check whether we need to send a flow control: we always
        // use STmin=0 and BS=0
      } else {
        // not all data available but DLC < 8; this is messed up: abort
        mc_RxService.e_Status = C_ServiceState::eIDLE;
        m_LogWarningWithHeader("Consecutive frame with incorrect DLC received.",
                               TGL_UTIL_FUNC_ID);
        s32_Return = C_CONFIG;
      }
    } else {
    ...
    // this was unexpected -> ignore
    m_LogWarningWithHeader("Unexpected consecutive frame received. Ignoring.",
                           TGL_UTIL_FUNC_ID);
    s32_Return = C_NOACT;
  }
  return s32_Return;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_HandleIncomingOsySpecificMultiFrame(
    const T_STWCAN_Msg_RX &orc_CanMessage) {
  int32_t s32_Return = C_NO_ERR;

  if (mc_RxService.e_Status != C_ServiceState::eIDLE) {
    m_LogWarningWithHeader(
        "OMF: Unexpected first frame received. Incomplete transfer dumped.",
        TGL_UTIL_FUNC_ID);
    mc_RxService.e_Status = C_ServiceState::eIDLE;
  }
  ...
  if ((orc_CanMessage.au8_Data[0] & 0x0FU) == 0U) {
    if (orc_CanMessage.u8_DLC == 8U) {
      ...
      mc_RxService.c_ServiceData.c_Data.resize(u16_ServiceSize);
      (void)memcpy(&mc_RxService.c_ServiceData.c_Data[0],
                   &orc_CanMessage.au8_Data[2], 6U);
      mc_RxService.u16_TransmissionIndex = 6U;
      mc_RxService.u8_SequenceNumber = 1U;
    } else {
      // malformed service -> ignore
      m_LogWarningWithHeader(
          "OMF: First frame with invalid DLC received. Ignoring.",
          TGL_UTIL_FUNC_ID);
      s32_Return = C_CONFIG;
    }
  }
  // consecutive frame
    ...
    if (mc_RxService.u8_SequenceNumber ==
      ...
      if (u8_NumBytes >= (mc_RxService.c_ServiceData.c_Data.size() -
                          mc_RxService.u16_TransmissionIndex)) {
        // enough data received to fill our buffer:
        (void)memcpy(&mc_RxService.c_ServiceData
                          .c_Data[mc_RxService.u16_TransmissionIndex],
                     &orc_CanMessage.au8_Data[1],
                     mc_RxService.c_ServiceData.c_Data.size() -
                         mc_RxService.u16_TransmissionIndex);
        ...
        if (s32_Return != C_NO_ERR) {
          m_LogWarningWithHeader(
              "OMF: Rx Queue overflow. Incoming consecutive frame dumped.",
              TGL_UTIL_FUNC_ID);
          s32_Return = C_OVERFLOW;
        }
        mc_RxService.e_Status = C_ServiceState::eIDLE;
        ...
        mc_RxService.u8_SequenceNumber = 0U;
        m_LogWarningWithHeader(
            "OMF: Consecutive frame with incorrect DLC received.",
            TGL_UTIL_FUNC_ID);
        s32_Return = C_CONFIG;
      }
    } else {
      ...
      mc_RxService.u8_SequenceNumber = 0U;
      m_LogWarningWithHeader(
          "OMF: Consecutive frame with incorrect sequence number received.",
          TGL_UTIL_FUNC_ID);
      s32_Return = C_RANGE;
    }
  }
...
/*! \brief   Send broadcast service EnterDiagnosticSession
   ...
   Purpose: hit into the time window when the devices are started to keep the
   devices in flashloader.

   The suppressPosRspMsgIndicationBit will be set to avoid flooding the bus with
   response messages.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::m_BroadcastSendDiagnosticSessionControl(
    const uint8_t ou8_Session) const {
  int32_t s32_Return;
  ...
  if (mpc_CanDispatcher == NULL) {
    ...
    // We do not want any answer for this broadcast. Use the
    ...
    if (s32_Return != C_NO_ERR) {
      m_LogWarningWithHeader(
          "Could not send single frame broadcast CAN message.",
          TGL_UTIL_FUNC_ID);
      s32_Return = C_COM;
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsyTpCan::mh_ComposeSingleFrame(
    const C_OscProtocolDriverOsyService &orc_Service,
    const uint32_t ou32_Identifier, T_STWCAN_Msg_TX &orc_CanMessage) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Handling the response to the services
   BroadcastSetNodeIdBySerialNumber/-Extended

   Incoming responses to other services will be dumped: we are strictly
   handshaking here ... (why would someone send other broadcast requests in
   parallel ?)

   \param[in]    ou8_RoutineIdMsb     MSB part of the last sent routine
   identifier for matching the correct response
   \param[in]    ou8_RoutineIdLsb     LSB part of the last sent routine
   identifier for matching the correct response
   \param[out]   opu8_NrCode          if not NULL: negative response code (if
   C_WARN is returned)
   ...
   called ?) C_OVERFLOW  multiple positive responses received
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsyTpCan::m_HandleBroadcastSetNodeIdBySerialNumberResponse(
    const uint8_t ou8_RoutineIdMsb, const uint8_t ou8_RoutineIdLsb,
  ...
  // No further abort condition. Wait always the entire timeout time to get all
  // positive and negative responses
  while (c_Timer.hasExpired(mu32_BroadcastTimeoutMs) == false) {
    int32_t s32_ReturnLocal;

    // trigger dispatcher
    // ignore return value: we cannot be sure some other client did not check
    // before us
    (void)mpc_CanDispatcher->DispatchIncoming();

    s32_ReturnLocal = mpc_CanDispatcher->ReadFromQueue(
        mu16_DispatcherClientHandle, c_Response);
    if (s32_ReturnLocal == C_NO_ERR) {
      // format OK ?
      if ((c_Response.u8_DLC == 5U) &&
          (c_Response.au8_Data[0] == (mhu8_ISO15765_N_PCI_SF + 4U)) &&
          (c_Response.au8_Data[1] ==
           (mhu8_OSY_BC_SI_ROUTINE_CONTROL | 0x40U)) &&
          (c_Response.au8_Data[2] ==
           mhu8_OSY_BC_RC_SUB_FUNCTION_START_ROUTINE) &&
      ...
      } else if ((c_Response.u8_DLC == 4U) &&
                 (c_Response.au8_Data[0] == (mhu8_ISO15765_N_PCI_SF + 3U)) &&
        ...
        if (c_Response.au8_Data[3] == 0x22U) {
          m_LogWarningWithHeader("Broadcast: Negative response received. Node "
                                 "has security activated.",
                                 TGL_UTIL_FUNC_ID);
        } else {
          m_LogWarningWithHeader("Broadcast: Negative response received. "
                                 "Probably a node with an other SN.",
                                 TGL_UTIL_FUNC_ID);
        }
        if (opu8_NrCode != NULL) {
        ...
        m_LogWarningWithHeader(
            "Broadcast: unexpected response received. Ignoring.",
            TGL_UTIL_FUNC_ID);
      }
    }
  }
  ...
  if (q_MultiplePositiveResponsesReceived == true) {
    // Multiple positive responses received. At least two nodes with same SN.
    // Error case.
    s32_Return = C_OVERFLOW;
...
int32_t C_OscProtocolDriverOsyTpCan::Cycle(void) {
  ...
  if (mpc_CanDispatcher == NULL) {
    ...
    if (mc_TxService.e_Status == C_ServiceState::eIDLE) {
      ...
      while (s32_Return == C_NO_ERR) {
        ...
        if (s32_Return == C_NO_ERR) {
          if (mc_TxService.c_ServiceData.c_Data.size() <= 7) {
            // simple single frame:
            T_STWCAN_Msg_TX c_Msg;
            mh_ComposeSingleFrame(mc_TxService.c_ServiceData,
                                  m_GetTxIdentifier(), c_Msg);

            s32_Return = mpc_CanDispatcher->CAN_Send_Msg(c_Msg);
            if (s32_Return != C_NO_ERR) {
              m_LogWarningWithHeader("Could not send single frame CAN message.",
                                     TGL_UTIL_FUNC_ID);
              s32_Return = C_COM; // terminate loop
              s32_ReturnFunc = C_COM;
            }
          } else if (mc_TxService.c_ServiceData
                         .q_CanTransferWithoutFlowControl == true) {
            // openSYDE specific multi frame without flow control
            T_STWCAN_Msg_TX c_TxMsg;
            const uint32_t u32_CountBytes =
                static_cast<uint32_t>(mc_TxService.c_ServiceData.c_Data.size());
            ...
            // First frame
            mc_TxService.u8_SequenceNumber = 0U;
            // The number of bytes in the first frame. Start with this byte in
            ...
            (void)std::memcpy(&c_TxMsg.au8_Data[2],
                              &mc_TxService.c_ServiceData.c_Data[0], 6U);
            s32_Return = mpc_CanDispatcher->CAN_Send_Msg(c_TxMsg);

            if (s32_Return != C_NO_ERR) {
              m_LogWarningWithHeader("Could not send first frame CAN message.",
                                     TGL_UTIL_FUNC_ID);
            } else {
              // Following frames
              do {
                uint32_t u32_BytesForNextMessage = static_cast<uint32_t>(
                    u32_CountBytes - mc_TxService.u16_TransmissionIndex);
                ...
                c_TxMsg.au8_Data[0] =
                    mhu8_ISO15765_N_PCI_OMF + mc_TxService.u8_SequenceNumber;
                // std::vector reference returned by [] is guaranteed to have
                // linear data in memory: lint -e{670}
                (void)std::memcpy(
                    &c_TxMsg.au8_Data[1],
                    &mc_TxService.c_ServiceData
                         .c_Data[mc_TxService.u16_TransmissionIndex],
                    u32_BytesForNextMessage);

                c_TxMsg.u8_DLC =
                    static_cast<uint8_t>(u32_BytesForNextMessage + 1U);
                s32_Return = mpc_CanDispatcher->CAN_Send_Msg(c_TxMsg);
                if (s32_Return != C_NO_ERR) {
                  // We do not handle Tx buffer issues with the OMF-CFs (like we
                  // do with regular CFs) The OMF mechanism is only used for
                  // smaller services with low numbers of messages without
                  //  handshakes (typical maximum: STW Flashloader flashing ->
                  //  37 * 3 messages)
                  // Assumption: this number of Tx buffers is available on all
                  // targets
                  m_LogWarningWithHeader(
                      "Could not send consecutive CAN message.",
                      TGL_UTIL_FUNC_ID);
                  break;
                }

                mc_TxService.u16_TransmissionIndex +=
                    static_cast<uint16_t>(u32_BytesForNextMessage);
            ...
            c_TxMsg.au8_Data[0] = static_cast<uint8_t>(
                mhu8_ISO15765_N_PCI_FF +
                ((mc_TxService.c_ServiceData.c_Data.size() >> 8U) & 0x0FU));
            c_TxMsg.au8_Data[1] = static_cast<uint8_t>(
                mc_TxService.c_ServiceData.c_Data.size() & 0xFFU);
            (void)std::memcpy(&c_TxMsg.au8_Data[2],
                              &mc_TxService.c_ServiceData.c_Data[0], 6U);
            mc_TxService.u16_TransmissionIndex = 6U;
    ...
    } else if (mc_TxService.e_Status ==
               C_ServiceState::eWAITING_FOR_FLOW_CONTROL) {
      // check for Tx timeout:
      if (mc_TxService.c_Timer.hasExpired(mhu16_NBS_TIMEOUTS_MS)) {
        // transfer timed out ...
        m_LogWarningWithHeader("N_Bs timeout reached before receiving flow "
                               "control. Aborting ongoing Tx transfer.",
                               TGL_UTIL_FUNC_ID);
        mc_TxService.e_Status = C_ServiceState::eIDLE;
      }
    } else if (mc_TxService.e_Status ==
      ...
      // still not finished ?
      if (mc_TxService.e_Status ==
          C_ServiceState::eMORE_CONSECUTIVE_FRAMES_TO_SEND) {
        if (mc_TxService.c_Timer.hasExpired(
                mc_TxService.u32_SendCfTimeoutDuration)) {
          // transfer timed out ...
          m_LogWarningWithHeader("Could not send all CFs within timeout. "
                                 "Aborting ongoing Tx transfer.",
                                 TGL_UTIL_FUNC_ID);
          mc_TxService.e_Status = C_ServiceState::eIDLE;
        }
      }
    ...
    while (s32_Return == C_NO_ERR) {
      s32_Return =
          mpc_CanDispatcher->ReadFromQueue(mu16_DispatcherClientHandle, c_Msg);
      if ((s32_Return == C_NO_ERR) && (c_Msg.u8_DLC > 0U)) {
        ...
        case mhu8_ISO15765_N_PCI_OMF:
          ...
          break;
        default:
          m_LogWarningWithHeader("Unexpected frame type received. Ignoring.",
                                 TGL_UTIL_FUNC_ID);
          break;
        }
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::SetNodeIdentifiers(
    const C_OscProtocolDriverOsyNode &orc_ClientIdentifier,
    const C_OscProtocolDriverOsyNode &orc_ServerIdentifier) {
  int32_t s32_Return;

  s32_Return = C_OscProtocolDriverOsyTpBase::SetNodeIdentifiers(
      orc_ClientIdentifier, orc_ServerIdentifier);
...
/*! \brief   Configure this class for broadcasting
   ...
   * remember client IDs
   * clear Rx queue
   * recalculate and change receive filter of C_CAN_Dispatcher to be able to
   receive responses from all nodes on our bus
...
int32_t C_OscProtocolDriverOsyTpCan::SetNodeIdentifiersForBroadcasts(
  ...
  c_ServerId.u8_NodeIdentifier =
      C_OscProtocolDriverOsyNode::mhu8_NODE_ID_BROADCASTS;

  s32_Return = C_OscProtocolDriverOsyTpBase::SetNodeIdentifiers(
      orc_ClientIdentifier, c_ServerId);
  if ((s32_Return == C_NO_ERR) && (mpc_CanDispatcher != NULL)) {
    // Clear Rx queue; we are no longer interested in that old stuff:
    mpc_CanDispatcher->ClearQueue(mu16_DispatcherClientHandle);
...
uint32_t C_OscProtocolDriverOsyTpCan::m_GetTxIdentifier(void) const {
  ...
  if (mc_ServerId.u8_BusIdentifier == mc_ClientId.u8_BusIdentifier) {
    ...
    u32_Identifier =
        static_cast<uint32_t>(0x18DA0000U) +
    ...
    u32_Identifier =
        static_cast<uint32_t>(0x1BC00000U) +
...
/*! \brief   Calculate Tx ID to send broadcasts on.
   ...
   Broadcasts only work in the local network, so normal fixed functional
   addressing is used.
...
/*! \brief   Set CAN dispatcher Rx filter
   ...
   Configure CAN dispatcher Rx filter to receive only TP messages that we are
   interested in.
  ...
  if (oq_ForBroadcast == false) {
    ...
    if (mc_ServerId.u8_BusIdentifier == mc_ClientId.u8_BusIdentifier) {
      ...
      c_Filter.u32_Code =
          ...
          mc_ServerId.u8_NodeIdentifier;
      c_Filter.u32_Mask =
          0x1FFFFFFFU; // must be exactly for us and from specific sender
    } else {
      // different bus; routing -> we use 15765-3 section 8.3 IDs
      c_Filter.u32_Code =
          static_cast<uint32_t>(0x1BC00000U) +
    ...
    // normal fixed addressing physical (responses will not be addresses as
    // broadcasts)
    c_Filter.u32_Code =
        static_cast<uint32_t>(0x18DA0000U) +
        (static_cast<uint32_t>(mc_ClientId.u8_NodeIdentifier) << 8U);
    c_Filter.u32_Mask =
        0x1FFFFF80U; // must be exactly for us; but sender may be anyone
  }
  // lint -e{613}  //caller is responsible for valid dispatcher
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::SetDispatcher(
    C_CanDispatcher *const opc_Dispatcher) {
  int32_t s32_Return = C_NO_ERR;
  ...
  if (mpc_CanDispatcher != NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      s32_Return =
          m_SetRxFilter((mc_ServerId.u8_NodeIdentifier ==
                         C_OscProtocolDriverOsyNode::mhu8_NODE_ID_BROADCASTS)
                            ? true
                            : false);
      if (s32_Return != C_NO_ERR) {
...
/*! \brief   Read serial number of all devices on local bus
   ...
   Report back list of nodes that sent a response (with the sent serial number
   and node IDs). The function will wait for responses for the time configured
   with SetBroadcastTimeout() Only positive responses are reported. Negative
  ...
  if (mpc_CanDispatcher == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      while (c_TimerStd.hasExpired(mu32_BroadcastTimeoutMs) == false) {
        // trigger dispatcher
        // ignore return value: we cannot be sure some other client did not
        // check before us
        (void)mpc_CanDispatcher->DispatchIncoming();
        ...
        if (s32_ReturnLocal == C_NO_ERR) {
          ...
          if ((c_Response.u8_DLC == 8U) &&
              ...
              (c_Response.au8_Data[1] ==
               (mhu8_OSY_BC_SI_READ_SERIAL_NUMBER | 0x40U))) {
            // looks legit; extract payload ...
            C_BroadcastReadEcuSerialNumberResults c_Result;
            uint8_t au8_SerialNumber[6];
            (void)std::memcpy(&au8_SerialNumber[0], &c_Response.au8_Data[2],
                              6U);
            c_Result.c_SerialNumber.SetPosSerialNumber(au8_SerialNumber);

            // extract node-id from sender
            // must be local addressing ...
            c_Result.c_SenderId.u8_BusIdentifier =
                mc_ClientId.u8_BusIdentifier; // same as ours ...
            c_Result.c_SenderId.u8_NodeIdentifier =
                static_cast<uint8_t>(c_Response.u32_ID & 0x7FU);
            orc_Responses.push_back(c_Result);
          } else if ((c_Response.u8_DLC == 4U) &&
                     (c_Response.au8_Data[0] ==
                      (mhu8_ISO15765_N_PCI_SF + 3U)) &&
                     (c_Response.au8_Data[1] == mhu8_BC_OSY_NR_SI) &&
                     (c_Response.au8_Data[2] ==
                      mhu8_OSY_BC_SI_READ_SERIAL_NUMBER)) {
            // negative response detected
            m_LogWarningWithHeader("Broadcast: Negative response received. "
                                   "Probably a node with an extended SN.",
                                   TGL_UTIL_FUNC_ID);
          } else {
            m_LogWarningWithHeader(
                "Broadcast: unexpected response received. Ignoring.",
      ...
      do {
        ...
        if (s32_Return != C_NO_ERR) {
          ...
          while (c_TimerExt.hasExpired(mu32_BroadcastTimeoutMs) == false) {
            ...
            if (s32_ReturnLocal == C_NO_ERR) {
              ...
              if ((c_Response.u8_DLC == 8U) &&
                ...
                const uint8_t u8_ReceivedBlockNumber =
                    c_Response.au8_Data[2] & 0x0FU;
                const uint8_t u8_ReceivedSubNodeId =
                    c_Response.au8_Data[2] >> 4U;

                if (u8_ReceivedBlockNumber == c_Service.c_Data[1]) {
                  const uint32_t u32_UniqueId =
                      (static_cast<uint32_t>(c_Response.au8_Data[3]) << 16U) +
                      (static_cast<uint32_t>(c_Response.au8_Data[4]) << 8U) +
                      static_cast<uint32_t>(c_Response.au8_Data[5]);

                  // The serial number payload in the last two bytes will
                  // contain following information separated in the different
                  // block
                  ...
                  if (u8_ReceivedBlockNumber == 0U) {
                    ...
                    // extract node-id from sender
                    // must be local addressing ...
                    c_ResultExt.c_SenderId.u8_BusIdentifier =
                        mc_ClientId.u8_BusIdentifier; // same as ours ...
                    c_ResultExt.c_SenderId.u8_NodeIdentifier =
                        static_cast<uint8_t>(c_Response.u32_ID & 0x7FU);
                    c_ResultExt.u8_SubNodeId = u8_ReceivedSubNodeId;
                    c_ResultExt.c_SerialNumber.u8_SerialNumberByteLength =
                        0U; // Will be filled in the
                            // next
                            // block

                    // Serial number payload of first block
                    // First byte: Bit 0 security flag, all other bits are
                    // reserved
                    c_ResultExt.q_SecurityActivated =
                        ((c_Response.au8_Data[6] & 0x01U) == 0x01U);

                    // Last byte of payload in first block
                    c_ResultExt.c_SerialNumber
                    ...
                    // Add the initial result to the map with its unique id as
                    // key
                    ...
                    if (c_ItResult != c_UniqueIdToResult.end()) {
                      ...
                      if (rc_CurrentResult.u8_SubNodeId ==
                        ...
                        if (u8_ReceivedBlockNumber == 1U) {
                          ...
                          if ((rc_CurrentResult.c_SerialNumber
                                   .u8_SerialNumberByteLength > 0) &&
                              (rc_CurrentResult.c_SerialNumber
                                   .u8_SerialNumberByteLength <= 29)) {
                            // Second byte of payload is first byte of serial
                            // number
                            if (rc_CurrentResult.c_SerialNumber
                                    .q_FsnSerialNumber == true) {
                              // FSN format
                              rc_CurrentResult.c_SerialNumber
                                  .c_SerialNumberExt =
                              ...
                              // POS format
                              rc_CurrentResult.c_SerialNumber
                                  .au8_SerialNumber[0] = c_Response.au8_Data[7];
                            }

                            if (rc_CurrentResult.c_SerialNumber
                                    .u8_SerialNumberByteLength > 1) {
                          ...
                          // The first block has no part of the serial number
                          // and the second block had only the first sign.
                          const uint8_t u8_SnrBlock =
                              u8_ReceivedBlockNumber - 2U;
                          uint32_t u32_NextSnrSignIndex =
                              (static_cast<uint32_t>(u8_SnrBlock) * 2U) + 1U;
                          ...
                          if (u32_NextSnrSignIndex <
                            ...
                            if (rc_CurrentResult.c_SerialNumber
                              ...
                              // POS format
                              rc_CurrentResult.c_SerialNumber
                                  .au8_SerialNumber[u32_NextSnrSignIndex] =
                                  c_Response.au8_Data[6];
                            } else {
                              // Nothing to do
                            }
                          ...
                          // It is possible that the serial number has a length
                          // which does not need the last byte of the last
                          // necessary block
                          if (u32_NextSnrSignIndex <
                              rc_CurrentResult.c_SerialNumber
                                  .u8_SerialNumberByteLength) {
                          ...
                          if (rc_CurrentResult.c_SerialNumber
                            ...
                            if (u32_NextSnrSignIndex < 5U) {
                              // Serial number is not finished yet. Exact 6 are
                              // necessary for POS
                              q_Continue = true;
                            }
                          }
                        ...
                        // Should not happen. Same UniqueId but different
                        // SubNodeId
                        m_LogWarningWithHeader(
                            "Broadcast: Different SubNode Id with same "
                            "UniqueId in response received. Ignoring.",
                            TGL_UTIL_FUNC_ID);
                      }
                    } else {
                      // u32_UniqueId not found. Error on transmitting or
                      // calculation of CRC or the device did not responded on
                      // the first block
                      m_LogWarningWithHeader("Broadcast: unexpected UniqueId "
                                             "in response received. Ignoring.",
                                             TGL_UTIL_FUNC_ID);
                    }
                  }
                } else {
                  // Not requested block number
                  m_LogWarningWithHeader("Broadcast: unexpected block number  "
                                         "in response received. Ignoring.",
                                         TGL_UTIL_FUNC_ID);
                }
              } else if ((c_Response.u8_DLC == 4U) &&
                ...
                // negative response detected
                m_LogWarningWithHeader("Broadcast: Negative response received. "
                                       "Probably a node with a standard SN.",
                                       TGL_UTIL_FUNC_ID);
              } else {
                m_LogWarningWithHeader(
                    "Broadcast: unexpected response received. Ignoring.",
          ...
          if (q_Continue == true) {
            ...
            mh_ComposeSingleFrame(c_Service, m_GetTxBroadcastIdentifier(),
                                  c_Msg);
          }
        }
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        for (c_ItResult = c_UniqueIdToResult.begin();
          ...
          if ((c_ItResult->second.c_SerialNumber.u8_SerialNumberByteLength >
            ...
            m_LogWarningWithHeader(
                "An extended serial number had not the correct length.",
                TGL_UTIL_FUNC_ID);
          }
        }
      }
...
/*! \brief   Request all nodes on the bus to stay in the flashloader
   ...
   Report back list of nodes that sent a response (with the status and node
   IDs). The function will wait for responses for the time configured with
   SetBroadcastTimeout()
   ...
   \return
   C_NO_ERR   no problems; zero or more responses received; data placed in
   orc_Results C_COM      could not send request C_CONFIG   no dispatcher
   installed
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::BroadcastRequestProgramming(
  ...
  if (mpc_CanDispatcher == NULL) {
    ...
    c_Service.c_Data[2] =
        static_cast<uint8_t>(mhu16_OSY_BC_RC_SID_REQUEST_PROGRAMMING >> 8U);
    c_Service.c_Data[3] =
        static_cast<uint8_t>(mhu16_OSY_BC_RC_SID_REQUEST_PROGRAMMING & 0xFFU);
    mh_ComposeSingleFrame(c_Service, m_GetTxBroadcastIdentifier(), c_Msg);
        ...
        if (s32_ReturnLocal == C_NO_ERR) {
          ...
          if ((c_Response.u8_DLC == 5U) &&
            ...
            c_Result.q_RequestAccepted = true;
                     ...
                     (c_Response.au8_Data[1] == mhu8_BC_OSY_NR_SI) &&
                     (c_Response.au8_Data[2] ==
                      mhu8_OSY_BC_SI_ROUTINE_CONTROL)) {
...
/*! \brief   Set node-id of node specified by serial number
   ...
   The function will wait for responses for the time configured with
   SetBroadcastTimeout().

   As specified the serial number is expected to be unique and non-addressed
   nodes shall keep their gob shut. So
...
int32_t C_OscProtocolDriverOsyTpCan::BroadcastSetNodeIdBySerialNumber(
  ...
  if (mpc_CanDispatcher == NULL) {
    s32_Return = C_CONFIG;
  } else if ((orc_NewNodeId.u8_BusIdentifier >
    ...
    c_Service.c_Data[0] = mhu8_OSY_BC_SI_ROUTINE_CONTROL;
    c_Service.c_Data[1] = mhu8_OSY_BC_RC_SUB_FUNCTION_START_ROUTINE;
    c_Service.c_Data[2] = static_cast<uint8_t>(
        mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART1 >> 8U);
    c_Service.c_Data[3] = static_cast<uint8_t>(
        mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART1 & 0xFFU);
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      c_Service.c_Data[2] = static_cast<uint8_t>(
          mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART2 >> 8U);
      c_Service.c_Data[3] = static_cast<uint8_t>(
          mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART2 & 0xFFU);
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        c_Service.c_Data[1] = mhu8_OSY_BC_RC_SUB_FUNCTION_START_ROUTINE;
        c_Service.c_Data[2] = static_cast<uint8_t>(
            mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART3 >> 8U);
        c_Service.c_Data[3] = static_cast<uint8_t>(
            mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART3 & 0xFFU);
    ...
    if (s32_Return == C_NO_ERR) {
      s32_Return = this->m_HandleBroadcastSetNodeIdBySerialNumberResponse(
          c_Service.c_Data[2], c_Service.c_Data[3], opu8_NrCode);
    }
  }
  return s32_Return;
}
...
/*! \brief   Set node-id of node specified by serial number extended
   ...
   * multiple responses are interpreted as error
   ...
   The service consists of at least 3 and maximum 12 individual single frame
   services. A response is only expected after the last single frame. The number
   of used single frames depends of the length of the serial number (1 to 29
   bytes)

   \param[in]    orc_SerialNumber                     serial number of node to
   address (1 to 29 bytes allowed)
   \param[in]    ou8_SubNodeId                        sub node id of sub node to
   address (in case of a device without sub nodes: 0)
   \param[in]    orc_NewNodeId                        node ID to set
   \param[out]   opu8_NrCode                          if not NULL: negative
   response code (if C_WARN is returned)

   \return
   C_NO_ERR    no problems; one positive response received
   C_RANGE     invalid node ID (bus ID or node-ID out of range); "0x7F" is not
   permitted as node ID as it's reserved for broadcasts Serial number is empty
   or has more than 29 bytes C_WARN      negative response received and no
   positive response received C_COM       could not send requests C_CONFIG    no
   dispatcher installed C_TIMEOUT   no response within timeout (was
   SetNodeIdentifiersForBroadcasts() called ?) C_OVERFLOW  multiple positive
   responses received
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::BroadcastSetNodeIdBySerialNumberExtended(
  ...
  if (mpc_CanDispatcher == NULL) {
    ...
    // -- SubNodeId; 1 byte (0..15); set to zero if the device does not have
    // logical sub-nodes
    // -- SerialNumberManufacturerFormat (1 byte)
    ...
    c_Service.c_Data[1] = mhu8_OSY_BC_RC_SUB_FUNCTION_START_ROUTINE;
    c_Service.c_Data[2] = static_cast<uint8_t>(
        mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_EXT_START >> 8U);

    do {
      c_Service.c_Data[3] =
          static_cast<uint8_t>(
              mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_EXT_START &
              0xFFU) +
      ...
      if (u16_PartCounter == 0U) {
        ...
        const uint8_t u8_BytesLeft =
            orc_SerialNumber.u8_SerialNumberByteLength -
            u8_SerialNumberBytesSent;
        const uint8_t u8_BytesToCopy =
            (u8_BytesLeft >= 3U) ? 3U : (u8_BytesLeft % 3U);

        if (u8_BytesToCopy < 3U) {
          // Set the not needed bytes to 0
          memset(&c_Service.c_Data[static_cast<size_t>(u8_BytesToCopy) + 4U],
                 0U, 3U - static_cast<size_t>(u8_BytesToCopy));
        }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::BroadcastEcuReset(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsyTpCan::BroadcastSendEnterPreProgrammingSession(
    void) const {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsyTpCan::BroadcastSendEnterDefaultSession(void) const {
...
/*! \brief   Print logging text
   ...
   call logging handler function to perform the actual logging.

   \param[in]     orc_Information     text to log
   \param[in]     opcn_Function       function name
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsyTpCan::m_LogWarningWithHeader(
    const QString &orc_Information, const char_t *const opcn_Function) const {
  C_OscLoggingHandler::h_WriteLogError(
      "openSYDE CAN-TP",
      "openSYDE CAN-TP node " + QString::number(mc_ServerId.u8_BusIdentifier) +
          "." + QString::number(mc_ServerId.u8_NodeIdentifier) + ": " +
          orc_Information,
      __FILE__, opcn_Function);
}
