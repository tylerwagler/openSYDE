...
/*! \brief   get address of signature block from hex file
   ...
   Possible approach when we have the flash packages: put the address into the
   package information. The signature generator has this information anyway.
...
int32_t C_OscHexFile::GetSignatureBlockAddress(uint32_t &oru32_Address) {
  ...
  // lint -e{926}
  s32_Return = this->FindPattern(
      oru32_Address, 10, reinterpret_cast<const uint8_t *>(";zwm2KgUZ!"));
  if (s32_Return == -1) {
    s32_Return = C_NOACT;
  }
  return s32_Return;
...
/*! \brief   extract device ID from hex-file
   ...
   If the application_info structure with the device ID information is in the
   hex file more than once, AND the device ID in at least one of the copies is
   different, we fail.
...
int32_t C_OscHexFile::ScanDeviceIdFromHexFile(QString &orc_DeviceId) {
  ...
  s32_Return =
      this->GetECUInformationBlocks(c_InfoBlocks, 0x0U, false, false, true);
...
/*! \brief   extract application information block from hex-file
   ...
   C_WARN       multiple application blocks detected in hex file but device
   names match output in this case is the first found application block C_NOACT
   no application information block detected in hex file C_OVERFLOW   multiple
   application information blocks detected in hex file and device names differ
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscHexFile::ScanApplicationInformationBlockFromHexFile(
    C_XFLECUInformation &orc_InfoBlock) {
  int32_t s32_Return;
  ...
  if (c_InfoBlocks.size() == 0) {
    ...
    for (int32_t s32_Pos = 1; s32_Pos < c_InfoBlocks.size(); s32_Pos++) {
      // compare every device name with first device name, this is enough
      // because all must be equal
      if (c_InfoBlocks[0].GetDeviceID() !=
          c_InfoBlocks[s32_Pos].GetDeviceID()) {
