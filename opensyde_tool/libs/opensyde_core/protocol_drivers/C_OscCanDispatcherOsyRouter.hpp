...
class C_OscCanDispatcherOsyRouter : public stw::can::C_CanDispatcher {
public:
  C_OscCanDispatcherOsyRouter(C_OscProtocolDriverOsy &orc_OsyProtocol);
  C_OscCanDispatcherOsyRouter(C_OscProtocolDriverOsy &orc_OsyProtocol,
                              const uint8_t ou8_CommChannel);

  void SetFilterParameters(const uint8_t ou8_RoutingChannel,
...
private:
  ...
  C_OscCanDispatcherOsyRouter &
  operator=(const C_OscCanDispatcherOsyRouter &orc_Source);

  static void mh_OsyTunnelCanMessageReceived(
      void *const opv_Instance, const uint8_t ou8_Channel,
      const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  void m_OsyTunnelCanMessageReceived(
  ...
  QList<stw::can::T_STWCAN_Msg_RX> mc_AsyncMessages;
};
