...
class C_OscProtocolDriverOsyTpIp : public C_OscProtocolDriverOsyTpBase {
private:
  /// DoIp protocol header
  class C_DoIpHeader {
  public:
    // constants:
    static const uint8_t hu8_DOIP_HEADER_SIZE =
        0x08U; ///< size of DoIp header in bytes
    ...
    static const uint16_t hu16_PAYLOAD_TYPE_OSY_GET_DEVICE_INFO_EXT_RES =
        0xF009U;
    static const uint16_t hu16_PAYLOAD_TYPE_SET_IP_ADDRESS_MESSAGE_REQ =
        0xF002U;
    static const uint16_t hu16_PAYLOAD_TYPE_SET_IP_ADDRESS_MESSAGE_RES =
        0xF003U;
    static const uint16_t hu16_PAYLOAD_TYPE_SET_IP_ADDRESS_MESSAGE_EXT_REQ =
        0xF00BU;
    static const uint16_t hu16_PAYLOAD_TYPE_SET_IP_ADDRESS_MESSAGE_EXT_RES =
        0xF00CU;
    ...
    static const uint16_t hu16_PAYLOAD_TYPE_CAN_ROUTING_WITHOUT_FC_REQ =
        0xF100U;

    C_DoIpHeader(void);
    C_DoIpHeader(const uint16_t ou16_PayloadType,
  ...
  public:
    ...
    enum E_ServiceStatus { eIDLE, ePARTIAL };
  ...
  C_OscIpDispatcher
      *mpc_Dispatcher; ///< driver to use for accessing local IP stack
  uint32_t
      mu32_DispatcherHandle; ///< handler to use for dispatcher TDP functions
...
protected:
  ...
  void m_LogWarningWithHeaderAndIp(const QString &orc_Information,
...
public:
  ...
  public:
    ...
    bool operator<(const C_BroadcastGetDeviceInfoResults &orc_Cmp) const;
    virtual void ParseFromArray(const QByteArray &orc_Data,
  ...
  public:
    uint8_t u8_SubNodeId;     ///< ID of sub node in case of multi CPU devices.
                              ///< Default is 0
    bool q_SecurityActivated; ///< flag if node has security feature activated

    bool
    operator==(const C_BroadcastGetDeviceInfoExtendedResults &orc_Cmp) const;
    bool
    operator<(const C_BroadcastGetDeviceInfoExtendedResults &orc_Cmp) const;
    virtual void ParseFromArray(const QByteArray &orc_Data,
                                const uint8_t ou8_DataStartIndex);
  ...
  explicit C_OscProtocolDriverOsyTpIp(
  ...
  int32_t SetDispatcher(C_OscIpDispatcher *const opc_Dispatcher,
  ...
  int32_t BroadcastGetDeviceInfo(
  ...
  int32_t
  BroadcastNetReset(const uint8_t ou8_ResetType,
