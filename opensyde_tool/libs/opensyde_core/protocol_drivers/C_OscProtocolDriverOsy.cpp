...
C_OscProtocolDriverOsy::C_OscProtocolDriverOsy(void)
    : mpr_OnOsyTunnelCanMessageReceived(NULL),
      mpv_OnAsyncTunnelCanMessageInstance(NULL), mpr_OnOsyWaitTime(NULL),
      mpv_OnOsyWaitTimeInstance(NULL), mpc_TransportProtocol(NULL),
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::SetTimeoutPolling(
    const uint32_t ou32_TimeoutPollingMs) {
...
/*! \brief   Set maximum size of service
   ...
   If this function is not called the default value of
   C_OscProtocolDriverOsyTpBase::hu16_OSY_MAXIMUM_SERVICE_SIZE will be used.

   \param[in]  ou16_MaxServiceSize   Maximum service size to use (including
   header)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::SetMaxServiceSize(
    const uint16_t ou16_MaxServiceSize) {
...
/*! \brief   Initialization of the async handling of tunneled CAN messages
   ...
   \param[in]  opr_OsyTunnelCanMessageReceived     function to be called if an
   async Tunnel CAN message is received
   \param[in]  opv_Instance                        instance pointer to pass back
   when invoking read event callback
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::InitializeTunnelCanMessage(
    const C_OscProtocolDriverOsy::PR_OsyTunnelCanMessageReceived
        opr_OsyTunnelCanMessageReceived,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Initialization of the handling of long waiting times for services
   with high timeout times

   \param[in]  opr_OsyHandleWaitTime               function to be called if a
   service has a long waiting time for the response
   \param[in]  opv_Instance                        instance pointer to pass back
   when invoking read event callback
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::InitializeHandleWaitTime(
    const C_OscProtocolDriverOsy::PR_OsyHandleWaitTime opr_OsyHandleWaitTime,
    void *const opv_Instance) {
...
/*! \brief   Add log entry for common protocol problems
   ...
   \param[in]   os32_ReturnCode   Protocol driver function return code (C_NO_ERR
   -> no log entry)
   \param[in]   ou8_NrCode        Negative response code (if os32_ReturnCode is
   C_WARN)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::m_LogServiceError(const QString &orc_Service,
  ...
  if (os32_ReturnCode != C_NO_ERR) {
    ...
    const QString c_ErrorText =
        C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
            os32_ReturnCode, ou8_NrCode, &q_IsHardError);
    m_LogErrorWithHeader("openSYDE protocol driver",
                         "Service " + orc_Service +
                             " failed. Error: " + c_ErrorText,
...
/*! \brief   Perform cyclic communication tasks
   ...
   If a non event-driven response with service ID "ou8_ExpectedServiceId" is
    received the function will return, otherwise it will continue to read all
   incoming responses from the Rx queue.

   \param[in]   oq_CheckForSpecificServiceId    false: try to treat all
   responses as event-driven true: return if a response to service
   ou8_ExpectedServiceId is received
   \param[in]   ou8_ExpectedServiceId           if oq_CheckForSpecificServiceId
   is true: expected service ID
   \param[out]  opc_ReceivedService             if oq_CheckForSpecificServiceId
   is true and if != NULL: received non event-driven response

   \return
   C_NO_ERR   if oq_CheckForSpecificServiceId is true: expected service response
   (positive or negative) received and placed in orc_ReceivedService if
   oq_CheckForSpecificServiceId is false: finished with handling Rx queue C_WARN
   if oq_CheckForSpecificServiceId is true: expected service not received
   C_CONFIG   no transport protocol or transport protocol returns error
   C_NOACT    nothing received
   C_COM      communication error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::m_Cycle(
    const bool oq_CheckForSpecificServiceId,
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      while (s32_Return == C_NO_ERR) {
        ...
        if (s32_Return == C_NO_ERR) {
          ...
          //  It might be a negative response with more information than UDS
          //  specified
          if ((c_Service.c_Data.size() >= 3U) &&
              (c_Service.c_Data[0] == mhu8_OSY_NR_SI)) {
            //... to our request ?
            if ((oq_CheckForSpecificServiceId == true) &&
                (c_Service.c_Data[1] == ou8_ExpectedServiceId)) {
              ...
              // trigger async virtual functions for all received async
              // responses
      ...
      if ((oq_CheckForSpecificServiceId == true) &&
          (q_ExpectedServiceReceived == false)) {
        s32_Return = C_WARN;
      }
    }
  }
...
/*! \brief   Poll for one specific response service
   ...
   The function will not actively wait and poll but use a platform specific
   mechanism the put the calling thread to sleep between individual polling
   calls.

   \param[in]  ou8_ExpectedServiceId   ID of service the response is expected
   for
   \param[in]  ou16_ExpectedSize       expected size of response service
   \param[out] orc_Service             response service
   \param[in]  oru8_NrCode             if negative response: negative response
   error code
   \param[in]  oq_ExactSizeExpected    true: response size must exactly match
   ou16_ExpectedSize false: response size must be >= ou16_ExpectedSize
   \param[in]  opc_ExpectedErrData     optional pointer to a vector with data
   for more specific identifying the possible matching negative response after
   the first three byte

   \return
   C_NO_ERR   non event-driven service matching the definition received and
   placed in orc_Service C_WARN     error response C_TIMEOUT  expected response
   not received within timeout C_COM      communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::m_PollForSpecificServiceResponse(
  ...
  // If another thread checking for async responses kicks in (calling ::Cycle)
  // is could otherwise
  //  snatch away the response we want to get.
  {
    QMutexLocker c_Lock(&mc_LockReception);

    while ((c_Timer.hasExpired(mu32_TimeoutPollingMs) == false) &&
           (q_Finished == false)) {
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        if ((u16_RxSize >= 3U) && (orc_Service.c_Data[0] == mhu8_OSY_NR_SI)) {
          ...
          if (orc_Service.c_Data[1] == ou8_ExpectedServiceId) {
            ...
            if ((opc_ExpectedErrData != NULL) && (u16_RxSize > 3U)) {
              ...
              const uint32_t u32_NumberOfBytes =
                  static_cast<uint32_t>(opc_ExpectedErrData->size());

              // Enough bytes for comparing?
              if (orc_Service.c_Data.size() >=
                  (static_cast<size_t>(u32_NumberOfBytes) + 3U)) {
                // Check all bytes for
                for (uint32_t u32_Counter = 0U; u32_Counter < u32_NumberOfBytes;
                     ++u32_Counter) {
                  if ((*opc_ExpectedErrData)[u32_Counter] !=
                      orc_Service
                          .c_Data[static_cast<size_t>(u32_Counter) + 3U]) {
                    q_Match = false;
                    // when starting cyclic calls we might interpret the first
                    // data transmission of the last call
                    //  as the response of the next cyclic service registration,
                    //  so we should not discard this message but instead handle
                    //  the error accordingly
                    if (orc_Service.c_Data[1] ==
                        mhu8_OSY_SI_READ_DATA_POOL_DATA_EVENT_DRIVEN) {
                      // handle data error
                      s32_Return = m_HandleAsyncResponse(orc_Service);
                    } else {
                      m_LogErrorWithHeader("Synchronous communication",
                                           "Sync negative response to expected "
                                           " service but unexpected data bytes "
                                           "received. Ignoring.",
                                           TGL_UTIL_FUNC_ID);
                    }
                    break;
                ...
                m_LogErrorWithHeader(
                    ...
                    "Sync negative response to expected service"
                    " but unexpected size received. Ignoring.",
                    TGL_UTIL_FUNC_ID);
              }
            }
            ...
            if (q_Match == true) {
              ...
              // special handling for "responsePending": rewind Rx timeout
              // expectation
            ...
            m_LogErrorWithHeader(TGL_UTIL_FUNC_ID, "Synchronous communication",
                                 "Sync negative response to unexpected service "
                                 "received. Ignoring.");
          }
        }
        ...
        if (q_Finished == false) {
          ...
          if (((oq_ExactSizeExpected == true) &&
               (u16_RxSize == ou16_ExpectedSize)) ||
              ((oq_ExactSizeExpected == false) &&
               (u16_RxSize >= ou16_ExpectedSize))) {
            ...
            // when initiating event driven transfers we might get a response to
            // the expected service but it's
            //  an actual data response to an already ongoing transfer
            if (orc_Service.c_Data[0] ==
                (mhu8_OSY_SI_READ_DATA_POOL_DATA_EVENT_DRIVEN | 0x40U)) {
              // last chance ...
              s32_Return = m_HandleAsyncResponse(orc_Service);
            }
            if (s32_Return != C_NO_ERR) {
              ...
              m_LogErrorWithHeader(
                  "Synchronous communication",
                  "Sync response with unexpected length received. Ignoring.",
                  TGL_UTIL_FUNC_ID);
            }
          }
        }
        ...
        if (this->mpr_OnOsyWaitTime != NULL) {
          const uint32_t u32_CurrentTime =
              static_cast<uint32_t>(c_Timer.elapsed());
          if (u32_CurrentTime >
              (u32_LastWaitTimeHandled + hu32_DEFAULT_HANDLE_WAIT_TIME)) {
  ...
  if (s32_Return == C_COM) {
    m_LogErrorWithHeader("Synchronous communication",
                         "No response received (communication error).",
                         TGL_UTIL_FUNC_ID);
  } else if (q_Finished == false) {
    m_LogErrorWithHeader("Synchronous communication",
                         "No response received (timed out).", TGL_UTIL_FUNC_ID);
...
/*! \brief   DiagnosticSessionControl service implementation
   ...
   \param[out] opu8_NrCode           if != NULL: negative response code in case
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyDiagnosticSessionControl(
    const uint8_t ou8_SessionId, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  C_OscProtocolDriverOsyService c_Service;
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_DIAGNOSTIC_SESSION_CONTROL, 6U, c_Service,
          u8_NrErrorCode);
...
/*! \brief   ReadDataByIdentifier utility wrapper
   ...
   \param[in]  ou16_ExpectedPayloadSize   number of payload bytes to expect in
   response
   \param[in]  oq_ExactSizeExpected       true: expect exactly
   ou16_ExpectedPayloadSize bytes false: expect at least
   ou16_ExpectedPayloadSize bytes
   \param[out] orc_ReadData               payload data read
   \param[out] oru8_NrCode                negative response code in case of an
   error response
...
int32_t C_OscProtocolDriverOsy::m_ReadDataByIdentifier(
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_READ_DATA_BY_IDENTIFIER, ou16_ExpectedPayloadSize + 3U,
          c_Response, u8_NrErrorCode, oq_ExactSizeExpected);
      switch (s32_Return) {
      case C_NO_ERR:
        // check DI:
...
/*! \brief   Utility: read string data identifier
   ...
   Allows to perform a ReadDataByIdentifier service with only an
   (non-terminated) string expected as response. Send request and wait for
   response. See class description for general handling of "polled" services.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::m_ReadStringDataIdentifier(
    const uint16_t ou16_DataIdentifier, QString &orc_String,
    uint8_t &oru8_NrCode) {
  int32_t s32_Return;
  ...
  if (s32_Return == C_NO_ERR) {
    // extract text:
    orc_String =
        QString::fromUtf8(reinterpret_cast<const char *>(c_Data.data()),
                          static_cast<int>(c_Data.size()));
  }
...
int32_t C_OscProtocolDriverOsy::m_WriteDataByIdentifier(
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      uint8_t u8_NrErrorCode;
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_WRITE_DATA_BY_IDENTIFIER, 3U, c_Response, u8_NrErrorCode,
          true);
      switch (s32_Return) {
      case C_NO_ERR:
        // check response:
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadEcuSerialNumber(
  ...
  m_LogServiceError("ReadDataByIdentifier::EcuSerialNumber", s32_Return,
                    u8_NrErrorCode);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyReadHardwareNumber(uint32_t &oru32_HardwareNumber,
  ...
  m_LogServiceError("ReadDataByIdentifier::HardwareNumber", s32_Return,
                    u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadHardwareVersionNumber(
    QString &orc_HardwareVersionNumber, uint8_t *const opu8_NrCode)

{
  ...
  s32_Return =
      m_ReadStringDataIdentifier(mhu16_OSY_DI_SYS_SUPPLIER_ECU_HW_VERSION,
                                 orc_HardwareVersionNumber, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("ReadDataByIdentifier::HardwareVersionNumber", s32_Return,
                    u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadListOfFeatures(
  ...
  if (s32_Return == C_NO_ERR) {
    // extract bits:
    orc_ListOfFeatures.q_FlashloaderCanWriteToNvm =
        ((c_Data[7] & 0x01U) == 0x01U) ? true : false;
    orc_ListOfFeatures.q_MaxNumberOfBlockLengthAvailable =
        ((c_Data[7] & 0x02U) == 0x02U) ? true : false;
    orc_ListOfFeatures.q_EthernetToEthernetRoutingSupported =
        ((c_Data[7] & 0x04U) == 0x04U) ? true : false;
    orc_ListOfFeatures.q_FileBasedTransferExitResultAvailable =
        ((c_Data[7] & 0x08U) == 0x08U) ? true : false;
    orc_ListOfFeatures.q_ExtendedSerialNumberModeImplemented =
        ((c_Data[7] & 0x10U) == 0x10U) ? true : false;
    orc_ListOfFeatures.q_SupportsSecurity =
        ((c_Data[7] & 0x20U) == 0x20U) ? true : false;
    orc_ListOfFeatures.q_SupportsDebuggerOff =
        ((c_Data[7] & 0x40U) == 0x40U) ? true : false;
    orc_ListOfFeatures.q_SupportsDebuggerOn =
        ((c_Data[7] & 0x80U) == 0x80U) ? true : false;
    // we don't know anything about the meaning of the rest of the bits as we
  ...
  m_LogServiceError("ReadDataByIdentifier::ListOfFeatures", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   MaxNumberOfBlockLength service implementation
   ...
   Whether this service is available on the target can be checked with
   OsyReadListOfFeatures().

   \param[out] oru16_MaxNumberOfBlockLength     read MaxNumberOfBlockLength
   \param[out] opu8_NrCode                      if != NULL: negative response
   code in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadMaxNumberOfBlockLength(
  ...
  s32_Return = m_ReadDataByIdentifier(mhu16_OSY_DI_MAX_NUMBER_OF_BLOCK_LENGTH,
                                      2U, true, c_Data, u8_NrErrorCode);
  if (s32_Return == C_NO_ERR) {
    // extract information:
    oru16_MaxNumberOfBlockLength =
        (static_cast<uint16_t>((static_cast<uint16_t>(c_Data[0])) << 8U)) +
        (c_Data[1]);
  }
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  ...
  m_LogServiceError("ReadDataByIdentifier::ReadMaxNumberOfBlockLength",
                    s32_Return, u8_NrErrorCode);
...
/*! \brief   ReadDeviceName service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   Per specification the maximum length is 28 characters. This is not checked by
   this function.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadDeviceName(QString &orc_DeviceName,
                                                  uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;

  s32_Return = m_ReadStringDataIdentifier(mhu16_OSY_DI_DEVICE_NAME,
                                          orc_DeviceName, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("ReadDataByIdentifier::DeviceName", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadApplicationName service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   Per specification the maximum length is 32 characters. This is not checked by
   this function.
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyReadApplicationName(QString &orc_ApplicationName,
                                               uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;

  s32_Return = m_ReadStringDataIdentifier(mhu16_OSY_DI_APPLICATION_NAME,
                                          orc_ApplicationName, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("ReadDataByIdentifier::ApplicationName", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadApplicationName service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   Per specification the maximum length is 16 characters. This is not checked by
   this function.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadApplicationVersion(
    QString &orc_ApplicationVersion, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;

  s32_Return = m_ReadStringDataIdentifier(
      mhu16_OSY_DI_APPLICATION_VERSION, orc_ApplicationVersion, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("ReadDataByIdentifier::ApplicationVersion", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadBootSoftwareIdentification service implementation
   ...
   Technically the protocol would support multiple version numbers in the
   response. This is for UDS compatibility. In openSYDE we only send one
   version. This function also only supports one version.
   ...
   C_WARN     error response (negative response code placed in *opu8_NrCode)
   C_RD_WR    unexpected content in response (here: wrong data identifier ID,
   or: number of modules is not 1) C_COM      communication driver reported
   error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadBootSoftwareIdentification(
  ...
  m_LogServiceError("ReadDataByIdentifier::BootSoftwareIdentification",
                    s32_Return, u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadActiveDiagnosticSession(
  ...
  m_LogServiceError("ReadDataByIdentifier::ActiveDiagnosticSession", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadApplicationSoftwareFingerprint service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   Per specification the length is minimum 7 bytes, maximum 21 bytes. This is
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadApplicationSoftwareFingerprint(
    uint8_t (&orau8_Date)[3], uint8_t (&orau8_Time)[3], QString &orc_Username,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Data.size() > 7) {
      orc_Username =
          QString::fromUtf8(reinterpret_cast<const char *>(&c_Data[7]),
                            static_cast<int>(c_Data.size() - 7));
    } else {
      orc_Username.clear();
    }
  }
  ...
  m_LogServiceError("ReadDataByIdentifier::ApplicationSoftwareFingerprint",
                    s32_Return, u8_NrErrorCode);
...
/*! \brief   ReadFileBasedTransferExitResult service implementation
   ...
   \param[out] opu8_NrCode              if != NULL: negative response code in
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadFileBasedTransferExitResult(
    QString &orc_TransferExitResult, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;

  s32_Return =
      m_ReadStringDataIdentifier(mhu16_OSY_DI_FILE_BASED_TRANSFER_EXIT_RESULT,
                                 orc_TransferExitResult, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("ReadDataByIdentifier::FileBasedTransferExitResult",
                    s32_Return, u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadEcuSerialNumberExt(
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Data.size() > 2) {
      ...
      if ((u8_SnrLength <= 29U) &&
          ((static_cast<uint32_t>(u8_SnrLength) + 2U) ==
        ...
        s32_Return = orc_SerialNumberExt.SetExtSerialNumber(
            c_Data, u8_SerialNumberManufacturerFormat);
      } else {
        s32_Return = C_RANGE;
      }
    } else {
  ...
  m_LogServiceError("ReadDataByIdentifier::ECUSerialNumberDataIdentifierExt",
                    s32_Return, u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadSubNodeId(uint8_t &oru8_SubNodeId,
                                                 uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
...
/*! \brief   WriteApplicationSoftwareFingerprint service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   If a user name with more than 20 characters is passed only the first 20 will
   be used.
...
int32_t C_OscProtocolDriverOsy::OsyWriteApplicationSoftwareFingerprint(
  ...
  m_LogServiceError("WriteDataByIdentifier::ApplicationSoftwareFingerprint",
                    s32_Return, u8_NrErrorCode);
...
/*! \brief  FactoryMode implementation.
   ...
   Currently the only known operation is "master reset" with ou8_Operation =
   hu8_OSY_FACTORY_MODE_MASTER_RESET

   \param[in]   ou8_Operation    factory mode operation: 0 = MasterReset, 1..255
   reserved
   \param[out]  opu8_NrCode      if != NULL: negative response code in case of
   an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyFactoryMode(const uint8_t ou8_Operation,
                                               uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
...
/*! \brief   ReadCertificateSerialNumber service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   Serial number is a byte array with maximum length of 20 Bytes. Leading zeroes
   will be stripped and only the remainder be reported to the caller.
   ...
   C_RD_WR    unexpected content in response (here: wrong data identifier ID)
   C_COM      communication driver reported error
   C_RANGE    count of read bytes does not match the expectation (more than 20
   bytes received)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadCertificateSerialNumber(
  ...
  // According to the protocol specification the server shall respond with a
  // "conditionsNotCorrect" NRC
  //  if it does not have a valid certificate serial number.
  // Unfortunately there are incorrect implementations that send "ok" with a
  // zero length value. We will play nice and accept such responses.
  s32_Return =
      m_ReadDataByIdentifier(mhu16_OSY_DI_CERTIFICATE_SERIAL_NUMBER, 0U, false,
                             orc_SerialNumber, u8_NrErrorCode);
  if (s32_Return == C_NO_ERR) {
    if (orc_SerialNumber.size() == 0) {
      osc_write_log_warning(
          "Protocol driver",
          "Protocol violation: received an empty certificate serial number. "
          "Reporting an invalid value to application.");
  ...
  m_LogServiceError("ReadDataByIdentifier::CertificateSerialNumber", s32_Return,
                    u8_NrErrorCode);
...
int32_t C_OscProtocolDriverOsy::OsyReadCertificateSerialNumberL7(
  ...
  s32_Return =
      m_ReadDataByIdentifier(mhu16_OSY_DI_CERTIFICATE_SERIAL_NUMBER_L7, 1U,
                             false, orc_SerialNumber, u8_NrErrorCode);
  if (s32_Return == C_NO_ERR) {
    // Strip leading zeroes for compatibility (see #100795 for details)
    while ((orc_SerialNumber.size() > 0) && (orc_SerialNumber[0] == 0x00U)) {
      orc_SerialNumber.erase(orc_SerialNumber.begin());
  ...
  m_LogServiceError("ReadDataByIdentifier::CertificateSerialNumberL7",
                    s32_Return, u8_NrErrorCode);
...
/*! \brief   WriteSecurityKey service implementation
   ...
   Send request and wait for response.
   See class description for general handling of "polled" services.
   Public key modulus is a byte array with a length of 128 Bytes (high byte
   first). Public key exponent is a byte array with a length of 1 to 4 Bytes
   containing the exponent (high byte first). Serial number is a byte array with
   ...
   C_RD_WR    unexpected content in response (here: wrong data identifier ID)
   C_COM      communication driver reported error
   C_RANGE    parameter orc_PublicKeyModulus, orc_PublicKeyExponent,
   orc_SerialNumber does not match the size expectation
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyWriteSecurityKey(
  ...
  m_LogServiceError("WriteDataByIdentifier::SecurityKey", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadSecurityActivation service implementation
   ...
   \param[out] orq_SecurityOn               read flag if security is on or off
   on the node
   \param[out] oru8_SecurityAlgorithm       read used security algorithm of the
   node
   \param[out] opu8_NrCode                  if != NULL: negative response code
   in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadSecurityActivation(
  ...
  m_LogServiceError("ReadDataByIdentifier::SecurityActivation", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   WriteSecurityActivation service implementation
   ...
   \param[in]  oq_SecurityOn          new flag if security is on or off on the
   node
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyWriteSecurityActivation(
  ...
  s32_Return = m_WriteDataByIdentifier(mhu16_OSY_DI_SECURITY_ACTIVATION, c_Data,
                                       u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("WriteDataByIdentifier::SecurityActivation", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadDebuggerEnabled service implementation
   ...
   \param[out] orq_DebuggerEnabled          read flag if debugger is on or off
   on the node
   \param[out] opu8_NrCode                  if != NULL: negative response code
   in case of an error response
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyReadDebuggerEnabled(bool &orq_DebuggerEnabled,
                                               uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  m_LogServiceError("ReadDataByIdentifier::DebuggerEnabled", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   WriteDebuggerEnabled service implementation
   ...
   If the debugger interface of device can be activated or deactivated depends
   on the device. This information can be read with the function
   OsyReadListOfFeatures and the resulting flags q_SupportsDebuggerOff and
   q_SupportsDebuggerOn.

   \param[in]  oq_DebuggerEnabled     new flag if debugger is on or off on the
   node
   \param[out] opu8_NrCode            if != NULL: negative response code in case
   of an error response
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyWriteDebuggerEnabled(const bool oq_DebuggerEnabled,
                                                uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;
  ...
  s32_Return = m_WriteDataByIdentifier(mhu16_OSY_DI_DEBUGGER_ACTIVATION, c_Data,
                                       u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }

  m_LogServiceError("WriteDataByIdentifier::DebuggerEnabled", s32_Return,
                    u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadMaxNumOfEventDrivenTransmissions(
  ...
  m_LogServiceError("ReadDataByIdentifier::MaxNumOfEventDrivenTransmissions",
                    s32_Return, u8_NrErrorCode);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyReadProtocolVersion(uint8_t (&orau8_Version)[3],
                                               uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  m_LogServiceError("ReadDataByIdentifier::ProtocolVersion", s32_Return,
                    u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadFlashloaderProtocolVersion(
    uint8_t (&orau8_Version)[3], uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  m_LogServiceError("ReadDataByIdentifier::FlashloaderProtocolVersion",
                    s32_Return, u8_NrErrorCode);
...
/*! \brief   ReadFlashCount service implementation
   ...
   \param[out] oru32_FlashCount       read number of flash programming cycles
   performed on server
   \param[out] opu8_NrCode            if != NULL: negative response code in case
   of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadFlashCount(uint32_t &oru32_FlashCount,
                                                  uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  m_LogServiceError("ReadDataByIdentifier::FlashCount", s32_Return,
                    u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadProtocolDriverImplementationVersion(
    uint8_t (&orau8_Version)[3], uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  m_LogServiceError("ReadDataByIdentifier::ProtocolDriverImplementationVersion",
                    s32_Return, u8_NrErrorCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::m_PackDataPoolIdentifier(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, uint8_t (&orau8_PackedId)[3]) const {
  int32_t s32_Return = C_RANGE;
  const uint8_t ou8_ServerDpIndex =
      this->m_GetDataPoolIndexClientToServer(ou8_DataPoolIndex);

  if ((ou8_ServerDpIndex < mhu8_OSY_MAX_NUM_DATA_POOLS) &&
      (ou16_ListIndex < mhu8_OSY_MAX_NUM_DATA_POOL_LISTS) &&
      (ou16_ElementIndex < mhu16_OSY_MAX_NUM_DATA_POOL_LIST_ELEMENTS)) {
    const uint32_t u32_PackedId =
        (static_cast<uint32_t>(ou8_ServerDpIndex) << 18U) +
        (static_cast<uint32_t>(ou16_ListIndex) << 11U) + ou16_ElementIndex;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::m_UnpackDataPoolIdentifier(
    const uint8_t (&orau8_PackedId)[3], uint8_t &oru8_DataPoolIndex,
    uint16_t &oru16_ListIndex, uint16_t &oru16_ElementIndex) const {
  const uint32_t u32_PackedId =
      (static_cast<uint32_t>(orau8_PackedId[0]) << 16U) +
      (static_cast<uint32_t>(orau8_PackedId[1]) << 8U) +
      static_cast<uint32_t>(orau8_PackedId[2]);

  const uint8_t u8_ServerDataPoolIndex =
      static_cast<uint8_t>(u32_PackedId >> 18U) & 0x1FU;

  oru16_ListIndex = static_cast<uint16_t>(u32_PackedId >> 11U) & 0x7FU;
  oru16_ElementIndex = static_cast<uint16_t>(u32_PackedId) & 0x7FFU;

  oru8_DataPoolIndex =
      this->m_GetDataPoolIndexServerToClient(u8_ServerDataPoolIndex);
}
...
int32_t C_OscProtocolDriverOsy::OsyReadDataPoolData(
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    s32_Return = m_PackDataPoolIdentifier(ou8_DataPoolIndex, ou16_ListIndex,
                                          ou16_ElementIndex, au8_Identifier);
    if (s32_Return != C_NO_ERR) {
      s32_Return = C_RANGE;
    } else {
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        s32_Return = m_PollForSpecificServiceResponse(
            mhu8_OSY_SI_READ_DATA_POOL_DATA_BY_ID, 4U, c_Response,
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    c_ErrorText = QString::asprintf(
        "ReadDataPoolData(Client indexes: Datapool: %d, List: %d, Element: %d)",
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        s32_Return = m_PollForSpecificServiceResponse(
            mhu8_OSY_SI_WRITE_DATA_POOL_DATA_BY_ID, 4U, c_Response,
            u8_NrErrorCode, true);
        switch (s32_Return) {
        case C_NO_ERR:
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyWriteDataPoolEventDataRate(
    const uint8_t ou8_TransmissionRail, const uint16_t ou16_DataRate,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return = C_RANGE;
  ...
  if (ou8_TransmissionRail <= 2) {
    ...
    s32_Return =
        m_WriteDataByIdentifier(u16_DataIdentifier, c_Data, u8_NrErrorCode);
  ...
  m_LogServiceError("WriteDataByIdentifier::DataPoolEventDataRate", s32_Return,
                    u8_NrErrorCode);
...
/*! \brief   ReadDataPoolDataCyclic service request implementation
   ...
   C_RANGE    data pool, list, element index, rail index out of range; length of
   data zero C_NOACT    could not put request in Tx queue C_CONFIG   no
   transport protocol installed C_WARN     error response (negative response
   code placed in *opu8_NrCode) C_RD_WR    unexpected content in response (here:
   wrong data pool index) C_COM      communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadDataPoolDataCyclic(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        s32_Return = m_PollForSpecificServiceResponse(
            mhu8_OSY_SI_READ_DATA_POOL_DATA_EVENT_DRIVEN, 4U, c_Response,
...
/*! \brief   ReadDataPoolDataChangeDriver service request implementation
   ...
   \param[in]  ou16_ElementIndex     element index to read
   \param[in]  ou8_TransmissionRail  index of interval "rail" to send on (0..2)
   \param[in]  ou32_Hysteresis       threshold the value must change before it
   is re-sent
   \param[out] opu8_NrCode           if != NULL: negative response code in case
   of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadDataPoolDataChangeDriven(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_TransmissionRail,
    const uint32_t ou32_Hysteresis, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  C_OscProtocolDriverOsyService c_Request;
  C_OscProtocolDriverOsyService c_Response;
  uint8_t u8_NrErrorCode = 0U;
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "ReadDataPoolDataChangeDriven(Client indexes: Datapool: %d, List: %d, "
        "Element: %d, Rail: %d, Hysteresis: 0x%08X)",
        ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex,
        ou8_TransmissionRail, ou32_Hysteresis);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyStopDataPoolEvents(uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_READ_DATA_POOL_DATA_EVENT_DRIVEN, 1U, c_Response,
          u8_NrErrorCode, true);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadDataPoolMetaData(
    const uint8_t ou8_DataPoolIndex, C_DataPoolMetaData &orc_MetaData,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;
  const uint8_t ou8_ServerDpIndex =
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_READ_DATAPOOL_META_DATA,
                       mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0,
                       false, c_ReceiveData, u8_NrErrorCode);
  if ((s32_Return == C_NO_ERR) && (c_ReceiveData.size() >= 1)) {
    if (c_ReceiveData[0] == ou8_ServerDpIndex) {
      ...
      // Parse received data
      for (uint32_t u32_ItReceivedData = 1;
           (u32_ItReceivedData < c_ReceiveData.size()) && (q_Stop == false);) {
        for (uint32_t u32_ItSegment = 0; u32_ItSegment < 2; ++u32_ItSegment) {
          if (u32_ItReceivedData < c_ReceiveData.size()) {
            ...
            case 1UL:
              ...
              ++u32_ItReceivedData;
              for (uint32_t u32_ItVersion = 0;
                   (u32_ItReceivedData < c_ReceiveData.size()) &&
                   (u32_ItVersion < 3);
                   ++u32_ItVersion) {
                orc_MetaData.au8_Version[u32_ItVersion] =
                    c_ReceiveData[u32_ItReceivedData];
            ...
            case 2UL:
              ...
              if (u32_ItReceivedData < c_ReceiveData.size()) {
                ...
                ++u32_ItReceivedData;
                for (uint32_t u32_ItName = 0;
                     (u32_ItReceivedData < c_ReceiveData.size()) &&
                     (u32_ItName < u32_Count);
                     ++u32_ItName) {
                  orc_MetaData.c_Name +=
                      static_cast<char_t>(c_ReceiveData[u32_ItReceivedData]);
                  // Iterate segment
                  ++u32_ItReceivedData;
                }
              }
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::ReadDataPoolMetaData(Client indexes: Datapool: %d)",
        ou8_DataPoolIndex);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyVerifyDataPool(
    const uint8_t ou8_DataPoolIndex, const uint32_t ou32_DataPoolChecksum,
  ...
  s32_Return = m_RoutineControl(
      mhu16_OSY_RC_SID_VERIFY_DATAPOOL, mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Route diagnosis communication activation service request
   implementation

   Send request and wait for response.
   See class description for general handling of "polled" services.

   \param[in]     ou8_InputChannelType    Channel type of input channel (0
   equals CAN, 1 equals Ethernet)
   \param[in]     ou8_InputChannelIndex   Channel index of input channel
   \param[in]     ou8_OutputChannelType   Channel type of output channel (0
   equals CAN, 1 equals Ethernet)
   \param[in]     ou8_OutputChannelIndex  Channel index of output channel
   \param[in]     ou8_SourceBusId         The bus ID on client side on the input
   channel direction (0..15)
   \param[in]     ou8_TargetBusId         The bus ID on target server side on
   the output channel direction (0..15)
   \param[out]    opu8_NrCode             if != NULL: negative response code in
   case of an error response
   ...
   C_RANGE    - input channel type is CAN and output channel type is Ethernet.
   This combination is not supported.
              - a bus ID is out of range
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySetRouteDiagnosisCommunication(
    const uint8_t ou8_InputChannelType, const uint8_t ou8_InputChannelIndex,
  ...
  if ((ou8_SourceBusId <= C_OscProtocolDriverOsyNode::mhu8_MAX_BUS) &&
    ...
    s32_Return =
        m_RoutineControl(mhu16_OSY_RC_SID_ROUTE_DIAGNOSIS_COMMUNICATION,
                         mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0,
                         true, c_ReceiveData, u8_NrErrorCode);
  }
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::SetRouteDiagnosisCommunication(InputType: %d, "
        "InputIndex: %d, "
        "OutputType: %d, OutputIndex: %d, SourceId: %d, TargetId: %d)",
        ou8_InputChannelType, ou8_InputChannelIndex, ou8_OutputChannelType,
        ou8_OutputChannelIndex, ou8_SourceBusId, ou8_TargetBusId);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Route diagnosis communication deactivation service request
   implementation
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyStopRouteDiagnosisCommunication(
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_ROUTE_DIAGNOSIS_COMMUNICATION,
                       mhu8_OSY_RC_SUB_FUNCTION_STOP_ROUTINE, c_SendData, 0,
                       true, c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
  m_LogServiceError("RoutineControl::StopRouteDiagnosisCommunication",
                    s32_Return, u8_NrErrorCode);
...
/*! \brief   Activates the IP to IP routing for a specific node
   ...
   Caution: The router node will drop out of its session after the check. All
   previous activated CAN routes will be dropped on the router node!
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySetRouteIp2IpCommunication(
    const uint8_t ou8_OutputChannelType, const uint8_t ou8_OutputChannelIndex,
    const uint8_t ou8_SourceBusId, const uint8_t ou8_TargetBusId,
    const uint8_t (&orau8_IpAddress)[4], uint8_t *const opu8_NrCode) {
  int32_t s32_Return = C_RANGE;
  uint8_t u8_NrErrorCode = 0U;
  ...
  if ((ou8_SourceBusId <= C_OscProtocolDriverOsyNode::mhu8_MAX_BUS) &&
    ...
    s32_Return =
        m_RoutineControl(mhu16_OSY_RC_SID_ROUTE_IP_2_IP_COMMUNICATION,
                         mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0,
                         true, c_ReceiveData, u8_NrErrorCode);
  }
  if (opu8_NrCode != NULL) {
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::OsySetRouteIp2IpCommunication("
        "OutputType: %d, OutputIndex: %d, SourceId: %d, TargetId: %d, "
        "IP: [%d.%d.%d.%d])",
        ou8_OutputChannelType, ou8_OutputChannelIndex, ou8_SourceBusId,
        ou8_TargetBusId, orau8_IpAddress[0], orau8_IpAddress[1],
        orau8_IpAddress[2], orau8_IpAddress[3]);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
/*! \brief   Check status of IP to IP routing
   ...
   \param[out] oru8_Status        Status information of IP to IP routing
                                    ...
                                    1: in progress (requested activation in
   progress) 2: error (requests routing activation failed) 3: connected
   (requested activation finished) 4..255: reserved
   \param[out] opu8_NrCode        if != NULL: negative response code in case of
   an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyCheckRouteIp2IpCommunication(
    uint8_t &oru8_Status, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;
...
/*! \brief   StartRoutine: SendCanMessage service implementation
   ...
   See class description for general handling of "polled" services.
   Payload data of the CAN message which are not used (DLC configuration) are
   set to 0.
   ...
   C_NO_ERR   request sent, positive response received
   C_TIMEOUT  expected response not received within timeout
   C_RANGE    CAN message invalid (RTR bit set; ID out of range; DLC out of
   range) C_NOACT    could not put request in Tx queue ... C_CONFIG   no
   transport protocol installed C_WARN     error response (negative response
   code placed in *opu8_NrCode) C_RD_WR    unexpected content in response (here:
   wrong data identifier ID) C_COM      communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySendCanMessage(
  ...
  if ((orc_CanMessage.u8_RTR != 0U) || (orc_CanMessage.u8_XTD > 1U) ||
      ...
      ((orc_CanMessage.u8_XTD == 1U) &&
       (orc_CanMessage.u32_ID > 0x3FFFFFFFU)) ||
    ...
    (void)std::memcpy(&c_SendData[5], &orc_CanMessage.au8_Data[0],
                      orc_CanMessage.u8_DLC);
    // keep a clean (reproducible) message; set empty bytes to zero
    (void)std::memset(
        &c_SendData[5U + static_cast<size_t>(orc_CanMessage.u8_DLC)], 0x00U,
    ...
    s32_Return =
        m_RoutineControl(mhu16_OSY_RC_SID_SEND_CAN_MESSAGE,
                         mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0U,
                         true, c_ReceiveData, u8_NrErrorCode, true);
  }
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::SendCanMessage(Channel: %d, Id: 0x%08X, DLC: %d)",
        ou8_ChannelIndex, orc_CanMessage.u32_ID, orc_CanMessage.u8_DLC);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
  return s32_Return;
}
...
/*! \brief   Tunnel can messages activation service request implementation
   ...
   \param[in]     ou32_FilterId           Filter id for all relevant IDs
                                          The MSB is the flag for 11 Bit (= 0)
   or 29 Bit (= 1) Bits 29 and 30 are reserved (shall be set to zero)
   \param[in]     ou32_FilterMask         Mask that is applied to both the
   received CAN frame identifiers an the FilterId before comparison. Bits 29, 30
   and 31 are reserved (shall be set to zero)
   \param[out]    opu8_NrCode             if != NULL: negative response code in
   case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySetTunnelCanMessages(
    const uint8_t ou8_CanChannelIndex, const uint32_t ou32_FilterId,
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_TUNNEL_CAN_MESSAGE,
                       mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0,
                       true, c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
...
C_OscProtocolDriverOsy::OsyStopTunnelCanMessages(uint8_t *const opu8_NrCode) {
  ...
  if (s32_Return != C_NO_ERR) {
    m_LogServiceError("RoutineControl::StopTunnelCanMessages", s32_Return,
                      u8_NrErrorCode);
  }
...
/*! \brief   Print logging text
   ...
   Add information about this class and the current server-id configuration and
   call logger to perform the actual logging.
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::m_LogErrorWithHeader(
    const QString &orc_Activity, const QString &orc_Information,
    const char_t *const opcn_Function, const bool oq_AsError) const {
  const QString c_LogText = QString("openSYDE protocol driver node %1.%2: %3")
                                .arg(mc_ServerId.u8_BusIdentifier)
                                .arg(mc_ServerId.u8_NodeIdentifier)
                                .arg(orc_Information);

  if (oq_AsError == true) {
    C_OscLoggingHandler::h_WriteLogError(orc_Activity, c_LogText, __FILE__,
                                         opcn_Function);
  } else {
    C_OscLoggingHandler::h_WriteLogWarning(orc_Activity, c_LogText, __FILE__,
                                           opcn_Function);
  }
}
...
/*! \brief   Make transport protocol to use known
   ...
   The passed protocol will be remembered for further use and the client and
   server addresses will be assigned to the newly installed protocol.

   \param[in] opc_TransportProtocol   transport protocol to use; NULL: do not
   use protocol

   \return
   C_NO_ERR   no problems
   C_CONFIG   transport protocol was set, but could not set node identifiers in
   installed protocol
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::SetTransportProtocol(
    C_OscProtocolDriverOsyTpBase *const opc_TransportProtocol) {
  int32_t s32_Return = C_NO_ERR;

  mpc_TransportProtocol = opc_TransportProtocol;
  if (mpc_TransportProtocol != NULL) {
    s32_Return =
        mpc_TransportProtocol->SetNodeIdentifiers(mc_ClientId, mc_ServerId);
    if (s32_Return != C_NO_ERR) {
      s32_Return = C_CONFIG;
    }
  }
...
/*! \brief   Make client and server identifiers known
   ...
   If a transport protocol ist installed the new identifiers will be set there,
   too.
   ...
   \return
   C_NO_ERR   no problems
   C_CONFIG   IDs were set, but could not be propagated to the installed
   transport protocol
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::SetNodeIdentifiers(
    const C_OscProtocolDriverOsyNode &orc_ClientId,
...
C_OscProtocolDriverOsy::GetTransportProtocol(void) {
  return mpc_TransportProtocol; // lint !e1535  TP is still owned by us; caller
                                // is responsible not to mess things up
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::GetNodeIdentifiers(
    C_OscProtocolDriverOsyNode &orc_ClientId,
...
/*! \brief   Registers a Datapool index mapping
   ...
   The key of the map is the Datapool index on client side defined in the
   project. The value of the map is the Datapool index on server side which is
...
/*! \brief   Perform cyclic communication tasks
   ...
   * Invoke virtual functions to report all incoming event driven responses.
     ...
     otherwise it will continue to read all incoming responses from the Rx
   queue.
...
int32_t C_OscProtocolDriverOsy::Cycle(void) {
  ...
  // while we are checking for async responses, prevent other threads starting
  // to poll
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscProtocolDriverOsy::m_OsyReadDataPoolDataEventErrorReceived(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyCheckFlashMemoryAvailable(
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_CHECK_FLASH_MEMORY_AVAILABILITY,
                       mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0U,
                       true, c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
...
/*! \brief   RoutineControl utility wrapper
   ...
   \param[in]  ou8_SubFunction            1: start routine, 2: stop routine, 3:
   ...
   \param[out] oru8_NrCode                negative response code in case of an
   error response
   \param[in]  oq_CanTransferWithoutFlowControl   flag if openSYDE specific
   frame shall be used for the service
...
int32_t C_OscProtocolDriverOsy::m_RoutineControl(
  ...
  C_OscProtocolDriverOsyService c_Request;
  C_OscProtocolDriverOsyService c_Response;
  const uint32_t u32_SendPayloadSize =
      static_cast<uint32_t>(orc_SendData.size());
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    c_Request.q_CanTransferWithoutFlowControl =
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      uint8_t u8_NrErrorCode;
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_ROUTINE_CONTROL, ou16_ExpectedPayloadSize + 4U,
          c_Response, u8_NrErrorCode, oq_ExactSizeExpected);
      switch (s32_Return) {
      case C_NO_ERR:
        // check response:
        if ((c_Request.c_Data[1] != c_Response.c_Data[1]) ||
          ...
          orc_ReadData.resize(c_Response.c_Data.size() - 4);
          // don't execute memcpy if payload size is 0; depending on the
...
/*! \brief   Security access - Request Seed service implementation
   ...
   C_CONFIG   no transport protocol installed
   C_WARN     error response (negative response code placed in *opu8_NrCode)
   C_RD_WR    unexpected content in response (here: wrong data identifier ID or
   unexpected size) C_COM      communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySecurityAccessRequestSeed(
    const uint8_t ou8_SecurityLevel, bool &orq_SecureMode, uint64_t &oru64_Seed,
  ...
  s32_Return = m_SecurityAccess(ou8_SecurityLevel, c_SendData, 0U, 4U,
                                c_ReceiveData, u8_NrErrorCode);
...
int32_t C_OscProtocolDriverOsy::OsySecurityAccessSendKey(
    const uint8_t ou8_SecurityLevel, const uint32_t ou32_Key,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  s32_Return = m_SecurityAccess(ou8_SecurityLevel + 1U, c_SendData, 4U, 0U,
                                c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
...
int32_t C_OscProtocolDriverOsy::OsySecurityAccessSendKey(
  ...
  s32_Return = m_SecurityAccess(ou8_SecurityLevel + 1U, orc_Key,
                                static_cast<uint16_t>(orc_Key.size()), 0U,
                                c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
...
/*! \brief   Security access utility wrapper
   ...
   The function either request the seed from server or send the calculated key
   to the server.
...
int32_t C_OscProtocolDriverOsy::m_SecurityAccess(
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      uint8_t u8_NrErrorCode;
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_SECURITY_ACCESS, ou16_ExpectedPayloadSize + 2U,
          c_Response, u8_NrErrorCode, false);
      switch (s32_Return) {
      case C_NO_ERR:
        // check response:
        if ((c_Request.c_Data[1] != c_Response.c_Data[1])) {
...
/*! \brief   Handle incoming async response
   ...
   * ReadSerialNumber (flags a server restart)
   ...
   \param[in]  orc_ReceivedService        received service suspected to be an
   async response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::m_HandleAsyncResponse(
  ...
  if ((orc_ReceivedService.c_Data[0] & 0x40U) == 0U) {
    m_LogErrorWithHeader(
        "Asynchronous communication",
        "Unexpectedly received service request (not response). Ignoring.",
        TGL_UTIL_FUNC_ID);
  } else {
    // Check for error message
    if (orc_ReceivedService.c_Data[0] != 0x7FU) {
      const uint8_t u8_ServiceId =
          (orc_ReceivedService.c_Data[0] & 0xBFU); // filter out "response" bit
      switch (u8_ServiceId) {
      case mhu8_OSY_SI_READ_DATA_POOL_DATA_EVENT_DRIVEN:
        s32_Return =
            this->m_HandleAsyncOsyReadDataPoolDataEvent(orc_ReceivedService);
        break;
      case mhu8_OSY_SI_TUNNEL_CAN_MESSAGE:
        s32_Return =
            this->m_HandleAsyncOsyTunnelCanMessagesEvent(orc_ReceivedService);
        break;
      case mhu8_OSY_SI_READ_SERIAL_NUMBER:
        ...
        m_LogErrorWithHeader("Asynchronous communication",
                             "Received read serial number response (possibly "
                             "startup message). Ignoring.",
                             TGL_UTIL_FUNC_ID);
        break;
      default:
      ...
      case mhu8_OSY_SI_READ_DATA_POOL_DATA_EVENT_DRIVEN:
        s32_Return = this->m_HandleAsyncOsyReadDataPoolDataErrorEvent(
            orc_ReceivedService);
        break;
      case mhu8_OSY_SI_READ_SERIAL_NUMBER:
        m_LogErrorWithHeader("Asynchronous communication",
...
int32_t C_OscProtocolDriverOsy::m_HandleAsyncOsyReadDataPoolDataEvent(
  ...
  if (orc_ReceivedService.c_Data.size() > 4) {
    ...
    const uint16_t u16_NumberOfBytes =
        static_cast<uint16_t>(orc_ReceivedService.c_Data.size()) - 4U;
    uint8_t au8_Identifier[3];
    ...
    m_UnpackDataPoolIdentifier(au8_Identifier, u8_DataPoolIndex, u16_ListIndex,
                               u16_ElementIndex);

    m_OsyReadDataPoolDataEventReceived(u8_DataPoolIndex, u16_ListIndex,
                                       u16_ElementIndex, c_Value);

    s32_Return = C_NO_ERR;
  } else {
    m_LogErrorWithHeader(
        "Asynchronous communication",
        "Received async ReadDataPoolDataEvent with invalid length. Ignoring.",
        TGL_UTIL_FUNC_ID);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Handle incoming async error response of service
   OsyReadDataPoolDataEvent

   Check the negative response only if the additional information with the
   datapool index is available. If a negative response was received without the
   indexes and this service, it was caused by the protocol mechanisms.
...
int32_t C_OscProtocolDriverOsy::m_HandleAsyncOsyReadDataPoolDataErrorEvent(
  ...
  if (orc_ReceivedService.c_Data.size() == 6) {
    ...
    m_OsyReadDataPoolDataEventErrorReceived(u8_DataPoolIndex, u16_ListIndex,
                                            u16_ElementIndex,
    ...
    m_LogErrorWithHeader("Asynchronous communication",
                         "Received async ReadDataPoolDataErrorEvent with "
                         "invalid length. Ignoring.",
                         TGL_UTIL_FUNC_ID);
  }
...
int32_t C_OscProtocolDriverOsy::m_HandleAsyncOsyTunnelCanMessagesEvent(
  ...
  if (orc_ReceivedService.c_Data.size() == 15U) {
    ...
    c_CanMessage.u8_XTD =
        ((orc_ReceivedService.c_Data[1] & 0x80U) == 0x80U) ? 1U : 0U;
    // Bits 30 and 29 are reserved
    c_CanMessage.u32_ID =
        ((static_cast<uint32_t>(orc_ReceivedService.c_Data[1]) & 0x1FU)
         << 24U) +
    ...
    if ((c_CanMessage.u8_XTD == 1U) || (c_CanMessage.u32_ID <= 0x7FFU)) {
      ...
      if (this->mpr_OnOsyTunnelCanMessageReceived != NULL) {
        this->mpr_OnOsyTunnelCanMessageReceived(
            this->mpv_OnAsyncTunnelCanMessageInstance,
    ...
    m_LogErrorWithHeader(
        "Asynchronous communication",
        "Received async TunnelCanMessagesEvent with invalid length. Ignoring.",
        TGL_UTIL_FUNC_ID);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint8_t C_OscProtocolDriverOsy::m_GetDataPoolIndexClientToServer(
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint8_t C_OscProtocolDriverOsy::m_GetDataPoolIndexServerToClient(
...
/*! \brief   Request Download service implementation
   ...
   Data format is set to 0x00 (no encryption and no compression on transferred
   data).

   \param[in]  ou32_StartAddress     first address in flash to write to
   \param[in]  ou32_Size             number of bytes to be written to flash
   memory
   \param[out] oru32_MaxBlockLength  maximum number of data bytes for a single
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyRequestDownload(
    const uint32_t ou32_StartAddress, const uint32_t ou32_Size,
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_REQUEST_DOWNLOAD, 3U, c_Response, u8_NrErrorCode, false);
      ...
      case C_NO_ERR:
        ...
        if (((u8_LengthFormat == 0U) || (u8_LengthFormat > 4U)) ||
            (c_Response.c_Data.size() !=
             (2U + static_cast<size_t>(u8_LengthFormat)))) {
          s32_Return = C_RD_WR;
        } else {
          uint8_t u8_Index;
          oru32_MaxBlockLength = 0U;
          for (u8_Index = 0U; u8_Index < u8_LengthFormat; u8_Index++) {
            oru32_MaxBlockLength +=
                (static_cast<uint32_t>(
                     c_Response.c_Data[2U + static_cast<size_t>(u8_Index)])
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RequestDownload(Address: 0x%08X, Size: 0x%08X, MaxBlockLength: %u)",
...
/*! \brief   Request File Transfer service implementation
   ...
   Initiate a file transfer sequence. Must be called before service
   TransferData. Size is fixed to 4bytes. Mode of operation is set to 0x03
   ("replace file") Data format is set to 0x00 (no encryption and no compression
   of transferred data).
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyRequestFileTransfer(
    const QString &orc_FilePath, const uint32_t ou32_FileSize,
    uint32_t &oru32_MaxBlockLength, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  C_OscProtocolDriverOsyService c_Request;
  C_OscProtocolDriverOsyService c_Response;
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    const uint16_t u16_PathLength =
        static_cast<uint16_t>(orc_FilePath.length());
    ...
    (void)std::memcpy(&c_Request.c_Data[4], orc_FilePath.toUtf8().constData(),
                      u16_PathLength);
    c_Request.c_Data[4 + static_cast<size_t>(u16_PathLength)] =
        0x00U; // no compression; no encryption
    c_Request.c_Data[5 + static_cast<size_t>(u16_PathLength)] =
        0x04U; // 4 bytes with address information
    c_Request.c_Data[6 + static_cast<size_t>(u16_PathLength)] =
        static_cast<uint8_t>(ou32_FileSize >> 24U);
    c_Request.c_Data[7 + static_cast<size_t>(u16_PathLength)] =
        static_cast<uint8_t>(ou32_FileSize >> 16U);
    c_Request.c_Data[8 + static_cast<size_t>(u16_PathLength)] =
        static_cast<uint8_t>(ou32_FileSize >> 8U);
    c_Request.c_Data[9 + static_cast<size_t>(u16_PathLength)] =
        static_cast<uint8_t>(ou32_FileSize);
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      uint8_t u8_LengthFormat;
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_REQUEST_FILE_TRANSFER, 4U, c_Response, u8_NrErrorCode,
          false);
      switch (s32_Return) {
      case C_NO_ERR:
        // check response:
        ...
        if ((c_Response.c_Data[1] != 0x03U) || // mode of operation OK ?
            ((u8_LengthFormat == 0U) ||
             (u8_LengthFormat > 4U)) || // valid length format ?
            (c_Response.c_Data.size() !=
             (4U +
              static_cast<size_t>(u8_LengthFormat))) || // enough data bytes ?
            (c_Response.c_Data[static_cast<size_t>(u8_LengthFormat) + 3U] !=
             0x00U)) // data format OK ?
        {
          s32_Return = C_RD_WR;
        } else {
          uint8_t u8_Index;
          ...
          for (u8_Index = 0U; u8_Index < u8_LengthFormat; u8_Index++) {
            oru32_MaxBlockLength +=
                (static_cast<uint32_t>(
                     c_Response.c_Data[3U + static_cast<size_t>(u8_Index)])
                 << (((u8_LengthFormat - 1U) - u8_Index) * 8U));
          }
        }
        break;
...
/*! \brief   Transfer Data service implementation
   ...
   Send data in ascending order to be programmed to flash memory starting at
   address set by service RequestDownload. The maximum number of bytes
   transferred is limited by the buffer size provided by the response message of
   service RequestDownload.

   \param[in]  ou8_BlockSequenceCounter   sequence counter 0-255; first black
   starts with 1
...
C_OscProtocolDriverOsy::OsyTransferData(const uint8_t ou8_BlockSequenceCounter,
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      C_OscProtocolDriverOsyService c_Response;
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_TRANSFER_DATA, 2U, c_Response, u8_NrErrorCode, true);
      switch (s32_Return) {
      case C_NO_ERR:
        // check response:
...
/*! \brief   Request Transfer Exit service implementation based on address
   ...
   Finalize a programming sequence. Must be called after last service
   TransferData. The signature block must be located in the previously
   programmed range of flash memory.

   \param[in]  oq_SendSignatureBlockAddress  true: it's the last area of a
   block; send the signature address false: more areas will follow; do not send
   the signature address
   \param[in]  ou32_SignatureBlockAddress    address of signature block in flash
   covering the application/data previously programmed by service TransferData.
                                             (only used if
   oq_SendSignatureBlockAddress is true)
   \param[out] opu8_NrCode                   if != NULL: negative response code
   in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyRequestTransferExitAddressBased(
    const bool oq_SendSignatureBlockAddress,
    const uint32_t ou32_SignatureBlockAddress, uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  C_OscProtocolDriverOsyService c_Request;
  C_OscProtocolDriverOsyService c_Response;
  uint8_t u8_NrErrorCode = 0U;
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (oq_SendSignatureBlockAddress == true) {
      ...
      c_Request.c_Data[1] =
          static_cast<uint8_t>(ou32_SignatureBlockAddress >> 24U);
      c_Request.c_Data[2] =
          static_cast<uint8_t>(ou32_SignatureBlockAddress >> 16U);
      c_Request.c_Data[3] =
          static_cast<uint8_t>(ou32_SignatureBlockAddress >> 8U);
      c_Request.c_Data[4] =
          static_cast<uint8_t>(ou32_SignatureBlockAddress & 0xFFU);
    } else {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      s32_Return = m_PollForSpecificServiceResponse(
          mhu8_OSY_SI_REQUEST_TRANSFER_EXIT, 1U, c_Response, u8_NrErrorCode,
          true);
      switch (s32_Return) {
      case C_NO_ERR:
        // check response:
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RequestTransferExitAddressBased(WithSignatureAddress: %d, "
        "SignatureAddress: 0x%08X)",
        (oq_SendSignatureBlockAddress == true) ? 1 : 0,
        ou32_SignatureBlockAddress);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
/*! \brief   Request Transfer Exit service implementation based on file system
   ...
   \param[in]  orau8_Signature               calculated signature
                                             previously programmed by service
   TransferData.
   \param[out] opu8_NrCode                   if != NULL: negative response code
   in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyRequestTransferExitFileBased(
    const uint8_t (&orau8_Signature)[8], uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  C_OscProtocolDriverOsyService c_Request;
  C_OscProtocolDriverOsyService c_Response;
  uint8_t u8_NrErrorCode = 0U;
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    for (u32_Counter = 0U; u32_Counter < 8U; ++u32_Counter) {
      c_Request.c_Data[static_cast<size_t>(u32_Counter) + 1U] =
          orau8_Signature[u32_Counter];
    }
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RequestTransferExitFileBased(Signature: [%d,%d,%d,%d,%d,%d,%d,%d]",
        orau8_Signature[0], orau8_Signature[1], orau8_Signature[2],
        orau8_Signature[3], orau8_Signature[4], orau8_Signature[5],
        orau8_Signature[6], orau8_Signature[7]);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
/*! \brief   Read NVM memory by address
   ...
   As the openSYDE protocol can only handle transfers up to 4kB bigger packages
   are split up into multiple services (each transferring
   (C_OscProtocolDriverOsyTpBase::hu16_OSY_MAXIMUM_SERVICE_SIZE - 10) bytes).

   \param[in]     ou32_MemoryAddress NVM memory address to read (first read
   byte)
   ...
   C_COM      expected server response not received because of communication
   error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyReadMemoryByAddress(
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    // 1..4 bytes address
    // 1..4 bytes size
    const uint32_t u32_BLOCK_SIZE =
        (C_OscProtocolDriverOsyTpBase::hu16_OSY_MAXIMUM_SERVICE_SIZE - 10U);
    ...
    for (uint32_t u32_ReadIndex = 0U; u32_ReadIndex < orc_DataRecord.size();
      ...
      uint32_t u32_Size =
          static_cast<uint32_t>(orc_DataRecord.size() - u32_ReadIndex);
      if (u32_Size > u32_BLOCK_SIZE) {
        u32_Size = u32_BLOCK_SIZE;
      }

      // Convert to bytes
      C_OscProtocolDriverOsy::mh_ConvertVariableToNecessaryBytes(
          u32_MemoryAddress, c_MemoryAddress);
      C_OscProtocolDriverOsy::mh_ConvertVariableToNecessaryBytes(u32_Size,
                                                                 c_MemorySize);
      ...
      // Request
      c_Request.c_Data.resize(
          2U + static_cast<size_t>(
                   static_cast<size_t>(u8_MemoryAddressByteCount) +
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        C_OscProtocolDriverOsyService c_Response;
        s32_Return = m_PollForSpecificServiceResponse(
            mhu8_OSY_SI_READ_MEMORY_BY_ADDRESS,
            static_cast<uint16_t>(1UL + u32_Size), c_Response, u8_NrErrorCode,
            true);
        switch (s32_Return) {
        case C_NO_ERR:
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "ReadMemoryByAddress(Address: 0x%08X, Size: 0x%08X)",
        ou32_MemoryAddress, static_cast<uint32_t>(orc_DataRecord.size()));
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
/*! \brief   Write NVM memory by address
   ...
   The maximum size of transfers the openSYDE protocol can handle in one package
   is 4kB. This can be further limited by the server's capabilities. A client
   can check for this limit using OsyReadMaxNumberOfBlockLength().

   This function uses "mu16_MaxServiceSize" to split the individual write
   services into suitable chunks.

   \param[in]  ou32_MemoryAddress   NVM memory address to write to (first
   written byte)
   ...
   C_CONFIG   no transport protocol installed
   C_WARN     error response (negative response code placed in *opu8_NrCode)
   C_RD_WR    unexpected content in response (here: unexpected length or
   address) C_RANGE    orc_DataRecord empty C_COM      expected server response
   not received because of communication error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyWriteMemoryByAddress(
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    // 1..4 bytes address
    // 1..4 bytes size
    const uint32_t u32_BlockSize =
        static_cast<uint32_t>(mu16_MaxServiceSize) - 10U;
    ...
    for (uint32_t u32_WriteIndex = 0U; u32_WriteIndex < orc_DataRecord.size();
      ...
      uint32_t u32_Size =
          static_cast<uint32_t>(orc_DataRecord.size() - u32_WriteIndex);
      ...
      // Request
      c_Request.c_Data.resize(
          2U + (((static_cast<size_t>(u8_MemoryAddressByteCount) +
                  static_cast<size_t>(u8_MemorySizeByteCount)) +
                 u32_Size)),
          0U);
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        C_OscProtocolDriverOsyService c_Response;
        int x_Diff; // lint !e8080 !e970  //using type to match library
                    // interface
        const uint8_t u8_ExpectedSize =
            2U + u8_MemoryAddressByteCount + u8_MemorySizeByteCount;

        s32_Return = m_PollForSpecificServiceResponse(
            mhu8_OSY_SI_WRITE_MEMORY_BY_ADDRESS, u8_ExpectedSize, c_Response,
            u8_NrErrorCode, true);
        switch (s32_Return) {
        case C_NO_ERR:
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "WriteMemoryByAddress(Address: 0x%08X, Size: 0x%08X)",
        ou32_MemoryAddress, static_cast<uint32_t>(orc_DataRecord.size()));
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
/*! \brief   Notify server of changes in NVM
   ...
   \param[out] orq_ApplicationAcknowledge  true: positive acknowledge from
   server false: negative acknowledge from server
   \param[out] opu8_NrCode                 if != NULL: negative response code in
   case of an error response
   ...
   C_CONFIG   no transport protocol installed
   C_WARN     error response (negative response code placed in *opu8_NrCode)
   C_RD_WR    unexpected content in response (here: wrong data pool or list
   index) C_COM      communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyNotifyNvmDataChanges(
    const uint8_t ou8_DataPoolIndex, const uint8_t ou8_ListIndex,
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_NOTIFY_NVM_DATA_CHANGED,
                       mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 3,
                       true, c_ReceiveData, u8_NrErrorCode);
  if (s32_Return == C_NO_ERR) {
    if ((c_ReceiveData[0] == ou8_ServerDpIndex) &&
        (c_ReceiveData[1] == ou8_ListIndex)) {
      orq_ApplicationAcknowledge =
          ((c_ReceiveData[2] & 0x01U) == 0x01U) ? false : true;
    } else {
      s32_Return = C_RD_WR;
    }
  }
...
/*! \brief   Tester Present service implementation
   ...
   Send the tester present service to indicate to the server that the client is
   still active. Sub-function byte is always zero. Usually the
   suppressPosRspMsgIndicationBit may be set to avoid flooding the bus with
   response messages.

   \param[in]  ou8_SuppressResponseMsg    0: server shall send a response
   message 1: server shall send no response message
   \param[out] opu8_NrCode                if != NULL: negative response code in
   case of an error response
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyTesterPresent(const uint8_t ou8_SuppressResponseMsg,
                                         uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  C_OscProtocolDriverOsyService c_Request;
  C_OscProtocolDriverOsyService c_Response;
  ...
  if (mpc_TransportProtocol == NULL) {
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      if (ou8_SuppressResponseMsg == 0U) {
        s32_Return = m_PollForSpecificServiceResponse(
            mhu8_OSY_SI_TESTER_PRESENT, 2U, c_Response, u8_NrErrorCode, true);
...
/*! \brief   ECU Reset service implementation
   ...
   \param[in]  ou8_ResetType        Reset type (Default by openSYDE protocol:
   0x02: keyOffOnReset)
...
int32_t C_OscProtocolDriverOsy::OsyEcuReset(const uint8_t ou8_ResetType) {
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf("EcuReset(Type: %d)", ou8_ResetType);
...
/*! \brief   SetNodeIdForChannel service implementation
   ...
   Request the server to reconfigure the settings for node id and bus id for the
   provided channel.

   Send format: 2 bytes channel identifier, 2 bytes new node identifier
   Example: (0x00, 0x03, 0x01, 0x09) -> , type:CAN, channel:3, bus id:1, node
   id:9

   \param[in]   ou8_ChannelType     selected channel type (0 equals CAN, 1
   equals Ethernet)
   \param[in]   ou8_ChannelIndex    selected channel index
   \param[in]   orc_NewNodeId       new bus id (0..15) and node id (0..126)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySetNodeIdForChannel(
  ...
  if ((orc_NewNodeId.u8_BusIdentifier <=
    ...
    s32_Return =
        m_RoutineControl(mhu16_OSY_RC_SID_SET_NODE_ID_FOR_CHANNEL,
                         mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0U,
                         true, c_ReceiveData, u8_NrErrorCode);
  }
...
/*! \brief   SetBitrate service implementation
   ...
   Request the server to reconfigure the setting for bitrate for the provided
   channel.

   Send format:  2 bytes channel identifier, 4 bytes new bitrate
   Example: (0x00, 0x03, 0x00, 0x0F, 0x42, 0x40) -> , type:CAN, channel:3,
   bitrate:1000000

   \param[in]  ou8_ChannelType       selected channel type (0 equals CAN, 1
   equals Ethernet)
   \param[in]  ou8_ChannelIndex      selected channel index
   \param[in]  ou32_Bitrate          new bitrate configuration for the server in
   bit/s
   \param[out] opu8_NrCode           if != NULL: negative response code in case
   of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySetBitrate(const uint8_t ou8_ChannelType,
                                              const uint8_t ou8_ChannelIndex,
  ...
  s32_Return = m_RoutineControl(
      mhu16_OSY_RC_RC_SID_SET_BITRATE, mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE,
      c_SendData, 0U, true, c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::SetBitrate(Type: %d, Index: %d, Bitrate: %u)",
...
/*! \brief   ReadFlashBlockData service implementation
   ...
   Request from server the application information for the selected block in
   flash memory on an address based server. Flash block index 0 requests
   information about the flashloader.

   To read out information about all flash blocks present on the server call
   this function in a loop starting from block index 0. Stop if the server
   responds with "requestOutOfRange".
   ...
   \param[out] orc_BlockInfo         structure to store flash block info
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyReadFlashBlockData(const uint8_t ou8_FlashBlock,
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_READ_FLASH_BLOCK_DATA,
                       mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0U,
                       false, c_ReceiveData, u8_NrErrorCode);
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_ReceiveData[u32_Counter] ==
        C_FlashBlockInfo::hu8_ID_BLOCK_ADDRESSES) {
      const size_t x_Offset = static_cast<size_t>(
          u32_Counter); // lint !e8080 //using correct type for vector index

      orc_BlockInfo.u32_BlockStartAddress =
          ((static_cast<uint32_t>(c_ReceiveData[x_Offset + 1U])) << 24U) +
      ...
      orc_BlockInfo.u32_BlockEndAddress =
          ((static_cast<uint32_t>(c_ReceiveData[x_Offset + 5U])) << 24U) +
    ...
    if (c_ReceiveData[u32_Counter] ==
        C_FlashBlockInfo::hu8_ID_RESULT_SIGNATURE) {
      orc_BlockInfo.u8_SignatureValid =
          c_ReceiveData[static_cast<size_t>(u32_Counter) + 1U];
      u32_Counter += 2U;
    }
    if (c_ReceiveData[u32_Counter] ==
        C_FlashBlockInfo::hu8_ID_APPLICATION_VERSION) {
      const uint32_t u32_Length = static_cast<uint32_t>(
          c_ReceiveData[static_cast<size_t>(u32_Counter) + 1U]);
    ...
    if (c_ReceiveData[u32_Counter] ==
        C_FlashBlockInfo::hu8_ID_APPLICATION_NAME) {
      const uint32_t u32_Length = static_cast<uint32_t>(
          c_ReceiveData[static_cast<size_t>(u32_Counter) + 1U]);
      c_Text.resize(static_cast<size_t>(u32_Length) +
                    1U);                 // plus 1 for termination
    ...
    if (c_ReceiveData[u32_Counter] ==
        C_FlashBlockInfo::hu8_ID_ADDITIONAL_INFORMATION) {
      const uint32_t u32_Length = static_cast<uint32_t>(
          c_ReceiveData[static_cast<size_t>(u32_Counter) + 1U]);
      c_Text.resize(static_cast<size_t>(u32_Length) +
                    1U);                 // plus 1 for termination
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscProtocolDriverOsy::OsyRequestProgramming(uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  uint8_t u8_NrErrorCode = 0U;
  ...
  if (s32_Return != C_NO_ERR) {
    m_LogServiceError("RoutineControl::RequestProgramming", s32_Return,
                      u8_NrErrorCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   This service is for activating/deactivating a CAN/Ethernet channel
   for flashloader communication use

   \param[in]  ou8_ChannelType  selected channel type (0 equals CAN, 1 equals
   Ethernet)
   \param[in]  ou8_ChannelIndex selected channel index
   \param[in]  oq_Activated     input parameter description
   \param[out] opu8_NrCode      if != NULL: negative response code in case of an
   error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsyConfigureFlashloaderCommunicationChannel(
    const uint8_t ou8_ChannelType, const uint8_t ou8_ChannelIndex,
  ...
  s32_Return = m_RoutineControl(
      mhu16_OSY_RC_SID_CONFIGURE_FLASHLOADER_COMMUNICATION_CHANNEL,
      mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0, true,
      c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  }
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::ConfigureFlashloaderCommunicationChannel(Type: %d, "
        "Index: %d, Active: %d)",
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProtocolDriverOsy::OsySetIpAddressForChannel(
    const uint8_t ou8_ChannelType, const uint8_t ou8_ChannelIndex,
  ...
  s32_Return =
      m_RoutineControl(mhu16_OSY_RC_SID_SET_IP_ADDRESS_FOR_CHANNEL,
                       mhu8_OSY_RC_SUB_FUNCTION_START_ROUTINE, c_SendData, 0,
                       true, c_ReceiveData, u8_NrErrorCode);
  if (opu8_NrCode != NULL) {
    (*opu8_NrCode) = u8_NrErrorCode;
  ...
  if (s32_Return != C_NO_ERR) {
    QString c_ErrorText;
    c_ErrorText = QString::asprintf(
        "RoutineControl::SetIpAddressForChannel(Type: %d, Index: %d, IP: "
        "[%d.%d.%d.%d], "
        " NetMask: [%d.%d.%d.%d], Gateway: [%d.%d.%d.%d]",
        ou8_ChannelType, ou8_ChannelIndex, orau8_IpAddress[0],
        orau8_IpAddress[1], orau8_IpAddress[2], orau8_IpAddress[3],
        orau8_NetMask[0], orau8_NetMask[1], orau8_NetMask[2], orau8_NetMask[3],
        orau8_DefaultGateway[0], orau8_DefaultGateway[1],
        orau8_DefaultGateway[2], orau8_DefaultGateway[3]);
    m_LogServiceError(c_ErrorText, s32_Return, u8_NrErrorCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Utility: create textual representation of openSYDE protocol service
   result

   \param[in]  os32_FunctionResult   error result as returned by openSYDE
   protocol driver service function
   \param[in]  ou8_NrCode            negative response code received
   \param[out] opq_IsHardError       set by function if not NULL:
                                     false: service was performed but an error
   response was received true: all other errors

   \return
   string representation
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
    const int32_t os32_FunctionResult, const uint8_t ou8_NrCode,
  ...
  case C_WARN:
    ...
    case (C_OscProtocolDriverOsy::hu8_NR_CODE_SUB_FUNCTION_NOT_SUPPORTED):
      ...
      break;
    case (
        C_OscProtocolDriverOsy::hu8_NR_CODE_INCORRECT_MESSAGE_LENGTH_OR_FORMAT):
    ...
    case (C_OscProtocolDriverOsy::hu8_NR_CODE_GENERAL_PROGRAMMING_FAILURE):
      ...
      break;
    case (C_OscProtocolDriverOsy::
              hu8_NR_CODE_SERVICE_NOT_SUPPORTED_IN_ACTIVE_SESSION):
  ...
  case C_RD_WR:
    ...
    break;
  case C_OVERFLOW:
    c_Text = "Data overflow";
    break;
  case C_RANGE:
    c_Text = "Parameter out of range";
    break;
  default:
