...
/*!
   ...
   Concept for handling of incoming responses when waiting for a specific
   service response:
   ...
   Unfortunately the pre-existing implementation of the KEFEX protocol does not
   allow to clearly identify error reasons. We can clearly differentiate
   between:
   * no error (C_NO_ERR)
   * error response (C_WARN)
   The other error codes cannot necessarily be mapped to correct error codes
   defined by the base class of this class. See the individual function headers
   for details.
...
//----------------------------------------------------------------------------------------------------------------------
/// interface class
class C_OscDiagProtocolKfx : public C_OscDiagProtocolBase {
private:
  ...
  operator=(const C_OscDiagProtocolKfx
                &orc_Source); // lint !e1511 //we want to hide the base

  static uint16_t mh_PackDataPoolIndex(const uint16_t ou16_List,
                                       const uint16_t ou16_Variable);
  static void mh_UnpackDataPoolIndex(const uint16_t ou16_Index,
                                     uint16_t &oru16_List,
                                     uint16_t &oru16_Variable);

  bool mq_HasDispatcher; ///< flag to remember whether dispatcher was installed
  bool mq_NvmValidFlagIsUsed; ///< true: use "SSL" layer services when writing
                              ///< to NVM

  static void mh_CyclicResponseReceived(void *const opv_Instance,
                                        const uint32_t ou32_Index,
                                        const int64_t os64_Value,
                                        const uint32_t ou32_TimeStamp,
                                        const bool oq_IsTimeStamped,
                                        const bool oq_Error);
  void m_CyclicResponseReceived(const uint32_t ou32_Index,
                                const int64_t os64_Value,
                                const uint32_t ou32_TimeStamp,
                                const bool oq_IsTimeStamped,
                                const bool oq_Error);
...
public:
  ...
  void SetCommunicationParameters(
  ...
  virtual int32_t NvmRead(const uint32_t ou32_MemoryAddress,
                          ...
                          uint8_t *const opu8_NrCode);
  virtual int32_t NvmWriteStartTransaction(const uint8_t ou8_DataPoolIndex,
