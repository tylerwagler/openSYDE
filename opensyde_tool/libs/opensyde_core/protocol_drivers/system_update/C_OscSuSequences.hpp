...
class C_OscSuSequences : public C_OscComSequencesBase {
public:
  ...
  public:
    ...
    C_OscComFlashloaderInformation
        c_MoreInformation; ///< collection of additional information
  };

  /// basic information about status of one STW Flashloader device
  class C_XflDeviceInformation {
  public:
    stw::diag_lib::C_XFLInformationFromServer
        c_BasicInformation; ///< basic information from server
    stw::diag_lib::C_XFLChecksumAreas
        c_ChecksumInformation; ///< information about sector or block based
                               // checksums
  };

  /// setting which files(s) to flash for one node and all flash parameters for
  /// each node
  class C_DoFlash {
  public:
    C_DoFlash(void);
  ...
  // we use enums so the application can use static checkers to verify all are
  // evaluated
  enum E_ProgressStep {
    ...
    eACTIVATE_FLASHLOADER_ROUTING_AVAILABLE_FEATURE_ERROR, // problem with
                                                           // available features
                                                           // of flashloader
    ...
    eUPDATE_SYSTEM_OSY_NODE_READ_FEATURE_ERROR, // problem getting the available
                                                // features
    ...
    eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_START, // starting to write file based
                                              // file
    ...
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_START, // reported once for each node (if
                                             // there are > 0 NVM
                                             //  files)
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_AVAILABLE_FEATURE_ERROR, // problem with
                                                               // available
                                                               // features of
                                                               // flashloader
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_SESSION_ERROR,
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_MAX_SIZE_ERROR, // problem getting maximum
                                                      // block size from device
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_OPEN_FILE_START, // reported once for each
                                                       // file; supports
                                                       // aborting
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_OPEN_FILE_ERROR, // problem opening input
                                                       // file
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_WRITE_FILE_START, // reported once for
                                                        // each file; supports
                                                        // aborting
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_WRITE_FILE_ERROR, // problem writing file
                                                        // data to device
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_FILE_FINISHED,    // reported for each
                                                        // finished file
    eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_FINISHED, // reported once for each node
                                                // (if there are > 0 NVM files)

    eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_START,
    eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_AVAILABLE_FEATURE_ERROR, // problem
                                                                    // with
                                                                    // available
                                                                    // features
                                                                    // of
    ...
    eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_AVAILABLE_FEATURE_ERROR, // problem
                                                                          // with
                                                                          // available
                                                                          // features
                                                                          // of
    ...
    eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_AVAILABLE_FEATURE_ERROR, // problem
                                                                          // with
                                                                          // available
                                                                          // features
                                                                          // of
  ...
  static void
  h_OpenSydeFlashloaderInformationToText(const C_OsyDeviceInformation &orc_Info,
                                         QStringList &orc_Text);
  static void
  h_StwFlashloaderInformationToText(const C_XflDeviceInformation &orc_Info,
                                    QStringList &orc_Text);

protected:
  // functions we use to report to application:
  virtual bool m_ReportProgress(const E_ProgressStep oe_Step,
  ...
  virtual void m_ReportOpenSydeFlashloaderInformationRead(
  ...
  virtual int32_t m_XflReportProgress(const uint8_t ou8_Progress,
                                      const QString &orc_Text);

  uint32_t mu32_CurrentNode; ///< index of currently addressed node within
                             ///< System Definition
...
private:
  ...
  C_OscSuSequences &
  operator=(const C_OscSuSequences
                &orc_Source); // lint !e1511 //we want to hide the base func.
  ...
  int32_t m_WriteOpenSydeNodeStates(
  ...
  int32_t m_ReadDeviceInformationStwFlashloader(
      const uint8_t ou8_ProgressToReport, const uint32_t ou32_NodeIndex,
      C_OscSuSequencesNodeConnectStates &orc_NodeState);

  bool
  m_IsNodeActive(const uint32_t ou32_NodeIndex, const uint32_t ou32_BusIndex,
                 C_OscNodeProperties::E_FlashLoaderProtocol &ore_ProtocolType,
                 C_OscProtocolDriverOsyNode &orc_NodeId) const;

  int32_t m_ReconnectToTargetServer(const bool oq_RestartRouting = false,
                                    const uint32_t ou32_NodeIndex = 0U);
  int32_t
  m_DisconnectFromTargetServer(const bool oq_DisconnectOnIp2IpRouting = true);

  uint32_t
  m_GetAdaptedTransferDataTimeout(const uint32_t ou32_DeviceTransferDataTimeout,
