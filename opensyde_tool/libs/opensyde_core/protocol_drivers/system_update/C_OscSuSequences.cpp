...
C_OscSuSequences::C_DoFlash::C_DoFlash(void)
    : q_SendSecurityEnabledState(false), q_SecurityEnabled(false),
      q_SendDebuggerEnabledState(false), q_DebuggerEnabled(false) {}
...
/*! \brief   Utility: check whether node is an active node on a specific bus
   ...
   * node and bus index must be within the range of nodes contained in
   mpc_SystemDefinition
   ...
   \param[out] ore_ProtocolType   if found: type of flashloader protocol defined
   for node (otherwise: eFL_NONE)
   \param[out] orc_NodeId         if found: node id of the node on the specified
   bus
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscSuSequences::m_IsNodeActive(
    const uint32_t ou32_NodeIndex, const uint32_t ou32_BusIndex,
    C_OscNodeProperties::E_FlashLoaderProtocol &ore_ProtocolType,
    C_OscProtocolDriverOsyNode &orc_NodeId) const {
  bool q_Return = false;
  const C_OscNode &rc_Node =
      this->mpc_SystemDefinition->c_Nodes[ou32_NodeIndex];

  ore_ProtocolType = C_OscNodeProperties::eFL_NONE;

  // is the node intends to be addressed at all (defined by parameter set in
  // Init() function)
  if (this->mc_ActiveNodes[ou32_NodeIndex] == 1U) {
    for (uint16_t u16_Interface = 0U;
         u16_Interface < rc_Node.c_Properties.c_ComInterfaces.size();
         u16_Interface++) {
      const C_OscNodeComInterfaceSettings &rc_Settings =
          rc_Node.c_Properties.c_ComInterfaces[u16_Interface];

      if ((rc_Settings.GetBusConnected() == true) &&
          (rc_Settings.u32_BusIndex == ou32_BusIndex) &&
          (rc_Settings.q_IsUpdateEnabled == true)) {
        // we cannot know for sure whether the target is in flashloader or
        // application mode prerequisite: a node is either
        //* a full openSYDE node (Flashloader + Application) or
        //* an STW Flashloader node
        // openSYDE node ?
        if (rc_Node.c_Properties.e_DiagnosticServer ==
            C_OscNodeProperties::eDS_OPEN_SYDE) {
          Q_ASSERT(rc_Node.c_Properties.e_FlashLoader ==
        ...
        orc_NodeId.u8_BusIdentifier =
            this->mpc_SystemDefinition->c_Buses[ou32_BusIndex].u8_BusId;
...
/*! \brief   Reports some information about the current sequence
   ...
   The public procedure functions report their progress using either this
   function or the polymorphous variation. If a procedure function finishes
   without problems it reports a progress of 100 before it returns. If it has
   problems the last progress reported can be < 100. Depending on the function
   it can report its whole process from "0 to 100" or individual parts (e.g.:
   SystemUpdate() will report from 0..100 for each node that is updated). The
   return value can be used to abort an ongoing sequence. However, not all calls
   to the function check the return value.
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscSuSequences::m_ReportProgress(const E_ProgressStep oe_Step,
                                        const int32_t os32_Result,
                                        const uint8_t ou8_Progress,
                                        const QString &orc_Information) {
  std::cout << "Step: " << static_cast<uint32_t>(oe_Step)
            << " Result: " << os32_Result
            << " Progress: " << static_cast<uint32_t>(ou8_Progress)
            << " Info: " << orc_Information.toLocal8Bit().constData() << "\n";

  return false;
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Reports some information about the current sequence for a specific
   server
   ...
   \param[in]  oe_Step           Step of node configuration
   \param[in]  os32_Result       Result of service
   \param[in]  ou8_Progress      Progress of sequence in percentage (goes from
   0..100 for each function)
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscSuSequences::m_ReportProgress(
  ...
  std::cout << "Step: " << static_cast<uint32_t>(oe_Step)
            << " Result: " << os32_Result
            << " Progress: " << static_cast<uint32_t>(ou8_Progress)
            << " Bus Id: " << static_cast<uint32_t>(orc_Server.u8_BusIdentifier)
            << " Node Id: "
            << static_cast<uint32_t>(orc_Server.u8_NodeIdentifier)
            << " Info: " << orc_Information.toLocal8Bit().constData() << "\n";
...
/*! \brief   Reports information read from openSYDE server node
   ...
   Called by ReadDeviceInformation() after it has read information from an
   openSYDE node. Default implementation here: print read information to console

   \param[in]     orc_Info         Information read from node
   \param[in]     ou32_NodeIndex   Index of node within mpc_SystemDefinition
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSuSequences::m_ReportOpenSydeFlashloaderInformationRead(
    const C_OsyDeviceInformation &orc_Info, const uint32_t ou32_NodeIndex) {
  QStringList c_Text;

  h_OpenSydeFlashloaderInformationToText(orc_Info, c_Text);

  std::cout << "openSYDE device information found for node with index "
            << ou32_NodeIndex << "\n";
  for (int32_t s32_Line = 0; s32_Line < c_Text.size(); s32_Line++) {
    std::cout << c_Text[s32_Line].toLocal8Bit().constData() << "\n";
  }
}
...
/*! \brief   Reports information read from STW flashloader server node
   ...
   Called by ReadDeviceInformation() after it has read information from an STW
   flashloader node. Default implementation here: print read information to
   console; checksum block or sector information is not printed

   \param[in]     orc_Info         Information read from node
   \param[in]     ou32_NodeIndex   Index of node within mpc_SystemDefinition
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSuSequences::m_ReportStwFlashloaderInformationRead(
    const C_XflDeviceInformation &orc_Info, const uint32_t ou32_NodeIndex) {
  QStringList c_Text;

  h_StwFlashloaderInformationToText(orc_Info, c_Text);

  std::cout << "STW Flashloader device information found for node with index "
            << ou32_NodeIndex << "\n";
  for (int32_t s32_Line = 0; s32_Line < c_Text.size(); s32_Line++) {
    std::cout << c_Text[s32_Line].toLocal8Bit().constData() << "\n";
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_XflReportProgress(const uint8_t ou8_Progress,
                                              const QString &orc_Text) {
  int32_t s32_Return = C_NO_ERR;

  // add node information:
  const bool q_Return = this->m_ReportProgress(
      eXFL_PROGRESS, C_NO_ERR, ou8_Progress, mc_CurrentNode, orc_Text);
...
/*! \brief   Flash one openSYDE address based node
   ...
   * Reports 0..100 for each file being flashed
   ...
   \param[in]      ou32_RequestDownloadTimeout   Maximum time in ms it can take
   to erase one continuous area in flash
   \param[in]      ou32_TransferDataTimeout      Maximum time in ms it can take
   to write up to 4kB of data to flash
   \param[in,out]  orq_SetProgrammingMode        In: Flag if programming mode
   must be set. Out: Flag if programming mode was set.
   ...
   C_COM       communication driver reported problem (details will be written to
   log file) C_NOACT     could not extract device name from hex file C_OVERFLOW
   device name of device does not match name contained in hex file C_BUSY
   procedure aborted by user (as returned by m_ReportProgress) C_CHECKSUM
   Security related error (something went wrong while handshaking with the
   server)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_FlashNodeOpenSydeHex(
  ...
  // try to open files to check whether we have valid hex files before we start
  // messing with the target's flash memory:
  for (uint32_t u32_File = 0U;
       (u32_File < orc_FilesToFlash.size()) && (s32_Return == C_NO_ERR);
       u32_File++) {
    (void)m_ReportProgress(
        eUPDATE_SYSTEM_OSY_NODE_HEX_OPEN_START, C_NO_ERR, 0U, mc_CurrentNode,
    ...
    if (u32_Return != stw::hex_file::NO_ERR) {
      ...
      s32_Return = c_Files[u32_File]->GetSignatureBlockAddress(
          c_SignatureAddresses[u32_File]);
      if (s32_Return != C_NO_ERR) {
        (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_HEX_SIGNATURE_ERROR,
                               s32_Return, 0U, mc_CurrentNode,
                               "Could not find signature block in HEX file.");
        orc_StateHexFiles[u32_File].e_SignatureBlockAddressFromFileRead =
            eSUSEQ_STATE_ERROR;
        s32_Return = C_CONFIG;
      } else {
        orc_StateHexFiles[u32_File].e_SignatureBlockAddressFromFileRead =
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    (void)m_ReportProgress(
        eUPDATE_SYSTEM_OSY_NODE_CHECK_DEVICE_NAME_START, C_NO_ERR, 10U,
        mc_CurrentNode,
        "X-checking device name of device against HEX file contents ...");
    // get target device name for comparison with PC-side files:
    s32_Return = this->mpc_ComDriver->SendOsyReadDeviceName(
        mc_CurrentNode, c_DeviceName, &u8_NrCode);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_CHECK_DEVICE_NAME_COMM_ERROR, s32_Return, 10U,
          mc_CurrentNode,
          "Could not read device name from device. Details:" +
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (uint32_t u32_File = 0U;
        ...
        if (s32_Return != C_NO_ERR) {
          ...
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_CHECK_DEVICE_NAME_FILE_ERROR, s32_Return,
              10U, mc_CurrentNode, c_ErrorText);
          orc_StateHexFiles[u32_File].e_NodeNameCompared = eSUSEQ_STATE_ERROR;
          s32_Return = C_NOACT;
        } else {
          bool q_IsSame = false;
          // Check actual device name
          if (c_DeviceName.trimmed().toUpper() ==
            ...
            for (uint32_t u32_ItName = 0UL;
                 (u32_ItName < orc_OtherAcceptedDeviceNames.size()) &&
                 (q_IsSame == false);
                 ++u32_ItName) {
              if (orc_OtherAcceptedDeviceNames[u32_ItName]
                      .trimmed()
                      .toUpper() == c_DeviceNameHexFile.trimmed().toUpper()) {
                orc_StateHexFiles[u32_File].e_NodeNameCompared =
                    eSUSEQ_STATE_NO_ERR;
                q_IsSame = true;
              }
          ...
          if (q_IsSame == false) {
            const QString c_ErrorText =
                ...
                c_DeviceName.trimmed().toUpper() + "\". HEX file contains: \"" +
                c_DeviceNameHexFile.trimmed().toUpper() + "\".";
            (void)m_ReportProgress(
                eUPDATE_SYSTEM_OSY_NODE_CHECK_DEVICE_NAME_MATCH_ERROR,
                C_OVERFLOW, 10U, mc_CurrentNode, c_ErrorText);
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_CHECK_MEMORY_START, C_NO_ERR,
                           20U, mc_CurrentNode,
                           "Checking memory availability ...");
    if (orq_SetProgrammingMode == true) {
      // In the hole update sequence, setting the programming mode only one time
      s32_Return =
          this->mpc_ComDriver->SendOsySetProgrammingMode(mc_CurrentNode);
      orq_SetProgrammingMode = false;
    }
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_CHECK_MEMORY_SESSION_ERROR,
                             s32_Return, 20U, mc_CurrentNode,
      ...
      for (uint32_t u32_File = 0U; u32_File < orc_FilesToFlash.size();
           u32_File++) {
        ...
        if (u32_Return != stw::hex_file::NO_ERR) {
          ...
          c_ErrorText = "Could not split up HEX file data of file " +
                        ...
                        c_Files[u32_File]->ErrorCodeToErrorText(u32_Return);
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_CHECK_MEMORY_FILE_ERROR, C_RD_WR, 20U,
              mc_CurrentNode, c_ErrorText);
          orc_StateHexFiles[u32_File].e_DataDumpFromFileRead =
              eSUSEQ_STATE_ERROR;
          s32_Return = C_RD_WR;
          break;
        } else {
          ...
          for (uint16_t u16_Area = 0U; u16_Area < pc_HexDump->at_Blocks.size();
               u16_Area++) {
            uint8_t u8_NrCode;
            s32_Return = this->mpc_ComDriver->SendOsyCheckFlashMemoryAvailable(
                mc_CurrentNode,
          ...
          orc_StateHexFiles[u32_File].e_AvailableFlashMemoryChecked =
              (s32_Return == C_NO_ERR) ? eSUSEQ_STATE_NO_ERR
                                       : eSUSEQ_STATE_ERROR;
        }
      }
    }
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (uint32_t u32_File = 0U; u32_File < orc_FilesToFlash.size();
        ...
        if (pc_HexDump != NULL) {
          ...
          s32_Return = m_FlashOneFileOpenSydeHex(
              *pc_HexDump, c_SignatureAddresses[u32_File],
...
/*! \brief   Flash one hex file to openSYDE address based node
   ...
   * file matches the target device name
   ...
   \param[in]      ou32_SignatureAddress         address of signature block
   within hex file
   \param[in]      ou32_RequestDownloadTimeout   Maximum time in ms it can take
   to erase one continuous area in flash
   \param[in]      ou32_TransferDataTimeout      Maximum time in ms it can take
   ...
   C_COM      communication driver reported problem (details will be written to
   log file) C_BUSY     procedure aborted by user (as returned by
   m_ReportProgress)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_FlashOneFileOpenSydeHex(
  ...
  (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_START, C_NO_ERR, 0U,
                         mc_CurrentNode, "Flashing HEX file ...");

  // get total number of bytes for progress calculations:
  for (int32_t s32_Area = 0U; s32_Area < orc_HexDataDump.at_Blocks.size();
       s32_Area++) {
    u32_TotalNumberOfBytes += static_cast<uint32_t>(
  ...
  for (int32_t s32_Area = 0U; s32_Area < orc_HexDataDump.at_Blocks.size();
    ...
    //  (we just need a rough approximation; so integer calculation will
    //  suffice)
    ...
    uint8_t u8_ProgressPercentage = static_cast<uint8_t>(
        (static_cast<uint64_t>(u32_TotalNumberOfBytesFlashed) * 100ULL) /
        static_cast<uint64_t>(u32_TotalNumberOfBytes));
    q_Abort = m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_AREA_START,
                               C_NO_ERR, u8_ProgressPercentage, mc_CurrentNode,
                               "Erasing flash memory for area " +
                                   QString::number(s32_Area + 1) + "...");
    if (q_Abort == true) {
      (void)m_ReportProgress(eUPDATE_SYSTEM_ABORTED, C_NO_ERR,
      ...
      (void)this->mpc_ComDriver->OsySetPollingTimeout(
          mc_CurrentNode, ou32_RequestDownloadTimeout);

      s32_Return = this->mpc_ComDriver->SendOsyRequestDownload(
          mc_CurrentNode, orc_HexDataDump.at_Blocks[s32_Area].u32_AddressOffset,
          orc_HexDataDump.at_Blocks[s32_Area].au8_Data.size(),
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        c_Error = QString::asprintf(
            ...
            "0x%08X). Details: %s",
            s32_Area + 1, orc_HexDataDump.at_Blocks[s32_Area].u32_AddressOffset,
            static_cast<uint32_t>(
                orc_HexDataDump.at_Blocks[s32_Area].au8_Data.size()),
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      while (u32_RemainingBytes > 0U) {
        ...
        c_Text = QString::asprintf(
            ...
            u32_AreaSize);
        q_Abort = m_ReportProgress(
            eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_AREA_TRANSFER_START, C_NO_ERR,
        ...
        if (q_Abort == true) {
          ...
          // subtract 4 bytes from the reported size; compensated for an issue
          // in older server implementations
          ...
          (void)memcpy(
              ...
              reinterpret_cast<const uint8_t *>(
                  orc_HexDataDump.at_Blocks[s32_Area].au8_Data.constData() +
                  static_cast<int32_t>(u32_AreaSize - u32_RemainingBytes)),
              c_Data.size());
          ...
          if (s32_Return == C_NO_ERR) {
            ...
            u8_BlockSequenceCounter = (u8_BlockSequenceCounter < 0xFFU)
                                          ? (u8_BlockSequenceCounter + 1U)
                                          : 0x00U;
            u32_TotalNumberOfBytesFlashed +=
                static_cast<uint32_t>(c_Data.size());
            // Prevent an overflow when file is bigger than 43MB
            // lint -e{414}  //see assertion at initial assignment
            u8_ProgressPercentage = static_cast<uint8_t>(
                (static_cast<uint64_t>(u32_TotalNumberOfBytesFlashed) *
                 100ULL) /
                static_cast<uint64_t>(u32_TotalNumberOfBytes));
          } else {
            (void)m_ReportProgress(
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      if (s32_Area == (orc_HexDataDump.at_Blocks.size() - 1)) {
        (void)m_ReportProgress(
            eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_AREA_EXIT_FINAL_START, C_NO_ERR,
            u8_ProgressPercentage, mc_CurrentNode,
            "Finalizing the final area and checking the signature ...");
        s32_Return =
            this->mpc_ComDriver->SendOsyRequestTransferExitAddressBased(
                mc_CurrentNode, true, ou32_SignatureAddress, &u8_NrCode);
      } else {
        (void)m_ReportProgress(
            eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_AREA_EXIT_START, C_NO_ERR,
            u8_ProgressPercentage, mc_CurrentNode, "Finalizing the area ...");
        s32_Return =
            this->mpc_ComDriver->SendOsyRequestTransferExitAddressBased(
                mc_CurrentNode, false, 0U, &u8_NrCode);
      }

      if (s32_Return != C_NO_ERR) {
        (void)m_ReportProgress(
            eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_AREA_EXIT_ERROR, s32_Return,
            u8_ProgressPercentage, mc_CurrentNode,
            "Could not finalize the area. Details: " +
                C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                    s32_Return, u8_NrCode));
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_HEX_FINISHED, C_NO_ERR,
                           100U, mc_CurrentNode, "Flashing HEX file finished.");
  }
...
/*! \brief   Flash one openSYDE file based node
   ...
   * Reports 0..100 for each file being flashed
   ...
   \param[in]      ou32_RequestDownloadTimeout   Maximum time in ms it can take
   to prepare one file on the target file
   \param[in]      ou32_TransferDataTimeout      Maximum time in ms it can take
   to write up to 4kB of data to the target file
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (uint32_t u32_File = 0U; u32_File < orc_FilesToFlash.size();
        ...
        s32_Return = m_FlashOneFileOpenSydeFile(
            orc_FilesToFlash[u32_File], ou32_RequestDownloadTimeout,
...
/*! \brief   Flash one file to openSYDE file based node
   ...
   * file was checked to be present
   ...
   When reporting the file name to the target only the file name of the file
   will be transferred. (i.e. path information will be removed).
   ...
   C_NO_ERR   file flashed
   C_COM      communication driver reported problem (details will be written to
   log file) C_RD_WR    could not read from input file C_BUSY     procedure
   aborted by user (as returned by m_ReportProgress)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_FlashOneFileOpenSydeFile(
    const QString &orc_FileToFlash, const uint32_t ou32_RequestDownloadTimeout,
    const uint32_t ou32_TransferDataTimeout,
    const C_OscProtocolDriverOsy::C_ListOfFeatures &orc_ProtocolFeatures,
  ...
  if (pc_File == NULL) {
    ...
    if (s32_FileApiReturn != 0) {
      ...
      if (s32_FileApiReturn < 0) {
        ...
        // s32_FileApiReturn already checked for negative value (so we can only
        // flash files up to 2GB)
  ...
  if (s32_Return == C_NO_ERR) {
    const bool q_Abort =
        m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_PREPARE_START,
                         C_NO_ERR, 0U, mc_CurrentNode,
    ...
    if (q_Abort == true) {
      ...
      s32_Return = this->mpc_ComDriver->SendOsyRequestFileTransfer(
          mc_CurrentNode, QFileInfo(orc_FileToFlash).fileName(),
      ...
      if (s32_Return != C_NO_ERR) {
        ...
        c_Error = QString::asprintf(
            ...
            QFileInfo(orc_FileToFlash).fileName().toUtf8().constData(),
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(s32_Return,
                                                                     u8_NrCode)
                .toUtf8()
                .constData());
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    while (u32_RemainingBytes > 0U) {
      ...
      q_Abort = m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_TRANSFER_START, C_NO_ERR,
          u8_ProgressPercentage, mc_CurrentNode, c_Text);
      if (q_Abort == true) {
        (void)m_ReportProgress(eUPDATE_SYSTEM_ABORTED, C_NO_ERR,
                               u8_ProgressPercentage, mc_CurrentNode,
        ...
        // subtract 4 bytes from the reported size; compensated for an issue in
        ...
        s32_FileApiReturn = static_cast<int32_t>(
            std::fread(reinterpret_cast<uint8_t *>(c_Data.data()), 1U,
                       static_cast<size_t>(c_Data.size()), pc_File));
        if (s32_FileApiReturn != static_cast<int32_t>(c_Data.size())) {
          ...
          // But the files might get large and the client might be limited in
          // RAM.
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_TRANSFER_ERROR, s32_Return,
              u8_ProgressPercentage, mc_CurrentNode,
          ...
          if (s32_Return == C_NO_ERR) {
            ...
            C_SclChecksums::CalcCRC32(
                reinterpret_cast<uint8_t *>(c_Data.data()),
                static_cast<uint32_t>(c_Data.size()), u32_TransferCrc);
    ...
    orc_StateOtherFile.e_AllTransferDataSent =
        (s32_Return == C_NO_ERR) ? eSUSEQ_STATE_NO_ERR : eSUSEQ_STATE_ERROR;
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_TRANSFER_START,
                           C_NO_ERR, u8_ProgressPercentage, mc_CurrentNode,
                           c_Text);

    // finalize transfer:
    (void)m_ReportProgress(
        eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_EXIT_START, C_NO_ERR,
        u8_ProgressPercentage, mc_CurrentNode,
        "Finalizing the transfer and checking the transfer CRC ...");
    // finalize transfer CRC:
    u32_TransferCrc ^= 0xFFFFFFFFU;

    s32_Return = this->mpc_ComDriver->SendOsyRequestTransferExitFileBased(
        mc_CurrentNode, u32_TransferCrc, &u8_NrCode);
    if (s32_Return != C_NO_ERR) {
      orc_StateOtherFile.e_RequestTransferFileExitSent = eSUSEQ_STATE_ERROR;
      (void)m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_EXIT_ERROR, s32_Return,
          u8_ProgressPercentage, mc_CurrentNode,
          "Could not finalize the transfer. Details: " +
              C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                  s32_Return, u8_NrCode));
      if ((s32_Return == C_WARN) &&
          (u8_NrCode ==
           C_OscProtocolDriverOsy::hu8_NR_CODE_GENERAL_PROGRAMMING_FAILURE)) {
    ...
    // if the target supports reading the "FileBasedTransferExitResult" and the
    // result of "TransferExit" was either
    ...
    if (orc_ProtocolFeatures.q_FileBasedTransferExitResultAvailable == true) {
      ...
      if ((s32_Return == C_NO_ERR) || (q_RejectedByTargetLayer == true)) {
        ...
        s32_LocalReturn =
            this->mpc_ComDriver->SendOsyRequestFileBasedTransferExitResult(
                mc_CurrentNode, c_TransferExitResult, &u8_NrCode);
        if (s32_LocalReturn != C_NO_ERR) {
          orc_StateOtherFile.e_RequestTransferFileExitResultSent =
              eSUSEQ_STATE_ERROR;
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_EXIT_ERROR, s32_LocalReturn,
              u8_ProgressPercentage, mc_CurrentNode,
              "Could not read exit result text. Details: " +
                  C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                      s32_LocalReturn, u8_NrCode));
          s32_Return = C_COM;
        ...
        c_TransferExitResult =
            "unkown (reading information not supported by server device)";
      }
      m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_RESULT_STRING,
                       C_NO_ERR, 100U, mc_CurrentNode,
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FLASH_FILE_FINISHED,
                           C_NO_ERR, 100U, mc_CurrentNode,
...
/*! \brief   Write one or more NVM parameter set file(s) to openSYDE node
   ...
   ** write file content to device
   ...
   Strategy for setting up required "DataDealer":
    To set up the DataDealerNvm we need to set up a DiagProtocol; but we only
   have a ProtocolDriverOsy So we create a DiagProtocol and copy over the known
   settings from the already up-and-running ProtocolDriverOsy.

   \param[in]      orc_FilesToWrite              Files to write to NVM
   \param[in]      orc_ProtocolFeatures          Information about available
   protocol features
   \param[in]      oq_SetProgrammingMode         In: Flag if programming mode
   must be set.
   ...
   C_CONFIG    one of the files contains data for zero or more than one device
   (expected: data for exactly one device) C_DEFAULT   one of the files is
   present but checksum is invalid C_CHECKSUM  Security related error (something
   went wrong while handshaking with the server) C_COM       communication
   driver reported problem (details will be written to log file) C_BUSY
   procedure aborted by user (as returned by m_ReportProgress) C_RANGE     At
   least one feature of the openSYDE Flashloader is not available for NVM
   writing
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_WriteNvmOpenSyde(
  ...
  C_OscProtocolDriverOsyTpBase *const pc_TransportProtocol =
      mpc_ComDriver->GetOsyTransportProtocol(mu32_CurrentNode);
  ...
  if (pc_TransportProtocol != NULL) {
    ...
    // set up DataDealer:
    C_OscDataDealerNvmSafe c_Dealer(&rc_Node, mu32_CurrentNode,
                                    &c_DiagProtocol);

    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_START, C_NO_ERR,
                           0U, mc_CurrentNode,
    ...
    if ((orc_ProtocolFeatures.q_FlashloaderCanWriteToNvm == false) ||
      ...
      (void)m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_AVAILABLE_FEATURE_ERROR, s32_Return,
          5U, mc_CurrentNode,
          "The node has not the Flashloader feature to write a Parameter Set "
          "Image file.");
    }

    if (s32_Return == C_NO_ERR) {
      if (oq_SetProgrammingMode == true) {
        // In the hole update sequence, setting the programming mode only one
        // time
        s32_Return =
            this->mpc_ComDriver->SendOsySetProgrammingMode(mc_CurrentNode);
      ...
      if (s32_Return != C_NO_ERR) {
        (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_SESSION_ERROR,
                               s32_Return, 5U, mc_CurrentNode,
                               "Could not activate programming session.");
        if (s32_Return != C_CHECKSUM) {
          s32_Return = C_COM;
        }
    ...
    if (s32_Return == C_NO_ERR) {
      // check server for MaxNumberOfBlockLength
      s32_Return =
          c_DiagProtocol.OsyReadMaxNumberOfBlockLength(u16_MaxBlockLength);
      if (s32_Return != C_NO_ERR) {
        (void)m_ReportProgress(
            eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_MAX_SIZE_ERROR, s32_Return, 5U,
            mc_CurrentNode,
            "Could not get max number of block length from device.");
        // this information is not available in older Flashloaders
        // the application is responsible to not try this sequence for those
        // servers
        s32_Return = C_COM;
      }
    }
    if (s32_Return == C_NO_ERR) {
      ...
      for (uint16_t u16_File = 0U; u16_File < orc_FilesToWrite.size();
           u16_File++) {
        C_OscSuSequencesNodePsiFileStates &rc_State =
            orc_StatePsiFiles[u16_File];
        // logic: split 80% (between 10% and 90%) by the number of files
        const float32_t f32_Percent =
            (((80.0F / static_cast<float32_t>(orc_FilesToWrite.size()))) *
             static_cast<float32_t>(u16_File));
        const uint8_t u8_Percent = 10U + static_cast<uint8_t>(f32_Percent);
        bool q_Abort =
            m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_OPEN_FILE_START,
                             C_NO_ERR, u8_Percent, mc_CurrentNode,
                             "Reading parameter set image file \"" +
                                 orc_FilesToWrite[u16_File] + "\"...");
        if (q_Abort == true) {
          s32_Return = C_BUSY;
        } else {
          s32_Return =
              c_Dealer.NvmSafeReadFileWithCrc(orc_FilesToWrite[u16_File]);
          if (s32_Return != C_NO_ERR) {
        ...
        if (s32_Return != C_NO_ERR) {
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_OPEN_FILE_ERROR, s32_Return,
              u8_Percent, mc_CurrentNode,
              "Could not read parameter set image file \"" +
                  orc_FilesToWrite[u16_File] + "\"!");
        } else {
          q_Abort = m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_WRITE_FILE_START, C_NO_ERR,
              u8_Percent, mc_CurrentNode,
          ...
          if (q_Abort == true) {
            ...
            s32_Return = c_Dealer.NvmSafeWriteParameterSetFile(
                orc_FilesToWrite[u16_File], s32_ResultDetail);
          }
          if (s32_Return != C_NO_ERR) {
            rc_State.e_PsiFileWrote = eSUSEQ_STATE_ERROR;
            (void)m_ReportProgress(
                eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_WRITE_FILE_ERROR, s32_Return,
                u8_Percent, mc_CurrentNode,
                "Could not write data of parameter set image file \"" +
                    orc_FilesToWrite[u16_File] + "\" to device !");
            // C_OVERFLOW, C_CONFIG: would be a systematic error in this
            // implementation rest: C_COM
            s32_Return = C_COM;
          } else {
        ...
        if (s32_Return == C_NO_ERR) {
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_FILE_FINISHED, s32_Return,
              u8_Percent, mc_CurrentNode,
              "Writing parameter set image file to device finished.");
        } else {
          break;
        }
      }
    ...
    if (s32_Return == C_NO_ERR) {
      (void)m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_NVM_WRITE_FINISHED, s32_Return, 100U,
          mc_CurrentNode,
...
/*! \brief   Write one PEM file to openSYDE node
   ...
   * Reports progress from 0..100 for the overall process
   ...
   C_WARN      the file is present but key details of PEM file could not be
   extracted C_COM       communication driver reported problem (details will be
   written to log file) C_RANGE     At least one feature of the openSYDE
   Flashloader is not available for NVM writing C_CHECKSUM  Security related
   error (something went wrong while handshaking with the server)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_WritePemOpenSydeFile(
  ...
  if (orc_FileToWrite != "") {
    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_START,
                           C_NO_ERR, 0U, mc_CurrentNode,
                           "Writing PEM file ...");
    ...
    if (orc_ProtocolFeatures.q_SupportsSecurity == false) {
      ...
      (void)m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_AVAILABLE_FEATURE_ERROR,
          s32_Return, 5U, mc_CurrentNode,
          "The node has not the Flashloader features to support security and "
          "to write PEM files.");
    } else {
      C_OscSecurityPem c_PemFile;
      QString c_ErrorMessage;

      s32_Return = c_PemFile.LoadFromFile(orc_FileToWrite,
                                          c_ErrorMessage);
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        s32_Return = C_OscSecurityPem::h_ExtractModulusAndExponent(
            c_PubKeyDecoded, c_PubKeyModulus, c_PubKeyExponent, c_ErrorMessage);

        if (s32_Return == C_NO_ERR) {
          orc_StatePemFile.e_PemFileExtracted = eSUSEQ_STATE_NO_ERR;

          if (orq_SetProgrammingMode == true) {
            // In the hole update sequence, setting the programming mode only
            // one time
            const uint8_t u8_SECURITY_LEVEL = 1U;
            s32_Return = this->mpc_ComDriver->SendOsySetProgrammingMode(
                mc_CurrentNode, &u8_SECURITY_LEVEL);
            orq_SetProgrammingMode = false;
          }
          if (s32_Return != C_NO_ERR) {
            (void)m_ReportProgress(
                eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_SESSION_ERROR,
                s32_Return, 10U, mc_CurrentNode,
                "Could not activate programming session.");
            if (s32_Return != C_CHECKSUM) {
              s32_Return = C_COM;
            }
        ...
        if (s32_Return == C_NO_ERR) {
          ...
          s32_Return = this->mpc_ComDriver->SendOsyWriteSecurityKey(
              this->mc_CurrentNode, c_PubKeyModulus, c_PubKeyExponent,
              c_KeySerialNumber, &u8_NrCode);

          if (s32_Return != C_NO_ERR) {
            (void)m_ReportProgress(
                eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_SEND_ERROR, s32_Return,
                75U, mc_CurrentNode,
          ...
          (void)m_ReportProgress(
              ...
              QString("Could not load PEM file. Details: ") +
                  c_ErrorMessage);
          s32_Return = C_WARN;
        }
      } else {
        (void)m_ReportProgress(
            ...
            25U, mc_CurrentNode,
            QString("Could not extract security key from decoded public key. "
                    "Details: ") +
                c_ErrorMessage);
    ...
    if (s32_Return == C_NO_ERR) {
      (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_PEM_FILE_WRITE_FINISHED,
                             s32_Return, 100U, mc_CurrentNode,
...
/*! \brief   Write device states to openSYDE node
   ...
   * node is active
   ...
   \param[in]      orc_ApplicationsToWrite       Update configuration with all
   states for sending or not sending
   \param[in]      orc_ProtocolFeatures          Information about available
   protocol features
   \param[in,out]  orq_SetProgrammingMode        In: Flag if programming mode
   must be set. Out: Flag if programming mode was set.
   \param[out]     orc_StatePemFile              State of PEM file and its
   associated handled states

   \return
   C_NO_ERR    all states were written or no write process was necessary
   C_COM       communication driver reported problem (details will be written to
   log file) C_RANGE     At least one feature of the openSYDE Flashloader is not
   available for state writing C_CHECKSUM  Security related error (something
   went wrong while handshaking with the server)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_WriteOpenSydeNodeStates(
  ...
  if (orc_ApplicationsToWrite.q_SendSecurityEnabledState == true) {
    (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_START,
                           C_NO_ERR, 0U, mc_CurrentNode,
    ...
    if (orc_ProtocolFeatures.q_SupportsSecurity == true) {
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        s32_Return = this->mpc_ComDriver->SendOsyWriteSecurityActivation(
            this->mc_CurrentNode, orc_ApplicationsToWrite.q_SecurityEnabled, 0U,
            &u8_NrCode);

        if (s32_Return == C_NO_ERR) {
          orc_StatePemFile.e_SecurityFlagSent = eSUSEQ_STATE_NO_ERR;
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_FINISHED, s32_Return,
              100U, mc_CurrentNode,
              "Writing security activation to device finished.");
        } else {
      ...
      // Security feature is necessary to write security activation state to
      // flashloader
      (void)m_ReportProgress(
          eUPDATE_SYSTEM_OSY_NODE_STATE_SECURITY_WRITE_AVAILABLE_FEATURE_ERROR,
          s32_Return, 5U, mc_CurrentNode,
  ...
  if (orc_ApplicationsToWrite.q_SendDebuggerEnabledState == true) {
    if (s32_Return == C_NO_ERR) {
      (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_START,
                             C_NO_ERR, 0U, mc_CurrentNode,
      ...
      if (((orc_ProtocolFeatures.q_SupportsDebuggerOn == true) &&
        ...
        if (s32_Return == C_NO_ERR) {
          uint8_t u8_NrCode;
          s32_Return = this->mpc_ComDriver->SendOsyWriteDebuggerEnabled(
              this->mc_CurrentNode, orc_ApplicationsToWrite.q_DebuggerEnabled,
              &u8_NrCode);
          ...
          if (s32_Return == C_NO_ERR) {
            ...
            (void)m_ReportProgress(
                ...
                s32_Return, 100U, mc_CurrentNode,
                "Writing debugger state to device finished.");
          } else {
        ...
        // Debugger state change feature is necessary to write debugger state to
        // flashloader
        if (orc_ApplicationsToWrite.q_DebuggerEnabled == true) {
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_AVAILABLE_FEATURE_ERROR,
              s32_Return, 5U, mc_CurrentNode,
              "The node has not the Flashloader feature to enable the "
              "debugger.");
        } else {
          (void)m_ReportProgress(
              eUPDATE_SYSTEM_OSY_NODE_STATE_DEBUGGER_WRITE_AVAILABLE_FEATURE_ERROR,
              s32_Return, 5U, mc_CurrentNode,
              "The node has not the Flashloader feature to disable the "
              "debugger.");
        }
        s32_Return = C_RANGE;
      }
...
int32_t C_OscSuSequences::m_WriteFingerPrintOsy(void) {
  ...
  (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FINGERPRINT_START, C_NO_ERR,
                         30U, mc_CurrentNode, "Writing fingerprint ...");
  q_Return = stw::opensyde_core::C_OscUtils::h_GetSystemUserName(c_UserName);
  if (q_Return != true) {
    (void)m_ReportProgress(
        eUPDATE_SYSTEM_OSY_NODE_FINGERPRINT_NAME_NOT_READABLE, C_WARN, 30U,
        mc_CurrentNode,
        "Could not get user name from system. Using \"unknown\".");
    c_UserName = "unknown";
  }

  s32_Return = this->mpc_ComDriver->SendOsyWriteApplicationSoftwareFingerprint(
      mc_CurrentNode, au8_Date, au8_Time, c_UserName, &u8_NrCode);
  if (s32_Return != C_NO_ERR) {
    (void)m_ReportProgress(
        eUPDATE_SYSTEM_OSY_NODE_FINGERPRINT_ERROR, s32_Return, 30U,
        mc_CurrentNode,
...
int32_t C_OscSuSequences::m_FlashNodeXfl(
  ...
  (void)m_ReportProgress(eUPDATE_SYSTEM_XFL_NODE_START, C_NO_ERR, 0U,
                         mc_CurrentNode, "Flashing node ...");

  for (uint32_t u32_File = 0U; u32_File < orc_FilesToFlash.size(); u32_File++) {
    // Save file index
    this->mu32_CurrentFile = u32_File;

    (void)m_ReportProgress(eUPDATE_SYSTEM_XFL_NODE_FLASH_HEX_START, C_NO_ERR,
                           0U, mc_CurrentNode, "Flashing HEX file ...");

    // Loading errors can not be detected on this layer separated from the
    // sending process
    orc_StateHexFiles[u32_File].e_FileLoaded = eSUSEQ_STATE_NOT_NEEDED;

    s32_Return = this->mpc_ComDriver->SendStwDoFlash(
        mc_CurrentNode, orc_FilesToFlash[u32_File]);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(eUPDATE_SYSTEM_XFL_NODE_FLASH_HEX_ERROR, C_NO_ERR,
                             100U, mc_CurrentNode,
    ...
    (void)m_ReportProgress(eUPDATE_SYSTEM_XFL_NODE_FLASH_HEX_FINISHED, C_NO_ERR,
                           100U, mc_CurrentNode, "Flashing HEX file finished.");
  }
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eUPDATE_SYSTEM_XFL_NODE_FINISHED, C_NO_ERR, 100U,
                           mc_CurrentNode, "Flashing node finished.");
  }
  return s32_Return;
}
...
/*! \brief   Read information of openSYDE server node
   ...
   Utility function used to read openSYDE information from server node and
   report it. See documentation of ReadDeviceInformation() for full description.

   \param[in]     ou8_ProgressToReport    progress value to report when invoking
   virtual reporting function
   \param[in]     ou32_NodeIndex          Index of node within
   mpc_SystemDefinition
   \param[in,out] orc_NodeState           Connect states of node

   \return
   C_COM      communication driver reported problem (details will be written to
   log file) C_CHECKSUM Security related error (something went wrong while
   handshaking with the server) C_NO_ERR   information read
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_ReadDeviceInformationOpenSyde(
  ...
  // If connected via Ethernet we might need to reconnect (in case we ran into
  // the session timeout)
  s32_Return = this->m_ReconnectToTargetServer();

  if (s32_Return != C_NO_ERR) {
    (void)m_ReportProgress(eREAD_DEVICE_INFO_OSY_RECONNECT_ERROR, s32_Return,
  ...
  if (s32_Return == C_NO_ERR) {
    // node is active and flashable directly on the bus that the client is
    // connected to to prevent any session timeout issues request the
    // pre-programming session and activate security
    //  level 1
    (void)m_ReportProgress(eREAD_DEVICE_INFO_OSY_SET_SESSION_START, C_NO_ERR,
                           ou8_ProgressToReport, mc_CurrentNode,
                           "Activating PreProgramming session ...");
    s32_Return = this->mpc_ComDriver->SendOsySetPreProgrammingMode(
        mc_CurrentNode, false, &u8_NrCode);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(
          eREAD_DEVICE_INFO_OSY_SET_SESSION_ERROR, s32_Return,
          ou8_ProgressToReport, mc_CurrentNode,
  ...
  if (s32_Return == C_NO_ERR) {
    // get device name
    (void)m_ReportProgress(eREAD_DEVICE_INFO_OSY_DEVICE_NAME_START, C_NO_ERR,
                           ou8_ProgressToReport, mc_CurrentNode,
                           "Reading device name ...");
    s32_Return = this->mpc_ComDriver->SendOsyReadDeviceName(
        mc_CurrentNode, c_Info.c_DeviceName, &u8_NrCode);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(
          eREAD_DEVICE_INFO_OSY_DEVICE_NAME_ERROR, s32_Return,
          ou8_ProgressToReport, mc_CurrentNode,
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eREAD_DEVICE_INFO_OSY_FLASH_BLOCKS_START, C_NO_ERR,
                           ou8_ProgressToReport, mc_CurrentNode,
                           "Reading flash block information ...");

    // we need security level 1 for that:
    s32_Return = this->mpc_ComDriver->SendOsySetSecurityLevel(mc_CurrentNode,
                                                              1U, &u8_NrCode);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(
          eREAD_DEVICE_INFO_OSY_FLASH_BLOCKS_SECURITY_ERROR, s32_Return,
          ou8_ProgressToReport, mc_CurrentNode,
      ...
      if (rc_CurNode.pc_DeviceDefinition
              ->c_SubDevices[rc_CurNode.u32_SubDeviceIndex]
              .q_FlashloaderOpenSydeIsFileBased == false) {
        s32_Return = this->mpc_ComDriver->SendOsyReadAllFlashBlockData(
            mc_CurrentNode, c_Info.c_Applications, &u8_NrCode);
        if (s32_Return != C_NO_ERR) {
          (void)m_ReportProgress(
              eREAD_DEVICE_INFO_OSY_FLASH_BLOCKS_ERROR, s32_Return,
              ou8_ProgressToReport, mc_CurrentNode,
              "Error reading flash block information. Details:" +
                  C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                      s32_Return, u8_NrCode));
          s32_Return = C_COM;
        }
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eREAD_DEVICE_INFO_OSY_FLASHLOADER_INFO_START,
                           C_NO_ERR, ou8_ProgressToReport, mc_CurrentNode,
                           "Reading even more information ...");
    s32_Return = this->mpc_ComDriver->SendOsyReadInformationFromFlashloader(
        mc_CurrentNode, c_Info.c_MoreInformation, &u8_NrCode);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(
          eREAD_DEVICE_INFO_OSY_FLASHLOADER_INFO_ERROR, s32_Return,
          ou8_ProgressToReport, mc_CurrentNode,
          "Error reading even more information. Details:" +
              C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                  s32_Return, u8_NrCode));
      s32_Return = C_COM;
    } else {
      orc_NodeState.c_AvailableFeatures =
          c_Info.c_MoreInformation.c_AvailableFeatures;
    }
  }

  if ((s32_Return == C_NO_ERR) &&
      (c_Info.c_MoreInformation.c_AvailableFeatures.q_SupportsSecurity ==
       true)) {
    // Check for activated security
    // By calling SendOsySetPreProgrammingMode on the beginning we know it is
    // successful, but not if security is activated
    uint8_t u8_Algorithm;
    (void)m_ReportProgress(
        eREAD_DEVICE_INFO_OSY_FLASHLOADER_CHECK_SECURITY_ACTIVATION_START,
        C_NO_ERR, ou8_ProgressToReport, mc_CurrentNode,
        "Reading security activation state ...");
    s32_Return = this->mpc_ComDriver->SendOsyReadSecurityActivation(
        mc_CurrentNode, orc_NodeState.q_AuthenticationNecessary, u8_Algorithm,
        &u8_NrCode);

    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(
          eREAD_DEVICE_INFO_OSY_FLASHLOADER_CHECK_SECURITY_ACTIVATION_ERROR,
          s32_Return, ou8_ProgressToReport, mc_CurrentNode,
          "Error reading security activation state. Details:" +
              C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                  s32_Return, u8_NrCode));
      s32_Return = C_COM;
    } else {
...
/*! \brief   Read information of STW flashloader server node
   ...
   Utility function used to read STW flashloader information from server node
   and report it. See documentation of ReadDeviceInformation() for full
   description.
   ...
   \return
   C_COM      communication driver reported problem (details will be written to
   log file) C_NO_ERR   information read
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_ReadDeviceInformationStwFlashloader(
    const uint8_t ou8_ProgressToReport, const uint32_t ou32_NodeIndex,
    C_OscSuSequencesNodeConnectStates &orc_NodeState) {
  int32_t s32_Return;
  ...
  if (s32_Return != C_NO_ERR) {
    (void)m_ReportProgress(eREAD_DEVICE_INFO_XFL_WAKEUP_ERROR, s32_Return,
                           ou8_ProgressToReport, mc_CurrentNode,
                           "Could not perform node wakeup.");
    s32_Return = C_COM;
  } else {
    // read everything we can get ...
    (void)m_ReportProgress(eREAD_DEVICE_INFO_XFL_READING_INFORMATION_START,
                           C_NO_ERR, ou8_ProgressToReport, mc_CurrentNode,
                           "Reading information from node ...");
    s32_Return = this->mpc_ComDriver->SendStwReadDeviceInformation(
        mc_CurrentNode, c_Information.c_BasicInformation,
        c_Information.c_ChecksumInformation);
    if (s32_Return != C_NO_ERR) {
      (void)m_ReportProgress(eREAD_DEVICE_INFO_XFL_READING_INFORMATION_ERROR,
                             s32_Return, ou8_ProgressToReport, mc_CurrentNode,
      ...
      // report findings to application:
      this->m_ReportStwFlashloaderInformationRead(c_Information,
                                                  ou32_NodeIndex);
    }
  }

  orc_NodeState.q_AuthenticationNecessarySet = true;
  orc_NodeState.e_InformationRead =
      (s32_Return == C_NO_ERR) ? eSUSEQ_STATE_NO_ERR : eSUSEQ_STATE_ERROR;
...
C_OscSuSequences::C_OscSuSequences(void)
    : C_OscComSequencesBase(true, true), mu32_CurrentNode(0U),
      mu32_CurrentFile(0U) {}
...
/*! \brief   Copy referenced files to a temporary folder
   ...
   To be called optionally before starting the procedure with
   "ActivateFlashloader()".

   * Can be called before "Init()" as all required information is passed as
   parameters.
   ...
   * Files for address based targets and NVM files will be renamed to prevent
   possible conflicts
   * Flash files for file based devices will not be renamed (as they need to be
   unique anyway)
   * Does not report any progress via m_ReportProgress().

   \param[in]     orc_Nodes               List of nodes (part of system
   definition)
   \param[in]     orc_ActiveNodes         Vector of flags for each node in
   orc_Nodes
                                          - true: file path(s) for the target
   are expected to be present
                                          - false: file path(s) for the target
   are not expected to be present
   \param[in]     orc_TargetPath          Path to place files in (with trailing
   path separator)
   \param[in,out] orc_ApplicationsToWrite in: Paths to files to copy
                                          out: Paths to resulting files copied
   and renamed by this function if the function returns an error this parameter
   will not be modified
   \param[in,out] opc_ErrorPath           Optional pointer to store path error
   details (current: which file did fail)

   \return
   C_NO_ERR    files copied
   C_OVERFLOW  size of orc_ApplicationsToWrite is not the same as the size of
   nodes in orc_Nodes size of orc_ActiveNodes is not the same as the size of
   nodes in orc_Nodes C_NOACT     orc_ApplicationsToWrite has non-empty list of
   files for node that was not set as active in orc_ActiveNodes size of files in
   orc_ApplicationsToWrite[node] differs from the number of applications of the
   node in orc_Nodes (for an active and address based node) C_RANGE     Flash or
   NVM file referenced by orc_ApplicationsToWrite does not exist orc_TargetPath
   does not end in "\" or "/" C_BUSY      could not erase pre-existing target
   path (note: can result in partially erased target path) C_RD_WR     could not
   copy file C_TIMEOUT   could not create target directory C_CONFIG    at least
   one file based node has at least two identical named files (independent of
   character case)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::h_CreateTemporaryFolder(
  ...
  const QChar cn_LastCharacter =
      orc_TargetPath.isEmpty() ? QChar()
                               : orc_TargetPath[orc_TargetPath.length() - 1];
  ...
  if ((cn_LastCharacter != '\\') && (cn_LastCharacter != '/')) {
    ...
    if ((orc_ApplicationsToWrite.size() != orc_Nodes.size()) ||
        (orc_ActiveNodes.size() != orc_Nodes.size())) {
      s32_Return = C_OVERFLOW;
    }
  }
  if (s32_Return == C_NO_ERR) {
    ...
    for (uint16_t u16_Node = 0U; u16_Node < orc_Nodes.size(); u16_Node++) {
      ...
      if ((orc_ActiveNodes[u16_Node] == 0U) &&
          ((orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size() != 0) ||
           (orc_ApplicationsToWrite[u16_Node].c_FilesToWriteToNvm.size() !=
            0) ||
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        if (orc_ActiveNodes[u16_Node] == 1U) {
          // files for flash:
          for (uint16_t u16_File = 0U;
               u16_File <
               orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size();
               u16_File++) {
            const QString c_File =
                orc_ApplicationsToWrite[u16_Node].c_FilesToFlash[u16_File];
            if (!QFileInfo(c_File).exists() || !QFileInfo(c_File).isFile()) {
              s32_Return = C_RANGE;
              if (opc_ErrorPath != NULL) {
          ...
          for (uint16_t u16_File = 0U;
               ...
               u16_File++) {
            const QString c_File =
                orc_ApplicationsToWrite[u16_Node].c_FilesToWriteToNvm[u16_File];
            if (!QFileInfo(c_File).exists() || !QFileInfo(c_File).isFile()) {
              s32_Return = C_RANGE;
              if (opc_ErrorPath != NULL) {
                *opc_ErrorPath = c_File;
          ...
          if (orc_ApplicationsToWrite[u16_Node].c_PemFile != "") {
            const QString c_File = orc_ApplicationsToWrite[u16_Node].c_PemFile;
            if (!QFileInfo(c_File).exists() || !QFileInfo(c_File).isFile()) {
              s32_Return = C_RANGE;
              if (opc_ErrorPath != NULL) {
                *opc_ErrorPath = c_File;
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        Q_ASSERT(orc_Nodes[u16_Node].u32_SubDeviceIndex <
                 orc_Nodes[u16_Node].pc_DeviceDefinition->c_SubDevices.size());
        if (orc_Nodes[u16_Node]
                .pc_DeviceDefinition
          ...
          for (uint16_t u16_File = 0U; u16_File < c_Files.size(); u16_File++) {
            c_Files[u16_File] =
                QFileInfo(c_Files[u16_File]).fileName().toLower();
          }
          // get same names next to each other:
          std::sort(c_Files.begin(), c_Files.end());
          // remove duplicates:
          c_Files.erase(std::unique(c_Files.begin(), c_Files.end()),
                        c_Files.end());
          if (c_Files.size() !=
              orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size()) {
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (QFileInfo(orc_TargetPath).isDir()) {
      s32_Return = (QDir(orc_TargetPath).removeRecursively() ? 0 : -1);
      if (s32_Return != 0) {
        s32_Return = C_BUSY;
        if (opc_ErrorPath != NULL) {
          *opc_ErrorPath = orc_TargetPath;
        ...
        if (orc_ActiveNodes[u16_Node] == 1U) {
          c_NodeTargetPaths[u16_Node] =
              stw::opensyde_core::C_OscUtils::h_IncludeTrailingDelimiter(
                  orc_TargetPath +
                  C_OscUtils::h_NiceifyStringForFileName(
                      orc_Nodes[u16_Node].c_Properties.c_Name));

          s32_Return = (QDir().mkpath(c_NodeTargetPaths[u16_Node]) ? 0 : -1);
          if (s32_Return != 0) {
            if (opc_ErrorPath != NULL) {
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    for (uint16_t u16_Node = 0U; u16_Node < orc_Nodes.size(); u16_Node++) {
      if (orc_ActiveNodes[u16_Node] == 1U) {
        // flash files
        for (uint16_t u16_File = 0U;
             u16_File < orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size();
             u16_File++) {
          // get source file name
          const QString c_SourceFileName =
              orc_ApplicationsToWrite[u16_Node].c_FilesToFlash[u16_File];
          // compose target file name
          QString c_TargetFileName;
          ...
          if (orc_Nodes[u16_Node]
            ...
            // The original source file name is not relevant and does not need
            // to be unique Add an counter to the target file name
            c_TargetFileName =
                c_NodeTargetPaths[u16_Node] +
                QString::number(static_cast<uint32_t>(u16_File) + 1U) + "_" +
                QFileInfo(
                    orc_ApplicationsToWrite[u16_Node].c_FilesToFlash[u16_File])
                    .fileName();
          }
          ...
          // copy file
          s32_Return = C_OscUtils::h_CopyFile(c_SourceFileName,
                                              c_TargetFileName, opc_ErrorPath);
          if (s32_Return == C_NO_ERR) {
            c_NodesToFlashNewPaths[u16_Node].c_FilesToFlash[u16_File] =
                c_TargetFileName;
          } else {
            break;
          }
        ...
        for (uint16_t u16_File = 0U;
          ...
          const QString c_TargetFileName =
              c_NodeTargetPaths[u16_Node] +
              QString::number(static_cast<uint32_t>(u16_File) + 1U) + "_" +
              QFileInfo(orc_ApplicationsToWrite[u16_Node]
                            .c_FilesToWriteToNvm[u16_File])
                  .fileName();
          ...
          s32_Return = C_OscUtils::h_CopyFile(c_SourceFileName,
                                              c_TargetFileName, opc_ErrorPath);
          if (s32_Return == C_NO_ERR) {
            c_NodesToFlashNewPaths[u16_Node].c_FilesToWriteToNvm[u16_File] =
                c_TargetFileName;
          } else {
            break;
          }
        ...
        if (c_NodesToFlashNewPaths[u16_Node].c_PemFile != "") {
          ...
          const QString c_TargetFileName =
              c_NodeTargetPaths[u16_Node] +
              QFileInfo(orc_ApplicationsToWrite[u16_Node].c_PemFile).fileName();
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Check whether application on client side match applications on
   devices

   Compares a list of application properties to a second list of application
   properties. The function will report which elements of the first list are
   ...
   Workflow:
   * Read application properties from servers (e.g. using
   ...
   * set up parameter for UpdateSystem() based on the results of this function
   ...
   \param[in]     orc_ClientSideApplications       properties of client side
   applications
   \param[in]     orc_ServerSideApplications       properties of server side
   applications
   \param[out]    orc_ApplicationsPresentOnServer   size matches size of
   orc_ClientSideApplications 0: application not contained in
   orc_ServerSideApplications 1: application contained in
   orc_ServerSideApplications
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSuSequences::h_CheckForChangedApplications(
  ...
  for (uint32_t u32_ClientApplIndex = 0U;
    ...
    for (uint32_t u32_ServerApplIndex = 0U;
         u32_ServerApplIndex < orc_ServerSideApplications.size();
         u32_ServerApplIndex++) {
      if (orc_ServerSideApplications[u32_ServerApplIndex] ==
          orc_ClientSideApplications[u32_ClientApplIndex]) {
...
/*! \brief   Initialization of the protocol
   ...
   ** if connected via CAN: send "EnterPreProgrammingSession" as broadcast (for
   a few seconds in short intervals)
   ...
   ** send "FLASH" (for a few seconds in short intervals;
   ...
   For confirmation:
   * use simple read service for all nodes that are expected to be present (as
   "ping")
   ...
   * all nodes defined to be present on all buses are in flashloader mode
   ...
   \param[in]   oq_FailOnFirstError   true: abort all further communication if
   connecting to one device fails false: try to continue with other devices in
   this case
   ...
   C_COM      communication driver reported problem (details will be written to
   log file) C_WARN     activation for at least one device failed (see log for
   details) C_CHECKSUM Security related error (something went wrong while
...
int32_t C_OscSuSequences::ActivateFlashloader(const bool oq_FailOnFirstError) {
  ...
  if (this->mpc_SystemDefinition == NULL) {
    ...
    Q_ASSERT(this->mpc_SystemDefinition->c_Nodes.size() ==
             this->mc_ActiveNodes.size());
    Q_ASSERT(this->mu32_ActiveBusIndex <
             this->mpc_SystemDefinition->c_Buses.size());

    // send openSYDE CAN-TP "RequestProgramming" to each node
    (void)m_ReportProgress(
        eACTIVATE_FLASHLOADER_OSY_REQUEST_PROGRAMMING_START, C_NO_ERR, 0U,
    ...
    for (uint16_t u16_Node = 0U;
         u16_Node < this->mpc_SystemDefinition->c_Nodes.size(); u16_Node++) {
      C_OscNodeProperties::E_FlashLoaderProtocol e_ProtocolType;
      const bool q_IsActive = m_IsNodeActive(
          u16_Node, this->mu32_ActiveBusIndex, e_ProtocolType, mc_CurrentNode);
        ...
        if (s32_Return == C_NOACT) {
          ...
          if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
            ...
            if (s32_Return != C_NO_ERR) {
              ...
              // the state eACTIVATE_FLASHLOADER_OSY_XFL_BC_PING_START was set
              // the first time to have the same process sequence like using
              // CAN. Therefore do not abort and send only a warning as progress
              // information. This can only happen in case of Ethernet. The node
              // is not connected, so the next request did not make any sense to
              // send and will be skipped till the "EnterPreProgrammingSession"
              // broadcasts. By skipping the other request, further sending
              // errors will be avoided which would occur a "real" sending
              // error.
              (void)m_ReportProgress(
                  eACTIVATE_FLASHLOADER_OSY_RECONNECT_WARNING, s32_Return, 10U,
                  mc_CurrentNode,
                  "Device does not respond to RequestProgramming request.");

              s32_Return = C_NO_ERR;
            } else {
              // Set the request programming flag to bring the server into
              // flashloader
              s32_Return = this->mpc_ComDriver->SendOsyRequestProgramming(
                  mc_CurrentNode);

              this->mpc_ComDriver->DisconnectNode(mc_CurrentNode);

              if (s32_Return != C_NO_ERR) {
                // In this case, the node does not respond and maybe the next
                // step can help to bring it into the flashloader by sending
                // EnterPreProgrammingSession as broadcast Therefore do not
                // abort and send only a warning as progress information.
                (void)m_ReportProgress(
            ...
            // No error, nothing to do for STW Flashloader nodes here, reset the
            // error code
            s32_Return = C_NO_ERR;
          }
        }
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (uint16_t u16_Node = 0U;
        ...
        const bool q_IsActive =
            m_IsNodeActive(u16_Node, this->mu32_ActiveBusIndex, e_ProtocolType,
                           mc_CurrentNode);
          ...
          if (s32_Return == C_NOACT) {
            if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
              ...
              if (s32_Return != C_NO_ERR) {
                ...
                // the state eACTIVATE_FLASHLOADER_OSY_XFL_BC_PING_START was set
                // the first time to have the same process sequence like using
                // CAN. Therefore do not abort and send only a warning as
                // progress information. This can only happen in case of
                // Ethernet. The node is not connected, so the next request did
                // not make any sense to send and will be skipped till the
                // "EnterPreProgrammingSession" broadcasts. By skipping the
                // other request, further sending errors will be avoided which
                // would occur a "real" sending error.
                (void)m_ReportProgress(
                    eACTIVATE_FLASHLOADER_OSY_RECONNECT_WARNING, s32_Return,
                    15U, mc_CurrentNode,
                    "Device does not respond to RequestProgramming request.");
                ...
                s32_Return = this->mpc_ComDriver->SendOsyEcuReset(
                    ...
                    C_OscProtocolDriverOsyTpBase::
                        hu8_OSY_RESET_TYPE_RESET_TO_FLASHLOADER);

                this->mpc_ComDriver->DisconnectNode(mc_CurrentNode);

                if (s32_Return != C_NO_ERR) {
                  // In this case, the node will not get the request, maybe the
                  // next step can help to bring it into the flashloader by
                  // sending EnterPreProgrammingSession as broadcast Therefore
                  // do not abort and send only a warning as progress
                  // information.
                  (void)m_ReportProgress(
                      eACTIVATE_FLASHLOADER_OSY_ECU_RESET_WARNING, s32_Return,
                      10U, mc_CurrentNode,
              ...
              if (s32_Return != C_NO_ERR) {
                if (s32_Return == C_NOACT) {
                  // No reset message configured. The next step can help to
                  // bring it into the flashloader by sending STW Flashloader
                  // "FLASH" Therefore do not abort and send only a warning as
                  // progress information. All other errors are errors which can
                  // not be "healed" by the next step
                  (void)m_ReportProgress(
                      eACTIVATE_FLASHLOADER_XFL_ECU_RESET_WARNING, s32_Return,
                      15U, mc_CurrentNode, "No EcuReset message configured.");

                  s32_Return = C_NO_ERR;
                } else {
                  // All other errors are errors which can not be "healed" by
                  // the next step
                  (void)m_ReportProgress(
                      eACTIVATE_FLASHLOADER_XFL_ECU_RESET_ERROR, s32_Return,
                      15U, mc_CurrentNode, "Sending EcuReset request failed.");
    ...
    // send "EnterPreProgrammingSession" as broadcast (for a few seconds in
    // short intervals)
    if (s32_Return == C_NO_ERR) {
      // but only if we are communicating via CAN bus; keeping the target in the
      // flashloader via the time window
      //  is not possible via Ethernet; it really has to react to
      //  RequestProgramming
      if (this->mpc_SystemDefinition->c_Buses[this->mu32_ActiveBusIndex]
              .e_Type == C_OscSystemBus::eCAN) {
        const bool q_UseOpenSydeProtocol = this->mq_OpenSydeDevicesActive;
        const bool q_UseStwFlashloaderProtocol =
            this->mq_StwFlashloaderDevicesActiveOnLocalBus;

        QElapsedTimer c_Timer;
        c_Timer.start();
        uint32_t u32_WaitTime = this->GetMinimumFlashloaderResetWaitTime(
        ...
        (void)m_ReportProgress(
            eACTIVATE_FLASHLOADER_OSY_XFL_BC_ENTER_FLASHLOADER_START, C_NO_ERR,
            20U, "Broadcasting enter Flashloader request ...");
        do {
          if (q_UseOpenSydeProtocol == true) {
            // openSYDE "DiagnosticSessionControl(PreProgramming)" broadcast
            s32_Return = this->mpc_ComDriver
                             ->SendOsyCanBroadcastEnterPreProgrammingSession();
            if (s32_Return != C_NO_ERR) {
              (void)m_ReportProgress(
                  eACTIVATE_FLASHLOADER_OSY_BC_ENTER_PRE_PROGRAMMING_ERROR,
                  s32_Return, 20U, "EnterPreProgramming broadcast failed.");
              s32_Return = C_COM;
            }
          }
          ...
          if ((q_UseStwFlashloaderProtocol == true) &&
            ...
            s32_Return = this->mpc_ComDriver->SendStwSendFlash(
                this->mc_StwFlashloaderDeviceOnLocalBus);
            if (s32_Return != C_NO_ERR) {
              (void)m_ReportProgress(eACTIVATE_FLASHLOADER_XFL_BC_FLASH_ERROR,
                                     s32_Return, 20U,
        ...
        // Ethernet. Give the targets the minimum reset time to reset and
        // initialize their Ethernet interfaces ...
        QThread::msleep(this->GetMinimumFlashloaderResetWaitTime(
            C_OscComDriverFlash::eNO_CHANGES_ETHERNET));
      }
    }

    // Previous broadcasts might have caused responses placed in the receive
    ...
    // "ping")
    if (s32_Return == C_NO_ERR) {
      (void)m_ReportProgress(eACTIVATE_FLASHLOADER_OSY_XFL_BC_PING_START,
                             C_NO_ERR, 30U, "Checking node states ...");
      ...
      for (uint16_t u16_Node = 0U;
        ...
        if (q_Return == true) {
          ...
          if (s32_Return == C_NOACT) {
            if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
              // if connected via Ethernet we need to reconnect as the reset
              // will break the active TCP
              //  connection
              s32_Return = this->mpc_ComDriver->ReConnectNode(mc_CurrentNode);

              if (s32_Return != C_NO_ERR) {
                (void)m_ReportProgress(
                    eACTIVATE_FLASHLOADER_OSY_RECONNECT_ERROR, s32_Return, 30U,
                    mc_CurrentNode, "Could not reconnect to node");

                // Node is not reachable
                this->mc_TimeoutNodes[u16_Node] =
                    static_cast<uint8_t>(s32_Return == C_BUSY);

                s32_Return = C_COM;
              }
              if (s32_Return == C_NO_ERR) {
                // If we are on Ethernet the node is in flashloader but not in
                // programming mode yet
                //  we need to send one EnterPreProgramming request to get it
                //  there
                // If we're on CAN one more of this services does not hurt as
                // well :-) We want to confirm the device is in flashloader
                // anyway. So we use this service (it will fail if the node is
                // in the application
                //  as there is no "PreProgramming" session there.
                s32_Return = this->mpc_ComDriver->SendOsySetPreProgrammingMode(
                    mc_CurrentNode, true, &u8_NrCode);
                if (s32_Return != C_NO_ERR) {
                  (void)m_ReportProgress(
                      eACTIVATE_FLASHLOADER_OSY_SET_SESSION_ERROR, s32_Return,
                      30U, mc_CurrentNode,
                      "Request to set active session failed. Details:" +
                          C_OscProtocolDriverOsy::
                              h_GetOpenSydeServiceErrorDetails(s32_Return,
                                                               u8_NrCode));

                  // Node is not reachable
                  this->mc_TimeoutNodes[u16_Node] =
                      static_cast<uint8_t>(s32_Return == C_TIMEOUT);

                  if (s32_Return != C_CHECKSUM) {
                    s32_Return = C_COM;
                  } else {
                    this->mc_ConnectStatesNodes[u16_Node]
                        .q_AuthenticationError = true;
                  }
                } else {
                  // looks good ...
                  // node is active and flashable directly on the bus that the
              ...
              // STW Flashloader node
              // it should be in Sleep state now. The only service that we can
              // use is wakeup. will not cause any problems as the node will go
              // back to sleep if another node is addressed
              //  later
              //  on
              s32_Return = this->mpc_ComDriver->SendStwWakeupLocalId(
                  mc_CurrentNode, NULL);
              if (s32_Return != C_NO_ERR) {
                (void)m_ReportProgress(eACTIVATE_FLASHLOADER_XFL_WAKEUP_ERROR,
                                       s32_Return, 30U, mc_CurrentNode,
                                       "Could not perform node wakeup.");
    ...
    if (s32_Return == C_NO_ERR) {
      for (uint16_t u16_Node = 0U;
        ...
        if (this->mpc_ComDriver->GetBusIndexOfRoutingNode(
                u16_Node, u32_BusIndex) == C_NO_ERR) {
          const bool q_IsActive = m_IsNodeActive(
              u16_Node, u32_BusIndex, e_ProtocolType, mc_CurrentNode);
          ...
          if (q_IsActive == true) {
            ...
            if (q_IsNodeReachable == true) {
              ...
              (void)m_ReportProgress(eACTIVATE_FLASHLOADER_ROUTING_START,
                                     C_NO_ERR, 40U, mc_CurrentNode,
                                     "Starting routing for node ...");

              s32_Return =
                  this->mpc_ComDriver->StartRouting(u16_Node, &u32_ErrorIndex);
              ...
              if (s32_Return == C_NO_ERR) {
                if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
                  ...
                  if (s32_Return == C_NO_ERR) {
                    ...
                    if (s32_Return != C_NO_ERR) {
                      (void)m_ReportProgress(
                          eACTIVATE_FLASHLOADER_OSY_ECU_RESET_ERROR, s32_Return,
                          50U, mc_CurrentNode,
                    ...
                    (void)m_ReportProgress(
                        ...
                        s32_Return, 50U, mc_CurrentNode,
                        "RequestProgramming request for routing device "
                        "failed.");
                  ...
                  if (s32_Return == C_NO_ERR) {
                    ...
                    C_OscSystemBus::E_Type e_TargetInterfaceType =
                        C_OscSystemBus::eCAN;
                    C_OscComDriverFlash::E_MinimumFlashloaderResetWaitTimeType
                        e_WaitType;
                    stw::opensyde_core::C_OscProtocolDriverOsyNode c_LastRouter;

                    // Check what interface is used of target (not the local
                    // bus)
                    ...
                    Q_ASSERT(this->GetMinimumFlashloaderResetWaitTime(
                                 e_WaitType, this->mc_CurrentNode,
                                 u32_WaitTime) == C_NO_ERR);

                    Q_ASSERT(this->mpc_ComDriver->GetServerIdOfLastRouter(
                                 u16_Node, c_LastRouter) == C_NO_ERR);

                    QElapsedTimer c_Timer;
                    c_Timer.start();
                    u32_LastSentTesterPresent = 0; // Relative to timer start

                    // Give the targets some time to reset and initialize their
                    // interfaces and wait the minimum time
                    do {
                      u32_CurrentTime =
                          static_cast<uint32_t>(c_Timer.elapsed());

                      // Tester present is only necessary in case of CAN. In
                      // case of Ethernet the entire routing will be restarted
                      // anyway by m_ReconnectToTargetServer
                      if ((e_TargetInterfaceType == C_OscSystemBus::eCAN) &&
                          (u32_CurrentTime > (u32_LastSentTesterPresent +
                                              u32_INTERVAL_TESTER_PRESENT))) {
                        // We need TesterPresent to keep the routing connection
                        // alive Send it to the last routing point
                        s32_Return = this->mpc_ComDriver->SendTesterPresent(
                            c_LastRouter);

                        if (s32_Return != C_NO_ERR) {
                          (void)m_ReportProgress(
                              eACTIVATE_FLASHLOADER_OSY_RECONNECT_ERROR,
                              s32_Return, 50U, this->mc_CurrentNode,
                    ...
                    if (s32_Return == C_NO_ERR) {
                      s32_Return =
                          this->m_ReconnectToTargetServer(true, u16_Node);
                    }
                  }

                  if (s32_Return == C_NO_ERR) {
                    // If we are on Ethernet the node is in flashloader but not
                    // in programming mode yet
                    //  we need to send one EnterPreProgramming request to get
                    //  it there
                    // If we're on CAN one more of this services does not hurt
                    // as well :-) We want to confirm the device is in
                    // flashloader anyway. So we use this service (it will fail
                    // if the node is in the application
                    //  as there is no "PreProgramming" session there.
                    s32_Return =
                        this->mpc_ComDriver->SendOsySetPreProgrammingMode(
                            mc_CurrentNode, true, &u8_NrCode);
                    ...
                    if (s32_Return != C_NO_ERR) {
                      (void)m_ReportProgress(
                          ...
                          s32_Return, 50U, mc_CurrentNode,
                          "Request to set active session for routing device "
                          "failed. Details:" +
                              C_OscProtocolDriverOsy::
                                  h_GetOpenSydeServiceErrorDetails(s32_Return,
                                                                   u8_NrCode));
                  ...
                  if (s32_Return != C_NO_ERR) {
                    (void)m_ReportProgress(
                        eACTIVATE_FLASHLOADER_XFL_ECU_RESET_ERROR, s32_Return,
                        50U, mc_CurrentNode, "Sending request failed.");

                    s32_Return = C_COM;
                  }

                  if (s32_Return == C_NO_ERR) {
                    const qint64 s64_StartTime =
                    ...
                    do {
                      ...
                      QThread::msleep(5);
                        ...
                        QDateTime::currentMSecsSinceEpoch() <
                        (static_cast<qint64>(u32_WaitTime) + s64_StartTime));

                    if (s32_Return == C_NO_ERR) {
                      // it should be in Sleep state now. The only service that
                      // we can use is wakeup. will not cause any problems as
                      // the node will go back to sleep if another node is
                      //  addressed later on
                      s32_Return = this->mpc_ComDriver->SendStwWakeupLocalId(
                          mc_CurrentNode, NULL);
                      ...
                      if (s32_Return != C_NO_ERR) {
                        (void)m_ReportProgress(
                            eACTIVATE_FLASHLOADER_XFL_WAKEUP_ERROR, s32_Return,
                            50U, mc_CurrentNode,
                            "Could not perform node wakeup for routing "
                            "device.");
                ...
                if (s32_Return == C_NOACT) {
                  (void)m_ReportProgress(
                      eACTIVATE_FLASHLOADER_ROUTING_AVAILABLE_FEATURE_ERROR,
                      s32_Return, 50U, mc_CurrentNode,
                      "Starting routing for node failed due to not capable "
                      "node");
                } else {
                  (void)m_ReportProgress(eACTIVATE_FLASHLOADER_ROUTING_ERROR,
                                         s32_Return, 50U, mc_CurrentNode,
                                         "Starting routing for node failed");
                }
                osc_write_log_error(
                    "Activate Flashloader",
                    "Activate Flashloader: Start of routing for node (" +
                        QString::number(u16_Node) +
                ...
                this->mc_TimeoutNodes[u16_Node] =
                    static_cast<uint8_t>(s32_Return == C_TIMEOUT);
                this->mc_TimeoutNodes[u32_ErrorIndex] =
                    static_cast<uint8_t>(s32_Return == C_TIMEOUT);

                this->mc_ConnectStatesNodes[u16_Node]
                    .q_CouldNotReachedDueToRoutingError = true;

                if ((s32_Return != C_CONFIG) && (s32_Return != C_RANGE)) {
                  this->mc_ConnectStatesNodes[u32_ErrorIndex]
                      .q_RoutingNodeError = true;
                }
              ...
              osc_write_log_error(
                  ...
                  "Activate Flashloader: Start of routing for node (" +
                      QString::number(u16_Node) +
                      ") failed due to not available node on route with error "
                      "code: " +
                      C_OscLoggingHandler::h_StwError(s32_Return));
            }
...
/*! \brief   Read information of server nodes
   ...
   Prerequisite:
   All nodes marked as active in mc_ActiveNodes are present and are in
   flashloader mode i.e. the system is in the state after a successful call to
   ActivateFlashloader()

   For the API it would be better to first read all information from STW
   Flashloader nodes and return it and then from openSYDE nodes in another
   function or vice versa. But for routing it makes sense to set it up
   sequentially and handle the system bus by bus.
   ...
   * Use "C_XFLActions::Read*" functions; basically most of the stuff that
   WinFlash displays in the "Configure Node" dialog

   \param[in]   oq_FailOnFirstError   true: abort all further communication if
   communication with one device fails false: try to continue with other devices
   in this case
   ...
   C_COM      communication driver reported error (details will be written to
   log file) C_CHECKSUM Security related error (something went wrong while
   handshaking with the server)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscSuSequences::ReadDeviceInformation(const bool oq_FailOnFirstError) {
  int32_t s32_Return = C_NO_ERR;
  bool q_AtLeastOneError = false;
  ...
  if (this->mpc_SystemDefinition == NULL) {
    ...
    for (uint16_t u16_Node = 0U;
      ...
      const uint8_t u8_Progress = static_cast<uint8_t>(
          ((static_cast<size_t>(u16_Node) + 1U) * 100U) /
          (this->mpc_SystemDefinition->c_Nodes.size() + 1U));
      const bool q_Abort =
          m_ReportProgress(eREAD_DEVICE_INFO_START, C_NO_ERR, u8_Progress,
      ...
      if (q_Abort == true) {
        ...
        if (q_IsNodeReachable == true) {
          ...
          s32_Return = this->mpc_ComDriver->GetBusIndexOfRoutingNode(
              u16_Node, u32_BusIndex);
          ...
          if (s32_Return == C_NO_ERR) {
            C_OscNodeProperties::E_FlashLoaderProtocol e_ProtocolType;
            const bool q_Return = m_IsNodeActive(
                u16_Node, u32_BusIndex, e_ProtocolType, mc_CurrentNode);
            if (q_Return == true) {
              if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
                (void)m_ReportProgress(
                    eREAD_DEVICE_INFO_OSY_START, C_NO_ERR, u8_Progress,
                    mc_CurrentNode, "Reading openSYDE device information ...");
                s32_Return = this->m_ReadDeviceInformationOpenSyde(
                    u8_Progress, u16_Node,
                    this->mc_ConnectStatesNodes[u16_Node]);

                if (s32_Return == C_NO_ERR) {
                  (void)m_ReportProgress(eREAD_DEVICE_INFO_OSY_FINISHED,
                                         C_NO_ERR, u8_Progress, mc_CurrentNode,
                                         "openSYDE device information read.");
                }
              } else {
                (void)m_ReportProgress(
                    eREAD_DEVICE_INFO_XFL_START, C_NO_ERR, u8_Progress,
                    mc_CurrentNode,
                    "Reading STW Flashloader device information ...");
                s32_Return = this->m_ReadDeviceInformationStwFlashloader(
                    u8_Progress, u16_Node,
                    this->mc_ConnectStatesNodes[u16_Node]);

                if (s32_Return == C_NO_ERR) {
                  (void)m_ReportProgress(
                      eREAD_DEVICE_INFO_XFL_FINISHED, C_NO_ERR, u8_Progress,
                      mc_CurrentNode,
          ...
          osc_write_log_warning(
              ...
              "Read device information: Node (" + QString::number(u16_Node) +
...
/*! \brief   Read information of server nodes
   ...
   * Reports 0..100 for each file of each individual node being flashed
   ...
   \param[in]  orc_ApplicationsToWrite   list of files to flash per node; must
   have the same size as the system definition contains nodes
   \param[in]  orc_NodesOrder            Vector with node update order (index is
   update position, value is node index)

   \return
   C_NO_ERR    flashed all files
   C_CONFIG    mpc_SystemDefinition is NULL (Init() not called)
               parameter writing: one of the files contains data for zero or
   more than one device (expected: data for exactly one device) C_OVERFLOW  size
   of orc_ApplicationsToWrite is not the same as the size of nodes in
   mpc_SystemDefinition for address based targets: device name of device does
   not match name contained in hex file C_NOACT     orc_ApplicationsToWrite has
   non-empty list of flash or NVM files for node that was not set as active
               orc_ApplicationsToWrite has non-empty list of flash or NVM files
   for node that has no position in the update order orc_ApplicationsToWrite has
   non-empty list of flash or NVM files for node that has more than one position
                in the update order
               for address based targets: could not extract device name from hex
   file C_RD_WR     file referenced by orc_ApplicationsToWrite does not exist
               for address based targets: one of the flash files is not a valid
   Intel or Motorola hex file for address based targets: could not split up
   flash file into individual memory areas parameter writing: one of the files
   is not a valid .psi_syde file or does not exist C_CONFIG    for address based
   targets: no signature block found in flash hex file for STW Flashloader
   targets: NVM files are defined (not supported by STW Flashloader) C_COM
   communication driver reported problem (details will be written to log file)
   C_BUSY      procedure aborted by user (as returned by m_ReportProgress)
   C_DEFAULT   parameter writing: one of the files is present but checksum is
   invalid C_WARN      The file is present but key details of PEM file could not
   be extracted C_CHECKSUM  Security related error (something went wrong while
   handshaking with the server) C_RANGE     At least one feature of the openSYDE
   Flashloader is not available for NVM writing
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::UpdateSystem(
  ...
  if (this->mpc_SystemDefinition == NULL) {
    s32_Return = C_CONFIG;
  } else if (orc_ApplicationsToWrite.size() !=
             this->mpc_SystemDefinition->c_Nodes.size()) {
    s32_Return = C_OVERFLOW;
  } else {
    ...
    for (uint16_t u16_Node = 0U;
         u16_Node < this->mpc_SystemDefinition->c_Nodes.size(); u16_Node++) {
      C_OscSuSequencesNodeUpdateStates &rc_State =
          this->mc_UpdateStatesNodes[u16_Node];
      // Prepare the node states for its files
      if (this->mpc_SystemDefinition->c_Nodes[u16_Node]
              .c_Properties.e_FlashLoader ==
          C_OscNodeProperties::eFL_OPEN_SYDE) {
        ...
        if (pc_DeviceDefinition != NULL) {
          if (pc_DeviceDefinition->c_SubDevices[u32_SubDeviceIndex]
                  .q_FlashloaderOpenSydeIsFileBased == false) {
            rc_State.c_StateHexFiles.resize(
                orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size());
          } else {
            rc_State.c_StateOtherFiles.resize(
                orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size());
          }
        }

        // PSI files
        rc_State.c_StatePsiFiles.resize(
            orc_ApplicationsToWrite[u16_Node].c_FilesToWriteToNvm.size());
      } else if (this->mpc_SystemDefinition->c_Nodes[u16_Node]
                     .c_Properties.e_FlashLoader ==
                 C_OscNodeProperties::eFL_STW) {
        // STW Flashloader
        rc_State.c_StateStwFlHexFiles.resize(
            orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size());
      } else {
        // Nothing to do
      }
      ...
      if ((orc_ApplicationsToWrite[u16_Node].c_FilesToFlash.size() != 0) ||
        ...
        if (this->mc_ActiveNodes[u16_Node] == false) {
          osc_write_log_error(
              "System Update",
              "File(s) to flash or write to NVM configured for node " +
                  QString::number(u16_Node) +
                  " which is not marked as active !");
          s32_Return = C_NOACT;
        } else {
          ...
          s32_Return = C_NOACT;
          for (u32_PositionCounter = 0U;
               u32_PositionCounter < orc_NodesOrder.size();
               ++u32_PositionCounter) {
            if (orc_NodesOrder[u32_PositionCounter] ==
                static_cast<uint32_t>(u16_Node)) {
              s32_Return = C_NO_ERR;
              break;
            }
          }
          ...
          if (s32_Return == C_NO_ERR) {
            ...
            for (uint32_t u32_File = 0U;
              ...
              if (!QFileInfo(orc_ApplicationsToWrite[u16_Node]
                ...
                osc_write_log_error("System Update",
                                    "Could not find file \"" +
                                        orc_ApplicationsToWrite[u16_Node]
                                            .c_FilesToFlash[u32_File] +
                                        "\" !");
                if (pc_FileState != NULL) {
                ...
                osc_write_log_error("System Update",
                                    "Could not find file \"" +
                                        orc_ApplicationsToWrite[u16_Node]
                                            .c_FilesToWriteToNvm[u32_File] +
                                        "\" !");
                rc_State.c_StatePsiFiles[u32_File].e_FileExists =
                    eSUSEQ_STATE_ERROR;
                s32_Return = C_RD_WR;
                break;
              } else {
            ...
            if (orc_ApplicationsToWrite[u16_Node].c_PemFile != "") {
              rc_State.c_StatePemFile.c_FileName =
                  QFileInfo(orc_ApplicationsToWrite[u16_Node].c_PemFile)
                      .fileName();
              if (!QFileInfo(orc_ApplicationsToWrite[u16_Node].c_PemFile)
                       .exists() ||
                  !QFileInfo(orc_ApplicationsToWrite[u16_Node].c_PemFile)
                       .isFile()) {
                osc_write_log_error(
              ...
              rc_State.c_StatePemFile.e_PemFileExtracted =
                  eSUSEQ_STATE_NOT_NEEDED;
              rc_State.c_StatePemFile.e_SecurityKeySent =
                  eSUSEQ_STATE_NOT_NEEDED;
              rc_State.c_StatePemFile.e_SecurityFlagSent =
                  eSUSEQ_STATE_NOT_NEEDED;
              rc_State.c_StatePemFile.e_DebuggerFlagSent =
                  eSUSEQ_STATE_NOT_NEEDED;
            }
          }
        }
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      if (orc_NodesOrder.size() == u32_NodesToFlash) {
        ...
        for (u32_OrderPos = 0U; u32_OrderPos < orc_NodesOrder.size();
             ++u32_OrderPos) {
          for (u32_OrderPosCompare = 0U;
               u32_OrderPosCompare < orc_NodesOrder.size();
               ++u32_OrderPosCompare) {
            if ((u32_OrderPos != u32_OrderPosCompare) &&
                (orc_NodesOrder[u32_OrderPos] ==
                 orc_NodesOrder[u32_OrderPosCompare])) {
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eUPDATE_SYSTEM_START, C_NO_ERR, 0U,
                           "Starting System Update ...");
    for (uint32_t u32_Position = 0U; u32_Position < orc_NodesOrder.size();
         u32_Position++) {
      const uint32_t u32_NodeIndex = orc_NodesOrder[u32_Position];
      C_OscSuSequencesNodeUpdateStates &rc_NodeUpdateStates =
          this->mc_UpdateStatesNodes[u32_NodeIndex];
      ...
      if ((orc_ApplicationsToWrite[u32_NodeIndex].c_FilesToFlash.size() > 0) ||
          (orc_ApplicationsToWrite[u32_NodeIndex].c_FilesToWriteToNvm.size() >
           0) ||
        ...
        if (s32_Return == C_NOACT) {
          ...
          // Routing necessary
          s32_Return =
              this->mpc_ComDriver->StartRouting(u32_NodeIndex, &u32_ErrorIndex);

          if (s32_Return != C_NO_ERR) {
            // Handle routing specific errors
            this->mc_ConnectStatesNodes[u32_NodeIndex]
                .q_CouldNotReachedDueToRoutingError = true;
            if ((s32_Return != C_CONFIG) && (s32_Return != C_RANGE)) {
              this->mc_ConnectStatesNodes[u32_ErrorIndex].q_RoutingNodeError =
                  true;
            }
        ...
        if (s32_Return == C_NO_ERR) {
          ...
          const bool q_Return = m_IsNodeActive(u32_NodeIndex, u32_BusIndex,
                                               e_ProtocolType, mc_CurrentNode);
          if (q_Return == true) {
            if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
              const C_OscDeviceDefinition *const pc_DeviceDefinition =
                  this->mpc_SystemDefinition->c_Nodes[u32_NodeIndex]
                      .pc_DeviceDefinition;
              const uint32_t u32_SubDeviceIndex =
                  this->mpc_SystemDefinition->c_Nodes[u32_NodeIndex]
                      .u32_SubDeviceIndex;
              (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_START, C_NO_ERR,
                                     10U, mc_CurrentNode,
              ...
              if (pc_DeviceDefinition != NULL) {
                ...
                // if connected via Ethernet we might need to reconnect (in case
                // we ran into the session timeout)
                s32_Return = this->m_ReconnectToTargetServer();
                if (s32_Return != C_NO_ERR) {
                  (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_RECONNECT_ERROR,
                                         s32_Return, 10U, mc_CurrentNode,
                                         "Could not reconnect to node");
                  if (s32_Return != C_CHECKSUM) {
                    s32_Return = C_COM;
                ...
                if (s32_Return == C_NO_ERR) {
                  ...
                  s32_Return = this->mpc_ComDriver->SendOsyReadListOfFeatures(
                      this->mc_CurrentNode, c_AvailableFeatures);
                  if (s32_Return != C_NO_ERR) {
                    (void)m_ReportProgress(
                        eUPDATE_SYSTEM_OSY_NODE_READ_FEATURE_ERROR, s32_Return,
                        10U, mc_CurrentNode,
                        "Could not read available openSYDE Flashloader "
                        "features.");
                    s32_Return = C_COM;
                  }
                }
                ...
                // files to flash ?
                if ((s32_Return == C_NO_ERR) &&
                    (orc_ApplicationsToWrite[u32_NodeIndex]
                         .c_FilesToFlash.size() > 0)) {
                  Q_ASSERT(u32_SubDeviceIndex <
                           pc_DeviceDefinition->c_SubDevices.size());
                  ...
                  if (pc_DeviceDefinition->c_SubDevices[u32_SubDeviceIndex]
                    ...
                    s32_Return = m_FlashNodeOpenSydeFile(
                        ...
                        pc_DeviceDefinition->c_SubDevices[u32_SubDeviceIndex]
                            .u32_FlashloaderOpenSydeTransferDataTimeout,
                        c_AvailableFeatures, q_SetProgrammingMode,
                ...
                if ((s32_Return == C_NO_ERR) &&
                    (orc_ApplicationsToWrite[u32_NodeIndex]
                         .c_FilesToWriteToNvm.size() > 0)) {
                  s32_Return = m_WriteNvmOpenSyde(
                      orc_ApplicationsToWrite[u32_NodeIndex]
                          .c_FilesToWriteToNvm,
                      c_AvailableFeatures, q_SetProgrammingMode,
                ...
                // Special case: An other security level is necessary for the
                // next steps. The next step must set the programming mode with
                // the other security level again
                q_SetProgrammingMode = true;

                // PEM file to write?
                if (s32_Return == C_NO_ERR) {
                  s32_Return = m_WritePemOpenSydeFile(
                      orc_ApplicationsToWrite[u32_NodeIndex].c_PemFile,
                      c_AvailableFeatures, q_SetProgrammingMode,
                ...
                // States to write?
                if (s32_Return == C_NO_ERR) {
                  s32_Return = m_WriteOpenSydeNodeStates(
                      orc_ApplicationsToWrite[u32_NodeIndex],
                      c_AvailableFeatures, q_SetProgrammingMode,
                      rc_NodeUpdateStates.c_StatePemFile);
                }
              ...
              if (s32_Return == C_NO_ERR) {
                (void)m_ReportProgress(eUPDATE_SYSTEM_OSY_NODE_FINISHED,
                                       C_NO_ERR, 100U, mc_CurrentNode,
                                       "Finishing device update ...");
              }
            } else {
              // flash STW Flashloader nodes
              s32_Return = m_FlashNodeXfl(
                  orc_ApplicationsToWrite[u32_NodeIndex].c_FilesToFlash,
              ...
              if ((s32_Return == C_NO_ERR) &&
                  ((orc_ApplicationsToWrite[u32_NodeIndex]
                        .c_FilesToWriteToNvm.size() > 0) ||
                   (orc_ApplicationsToWrite[u32_NodeIndex].c_PemFile != ""))) {
                // writing to NVM and PEM files are not supported by STW
                // Flashloader
                s32_Return = C_CONFIG;
              }
            }
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eUPDATE_SYSTEM_FINISHED, C_NO_ERR, 100U,
...
/*! \brief   Reset all devices in the system
   ...
   ** for openSYDE nodes: send ECU-Reset requests (do not use broadcast as we
   cannot route it)
...
int32_t C_OscSuSequences::ResetSystem(void) {
  ...
  if (this->mpc_SystemDefinition == NULL) {
    ...
    const uint32_t u32_LongestRoute =
        this->mpc_ComDriver->GetRoutingPointMaximum();
    int32_t s32_RouteSizeCounter;
    QSet<uint32_t> c_XflResetSentForBus;

    (void)m_ReportProgress(eRESET_SYSTEM_START, C_NO_ERR, 0U,
                           "Resetting System ...");

    // Reset the system beginning with the longest route.
    // Avoiding conflicts with resetting a node, which is necessary for an other
    // route
    for (s32_RouteSizeCounter = static_cast<int32_t>(u32_LongestRoute);
         s32_RouteSizeCounter >= 0; --s32_RouteSizeCounter) {
      for (uint32_t u32_Node = 0U;
           u32_Node < this->mpc_SystemDefinition->c_Nodes.size(); u32_Node++) {
        const bool q_IsNodeReachable = this->m_IsNodeReachable(u32_Node);

        if (q_IsNodeReachable == true) {
          bool q_Return;
          const uint32_t u32_RoutePointCount =
              this->mpc_ComDriver->GetRoutingPointCount(u32_Node, q_Return);

          if ((static_cast<int32_t>(u32_RoutePointCount) ==
               s32_RouteSizeCounter) &&
              (q_Return == true)) {
            uint32_t u32_BusIndex;
            bool q_RoutingActivated = false;

            s32_Return = this->mpc_ComDriver->GetBusIndexOfRoutingNode(
                u32_Node, u32_BusIndex);
            ...
            if (s32_Return == C_NO_ERR) {
              // reset nodes
              C_OscNodeProperties::E_FlashLoaderProtocol e_ProtocolType;
              q_Return = m_IsNodeActive(u32_Node, u32_BusIndex, e_ProtocolType,
                                        mc_CurrentNode);
              if (q_Return == true) {
                if (e_ProtocolType == C_OscNodeProperties::eFL_OPEN_SYDE) {
                  s32_Return = this->m_ReconnectToTargetServer();
                  ...
                  if (s32_Return == C_NO_ERR) {
                    s32_Return = this->mpc_ComDriver->SendOsyEcuReset(
                        mc_CurrentNode, C_OscProtocolDriverOsyTpBase::
                                            hu8_OSY_RESET_TYPE_KEY_OFF_ON);

                    (void)this->m_DisconnectFromTargetServer();
                  }
                } else {
                  // send reset broadcast (only required once per network):
                  if (!c_XflResetSentForBus.contains(u32_BusIndex)) {
                    // In case of routing, we need a concrete device for the
                    // reset
              ...
              if (s32_Return != C_NO_ERR) {
                ...
                if (s32_RouteSizeCounter == 0) {
                  (void)m_ReportProgress(eRESET_SYSTEM_OSY_NODE_ERROR,
                                         s32_Return, 0U, mc_CurrentNode,
                                         "Could not reset routed node.");
                } else {
                  (void)m_ReportProgress(eRESET_SYSTEM_OSY_ROUTED_NODE_ERROR,
                                         s32_Return, 0U, mc_CurrentNode,
                                         "Could not reset routed node.");
                }
                s32_Return = C_COM;
            ...
            if (q_RoutingActivated == true) {
              // Reset command does not send a response. Need a little wait time
              // to guarantee that the routing nodes really send the reset
              // command
              QThread::msleep(20);

              // Stop routing always because of cleaning up the tp and legacy
              // routing dispatcher
  ...
  if (s32_Return == C_NO_ERR) {
    (void)m_ReportProgress(eRESET_SYSTEM_FINISHED, C_NO_ERR, 100U,
                           "System was reset.");
  }
...
/*! \brief   Returns the node states of the connect process
   ...
   * ReadDeviceInformation
   ...
   \param[out]      orc_ConnectStatesNodes   Detailed output parameter
   description
...
/*! \brief   Returns the node states of the connect process
   ...
   * ReadDeviceInformation
   ...
   \param[out]      orc_UpdateStatesNodes   Detailed output parameter
   description
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSuSequences::h_FillDoFlashWithPemStates(
...
/*! \brief   Convert information read from openSYDE server node to string list
   ...
   Can be used by the application for a no-frills approach to get a textual
   representation of the information reported by
   m_ReportOpenSydeFlashloaderInformationRead.
...
void C_OscSuSequences::h_OpenSydeFlashloaderInformationToText(
  ...
  orc_Text.clear();
  orc_Text.append("Device name: " + orc_Info.c_DeviceName);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Convert information read from STW Flashloader server node to string
   list

   Can be used by the application for a no-frills approach to get a textual
   representation of the information reported by
   m_ReportStwFlashloaderInformationRead. Checksum block or sector information
...
void C_OscSuSequences::h_StwFlashloaderInformationToText(
  ...
  if (orc_Info.c_BasicInformation.q_ProtocolVersionValid == true) {
    c_Line = QString::asprintf(
        ...
        (static_cast<uint16_t>(
             orc_Info.c_BasicInformation.u16_ProtocolVersion >> 12U) &
         0x0FU),
        (static_cast<uint16_t>(
             orc_Info.c_BasicInformation.u16_ProtocolVersion >> 8U) &
         0x0FU),
        (static_cast<uint16_t>(
             orc_Info.c_BasicInformation.u16_ProtocolVersion >> 4U) &
         0x0FU),
        (static_cast<uint16_t>(
             orc_Info.c_BasicInformation.u16_ProtocolVersion) &
         0x0FU));
  } else {
  ...
  if (orc_Info.c_BasicInformation.c_AvailableFeatures.q_FingerPrint == false) {
    ...
    if (orc_Info.c_BasicInformation.c_FingerPrintData.q_SupportedIndexesValid ==
      ...
      if ((orc_Info.c_BasicInformation.c_FingerPrintData.c_SupportedIndexes
          ...
          (orc_Info.c_BasicInformation.c_FingerPrintData
               .q_ProgrammingDateValid == true)) {
        c_Line = QString::asprintf("%02d-%02d-%02d (yy-mm-dd)",
                                   orc_Info.c_BasicInformation.c_FingerPrintData
                                       .u8_ProgrammingDateYear,
                                   orc_Info.c_BasicInformation.c_FingerPrintData
                                       .u8_ProgrammingDateMonth,
                                   orc_Info.c_BasicInformation.c_FingerPrintData
                                       .u8_ProgrammingDateDay);
      } else {
        c_Line = "unknown";
      }
      ...
      if ((orc_Info.c_BasicInformation.c_FingerPrintData.c_SupportedIndexes
          ...
          (orc_Info.c_BasicInformation.c_FingerPrintData
               .q_ProgrammingTimeValid == true)) {
        c_Line = QString::asprintf("%02d:%02d:%02d",
                                   orc_Info.c_BasicInformation.c_FingerPrintData
                                       .u8_ProgrammingTimeHour,
                                   orc_Info.c_BasicInformation.c_FingerPrintData
                                       .u8_ProgrammingTimeMinute,
                                   orc_Info.c_BasicInformation.c_FingerPrintData
                                       .u8_ProgrammingTimeSecond);
      } else {
        c_Line = "unknown";
      }
      ...
      if ((orc_Info.c_BasicInformation.c_FingerPrintData.c_SupportedIndexes
          ...
          (orc_Info.c_BasicInformation.c_FingerPrintData.q_UserNameValid ==
           true)) {
      ...
      if ((orc_Info.c_BasicInformation.c_FingerPrintData.c_SupportedIndexes
          ...
          (orc_Info.c_BasicInformation.c_FingerPrintData.q_ChecksumValid ==
           true)) {
        c_Line = QString::asprintf(
            "Flash fingerprint checksum: 0x%08x",
  ...
  if (orc_Info.c_BasicInformation.q_FlashloaderVersionValid == true) {
    ...
    for (s32_Index = 0; s32_Index < rc_FlashInfo.c_ICs.size(); s32_Index++) {
      c_Line = QString::asprintf(
          "  IC %03d: size: 0x%08X; sec0: 0x%08X; regions: 0x%02X; erasetime: "
          "%ums; progtime: %ums",
          s32_Index, rc_FlashInfo.c_ICs[s32_Index].u32_TotalSize,
      ...
      for (int32_t s32_RegionIndex = 0;
           ...
           s32_RegionIndex++) {
        c_Line = QString::asprintf(
            "  IC %03d, region 0x%02X: blocksize: 0x%08X; numblocks: 0x%04X",
            s32_Index, static_cast<uint32_t>(s32_RegionIndex),
            rc_FlashInfo.c_ICs[s32_Index]
    ...
    for (s32_Index = 0; s32_Index < c_SectorTable.size(); s32_Index++) {
      ...
      if (s32_Index > 0) {
        if (c_SectorTable[s32_Index].u32_LowestAddress >
            (c_SectorTable[s32_Index - 1].u32_HighestAddress + 1)) {
          // gap of at least one byte !
          c_Line = QString::asprintf(
              "  Gap: Start: 0x%08X; Size: 0x%08X",
              c_SectorTable[s32_Index - 1].u32_HighestAddress + 1,
      ...
      u32_SectorSize = (c_SectorTable[s32_Index].u32_HighestAddress -
                        c_SectorTable[s32_Index].u32_LowestAddress) +
                       1;
      ...
      c_Line = QString::asprintf(
          "  Sector %05d: Start: 0x%08X; Size: 0x%08X; IC: %03d; Read: yes; "
          "Write: %s",
    ...
    if (rc_FlashInfo.c_Aliases.size() != 0) {
      ...
      for (s32_Index = 0; s32_Index < rc_FlashInfo.c_Aliases.size();
           s32_Index++) {
        c_Line = QString::asprintf(
            "  Aliased region %03d: Physical: 0x%08X; Aliased: 0x%08X; Size: "
            "0x%08X",
  ...
  if (orc_Info.c_BasicInformation
    ...
    orc_Text.append(
        " HEX file: Maximum record length: " +
        QString::number(orc_Info.c_BasicInformation
                            .u8_ImplementationInformationMaxHexRecordLength));
    orc_Text.append(
        " HEX file: Granularity: " +
        QString::number(orc_Info.c_BasicInformation
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscSuSequences::C_ApplicationProperties::operator==(
    const C_ApplicationProperties &orc_Source) const {
  return ((this->c_Name.trimmed() == orc_Source.c_Name.trimmed()) &&
          (this->c_Version.trimmed() == orc_Source.c_Version.trimmed()) &&
          (this->c_BuildDate.trimmed() == orc_Source.c_BuildDate.trimmed()) &&
          (this->c_BuildTime.trimmed() == orc_Source.c_BuildTime.trimmed()));
}
...
/*! \brief   Reconnects the current server
   ...
   not in case of ETH -> CAN routing! In case of ETH -> CAN routing the target
   and restarted device is not connected to Ethernet and the Ethernet routing
   device must not be reconnected to hold the routing configuration active.

   \param[in]     oq_RestartRouting    In case of ETH -> ETH routing the routing
   must be restarted if necessary
   \param[in]     ou32_NodeIndex       Node index (only used if
   oq_RestartRouting is true)
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscSuSequences::m_ReconnectToTargetServer(const bool oq_RestartRouting,
                                            const uint32_t ou32_NodeIndex) {
  uint32_t u32_BusCounter;
  int32_t s32_Return = C_NO_ERR;

  for (u32_BusCounter = 0U;
       u32_BusCounter < this->mpc_SystemDefinition->c_Buses.size();
       ++u32_BusCounter) {
    if (this->mpc_SystemDefinition->c_Buses[u32_BusCounter].u8_BusId ==
        this->mc_CurrentNode.u8_BusIdentifier) {
      if (this->mpc_SystemDefinition->c_Buses[u32_BusCounter].e_Type ==
          C_OscSystemBus::eETHERNET) {
        if (oq_RestartRouting == true) {
          if (this->mc_CurrentNode.u8_BusIdentifier !=
              this->mpc_ComDriver->GetClientId().u8_BusIdentifier) {
            // Restart routing when the routing is still necessary. Only a
            // problem when using Ethernet to Ethernet routing a target is
            // connected to Ethernet and not CAN. The routing connection on the
            // last router server will be dropped by reseting the target server.
...
/*! \brief   Disconnects the current server
   ...
   \param[in]  oq_DisconnectOnIp2IpRouting    Flag if a disconnect in case of
   Ethernet to Ethernet routing shall be executed. If the routing is still
   necessary after this call, the disconnect shall not be executed.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSuSequences::m_DisconnectFromTargetServer(
    const bool oq_DisconnectOnIp2IpRouting) {
  uint32_t u32_BusCounter;
  int32_t s32_Return = C_NO_ERR;
    ...
    if (this->mpc_SystemDefinition->c_Buses[u32_BusCounter].u8_BusId ==
      ...
      if (this->mpc_SystemDefinition->c_Buses[u32_BusCounter].e_Type ==
        ...
        if (this->mc_CurrentNode.u8_BusIdentifier ==
          ...
          this->mpc_ComDriver->GetNodeIndex(this->mc_CurrentNode,
                                            u32_NodeIndex);
          this->mpc_ComDriver->StopRouting(u32_NodeIndex);
        } else {
          // Nothing to do
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns an adapted device transfer data timeout time for
   compensating a potential higher bus load

   See C_OscUpdateUtil::h_GetAdaptedTransferDataTimeout for detailed calculation
   description.

   In a routing scenario when considering the transfer time we need to consider
   the slowest possible bus. As we support routing from Ethernet to CAN, but not
   the other way around this means that the last bus (i.e. the one connected to
   the target device) is the slowest (for our purpose we can assume CAN is
   always slower than Ethernet). However there is one scenario not covered by
   this: PC <-> CAN@LowSpeed <-> CAN@HighSpeed <-> Device. So there is room for
   further improvement.

   \param[in]       ou32_DeviceTransferDataTimeout     Device specific timeout
   time as base
   \param[in]       ou32_MaxBlockLength                Maximum number of bytes
   of each block
   \param[in]       ou8_BusIdentifier                  Bus identifier of the bus
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscSuSequences::m_GetAdaptedTransferDataTimeout(
    const uint32_t ou32_DeviceTransferDataTimeout,
  ...
  for (u32_BusCounter = 0U;
       u32_BusCounter < this->mpc_SystemDefinition->c_Buses.size();
       ++u32_BusCounter) {
    const C_OscSystemBus &rc_Bus =
        this->mpc_SystemDefinition->c_Buses[u32_BusCounter];
  ...
  if (q_IsCan == true) {
    u32_AdaptedTime = C_OscUpdateUtil::h_GetAdaptedTransferDataTimeout(
        ou32_DeviceTransferDataTimeout, ou32_MaxBlockLength, u32_Bitrate);
  }
