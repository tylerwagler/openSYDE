...
public:
  ...
  // Node is necessary for routing to an other node and caused a problem for
  // building up the route
  bool q_RoutingNodeError;
};

// States of node of steps ActivateFlashloader and ReadDeviceInformation ("Enter
// Update Mode")
class C_OscSuSequencesNodeConnectStates : public C_OscSuSequencesNodeStates {
public:
  C_OscSuSequencesNodeConnectStates(void);

  E_OscSuSequencesNodeState e_InformationRead;
  bool q_AuthenticationNecessarySet; // Flag if the check for the need of
                                     // security was made
...
class C_OscSuSequencesNodeUpdateStates : public C_OscSuSequencesNodeStates {
public:
