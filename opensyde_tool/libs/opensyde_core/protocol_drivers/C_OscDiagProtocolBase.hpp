...
/*!
   ...
   Abstract base class for diagnostic protocol (i.e.: reading/writing of server
...
private:
  C_OscDiagProtocolBase(const C_OscDiagProtocolBase
                            &orc_Source); // not implemented -> prevent copying
  C_OscDiagProtocolBase &operator=(
      const C_OscDiagProtocolBase &orc_Source); // not implemented -> prevent
                                                //  assignment

protected:
  // callbacks for reporting async responses
  // The endianness of reported value is specific to the protocol
  typedef void (*PR_DataPoolReadEventReceived)(void *const opv_Instance,
                                               const uint8_t ou8_DataPoolIndex,
                                               const uint16_t ou16_ListIndex,
                                               const uint16_t ou16_ElementIndex,
                                               const QByteArray &orc_Value);
  typedef void (*PR_DataPoolReadEventErrorReceived)(
...
public:
  ...
  void
  Initialize(const PR_DataPoolReadEventReceived opr_DataPoolReadEventReceived,
             const PR_DataPoolReadEventErrorReceived
                 opr_DataPoolReadEventErrorReceived,
  ...
  /*!
     ...
     The endianness of data returned by and passed to protocol services depends
     on the protocol used. As we do not know the content of any specific data we
     cannot swap to a defined endianness in the implementation. But we can
     return information about the endianness to the caller. The endianness
     affects binary data passed to and returned by:
     * DataPoolReadNumeric
     * DataPoolReadArray
     ...
     * PR_DataPoolReadEventReceived
     ...
     Check GetEndianness() and swap byte order to the client's native byte order
  ...
  */
  ...
  /*!
     ...
     * invoke async response function pointers for detected OK or error
     responses

     The function shall check whether another "polled" transfer is already
     ongoing (typically by using a critical section). If so, then it shall
     return without further actions (as "cycling" is in progress anyway)
  ...
  */
  ...
  /*!
     ...
     This function is used for plain numeric Datapool elements (i.e. non-array
     elements). The endianness of read data is specific to the protocol.
     ...
     \param[in,out] orc_ReadData     in: size must match expected number of
     bytes out: read data \param[out]    opu8_NrCode      if != NULL: negative
     response code in case of an error response
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolReadNumeric(const uint8_t ou8_DataPoolIndex,
                                      const uint16_t ou16_ListIndex,
                                      const uint16_t ou16_ElementIndex,
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolReadArray(const uint8_t ou8_DataPoolIndex,
                                    const uint16_t ou16_ListIndex,
                                    const uint16_t ou16_ElementIndex,
                                    QByteArray &orc_ReadData,
                                    uint8_t *const opu8_NrCode) = 0;
  ...
  /*!
     ...
     Send request and wait for response.
     This function is used for plain numeric Datapool elements (i.e. non-array
     elements). The endianness of data to write is specific to the protocol.
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolWriteNumeric(const uint8_t ou8_DataPoolIndex,
                                       const uint16_t ou16_ListIndex,
                                       const uint16_t ou16_ElementIndex,
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolWriteArray(const uint8_t ou8_DataPoolIndex,
                                     const uint16_t ou16_ListIndex,
                                     const uint16_t ou16_ElementIndex,
                                     const QByteArray &orc_DataToWrite,
                                     uint8_t *const opu8_NrCode) = 0;
  ...
  /*!
     ...
     * remember the configured rail time for starting subsequent event driven
     ...
     function) C_TIMEOUT  expected response not received within timeout C_NOACT
     could not send request (e.g. Tx buffer full) C_CONFIG   pre-requisites not
     correct; e.g. driver not initialized C_WARN     error response C_RD_WR
     malformed protocol response
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolSetEventDataRate(const uint8_t ou8_Rail,
                                           const uint16_t ou16_IntervalMs) = 0;
  ...
  /*!
     ...
     Data responses shall be handled asynchronously using the Cycle() function
     and be reported via calls to installed function pointer.

     \param[in]  ou8_DataPoolIndex   index of Datapool to read from
     \param[in]  ou16_ListIndex      list index to read from
     \param[in]  ou16_ElementIndex   element index to read
     \param[in]  ou8_Rail            0..2   "rail" referring to time interval to
     use 0: slow 1: medium 2: fast \param[out] opu8_NrCode         if != NULL:
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolReadCyclic(const uint8_t ou8_DataPoolIndex,
                                     const uint16_t ou16_ListIndex,
                                     const uint16_t ou16_ElementIndex,
                                     const uint8_t ou8_Rail,
                                     uint8_t *const opu8_NrCode) = 0;
  ...
  /*!
     ...
     \param[in]  ou16_ElementIndex   element index to read
     \param[in]  ou8_Rail            0..2   "rail" referring to time interval to
     use for maximum timeout 0: slow 1: medium 2: fast \param[in] ou32_Threshold
     threshold the value must cross before getting sent again \param[out]
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolReadChangeDriven(const uint8_t ou8_DataPoolIndex,
                                           const uint16_t ou16_ListIndex,
                                           const uint16_t ou16_ElementIndex,
                                           const uint8_t ou8_Rail,
  ...
  */
  ...
  /*!
     ...
     byte) \param[in,out] orc_DataRecord      in: size set to number of bytes to
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t NvmRead(const uint32_t ou32_MemoryAddress,
  ...
  /*!
     ...
     * NvmWriteFinalizeTransaction()
     ...
     \return
     C_NO_ERR   request sent, positive response received
     C_RANGE    parameter out of range (checked by client side); access count
     zero C_TIMEOUT  expected response not received within timeout C_NOACT could
     not send request (e.g. Tx buffer full) C_CONFIG   pre-requisites not
     correct; e.g. driver not initialized C_WARN     error response C_RD_WR
     malformed protocol response
  */
  //-----------------------------------------------------------------------------
  virtual int32_t
  NvmWriteStartTransaction(const uint8_t ou8_DataPoolIndex,
  ...
  /*!
     ...
     Usage:
     * First call NvmWriteStartTransaction(). (Here you can register any non
     zero amount of NVM write operations)
     * Execute the specified amount NVM write operations
     * Afterwards call NvmWriteFinalizeTransaction() to complete this
     transaction.

     \param[in]  ou32_MemoryAddress NVM memory address to write to (first writen
     ...
     \return
     C_NO_ERR   request sent, positive response received
     C_RANGE    parameter out of range (checked by client side); e.g. data
     record size zero C_TIMEOUT  expected response not received within timeout
     C_NOACT    could not send request (e.g. Tx buffer full)
     C_CONFIG   pre-requisites not correct; e.g. driver not initialized
     C_WARN     error response
     C_RD_WR    malformed protocol response
  */
  //-----------------------------------------------------------------------------
  virtual int32_t NvmWrite(const uint32_t ou32_MemoryAddress,
  ...
  */
  ...
  /*!
     ...
     \param[in]  ou8_DataPoolIndex   Datapool index
     \param[out] orau8_Version       Read version; format: see function
     description \param[out] orc_Name            Read name of Datapool Empty
     string if not supported \param[out] opu8_NrCode         if != NULL:
     negative response code in case of an error response
  ...
  */
  ...
  /*!
     ...
     Used to check whether the referenced Datapool matches with the passed
     criteria. Depending on the protocol this can involve verification of:
     ...
     * the version number of the Datapool
     ...
     \param[out] orq_Match                       Datapool checksum match
     response
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t DataPoolVerify(const uint8_t ou8_DataPoolIndex,
                                 const uint16_t ou16_NumberOfDataPoolElements,
                                 const uint16_t ou16_DataPoolVersion,
                                 const uint32_t ou32_DataPoolChecksum,
                                 bool &orq_Match) = 0;
  ...
  /*!
     ...
     The server responds with an information showing whether it acknowledges the
  ...
  */
  //-----------------------------------------------------------------------------
  virtual int32_t NvmNotifyOfChanges(const uint8_t ou8_DataPoolIndex,
