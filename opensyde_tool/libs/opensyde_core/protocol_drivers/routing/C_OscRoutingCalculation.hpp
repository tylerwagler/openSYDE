...
public:
  enum E_Mode ///< mode of routing
  {
    ...
    eROUTING_CHECK ///< routing calculation only for validation of an existing
                   ///< route independent of the use case
  };

  C_OscRoutingCalculation(const QList<C_OscNode> &orc_AllNodes,
                          const QByteArray &orc_ActiveNodes,
                          const uint32_t ou32_StartBusIndex,
...
private:
  ...
  QList<uint32_t>
  m_GetAllRoutePointsOfNodeOnOneInput(const C_OscRoutingRoutePoint &orc_InPoint,
  ...
  QList<C_OscRoutingRoute> mc_AllOpenRoutes;
