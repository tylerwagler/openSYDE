...
/*!
   \file
   \brief       Calculation of all potential routes to a specific node in a
...
/*! \brief   Default constructor
   ...
   system \param[in]     ou32_StartBusIndex   Start bus for the routing
...
C_OscRoutingCalculation::C_OscRoutingCalculation(
    const QList<C_OscNode> &orc_AllNodes, const QByteArray &orc_ActiveNodes,
    ...
    : mu32_StartBusIndex(ou32_StartBusIndex),
      mu32_TargetNodeIndex(ou32_TargetNodeIndex), me_Mode(oe_Mode),
      ms32_ResultState(C_COM), mq_PcBus(true), mrc_AllNodes(orc_AllNodes),
...
const C_OscRoutingRoute *C_OscRoutingCalculation::GetBestRoute(void) const {
  ...
  if (this->mc_RoutesToTarget.size() > 0) {
    ...
    for (u32_Counter = 0U; u32_Counter < this->mc_RoutesToTarget.size();
         ++u32_Counter) {
      if (this->mc_RoutesToTarget[u32_Counter].c_VecRoutePoints.size() <
          u32_CountHops) {
        u32_CountHops = static_cast<uint32_t>(
            this->mc_RoutesToTarget[u32_Counter].c_VecRoutePoints.size());
    ...
    for (u32_Counter = 0U; u32_Counter < this->mc_RoutesToTarget.size();
      ...
      if ((this->mc_RoutesToTarget[u32_Counter].c_VecRoutePoints.size() ==
        ...
        for (u32_PointCounter = 0U;
             ...
             this->mc_RoutesToTarget[u32_Counter].c_VecRoutePoints.size();
             ++u32_PointCounter) {
          if (this->mc_RoutesToTarget[u32_Counter]
                  .c_VecRoutePoints[u32_PointCounter]
...
/*! \brief   Returns the result state of the calculation
   ...
   C_CONFIG    A route was removed due to not possible routing from CAN to
   Ethernet C_NOACT     Target function (update or diagnostic) deactivated on
   all connected bus. No routing necessary.
*/
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Checks a specific interface of a specific node if it is relevant
   and usable
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscRoutingCalculation::CheckItfNumberForRouting(
  ...
  if (orc_ComItfSettings.GetBusConnected() == true) {
    if (ou32_TargetNodeIndex == this->mu32_TargetNodeIndex) {
      ...
      if ((this->me_Mode == C_OscRoutingCalculation::eDIAGNOSTIC) &&
        ...
        // Independent of any update or diagnostic functionality of the
        // interface
        q_Return = true;
      } else {
        // Nothing to do
...
void C_OscRoutingCalculation::m_SearchRoute(void) {
  ...
  if (this->mu32_TargetNodeIndex < this->mrc_AllNodes.size()) {
    // Is the node configured for update or diagnostic on its connected buses
    ...
    if (s32_Result == C_NO_ERR) {
      ...
      if (this->mc_RoutesToTarget.size() > 0) {
        ...
        osc_write_log_info("Routing calculation",
                           "No valid route found (target node index: " +
                               QString::number(this->mu32_TargetNodeIndex) +
                               "). CAN to Ethernet routing is not possible.");
        // A route was available, but was removed due to limitations. No valid
        // routes are left
        this->ms32_ResultState = s32_Result;
      } else {
        osc_write_log_info("Routing calculation",
                           "No route found (target node index: " +
                               QString::number(this->mu32_TargetNodeIndex) +
                               ")");
        // No route available
...
/*! \brief   Checks the target node configuration for a routable setting
   ...
   C_NOACT     Target node must not be routed, because the relevant function is
   deactivated on all connected buses C_COM       No bus is connected to minimum
   one activated relevant function
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscRoutingCalculation::m_CheckTargetNodeConfig(void) const {
  ...
  const C_OscNode *const pc_Node =
      &this->mrc_AllNodes[this->mu32_TargetNodeIndex];
  ...
  if (pc_Node->pc_DeviceDefinition != NULL) {
    ...
    if (pc_Node->u32_SubDeviceIndex <
      ...
      // But with one exception: A node without any functionality like a third
      // party device. This must be handled without functionality and without
      // error. All other nodes must be checked
      if ((rc_SubDevDef.q_FlashloaderOpenSydeEthernet == true) ||
        ...
        uint32_t u32_Counter;
        for (u32_Counter = 0U;
             u32_Counter < pc_Node->c_Properties.c_ComInterfaces.size();
             ++u32_Counter) {
          // Bus is connected and for the routing and check relevant
          if ((this->me_Mode == C_OscRoutingCalculation::eDIAGNOSTIC) &&
              (pc_Node->c_Properties.c_ComInterfaces[u32_Counter]
                   .q_IsDiagnosisEnabled == true)) {
            q_AtLeastOneFunctionActive = true;

            if (pc_Node->c_Properties.c_ComInterfaces[u32_Counter]
            ...
            // Independent of any update or diagnostic functionality of the
            // interface, but the error must be detected, so handle it as
            // interface with active functionality
            q_AtLeastOneFunctionActive = true;
  ...
  if (q_UsableBusFound == true) {
    // Function (diagnosis or update) is active and minimum one bus is connected
    // to the matching interface
    s32_Return = C_NO_ERR;
  } else if (q_AtLeastOneFunctionActive == true) {
    // Minimum one function (diagnosis or update) is active but no bus is
    // connected to a matching interface No routing possible
    osc_write_log_info("Routing calculation",
                       "No connected bus found (target node index: " +
                           QString::number(this->mu32_TargetNodeIndex) + ")");
    s32_Return = C_COM;
  } else {
    // Target node must not be routed, because the relevant function is
    // deactivated on all buses
    osc_write_log_info("Routing calculation",
                       "No usable bus found (target node index: " +
                           QString::number(this->mu32_TargetNodeIndex) + ")");
    s32_Return = C_NOACT;
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Searching of all routing points which has as the input interfaces
   connected to the specific bus

   All points with all interfaces which are connected to the input bus with the
   concrete bus index and the node of the interface has usable interfaces as
   output to a further bus or the node is the concrete target node will be
   registered as routing points.

   \param[in]       ou32_BusIndex     Bus index for concrete bus to analyze
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscRoutingCalculation::m_SearchRoutePointsOnBus(
  ...
  for (u32_NodeCounter = 0U; u32_NodeCounter < this->mrc_AllNodes.size();
       ++u32_NodeCounter) {
    if (this->mrc_ActiveNodes[u32_NodeCounter] == 1U) {
      ...
      for (u32_InItfCounter = 0U;
           ...
           ++u32_InItfCounter) {
        if ((pc_ActNode->c_Properties.c_ComInterfaces[u32_InItfCounter]
                 .u32_BusIndex == ou32_BusIndex) &&
            (this->CheckItfNumberForRouting(
                 u32_NodeCounter,
                 pc_ActNode->c_Properties.c_ComInterfaces[u32_InItfCounter]) ==
             true)) {
          ...
          c_VecBusesToSearchOfNode = this->m_GetAllRoutePointsOfNodeOnOneInput(
              c_PointIn, u32_InItfCounter);

          // add all new buses of this node for the next search
          for (u32_NodeBuses = 0U;
               u32_NodeBuses < c_VecBusesToSearchOfNode.size();
               ++u32_NodeBuses) {
  ...
  for (c_ItBusToSearch = c_SetBusesToSearch.begin();
       c_ItBusToSearch != c_SetBusesToSearch.end(); ++c_ItBusToSearch) {
    this->m_SearchRoutePointsOnBus(*c_ItBusToSearch);
  }
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Searching of all routing points which are possible for a specific
   input interface

   All points with the interface which is connected to the input bus and all
   other interfaces of the node which are usable interfaces as output to a
   further bus or the node is the concrete target node will be registered as
   routing points.

   \param[in]       orc_InPoint       Point with input interface information
   \param[in]       ou32_InItfNumber  The interface index of the interface set
   in orc_InPoint
*/
//----------------------------------------------------------------------------------------------------------------------
QList<uint32_t> C_OscRoutingCalculation::m_GetAllRoutePointsOfNodeOnOneInput(
    const C_OscRoutingRoutePoint &orc_InPoint,
    const uint32_t ou32_InItfNumber) {
  const C_OscNode *const pc_ActNode =
      &this->mrc_AllNodes[orc_InPoint.u32_NodeIndex];
  ...
  if (c_Point.u32_NodeIndex == this->mu32_TargetNodeIndex) {
    ...
    for (u32_OutItfCounter = 0U;
      ...
      if ((u32_OutItfCounter != ou32_InItfNumber) &&
          (this->CheckItfNumberForRouting(
               orc_InPoint.u32_NodeIndex,
               pc_ActNode->c_Properties.c_ComInterfaces[u32_OutItfCounter]) ==
           true) &&
          (orc_InPoint.u32_InBusIndex !=
           pc_ActNode->c_Properties.c_ComInterfaces[u32_OutItfCounter]
               .u32_BusIndex)) {
        // not the same interface and is connected too and is not connected to
        // same bus as the in point
        ...
        for (u32_PreviousBusCounter = 0U;
          ...
          if (pc_ActNode->c_Properties.c_ComInterfaces[u32_OutItfCounter]
                  .u32_BusIndex ==
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscRoutingCalculation::m_AddOneRoutePoint(
  ...
  for (c_ItPoint = this->mc_AllRoutePoints.begin();
       c_ItPoint != this->mc_AllRoutePoints.end(); ++c_ItPoint) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Calculation of the possible routes based on the found routing
   points
...
void C_OscRoutingCalculation::m_CalculateRoutes(const uint32_t ou32_BusIndex) {
  ...
  for (u32_PointCounter = 0U; u32_PointCounter < this->mc_AllRoutePoints.size();
       ++u32_PointCounter) {
    if (this->mc_AllRoutePoints[u32_PointCounter].u32_InBusIndex ==
      ...
      if (this->mq_PcBus == true) {
        ...
        if (this->mc_AllRoutePoints[u32_PointCounter].u32_NodeIndex ==
            this->mu32_TargetNodeIndex) {
          ...
          c_Route.c_VecRoutePoints.push_back(
              this->mc_AllRoutePoints[u32_PointCounter]);
          this->mc_AllOpenRoutes.push_back(c_Route);
          // Check the next bus of this routing point
          c_SetBussesToSearch.insert(
              this->mc_AllRoutePoints[u32_PointCounter].u32_OutBusIndex);
        }
      } else {
        // Searching the matching routing points on the previous routes
        const QList<C_OscRoutingRoute> c_AllOldOpenRoutes =
            this->mc_AllOpenRoutes;
        QList<C_OscRoutingRoute>::const_iterator c_ItOldRoute;

        // Further searching
        // The input bus of the actual route point must be equal to the output
        // bus of the last routing point of the route One node shall be used in
        // a route maximum one time
        for (c_ItOldRoute = c_AllOldOpenRoutes.begin();
             c_ItOldRoute != c_AllOldOpenRoutes.end(); ++c_ItOldRoute) {
          if ((*c_ItOldRoute).c_VecRoutePoints.size() > 0) {
            const QList<C_OscRoutingRoutePoint> *const pc_RoutePoints =
                &((*c_ItOldRoute).c_VecRoutePoints);
            const uint32_t u32_RoutePointIndex =
                static_cast<uint32_t>(pc_RoutePoints->size() - 1U);
            const C_OscRoutingRoutePoint *const pc_LastPointOfRoute =
                &((*pc_RoutePoints)[u32_RoutePointIndex]);
            ...
            if (this->mc_AllRoutePoints[u32_PointCounter].u32_InBusIndex ==
              ...
              if (this->mc_AllRoutePoints[u32_PointCounter].u32_NodeIndex ==
                ...
                // No further analysis of this variant, do not add it the open
                // routes again.
                ...
                if (q_AlreadyUsed == false) {
                  ...
                  c_NewRoute.c_VecRoutePoints.push_back(
                      this->mc_AllRoutePoints[u32_PointCounter]);
  ...
  // search the other buses recursively
  for (c_ItBusToSearch = c_SetBussesToSearch.begin();
       c_ItBusToSearch != c_SetBussesToSearch.end(); ++c_ItBusToSearch) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscRoutingCalculation::m_AddOneOpenRoute(
  ...
  for (c_ItRoute = this->mc_AllOpenRoutes.begin();
       c_ItRoute != this->mc_AllOpenRoutes.end(); ++c_ItRoute) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscRoutingCalculation::m_AddOneRouteToTarget(
    const C_OscRoutingRoute &orc_Route) {
  // add the route only if it is not already in the vector
  QList<C_OscRoutingRoute>::const_iterator c_ItRoute;
  bool q_Found = false;

  for (c_ItRoute = this->mc_RoutesToTarget.begin();
       c_ItRoute != this->mc_RoutesToTarget.end(); ++c_ItRoute) {
    if (orc_Route == (*c_ItRoute)) {
      q_Found = true;
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Checks all routes to target for limitations and removes the not
   valid routes

   Routing from CAN as in and Ethernet as out is not supported. These routes are
   not usable.

   \retval   C_NO_ERR    Route is valid and usable
   \retval   C_CONFIG    A route was removed due to not possible routing from
   CAN to Ethernet
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscRoutingCalculation::m_CheckRoutesForLimitations(void) {
  ...
  // Check all routes
  for (c_ItRoute = this->mc_RoutesToTarget.begin();
       c_ItRoute != this->mc_RoutesToTarget.end();) {
    ...
    for (u32_PointCounter = 0U;
         u32_PointCounter < rc_Route.c_VecRoutePoints.size();
         ++u32_PointCounter) {
      const C_OscRoutingRoutePoint &rc_Point =
          rc_Route.c_VecRoutePoints[u32_PointCounter];

      // for the limitations. No CAN to Ethernet routing. The other direction is
      // possible.
