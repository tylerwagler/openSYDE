...
class C_OscProtocolDriverOsyTpCan : public C_OscProtocolDriverOsyTpBase {
private:
  ...
  public:
    ...
    uint32_t u32_SendCfTimeoutDuration; ///< Tx: calculated timeout duration for
                                        ///< sending all CFs
  };

  /// CAN-TP PCI types:
  static const uint8_t mhu8_ISO15765_N_PCI_SF = 0x00U; // single frame <=8bytes
  static const uint8_t mhu8_ISO15765_N_PCI_FF =
      0x10U; // first frame <=4095bytes
  ...
  static const uint8_t mhu8_ISO15765_N_PCI_OSF =
      0xF0U; // openSYDE single frame (special handling of message
  ...
  static const uint8_t mhu8_OSY_OSF_TYPE_EVENT_DRIVEN_DP_SINGLE_FRAME =
      0x01U; // openSYDE single frame
  ...
  static const uint16_t mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART1 =
      0x0216U;
  static const uint16_t mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART2 =
      0x0217U;
  static const uint16_t mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART3 =
      0x0218U;
  static const uint16_t
      mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_EXT_START = 0x0219U;
  static const uint16_t
      mhu16_OSY_BC_RC_SID_SET_NODEID_BY_SERIALNUMBER_EXT_LAST = 0x0224U;
  static const uint8_t mhu8_BC_OSY_NR_SI = 0x7FU;

  /// timeout when waiting for flow control; 1000ms specified in 15765
  /// for a local connection 1000ms sounds a lot but there might be more complex
  /// scenarios
  ///  e.g. with routing over multiple hops
  static const uint16_t mhu16_NBS_TIMEOUTS_MS = 1000U;

  stw::can::C_CanDispatcher
      *mpc_CanDispatcher; ///< CAN dispatcher to use for communication
  uint16_t
      mu16_DispatcherClientHandle; ///< our handle for dispatcher interaction
  ...
  int32_t
  m_HandleIncomingSingleFrame(const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  int32_t m_HandleIncomingOsySpecificSingleFrame(
      const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  int32_t
  m_HandleIncomingFirstFrame(const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  int32_t
  m_HandleIncomingFlowControl(const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  int32_t m_HandleIncomingConsecutiveFrame(
      const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  int32_t m_HandleIncomingOsySpecificMultiFrame(
      const stw::can::T_STWCAN_Msg_RX &orc_CanMessage);
  int32_t
  m_BroadcastSendDiagnosticSessionControl(const uint8_t ou8_Session) const;

  int32_t m_SendNextConsecutiveFrames(void);

  static void
  mh_ComposeSingleFrame(const C_OscProtocolDriverOsyService &orc_Service,
                        const uint32_t ou32_Identifier,
                        stw::can::T_STWCAN_Msg_TX &orc_CanMessage);

  int32_t m_HandleBroadcastSetNodeIdBySerialNumberResponse(
      const uint8_t ou8_RoutineIdMsb, const uint8_t ou8_RoutineIdLsb,
      uint8_t *const opu8_NrCode = NULL) const;

protected:
  void m_LogWarningWithHeader(const QString &orc_Information,
...
public:
  ...
  public:
    uint8_t u8_SubNodeId; ///< sub node id of sender in case of a multiple CPU
                          ///< device
  ...
  explicit C_OscProtocolDriverOsyTpCan(
      const uint16_t ou16_MaxServiceQueueSize = 200U);
  virtual ~C_OscProtocolDriverOsyTpCan(void);

  virtual int32_t Cycle(void);
  virtual int32_t
  SetNodeIdentifiers(const C_OscProtocolDriverOsyNode &orc_ClientIdentifier,
                     const C_OscProtocolDriverOsyNode &orc_ServerIdentifier);
  int32_t SetNodeIdentifiersForBroadcasts(
  ...
  int32_t BroadcastReadSerialNumber(
