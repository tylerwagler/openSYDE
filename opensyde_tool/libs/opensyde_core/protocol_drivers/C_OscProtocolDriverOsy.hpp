...
private:
  typedef void (*PR_OsyTunnelCanMessageReceived)(
      void *const opv_Instance, const uint8_t ou8_CanChannel,
  ...
  C_OscProtocolDriverOsyTpBase
      *mpc_TransportProtocol; ///< installed transport protocol
  ...
  int32_t
  m_Cycle(const bool oq_CheckForSpecificServiceId = false,
          const uint8_t ou8_ExpectedServiceId = 0xFFU,
          C_OscProtocolDriverOsyService *const opc_ReceivedService = NULL);
  // poll for exactly one incoming service:
  int32_t m_PollForSpecificServiceResponse(
  ...
  void m_UnpackDataPoolIdentifier(const uint8_t (&orau8_PackedId)[3],
                                  uint8_t &oru8_DataPoolIndex,
                                  uint16_t &oru16_ListIndex,
  ...
  int32_t m_HandleAsyncOsyReadDataPoolDataEvent(
      const C_OscProtocolDriverOsyService &orc_ReceivedService);
  int32_t m_HandleAsyncOsyReadDataPoolDataErrorEvent(
      const C_OscProtocolDriverOsyService &orc_ReceivedService);
  int32_t m_HandleAsyncOsyTunnelCanMessagesEvent(
      const C_OscProtocolDriverOsyService &orc_ReceivedService);

  uint8_t
  m_GetDataPoolIndexClientToServer(const uint8_t ou8_ClientDataPoolIndex) const;
  uint8_t
  ...
  static const uint16_t mhu16_OSY_DI_PROTOCOL_DRIVER_IMPLEMENTATION_VERSION =
      0xA814U;
  ...
  // routine identifiers
  static const uint16_t mhu16_OSY_RC_SID_ROUTE_DIAGNOSIS_COMMUNICATION =
      0x0202U;
  ...
  static const uint16_t mhu16_OSY_RC_SID_CHECK_FLASH_MEMORY_AVAILABILITY =
      0x0208U;
  static const uint16_t mhu16_OSY_RC_SID_READ_FLASH_BLOCK_DATA = 0x0209U;
  static const uint16_t
      mhu16_OSY_RC_SID_CONFIGURE_FLASHLOADER_COMMUNICATION_CHANNEL = 0x0210U;
  ...
  static const uint16_t mhu16_OSY_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART1 =
      0x0216U;
  static const uint16_t mhu16_OSY_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART2 =
      0x0217U;
  static const uint16_t mhu16_OSY_RC_SID_SET_NODEID_BY_SERIALNUMBER_PART3 =
      0x0218U;
...
protected:
  void m_LogErrorWithHeader(const QString &orc_Activity,
                            const QString &orc_Information,
                            const char_t *const opcn_Function,
                            const bool oq_AsError = true) const;
...
public:
  ...
  // lint
  // -sem(stw::opensyde_core::C_OscProtocolDriverOsy::C_FlashBlockInfo::ClearContent,initializer)
  ...
  public:
    ...
    QString c_AdditionalInformation;
  ...
  public:
    ...
    bool q_FlashloaderCanWriteToNvm; ///< set to true in Flashloader to show
                                     ///< that "Writing to NVM" is
                                     // supported
    ...
    bool q_SupportsDebuggerOff; ///< true: The debugger interface of the device
                                ///< can be deactivated
    bool q_SupportsDebuggerOn;  ///< true: The debugger interface of the device
                                ///< can be activated
  };
  ...
  void InitializeTunnelCanMessage(
      const PR_OsyTunnelCanMessageReceived opr_OsyTunnelCanMessageReceived,
      void *const opv_Instance);
  void
  InitializeHandleWaitTime(const PR_OsyHandleWaitTime opr_OsyHandleWaitTime,
                           void *const opv_Instance);
  ...
  static QString
  h_GetOpenSydeServiceErrorDetails(const int32_t os32_FunctionResult,
                                   const uint8_t ou8_NrCode,
                                   bool *const opq_IsHardError = NULL);

  // openSYDE protocol services:
  // Session management:
  int32_t OsyDiagnosticSessionControl(const uint8_t ou8_SessionId,
                                      uint8_t *const opu8_NrCode = NULL);

  // Device information services:
  int32_t OsyReadEcuSerialNumber(C_OscProtocolSerialNumber &orc_SerialNumber,
                                 uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadHardwareNumber(uint32_t &oru32_HardwareNumber,
                                uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadHardwareVersionNumber(QString &orc_HardwareVersionNumber,
                                       uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadListOfFeatures(C_ListOfFeatures &orc_ListOfFeatures,
                                uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadMaxNumberOfBlockLength(uint16_t &oru16_MaxNumberOfBlockLength,
                                        uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadDeviceName(QString &orc_DeviceName,
                            uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadApplicationName(QString &orc_ApplicationName,
                                 uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadApplicationVersion(QString &orc_ApplicationVersion,
                                    uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadActiveDiagnosticSession(uint8_t &oru8_SessionId,
                                         uint8_t *const opu8_NrCode = NULL);
  ...
  int32_t OsyReadFlashloaderProtocolVersion(uint8_t (&orau8_Version)[3],
                                            uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadFlashCount(uint32_t &oru32_FlashCount,
                            uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadApplicationSoftwareFingerprint(
      uint8_t (&orau8_Date)[3], uint8_t (&orau8_Time)[3], QString &orc_Username,
      uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadFileBasedTransferExitResult(QString &orc_TransferExitResult,
                                             uint8_t *const opu8_NrCode = NULL);
  int32_t
  OsyReadEcuSerialNumberExt(C_OscProtocolSerialNumber &orc_SerialNumberExt,
                            uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadSubNodeId(uint8_t &oru8_SubNodeId,
                           uint8_t *const opu8_NrCode = NULL);

  // Device configuration
  int32_t
  OsySetNodeIdForChannel(const uint8_t ou8_ChannelType,
                         const uint8_t ou8_ChannelIndex,
                         const C_OscProtocolDriverOsyNode &orc_NewNodeId,
                         uint8_t *const opu8_NrCode = NULL);
  int32_t OsySetBitrate(const uint8_t ou8_ChannelType,
                        const uint8_t ou8_ChannelIndex,
                        const uint32_t ou32_Bitrate,
                        uint8_t *const opu8_NrCode = NULL);
  int32_t OsyConfigureFlashloaderCommunicationChannel(
      const uint8_t ou8_ChannelType, const uint8_t ou8_ChannelIndex,
      const bool oq_Activated, uint8_t *const opu8_NrCode = NULL);
  int32_t OsySetIpAddressForChannel(const uint8_t ou8_ChannelType,
                                    const uint8_t ou8_ChannelIndex,
  ...
  int32_t OsySetRouteDiagnosisCommunication(
  ...
  int32_t OsySetRouteIp2IpCommunication(const uint8_t ou8_OutputChannelType,
                                        const uint8_t ou8_OutputChannelIndex,
                                        const uint8_t ou8_SourceBusId,
                                        const uint8_t ou8_TargetBusId,
                                        const uint8_t (&orau8_IpAddress)[4],
                                        uint8_t *const opu8_NrCode = NULL);
  int32_t OsyCheckRouteIp2IpCommunication(uint8_t &oru8_Status,
                                          uint8_t *const opu8_NrCode = NULL);
  int32_t OsySendCanMessage(const uint8_t ou8_ChannelIndex,
                            const stw::can::T_STWCAN_Msg_TX &orc_CanMessage,
                            uint8_t *const opu8_NrCode = NULL);
  int32_t OsySetTunnelCanMessages(const uint8_t ou8_CanChannelIndex,
  ...
  int32_t OsyWriteDataPoolEventDataRate(const uint8_t ou8_TransmissionRail,
                                        ...
                                        uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadDataPoolDataCyclic(const uint8_t ou8_DataPoolIndex,
                                    const uint16_t ou16_ListIndex,
                                    const uint16_t ou16_ElementIndex,
                                    const uint8_t ou8_TransmissionRail,
                                    uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadDataPoolDataChangeDriven(const uint8_t ou8_DataPoolIndex,
                                          const uint16_t ou16_ListIndex,
                                          const uint16_t ou16_ElementIndex,
                                          const uint8_t ou8_TransmissionRail,
  ...
  int32_t OsyReadDataPoolMetaData(const uint8_t ou8_DataPoolIndex,
                                  C_DataPoolMetaData &orc_MetaData,
                                  uint8_t *const opu8_NrCode = NULL);
  int32_t OsyVerifyDataPool(const uint8_t ou8_DataPoolIndex,
  ...
  int32_t OsyCheckFlashMemoryAvailable(const uint32_t ou32_StartAddress,
                                       const uint32_t ou32_Size,
                                       uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadFlashBlockData(const uint8_t ou8_FlashBlock,
                                C_FlashBlockInfo &orc_BlockInfo,
                                uint8_t *const opu8_NrCode = NULL);
  int32_t OsySecurityAccessRequestSeed(const uint8_t ou8_SecurityLevel,
                                       bool &orq_SecureMode,
                                       uint64_t &oru64_Seed,
  ...
  int32_t OsyRequestDownload(const uint32_t ou32_StartAddress,
                             ...
                             uint8_t *const opu8_NrCode = NULL);
  int32_t OsyRequestFileTransfer(const QString &orc_FilePath,
  ...
  OsyRequestTransferExitAddressBased(const bool oq_SendSignatureBlockAddress,
                                     ...
                                     uint8_t *const opu8_NrCode = NULL);
  int32_t OsyRequestTransferExitFileBased(const uint8_t (&orau8_Signature)[8],
                                          uint8_t *const opu8_NrCode = NULL);
  int32_t OsyWriteApplicationSoftwareFingerprint(
      const uint8_t (&orau8_Date)[3], const uint8_t (&orau8_Time)[3],
      const QString &orc_UserName, uint8_t *const opu8_NrCode = NULL);
  int32_t OsyFactoryMode(const uint8_t ou8_Operation,
                         uint8_t *const opu8_NrCode = NULL);
  ...
  int32_t OsyReadSecurityActivation(bool &orq_SecurityOn,
                                    uint8_t &oru8_SecurityAlgorithm,
                                    uint8_t *const opu8_NrCode = NULL);
  int32_t OsyWriteSecurityActivation(const bool oq_SecurityOn,
                                     const uint8_t ou8_SecurityAlgorithm,
                                     uint8_t *const opu8_NrCode = NULL);
  int32_t OsyReadDebuggerEnabled(bool &orq_DebuggerEnabled,
                                 uint8_t *const opu8_NrCode = NULL);
  int32_t OsyWriteDebuggerEnabled(const bool oq_DebuggerEnabled,
                                  uint8_t *const opu8_NrCode = NULL);

  // Common
  int32_t OsyTesterPresent(const uint8_t ou8_SuppressResponseMsg,
                           uint8_t *const opu8_NrCode = NULL);
  int32_t
  OsyEcuReset(const uint8_t ou8_ResetType =
                  C_OscProtocolDriverOsyTpBase::hu8_OSY_RESET_TYPE_KEY_OFF_ON);

  // Default Timeout
  static const uint32_t hu32_DEFAULT_TIMEOUT = 1000U; // In ms
  // Cyclic time till the registered function mpr_OnOsyWaitTime will be called
  // in m_PollForSpecificServiceResponse
  ...
  static const uint8_t hu8_NR_CODE_SERVICE_NOT_SUPPORTED_IN_ACTIVE_SESSION =
      0x7FU;
