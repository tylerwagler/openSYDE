 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscDcBasicSequences::C_OscDcBasicSequences(void) : mpc_CanDispatcher(NULL) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDcBasicSequences::Init(
  ...
  m_ReportProgress(
      s32_Return,
      "Starting the initialization of CAN driver and protocol ... ");
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (s32_Return != C_NO_ERR) {
      osc_write_log_warning(
          c_LogActivity, "Could not configure the clients broadcast node ID.");
    }
  }
...
/*! \brief  Bring openSYDE devices in flashloader state
   ...
   * send openSYDE "DiagnosticSessionControl(PreProgramming)" broadcast for 5
   seconds every 5 milliseconds

   The com driver is expected to be initialized for broadcasting as it is done
   in Init().
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDcBasicSequences::ScanEnterFlashloader(
    const uint32_t ou32_FlashloaderResetWaitTime) {
  int32_t s32_Return = C_NO_ERR;
  const QString c_LogActivity = "Scan Activate Flashloader";
  ...
  m_ReportProgress(s32_Return,
                   "Starting the scan for flashloader activation ... ");
  ...
  m_ReportProgress(C_NO_ERR, "Broadcasting \"request programming\" flag: " +
                                 QString::number(c_Results.size()) +
                                 " device(s) answered. ");

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(c_LogActivity,
                        "openSYDE ECU reset broadcast failed with error: " +
                            QString::number(s32_Return));
  } else {
    // check results for request accepted flags
    for (uint32_t u32_ResponseIndex = 0U; u32_ResponseIndex < c_Results.size();
         u32_ResponseIndex++) {
  ...
  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(c_LogActivity,
                        "At least one reached device did not accept setting "
                        "the \"request programming\" flag.");
  } else {
    ...
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(c_LogActivity,
                          "openSYDE request node reset failed with error: " +
                              QString::number(s32_Return));
    }
  }
...
/*! \brief  Scan for connected openSYDE devices
   ...
   * The com driver is initialized and has set up for broadcasting as it is done
   in Init()
...
int32_t C_OscDcBasicSequences::ScanGetInfo(void) {
  ...
  m_ReportProgress(s32_Return,
                   "Starting the scan for getting devices information ... ");

  // broadcast: "ReadSerialNumber"
  s32_Return = this->mc_TpCan.BroadcastReadSerialNumber(c_ReadSnResult,
                                                        c_ReadSnResultExt);
...

  {
    ...
    osc_write_log_info("Scan CAN for openSYDE devices",
                       "Sequence finished. Standard nodes found: " +
                           QString::number(c_ReadSnResult.size()));
    osc_write_log_info("Scan CAN for openSYDE devices",
                       "Sequence finished. Extended nodes found: " +
                           QString::number(c_ReadSnResultExt.size()));

    // Fill the result container with the standard results
    for (u32_ResultCounter = 0U; u32_ResultCounter < c_ReadSnResult.size();
         ++u32_ResultCounter) {
      C_OscDcDeviceInformation c_DeviceInfo;
      c_DeviceInfo.SetSerialNumber(
          c_ReadSnResult[u32_ResultCounter].c_SerialNumber);
      c_DeviceInfo.SetNodeId(
          c_ReadSnResult[u32_ResultCounter].c_SenderId.u8_NodeIdentifier);
      c_DeviceInfoResult.push_back(c_DeviceInfo);
    }

    // Fill the result container with the extended results
    for (u32_ResultCounter = 0U; u32_ResultCounter < c_ReadSnResultExt.size();
         ++u32_ResultCounter) {
      C_OscDcDeviceInformation c_DeviceInfo;
      // Serial number length is not necessary, it was already cross checked
      // with the string
      c_DeviceInfo.SetSerialNumber(
          c_ReadSnResultExt[u32_ResultCounter].c_SerialNumber);
      c_DeviceInfo.SetNodeId(
          c_ReadSnResultExt[u32_ResultCounter].c_SenderId.u8_NodeIdentifier);
      c_DeviceInfo.SetExtendedInfo(
          c_ReadSnResultExt[u32_ResultCounter].u8_SubNodeId,
      ...
      // accident
      q_SecurityFeatureUsed |=
          c_ReadSnResultExt[u32_ResultCounter].q_SecurityActivated;
      c_DeviceInfoResult.push_back(c_DeviceInfo);
    }

    // Search all unique ID's
    for (u32_ResultCounter = 0U; u32_ResultCounter < c_DeviceInfoResult.size();
         ++u32_ResultCounter) {
      ...
      for (uint32_t u32_UniqueIdCheckCounter = 0U;
        ...
        if ((u32_ResultCounter != u32_UniqueIdCheckCounter) &&
          ...
          if (q_SecurityFeatureUsed == true) {
            // Special case: In case of at least one node with active security
            // no broadcasts can be used by the configuration sequence.
            // Therefore the node IDs must be unique for using direct
            // communication.
            s32_Return = C_CHECKSUM;

            osc_write_log_error(
                c_LogActivity,
                "At least one node has the security feature activated and at "
                "least"
    ...
    if (s32_Return == C_NO_ERR)
    {
       ...
       {
          s32_Return =
    this->mpc_ComDriver->SendStwSendFlash(this->mc_CurrentNode);

          if (s32_Return != C_NO_ERR)
          {
             (void)m_ReportProgress(eACTIVATE_FLASHLOADER_XFL_BC_FLASH_ERROR,
    s32_Return, 50U, mc_CurrentNode, "Send \"FLASH\" broadcast failed.");
             s32_Return = C_COM;
             break;
          }
    ...
    */
    ...
    Q_ASSERT((c_ReadSnResult.size() + c_ReadSnResultExt.size()) ==
             c_DeviceInfoResult.size());
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (u32_UniqueIdIndicesCounter = 0U;
        ...
        const uint32_t u32_DeviceInfoIndex =
            c_UniqueIdIndices[u32_UniqueIdIndicesCounter];
        ...
        if (s32_Return != C_NO_ERR) {
          osc_write_log_error(c_LogActivity,
                              "Could not configure the node IDs! Is the server "
                              "node ID within range?");
        } else {
          s32_Return =
              mc_OsyProtocol.OsyReadDeviceName(c_Result, &u8_NumberCode);
        }
    ...
    m_ReportProgress(s32_Return,
                     "Found " + QString::number(c_DeviceInfoResult.size()) +
                         " device(s).");
  ...
  /*! \brief  Broadcast to reset devices
     ...
     initialized for broadcasting as it is done in Init().
