...
// we do not use the unspecified values; just "pass through" to get the clearly
// defined structure "struct tm" lint -estring(829,ctime) lint
// -estring(586,time) lint -estring(586,localtime)
#include <ctime>
...
/*! \brief   Default constructor
   ...
   \param[in]  oq_HexActive                 Mode for writing CAN Id and CAN data
   (hexadecimal or decimal) \param[in]  oq_RelativeTimeStampActive   Mode for
   writing CAN timestamp (relative or absolute)
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscComMessageLoggerFileAsc::C_OscComMessageLoggerFileAsc(
    const QString &orc_FilePath, const QString &orc_ProtocolName,
    const bool oq_HexActive, const bool oq_RelativeTimeStampActive)
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Creates, if necessary, and opens file and adds the default header
   of the file.
...
int32_t C_OscComMessageLoggerFileAsc::OpenFile(void) {
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (!QFileInfo(this->mc_FilePath).exists() ||
        !QFileInfo(this->mc_FilePath).isFile()) {
...
void C_OscComMessageLoggerFileAsc::AddMessageToFile(
  ...
  if (this->mc_File.is_open() == true) {
    ...
    if (this->mq_RelativeTimeStampActive == true) {
      ...
      c_LogEntry +=
          mh_AdaptTimeStamp(orc_MessageData.u64_TimeStampAbsoluteStart);
    }
    ...
    // CAN Id
    // ASC specification defines a width of fixed 15 chars for CAN Id. Plus one
    // for the space to the direction
    if (this->mq_HexActive == true) {
      c_Temp = QString::asprintf(
          "%-16s", orc_MessageData.c_CanIdHex.toUtf8().constData());
    } else {
      c_Temp = QString::asprintf(
          "%-16s", orc_MessageData.c_CanIdDec.toUtf8().constData());
    }
    c_LogEntry += c_Temp;

    // Direction
    // ASC specification defines a width of fixed 4 chars for direction. Plus
    // one for the space to the next section
    ...
    // Detected signals
    for (u32_SignalCounter = 0U;
         u32_SignalCounter < orc_MessageData.c_Signals.size();
         u32_SignalCounter++) {
      const C_OscComMessageLoggerDataSignal &rc_Signal =
          orc_MessageData.c_Signals[u32_SignalCounter];
      c_LogEntry +=
          "   //Signal   " + rc_Signal.c_Name + " = " + rc_Signal.c_Value;
      if (rc_Signal.c_Unit != "") {
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_OscComMessageLoggerFileAsc::mh_AdaptTimeStamp(const uint64_t ou64_TimeStamp) {
  QString c_TimeStamp;

  c_TimeStamp = QString::asprintf(
      "%d.%.6d", static_cast<int32_t>(ou64_TimeStamp / 1000000ULL),
