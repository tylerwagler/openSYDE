...
/*!
   \file
   \brief       Core communication driver for flashloader protocols
   (implementation)
 ...
 */
extern void
XFLSetInternalKey(const uint16_t ou16_Key); // lint !e526 !e2701 !e8001 //very
                                            // special case here
...
/*! \brief   Default constructor
   ...
   \param[in]  oq_UpdateRoutingMode          Flag for update specific routing or
   generic routing (m_GetRoutingMode)
   \param[in]  opr_XflReportProgress         function to call if STW Flashloader
   driver has something to report
   \param[in]  opv_XflReportProgressInstance Instance pointer to pass when
   invoking opv_XflReportProgressInstance
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscComDriverFlash::C_OscComDriverFlash(
    const bool oq_RoutingActive, const bool oq_UpdateRoutingMode,
    const C_OscFlashProtocolStwFlashloader::PR_ReportProgress
        opr_XflReportProgress,
    void *const opv_XflReportProgressInstance)
    : C_OscComDriverProtocol(), pr_XflReportProgress(opr_XflReportProgress),
...
C_OscComDriverFlash::~C_OscComDriverFlash(void) {
  ...
  for (u32_Counter = 0U; u32_Counter < this->mc_StwFlashProtocols.size();
       ++u32_Counter) {
...
/*! \brief   Initialize all members
   ...
   \param[in]  orc_ActiveNodes         Flags for all available nodes in the
   system
   ...
   \param[in]  opc_SecurityPemDb       Pointer to PEM database (optional)
                                       Needed if nodes with enabled security are
   used in the system
...
C_OscComDriverFlash::Init(const C_OscSystemDefinition &orc_SystemDefinition,
  ...
  int32_t s32_Return = C_OscComDriverProtocol::Init(
      orc_SystemDefinition, ou32_ActiveBusIndex, orc_ActiveNodes,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverFlash::InitCanAndSetCanBitrate(const uint32_t ou32_Bitrate) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::OsySetPollingTimeout(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::OsyResetPollingTimeout(
...
/*! \brief  Returns the minimum Flashloader reset wait time in ms
   ...
   \return
   Time in ms all nodes need at least to get from application to the Flashloader
   or from Flashloader to Flashloader
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscComDriverFlash::GetMinimumFlashloaderResetWaitTime(
    const E_MinimumFlashloaderResetWaitTimeType oe_Type) const {
  uint32_t u32_WaitTime = 500U;
  uint32_t u32_Counter;

  for (u32_Counter = 0U; u32_Counter < this->mc_ActiveNodesIndexes.size();
       ++u32_Counter) {
    uint32_t u32_NodeWaitTime = 0;
    Q_ASSERT(m_GetMinimumFlashloaderResetWaitTime(
                 oe_Type, this->mc_ActiveNodesIndexes[u32_Counter],
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Returns the minimum Flashloader reset wait time in ms for a specific
   node
   ...
   \param[out]  oru32_TimeValue     Time in ms the node need at least to get
   from application to the Flashloader or from Flashloader to Flashloader

   \retval   C_NO_ERR   Time returned
   \retval   C_RANGE    Node with orc_ServerId does not exist or is not active
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::GetMinimumFlashloaderResetWaitTime(
    const E_MinimumFlashloaderResetWaitTimeType oe_Type,
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (q_Found == true) {
    s32_Return = this->m_GetMinimumFlashloaderResetWaitTime(
        oe_Type, this->mc_ActiveNodesIndexes[u32_ActiveNodeIndex],
        oru32_TimeValue);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Initialize and prepare the temporary (transport) protocol and
   connect to a openSYDE node

   \param[in]       orc_ServerId            node to connect to
   \param[in]       orau8_IpAddress         IP address to connect to
   \param[in,out]   orc_TemporaryProtocol   Temporary protocol object for
   initialization and using
   \param[in,out]   orc_TpIp                Temporary transport protocol object
   for initialization and using
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::EthConnectNode(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      if (s32_Return == C_NO_ERR) {
        s32_Return = orc_TemporaryProtocol.SetNodeIdentifiers(
            this->GetClientId(), orc_ServerId);
      }
    }
  }
...
/*! \brief   Disconnects of a specific and temporary protocol connection
  ...
  \param[in]   orc_TemporaryProtocol   Temporary protocol object for
  initialization and using
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::h_EthDisconnectNode(
    C_OscProtocolDriverOsy &orc_TemporaryProtocol) {
...
/*! \brief   Send request programming
   ...
   \param[out]    orq_NotAccepted   true: at least one "not accepted" response
   was received false: no "not accepted" response was received
   ...
   C_CONFIG   no transport protocol installed or broadcast protocol not
   initialized
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyBroadcastRequestProgramming(
    bool &orq_NotAccepted) const {
  int32_t s32_Return;
  ...
  if (this->mpc_CanTransportProtocolBroadcast != NULL) {
    ...
    s32_Return =
        this->mpc_CanTransportProtocolBroadcast->BroadcastRequestProgramming(
            c_Results);
    if (s32_Return == C_NO_ERR) {
      for (uint32_t u32_ResponseIndex = 0U;
           u32_ResponseIndex < c_Results.size(); u32_ResponseIndex++) {
...
/*! \brief   Send network reset broadcast
   ...
   \param[in]  ou8_ResetType        Reset type (0x02: keyOffOnReset, 0x60:
   resetToFlashloader)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyBroadcastEcuReset(
  ...
  if ((this->mpc_CanTransportProtocolBroadcast == NULL) &&
    ...
    if (this->mpc_CanTransportProtocolBroadcast != NULL) {
      s32_Return = this->mpc_CanTransportProtocolBroadcast->BroadcastEcuReset(
          ou8_ResetType);
    } else {
      s32_Return = this->mpc_IpTransportProtocolBroadcast->BroadcastNetReset(
          ou8_ResetType);
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverFlash::SendOsyCanBroadcastEnterPreProgrammingSession(void) const {
  int32_t s32_Return;

  if (this->mpc_CanTransportProtocolBroadcast == NULL) {
    s32_Return = C_CONFIG;
  } else {
    s32_Return = this->mpc_CanTransportProtocolBroadcast
                     ->BroadcastSendEnterPreProgrammingSession();
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverFlash::SendOsyCanBroadcastEnterDefaultSession(void) const {
  int32_t s32_Return;
  ...
  if (this->mpc_CanTransportProtocolBroadcast == NULL) {
    ...
    s32_Return = this->mpc_CanTransportProtocolBroadcast
                     ->BroadcastSendEnterDefaultSession();
  }
...
/*! \brief   Read serial numbers of all devices on local bus
   ...
   \param[out]    orc_Responses           information about all nodes that sent
   a response
   \param[out]    orc_ExtendedResponses   information about all nodes that sent
   an extended response
   \return
   C_NO_ERR   no problems; zero or more responses received; data placed in
   orc_Responses C_COM      could not send request C_CONFIG   no dispatcher
   installed or broadcast protocol not initialized
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyCanBroadcastReadSerialNumber(
  ...
  if (this->mpc_CanTransportProtocolBroadcast == NULL) {
    ...
    s32_Return =
        this->mpc_CanTransportProtocolBroadcast->BroadcastReadSerialNumber(
            orc_Responses, orc_ExtendedResponses);
  }
...
/*! \brief   Reads the device name by service ReadDataByIdentifier
   ...
   \param[out]    opu8_NrCode       if != NULL and error response: negative
   response code
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadDeviceName(
    const C_OscProtocolDriverOsyNode &orc_ServerId, QString &orc_DeviceName,
  ...
  if (pc_ExistingProtocol != NULL) {
    ...
    // Only necessary in case of combination of CAN and device configuration
    // initial scan
    C_OscProtocolDriverOsyTpCan c_TpCan;
    C_OscProtocolDriverOsy c_OsyProtocol;

    // No device with this server id with openSYDE protocol exist. We need a
    // temporary protocol.
    s32_Return = this->m_PrepareTemporaryOsyProtocol(orc_ServerId,
                                                     c_OsyProtocol, c_TpCan);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadSerialNumber(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadEcuSerialNumber(
        orc_SerialNumberExt, opu8_NrCode);
  }
...
/*! \brief   Reads the serial number ext by service ReadDataByIdentifier
   ...
   \param[out] opu8_NrCode                         if != NULL and error
   response: negative response code
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadSerialNumberExt(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    C_OscProtocolSerialNumber &orc_SerialNumberExt,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadEcuSerialNumberExt(
        orc_SerialNumberExt, opu8_NrCode);
  }
...
/*! \brief   Set node-id of node specified by serial number
   ...
   \param[out]   opu8_NrCode          if not NULL: code of error response (if
   C_WARN is returned or addressed node has security activated)

   \return
   C_NO_ERR    no problems; one positive response received
   C_RANGE     invalid node ID (bus id or node-ID out of range); "0x7F" is not
   permitted as node ID as it's reserved for broadcasts invalid serial number
   C_WARN      negative response received
   C_COM       could not send requests
   C_CONFIG    no dispatcher installed or broadcast protocol not initialized
   C_TIMEOUT   no response within timeout (was SetNodeIdentifiersForBroadcasts()
   called ?) C_OVERFLOW  multiple responses received
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyCanBroadcastSetNodeIdBySerialNumber(
  ...
  if (this->mpc_CanTransportProtocolBroadcast == NULL) {
    ...
    s32_Return = this->mpc_CanTransportProtocolBroadcast
                     ->BroadcastSetNodeIdBySerialNumber(
                         orc_SerialNumber, orc_NewNodeId, opu8_NrCode);
  }
...
/*! \brief   Set node-id of node specified by serial number extended
   ...
   \param[in]    orc_SerialNumber    serial number of node to address (1 to 29
   bytes allowed)
   \param[in]    ou8_SubNodeId       sub node id of sub node to address (in case
   of a device without sub nodes: 0)
   \param[in]    orc_NewNodeId       node ID to set
   \param[out]   opu8_NrCode         if not NULL: code of error response (if
   C_WARN is returned or addressed node has security activated)
...
int32_t C_OscComDriverFlash::SendOsyCanBroadcastSetNodeIdBySerialNumberExtended(
  ...
  if (this->mpc_CanTransportProtocolBroadcast == NULL) {
    ...
    s32_Return =
        this->mpc_CanTransportProtocolBroadcast
            ->BroadcastSetNodeIdBySerialNumberExtended(
                orc_SerialNumber, ou8_SubNodeId, orc_NewNodeId, opu8_NrCode);
  }
...
/*! \brief   Send broadcast to get device information via ETH-TP
   ...
   \param[out]    orc_ReadDeviceInfoExtendedResults   received extended
   responses

   \return
   C_NO_ERR    no problems; zero or more responses received; data placed in
   c_ReadDeviceInfoResults C_COM       could not send request C_CONFIG    no
   dispatcher installed or ETH broadcast protocol not initialized
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyEthBroadcastGetDeviceInformation(
  ...
  if (this->mpc_IpTransportProtocolBroadcast == NULL) {
    ...
    s32_Return = this->mpc_IpTransportProtocolBroadcast->BroadcastGetDeviceInfo(
        orc_ReadDeviceInfoResults, orc_ReadDeviceInfoExtendedResults);
  }
...
/*! \brief   Send broadcast to set IP address via ETH-TP
   ...
   Only the node with a specified serial number is expected to send a response
   and change its IP address. The function will return as soon as it has
   received one response.

   Incoming UDP responses to other services will be dumped: we are strictly
   handshaking here ...
   ...
   \param[out]   opu8_ErrorResult      if not NULL: code of error response (if
   C_WARN is returned or addressed node has security activated))
   ...
   \return
   C_NO_ERR    no problems; one OK response received; response IP placed in
   orau8_ResponseIp C_WARN      error response C_COM       could not send
   request C_CONFIG    no dispatcher installed or ETH broadcast protocol not
   initialized C_RANGE     serial number is invalid or wrong format of serial
   number is configured C_TIMEOUT   no response within timeout
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyEthBroadcastSetIpAddress(
    const C_OscProtocolSerialNumber &orc_SerialNumber,
  ...
  if (this->mpc_IpTransportProtocolBroadcast == NULL) {
    ...
    s32_Return = this->mpc_IpTransportProtocolBroadcast->BroadcastSetIpAddress(
        orc_SerialNumber, orau8_NewIpAddress, orau8_NetMask,
        orau8_DefaultGateway, orc_NewNodeId, orau8_ResponseIp,
        opu8_ErrorResult);
  }
...
/*! \brief   Send broadcast to set IP address extended via ETH-TP
   ...
   \param[in]    orc_SerialNumber         serial number of server to change IP
   on
   \param[in]    orau8_NewIpAddress       IP address to set
   \param[in]    orau8_NetMask            Net mask to set
   \param[in]    orau8_DefaultGateway     Default gateway to set
   \param[in]    orc_NewNodeId            New bus id and node id for the
   interface
   \param[in]    ou8_SubNodeId            Sub node id of node for identification
   in case of a multi CPU node
   \param[out]   orau8_ResponseIp         IP address the response was received
   from
   \param[out]   opu8_ErrorResult         if not NULL: code of error response
   (if C_WARN is returned or addressed node has security activated))
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyEthBroadcastSetIpAddressExtended(
    const C_OscProtocolSerialNumber &orc_SerialNumber,
    const uint8_t (&orau8_NewIpAddress)[4], const uint8_t (&orau8_NetMask)[4],
    const uint8_t (&orau8_DefaultGateway)[4],
    const C_OscProtocolDriverOsyNode &orc_NewNodeId,
    const uint8_t ou8_SubNodeId, uint8_t (&orau8_ResponseIp)[4],
  ...
  if (this->mpc_IpTransportProtocolBroadcast == NULL) {
    ...
    s32_Return =
        this->mpc_IpTransportProtocolBroadcast->BroadcastSetIpAddressExtended(
            orc_SerialNumber, orau8_NewIpAddress, orau8_NetMask,
            orau8_DefaultGateway, orc_NewNodeId, ou8_SubNodeId,
...
/*! \brief   Send the request programming service to one node
   ...
   not initialized properly. C_NOACT     Could not put request in Tx queue
   C_WARN      Error response received
   C_TIMEOUT   Expected response not received within timeout
   C_COM       communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyRequestProgramming(
    const C_OscProtocolDriverOsyNode &orc_ServerId) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    // If the device is already in flashloader, the preprogramming session is
    // needed. If the device is in the application, this session request will
    // return with an error. This error can be ignored.
    pc_ExistingProtocol->OsyDiagnosticSessionControl(
        C_OscProtocolDriverOsy::hu8_DIAGNOSTIC_SESSION_PREPROGRAMMING, NULL);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Send the ReadActiveDiagnosticSession service to one node and report
   response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadActiveDiagnosticSession(
    const C_OscProtocolDriverOsyNode &orc_ServerId, uint8_t &oru8_SessionId,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadActiveDiagnosticSession(
        oru8_SessionId, opu8_NrCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Send the ReadFlashBlockData service to read information about all
   flash blocks

   Executes the service in a loop starting from 0.
   A "requestOutOfRange" NRC is interpreted as "this is the first block not
   available" and will terminate the loop.

   Technically block 0 (Flashloader) should always be present. As a defensive
   measure we will not fail in this case. Instead we will return an empty list
   of identified blocks. So the application can decide how to deal with this
    situation.
...
int32_t C_OscComDriverFlash::SendOsyReadAllFlashBlockData(
  ...
  if (pc_ExistingProtocol != NULL) {
    for (uint16_t u16_Block = 0U; u16_Block <= 0xFFU; u16_Block++) {
      ...
      s32_Return = pc_ExistingProtocol->OsyReadFlashBlockData(
          static_cast<uint8_t>(u16_Block), c_BlockInfo, &u8_NrCode);
      if (s32_Return == C_NO_ERR) {
        ...
        if ((s32_Return == C_WARN) &&
            (u8_NrCode ==
             C_OscProtocolDriverOsy::hu8_NR_CODE_REQUEST_OUT_OF_RANGE)) {
          // no more blocks
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadInformationFromFlashloader(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadBootSoftwareIdentification(
        orc_Information.au8_FlashloaderSoftwareVersion, opu8_NrCode);
    if (s32_Return == C_NO_ERR) {
      s32_Return = pc_ExistingProtocol->OsyReadApplicationSoftwareFingerprint(
          orc_Information.au8_FlashFingerprintDate,
          orc_Information.au8_FlashFingerprintTime,
          orc_Information.c_FlashFingerprintUserName, opu8_NrCode);
    }

    if (s32_Return == C_NO_ERR) {
      s32_Return = pc_ExistingProtocol->OsyReadHardwareNumber(
          orc_Information.u32_EcuArticleNumber, opu8_NrCode);
    }

    if (s32_Return == C_NO_ERR) {
      s32_Return = pc_ExistingProtocol->OsyReadHardwareVersionNumber(
          orc_Information.c_EcuHardwareVersionNumber, opu8_NrCode);
    }

    if (s32_Return == C_NO_ERR) {
      s32_Return = pc_ExistingProtocol->OsyReadProtocolVersion(
          orc_Information.au8_ProtocolVersion, opu8_NrCode);
    }
    ...
    if (s32_Return == C_NO_ERR) {
      s32_Return = pc_ExistingProtocol->OsyReadFlashCount(
          orc_Information.u32_FlashCount, opu8_NrCode);
    }

    // Get available flashloader features
    if (s32_Return == C_NO_ERR) {
      s32_Return = pc_ExistingProtocol->OsyReadListOfFeatures(
          orc_Information.c_AvailableFeatures, opu8_NrCode);
    }

    if (s32_Return == C_NO_ERR) {
      if (orc_Information.c_AvailableFeatures
              .q_ExtendedSerialNumberModeImplemented == false) {
        s32_Return = pc_ExistingProtocol->OsyReadEcuSerialNumber(
            orc_Information.c_SerialNumber, opu8_NrCode);
      } else {
        s32_Return = pc_ExistingProtocol->OsyReadEcuSerialNumberExt(
            orc_Information.c_SerialNumber, opu8_NrCode);
      }
    ...
    if ((s32_Return == C_NO_ERR) &&
        (orc_Information.c_AvailableFeatures
             .q_MaxNumberOfBlockLengthAvailable == true)) {
      s32_Return = pc_ExistingProtocol->OsyReadMaxNumberOfBlockLength(
          orc_Information.u16_MaxNumberOfBlockLength, opu8_NrCode);
      if (s32_Return != C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyCheckFlashMemoryAvailable(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyCheckFlashMemoryAvailable(
        ou32_StartAddress, ou32_Size, opu8_NrCode);
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyWriteApplicationSoftwareFingerprint(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const uint8_t (&orau8_Date)[3], const uint8_t (&orau8_Time)[3],
    const QString &orc_Username, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);

  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyWriteApplicationSoftwareFingerprint(
        orau8_Date, orau8_Time, orc_Username, opu8_NrCode);
  }
  return s32_Return;
}
...
/*! \brief   Execute RequestDownload service
   ...
   \param[in] ou32_Size              Number of bytes we want to write to the
   start address
   \param[out] oru32_MaxBlockLength  Maximum number of bytes we may send in one
   subsequent "TransferData" service
   \param[out] opu8_NrCode           if != NULL and error response: negative
   response code
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyRequestDownload(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const uint32_t ou32_StartAddress, const uint32_t ou32_Size,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyRequestDownload(
        ou32_StartAddress, ou32_Size, oru32_MaxBlockLength, opu8_NrCode);
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyRequestFileTransfer(
    const C_OscProtocolDriverOsyNode &orc_ServerId, const QString &orc_FilePath,
    const uint32_t ou32_FileSize, uint32_t &oru32_MaxBlockLength,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyRequestFileTransfer(
        orc_FilePath, ou32_FileSize, oru32_MaxBlockLength, opu8_NrCode);
  }
  return s32_Return;
}
...
int32_t C_OscComDriverFlash::SendOsyTransferData(
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyTransferData(ou8_BlockSequenceCounter,
                                                      orc_Data, opu8_NrCode);
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyRequestTransferExitAddressBased(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyRequestTransferExitAddressBased(
        oq_SendSignatureBlockAddress, ou32_SignatureBlockAddress, opu8_NrCode);
  }
  return s32_Return;
}
...
/*! \brief   Execute TransferExitFileBased service
   ...
   \param[out] opu8_NrCode                if != NULL and error response:
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyRequestTransferExitFileBased(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const uint32_t ou32_CrcOverData, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    ...
    s32_Return = pc_ExistingProtocol->OsyRequestTransferExitFileBased(
        au8_Signature, opu8_NrCode);
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyRequestFileBasedTransferExitResult(
    const C_OscProtocolDriverOsyNode &orc_ServerId, QString &orc_Result,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadFileBasedTransferExitResult(
        orc_Result, opu8_NrCode);
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyEcuReset(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const uint8_t ou8_ResetType) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sets the session to preprogramming and the necessary security
   access to level 1

   \param[in]     orc_Protocol      Protocol object for communication
   \param[in]     oq_SessionOnly    true: only set session; don't set security
   access
   ...
   Nodes has no openSYDE protocol C_WARN      Error response received C_TIMEOUT
   Expected response not received within timeout C_CHECKSUM  Security related
   error (something went wrong while handshaking with the server) Detailed error
   codes are logged with opu8_NrCode
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetPreProgrammingMode(
    C_OscProtocolDriverOsy &orc_Protocol, const bool oq_SessionOnly,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = this->m_SetNodeSessionId(
      &orc_Protocol,
      C_OscProtocolDriverOsy::hu8_DIAGNOSTIC_SESSION_PREPROGRAMMING, false,
      opu8_NrCode);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetPreProgrammingMode(
    const C_OscProtocolDriverOsyNode &orc_ServerId, const bool oq_SessionOnly,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return = C_RANGE;
  bool q_Found = false;
  const uint32_t u32_ActiveNodeIndex =
  ...
  if (q_Found == true) {
    ...
    if ((s32_Return == C_NO_ERR) && (oq_SessionOnly == false)) {
      s32_Return =
          this->m_SetNodeSecurityAccess(u32_ActiveNodeIndex, 1, opu8_NrCode);
    }
  } else {
    // Only necessary in case of combination of CAN and device configuration
    // initial scan In case of ETH and device configuration a ETH specific
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      if ((s32_Return == C_NO_ERR) && (oq_SessionOnly == false)) {
        s32_Return =
            this->m_SetNodeSecurityAccess(&c_OsyProtocol, 1, opu8_NrCode);
      }
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sets the session to programming and the necessary security access
   to level 3

   \param[in]     orc_ServerId        Server id for communication
   \param[in]     opu8_SecurityLevel  Optional security level. If not set the
   security level 3 is used
   \param[out]    opu8_NrCode         if != NULL: negative response code
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetProgrammingMode(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const uint8_t *const opu8_SecurityLevel, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  bool q_Found = false;
  const uint32_t u32_ActiveNodeIndex =
      this->m_GetActiveIndex(orc_ServerId, q_Found);
  ...
  if (q_Found == true) {
    s32_Return = this->m_SetNodeSessionId(
        u32_ActiveNodeIndex,
        C_OscProtocolDriverOsy::hu8_DIAGNOSTIC_SESSION_PROGRAMMING, false,
        opu8_NrCode);
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      s32_Return = this->m_SetNodeSecurityAccess(u32_ActiveNodeIndex,
                                                 u8_SecurityLevel, opu8_NrCode);
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetSecurityLevel(
    const C_OscProtocolDriverOsyNode &orc_ServerId, const uint8_t ou8_Level,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  bool q_Found = false;
  const uint32_t u32_ActiveNodeIndex =
  ...
  if (q_Found == true) {
    s32_Return = this->m_SetNodeSecurityAccess(u32_ActiveNodeIndex, ou8_Level,
                                               opu8_NrCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetBitrate(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsySetBitrate(0U, ou8_ChannelIndex,
                                                    ou32_Bitrate, opu8_NrCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::h_SendOsySetIpAddressForChannel(
    C_OscProtocolDriverOsy &orc_Protocol, const uint8_t ou8_ChannelIndex,
  ...
  return orc_Protocol.OsySetIpAddressForChannel(
      1U, ou8_ChannelIndex, orau8_IpAddress, orau8_NetMask,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetIpAddressForChannel(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsySetIpAddressForChannel(
        1U, ou8_ChannelIndex, orau8_IpAddress, orau8_NetMask,
        orau8_DefaultGateway, opu8_NrCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::h_SendOsySetNodeIdForChannel(
    C_OscProtocolDriverOsy &orc_Protocol, const uint8_t ou8_ChannelType,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsySetNodeIdForChannel(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsySetNodeIdForChannel(
        ou8_ChannelType, ou8_ChannelIndex, orc_NewNodeId, opu8_NrCode);
  } else {
    // Only necessary in case of combination of CAN and device configuration
    // initial scan In case of ETH and device configuration a ETH specific
    // version of this function exists
    ...
    if (s32_Return == C_NO_ERR) {
      s32_Return = c_OsyProtocol.OsySetNodeIdForChannel(
          ou8_ChannelType, ou8_ChannelIndex, orc_NewNodeId, opu8_NrCode);
    } else {
      s32_Return = C_COM;
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadListOfFeatures(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadListOfFeatures(orc_ListOfFeatures,
                                                            opu8_NrCode);
  }
...
int32_t C_OscComDriverFlash::SendOsyReadCertificateSerialNumber(
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadCertificateSerialNumber(
        orc_SerialNumber, opu8_NrCode);
  }
...
int32_t C_OscComDriverFlash::SendOsyReadCertificateSerialNumberL7(
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadCertificateSerialNumberL7(
        orc_SerialNumber, opu8_NrCode);
  }
...
/*! \brief   Writes the public security key with its certificate serial number
   ...
   Public key exponent is a byte array with a length of 1 to 4 Bytes containing
   the exponent (high byte first). Serial number is a byte array with maximum
   length of 20 Bytes.
   ...
   C_RD_WR    unexpected content in response (here: wrong data identifier ID)
   C_COM      communication driver reported error
   C_RANGE    parameter orc_SerialNumber, orc_PublicKeyModulus,
   orc_PublicKeyExponent does not match the size expectation
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyWriteSecurityKey(
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyWriteSecurityKey(
        orc_PublicKeyModulus, orc_PublicKeyExponent,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadSecurityActivation(
    const C_OscProtocolDriverOsyNode &orc_ServerId, bool &orq_SecurityOn,
    uint8_t &oru8_SecurityAlgorithm, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);

  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadSecurityActivation(
        orq_SecurityOn, oru8_SecurityAlgorithm, opu8_NrCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyWriteSecurityActivation(
    const C_OscProtocolDriverOsyNode &orc_ServerId, const bool oq_SecurityOn,
    const uint8_t ou8_SecurityAlgorithm, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);

  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyWriteSecurityActivation(
        oq_SecurityOn, ou8_SecurityAlgorithm, opu8_NrCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyReadDebuggerEnabled(
    const C_OscProtocolDriverOsyNode &orc_ServerId, bool &orq_DebuggerEnabled,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyReadDebuggerEnabled(
        orq_DebuggerEnabled, opu8_NrCode);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyWriteDebuggerEnabled(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const bool oq_DebuggerEnabled, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
      this->m_GetOsyProtocol(orc_ServerId);

  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyWriteDebuggerEnabled(
        oq_DebuggerEnabled, opu8_NrCode);
  }
...
/*! \brief  Call factory mode master reset on an openSYDE server
   ...
   \param[in]   orc_ServerId  Server id for communication
   \param[out]  opu8_NrCode   if != NULL and error response: negative response
   code
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendOsyFactoryModeMasterReset(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_RANGE;
  C_OscProtocolDriverOsy *const pc_ExistingProtocol =
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->OsyFactoryMode(
        C_OscProtocolDriverOsy::hu8_OSY_FACTORY_MODE_MASTER_RESET, opu8_NrCode);
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Send reset request for all STW flashloader by specific reset
   message
...
int32_t C_OscComDriverFlash::SendStwRequestNodeReset(void) {
  ...
  if (this->mq_Initialized == true) {
    ...
    for (u32_Counter = 0U; u32_Counter < this->m_GetActiveNodeCount();
         ++u32_Counter) {
      if (this->mc_ActiveNodesIndexes[u32_Counter] <
          this->mpc_SysDef->c_Nodes.size()) {
        T_STWCAN_Msg_TX c_ResetMsg;

        s32_Return = this->m_GetStwResetMessage(
            this->mc_ActiveNodesIndexes[u32_Counter], c_ResetMsg);
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (u32_Counter = 0U; u32_Counter < this->m_GetActiveNodeCount();
        ...
        if (this->mc_StwFlashProtocols[u32_Counter] != NULL) {
          ...
          // Send all reset messages with the same protocol. No dependency to
          // the concrete node id
          for (u32_MsgCounter = 0U; u32_MsgCounter < c_ResetMsgs.size();
               ++u32_MsgCounter) {
            s32_Return =
                this->mc_StwFlashProtocols[u32_Counter]->RequestNodeReset(
                    &c_ResetMsgs[u32_MsgCounter]);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Send reset request for one STW flashloader by specific reset
   message
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwRequestNodeReset(
    const C_OscProtocolDriverOsyNode &orc_ServerId) {
  int32_t s32_Return = C_CONFIG;
  ...
  if (this->mq_Initialized == true) {
    ...
    if (this->GetNodeIndex(orc_ServerId, u32_NodeIndex) == true) {
      T_STWCAN_Msg_TX c_ResetMsg;
      C_OscFlashProtocolStwFlashloader c_StwProtocol(
          pr_XflReportProgress, pv_XflReportProgressInstance);
      C_OscFlashProtocolStwFlashloader *pc_ExistingProtocol =
          this->m_GetStwFlashloaderProtocol(orc_ServerId);

      if (pc_ExistingProtocol == NULL) {
        // No device with this server id with STW protocol exists. We need a
        // temporary protocol.
        this->m_InitFlashProtocolStw(&c_StwProtocol,
                                     orc_ServerId.u8_NodeIdentifier);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwSendFlash(
    const C_OscProtocolDriverOsyNode &orc_ServerId) {
  int32_t s32_Return;
  C_OscFlashProtocolStwFlashloader c_StwProtocol(pr_XflReportProgress,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwSearchId(
    uint8_t (&orau8_LocalIds)[XFL_NUM_DIFFERENT_LOCAL_IDS],
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwWakeupLocalId(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    uint8_t *const opu8_NodesFound) {
  int32_t s32_Return;
  C_OscFlashProtocolStwFlashloader *const pc_ExistingProtocol =
      this->m_GetStwFlashloaderProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    s32_Return = pc_ExistingProtocol->WakeupLocalId(
        this->mc_CompanyId, opu8_NodesFound, &c_ReceivedCompanyId);
  } else {
    C_OscFlashProtocolStwFlashloader c_StwProtocol(
        pr_XflReportProgress, pv_XflReportProgressInstance);

    // No device with this server id with STW protocol exist. We need a
    // temporary protocol.
    this->m_InitFlashProtocolStw(&c_StwProtocol,
                                 orc_ServerId.u8_NodeIdentifier);
    s32_Return = c_StwProtocol.WakeupLocalId(
        this->mc_CompanyId, opu8_NodesFound, &c_ReceivedCompanyId);
  }
  ...
  if (s32_Return == C_WARN) {
    ...
    C_OscFlashProtocolStwFlashloader::CompIDStructToString(c_ReceivedCompanyId,
                                                           c_CompanyId);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwWakeupLocalSerialNumber(
    const C_OscProtocolSerialNumber &orc_SerialNumber, uint8_t &oru8_LocalId) {
  int32_t s32_Return = C_RANGE;
  ...
  if ((orc_SerialNumber.q_IsValid == true) &&
    ...
    s32_Return = c_StwProtocol.WakeupSerialNumber(
        orc_SerialNumber.au8_SerialNumber, this->mc_CompanyId, oru8_LocalId,
        &c_ReceivedCompanyId);
    ...
    if (s32_Return == C_WARN) {
      ...
      C_OscFlashProtocolStwFlashloader::CompIDStructToString(
...
/*! \brief   Read the SNR(s) from the controller(s) specified by LocalId
   ...
   \param[in]     orc_ServerId             Server id for communication
   \param[out]    opu8_SerialNumbers       Returns the received SNR(s), each 6
   bytes the calling function has to provide a buffer large enough Set to NULL
   if only interested in the number of responses.
   \param[in]     ou8_NumMax               Maximum number of answers to record
   to prevent overflow in opu8_SerialNumbers
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwGetSerialNumbers(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  s32_Return = pc_ExistingProtocol->GetSNRExt(opu8_SerialNumbers, ou8_NumMax,
                                              oru8_NumFound);
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwGetDeviceId(
    const C_OscProtocolDriverOsyNode &orc_ServerId, QString &orc_DeviceName) {
  int32_t s32_Return;
  C_OscFlashProtocolStwFlashloader c_StwProtocol(pr_XflReportProgress,
                                                 pv_XflReportProgressInstance);
  ...
  s32_Return = pc_ExistingProtocol->GetImplementationInformationProtocolVersion(
      u16_ProtocolVersion);
  ...
  q_LongId =
      ((s32_Return == C_NO_ERR) &&
       (u16_ProtocolVersion >= mhu16_STW_FLASHLOADER_PROTOCOL_VERSION_3_00));
  orc_DeviceName = "";
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwSetLocalId(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  // We change the server Id and do not want to change the server Id of existing
  // protocols. We need a temporary protocol.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwSetBitrateCan(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  // We support only 16 bit bitrate for STW flashloader -> bitrate resolution
  // kbit/second
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sends net reset request for STW flashloader devices for a specific
   device
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwNetReset(
    const C_OscProtocolDriverOsyNode &orc_ServerId) {
  int32_t s32_Return;
  C_OscFlashProtocolStwFlashloader c_StwProtocol(pr_XflReportProgress,
                                                 pv_XflReportProgressInstance);
...
/*! \brief   Read all kinds of information from STW flashloader
   ...
   \param[out]    orc_ChecksumInformation  information about block or sector
   checksums

   \return
   C_NO_ERR    information read
   C_COM       communication error
   C_CONFIG    node identified by orc_ServerId is unknown or does not speak STW
   Flashloader
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwReadDeviceInformation(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  if (pc_ExistingProtocol != NULL) {
    ...
    if ((orc_Information.c_AvailableFeatures.q_BlockBasedCRCsEEPROM == true) ||
      ...
      s32_Return = pc_ExistingProtocol->ReadServerBlockChecksumInformation(
          orc_ChecksumInformation);
    } else {
      s32_Return = pc_ExistingProtocol->ReadServerSectorChecksumInformation(
          orc_Information.u16_SectorCount, orc_ChecksumInformation);
    }
    if (s32_Return != C_NO_ERR) {
      s32_Return = C_COM;
    }
...
/*! \brief   Write one hex file to an STW Flashloader node
   ...
   * node is already in flashloader (no "FLASH" sequence will be performed)
   ...
   C_CONFIG    node identified by orc_ServerId is unknown or does not speak STW
   Flashloader else        problem during flashing; details can be seen in the
   progress log
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::SendStwDoFlash(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    const QString &orc_HexFilePath) const {
  int32_t s32_Return = C_CONFIG;
  C_OscFlashProtocolStwFlashloader *const pc_ExistingProtocol =
      this->m_GetStwFlashloaderProtocol(orc_ServerId);
  ...
  if (pc_ExistingProtocol != NULL) {
    ...
    s32_Return = C_XFLActions::CompIDStructToString(
        this->mc_CompanyId, c_Params.c_WakeupConfig.c_CompanyID);
    Q_ASSERT(s32_Return == C_NO_ERR); // no plausible reason for this
    c_Params.c_WakeupConfig.q_SendFLASHRequired =
        false; // we are already in flashloader
    c_Params.c_WakeupConfig.q_SendResetRQ =
        false; // we are already in flashloader
    c_Params.c_WakeupConfig.u8_LocalID = orc_ServerId.u8_NodeIdentifier;
    c_Params.e_EraseMode =
        eXFL_ERASE_MODE_AUTOMATIC; // only automatic mode supported
    c_Params.e_FlashFinishedAction =
        eXFL_FLASH_FINISHED_ACTION_NODE_SLEEP; // go back to sleep after
                                               // flashing
    c_Params.u16_InterFrameDelayUs =
        0U; // don't consider ancient hardware and targets
    c_Params.u8_HexRecordLength = 0U; // keep hex record length as it is (unless
                                      // we have the V3.x mechanisms)
    ...
    c_Params.u8_IgnoreInvalidHexfileError =
        0U; // always fail if the hex file is invalid
...
void C_OscComDriverFlash::PrepareForDestructionFlash(void) {
  ...
  for (uint32_t u32_Counter = 0U;
       u32_Counter < this->mc_StwFlashProtocols.size(); ++u32_Counter) {
    C_OscFlashProtocolStwFlashloader *const pc_FlashProtocol =
        this->mc_StwFlashProtocols[u32_Counter];
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComDriverFlash::m_GetRoutingMode(
    C_OscRoutingCalculation::E_Mode &ore_Mode) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComDriverFlash::m_IsRoutingSpecificNecessary(
    const C_OscNode &orc_Node) const {
  bool q_Return = false;
...
/*! \brief   Prepares the routing for STW flashloader protocol
   ...
   \param[in]     orc_LastNodeOfRouting                The last node in the
   routing chain before the final target server
   \param[in]     opc_ProtocolOsyOfLastNodeOfRouting   The protocol of the last
   node
   \param[out]    oppc_RoutingDispatcher               The legacy routing
   dispatcher
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::m_StartRoutingSpecific(
  ...
  if (opc_Node->c_Properties.e_FlashLoader == C_OscNodeProperties::eFL_STW) {
    C_OscFlashProtocolStwFlashloader *const pc_StwFlashloader =
        dynamic_cast<C_OscFlashProtocolStwFlashloader *>(
            this->mc_StwFlashProtocols[ou32_ActiveNode]);

    if ((pc_StwFlashloader != NULL) &&
        (opc_ProtocolOsyOfLastNodeOfRouting != NULL)) {
      (*oppc_RoutingDispatcher) =
          new C_OscCanDispatcherOsyRouter(*opc_ProtocolOsyOfLastNodeOfRouting);

      // Only 0x52 for Rx is relevant
      (*oppc_RoutingDispatcher)
          ->SetFilterParameters(orc_LastNodeOfRouting.u8_OutInterfaceNumber,
                                0x00000052, 0x000007FF);

      this->mc_LegacyRouterDispatchers[ou32_ActiveNode] =
          (*oppc_RoutingDispatcher);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverFlash::m_StopRoutingSpecific(
    const uint32_t ou32_ActiveNode) {
  const C_OscNode *const pc_Node =
      &this->mpc_SysDef->c_Nodes[this->mc_ActiveNodesIndexes[ou32_ActiveNode]];
...
/*! \brief   Checks if the interface has relevant functions activated
   ...
   In this case routing and in case of update specific routing the update
   functionality
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComDriverFlash::m_CheckInterfaceForFunctions(
  ...
  if ((orc_ComItfSettings.GetBusConnected() == true) &&
      ((orc_ComItfSettings.q_IsRoutingEnabled == true) ||
       ((orc_ComItfSettings.q_IsUpdateEnabled == true) ||
        (this->mq_UpdateRoutingMode == false)))) {
    q_Return = true;
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns the pointer to the STW flashloader protocol of specific
   server id
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscFlashProtocolStwFlashloader *
C_OscComDriverFlash::m_GetStwFlashloaderProtocol(
    const C_OscProtocolDriverOsyNode &orc_ServerId) const {
  C_OscFlashProtocolStwFlashloader *pc_Return = NULL;

  for (uint32_t u32_Counter = 0U; u32_Counter < this->mc_ServerIds.size();
       ++u32_Counter) {
...
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverFlash::m_InitFlashProtocolStw(
    C_OscFlashProtocolStwFlashloader *const opc_FlashProtocolStw,
  ...
  opc_FlashProtocolStw->CfgSetFlashId(mhu32_XFL_ID_TX,
                                      mhu32_XFL_ID_RX); // SendId, ReceiveId
  ...
  // The bitrate configuration is not necessary. It will be used for calculation
  // of defined busload which is insignificant
...
/*! \brief   Initialize flash protocols
   ...
   C_OVERFLOW Unknown diagnostic server for at least one node or invalid node
   identifier was set in diagnostic protocol
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::m_InitFlashProtocol(void) {
  ...
  if ((this->mc_TransportProtocols.size() >= u32_ActiveNodeCount) &&
    ...
    for (u32_ActiveNodeCounter = 0U;
         u32_ActiveNodeCounter < this->mc_ActiveNodesIndexes.size();
         ++u32_ActiveNodeCounter) {
      if (this->mc_ActiveNodesIndexes[u32_ActiveNodeCounter] <
          this->mpc_SysDef->c_Nodes.size()) {
        const C_OscNode *const pc_Node =
            &this->mpc_SysDef
                 ->c_Nodes[this->mc_ActiveNodesIndexes[u32_ActiveNodeCounter]];
        ...
        case C_OscNodeProperties::eFL_OPEN_SYDE:
          ...
          pc_ProtocolOsy->InitializeHandleWaitTime(
              &C_OscComDriverFlash::mh_HandleWaitTime, this);
          s32_Return = pc_ProtocolOsy->SetTransportProtocol(
              this->mc_TransportProtocols[u32_ActiveNodeCounter]);
          if (s32_Return == C_NO_ERR) {
            s32_Return = pc_ProtocolOsy->SetNodeIdentifiers(
                this->GetClientId(), this->mc_ServerIds[u32_ActiveNodeCounter]);
            if (s32_Return != C_NO_ERR) {
        ...
        case C_OscNodeProperties::eFL_STW:
          pc_ProtocolStw = new C_OscFlashProtocolStwFlashloader(
              pr_XflReportProgress, pv_XflReportProgressInstance);
          this->m_InitFlashProtocolStw(
              pc_ProtocolStw,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverFlash::m_PrepareTemporaryOsyProtocol(
    const C_OscProtocolDriverOsyNode &orc_ServerId,
    C_OscProtocolDriverOsy &orc_OsyProtocol,
    C_OscProtocolDriverOsyTpCan &orc_CanTransportProtocol) {
  int32_t s32_Return;

  s32_Return = orc_CanTransportProtocol.SetNodeIdentifiers(this->GetClientId(),
                                                           orc_ServerId);
  if (s32_Return == C_NO_ERR) {
    s32_Return =
        orc_CanTransportProtocol.SetDispatcher(this->m_GetCanDispatcher());

    if (s32_Return == C_NO_ERR) {
      s32_Return =
          orc_OsyProtocol.SetTransportProtocol(&orc_CanTransportProtocol);
      if (s32_Return == C_NO_ERR) {
        s32_Return = orc_OsyProtocol.SetNodeIdentifiers(this->GetClientId(),
                                                        orc_ServerId);
      }
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverFlash::m_GetStwResetMessage(const uint32_t ou32_NodeIndex,
                                          T_STWCAN_Msg_TX &orc_Message) const {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  In C_OscProtocolDriverOsy registered function for handling long
   waiting times

   See m_HandleWaitTime for detailed description

   \param[in]     opv_Instance     Pointer to the instance of
   C_OscComDriverFlash
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverFlash::mh_HandleWaitTime(void *const opv_Instance) {
  ...
  C_OscComDriverFlash *const pc_ComDriver =
      reinterpret_cast<C_OscComDriverFlash *>(opv_Instance);
...
/*! \brief  In C_OscProtocolDriverOsy registered function for handling long
   ...
   For example erasing of flash can have a long timeout time. It can be longer
   than five seconds. An active CAN routing would be stopped of the server by
   the session timeout. This function sends the tester present to the last CAN
   router node on the route to keep the route alive
...
void C_OscComDriverFlash::m_HandleWaitTime(void) {
  if (this->mq_Initialized == true) {
    ...
    for (u32_Counter = 0U;
         u32_Counter < this->mc_ActiveNodesLastCanRouters.size();
         ++u32_Counter) {
      const uint32_t u32_ActiveNodeRouter =
          this->mc_ActiveNodesLastCanRouters[u32_Counter];
      ...
      if (u32_ActiveNodeRouter < this->mc_OsyProtocols.size()) {
        C_OscProtocolDriverOsy *const pc_ProtocolOsy =
            this->mc_OsyProtocols[u32_ActiveNodeRouter];
        if (pc_ProtocolOsy != NULL) {
          ...
          if (s32_Return != C_NO_ERR) {
            osc_write_log_error(
                ...
                "Sending Tester Present failed with error code: " +
                    C_OscLoggingHandler::h_StwError(s32_Return));
          } else {
            osc_write_log_info("Sending Tester Present",
                               "Tester Present sent successfully.");
          }
        }
      }
    }
