...
class C_OscComDriverFlash : public C_OscComDriverProtocol {
public:
  // Description of types see
  // C_OscDeviceDefinition::u32_FlashloaderResetWaitTimeXXX
  ...
  int32_t OsySetPollingTimeout(const C_OscProtocolDriverOsyNode &orc_ServerId,
                               const uint32_t ou32_TimeoutMs) const;
  int32_t
  OsyResetPollingTimeout(const C_OscProtocolDriverOsyNode &orc_ServerId) const;

  uint32_t GetMinimumFlashloaderResetWaitTime(
      const E_MinimumFlashloaderResetWaitTimeType oe_Type) const;
  int32_t GetMinimumFlashloaderResetWaitTime(
  ...
  // ETH specific functions for connect when no protocol and/or IP address is
  // initialized
  int32_t EthConnectNode(const C_OscProtocolDriverOsyNode &orc_ServerId,
                         const uint8_t (&orau8_IpAddress)[4],
                         C_OscProtocolDriverOsy &orc_TemporaryProtocol,
                         C_OscProtocolDriverOsyTpIp &orc_TpIp);
  static int32_t
  h_EthDisconnectNode(C_OscProtocolDriverOsy &orc_TemporaryProtocol);
  ...
  int32_t SendOsyCanBroadcastSetNodeIdBySerialNumberExtended(
      const C_OscProtocolSerialNumber &orc_SerialNumber,
  ...
  int32_t SendOsyEthBroadcastSetIpAddressExtended(
      const C_OscProtocolSerialNumber &orc_SerialNumber,
      const uint8_t (&orau8_NewIpAddress)[4], const uint8_t (&orau8_NetMask)[4],
      const uint8_t (&orau8_DefaultGateway)[4],
      const C_OscProtocolDriverOsyNode &orc_NewNodeId,
  ...
  int32_t SendOsyReadDeviceName(const C_OscProtocolDriverOsyNode &orc_ServerId,
                                QString &orc_DeviceName,
                                uint8_t *const opu8_NrCode = NULL);
  int32_t
  SendOsyReadSerialNumber(const C_OscProtocolDriverOsyNode &orc_ServerId,
                          C_OscProtocolSerialNumber &orc_SerialNumberExt,
                          uint8_t *const opu8_NrCode = NULL) const;
  int32_t
  SendOsyReadSerialNumberExt(const C_OscProtocolDriverOsyNode &orc_ServerId,
                             C_OscProtocolSerialNumber &orc_SerialNumberExt,
                             uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyReadActiveDiagnosticSession(
  ...
  int32_t SendOsyReadInformationFromFlashloader(
      ...
      uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyCheckFlashMemoryAvailable(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
      const uint32_t ou32_StartAddress, const uint32_t ou32_Size,
      uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyWriteApplicationSoftwareFingerprint(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
      const uint8_t (&orau8_Date)[3], const uint8_t (&orau8_Time)[3],
      const QString &orc_Username, uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyRequestDownload(const C_OscProtocolDriverOsyNode &orc_ServerId,
                                 const uint32_t ou32_StartAddress,
                                 const uint32_t ou32_Size,
                                 uint32_t &oru32_MaxBlockLength,
                                 uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyRequestFileTransfer(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  int32_t SendOsyRequestTransferExitAddressBased(
      ...
      uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyRequestTransferExitFileBased(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
      const uint32_t ou32_CrcOverData, uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsyRequestFileBasedTransferExitResult(
      const C_OscProtocolDriverOsyNode &orc_ServerId, QString &orc_Result,
      uint8_t *const opu8_NrCode = NULL) const;

  int32_t SendOsyRequestProgramming(
      const C_OscProtocolDriverOsyNode &orc_ServerId) const;
  int32_t SendOsyEcuReset(const C_OscProtocolDriverOsyNode &orc_ServerId,
                          const uint8_t ou8_ResetType) const;
  int32_t SendOsySetPreProgrammingMode(C_OscProtocolDriverOsy &orc_Protocol,
                                       const bool oq_SessionOnly,
                                       uint8_t *const opu8_NrCode = NULL) const;
  int32_t
  SendOsySetPreProgrammingMode(const C_OscProtocolDriverOsyNode &orc_ServerId,
                               const bool oq_SessionOnly,
                               uint8_t *const opu8_NrCode = NULL);
  int32_t
  SendOsySetProgrammingMode(const C_OscProtocolDriverOsyNode &orc_ServerId,
                            const uint8_t *const opu8_SecurityLevel = NULL,
                            uint8_t *const opu8_NrCode = NULL) const;
  int32_t
  SendOsySetSecurityLevel(const C_OscProtocolDriverOsyNode &orc_ServerId,
                          const uint8_t ou8_Level,
                          uint8_t *const opu8_NrCode = NULL) const;
  int32_t SendOsySetBitrate(const C_OscProtocolDriverOsyNode &orc_ServerId,
                            const uint8_t ou8_ChannelIndex,
                            const uint32_t ou32_Bitrate,
                            uint8_t *const opu8_NrCode = NULL) const;
  static int32_t h_SendOsySetIpAddressForChannel(
      ...
      uint8_t *const opu8_NrCode = NULL);
  int32_t
  SendOsySetIpAddressForChannel(const C_OscProtocolDriverOsyNode &orc_ServerId,
                                const uint8_t ou8_ChannelIndex,
                                const uint8_t (&orau8_IpAddress)[4],
                                ...
                                uint8_t *const opu8_NrCode = NULL) const;
  static int32_t
  h_SendOsySetNodeIdForChannel(C_OscProtocolDriverOsy &orc_Protocol,
                               const uint8_t ou8_ChannelType,
                               const uint8_t ou8_ChannelIndex,
                               const C_OscProtocolDriverOsyNode &orc_NewNodeId,
                               uint8_t *const opu8_NrCode = NULL);
  int32_t
  SendOsySetNodeIdForChannel(const C_OscProtocolDriverOsyNode &orc_ServerId,
                             const uint8_t ou8_ChannelType,
                             const uint8_t ou8_ChannelIndex,
                             const C_OscProtocolDriverOsyNode &orc_NewNodeId,
                             uint8_t *const opu8_NrCode = NULL);

  int32_t SendOsyReadListOfFeatures(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  int32_t
  SendOsyWriteSecurityActivation(const C_OscProtocolDriverOsyNode &orc_ServerId,
                                 const bool oq_SecurityOn,
                                 const uint8_t ou8_SecurityAlgorithm,
                                 uint8_t *const opu8_NrCode = NULL) const;
  int32_t
  SendOsyReadDebuggerEnabled(const C_OscProtocolDriverOsyNode &orc_ServerId,
                             bool &orq_DebuggerEnabled,
                             uint8_t *const opu8_NrCode = NULL) const;
  int32_t
  SendOsyWriteDebuggerEnabled(const C_OscProtocolDriverOsyNode &orc_ServerId,
                              const bool oq_DebuggerEnabled,
                              uint8_t *const opu8_NrCode = NULL) const;
  int32_t
  SendOsyFactoryModeMasterReset(const C_OscProtocolDriverOsyNode &orc_ServerId,
                                uint8_t *const opu8_NrCode = NULL) const;

  // STW Flashloader services
  int32_t SendStwRequestNodeReset(void);
  int32_t
  SendStwRequestNodeReset(const C_OscProtocolDriverOsyNode &orc_ServerId);
  int32_t SendStwSendFlash(const C_OscProtocolDriverOsyNode &orc_ServerId);
  int32_t SendStwSearchId(
      uint8_t (&orau8_LocalIds)[stw::diag_lib::XFL_NUM_DIFFERENT_LOCAL_IDS],
      uint8_t &oru8_NodeFounds);
  int32_t SendStwWakeupLocalId(const C_OscProtocolDriverOsyNode &orc_ServerId,
                               uint8_t *const opu8_NodesFound);
  int32_t SendStwWakeupLocalSerialNumber(
      const C_OscProtocolSerialNumber &orc_SerialNumber, uint8_t &oru8_LocalId);

  int32_t
  SendStwGetSerialNumbers(const C_OscProtocolDriverOsyNode &orc_ServerId,
                          uint8_t *const opu8_SerialNumbers,
                          const uint8_t ou8_NumMax, uint8_t &oru8_NumFound);
  int32_t SendStwGetDeviceId(const C_OscProtocolDriverOsyNode &orc_ServerId,
                             QString &orc_DeviceName);

  int32_t SendStwSetLocalId(const C_OscProtocolDriverOsyNode &orc_ServerId,
                            const uint8_t ou8_NewLocalId);
  int32_t SendStwSetBitrateCan(const C_OscProtocolDriverOsyNode &orc_ServerId,
                               const uint32_t ou32_Bitrate);
  int32_t SendStwNetReset(const C_OscProtocolDriverOsyNode &orc_ServerId);
  int32_t SendStwNetReset(void);
  int32_t SendStwReadDeviceInformation(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
...
private:
  ...
  C_OscComDriverFlash &
  operator=(const C_OscComDriverFlash
                &); // lint !e1511 //we want to hide the base function

  void m_InitFlashProtocolStw(
      C_OscFlashProtocolStwFlashloader *const opc_FlashProtocolStw,
      const uint8_t ou8_LocalId);
  int32_t m_InitFlashProtocol(void);

  int32_t m_PrepareTemporaryOsyProtocol(
      const C_OscProtocolDriverOsyNode &orc_ServerId,
      C_OscProtocolDriverOsy &orc_OsyProtocol,
      C_OscProtocolDriverOsyTpCan &orc_CanTransportProtocol);
  int32_t m_GetStwResetMessage(const uint32_t ou32_NodeIndex,
                               stw::can::T_STWCAN_Msg_TX &orc_Message) const;
  int32_t m_GetMinimumFlashloaderResetWaitTime(
      const E_MinimumFlashloaderResetWaitTimeType oe_Type,
