...
public:
  ...
  uint32_t u32_StartId;  ///< First CAN ID which is relevant for the filter
                         ///< configuration
  uint32_t
      u32_EndId; ///< Last CAN ID which is relevant for the filter configuration
  bool q_PassFilter; ///< true -> Filter for passing; false -> Filter for
                     ///< blocking configured CAN IDs
};
...
public:
  ...
  virtual void
  SetProtocol(const stw::cmon_protocol::e_CanMonL7Protocols oe_Protocol);
  ...
  virtual int32_t SetOsySysDefBus(const QString &orc_PathSystemDefinition,
                                  const uint32_t ou32_BusIndex);
  virtual int32_t
  GetOsySysDef(const QString &orc_PathSystemDefinition,
  ...
  virtual int32_t ActivateDatabase(const QString &orc_Path,
                                   const bool oq_Active);

  // Logging handling
  virtual int32_t AddLogFileAsc(const QString &orc_FilePath,
  ...
  // CAN bus handling
  virtual int32_t HandleCanMessage(const stw::can::T_STWCAN_Msg_RX &orc_Msg,
                                   const bool oq_IsTx);
...
protected:
  ...
  QString
  m_GetProtocolStringHex(const stw::can::T_STWCAN_Msg_RX &orc_Msg) const;
  QString
  m_GetProtocolStringDec(const stw::can::T_STWCAN_Msg_RX &orc_Msg) const;
  ...
  virtual bool m_InterpretSysDef(
  ...
  virtual bool
  m_CheckInterpretation(C_OscComMessageLoggerData &orc_MessageData);

  static void mh_InterpretCanSignalValue(
      stw::opensyde_core::C_OscComMessageLoggerDataSignal &orc_Signal,
  ...
  // Generic database (for example openSYDE system definitions) activation flag
  // register
  ...
  // Logging
  QMap<QString, C_OscComMessageLoggerFileBase *> mc_LoggingFiles;
...
private:
  ...
  void m_ConvertCanMessage(const stw::can::T_STWCAN_Msg_RX &orc_Msg,
                           const bool oq_IsTx);
  ...
  static void mh_PostProcessSysDef(
      stw::opensyde_core::C_OscSystemDefinition &orc_SystemDefinition);
  static void
  mh_AddSpecialEcesSignals(C_OscNode &orc_Node,
  ...
  QMap<QString, C_OscComMessageLoggerOsySysDefConfig> mc_OsySysDefs;

  // Filtering
  QList<C_OscComMessageLoggerFilter> mc_CanFilterConfig;
  uint32_t
      mu32_FilteredMessages; // number of messages that did not pass the filter
  ...
  public:
    ...
    C_EcosMessage() : u32_CanId(0), c_MessageName("") {}
  };
