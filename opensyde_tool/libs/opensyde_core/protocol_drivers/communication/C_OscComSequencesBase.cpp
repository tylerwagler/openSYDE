...
/*!
   \file
   \brief       Basic sequences to communicate with flashloader protocols.
   (implementation)
...
/*! \brief   Default constructor
   ...
   \param[in]  oq_UpdateRoutingMode Flag for update specific routing or generic
   routing
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscComSequencesBase::C_OscComSequencesBase(const bool oq_RoutingActive,
                                             const bool oq_UpdateRoutingMode)
    : mpc_ComDriver(new C_OscComDriverFlash(oq_RoutingActive,
                                            oq_UpdateRoutingMode,
                                            &mh_MyXflReportProgress, this)),
      mpc_SystemDefinition(NULL), mu32_ActiveBusIndex(0U),
      mq_OpenSydeDevicesActive(false), mq_StwFlashloaderDevicesActive(false),
...
/*! \brief   Initialize class parameters to use for communication
   ...
   * flags: which of the nodes are active; this determines which nodes the
   client expected to be present
   * index of bus the client is connected to

   The caller is responsible to keep the referenced system definition stable
   while functions in this class are executed (this class does not create a
   copy)

   \param[in]  orc_SystemDefinition     Entire system definition
   \param[in]  ou32_ActiveBusIndex      Index of bus within system definition
   that the client tool is connected to (0 = first bus)
   \param[in]  orc_ActiveNodes          Vector of flags for each node in the
   system definition
                                           - set the entry to true if the device
   is expected to be present
                                           - set the entry to false if the
   device is not expected to be present
   ...
   \param[in]  opc_SecurityPemDb        Pointer to PEM database (optional)
                                        Needed if nodes with enabled security
   are used in the system

   \return
   C_NO_ERR      Configuration set
   C_CONFIG      Invalid system definition for parameters
                 Active bus index refers to a bus that is not part of the system
   definition Length of active nodes vector is not identical to number of nodes
   in system definition No STW flashloader devices and no openSYDE devices are
   active No STW flashloader devices on active bus and no openSYDE devices are
   active, but STW flashloader on other buses are active C_OVERFLOW    Unknown
   transport protocol or unknown diagnostic server for at least one node C_NOACT
   No active nodes C_COM         CAN initialization failed or no route found for
   ...
   failed (can all nodes marked as active be reached from the defined bus ?)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComSequencesBase::Init(C_OscSystemDefinition &orc_SystemDefinition,
  ...
  if ((orc_SystemDefinition.c_Nodes.size() == orc_ActiveNodes.size()) &&
    ...
    s32_Return = this->mpc_ComDriver->Init(
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      this->mq_StwFlashloaderDevicesActive =
          this->m_IsAtLeastOneStwFlashloaderNodeActive();
      this->mq_StwFlashloaderDevicesActiveOnLocalBus =
          this->m_IsAtLeastOneStwFlashloaderNodeOnLocalBusActive(
              this->mc_StwFlashloaderDeviceOnLocalBus);

      // Either no devices are active or only STW flashloader devices which need
      // routing to get reached are active
      if (((this->mq_OpenSydeDevicesActive == false) &&
           (this->mq_StwFlashloaderDevicesActive == false)) ||
          ((this->mq_OpenSydeDevicesActive == false) &&
           (this->mq_StwFlashloaderDevicesActiveOnLocalBus == false) &&
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComSequencesBase::GetNodeIndex(
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Gets the information if at least one STW flashloader device is
   active
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Gets the information if at least one STW flashloader device is on
   the local bus active
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComSequencesBase::IsAtLeastOneStwFlashloaderNodeActiveOnLocalBus(
    void) const {
...
/*! \brief  Check if a specific node must be capable of Ethernet to Ethernet
   ...
   node
   \retval   false    Ethernet to Ethernet Routing must not be supported by
   router node
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComSequencesBase::IsEthToEthRoutingNecessary(
...
/*! \brief  Returns the minimum Flashloader reset wait time in ms
   ...
   \return
   Time in ms all nodes needs at least to get from application to the
   Flashloader
*/
//----------------------------------------------------------------------------------------------------------------------
...
int32_t C_OscComSequencesBase::GetMinimumFlashloaderResetWaitTime(
  ...
  return this->mpc_ComDriver->GetMinimumFlashloaderResetWaitTime(
      oe_Type, orc_ServerId, oru32_TimeValue);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComSequencesBase::m_IsNodeReachable(
  ...
  if (this->mc_TimeoutNodes[ou32_NodeIndex] == 1U) {
    ...
    for (u32_PointCounter = 0U;
         u32_PointCounter < c_Route.c_VecRoutePoints.size();
         ++u32_PointCounter) {
      if (this->mc_TimeoutNodes[c_Route.c_VecRoutePoints[u32_PointCounter]
                                    .u32_NodeIndex] == 1U) {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComSequencesBase::mh_MyXflReportProgress(void *const opv_Instance,
  ...
  return reinterpret_cast<C_OscComSequencesBase *>(opv_Instance)
      ->m_MyXflReportProgress(ou8_Progress, orc_Text);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComSequencesBase::m_MyXflReportProgress(const uint8_t ou8_Progress,
                                                     const QString &orc_Text) {
...
bool C_OscComSequencesBase::m_IsAtLeastOneOpenSydeNodeActive(void) const {
  ...
  if (this->mpc_SystemDefinition != NULL) {
    for (uint32_t u32_Counter = 0U; u32_Counter < this->mc_ActiveNodes.size();
         ++u32_Counter) {
      if ((u32_Counter < this->mpc_SystemDefinition->c_Nodes.size()) &&
          (this->mc_ActiveNodes[u32_Counter] == 1U)) {
        const C_OscNode &rc_Node =
            this->mpc_SystemDefinition->c_Nodes[u32_Counter];

        if (rc_Node.c_Properties.e_FlashLoader ==
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Gets the information if at least one STW flashloader device is
   active on the local bus

   \param[out]    orc_StwFlashloaderDeviceOnLocalBus     The first found STW
   flashloader node on the local bus
    ...
    for (uint32_t u32_Counter = 0U; u32_Counter < this->mc_ActiveNodes.size();
      ...
      if ((u32_Counter < this->mpc_SystemDefinition->c_Nodes.size()) &&
        ...
        if (rc_Node.c_Properties.e_FlashLoader ==
          ...
          // Check all interfaces of node if connected to the bus which is
          ...
          for (u32_IntfCounter = 0U;
            ...
            if ((rc_ComIntf.u32_BusIndex == this->mu32_ActiveBusIndex) &&
              ...
              // Save the node identifier for a local STW flashloader device to
              // get easy access to the protocol for the local bus and not
              // server specific services
              orc_StwFlashloaderDeviceOnLocalBus.u8_NodeIdentifier =
                  rc_ComIntf.u8_NodeId;
              orc_StwFlashloaderDeviceOnLocalBus.u8_BusIdentifier =
                  this->mpc_SystemDefinition->c_Buses[this->mu32_ActiveBusIndex]
                      .u8_BusId;
