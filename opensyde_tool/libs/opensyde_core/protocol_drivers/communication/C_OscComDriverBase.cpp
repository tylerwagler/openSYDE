...
/*!
   ...
   Offers a interface to deliver CAN messages to all registered
   C_OscMessageLogger instances. Distributes all received messages to the
   registered C_OscMessageLogger instances.
...
C_OscComDriverBaseCanMessage::C_OscComDriverBaseCanMessage(void)
    : u32_TimeToSend(0U), u32_Interval(0U) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComDriverBaseCanMessage::operator==(
    const C_OscComDriverBaseCanMessage &orc_Cmp) const {
  bool q_Return = false;
  ...
  if ((this->c_Msg.u32_ID == orc_Cmp.c_Msg.u32_ID) &&
    ...
    for (u8_Counter = 0U; u8_Counter < this->c_Msg.u8_DLC; ++u8_Counter) {
      // Check the data
      if (this->c_Msg.au8_Data[u8_Counter] !=
          orc_Cmp.c_Msg.au8_Data[u8_Counter]) {
        q_Return = false;
        break;
      }
    }
...
C_OscComDriverBase::C_OscComDriverBase(void)
    : mpc_CanDispatcher(NULL), mu16_DispatcherClientHandle(0U),
      mq_Started(false), mq_Paused(false), mu32_CanMessageBits(0U),
      ms32_CanBitrate(1000U), mu32_CanTxCounter(0U), mu32_CanTxErrors(0U) {
...
int32_t C_OscComDriverBase::InitBase(C_CanDispatcher *const opc_CanDispatcher) {
  ...
  if (this->mpc_CanDispatcher != NULL) {
    s32_Return = this->mpc_CanDispatcher->RegisterClient(
        this->mu16_DispatcherClientHandle);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::RegisterLogger(
    C_OscComMessageLogger *const opc_Logger) {
...
/*! \brief   Starts the logging
   ...
   \param[in]  os32_Bitrate          CAN bitrate in kbit/s. Is used for the bus
   load calculation not the initialization
...
int32_t C_OscComDriverBase::StartLogging(const int32_t os32_Bitrate) {
  ...
  if (this->mpc_CanDispatcher != NULL) {
    ...
    for (u32_Counter = 0U; u32_Counter < this->mc_Logger.size();
         ++u32_Counter) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Distributes the CAN message to all registered C_OscMessageLogger
   instances.

   Gets all received CAN messages and informs all logger about the received
   messages. This function must be called cyclic to have a continuous logging.

   Deriving classes can override m_HandleCanMessage to handle the CAN messages
   too.

   When making DistributeMessages and SendCanMessageQueued thread safe by
   overwriting to use sending CAN messages by separated threads.
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::DistributeMessages(void) {
  if ((this->mpc_CanDispatcher != NULL) && (this->mq_Started == true)) {
    int32_t s32_Return;
    ...
    do {
      // Get the messages even if paused to clean the queue. The messages in the
      // pause phase are not relevant
      s32_Return = this->mpc_CanDispatcher->ReadFromQueue(
          this->mu16_DispatcherClientHandle, c_Msg);
      ...
      if (s32_Return == C_NO_ERR) {
        if (c_Msg.u8_DLC <= 8) {
          ...
          osc_write_log_error(
              "Reading CAN message",
              "Ignored CAN message (ID: " + QString::number(c_Msg.u32_ID) +
                  ") due to invalid DLC (" + QString::number(c_Msg.u8_DLC));
        }
      }
    } while (s32_Return == C_NO_ERR);

    // Check and update bus load
    const uint32_t u32_Now = static_cast<uint32_t>(
        std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now().time_since_epoch())
            .count());
    u32_BusLoadTimeDiff = u32_Now - hu32_BusLoadTimeRefresh;
    if (u32_BusLoadTimeDiff >= 1000U) {
      const uint32_t u32_MaxBitsSec =
          static_cast<uint32_t>(this->ms32_CanBitrate) * 1024U;
      // Scale to the real elapsed time
      const uint32_t u32_MaxBits =
          static_cast<uint32_t>((static_cast<uint64_t>(u32_MaxBitsSec) *
                                 static_cast<uint64_t>(u32_BusLoadTimeDiff)) /
                                1000U);
      if (u32_MaxBits > 0U) {
        ...
        for (u32_LoggerCounter = 0U; u32_LoggerCounter < this->mc_Logger.size();
             ++u32_LoggerCounter) {
          this->mc_Logger[u32_LoggerCounter]->UpdateBusLoad(
              static_cast<uint8_t>(u32_Load));
        }
    ...
    for (u32_LoggerCounter = 0U; u32_LoggerCounter < this->mc_Logger.size();
         ++u32_LoggerCounter) {
      this->mc_Logger[u32_LoggerCounter]->UpdateTxErrors(
          this->mu32_CanTxErrors);
      this->mc_Logger[u32_LoggerCounter]->UpdateTxCounter(
...
int32_t C_OscComDriverBase::SendCanMessageDirect(T_STWCAN_Msg_TX &orc_Msg) {
  orc_Msg.au8_Data[6] = mpc_AutoSupportProtocol->MessageCounter(
      orc_Msg.u32_ID, orc_Msg.au8_Data[6]);
  orc_Msg.au8_Data[7] =
      mpc_AutoSupportProtocol->SetCyclicRedundancyCheckCalculation(
          orc_Msg.u32_ID, orc_Msg.u8_DLC - 2, &orc_Msg.au8_Data[0]);
  ...
  if (this->mpc_CanDispatcher != NULL) {
    ...
    if (mpc_AutoSupportProtocol->SupportInvertedCanMessage(orc_Msg.u32_ID)) {
      ...
      C_OscComAutoSupport::h_InvertCanMessage(&orc_Msg.au8_Data[0],
                                              &au8_InvertedData[0]);
      T_STWCAN_Msg_TX orc_TransmitInvertedMsg;
      (void)std::memcpy(&orc_TransmitInvertedMsg.au8_Data[0],
                        &au8_InvertedData[0], 8U);
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      c_Msg.u64_TimeStamp = static_cast<uint64_t>(
      ...
      if (mpc_AutoSupportProtocol->SupportInvertedCanMessage(orc_Msg.u32_ID)) {
        ...
        (void)std::memcpy(&c_RecieveInvertedMsg.au8_Data[0],
                          &au8_InvertedData[0], 8U);
        ...
        c_RecieveInvertedMsg.u64_TimeStamp = static_cast<uint64_t>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                now.time_since_epoch())
                .count());
  ...
  if (s32_Return != C_NO_ERR) {
    osc_write_log_error("Sending CAN message",
                        "Could not send CAN message. Error code: " +
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::SendCanMessage(
    C_OscComDriverBaseCanMessage &orc_MsgCfg, const bool oq_SetAutoSupportMode,
    const C_OscCanProtocol::E_Type oe_ProtocolType) {
  mpc_AutoSupportProtocol->AutoSupportModeInfo(
      orc_MsgCfg.c_Msg.u32_ID, oq_SetAutoSupportMode, oe_ProtocolType);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::AddCyclicCanMessage(
    const C_OscComDriverBaseCanMessage &orc_MsgCfg,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::RemoveCyclicCanMessage(
    const C_OscComDriverBaseCanMessage &orc_MsgCfg,
    const bool oq_SetAutoSupportMode,
    const C_OscCanProtocol::E_Type oe_ProtocolType) {
  mpc_AutoSupportProtocol->AutoSupportModeInfo(orc_MsgCfg.c_Msg.u32_ID, false,
                                               oe_ProtocolType);
  C_OscComDriverBaseCanMessage c_MsgCfg = orc_MsgCfg;

  QList<C_OscComDriverBaseCanMessage>::iterator c_ItConfig;

  if (c_MsgCfg.u32_Interval == 0U) {
    c_MsgCfg.u32_Interval = 1U;
  }

  for (c_ItConfig = this->mc_CanMessageConfigs.begin();
       c_ItConfig != this->mc_CanMessageConfigs.end(); ++c_ItConfig) {
    if (((false == oq_SetAutoSupportMode) && (*c_ItConfig == c_MsgCfg)) ||
        ((true == oq_SetAutoSupportMode) &&
         (oe_ProtocolType == C_OscCanProtocol::eCAN_OPEN_SAFETY) &&
         (*c_ItConfig == c_MsgCfg)) ||
        ((true == oq_SetAutoSupportMode) &&
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::UpdateAutoSupportProtocol(
    const int32_t os32_CanId, const bool oq_SetAutoSupportMode,
    const C_OscCanProtocol::E_Type oe_ProtocolType) {
  mpc_AutoSupportProtocol->AutoSupportModeInfo(
      os32_CanId, oq_SetAutoSupportMode, oe_ProtocolType);
  mpc_AutoSupportProtocol->ResetMessageCounters(
      os32_CanId, oq_SetAutoSupportMode, oe_ProtocolType);
  QList<C_OscComDriverBaseCanMessage>::iterator c_ItConfig;
  for (c_ItConfig = this->mc_CanMessageConfigs.begin();
       c_ItConfig != this->mc_CanMessageConfigs.end(); ++c_ItConfig) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::m_HandleCanMessage(const T_STWCAN_Msg_RX &orc_Msg,
                                            const bool oq_IsTx) {
  if (this->mq_Paused == false) {
    ...
    this->mu32_CanMessageBits +=
        C_OscComDriverBase::mh_GetCanMessageSizeInBits(orc_Msg);
  }
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sending all CAN messages which are queued or registered for cyclic
 * transmission
 */
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverBase::m_HandleCanMessagesForSending(void) {
  QList<stw::can::T_STWCAN_Msg_TX>::iterator c_ItCanMessage;
  QList<C_OscComDriverBaseCanMessage>::iterator c_ItCanMessageConfig =
      this->mc_CanMessageConfigs.begin();

  // Send all queued CAN messages
  for (c_ItCanMessage = this->mc_CanMessages.begin();
       c_ItCanMessage != this->mc_CanMessages.end(); ++c_ItCanMessage) {
  ...
  while (c_ItCanMessageConfig != this->mc_CanMessageConfigs.end()) {
    const uint32_t u32_CurTimeStamp = static_cast<uint32_t>(
        std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now().time_since_epoch())
            .count());
    ...
    if ((*c_ItCanMessageConfig).u32_TimeToSend <= u32_CurTimeStamp) {
      ...
      if ((*c_ItCanMessageConfig).u32_Interval != 0) {
        ...
        (*c_ItCanMessageConfig).u32_TimeToSend =
            u32_CurTimeStamp + (*c_ItCanMessageConfig).u32_Interval;
        ++c_ItCanMessageConfig;
      } else {
        // No interval configured. Sending this CAN message only once and erase
        // the configuration
        c_ItCanMessageConfig =
            this->mc_CanMessageConfigs.erase(c_ItCanMessageConfig);
      }
    } else {
...
/*! \brief   Calculates the CAN message size
   ...
   The following table shows the maximum determined message lengths in bits with
   stuff bits Standard id: DLC | number of bits | number of stuff bits 0   | 53
   | 6 1   | 59             | 4 2   | 69             | 6 3   | 79             |
   8 4   | 89             | 10 5   | 98             | 11 6   | 107            |
   12 7   | 117            | 14 8   | 127            | 16 Extended id: 0   | 74
   | 7 1   | 83             | 8 2   | 92             | 9 3   | 101            |
   10 4   | 112            | 13 5   | 122            | 15 6   | 131            |
   16 7   | 140            | 17 8   | 150            | 19

   CAN messages with extended id can have up to 4 stuff bits more than standard
   id with same DLC. In average we calculate with 3 more stuff bits for extended
   id.

   Additional to that we approach to the worst case of stuff bits by using the
   calculation DLC * 10 bits.
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscComDriverBase::mh_GetCanMessageSizeInBits(const T_STWCAN_Msg_RX &orc_Msg) {
  // Stuff bits dependent of DLC +
  // minimum size of CAN message with standard identifier +
  // Optional extended id with 18 bits for the extended id itself, SRR,
  // additional reserved bit and the 3 stuff bits
  const uint8_t u8_ExtendedBits = ((orc_Msg.u8_XTD == 1U) ? 23U : 0U);

  return ((static_cast<uint32_t>(orc_Msg.u8_DLC) * 10U) + 47U +
          u8_ExtendedBits);
}
