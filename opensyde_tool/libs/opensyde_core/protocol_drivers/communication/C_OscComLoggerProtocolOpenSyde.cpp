...
/*!
   \file
   \brief       Translate L2 CAN message to openSYDE interpretation
   (implementation)
...
#define OSY_CLIENT                                                             \
  (0x7EU) ///< Standard client ID (can vary, it is the default ID of the
          ///< openSYDE gui)
...
/*! \brief   Adds an openSYDE system definition configuration for analyzing
   ...
   \param[in]     opc_SysDefConfig               Pointer to openSYDE system
   definition configuration
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComLoggerProtocolOpenSyde::AddOsySysDef(
    const C_OscComMessageLoggerOsySysDefConfig *const opc_SysDefConfig) {
  QList<const C_OscComMessageLoggerOsySysDefConfig *>::const_iterator
      c_ItConfig;
  bool q_AddConfig = true;

  // Add only if not already added
  for (c_ItConfig = this->mc_SysDefConfigs.begin();
       c_ItConfig != this->mc_SysDefConfigs.end(); ++c_ItConfig) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComLoggerProtocolOpenSyde::RemoveOsySysDef(
    const C_OscComMessageLoggerOsySysDefConfig *const opc_SysDefConfig) {
  this->mc_SysDefConfigs.removeAll(opc_SysDefConfig);
}
...
QString C_OscComLoggerProtocolOpenSyde::m_AddressInformationToText(
  ...
  for (c_ItConfig = this->mc_SysDefConfigs.begin();
    ...
    if (orc_CanAddressInformation.u8_RoutingMode == OSY_CTP_ROUTING_INACTIVE) {
      ...
      if (pc_Config->u32_BusIndex < pc_Config->c_OsySysDef.c_Buses.size()) {
        ...
        c_SourceBusName =
            pc_Config->c_OsySysDef.c_Buses[u32_SourceBusIndex].c_Name;
      ...
      for (u32_BusCounter = 0U;
        ...
        if ((q_SourceBusFound == true) && (q_TargetBusFound == true)) {
          // Both buses found
          break;
        }
      }
    ...
    if ((q_SourceBusFound == true) && (q_TargetBusFound == true)) {
      ...
      } else if (orc_CanAddressInformation.t_NodeIdTarget.u8_NodeId ==
                 OSY_CLIENT) {
      ...
      for (u32_NodeCounter = 0U;
           u32_NodeCounter < pc_Config->c_OsySysDef.c_Nodes.size();
           ++u32_NodeCounter) {
        const C_OscNode &rc_Node =
            pc_Config->c_OsySysDef.c_Nodes[u32_NodeCounter];
        uint32_t u32_InterfaceCounter;

        for (u32_InterfaceCounter = 0U;
             u32_InterfaceCounter < rc_Node.c_Properties.c_ComInterfaces.size();
             ++u32_InterfaceCounter) {
          if ((q_SourceNodeFound == false) &&
              (rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .GetBusConnected() == true) &&
              (rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .u8_NodeId ==
               orc_CanAddressInformation.t_NodeIdSource.u8_NodeId) &&
              (rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .u32_BusIndex == u32_SourceBusIndex)) {
            // Source Node
          ...
          if ((q_TargetNodeFound == false) &&
              ...
              (rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .u32_BusIndex == u32_TargetBusIndex)) {
            // Target Node
        ...
        if ((q_SourceNodeFound == true) && (q_TargetNodeFound == true)) {
          // Both nodes found
          break;
        }
      }
      ...
      if (q_TargetNodeFound == false) {
        c_TargetNodeName = m_GetByteAsStringFormat(
            orc_CanAddressInformation.t_NodeIdTarget.u8_NodeId);
      }
      if (q_SourceNodeFound == false) {
        c_SourceNodeName = m_GetByteAsStringFormat(
            orc_CanAddressInformation.t_NodeIdSource.u8_NodeId);
      }
  ...
  if (q_ValidInterpretationFound == false) {
    // Fallback if no matching information found in at least one system
    // definition
    c_Text = C_CanMonProtocolOpenSyde::m_AddressInformationToText(
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscComLoggerProtocolOpenSyde::m_DataPoolIdentifierToText(
    ...
    if (q_BusFound == true) {
      ...
      for (u32_NodeCounter = 0U;
        ...
        for (u32_InterfaceCounter = 0U;
             u32_InterfaceCounter < rc_Node.c_Properties.c_ComInterfaces.size();
             ++u32_InterfaceCounter) {
          if ((rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .GetBusConnected() == true) &&
              (rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .u8_NodeId == u8_NodeId) &&
              (rc_Node.c_Properties.c_ComInterfaces[u32_InterfaceCounter]
                   .u32_BusIndex == u32_BusIndexWithNode)) {
            // This is the node
            q_NodeFound = true;
            break;
          }
        ...
        if (q_NodeFound == true) {
          const uint8_t u8_DataPoolIndex =
              static_cast<uint8_t>(ou32_DataPoolIdentifier >> 18U) & 0x1FU;
          const uint8_t u8_ListIndex =
              static_cast<uint8_t>(ou32_DataPoolIdentifier >> 11U) & 0x7FU;
          ...
          if ((u8_DataPoolIndex < rc_Node.c_DataPools.size()) &&
              (u8_ListIndex <
               rc_Node.c_DataPools[u8_DataPoolIndex].c_Lists.size()) &&
              (u16_ElementIndex < rc_Node.c_DataPools[u8_DataPoolIndex]
                                      .c_Lists[u8_ListIndex]
                                      .c_Elements.size())) {
            ...
            c_Text +=
                ("[" + rc_Node.c_DataPools[u8_DataPoolIndex].c_Name + "." +
                 rc_Node.c_DataPools[u8_DataPoolIndex]
                     .c_Lists[u8_ListIndex]
                     .c_Name +
                 "." +
                 rc_Node.c_DataPools[u8_DataPoolIndex]
                     .c_Lists[u8_ListIndex]
                     .c_Elements[u16_ElementIndex]
                     .c_Name +
                 "]");
  ...
  if (q_ValidElementFound == false) {
    // No element found in at least one openSYDE system definition found, use
    // the standard interpretation
    c_Text = C_CanMonProtocolOpenSyde::m_DataPoolIdentifierToText(
        ou32_DataPoolIdentifier, oq_IsResponse, orc_CanAddressInformation);
  }
