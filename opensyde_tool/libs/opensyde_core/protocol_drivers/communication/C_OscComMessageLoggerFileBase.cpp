...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscComMessageLoggerFileBase::C_OscComMessageLoggerFileBase(
    const QString &orc_FilePath, const QString &orc_ProtocolName)
    : mc_FilePath(orc_FilePath), mc_ProtocolName(orc_ProtocolName) {}
...
/*! \brief   Creates, if necessary, and opens file and adds the default header
   ...
   Base implementation only creates the folders and deletes an already existing
   file
...
int32_t C_OscComMessageLoggerFileBase::OpenFile(void) {
  ...
  const QString c_FolderPath =
      QFileInfo(this->mc_FilePath).absolutePath() + "/";
  ...
  } else if (QFileInfo(this->mc_FilePath).exists() &&
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLoggerFileBase::SetProtocolName(
