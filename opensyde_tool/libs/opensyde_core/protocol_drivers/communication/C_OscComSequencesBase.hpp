...
protected:
  ...
  virtual int32_t m_XflReportProgress(const uint8_t ou8_Progress,
                                      const QString &orc_Text) = 0;

private:
  static int32_t mh_MyXflReportProgress(void *const opv_Instance,
                                        const uint8_t ou8_Progress,
                                        const QString &orc_Text);
  int32_t m_MyXflReportProgress(const uint8_t ou8_Progress,
                                const QString &orc_Text);

  C_OscComSequencesBase(const C_OscComSequencesBase
                            &orc_Source); // not implemented -> prevent copying
  C_OscComSequencesBase &operator=(
      const C_OscComSequencesBase &orc_Source); // not implemented -> prevent
                                                //  assignment
