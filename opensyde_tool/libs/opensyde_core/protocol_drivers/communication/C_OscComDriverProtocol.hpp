...
class C_OscComDriverProtocol : public C_OscComDriverBase {
public:
  ...
  int32_t
  SendTesterPresent(const QSet<uint32_t> *const opc_SkipNodes = NULL);
  int32_t SendTesterPresent(const QList<uint32_t> &orc_ActiveNodes) const;
  int32_t
  SendTesterPresent(const C_OscProtocolDriverOsyNode &orc_ServerId) const;
  int32_t StartRouting(const uint32_t ou32_NodeIndex,
                       uint32_t *const opu32_ErrorNodeIndex = NULL);
  ...
  int32_t GetBusIndexOfRoutingNode(const uint32_t ou32_NodeIndex,
                                   uint32_t &oru32_BusIndex);
  uint32_t GetRoutingPointMaximum(void) const;
  uint32_t GetRoutingPointCount(const uint32_t ou32_NodeIndex,
                                bool &orq_Active) const;
  void GetRouteOfNode(const uint32_t ou32_NodeIndex,
                      C_OscRoutingRoute &orc_Route) const;
  int32_t GetRoutingTargetInterfaceType(
      const uint32_t ou32_NodeIndex,
      C_OscSystemBus::E_Type &ore_InterfaceType) const;
  int32_t
  GetServerIdOfLastRouter(const uint32_t ou32_NodeIndex,
                          C_OscProtocolDriverOsyNode &orc_RouterServerId) const;

  const C_OscProtocolDriverOsyNode &GetClientId(void) const;
  bool GetNodeIndex(const C_OscProtocolDriverOsyNode &orc_ServerId,
  ...
  GetOsyTransportProtocol(const uint32_t ou32_NodeIndex);

protected:
  /// Init state
  bool mq_Initialized;
  /// Holds all indices of all relevant nodes for the current use case. This
  /// active nodes represent the initialized
  ///  active node (for example mc_OsyProtocols) indexes which are part of
  ///  mc_ActiveNodesSystem.
  QList<uint32_t> mc_ActiveNodesIndexes;
  /// Has pointer to created instances of generic openSYDE protocol if
  /// available. Lifetime is handled by child classes. Length matches number of
  /// active nodes.
  ...
  /// Holds created instances of either CAN or IP TP. Length matches number of
  /// active nodes.
  QList<C_OscProtocolDriverOsyTpBase *> mc_TransportProtocols;
  /// Holds created instances of routing dispatcher (one for each routed KEFEX
  /// diagnostic server). Length matches number of active nodes.
  QList<C_OscCanDispatcherOsyRouter *> mc_LegacyRouterDispatchers;

  /// Active nodes which are the last CAN node on a route before the concrete
  /// target
  ...
  uint32_t m_GetActiveIndex(const uint32_t ou32_NodeIndex,
                            bool *const opq_Found = NULL) const;
  uint32_t m_GetActiveIndex(const C_OscProtocolDriverOsyNode &orc_ServerId,
                            bool &orq_Found) const;
  stw::can::C_CanDispatcher *m_GetCanDispatcher(void);
  C_OscIpDispatcher *m_GetIpDispatcher(void);
  virtual bool
  m_GetRoutingMode(C_OscRoutingCalculation::E_Mode &ore_Mode) const = 0;
  virtual uint8_t m_GetRoutingSessionId(void) const = 0;
  C_OscProtocolDriverOsy *
  m_GetOsyProtocol(const C_OscProtocolDriverOsyNode &orc_ServerId) const;
  QString m_GetActiveNodeName(const uint32_t ou32_ActiveNodeIndex) const;

  int32_t m_SetNodeSessionId(const uint32_t ou32_ActiveNode,
                             const uint8_t ou8_SessionId,
                             const bool oq_CheckForSession,
                             uint8_t *const opu8_NrCode) const;
  int32_t m_SetNodeSessionId(C_OscProtocolDriverOsy *const opc_ExistingProtocol,
                             const uint8_t ou8_SessionId,
                             const bool oq_CheckForSession,
                             uint8_t *const opu8_NrCode) const;
  int32_t m_SetNodesSessionId(const uint8_t ou8_SessionId,
                              const bool oq_CheckForSession,
                              QSet<uint32_t> &orc_DefectNodeIndices) const;
  int32_t m_SetNodesSessionId(const QList<uint32_t> &orc_ActiveNodes,
                              const uint8_t ou8_SessionId,
                              const bool oq_CheckForSession,
                              QSet<uint32_t> &orc_DefectNodeIndices) const;
  int32_t m_SetNodeSessionIdWithExpectation(
      const uint32_t ou32_ActiveNode,
      const uint8_t ou8_ExpectedNeededSession) const;
  int32_t m_SetNodeSecurityAccess(const uint32_t ou32_ActiveNode,
                                  const uint8_t ou8_SecurityLevel,
                                  uint8_t *const opu8_NrCode) const;
  int32_t
  m_SetNodeSecurityAccess(C_OscProtocolDriverOsy *const opc_ExistingProtocol,
                          const uint8_t ou8_SecurityLevel,
                          uint8_t *const opu8_NrCode) const;
  int32_t
  m_SetNodesSecurityAccess(const uint8_t ou8_SecurityLevel,
                           QSet<uint32_t> &orc_ErrorActiveNodes) const;
  int32_t
  m_SetNodesSecurityAccess(const QList<uint32_t> &orc_ActiveNodes,
                           const uint8_t ou8_SecurityLevel,
                           QSet<uint32_t> &orc_ErrorActiveNodes) const;

  int32_t m_StartRoutingIp2Ip(const uint32_t ou32_ActiveNode,
                              uint32_t *const opu32_ErrorActiveNodeIndex);
  int32_t m_StartRouting(const uint32_t ou32_ActiveNode,
                         uint32_t *const opu32_ErrorActiveNodeIndex);
  void m_StopRouting(const uint32_t ou32_ActiveNode);
  void m_StopRoutingOfActiveNodes(void);
  int32_t m_StopRoutingOfRoutingPoint(const uint32_t ou32_ActiveNode,
  ...
  */
  //-----------------------------------------------------------------------------
  virtual bool
  m_IsRoutingSpecificNecessary(const C_OscNode &orc_Node) const = 0;

  //-----------------------------------------------------------------------------
  /*!
     \brief   Prepares the routing for a not openSYDE server

     The legacy routing dispatcher must be created and assigned to
     opc_ProtocolOsyOfLastNodeOfRouting if necessary.

     \param[in]     ou32_ActiveNode                      Active node index of
     vector mc_ActiveNodes \param[in]     opc_Node Pointer to current node
     \param[in]     orc_LastNodeOfRouting                The last node in the
     routing chain before the final target server \param[in]
     opc_ProtocolOsyOfLastNodeOfRouting   The protocol of the last node
  ...
  */
  ...
  virtual bool m_CheckInterfaceForFunctions(
      const C_OscNodeComInterfaceSettings &orc_ComItfSettings) const = 0;
...
private:
  ...
  uint32_t
      mu32_ActiveNodeCount; ///< Active node count in mc_ActiveNodesIndexes, not
                            ///< in mc_ActiveNodesSystem
  C_OscIpDispatcher
      *mpc_IpDispatcher; ///< Holds created instance of IP dispatcher

  QMap<uint32_t, uint32_t>
      mc_ActiveNodeIp2IpDispatcher; ///< Mapping from active node index to
  // node
  // TCP dispatcher handle for IP to IP
  // routing
  QList<uint32_t> mc_ActiveNodeIp2IpDispatcherNodeCount; ///< Count of nodes
                                                         ///< which use the TCP
  // dispatcher handle for IP to IP
  // routing

  QMap<uint32_t, uint32_t>
      mc_ActiveNodeIp2CanDispatcher; ///< Mapping from active node index to
  // node
  // TCP dispatcher handle for IP to CAN
  // routing
  QList<uint32_t> mc_ActiveNodeIp2CanDispatcherNodeCount; ///< Count of nodes
                                                          ///< which use the TCP
  // dispatcher handle for IP to CAN
  // routing

  QByteArray mc_ActiveNodesSystem; ///< List of flags for all nodes which are
  ...
  int32_t
  m_GetActiveIndexOfIp2IpRouter(const uint32_t ou32_ActiveIndexTarget,
                                uint32_t &oru32_ActiveIndexRouterClient,
                                uint32_t &oru32_ActiveIndexRouterTarget);
  int32_t m_GetActiveIndexOfIp2CanRouter(const uint32_t ou32_ActiveIndexTarget,
  ...
  operator=(const C_OscComDriverProtocol
