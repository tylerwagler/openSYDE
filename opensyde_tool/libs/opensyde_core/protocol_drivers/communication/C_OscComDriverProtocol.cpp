...
/*!
   ...
   * manages instantiation and cleanup of all required subclasses that
     use used for all kinds of communication purposes (i.e. flashloader and
   diagnostics)
...
C_OscComDriverProtocol::C_OscComDriverProtocol(void)
    : C_OscComDriverBase(), mq_Initialized(false),
      mpc_CanTransportProtocolBroadcast(NULL),
      mpc_IpTransportProtocolBroadcast(NULL), mpc_SysDef(NULL),
      mu32_ActiveNodeCount(0), mpc_IpDispatcher(NULL), mu32_ActiveBusIndex(0U),
      mpc_SecurityPemDb(NULL) {
  // Check if client and server use same float standard, see #84517 for more
  // details
...
C_OscComDriverProtocol::~C_OscComDriverProtocol(void) {
  ...
  for (u32_Counter = 0; u32_Counter < this->mc_TransportProtocols.size();
       ++u32_Counter) {
    delete (this->mc_TransportProtocols[u32_Counter]);
    this->mc_TransportProtocols[u32_Counter] = NULL;
  }

  for (u32_Counter = 0; u32_Counter < this->mc_LegacyRouterDispatchers.size();
       ++u32_Counter) {
...
/*! \brief   Initialize all members
   ...
   \param[in]  ou32_ActiveBusIndex     Bus index of bus in system definition
   where we are connected to \param[in]  orc_ActiveNodes         Flags for all
   available nodes in the system \param[in]  opc_CanDispatcher       Pointer to
   concrete CAN dispatcher \param[in]  opc_IpDispatcher        Pointer to
   concrete IP dispatcher \param[in]  opc_SecurityPemDb       Pointer to PEM
   database (optional) Needed if nodes with enabled security are used in the
   system
   ...
   C_OVERFLOW    Unknown transport protocol or unknown diagnostic server for at
   least one node C_COM         CAN initialization failed or no route found for
   at least one node C_CHECKSUM    Internal buffer overflow detected C_DEFAULT
   Parameter ou32_ActiveBusIndex invalid C_RANGE       Routing configuration
   failed
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::Init(
      ...
      if (this->mu32_ActiveNodeCount > 0) {
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          this->mc_ClientId.u8_BusIdentifier =
              orc_SystemDefinition.c_Buses[this->mu32_ActiveBusIndex].u8_BusId;
          // Detect communication type
          switch (
              orc_SystemDefinition.c_Buses[this->mu32_ActiveBusIndex].e_Type) {
        ...
        osc_write_log_info("COMM driver initialization",
                           "Information: no nodes active (or not configured "
                           "for update or diagnostics)");
        s32_Retval = C_NOACT;
      }
    } else {
      // Routing error
      osc_write_log_error("COMM driver initialization",
                          "Failed: routing initialization error");
      s32_Retval = C_RANGE;
    }
  }
...
/*! \brief   Sends the tester present message to all active nodes
   ...
   \param[in]     opc_SkipNodes     optional pointer to a container with nodes
   for not sending the tester present

   \return
   C_NO_ERR    All nodes set to session successfully
   C_CONFIG    Init function was not called or not successful or protocol was
   not initialized properly. C_COM       Error of service
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::SendTesterPresent(
    const QSet<uint32_t> *const opc_SkipNodes) {
  int32_t s32_Return = C_CONFIG;

  if (this->mq_Initialized == true) {
    uint32_t u32_Counter;

    for (u32_Counter = 0U; u32_Counter < this->mc_OsyProtocols.size();
         ++u32_Counter) {
      // Check the optional set for skipping nodes
      if ((opc_SkipNodes == NULL) ||
          (!opc_SkipNodes->contains(u32_Counter))) {
        C_OscProtocolDriverOsy *const pc_ProtocolOsy =
            this->mc_OsyProtocols[u32_Counter];
        if (pc_ProtocolOsy != NULL) {
          ...
          if (s32_Return != C_NO_ERR) {
            // No response expected. All errors caused by client. We can break
            // here.
            s32_Return = C_COM;
            break;
          }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::SendTesterPresent(
    const QList<uint32_t> &orc_ActiveNodes) const {
  int32_t s32_Return = C_CONFIG;

  if (this->mq_Initialized == true) {
    uint32_t u32_Counter;

    for (u32_Counter = 0U; u32_Counter < orc_ActiveNodes.size();
         ++u32_Counter) {
      const uint32_t u32_ActiveNode = orc_ActiveNodes[u32_Counter];
      if (u32_ActiveNode < this->mc_OsyProtocols.size()) {
        C_OscProtocolDriverOsy *const pc_ProtocolOsy =
            this->mc_OsyProtocols[u32_ActiveNode];
        if (pc_ProtocolOsy != NULL) {
          // Send tester present message without expecting a response
          s32_Return = pc_ProtocolOsy->OsyTesterPresent(1U);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Initialize the necessary routing configuration to start the routing
   ...
   - legacy routing for a non openSYDE after openSYDE routing to an openSYDE
   server

   \param[in]   ou32_NodeIndex         node index to read from
   \param[out]  opu32_ErrorNodeIndex   optional pointer for node index which
   caused the error on starting routing if an error occurred is set when return
   value is not C_NO_ERR, C_RANGE and C_CONFIG
   ...
   C_CHECKSUM Security related error (something went wrong while handshaking
   with the server)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverProtocol::StartRouting(const uint32_t ou32_NodeIndex,
  ...
  if (u32_ActiveIndex >= this->mc_ActiveNodesIndexes.size()) {
    ...
    s32_Return =
        this->m_StartRoutingIp2Ip(u32_ActiveIndex, opu32_ErrorNodeIndex);

    if (s32_Return == C_NO_ERR) {
      s32_Return = this->m_StartRouting(u32_ActiveIndex, opu32_ErrorNodeIndex);
    }

    if ((opu32_ErrorNodeIndex != NULL) && (s32_Return != C_NO_ERR)) {
      ...
      if (*opu32_ErrorNodeIndex < this->mc_ActiveNodesIndexes.size()) {
        *opu32_ErrorNodeIndex =
            this->mc_ActiveNodesIndexes[*opu32_ErrorNodeIndex];
      }
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverProtocol::IsRoutingNecessary(const uint32_t ou32_NodeIndex) {
  int32_t s32_Return;
  bool q_Found;
  const uint32_t u32_ActiveIndex =
      this->m_GetActiveIndex(ou32_NodeIndex, &q_Found);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check if a specific node must be capable of Ethernet to Ethernet
   routing

   \param[in]       ou32_RouterNodeIndex     Node to check if node must be
   capable of Ethernet to Ethernet routing for at least one route

   \retval   true     Ethernet to Ethernet Routing must be supported by router
   node \retval   false    Ethernet to Ethernet Routing must not be supported by
   router node
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComDriverProtocol::IsEthToEthRoutingNecessary(
    const uint32_t ou32_RouterNodeIndex) const {
  bool q_Return = false;
  uint32_t u32_RouteCounter;

  // Check all routes
  for (u32_RouteCounter = 0U; u32_RouteCounter < this->mc_Routes.size();
       ++u32_RouteCounter) {
    ...
    for (u32_RoutePointCounter = 0U;
         u32_RoutePointCounter < rc_Route.c_VecRoutePoints.size();
         ++u32_RoutePointCounter) {
      const C_OscRoutingRoutePoint &rc_Point =
          rc_Route.c_VecRoutePoints[u32_RoutePointCounter];
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Checks the need of routing for a specific node and returns the
   connected bus to the target by bus index
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComDriverProtocol::GetBusIndexOfRoutingNode(const uint32_t ou32_NodeIndex,
  ...
  if (q_Found == true) {
    if (u32_ActiveIndex >= this->mc_Routes.size()) {
      ...
      oru32_BusIndex =
          this->mc_Routes[u32_ActiveIndex]
              .c_VecRoutePoints
                  [this->mc_Routes[u32_ActiveIndex].c_VecRoutePoints.size() - 1]
              .u32_OutBusIndex;
    } else {
      s32_Return = C_NOACT;
    }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns the maximum count of route points which are necessary to
   reach a node
...
uint32_t C_OscComDriverProtocol::GetRoutingPointMaximum(void) const {
  ...
  for (u32_Counter = 0U; u32_Counter < this->mc_Routes.size(); ++u32_Counter) {
    if (this->mc_Routes[u32_Counter].c_VecRoutePoints.size() > u32_Maximum) {
      u32_Maximum = static_cast<uint32_t>(
          this->mc_Routes[u32_Counter].c_VecRoutePoints.size());
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Returns the count of route points which are necessary to reach the
   node
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscComDriverProtocol::GetRoutingPointCount(const uint32_t ou32_NodeIndex,
                                             bool &orq_Active) const {
  uint32_t u32_Count = 0U;
  const uint32_t u32_ActiveIndex =
      this->m_GetActiveIndex(ou32_NodeIndex, &orq_Active);

  if (orq_Active == true) {
    if (u32_ActiveIndex < this->mc_Routes.size()) {
      u32_Count = static_cast<uint32_t>(
          this->mc_Routes[u32_ActiveIndex].c_VecRoutePoints.size());
    } else {
      orq_Active = false;
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverProtocol::GetRouteOfNode(
    const uint32_t ou32_NodeIndex, C_OscRoutingRoute &orc_Route) const {
  bool q_Active;
  const uint32_t u32_ActiveIndex =
      this->m_GetActiveIndex(ou32_NodeIndex, &q_Active);
...
/*! \brief   Returns the used interface type of the node on its route
   ...
   \param[out]  ore_InterfaceType      Used interface type of target node on its
   route
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::GetRoutingTargetInterfaceType(
  ...
  if (u32_ActiveIndex >= this->mc_ActiveNodesIndexes.size()) {
    ...
    if (rc_ActRoute.c_VecRoutePoints.size() > 0) {
      // Check the last routing point. The out interface type is connected to
      // the bus which is connected to the target
      ore_InterfaceType =
          rc_ActRoute.c_VecRoutePoints[rc_ActRoute.c_VecRoutePoints.size() - 1]
              .e_OutInterfaceType;
...
/*! \brief   Returns the last route point on route before target
   ...
   \param[out]  orc_RouterServerId     Router server Id of last router on route
   to target ou32_NodeIndex
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::GetServerIdOfLastRouter(
    const uint32_t ou32_NodeIndex,
  ...
  if (u32_ActiveIndex >= this->mc_ActiveNodesIndexes.size()) {
    ...
    if (rc_ActRoute.c_VecRoutePoints.size() > 0) {
      ...
      orc_RouterServerId =
          this->mc_ServerIds[this->m_GetActiveIndex(u32_RouterNodeIndex)];
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscProtocolDriverOsyNode &
C_OscComDriverProtocol::GetClientId(void) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComDriverProtocol::GetNodeIndex(
  ...
  for (u32_Counter = 0U; u32_Counter < this->mc_ServerIds.size();
       ++u32_Counter) {
...
void C_OscComDriverProtocol::ClearDispatcherQueue(void) {
  if (this->mq_Initialized == true) {
    ...
    for (u32_Counter = 0U; u32_Counter < this->mc_TransportProtocols.size();
         ++u32_Counter) {
      C_OscProtocolDriverOsyTpCan *const pc_CanTp =
          dynamic_cast<C_OscProtocolDriverOsyTpCan *>(
              this->mc_TransportProtocols[u32_Counter]);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::ReConnectNode(
    const stw::opensyde_core::C_OscProtocolDriverOsyNode &orc_ServerId) const {
  int32_t s32_Return = C_RANGE;
  bool q_Found;
  const uint32_t u32_ActiveNodeIndex =
      this->m_GetActiveIndex(orc_ServerId, q_Found);

  if (q_Found == true) {
    C_OscProtocolDriverOsy *const pc_ProtocolOsy =
        this->mc_OsyProtocols[u32_ActiveNodeIndex];
    if (pc_ProtocolOsy != NULL) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::DisconnectNode(
...
void C_OscComDriverProtocol::PrepareForDestruction(void) {
  ...
  // ...
  for (uint32_t u32_ItTp = 0; u32_ItTp < this->mc_TransportProtocols.size();
       ++u32_ItTp) {
    C_OscProtocolDriverOsyTpCan *const pc_Tp =
        dynamic_cast<C_OscProtocolDriverOsyTpCan *>(
            this->mc_TransportProtocols[u32_ItTp]);
    // do we have a CAN TP ?
    if (pc_Tp != NULL) {
      pc_Tp->SetDispatcher(NULL); // we are about to destroy the dispatcher;
                                  // make sure TP disconnects from it
    } else {
      C_OscProtocolDriverOsyTpIp *const pc_TpIp =
          dynamic_cast<C_OscProtocolDriverOsyTpIp *>(
              this->mc_TransportProtocols[u32_ItTp]);
      if (pc_TpIp != NULL) {
...
C_OscComDriverProtocol::GetOsyTransportProtocol(const uint32_t ou32_NodeIndex) {
  C_OscProtocolDriverOsyTpBase *pc_Tp = NULL;

  for (uint16_t u16_Index = 0U; u16_Index < mc_TransportProtocols.size();
       u16_Index++) {
...
/*! \brief   Get index of node in list of active nodes
   ...
   If no active node is found that matches the passed absolute index the
   function will fail with an assertion.

   \param[in]     ou32_NodeIndex    absolute index of node within system
   description \param[out]    opq_Found         Optional flag if server id was
   found

   \return   index of node within list of active nodes
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscComDriverProtocol::m_GetActiveIndex(const uint32_t ou32_NodeIndex,
                                                  bool *const opq_Found) const {
  uint32_t u32_Index;
  bool q_Found = false;

  for (u32_Index = 0U; u32_Index < this->mc_ActiveNodesIndexes.size();
       ++u32_Index) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscComDriverProtocol::m_GetActiveIndex(
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscProtocolDriverOsy *C_OscComDriverProtocol::m_GetOsyProtocol(
    const C_OscProtocolDriverOsyNode &orc_ServerId) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscComDriverProtocol::m_GetActiveNodeName(
    const uint32_t ou32_ActiveNodeIndex) const {
  QString c_Retval = "Unknown";

  if (ou32_ActiveNodeIndex < this->mc_ActiveNodesIndexes.size()) {
    const uint32_t u32_NodeIndex =
        this->mc_ActiveNodesIndexes[ou32_ActiveNodeIndex];
...
/*! \brief   Sets a node into a session
   ...
   \param[in]     ou32_ActiveNode       active node index of vector
   mc_ActiveNodes \param[in]     ou8_SessionId         session ID to switch to
   \param[in]     oq_CheckForSession    checks the current session id on the
   server. only if it is different, the new session id will be set \param[out]
   opu8_NrCode           if != NULL: negative response code

   \return
   C_NO_ERR    All nodes set to session successfully
   C_CONFIG    Init function was not called or not successful or protocol was
   not initialized properly. C_NOACT     Nodes has no openSYDE protocol C_COM
   Communication problem C_WARN      Error response received C_TIMEOUT Expected
   response not received within timeout
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodeSessionId(
    const uint32_t ou32_ActiveNode, const uint8_t ou8_SessionId,
  ...
  if (ou32_ActiveNode < this->mc_OsyProtocols.size()) {
    ...
    s32_Return = this->m_SetNodeSessionId(pc_ProtocolOsy, ou8_SessionId,
                                          oq_CheckForSession, opu8_NrCode);
  } else {
    s32_Return = C_CONFIG;
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodeSessionId(
    C_OscProtocolDriverOsy *const opc_ExistingProtocol,
  ...
  if (this->mq_Initialized == true) {
    if (opc_ExistingProtocol != NULL) {
      ...
      if (oq_CheckForSession == true) {
        ...
        s32_Return = opc_ExistingProtocol->OsyReadActiveDiagnosticSession(
            u8_CurrentSession, opu8_NrCode);
      ...
      if (s32_Return == C_NO_ERR) {
        if (q_SetNewSession == true) {
          // Set the session
          s32_Return = opc_ExistingProtocol->OsyDiagnosticSessionControl(
              ou8_SessionId, opu8_NrCode);

          if ((s32_Return != C_NO_ERR) && (s32_Return != C_TIMEOUT) &&
              (s32_Return != C_WARN)) {
            // Do not change the C_TIMEOUT error
            s32_Return = C_COM;
          }
...
/*! \brief   Sets a node into a session
   ...
   \param[in]        ou8_SessionId         session ID to switch to
   \param[in]        oq_CheckForSession    checks the current session id on the
   server. only if it is different, the new session id will be set
   \param[in,out]    orc_DefectNodeIndices List of active node indices which
   encountered an error

   Nodes with previous errors registered in orc_DefectNodeIndices will be
   skipped

   \return
   C_NO_ERR    All nodes set to session successfully
   C_CONFIG    Init function was not called or not successful or protocol was
   not initialized properly. C_COM       Error of service C_TIMEOUT   Expected
   response not received within timeout or at least one node was registered in
   orc_DefectNodeIndices
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodesSessionId(
    const uint8_t ou8_SessionId, const bool oq_CheckForSession,
    QSet<uint32_t> &orc_DefectNodeIndices) const {
  ...
  return this->m_SetNodesSessionId(c_AllActiveNodes, ou8_SessionId,
                                   oq_CheckForSession, orc_DefectNodeIndices);
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Sets a node into a session

   \param[in]        orc_ActiveNodes       All indexes of active nodes to set
   the session (indexes of mc_ActiveNodesIndexes) \param[in] ou8_SessionId
   session ID to switch to \param[in]        oq_CheckForSession    checks the
   current session id on the server. only if it is different, the new session id
   will be set \param[in,out]    orc_DefectNodeIndices List of active node
   indices which encountered an error
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodesSessionId(
    const QList<uint32_t> &orc_ActiveNodes, const uint8_t ou8_SessionId,
  ...
  if (this->mq_Initialized == true) {
    ...
    for (u32_Counter = 0U; u32_Counter < orc_ActiveNodes.size();
      ...
      // Further communication is only necessary if the node was ok in the first
      // place
      if (!orc_DefectNodeIndices.contains(u32_ActiveNode)) {
        const int32_t s32_Return = this->m_SetNodeSessionId(
            u32_ActiveNode, ou8_SessionId, oq_CheckForSession, NULL);
        ...
        // It was a previous error. Only in case of a previous timeout it would
        // be continued to here
...
/*! \brief   Sets a node into an expected session
   ...
   If ou8_ExpectedNeededSession session can not be set on the server (error
   response), the other session will be set.

   \param[in]     ou32_ActiveNode             Active node which session shall be
   changed \param[in]     ou8_ExpectedNeededSession   Expected and needed
   session. Two sessions possible:
                                              -
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodeSessionIdWithExpectation(
    const uint32_t ou32_ActiveNode,
    const uint8_t ou8_ExpectedNeededSession) const {
  // We need a new session
  int32_t s32_Return = this->m_SetNodeSessionId(
      ou32_ActiveNode, ou8_ExpectedNeededSession, true, NULL);
  ...
  if (s32_Return == C_WARN) {
    ...
    // server or in the flashloader as expected. It returns an error that it
    // does not support the session. Try the other possibility.
    s32_Return = this->m_SetNodeSessionId(
        ou32_ActiveNode, u8_NewExpectedNeededSession, false, NULL);
  }
...
/*! \brief   Sets a node into a security level
   ...
   \param[in]     ou32_ActiveNode       active node index of vector
   mc_ActiveNodes \param[in]     ou8_SecurityLevel     level of requested
   security \param[out]    opu8_NrCode           if != NULL: negative response
   code

   \return
   C_NO_ERR    All nodes set to session successfully
   C_CONFIG    Init function was not called or not successful or protocol was
   not initialized properly or PEM database was needed but not set. C_NOACT
   Nodes has no openSYDE protocol C_COM       Communication problem C_WARN Error
   response C_TIMEOUT   Expected response not received within timeout C_CHECKSUM
   Security related error (something went wrong while handshaking with the
   server) Detailed error codes are logged with opu8_NrCode
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodeSecurityAccess(
    const uint32_t ou32_ActiveNode, const uint8_t ou8_SecurityLevel,
    uint8_t *const opu8_NrCode) const {
  C_OscProtocolDriverOsy *const pc_ProtocolOsy =
      this->mc_OsyProtocols[ou32_ActiveNode];
  const int32_t s32_Return = this->m_SetNodeSecurityAccess(
      pc_ProtocolOsy, ou8_SecurityLevel, opu8_NrCode);
...
/*! \brief   Sets a node into a security level
   ...
   If the first request fails with the error
   hu8_NR_CODE_REQUIRED_TIME_DELAY_NOT_EXPIRED, the node is not ready for the
   security access request. The reason is to avoid brute force attacks. Waiting
   for one second before try again in this case.
...
int32_t C_OscComDriverProtocol::m_SetNodeSecurityAccess(
    C_OscProtocolDriverOsy *const opc_ExistingProtocol,
    const uint8_t ou8_SecurityLevel, uint8_t *const opu8_NrCode) const {
  int32_t s32_Return = C_CONFIG;
  ...
  if (this->mq_Initialized == true) {
    if (opc_ExistingProtocol != NULL) {
      ...
      s32_Return = opc_ExistingProtocol->OsySecurityAccessRequestSeed(
          ou8_SecurityLevel, q_SecureMode, u64_Seed, ou8_SecurityAlgorithm,
          &u8_NrErrorCode);
      ...
      if ((s32_Return == C_WARN) &&
          (u8_NrErrorCode == C_OscProtocolDriverOsy::
                                 hu8_NR_CODE_REQUIRED_TIME_DELAY_NOT_EXPIRED)) {
        // Special case: The server must wait for a second to allow a request
        // after a reset. In some scenarios the normal waiting times after
        // resets are not enough (this times are not calculated for this
        // security scenario), so wait in this case for another second and try
        // again
        osc_write_log_warning(
            "Security Access",
            "The node was not ready for the Security Access request. Trying"
        ...
        if (q_SecureMode == false) {
          if (ou8_SecurityLevel == 7U) {
            ...
            osc_write_log_error("Security Access",
                                "Unexpected secure mode: disabled. For level 7 "
                                "secure mode should always be enabled. (No "
                                "need to use level 7 without secure mode)");
          } else {
            ...
            if (u64_Seed != 42U) {
              const QString c_Tmp =
                  "Received seed in non secure mode does not match the "
                  "expected value, expected: 42, got " +
                  QString::number(u64_Seed);
              osc_write_log_warning("Security Access",
                                    c_Tmp.toUtf8().constData());
            }

            s32_Return = opc_ExistingProtocol->OsySecurityAccessSendKey(
                ou8_SecurityLevel, u32_KEY, opu8_NrCode);
          }
        } else {
          ...
          if (mpc_SecurityPemDb != NULL) {
            ...
            if (ou8_SecurityLevel == 7U) {
              ...
              s32_Return = opc_ExistingProtocol->OsyReadCertificateSerialNumber(
                  c_CertSnr, opu8_NrCode);

              if (s32_Return == C_NO_ERR) {
                // get PEM file by serial number from database
                pc_PemKeyInfo =
                    this->mpc_SecurityPemDb->GetPemFileBySerialNumber(
                        c_CertSnr);
              }
            }
            ...
            if (pc_PemKeyInfo != NULL) {
              ...
              // calculate RSA signature with private key and random value from
              // server (u64_Seed)
              s32_Return = C_OscSecurityRsa::h_SignSignature(
                  c_PrivKey, c_RandomValue, c_Signature);

              if (s32_Return != C_NO_ERR) {
                QString c_Tmp;
                c_Tmp = QString::asprintf(
                    "Error on calculating RSA signature: %d", s32_Return);
                osc_write_log_error("Security Access",
                                    c_Tmp.toUtf8().constData());
                s32_Return = C_CHECKSUM;
              } else {
                ...
                if (c_Signature.size() == 128U) {
                  ...
                  s32_Return = opc_ExistingProtocol->OsySecurityAccessSendKey(
                      ou8_SecurityLevel, c_Signature, opu8_NrCode);
                  if (s32_Return != C_NO_ERR) {
                    QString c_Tmp;
                    c_Tmp = QString::asprintf(
                        "Error on calculating RSA signature: %d", s32_Return);
              ...
              if (ou8_SecurityLevel == 7U) {
                osc_write_log_error("Security Access",
                                    "No level 7 PEM file found in database.");
              } else {
                osc_write_log_error(
                    "Security Access",
                    "No PEM file found for received serial number.");
              }
              s32_Return = C_CHECKSUM;
            }
          } else {
            osc_write_log_error("Security Access",
                                "PEM database not initialized.");
            s32_Return = C_CONFIG;
          }
        }
      }
      ...
      if ((s32_Return != C_NO_ERR) && (s32_Return != C_TIMEOUT) &&
          (s32_Return != C_WARN) && (s32_Return != C_CONFIG) &&
...
/*! \brief   Sets all nodes into a specific security level
   ...
   \param[in]     ou8_SecurityLevel     level of requested security
   \param[out]    orc_ErrorActiveNodes  All active node indexes of nodes which
   can not be reached
   ...
   C_CONFIG    Init function was not called or not successful or protocol was
   not initialized properly. C_COM       Error of service C_TIMEOUT   Expected
   response not received within timeout C_CHECKSUM  Security related error
   (something went wrong while handshaking with the server) Detailed error codes
   are logged with opu8_NrCode
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodesSecurityAccess(
    const uint8_t ou8_SecurityLevel,
    QSet<uint32_t> &orc_ErrorActiveNodes) const {
  QList<uint32_t> c_AllActiveNodes;
  uint32_t u32_Counter;
  ...
  return this->m_SetNodesSecurityAccess(c_AllActiveNodes, ou8_SecurityLevel,
                                        orc_ErrorActiveNodes);
}
...
/*! \brief   Sets all nodes into a specific security level
   ...
   \param[in]     orc_ActiveNodes       All indexes of active nodes to set the
                                        security access (indexes of
   mc_ActiveNodesIndexes) \param[in]     ou8_SecurityLevel     level of
   requested security \param[out] orc_ErrorActiveNodes     All active node
   indexes of nodes which can not be reached
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_SetNodesSecurityAccess(
    const QList<uint32_t> &orc_ActiveNodes, const uint8_t ou8_SecurityLevel,
    QSet<uint32_t> &orc_ErrorActiveNodes) const {
  int32_t s32_Return = C_CONFIG;
  ...
  if (this->mq_Initialized == true) {
    ...
    for (u32_Counter = 0U; u32_Counter < orc_ActiveNodes.size();
         ++u32_Counter) {
      const uint32_t u32_ActiveNode = orc_ActiveNodes[u32_Counter];
      s32_Return = this->m_SetNodeSecurityAccess(u32_ActiveNode,
                                                 ou8_SecurityLevel, NULL);

      if ((s32_Return != C_NO_ERR) && (s32_Return != C_NOACT)) {
        // Store invalid node
        if (u32_ActiveNode < this->mc_ActiveNodesIndexes.size()) {
          orc_ErrorActiveNodes.insert(
              this->mc_ActiveNodesIndexes[u32_ActiveNode]);
        }
        // Do not change the C_TIMEOUT and C_CHECKSUM error
        if ((s32_Return != C_TIMEOUT) && (s32_Return != C_CHECKSUM)) {
          s32_Return = C_COM;
        }
        break;
      }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Initialize the necessary routing configuration for IP to IP routing
   for a specific node

   Prepares the active nodes on the route with its configuration for IP to IP
   routing.

   Be careful: In case of using IP to IP routing all previous CAN routing
   configurations will be reseted and closed Call m_StartRoutingIp2Ip before
   m_StartRouting! Both functions must be called in correct order to get routing
   work in all cases.

   \param[in]     ou32_ActiveNode             active node index of vector
   mc_ActiveNodes \param[out]    opu32_ErrorActiveNodeIndex  optional pointer
   for active node index which caused the error on starting routing if an error
   occurred
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_StartRoutingIp2Ip(
    const uint32_t ou32_ActiveNode,
  ...
  if (rc_ActRoute.c_VecRoutePoints.size() > 0) {
    if ((this->mpc_SysDef != NULL) &&
        (this->mc_ActiveNodesIndexes[ou32_ActiveNode] <
         this->mpc_SysDef->c_Nodes.size())) {
      C_OscProtocolDriverOsy *pc_ProtocolOsy = NULL;
      uint32_t u32_OsyRoutingTarget;
      bool q_Found;

      if (this->m_IsRoutingSpecificNecessary(
              this->mpc_SysDef
                  ->c_Nodes[this->mc_ActiveNodesIndexes[ou32_ActiveNode]]) ==
          true) {
        ...
        u32_OsyRoutingTarget = this->m_GetActiveIndex(
            rc_ActRoute
                .c_VecRoutePoints[rc_ActRoute.c_VecRoutePoints.size() - 1]
                .u32_NodeIndex,
            &q_Found);
      } else {
        // No error. No specific protocol necessary. We need only openSYDE
        // protocol.
      ...
      if (q_Found == true) {
        // Using the same connection for configuring the router and the final
        // openSYDE target
        pc_ProtocolOsy = dynamic_cast<C_OscProtocolDriverOsy *>(
            this->mc_OsyProtocols[u32_OsyRoutingTarget]);
      }

      if (pc_ProtocolOsy != NULL) {
        // In case of Ethernet save the number of protocols which use the same
        // TCP connection
        const QMap<uint32_t, uint32_t>::const_iterator c_Iter =
            this->mc_ActiveNodeIp2IpDispatcher.constFind(ou32_ActiveNode);

        if ((c_Iter != this->mc_ActiveNodeIp2IpDispatcher.constEnd()) &&
            (c_Iter.value() <
             this->mc_ActiveNodeIp2IpDispatcherNodeCount.size())) {
          this->mc_ActiveNodeIp2IpDispatcherNodeCount[c_Iter.value()] =
              this->mc_ActiveNodeIp2IpDispatcherNodeCount[c_Iter.value()] + 1U;
        }
      }

      // Connect only one time if connection was used already for the node
      // itself or an legacy routing target
      if ((pc_ProtocolOsy != NULL) &&
          (pc_ProtocolOsy->IsConnected() != C_NO_ERR)) {
        uint32_t u32_RoutePoint;

        for (u32_RoutePoint = 0U;
             u32_RoutePoint < rc_ActRoute.c_VecRoutePoints.size();
             ++u32_RoutePoint) {
          const C_OscRoutingRoutePoint &rc_RoutePoint =
              rc_ActRoute.c_VecRoutePoints[u32_RoutePoint];
          ...
          if ((rc_RoutePoint.e_InInterfaceType == C_OscSystemBus::eETHERNET) &&
            ...
            if (q_Found == true) {
              ...
              // Clear all queues. In case of CAN tp the change causes that more
              // than one queue receives service responses
              this->ClearDispatcherQueue();
              // Because of using the same TCP connection with the IP of the
              // router, the transport protocol must be adapted to configure the
              // router
              pc_ProtocolOsy->SetNodeIdentifiers(
                  this->mc_ClientId, this->mc_ServerIds[u32_RouterActiveIndex]);

              if (u32_NextRoutePoint < rc_ActRoute.c_VecRoutePoints.size()) {
                // Next node is a further routing node
                u32_NextEthernetNode = this->m_GetActiveIndex(
                    rc_ActRoute.c_VecRoutePoints[u32_NextRoutePoint]
                        .u32_NodeIndex,
                    &q_Found);
            ...
            if (s32_Return == C_NO_ERR) {
              // Do not disconnect after. The connection is necessary to hold
              // the routing alive.
              s32_Return = pc_ProtocolOsy->ReConnect();
            }
            ...
            if (s32_Return == C_NO_ERR) {
              s32_Return = this->m_SetNodeSessionIdWithExpectation(
                  u32_OsyRoutingTarget, this->m_GetRoutingSessionId());
            }

            if (s32_Return == C_NO_ERR) {
              // We need an other security level
              s32_Return =
                  this->m_SetNodeSecurityAccess(u32_OsyRoutingTarget, 5, NULL);
            }

            if (s32_Return == C_NO_ERR) {
              // Get the Bus IDs
              if ((rc_RoutePoint.u32_InBusIndex <
                   this->mpc_SysDef->c_Buses.size()) &&
                  (rc_RoutePoint.u32_OutBusIndex <
                   this->mpc_SysDef->c_Buses.size())) {
                const uint8_t u8_SourceBusId =
                    this->mpc_SysDef->c_Buses[rc_RoutePoint.u32_InBusIndex]
                        .u8_BusId;
                const uint8_t u8_TargetBusId =
                    this->mpc_SysDef->c_Buses[rc_RoutePoint.u32_OutBusIndex]
                        .u8_BusId;
                ...
                s32_Return = pc_ProtocolOsy->OsySetRouteIp2IpCommunication(
                    ...
                    rc_RoutePoint.u8_OutInterfaceNumber, u8_SourceBusId,
                    u8_TargetBusId,
            ...
            if (s32_Return == C_NO_ERR) {
              ...
              do {
                uint8_t u8_Status;
                s32_Return =
                    pc_ProtocolOsy->OsyCheckRouteIp2IpCommunication(u8_Status);

                if ((s32_Return != C_NO_ERR) ||
                    (u8_Status >=
                     C_OscProtocolDriverOsy::hu8_OSY_IP_2_IP_STATUS_ERROR)) {
                  // Finished or error
                  q_Started = true;
                  ...
                  if ((s32_Return == C_NO_ERR) &&
                    ...
                    // Error of service
                    s32_Return = C_WARN;
                    // If this service will not be finished without error, the
                    // router node is available and the target of the next point
                    // is not reachable
                    q_NextOneError = true;
                  } else {
                  ...
                  // No error and not finished yet, set timeout in case of loop
                  // abort condition
                  s32_Return = C_TIMEOUT;
                  // If this service will not be finished in its time, the
                  // router node is available and the target of the next point
                  // is not reachable
                  q_NextOneError = true;
            ...
            if ((s32_Return != C_NO_ERR) &&
                (opu32_ErrorActiveNodeIndex != NULL)) {
            ...
            // than one queue receives service responses
            this->ClearDispatcherQueue();
            // Configuring IP to IP routing finished, reset the transport
            // protocol configuration
            pc_ProtocolOsy->SetNodeIdentifiers(
                this->mc_ClientId, this->mc_ServerIds[u32_OsyRoutingTarget]);
          }
        }
      }
    } else {
  ...
  if ((s32_Return == C_NO_ERR) && (q_Started == true)) {
    osc_write_log_info(
        "Start IP to IP Routing",
        "IP to IP Routing to node " +
            QString::number(this->mc_ActiveNodesIndexes[ou32_ActiveNode]) +
            " over " +
            QString::number(
                this->mc_Routes[ou32_ActiveNode].c_VecRoutePoints.size()) +
            " routing points started.");
  } else if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Start IP to IP Routing",
        "Error on starting IP to IP routing to node " +
            QString::number(this->mc_ActiveNodesIndexes[ou32_ActiveNode]) +
            " with error " + C_OscLoggingHandler::h_StwError(s32_Return));
  } else {
    // Nothing to do
  }
...
/*! \brief   Initialize the necessary routing configuration to start the routing
   ...
   Prepares the active nodes on the route with its routing configurations if
   necessary Three different types of routing:
   - openSYDE routing for an openSYDE server
   - legacy routing for a non openSYDE server
   - legacy routing for a non openSYDE after openSYDE routing to an openSYDE
   server
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_StartRouting(
    const uint32_t ou32_ActiveNode,
    uint32_t *const opu32_ErrorActiveNodeIndex) {
  int32_t s32_Return = C_NO_ERR;
  ...
  if (this->mc_Routes[ou32_ActiveNode].c_VecRoutePoints.size() > 0) {
    ...
    if (this->mpc_SysDef != NULL) {
      ...
      if (this->mc_ActiveNodesIndexes[ou32_ActiveNode] <
          this->mpc_SysDef->c_Nodes.size()) {
        ...
        const uint32_t u32_ActiveLastNode =
            this->m_GetActiveIndex(c_LastNodeOfRouting.u32_NodeIndex);
        C_OscProtocolDriverOsy *pc_ProtocolOsyOfLastNodeOfRouting =
            dynamic_cast<C_OscProtocolDriverOsy *>(
                this->mc_OsyProtocols[u32_ActiveLastNode]);
        bool q_EthernetRouter = false;
        uint32_t u32_ActiveOsyTargetNode = ou32_ActiveNode;

        s32_Return = this->m_StartRoutingSpecific(
            ou32_ActiveNode, pc_Node, c_LastNodeOfRouting,
            pc_ProtocolOsyOfLastNodeOfRouting, &pc_RoutingDispatcher);
        ...
        if (s32_Return == C_NO_ERR) {
          ...
          if (c_ActRoute.c_VecRoutePoints.size() > 1) {
            ...
            // Remove the last routing point. This last step will be made by the
            // legacy routing
            c_ActRoute.c_VecRoutePoints.resize(
        ...
        // Start the routing
        if ((s32_Return == C_NO_ERR) && (q_OsyRouting == true)) {
          // Setup and start openSYDE routing configuration
          uint32_t u32_CounterRoutePoints;
          const uint32_t u32_SourceBusIndex =
              c_ActRoute.c_VecRoutePoints[0].u32_InBusIndex;

          for (u32_CounterRoutePoints = 0U;
               u32_CounterRoutePoints < c_ActRoute.c_VecRoutePoints.size();
               ++u32_CounterRoutePoints) {
            const C_OscRoutingRoutePoint &rc_Point =
                c_ActRoute.c_VecRoutePoints[u32_CounterRoutePoints];

            // In case of Ethernet as out interface the ip to ip routing handles
            // this server
            if (rc_Point.e_OutInterfaceType != C_OscSystemBus::eETHERNET) {
              const uint32_t u32_ActiveRouterNode =
                  this->m_GetActiveIndex(rc_Point.u32_NodeIndex);

              C_OscProtocolDriverOsy *const pc_ProtocolOsyTarget =
                  dynamic_cast<C_OscProtocolDriverOsy *>(
                      this->mc_OsyProtocols[u32_ActiveOsyTargetNode]);

              if (pc_ProtocolOsyTarget != NULL) {
                // Clear all queues. In case of CAN tp the change causes that
                // more than one queue receives service responses
                this->ClearDispatcherQueue();
                // Because of using the same connection,
                // the protocol must be adapted to configure the router
                pc_ProtocolOsyTarget->SetNodeIdentifiers(
                    this->mc_ClientId,
                ...
                if (s32_Return == C_NO_ERR) {
                  // We need a new session
                  s32_Return = this->m_SetNodeSessionIdWithExpectation(
                      u32_ActiveOsyTargetNode, this->m_GetRoutingSessionId());
                }

                if (s32_Return == C_NO_ERR) {
                  // We need an other security level
                  s32_Return = this->m_SetNodeSecurityAccess(
                      u32_ActiveOsyTargetNode, 5, NULL);
                }
                ...
                if (s32_Return == C_NO_ERR) {
                  ...
                  for (u32_CounterTargetPoint = u32_CounterRoutePoints;
                       ...
                       c_ActRoute.c_VecRoutePoints.size();
                       ++u32_CounterTargetPoint) {
                    const C_OscRoutingRoutePoint &rc_PointTarget =
                        c_ActRoute.c_VecRoutePoints[u32_CounterTargetPoint];

                    // Get the Bus IDs
                    if ((u32_SourceBusIndex <
                         this->mpc_SysDef->c_Buses.size()) &&
                        (rc_PointTarget.u32_OutBusIndex <
                         this->mpc_SysDef->c_Buses.size())) {
                      const uint8_t u8_SourceBusId =
                          this->mpc_SysDef->c_Buses[u32_SourceBusIndex]
                              .u8_BusId;
                      const uint8_t u8_TargetBusId =
                          this->mpc_SysDef
                              ->c_Buses[rc_PointTarget.u32_OutBusIndex]
                              .u8_BusId;

                      // Configure the real node for each layer
                      s32_Return = pc_ProtocolOsyTarget
                                       ->OsySetRouteDiagnosisCommunication(
                                           static_cast<uint8_t>(
                                               rc_Point.e_InInterfaceType),
                                           rc_Point.u8_InInterfaceNumber,
                                           static_cast<uint8_t>(
                                               rc_Point.e_OutInterfaceType),
                                           rc_Point.u8_OutInterfaceNumber,
                                           u8_SourceBusId, u8_TargetBusId);
                    } else {
                      s32_Return = C_CONFIG;
                    }
                ...
                // more than one queue receives service responses
                this->ClearDispatcherQueue();
                // Configuring of routing finished, reset the protocol
                // configuration
                pc_ProtocolOsyTarget->SetNodeIdentifiers(
                    this->mc_ClientId,
        ...
        if ((pc_RoutingDispatcher != NULL) &&
          ...
          // In case of Ethernet the connection to the legacy routing server
          // must be established.
          if ((q_OsyRouting == false) &&
            ...
            if ((pc_ProtocolOsyRouter != NULL) && (q_EthernetRouter == false)) {
          ...
          if (s32_Return == C_NO_ERR) {
            ...
            // We need a new session
            s32_Return = this->m_SetNodeSessionIdWithExpectation(
                u32_ActiveLastNode, this->m_GetRoutingSessionId());

            if (s32_Return == C_NO_ERR) {
              // We need an other security level
              s32_Return =
                  this->m_SetNodeSecurityAccess(u32_ActiveLastNode, 5, NULL);
            }
        ...
        if (s32_Return == C_NO_ERR) {
          const QMap<uint32_t, uint32_t>::const_iterator c_ItRouterNode =
              this->mc_ActiveNodeIp2CanDispatcher.constFind(ou32_ActiveNode);

          if (c_ItRouterNode != this->mc_ActiveNodeIp2CanDispatcher.constEnd()) {
            // Special case: IP to CAN routing with an dispatcher of an other
            // node
            if (c_ItRouterNode.value() <
                this->mc_ActiveNodeIp2CanDispatcherNodeCount.size()) {
              this->mc_ActiveNodeIp2CanDispatcherNodeCount[c_ItRouterNode
                                                               .value()] =
                  this->mc_ActiveNodeIp2CanDispatcherNodeCount[c_ItRouterNode
                                                                   .value()] +
                  1U;
          ...
          // Routing started. Save the active node index of the last router node
          // on the route
          this->mc_ActiveNodesLastCanRouters.push_back(u32_ActiveLastNode);
        }
      } // lint !e429  //pc_RoutingDispatcher will be saved in
        // mc_LegacyRouterDispatchers and deleted by destructor
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    osc_write_log_error(
        "Start Routing",
        "Error on starting routing to node " +
            QString::number(this->mc_ActiveNodesIndexes[ou32_ActiveNode]) +
            " with error " + C_OscLoggingHandler::h_StwError(s32_Return));
  }
  ...
  if ((ou32_ActiveNode < this->mc_Routes.size()) &&
    ...
    const C_OscNode *const pc_Node =
        &this->mpc_SysDef
             ->c_Nodes[this->mc_ActiveNodesIndexes[ou32_ActiveNode]];
    const C_OscRoutingRoute &rc_ActRoute = this->mc_Routes[ou32_ActiveNode];
    const uint32_t u32_LastRoutePointIndex =
        static_cast<uint32_t>(rc_ActRoute.c_VecRoutePoints.size() - 1);
    ...
    if (q_Found == true) {
      ...
      for (s32_CounterRoutePoints =
               static_cast<int32_t>(u32_LastRoutePointIndex);
           s32_CounterRoutePoints >= 0; --s32_CounterRoutePoints) {
        // Stop the routing from behind
        if (this->m_StopRoutingOfRoutingPoint(
                ou32_ActiveNode, u32_ActiveOsyTargetNode,
...
/*! \brief   Stops the entire routing configuration for all active nodes
   ...
   Calling m_StopRouting for all nodes would cause following problem: The
   routing of one route can be necessary to get to an other routing point for a
   clean stopping. The stop service will close all active routing configurations
   on one node.

   For example closing A - B will close A - C as well. C - E could no be stopped
   properly.
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverProtocol::m_StopRoutingOfActiveNodes(void) {
  if (this->mpc_SysDef != NULL) {
    ...
    for (u32_ActiveNode = 0U;
         u32_ActiveNode < this->mc_ActiveNodesIndexes.size();
         ++u32_ActiveNode) {
      ...
      if (rc_ActRoute.c_VecRoutePoints.size() > 0) {
        const C_OscNode *const pc_Node =
            &this->mpc_SysDef
                 ->c_Nodes[this->mc_ActiveNodesIndexes[u32_ActiveNode]];

        // Stop specific routing of all nodes first. It needs the openSYDE
        // routing if configured
        this->m_StopRoutingSpecific(u32_ActiveNode);

        // Get the necessary active node indexes of the last openSYDE nodes on
        // the route
        if (this->m_IsRoutingSpecificNecessary(*pc_Node) == true) {
          bool q_Found;

          // Specific protocol necessary. Routing dispatcher necessary.
          // Use the last openSYDE node protocol instance for the IP to IP
          // routing
        ...
        if (rc_ActRoute.c_VecRoutePoints.size() >
            static_cast<uint32_t>(s32_LastRouteHop)) {
          s32_LastRouteHop =
              static_cast<uint32_t>(rc_ActRoute.c_VecRoutePoints.size());
        }
      }
    }

    // Stop the routing for each 'hop distance' from behind
    for (s32_RouteHopCounter = s32_LastRouteHop; s32_RouteHopCounter >= 0;
         --s32_RouteHopCounter) {
      for (u32_ActiveNode = 0U;
           u32_ActiveNode < this->mc_ActiveNodesIndexes.size();
           ++u32_ActiveNode) {
        if (c_ActiveOsyTargetNodes[u32_ActiveNode] >= 0) {
          const C_OscRoutingRoute &rc_ActRoute =
              this->mc_Routes[u32_ActiveNode];

          if (static_cast<uint32_t>(s32_RouteHopCounter) <
              rc_ActRoute.c_VecRoutePoints.size()) {
            if (this->m_StopRoutingOfRoutingPoint(
                    u32_ActiveNode, c_ActiveOsyTargetNodes[u32_ActiveNode],
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Stops the routing configuration for one specific point in the route
   to a node

   \param[in]   ou32_ActiveNode            active node index to stop routing for
   \param[in]   ou32_ActiveOsyTargetNode   active node index of last openSYDE
   node in the route can be the same as ou32_ActiveNode \param[in]   orc_Point
   current routing point to stop \param[in]   oq_FirstPoint              flag if
   point is the first point in the route

   \return
   C_NO_ERR    Routing for point deactivated
   C_NOACT     Routing for point deactivated and no further stopping necessary
   for this node C_COM       communication driver reported error C_CHECKSUM
   Security related error (something went wrong while handshaking with the
   server)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_StopRoutingOfRoutingPoint(
    const uint32_t ou32_ActiveNode, const uint32_t ou32_ActiveOsyTargetNode,
    const C_OscRoutingRoutePoint &orc_Point, const bool oq_FirstPoint) {
  int32_t s32_Return = C_NO_ERR;

  // Using the same connection for configuring the router and the final openSYDE
  // target
  C_OscProtocolDriverOsy *const pc_ProtocolOsyTarget =
      dynamic_cast<C_OscProtocolDriverOsy *>(
          this->mc_OsyProtocols[ou32_ActiveOsyTargetNode]);

  if (pc_ProtocolOsyTarget != NULL) {
    bool q_DoDisconnect = true;

    if (orc_Point.e_OutInterfaceType == C_OscSystemBus::eCAN) {
      const uint32_t u32_CurrentNode =
          this->m_GetActiveIndex(orc_Point.u32_NodeIndex);
      int32_t s32_Retval;

      // Clear all queues. In case of CAN tp the change causes that more than
      // one queue receives service responses
      this->ClearDispatcherQueue();
      // Because of using the same connection,
      // the protocol must be adapted to configure the router
      pc_ProtocolOsyTarget->SetNodeIdentifiers(
          this->mc_ClientId, this->mc_ServerIds[u32_CurrentNode]);

      // We need an other security level
      s32_Retval =
          this->m_SetNodeSecurityAccess(ou32_ActiveOsyTargetNode, 5, NULL);
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        if (s32_Retval != C_NO_ERR) {
          osc_write_log_error("Stop Routing",
                              "Error on stopping routing to node " +
                                  QString::number(orc_Point.u32_NodeIndex) +
                                  " with error " +
                                  C_OscLoggingHandler::h_StwError(s32_Retval));
        }
      } else {
        osc_write_log_error("Stop Routing",
                            "Error on setting security access on node " +
                                QString::number(orc_Point.u32_NodeIndex) +
                                " with error " +
                                C_OscLoggingHandler::h_StwError(s32_Retval));
      }
      ...
      if (oq_FirstPoint == true) {
        // In case of Ethernet to CAN routing check the number of protocols
        // which use the same TCP connection Disconnect only if no one is left
        const QMap<uint32_t, uint32_t>::const_iterator c_ItRouterNode =
            this->mc_ActiveNodeIp2CanDispatcher.constFind(ou32_ActiveNode);

        if ((c_ItRouterNode != this->mc_ActiveNodeIp2CanDispatcher.constEnd()) &&
            (c_ItRouterNode.value() <
             this->mc_ActiveNodeIp2CanDispatcherNodeCount.size())) {
          // Special case: IP to CAN routing with an dispatcher of an other node
          if (this->mc_ActiveNodeIp2CanDispatcherNodeCount[c_ItRouterNode
                                                               .value()] > 0U) {
            this->mc_ActiveNodeIp2CanDispatcherNodeCount[c_ItRouterNode
                                                             .value()] =
                this->mc_ActiveNodeIp2CanDispatcherNodeCount[c_ItRouterNode
      ...
      pc_ProtocolOsyTarget->SetNodeIdentifiers(
          this->mc_ClientId, this->mc_ServerIds[ou32_ActiveOsyTargetNode]);
    }

    // In case of Ethernet as out interface the ip to ip routing is active and
    // will be closed by closing the TCP connection with the disconnect
    if (orc_Point.e_InInterfaceType == C_OscSystemBus::eETHERNET) {
      // Ethernet routing to this point

      // Special case: IP to IP routing. The protocol instance of the target
      // server is used to communicate with the first router server

      // In case of Ethernet check the number of protocols which use the same
      // TCP connection Disconnect only if no one is left
      const QMap<uint32_t, uint32_t>::const_iterator c_Iter =
          this->mc_ActiveNodeIp2IpDispatcher.constFind(ou32_ActiveNode);
      ...
      if ((c_Iter != this->mc_ActiveNodeIp2IpDispatcher.constEnd()) &&
          (c_Iter.value() <
           this->mc_ActiveNodeIp2IpDispatcherNodeCount.size())) {
        if (this->mc_ActiveNodeIp2IpDispatcherNodeCount[c_Iter.value()] > 0U) {
          this->mc_ActiveNodeIp2IpDispatcherNodeCount[c_Iter.value()] =
              this->mc_ActiveNodeIp2IpDispatcherNodeCount[c_Iter.value()] - 1U;
        }
...
/*! \brief   Stops the specific routing configuration for one specific node
   ...
   Cleans up the legacy router dispatcher. Must be called be overloaded
   functions

   \param[in]     ou32_ActiveNode                      Active node index of
   vector mc_ActiveNodes
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComDriverProtocol::m_StopRoutingSpecific(
    const uint32_t ou32_ActiveNode) {
  ...
  if ((ou32_ActiveNode < this->mc_LegacyRouterDispatchers.size()) &&
    ...
    if (this->mc_Routes[ou32_ActiveNode].c_VecRoutePoints.size() > 0) {
      ...
      const uint32_t u32_ActiveRouterNode = this->m_GetActiveIndex(
          this->mc_Routes[ou32_ActiveNode]
              .c_VecRoutePoints
                  [this->mc_Routes[ou32_ActiveNode].c_VecRoutePoints.size() - 1]
              .u32_NodeIndex);
...
int32_t C_OscComDriverProtocol::m_InitRoutesAndActiveNodes(void) {
  ...
  if (this->mpc_SysDef != NULL) {
    ...
    // Init routes
    for (uint32_t u32_ItActiveFlag = 0;
         u32_ItActiveFlag < this->mc_ActiveNodesSystem.size();
         ++u32_ItActiveFlag) {
      if (this->mc_ActiveNodesSystem[u32_ItActiveFlag] == 1U) {
        // Calculate all routes
        const C_OscRoutingCalculation c_RouteCalculation(
            this->mpc_SysDef->c_Nodes, this->mc_ActiveNodesSystem,
            this->mu32_ActiveBusIndex, u32_ItActiveFlag, e_Mode);
        s32_Retval = c_RouteCalculation.GetState();
        if (s32_Retval == C_NO_ERR) {
          // Get the best route for this node
          const C_OscRoutingRoute *const pc_Route =
              c_RouteCalculation.GetBestRoute();
          if (pc_Route != NULL) {
            // If no routing shall be used, register only nodes which are
            // connected to the PC bus directly
...
/*! \brief   Initialize Server ID for each node
   ...
   Assemble a table of all server IDs seen from the bus interface we are
   connected to. Also create table of IP addresses.
...
int32_t C_OscComDriverProtocol::m_InitServerIds(void) {
  ...
  if ((this->mpc_SysDef != NULL) &&
    ...
    for (uint32_t u32_ItActiveNode = 0;
         (u32_ItActiveNode < this->mc_ActiveNodesIndexes.size()) &&
         (s32_Retval == C_NO_ERR);
         ++u32_ItActiveNode) {
      ...
      if (rc_Route.c_VecRoutePoints.size() == 0) {
        ...
        if (rc_LastHop.u32_NodeIndex < this->mpc_SysDef->c_Nodes.size()) {
          const C_OscNode *const pc_LastHopNode =
              &this->mpc_SysDef->c_Nodes[rc_LastHop.u32_NodeIndex];

          const C_OscNodeComInterfaceSettings *const pc_Interface =
              pc_LastHopNode->c_Properties.GetComInterface(
                  rc_LastHop.e_OutInterfaceType,
      ...
      if (u32_BusIndex < this->mpc_SysDef->c_Buses.size()) {
        ...
        if (u32_ItActiveNode < this->mpc_SysDef->c_Nodes.size()) {
          const C_OscNode *const pc_Node =
              &this->mpc_SysDef
                   ->c_Nodes[this->mc_ActiveNodesIndexes[u32_ItActiveNode]];
          ...
          for (uint32_t u32_ItComInterface = 0;
               u32_ItComInterface < rc_ComInterfaces.size();
               ++u32_ItComInterface) {
            ...
            if (rc_CurComInterface.GetBusConnected() == true) {
              if (rc_CurComInterface.u32_BusIndex == u32_BusIndex) {
                ...
                // The first interface with at least one activated feature is
                // enough If no feature is active on this interface, keep
                // searching for a 'better' interface. If no better interface is
                // found, use the already found interface Handling of only one
                // server id is possible
                if (this->m_CheckInterfaceForFunctions(rc_CurComInterface) ==
                    true) {
...
/*! \brief   Initialize for CAN connection
   ...
   C_OVERFLOW    Error on setting dispatcher or node identifier in transport
   protocol C_RANGE       Count of server ids does not match to the count of
   nodes
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_InitForCan(void) {
  ...
  if (this->mc_ServerIds.size() == this->mu32_ActiveNodeCount) {
    ...
    if (this->mpc_CanDispatcher != NULL) {
      // Transport protocols
      this->mc_TransportProtocols.resize(
          static_cast<uint32_t>(this->mu32_ActiveNodeCount), NULL);
      this->mc_LegacyRouterDispatchers.resize(
          static_cast<uint32_t>(this->mu32_ActiveNodeCount), NULL);
      for (uint32_t u32_ItActiveNode = 0;
           (u32_ItActiveNode < this->mu32_ActiveNodeCount) &&
           (s32_Retval == C_NO_ERR);
           ++u32_ItActiveNode) {
        pc_TransportProtocol = new C_OscProtocolDriverOsyTpCan();
        s32_Retval = pc_TransportProtocol->SetNodeIdentifiers(
            this->mc_ClientId, this->mc_ServerIds[u32_ItActiveNode]);
        if (s32_Retval == C_NO_ERR) {
          s32_Retval =
              pc_TransportProtocol->SetDispatcher(this->mpc_CanDispatcher);
          if (s32_Retval != C_NO_ERR) {
            QString c_Text = "Node \"";
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        s32_Retval = this->mpc_CanTransportProtocolBroadcast
                         ->SetNodeIdentifiersForBroadcasts(this->mc_ClientId);
        if (s32_Retval == C_NO_ERR) {
          s32_Retval = this->mpc_CanTransportProtocolBroadcast->SetDispatcher(
    ...
    // lint -e{593}  pc_TransportProtocol will be saved in mc_TransportProtocols
    // and deleted by destructor
  } else {
    // Routing error
    osc_write_log_error("Comm driver initialization",
                        "Failed: CAN initialization error because of different "
                        "node count to server IDs");
    s32_Retval = C_RANGE;
  }
  return s32_Retval;
}
...
int32_t C_OscComDriverProtocol::m_InitForEthernet(void) {
  ...
  if (this->mc_ServerIds.size() == this->mu32_ActiveNodeCount) {
    ...
    this->mc_ActiveNodeIp2IpDispatcherNodeCount.resize(
        this->mu32_ActiveNodeCount, 0U);

    this->mc_ActiveNodeIp2CanDispatcher.clear();
    this->mc_ActiveNodeIp2CanDispatcherNodeCount.resize(
        this->mu32_ActiveNodeCount, 0U);

    if (this->mpc_IpDispatcher != NULL) {
      uint32_t u32_ItActiveNode;
      QMap<uint32_t, uint32_t> c_ActiveNodeIp2IpRouters;

      // We can communicate only with 'direct' connected Ethernet devices. All
      // other devices will be reached by routing and shall use the IP of the
      // first router node. But for each routing must exist an own connection in
      // case of IP to IP routing, but not in case of IP to CAN routing.

      Q_ASSERT(this->mc_Routes.size() == this->mu32_ActiveNodeCount);

      // Initialize all Ethernet connections without routing
      for (u32_ItActiveNode = 0; u32_ItActiveNode < this->mu32_ActiveNodeCount;
           ++u32_ItActiveNode) {
        ...
        if (this->mc_Routes[u32_ItActiveNode].c_VecRoutePoints.size() == 0) {
          s32_Retval = this->m_InitTcp(
              mc_ServerIpAddresses[u32_ItActiveNode].au8_IpAddress,
      ...
      if (s32_Retval == C_NO_ERR) {
        ...
        for (u32_ItActiveNode = 0;
          ...
          if (this->mc_Routes[u32_ItActiveNode].c_VecRoutePoints.size() != 0) {
            ...
            s32_Retval = m_GetActiveIndexOfIp2IpRouter(
                u32_ItActiveNode, u32_Ip2IpRouterActiveNodeClientSide,
                u32_Ip2IpRouterActiveNodeTargetSide);

            if (s32_Retval == C_NO_ERR) {
              // Exist a dispatcher handle for this router server
              const QMap<uint32_t, uint32_t>::const_iterator
                  c_IterUniqueIp2IpRouters = c_ActiveNodeIp2IpRouters.constFind(
                      u32_Ip2IpRouterActiveNodeTargetSide);

              if (c_IterUniqueIp2IpRouters == c_ActiveNodeIp2IpRouters.constEnd()) {
                // New dispatcher handle necessary
                s32_Retval = this->m_InitTcp(
                    mc_ServerIpAddresses[u32_Ip2IpRouterActiveNodeClientSide]
                        .au8_IpAddress,
                    c_IpDispatcherHandles[u32_ItActiveNode]);

                // Save the associated position of the dispatcher handle for
                // this specific router
                c_ActiveNodeIp2IpRouters.insert(
                    u32_Ip2IpRouterActiveNodeTargetSide, u32_ItActiveNode);
              } else {
                uint32_t u32_Handle;

                // Reuse the dispatcher handle of the same router
                Q_ASSERT(c_IterUniqueIp2IpRouters.value() <
                         c_IpDispatcherHandles.size());

                u32_Handle =
                    c_IpDispatcherHandles[c_IterUniqueIp2IpRouters.value()];
                c_IpDispatcherHandles[u32_ItActiveNode] = u32_Handle;
              }

              // Save the associated IP to IP router of the active node for
              // later stopping the route
              this->mc_ActiveNodeIp2IpDispatcher.insert(
                      u32_ItActiveNode, u32_Ip2IpRouterActiveNodeTargetSide);
            } else if (s32_Retval == C_NOACT) {
              uint32_t u32_Ip2CanRouterActiveNode;

              // No error, no IP to IP routing, check for IP to CAN routing
              s32_Retval = this->m_GetActiveIndexOfIp2CanRouter(
                  u32_ItActiveNode, u32_Ip2CanRouterActiveNode);

              if (s32_Retval == C_NO_ERR) {
                uint32_t u32_Handle;

                // Reuse the dispatcher handle of the same router
                Q_ASSERT(u32_Ip2CanRouterActiveNode <
                         c_IpDispatcherHandles.size());
                ...
                // Save the associated IP to CAN router of the active node for
                // later stopping the route
        ...
        if (s32_Retval == C_NO_ERR) {
          ...
          for (u32_ItActiveNode = 0;
            ...
            s32_Retval = pc_TransportProtocol->SetNodeIdentifiers(
                this->mc_ClientId, this->mc_ServerIds[u32_ItActiveNode]);
            if (s32_Retval == C_NO_ERR) {
              s32_Retval = pc_TransportProtocol->SetDispatcher(
                  this->mpc_IpDispatcher,
                  c_IpDispatcherHandles[u32_ItActiveNode]);
              if (s32_Retval != C_NO_ERR) {
                osc_write_log_error(
                    "Ethernet initialization",
                    "Could not set IP dispatcher. Error Code: " +
                        QString::number(s32_Retval));
              ...
              osc_write_log_error(
                  "Ethernet initialization",
                  "Could not set node identifiers. Error Code: " +
                      QString::number(s32_Retval));
            ...
            this->mc_TransportProtocols[u32_ItActiveNode] =
                pc_TransportProtocol;
          }
          if (s32_Retval == C_NO_ERR) {
            // Broadcast
            mpc_IpTransportProtocolBroadcast = new C_OscProtocolDriverOsyTpIp();
            s32_Retval = this->mpc_IpTransportProtocolBroadcast->SetDispatcher(
                this->mpc_IpDispatcher, 0U);
            if (s32_Retval != C_NO_ERR) {
              // Invalid configuration = programming error
              osc_write_log_error(
                  "Ethernet initialization",
                  "Could not set broadcast dispatcher. Error Code: " +
                      QString::number(s32_Retval));
          ...
          osc_write_log_error("Ethernet initialization",
                              "Could not initialize UDP. Error Code: " +
                                  QString::number(s32_Retval));
          s32_Retval = C_COM;
        }
      }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Gets the active index of the node which is necessary for IP to IP
   routing

   The IP to IP router oru32_ActiveIndexRouterClient is connected to the client
   directly. The IP to IP router target oru32_ActiveIndexRouterTarget is the
   last server connected to Ethernet bus. It can be the target itself or a
   further router server which routes in the next step over CAN.

   Using IP to IP routing, the IP and connection of the router server and the
   server ID of the target server will be used with an own connection.

   \param[in]     ou32_ActiveIndexTarget        The node which will be
   communicated to \param[out]    oru32_ActiveIndexRouterClient The node which
   is on the route for IP to IP routing on client side \param[out]
   oru32_ActiveIndexRouterTarget The node which is the last on the route for IP
   to IP routing on target side or the target itself
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_GetActiveIndexOfIp2IpRouter(
    const uint32_t ou32_ActiveIndexTarget,
  ...
  if (ou32_ActiveIndexTarget < this->mc_Routes.size()) {
    ...
    if (rc_Route.c_VecRoutePoints.size() == 0) {
      // No routing necessary for the target, but Ethernet connection is
      // necessary
      s32_Return = C_NOACT;
    } else {
      if ((rc_Route.c_VecRoutePoints[0].e_InInterfaceType ==
           C_OscSystemBus::eETHERNET) &&
          (rc_Route.c_VecRoutePoints[0].e_OutInterfaceType ==
           C_OscSystemBus::eETHERNET)) {
        bool q_Found;

        oru32_ActiveIndexRouterClient = this->m_GetActiveIndex(
            rc_Route.c_VecRoutePoints[0].u32_NodeIndex, &q_Found);
        ...
        if (s32_Return == C_NO_ERR) {
          ...
          const int32_t s32_LastRoutePoint =
              static_cast<int32_t>(rc_Route.c_VecRoutePoints.size()) - 1;

          // Search the other 'side'
          for (s32_RoutePointCounter = s32_LastRoutePoint;
               s32_RoutePointCounter >= 0; --s32_RoutePointCounter) {
            if (rc_Route.c_VecRoutePoints[s32_RoutePointCounter]
                    .e_InInterfaceType == C_OscSystemBus::eETHERNET) {
              if ((s32_RoutePointCounter == s32_LastRoutePoint) &&
                  (rc_Route.c_VecRoutePoints[s32_RoutePointCounter]
                       .e_OutInterfaceType == C_OscSystemBus::eETHERNET)) {
                ...
                oru32_ActiveIndexRouterTarget = this->m_GetActiveIndex(
                    rc_Route.c_VecRoutePoints[s32_RoutePointCounter]
                        .u32_NodeIndex,
                    &q_Found);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Gets the active index of the node which is necessary for IP to CAN
   routing

   \param[in]     ou32_ActiveIndexTarget        The node which will be
   communicated to \param[out]    oru32_ActiveIndexRouter       The node which
   is on the first route point for IP to CAN routing on client side
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_GetActiveIndexOfIp2CanRouter(
    const uint32_t ou32_ActiveIndexTarget, uint32_t &oru32_ActiveIndexRouter) {
  int32_t s32_Return = C_RANGE;
  ...
  if (ou32_ActiveIndexTarget < this->mc_Routes.size()) {
    ...
    if (rc_Route.c_VecRoutePoints.size() == 0) {
      ...
      if ((rc_Route.c_VecRoutePoints[0].e_InInterfaceType ==
        ...
        oru32_ActiveIndexRouter = this->m_GetActiveIndex(
            rc_Route.c_VecRoutePoints[0].u32_NodeIndex, &q_Found);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComDriverProtocol::m_InitTcp(const uint8_t (&orau8_Ip)[4],
                                          uint32_t &oru32_Handle) {
  ...
  if (this->mpc_IpDispatcher != NULL) {
    ...
    if (s32_Retval != C_NO_ERR) {
      ...
      c_Text =
          QString::asprintf("Could not set up TCP connection to %d.%d.%d.%d",
                            orau8_Ip[0], orau8_Ip[1], orau8_Ip[2], orau8_Ip[3]);
