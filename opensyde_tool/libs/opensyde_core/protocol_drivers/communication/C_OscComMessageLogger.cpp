...
const QString C_OscComMessageLogger::mhc_ECES_MESSAGE_COUNTER =
    "ECeS_Message_Counter";
...
C_OscComMessageLoggerFilter::C_OscComMessageLoggerFilter(void)
    : u8_ExtendedId(0U), u32_StartId(0U), u32_EndId(0U), q_PassFilter(false) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComMessageLoggerFilter::operator==(
    const C_OscComMessageLoggerFilter &orc_Cmp) const {
  bool q_Return = false;
...
C_OscComMessageLogger::C_OscComMessageLogger(void)
    : mpc_OsySysDefMessage(NULL), mpc_OsySysDefDataPoolList(NULL),
      me_Protocol(stw::cmon_protocol::eCMON_L7_PROTOCOL_NONE), mq_Paused(false),
      mu64_FirstTimeStampStart(0U), mu64_FirstTimeStampDayOfTime(0U),
      mu64_LastTimeStamp(0U), mu32_FilteredMessages(0U),
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::Continue(void) { this->mq_Paused = false; }

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Pauses the logging.

   Pauses the logging of this logger.
   The delivering instance of C_OscComDriverBase will not be changed by this
   pause. Other registered logger will still log.
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::Pause(void) { this->mq_Paused = true; }
...
/*! \brief  Function to react on the stop of the communication
   ...
   Reset filtered message counter and set the first timestamp for calculation of
   absolute timestamp relative to start
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::Start(void) {
  ...
  this->mu64_FirstTimeStampStart =
      std::chrono::duration_cast<std::chrono::microseconds>(duration).count();
  this->mu64_LastTimeStamp = this->mu64_FirstTimeStampStart;

  // Calculate the start time beginning at midnight too, for showing the
  // absolute timestamp as time of day In seconds
  ...
  this->mu64_FirstTimeStampDayOfTime +=
      static_cast<uint64_t>(c_Time.time().msec());
  // in us
  this->mu64_FirstTimeStampDayOfTime *= 1000ULL;
}
...
void C_OscComMessageLogger::SetProtocol(const e_CanMonL7Protocols oe_Protocol) {
  QMap<QString, C_OscComMessageLoggerFileBase *>::iterator c_ItFile;
  ...
  QString c_ProtocolNameScl;
  this->mc_ProtocolDec.GetProtocolName(this->me_Protocol, c_ProtocolNameScl);
  QString c_ProtocolName = c_ProtocolNameScl;
  for (c_ItFile = this->mc_LoggingFiles.begin();
       c_ItFile != this->mc_LoggingFiles.end(); ++c_ItFile) {
    c_ItFile.value()->SetProtocolName(c_ProtocolName);
  }
}
...
/*! \brief  Adds an openSYDE system definition for analyzing
   ...
   \param[in]  orc_PathSystemDefinition Path of system definition file (Must be
   .syde_sysdef) \param[out] orc_Buses                All CAN buses of system
   definition

   \return
   C_NO_ERR    data read
   C_RANGE     specified system definition file does not exist or has a wrong
   extension C_NOACT     specified file is present but structure is invalid
   (e.g. invalid XML file) C_CONFIG    system definition file content is invalid
   or incomplete device definition file could not be loaded C_OVERFLOW  node in
   system definition references a device not part of the device definitions
   C_COM       no CAN bus in system definition
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComMessageLogger::AddOsySysDef(const QString &orc_PathSystemDefinition,
...
/*! \brief  Adds an openSYDE system definition for analyzing
   ...
   \param[in]  orc_PathSystemDefinition Path of system definition file (Must be
   .syde_sysdef) \param[in]  ou32_BusIndex            Bus index of CAN bus of
   system definition for monitoring \param[out] orc_Buses               All CAN
   buses of system definition
...
C_OscComMessageLogger::AddOsySysDef(const QString &orc_PathSystemDefinition,
                                    ...
                                    QList<C_OscSystemBus> &orc_Buses) {
  const QString c_FileExtension =
      "." + QFileInfo(orc_PathSystemDefinition).suffix();
  ...
  if (c_FileExtension == ".syde_sysdef") {
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      for (u32_BusCounter = 0U; u32_BusCounter < c_SysDef.c_Buses.size();
           ++u32_BusCounter) {
        ...
        if (c_SysDef.c_Buses[u32_BusCounter].e_Type == C_OscSystemBus::eCAN) {
          if (q_CanBusFound == false) {
            q_CanBusFound = true;
            // And save the first CAN bus as initial bus if no concrete bus was
            // set or found
      ...
      if (q_CanBusFound == true) {
        ...
        this->m_InsertOsySysDef(orc_PathSystemDefinition, c_SysDef,
                                u32_UsedBusIndex);
      } else {
        s32_Return = C_COM;
      }
  ...
  if (s32_Return != C_NO_ERR) {
    osc_write_log_error("Loading System Definition",
                        "Could not load System Definition. Error code: " +
...
/*! \brief  Updates the set bus index of the specific system definition
   ...
   \param[in] orc_PathSystemDefinition Path of system definition file (Must be
   .syde_sysdef) \param[in] ou32_BusIndex            Bus index of CAN bus of
   system definition for monitoring
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComMessageLogger::SetOsySysDefBus(const QString &orc_PathSystemDefinition,
                                       const uint32_t ou32_BusIndex) {
  int32_t s32_Return = C_NOACT;

  const QMap<QString, C_OscComMessageLoggerOsySysDefConfig>::iterator
  ...
  if (c_ItSysDef != this->mc_OsySysDefs.end()) {
    ...
    if ((ou32_BusIndex < c_ItSysDef.value().c_OsySysDef.c_Buses.size()) &&
        (c_ItSysDef.value().c_OsySysDef.c_Buses[ou32_BusIndex].e_Type ==
         C_OscSystemBus::eCAN)) {
      c_ItSysDef.value().u32_BusIndex = ou32_BusIndex;
      s32_Return = C_NO_ERR;
    } else {
      s32_Return = C_WARN;
    }
...
/*! \brief  Returns the entire loaded openSYDE system definition
   ...
   \param[in]  orc_PathSystemDefinition Path and filename of openSYDE system
   definition file \param[out] orc_SystemDefinition     Loaded system definition
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComMessageLogger::GetOsySysDef(
    const QString &orc_PathSystemDefinition,
    C_OscComMessageLoggerOsySysDefConfig &orc_SystemDefinition) {
  int32_t s32_Return = C_RANGE;

  QMap<QString, C_OscComMessageLoggerOsySysDefConfig>::iterator c_ItSysDef;

  c_ItSysDef = this->mc_OsySysDefs.find(orc_PathSystemDefinition);
  if (c_ItSysDef != this->mc_OsySysDefs.end()) {
    // Copy the DBC definition
    orc_SystemDefinition = c_ItSysDef.value();
    s32_Return = C_NO_ERR;
  }
...
int32_t C_OscComMessageLogger::RemoveDatabase(const QString &orc_Path) {
  ...
  const QMap<QString, C_OscComMessageLoggerOsySysDefConfig>::iterator
      c_ItSysDef = this->mc_OsySysDefs.find(orc_Path);

  if (c_ItSysDef != this->mc_OsySysDefs.end()) {
    this->mc_ProtocolDec.RemoveOsySysDef(&(c_ItSysDef.value()));
    this->mc_ProtocolHex.RemoveOsySysDef(&(c_ItSysDef.value()));
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComMessageLogger::ActivateDatabase(const QString &orc_Path,
                                                const bool oq_Active) {
  int32_t s32_Return = C_NOACT;

  const QHash<QString, bool>::iterator c_ItFlag =
      this->mc_DatabaseActiveFlags.find(orc_Path);
  ...
  if (c_ItFlag != this->mc_DatabaseActiveFlags.end()) {
    ...
    if (c_ItSysDef != this->mc_OsySysDefs.end()) {
      ...
      if (oq_Active == true) {
        this->mc_ProtocolDec.AddOsySysDef(&(c_ItSysDef.value()));
        this->mc_ProtocolHex.AddOsySysDef(&(c_ItSysDef.value()));
      } else {
        this->mc_ProtocolDec.RemoveOsySysDef(&(c_ItSysDef.value()));
        this->mc_ProtocolHex.RemoveOsySysDef(&(c_ItSysDef.value()));
      }
...
/*! \brief  Adds an ASC log file
   ...
   must be .asc \param[in] oq_HexActive               Mode for writing CAN Id
   and CAN data (hexadecimal or decimal) \param[in] oq_RelativeTimeStampActive
   Mode for writing CAN timestamp (relative or absolute)
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscComMessageLogger::AddLogFileAsc(const QString &orc_FilePath,
  ...
  this->mc_ProtocolDec.GetProtocolName(this->me_Protocol, c_ProtocolNameScl);
  c_ProtocolName = c_ProtocolNameScl;
  pc_File = new C_OscComMessageLoggerFileAsc(
      orc_FilePath, c_ProtocolName, oq_HexActive, oq_RelativeTimeStampActive);
  s32_Return = pc_File->OpenFile();

  this->mc_LoggingFiles.insert(orc_FilePath, pc_File);

  return s32_Return; // lint !e429  //no memory leak of pc_File because of
                     // handling of instance in map mc_LoggingFiles
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::AddFilter(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::RemoveFilter(
    const C_OscComMessageLoggerFilter &orc_Filter) {
  uint32_t u32_Counter;

  for (u32_Counter = 0U; u32_Counter < this->mc_CanFilterConfig.size();
       ++u32_Counter) {
    if (orc_Filter == this->mc_CanFilterConfig[u32_Counter]) {
      // Filter found, remove it
      this->mc_CanFilterConfig.erase(this->mc_CanFilterConfig.begin() +
                                     u32_Counter);
      break;
    }
  }
...
/*! \brief  Returns the current count of filtered CAN messages
   ...
   Filtered messages refers to the number of messages that did not pass active
   filter(s). This function is thread safe
...
/*! \brief  If necessary handle and log the CAN message
   ...
   Fills an instance of C_OscComMessageLoggerData which can get by calling
   GetHandledCanMessage. This base class implementation must be called first.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscComMessageLogger::HandleCanMessage(const T_STWCAN_Msg_RX &orc_Msg,
                                                const bool oq_IsTx) {
  int32_t s32_Return = C_BUSY;

  if (this->mq_Paused == false) {
    if (this->m_CheckFilter(orc_Msg) == true) {
      QMap<QString, C_OscComMessageLoggerFileBase *>::const_iterator
          c_ItFileLogger;
      ...
      if (this->m_CheckSysDef(orc_Msg) == true) {
        q_OpenSydeInterpretationFound =
            this->m_InterpretSysDef(this->mc_HandledCanMessage);
      }
      ...
      if (q_OpenSydeInterpretationFound == true) {
        ...
        // No message in openSYDE system definitions found, check and let
        // interpret other
        if (this->m_CheckInterpretation(this->mc_HandledCanMessage) == false) {
          // No interpretation found, check for classical protocol
          // interpretation
          this->mc_HandledCanMessage.c_ProtocolTextHex =
              this->m_GetProtocolStringHex(orc_Msg);
          if (this->mc_HandledCanMessage.c_ProtocolTextHex != "") {
            ...
            this->mc_HandledCanMessage.c_ProtocolTextDec =
                this->m_GetProtocolStringDec(orc_Msg);
          }

          // Message not found in any other place. Check and process for ECoS
          // inverted message
          else {
      ...
      // File logging
      for (c_ItFileLogger = this->mc_LoggingFiles.begin();
           c_ItFileLogger != this->mc_LoggingFiles.end(); ++c_ItFileLogger) {
        c_ItFileLogger.value()->AddMessageToFile(this->mc_HandledCanMessage);
      }
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::ResetCounter(void) { this->m_ResetCounter(); }

//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Optional function for getting the current bus load

   This implementation does nothing with the value. This function must be
   overridden to use the bus load value
...
/*! \brief  Optional function for getting the current detected CAN Tx errors
   ...
   This implementation does nothing with the value. This function must be
   overridden to use the detected CAN Tx errors
...
/*! \brief  Optional function for getting the current counted CAN Tx messages
   ...
   This implementation does nothing with the value. This function must be
   overridden to use the counted CAN Tx messages.
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Returns the current handled CAN message after call of
   HandleCanMessage
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscComMessageLoggerData &
C_OscComMessageLogger::m_GetHandledCanMessage(void) const {
...
bool C_OscComMessageLogger::m_CheckFilter(const T_STWCAN_Msg_RX &orc_Msg) {
  ...
  if (this->mc_CanFilterConfig.size() > 0) {
    ...
    for (u32_FilterCounter = 0U;
         u32_FilterCounter < this->mc_CanFilterConfig.size();
         ++u32_FilterCounter) {
      if (this->mc_CanFilterConfig[u32_FilterCounter].q_PassFilter == false) {
        if ((orc_Msg.u8_XTD ==
             this->mc_CanFilterConfig[u32_FilterCounter].u8_ExtendedId) &&
            (orc_Msg.u32_ID >=
             this->mc_CanFilterConfig[u32_FilterCounter].u32_StartId) &&
            (orc_Msg.u32_ID <=
             this->mc_CanFilterConfig[u32_FilterCounter].u32_EndId)) {
    ...
    if (q_Blocked == false) {
      if (q_PassFilterAvailable == true) {
        // Check only if the CAN message was not already blocked explicitly and
        // at least one pass filter was configured
        for (u32_FilterCounter = 0U;
             u32_FilterCounter < this->mc_CanFilterConfig.size();
             ++u32_FilterCounter) {
          if ((orc_Msg.u8_XTD ==
              ...
              (orc_Msg.u32_ID <=
               this->mc_CanFilterConfig[u32_FilterCounter].u32_EndId)) {
            // This pass filter matched. CAN message is not blocked and
            // relevant.
            q_Return = true;
            break;
          }
  ...
  if ((q_Return == false) && (this->mu32_FilteredMessages < 0xFFFFFFFFU)) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Returns the CAN message as protocol specific interpreted string with
   values as hex
   ...
   Empty string if no protocol is configured or the CAN message does not match
   to the configured protocol
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscComMessageLogger::m_GetProtocolStringHex(
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscComMessageLogger::m_GetProtocolStringDec(
    const T_STWCAN_Msg_RX &orc_Msg) const {
  QString c_Result = "";
...
/*! \brief  Inserts openSYDE system definition to parsing configuration
   ...
   Splitting of parts of function AddOsySysDef into this one because of
   performance. Loading of file needs a lot more time and shall not be part of
   the synchronized parts. Deriving classes can override this function to use
   synchronization.

   \param[in] orc_PathSystemDefinition Path of system definition file (Must be
   .syde_sysdef) \param[in] orc_OsySysDef            Loaded openSYDE system
   definition \param[in] ou32_BusIndex            Used CAN bus index
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::m_InsertOsySysDef(
    const QString &orc_PathSystemDefinition,
    const C_OscSystemDefinition &orc_OsySysDef, const uint32_t ou32_BusIndex) {
  const C_OscComMessageLoggerOsySysDefConfig c_SysDefConfig(orc_OsySysDef,
                                                            ou32_BusIndex);

  this->mc_OsySysDefs.insert(orc_PathSystemDefinition, c_SysDefConfig);
  // Register the database in the activation flag map
  this->mc_DatabaseActiveFlags.insert(orc_PathSystemDefinition, true);

  this->mc_ProtocolDec.AddOsySysDef(&(this->mc_OsySysDefs[orc_PathSystemDefinition]));
  this->mc_ProtocolHex.AddOsySysDef(&(this->mc_OsySysDefs[orc_PathSystemDefinition]));
}
...
/*! \brief  Checks if a matching CAN message is defined in at least one
   ...
   The result will be saved to mpc_OsySysDefMessage and
   mpc_OsySysDefDataPoolList. If mpc_OsySysDefMessage and
   mpc_OsySysDefDataPoolList is NULL, no CAN message found in any system
   definition.
...
bool C_OscComMessageLogger::m_CheckSysDef(const T_STWCAN_Msg_RX &orc_Msg) {
  ...
  QMap<QString, C_OscComMessageLoggerOsySysDefConfig>::const_iterator
      c_ItSysDef;

  this->mpc_OsySysDefMessage = NULL;
  this->mpc_OsySysDefDataPoolList = NULL;

  for (c_ItSysDef = this->mc_OsySysDefs.constBegin();
       c_ItSysDef != this->mc_OsySysDefs.constEnd(); ++c_ItSysDef) {
    // Check if the database is active
    if (this->mc_DatabaseActiveFlags[c_ItSysDef.key()] == true) {
      const C_OscSystemDefinition &rc_OsySysDef =
          c_ItSysDef.value().c_OsySysDef;
      uint32_t u32_NodeCounter;

      // Search all nodes which are connected to to the CAN bus
      for (u32_NodeCounter = 0U; u32_NodeCounter < rc_OsySysDef.c_Nodes.size();
           ++u32_NodeCounter) {
        ...
        for (u32_IntfCounter = 0U;
             u32_IntfCounter < rc_Node.c_Properties.c_ComInterfaces.size();
             ++u32_IntfCounter) {
          if ((rc_Node.c_Properties.c_ComInterfaces[u32_IntfCounter]
                   .GetBusConnected() == true) &&
              (rc_Node.c_Properties.c_ComInterfaces[u32_IntfCounter]
                   .u32_BusIndex == c_ItSysDef.value().u32_BusIndex)) {
            // Com Interface found
            q_IntfFound = true;
            break;
          }
        ...
        if (q_IntfFound == true) {
          ...
          for (u32_ProtCounter = 0U;
               u32_ProtCounter < rc_Node.c_ComProtocols.size();
               ++u32_ProtCounter) {
            const C_OscCanProtocol &rc_CanProt =
                rc_Node.c_ComProtocols[u32_ProtCounter];
            ...
            if (u32_IntfCounter < rc_CanProt.c_ComMessages.size()) {
              ...
              bool q_SearchAlsoInRxMessages =
                  true; // flag to improve performance
              uint32_t u32_CanMsgCounter;

              for (u32_CanMsgCounter = 0U;
                   u32_CanMsgCounter < rc_CanMsgContainerTx.size();
                   ++u32_CanMsgCounter) {
                const C_OscCanMessage &rc_OscMsg =
                    rc_CanMsgContainerTx[u32_CanMsgCounter];
                ...
                if ((orc_Msg.u32_ID == rc_OscMsg.u32_CanId) &&
                  ...
                  q_SearchAlsoInRxMessages =
                      false; // found message, no need to search in Rx messages
                             // anymore

                  // Get the associated list
                  Q_ASSERT(rc_CanProt.u32_DataPoolIndex <
                           rc_Node.c_DataPools.size());
                  if (rc_CanProt.u32_DataPoolIndex <
                      rc_Node.c_DataPools.size()) {
              ...
              // Only in case of CANopen protocol we want to visualize TXPDO
              // messages (sent from server to client, RX messages from client
              // view) and their signals in trace because this safes us a lot of
              // performance. Trigger of this feature is:
              // https://redmine.sensor-technik.de/issues/78633
              if ((rc_CanProt.e_Type == C_OscCanProtocol::eCAN_OPEN) &&
                  (q_SearchAlsoInRxMessages == true)) {
                ...
                for (u32_CanMsgCounterRx = 0U;
                     u32_CanMsgCounterRx < rc_CanMsgContainerRx.size();
                     ++u32_CanMsgCounterRx) {
                  const C_OscCanMessage &rc_OscMsg =
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComMessageLogger::m_InterpretSysDef(
  ...
  if ((this->mpc_OsySysDefMessage != NULL) &&
    ...
    for (u32_Counter = 0U;
         u32_Counter < this->mpc_OsySysDefMessage->c_Signals.size();
         ++u32_Counter) {
      const C_OscCanSignal &rc_OscSignal =
          this->mpc_OsySysDefMessage->c_Signals[u32_Counter];

      if (rc_OscSignal.e_MultiplexerType ==
    ...
    if (orc_MessageData.c_CanMsg.u8_DLC !=
        static_cast<uint8_t>(this->mpc_OsySysDefMessage->u16_Dlc)) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Returns the interpreted string of the found CAN message
   interpretation of an openSYDE system definition

   The result in mpc_OsySysDefMessage and mpc_OsySysDefDataPoolList will be
   used. All values are shown as hexadecimal value.

   \return
   Interpreted string if mpc_OsySysDefMessage and mpc_OsySysDefDataPoolList is
   not NULL Empty string if mpc_OsySysDefMessage or mpc_OsySysDefDataPoolList is
   NULL
*/
//----------------------------------------------------------------------------------------------------------------------
...
/*! \brief  Returns the interpreted string of the found CAN message
   ...
   The result in mpc_OsySysDefMessage and mpc_OsySysDefDataPoolList will be
   used. All values are shown as decimal value.
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Checks and interprets if a matching CAN message is defined in a
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComMessageLogger::m_CheckInterpretation(
    stw::opensyde_core::C_OscComMessageLoggerData &orc_MessageData) {
  // Nothing to do here.
  // Deriving class must override this function to use an own CAN message
...
/*! \brief  Interprets the received CAN signal value
   ...
   \param[in]         orc_OscValue      Datapool element content for type
   configuration \param[in]         of64_Factor       Factor for resulting value
   \param[in]         of64_Offset       Offset for resulting value
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::mh_InterpretCanSignalValue(
    C_OscComMessageLoggerDataSignal &orc_Signal,
    const uint8_t (&orau8_CanDb)[8], const uint8_t ou8_CanDlc,
    const C_OscCanSignal &orc_OscSignal,
    const C_OscNodeDataPoolContent &orc_OscValue, const float64_t of64_Factor,
    const float64_t of64_Offset) {
  const bool q_SignalFits =
      C_OscCanUtil::h_IsSignalInMessage(ou8_CanDlc, orc_OscSignal);
  ...
  if (q_SignalFits == true) {
    ...
    for (u8_RawDataCounter = 0U; u8_RawDataCounter < c_SignalRawData.size();
         ++u8_RawDataCounter) {
      const uint64_t u64_ShiftValue =
          static_cast<uint64_t>(u8_RawDataCounter) * 8U;
      u64_Value += static_cast<uint64_t>(c_SignalRawData[u8_RawDataCounter])
                   << u64_ShiftValue;
    }

    if ((orc_OscValue.GetType() == C_OscNodeDataPoolContent::eFLOAT32) ||
        (orc_OscValue.GetType() == C_OscNodeDataPoolContent::eFLOAT64)) {
      // In case of a float value, the raw value does not make any sense in byte
      // form. Use the value without scaling as raw value.
      QString c_StringValue;
      c_OscValue.GetValueAsScaledString(1.0, 0.0, c_StringValue, 0U, true,
                                        true);
      ...
      orc_Signal.c_RawValueHex =
          QString::number(static_cast<int64_t>(u64_Value), 16).toUpper();
    }

    // Interpreted value
    QString c_StringValue;
    c_OscValue.GetValueAsScaledString(of64_Factor, of64_Offset,
                                      c_StringValue, 0U, true, true);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Fills instance mc_HandledCanMessage of C_OscComMessageLoggerData
   dependent of the current message

   \param[in] orc_Msg Current CAN message
   \param[in] oq_IsTx Message was sent of this application itself
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::m_ConvertCanMessage(const T_STWCAN_Msg_RX &orc_Msg,
                                                const bool oq_IsTx) {
  ...
  this->mc_HandledCanMessage.c_CanIdHex =
      QString::number(orc_Msg.u32_ID, 16).toUpper();
  ...
  for (u8_DbCounter = 0U; u8_DbCounter < orc_Msg.u8_DLC; ++u8_DbCounter) {
    ...
    this->mc_HandledCanMessage.c_CanDataDec +=
        QString::number(orc_Msg.au8_Data[u8_DbCounter]);
    this->mc_HandledCanMessage.c_CanDataHex +=
        QString::number(orc_Msg.au8_Data[u8_DbCounter], 16)
            .rightJustified(2, '0')
            .toUpper();
  ...
  // It is possible that communication between a tool on the same CAN bus on the
  // virtual side and a real hardware communication partner cause a problem in
  // the CAN message order. The internal send CAN messages can be seen by other
  // tools on the same internal network earlier than the other CAN bus nodes.
  // The timestamp is set by the CAN hardware part (in case of PEAK). The
  // problem: The timestamp can be later than the timestamp of some CAN messages
  // received from CAN bus nodes, but are in the receive queue before the CAN
  // bus nodes. So we must prevent possible negative values.

  // Absolute
  if (orc_Msg.u64_TimeStamp > this->mu64_FirstTimeStampStart) {
    this->mc_HandledCanMessage.u64_TimeStampAbsoluteStart =
        orc_Msg.u64_TimeStamp - this->mu64_FirstTimeStampStart;
    // Calculating the us by starting at midnight to get the timestamp based on
    // time of day Days will not be counted and shown and will be dropped
    this->mc_HandledCanMessage.u64_TimeStampAbsoluteTimeOfDay =
        (mu64_FirstTimeStampDayOfTime +
         this->mc_HandledCanMessage.u64_TimeStampAbsoluteStart) %
  ...
  if (orc_Msg.u64_TimeStamp > this->mu64_LastTimeStamp) {
    this->mc_HandledCanMessage.u64_TimeStampRelative =
        orc_Msg.u64_TimeStamp - this->mu64_LastTimeStamp;
  } else {
    this->mc_HandledCanMessage.u64_TimeStampRelative = 0ULL;
  }

  this->mc_HandledCanMessage.c_TimeStampAbsoluteStart =
      C_OscComMessageLoggerData::h_GetTimestampAsString(
          this->mc_HandledCanMessage.u64_TimeStampAbsoluteStart);
  ...
  if (orc_Msg.u8_XTD == 0U) {
    if (orc_Msg.u32_ID <= 0x7FF) {
      ...
      this->mc_MsgCounterStandardId[orc_Msg.u32_ID] =
          this->mc_MsgCounterStandardId[orc_Msg.u32_ID] + 1U;
      this->mc_HandledCanMessage.c_Counter =
          QString::number(this->mc_MsgCounterStandardId[orc_Msg.u32_ID]);
    } else {
      // Should not happen
    ...
    if (c_ItCounter == this->mc_MsgCounterExtendedId.end()) {
      ...
      this->mc_HandledCanMessage.c_Counter =
          QString::number(c_ItCounter.value());
    }
  }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::m_InterpretSysDefCanSignal(
    C_OscComMessageLoggerData &orc_MessageData,
    const C_OscCanSignal &orc_OscSignal) const {
  if ((this->mpc_OsySysDefDataPoolList != NULL) &&
      (orc_OscSignal.u32_ComDataElementIndex <
       this->mpc_OsySysDefDataPoolList->c_Elements.size())) {
    C_OscComMessageLoggerDataSignal c_Signal;
    const C_OscNodeDataPoolListElement &rc_OscElement =
        this->mpc_OsySysDefDataPoolList
            ->c_Elements[orc_OscSignal.u32_ComDataElementIndex];
    ...
    mh_InterpretCanSignalValue(c_Signal, orc_MessageData.c_CanMsg.au8_Data,
                               orc_MessageData.c_CanMsg.u8_DLC, orc_OscSignal,
                               rc_OscElement.c_Value, rc_OscElement.f64_Factor,
...
void C_OscComMessageLogger::m_ResetCounter(void) {
  ...
  std::fill(this->mc_MsgCounterStandardId.begin(),
            this->mc_MsgCounterStandardId.end(), 0U);
...
/*! \brief  Post process a loaded openSYDE system definition.
   ...
   More precisely, manipulate all messages of protocol ECeS by adding two
   signals for message counter and checksum.

   \param[in,out]  orc_SystemDefinition   System definition
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::mh_PostProcessSysDef(
  ...
  // for all nodes
  for (u32_ItNodes = 0; u32_ItNodes < orc_SystemDefinition.c_Nodes.size();
       u32_ItNodes++) {
    C_OscNode &rc_Node = orc_SystemDefinition.c_Nodes[u32_ItNodes];
    c_Id.u32_NodeIndex = u32_ItNodes;

    // for all interfaces of this node
    for (u32_ItInterfaces = 0;
         u32_ItInterfaces < rc_Node.c_Properties.c_ComInterfaces.size();
         u32_ItInterfaces++) {
      c_Id.u32_InterfaceIndex = u32_ItInterfaces;

      // for all protocols of this node
      for (u32_ItProtocols = 0; u32_ItProtocols < rc_Node.c_ComProtocols.size();
           u32_ItProtocols++) {
        const C_OscCanProtocol &rc_Protocol =
            rc_Node.c_ComProtocols[u32_ItProtocols];
        ...
        if (rc_Protocol.e_Type == C_OscCanProtocol::eECES) {
          ...
          if (u32_ItInterfaces < rc_Protocol.c_ComMessages.size()) {
            const C_OscCanMessageContainer &rc_Container =
                rc_Protocol.c_ComMessages[u32_ItInterfaces];

            // Tx messages
            for (u32_ItMessages = 0;
                 u32_ItMessages < rc_Container.c_TxMessages.size();
                 u32_ItMessages++) {
              const C_OscCanMessage &rc_Message =
                  rc_Container.c_TxMessages[u32_ItMessages];
              c_Id.q_MessageIsTx = true;
              c_Id.u32_MessageIndex = u32_ItMessages;
              mh_AddSpecialEcesSignals(
                  rc_Node, c_Id,
                  static_cast<uint32_t>(rc_Message.c_Signals.size()));
            }
            // Rx messages
            for (u32_ItMessages = 0;
                 u32_ItMessages < rc_Container.c_RxMessages.size();
                 u32_ItMessages++) {
              const C_OscCanMessage &rc_Message =
                  rc_Container.c_RxMessages[u32_ItMessages];
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Add two signals incl. their Datapool elements for message counter
   and checksum to a message.
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::mh_AddSpecialEcesSignals(
    C_OscNode &orc_Node, const C_OscCanMessageIdentificationIndices &orc_Id,
  ...
  c_MessageCounterElement.c_Comment =
      "Automatically generated signal to represent message counter signal of "
      "the ECeS Message.";
  ...
  c_ChecksumElement.c_Comment = "Automatically generated signal to represent "
                                "CRC checksum signal of the ECeS Message.";

  // insert signals
  int32_t s32_Result;
  s32_Result = orc_Node.InsertSignal(
      orc_Id.e_ComProtocol, orc_Id.u32_InterfaceIndex, orc_Id.u32_DatapoolIndex,
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check if received message has protocol ECeS and process it
   accordingly

      Special signals ("ECeS_Message_Counter" and "ECeS_Checksum") indicate that
   it is an ECeS message Validate both their values and update message status
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscComMessageLogger::m_CheckAndHandleEcesMessage() {
  ...
  for (uint32_t u32_Counter = 0U;
       u32_Counter < this->mc_HandledCanMessage.c_Signals.size();
       ++u32_Counter) {
    C_OscComMessageLoggerDataSignal &rc_Signal =
        this->mc_HandledCanMessage.c_Signals[u32_Counter];

    // Look for special signal "ECeS_Message_Counter" in the message received
    if (rc_Signal.c_Name.compare(mhc_ECES_MESSAGE_COUNTER,
                                 Qt::CaseInsensitive) == 0) {
      // Search for the unique ECeS message (based on CAN Id) in the saved ECeS
      // messages
      const QHash<uint32_t, QString>::iterator c_Iterator =
          this->mc_EcesMessages.find(this->mpc_OsySysDefMessage->u32_CanId);
      ...
      if (c_Iterator != this->mc_EcesMessages.end()) {
        // compare the message counter values (ideally the counter of the
        // received message should be +1)
        int32_t s32_CounterDiff =
            rc_Signal.c_Value.toInt() - c_Iterator.value().toInt();

        // When max value of counter is reached, ensure that the increment is
        // valid E.g if max value = 255, next counter value = 0. Hence
        // s32_CounterDiff = -255. In order that the counting continues further,
        // set the difference to 1
        if (std::abs(s32_CounterDiff) ==
            static_cast<int32_t>(mhu32_ECES_MAX_MESSAGE_COUNTER)) {
          s32_CounterDiff = 1;
        }
        ...
        this->mc_HandledCanMessage.c_Status +=
            "Diff: " + c_DifferenceValue +
            "; Prev. Counter = " + c_Iterator.value() + ". ";

        // remove the old message
        this->mc_EcesMessages.remove(this->mpc_OsySysDefMessage->u32_CanId);
      }
    ...
    else if (rc_Signal.c_Name.compare(mhc_ECES_CHECKSUM, Qt::CaseInsensitive) ==
             0) {
      const uint8_t u8_OrigCrcValue =
          static_cast<uint8_t>(this->mc_HandledCanMessage.c_CanMsg.au8_Data[7]);
      const uint8_t u8_CalculatedCrcValue =
          C_OscComAutoSupport::h_GetCyclicRedundancyCheckCalculation(
              6, this->mc_HandledCanMessage.c_CanMsg.au8_Data);
      // Validate checksum
...
void C_OscComMessageLogger::m_SaveEcosMessage() {
  ...
  c_MessageData.append(reinterpret_cast<const char *>(&rc_Msg.au8_Data[0]),
                       rc_Msg.u8_DLC);
...
/*! \brief  Checks if the received message has protocol ECoS
   ...
   Searches in all loaded databases and finds if received message has protocol
   ECoS
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscComMessageLogger::m_CheckIfEcosMessage(
    const stw::can::T_STWCAN_Msg_RX &orc_Msg) {
  bool q_Return = false;
...
void C_OscComMessageLogger::m_HandleEcosInvertedMessage() {
  ...
  if (this->mc_EcosMessage.c_MessageData.size() > 0) {
    ...
    const uint8_t u8_DataSize =
        static_cast<uint8_t>(this->mc_EcosMessage.c_MessageData.size());

    // Compare CAN ID and DLC (data length code) of received messsage to that of
    // last received ECoS message.
    if (((this->mc_HandledCanMessage.c_CanMsg.u32_ID - 1) ==
         (this->mc_EcosMessage.u32_CanId)) &&
        (this->mc_HandledCanMessage.c_CanMsg.u8_DLC == u8_DataSize)) {
      QByteArray c_InvertedData;

      // Set message name
      this->mc_HandledCanMessage.c_Name =
          (this->mc_EcosMessage.c_MessageName) + " (Inverted Frame)";

      // Invert message data of the received CAN message
      C_OscComAutoSupport::h_InvertCanMessage(
          this->mc_HandledCanMessage.c_CanMsg.au8_Data, au8_InvertedData);

      c_InvertedData.append(
          reinterpret_cast<const char *>(&au8_InvertedData[0]),
