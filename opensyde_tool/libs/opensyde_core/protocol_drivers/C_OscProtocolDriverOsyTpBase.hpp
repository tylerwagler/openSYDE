...
/*!
   ...
   Abstract base class defining function for packing, unpacking, sending,
   receiving services. All elements and functionality common to all transport
   protocols are contained here. Elements and functionality that is specific to
   the transport protocol needs to be implemented there.
...
public:
  C_OscProtocolDriverOsyService() : q_CanTransferWithoutFlowControl(false) {}

  QByteArray c_Data;
  bool q_CanTransferWithoutFlowControl; // Flag if service shall use openSYDE
...
public:
  ...
  static const uint8_t mhu8_MAX_NODE =
      0x7FU; ///< Maximum count of nodes is mhu8_MAX_NODE - 1
  ...
  C_OscProtocolDriverOsyNode(const uint8_t ou8_BusIdentifier,
...
protected:
  ...
  C_OscProtocolDriverOsyNode
      mc_ServerId;                  ///< ID of server node to communicate with
  uint32_t mu32_BroadcastTimeoutMs; ///< timeout for collecting responses to
                                    ///< broadcasts
...
public:
  ...
  explicit C_OscProtocolDriverOsyTpBase(
  ...
  */
  ...
  virtual void
  GetNodeIdentifiers(C_OscProtocolDriverOsyNode &orc_ClientIdentifier,
