...
uint16_t
C_OscDiagProtocolKfx::mh_PackDataPoolIndex(const uint16_t ou16_List,
                                           const uint16_t ou16_Variable) {
  return ((static_cast<uint16_t>(ou16_List << 10U)) |
          (ou16_Variable & 0x03FFU));
}

//----------------------------------------------------------------------------------------------------------------------

void C_OscDiagProtocolKfx::mh_UnpackDataPoolIndex(const uint16_t ou16_Index,
                                                  uint16_t &oru16_List,
...
/*! \brief   Handling of ECRR/TCRR reception
   ...
   C_OscDiagProtocolKfx that we installed in constructor \param[in] ou32_Index
   packed variable index \param[in]     os64_Value           reported variable
   value (if oq_Error is true this contains the error code) \param[in]
   ou32_TimeStamp       timestamp of reported value \param[in] oq_IsTimeStamped
   true: response contained timestamp \param[in]     oq_Error             true:
   error response; false: data response
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDiagProtocolKfx::mh_CyclicResponseReceived(
    void *const opv_Instance, const uint32_t ou32_Index,
  ...
  C_OscDiagProtocolKfx *const pc_This =
      reinterpret_cast<C_OscDiagProtocolKfx *>(opv_Instance);

  Q_ASSERT(pc_This != NULL);
  if (pc_This != NULL) {
    pc_This->m_CyclicResponseReceived(ou32_Index, os64_Value, ou32_TimeStamp,
                                      oq_IsTimeStamped, oq_Error);
  }
}
...
/*! \brief   Handling an ECRR/TCRR reception
   ...
   \param[in]     os64_Value           reported variable value (if oq_Error is
   true this contains the error code) \param[in]     ou32_TimeStamp timestamp of
   reported value (ignored by us ...) \param[in]     oq_IsTimeStamped     true:
   response contained timestamp (ignored by us ...) \param[in]     oq_Error
   true: error response; false: data response
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDiagProtocolKfx::m_CyclicResponseReceived(
    const uint32_t ou32_Index, const int64_t os64_Value,
  ...
  Q_ASSERT(ou32_Index <=
           0xFFFF); // this should be true for any KEFEX transmission

  mh_UnpackDataPoolIndex(static_cast<uint16_t>(ou32_Index), u16_List,
                         u16_Variable);

  if (oq_Error == true) {
    if (mpr_OnDataPoolReadEventErrorReceived != NULL) {
      mpr_OnDataPoolReadEventErrorReceived(mpv_OnAsyncInstance, 0U, u16_List,
                                           u16_Variable,
                                           static_cast<uint8_t>(os64_Value));
    } else {
      osc_write_log_warning("Asynchronous communication",
                            "C_OscDiagProtocolKfx: Cyclic error response "
                            "received but no handler installed !");
    }
  } else {
    if (mpr_OnDataPoolReadEventReceived != NULL) {
      // convert value; we don't know the size of the element; we know the
      // maximum is 32bits so always return those
      ...
      mpr_OnDataPoolReadEventReceived(mpv_OnAsyncInstance, 0U, u16_List,
                                      u16_Variable, c_Value);
    } else {
      osc_write_log_warning("Asynchronous communication",
                            "C_OscDiagProtocolKfx: Cyclic data response "
                            "received but no handler installed !");
    }
  }
}
...
C_OscDiagProtocolKfx::C_OscDiagProtocolKfx(void)
    : C_OscDiagProtocolBase(), mq_HasDispatcher(false),
      mq_NvmValidFlagIsUsed(false), mu16_CommTimeoutMs(1000) {
  ...
  mpc_CommKefex->InstallCyclicTransmissionCallback(&mh_CyclicResponseReceived,
                                                   this);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDiagProtocolKfx::SetDispatcher(
    stw::can::C_CanDispatcher *const opc_Dispatcher) {
  stw::diag_lib::C_KFXCommConfiguration c_Config;
  this->mpc_CommKefex->SetCommDispatcher(opc_Dispatcher);
  // the protocol driver does not automatically apply the Rx filters when
  // setting a new dispatcher
  //-> trigger manually by "re-setting" parameters
  if (opc_Dispatcher == NULL) {
    (void)this->mpc_CommKefex->GetConfig(
        &c_Config); // can only fail if pointer is invalid; ignore result
    (void)this->mpc_CommKefex->SetConfig(
        &c_Config); // assume the read data is valid ...
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDiagProtocolKfx::SetCommunicationParameters(
    const stw::diag_lib::C_KFXCommConfiguration &orc_Config) {
  // returned errors are only checking for valid pointer
  (void)this->mpc_CommKefex->SetConfig(&orc_Config);

  // there are a few services we need to poll ourselves; so remember the timeout
  // here as well
  mu16_CommTimeoutMs = orc_Config.GetTimeout();
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Configure whether the NvmWrite Start and Finalize services are
   required

   Configure whether the NvmWriteStart and NvmWriteFinalize services need to be
   executed for the target. If set to "true" the service functions will try to
   execute the services. If set to "false" the service functions will simply
   return C_NO_ERR.

   This setting is required as those two services have been added to the
   protocol after its initial creation and are only supported on newer target
   driver's.
...
int32_t C_OscDiagProtocolKfx::Cycle(void) {
  ...
  if (mq_HasDispatcher == true) {
    this->mpc_CommKefex->Cycle(); // responses will be reported through
...
/*! \brief   Read numeric data from server's data pool
   ...
   This function is used for plain numeric data pool elements (i.e. non-array
   elements).
   ...
   \param[in,out] orc_ReadData     in: size must match expected number of bytes
                                   out: read data (in protocol-specific
   endianness) \param[out]    opu8_NrCode      if != NULL: negative response
   ...
   C_TIMEOUT  expected response not received within timeout (for elements <= 4
   bytes) C_NOACT    could not send request (Tx error or no CAN dispatcher
   installed) expected response not received within timeout (for elements with 8
   bytes size) C_CONFIG   segmented transfer required but BSmax set to zero;
   communication already in use by another thread CAN dispatcher not installed
   C_WARN     error response
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolReadNumeric(
  ...
  if (mq_HasDispatcher == false) {
    s32_Return = C_CONFIG; // not all protocol driver functions check for
                           // installed dispatcher ...
  } else {
    if ((ou8_DataPoolIndex == 0U) &&
        (ou16_ListIndex < mhu16_KFX_RAM_MAX_NUM_LISTS) &&
        (ou16_ElementIndex < mhu16_KFX_RAM_MAX_NUM_ELEMENTS) &&
        ((orc_ReadData.size() == 1U) || (orc_ReadData.size() == 2U) ||
         (orc_ReadData.size() == 4U) || (orc_ReadData.size() == 8U))) {
      const uint16_t u16_Index =
          mh_PackDataPoolIndex(ou16_ListIndex, ou16_ElementIndex);
      int64_t s64_Value;

      s32_Return = this->mpc_CommKefex->ReadNumericVariable(
          u16_Index, static_cast<uint8_t>(orc_ReadData.size()), s64_Value);
      ...
      case C_NO_ERR:
        ...
        case 2:
          orc_ReadData[0] = static_cast<uint8_t>(s64_Value);
          orc_ReadData[1] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 8U);
          break;
        case 4:
          orc_ReadData[0] = static_cast<uint8_t>(s64_Value);
          orc_ReadData[1] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 8U);
          orc_ReadData[2] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 16U);
          orc_ReadData[3] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 24U);
          break;
        case 8:
          orc_ReadData[0] = static_cast<uint8_t>(s64_Value);
          orc_ReadData[1] =
          ...
          orc_ReadData[3] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 24U);
          orc_ReadData[4] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 32U);
          orc_ReadData[5] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 40U);
          orc_ReadData[6] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 48U);
          orc_ReadData[7] =
              static_cast<uint8_t>(static_cast<uint64_t>(s64_Value) >> 56U);
          break;
        default:
          break; // checked at beginning of function
        }
        break;
      case C_RD_WR: // could not send request; no dispatcher installed; wrong
                    // size in response;
                    //  timeout of segmented transfer (no way to differentiate)
        s32_Return = C_NOACT;
        break;
      case C_COM: // no response of expedited transfer; transport protocol
                  // violation;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolReadArray(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, QByteArray &orc_ReadData,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return = C_RANGE;
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) &&
      ...
      s32_Return = this->mpc_CommKefex->ReadAggregateVariable(
          u16_Index, static_cast<uint32_t>(orc_ReadData.size()),
      ...
      case C_RD_WR: // could not send request; no dispatcher installed; wrong
        ...
        s32_Return = C_NOACT;
        break;
      case C_COM: // transport protocol violation; wrong index in response (no
...
/*! \brief   Write numeric data to server's data pool
   ...
   \param[out] orc_DataToWrite     data to write (in protocol-specific
   endianness) \param[out] opu8_NrCode         if != NULL: negative response
   code in case of an error response
   ...
   C_RANGE    parameter out of range (checked by client-side function):
              ...
              - expected size not 1,2,4,8 (valid sizes of numeric elements)
   C_TIMEOUT  expected response not received within timeout (for elements <= 4
   bytes) C_NOACT    could not send request (Tx error) expected response not
   received within timeout (for elements with 8 bytes size) C_CONFIG
   pre-requisites not correct; e.g. driver not initialized CAN dispatcher not
   installed C_WARN     error response C_RD_WR    malformed protocol response
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolWriteNumeric(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) &&
        (ou16_ListIndex < mhu16_KFX_RAM_MAX_NUM_LISTS) &&
        (ou16_ElementIndex < mhu16_KFX_RAM_MAX_NUM_ELEMENTS) &&
        ((orc_DataToWrite.size() == 1U) || (orc_DataToWrite.size() == 2U) ||
         (orc_DataToWrite.size() == 4U) || (orc_DataToWrite.size() == 8U))) {
      const uint16_t u16_Index =
          mh_PackDataPoolIndex(ou16_ListIndex, ou16_ElementIndex);
      uint64_t u64_Value;

      // sigh... the KEFEX protocol takes a sint64 parameter; convert back to
      // numeric value :-)
      ...
      case 1:
        ...
        break;
      case 2:
        u64_Value = static_cast<uint64_t>(
            orc_DataToWrite[0] +
      ...
      s32_Return = this->mpc_CommKefex->WriteNumericVariable(
          u16_Index, static_cast<uint8_t>(orc_DataToWrite.size()),
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolWriteArray(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const QByteArray &orc_DataToWrite,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return = C_RANGE;
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) &&
        (ou16_ListIndex < mhu16_KFX_RAM_MAX_NUM_LISTS) &&
        (ou16_ElementIndex < mhu16_KFX_RAM_MAX_NUM_ELEMENTS) &&
        ((orc_DataToWrite.size() > 0U) &&
         (orc_DataToWrite.size() <= 0xFFFFFFU))) {
      const uint16_t u16_Index =
          mh_PackDataPoolIndex(ou16_ListIndex, ou16_ElementIndex);

      s32_Return = this->mpc_CommKefex->WriteAggregateVariable(
          u16_Index, static_cast<uint32_t>(orc_DataToWrite.size()),
...
/*! \brief   Set rate of event driven transmissions
   ...
   In this implementation we remember the configured interval values to use in
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDiagProtocolKfx::DataPoolSetEventDataRate(const uint8_t ou8_Rail,
                                               const uint16_t ou16_IntervalMs) {
...
/*! \brief   Request cyclic driven transmission of data pool element.
   ...
   Base spec says: Shall send the request and wait for the initial confirmation
   (or error) response. However we do not get an initial confirmation in the
   KEFEX protocol. Therefore all data and error responses are handled
   asynchronously using the Cycle() function and be reported via calls to
   installed function pointer. So we only send the request here.

   \param[in]  ou8_DataPoolIndex   index of data pool to read from
   \param[in]  ou16_ListIndex      list index to read from
   \param[in]  ou16_ElementIndex   element index to read
   \param[in]  ou8_Rail            0..2   "rail" referring to time interval to
   use \param[out] opu8_NrCode         if != NULL: negative response code in
   case of an error response

   \return
   C_NO_ERR   request sent
   C_RANGE    ou8_DataPoolIndex is zero; list index > 63; element index > 1023;
   rail > 2 C_NOACT    could not send request (e.g. Tx buffer full) C_CONFIG CAN
   dispatcher not installed
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolReadCyclic(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) && (ou16_ListIndex < 64U) &&
        (ou16_ElementIndex < 1024U) && (ou8_Rail < 3U)) {
      const uint16_t u16_Index =
          mh_PackDataPoolIndex(ou16_ListIndex, ou16_ElementIndex);
      s32_Return = this->mpc_CommKefex->RequestTimeTriggeredTransmission(
          u16_Index, false, mau16_EventRailIntervalsMs[ou8_Rail]);
...
/*! \brief   Request change driven transmission of data pool element.
   ...
   \param[in]  ou16_ElementIndex   element index to read
   \param[in]  ou8_Rail            0..2   "rail" referring to time interval to
   use for maximum timeout \param[in]  ou32_Threshold      threshold the value
   must cross before getting sent again \param[out] opu8_NrCode         if !=
   NULL: negative response code in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolReadChangeDriven(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_Rail,
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) && (ou16_ListIndex < 64U) &&
      ...
      const uint16_t u16_Index =
          mh_PackDataPoolIndex(ou16_ListIndex, ou16_ElementIndex);
      s32_Return = this->mpc_CommKefex->RequestChangeTriggeredTransmission(
          u16_Index, mau16_EventRailIntervalsMs[ou8_Rail], ou32_Threshold,
          ou32_Threshold);
      if (s32_Return != C_NO_ERR) {
        // could not send request
        s32_Return = C_NOACT;
...
int32_t C_OscDiagProtocolKfx::DataPoolStopEventDriven(void) {
  ...
  if (mq_HasDispatcher == false) {
    ...
    s32_Return = this->mpc_CommKefex->TerminateAllCyclicTransmissions(
        true); // wait for handshake ...
    // map KEFEX to openSYDE error codes as good as possible:
    switch (s32_Return) {
    case C_NO_ERR:
      break;
...
/*! \brief   Request cyclic driven transmission of NVM content.
   ...
   byte) \param[in,out] orc_DataRecord       in: size defines number of bytes to
   read out: Read data bytes \param[out]    opu8_NrCode          if != NULL:
   negative response code in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::NvmRead(const uint32_t ou32_MemoryAddress,
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou32_MemoryAddress <= 0xFFFFFFU) && (orc_DataRecord.size() > 0)) {
      s32_Return = this->mpc_CommKefex->ReadEEPROM(
          ou32_MemoryAddress, static_cast<uint32_t>(orc_DataRecord.size()),
          reinterpret_cast<uint8_t *>(orc_DataRecord.data()));
      // map KEFEX to openSYDE error codes as good as possible:
      switch (s32_Return) {
      case C_NO_ERR:
        break;
...
/*! \brief   Announce writing data to NVM
   ...
   In this protocol the corresponding services is not available/required on all
   targets. So the function checks a configurable flag and returns "OK" if the
   service is marked as not available.

   \param[in]  ou8_DataPoolIndex    Accessed data pool index
   \param[in]  ou16_NvmAccessCount  Expected number of subsequent executions of
   service to write to NVM
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::NvmWriteStartTransaction(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_NvmAccessCount) {
  int32_t s32_Return = C_NO_ERR;
  ...
  if (mq_NvmValidFlagIsUsed != false) {
    ...
    if (mq_HasDispatcher == false) {
      ...
      if ((ou8_DataPoolIndex == 0U) && (ou16_NvmAccessCount > 0U)) {
        s32_Return = this->mpc_CommKefex->WriteEEPROMSSLStart(
            static_cast<uint32_t>(ou16_NvmAccessCount) * 2U);
        // map KEFEX to openSYDE error codes as good as possible:
        switch (s32_Return) {
        case C_NO_ERR:
          break;
...
/*! \brief   Cyclic driven request for NVM write operation
   ...
   Usage:
   * First call NvmWriteStartTransaction. (Here you can register any non zero
   amount of NVM write operations)
   ...
   * Afterwards call NvmWriteFinalizeTransaction to complete this transaction.
   ...
   \param[in]  ou32_MemoryAddress NVM memory address to write to (first written
   byte) \param[in]  orc_DataRecord     Data bytes to write \param[out]
   opu8_NrCode        if != NULL: negative response code in case of an error
   response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::NvmWrite(const uint32_t ou32_MemoryAddress,
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou32_MemoryAddress <= 0xFFFFFFU) && (orc_DataRecord.size() > 0)) {
      s32_Return = this->mpc_CommKefex->WriteEEPROM(
          ou32_MemoryAddress, static_cast<uint32_t>(orc_DataRecord.size()),
...
/*! \brief   Read data pool version
   ...
   \param[out] orau8_Version       Read version; format: see function
   description \param[out] opu8_NrCode         if != NULL: negative response
   code in case of an error response
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDiagProtocolKfx::DataPoolReadVersion(const uint8_t ou8_DataPoolIndex,
  ...
  if (mq_HasDispatcher == false) {
    ...
    if (ou8_DataPoolIndex == 0U) {
      uint32_t u32_Result;
      s32_Return = this->mpc_CommKefex->ReadService(
          hu16_KFX_SERVICE_PROJECT_VERSION, u32_Result);
      switch (s32_Return) {
      case C_NO_ERR:
...
/*! \brief   Read Datapool meta data
   ...
   \param[out] orau8_Version       Read version; format: see function
   description \param[out] orc_Name            Empty string. Is not supported
   \param[out] opu8_NrCode         if != NULL: negative response code in case of
   an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolReadMetaData(
    const uint8_t ou8_DataPoolIndex, uint8_t (&orau8_Version)[3],
    QString &orc_Name, uint8_t *const opu8_NrCode) {
  // KEFEX protocol does not support reading the Datapool name
  orc_Name = "";

  return this->DataPoolReadVersion(ou8_DataPoolIndex, orau8_Version,
                                   opu8_NrCode);
}
...
/*! \brief   Notify NVM data changes
   ...
   server false: negative acknowledge from server \param[out] opu8_NrCode if !=
   NULL: negative response code in case of an error response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::NvmNotifyOfChanges(
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) &&
        (ou8_ListIndex < mhu16_KFX_RAM_MAX_NUM_LISTS)) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolKfx::DataPoolVerify(
  ...
  if (mq_HasDispatcher == false) {
    ...
    if ((ou8_DataPoolIndex == 0U) && (ou32_DataPoolChecksum <= 0xFFFFU)) {
      s32_Return = this->mpc_CommKefex->Logon(
          static_cast<uint64_t>(ou32_DataPoolChecksum), ou16_DataPoolVersion,
...
/*! \brief   Perform a logoff
   ...
   \param[in]  oq_WaitForHandshake     true  -> will actively wait for error or
   OK handshake response false -> will not wait for handshake response
