...
public:
  ...
  int32_t UpdateNode(const QString &orc_HexFilePath,
                     const uint32_t ou32_RequestDownloadTimeout,
                     const uint32_t ou32_TransferDataTimeout);
  int32_t ResetSystem(void);

  static int32_t h_ReadHexFile(const QString &orc_HexFilePath,
...
protected:
  virtual void
  m_ReportProgressPercentage(const uint8_t ou8_ProgressInPercentage);
  virtual void m_ReportProgress(const int32_t os32_Result,
                                const QString &orc_Information);
  virtual void m_ReportFlashloaderInformationRead(
