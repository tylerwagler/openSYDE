...
/*!
   ...
   Provides functions to perform a very basic flash sequence: one HEX file for
   one openSYDE device via CAN
...
C_OscBuSequences::C_OscBuSequences(void)
    : mpc_CanDispatcher(NULL), ms32_CanBitrate(125) {}
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscBuSequences::Init(stw::can::C_CanDispatcher *const opc_CanDispatcher,
  ...
  m_ReportProgress(
      s32_Return, "Starting the initialization of CAN driver and protocol ...");
  ...
  if (this->mpc_CanDispatcher == NULL) {
    ...
    osc_write_log_error(c_LogActivity,
                        "Could not used CAN! CAN Dispatcher is invalid.");
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(
          c_LogActivity,
          "Setting CAN dispatcher for CAN transport protocol failed!");
    } else {
      s32_Return = mc_OsyProtocol.SetTransportProtocol(&mc_TpCan);
      if (s32_Return != C_NO_ERR) {
        osc_write_log_error(c_LogActivity,
                            "Setting CAN transport protocol to the openSYDE "
                            "protocol driver failed!");
      }
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(c_LogActivity, "Could not configure the node IDs! Is "
                                         "the server node ID within range?");
    }
  }
...
/*! \brief  Activate flashloader of device.
   ...
   Use the request programming flag, but if it does not work go on to allow a
   manual reset of the device. Enter preprogramming session via broadcast for at
   least the value of u32_SCAN_TIME_MS.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscBuSequences::ActivateFlashLoader(
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    c_Text = QString::asprintf(
        "Could not set the \"request programming\" flag: Failed with result "
        "%d. "
        "You still have the chance to reset the device manually ...",
        s32_Return);
    m_ReportProgress(C_WARN, c_Text);
  } else {
    osc_write_log_info(c_LogActivity,
                       "Succesfully set the \"request programming\" flag.");
  }
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    c_Text = QString::asprintf(
        "You now have %u seconds time to turn on your target device ...",
  ...
  // Always continue with broadcast. If previous steps did not work, user can do
  // the manual reset while we broadcast.
  ...
  do {
    ...
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(c_LogActivity,
                          "Sending broadcast to enter preprogramming session "
                          "failed with result " +
  ...
  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        c_LogActivity,
        "Could not connect to the target device! Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  }

  if (s32_Return == C_NO_ERR) {
    osc_write_log_info(
        c_LogActivity,
        "Connection to target device established. Flashloader activated.");
  }
...
int32_t C_OscBuSequences::ReadDeviceInformation(void) {
  ...
  s32_Return = mc_OsyProtocol.OsyReadHardwareNumber(c_Info.u32_EcuArticleNumber,
                                                    &u8_NumberCode);

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Article Number",
        "Could not read the device's article number! Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
    s32_Return = mc_OsyProtocol.OsyReadHardwareVersionNumber(
        c_Info.c_EcuHardwareVersionNumber, &u8_NumberCode);
  }

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Hardware Version",
        "Could not read the device's hardware version number! Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
  ...
  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Device Name",
        "Could not read the device's device name! Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
    s32_Return = mc_OsyProtocol.OsyReadProtocolVersion(
        c_Info.au8_ProtocolVersion, &u8_NumberCode);
  }

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Protocol Version",
        "Could not read the device's protocol version! Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
    s32_Return = mc_OsyProtocol.OsyReadBootSoftwareIdentification(
        c_Info.au8_FlashloaderSoftwareVersion, &u8_NumberCode);
  }

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Flashloader Versions",
        "Could not read the device's flashloader implementation version! "
        "Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
    s32_Return = mc_OsyProtocol.OsyReadFlashloaderProtocolVersion(
        c_Info.au8_FlashloaderProtocolVersion, &u8_NumberCode);
  }
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    s32_Return =
        mc_OsyProtocol.OsyReadFlashCount(c_Info.u32_FlashCount, &u8_NumberCode);
  }

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Flash Count",
        "Could not read number of times the device has been flashed! "
        "Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
    s32_Return = mc_OsyProtocol.OsyReadApplicationSoftwareFingerprint(
        c_Info.au8_FlashFingerprintDate, c_Info.au8_FlashFingerprintTime,
        c_Info.c_FlashFingerprintUserName, &u8_NumberCode);
  }

  if (s32_Return != C_NO_ERR) {
    osc_write_log_error(
        "Read Fingerprint",
        "Could not read the device's application software fingerprint! "
        "Details: " +
            C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                s32_Return, u8_NumberCode));
  } else {
    s32_Return = mc_OsyProtocol.OsyReadListOfFeatures(
        c_Info.c_AvailableFeatures, &u8_NumberCode);
  }
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    if (c_Info.c_AvailableFeatures.q_MaxNumberOfBlockLengthAvailable == true) {
      s32_Return = mc_OsyProtocol.OsyReadMaxNumberOfBlockLength(
          c_Info.u16_MaxNumberOfBlockLength, &u8_NumberCode);
    }
    if (s32_Return != C_NO_ERR) {
      c_Info.u16_MaxNumberOfBlockLength = 0U;
      osc_write_log_error(
          "Read MaxNumberOfBlockLength",
          "Could not read the device's maximum number of block length! "
          "Details: " +
              C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                  s32_Return, u8_NumberCode));
    }
  ...
  if (s32_Return == C_NO_ERR) {
    if (c_Info.c_AvailableFeatures.q_ExtendedSerialNumberModeImplemented ==
        false) {
      s32_Return = mc_OsyProtocol.OsyReadEcuSerialNumber(c_Info.c_SerialNumber,
                                                         &u8_NumberCode);
    } else {
      s32_Return = mc_OsyProtocol.OsyReadEcuSerialNumberExt(
          c_Info.c_SerialNumber, &u8_NumberCode);
    }
...
/*! \brief  Update node
   ...
   \param[in]  ou32_RequestDownloadTimeout   Maximum time in ms it can take to
   erase one continuous area in flash \param[in]  ou32_TransferDataTimeout
   Maximum time in ms it can take to write up to 4kB of data to flash
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscBuSequences::UpdateNode(const QString &orc_HexFilePath,
  ...
  s32_Return =
      h_ReadHexFile(orc_HexFilePath, c_HexFile, u32_SignatureBlockAddress);
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (u32_Return != stw::hex_file::NO_ERR) {
      s32_Return = C_RD_WR;
      osc_write_log_error(
          c_LogActivity, "Could not get the HEX file split into handy chunks!");
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(
          c_LogActivity,
          "Could not activate the programming session! Details: " +
              C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                  s32_Return, u8_NumberCode));
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if ((s32_Return == C_WARN) &&
        (u8_NumberCode ==
         C_OscProtocolDriverOsy::hu8_NR_CODE_REQUIRED_TIME_DELAY_NOT_EXPIRED)) {
      // Special case: The server must wait for a second to allow a request
      // after a reset. In some scenarios the normal waiting times after resets
      // are not enough (this times are not calculated for this security
      // scenario), so wait in this case for another second and try again
    ...
    if (q_SecureMode == true) {
      ...
      if (u64_Seed != 42U) {
        const QString c_Tmp = "Received seed in non secure mode does not match "
                              "the expected value, expected: 42, got " +
                              QString::number(u64_Seed);
        osc_write_log_warning(c_LogActivity, c_Tmp);
      }

      s32_Return =
          mc_OsyProtocol.OsySecurityAccessSendKey(3U, u32_KEY, &u8_NumberCode);
      if (s32_Return != C_NO_ERR) {
        osc_write_log_error(
            c_LogActivity,
  ...
  if ((s32_Return == C_NO_ERR) && (pc_HexDump != NULL)) {
    ...
    for (uint16_t u16_Area = 0U; u16_Area < pc_HexDump->at_Blocks.size();
      ...
      s32_Return = mc_OsyProtocol.OsyCheckFlashMemoryAvailable(
          ...
          pc_HexDump->at_Blocks[u16_Area].au8_Data.size(), &u8_NumberCode);
      if (s32_Return != C_NO_ERR) {
        QString c_Error;
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (q_Return != true) {
      osc_write_log_warning(
          c_LogActivity, "Could not get the name of the current Windows user!");
      c_UserName = "unknown";
    }

    s32_Return = mc_OsyProtocol.OsyWriteApplicationSoftwareFingerprint(
        au8_Date, au8_Time, c_UserName, &u8_NumberCode);
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(
          c_LogActivity,
          "Could not set the SW fingerprint! Details: " +
              C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                  s32_Return, u8_NumberCode));
    }
  }
  ...
  if ((s32_Return == C_NO_ERR) && (pc_HexDump != NULL)) {
    ...
    for (uint16_t u16_Area = 0U; u16_Area < pc_HexDump->at_Blocks.size();
      ...
      s32_Return = mc_OsyProtocol.OsyRequestDownload(
          pc_HexDump->at_Blocks[u16_Area].u32_AddressOffset,
          pc_HexDump->at_Blocks[u16_Area].au8_Data.size(), u32_MaxBlockLength,
          &u8_NumberCode);
      if (s32_Return != C_NO_ERR) {
        QString c_Error;
        c_Error = QString::asprintf(
        ...
        osc_write_log_error(
            c_LogActivity,
            "Could not request download " + c_Error + "! Details: " +
                C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                    s32_Return, u8_NumberCode));
        ...
        const uint32_t u32_AdaptedTimeout =
            C_OscUpdateUtil::h_GetAdaptedTransferDataTimeout(
                ou32_TransferDataTimeout, u32_MaxBlockLength, ms32_CanBitrate);
        ...
        while (s32_RemainingBytes > 0) {
          ...
          (void)memcpy(
              ...
              reinterpret_cast<const uint8_t *>(
                  pc_HexDump->at_Blocks[u16_Area].au8_Data.constData() +
                  (s32_Size - s32_RemainingBytes)),
              c_Data.size());

          s32_Return = mc_OsyProtocol.OsyTransferData(u8_BlockSequenceCounter,
                                                      c_Data, &u8_NumberCode);
          if (s32_Return == C_NO_ERR) {
            QString c_Text;
            c_Text =
                QString::asprintf("Transferring area %02d/%02d  byte %08d/%08d",
            ...
            const uint8_t u8_Percentage = static_cast<uint8_t>(
                (u8_MAX_PERCENTAGE * (s32_Size - s32_RemainingBytes)) /
                s32_Size);
            ...
            osc_write_log_error(
                c_LogActivity,
                "Could not transfer data. Details: " +
                    C_OscProtocolDriverOsy::h_GetOpenSydeServiceErrorDetails(
                        s32_Return, u8_NumberCode));
            break;
          }
      ...
      if (s32_Return == C_NO_ERR) {
        QString c_Text;
        c_Text = QString::asprintf(
            "Transferring area %02d/%02d  byte %08d/%08d", u16_Area + 1,
            pc_HexDump->at_Blocks.size(), s32_Size, s32_Size);
        ...
        if (u16_Area == (pc_HexDump->at_Blocks.size() - 1)) {
          osc_write_log_info(c_LogActivity,
                             "This is the last area, we'll better check the "
                             "signature as well ...");
          s32_Return = mc_OsyProtocol.OsyRequestTransferExitAddressBased(
              true, u32_SignatureBlockAddress, &u8_NumberCode);
        } else {
          s32_Return = mc_OsyProtocol.OsyRequestTransferExitAddressBased(
              false, 0U, &u8_NumberCode);
        }
...
int32_t C_OscBuSequences::ResetSystem(void) {
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    QThread::msleep(
        500); // wait a little to make sure the device has performed the reset
    osc_write_log_error(c_LogActivity,
                        "Successfully sent reset to target device!");
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscBuSequences::h_ReadHexFile(const QString &orc_HexFilePath,
                                        C_OscHexFile &orc_HexFile,
  ...
  if (u32_Return != stw::hex_file::NO_ERR) {
    ...
    osc_write_log_error(
        c_LogActivity,
        "Could not open the HEX file \"" + orc_HexFilePath +
            "\"! Details: " + orc_HexFile.ErrorCodeToErrorText(u32_Return));
  } else {
    QList<stw::diag_lib::C_XFLECUInformation> c_InfoBlocks;
    // no return value that can cause trouble for us
    (void)orc_HexFile.GetECUInformationBlocks(c_InfoBlocks, 0, false, false,
                                              false);

    osc_write_log_info(
        c_LogActivity,
        "Number of application information blocks in HEX file: " +
    ...
    for (int32_t s32_Index = 0; s32_Index < c_InfoBlocks.size(); s32_Index++) {
      ...
      osc_write_log_info(c_LogActivity,
                         "Application information block " + c_Help);
    ...
    s32_Return =
        orc_HexFile.GetSignatureBlockAddress(oru32_SignatureBlockAddress);
    if (s32_Return != C_NO_ERR) {
      osc_write_log_error(
          c_LogActivity,
          "Could not find a signature block in the HEX file data!");
    } else {
      QString c_Text =
          QString::asprintf("Signature block found at address 0x%08X.",
                            oru32_SignatureBlockAddress);
...
void C_OscBuSequences::PrepareForDestruction(void) {
  mc_TpCan.SetDispatcher(NULL); // we are about to destroy the dispatcher; make
                                // sure TP disconnects from it
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscBuSequences::m_ReportProgressPercentage(
    const uint8_t ou8_ProgressInPercentage) {
  std::cout << "Progress in Percentage: "
            << static_cast<int32_t>(ou8_ProgressInPercentage) << std::endl;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscBuSequences::m_ReportProgress(const int32_t os32_Result,
...
void C_OscBuSequences::m_ReportFlashloaderInformationRead(
  ...
  for (uint32_t u32_Line = 0U; u32_Line < static_cast<uint32_t>(c_Text.count());
       u32_Line++) {
