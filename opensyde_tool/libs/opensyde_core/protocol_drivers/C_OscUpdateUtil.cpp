...
/*!
   \file
   \brief       Utility class for static functions for usage in system update
   and basic update.
...
/*! \brief   Returns an adapted device transfer data timeout time for
   ...
   The device specific time does only cover the needed time of the device
   itself, but not the real communication time. In case of a high bus load this
   time can vary a lot.
   ...
   * -> 2ms for each CAN message roughly results in the number of messages for
   each block * 2ms
   * Assuming a high "Alien" busload of 80% we need to multiply by 5
   ...
   time as base \param[in]       ou32_MaxBlockLength                Maximum
   number of bytes of each block \param[in]       ou32_Bitrate Bus bitrate in
   kBit/s
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscUpdateUtil::h_GetAdaptedTransferDataTimeout(
  ...
  // Scale the offset to the expected bitrate on the bus in relation to the
  // slowest bitrate of 100 kBit/s
