...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Handle notification about incoming response to event driven data
   pool reading

   Translates the openSYDE protocol specific notification to a generic
   notification. Invokes the notification function of C_OscDiagProtocolBase to
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDiagProtocolOsy::m_OsyReadDataPoolDataEventReceived(
  ...
  if (mpr_OnDataPoolReadEventReceived != NULL) {
    mpr_OnDataPoolReadEventReceived(mpv_OnAsyncInstance, ou8_DataPoolIndex,
                                    ou16_ListIndex, ou16_ElementIndex,
                                    orc_Value);
  } else {
    m_LogErrorWithHeader("Asynchronous communication",
                         "Async ReadDataPool response received, but no handler "
                         "is installed. Ignoring.",
                         TGL_UTIL_FUNC_ID);
  }
}

//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Handle notification about incoming negative response to event
   driven data pool reading
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDiagProtocolOsy::m_OsyReadDataPoolDataEventErrorReceived(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_NrCode) {
  if (mpr_OnDataPoolReadEventErrorReceived != NULL) {
    mpr_OnDataPoolReadEventErrorReceived(mpv_OnAsyncInstance, ou8_DataPoolIndex,
                                         ou16_ListIndex, ou16_ElementIndex,
                                         ou8_NrCode);
  }
}
...
C_OscDiagProtocolOsy::C_OscDiagProtocolOsy(void)
    : C_OscDiagProtocolBase(), C_OscProtocolDriverOsy() {}
...
/*! \brief   Read numeric data from server's data pool
   ...
   In the openSYDE protocol the implementation is the same for array and numeric
   data.
   ...
   C_RANGE    data pool, list, element index out of range (checked by
   client-side function) C_NOACT    could not send request (Tx buffer full)
   C_CONFIG   transport protocol not installed
   C_WARN     error response
   C_RD_WR    malformed protocol response
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolReadNumeric(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
  ...
  return this->DataPoolReadArray(ou8_DataPoolIndex, ou16_ListIndex,
                                 ou16_ElementIndex, orc_ReadData, opu8_NrCode);
}
...
/*! \brief   Read array data from server's data pool
   ...
   C_CONFIG   transport protocol not installed
   C_WARN     error response
   C_RD_WR    malformed protocol response (e.g. reported size does not match
   expected size) C_COM      communication driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolReadArray(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, QByteArray &orc_ReadData,
    uint8_t *const opu8_NrCode) {
  int32_t s32_Return;
  ...
  s32_Return =
      this->OsyReadDataPoolData(ou8_DataPoolIndex, ou16_ListIndex,
                                ou16_ElementIndex, orc_ReadData, opu8_NrCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolWriteNumeric(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const QByteArray &orc_DataToWrite,
    uint8_t *const opu8_NrCode) {
  return this->OsyWriteDataPoolData(ou8_DataPoolIndex, ou16_ListIndex,
                                    ou16_ElementIndex, orc_DataToWrite,
                                    opu8_NrCode);
}
...
/*! \brief   Write array data to server's data pool
   ...
   Send request and wait for response.
   In the openSYDE protocol this results in the same protocol service as writing
   numeric data.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolWriteArray(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const QByteArray &orc_DataToWrite,
    uint8_t *const opu8_NrCode) {
  return this->DataPoolWriteNumeric(ou8_DataPoolIndex, ou16_ListIndex,
                                    ou16_ElementIndex, orc_DataToWrite,
                                    opu8_NrCode);
}
...
/*! \brief   Set rate of event driven transmissions
   ...
   * remember the configured rail time for starting subsequent event driven
   transmissions
   ...
   \return
   C_NO_ERR   request sent, positive response received (or: rail value
   remembered) C_RANGE    parameter out of range (checked by client-side
   function) C_NOACT    could not send request (e.g. Tx buffer full) C_CONFIG
   pre-requisites not correct; e.g. driver not initialized C_WARN     error
   response C_RD_WR    malformed protocol response C_COM      communication
   driver reported error
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDiagProtocolOsy::DataPoolSetEventDataRate(const uint8_t ou8_Rail,
...
/*! \brief   Request cyclic driven transmission of data pool element.
   ...
   Shall send the request and wait for the initial confirmation (or error)
   response.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolReadCyclic(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_Rail,
    uint8_t *const opu8_NrCode) {
  return this->OsyReadDataPoolDataCyclic(ou8_DataPoolIndex, ou16_ListIndex,
                                         ou16_ElementIndex, ou8_Rail,
                                         opu8_NrCode);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolReadChangeDriven(
    const uint8_t ou8_DataPoolIndex, const uint16_t ou16_ListIndex,
    const uint16_t ou16_ElementIndex, const uint8_t ou8_Rail,
    const uint32_t ou32_Threshold, uint8_t *const opu8_NrCode) {
  return this->OsyReadDataPoolDataChangeDriven(
      ou8_DataPoolIndex, ou16_ListIndex, ou16_ElementIndex, ou8_Rail,
...
/*! \brief   Request cyclic driven transmission of NVM content.
   ...
   byte) \param[in,out] orc_DataRecord     in: size defines number of data bytes
   to read out: read data bytes \param[out]    opu8_NrCode        if != NULL:
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::NvmRead(const uint32_t ou32_MemoryAddress,
                                      QByteArray &orc_DataRecord,
                                      uint8_t *const opu8_NrCode) {
  int32_t s32_Retval;

  s32_Retval = this->OsyReadMemoryByAddress(ou32_MemoryAddress, orc_DataRecord,
                                            opu8_NrCode);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::NvmWriteStartTransaction(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::NvmWrite(const uint32_t ou32_MemoryAddress,
                                       const QByteArray &orc_DataRecord,
                                       uint8_t *const opu8_NrCode) {
  int32_t s32_Retval;

  s32_Retval = this->OsyWriteMemoryByAddress(ou32_MemoryAddress, orc_DataRecord,
                                             opu8_NrCode);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscDiagProtocolOsy::DataPoolReadVersion(const uint8_t ou8_DataPoolIndex,
  ...
  s32_Retval =
      this->OsyReadDataPoolMetaData(ou8_DataPoolIndex, c_MetaData, opu8_NrCode);
...
/*! \brief   Read Datapool meta data
   ...
   \param[in]  ou8_DataPoolIndex   Datapool index
   \param[out] orau8_Version       Read version; format: see function
   description \param[out] orc_Name            Read name of Datapool \param[out]
   opu8_NrCode         if != NULL: negative response code in case of an error
   response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolReadMetaData(
...
/*! \brief   Verify data pool consistency
   ...
   Used to check whether the referenced data pool matches with the passed
   criteria. In the openSYDE protocol we verify:
   * a checksum over data data pool definition

   \param[in]  ou8_DataPoolIndex              Data pool index
   \param[in]  ou16_NumberOfDataPoolElements  number of elements in data pool
   (not used in this protocol) \param[in]  ou16_DataPoolVersion version number
   of data pool (not used in this protocol) \param[in]  ou32_DataPoolChecksum
   Data pool checksum \param[out] orq_Match                      Data pool
   checksum match response
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDiagProtocolOsy::DataPoolVerify(
  ...
  (void)ou16_NumberOfDataPoolElements; // not supported by this protocol
                                       // (checksum should be good enough)
  (void)ou16_DataPoolVersion; // not supported by this protocol (checksum should
                              // be good enough)

  s32_Retval = this->OsyVerifyDataPool(ou8_DataPoolIndex, ou32_DataPoolChecksum,
                                       orq_Match);
