...
public:
  ...
  static bool h_SerialNumberFromStringToArray(const QString &orc_SerialNumber,
                                              uint8_t (&orau8_SerialNumber)[6]);

  bool q_IsValid;       ///< flag if a valid serial number was already set
  bool q_ExtFormatUsed; ///< flag which protocol communication was used. true
                        ///< for extended, false for standard
  bool
      q_FsnSerialNumber; ///< flag if serial number was set in FSN or POS format
  ...
  // Special case: u8_SerialNumberManufacturerFormat equals 0, the old STW
  // standard format POS is used despite of the used extended format.
  QString c_SerialNumberExt; ///< serial number of sender in FSN format
  uint8_t u8_SerialNumberManufacturerFormat; ///< defines the used manufacturer
                                             ///< format for the sn
};
