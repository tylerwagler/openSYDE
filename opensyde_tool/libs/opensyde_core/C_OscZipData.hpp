...
class C_OscZipData {
public:
  static int32_t h_Zip(uint8_t *const opu8_Destination,
                       uint32_t &oru32_DestinationLength,
                       const uint8_t *const opu8_Source,
                       const uint32_t ou32_SourceLength);
  static int32_t h_Unzip(uint8_t *const opu8_Destination,
                         uint32_t &oru32_DestinationLength,
                         const uint8_t *const opu8_Source,
                         const uint32_t ou32_SourceLength);
  static uint32_t
  h_GetRequiredBufSizeForZipping(const uint32_t ou32_SourceLength);
};
