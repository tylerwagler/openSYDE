...
uint16_t C_OscChecksummedIniFile::m_CalcCheckSum(const bool oq_Fast) {
  ...
  for (uint32_t u32_Section = 0U; u32_Section < u32_NumSections;
       u32_Section++) {
    if (c_SectionList[u32_Section] != "INISAFE") {
      // if "fast": do NOT append
      uint32_t u32_NumDirectives;
      ReadSectionValues(c_SectionList[u32_Section], &c_DirectivesValues,
                        !oq_Fast);
      u32_NumDirectives = c_DirectivesValues.count();
      for (uint32_t u32_Directive = 0U; u32_Directive < u32_NumDirectives;
           u32_Directive++) {
        const QString &rc_String = c_DirectivesValues[u32_Directive];
        for (uint32_t u32_Byte = 0U; u32_Byte < rc_String.length();
             u32_Byte++) {
          uint8_t u8_Value;
          u8_Value =
              static_cast<uint8_t>(rc_String.toUtf8().constData()[u32_Byte]);
          u8_Value = static_cast<uint8_t>(
              (static_cast<uint8_t>(u8_Value << 3U)) + (u8_Value >> 5U));
...
/*! \brief    Verify checksum of INI file data
   ...
   Calculates checksum over INI file contents and compares it agains the
   checksum stored in the ini file data.
...
bool C_OscChecksummedIniFile::CheckCheckSum(void) {
  ...
  if (u16_Version == 1U) {
    ...
    if (u16_CheckSumIni2 == u16_CheckSumCalc) {
      ...
      // check V1 checksum (maybe file was created by an old version of this
      // class)
      u16_CheckSumCalc = this->m_CalcCheckSum(false);
      if (u16_CheckSumIni1 == u16_CheckSumCalc) {
        q_Match = true;
...
/*! \brief    Recalculate checksum
   ...
   Note: Does not write to the file immediately; this is only done in the
