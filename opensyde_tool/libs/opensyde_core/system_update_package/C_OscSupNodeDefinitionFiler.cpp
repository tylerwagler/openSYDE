...
static const QString mc_ROOT_NAME =
    "opensyde-secure-update-collection-definition";          // xml root node
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupNodeDefinitionFiler::h_SaveNodes(
  ...
  if (orc_Nodes.size() == orc_Files.size()) {
    for (uint32_t u32_Pos = 0;
         (u32_Pos < orc_Nodes.size()) && (s32_Retval == C_NO_ERR); u32_Pos++) {
      if (orc_Nodes[u32_Pos].u8_Active ==
          C_OscSupNodeDefinitionFiler::hu8_ACTIVE_NODE) {
        s32_Retval = C_OscSupNodeDefinitionFiler::mh_SaveNode(
            orc_Files[u32_Pos], orc_Nodes[u32_Pos]);
      }
    }
  } else {
    s32_Retval = C_CONFIG;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupNodeDefinitionFiler::h_LoadNodes(
  ...
  if ((orc_Files.size() == orc_NodeFoldersAbs.size()) &&
    ...
    for (uint32_t u32_NodeCounter = 0;
         (u32_NodeCounter < orc_ActiveNodes.size()) && (s32_Retval == C_NO_ERR);
         ++u32_NodeCounter) {
      QString c_Signature;
      C_OscSuSequences::C_DoFlash c_DoFlash;
      if (orc_ActiveNodes[u32_NodeCounter] ==
          C_OscSupNodeDefinitionFiler::hu8_ACTIVE_NODE) {
        C_OscXmlParser c_XmlParser;
        s32_Retval = c_XmlParser.LoadFromFile(orc_Files[u32_NodeCounter]);
        if (s32_Retval == C_NO_ERR) {
          // get update position
          const uint32_t u32_UpdatePosition =
              orc_UpdatePosition[u32_NodeCounter];
          Q_ASSERT(c_XmlParser.SelectRoot() ==
                   mc_ROOT_NAME); // we shall have a valid and
          // compatible update package
          C_OscSupNodeDefinitionFiler::mh_LoadFilesSection(
          ...
          C_OscSupNodeDefinitionFiler::mh_LoadFilesSection(
              c_DoFlash.c_FilesToWriteToNvm, u32_NodeCounter,
              u32_UpdatePosition, orc_UpdateOrderByNodes,
              orc_NodeFoldersAbs[u32_NodeCounter], c_XmlParser, mc_PARAM_FILES,
              mc_PARAM_FILE);
          C_OscSupNodeDefinitionFiler::mh_LoadPemConfigSection(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupNodeDefinitionFiler::mh_SaveNode(
    const QString &orc_File, const C_OscSupNodeDefinition &orc_Node) {
  int32_t s32_Result;
  C_OscXmlParser c_XmlParser;
  ...
  mh_SaveFiles(orc_Node.c_NvmFileNames, c_XmlParser, mc_PARAM_FILES,
               mc_PARAM_FILE);
...
/*! \brief  Load files section from service update package definition file
   ...
   positions \param[in]      orc_NodeFolderAbs      Path where all the files
   will be unzipped to \param[in,out]  orc_XmlParser          XMLParser for
   service update package definition file \param[in]      orc_BaseNodeName XML
   node name to use on base level \param[in]      orc_ElementNodeName    XML
   node name to use on item level
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSupNodeDefinitionFiler::mh_LoadFilesSection(
...
/*! \brief  Load PEM configuration section from service update package
   ...
   \param[in]      orc_NodeFolderAbs   Path where all the files will be unzipped
   to \param[in,out]  orc_XmlParser       XMLParser for service update package
   definition file
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSupNodeDefinitionFiler::mh_LoadPemConfigSection(
...
/*! \brief  Save files in XML
   ...
   definition file \param[in]      orc_BaseNodeName       XML node name to use
   on base level \param[in]      orc_ElementNodeName    XML node name to use on
   item level
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSupNodeDefinitionFiler::mh_SaveFiles(
...
/*! \brief  Save PEM file configuration in XML
   ...
   configuration \param[in,out]  orc_XmlParser    XMLParser for service update
   package definition file
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSupNodeDefinitionFiler::mh_SavePemConfig(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSupNodeDefinitionFiler::mh_SaveSignatureFile(
    const C_OscSupNodeDefinition &orc_Node, C_OscXmlParserBase &orc_XmlParser) {
  if (orc_Node.u8_SignaturePresent == hu8_ACTIVE_NODE) {
    // File
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(mc_SIG_FILE) ==
             mc_SIG_FILE);
    orc_XmlParser.SetAttributeString(mc_SIG_FILE_ATTR,
                                     orc_Node.c_SignatureFile);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == mc_ROOT_NAME);
  }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupNodeDefinitionFiler::mh_LoadSignatureFile(
    const QString &orc_NodeFolderAbs, QString &orc_Signature,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  if (orc_XmlParser.SelectNodeChild(mc_SIG_FILE) == mc_SIG_FILE) {
    const QString c_XmlAttr =
        orc_XmlParser.GetAttributeString(mc_SIG_FILE_ATTR);
