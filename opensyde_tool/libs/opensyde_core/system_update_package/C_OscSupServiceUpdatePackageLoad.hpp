...
public:
   static int32_t h_ProcessPackageUsingPemFiles(const QString & orc_PackagePath,
                                                ...
                                                const QByteArray & orc_DecryptNodes = QByteArray(),
   ...
   static int32_t h_ProcessPackage(const QString & orc_PackagePath,
                                   ...
                                   QString & orc_ErrorMessage, const bool oq_IsZip,
                                   const QByteArray & orc_DecryptNodes = QByteArray(),
                                   const QStringList & orc_DecryptNodesPassword = QStringList(), const QList<QByteArray > & orc_NodeSignatureKeys = QList<QByteArray >());
...
private:
   ...
   static int32_t mh_VerifySignatures(const QList<C_OscSuSequences::C_DoFlash> & orc_ApplicationsToWrite,
                                      const QByteArray & orc_ActiveNodes,
                                      const QList<QByteArray > & orc_NodeSignatureKeys,
