 ...
 * ---------------------------------------------------------------------------------------
 */
static const QString mc_PACKAGE_EXT = ".syde_sup";
static const QString mc_PACKAGE_EXT_TMP =
    ".syde_sup_tmp"; // intermediate directory before creating zip archive
static const QString mc_PACKAGE_UPDATE_DEF =
    "service_update_package.syde_supdef";
static const uint8_t mu8_ACTIVE_NODE = 1U;
static const uint16_t mu16_FILE_VERSION = 1U;
static const QString mc_SUP_SYSDEF = "sup_system_definition.syde_sysdef";
static const QString mc_INI_DEV = "devices.ini";
...
static const QString mc_PEM_FILE_CONFIG_SEC_ENAB_ATTR =
    "security-enabled"; // xml node attribute
static const QString mc_PEM_FILE_CONFIG_SEC_SEND_ATTR =
    "security-send"; // xml node attribute
static const QString mc_PEM_FILE_CONFIG_DEB_ENAB_ATTR =
    "debugger-enabled"; // xml node attribute
static const QString mc_PEM_FILE_CONFIG_DEB_SEND_ATTR =
    "debugger-send";                                    // xml node attribute
 ...
 */
QStringList
    C_OscSupServiceUpdatePackageV1::mhc_WarningMessages; // global warnings e.g.
                                                         // if update position
                                                         // of
                                                         // active node is not
                                                         // available
QString C_OscSupServiceUpdatePackageV1::
    mhc_ErrorMessage; // description of error which caused the
...
/*! \brief   Create an update package
   ...
   * write permission to target folder
   ...
   instance - no file!)
   \param[in]   ou32_ActiveBusIndex             index of bus the client is
   connected to
   \param[in]   orc_ActiveNodes                 list of all nodes contains
   information which node is available for update
   \param[in]   orc_NodesUpdateOrder            update order of nodes (index is
   ...
   package (empty list if no warnings)
   \param[out]  orc_ErrorMessage                error message in case of failure
   (empty string if no error)
   \param[in]   oq_SaveInCompatibilityFormat    Flag to export in compatibility
   format (V2)
   \param[in]   oq_SaveAsFile                   True: content of System Update
   Package is saved in .syde_sup format False: content of System Update Package
   is saved to a directory
   \param[in]   orc_TemporaryDirectory          Optional parameter for temporary
   directory. If empty, temporary directory is created next to package path.
...
int32_t C_OscSupServiceUpdatePackageV1::h_CreatePackage(
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (oq_SaveAsFile) {
      ...
      c_PackagePathTmp =
          c_PackagePathTmp.first(c_PackagePathTmp.indexOf(mc_PACKAGE_EXT)) +
          mc_PACKAGE_EXT_TMP;
    } else {
      // if we want to save the Update Package as a directory, we just use the
      // temp folder, don't rename and delete it. Obviously the zipping does not
      // happen either (see below).
      c_PackagePathTmp =
          orc_PackagePath.first(orc_PackagePath.indexOf(mc_PACKAGE_EXT));
    }
    ...
    if (s32_Return != C_NO_ERR) {
      ...
      for (uint32_t u32_It = 0; u32_It < c_ApplicationsToWrite.size();
           ++u32_It) {
        const C_OscSuSequences::C_DoFlash &c_DoFlash =
            c_ApplicationsToWrite[u32_It]; // current node
        // for service_update_package.syde_supdef we need relative paths!
        // Files
        C_OscZipFile::h_AppendFilesRelative(
            c_SupFiles, c_DoFlash.c_FilesToFlash, c_PackagePathTmp);
        // Parameter set files
        C_OscZipFile::h_AppendFilesRelative(
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    const QString c_SysDefPath = c_PackagePathTmp + mc_SUP_SYSDEF;
    ...
    if (s32_Return != C_NO_ERR) {
      // write log
      mhc_ErrorMessage = "Could not save System Definition file to path \"" +
                         c_PackagePathTmp + "\".";
      osc_write_log_error("Creating Update Package", mhc_ErrorMessage);
      s32_Return = C_RD_WR;
    }
  ...
  // definition files
  if (s32_Return == C_NO_ERR) {
    // go through nodes in system definition to get device definition names and
    // paths
    ...
    s32_Return =
        mh_CreateDeviceIniFile(c_PackagePathTmp, c_DeviceDefinitionFiles);
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return = mh_SupDefParamAdapter(
        orc_SystemDefinition, ou32_ActiveBusIndex, orc_ActiveNodes,
        orc_NodesUpdateOrder, c_ApplicationsToWrite, c_SupDefContent);
    if (s32_Return == C_NO_ERR) {
  ...
  if (oq_SaveAsFile) {
    ...
    if ((s32_Return == C_NO_ERR) || (s32_Return == C_WARN)) {
      s32_Return = C_OscZipFile::h_CreateZipFile(
          c_PackagePathTmp, c_SupFiles, c_TargetZipArchive, &mhc_ErrorMessage);
      if (s32_Return != C_NO_ERR) {
        osc_write_log_error("Creating Update Package", mhc_ErrorMessage);
      }
    }
    ...
    if (q_TemporaryFolderCreated == true) {
      ...
      if (s32_Tmp != 0) {
        ...
        if ((s32_Return == C_NO_ERR) ||
            (s32_Return == C_WARN)) // do not redefine error in case we had even
                                    // a problem earlier
        {
...
/*! \brief   Unpack an update package
   ...
   * write permission of target path for unzipped package
   ...
   e.g. 012345.syde_sup)
   \param[in]   orc_TargetUnzipPath       folder of path to unzip the service
   update package (works with or without trailing path delimiter)
   \param[out]  orc_SystemDefinition      system definition (class instance - no
   file!)
   \param[out]  oru32_ActiveBusIndex      index of bus the client is connected
   to
   \param[out]  orc_ActiveNodes           list of all nodes contains information
   which node is available for update
   \param[out]  orc_NodesUpdateOrder      update order of active nodes
   \param[out]  orc_ApplicationsToWrite   files for updating nodes
   \param[out]  orc_WarningMessages       warning messages for imported package
   (empty list if no warnings)
   \param[out]  orc_ErrorMessage          error message in case of failure
   (empty string if no error)
   \param[in]   oq_IsZip                  true: given package is ".syde_sup"
   format false: given package is just a plain directory
   ...
   C_NOACT     error code of a called core function (should not occur for valid
   and compatible service update package) C_OVERFLOW  error code of a called
   core function (should not occur for valid and compatible service update
   package) C_DEFAULT   error code of a called core function (should not occur
   for valid and compatible service update package)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupServiceUpdatePackageV1::h_ProcessPackage(
  ...
  if (oq_IsZip) {
    ...
    if ((QFileInfo(orc_PackagePath).exists() &&
         QFileInfo(orc_PackagePath).isFile()) == false) {
      mhc_ErrorMessage =
    ...
    if (s32_Return == C_NO_ERR) {
      if (QFileInfo(c_TargetUnzipPath).isDir() == true) {
        s32_Return = (QDir(c_TargetUnzipPath).removeRecursively() ? 0 : -1);
        if (s32_Return != 0) {
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      if (s32_Return != C_NO_ERR) {
        mhc_ErrorMessage = "Could not create folder \"" + c_TargetUnzipPath +
                           "\" for zip archive.";
        osc_write_log_error("Unpacking Update Package", mhc_ErrorMessage);
        s32_Return = C_RD_WR;
      }
    ...
    if (s32_Return == C_NO_ERR) {
      s32_Return = C_OscZipFile::h_UnpackZipFile(
          orc_PackagePath, c_TargetUnzipPath, &mhc_ErrorMessage);
      if (s32_Return != C_NO_ERR) {
        osc_write_log_error("Unpacking Update Package", mhc_ErrorMessage);
      }
    }
    ...
    if (s32_Return == C_NO_ERR) {
      s32_Return =
          C_OscSupServiceUpdatePackageV1::mh_CheckSupFiles(c_TargetUnzipPath);
      if (s32_Return != C_NO_ERR) {
    ...
    if (QFileInfo(orc_PackagePath).isDir() == true) {
      ...
      s32_Return =
          C_OscSupServiceUpdatePackageV1::mh_CheckSupFiles(orc_PackagePath);
      if (s32_Return != C_NO_ERR) {
        mhc_ErrorMessage = "Could not find necessary files within \"" +
                           orc_PackagePath +
                           "\" for update package to be complete.";
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    Q_ASSERT(
        ...
        mc_ROOT_NAME); // we shall have a valid and compatible update package
    ...
    do {
      ...
      const uint8_t u8_NodeActive = static_cast<uint8_t>(
          c_XmlParser.GetAttributeUint32(mc_NODE_ACTIVE_ATTR));
      ...
      if (u8_NodeActive == mu8_ACTIVE_NODE) {
        ...
        C_OscSupServiceUpdatePackageV1::mh_LoadFilesSection(
            c_DoFlash.c_FilesToFlash, u32_NodeCounter, u32_UpdatePosition,
            c_UpdateOrderByNodes, c_TargetUnzipPath, c_XmlParser, mc_FILES,
            mc_FILE);
        C_OscSupServiceUpdatePackageV1::mh_LoadFilesSection(
            c_DoFlash.c_FilesToWriteToNvm, u32_NodeCounter, u32_UpdatePosition,
            c_UpdateOrderByNodes, c_TargetUnzipPath, c_XmlParser,
            mc_PARAM_FILES, mc_PARAM_FILE);
        C_OscSupServiceUpdatePackageV1::mh_LoadPemConfigSection(
            c_DoFlash, u32_NodeCounter, u32_UpdatePosition,
            c_UpdateOrderByNodes, c_TargetUnzipPath, c_XmlParser);
      }
      u32_NodeCounter++; // next active node
      orc_ApplicationsToWrite.push_back(
          c_DoFlash); // push back in any case even if we have no applications
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscSupServiceUpdatePackageV1::h_GetPackageExtension() {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupServiceUpdatePackageV1::mh_CheckSupFiles(
    const QString &orc_PackagePath) {
  int32_t s32_Return = C_NO_ERR;
  ...
  for (uint32_t u32_It = 0; u32_It < c_NecessaryFiles.size(); ++u32_It) {
    ...
    const QString c_FileExt =
        "." + QFileInfo(c_NecessaryFiles[u32_It]).suffix();
    ...
    if (c_InfoList.count() == 1) {
      ...
      if (c_InfoList.at(0).fileName() != c_NecessaryFiles[u32_It]) {
...
int32_t C_OscSupServiceUpdatePackageV1::mh_CheckParamsToCreatePackage(
  ...
  if ((s32_Return == C_NO_ERR) && (oq_SaveAsFile == true)) {
    ...
    if ((orc_PackagePath.indexOf(mc_PACKAGE_EXT) == -1) ||
        (orc_PackagePath.length() !=
         (orc_PackagePath.indexOf(mc_PACKAGE_EXT) + mc_PACKAGE_EXT.length()))) {
      mhc_ErrorMessage =
          "Package name must have extension \"" + mc_PACKAGE_EXT + "\".";
      osc_write_log_error("Creating Update Package", mhc_ErrorMessage);
      s32_Return = C_RANGE;
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    if (ou32_ActiveBusIndex >= orc_SystemDefinition.c_Buses.size()) {
      mhc_ErrorMessage = "Active Bus Index \"" +
                         QString::number(ou32_ActiveBusIndex) +
  ...
  if (s32_Return == C_NO_ERR) {
    if (orc_ActiveNodes.size() != orc_ApplicationsToWrite.size()) {
      mhc_ErrorMessage = "The container of active nodes and update "
                         "applications have not the same size.";
      osc_write_log_error("Creating Update Package", mhc_ErrorMessage);
      s32_Return = C_OVERFLOW;
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupServiceUpdatePackageV1::mh_CreateUpdatePackageDefFile(
    const QString &orc_Path, const C_SupDefContent &orc_SupDefContent) {
  const QString c_FileName =
      stw::opensyde_core::C_OscUtils::h_IncludeTrailingDelimiter(orc_Path) +
      mc_PACKAGE_UPDATE_DEF;
  int32_t s32_Result;
  ...
  // List nodes
  for (uint32_t u32_Pos = 0; u32_Pos < orc_SupDefContent.c_Nodes.size();
       u32_Pos++) {
    const C_SupDefNodeContent c_CurrentNode =
        orc_SupDefContent.c_Nodes[u32_Pos];

    // Node
    Q_ASSERT(c_XmlParser.CreateAndSelectNodeChild(mc_NODE) == mc_NODE);
    c_XmlParser.SetAttributeUint32(
    ...
    if (c_CurrentNode.u8_Active == mu8_ACTIVE_NODE) {
      ...
      if ((c_CurrentNode.c_ApplicationFileNames.size() > 0) ||
        ...
        // Update Position
        c_XmlParser.SetAttributeUint32(mc_NODE_POSITION_ATTR,
                                       c_CurrentNode.u32_Position);
      }

      mh_SaveFiles(c_CurrentNode.c_ApplicationFileNames, c_XmlParser, mc_FILES,
                   mc_FILE);
      mh_SaveFiles(c_CurrentNode.c_NvmFileNames, c_XmlParser, mc_PARAM_FILES,
                   mc_PARAM_FILE);
...
/*! \brief   Creates specific device definition (internal function).
   ...
   * valid path (should be OK for an internal function)
   ...
   \param[in]  orc_Path                   destination path
   \param[in]  orc_DeviceDefinitionPaths  container of full device definition
   file paths
...
int32_t C_OscSupServiceUpdatePackageV1::mh_CreateDeviceIniFile(
  ...
  try {
    QSettings c_IniFile(orc_Path, QSettings::IniFormat);
    // Ensure file is fresh by clearing it? QSettings doesn't wipe on open.
    // But we are creating a new package presumably.
    // Let's assume we overwrite keys. But to be safe and match "Create"
    // semantics:
    c_IniFile.clear();

    uint32_t u32_DeviceCounter = 1;

    const QString c_GroupHead = c_HEAD_SECTION + "/";
    const QString c_GroupDevice = c_DEVICE_SECTION + "/";

    // write header section
    c_IniFile.setValue(c_GroupHead + c_FIRST_KEY, s32_FIRST_VALUE);
    c_IniFile.setValue(c_GroupHead + c_SECOND_KEY, c_DEVICE_SECTION);
  ...
  for (uint32_t u32_Pos = 0; u32_Pos < orc_ActiveNodes.size(); u32_Pos++) {
    ...
    if ((c_SupDefNodeContent.u8_Active == mu8_ACTIVE_NODE) &&
      ...
      if (s32_Tmp != C_NO_ERR) {
        ...
        osc_write_log_warning("Creating Update Package",
                              c_Message.toLocal8Bit().constData());
        s32_Return = C_WARN;
      }
    }

    // get relative application path of node
    const QString c_Folder = C_OscUtils::h_NiceifyStringForFileName(
        orc_SystemDefinition.c_Nodes[u32_Pos].c_Properties.c_Name);
    // get applications of node
    for (uint32_t u32_ItAppl = 0;
         u32_ItAppl < orc_ApplicationsToWrite[u32_Pos].c_FilesToFlash.size();
         ++u32_ItAppl) {
      const QString &c_File =
          orc_ApplicationsToWrite[u32_Pos].c_FilesToFlash[u32_ItAppl];
      // store application file names with relative path
      const QString c_Tmp = c_Folder + "/" + QFileInfo(c_File).fileName();
      c_SupDefNodeContent.c_ApplicationFileNames.push_back(c_Tmp);
    }
    // get parameter sets of node
    for (uint32_t u32_ItParam = 0;
         u32_ItParam <
         orc_ApplicationsToWrite[u32_Pos].c_FilesToWriteToNvm.size();
         ++u32_ItParam) {
      const QString &c_File =
          orc_ApplicationsToWrite[u32_Pos].c_FilesToWriteToNvm[u32_ItParam];
      // store application file names with relative path
      const QString c_Tmp = c_Folder + "/" + QFileInfo(c_File).fileName();
...
/*! \brief   Adapts nodes update order by interchanging index with value to
   ...
   position
   \param[out]  orc_NodesUpdateOrder      update position is index and value is
   node position
...
/*! \brief  Load files section from service update package definition file
   ...
   \param[in,out]  orc_PositionMap        Map for node indices and update
   positions
   \param[in]      orc_TargetUnzipPath    Path where all the files will be
   unzipped to
   \param[in,out]  orc_XmlParser          XMLParser for service update package
   definition file
...
void C_OscSupServiceUpdatePackageV1::mh_LoadFilesSection(
  ...
  if (orc_XmlParser.SelectNodeChild(orc_BaseNodeName) == orc_BaseNodeName) {
    ...
    do {
      ...
      const QString c_XmlAttr =
          orc_XmlParser.GetAttributeString(mc_FILE_NAME_ATTR);
      const QString c_FileName = QFileInfo(c_XmlAttr).fileName();
      // subfolder without '/'
      const QString c_SubFolder =
          c_XmlAttr.first(c_XmlAttr.length() - c_FileName.length() - 1);
      const QString c_FilePath =
          stw::opensyde_core::C_OscUtils::h_IncludeTrailingDelimiter(
              orc_TargetUnzipPath + c_SubFolder) +
          c_FileName;
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Load PEM configuration section from service update package
   definition file

   \param[in,out]  orc_DoFlash            Node configuration with PEM
   configuration
   \param[in]      ou32_NodeCounter       Current node index
   \param[in]      ou32_UpdatePos         Current update position
   \param[in,out]  orc_PositionMap        Map for node indices and update
...
void C_OscSupServiceUpdatePackageV1::mh_SaveFiles(
  ...
  if (orc_Files.size() > 0) {
    // Files
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(orc_BaseNodeName) ==
             orc_BaseNodeName);
    for (uint32_t u32_PosFile = 0; u32_PosFile < orc_Files.size();
         u32_PosFile++) {
      // File
      Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(orc_ElementNodeName) ==
               orc_ElementNodeName);
      orc_XmlParser.SetAttributeString(mc_FILE_NAME_ATTR,
                                       orc_Files[u32_PosFile]);
      // Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == orc_BaseNodeName);
    }
    // Return for next node
...
/*! \brief  Save PEM file configuration in XML
   ...
   \param[in,out]  orc_CurrentNode  Current node configuration with PEM file
   configuration
   \param[in,out]  orc_XmlParser    XMLParser for service update package
   definition file
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSupServiceUpdatePackageV1::mh_SavePemConfig(
    const C_SupDefNodeContent &orc_CurrentNode,
    C_OscXmlParserBase &orc_XmlParser) {
  if (orc_CurrentNode.c_PemFile != "") {
    // Files
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(mc_PEM_FILE_CONFIG) ==
             mc_PEM_FILE_CONFIG);
    ...
    // File
    Q_ASSERT(orc_XmlParser.CreateAndSelectNodeChild(mc_PEM_FILE) ==
             mc_PEM_FILE);
    orc_XmlParser.SetAttributeString(mc_FILE_NAME_ATTR,
