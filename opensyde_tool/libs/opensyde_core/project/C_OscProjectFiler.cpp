...
/*! \brief   Save project
   ...
   before saving; no write access to file) C_RANGE    orc_Path is empty
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscProjectFiler::h_Save(C_OscProject &orc_Project,
  ...
  if (orc_Path == "") {
    ...
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = mh_SaveInternal(orc_Project, orc_Path, orc_OpenSydeVersion,
                                   q_NewFile);
      if (s32_Retval != C_NO_ERR) {
        osc_write_log_error("Saving project file",
  ...
  if (c_FileInfo.exists() && c_FileInfo.isFile()) {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (c_Xml.SelectRoot() == "Project") {
        ...
        if (c_Tmp == "") {
          // use author if last editor is empty (reason: prior openSYDE versions
          // handled author as editor)
        ...
        // Time
        {
          orc_Project.c_CreationTime = C_OscProject::h_GetTimeOfString(
              c_Xml.GetAttributeString("creation_time"));
        }
        {
          orc_Project.c_ModificationTime = C_OscProject::h_GetTimeOfString(
              c_Xml.GetAttributeString("modification_time"));
        }
        orc_Project.c_OpenSydeVersion =
            c_Xml.GetAttributeString("openSYDE_version");
...
/*! \brief   Save project
   ...
   Before saving the:
   - Editor will be overwritten with the name of the user logged into the PC
   system
   - ModificationTime will be overwritten with the current time
