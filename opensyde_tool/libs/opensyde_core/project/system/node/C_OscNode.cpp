 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscNode::C_OscNode(void) { this->Initialize(); }
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscNode::~C_OscNode(void) { pc_DeviceDefinition = NULL; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::InsertDataPool(const uint32_t ou32_DataPoolIndex,
                                  const C_OscNodeDataPool &orc_DataPool) {
  int32_t s32_Retval = C_NO_ERR;

  if (ou32_DataPoolIndex <= this->c_DataPools.size()) {
    this->c_DataPools.insert(this->c_DataPools.begin() + ou32_DataPoolIndex,
                             orc_DataPool);
    // Sync data pool index with com protocol
    for (uint32_t u32_ItComProtocol = 0;
         u32_ItComProtocol < this->c_ComProtocols.size(); ++u32_ItComProtocol) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::MoveDataPool(const uint32_t ou32_Start,
                                const uint32_t ou32_Target) {
  int32_t s32_Return = C_RANGE;

  if ((ou32_Start < this->c_DataPools.size()) &&
      (ou32_Target < this->c_DataPools.size())) {
    // Copy
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::InsertMessage(
  ...
  int32_t s32_Retval = C_NO_ERR;
  C_OscCanProtocol *const pc_Protocol =
      this->GetCanProtocol(oe_ComProtocol, ou32_DatapoolIndex);
  C_OscNodeDataPool *const pc_DataPool =
      this->GetComDataPool(oe_ComProtocol, ou32_DatapoolIndex);

  if ((pc_Protocol != NULL) && (pc_DataPool != NULL)) {
    C_OscNodeDataPoolList *const pc_ListData = C_OscCanProtocol::h_GetComList(
        *pc_DataPool, ou32_InterfaceIndex, oq_MessageIsTx);
    if ((pc_ListData != NULL) &&
        (ou32_InterfaceIndex < pc_Protocol->c_ComMessages.size())) {
      C_OscCanMessageContainer &rc_MessageContainer =
          pc_Protocol->c_ComMessages[ou32_InterfaceIndex];
      QList<C_OscCanMessage> &rc_Messages =
          rc_MessageContainer.GetMessages(oq_MessageIsTx);
      const uint32_t u32_DataPoolListElementIndex =
          rc_MessageContainer.GetMessageSignalDataStartIndex(oq_MessageIsTx,
                                                             ou32_MessageIndex);
      if (ou32_MessageIndex <= rc_Messages.size()) {
        rc_Messages.insert(rc_Messages.begin() + ou32_MessageIndex,
                           orc_Message);
      } else {
        s32_Retval = C_RANGE;
      }
      ...
      if (u32_DataPoolListElementIndex <= pc_ListData->c_Elements.size()) {
        for (uint32_t u32_ItSignal = 0; u32_ItSignal < orc_SignalData.size();
             ++u32_ItSignal) {
          ...
          pc_ListData->c_Elements.insert(pc_ListData->c_Elements.begin() +
  ...
  if (pc_Protocol != NULL) {
    if (ou32_InterfaceIndex < pc_Protocol->c_ComMessages.size()) {
      ...
      if (oq_MessageIsTx == oq_NewMessageIsTx) {
        // Standard set (Move would handle both cases but vector resizing might
        ...
        if (ou32_MessageIndex < rc_Messages.size()) {
          ...
          if (pc_DataPool != NULL) {
            C_OscNodeDataPoolList *const pc_List =
                C_OscCanProtocol::h_GetComList(
                    *pc_DataPool, ou32_InterfaceIndex, oq_MessageIsTx);
            if (pc_List != NULL) {
              const uint32_t u32_DataPoolListElementIndex =
                  rc_MessageContainer.GetMessageSignalDataStartIndex(
                      oq_MessageIsTx, ou32_MessageIndex);
              // Get previous signal size
              const uint32_t u32_PreviousSignalSize = static_cast<uint32_t>(
                  rc_Messages[ou32_MessageIndex].c_Signals.size());
              // Delete old signals
              for (uint32_t u32_ItSignal = u32_PreviousSignalSize;
                   u32_ItSignal > 0; --u32_ItSignal) {
                const uint32_t u32_Add =
                    (u32_DataPoolListElementIndex + u32_ItSignal) - 1UL;
                pc_List->c_Elements.erase(pc_List->c_Elements.begin() +
                                          u32_Add);
              }
              // Insert new signals
              for (uint32_t u32_ItSignal = 0;
                   u32_ItSignal < orc_SignalData.size(); ++u32_ItSignal) {
                pc_List->c_Elements.insert(pc_List->c_Elements.begin() +
                                               u32_DataPoolListElementIndex +
                                               u32_ItSignal,
        ...
        // Move
        s32_Retval = this->DeleteMessage(oe_ComProtocol, ou32_InterfaceIndex,
                                         ou32_DatapoolIndex, oq_MessageIsTx,
                                         ou32_MessageIndex);

        if (s32_Retval == C_NO_ERR) {
          const QList<C_OscCanMessage> &rc_MatchingMessages =
              rc_MessageContainer.GetMessages(oq_NewMessageIsTx);
          const uint32_t u32_NewMessageIndex =
              static_cast<uint32_t>(rc_MatchingMessages.size());
          s32_Retval = this->InsertMessage(
              oe_ComProtocol, ou32_InterfaceIndex, ou32_DatapoolIndex,
              oq_NewMessageIsTx, u32_NewMessageIndex, orc_Message,
              orc_SignalData);
        }
      }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::DeleteMessage(const C_OscCanProtocol::E_Type oe_ComProtocol,
    ...
    if ((pc_ListData != NULL) &&
      ...
      if (ou32_MessageIndex < rc_Messages.size()) {
        ...
        if (rc_Message.c_Signals.size() > 0) {
          if (u32_DataPoolListElementIndex < pc_ListData->c_Elements.size()) {
            for (uint32_t u32_ItSignal =
                     static_cast<uint32_t>(rc_Message.c_Signals.size());
                 u32_ItSignal > 0UL; --u32_ItSignal) {
              const uint32_t u32_Add =
                  (u32_DataPoolListElementIndex + u32_ItSignal) - 1UL;
              pc_ListData->c_Elements.erase(pc_ListData->c_Elements.begin() +
                                            u32_Add);
            }
          } else {
            s32_Retval = C_RANGE;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::InsertSignal(
    const C_OscCanProtocol::E_Type oe_ComProtocol,
    const uint32_t ou32_InterfaceIndex, const uint32_t ou32_DatapoolIndex,
    const bool oq_MessageIsTx, const uint32_t ou32_MessageIndex,
    ...
    if ((pc_ListData != NULL) &&
      ...
      if (ou32_MessageIndex < rc_Messages.size()) {
        ...
        if ((u32_DataPoolListElementIndex <= pc_ListData->c_Elements.size()) &&
          ...
          // Message
          rc_Message.c_Signals.insert(
              rc_Message.c_Signals.begin() + ou32_SignalIndex, orc_Signal);
    ...
    if ((pc_ListData != NULL) &&
      ...
      if (ou32_MessageIndex < rc_Messages.size()) {
        ...
        if ((u32_DataPoolListElementIndex < pc_ListData->c_Elements.size()) &&
            (ou32_SignalIndex < rc_Message.c_Signals.size())) {
          // Data pool
          pc_ListData->c_Elements[u32_DataPoolListElementIndex] =
              orc_SignalData;

          // Message
          rc_Message.c_Signals[ou32_SignalIndex] = orc_Signal;
          // Just in case
          rc_Message.c_Signals[ou32_SignalIndex].u32_ComDataElementIndex =
              u32_DataPoolListElementIndex;
        } else {
          s32_Retval = C_RANGE;
        }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::SetSignalPosition(
    const C_OscCanProtocol::E_Type oe_ComProtocol,
    const uint32_t ou32_InterfaceIndex, const uint32_t ou32_DatapoolIndex,
    const bool oq_MessageIsTx, const uint32_t ou32_MessageIndex,
    const uint32_t ou32_SignalIndex, const C_OscCanSignal &orc_Signal) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::SetSignalMuxValue(
    const C_OscCanProtocol::E_Type oe_ComProtocol,
    const uint32_t ou32_InterfaceIndex, const uint32_t ou32_DatapoolIndex,
    const bool oq_MessageIsTx, const uint32_t ou32_MessageIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNode::DeleteSignal(const C_OscCanProtocol::E_Type oe_ComProtocol,
                                const uint32_t ou32_InterfaceIndex,
                                const uint32_t ou32_DatapoolIndex,
                                const bool oq_MessageIsTx,
                                const uint32_t ou32_MessageIndex,
    ...
    if ((pc_ListData != NULL) &&
      ...
      if (ou32_MessageIndex < rc_Messages.size()) {
        ...
        if ((u32_DataPoolListElementIndex < pc_ListData->c_Elements.size()) &&
            (ou32_SignalIndex < rc_Message.c_Signals.size())) {
          // Message
          rc_Message.c_Signals.erase(rc_Message.c_Signals.begin() +
                                     ou32_SignalIndex);
...
/*! \brief  Get all HEX applications
   ...
   This returns a vector of all applications that are not of type
   ePARAMETER_SET_HALC, i.e. those that on system commissioning side are handled
   as HEX files.
...
QList<C_OscNodeApplication> C_OscNode::GetHexApplications(void) const {
  ...
  for (uint32_t u32_Index = 0U; u32_Index < this->c_Applications.size();
       ++u32_Index) {
    if (this->c_Applications[u32_Index].e_Type !=
        C_OscNodeApplication::ePARAMETER_SET_HALC) {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNode::GetDataPoolIndex(const C_OscNodeDataPool::E_Type oe_DataPoolType,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNode::GetDataPoolTypeIndex(const uint32_t ou32_DataPoolIndex) const {
  int32_t s32_Retval = -1;
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscNode::GetDataPoolCount(
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscNode::h_CompareNameGreater(const C_OscNode &orc_Node1,
                                     const C_OscNode &orc_Node2) {
  bool q_Retval;

  if (orc_Node1.c_Properties.c_Name.length() ==
      orc_Node2.c_Properties.c_Name.length()) {
    q_Retval = (orc_Node1.c_Properties.c_Name < orc_Node2.c_Properties.c_Name);
  } else {
    q_Retval = orc_Node1.c_Properties.c_Name.length() <
               orc_Node2.c_Properties.c_Name.length();
  }
  return q_Retval;
}
...
uint32_t C_OscNode::GetListsSize(void) const {
  ...
  for (uint32_t u32_ItDataPool = 0; u32_ItDataPool < this->c_DataPools.size();
       ++u32_ItDataPool) {
    const C_OscNodeDataPool &rc_NodeDataPool =
        this->c_DataPools[u32_ItDataPool];
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscNode::GetDataPoolAbsoluteAddress(const uint32_t ou32_DataPoolIndex) const {
  uint32_t u32_Retval = 0;

  for (uint32_t u32_ItDataPool = 0;
       (u32_ItDataPool < this->c_DataPools.size()) &&
       (u32_ItDataPool < ou32_DataPoolIndex);
       ++u32_ItDataPool) {
    const C_OscNodeDataPool &rc_NodeDataPool =
        this->c_DataPools[u32_ItDataPool];
    if ((rc_NodeDataPool.e_Type == C_OscNodeDataPool::eNVM) ||
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscNode::GetListAbsoluteAddress(const uint32_t ou32_DataPoolIndex,
                                  const uint32_t ou32_ListIndex) const {
  uint32_t u32_Retval = GetDataPoolAbsoluteAddress(ou32_DataPoolIndex);

  if (ou32_DataPoolIndex < this->c_DataPools.size()) {
    const C_OscNodeDataPool &rc_NodeDataPool =
        this->c_DataPools[ou32_DataPoolIndex];

    for (uint32_t u32_ItList = 0;
         (u32_ItList < rc_NodeDataPool.c_Lists.size()) &&
         (u32_ItList < ou32_ListIndex);
         ++u32_ItList) {
      const C_OscNodeDataPoolList &rc_NodeDataPoolList =
          rc_NodeDataPool.c_Lists[u32_ItList];
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscNode::GetElementAbsoluteAddress(const uint32_t ou32_DataPoolIndex,
                                     const uint32_t ou32_ListIndex,
                                     const uint32_t ou32_ElementIndex) const {
  uint32_t u32_Retval =
      GetListAbsoluteAddress(ou32_DataPoolIndex, ou32_ListIndex);
  ...
  if (ou32_DataPoolIndex < this->c_DataPools.size()) {
    ...
    if (ou32_ListIndex < rc_NodeDataPool.c_Lists.size()) {
      const C_OscNodeDataPoolList &rc_NodeDataPoolList =
          rc_NodeDataPool.c_Lists[ou32_ListIndex];

      if (rc_NodeDataPoolList.q_NvmCrcActive == true) {
        u32_Retval += 2;
      }
      for (uint32_t u32_ItElement = 0;
           (u32_ItElement < rc_NodeDataPoolList.c_Elements.size()) &&
           (u32_ItElement < ou32_ElementIndex);
           ++u32_ItElement) {
...
/*! \brief  Get first HAL data pool of specified type
   ...
   Returns the first found HAL datapool (HALC or HALC_NVM) of specified type and
   ignores all other datapool copies.

   \param[in]  oq_SafeDatapool   Flag to differ HAL safe datapool and HAL
   nonsafe datapool
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPool *
C_OscNode::GetHalDataPoolConst(const bool oq_SafeDatapool) const {
  const C_OscNodeDataPool *pc_Retval = NULL;

  for (uint32_t u32_ItDatapools = 0; u32_ItDatapools < this->c_DataPools.size();
       ++u32_ItDatapools) {
...
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPool *
C_OscNode::GetComDataPoolConst(const C_OscCanProtocol::E_Type oe_ComProtocol,
                               const uint32_t ou32_DataPoolIndex) const {
  const C_OscNodeDataPool *pc_Retval = NULL;

  for (uint32_t u32_ItCanProtocol = 0;
       (u32_ItCanProtocol < this->c_ComProtocols.size()) && (pc_Retval == NULL);
       ++u32_ItCanProtocol) {
    const C_OscCanProtocol &rc_CanProtocol =
        this->c_ComProtocols[u32_ItCanProtocol];
...
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPool *
C_OscNode::GetComDataPool(const C_OscCanProtocol::E_Type oe_ComProtocol,
                          const uint32_t ou32_DataPoolIndex) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QList<const C_OscNodeDataPool *> C_OscNode::GetComDatapoolsConst(
    const C_OscCanProtocol::E_Type oe_ComProtocol) const {
  QList<const C_OscNodeDataPool *> c_Return;

  for (uint32_t u32_ItCanProtocol = 0;
       u32_ItCanProtocol < this->c_ComProtocols.size(); ++u32_ItCanProtocol) {
    const C_OscCanProtocol &rc_CanProtocol =
        this->c_ComProtocols[u32_ItCanProtocol];
    if (rc_CanProtocol.e_Type == oe_ComProtocol) {
      if (rc_CanProtocol.u32_DataPoolIndex < this->c_DataPools.size()) {
        c_Return.push_back(
            &this->c_DataPools[rc_CanProtocol.u32_DataPoolIndex]);
      }
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QList<C_OscNodeDataPool *>
C_OscNode::GetComDataPools(const C_OscCanProtocol::E_Type oe_ComProtocol) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Get protocol for specified communication protocol and specified
   Datapools
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscCanProtocol *
C_OscNode::GetCanProtocolConst(const C_OscCanProtocol::E_Type oe_ComProtocol,
                               const uint32_t ou32_DataPoolIndex) const {
  const C_OscCanProtocol *pc_Retval =
      this->GetRelatedCanProtocolConst(ou32_DataPoolIndex);

  if ((pc_Retval != NULL) && (pc_Retval->e_Type != oe_ComProtocol)) {
...
//----------------------------------------------------------------------------------------------------------------------
C_OscCanProtocol *
C_OscNode::GetCanProtocol(const C_OscCanProtocol::E_Type oe_ComProtocol,
                          const uint32_t ou32_DataPoolIndex) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QList<const C_OscCanProtocol *> C_OscNode::GetCanProtocolsConst(
    const C_OscCanProtocol::E_Type oe_ComProtocol) const {
  QList<const C_OscCanProtocol *> c_Return;

  for (uint32_t u32_ItCanProtocol = 0;
       u32_ItCanProtocol < this->c_ComProtocols.size(); ++u32_ItCanProtocol) {
    const C_OscCanProtocol &rc_Protocol =
        this->c_ComProtocols[u32_ItCanProtocol];
...
*/
//----------------------------------------------------------------------------------------------------------------------
QList<C_OscCanProtocol *>
C_OscNode::GetCanProtocols(const C_OscCanProtocol::E_Type oe_ComProtocol) {
...
//----------------------------------------------------------------------------------------------------------------------
const C_OscCanProtocol *
C_OscNode::GetRelatedCanProtocolConst(const uint32_t ou32_DataPoolIndex) const {
...
//----------------------------------------------------------------------------------------------------------------------
C_OscCanProtocol *
C_OscNode::GetRelatedCanProtocol(const uint32_t ou32_DataPoolIndex) {
...
/*! \brief   Checks a specific CAN protocol on all CAN interfaces for errors
   ...
   \param[in]   oe_ComProtocol                  Communication protocol
   \param[in]   oq_ComProtocolUsedByInterface   Flag for communication protocol
   bus connection to check for true: Check only protocol instances which are
   connected to a bus false: Check only protocol instances which are not
   connected to a bus \param[out]  orq_InvalidMaxRxSignalCount     The number of
   RX signals of this protocol with at least one Datapool is to high \param[out]
   orq_InvalidMaxTxSignalCount     The number of TX signals of this protocol
   with at least one Datapool is to high \param[out]  orq_InvalidCoRxPdoCount
   The number of active RPDOs (CANopen Manager viewpoint) of CANopen is to high
   \param[out]  orq_InvalidCoTxPdoCount         The number of active TPDOs
   (CANopen Manager viewpoint) of CANopen is to high \param[out]
   orq_InvalidMinSignalCount       The number of signals of this protocol with
   at least one Datapool is too low
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::CheckErrorCanProtocol(
    const C_OscCanProtocol::E_Type oe_ComProtocol,
  ...
  for (u32_InterfaceCounter = 0U;
    ...
    if (this->c_Properties.c_ComInterfaces[u32_InterfaceCounter]
      ...
      this->CheckErrorCanProtocol(
          u32_InterfaceCounter, oe_ComProtocol, oq_ComProtocolUsedByInterface,
...
void C_OscNode::CheckErrorCanProtocol(
    ...
    bool &orq_InvalidMaxTxSignalCount, bool &orq_InvalidCoRxPdoCount,
    bool &orq_InvalidCoTxPdoCount, bool &orq_InvalidMinSignalCount) const {
  const QList<const C_OscCanProtocol *> c_Protocols =
      this->GetCanProtocolsConst(oe_ComProtocol);
  uint32_t u32_Counter;
  ...
  for (u32_Counter = 0U; u32_Counter < c_Protocols.size(); ++u32_Counter) {
    ...
    if ((pc_Prot != NULL) && (pc_Prot->e_Type == oe_ComProtocol) &&
        (ou32_InterfaceIndex < pc_Prot->c_ComMessages.size()) &&
        (oq_ComProtocolUsedByInterface ==
         pc_Prot->c_ComMessages[ou32_InterfaceIndex]
             .q_IsComProtocolUsedByInterface)) {
      // Check Rx and RPDOs
      mh_CheckErrorCanProtocolDirection(
          pc_Prot->c_ComMessages[ou32_InterfaceIndex].c_RxMessages,
          oe_ComProtocol, orq_InvalidMaxRxSignalCount, orq_InvalidCoRxPdoCount,
          orq_InvalidMinSignalCount);

      // Check Tx and TPDOs
      mh_CheckErrorCanProtocolDirection(
          pc_Prot->c_ComMessages[ou32_InterfaceIndex].c_TxMessages,
          oe_ComProtocol, orq_InvalidMaxTxSignalCount, orq_InvalidCoTxPdoCount,
          orq_InvalidMinSignalCount);
    }
  }
}
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check whether the number of lists and elements is within supported
   range

   Goes through the data pool lists and element and check whether the number are
   within the supported range. The valid range is limited by the openSYDE
   communication protocol.

   \param[in]   ou32_DataPoolIndex           Data pool index
   \param[out]  orq_TooFewListsOrElements    true: number of lists or elements
   of at least one list too low \param[out]  orq_TooManyListsOrElements   true:
   number of lists or elements of at least one list too high
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::CheckErrorDataPoolNumListsAndElements(
    const uint32_t ou32_DataPoolIndex, bool &orq_TooFewListsOrElements,
    bool &orq_TooManyListsOrElements) const {
  orq_TooFewListsOrElements = true;
  orq_TooManyListsOrElements = true;
  if (ou32_DataPoolIndex < this->c_DataPools.size()) {
    const C_OscNodeDataPool &rc_CheckedDataPool =
        this->c_DataPools[ou32_DataPoolIndex];

    orq_TooManyListsOrElements = false;

    // number of lists ok ?
    if (rc_CheckedDataPool.c_Lists.size() >
        hu32_MAX_NUMBER_OF_LISTS_PER_DATA_POOL) {
      // no
      orq_TooManyListsOrElements = true;
    } else {
      // number of elements ok ?
      for (uint32_t u32_Index = 0U;
           u32_Index < rc_CheckedDataPool.c_Lists.size(); u32_Index++) {
        if (rc_CheckedDataPool.c_Lists[u32_Index].c_Elements.size() >
            hu32_MAX_NUMBER_OF_ELEMENTS_PER_LIST) {
          orq_TooManyListsOrElements = true;
          break;
        }
      }
    ...
    orq_TooFewListsOrElements =
        this->m_CheckErrorTooFewElements(ou32_DataPoolIndex);
  }
}
...
/*! \brief  Check error for data pool
   ...
   * naming conflict: multiple elements within one list have the same name
   ...
   \param[out]  opq_IsErrorInListOrMessage   true: conflict in any list or
   message was detected \param[out]  opq_TooFewListsOrElements    true: too few
   lists or elements \param[out]  opq_TooManyListsOrElements   true: too many
   lists or elements \param[out]  opc_InvalidListIndices       Optional storage
   for list of invalid list indices (see opq_IsErrorInListOrMessage) If COMM:
   interface index Else: list index
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::CheckErrorDataPool(
    const uint32_t ou32_DataPoolIndex, bool *const opq_NameConflict,
    bool *const opq_NameInvalid, bool *const opq_IsErrorInListOrMessage,
    bool *const opq_TooFewListsOrElements,
    bool *const opq_TooManyListsOrElements,
  ...
  if (ou32_DataPoolIndex < this->c_DataPools.size()) {
    ...
    if (opq_NameConflict != NULL) {
      *opq_NameConflict = false;
      for (uint32_t u32_ItElement = 0; u32_ItElement < this->c_DataPools.size();
           ++u32_ItElement) {
        if (u32_ItElement != ou32_DataPoolIndex) {
          const C_OscNodeDataPool &rc_CurrentDataPool =
              this->c_DataPools[u32_ItElement];
          if (rc_CheckedDataPool.c_Name.toLower() ==
              rc_CurrentDataPool.c_Name.toLower()) {
    ...
    if (opq_IsErrorInListOrMessage != NULL) {
      ...
      *opq_IsErrorInListOrMessage = false;
      ...
      if ((opq_TooManyListsOrElements != NULL) ||
          (opq_TooFewListsOrElements != NULL)) {
        bool q_TooFew;
        bool q_TooMany;
        CheckErrorDataPoolNumListsAndElements(ou32_DataPoolIndex, q_TooFew,
                                              q_TooMany);
      ...
      if (rc_CheckedDataPool.e_Type == C_OscNodeDataPool::eCOM) {
        ...
        if (pc_Protocol != NULL) {
          ...
          for (uint32_t u32_ItContainer = 0;
               u32_ItContainer < pc_Protocol->c_ComMessages.size();
               ++u32_ItContainer) {
            uint32_t u32_TxListIndex;
            uint32_t u32_RxListIndex;
            if ((C_OscCanProtocol::h_GetComListIndex(
                     rc_CheckedDataPool, u32_ItContainer, true,
                     u32_TxListIndex) == C_NO_ERR) &&
                (C_OscCanProtocol::h_GetComListIndex(
                     rc_CheckedDataPool, u32_ItContainer, false,
                     u32_RxListIndex) == C_NO_ERR)) {
              // Parameter oq_CanOpenPdoSyncValid is true in case of no CANopen
              // protocol to avoid a check for this scenario
              bool q_CanOpenPdoSyncValid = true;
              // Get Hash for all relevant data
              const uint32_t u32_HashCon =
                  this->m_GetContainerHash(ou32_DataPoolIndex, u32_ItContainer);
              const uint32_t u32_HashTx =
                  this->m_GetListHash(ou32_DataPoolIndex, u32_TxListIndex);
              const uint32_t u32_HashRx =
                  this->m_GetListHash(ou32_DataPoolIndex, u32_RxListIndex);
              ...
              if (pc_Protocol->e_Type == C_OscCanProtocol::eCAN_OPEN) {
                ...
                Q_ASSERT(u32_ItContainer <
                         this->c_Properties.c_ComInterfaces.size());
                if (u32_ItContainer <
                    this->c_Properties.c_ComInterfaces.size()) {
                   const QHash<uint8_t,
                                  C_OscCanOpenManagerInfo>::const_iterator
                       c_ItCoManager = this->c_CanOpenManagers.find(
                           this->c_Properties.c_ComInterfaces[u32_ItContainer]
                               .u8_InterfaceNumber);

                   // Check if it is the CANopen Manager of this bus
                   if (c_ItCoManager != this->c_CanOpenManagers.end()) {
                     uint32_t u32_HashManager;
                     const C_OscCanOpenManagerInfo &rc_Manager =
                         c_ItCoManager.value();
                    // When PDO SYNC message is produces, a configured TX method
                    // with PDO SYNC is valid
                    q_CanOpenPdoSyncValid = rc_Manager.q_ProduceSyncMessage;

                    // Due to impacts on the error detection in case of changes
                    // of CANopen Manger configuration, the hash of the CANopen
                    // Manager is relevant too
              ...
              c_It = hc_PreviousResults.constFind(c_Hashes);
              if (c_It == hc_PreviousResults.constEnd()) {
                if ((u32_TxListIndex < rc_CheckedDataPool.c_Lists.size()) &&
                  ...
                  const C_OscCanMessageContainer &rc_Container =
                      pc_Protocol->c_ComMessages[u32_ItContainer];
                  ...
                  const bool q_Error = rc_Container.CheckLocalError(
                      ...
                      C_OscCanProtocol::h_GetCanMessageSignalGapsValid(
                          pc_Protocol->e_Type),
                      C_OscCanProtocol::
                          h_GetCanMessageSignalByteAlignmentRequired(
                              pc_Protocol->e_Type),
                ...
                // Do not reset error
                *opq_IsErrorInListOrMessage =
                    (*opq_IsErrorInListOrMessage) || (c_It.value());
                if ((opc_InvalidListIndices != NULL) && (c_It.value())) {
                  opc_InvalidListIndices->push_back(u32_ItContainer);
                }
              }
            } else {
        ...
        for (uint32_t u32_ItList = 0;
             ...
             ((*opq_IsErrorInListOrMessage == false) ||
              (opc_InvalidListIndices != NULL));
             ++u32_ItList) {
          // Overarching checks
          const C_OscNodeDataPoolList &rc_List =
              rc_CheckedDataPool.c_Lists[u32_ItList];
          const QMap<QString, uint32_t>::const_iterator c_ItList =
              c_PreviousNames.find(rc_List.c_Name.toLower());
            ...
            *opq_IsErrorInListOrMessage = true;
            if (opc_InvalidListIndices != NULL) {
              ...
              for (uint32_t u32_It = 0UL;
                   u32_It < opc_InvalidListIndices->size(); ++u32_It) {
                if ((*opc_InvalidListIndices)[u32_It] == c_ItList.value()) {
                  q_Added = true;
                  break;
                }
              }
              ...
              if (q_Added == false) {
                opc_InvalidListIndices->push_back(c_ItList.value());
              }
              // Add itself
            ...
            const uint32_t u32_Hash =
                this->m_GetListHash(ou32_DataPoolIndex, u32_ItList);
            // Check if check was already performed in the past
            const QMap<uint32_t, bool>::const_iterator c_ItErr =
                hc_PreviousResults.find(u32_Hash);
            // Append new name
            ...
            if (c_ItErr == hc_PreviousResults.end()) {
              ...
              rc_CheckedDataPool.CheckErrorList(
                  u32_ItList, NULL, &q_NameInvalid, &q_UsageInvalid,
                  &q_OutOfDataPool, &q_DataSetsInvalid, &q_ElementsInvalid,
                  NULL, NULL);
              ...
              if (((((q_NameInvalid == true) || (q_UsageInvalid == true)) ||
                    (q_ElementsInvalid == true)) ||
                   (q_OutOfDataPool == true)) ||
                  (q_DataSetsInvalid == true)) {
                *opq_IsErrorInListOrMessage = true;
                if (opc_InvalidListIndices != NULL) {
                  opc_InvalidListIndices->push_back(u32_ItList);
                }
              ...
              // Do not reset error
              *opq_IsErrorInListOrMessage =
                  (*opq_IsErrorInListOrMessage) || (c_ItErr.value());
              if ((opc_InvalidListIndices != NULL) && (c_ItErr.value())) {
                opc_InvalidListIndices->push_back(u32_ItList);
              }
            }
          }
...
/*! \brief  Check if message id valid
   ...
   \param[in]   ope_SkipComProtocol       Optional parameter to skip one index
                                          (Only used if all other optional skip
   parameters are used as well) (Use-case: skip current message to avoid
   conflict with itself) \param[in]   opu32_SkipInterfaceIndex  Optional
   parameter to skip one index (Only used if all other optional skip parameters
   are used as well) (Use-case: skip current message to avoid conflict with
   itself) \param[in]   opq_SkipMessageIsTxFlag   Optional parameter to skip one
   index (Only used if all other optional skip parameters are used as well)
                                          (Use-case: skip current message to
   avoid conflict with itself) \param[in]   opu32_SkipMessageIndex    Optional
   parameter to skip one index (Only used if all other optional skip parameters
   are used as well) (Use-case: skip current message to avoid conflict with
   itself)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::CheckMessageId(
    const uint32_t ou32_InterfaceIndex,
  ...
  m_GetAllMessages(ou32_InterfaceIndex, c_Messages, ope_SkipComProtocol,
                   opu32_SkipInterfaceIndex, opq_SkipMessageIsTxFlag,
                   opu32_SkipMessageIndex);
  orq_Valid = true;
  for (uint32_t u32_ItMessage = 0; u32_ItMessage < c_Messages.size();
       ++u32_ItMessage) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Check if message name valid (invalid if duplicate or invalid for use
   as variable)

   \param[in]   ou32_InterfaceIndex       Interface index
   \param[in]   orc_MessageName           Message name
   \param[out]  orq_Valid                 Flag if valid (invalid if duplicate or
   invalid for use as variable) \param[in]   ope_SkipComProtocol       Optional
   parameter to skip one index (Only used if all other optional skip parameters
   are used as well) (Use-case: skip current message to avoid conflict with
   itself) \param[in]   opu32_SkipInterfaceIndex  Optional parameter to skip one
   index (Only used if all other optional skip parameters are used as well)
                                          (Use-case: skip current message to
   avoid conflict with itself) \param[in]   opq_SkipMessageIsTxFlag   Optional
   parameter to skip one index (Only used if all other optional skip parameters
   are used as well) (Use-case: skip current message to avoid conflict with
   itself) \param[in]   opu32_SkipMessageIndex    Optional parameter to skip one
   index (Only used if all other optional skip parameters are used as well)
                                          (Use-case: skip current message to
   avoid conflict with itself)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::CheckMessageName(
    const uint32_t ou32_InterfaceIndex, const QString &orc_MessageName,
...
/*! \brief  Check application process ID valid
   ...
   \param[out]  orq_Valid              Valid check result (should only be used
   if the function returned C_NO_ERR)
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNode::CheckApplicationProcessIdValid(const uint32_t ou32_ApplicationIndex,
                                          bool &orq_Valid) const {
  int32_t s32_Retval = C_NO_ERR;

  orq_Valid = true;
  if (ou32_ApplicationIndex < this->c_Applications.size()) {
    const C_OscNodeApplication &rc_CheckedApplication =
        this->c_Applications[ou32_ApplicationIndex];
    // Only check programmable applications
    if (rc_CheckedApplication.e_Type ==
        C_OscNodeApplication::ePROGRAMMABLE_APPLICATION) {
      for (uint32_t u32_ItApp = 0; u32_ItApp < this->c_Applications.size();
           ++u32_ItApp) {
        if (u32_ItApp != ou32_ApplicationIndex) {
          const C_OscNodeApplication &rc_Application =
              this->c_Applications[u32_ItApp];
          // Only check programmable applications
          if (rc_Application.e_Type ==
              C_OscNodeApplication::ePROGRAMMABLE_APPLICATION) {
            if (rc_Application.u8_ProcessId ==
                rc_CheckedApplication.u8_ProcessId) {
              orq_Valid = false;
            }
          }
        }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::CheckHalcConfigValid(
    bool *const opq_ConfigInvalid,
    QList<uint32_t> *const opc_InvalidDomainIndices) const {
  this->c_HalcConfig.CheckConfigValid(opq_ConfigInvalid,
                                      opc_InvalidDomainIndices);
}
...
bool C_OscNode::IsAnyUpdateAvailable(void) const {
  ...
  if (this->pc_DeviceDefinition != NULL) {
    for (uint32_t u32_ItInterface = 0;
         (u32_ItInterface < this->c_Properties.c_ComInterfaces.size()) &&
         (q_Retval == false);
         ++u32_ItInterface) {
      const C_OscNodeComInterfaceSettings &rc_CurInterface =
          this->c_Properties.c_ComInterfaces[u32_ItInterface];
      Q_ASSERT(this->u32_SubDeviceIndex <
               this->pc_DeviceDefinition->c_SubDevices.size());
      if (this->pc_DeviceDefinition->c_SubDevices[this->u32_SubDeviceIndex]
              .IsUpdateAvailable(rc_CurInterface.e_InterfaceType) == true) {
        q_Retval = true;
      }
    }
  }
...
bool C_OscNode::IsRoutingAvailable(const C_OscSystemBus::E_Type oe_Type) const {
  ...
  if ((this->pc_DeviceDefinition != NULL) &&
      (this->c_Properties.e_DiagnosticServer ==
       C_OscNodeProperties::eDS_OPEN_SYDE)) {
    Q_ASSERT(this->u32_SubDeviceIndex <
             this->pc_DeviceDefinition->c_SubDevices.size());
    q_Return = this->pc_DeviceDefinition->c_SubDevices[this->u32_SubDeviceIndex]
                   .IsRoutingAvailable(oe_Type);
  }
...
void C_OscNode::RecalculateAddress(void) {
  ...
  for (uint32_t u32_ItDataPool = 0UL; u32_ItDataPool < this->c_DataPools.size();
       ++u32_ItDataPool) {
    ...
    if ((rc_DataPool.e_Type == C_OscNodeDataPool::eNVM) ||
      ...
      if (this->q_DatapoolAutoNvmStartAddress == true) {
        // Updating the Datapool start address only if the auto mode is
        // activated This configuration affects the Datapool start address only.
        // The lists and elements are calculated automatically always
...
uint32_t C_OscNode::CountAllLocalMessages(void) const {
  ...
  for (uint32_t u32_ItProt = 0UL; u32_ItProt < this->c_ComProtocols.size();
       ++u32_ItProt) {
    const C_OscCanProtocol &rc_Prot = this->c_ComProtocols[u32_ItProt];
    for (uint32_t u32_ItCont = 0UL; u32_ItCont < rc_Prot.c_ComMessages.size();
         ++u32_ItCont) {
      const C_OscCanMessageContainer &rc_Cont =
          rc_Prot.c_ComMessages[u32_ItCont];
...
void C_OscNode::HandleNameMaxCharLimit(
  ...
  this->c_HalcConfig.HandleNameMaxCharLimit(ou32_NameMaxCharLimit,
                                            opc_ChangedItems);
  for (uint32_t u32_ItDatablock = 0UL;
       u32_ItDatablock < this->c_Applications.size(); ++u32_ItDatablock) {
    ...
    C_OscSystemNameMaxCharLimitChangeReportItem::h_HandleNameMaxCharLimitItem(
        ou32_NameMaxCharLimit, "node-datablock-name", rc_Datablock.c_Name,
        opc_ChangedItems);
  }
}
...
/*! \brief  Utility: get specific data pool element
   ...
   Note that the validity of the returned pointer is only guaranteed as long as
   the layout of the data pool is not changed.
   ...
   \return
   Pointer to specified data pool element; NULL if one of the parameters is out
   of range
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolListElement *
C_OscNode::GetDataPoolListElement(const uint32_t ou32_DataPoolIndex,
                                  const uint32_t ou32_ListIndex,
  ...
  if ((ou32_DataPoolIndex < this->c_DataPools.size()) &&
      ...
      (ou32_ElementIndex < this->c_DataPools[ou32_DataPoolIndex]
                               .c_Lists[ou32_ListIndex]
                               .c_Elements.size())) {
    pc_Element = &this->c_DataPools[ou32_DataPoolIndex]
                      .c_Lists[ou32_ListIndex]
                      .c_Elements[ou32_ElementIndex];
  }
  return pc_Element;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::m_GetAllMessages(
    const uint32_t ou32_InterfaceIndex,
  ...
  // L2
  m_AppendAllProtocolMessages(ou32_InterfaceIndex, C_OscCanProtocol::eLAYER2,
                              orc_Messages, ope_SkipComProtocol,
                              opu32_SkipInterfaceIndex, opq_SkipMessageIsTxFlag,
                              opu32_SkipMessageIndex);
  // ECeS
  m_AppendAllProtocolMessages(ou32_InterfaceIndex, C_OscCanProtocol::eECES,
                              orc_Messages, ope_SkipComProtocol,
                              opu32_SkipInterfaceIndex, opq_SkipMessageIsTxFlag,
                              opu32_SkipMessageIndex);
  // ECoS
  m_AppendAllProtocolMessages(ou32_InterfaceIndex,
                              C_OscCanProtocol::eCAN_OPEN_SAFETY, orc_Messages,
                              ope_SkipComProtocol, opu32_SkipInterfaceIndex,
                              opq_SkipMessageIsTxFlag, opu32_SkipMessageIndex);
  // CAN open manager
  m_AppendAllProtocolMessages(ou32_InterfaceIndex, C_OscCanProtocol::eCAN_OPEN,
                              orc_Messages, ope_SkipComProtocol,
                              opu32_SkipInterfaceIndex, opq_SkipMessageIsTxFlag,
                              opu32_SkipMessageIndex);

  // J1939
  m_AppendAllProtocolMessages(ou32_InterfaceIndex, C_OscCanProtocol::eJ1939,
                              orc_Messages, ope_SkipComProtocol,
                              opu32_SkipInterfaceIndex, opq_SkipMessageIsTxFlag,
                              opu32_SkipMessageIndex);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::m_AppendAllProtocolMessages(
    const uint32_t ou32_InterfaceIndex,
    const C_OscCanProtocol::E_Type oe_ComProtocol,
    QList<const C_OscCanMessage *> &orc_Messages,
    const C_OscCanProtocol::E_Type *const ope_SkipComProtocol,
      ...
      if (ou32_InterfaceIndex < rc_CanProtocol.c_ComMessages.size()) {
        ...
        for (uint8_t u8_ItToggle = 0; u8_ItToggle < 2; ++u8_ItToggle) {
          ...
          for (uint32_t u32_ItMessage = 0; u32_ItMessage < rc_Messages.size();
            ...
            bool q_Skip = false;
            if ((((ope_SkipComProtocol != NULL) &&
                  (opu32_SkipInterfaceIndex != NULL)) &&
                 (opq_SkipMessageIsTxFlag != NULL)) &&
                (opu32_SkipMessageIndex != NULL)) {
              // Match
              if ((((*ope_SkipComProtocol == oe_ComProtocol) &&
                    (*opu32_SkipMessageIndex == u32_ItMessage)) &&
                   (*opu32_SkipInterfaceIndex == ou32_InterfaceIndex)) &&
                  (((*opq_SkipMessageIsTxFlag == true) && (u8_ItToggle == 0)) ||
                   ((*opq_SkipMessageIsTxFlag == false) &&
                    (u8_ItToggle == 1)))) {
                q_Skip = true;
              }
            }
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscNode::m_GetListHash(const uint32_t ou32_DataPoolIndex,
                                  const uint32_t ou32_ListIndex) const {
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscNode::m_GetContainerHash(const uint32_t ou32_DataPoolIndex,
                              const uint32_t ou32_ContainerIndex) const {
  uint32_t u32_Retval = 0xFFFFFFFFUL;
  const C_OscCanProtocol *const pc_Protocol =
      this->GetRelatedCanProtocolConst(ou32_DataPoolIndex);
  ...
  if (pc_Protocol != NULL) {
    if (ou32_ContainerIndex < pc_Protocol->c_ComMessages.size()) {
      const C_OscCanMessageContainer &rc_Container =
          pc_Protocol->c_ComMessages[ou32_ContainerIndex];
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Checks a specific CAN protocol direction message container for
   errors
   ...
   \param[in]   orc_Messages                 All messages of one specific
   direction of one specific protocol \param[in]   oe_ComProtocol Communication
   protocol \param[out]  orq_InvalidMaxSignalCount    The number of signals of
   this messages is too high \param[out]  orq_InvalidCoPdoCount        The
   number of active PDOs (CANopen Manager viewpoint) of CANopen is to high
   \param[out]  orq_InvalidMinSignalCount    The number of signals of this
   messages is too low
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNode::mh_CheckErrorCanProtocolDirection(
  ...
  for (u32_MessageCounter = 0U; u32_MessageCounter < orc_Messages.size();
       ++u32_MessageCounter) {
    const C_OscCanMessage &rc_Msg = orc_Messages[u32_MessageCounter];
    // Check signal count, which is relevant for all protocol types
    // For each direction and interface is a Datapool list associated. The
    // maximum number of list entries is the limit for the count of signals
    ...
    if (oe_ComProtocol == C_OscCanProtocol::eCAN_OPEN) {
      if (rc_Msg.q_CanOpenManagerMessageActive == true) {
        ...
        if (u32_ActivePdoCounter >
            hu32_MAX_NUMBER_OF_ACTIVE_PDO_PER_DIRECTION) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscNode::m_CheckErrorTooFewElements(
    const uint32_t ou32_DataPoolIndex) const {
