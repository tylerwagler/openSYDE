...
public:
  ...
  {
    eBINARY = 0, ///< Type for address based flash access devices. E.g.: Intel
                 ///< Hex file.
    ePROGRAMMABLE_APPLICATION, ///< Type for user programmable applications,
                               ///< i.e. with source code generation support
    ePARAMETER_SET_HALC ///< Type for parameter set for NVM based hardware
                        ///< configuration, i.e. with SYDE_PSI
                        // file generation support
  };

  static QString
  h_ApplicationToString(const C_OscNodeApplication::E_Type &ore_Application);
  static void h_StringToApplication(const QString &orc_String,
  ...
  QString c_CodeGeneratorPath; ///< Path to file generator
                               //(if relative it is meant as relative to
                               //openSYDE.exe)
  QString c_GeneratePath; ///< Path to generate sources for this application in
                          //(if relative it is meant as relative to data block
                          //project)
