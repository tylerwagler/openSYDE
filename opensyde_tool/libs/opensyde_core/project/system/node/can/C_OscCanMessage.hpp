...
public:
  ...
  enum E_TxMethodType ///< Transmission trigger type
  {
    ...
    eTX_METHOD_ON_CHANGE,       ///< Transmission triggered by data change (with
                                ///< min/max time window)
    eTX_METHOD_ON_EVENT,        ///< Transmission triggered by application event
    eTX_METHOD_CAN_OPEN_TYPE_0, ///< Transmission ID 0 defined by CAN open
                                ///< standard
    eTX_METHOD_CAN_OPEN_TYPE_1_TO_240, ///< Transmission ID 1-240 defined by CAN
                                       ///< open standard,
    ///< exact value identified by other variable
    eTX_METHOD_CAN_OPEN_TYPE_254, ///< Transmission ID 254 defined by CAN open
                                  ///< standard
    eTX_METHOD_CAN_OPEN_TYPE_255  ///< Transmission ID 255 defined by CAN open
                                  ///< standard
  };

  bool operator!=(const C_OscCanMessage &orc_Cmp) const;

  void CalcHash(uint32_t &oru32_HashValue,
                const bool oq_R20Compatible = false) const;
  bool CheckErrorSignal(const C_OscNodeDataPoolList *const opc_List,
  ...
  static bool
  h_ContainsMultiplexer(const QList<C_OscCanSignal> &orc_Signals,
                        uint32_t *const opu32_MultiplexerIndex = NULL);
  void GetMultiplexerValues(QSet<uint16_t> &orc_Values) const;

  QString c_Name;     ///< Message name
  QString c_Comment;  ///< Message comment
  ...
  uint16_t u16_DelayTimeMs; ///< Minimum time between transmission in ms. (aka
                            ///< not earlier than)
  ...
  C_OscCanInterfaceId
      c_CanOpenManagerOwnerNodeIndex; ///< CANopen manager specific information
                                      ///< about
  ///< owner node:
  ///< TX-PDO: sender
  ///< RX-PDO: receiver
  bool q_CanOpenManagerCobIdIncludesNodeId; ///< CANopen manager specific
                                            ///< information about how to
                                            ///< calculate the
  ///< COB-ID value
  uint32_t
      u32_CanOpenManagerCobIdOffset;  ///< CANopen manager specific information
                                      ///< about the COB-ID offset value
  bool q_CanOpenManagerMessageActive; ///< CANopen manager specific information
                                      ///< about if the message is considered
  ///< active
  uint16_t u16_CanOpenManagerPdoIndex; ///< CANopen manager specific information
                                       ///< about which PDO index this
  /// message originated from
  uint8_t u8_CanOpenTxMethodAdditionalInfo; ///< CANopen manager specific
                                            ///< information about which exact
