...
C_OscCanOpenManagerInfo::C_OscCanOpenManagerInfo(void)
    : q_UseOpenSydeNodeId(true), u8_NodeIdValue(0), u16_GlobalSdoTimeoutMs(100),
      q_AutostartCanOpenManager(true), q_StartDevices(true),
      q_NmtStartAll(false), e_NmtErrorBehaviour(eRESTART_ALL_DEVICES),
      q_EnableHeartbeatProducing(true), u16_HeartbeatProducerTimeMs(100),
      q_ProduceSyncMessage(false), u32_SyncCyclePeriodUs(20000UL),
...
/*! \brief   Check for errors of CANopen Manager
   ...
   \param[out]     opq_CoNodeIdConflict           Duplicate error with CANopen
   Node ID detected \param[out]     opq_CoManagerNodeIdInvalid     Invalid error
   with CANopen Node ID of Manager detected \param[out]
   opq_CoDevicesNodeIdInvalid     Invalid error with CANopen Node ID of at least
   one Device detected \param[out]     opq_HearbeatTimeInvalid        Error with
   hearbeat time of Manager detected \param[in] oq_CheckDeviceToDeviceErrors
   true: run error checks between devices and between manager and devices false:
   run error checks between manager and device only
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanOpenManagerInfo::CheckErrorManager(
    bool *const opq_CoNodeIdConflict, bool *const opq_CoManagerNodeIdInvalid,
  ...
  if (opq_CoManagerNodeIdInvalid != NULL) {
    ...
    if ((this->u8_NodeIdValue > 0U) && (this->u8_NodeIdValue <= 127)) {
  ...
  if ((opq_CoNodeIdConflict != NULL) || (opq_CoDevicesNodeIdInvalid != NULL)) {
    ...
    // Using temporary pointers to beware the optional character of the output
    // parameters
    ...
    // Check all devices
    for (c_ItDevice = this->c_CanOpenDevices.begin();
         c_ItDevice != this->c_CanOpenDevices.end(); ++c_ItDevice) {
       Q_ASSERT(this->CheckErrorDeviceCoNodeId(
                    c_ItDevice.key(), pq_TempCoNodeIdConflict,
                    pq_TempCoDevicesNodeIdInvalid,
                    oq_CheckDeviceToDeviceErrors) == C_NO_ERR);

      // Transfer to the output pointer only true result to not overwrite
      // previous detections
      if ((opq_CoNodeIdConflict != NULL) && (q_TempCoNodeIdConflict == true)) {
  ...
  if (opq_HearbeatTimeInvalid != NULL) {
    *opq_HearbeatTimeInvalid = false;
    ...
    for (c_ItDevice = this->c_CanOpenDevices.begin();
         c_ItDevice != this->c_CanOpenDevices.end(); ++c_ItDevice) {
       Q_ASSERT(this->CheckErrorDeviceHeartbeat(
                    c_ItDevice.key(), opq_HearbeatTimeInvalid) == C_NO_ERR);
...
/*! \brief   Checks if a device has an consumer heartbeat error
   ...
   \param[out]     opq_CoNodeIdConflict           Duplicate error with CANopen
   Node ID detected \param[out]     opq_CoNodeIdInvalid            Invalid error
   with CANopen Node ID detected (only 1 - 127 are allowed) \param[in]
   oq_CheckDeviceToDeviceErrors   true: run error checks between devices and
   between manager and devices false: run error checks between manager and
   device only
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanOpenManagerInfo::CheckErrorDeviceCoNodeId(
    const C_OscCanInterfaceId &orc_DeviceId, bool *const opq_CoNodeIdConflict,
   ...
   if (c_ItDevice != this->c_CanOpenDevices.end()) {
    const C_OscCanOpenManagerDeviceInfo &rc_DevInfo = c_ItDevice.value();
      ...
      if (this->u8_NodeIdValue == rc_DevInfo.u8_NodeIdValue) {
        ...
        *opq_CoNodeIdConflict = true;
         ...
         QHash<C_OscCanInterfaceId,
                  C_OscCanOpenManagerDeviceInfo>::const_iterator c_ItDeviceComp;

        // Compare with all other devices
        for (c_ItDeviceComp = this->c_CanOpenDevices.begin();
             c_ItDeviceComp != this->c_CanOpenDevices.end(); ++c_ItDeviceComp) {
          if (c_ItDevice != c_ItDeviceComp) {
            const C_OscCanOpenManagerDeviceInfo &rc_DevInfoCompare =
                c_ItDeviceComp.value();
...
/*! \brief   Checks if a device has an consumer heartbeat error
   ...
   \param[in]       orc_DeviceId              CANopen device ID
   \param[out]      opq_HearbeatTimeInvalid   Error with Hearbeat time of
   Manager detected
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanOpenManagerInfo::CheckErrorDeviceHeartbeat(
  ...
  if ((c_ItDevice != this->c_CanOpenDevices.end()) &&
    ...
    *opq_HearbeatTimeInvalid = false;
    ...
    // If the heartbeat producing of the manager and consuming of the device is
    // enabled the consumer time must be higher than the producer time to valid
    if ((this->q_EnableHeartbeatProducing == true) &&
        (rc_DevInfo.q_EnableHeartbeatConsuming == true) &&
        (rc_DevInfo.u16_HeartbeatConsumerTimeMs <=
         this->u16_HeartbeatProducerTimeMs)) {
