...
public:
  ...
  enum E_NmtErrorBehaviourType ///< type of NMT error behavior
  {
    ...
    eRESTART_FAILURE_DEVICE, ///< Trigger restart failure CANopen Device on
                             ///< error
    eSTOP_ALL_DEVICES        ///< Trigger stop all CANopen Devices on error
  };

  void CalcHash(uint32_t &oru32_HashValue) const;
  void CheckErrorManager(bool *const opq_CoNodeIdConflict,
  ...
  int32_t
  CheckErrorDeviceCoNodeId(const C_OscCanInterfaceId &orc_DeviceId,
  ...
  uint16_t u16_GlobalSdoTimeoutMs; ///< Timeout for any SDO access from manger
                                   ///< to any device
  bool q_AutostartCanOpenManager;  ///< Autostart CANopen manager as soon as all
                                   ///< required devices are online
  bool q_StartDevices; ///< Setting if CANopen manager starts devices, or each
                       ///< own device application
                       // handles start
  bool q_NmtStartAll; ///< Only if option start devices active: Starting devices
                      ///< using "NMT Start All"
                      // command

  E_NmtErrorBehaviourType
      e_NmtErrorBehaviour;              ///< Error handling selection for NMT
  bool q_EnableHeartbeatProducing;      ///< Enable CANopen heartbeat
  uint16_t u16_HeartbeatProducerTimeMs; ///< Enable heartbeat frequency in
                                        ///< milliseconds
