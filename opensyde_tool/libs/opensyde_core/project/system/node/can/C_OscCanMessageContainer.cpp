...
void C_OscCanMessageContainer::CalcHash(uint32_t &oru32_HashValue) const {
  ...
  stw::scl::C_SclChecksums::CalcCRC32(
      &this->q_IsComProtocolUsedByInterface,
      sizeof(this->q_IsComProtocolUsedByInterface), oru32_HashValue);

  for (uint32_t u32_Counter = 0U; u32_Counter < this->c_RxMessages.size();
       ++u32_Counter) {
    this->c_RxMessages[u32_Counter].CalcHash(oru32_HashValue, false);
  }

  for (uint32_t u32_Counter = 0U; u32_Counter < this->c_TxMessages.size();
       ++u32_Counter) {
    this->c_TxMessages[u32_Counter].CalcHash(oru32_HashValue, false);
  }

  // Force CRC change if single Rx message is moved to empty Tx (previously no
  // change in CRC calculation)
...
/*! \brief   Recalculate all data element indices
   ...
   Based on assumption data elements are sorted in data pool by message and
   signal
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanMessageContainer::ReCalcDataElementIndices(void) {
  ...
  for (uint8_t u8_Toggle = 0; u8_Toggle < 2; ++u8_Toggle) {
    ...
    // Check consistency
    for (uint32_t u32_ItMessage = 0; u32_ItMessage < pc_ComMessages->size();
         ++u32_ItMessage) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_OscCanMessageContainer::GetMessageSignalDataStartIndex(
  ...
  // Check consistency
  for (uint32_t u32_ItMessage = 0; (u32_ItMessage < rc_ComMessages.size()) &&
                                   (u32_ItMessage < oru32_MessageIndex);
       ++u32_ItMessage) {
    const C_OscCanMessage &rc_Message = rc_ComMessages[u32_ItMessage];
    for (uint32_t u32_ItSignal = 0; u32_ItSignal < rc_Message.c_Signals.size();
         ++u32_ItSignal) {
      ++u32_Retval;
    }
  }
...
/*! \brief   Request reference to either list
   ...
   \param[in]  orq_IsTx    Flag if tx message vector was requested (else rx
   message vector is output)
...
/*! \brief   Check message error
   ...
   IMPORTANT: some error types (e.g. name & id conflict) can only be checked via
   system definition

   \param[in]   opc_List                              Node data pool list
   containing signal data (Optional as it is only required by some checks)
   \param[in]   oru32_MessageIndex                    Message index
   \param[in]   orq_IsTx                              Flag if message is tx
   (else rx)
   \param[out]  opq_NameConflict                      Name conflict
   \param[out]  opq_NameInvalid                       Name not usable as
   variable
   \param[out]  opq_DelayTimeInvalid                  Delay time is invalid
   \param[out]  opq_IdConflict                        Id conflict
   \param[out]  opq_IdInvalid                         Id out of 11 bit / 29 bit
   range
   \param[out]  opq_SignalInvalid                     An error found for a
   signal
   \param[out]  opq_NoSignalsInvalid                  An error found for having
   no signals
   \param[out]  opq_TxMethodInvalid                   An error found for having
   a TX method which is not valid
   \param[in]   ou32_CanMessageValidSignalsDlcOffset  CAN message DLC offset for
   valid signal range check
   \param[in]   oq_CanMessageSignalGapsValid          Flag if gaps between
   signals are valid or handled as errors
   \param[in]   oq_ByteAlignmentRequired              Flag if byte alignment is
   required and if the signal does not fit into byte alignment it will be
   handled as error
   \param[in]   oq_SignalsRequired                    Flag if signals are
   required for a message
   \param[in]   oq_CanOpenPdoSyncValid                Flag if the CANopen TX
   methods with PDO SYNC are valid or if no check is necessary (other protocols)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanMessageContainer::CheckMessageLocalError(
  ...
  if (oru32_MessageIndex < rc_Messages.size()) {
      ...
      *opq_NameConflict = false;
      ...
      if (rc_Message.q_CanOpenManagerMessageActive == true) {
        ...
        for (uint32_t u32_ItMessage = 0;
             (u32_ItMessage < this->c_TxMessages.size()) &&
             (*opq_NameConflict == false);
             ++u32_ItMessage) {
          bool q_Skip = false;
          ...
          if (q_Skip == false) {
            const C_OscCanMessage &rc_CurrentMessage =
                this->c_TxMessages[u32_ItMessage];
            if (rc_Message.c_Name.toLower() ==
                rc_CurrentMessage.c_Name.toLower()) {
        ...
        // Rx
        for (uint32_t u32_ItMessage = 0;
             (u32_ItMessage < this->c_RxMessages.size()) &&
             (*opq_NameConflict == false);
             ++u32_ItMessage) {
          bool q_Skip = false;
          ...
          if (q_Skip == false) {
            const C_OscCanMessage &rc_CurrentMessage =
                this->c_RxMessages[u32_ItMessage];
            if (rc_Message.c_Name.toLower() ==
                rc_CurrentMessage.c_Name.toLower()) {
              *opq_NameConflict = true;
            }
      ...
      *opq_DelayTimeInvalid = false;
      ...
      if (rc_Message.q_CanOpenManagerMessageActive == true) {
        if ((rc_Message.e_TxMethod == C_OscCanMessage::eTX_METHOD_ON_CHANGE) ||
            (rc_Message.e_TxMethod ==
             C_OscCanMessage::eTX_METHOD_CAN_OPEN_TYPE_254) ||
            (rc_Message.e_TxMethod ==
             C_OscCanMessage::eTX_METHOD_CAN_OPEN_TYPE_255)) {
          if (static_cast<uint32_t>(rc_Message.u16_DelayTimeMs) >
              rc_Message.u32_CycleTimeMs) {
    ...
    if (opq_SignalInvalid != NULL) {
      *opq_SignalInvalid = false;
      ...
      if (rc_Message.q_CanOpenManagerMessageActive == true) {
        for (uint32_t u32_ItSignal = 0;
             (u32_ItSignal < rc_Message.c_Signals.size()) &&
             (*opq_SignalInvalid == false);
             ++u32_ItSignal) {
          if (rc_Message.CheckErrorSignal(
                  opc_List, u32_ItSignal, ou32_CanMessageValidSignalsDlcOffset,
...
/*! \brief   Check messages error
   ...
   \param[in]      orc_ListTx                            Node data pool list
   containing tx signal data
   \param[in]      orc_ListRx                            Node data pool list
   containing rx signal data
   \param[in]      ou32_CanMessageValidSignalsDlcOffset  CAN message DLC offset
   for valid signal range check
   \param[in]      oq_CanMessageSignalGapsValid          Flag if gaps between
   signals are valid or handled as errors
   \param[in]      oq_ByteAlignmentRequired              Flag if byte alignment
   is required and if the signal does not fit into byte alignment it will be
   handled as error
   \param[in]      oq_SignalsRequired                    Flag if signals are
   required for a message
   \param[in]      oq_CanOpenPdoSyncValid                Flag if the CANopen TX
   methods with PDO SYNC are valid or if no check is necessary (other protocols)
   \param[in,out]  opc_InvalidTxMessages                 Optional vector of
   invalid Tx CAN message names
   \param[in,out]  opc_InvalidRxMessages                 Optional vector of
   invalid Rx CAN message names
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanMessageContainer::CheckLocalError(
    const C_OscNodeDataPoolList &orc_ListTx,
  ...
  for (uint32_t u32_ItMessage = 0;
       (u32_ItMessage < this->c_TxMessages.size()) &&
       ((q_Error == false) || (opc_InvalidTxMessages != NULL));
       ++u32_ItMessage) {
    ...
    this->CheckMessageLocalError(
        &orc_ListTx, u32_ItMessage, true, &q_NameConflict, &q_NameInvalid,
        &q_DelayInvalid, &q_IdConflict, &q_IdInvalid, &q_SignalInvalid,
        &q_NoSignalsInvalid, &q_TxMethodInvalid,
        ou32_CanMessageValidSignalsDlcOffset, oq_CanMessageSignalGapsValid,
        oq_ByteAlignmentRequired, oq_SignalsRequired, oq_CanOpenPdoSyncValid);
    if ((q_NameConflict == true) || (q_NameInvalid == true) ||
        (q_DelayInvalid == true) || (q_IdConflict == true) ||
        (q_IdInvalid == true) || (q_SignalInvalid == true) ||
  ...
  for (uint32_t u32_ItMessage = 0;
       (u32_ItMessage < this->c_RxMessages.size()) &&
       ((q_Error == false) || (opc_InvalidRxMessages != NULL));
       ++u32_ItMessage) {
    bool q_NameConflict = false;
    bool q_NameInvalid = false;
    bool q_DelayInvalid = false;
    ...
    bool q_TxMethodInvalid = false;
    this->CheckMessageLocalError(
        &orc_ListRx, u32_ItMessage, false, &q_NameConflict, &q_NameInvalid,
        &q_DelayInvalid, &q_IdConflict, &q_IdInvalid, &q_SignalInvalid,
        &q_NoSignalsInvalid, &q_TxMethodInvalid,
        ou32_CanMessageValidSignalsDlcOffset, oq_CanMessageSignalGapsValid,
        oq_ByteAlignmentRequired, oq_SignalsRequired, oq_CanOpenPdoSyncValid);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Check whether container contains at least one Tx or Rx message with
   "q_Active" flag set
...
bool C_OscCanMessageContainer::ContainsAtLeastOneActiveMessage(void) const {
  ...
  for (u32_ItMessage = 0U; u32_ItMessage < this->c_TxMessages.size();
       ++u32_ItMessage) {
    if (this->c_TxMessages[u32_ItMessage].q_CanOpenManagerMessageActive ==
        true) {
      q_Result = true;
      break;
    }
  ...
  if (q_Result == false) {
    ...
    for (u32_ItMessage = 0; u32_ItMessage < this->c_RxMessages.size();
         ++u32_ItMessage) {
      if (this->c_RxMessages[u32_ItMessage].q_CanOpenManagerMessageActive ==
          true) {
        q_Result = true;
        break;
      }
...
bool C_OscCanMessageContainer::CheckMinSignalError() const {
  bool q_Retval = C_OscCanMessageContainer::mh_CheckMinSignalErrorPerVector(
      this->c_RxMessages);

  if (q_Retval == false) {
    q_Retval = C_OscCanMessageContainer::mh_CheckMinSignalErrorPerVector(
        this->c_TxMessages);
  }
  return q_Retval;
}
...
void C_OscCanMessageContainer::HandleNameMaxCharLimit(
  ...
  for (uint32_t u32_ItTxMsg = 0UL; u32_ItTxMsg < this->c_TxMessages.size();
    ...
    QString c_TmpName = rc_TxMsg.c_Name;
    C_OscSystemNameMaxCharLimitChangeReportItem::h_HandleNameMaxCharLimitItem(
        ou32_NameMaxCharLimit, "tx-message-name", c_TmpName, opc_ChangedItems);
    rc_TxMsg.c_Name = c_TmpName;
  }
  for (uint32_t u32_ItRxMsg = 0UL; u32_ItRxMsg < this->c_RxMessages.size();
       ++u32_ItRxMsg) {
    C_OscCanMessage &rc_RxMsg = this->c_RxMessages[u32_ItRxMsg];
    QString c_TmpName = rc_RxMsg.c_Name;
    C_OscSystemNameMaxCharLimitChangeReportItem::h_HandleNameMaxCharLimitItem(
        ou32_NameMaxCharLimit, "rx-message-name", c_TmpName, opc_ChangedItems);
    rc_RxMsg.c_Name = c_TmpName;
  }
}
...
bool C_OscCanMessageContainer::mh_CheckMinSignalErrorPerVector(
  ...
  for (uint32_t u32_ItMessage = 0; u32_ItMessage < orc_Messages.size();
       ++u32_ItMessage) {
