...
/*!
   \file
   \brief       Utility class to store all necessary indices to identify a
   message stored inside a node (implementation)

   Utility class to store all necessary indices to identify a message stored
   inside a node
...
C_OscCanMessageIdentificationIndices::C_OscCanMessageIdentificationIndices(void)
    : u32_NodeIndex(0U), e_ComProtocol(C_OscCanProtocol::eLAYER2),
      u32_InterfaceIndex(0U), u32_DatapoolIndex(0U), q_MessageIsTx(false),
      u32_MessageIndex(0U) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscCanMessageIdentificationIndices::C_OscCanMessageIdentificationIndices(
    ...
    : u32_NodeIndex(ou32_NodeIndex), e_ComProtocol(oe_ComProtocol),
      u32_InterfaceIndex(ou32_InterfaceIndex),
      u32_DatapoolIndex(ou32_DatapoolIndex), q_MessageIsTx(oq_MessageIsTx),
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanMessageIdentificationIndices::operator==(
    const C_OscCanMessageIdentificationIndices &orc_Cmp) const {
  return (((((orc_Cmp.u32_NodeIndex == this->u32_NodeIndex) &&
             (orc_Cmp.e_ComProtocol == this->e_ComProtocol)) &&
            (orc_Cmp.u32_InterfaceIndex == this->u32_InterfaceIndex) &&
            (orc_Cmp.u32_DatapoolIndex == this->u32_DatapoolIndex)) &&
           (orc_Cmp.q_MessageIsTx == this->q_MessageIsTx)) &&
          (orc_Cmp.u32_MessageIndex == this->u32_MessageIndex));
}
