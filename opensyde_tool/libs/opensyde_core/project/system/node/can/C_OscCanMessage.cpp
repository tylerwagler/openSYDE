...
/*!
   \file
   \brief       Data class for CAN communication message related, additional
...
C_OscCanMessage::C_OscCanMessage(void)
    : c_Name("NewMessage"), c_Comment(""), u32_CanId(0x7FF),
      q_IsExtended(false), u16_Dlc(8), e_TxMethod(eTX_METHOD_CYCLIC),
      u32_CycleTimeMs(100), u16_DelayTimeMs(10), u32_TimeoutMs(310),
      q_CanOpenManagerCobIdIncludesNodeId(true),
      u32_CanOpenManagerCobIdOffset(0), q_CanOpenManagerMessageActive(true),
      u16_CanOpenManagerPdoIndex(0), u8_CanOpenTxMethodAdditionalInfo(0) {}
...
bool C_OscCanMessage::operator!=(const C_OscCanMessage &orc_Cmp) const {
  ...
  if ((this->c_Name != orc_Cmp.c_Name) ||
      ...
      (this->c_CanOpenManagerOwnerNodeIndex !=
       orc_Cmp.c_CanOpenManagerOwnerNodeIndex) ||
      (this->q_CanOpenManagerCobIdIncludesNodeId !=
       orc_Cmp.q_CanOpenManagerCobIdIncludesNodeId) ||
      (this->u32_CanOpenManagerCobIdOffset !=
       orc_Cmp.u32_CanOpenManagerCobIdOffset) ||
      (this->q_CanOpenManagerMessageActive !=
       orc_Cmp.q_CanOpenManagerMessageActive) ||
      (this->u16_CanOpenManagerPdoIndex !=
       orc_Cmp.u16_CanOpenManagerPdoIndex) ||
      (this->u8_CanOpenTxMethodAdditionalInfo !=
       orc_Cmp.u8_CanOpenTxMethodAdditionalInfo) ||
...
/*! \brief  Calculates the hash value over all data
   ...
   \param[in,out]  oru32_HashValue     Hash value with unit [in] value and
...
/*! \brief  Check error for specified signal
   ...
   \param[in]  opc_List                               Node data pool list
   containing signal data (Optional as it is only required by some checks)
   \param[in]  oru32_SignalIndex                      Signal index
   \param[in]  ou32_CanMessageValidSignalsDlcOffset   CAN message DLC offset for
   valid signal range check \param[in]  oq_CanMessageSignalGapsValid Flag if
   gaps between signals are valid or handled as errors \param[in]
   oq_ByteAlignmentRequired               Flag if byte alignment is required and
   if the signal does not fit into byte alignment it will be handled as error
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanMessage::CheckErrorSignal(
    const C_OscNodeDataPoolList *const opc_List,
  ...
  if (this->q_CanOpenManagerMessageActive == true) {
    ...
    const QList<uint32_t> c_Hashes =
        this->m_GetSignalHashes(opc_List, oru32_SignalIndex);
    static QMap<QList<uint32_t>, bool> hc_PreviousResults;
    // Check if check was already performed in the past
    const QMap<QList<uint32_t>, bool>::const_iterator c_It =
        hc_PreviousResults.find(c_Hashes);
    ...
    if (c_It == hc_PreviousResults.end()) {
      ...
      bool q_ValueOverMax;
      this->CheckErrorSignalDetailed(
          opc_List, oru32_SignalIndex, &q_LayoutConflict, &q_BorderConflict,
          &q_GapConflict, &q_ByteAlignmentLengthConflict,
          &q_ByteAlignmentStartbitConflict, &q_NameConflict, &q_NameInvalid,
          &q_MinOverMax, &q_ValueBelowMin, &q_ValueOverMax,
          &q_NoMultiplexerButMultiplexed, &q_MultiplexedValueOutOfRange,
          ou32_CanMessageValidSignalsDlcOffset, oq_CanMessageSignalGapsValid,
      ...
      if ((q_MultiplexedValueOutOfRange == false) &&
          ...
          (q_MinOverMax == false) && (q_ValueBelowMin == false) &&
      ...
      if ((q_NameInvalid == false) && (q_MinOverMax == false) &&
      ...
      this->CheckErrorSignalDetailed(
          opc_List, oru32_SignalIndex, &q_LayoutConflict, &q_BorderConflict,
          &q_GapConflict, &q_ByteAlignmentLengthConflict,
          &q_ByteAlignmentStartbitConflict, &q_NameConflict, NULL, NULL, NULL,
          NULL, &q_NoMultiplexerButMultiplexed, &q_MultiplexedValueOutOfRange,
          ou32_CanMessageValidSignalsDlcOffset, oq_CanMessageSignalGapsValid,
          oq_ByteAlignmentRequired);
      if ((q_NameConflict == true) || (q_LayoutConflict == true) ||
          (q_NoMultiplexerButMultiplexed == true) ||
          (q_MultiplexedValueOutOfRange == true) ||
          (q_BorderConflict == true) || (q_GapConflict == true) ||
...
/*! \brief  Check detailed error for specified signal
   ...
   \param[out]  opq_BorderConflict                    Border not usable as
   variable \param[out]  opq_GapConflict                       Gap between
   signals found and oq_CanMessageSignalGapsValid is false \param[out]
   opq_ByteAlignmentLengthConflict       Length of signal is not compatible to
   byte alignment and oq_ByteAlignmentRequired is true \param[out]
   opq_ByteAlignmentStartbitConflict     Start bit of signal is not compatible
   to byte alignment and oq_ByteAlignmentRequired is true \param[out]
   opq_NameConflict                      Name conflict \param[out]
   opq_NameInvalid                       Name not usable as variable \param[out]
   opq_MinOverMax                        Minimum value over maximum value
   \param[out]  opq_ValueBelowMin                     Init value below minimum
   \param[out]  opq_ValueOverMax                      Init value over maximum
   \param[out]  opq_NoMultiplexerButMultiplexed       Multiplexed signal(s) but
   no multiplexer \param[out]  opq_MultiplexedValueOutOfRange        Multiplexed
   signal multiplexer value out of range of multiplexer \param[in]
   ou32_CanMessageValidSignalsDlcOffset  CAN message DLC offset for valid signal
   range check \param[in]   oq_CanMessageSignalGapsValid          Flag if gaps
   between signals are valid or handled as errors \param[in]
   oq_ByteAlignmentRequired              Flag if byte alignment is required and
   if the signal does not fit into byte alignment it will be handled as error
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanMessage::CheckErrorSignalDetailed(
    const C_OscNodeDataPoolList *const opc_List,
  ...
  if (this->q_CanOpenManagerMessageActive == true) {
    if (oru32_SignalIndex < this->c_Signals.size()) {
      const C_OscCanSignal &rc_CheckedSignal =
          this->c_Signals[oru32_SignalIndex];
      if (opq_LayoutConflict != NULL) {
        QSet<uint16_t> c_CheckedSetPositions;
        rc_CheckedSignal.GetDataBytesBitPositionsOfSignal(
            c_CheckedSetPositions);
        for (uint32_t u32_ItSignal = 0; u32_ItSignal < this->c_Signals.size();
             ++u32_ItSignal) {
          const C_OscCanSignal &rc_CurrentSignal = c_Signals[u32_ItSignal];
          // Skip current signal
          if (u32_ItSignal != oru32_SignalIndex) {
            // Check relevant conflict
            // Basically: Not relevant if both are multiplexed and have
            // different multiplexer values
            if ((((rc_CheckedSignal.e_MultiplexerType ==
                   C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) &&
                  (rc_CurrentSignal.e_MultiplexerType ==
                   C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL)) &&
                 (rc_CheckedSignal.u16_MultiplexValue !=
                  rc_CurrentSignal.u16_MultiplexValue)) == false) {
              QSet<uint16_t> c_CurrentSetPositions;
              rc_CurrentSignal.GetDataBytesBitPositionsOfSignal(
                  c_CurrentSetPositions);
              for (QSet<uint16_t>::const_iterator c_ItCurrentPosition =
                       c_CurrentSetPositions.begin();
                   c_ItCurrentPosition != c_CurrentSetPositions.end();
                   ++c_ItCurrentPosition) {
                if (c_CheckedSetPositions.contains(*c_ItCurrentPosition)) {
                  // Conflict
      ...
      if (opq_BorderConflict != NULL) {
        if (static_cast<uint16_t>(ou32_CanMessageValidSignalsDlcOffset) <=
            this->u16_Dlc) {
          QSet<uint16_t> c_SetPositions;
          // Check over end (after dlc)
          rc_CheckedSignal.GetDataBytesBitPositionsOfSignal(c_SetPositions);
          for (QSet<uint16_t>::const_iterator c_ItPosition =
                   c_SetPositions.begin();
      ...
      if ((opq_GapConflict != NULL) &&
        ...
        *opq_GapConflict = true;
        if (rc_CheckedSignal.e_ComByteOrder ==
            C_OscCanSignal::eBYTE_ORDER_INTEL) {
          ...
          const uint16_t u16_ComBitStartModulo =
              rc_CheckedSignal.u16_ComBitStart % 8U;
          if ((u16_ComBitStartModulo + 1U) <=
              rc_CheckedSignal.u16_ComBitLength) {
            // Needs all lower bits of the byte, so the first bit before the
            // message is in the byte before
            if (rc_CheckedSignal.u16_ComBitStart >= 8U) {
              u16_RelevantBitPos =
                  static_cast<uint16_t>(rc_CheckedSignal.u16_ComBitStart -
                                        u16_ComBitStartModulo) -
                  1U;
            } else {
            ...
            // Needs not the entire byte, the first bit before the message is
            // next to the end of the message
            u16_RelevantBitPos = rc_CheckedSignal.u16_ComBitStart -
                                 rc_CheckedSignal.u16_ComBitLength;
          }
        }
        ...
        if (q_NoCheckNecessary == false) {
          ...
          for (uint32_t u32_ItSignal = 0; u32_ItSignal < this->c_Signals.size();
            ...
            // Skip current signal
            if (u32_ItSignal != oru32_SignalIndex) {
              if (rc_CurrentSignal.IsBitPosPartOfSignal(u16_RelevantBitPos) ==
                  true) {
                // At least one signal is on this position
                // In case of multiple elements on this position, an other part
                // of this error check will control for it
                *opq_GapConflict = false;
                break;
              }
            }
      ...
      if (oq_ByteAlignmentRequired == true) {
        if (opq_ByteAlignmentLengthConflict != NULL) {
          // Only standard sizes are supported in case of necessary byte
          // alignment
        ...
        if (opq_ByteAlignmentStartbitConflict != NULL) {
          if (rc_CheckedSignal.e_ComByteOrder ==
            ...
            *opq_ByteAlignmentStartbitConflict =
                ((rc_CheckedSignal.u16_ComBitStart % 8U) != 0);
          } else {
            // Motorola byte order needs the last bit of the byte as start bit
            *opq_ByteAlignmentStartbitConflict =
                ((rc_CheckedSignal.u16_ComBitStart % 8U) != 7);
          }
        }
      }
      ...
      if (opc_List != NULL) {
        // Name & min & max
        opc_List->CheckErrorElement(rc_CheckedSignal.u32_ComDataElementIndex,
                                    NULL, opq_NameInvalid, opq_MinOverMax, NULL,
                                    NULL);
        // Init
        opc_List->CheckErrorDataSetValue(
            rc_CheckedSignal.u32_ComDataElementIndex, 0, opq_ValueBelowMin,
            opq_ValueOverMax, NULL);

        // Name conflict (only for data elements which are part of the signal
        if (opq_NameConflict != NULL) {
          if (rc_CheckedSignal.u32_ComDataElementIndex <
              opc_List->c_Elements.size()) {
            ...
            for (uint32_t u32_ItSignal = 0;
                 u32_ItSignal < this->c_Signals.size(); ++u32_ItSignal) {
              const C_OscCanSignal &rc_SignalData = c_Signals[u32_ItSignal];
              // Skip current signal
              if (u32_ItSignal != oru32_SignalIndex) {
                if (rc_SignalData.u32_ComDataElementIndex <
                    opc_List->c_Elements.size()) {
                  const C_OscNodeDataPoolListElement &rc_ListElement =
                      opc_List
                          ->c_Elements[rc_SignalData.u32_ComDataElementIndex];
      ...
      if (opq_MultiplexedValueOutOfRange != NULL) {
        ...
        if (rc_CheckedSignal.e_MultiplexerType ==
          ...
          for (uint32_t u32_ItSignal = 0; u32_ItSignal < this->c_Signals.size();
            ...
            if (u32_ItSignal != oru32_SignalIndex) {
              const C_OscCanSignal &rc_SignalData = c_Signals[u32_ItSignal];
              if (rc_SignalData.e_MultiplexerType ==
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanMessage::h_IsTransmissionTypeOfCyclicType(
    const C_OscCanMessage::E_TxMethodType oe_Type) {
  return ((oe_Type == eTX_METHOD_CYCLIC) ||
          ((oe_Type == eTX_METHOD_CAN_OPEN_TYPE_254)) ||
...
/*! \brief  Gets the information if the CAN message is multiplexed
   ...
   This multiplexer must be unique, therefore the first found multiplexer is
   also the only multiplexer.
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanMessage::IsMultiplexed(
    uint32_t *const opu32_MultiplexerIndex) const {
  return C_OscCanMessage::h_ContainsMultiplexer(this->c_Signals,
                                                opu32_MultiplexerIndex);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanMessage::h_ContainsMultiplexer(
    const QList<C_OscCanSignal> &orc_Signals,
    uint32_t *const opu32_MultiplexerIndex) {
  bool q_Return = false;
  uint32_t u32_SignalCounter;

  for (u32_SignalCounter = 0U; u32_SignalCounter < orc_Signals.size();
       ++u32_SignalCounter) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanMessage::GetMultiplexerValues(
    QSet<uint16_t> &orc_Values) const {
  orc_Values.clear();
  uint32_t u32_SignalCounter;

  for (u32_SignalCounter = 0U; u32_SignalCounter < this->c_Signals.size();
       ++u32_SignalCounter) {
    if (this->c_Signals[u32_SignalCounter].e_MultiplexerType ==
        C_OscCanSignal::eMUX_MULTIPLEXED_SIGNAL) {
...
//----------------------------------------------------------------------------------------------------------------------
QList<uint32_t>
C_OscCanMessage::m_GetSignalHashes(const C_OscNodeDataPoolList *const opc_List,
    ...
    if (oru32_SignalIndex < this->c_Signals.size()) {
      ...
      if (rc_Signal.u32_ComDataElementIndex < opc_List->c_Elements.size()) {
        const C_OscNodeDataPoolListElement &rc_Element =
            opc_List->c_Elements[rc_Signal.u32_ComDataElementIndex];
