...
public:
  enum E_Type ///< type of process data communication protocol
  {
    ...
    eCAN_OPEN_SAFETY, ///< Data pool communication protocol CAN open safety
                      ///< (safety protocol)
  ...
  int32_t GetAllSignalsForMessage(
  ...
  static const C_OscNodeDataPoolList *
  h_GetComListConst(const C_OscNodeDataPool &orc_DataPool,
                    const uint32_t ou32_InterfaceIndex, const bool oq_IsTx);
  static int32_t h_GetListIndex(const C_OscNodeDataPool &orc_DataPool,
                                const uint32_t ou32_InterfaceIndex,
                                const bool oq_IsTx);
  static C_OscNodeDataPoolList *h_GetComList(C_OscNodeDataPool &orc_DataPool,
                                             const uint32_t ou32_InterfaceIndex,
                                             const bool oq_IsTx);
  static int32_t h_GetComListIndex(const C_OscNodeDataPool &orc_DataPool,
                                   const uint32_t ou32_InterfaceIndex,
                                   const bool oq_IsTx,
                                   uint32_t &oru32_ListIndex);
  const C_OscNodeDataPoolListElement *
  GetComListElementConst(const C_OscNodeDataPool &orc_DataPool,
                         const uint32_t ou32_InterfaceIndex, const bool oq_IsTx,
                         const uint32_t ou32_MessageIndex,
                         const uint32_t ou32_SignalIndex) const;
  C_OscNodeDataPoolListElement *
  GetComListElement(C_OscNodeDataPool &orc_DataPool,
                    const uint32_t ou32_InterfaceIndex, const bool oq_IsTx,
                    const uint32_t ou32_MessageIndex,
                    const uint32_t ou32_SignalIndex) const;
  void
  ...
  static const QList<C_OscCanProtocol::E_Type>
