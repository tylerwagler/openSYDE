...
public:
  ...
  static bool
  h_IsSignalInMessage(const uint8_t ou8_Dlc,
                      const stw::opensyde_core::C_OscCanSignal &orc_Signal);

  static void
  h_GetSignalValue(const uint8_t (&orau8_CanDb)[8],
                   const stw::opensyde_core::C_OscCanSignal &orc_Signal,
  ...
  static void
      h_SetSignalValue(uint8_t (&orau8_CanDb)[8],
                       const stw::opensyde_core::C_OscCanSignal &orc_Signal,
                       const C_OscNodeDataPoolContent &orc_Value);

  static void h_GetJ1939PgInfoFromCanId(const uint32_t ou32_CanId,
                                        C_OscCanUtilJ1939PgInfo &orc_PgInfo);
  static void
  h_GetCanIdFromJ1939PgInfo(const C_OscCanUtilJ1939PgInfo &orc_PgInfo,
                            uint32_t &oru32_CanId);
