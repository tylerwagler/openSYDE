...
/*!
   \file
   \brief       Data class for a node CAN communication via a specific CAN
   communication protocol (implementation)

   Data class for a node CAN communication via a specific CAN communication
   protocol
 ...
 * ---------------------------------------------------------------------------------------
 */
const QList<C_OscCanProtocol::E_Type> C_OscCanProtocol::hc_ALL_PROTOCOLS =
    C_OscCanProtocol::mh_GetAllProtocols();
...
C_OscCanProtocol::C_OscCanProtocol(void)
    : e_Type(eLAYER2), u32_DataPoolIndex(0) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanProtocol::GetAllSignalsForMessage(
    const C_OscNodeDataPool &orc_DataPool, const uint32_t ou32_InterfaceIndex,
  ...
  const C_OscNodeDataPoolList *const pc_List =
      h_GetComListConst(orc_DataPool, ou32_InterfaceIndex, oq_IsTx);
  ...
  if (pc_List != NULL) {
    if (ou32_InterfaceIndex < this->c_ComMessages.size()) {
      const C_OscCanMessageContainer &rc_ComMessageContainer =
          this->c_ComMessages[ou32_InterfaceIndex];
      const QList<C_OscCanMessage> &rc_ComMessages =
          rc_ComMessageContainer.GetMessagesConst(oq_IsTx);
      // Check consistency
      if (ou32_MessageIndex < rc_ComMessages.size()) {
        ...
        for (uint32_t u32_ItSignal = 0;
          ...
          const uint32_t u32_ListSignalIndex =
              rc_Signal.u32_ComDataElementIndex;
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPoolList *
C_OscCanProtocol::h_GetComListConst(const C_OscNodeDataPool &orc_DataPool,
                                    const uint32_t ou32_InterfaceIndex,
                                    const bool oq_IsTx) {
  const C_OscNodeDataPoolList *pc_Retval = NULL;
  const int32_t s32_ListIndex =
      h_GetListIndex(orc_DataPool, ou32_InterfaceIndex, oq_IsTx);
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanProtocol::h_GetListIndex(const C_OscNodeDataPool &orc_DataPool,
                                         const uint32_t ou32_InterfaceIndex,
                                         const bool oq_IsTx) {
  ...
  if ((u32_ListIndex + 1UL) <
      static_cast<uint32_t>(orc_DataPool.c_Lists.size())) {
    ...
    if (h_ListIsComTx(rc_List) == oq_IsTx) {
      ...
      const C_OscNodeDataPoolList &rc_List2 =
          orc_DataPool.c_Lists[static_cast<size_t>(u32_ListIndex) + 1];
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolList *
C_OscCanProtocol::h_GetComList(C_OscNodeDataPool &orc_DataPool,
                               const uint32_t ou32_InterfaceIndex,
                               const bool oq_IsTx) {
...
/*! \brief   Get index of specific com list Rx or Tx
   ...
   \param[in]   orc_DataPool           Data pool containing list
   \param[in]   ou32_InterfaceIndex    Interface index
   \param[in]   oq_IsTx                Flag if Tx list was requested (otherwise
   Rx) \param[out]  oru32_ListIndex        List index
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanProtocol::h_GetComListIndex(
    const C_OscNodeDataPool &orc_DataPool, const uint32_t ou32_InterfaceIndex,
...
/*! \brief   Get specific com list element
   ...
   \param[in]  ou32_InterfaceIndex  Interface index
   \param[in]  oq_IsTx              Flag if element is part of Tx list
   (otherwise Rx) \param[in]  ou32_MessageIndex    Message index \param[in]
   ou32_SignalIndex     Signal index
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscNodeDataPoolListElement *C_OscCanProtocol::GetComListElementConst(
    const C_OscNodeDataPool &orc_DataPool, const uint32_t ou32_InterfaceIndex,
  ...
  if ((pc_List != NULL) && (ou32_InterfaceIndex < this->c_ComMessages.size())) {
    ...
    const QList<C_OscCanMessage> &rc_Messages =
        rc_MessageContainer.GetMessagesConst(oq_IsTx);
    if (ou32_MessageIndex < rc_Messages.size()) {
      ...
      if (ou32_SignalIndex < rc_Message.c_Signals.size()) {
        const C_OscCanSignal &rc_Signal =
            rc_Message.c_Signals[ou32_SignalIndex];
...
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolListElement *C_OscCanProtocol::GetComListElement(
    C_OscNodeDataPool &orc_DataPool, const uint32_t ou32_InterfaceIndex,
    const bool oq_IsTx, const uint32_t ou32_MessageIndex,
    const uint32_t ou32_SignalIndex) const {
  C_OscNodeDataPoolListElement *pc_Retval = NULL;
  C_OscNodeDataPoolList *const pc_List =
      h_GetComList(orc_DataPool, ou32_InterfaceIndex, oq_IsTx);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanProtocol::HandleNameMaxCharLimit(
    const uint32_t ou32_NameMaxCharLimit,
    QList<C_OscSystemNameMaxCharLimitChangeReportItem>
        *const opc_ChangedItems) {
  for (uint32_t u32_ItMessageContain = 0UL;
       u32_ItMessageContain < this->c_ComMessages.size();
       ++u32_ItMessageContain) {
    C_OscCanMessageContainer &rc_MessageContain =
        this->c_ComMessages[u32_ItMessageContain];
    rc_MessageContain.HandleNameMaxCharLimit(ou32_NameMaxCharLimit,
                                             opc_ChangedItems);
  }
}
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscCanProtocol::h_GetCanMessageValidSignalsDlcOffset(const E_Type oe_Type) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanProtocol::h_GetCanMessageSignalByteAlignmentRequired(
