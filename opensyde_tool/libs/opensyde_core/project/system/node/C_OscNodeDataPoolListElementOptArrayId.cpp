...
/*!
   ...
   including an optional array

   Store all indices to identify a data element stored in a node including an
   optional array
...
C_OscNodeDataPoolListElementOptArrayId::C_OscNodeDataPoolListElementOptArrayId()
    : C_OscNodeDataPoolListElementId(), mq_UseArrayElementIndex(false),
...
C_OscNodeDataPoolListElementOptArrayId::C_OscNodeDataPoolListElementOptArrayId(
    ...
    : C_OscNodeDataPoolListElementId(ou32_NodeIndex, ou32_DataPoolIndex,
...
/*! \brief   Default constructor
   ...
   \param[in]  orc_Base                   Standard node data pool list element
   identification indices \param[in]  oq_UseArrayElementIndex    Flag to use
   array element index \param[in]  ou32_ArrayElementIndex     Array element
   index
*/
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeDataPoolListElementOptArrayId::C_OscNodeDataPoolListElementOptArrayId(
    ...
    : C_OscNodeDataPoolListElementId(
          orc_Base.u32_NodeIndex, orc_Base.u32_DataPoolIndex,
          orc_Base.u32_ListIndex, orc_Base.u32_ElementIndex),
      mq_UseArrayElementIndex(oq_UseArrayElementIndex),
      mu32_ArrayElementIndex(ou32_ArrayElementIndex) {}
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscNodeDataPoolListElementOptArrayId::operator<(
  ...
  if (pc_NonBase != NULL) {
    if (this->GetUseArrayElementIndex() == false) {
      if (this->GetUseArrayElementIndex() ==
          pc_NonBase->GetUseArrayElementIndex()) {
      ...
      if (this->GetUseArrayElementIndex() ==
        ...
        if (this->GetArrayElementIndex() > pc_NonBase->GetArrayElementIndex()) {
          q_Retval = false;
        } else if (this->GetArrayElementIndex() ==
                   pc_NonBase->GetArrayElementIndex()) {
          if (this->GetHalChannelName() == pc_NonBase->GetHalChannelName()) {
            q_Retval = C_OscNodeDataPoolListElementId::operator<(orc_Cmp);
          } else {
            q_Retval =
                this->GetHalChannelName() < pc_NonBase->GetHalChannelName();
          }
        } else {
          q_Retval = true;
        }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscNodeDataPoolListElementOptArrayId::GetHalChannelName(void) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeDataPoolListElementOptArrayId::SetHalChannelName(
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Combine array index valid check with return of array index, with
   default zero if index invalid
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t
C_OscNodeDataPoolListElementOptArrayId::GetArrayElementIndexOrZero(void) const {
  uint32_t u32_Retval;
