...
public:
  ...
  enum E_CrcType ///< Type of CRC for data pool
  {
    ...
    eCT_NON_NVM_DEFAULT_COMPAT_V1, ///< CRC type: for non NVM data pools with V1
                                   ///< calculation and default values
    eCT_NVM                        ///< CRC type: for NVM data pools
  };

  void CalcHash(uint32_t &oru32_HashValue) const;
  void CalcGeneratedDefinitionHash(uint32_t &oru32_HashValue) const;
  void CalcDefinitionHash(uint32_t &oru32_HashValue,
  ...
  void CheckErrorList(const uint32_t &oru32_ListIndex,
                      bool *const opq_NameConflict, bool *const opq_NameInvalid,
                      bool *const opq_UsageInvalid,
                      bool *const opq_OutOfDataPool,
                      bool *const opq_DataSetsInvalid,
  ...
  enum E_Type ///< Type of data pool
  {
    ...
    eHALC_NVM ///< Data pool type: non-volatile memory for HALC
              ///< (contains/handles HALC related parameters)
  };

  E_Type e_Type;          ///< Data pool type
  QString c_Name;         ///< User data pool name
  ...
  QString c_Comment;                 ///< User data pool comment
  ...
  bool q_ScopeIsPrivate; ///< Flag if data pools scope of content is private
                         ///< (vs. public)
