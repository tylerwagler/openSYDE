 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscNodeProperties::C_OscNodeProperties(void) { this->Initialize(); }
...
void C_OscNodeProperties::CalcHash(uint32_t &oru32_HashValue) const {
  ...
  stw::scl::C_SclChecksums::CalcCRC32(nameBytes.constData(), nameBytes.size(),
                                      oru32_HashValue);
  stw::scl::C_SclChecksums::CalcCRC32(commentBytes.constData(),
                                      commentBytes.size(), oru32_HashValue);
  stw::scl::C_SclChecksums::CalcCRC32(&this->e_DiagnosticServer,
                                      sizeof(this->e_DiagnosticServer),
                                      oru32_HashValue);
  stw::scl::C_SclChecksums::CalcCRC32(
      &this->e_FlashLoader, sizeof(this->e_FlashLoader), oru32_HashValue);

  for (u32_Counter = 0U; u32_Counter < this->c_ComInterfaces.size();
       ++u32_Counter) {
  ...
  stw::scl::C_SclChecksums::CalcCRC32(
      &this->q_XappSupport, sizeof(this->q_XappSupport), oru32_HashValue);
}
...
const C_OscNodeComInterfaceSettings *C_OscNodeProperties::GetComInterface(
  ...
  for (uint32_t u32_Index = 0;
       (u32_Index < this->c_ComInterfaces.size()) && (pc_Retval == NULL);
       ++u32_Index) {
    const C_OscNodeComInterfaceSettings &rc_CurComInterface =
        this->c_ComInterfaces[u32_Index];
    // Check if match
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeProperties::SetComInterface(
    const C_OscNodeComInterfaceSettings &orc_ComInterface) {
  for (uint32_t u32_Index = 0; u32_Index < this->c_ComInterfaces.size();
       ++u32_Index) {
    const C_OscNodeComInterfaceSettings &rc_CurComInterface =
        this->c_ComInterfaces[u32_Index];
    // Check if match
    if ((rc_CurComInterface.e_InterfaceType ==
         orc_ComInterface.e_InterfaceType) &&
        (orc_ComInterface.u8_InterfaceNumber ==
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeProperties::DisconnectComInterface(
    const C_OscSystemBus::E_Type oe_InterfaceType,
    const uint8_t ou8_InterfaceNumber) {
  const C_OscNodeComInterfaceSettings *const pc_Interface =
      GetComInterface(oe_InterfaceType, ou8_InterfaceNumber);
...
/*! \brief   Initialize com interfaces using the specified device definition
   ...
   \param[in]  orc_Device           Device definition to use as base for com
   interfaces \param[in]  ou32_SubDeviceIndex  Sub device index
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeProperties::CreateComInterfaces(
    const C_OscDeviceDefinition &orc_Device,
    const uint32_t ou32_SubDeviceIndex) {
  bool q_Found;

  // Check missing can
  for (uint8_t u8_ItCan = 0U; u8_ItCan < orc_Device.u8_NumCanBusses;
       ++u8_ItCan) {
    q_Found = false;
    for (uint32_t u32_CurInterface = 0U;
         u32_CurInterface < this->c_ComInterfaces.size(); ++u32_CurInterface) {
      C_OscNodeComInterfaceSettings &rc_CurInterface =
          this->c_ComInterfaces[u32_CurInterface];
      if ((rc_CurInterface.e_InterfaceType == C_OscSystemBus::eCAN) &&
          (rc_CurInterface.u8_InterfaceNumber == u8_ItCan)) {
        if (q_Found == true) {
          // Remove duplicates
          this->c_ComInterfaces.erase(this->c_ComInterfaces.begin() +
                                      u32_CurInterface);
          // lint -e850 Do not do an iterate step (There is a new element at the
          // current position
          --u32_CurInterface;
        } else {
          // First
          ...
          if (ou32_SubDeviceIndex < orc_Device.c_SubDevices.size()) {
            rc_CurInterface.SetInterfaceConnectedInDevice(
                orc_Device.c_SubDevices[ou32_SubDeviceIndex].IsConnected(
                    C_OscSystemBus::eCAN, u8_ItCan));
          }
        }
      }
    }
    if (q_Found == false) {
      ...
      if (ou32_SubDeviceIndex < orc_Device.c_SubDevices.size()) {
        c_NewInterface.SetInterfaceConnectedInDevice(
            orc_Device.c_SubDevices[ou32_SubDeviceIndex].IsConnected(
                C_OscSystemBus::eCAN, u8_ItCan));
      }
  ...
  for (uint8_t u8_ItEth = 0; u8_ItEth < orc_Device.u8_NumEthernetBusses;
       ++u8_ItEth) {
    q_Found = false;
    for (uint32_t u32_CurInterface = 0;
         u32_CurInterface < this->c_ComInterfaces.size(); ++u32_CurInterface) {
      C_OscNodeComInterfaceSettings &rc_CurInterface =
          this->c_ComInterfaces[u32_CurInterface];
      if ((rc_CurInterface.e_InterfaceType == C_OscSystemBus::eETHERNET) &&
        ...
        if (q_Found == true) {
          ...
          this->c_ComInterfaces.erase(this->c_ComInterfaces.begin() +
                                      u32_CurInterface);
          // Do not do an iterate step (There is a new element at the current
          // position
          --u32_CurInterface;
        } else {
          // First
          ...
          if (ou32_SubDeviceIndex < orc_Device.c_SubDevices.size()) {
            rc_CurInterface.SetInterfaceConnectedInDevice(
                orc_Device.c_SubDevices[ou32_SubDeviceIndex].IsConnected(
                    C_OscSystemBus::eETHERNET, u8_ItEth));
          }
        }
      }
    }
...
//----------------------------------------------------------------------------------------------------------------------
QList<C_OscNodeComInterfaceSettings>
C_OscNodeProperties::GetCanInterfaces() const {
...
//----------------------------------------------------------------------------------------------------------------------
QList<C_OscNodeComInterfaceSettings>
C_OscNodeProperties::GetEthernetInterfaces() const {
  const C_OscSystemBus::E_Type e_ETHERNET = C_OscSystemBus::eETHERNET;

  QList<C_OscNodeComInterfaceSettings> c_EthernetInterfaces =
      m_GetInterfaces(e_ETHERNET);
...
/*! \brief  Get the interfaces according to the argument
   ...
   QList<C_OscNodeComInterfaceSettings>
*///----------------------------------------------------------------------------------------------------------------------
QList<C_OscNodeComInterfaceSettings> C_OscNodeProperties::m_GetInterfaces(
    const C_OscSystemBus::E_Type oe_Type) const {
  const uint32_t u32_ComInterfacesSize =
      static_cast<uint32_t>(c_ComInterfaces.size());

  QList<C_OscNodeComInterfaceSettings> c_Interfaces;

  for (uint32_t u32_ComInterfacesIndex = 0;
       u32_ComInterfacesIndex < u32_ComInterfacesSize;
       ++u32_ComInterfacesIndex) {
    const C_OscNodeComInterfaceSettings c_OscNodeComInterfaceSettings =
        c_ComInterfaces[u32_ComInterfacesIndex];
