...
public:
  ...
  // lint
  // -esym(1539,stw::opensyde_core::C_OscNodeDataPoolContent::mc_CriticalSection)
  // The lock of the copy is intended to remain unchanged.
  C_OscNodeDataPoolContent &
  operator=(const C_OscNodeDataPoolContent &orc_Source);
  ...
  void SetValueArrU16Element(const uint16_t ou16_Value,
  ...
  void SetValueArrU32Element(const uint32_t ou32_Value,
                             const uint32_t ou32_Index);
  ...
  void SetValueArrU64Element(const uint64_t ou64_Value,
                             const uint32_t ou32_Index);
  ...
  void SetValueArrS16Element(const int16_t os16_Value,
                             const uint32_t ou32_Index);
  ...
  void SetValueArrS32Element(const int32_t os32_Value,
                             const uint32_t ou32_Index);
  ...
  void SetValueArrS64Element(const int64_t os64_Value,
                             const uint32_t ou32_Index);
  ...
  void SetValueArrF32Element(const float32_t of32_Value,
                             const uint32_t ou32_Index);
  ...
  void SetValueArrF64Element(const float64_t of64_Value,
                             const uint32_t ou32_Index);
  ...
  bool CompareArrayEqual(const C_OscNodeDataPoolContent &orc_Cmp,
                         const uint32_t ou32_Index) const;
  bool CompareArrayGreaterOrEqual(const C_OscNodeDataPoolContent &orc_Cmp,
                                  const uint32_t ou32_Index) const;
  bool CompareArrayGreater(const C_OscNodeDataPoolContent &orc_Cmp,
                           const uint32_t ou32_Index) const;

  void GetValueAsScaledString(const float64_t of64_Factor,
                              const float64_t of64_Offset,
                              QString &orc_Output,
                              const uint32_t ou32_Index,
                              const bool oq_AllowRangeAdaptation = true,
                              const bool oq_AllowSpecialHandling = false) const;
  void GetAnyValueAsFloat32(float32_t &orf32_Output,
                            const uint32_t ou32_Index) const;
  void GetAnyValueAsFloat64(float64_t &orf64_Output,
                            const uint32_t ou32_Index) const;
...
private:
  ...
  template <typename T>
  void m_SetValue(const T &orc_Value, const E_Type oe_Type);
  template <typename T>
  void m_GetValue(const E_Type oe_Type, T &orc_Value) const;
  template <typename T>
  void m_GetAnyValueAsTemplate(T &orc_Output, const uint32_t ou32_Index) const;

  template <typename T>
  void m_SetValueArray(const T &orc_Value, const E_Type oe_Type);
  // lint -e{8058,8080} //template parameter names are not properly handled by
  // naming convention check
  template <typename T>
  void m_SetValueArrayElement(const T &orc_Value, const uint32_t ou32_Index,
                              const E_Type oe_Type);
  template <typename T>
  void m_GetValueArray(const E_Type oe_Type, T &orc_Result) const;
  // lint -e{8058,8080} //template parameter names are not properly handled by
  // naming convention check
  template <typename T>
  void m_GetValueArrayElement(const E_Type oe_Type, const uint32_t ou32_Index,
                              T &orc_Result) const;
  ...
  static uint64_t
  mh_BinaryToUnsignedInt64Little(const uint8_t *const opu8_Data);
  static uint32_t
  mh_BinaryToUnsignedInt32Little(const uint8_t *const opu8_Data);
  static void mh_UnsignedInt64ToBinaryBig(const uint64_t ou64_Data,
                                          uint8_t *const opu8_Data);
  static void mh_UnsignedInt32ToBinaryBig(const uint32_t ou32_Data,
                                          uint8_t *const opu8_Data);
  static void mh_UnsignedInt64ToBinaryLittle(const uint64_t ou64_Data,
                                             uint8_t *const opu8_Data);
  ...
  // keyword mutable the getter functions must be non const and that is not
  // wanted.
  mutable QRecursiveMutex mc_CriticalSection;

  void m_GetBaseType(bool &orq_IsUintBase, bool &orq_IsSintBase,
                     bool &orq_IsFloatBase, uint64_t &oru64_UintValue,
                     int64_t &ors64_SintValue,
                     float64_t &orf64_FloatValue) const;
  void m_GetBaseTypeArray(const uint32_t &oru32_Index, bool &orq_IsUintBase,
