...
public:
  ...
  enum E_ValueChangedTo { eNO_CHANGE, eZERO, eMIN, eMAX };

  static void
  h_ZeroContent(stw::opensyde_core::C_OscNodeDataPoolContent &orc_Content);
  static void
  h_SetValueInContent(const float64_t of64_Value,
                      stw::opensyde_core::C_OscNodeDataPoolContent &orc_Content,
                      const uint32_t ou32_ArrayIndex = 0U);
  // set a value in min and max range
  static int32_t h_SetValueInMinMaxRange(
      ...
      C_OscNodeDataPoolContentUtil::E_ValueChangedTo &ore_ValueChangedTo,
      const C_OscNodeDataPoolContentUtil::E_SetValue oe_SetValue =
          C_OscNodeDataPoolContentUtil::eLEAVE_VALUE);
  static uint32_t
  h_GetDataTypeSizeInByte(const C_OscNodeDataPoolContent::E_Type oe_Type);
