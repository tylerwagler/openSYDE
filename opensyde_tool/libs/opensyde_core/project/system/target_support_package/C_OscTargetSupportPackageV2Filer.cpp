...
/*! \brief   Load target support package.
   ...
   \param[out]    orc_TargetSupportPackage   target support package information
   read from file \param[in]     orc_Path                   path to file
   ...
   XML file) C_CONFIG    in specified file is a XML node or attribute missing
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscTargetSupportPackageV2Filer::h_Load(
    C_OscTargetSupportPackageV2 &orc_TargetSupportPackage,
...
/*! \brief   Load data from xml file.
   ...
   \param[in]     orc_TargetSupportPackage      target support package
   information to write to file \param[in]     orc_XmlParser                 XML
   parser
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscTargetSupportPackageV2Filer::mh_Load(
    C_OscTargetSupportPackageV2 &orc_TargetSupportPackage,
  ...
  if (c_Text != "opensyde-target-support-package") {
    osc_write_log_error(
        "Loading target support package",
        "XML node \"opensyde-target-support-package\" not found.");
    s32_Return = C_CONFIG;
  } else {
    if (orc_XmlParser.SelectNodeChild("file-version") == "file-version") {
      ...
      if (s32_Return == C_NO_ERR) {
        osc_write_log_info("Loading target support package",
  ...
  // no if statement because of CPPCheck warning. Still safe, because we don't
  // throw error if <halc-included> is not in XML. We want to be compatible with
  // the old file version. Potential error from above gets caught in next if.
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "tsp-comment") {
      osc_write_log_error("Loading target support package",
                          "XML node \"tsp-comment\" not found.");
      s32_Return = C_CONFIG;
    } else {
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "open-syde-server-settings") {
      osc_write_log_error("Loading target support package",
                          "XML node \"open-syde-server-settings\" not found.");
      s32_Return = C_CONFIG;
    } else {
      if (orc_XmlParser.AttributeExists("dpd-application-index") == true) {
        orc_TargetSupportPackage.u8_ApplicationIndex = static_cast<uint8_t>(
            orc_XmlParser.GetAttributeUint32("dpd-application-index"));
      } else {
        osc_write_log_error("Loading target support package",
                            "XML attribute \"dpd-application-index\" not found "
                            "in node \"open-syde-server-setting\".");
        s32_Return = C_CONFIG;
      }
      if ((s32_Return == C_NO_ERR) &&
          (orc_XmlParser.AttributeExists("max-parallel-transmissions") ==
           true)) {
        ...
        osc_write_log_error(
            "Loading target support package",
            "XML attribute \"max-parallel-transmissions\" not found "
            "in node \"open-syde-server-setting\".");
        s32_Return = C_CONFIG;
      }
      if ((s32_Return == C_NO_ERR) &&
          (orc_XmlParser.AttributeExists("max-tx-message-buffer") == true)) {
        orc_TargetSupportPackage.u16_MaxMessageBufferTx = static_cast<uint16_t>(
            orc_XmlParser.GetAttributeUint32("max-tx-message-buffer"));
      } else {
        osc_write_log_error("Loading target support package",
                            "XML attribute \"max-tx-message-buffer\" not found "
                            "in node \"open-syde-server-setting\".");
        s32_Return = C_CONFIG;
      }
      if ((s32_Return == C_NO_ERR) &&
          (orc_XmlParser.AttributeExists("max-rx-routing-message-buffer") ==
           true)) {
        orc_TargetSupportPackage.u16_MaxRoutingMessageBufferRx =
            static_cast<uint16_t>(orc_XmlParser.GetAttributeUint32(
                "max-rx-routing-message-buffer"));
      } else {
        osc_write_log_error(
            "Loading target support package",
            "XML attribute \"max-rx-routing-message-buffer\" not found "
            "in node \"open-syde-server-setting\".");
        s32_Return = C_CONFIG;
      }
    }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "template-project") {
      osc_write_log_error("Loading target support package",
                          "XML node \"template-project\" not found.");
      s32_Return = C_CONFIG;
    } else {
      // template path
  ...
  if ((s32_Return == C_NO_ERR) && (c_HalcIncluded == "true")) {
    ...
    if (c_Text != "halc-definition") {
      osc_write_log_error("Loading target support package",
                          "XML node \"halc-definition\" not found.");
      s32_Return = C_CONFIG;
    } else {
    ...
    // if a TSP with halc was loaded those attributes were set. If one loads a
    // TSP without halc right afterwards the attributes remain set, so we need
    // to clear them
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "application") {
      osc_write_log_error("Loading target support package",
                          "XML node \"application\" not found.");
      s32_Return = C_CONFIG;
    } else {
      // parse this application
      do {
        s32_Return =
            mh_ParseApplication(orc_TargetSupportPackage, orc_XmlParser);
...
/*! \brief   Load applications data from XML file.
   ...
   \param[in]  orc_TargetSupportPackage   target support package information to
   write to file \param[in]  orc_XmlParser              XML parser

   \return
   C_NO_ERR    application data read and placed into target support package
   instance C_CONFIG    XML node or attribute missing
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscTargetSupportPackageV2Filer::mh_ParseApplication(
    C_OscTargetSupportPackageV2 &orc_TargetSupportPackage,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Return = C_NO_ERR;
  QString c_Text;
  ...
  if (orc_XmlParser.AttributeExists("is-programmable") == false) {
    ...
    c_Application.q_IsProgrammable =
        static_cast<bool>(orc_XmlParser.GetAttributeBool("is-programmable"));
  }

  // generates halc psi files. Required attribute, but no error, due to
  // compatibility with old tsp version
  if (orc_XmlParser.AttributeExists("generate-halc-psi") == true) {
    c_Application.q_GeneratesPsiFiles =
        static_cast<bool>(orc_XmlParser.GetAttributeBool("generate-halc-psi"));
  }
  ...
  if ((s32_Return == C_NO_ERR) && (c_Application.q_IsProgrammable == true)) {
    if (orc_XmlParser.AttributeExists("process-id") == false) {
      osc_write_log_error("Loading target support package",
                          "XML attribute \"process-id\" not found.");
      s32_Return = C_CONFIG;
    } else {
      c_Application.u8_ProcessId =
          static_cast<uint8_t>(orc_XmlParser.GetAttributeUint32("process-id"));
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "display-name") {
      osc_write_log_error("Loading target support package",
                          "No XML node \"display-name\" found.");
      s32_Return = C_CONFIG;
    } else {
  ...
  if ((s32_Return == C_NO_ERR) &&
      ((c_Application.q_IsProgrammable == true) ||
       (c_Application.q_GeneratesPsiFiles == true))) {
    c_Text = orc_XmlParser.SelectNodeChild("code-generation");
    if (c_Text != "code-generation") {
      osc_write_log_error("Loading target support package",
                          "XML node \"code-generation\" not found.");
      s32_Return = C_CONFIG;
    } else {
      // generated code structure version (optional attribute)
      if (orc_XmlParser.AttributeExists("generated-code-version") == true) {
        c_Application.u16_GenCodeVersion = static_cast<uint16_t>(
            orc_XmlParser.GetAttributeUint32("generated-code-version"));
      } else {
        // probably deprecated TSP -> default to first version
        // (note: if not programmable this is not reached and therefore defaults
        // to its initial value 0, but this does not harm as
        // "code-generation-version" makes no sense in this case)
      ...
      if (c_Text != "type") {
        ...
        if (orc_XmlParser.AttributeExists("is-standard-opensyde") == false) {
          ...
          c_Application.q_IsStandardSydeCoderCe =
              orc_XmlParser.GetAttributeBool("is-standard-opensyde");
        }
      }
      orc_XmlParser.SelectNodeParent(); // back to parent

      // if the file generator is not standard, a path do the file generator is
      // required
      if ((s32_Return == C_NO_ERR) &&
          (c_Application.q_IsStandardSydeCoderCe == false)) {
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "project-folder") {
      osc_write_log_error("Loading target support package",
                          "No XML node \"project-folder\" found.");
      s32_Return = C_CONFIG;
    } else {
  ...
  // invalid version
    ...
    osc_write_log_error("Loading target support package",
                        "No XML node \"result\" or \"result-paths\" found.");
    s32_Return = C_CONFIG;
    orc_XmlParser.SelectNodeParent(); // back to application
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscTargetSupportPackageV2Filer::mh_ParseHalcSection(
    C_OscTargetSupportPackageV2 &orc_TargetSupportPackage,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Return = C_NO_ERR;
  QString c_Text;
  ...
  if (c_Text != "halc-path") {
    ...
    if (c_Text != "halc-comment") {
      osc_write_log_error("Loading target support package",
                          "XML node \"halc-comment\" not found.");
      s32_Return = C_CONFIG;
    } else {
