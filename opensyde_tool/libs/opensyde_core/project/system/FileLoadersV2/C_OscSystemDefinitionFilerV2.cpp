...
/*! \brief   Load system definition
   ...
   * for each node set a pointer to the used device definition
   ...
   definitions, if the flag is true orc_PathDeviceDefinitions can be an empty
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSystemDefinitionFilerV2::h_LoadSystemDefinitionFile(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSystemDefinitionFilerV2::h_SaveSystemDefinitionFile(
...
/*! \brief   Load system definition
   ...
   * for each node set a pointer to the used device definition
   ...
   if the flag is true orc_PathDeviceDefinitions can be an empty string. It is
   highly recommended to use the device definitions. Purpose for not using the
   device definition is when only read access to a part of the system definition
   is necessary.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSystemDefinitionFilerV2::h_LoadSystemDefinitionString(
    C_OscSystemDefinition &orc_SystemDefinition, const QString &orc_Content,
    const QString &orc_PathDeviceDefinitions,
    const bool oq_UseDeviceDefinitions) {
  int32_t s32_Retval;
  // Set up parser
  ...
  if (s32_Retval == C_NO_ERR) {
    s32_Retval = h_LoadSystemDefinition(orc_SystemDefinition, c_XmlParser,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSystemDefinitionFilerV2::h_SaveSystemDefinitionString(
...

{
  ...
  if (c_SelectedNode == "node") {
    do {
      C_OscNode c_Item;
      s32_Retval = C_OscNodeFilerV2::h_LoadNode(ou16_XmlFormatVersion, c_Item,
                                                orc_XmlParser);
      if (s32_Retval != C_NO_ERR) {
        break;
      }
  ...
  if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
    if (u32_ExpectedSize != orc_Nodes.size()) {
      QString c_Tmp = QString("Unexpected nodes count, expected: %1, got %2")
...
int32_t C_OscSystemDefinitionFilerV2::h_LoadBuses(
  ...
  if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
    if (u32_ExpectedSize != orc_Buses.size()) {
      QString c_Tmp = QString("Unexpected bus count, expected: %1, got %2")
                          .arg(u32_ExpectedSize)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSystemDefinitionFilerV2::h_LoadSystemDefinition(
    C_OscSystemDefinition &orc_SystemDefinition,
    ...
    if (s32_Retval == C_NO_ERR) {
      if (orc_XmlParser.SelectNodeChild("nodes") == "nodes") {
        s32_Retval = h_LoadNodes(
            u16_FileVersion, orc_SystemDefinition.c_Nodes, orc_XmlParser,
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSystemDefinitionFilerV2::h_SaveSystemDefinition(
    const C_OscSystemDefinition &orc_SystemDefinition,
    C_OscXmlParserBase &orc_XmlParser) {
