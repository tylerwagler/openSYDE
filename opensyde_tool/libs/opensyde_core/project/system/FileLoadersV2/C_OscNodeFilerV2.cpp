...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::h_LoadNode(const uint16_t ou16_XmlFormatVersion,
                                     C_OscNode &orc_Node,
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (s32_Retval == C_NO_ERR) {
        if (orc_XmlParser.SelectNodeChild("com-protocols") == "com-protocols") {
          s32_Retval =
              h_LoadNodeComProtocols(orc_Node.c_ComProtocols, orc_XmlParser);
          if (s32_Retval == C_NO_ERR) {
            // Return
            Q_ASSERT(orc_XmlParser.SelectNodeParent() ==
                     "core"); // back up to core
          }
        } else {
        ...
        if (s32_Retval == C_NO_ERR) {
          s32_Retval =
              mh_LoadDataPools(ou16_XmlFormatVersion, orc_Node, orc_XmlParser);
        }
      }
    }
    // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNode(const C_OscNode &orc_Node,
                                  C_OscXmlParserBase &orc_XmlParser) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::h_LoadNodeComProtocols(
  ...
  if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
    if (u32_ExpectedSize != orc_NodeComProtocols.size()) {
      osc_write_log_warning(
          "Load file",
          QString("Unexpected com-protocol count, expected: %1, got %2")
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComProtocols(
  ...
  for (uint32_t u32_ItComProtocol = 0;
       u32_ItComProtocol < orc_NodeComProtocols.size(); ++u32_ItComProtocol) {
    orc_XmlParser.CreateAndSelectNodeChild("com-protocol");
    h_SaveNodeComProtocol(orc_NodeComProtocols[u32_ItComProtocol],
                          orc_XmlParser);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "com-protocols");
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNodeFilerV2::h_LoadNodeComProtocol(C_OscCanProtocol &orc_NodeComProtocol,
                                        C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;

  orc_NodeComProtocol.u32_DataPoolIndex =
      orc_XmlParser.GetAttributeUint32("data-pool-index");
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComProtocol(
    const C_OscCanProtocol &orc_NodeComProtocol,
    C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.SetAttributeUint32("data-pool-index",
                                   orc_NodeComProtocol.u32_DataPoolIndex);
  ...
  if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
    if (u32_ExpectedSize != orc_NodeComMessageContainers.size()) {
      osc_write_log_warning(
          "Load file",
          QString(
              "Unexpected com-message-container count, expected: %1, got %2")
              .arg(u32_ExpectedSize)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::h_LoadNodeComMessageContainer(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComMessageContainer(
  ...
  h_SaveNodeComMessages(orc_NodeComMessageContainer.c_TxMessages,
                        orc_XmlParser);
  // Return
  Q_ASSERT(orc_XmlParser.SelectNodeParent() == "com-message-container");
  // Rx messages
  orc_XmlParser.CreateAndSelectNodeChild("rx-messages");
  h_SaveNodeComMessages(orc_NodeComMessageContainer.c_RxMessages,
                        orc_XmlParser);
  // Return
  Q_ASSERT(orc_XmlParser.SelectNodeParent() == "com-message-container");
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::h_LoadNodeComMessages(
  ...
  if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
    if (u32_ExpectedSize != orc_NodeComMessages.size()) {
      osc_write_log_warning(
          "Load file",
          QString("Unexpected com-message count, expected: %1, got %2")
              .arg(u32_ExpectedSize)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComMessages(
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNodeFilerV2::h_LoadNodeComMessage(C_OscCanMessage &orc_NodeComMessage,
                                       C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComMessage(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::h_LoadNodeComSignals(
  ...
  if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
    if (u32_ExpectedSize != orc_NodeComSignals.size()) {
      osc_write_log_warning(
          "Load file",
          QString("Unexpected com-signal count, expected: %1, got %2")
              .arg(u32_ExpectedSize)
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComSignals(
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNodeFilerV2::h_LoadNodeComSignal(C_OscCanSignal &orc_NodeComSignal,
                                      C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::h_SaveNodeComSignal(
    const C_OscCanSignal &orc_NodeComSignal,
    C_OscXmlParserBase &orc_XmlParser) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscNodeFilerV2::h_CommunicationProtocolToString(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::h_StringToCommunicationProtocol(
  ...
  if (orc_String == "can open safety") {
    ...
    osc_write_log_error("Loading node definition",
                        "Invalid value for \"com-protocol\".\"type\":" +
                            orc_String);
    s32_Retval = C_RANGE;
  }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscNodeFilerV2::mh_LoadProperties(C_OscNodeProperties &orc_NodeProperties,
                                    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("properties") == "properties") {
    ...
    if ((orc_XmlParser.SelectNodeChild("communication-interfaces") ==
        ...
        (s32_Retval == C_NO_ERR)) {
      QString c_CurNode =
          orc_XmlParser.SelectNodeChild("communication-interface");
      if (c_CurNode == "communication-interface") {
        do {
          C_OscNodeComInterfaceSettings c_ComInterface;

          c_ComInterface.u8_InterfaceNumber = static_cast<uint8_t>(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::mh_SaveProperties(
  ...
  for (uint32_t u32_ItComInterface = 0;
       u32_ItComInterface < orc_NodeProperties.c_ComInterfaces.size();
       ++u32_ItComInterface) {
    const C_OscNodeComInterfaceSettings &rc_CurComInterface =
        orc_NodeProperties.c_ComInterfaces[u32_ItComInterface];
    ...
    orc_XmlParser.CreateNodeChild("type",
                                  C_OscSystemFilerUtil::h_BusTypeEnumToString(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::mh_LoadStwFlashloaderOptions(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::mh_SaveStwFlashloaderOptions(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::mh_LoadApplications(
  ...
  if (orc_XmlParser.SelectNodeChild("applications") == "applications") {
    ...
    if (c_CurNode == "application") {
      do {
        ...
        // Set generated code version to 1 (new feature not supported by V2; but
        // default would be 0)
    ...
    if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
      if (u32_ExpectedSize != orc_NodeApplications.size()) {
        osc_write_log_warning(
            "Load file",
            QString("Unexpected application count, expected: %1, got %2")
                .arg(u32_ExpectedSize)
                .arg(static_cast<uint32_t>(orc_NodeApplications.size())));
      }
    }
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "core");
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::mh_SaveApplications(
  ...
  for (uint32_t u32_ItApplication = 0;
    ...
    orc_XmlParser.CreateNodeChild("result-path",
                                  rc_CurApplication.c_ResultPaths[0]);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "applications");
  }
  // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::mh_LoadDataPools(const uint16_t ou16_XmlFormatVersion,
                                           C_OscNode &orc_Node,
                                           C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("data-pools") == "data-pools") {
    ...
    if (c_CurNode == "data-pool") {
      do {
        ...
        s32_Retval = C_OscNodeDataPoolFilerV2::h_LoadDataPool(
            ou16_XmlFormatVersion, c_CurDataPool, orc_XmlParser);
        if (s32_Retval != C_NO_ERR) {
          break;
        }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::mh_SaveDataPools(
  ...
  for (uint32_t u32_ItDataPool = 0; u32_ItDataPool < orc_NodeDataPools.size();
       ++u32_ItDataPool) {
    orc_XmlParser.CreateAndSelectNodeChild("data-pool");
    C_OscNodeDataPoolFilerV2::h_SaveDataPool(orc_NodeDataPools[u32_ItDataPool],
                                             orc_XmlParser);
    // Return
    Q_ASSERT(orc_XmlParser.SelectNodeParent() == "data-pools");
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::mh_StringToDiagnosticServer(
  ...
  if (orc_String == "kefex") {
    ...
    osc_write_log_error(
        "Loading node definition",
        "Invalid value for \"properties\".\"diagnostic-server\":" + orc_String);
    s32_Retval = C_RANGE;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_OscNodeFilerV2::mh_FlashLoaderToString(
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::mh_StringToFlashLoader(
    const QString &orc_String,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscNodeFilerV2::mh_StringToCommunicationByteOrder(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscNodeFilerV2::mh_StringToNodeComMessageTxMethod(
