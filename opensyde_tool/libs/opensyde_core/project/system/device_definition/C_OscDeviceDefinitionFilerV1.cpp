...
void C_OscDeviceDefinitionFilerV1::mh_ParseOpenSydeAvailability(
...
/*! \brief   Gets the openSYDE flashloader parameter
   ...
   Timeout \param[out]  oru32_TransferDataTimeout       Parameter for Transfer
   Data Timeout \param[out]  orq_IsFileBased                 Flag if file based
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscDeviceDefinitionFilerV1::mh_ParseOpenSydeFlashloaderParameter(
...
void C_OscDeviceDefinitionFilerV1::mh_ParseStwFlashloaderAvailability(
    const C_OscXmlParser &orc_Parser, bool &orq_ProtocolSupportedCan) {
  // no check for existence of entries: fall back to "not supported" in this
  // case
  bool q_Support;
...
/*! \brief   Load device definition
   ...
   file \param[in]   orc_Parser             XML Parser \param[in]   orc_Path
   path to file
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscDeviceDefinitionFilerV1::h_Load(
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text == "programming-properties") {
      if (orc_Parser.AttributeExists("is-programmable") == true) {
        c_SubDevice.q_ProgrammingSupport =
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if (c_Text != "protocols-diagnostics") {
      ...
      if (c_Text != "kefex") {
        ...
        for (uint16_t u16_Index = 0U; u16_Index < c_Attributes.size();
          ...
          if (c_Attributes[u16_Index].c_Name == "support") {
            c_SubDevice.q_DiagnosticProtocolKefex =
      ...
      if (c_Text != "opensyde") {
        ...
        mh_ParseOpenSydeAvailability(
            orc_Parser, c_SubDevice.q_DiagnosticProtocolOpenSydeCan,
    ...
    if (c_Text != "protocols-flashloader") {
      ...
      if (c_Text != "flashloader-reset-wait-times") {
        ...
        if (c_Text != "flashloader-reset-wait-time") {
          ...
          c_SubDevice.u32_FlashloaderResetWaitTimeNoFundamentalChangesEthernet =
              u32_Value;
          c_SubDevice.u32_FlashloaderResetWaitTimeFundamentalChangesCan =
              u32_Value;
        ...
        if (c_Text == "no-changes-can") {
          c_SubDevice.u32_FlashloaderResetWaitTimeNoChangesCan =
        ...
        if (c_Text == "no-changes-ethernet") {
          c_SubDevice.u32_FlashloaderResetWaitTimeNoChangesEthernet =
              orc_Parser.GetAttributeUint32("value");
          c_Text = orc_Parser.SelectNodeParent(); // back to parent ...
          Q_ASSERT(c_Text == "flashloader-reset-wait-times");
        } else {
      ...
      if (c_Text != "stw-flashloader") {
        ...
        mh_ParseStwFlashloaderAvailability(orc_Parser,
                                           c_SubDevice.q_FlashloaderStwCan);
        c_Text = orc_Parser.SelectNodeParent(); // back to parent ...
        Q_ASSERT(c_Text == "protocols-flashloader");
      }
      c_Text = orc_Parser.SelectNodeChild("opensyde");
      if (c_Text != "opensyde") {
        ...
        mh_ParseOpenSydeAvailability(orc_Parser,
                                     c_SubDevice.q_FlashloaderOpenSydeCan,
    ...
    if (c_Text != "memory") {
      ...
      if (c_Text != "user-eeprom") {
        ...
        c_SubDevice.u32_UserEepromSizeBytes =
