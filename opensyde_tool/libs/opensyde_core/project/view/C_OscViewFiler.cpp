...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscViewFiler::h_LoadSystemViewsFile(QList<C_OscViewData> &orc_Views,
                                      const QString &orc_PathSystemViews,
                                      const QList<C_OscNode> &orc_OscNodes) {
  int32_t s32_Retval = C_NO_ERR;

  if (QFileInfo(orc_PathSystemViews).exists() &&
      QFileInfo(orc_PathSystemViews).isFile()) {
    C_OscXmlParserLog c_XmlParser;
    c_XmlParser.SetLogHeading("Loading System Views");
    s32_Retval = c_XmlParser.LoadFromFile(orc_PathSystemViews);
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = h_LoadViewsOsc(orc_Views, orc_OscNodes, c_XmlParser,
                                  orc_PathSystemViews);
    } else {
      osc_write_log_error("Loading System Views",
                          "File \"" + orc_PathSystemViews +
                              "\" could not be opened.");
      s32_Retval = C_NOACT;
    }
  } else {
    osc_write_log_error("Loading System Views",
                        "File \"" + orc_PathSystemViews + "\" does not exist.");
    s32_Retval = C_RANGE;
  }
  return s32_Retval;
}
...
/*! \brief  Load core part of system views
   ...
   \param[in]      orc_OscNodes     OSC node information (Necessary for update
   information) \param[in,out]  orc_XmlParser    XML parser with the "current"
   element set to the "opensyde-system-views" element \param[in] orc_BasePath
   Base path
...
int32_t C_OscViewFiler::h_LoadViewsOsc(QList<C_OscViewData> &orc_Views,
  ...
  if (orc_XmlParser.SelectNodeChild("opensyde-system-views") ==
      "opensyde-system-views") {
    ...
    if (c_CurrentViewNode == "opensyde-system-view") {
      do {
        ...
        if (orc_BasePath.isEmpty() == false) {
          const QString c_File = C_OscSystemFilerUtil::h_CombinePaths(
        ...
        // Next
        c_CurrentViewNode =
            orc_XmlParser.SelectNodeNext("opensyde-system-view");
      } while ((c_CurrentViewNode == "opensyde-system-view") &&
               (s32_Retval == C_NO_ERR));
      // Return
    ...
    if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true)) {
      if (u32_ExpectedSize != orc_Views.size()) {
        QString c_Tmp = QString("Unexpected view count, expected: %1, got %2")
                            .arg(u32_ExpectedSize)
                            .arg(static_cast<uint32_t>(orc_Views.size()));
    ...
    osc_write_log_error("Loading views",
                        "Node \"opensyde-system-views\" not found.");
  }
...
/*! \brief  Load core parts of view element
   ...
   \param[in,out]  orc_XmlParser    XML parser with the "current" element set to
   the "opensyde-system-view" element \param[in]      orc_OscNodes     OSC node
   information (Necessary for update information)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscViewFiler::h_LoadViewOsc(C_OscViewData &orc_View,
                                      C_OscXmlParserBase &orc_XmlParser,
                                      const QList<C_OscNode> &orc_OscNodes) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    s32_Retval = C_OscViewFiler::mh_LoadNodeActiveFlags(c_NodeActiveFlags,
                                                        orc_XmlParser);
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      s32_Retval = C_OscViewFiler::mh_LoadNodeUpdateInformation(
          c_NodeUpdateInformation, orc_XmlParser, orc_OscNodes);
...
/*! \brief   Save node active flags
   ...
   \param[in,out]  orc_XmlParser          XML parser with the "current" element
   set to the "opensyde-system-view" element
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::h_SaveNodeActiveFlags(
    const QByteArray &orc_NodeActiveFlags, C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateAndSelectNodeChild("active-nodes");
  for (uint32_t u32_ItNodeActiveFlag = 0;
       u32_ItNodeActiveFlag < orc_NodeActiveFlags.size();
       ++u32_ItNodeActiveFlag) {
    orc_XmlParser.CreateAndSelectNodeChild("active-node");
    orc_XmlParser.SetAttributeBool(
        "state", (orc_NodeActiveFlags[u32_ItNodeActiveFlag] == 1U));
    // Return
...
/*! \brief   Save node update information
   ...
   \param[in,out]  orc_XmlParser                XML parser with the "current"
   element set to the "opensyde-system-view" element
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::h_SaveNodeUpdateInformation(
    const QList<C_OscViewNodeUpdate> &orc_NodeUpdateInformation,
    C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateAndSelectNodeChild("node-update-information");
  for (uint32_t u32_ItNodeActiveFlag = 0;
       u32_ItNodeActiveFlag < orc_NodeUpdateInformation.size();
       ++u32_ItNodeActiveFlag) {
    const C_OscViewNodeUpdate &rc_NodeUpdateInformation =
        orc_NodeUpdateInformation[u32_ItNodeActiveFlag];
    const QStringList &rc_DataBlockPaths =
        rc_NodeUpdateInformation.GetPaths(C_OscViewNodeUpdate::eFTP_DATA_BLOCK);
    const QList<C_OscViewNodeUpdateParamInfo> &rc_ParamSetPaths =
        rc_NodeUpdateInformation.GetParamInfos();
    const QStringList &rc_FileBasedPaths =
        rc_NodeUpdateInformation.GetPaths(C_OscViewNodeUpdate::eFTP_FILE_BASED);
    QList<bool> c_SkipFlags;
    const QList<bool> &rc_PathSkipFlags =
        rc_NodeUpdateInformation.GetSkipUpdateOfPathsFlags(
            C_OscViewNodeUpdate::eFTP_DATA_BLOCK);
    const QList<bool> &rc_ParamSetSkipFlags =
        rc_NodeUpdateInformation.GetSkipUpdateOfParamInfosFlags();
    const QList<bool> &rc_FileBasedSkipFlags =
        rc_NodeUpdateInformation.GetSkipUpdateOfPathsFlags(
    ...
    orc_XmlParser.SetAttributeUint32(
        "position", rc_NodeUpdateInformation.u32_NodeUpdatePosition);

    C_OscViewFiler::mh_SaveNodeUpdateInformationPaths(
        rc_DataBlockPaths, "data-block-path", orc_XmlParser);

    mh_SaveNodeUpdateInformationParamInfo(rc_ParamSetPaths, orc_XmlParser);

    C_OscViewFiler::mh_SaveNodeUpdateInformationPaths(
        rc_FileBasedPaths, "file-based-path", orc_XmlParser);

    // Save the skip flags
    if (rc_PathSkipFlags.size() > 0) {
      std::copy(rc_PathSkipFlags.begin(),
                rc_PathSkipFlags.begin() + rc_PathSkipFlags.size(),
                std::back_inserter(c_SkipFlags));
    }
    if (rc_ParamSetSkipFlags.size() > 0) {
      std::copy(rc_ParamSetSkipFlags.begin(),
                rc_ParamSetSkipFlags.begin() + rc_ParamSetSkipFlags.size(),
                std::back_inserter(c_SkipFlags));
    }
    if (rc_FileBasedSkipFlags.size() > 0) {
      std::copy(rc_FileBasedSkipFlags.begin(),
                rc_FileBasedSkipFlags.begin() + rc_FileBasedSkipFlags.size(),
                std::back_inserter(c_SkipFlags));
    }
    mh_SaveNodeUpdateInformationSkipUpdateOfFiles(c_SkipFlags, orc_XmlParser);

    C_OscViewFiler::mh_SaveNodeUpdateInformationPem(rc_NodeUpdateInformation,
                                                    orc_XmlParser);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::h_SavePc(const C_OscViewPc &orc_OscPc,
                              C_OscXmlParserBase &orc_XmlParser) {
...
/*! \brief   Load node active flags
   ...
   \param[in,out]  orc_NodeActiveFlags    Node active flags (Cleared if
   necessary) \param[in,out]  orc_XmlParser          XML parser with the
   "current" element set to the "opensyde-system-view" element
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscViewFiler::mh_LoadNodeActiveFlags(QByteArray &orc_NodeActiveFlags,
                                       C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("active-nodes") == "active-nodes") {
    QString c_CurrentNodeActiveFlagNode =
        orc_XmlParser.SelectNodeChild("active-node");
    if (c_CurrentNodeActiveFlagNode == "active-node") {
      do {
        ...
        // Next
        c_CurrentNodeActiveFlagNode =
            orc_XmlParser.SelectNodeNext("active-node");
...
/*! \brief   Load node update information
   ...
   \param[in,out]  orc_NodeUpdateInformation    Node update information (Cleared
   if necessary) \param[in,out]  orc_XmlParser                XML parser with
   the "current" element set to the "opensyde-system-view" element \param[in]
   orc_OscNodes                 OSC node information (Necessary for update
   information)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscViewFiler::mh_LoadNodeUpdateInformation(
  ...
  if (orc_XmlParser.SelectNodeChild("node-update-informations") ==
      "node-update-informations") {
    QString c_CurrentNodeUpdateInformationNode =
        orc_XmlParser.SelectNodeChild("node-update-information");
    if (c_CurrentNodeUpdateInformationNode == "node-update-information") {
      ...
      do {
        if (u32_Counter < orc_OscNodes.size()) {
          ...
          s32_Retval = mh_LoadOneNodeUpdateInformation(c_UpdateInfo,
                                                       orc_XmlParser, rc_Node);
          orc_NodeUpdateInformation.push_back(c_UpdateInfo);
          // Next
          c_CurrentNodeUpdateInformationNode =
              orc_XmlParser.SelectNodeNext("node-update-information");
          // Iterate the counter!
          ++u32_Counter;
        } else {
          ...
          (c_CurrentNodeUpdateInformationNode == "node-update-information") &&
          (s32_Retval == C_NO_ERR));
      // Return
    ...
    // New implementation
    if (orc_XmlParser.SelectNodeChild("node-update-information") ==
        "node-update-information") {
      QString c_CurrentNodeUpdateInformationNode =
          orc_XmlParser.SelectNodeChild("node-specific-update-information");
      if (c_CurrentNodeUpdateInformationNode ==
        ...
        do {
          if (u32_Counter < orc_OscNodes.size()) {
            const C_OscNode &rc_Node = orc_OscNodes[u32_Counter];
            C_OscViewNodeUpdate c_UpdateInfo;
            s32_Retval = mh_LoadOneNodeUpdateInformation(
                c_UpdateInfo, orc_XmlParser, rc_Node);
            orc_NodeUpdateInformation.push_back(c_UpdateInfo);
            // Next
            c_CurrentNodeUpdateInformationNode = orc_XmlParser.SelectNodeNext(
                "node-specific-update-information");
            // Iterate the counter!
            ++u32_Counter;
          } else {
        ...
        } while ((c_CurrentNodeUpdateInformationNode ==
                  "node-specific-update-information") &&
                 (s32_Retval == C_NO_ERR));
        // Return
        Q_ASSERT(orc_XmlParser.SelectNodeParent() == "node-update-information");
      ...
      osc_write_log_error("Loading view",
                          "Node \"node-update-information\" not found.");
      s32_Retval = C_CONFIG;
    }
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_LoadPc(C_OscViewPc &orc_OscPc,
                               const C_OscXmlParserBase &orc_XmlParser) {
  orc_OscPc.SetConnected(orc_XmlParser.GetAttributeBool("connected"),
                         orc_XmlParser.GetAttributeUint32("bus-index"), true);
}
...
/*! \brief   Load any update information path
   ...
   \param[in]      orc_XmlTagBaseName  XML tag base name (parent with s and
   children as is) \param[in,out]  orc_XmlParser       XML parser
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_LoadNodeUpdateInformationPaths(
    QStringList &orc_Paths, const QString &orc_XmlTagBaseName,
    C_OscXmlParserBase &orc_XmlParser) {
...
/*! \brief   Load node update information
   ...
   \param[in,out]  orc_XmlParser                XML parser with the "current"
   element set to the "node-update-information" or
                                                "node-specific-update-information"
   element \param[in]      orc_Node                     Node
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscViewFiler::mh_LoadOneNodeUpdateInformation(
  ...
  if (orc_XmlParser.AttributeExists("position") == true) {
    orc_NodeUpdateInformation.u32_NodeUpdatePosition =
        orc_XmlParser.GetAttributeUint32("position");
  } else {
  ...
  if (orc_XmlParser.SelectNodeChild("paths") == "paths") {
    ...
    if (orc_Node.c_Applications.size() == c_Paths.size()) {
      // matching size
      orc_NodeUpdateInformation.SetPaths(c_Paths,
                                         C_OscViewNodeUpdate::eFTP_DATA_BLOCK);
      orc_NodeUpdateInformation.SetSkipUpdateOfPathsFlags(
          c_SkipFlags, C_OscViewNodeUpdate::eFTP_DATA_BLOCK);
    } else {
      // invalid size
      c_Paths.clear();
    ...
    // New format
    C_OscViewFiler::mh_LoadNodeUpdateInformationPaths(
        c_Paths, "data-block-path", orc_XmlParser);
    orc_NodeUpdateInformation.SetPaths(c_Paths,
                                       C_OscViewNodeUpdate::eFTP_DATA_BLOCK);

    C_OscViewFiler::mh_LoadNodeUpdateInformationParam(c_ParamInfo,
                                                      orc_XmlParser);
    orc_NodeUpdateInformation.SetParamInfos(c_ParamInfo);

    C_OscViewFiler::mh_LoadNodeUpdateInformationPaths(
        c_FileBasedPaths, "file-based-path", orc_XmlParser);
    orc_NodeUpdateInformation.SetPaths(c_FileBasedPaths,
                                       C_OscViewNodeUpdate::eFTP_FILE_BASED);

    // Handling of the skip flags
    C_OscViewFiler::mh_LoadNodeUpdateInformationSkipUpdateOfFiles(
        c_SkipFlags, orc_XmlParser);
    // Check for a correct loading of the flags
    if (c_SkipFlags.size() ==
        (c_Paths.size() + c_ParamInfo.size() + c_FileBasedPaths.size())) {
      if (c_Paths.size() > 0) {
        std::copy(c_SkipFlags.begin(), c_SkipFlags.begin() + c_Paths.size(),
                  std::back_inserter(c_PathSkipFlags));
        c_SkipFlags.erase(c_SkipFlags.begin(),
                          c_SkipFlags.begin() + c_Paths.size());
      }
      ...
      if (c_ParamInfo.size() > 0) {
        ...
        c_SkipFlags.erase(c_SkipFlags.begin(),
                          c_SkipFlags.begin() + c_ParamInfo.size());
      }

      if (c_FileBasedPaths.size() > 0) {
        std::copy(c_SkipFlags.begin(),
                  c_SkipFlags.begin() + c_FileBasedPaths.size(),
    ...
    orc_NodeUpdateInformation.SetSkipUpdateOfPathsFlags(
        c_PathSkipFlags, C_OscViewNodeUpdate::eFTP_DATA_BLOCK);
    orc_NodeUpdateInformation.SetSkipUpdateOfParamInfosFlags(
        c_ParamSetSkipFlags);
    orc_NodeUpdateInformation.SetSkipUpdateOfPathsFlags(
        c_FileBasedSkipFlags, C_OscViewNodeUpdate::eFTP_FILE_BASED);

    s32_Retval = C_OscViewFiler::mh_LoadNodeUpdateInformationPem(
        orc_NodeUpdateInformation, orc_XmlParser);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_LoadNodeUpdateInformationParam(
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_LoadNodeUpdateInformationSkipUpdateOfFiles(
    QList<bool> &orc_Flags, C_OscXmlParserBase &orc_XmlParser) {
  // No error in case of not existing flags due to compatibility
  orc_Flags.clear();
  if (orc_XmlParser.SelectNodeChild("skip-update-of-files") ==
      "skip-update-of-files") {
    QString c_CurrentNodeActiveFlagNode =
        orc_XmlParser.SelectNodeChild("skip-update-of-file");
    if (c_CurrentNodeActiveFlagNode == "skip-update-of-file") {
      do {
        ...
        // Next
        c_CurrentNodeActiveFlagNode =
            orc_XmlParser.SelectNodeNext("skip-update-of-file");
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscViewFiler::mh_LoadNodeUpdateInformationPem(
    C_OscViewNodeUpdate &orc_NodeUpdateInformation,
    C_OscXmlParserBase &orc_XmlParser) {
  int32_t s32_Retval = C_NO_ERR;
  ...
  if (orc_XmlParser.SelectNodeChild("pem-file") == "pem-file") {
    ...
    if (s32_Retval == C_NO_ERR) {
      ...
      if (s32_Retval == C_NO_ERR) {
        orc_NodeUpdateInformation.SetPemFilePath(
    ...
    if (s32_Retval == C_NO_ERR) {
      s32_Retval = C_OscViewFiler::mh_LoadNodeUpdateInformationPemStates(
          orc_NodeUpdateInformation, orc_XmlParser);
    }
    if (s32_Retval == C_NO_ERR) {
      // Return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscViewFiler::mh_LoadNodeUpdateInformationPemStates(
    C_OscViewNodeUpdate &orc_NodeUpdateInformation,
    C_OscXmlParserBase &orc_XmlParser) {
  ...
  if (s32_Retval == C_NO_ERR) {
    ...
    C_OscViewNodeUpdate::E_StateDebugger e_StateDebugger =
        C_OscViewNodeUpdate::eST_DEB_NO_CHANGE;
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscViewFiler::mh_LoadViewFileOsc(C_OscViewData &orc_View,
                                   const QString &orc_FilePath,
                                   const QList<C_OscNode> &orc_OscNodes) {
  C_OscXmlParser c_XmlParser;
  int32_t s32_Retval = C_OscSystemFilerUtil::h_GetParserForExistingFile(
      c_XmlParser, orc_FilePath, "opensyde-view-definition");
  ...
  if (s32_Retval == C_NO_ERR) {
    if (c_XmlParser.SelectNodeChild("opensyde-system-view") ==
        "opensyde-system-view") {
      s32_Retval =
          C_OscViewFiler::h_LoadViewOsc(orc_View, c_XmlParser, orc_OscNodes);
    } else {
      osc_write_log_error("Loading view",
                          "Could not find \"opensyde-system-view\" node.");
      s32_Retval = C_CONFIG;
    }
  } else {
...
/*! \brief   Save node update path information
   ...
   \param[in,out]  orc_XmlParser       XML parser with the "current" element set
                                       to the "node-specific-update-information"
   element
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_SaveNodeUpdateInformationPaths(
    const QStringList &orc_Paths, const QString &orc_XmlTagBaseName,
    C_OscXmlParserBase &orc_XmlParser) {
  const QString c_ParentName = orc_XmlTagBaseName + "s";
  const QString c_ChildName = orc_XmlTagBaseName;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_SaveNodeUpdateInformationParamInfo(
    const QList<C_OscViewNodeUpdateParamInfo> &orc_Info,
    C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateAndSelectNodeChild("param-sets");
  for (uint32_t u32_ItParamSet = 0UL; u32_ItParamSet < orc_Info.size();
       ++u32_ItParamSet) {
    const C_OscViewNodeUpdateParamInfo &rc_ParamSetInfo =
        orc_Info[u32_ItParamSet];
    orc_XmlParser.CreateAndSelectNodeChild("param-set");
    orc_XmlParser.SetAttributeUint32("last-known-crc",
                                     rc_ParamSetInfo.GetLastKnownCrc());
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_SaveNodeUpdateInformationSkipUpdateOfFiles(
    const QList<bool> &orc_Flags, C_OscXmlParserBase &orc_XmlParser) {
  orc_XmlParser.CreateAndSelectNodeChild("skip-update-of-files");
  for (uint32_t u32_ItSkipFlag = 0U; u32_ItSkipFlag < orc_Flags.size();
       ++u32_ItSkipFlag) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscViewFiler::mh_SaveNodeUpdateInformationPem(
  ...
  orc_XmlParser.SetAttributeBool(
      "skip", orc_NodeUpdateInformation.GetSkipUpdateOfPemFile());
  orc_XmlParser.CreateNodeChild("path",
                                orc_NodeUpdateInformation.GetPemFilePath());
  ...
  orc_XmlParser.CreateNodeChild(
      "security",
      C_OscViewFiler::h_PemFileStateSecurityToString(e_StateSecurity));
  orc_XmlParser.CreateNodeChild(
      "debugger",
      C_OscViewFiler::h_PemFileStateDebuggerToString(e_StateDebugger));
  // Return
  Q_ASSERT(orc_XmlParser.SelectNodeParent() == "pem-file");
  // Return
  Q_ASSERT(orc_XmlParser.SelectNodeParent() ==
