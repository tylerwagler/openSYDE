...
public:
 ...
 // --------------------------------------------------------------------------
  ...
  void SetNodeUpdateInformation(
      const QList<C_OscViewNodeUpdate> &orc_NodeUpdateInformation);
  const C_OscViewNodeUpdate *
  GetNodeUpdateInformation(const uint32_t ou32_NodeIndex) const;
  int32_t SetNodeUpdateInformation(
      const uint32_t ou32_NodeIndex,
      const C_OscViewNodeUpdate &orc_NodeUpdateInformation);
  int32_t SetNodeUpdateInformationPath(
  ...
  int32_t SetNodeUpdateInformationParamInfo(
      const uint32_t ou32_NodeIndex, const uint32_t ou32_Index,
      const C_OscViewNodeUpdateParamInfo &orc_Value);
  int32_t SetNodeUpdateInformationPemFilePath(const uint32_t ou32_NodeIndex,
                                              const QString &orc_Value);
  int32_t SetNodeUpdateInformationSkipUpdateOfPath(
      const uint32_t ou32_NodeIndex, const uint32_t ou32_Index,
      const bool oq_SkipFile,
      const C_OscViewNodeUpdate::E_GenericFileType oe_Type);
  int32_t
  SetNodeUpdateInformationSkipUpdateOfParamInfo(const uint32_t ou32_NodeIndex,
                                                const uint32_t ou32_Index,
                                                const bool oq_SkipFile);
  int32_t
  SetNodeUpdateInformationSkipUpdateOfPemFile(const uint32_t ou32_NodeIndex,
                                              const bool oq_SkipFile);
  int32_t SetNodeUpdateInformationStates(
      const uint32_t ou32_NodeIndex,
      const C_OscViewNodeUpdate::E_StateSecurity oe_StateSecurity,
      const C_OscViewNodeUpdate::E_StateDebugger oe_StateDebugger);
  int32_t SetNodeUpdateInformationParamInfoContent(
      const uint32_t ou32_NodeIndex, const uint32_t ou32_Index,
      const QString &orc_FilePath, const uint32_t ou32_LastKnownCrc);
  int32_t AddNodeUpdateInformationPath(
      const uint32_t ou32_NodeIndex, const QString &orc_Value,
      const C_OscViewNodeUpdate::E_GenericFileType oe_Type);
  int32_t AddNodeUpdateInformationParamInfo(
      const uint32_t ou32_NodeIndex,
      const C_OscViewNodeUpdateParamInfo &orc_Value);
  ...
  // Delete
  int32_t RemoveNodeUpdateInformationPath(
      const uint32_t ou32_NodeIndex, const uint32_t ou32_Index,
      const C_OscViewNodeUpdate::E_GenericFileType oe_Type);
  int32_t RemoveNodeUpdateInformationParamInfo(const uint32_t ou32_NodeIndex,
                                               const uint32_t ou32_Index);
  int32_t RemoveNodeUpdateInformationPemFilePath(const uint32_t ou32_NodeIndex);
  int32_t ClearNodeUpdateInformationAsAppropriate(
      const uint32_t ou32_NodeIndex,
      const C_OscViewNodeUpdate::E_GenericFileType oe_Type);
