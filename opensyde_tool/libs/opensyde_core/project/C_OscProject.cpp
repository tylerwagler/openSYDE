...
C_OscProject::C_OscProject(void)
    : c_CreationTime(QDateTime::currentDateTime()),
      ...
      c_OpenSydeVersion("0.01r0"), c_Template("Empty project"),
      c_Version("0.01r0b0") {
...
QDateTime C_OscProject::h_GetTimeOfString(const QString &orc_Str) {
  ...
  QStringList c_Dyn =
      orc_Str.split(QRegularExpression("[. :]+"), Qt::SkipEmptyParts);
  if (c_Dyn.size() == 5) {
    try {
      ...
      c_Retval = QDateTime(QDate(s32_Year, s32_Month, s32_Day),
                           QTime(s32_Hour, s32_Minute));
      if (c_Retval.isValid()) {
        q_Err = false;
      }
    } catch (...) {
