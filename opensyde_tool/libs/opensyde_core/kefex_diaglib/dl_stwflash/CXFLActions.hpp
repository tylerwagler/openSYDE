...
public:
  ...
  QString c_ParameterName;     /// name of parameter
...
public:
  ...
  int32_t SaveToINI(QSettings &orc_IniFile, const QString &orc_Section);
};
...
private:
  /// indexes in this table (do not neccessarily match the parameter index used
  /// in the protocol)
...
public:
  ...
  QString c_SNR; ///< serial number of node (String with "." separators, e.g.
                 ///< "11.123456.1001")
  QString c_DeviceId; ///< name of device
  bool q_Selected;
};
...
public:
  ...
  uint16_t u16_NumBlocks; ///< number of blocks in this region (i.e. blocks of
                          ///< the same size)
};
...
public:
  ...
  uint32_t u32_ProgrammingTime; ///< time in [ms] that it takes to write up to
                                // 255 bytes
  QList<C_XFLFlashRegionInformation>
      c_Regions; ///< individual regions within the IC
...
class C_XFLFlashSectors : public QList<C_XFLFlashSector> {
public:
  int32_t GetSectorOccupiedByAddress(const uint32_t ou32_Address,
...
/// which features are available (one layer of abstraction above
/// T_XFL_ImplementedServices)
...
public:
  ...
  void Clear(void);
  void DetermineFeatures(const C_XFLImplementedServices &orc_Services,
...
public:
  ...
  C_XFLFeaturesAvailable c_AvailableFeatures; ///< "compiled" information from
                                              ///< T_XFLImplementedServices
...
public:
  ...
  bool q_CheckAtStartup;     ///< true: flashloader shall check checksum against
                             ///< flash content at startup
  bool q_CheckAfterFlashing; ///< true: flashloader shall check checksum against
                             ///< flash content after flashing
};
...
public:
  QList<C_XFLChecksumBlock>
      c_BlockConfig; ///< start / end address (not available if sector
                     // based)
...
enum E_XFLWakeupMode {
  ...
  eXFL_WAKEUP_MODE_LID_THEN_SNR, ///< address node by its local ID, if there are
                                 ///< multiple nodes with the same local ID
  ///<   use the SNR to distinguish those
  eXFL_WAKEUP_MODE_NO_WAKEUP ///< do not perform any wakeup (can be used if the
                             ///< node is already in wakeup mode)
};
...
public:
  ...
  bool q_SendResetRQ; ///< send reset request message ? true -> send; false ->
                      ///< don't send
  stw::can::T_STWCAN_Msg_TX
      t_ResetMsg; ///< reset request message (only used if q_SendResetRQ = true)

  bool q_SendFLASHRequired;   ///< true -> send "FLASH" at startup
  uint32_t u32_StartTimeMs;   ///< in ms (defines how long the "FLASH" command
                              ///< should be sent)
  uint8_t u8_FLASHIntervalMs; ///< interval between individual "FLASH" commands
                              ///< in ms.
  ///< set to 10 for most target nodes

  uint8_t u8_LocalID; ///< target node's local ID (only required with
                      ///< e_WakeupMode == eXFL_WAKEUP_MODE_LID)
  uint8_t au8_SNR[6]; ///< target node's serial number (only required with
                      ///< e_WakeupMode == eXFL_FLASH_MODE_SNR)

  QString c_CompanyID; ///< target node's company ID (2,3 or 5 characters)
};

//----------------------------------------------------------------------------------------------------------------------
/// Higher level comfort layer on top of the flashloader protocol driver
class C_XFLActions : public C_XFLProtocol {
private:
  bool m_AllowSTWCID(const C_XFLCompanyID &orc_ConfiguredCompanyID,
                     const C_XFLCompanyID &orc_RespondedCompanyID);

  void
  m_ReadFingerPrintInformation(C_XFLInformationFromServer &orc_Information);
  void m_ReadDeviceInfo(C_XFLInformationFromServer &orc_Information);

  int32_t m_WakeupLocalID(const C_XFLCompanyID &orc_CompanyID,
                          const bool oq_MultiResponsesOK = false);
  int32_t m_WakeupSNR(const C_XFLCompanyID &orc_CompanyID,
                      const uint8_t (&orau8_SNR)[6], uint8_t &oru8_LocalID);
  int32_t m_WakeupLocalIDAndSNR(const C_XFLCompanyID &orc_CompanyID,
...
public:
  ...
  int32_t
  RequestNodeReset(const stw::can::T_STWCAN_Msg_TX *const opt_ResetMessage,
                   const bool oq_SingleNode = true);

  int32_t DivertStreamOnOffBBB(const bool oq_OnOff,
                               const uint8_t ou8_TargetPosition,
                               const uint8_t (&orau8_UserID)[2]);
  int32_t DivertStreamOnOff(const bool oq_On,
                            const C_XFLDivertParameters &orc_DivertParams,
                            const bool oq_UseHopHandle,
                            const uint8_t ou8_HopHandle);

  int32_t PerformWakeup(const C_XFLWakeupParameters &orc_Parameters,
                        uint8_t *const opu8_ActualLocalID = NULL);

  int32_t ReadFlashInformation(C_XFLFlashInformation &orc_Information,
                               QString &orc_ErrorText);
  int32_t SearchNodes(const QString &orc_CompanyID,
  ...
  int32_t ReadServerSectorChecksumInformation(
      const uint16_t ou16_SectorCount,
      C_XFLChecksumAreas &orc_ChecksumInformation);
  int32_t ReadServerBlockChecksumInformation(
      C_XFLChecksumAreas &orc_ChecksumInformation);

  static int32_t CompIDStringToStruct(const QString &orc_String,
                                      C_XFLCompanyID &orc_Data);
  static int32_t CompIDStructToString(const C_XFLCompanyID &orc_Data,
                                      QString &orc_String);
  static QString SNRBytesToString(const uint8_t (&orau8_Data)[6],
                                  const bool oq_IncludeDots);
  static int32_t SNRStringToBytes(const QString &orc_Text,
                                  uint8_t (&orau8_Data)[6]);

  static QString XFLProtocolErrorToText(const int32_t os32_ReturnValue,
