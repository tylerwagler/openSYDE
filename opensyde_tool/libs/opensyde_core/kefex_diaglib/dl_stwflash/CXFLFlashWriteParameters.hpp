...
namespace stw {
namespace diag_lib {
// 1.02r0 -> replaced all remaining function pointers; these are now
// overloadable functions in C_XFLDownload
...
// pointers (not closures anymore)
//           also changed all prefixes to match STW C-Rules 3.x
//           (m_ExecuteStatusCallbackas API was changed anyway ...) even changed
//           name of parameter structure ...
// 0.01r1 -> abCompanyID now consists of up to 5 bytes
// 0.01r0 -> added e_DevTypeCheck, e_DevTypeCheckGetIDFailedReaction,
// e_DevTypeCheckMatchIDFailedReaction 0.00r4 -> added
// q_IgnoreInvalidHexfileError 0.00r3 -> added q_XFLExchange
static const uint16_t CXFLFLASHWRITE_VERSION = 0x1020U;

enum E_XFLEraseMode ///< determines which flash sectors shall be erased before
                    ///< flashing
{
  eXFL_ERASE_MODE_C_APPLICATION =
      0,                          ///< erase all sectors except flashloader area
  eXFL_ERASE_MODE_CANOPEN_CONFIG, ///< erase only CANopen configuration data
                                  ///< area
  eXFL_ERASE_MODE_IEC_APPLICATION,     ///< erase only CoDeSys application area
  eXFL_ERASE_MODE_IEC_RUN_TIME_SYSTEM, ///< erase only CoDeSys run-time-system
                                       ///< area
  eXFL_ERASE_MODE_USER_DEFINED, ///< sectors to be erased can be set manually
  eXFL_ERASE_MODE_AUTOMATIC     ///< automatically detect which sectors to erase
                                ///< (only from protocol version
                                // V3.00r0 on)
};

enum E_XFLFlashFinishedAction ///< which action shall be taken after flashing
                              ///< has finished ?
{
...
enum E_XFLDevTypeCheck {
  ...
  eXFL_DEV_TYPE_CHECK_SCAN_HEX_FILE =
      1, ///< scan hexfile for T_XFLECUInformation block
  eXFL_DEV_TYPE_CHECK_GET_ADDRESS_THEN_SCAN =
      2, ///< read the expected address from ECU
  ...
  ///<  then scan it
  eXFL_DEV_TYPE_CHECK_GET_ADDRESS_THEN_FAIL =
      3 ///< read the expected address from ECU
        ///<  if there's no data at this address in the hex-file
        ///<   (or: the service is not available on the ECU)
        ///<   (or: the service returns an error)
        ///<  then fail
...
/// define how to react to failed X-check (could not get device ID from
/// hex-file; or could not get address from server)
enum E_XFLDevTypeCheckGetIDFailReaction {
  eXFL_DEV_TYPE_CHECK_GETID_FAIL_REACTION_IGNORE =
      0, ///< ignore (but protocol as text) -> no x-check
  eXFL_DEV_TYPE_CHECK_GETID_FAIL_REACTION_FAIL =
      1, ///< fail: do not start downloading
  eXFL_DEV_TYPE_CHECK_GETID_FAIL_REACTION_ASK =
      2 ///< ask user whether to continue  -> no x-check if he/she
        // selects "yes"
};
...
enum E_XFLDevTypeCheckMatchIDFailReaction {
  eXFL_DEV_TYPE_CHECK_MATCHID_FAIL_REACTION_IGNORE =
      0, ///< ignore (but protocol as text)
  eXFL_DEV_TYPE_CHECK_MATCHID_FAIL_REACTION_FAIL =
      1, ///< fail: do not start downloading
  eXFL_DEV_TYPE_CHECK_MATCHID_FAIL_REACTION_ASK =
      2 ///< ask user whether to continue  -> ok if he/she selects
        // "yes"
};
...
public:
  ...
  uint16_t u16_Version; ///< used to check consistency between header-file and
                        ///< library, use
                        // CXFLWRITEFLASH_VERSION
  QString c_HexFile;    ///< full path the hex file

  C_XFLWakeupParameters
      c_WakeupConfig; ///< parameters defining how to address the target node

  E_XFLEraseMode e_EraseMode;   ///< determines the erase mode
  QString c_UserDefinedSectors; ///< comma separated (e.g. "10,11,4"), only used
                                ///< if e_EraseMode =
  ///<  XFLFileTypeUserDefined

  E_XFLFlashFinishedAction
      e_FlashFinishedAction; ///< what to do after flashing has finished ?

  uint16_t u16_InterFrameDelayUs; ///< delay in us between individual frames
                                  ///< when sending hex-lines.
  ///< helpful for low-performance targets at high bitrates.
  ///< set to 0 for most communication interfaces and target nodes

  uint8_t u8_HexRecordLength; ///< patch hexfile records to different length; 0
                              ///< = don't modify

  bool q_XtdID; ///< false: use 11bit CAN message IDs; true: use 29bit CAN
                ///< message IDs
  uint32_t u32_SendID;    ///< CAN message TX ID used in flashloader protocol
  uint32_t u32_ReceiveID; ///< CAN message RX ID used in flashloader protocol
                          ///< (usually TX-ID + 1).
  ///< 0xffffffff -> accept response on any ID (e.g. for RS232 flashing)

  ///< ESX2 Babyboard B compatibility entries (not required for any other
  ///< targets (set "q_Divertstream = false;")
  bool q_DivertStream; ///< true: target ESX2 babyboard B; false -> target
                       ///< mainboard
  uint8_t u8_DivertTargetIndex; ///< slot of babyboard B
  uint8_t au8_DivertUserID[2];  ///< key to be able to write to ESX2 babyboard B

  bool q_WriteCRCsIfSupported; ///< true -> write flash sector checksums; false
                               ///< -> don't

  bool q_VerboseMode; ///< true -> report some more details about progress ...

  bool q_XFLExchange; ///< false -> regular download; true -> send XFL_EXCHANGE
                      ///< request
  ...
  E_XFLDevTypeCheckGetIDFailReaction
      e_DevTypeCheckGetIDFailedReaction; ///< cf. comments at enum definition
  E_XFLDevTypeCheckMatchIDFailReaction
      e_DevTypeCheckMatchIDFailedReaction; ///< cf. comments at enum definition

  void LoadFromIni(QSettings &orc_IniFile, const QString &orc_Section);
  int32_t SaveToIni(QSettings &orc_IniFile, const QString &orc_Section) const;
};
