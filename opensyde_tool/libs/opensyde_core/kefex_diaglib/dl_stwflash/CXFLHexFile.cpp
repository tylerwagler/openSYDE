...
//-----------------------------------------------------------------------------
QString
C_XFLHexFile::ErrorCodeToErrorText(const uint32_t ou32_ErrorCode) const {
  QString c_Text;
  ...
  case WRN_RECORD_OVERLAY:
    u32_Address =
        this->GetLastOverlayErrorAddress(); // ou32_ErrorCode only contains
                                            // lowest 7 nibbles !
    c_Text = QString("Error in Hexfile: Address 0x%1 used twice !")
                 .arg(u32_Address, 8, 16, QChar('0'));
    break;
  case ERR_HEXLINE_SYNTAX:
    c_Text = QString("%1 %2")
                 .arg(stw::opensyde_core::C_OscUtils::h_LoadString(
                     STR_FM_ERR_RD_HF_LINE_SYNTAX))
                 .arg(ou32_ErrorCode & 0x0FFFFFFFUL);
    break;
  case ERR_HEXLINE_CHECKSUM:
    c_Text = QString("%1 %2")
                 .arg(stw::opensyde_core::C_OscUtils::h_LoadString(
                     STR_FM_ERR_RD_HF_LINE_CHECKSUM))
                 .arg(ou32_ErrorCode & 0x0FFFFFFFUL);
    break;
  case ERR_HEXLINE_COMMAND:
    c_Text = QString("%1 %2")
                 .arg(stw::opensyde_core::C_OscUtils::h_LoadString(
                     STR_FM_ERR_RD_HF_LINE_COMMAND))
                 .arg(ou32_ErrorCode & 0x0FFFFFFFUL);
    break;
  case ERR_NOT_ENOUGH_MEMORY:
    c_Text =
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_ERR_RD_HF_MEMORY);
    break;
  case ERR_CANT_OPEN_FILE:
    c_Text = stw::opensyde_core::C_OscUtils::h_LoadString(
        STR_FM_ERR_RD_HF_OPEN_FILE);
    break;
  default:
    c_Text = stw::opensyde_core::C_OscUtils::h_LoadString(
        STR_FM_ERR_RD_HF_UNDEFINED);
    break;
  }
  return c_Text;
}
...
/*!
   ...
   \param[out]     orc_InfoBlocks            list of detected device information
   blocks
   \param[in]      ou32_SearchStartAddress   linear offset address to start
   search at
   \param[in]      oq_OnlyOneBlock           true -> read only one block then
   stop searching more
   \param[in]      oq_ExactAddressMatch      true -> info block must be at
   exactly ou32_SearchStartAddress
   \param[in]      oq_Block0Only             true -> only find "Block0"
   information blocks These are the blocks with application information. Other
   Blocks (1..9) can contain information about various libraries. e.g. 1 =
   "BIOS".

   \return
   C_NO_ERR     everything OK (data in oat_InfoBlocks); but maybe there are 0
   blocks C_NOACT      if oq_ExactAddressMatch: no block found at specified
   address
*/
//-----------------------------------------------------------------------------
int32_t C_XFLHexFile::GetECUInformationBlocks(
    QList<C_XFLECUInformation> &orc_InfoBlocks,
  ...
  while (true) {
    ...
    s32_Return = this->FindPattern(u32_Address, 5,
                                   reinterpret_cast<const uint8_t *>("Lx_?z"));
    if (s32_Return != C_NO_ERR) {
      break; // only stop if we cannot find any more pattern; in all other
             // cases: continue searching
    }

    // get part of dump:
    u16_Help = XFL_DEVICE_INFO_MAGIC_LENGTH_V2;
    s32_Return = this->GetDataByAddress(
        u32_Address, u16_Help, reinterpret_cast<uint8_t *>(&acn_Magic[0]));
    if (s32_Return == C_NO_ERR) {
      s32_Return = C_CONFIG;
      //"Block0" ?
      if ((acn_Magic[6] == '.') || (oq_Block0Only == false)) {
        ...
        if (acn_Magic[5] == '2') {
          s32_Return = C_NO_ERR;
          (void)memcpy(&c_Block.acn_Magic[0], &acn_Magic[0],
    ...
    if (oq_ExactAddressMatch == true) {
      if ((u32_Address != ou32_SearchStartAddress) ||
          (s32_Return != C_NO_ERR)) {
    ...
    if (s32_Return == C_NO_ERR) {
      // for performance reasons we try to read the maximum possible size of the
      // struct on the ECU:
      ...
      case -2: // data read but not fully
        s32_Return = 0;
        if (u16_Size <
            2U) // we need at least 2 bytes for the header information
        {
          continue;
        }
        break;
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        orc_InfoBlocks[(orc_InfoBlocks.size() > 0 ? orc_InfoBlocks.size() - 1
                                                  : 0)] = c_Block;
...
/*!
   ...
   So the sequence of hex-lines in the physical file will not affect the
   resulting checksum. If you need to check for a modified file, an MD5 checksum
   will be more helpful. The used algorithm is a CRC32 with a fixed start value.
   For a correct checksum we need to count in the address offsets and data
   bytes. We want the checksum to match for identical data. So we have to make
   sure to have a fixed concept of when to count in the address offset. Doing so
   at the beginning of a hex-line will not work, as then the checksum will
   change for a file with the same content but different record-lengths. So wo
   use the binary-block dump of the data and count in the address offset at the
...
int32_t C_XFLHexFile::CalcFileChecksum(uint32_t &oru32_Checksum) {
  ...
  for (uint32_t u32_Index = 0U;
       u32_Index < static_cast<uint32_t>(pc_Dump->at_Blocks.size());
       u32_Index++) {
    // address (serialize to make the code endian-safe):
    const uint32_t u32_AddressOffset =
        pc_Dump->at_Blocks[u32_Index].u32_AddressOffset;
