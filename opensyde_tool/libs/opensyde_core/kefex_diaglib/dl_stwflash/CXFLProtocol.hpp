...
const int32_t XFL_ERR_DEVICE_SPECIFIC =
    -20; ///< error value can be retrieved with GetLastXFLError
...
const uint16_t XFL_NUM_DIFFERENT_LOCAL_IDS =
    256U; ///< 0..255 (255 is reserved, but we never know)
...
class C_XFLAliasedRanges : public QList<C_XFLAliasedRange> {
public:
  int32_t GetRangeOccupiedByAddress(const uint32_t ou32_Address,
...
public:
  ...
  bool q_BlockDefinitionValid; // true -> valid information contained in start
                               // and end address
};
...
public:
  uint8_t au8_Data[5];
  uint8_t
      u8_NumBytes; ///< number of actually used bytes (only 2 or 5 are valid)
};
...
public:
  // do not prevent copy and assignment operator: we really do want a flat copy
  // here !
  ...
  uint32_t u32_Bitrate;   ///< communication bitrate in kBit/s (used for busload
...
class C_XFLProtocol : virtual public C_DLReportEvents {
protected:
  static const uint16_t TIMEOUT_ERASE_MS =
      15500U; // must be >15s for AK4 application with old ESX2 flashloader
  static const uint16_t TIMEOUT_FLASH_MS =
      1000U; //(total timeout = Value * (MAX_TRIES-1))
  ...
  int32_t m_SendCompanyIDExt(const uint8_t ou8_LocalID,
  ...
  int32_t m_SendHexLine(const uint8_t *const opu8_HexLine,
  ...
  int32_t m_WaitForResponse(const uint8_t ou8_ExpectedLocalID,
                            const uint32_t ou32_TimeOutMS,
                            const uint8_t (&orau8_Match)[8],
                            const uint8_t ou8_NumExpectedToMatch,
                            stw::can::T_STWCAN_Msg_RX *const opc_MSG,
                            const int16_t os16_CheckDLC = -1);
  int32_t m_GetDeviceIDBlock(const uint8_t ou8_BlockIndex,
                             char_t (&oracn_Data)[4]);

  int32_t m_NodeFlashFunc(const uint8_t ou8_SubCommand);

  int32_t m_GetDeviceInfoAddress(const uint8_t ou8_Index,
                                 uint8_t (&orau8_Result)[4]);

  /// to be overloaded if user interaction is required:
  /// Report progress in 1/1000; return a value != C_NO_ERR to signal the
  /// flashing procedure to abort. This is just for informational purposes and
  /// not neccessarily exactly linear to the actual progress.
  ...
  virtual void TRG_ReportStatus(const QString &orc_Text,
                                const uint8_t ou8_Type);

  // Functions affecting timing and multithreading behaviour. Have default
  // implementations suitable for most cases
  //  but can be overloaded for special requirements.
  virtual void TRG_WaitMicroSeconds(const uint32_t ou32_NumUs); // active
                                                                // waiting
  virtual void
  TRG_HandleSystemMessages(void); // perform operating system message handling
...
public:
  ...
  // protocol functions:
  // Flash functions:
  int32_t EraseSector(const uint16_t ou16_SectorNumber,
                      const uint32_t ou32_TimeOut = TIMEOUT_ERASE_MS);
  ...
  int32_t ReadFlash(const uint8_t ou8_NumBytes,
                    const uint32_t ou32_StartAddress, uint8_t *const opu8_Data);
  int32_t SendMultiHexLine(uint16_t &oru16_NumLines,
                           const uint32_t ou32_InterFrameDelayUs,
                           uint32_t &oru32_NumBytesSent,
                           stw::hex_file::C_HexFile &orc_HexFile,
                           const uint32_t ou32_TimeOut = TIMEOUT_FLASH_MS);

  // Utility functions:
  int32_t SendFLASH(const uint32_t ou32_StartTimeMs,
  ...
  int32_t WakeupLocalId(const C_XFLCompanyID &orc_CompanyID,
                        uint8_t *const opu8_NumFound = NULL,
                        C_XFLCompanyID *const opc_CompanyIDOut = NULL);
  int32_t WakeupSerialNumber(const uint8_t (&orau8_SerialNumber)[6],
                             const C_XFLCompanyID &orc_CompanyID,
                             uint8_t &oru8_LocalID,
                             C_XFLCompanyID *const opc_CompanyIDOut = NULL);
  int32_t SendSingleMessage(const stw::can::T_STWCAN_Msg_TX &orc_Msg);
  int32_t DivertStream(const uint8_t ou8_Device, const uint8_t ou8_Position,
                       const uint8_t ou8_State);
  int32_t DivertStream(const uint8_t ou8_Device, const uint8_t ou8_Position,
                       const uint8_t ou8_State, const uint8_t ou8_HopHandle);

  // EEPROM services
  int32_t EEPROMRead(const uint8_t ou8_NumBytes,
                     const uint16_t ou16_StartAddress,
                     uint8_t *const opu8_Data);
  int32_t EEPROMWrite(const uint8_t ou8_NumBytes,
                      const uint16_t ou16_StartAddress,
                      const uint8_t *const opu8_Data);

  // get services
  int32_t GetLocalID(uint8_t &oru8_LocalID);
  int32_t GetLocalIDExt(uint8_t *const opu8_LocalId,
                        const uint8_t ou8_NumMaxLocalId,
                        uint8_t &oru8_NumFoundECUs);
  int32_t GetSectorCount(uint16_t &oru16_NumSectors);
  int32_t GetVersionNumber(uint8_t (&orau8_Version)[5],
                           uint8_t &oru8_VersionSize);
  int32_t GetEraseCount(uint32_t &oru32_EraseCount);
  int32_t GetDeviceID(const bool oq_LongID, QString &orc_DeviceId);
  int32_t GetControlID(uint32_t &oru32_ControlID);
  int32_t GetSecCRC(const uint16_t ou16_Sector, uint16_t &oru16_CRC,
                    uint16_t &oru16_EECRC);
  int32_t GetModeCompare(const uint16_t ou16_Sector, uint8_t &oru8_ModeCompare);

  int32_t GetFlashInformation(const uint8_t ou8_Index,
  ...
  int32_t GetFlashInformationTotalMemorySize(const uint8_t ou8_ICIndex,
                                             uint32_t &oru32_Size);
  int32_t GetFlashInformationProtectedSectors(
      QList<C_XFLProtectedSectorInfo> &orc_ProtectedSectors);
  int32_t GetFlashInformationOffsetSector0(const uint8_t ou8_ICIndex,
                                           uint32_t &oru32_Offset);
  int32_t GetFlashInformationNumberRegions(const uint8_t ou8_ICIndex,
                                           uint8_t &oru8_RegionCount);
  int32_t GetFlashInformationRegionInformation(const uint8_t ou8_ICIndex,
                                               const uint8_t ou8_RegionIndex,
                                               uint32_t &oru32_BlockSize,
                                               uint16_t &oru16_BlockCount);
  int32_t GetFlashInformationEraseTime(const uint8_t ou8_ICIndex,
                                       uint32_t &oru32_EraseTime);
  int32_t GetFlashInformationWriteTime(const uint8_t ou8_ICIndex,
  ...
  int32_t BBWakeup(const uint8_t ou8_Position,
