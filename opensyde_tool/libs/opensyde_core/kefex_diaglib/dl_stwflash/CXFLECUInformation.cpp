...
/*!
   ...
   \copyright   Copyright 2015 Sensor-Technik Wiedemann GmbH. All rights
   reserved.
*/
//----------------------------------------------------------------------------------------------------------------------
...
const char_t
    stw::diag_lib::XFL_DEVICE_INFO_MAGIC_V1[XFL_DEVICE_INFO_MAGIC_LENGTH_V1 +
                                            1U] = "Lx_?z2.";
const char_t
    stw::diag_lib::XFL_DEVICE_INFO_MAGIC_V2[XFL_DEVICE_INFO_MAGIC_LENGTH_V2] =
        "Lx_?zg."; /// \0 already
                   // included

const uint8_t C_XFLECUInformation::hau8_LENGTHS_DEVICE_ID[3] = {16U, 17U, 29U};
const uint8_t C_XFLECUInformation::hau8_LENGTHS_DATE[3] = {11U, 12U, 12U};
const uint8_t C_XFLECUInformation::hau8_LENGTHS_TIME[3] = {8U, 9U, 9U};
const uint8_t C_XFLECUInformation::hau8_LENGTHS_PROJECT_NAME[3] = {15U, 16U,
                                                                   33U};
const uint8_t C_XFLECUInformation::hau8_LENGTHS_PROJECT_VERSION[3] = {10U, 11U,
                                                                      17U};
...
C_XFLECUInformation::C_XFLECUInformation(void)
    : u8_StructVersion(0U), u8_ContentMap(0U), u8_LenAdditionalInfo(0U) {
  this->ClearContents();
}
...
/*! \brief   Parse ECU information payload data from BLOB
   ...
   \param[in]     opu8_Data                 data to parse, starting with the
   payload (first byte after version resp. content map byte)
   \param[in]     ou16_NumBytesAvailable    number of bytes available in
   opu8_Data
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLECUInformation::m_ParsePayload(const uint8_t *const opu8_Data,
  ...
  if ((s32_Return == C_NO_ERR) && (this->ContainsProjectName() == true)) {
    ...
    if ((ou16_NumBytesAvailable - u16_Index) <= u16_Length) {
      ...
      (void)memcpy(&this->acn_ProjectName[0], &opu8_Data[u16_Index],
                   u16_Length);
      u16_Index = static_cast<uint16_t>(u16_Index + u16_Length);
    }
  }
  ...
  if ((s32_Return == C_NO_ERR) && (this->ContainsProjectName() == true)) {
    if (this->ContainsProjectVersion() == true) {
      ...
      if ((ou16_NumBytesAvailable - u16_Index) <= u16_Length) {
        ...
        (void)memcpy(&this->acn_ProjectVersion[0], &opu8_Data[u16_Index],
                     u16_Length);
        u16_Index = static_cast<uint16_t>(u16_Index + u16_Length);
      }
    }
  ...
  if ((s32_Return == C_NO_ERR) && (this->ContainsAdditionalInfo() == true)) {
    ...
    if ((ou16_NumBytesAvailable - u16_Index) < u16_Length) {
      ...
      (void)memcpy(&this->acn_AdditionalInfo[0], &opu8_Data[u16_Index],
                   this->u8_LenAdditionalInfo);
      //            u16_Index = static_cast<uint16_t>(u16_Index + u16_Length);
    }
  }
  return s32_Return;
...
/*! \brief   Parse ECU information data from BLOB
   ...
   \param[in]     opu8_Data                 data to parse, starting with magic
   pattern
   \param[in]     ou16_NumBytesAvailable    number of bytes available in
   opu8_Data (must be >= than the area occupied by the information block itseld)
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLECUInformation::ParseFromBLOB(const uint8_t *const opu8_Data,
                                   const uint16_t ou16_NumBytesAvailable) {
  ...
  // We need at least the magic + version + content map (or at least a few bytes
  // of valid data for V3)
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return = memcmp(&opu8_Data[0], XFL_DEVICE_INFO_MAGIC_V1,
                        XFL_DEVICE_INFO_MAGIC_LENGTH_V1 - 1);
    if (s32_Return != 0) {
      // try V2/V3:
      s32_Return = memcmp(&opu8_Data[0], XFL_DEVICE_INFO_MAGIC_V2,
                          XFL_DEVICE_INFO_MAGIC_LENGTH_V2 - 2);
      if (s32_Return == 0) {
        // final byte zero ?
    ...
    if (s32_Return != 0) {
      ...
      u16_Length = (static_cast<uint16_t>(XFL_DEVICE_INFO_MAGIC_LENGTH_V1) +
                    ((q_V2OrV3Detected == true) ? 1 : 0));
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    if ((c_Temp.u8_StructVersion != 1U) && (c_Temp.u8_StructVersion != 2U) &&
        (c_Temp.u8_StructVersion != 3U)) {
  ...
  if (s32_Return == C_NO_ERR) {
    s32_Return = c_Temp.m_ParsePayload(&opu8_Data[u16_Index],
                                       ou16_NumBytesAvailable - u16_Index,
...
QString C_XFLECUInformation::m_GetNonTerminatedString(
    const char_t *const opcn_Chars, const uint8_t ou8_MaxLength) const {
  QString c_Help;
  char_t *pcn_Text;
  ...
  c_Help = pcn_Text;
  delete[] pcn_Text;
  c_Help = c_Help.trimmed();
  return c_Help;
}

//----------------------------------------------------------------------------------------------------------------------

void C_XFLECUInformation::AddInfoToList(QStringList &orc_List) const {
  (void)orc_List.append("Block type:      " + GetInfoLevelAsString());
  (void)orc_List.append("Version:         " +
                        QString::number(u8_StructVersion));

  if (ContainsDeviceID() == true) {
    (void)orc_List.append("DeviceID:        " + GetDeviceID());
  } else {
    (void)orc_List.append("DeviceID:        undefined");
  }
  if (ContainsDateAndTime() == true) {
    (void)orc_List.append("Date:            " + GetDate());
    (void)orc_List.append("Time:            " + GetTime());
  } else {
    (void)orc_List.append("Date and Time:   undefined");
  }

  if (ContainsProjectName() == true) {
    (void)orc_List.append("Project name:    " + GetProjectName());
  } else {
    (void)orc_List.append("Project name:    undefined");
  }

  if (ContainsProjectVersion() == true) {
    (void)orc_List.append("Project version: " + GetProjectVersion());
  } else {
    (void)orc_List.append("Project version: undefined");
  }

  if (ContainsAdditionalInfo() == true) {
    (void)orc_List.append("Additional info: " + GetAdditionalInfo());
  } else {
    (void)orc_List.append("Additional info: undefined");
  }
}

//----------------------------------------------------------------------------------------------------------------------

QString C_XFLECUInformation::GetInfoLevelAsString(void) const {
  int32_t s32_Return;
  QString c_Text;
...
QString C_XFLECUInformation::GetDeviceID(void) const {
  return m_GetNonTerminatedString(&acn_DeviceID[0],
                                  static_cast<uint8_t>(sizeof(acn_DeviceID)));
}

//----------------------------------------------------------------------------------------------------------------------

QString C_XFLECUInformation::GetDate(void) const {
  return m_GetNonTerminatedString(&acn_Date[0],
                                  static_cast<uint8_t>(sizeof(acn_Date)));
}

//----------------------------------------------------------------------------------------------------------------------

QString C_XFLECUInformation::GetTime(void) const {
  return m_GetNonTerminatedString(&acn_Time[0],
                                  static_cast<uint8_t>(sizeof(acn_Time)));
}

//----------------------------------------------------------------------------------------------------------------------

QString C_XFLECUInformation::GetProjectName(void) const {
  return m_GetNonTerminatedString(
      &acn_ProjectName[0], static_cast<uint8_t>(sizeof(acn_ProjectName)));
}

//----------------------------------------------------------------------------------------------------------------------

QString C_XFLECUInformation::GetProjectVersion(void) const {
  return m_GetNonTerminatedString(
      &acn_ProjectVersion[0], static_cast<uint8_t>(sizeof(acn_ProjectVersion)));
}

//----------------------------------------------------------------------------------------------------------------------

QString C_XFLECUInformation::GetAdditionalInfo(void) const {
