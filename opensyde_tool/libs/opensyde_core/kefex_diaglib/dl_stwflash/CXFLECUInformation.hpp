...
extern const char_t XFL_DEVICE_INFO_MAGIC_V1[XFL_DEVICE_INFO_MAGIC_LENGTH_V1 +
                                             1U]; /// +1 for \0
extern const char_t
    XFL_DEVICE_INFO_MAGIC_V2[XFL_DEVICE_INFO_MAGIC_LENGTH_V2]; /// \0 already
                                                               /// included
...
private:
  QString m_GetNonTerminatedString(const char_t *opcn_Chars,
                                   const uint8_t ou8_MaxLength) const;

  int32_t m_ParsePayload(const uint8_t *const opu8_Data,
                         const uint16_t ou16_NumBytesAvailable,
                         const uint8_t ou8_BlockVersion);

public:
  // sizes of content fields by version (1st element: V1, 2nd element: V2, 3rd
  // element: V3)
  ...
  int32_t ParseFromBLOB(const uint8_t *const opu8_Data,
                        const uint16_t ou16_NumBytesAvailable);
  ...
  // the lenghts of the char-arrays are set to accomodate the longest possible
  // texts of all defined versions
  char_t
      acn_DeviceID[29]; ///< ASCII device ID e.g. "ESX2" (optional; recommended)
  char_t acn_Date[12];  ///< date of compilation(format: __DATE__ ANSI C macro)
                        ///< (optional)
  char_t acn_Time[9];   ///< time of compilation (format: __TIME__ ANSI C macro)
                        ///< (optional)
  ...
  char_t acn_AdditionalInfo[255]; ///< does not hurt on PC side, if we simply
                                  ///< use the theoretical max. of 255 bytes

  QString GetDeviceID(void) const;
  QString GetDate(void) const;
  QString GetTime(void) const;
  QString GetProjectName(void) const;
  QString GetProjectVersion(void) const;
  QString GetAdditionalInfo(void) const;

  void AddInfoToList(QStringList &orc_List) const;

  int32_t GetInfoLevel(void) const;
  QString GetInfoLevelAsString(void) const;
