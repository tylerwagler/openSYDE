...
/*!
   \file
   \brief       Implements the software update procedure based on the STW
 ...
 * ------------------------------------------------------------------------------------------------
 */
void C_XFLFlashWrite::m_InitProtocol(
...
int32_t C_XFLFlashWrite::m_Wakeup(const C_XFLWakeupParameters &orc_Params) {
  ...
  if (s32_Return == C_NO_ERR) {
    m_ReportVerboseStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_CONNECTED_NODE) +
        QString::number(mu8_ActualLocalID));
  }
...
void C_XFLFlashWrite::m_FlashingFinished(
    const E_XFLFlashFinishedAction oe_Action) {
  uint32_t u32_Action;
  int32_t s32_Return;
  E_XFLFlashFinishedAction e_Action;

  if (oe_Action == eXFL_FLASH_FINISHED_ACTION_ASK_USER) {
    s32_Return = TRG_UserInteraction(
        eXFL_USER_INTERACTION_REASON_FINISHED,
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_FLASHING_FINISHED),
        u32_Action);
    if (s32_Return != C_NO_ERR) {
  ...
  this->TRG_ReportStatus(
      stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_FINISHED),
      gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
}

//----------------------------------------------------------------------------------------------------------------------
// opq_SectorBasedCRCsSupported -> For servers below protocol V3.00r0 we have to
//                                detect whether they support checksums
//                                hard-coded For newer servers this does not
//                                matter anyway.
int32_t C_XFLFlashWrite::m_GetVersionNumber(uint8_t &oru8_Version,
                                            bool &orq_SectorBasedCRCsSupported,
  ...
  if (s32_Return != C_NO_ERR) {
    c_Text =
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_VERSION_ERR) +
  ...
  case 5U:
    oru8_Version = 1U;
    u32_Version = static_cast<uint32_t>(
        ((static_cast<uint16_t>(au8_Version[0]) - 48U) * 100U) +
...
/*! \brief   write block based checksums to server
   ...
   We do NOT: Only request calculation for blocks that we have touched when
   writing. This would require a more complex logic: Match hex file content with
   defined blocks.
...
int32_t C_XFLFlashWrite::m_WriteBlockBasedChecksums(void) {
  ...
  for (s32_Index = 0; s32_Index < c_Blocks.size(); s32_Index++) {
    // for blocks with CRCs in EEPROM there should be no way for them to become
    // invalid
    Q_ASSERT(c_Blocks[s32_Index].q_BlockDefinitionValid == true);

    if (c_Blocks[s32_Index].u32_EndAddress != XFL_CHK_BLOCK_END_INVALID) {
      s32_Return =
          SetBlockChecksum(static_cast<uint8_t>(s32_Index), u32_CheckSum);
      if (s32_Return != C_NO_ERR) {
        break;
      }
    }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLFlashWrite::ExecuteWrite(const C_XFLFlashWriteParameters &orc_Params) {
  int32_t s32_Return;
  ...
  if (mc_Config.pc_CANDispatcher == NULL) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_ERR_DISPATCHER_NA),
        gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return C_CONFIG;
  }
  ...
  if (orc_Params.u16_Version != CXFLFLASHWRITE_VERSION) {
    TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                         STR_FM_ERR_VERSION_MISMATCH),
                     gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return C_CONFIG;
  }

  mu8_ActualLocalID =
      orc_Params.c_WakeupConfig.u8_LocalID; // copied as it might change during
                                            // the process when doing
                                            //  wakeup with SNR
  ...
  if (s32_Return != C_NO_ERR) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_ERR_WAKEUP),
        gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return C_NOACT;
  }
  ...
  if (orc_Params.q_XFLExchange == true) {
    ...
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(
          stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_ERR_XFL_EXCHANGE),
          gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return C_NOACT;
    }
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_XFL_EXCHANGE),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);

    // STW Flashloader-Spec: set_xfl_exchange will cause the server to go back
    // to sleep ...
    s32_Return = m_Wakeup(orc_Params.c_WakeupConfig);
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    // <= 3.00r0
    TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                         STR_FM_ERR_FL_PROTOCOL_VERSION),
                     gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
  ...
  s32_Return = this->m_GetVersionNumber(u8_FlashloaderVersion, q_CRCsSupported,
                                        orc_Params.q_DivertStream);
  if (s32_Return != C_NO_ERR) {
    return C_NOACT;
  }
  ...
  // not exactly correct, as older flashloader version have some services
  // available but for now we don't check for any of these here ...
  if (u16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00) {
    ...
    if (c_Features.q_FingerPrint == true) {
      ...
      if (s32_Return != C_NO_ERR) {
        TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                             STR_FM_ERR_READ_FINGER_PRINT),
                         gu8_DL_REPORT_STATUS_TYPE_ERROR);
        return C_NOACT;
      }
    } else {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FM_ERR_FINGER_PRINT_NA),
                       gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
  ...
  if (orc_Params.q_DivertStream == true) {
    s32_Return = DivertStreamOnOffBBB(true, orc_Params.u8_DivertTargetIndex,
                                      orc_Params.au8_DivertUserID);
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
    ...
    // if the CRC mechanism is implemented this has to be handled via the
    // version number
    u8_ChecksumType = 0U;
  }
  ...
  case 0U: // not supported
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_CRCS_NO_SUPP),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    break;
  case 1U: // sector based
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_SBC_SUPPORTED),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    break;
  case 2U: // no break
  case 3U: // block based EEPROM or FLASH
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_BBC_SUPPORTED),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    break;
  case 4U: // deactivated
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_CS_DEACTIVATED),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    break;
  default:
    Q_ASSERT(false);
  ...
  if (s32_Return != C_NO_ERR) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_SEC_COUNT_ERR),
        gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return C_NOACT;
  }
  TRG_ReportStatus(
      stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_NUM_SECTORS) + " " +
          QString::number(u16_NumSectors),
      gu8_DL_REPORT_STATUS_TYPE_INFORMATION);

  s32_Return = GetDeviceID(u16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00,
                           c_DeviceId);
  if (s32_Return != C_NO_ERR) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FW_ERR_READ_DEV_ID),
        gu8_DL_REPORT_STATUS_TYPE_WARNING);
    c_DeviceId = "";
  } else {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FW_DEV_ID) + " \"" +
            c_DeviceId + "\"",
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
  }
  ...
  s32_Return = m_FlashESXLokalID(c_DeviceId, c_FingerPrintIndexes, orc_Params,
                                 u16_ProtocolVersion);
  if (s32_Return != C_NO_ERR) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ERR_FLASHING),
        gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return C_NOACT;
  }
  ...
  if (orc_Params.q_XFLExchange ==
      false) // In XFL exchange mode updating the CRCs may fail as not all
             // services
             //  may be available in this mode.
             // cf. Flashloader specification chapter set_xfl_exchange
             // for a clean solution (e.g. perform the reset sequence as
             // described there
             //  before continuing with this block)
  {
    s32_Return =
        m_WriteFlashChecksums(u8_ChecksumType, orc_Params.c_WakeupConfig);
    if (s32_Return != C_NO_ERR) {
      return C_NOACT;
    }
  }
  ...
  if (orc_Params.q_DivertStream == true) {
    (void)DivertStreamOnOffBBB(false, orc_Params.u8_DivertTargetIndex,
                               orc_Params.au8_DivertUserID);
...
/*! \brief   update resp. check flash content checksums
   ...
   * block based CRCs/flash: read back CRCs from server and let user know
   mismatches
   * deactivated or not supported: do nothing
   ...
   \param[in]   orc_WakeupConfig  configuration for wakeup (only required for
   mode = 1)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLFlashWrite::m_WriteFlashChecksums(
    const uint8_t ou8_Mode, const C_XFLWakeupParameters &orc_WakeupConfig) {
  int32_t s32_Block;
  uint16_t u16_Dummy;
  ...
  case 1U: // sector based
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_WRITING_CRCS),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    ...
    if (s32_Return == C_NO_ERR) {
      NodeSleep(); // make sure CRC over CRCs is written (required on some older
  ...
  case 3U: // block based flash
    ...
    // Helps to troubleshoot if the flashloader does not start the application
    // due to CRC mismatch
    TRG_ReportStatus("Verifying flash block checksums ...",
                     gu8_DL_REPORT_STATUS_TYPE_INFORMATION);

    s32_Return = this->ReadServerBlockChecksumInformation(c_CRCs);
    if (s32_Return == C_NO_ERR) // information read, check the content ...
    {
      for (s32_Block = 0; s32_Block < c_CRCs.c_BlockConfig.size();
           s32_Block++) {
        if (c_CRCs.c_BlockConfig[s32_Block].q_BlockDefinitionValid == false) {
          // this is a hard error: invalid blocks will most likely cause the
          ...
          s32_Return = C_CONFIG;
                   ...
                   c_CRCs.c_Areas[s32_Block].u32_ChecksumCalc) {
          // Mismatching CRCs are no hard errors as we might want to flash
          // multiple files in sequence So for the user this could just be an
          // information. So we do not return an error if we only have this type
          // of information.
          TRG_ReportStatus("Information: flash checksum of block " +
                               QString::number(s32_Block) +
                               " does not match the flash contents !",
                           gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
        } else {
          // looks good ...
        }
...
/*! \brief   do a auto-detect of the sectors to erase
   ...
   \param[in]   orc_FlashInfo       flash layout and timing information (as read
   with ::ReadFlashInformation)

   \return
   C_NO_ERR           no errors \n C_COM              could not read flash
   information from server C_CONFIG           flash information table reported
   from server inconsistent with reported  \n number of sectors C_RANGE hex file
   data occupies protected flash sector C_OVERFLOW         hex file data
   occupies memory not available on the server
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLFlashWrite::m_SetAutoSectors(C_HexFile &orc_HexFile,
  ...
  if (oq_AllButProtected == true) {
    ...
    for (s32_Index = 0; s32_Index < mau8_SectorsToErase.size(); s32_Index++) {
      mau8_SectorsToErase[s32_Index] =
          (c_SectorTable[s32_Index].q_IsProtected == false) ? 1U : 0U;
    }
  } else {
    ...
    while (pu8_NextLine != NULL) {
      ...
      // is it aliased ?
      if (orc_FlashInfo.c_Aliases.GetRangeOccupiedByAddress(
              u32_StartAddress, u8_Range) == C_NO_ERR) {
        ...
        if ((((static_cast<int64_t>(u32_StartAddress) + s64_Offset) < 0LL) ||
             ((static_cast<int64_t>(u32_StartAddress) + s64_Offset) >
              0xFFFFFFFFLL)) ||
            (((static_cast<int64_t>(u32_EndAddress) + s64_Offset) < 0LL) ||
             ((static_cast<int64_t>(u32_EndAddress) + s64_Offset) >
              0xFFFFFFFFLL))) {
          TRG_ReportStatus("Error: Aliasing would result in address overflow !",
                           gu8_DL_REPORT_STATUS_TYPE_ERROR);
          return C_RANGE;
        }

        u32_StartAddress = static_cast<uint32_t>(
            static_cast<int64_t>(u32_StartAddress) +
            orc_FlashInfo.c_Aliases[u8_Range].GetOffset());
        u32_EndAddress = static_cast<uint32_t>(
            static_cast<int64_t>(u32_EndAddress) +
            orc_FlashInfo.c_Aliases[u8_Range].GetOffset());
      }

      s32_Return = c_SectorTable.GetSectorOccupiedByAddress(u32_StartAddress,
                                                            u16_Sector);
      if (s32_Return == C_NO_ERR) {
        mau8_SectorsToErase[u16_Sector] = 1U;
        if (c_SectorTable[u16_Sector].q_IsProtected == true) {
          TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                               STR_FDL_ERR_HEX_ON_PROTECTED_S),
                           gu8_DL_REPORT_STATUS_TYPE_ERROR);
          return C_RANGE;
        }
      ...
      s32_Return =
          c_SectorTable.GetSectorOccupiedByAddress(u32_EndAddress, u16_Sector);
      if (s32_Return == C_NO_ERR) {
        mau8_SectorsToErase[u16_Sector] = 1U;
        if (c_SectorTable[u16_Sector].q_IsProtected == true) {
          TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
      ...
      if ((q_FoundStartSector == false) || (q_FoundEndSector == false)) {
        c_Text = QString::asprintf(
            ...
            "0x%X to 0x%X does not fit into an available flash sector !",
...
int32_t C_XFLFlashWrite::m_SetSectorsToErase(
  ...
  int32_t s32_Return;
  const uint16_t u16_NumSectorsTotal =
      static_cast<uint16_t>(mau8_SectorsToErase.size());

  for (int32_t s32_Sector = 0; s32_Sector < mau8_SectorsToErase.size();
       s32_Sector++) {
  ...
  if (ou16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00) {
    ...
    default:
      // do not hint at using "C-Application" (we do not really want the user to
      // use it)
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_ONLY_F_AUTO_UD),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
  } else {
  ...
  case eXFL_ERASE_MODE_C_APPLICATION:
    if (ou16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00) {
      ...
      for (uint16_t u16_Sector = 1; u16_Sector < u16_NumSectorsTotal;
           u16_Sector++) // erase everything except sector
                         //  0
      {
        mau8_SectorsToErase[u16_Sector] = 1U;
      }
      if (orc_Params.q_DivertStream ==
          true) // BBB -> don't erase parameter blocks
      {
        if (u16_NumSectorsTotal < 3U) {
          TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                               STR_FDL_ERR_C_APPL_NOT_POSSIBLE),
                           gu8_DL_REPORT_STATUS_TYPE_ERROR);
          return -1;
        }
  ...
  case eXFL_ERASE_MODE_CANOPEN_CONFIG:
    if (u16_NumSectorsTotal < 3U) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_CANOPEN_NOT_POSSIBLE),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
  ...
  case eXFL_ERASE_MODE_IEC_APPLICATION:
    ...
    default:
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_IECAPPL_NOT_POSSIBLE),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
    break;
  case eXFL_ERASE_MODE_IEC_RUN_TIME_SYSTEM:
    ...
    default:
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_IECRTS_NOT_POSSIBLE),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
    break;
...
int32_t C_XFLFlashWrite::m_SetUserDefinedSectors(const QString &orc_Sectors) {
  uint16_t u16_Index;
  uint16_t u16_Value;
  uint16_t u16_Value2;
  C_SclString c_Help;
  ...
  for (uint32_t u32_Sector = 0;
       u32_Sector < static_cast<uint32_t>(mau8_SectorsToErase.size());
       u32_Sector++) {
    mau8_SectorsToErase[u32_Sector] =
  ...
  for (uint32_t u32_Sector = 0U; u32_Sector < orc_Sectors.length();
       u32_Sector++) {
    if (pcn_String[u32_Sector] == ',') {
      pcn_String[u32_Sector] = '\0'; // separate tokens by terminating with "0"

      c_Help = (QString(&pcn_String[u16_Index])).trimmed();
      if (c_Help != "") // ignore "empty" commas
      {
        (void)c_Strings.append(c_Help);
      }
  ...
  for (uint32_t u32_Sector = 0U; u32_Sector < c_Strings.count(); u32_Sector++) {
    if (c_Strings[u32_Sector].indexOf("-") != -1) {
      // it's a "from - to" area
      c_Help = c_Strings[u32_Sector]
                   .mid(0, c_Strings[u32_Sector].indexOf("-"))
                   .trimmed();
      c_Help2 = c_Strings[u32_Sector]
                    .mid(c_Strings[u32_Sector].indexOf("-") + 1)
                    .trimmed();
      try {
      ...
      if ((u16_Value2 < u16_Value) ||
          (u16_Value >= static_cast<uint16_t>(mau8_SectorsToErase.size()))) {
        TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                             STR_FDL_ERR_SEC_ERASE_INV2),
                         gu8_DL_REPORT_STATUS_TYPE_ERROR);
        return -1;
      }
      for (uint16_t u16_IndexValue = u16_Value; u16_IndexValue <= u16_Value2;
           u16_IndexValue++) {
      ...
      // simple value
      try {
        u16_Value = static_cast<uint16_t>(c_Strings[u32_Sector].toInt());
      } catch (...) {
        return -1;
      }
...
/*! \brief   write fingerprint data to server
   ...
   Set application checksum to "0" to signal that programming was started, but
   not finished. For finger-print sequence cf. the STW flashloader
   specification.

   \param[in]    orc_FingerPrintIndexes  flags signaling which finger print
   services are available

   \return
   C_NO_ERR    finger print information written
   -1          could not write finger print information (parts may have been
   written)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLFlashWrite::m_WriteStartFingerPrint(
  ...
  if (orc_FingerPrintIndexes.q_ProgrammingDate == true) {
    s32_Return =
        SetFingerPrintProgrammingDate(static_cast<uint8_t>(c_Date.year() % 100),
                                      static_cast<uint8_t>(c_Date.month()),
                                      static_cast<uint8_t>(c_Date.day()));
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_WR_PROG_DATA),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
  }
  ...
  if (orc_FingerPrintIndexes.q_ProgrammingTime == true) {
    s32_Return =
        SetFingerPrintProgrammingTime(static_cast<uint8_t>(c_Time.hour()),
                                      static_cast<uint8_t>(c_Time.minute()),
                                      static_cast<uint8_t>(c_Time.second()));
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_WR_PROG_TIME),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
  }
  ...
  if (orc_FingerPrintIndexes.q_UsernamePart1 == true) {
    ...
    if (q_Return != true) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_RD_USER_NAME),
                       gu8_DL_REPORT_STATUS_TYPE_WARNING);
      c_UserName = "unknown";
    }
    TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                         STR_FDL_TXT_REC_USER_N_P1) +
                         c_UserName +
                         stw::opensyde_core::C_OscUtils::h_LoadString(
                             STR_FDL_TXT_REC_USER_N_P2),
                     gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    s32_Return = SetFingerPrintUserName(c_UserName);
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_WR_USER_NAME),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
  }
...
int32_t C_XFLFlashWrite::m_FlashESXLokalID(
    const QString &orc_DeviceId,
    const C_XFLFingerPrintSupportedIndexes &orc_FingerPrintIndexes,
    const C_XFLFlashWriteParameters &orc_Params,
  ...
  C_XFLActions::CompIDStringToStruct(orc_Params.c_WakeupConfig.c_CompanyID,
                                     c_CompID);

  s32_Return = 0U;

  u32_Return =
      c_HexFile.LoadFromFile(orc_Params.c_HexFile.toUtf8().constData());
  if (u32_Return != stw::hex_file::NO_ERR) {
    ...
    case 0U:
      ...
      break;
    case 1U:
      TRG_ReportStatus(c_Text + stw::opensyde_core::C_OscUtils::h_LoadString(
                                    STR_FDL_TXT_IGNORE),
                       gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
      break;
    case 2U:
      if ((u32_Return & stw::hex_file::ERR_MASK) ==
          stw::hex_file::WRN_RECORD_OVERLAY) {
        s32_Return = TRG_UserInteraction(
            eXFL_USER_INTERACTION_REASON_HEXFILE_OVERLAP, c_Text, u32_Return);
        if (s32_Return != C_NO_ERR) {
          s32_Return = -1;
        }
      } else {
  ...
  if (s32_Return != C_NO_ERR) {
    u32_FileChecksum = 0U;
    TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                         STR_FDL_WARN_COULD_NOT_CALC_APPL_CHK),
                     gu8_DL_REPORT_STATUS_TYPE_WARNING);
  }

  // device ID x-check:
  // perform device-id x-check
  s32_Return = m_DoDeviceIDXCheck(c_HexFile, orc_DeviceId, orc_Params,
                                  ou16_ProtocolVersion);
  if (s32_Return != 0) {
  ...
  if (ou16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00) {
    ...
    s32_Return =
        GetImplementationInformationHexRecords(u8_RecordLength, u8_Granularity);
    if (s32_Return != C_NO_ERR) {
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_RD_HEX_REC_INFO),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }

    m_ReportVerboseStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                              STR_FDL_TXT_CHG_HEX_REC_LENGTH) +
                          QString::number(u8_RecordLength) + ")");

    u32_Return = c_HexFile.Optimize(u8_RecordLength);
    if (u32_Return != NO_ERR) {
      TRG_ReportStatus(c_HexFile.ErrorCodeToErrorText(u32_Return),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      return -1;
    }
  } else {
    if (orc_Params.u8_HexRecordLength != 0U) {
      m_ReportVerboseStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                                STR_FDL_TXT_DO_CHG_HEX_REC_LENGTH) +
                            QString::number(orc_Params.u8_HexRecordLength));
  ...
  s32_Return = m_SetSectorsToErase(c_HexFile, orc_DeviceId, orc_Params,
                                   t_FlashInfo, ou16_ProtocolVersion);
  if (s32_Return != 0) {
    // error message already in subfunction
    return s32_Return;
  }
  ...
  c_Text =
      stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_SEC_ERASE) + " ";
  for (int32_t s32_Sector = 0; s32_Sector < mau8_SectorsToErase.size();
       s32_Sector++) {
    if (mau8_SectorsToErase[s32_Sector] == 1U) {
      c_Text = c_Text + QString::number(s32_Sector) + " ";
    }
  }
  TRG_ReportStatus(c_Text, gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
  TRG_ReportStatus(
      stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ERASING),
      gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
  ...
  for (int32_t s32_Sector = 0; s32_Sector < mau8_SectorsToErase.size();
    ...
    if (mau8_SectorsToErase[s32_Sector] == 1U) {
      c_Text =
          stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ERASING_SECTOR) +
          " " + QString::number(s32_Sector);
      ...
      if (ou16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00) {
        ...
        s32_Return =
            this->EraseSector(static_cast<uint16_t>(s32_Sector),
      ...
      if (s32_Return != C_NO_ERR) {
        c_Text =
            stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ERR_ERASE) +
            " " + QString::number(s32_Sector) + ": " +
            C_XFLActions::XFLProtocolErrorToText(s32_Return, GetLastXFLError());
        TRG_ReportStatus(c_Text, gu8_DL_REPORT_STATUS_TYPE_ERROR);
        return -1;
  ...
  c_Text = QString::asprintf(
      "%s %d s",
      stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ERASE_TIME)
          .toUtf8()
          .constData(),
      u32_EraseTime / 1000);
  m_ReportVerboseStatus(c_Text);
...

  {
    ...
    s32_Size = static_cast<int32_t>(QFileInfo(orc_Params.c_HexFile).size());
    if (s32_Size != -1) {
      m_ReportVerboseStatus(
          stw::opensyde_core::C_OscUtils::h_LoadString(STR_FILESIZE) + " " +
          QString::number(s32_Size) +
          stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_BYTES));
      const QFileInfo c_FileInfo(orc_Params.c_HexFile);
      c_Text = c_FileInfo.lastModified().toString("dd.MM.yyyy HH:mm:ss");
      m_ReportVerboseStatus(
          stw::opensyde_core::C_OscUtils::h_LoadString(STR_TIMESTAMP) + " " +
          c_Text);
    }
  }
  TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_WRITING),
                   gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
  m_ReportVerboseStatus("");
  ...
  if (pu8_Line != NULL) {
    ...
    for (;;) {
      // send 5 lines per call; bigger blocks might result in the callbacks not
      // executed for a long time with slow
      //  communication bitrates
      u16_NumSentNow = 5U;
      if (ou16_ProtocolVersion >= mu16_PROTOCOL_VERSION_3_00) {
        // pass erase timeout read with ::ReadFlashInformation
        // be a little conservative here: use the timeout required for the
        // slowest flash IC
        s32_Return = SendMultiHexLine(
            u16_NumSentNow, orc_Params.u16_InterFrameDelayUs, u32_NumBytes,
            c_HexFile, t_FlashInfo.GetWriteTimeOfSlowestIC());
      } else {
        // use default timeout
        s32_Return =
            SendMultiHexLine(u16_NumSentNow, orc_Params.u16_InterFrameDelayUs,
                             u32_NumBytes, c_HexFile);
      }
      ...
      case C_NO_ERR:
        ...
        break;
      case C_CHECKSUM:
        c_Text = QString::asprintf("%s %d)",
                                   stw::opensyde_core::C_OscUtils::h_LoadString(
                                       STR_FDL_ERR_WRITE_CHECKSUM)
                                       .toUtf8()
                                       .constData(),
                                   u32_LineCount);
        break;
      default:
        c_Text = (stw::opensyde_core::C_OscUtils::h_LoadString(
                      STR_FDL_ERR_WR_FLASH_DO_PT1) +
                  C_XFLActions::XFLProtocolErrorToText(s32_Return,
                                                       GetLastXFLError()) +
                  stw::opensyde_core::C_OscUtils::h_LoadString(
                      STR_FDL_ERR_WR_FLASH_DO_PT2) +
                  QString::number(u32_LineCount));
        break;
      }
      if (c_Text != "") {
        TRG_ReportStatus(c_Text, gu8_DL_REPORT_STATUS_TYPE_ERROR);
      ...
      if ((c_StepTimer.elapsed() > 750U) ||
          (u32_LineCount == u32_NumLinesTotal)) {
        const uint32_t u32_Diff = c_StepTimer.elapsed();

        c_Text = QString::asprintf(
            "%s %d / %d",
            stw::opensyde_core::C_OscUtils::h_LoadString(STR_FM_LINE)
                .toUtf8()
                .constData(),
            u32_LineCount, u32_NumLinesTotal);
        TRG_ReportStatus("<<<CLRLINE", gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
        ...
        if (u32_Diff != 0U) {
          f32_Rate =
              ((((static_cast<float32_t>(u32_NumSentProgress)) / 1024.0F) /
                (static_cast<float32_t>(u32_Diff))) *
               1000.0F);
          c_Text = QString::asprintf(
              "%s %d / %d at %2.2fkB/s",
              stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_BYTE)
                  .toUtf8()
                  .constData(),
              u32_NumBytesFlashed, u32_NumBytesTotal,
      ...
      s32_Return2 = TRG_ReportProgress(
          static_cast<uint16_t>((u32_LineCount * 1000U) / u32_NumLinesTotal),
          stw::opensyde_core::C_OscUtils::h_LoadString(
              STR_FDL_TXT_WR_FLASH_DO));
      if (s32_Return2 != C_NO_ERR) {
        TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                             STR_FDL_ERR_USER_ABORT),
                         gu8_DL_REPORT_STATUS_TYPE_ERROR);
        return -1;
      }
      if (s32_Return == 1) {
    ...
    c_Text = QString::asprintf(
        "%s %d s",
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_DOWNLOAD_TIME)
            .toUtf8()
            .constData(),
        u32_FlashTime / 1000U);
    m_ReportVerboseStatus(c_Text);
    ...
    f32_Rate = (((static_cast<float32_t>(u32_NumBytesTotal) / 1024.0F) /
                 (static_cast<float32_t>(u32_FlashTime))) *
                1000.0F);
    c_Text = QString::asprintf(
        "%s %2.2fkB/s",
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_AVERAGE_RATE)
            .toUtf8()
            .constData(),
        static_cast<float64_t>(f32_Rate));
    m_ReportVerboseStatus(c_Text);
    ...
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_UNKNOWN_ERR),
        gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return -1;
  }
  ...
  (void)m_WriteFingerPrintCheckSum(orc_FingerPrintIndexes.q_ApplicationCheckSum,
                                   u32_FileChecksum);
  return C_NO_ERR;
}
...
void C_XFLFlashWrite::m_ReportVerboseStatus(const QString &orc_StatusText) {
...
int32_t
C_XFLFlashWrite::m_WriteFingerPrintCheckSum(const bool oq_FingerPrintAvailable,
                                            const uint32_t ou32_Checksum) {
  int32_t s32_Return;
  ...
  if (s32_Return != C_NO_ERR) {
    this->TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                               STR_FDL_WARN_COULD_NOT_WR_APPL_CHK),
                           gu8_DL_REPORT_STATUS_TYPE_WARNING);
  }
  return s32_Return;
}
...
/*! \brief   extract device ID from hex-file
   ...
   If the device_info structure with the device ID information is in the hexfile
   more than once, AND the device ID in at least one of the copies is different,
   we fail.

   \param[in]     ou32_StartAddress                address offset to start
   searching at
   \param[in]     oq_FailIfNotFoundAtStartAddress  true -> the ID must be
   EXACTLY at start address
   \param[in]     orc_HexFile                      hexfile potentially
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLFlashWrite::m_ScanDeviceIDFromHexFile(
    const uint32_t ou32_StartAddress,
    const bool oq_FailIfNotFoundAtStartAddress, C_XFLHexFile &orc_HexFile,
    QString &orc_DeviceId) const {
  int32_t s32_Return;
  int32_t s32_Index;
  QString c_DeviceId = "";
  QString c_Help;
  ...
  s32_Return = orc_HexFile.GetECUInformationBlocks(
      ac_InfoBlocks, ou32_StartAddress, oq_FailIfNotFoundAtStartAddress,
      oq_FailIfNotFoundAtStartAddress, true);
  if ((s32_Return != C_NO_ERR) || (ac_InfoBlocks.size() == 0)) {
    if ((oq_FailIfNotFoundAtStartAddress == false) &&
        (ou32_StartAddress > 0U)) {
      // retry from beginning:
      s32_Return = orc_HexFile.GetECUInformationBlocks(
          ac_InfoBlocks, 0x00000000UL, false, false, true);
      if ((s32_Return != C_NO_ERR) || (ac_InfoBlocks.size() == 0)) {
        return C_NOACT;
      }
    } else {
...
/*! \brief   perform device ID x-check between hex-file and server's device ID
   ...
   Depending on configuration settings check whether the specified device ID
   matches the one in the hex-file.

   \param[in]     orc_HexFile           hexfile potentially containing the
   device-ID
   \param[in]     orc_DeviceId          device ID we expect to find in the
   hex-file
   \param[in]     orc_Params            flashloader parameters
   \param[in]     ou16_ProtocolVersion  protocol version as reported by server

   \return
   C_NO_ERR     everything OK (either device ID matches, override by user, not
   configured to be checked) C_CHECKSUM   device-ID does not match and no
   override (or other problems, cf. comments at parameter enums) -> fail
   C_CONFIG     at least one of the parameters is invalid
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLFlashWrite::m_DoDeviceIDXCheck(C_XFLHexFile &orc_HexFile,
                                    const QString &orc_DeviceId,
                                    const C_XFLFlashWriteParameters &orc_Params,
                                    const uint16_t ou16_ProtocolVersion) {
  ...
  int32_t s32_Return;
  uint32_t u32_Dummy;
  QString c_DeviceIDHexFile;
  QString c_Error;

  if ((orc_Params.e_DevTypeCheck > eXFL_DEV_TYPE_CHECK_GET_ADDRESS_THEN_FAIL) ||
      (orc_Params.e_DevTypeCheckGetIDFailedReaction >
       eXFL_DEV_TYPE_CHECK_GETID_FAIL_REACTION_ASK) ||
      (orc_Params.e_DevTypeCheckMatchIDFailedReaction >
       eXFL_DEV_TYPE_CHECK_MATCHID_FAIL_REACTION_ASK)) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ERR_CFG_DIC),
        gu8_DL_REPORT_STATUS_TYPE_ERROR);
    return C_CONFIG;
  }
  ...
  case eXFL_DEV_TYPE_CHECK_GET_ADDRESS_THEN_FAIL:
    ...
    if (mt_AvailableServices.q_GetDeviceInfoAddress == true) {
      ...
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_ID_CHECK_ID_ADDR_NA),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      s32_Return = C_CHECKSUM;
    }
  ...
  if (s32_Return == C_NO_ERR) // maybe there was already an error getting the
                              // address with the ...._THEN_FAIL option
  {
    s32_Return = this->m_ScanDeviceIDFromHexFile(
        u32_StartAddress, q_ExactAddressRequired, orc_HexFile,
        c_DeviceIDHexFile);
    switch (s32_Return) {
    case C_NO_ERR:
      break;
    case C_NOACT:
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_ID_CHECK_ID_NA_HEX),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      break;
    case C_CONFIG:
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_ID_CHECK_ID_AMBIG_HEX),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      break;
    default:
      TRG_ReportStatus(stw::opensyde_core::C_OscUtils::h_LoadString(
                           STR_FDL_ERR_ID_CHECK_ID_UNDEF),
                       gu8_DL_REPORT_STATUS_TYPE_ERROR);
      break;
    }
  }
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    case eXFL_DEV_TYPE_CHECK_GETID_FAIL_REACTION_ASK:
      s32_Return =
          TRG_UserInteraction(eXFL_USER_INTERACTION_REASON_GETID_FAIL,
                              stw::opensyde_core::C_OscUtils::h_LoadString(
                                  STR_FDL_TXT_ID_NA_CHECK_HEX_FILE),
                              u32_Dummy);
      if (s32_Return != C_NO_ERR) {
        s32_Return = C_CONFIG;
      }
  ...
  if (c_DeviceIDHexFile.trimmed() == orc_DeviceId.trimmed()) {
    TRG_ReportStatus(
        stw::opensyde_core::C_OscUtils::h_LoadString(STR_FDL_ID_CHECK_MATCH),
        gu8_DL_REPORT_STATUS_TYPE_INFORMATION);
    return C_NO_ERR;
  }

  c_Error = stw::opensyde_core::C_OscUtils::h_LoadString(
                STR_FDL_ERR_ID_CHECK_MISMATCH_PT1) +
            c_DeviceIDHexFile.trimmed() +
            stw::opensyde_core::C_OscUtils::h_LoadString(
                STR_FDL_ERR_ID_CHECK_MISMATCH_PT2) +
            orc_DeviceId.trimmed() + "\"";
  ...
  case eXFL_DEV_TYPE_CHECK_MATCHID_FAIL_REACTION_ASK:
    s32_Return = TRG_UserInteraction(eXFL_USER_INTERACTION_REASON_MATCHID_FAIL,
                                     c_Error, u32_Dummy);
    if (s32_Return != C_NO_ERR) {
      s32_Return = C_CONFIG;
    }
...
/*! \brief   Default implementation of user interaction interface
   ...
   Derived implementation should return C_NO_ERR to continue the ongoing
   process.
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLFlashWrite::TRG_UserInteraction(const E_XFLUserInteractionReason oe_Reason,
                                     const QString &orc_MessageText,
                                     uint32_t &oru32_AdditionalValue) {
  (void)oe_Reason;
  (void)orc_MessageText;
  (void)oru32_AdditionalValue;
  TRG_ReportStatus(
      "CXFLDownload configuration error: Configuration requires user "
      "interaction, but TRG_UserInteraction is not overloaded !",
      gu8_DL_REPORT_STATUS_TYPE_ERROR);
  return C_CONFIG;
}
