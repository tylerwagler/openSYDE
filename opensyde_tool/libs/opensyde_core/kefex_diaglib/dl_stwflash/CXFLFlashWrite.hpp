...
/// User interaction reasons passed to TRG_UserInteraction by
/// C_XFLDownload::Execute:
enum E_XFLUserInteractionReason {
  ...
  /// used with C_XFLDownloadParameters::E_XFLDevTypeCheckGetIDFailReaction =
  /// ASK
  eXFL_USER_INTERACTION_REASON_GETID_FAIL = 1,

  /// device-id from hex-file does not match the one of target \n
  /// used with C_XFLDownloadParameters::E_XFLDevTypeCheckMatchIDFailReaction =
  /// ASK
  eXFL_USER_INTERACTION_REASON_MATCHID_FAIL = 2,

  /// flashing finished, ask for action \n
  /// used with C_XFLDownloadParameters::E_XFLFlashFinishedAction =
  /// XFLFlashFinishedActionAskUser \n opu32_AdditionalReturnValue out: action
  /// to perform (E_XFLFlashFinishedAction)
  eXFL_USER_INTERACTION_REASON_FINISHED = 3
};

//----------------------------------------------------------------------------------------------------------------------
/// Class for performing the actual write flash procedure
class C_XFLFlashWrite : public C_XFLActions {
private:
  void m_ReportVerboseStatus(const QString &orc_StatusText);

  int32_t m_SetUserDefinedSectors(const QString &orc_Sectors);
  int32_t m_FlashESXLokalID(
  ...
  int32_t m_SetSectorsToErase(stw::hex_file::C_HexFile &orc_HexFile,
                              const QString &orc_DeviceId,
                              const C_XFLFlashWriteParameters &orc_Params,
                              C_XFLFlashInformation &orc_FlashInfo,
                              const uint16_t ou16_ProtocolVersion);
  int32_t m_SetAutoSectors(stw::hex_file::C_HexFile &orc_HexFile,
  ...
  int32_t m_WriteFingerPrintCheckSum(const bool oq_FingerPrintAvailable,
...
protected:
  ...
  // Used when feedback from application is required; only required with "ASK"
  // configurations \n return C_NO_ERR to ignore and continue
  virtual int32_t
  TRG_UserInteraction(const E_XFLUserInteractionReason oe_Reason,
