...
/*!
   ...
   Provides functions to use STW flashloader protocol services to interact with
   flashloader server nodes.

   \copyright   Copyright 1999 Sensor-Technik Wiedemann GmbH. All rights
   reserved.
*/
//----------------------------------------------------------------------------------------------------------------------
...
// timeout values (sum of timeout specified in protocol + 100ms tolerance for
...
static const uint8_t mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_PROTECTED_SECTORS =
    0x03U;
static const uint8_t mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_OFFSET_ADDRESS =
    0x04U;
...
static const uint8_t mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ALIAS_PHYSICAL =
    0x0AU;
static const uint8_t mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ALIAS_SIZE = 0x0BU;
static const uint8_t mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ALIAS_ADDRESS =
    0x0CU;

// get_implementation_information
static const uint8_t
    mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_PROTOCOL_VERSION = 0x01U;
static const uint8_t
    mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_SPECIAL_SERVICES = 0x02U;
static const uint8_t mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_GET_SERVICES =
    0x03U;
static const uint8_t mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_SET_SERVICES =
    0x04U;
static const uint8_t mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_FLASH_SERVICES =
    0x05U;
static const uint8_t
    mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_TARGET_SPECIFIC_SERVICES = 0x06U;
static const uint8_t mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_HEX_RECORDS =
    0x07U;
...
C_XFLProtocolConfig::C_XFLProtocolConfig(void)
    : pc_CANDispatcher(NULL), u32_SendID(XFL_DEFAULT_SEND_ID),
      u32_ReceiveID(XFL_DEFAULT_SEND_ID + 1U), u8_LocalID(0U),
      u32_Bitrate(125U), q_XtdID(false) {}

//----------------------------------------------------------------------------------------------------------------------
// returns offset to add to aliased address to reach physical address:
int64_t C_XFLAliasedRange::GetOffset(void) const {
  return (static_cast<int64_t>(this->u32_PhysicalAddress) -
          static_cast<int64_t>(this->u32_AliasedAddress));
}
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLAliasedRanges::GetRangeOccupiedByAddress(const uint32_t ou32_Address,
                                              uint8_t &oru8_Range) const {
  ...
  for (s32_Index = 0; s32_Index < this->length(); s32_Index++) {
    if ((ou32_Address >= this->operator[](s32_Index).u32_AliasedAddress) &&
        (ou32_Address < (this->operator[](s32_Index).u32_AliasedAddress +
                         this->operator[](s32_Index).u32_Size))) {
...
C_XFLImplementedServices::C_XFLImplementedServices(void) { this->Clear(); }
...
uint8_t C_XFLProtocol::mh_BCD2DEC(const uint8_t ou8_BCDValue) {
  return (static_cast<uint8_t>(
      ((((ou8_BCDValue) / 16U) * 10U) + ((ou8_BCDValue) % 16U))));
}
...
uint8_t C_XFLProtocol::mh_DEC2BCD(const uint8_t ou8_DECValue) {
  return (static_cast<uint8_t>(
      ((((ou8_DECValue) / 10U) * 16U) + ((ou8_DECValue) % 10U))));
}
...
/*! \brief   Make comm dispatcher to use known
   ...
   \param[in]   opc_CANDispatcher  comm dispatcher to use (NULL: remove
   installed dispatcher)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_XFLProtocol::CfgSetCommDispatcher(
...
/*! \brief   Set CAN TX and RX CAN identifiers
   ...
   If the ReceiveID is set to 0xFFFFFFFF we accept all responses for
   compatibility with stwrs232.dll

   \param[in]   ou32_SendId      TX CAN ID to send requests on
   \param[in]   ou32_ReceiveId   TX CAN ID to expect responses on
*/
//----------------------------------------------------------------------------------------------------------------------
void C_XFLProtocol::CfgSetFlashId(const uint32_t ou32_SendId,
                                  uint32_t const ou32_ReceiveId) {
...
/*! \brief   Send "FLASH" requests
   ...
   Send the "FLASH" request for a defined period of time with a configurable
   interval. Will report progress to application via the installed progress
   callback. Using the progress callback this function can also be aborted.

   \param[in]   ou32_StartTimeMs       period to send the message for in
   milli-seconds
   \param[in]   ou8_FLASHIntervalMs    interval between individual messages in
   milli-seconds
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SendFLASH(const uint32_t ou32_StartTimeMs,
                                 const uint8_t ou8_FLASHIntervalMs) {
  ...
  c_Timer.start();
  TRG_ReportProgress(0U, stw::opensyde_core::C_OscUtils::h_LoadString(
                             STR_FDL_TXT_WR_FLASH_RQ));
  do {
    (void)m_SendFlashMessage(); // do not check for return = OK here:
                                // network might still be down resulting in send
                                // errors
    ...
    if (ou32_StartTimeMs != 0U) {
      ...
      s32_Return =
          TRG_ReportProgress(static_cast<uint16_t>(u32_Progress),
  ...
  TRG_ReportProgress(1000U, stw::opensyde_core::C_OscUtils::h_LoadString(
                                STR_FDL_TXT_WR_FLASH_RQ));
  return C_NO_ERR;
}
...
/*! \brief   Implementation of erase_sector service.
   ...
   For compatibility reasons ou32_TimeOut will only be used if it is greater
   than TIMEOUT_ERASE. Otherwise TIMEOUT_ERASE will be used.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::EraseSector(const uint16_t ou16_SectorNumber,
  ...
  if (ou16_SectorNumber > 255U) {
    mc_CanWriteMessage.au8_Data[4] =
        static_cast<uint8_t>(ou16_SectorNumber >> 8U);
    mc_CanWriteMessage.u8_DLC = 5U;
  }
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, u32_TimeOut,
                           mc_CanWriteMessage.au8_Data,
                           static_cast<uint8_t>(mc_CanWriteMessage.u8_DLC - 1U),
                           NULL, mc_CanWriteMessage.u8_DLC);
}
...
/*! \brief   wait for a "regular" response from the server node
   ...
   \param[in]     os16_CheckDLC              -1 -> do not check response DLC,
   else: response DLC must be ==os16_CheckDLC

   \param[out]    opc_MSG                    received message (NULL -> do not
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::m_WaitForResponse(const uint8_t ou8_ExpectedLocalID,
                                         const uint32_t ou32_TimeOutMS,
                                         const uint8_t (&orau8_Match)[8],
                                         const uint8_t ou8_NumExpectedToMatch,
                                         T_STWCAN_Msg_RX *const opc_MSG,
                                         const int16_t os16_CheckDLC) {
  uint32_t u32_StartTime;
  ...
  do {
    ...
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      if (t_RXMsg.au8_Data[0] == ou8_ExpectedLocalID) {
        if (memcmp(&orau8_Match[1], &t_RXMsg.au8_Data[1],
                   ou8_NumExpectedToMatch) == 0) {
          if ((os16_CheckDLC == -1) ||
              (t_RXMsg.u8_DLC == static_cast<uint8_t>(os16_CheckDLC))) {
            if (opc_MSG != NULL) {
              (*opc_MSG) = t_RXMsg;
            }
...
int32_t C_XFLProtocol::ProgFlash(void) {
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_CHANGE_STATE_MS,
                           mc_CanWriteMessage.au8_Data, 2U, NULL, 3);
}
...
/*! \brief   Send one hex file line to server ECU
   ...
   For compatibility reasons ou32_TimeOut will only be used if it is greater
   than TIMEOUT_FLASH. Otherwise TIMEOUT_FLASH will be used.

   \param[in]     opu8_HexLine              data to send
   \param[in]     ou16_LineSize             number of bytes in opu8_HexLine
   \param[in]     ou32_InterFrameDelayUs    delay to keep between individual CAN
   frames in micro-seconds
   \param[in]     ou32_TimeOut              maximum timeout for writing one hex
   line in ms
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::m_SendHexLine(const uint8_t *const opu8_HexLine,
                                     const uint16_t ou16_LineSize,
                                     const uint32_t ou32_InterFrameDelayUs,
                                     const uint32_t ou32_TimeOut) {
  ...
  do {
    ...
    while (s32_RemainingLength > 0) {
      u8_SendNow = static_cast<uint8_t>(
          (s32_RemainingLength > 7) ? 7 : s32_RemainingLength);
      (void)memcpy(&mc_CanWriteMessage.au8_Data[1],
                   &opu8_HexLine[s32_Counter - 1], u8_SendNow);
      ...
      if (s32_RemainingLength > 0) {
        TRG_WaitMicroSeconds(
            ou32_InterFrameDelayUs); // TX-delay between individual messages
        TRG_HandleSystemMessages();
      }
    }
      ...
      if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
        if (t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) {
          ...
          case 0x00U:
            ...
            u32_Tries =
                hu16_MAX_TRIES + 1U; // checksum OK -> finished with this line
            break;
          case mu8_COMMAND_ERR:
            switch (t_RXMsg.au8_Data[2]) {
            case XFL_ERR_CHECKSUM_ERR:
            case 0xF5U: // incorrect checksum return value in ESX2 flashloader
                        // up to V2.01
          ...
          default:
            break;
             ...
             (c_Timer.hasExpired(u32_TimeOut) == false));
    TRG_HandleSystemMessages();
  } while (u32_Tries < hu16_MAX_TRIES);
  if (u32_Tries != (hu16_MAX_TRIES + 1U)) {
...
/*! \brief   Send lines of a hex file to server ECU
   ...
   \param[in,out] oru16_NumLines            number of lines to send / number of
   lines sent
   \param[in]     ou32_InterFrameDelayUs    delay to keep between individual CAN
   frames in micro-seconds
   \param[in,out] orc_HexFile               hex file class instance with opened
   hex file
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SendMultiHexLine(uint16_t &oru16_NumLines,
                                        const uint32_t ou32_InterFrameDelayUs,
                                        uint32_t &oru32_NumBytesSent,
                                        C_HexFile &orc_HexFile,
                                        const uint32_t ou32_TimeOut) {
  ...
  do {
    ...
    u16_LineLength =
        static_cast<uint16_t>((static_cast<uint16_t>(pu8_NextLine[0])) + 5U);

    s32_Return = m_SendHexLine(pu8_NextLine, u16_LineLength,
                               ou32_InterFrameDelayUs, ou32_TimeOut);
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
    TRG_HandleSystemMessages();
...
/*! \brief   perform a wakeup with serial number
   ...
   \param[out]    opc_CompanyIDOut   server's reported company ID (NULL -> not
   interested)

   \return
   C_NO_ERR           no errors \n C_WARN             server responded with
   company ID different from ou8_CompanyID  \n C_COM              no response
   from server
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::WakeupSerialNumber(
    const uint8_t (&orau8_SerialNumber)[6], const C_XFLCompanyID &orc_CompanyID,
  ...
  do {
    ...
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      if ((t_RXMsg.au8_Data[1] == mu8_XFL_CMD_GRP_NODE_WAKEUP) &&
        ...
        return m_SendCompanyIDExt(oru8_LocalID, orc_CompanyID, u8_Dummy,
                                  opc_CompanyIDOut);
      }
    }
...
int32_t C_XFLProtocol::m_NodeFlashFunc(const uint8_t ou8_SubCommand) {
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_CHANGE_STATE_MS,
                           mc_CanWriteMessage.au8_Data, 2U, NULL);
}
...
int32_t C_XFLProtocol::GetSectorCount(uint16_t &oru16_NumSectors) {
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
                        mc_CanWriteMessage.au8_Data, 2U, &t_RXMsg);
  if (s32_Return == C_NO_ERR) {
...
// .PARAMETERS
// INPUT:
//              uint8 *opu8_LocalId          Array with all received Local_Ids
//              from CAN network uint8 oru8_NumMaxLocalId     Length of record
//              *pbLocalID
// OUTPUT:
...
// else error
//************************************************************************
int32_t C_XFLProtocol::GetLocalIDExt(uint8_t *const opu8_LocalId,
                                     const uint8_t ou8_NumMaxLocalId,
...
int32_t C_XFLProtocol::NodeSleep(const uint8_t ou8_LocalID) {
  ...
  // use write timeout for compatibility reasons: some ancient flashloader
  // implementations update a CRC over
  //  configuration data when going to sleep ...
  return m_WaitForResponse(ou8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data, 2U, NULL, 3);
}
...
int32_t C_XFLProtocol::GetSNR(uint8_t (&orau8_SNR)[6]) {
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
                        mc_CanWriteMessage.au8_Data, 1U, &t_RXMsg, 8);
  if (s32_Return == C_NO_ERR) {
...
int32_t C_XFLProtocol::SetLocalID(const uint8_t ou8_LocalIDNew) {
  ...
  do {
    TRG_HandleSystemMessages();
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      // some ESX implementations will answer with old local ID in contradiction
...
/*! \brief   Change the flash bitrate of a node
   ...
   \param[in]      oq_32BitBitrate     true  -> 32 bit bitrate -> bitrate
   resolution bits/second \n false -> 16 bit bitrate -> bitrate resolution
   kBits/second
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetBitrateCAN(const uint32_t ou32_Bitrate,
                                     const bool oq_32BitBitrate) {
  mc_CanWriteMessage.au8_Data[1] = mu8_XFL_CMD_GRP_SET_COMMAND;
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data,
                           static_cast<uint8_t>(mc_CanWriteMessage.u8_DLC - 1U),
                           NULL, mc_CanWriteMessage.u8_DLC);
}
...
int32_t C_XFLProtocol::SetCANID(const uint32_t ou32_CANID) {
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data, 6U, NULL, 7);
}
...
int32_t C_XFLProtocol::SetCANType(const bool oq_XtdOn) {
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data, 3U, NULL, 4);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::ReadFlash(const uint8_t ou8_NumBytes,
  ...
  mc_CanWriteMessage.au8_Data[5] =
      static_cast<uint8_t>(ou32_StartAddress >> 8U);
  mc_CanWriteMessage.au8_Data[6] =
      static_cast<uint8_t>(ou32_StartAddress >> 16U);
  mc_CanWriteMessage.au8_Data[7] =
      static_cast<uint8_t>(ou32_StartAddress >> 24U);
  ...
  do {
    ...
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      ...
      if (memcmp(&mc_CanWriteMessage.au8_Data[0], &t_RXMsg.au8_Data[0], 7) ==
          0) {
        s32_Return = C_NO_ERR; // correct response message
      } else if ((t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) &&
                 (t_RXMsg.au8_Data[1] == mu8_COMMAND_ERR)) {
        mu8_LastError = t_RXMsg.au8_Data[2];
        s32_Return = C_NOACT;
      } else {
        // not what we expect ...
  ...
  } while (c_Timer.hasExpired(TIMEOUT_FLASH_MS) ==
           false); //"TIMEOUT_FLASH" used for compatibility reasons
  ...
  for (u8_Loop = 0U; u8_Loop < static_cast<uint8_t>((ou8_NumBytes / 7U) + 1U);
       u8_Loop++) {
    QElapsedTimer c_Timer;
    c_Timer.start();
    s32_Return = C_COM;
    do {
      ...
      if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
        if (t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) {
          ...
          u8_CopyNow = static_cast<uint8_t>(((ou8_NumBytes - u16_Counter) > 7)
                                                ? 7U
                                                : (ou8_NumBytes - u16_Counter));
          if (t_RXMsg.u8_DLC < static_cast<uint8_t>(1U + u8_CopyNow)) {
            s32_Return = C_NOACT;
          } else {
            (void)memcpy(&opu8_Data[u16_Counter], &t_RXMsg.au8_Data[1],
                         u8_CopyNow);
...
/*! \brief   Implementation of get_version_number.
   ...
   - The size (1/5) is returned to the caller to determine how to handle the
   server node
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetVersionNumber(uint8_t (&orau8_Version)[5],
                                        uint8_t &oru8_VersionSize) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
...
//              oru8_NumFound   Returns the number of responses (i.e. number of
//              controllers) opc_CompIDOut   Returns reported CompID; (for "Y*"
//              Flashing); pass NULL if not interested
//----------------------------------------------------------------------------------------------------------------------
// .RETURNVALUE sint32
...
// C_COM      -> no response
//************************************************************************
int32_t C_XFLProtocol::m_SendCompanyIDExt(const uint8_t ou8_LocalID,
                                          const C_XFLCompanyID &orc_CompIDIn,
                                          uint8_t &oru8_NumFound,
                                          C_XFLCompanyID *const opc_CompIDOut) {
  uint32_t u32_StartTime;
  ...
  mc_CanWriteMessage.u32_ID = mc_Config.u32_SendID;
  mc_CanWriteMessage.u8_DLC =
      static_cast<uint8_t>(3U + orc_CompIDIn.u8_NumBytes);
  mc_CanWriteMessage.au8_Data[0] = ou8_LocalID;
  mc_CanWriteMessage.au8_Data[1] = mu8_XFL_CMD_GRP_FLASH_COMMAND;
  ...
  do {
    ...
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      if ((t_RXMsg.au8_Data[0] == ou8_LocalID) &&
        ...
        u8_Found++; // mark as found even if the returned company id is not the
                    // requested one an OK response means the target has
                    // accepted the company-ID

        s32_Diff = memcmp(&mc_CanWriteMessage.au8_Data[3], &t_RXMsg.au8_Data[3],
                          orc_CompIDIn.u8_NumBytes);
        // is it the exact company-ID we sent ?
        if ((t_RXMsg.u8_DLC != mc_CanWriteMessage.u8_DLC) || (s32_Diff != 0)) {
          if (s32_Return ==
              C_NO_ERR) // only report the first differing response
          {
            if (opc_CompIDOut != NULL) {
              opc_CompIDOut->u8_NumBytes =
                  static_cast<uint8_t>(t_RXMsg.u8_DLC - 3U);
              (void)memcpy(&opc_CompIDOut->au8_Data[0], &t_RXMsg.au8_Data[3],
                           opc_CompIDOut->u8_NumBytes);
            }
          }
          s32_Return =
              C_WARN; // at least one server responded with different CID !
        }
      }
    }
  } while (c_Timer.hasExpired(mu16_TIMEOUT_CHANGE_STATE_MS) == false);
...
/*! \brief   perform a wakeup with local ID
   ...
   \param[in]      orc_CompanyID     server's company ID
   \param[out]     opu8_NumFound     Number of responses (i.e. number of
   controllers)
   \param[out]     opc_CompanyIDOut  Returns reported CompID (reports company ID
   of first node that differs from orc_CompanyID); pass NULL if not interested
   \return
   C_NO_ERR           no errors \n C_WARN             at least one server
   responded with company ID different from ou8_CompanyID  \n C_COM no response
   from server
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::WakeupLocalId(const C_XFLCompanyID &orc_CompanyID,
                                     uint8_t *const opu8_NumFound,
  ...
  for (;;) {
    ...
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      if ((t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) &&
          (t_RXMsg.au8_Data[1] == mu8_XFL_CMD_GRP_NODE_WAKEUP)) {
        u8_NumFoundWakeup++;
      }
    }
  ...
  s32_Return = m_SendCompanyIDExt(mc_Config.u8_LocalID, orc_CompanyID,
                                  u8_NumFoundCompID, opc_CompanyIDOut);
  if (((s32_Return == C_NO_ERR) || (s32_Return == C_WARN)) &&
      (opu8_NumFound != NULL)) {
    // whatever reported more:
    (*opu8_NumFound) = (u8_NumFoundWakeup > u8_NumFoundCompID)
                           ? u8_NumFoundWakeup
                           : u8_NumFoundCompID;
  }
...
/*! \brief   Scan for nodes in the network.
   ...
   \param[out]     orau8_LocalIds     Number of responses for each ID.
                                       e.g. orau8_LocalIds[1] contains the
   number of responding controllers on Local ID 1 after call.
   \param[out]     opu8_NumIDs        Total number of found nodes
   \param[in]      ou32_StartTime     Period to send the message for in
   milli-seconds
   \param[in]      ou8_FLASHInterval  Interval between individual messages in
   milli-seconds
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::SearchId(uint8_t (&orau8_LocalIds)[XFL_NUM_DIFFERENT_LOCAL_IDS],
                        uint8_t *const opu8_NumIDs,
                        const uint32_t ou32_StartTime,
  ...
  for (u8_LID = 0U; u8_LID < (XFL_NUM_DIFFERENT_LOCAL_IDS - 1U);
       u8_LID++) //-1: ID 255 is reserved !
  {
    ...
    if (mc_Config.u32_Bitrate != 0U) {
      TRG_WaitMicroSeconds(static_cast<uint32_t>(250000U) /
                           mc_Config.u32_Bitrate); // use ca 50% busload ...
    } else {
...
// .PARAMETERS
// INPUT:
//    ou8_NumMax     Maximum number of answers to record to prevent overflow in
//    pbSNR
// OUTPUT:
//    *opu8_SNR         Returns the received SNR(s), each 6 bytes
//                       the calling function has to provide a buffer
//                       large enough
//                      Set to NULL if only interested in the number of
//                      responses.
...
//************************************************************************
int32_t C_XFLProtocol::GetSNRExt(uint8_t *const opu8_SNR,
                                 const uint8_t ou8_NumMax,
                                 uint8_t &oru8_NumFound) {
  uint32_t u32_StartTime;
  T_STWCAN_Msg_RX t_RXMsg;
  uint8_t u8_Found = 0U;
...
/*! \brief   Implementation of get_sec_crc service.
   ...
   \param[out]     oru16_CRC      freshly calculated checksum returned by the
   server
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetSecCRC(const uint16_t ou16_Sector,
                                 uint16_t &oru16_CRC, uint16_t &oru16_EECRC) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_CALC_CHECKSUM_MS,
                        mc_CanWriteMessage.au8_Data, 2U, &t_RXMsg, 7);
  if (s32_Return == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetSecCRC(const uint16_t ou16_Sector,
                                 uint16_t &oru16_CRC) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_CALC_CHECKSUM_MS,
                        mc_CanWriteMessage.au8_Data, 2U, &t_RXMsg, 5);
  if (s32_Return == C_NO_ERR) {
    oru16_CRC = mh_AU8ToU16LE(&t_RXMsg.au8_Data[3]);
  }
  return s32_Return;
...
//  C_NOACT        -> error response
//  C_NO_ERR       -> no errors
int32_t C_XFLProtocol::GetModeCompare(const uint16_t ou16_Sector,
                                      uint8_t &oru8_ModeCompare) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
  ...
  // might take some time to create checksum on controller
  s32_Return = m_WaitForResponse(
      mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
      mc_CanWriteMessage.au8_Data,
...
//  C_NOACT        -> error response
//  C_NO_ERR       -> no errors
int32_t C_XFLProtocol::SetModeCompare(const uint16_t ou16_Sector,
                                      const uint8_t ou8_ModeCompare) {
...
int32_t C_XFLProtocol::GetEraseCount(uint32_t &oru32_EraseCount) {
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
                        mc_CanWriteMessage.au8_Data, 2U, &t_RXMsg, 6);
  if (s32_Return == C_NO_ERR) {
...
// C_NOACT        -> error response
//************************************************************************
int32_t C_XFLProtocol::SetDivertParameter(const uint8_t ou8_Device,
                                          const uint8_t ou8_Position,
                                          const uint8_t ou8_ParameterNumber,
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_CHANGE_STATE_MS,
                           mc_CanWriteMessage.au8_Data, 5U, NULL, 8);
}
...
// C_NOACT      -> error response
//************************************************************************
int32_t C_XFLProtocol::SetTimeoutFactor(const uint8_t ou8_Type,
                                        const uint8_t ou8_Factor) {
  if (ou8_Type > 1U) {
    return C_RANGE;
  }
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data, 4U, NULL);
}
...
//              oru8_Factor     timeout value in controller (cf.
//              flashloader-spec)
//----------------------------------------------------------------------------------------------------------------------
// .RETURNVALUE sint32
// C_NO_ERR         -> no errors
...
// C_NOACT          -> error response
//************************************************************************
int32_t C_XFLProtocol::GetTimeoutFactor(const uint8_t ou8_Type,
                                        uint8_t &oru8_Factor) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
                        mc_CanWriteMessage.au8_Data, 3U, &t_RXMsg);
  if (s32_Return == C_NO_ERR) {
...
// =====================================================================
//  ESX2 specific function
int32_t C_XFLProtocol::BBWakeup(const uint8_t ou8_Position,
                                const uint8_t (&orau8_LastUserSignature)[2]) {
  mc_CanWriteMessage.u8_DLC = 6U;
  mc_CanWriteMessage.au8_Data[1] = mu8_XFL_CMD_GRP_FLASH_COMMAND;
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_CHANGE_STATE_MS,
                           mc_CanWriteMessage.au8_Data, 5U, NULL, 6);
}

// DivertStream
// =====================================================================
// Compatibility version for flashloader protocol < V3.02r0
int32_t C_XFLProtocol::DivertStream(const uint8_t ou8_Device,
                                    const uint8_t ou8_Position,
                                    const uint8_t ou8_State) {
  mc_CanWriteMessage.u8_DLC = 6U;
  mc_CanWriteMessage.au8_Data[1] = mu8_XFL_CMD_GRP_FLASH_COMMAND;
...
/*! \brief   Activate or deactivate divert stream mechanism
   ...
   For previous versions use the DivertStream() function without the "handle"
   parameter. For details about the "handle" cf. the STW flashloader protocol
   specification.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::m_GetDeviceIDBlock(const uint8_t ou8_BlockIndex,
                                          char_t (&oracn_Data)[4]) {
  uint32_t u32_StartTime;
  T_STWCAN_Msg_RX t_RXMsg;
    ...
    if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
      if (t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) {
        if ((t_RXMsg.au8_Data[1] == mu8_XFL_CMD_GRP_GET_COMMAND) &&
            ...
            (t_RXMsg.au8_Data[3] == ou8_BlockIndex) && (t_RXMsg.u8_DLC >= 4U)) {
          (void)memcpy(&oracn_Data[0], &t_RXMsg.au8_Data[4],
                       static_cast<uint32_t>(t_RXMsg.u8_DLC) - 4U);
          if (t_RXMsg.u8_DLC < 8U) {
            return C_WARN;
          }
          return C_NO_ERR;
...
/*! \brief   Read device-ID from server node
   ...
   \param[in]      oq_LongID      true  -> use indexed protocol (up to 16
   characters, requires protocol > V3.0 on server false -> use simple protocol
   (up to 5 characters, < protocol version V3.0)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetDeviceID(const bool oq_LongID,
                                   QString &orc_DeviceId) {
  uint32_t u32_StartTime;
  int32_t s32_Return;
  uint8_t u8_Index;
  char_t acn_Buffer[4];
      ...
      if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
        if (t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) {
          if ((t_RXMsg.au8_Data[1] == mu8_XFL_CMD_GRP_GET_COMMAND) &&
              (t_RXMsg.au8_Data[2] == mu8_XFL_CMD_GET_DEVICE_ID) &&
              (t_RXMsg.u8_DLC > 3U)) {
            (void)memcpy(&acn_DeviceID[0], &t_RXMsg.au8_Data[3],
                         static_cast<uint32_t>(t_RXMsg.u8_DLC) - 3U);
            s32_Return = C_NO_ERR;
          }
          if (t_RXMsg.au8_Data[1] == mu8_COMMAND_ERR) {
            mu8_LastError = t_RXMsg.au8_Data[2];
...
//************************************************************************
uint8_t C_XFLProtocol::GetLastXFLError(void) const { return mu8_LastError; }
...
int32_t C_XFLProtocol::m_CANGetResponse(T_STWCAN_Msg_RX &orc_MSG) {
  ...
  while (s32_Return == C_NO_ERR) {
    s32_Return = mc_Config.pc_CANDispatcher->CAN_Read_Msg(
        mu16_CANDispatcherHandle, orc_MSG);
    if ((s32_Return == C_NO_ERR) &&
...
int32_t C_XFLProtocol::EEPROMRead(const uint8_t ou8_NumBytes,
                                  const uint16_t ou16_StartAddress,
                                  uint8_t *const opu8_Data) {
  uint32_t u32_StartTime;
  int32_t s32_Return;
  ...
  mc_CanWriteMessage.au8_Data[5] =
      static_cast<uint8_t>(ou16_StartAddress >> 8U);
  m_SendMessageWithIDs();
  ...
  u8_NumMessages = static_cast<uint8_t>(
      (ou8_NumBytes + 1U + 6U) / 7U); // 7 bytes per message (+1 for checksum)
  u8_BytesReceived = 0U;
  u16_BytesLeft = static_cast<uint16_t>(static_cast<uint16_t>(ou8_NumBytes) +
                                        1U); //+1 for checksum
  for (uint8_t u8_Message = 0U; u8_Message < u8_NumMessages; u8_Message++) {
    ...
    do {
      ...
      if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
        if (t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) {
          const uint8_t u8_Temp =
              static_cast<uint8_t>((u16_BytesLeft > 7U) ? 7U : u16_BytesLeft);
          if (t_RXMsg.u8_DLC == static_cast<uint8_t>(u8_Temp + 1U)) {
            // make sure to extract checksum directly to the correct uint8 as
            // the
...
int32_t C_XFLProtocol::EEPROMWrite(const uint8_t ou8_NumBytes,
                                   const uint16_t ou16_StartAddress,
  ...
  for (u8_Message = 0U; u8_Message < u8_NumMessages; u8_Message++) {
    u16_Address =
        (ou16_StartAddress + (static_cast<uint16_t>(u8_Message) * 4U));
    ...
    for (u8_Byte = 0U; u8_Byte < static_cast<uint8_t>(
                                     (u8_BytesLeft > 4U) ? 4U : u8_BytesLeft);
         u8_Byte++) {
      mc_CanWriteMessage.au8_Data[4U + u8_Byte] =
          opu8_Data[u8_Byte + (u8_Message * 4U)];
    }
    m_SendMessageWithIDs();
    ...
    do {
      ...
      if (m_CANGetResponse(t_RXMsg) == C_NO_ERR) {
        ...
        if (t_RXMsg.au8_Data[0] == mc_Config.u8_LocalID) {
          ...
          if (memcmp(&mc_CanWriteMessage.au8_Data[0], &t_RXMsg.au8_Data[0],
                     u8_NumBytesNow) == 0) {
            // ok
            q_OK = true;
            break;
    ...
    u8_BytesLeft -=
        static_cast<uint8_t>(((u8_BytesLeft > 4U) ? 4U : u8_BytesLeft));
  }
  return C_NO_ERR;
}
...
C_XFLProtocol::C_XFLProtocol(const C_XFLProtocolConfig &orc_Config)
    : C_DLReportEvents(), mu16_CANDispatcherHandle(0U), mu8_LastError(0x00U) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFlashInformation(const uint8_t ou8_Index,
                                           const uint8_t ou8_NumParamsToUse,
                                           const uint8_t ou8_Parameter1,
                                           const uint8_t ou8_Parameter2,
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
                        mc_CanWriteMessage.au8_Data, 3U, &t_RXMsg, 8);
  if (s32_Return == C_NO_ERR) {
...
int32_t C_XFLProtocol::GetFlashInformationNumberOfICs(uint8_t &oru8_ICCount) {
  ...
  s32_Return = GetFlashInformation(
      mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_IC_COUNT, 0U, 0U, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetFlashInformationTotalMemorySize(const uint8_t ou8_ICIndex,
                                                  uint32_t &oru32_Size) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_TOTAL_SIZE, 1U,
                          ou8_ICIndex, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_flash_information protected sectors
   sub-service.

   get_flash_information get-protected-sectors client implementation

   \param[out]    orc_ProtectedSectors   array containing description of
   protected sectors
...
int32_t C_XFLProtocol::GetFlashInformationProtectedSectors(
  ...
  while (q_Finished == false) {
    s32_Return = GetFlashInformation(
        mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_PROTECTED_SECTORS, 1U,
        u8_BlockIndex, 0U, au8_Result);
    if (s32_Return == C_NO_ERR) {
      if (au8_Result[3] == 0x02U) {
        ...
        orc_ProtectedSectors[(orc_ProtectedSectors.size() > 0
                                  ? orc_ProtectedSectors.size() - 1
                                  : 0)]
            .u8_ICIndex = au8_Result[2];
        orc_ProtectedSectors[(orc_ProtectedSectors.size() > 0
                                  ? orc_ProtectedSectors.size() - 1
                                  : 0)]
            .u16_SectorNumber = mh_AU8ToU16LE(&au8_Result[0]);
        switch (au8_Result[3]) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_flash_information offset sector 0
   sub-service.

   get_flash_information offset-sector-0 client implementation

   \param[in]     ou8_ICIndex     index of flash IC on server hardware
   \param[out]    oru32_Offset    address offset of sector 0 of flash IC in
   bytes
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetFlashInformationOffsetSector0(const uint8_t ou8_ICIndex,
                                                uint32_t &oru32_Offset) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_OFFSET_ADDRESS,
                          1U, ou8_ICIndex, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_flash_information number of regions
   sub-service.
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetFlashInformationNumberRegions(const uint8_t ou8_ICIndex,
                                                uint8_t &oru8_RegionCount) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_NUM_REGIONS, 1U,
                          ou8_ICIndex, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_flash_information region-information
   sub-service.
   ...
   \param[out]    oru32_BlockSize    size of each block in the specified region
   in bytes
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFlashInformationRegionInformation(
    const uint8_t ou8_ICIndex, const uint8_t ou8_RegionIndex,
  ...
  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_REGION_INFO, 2U,
                          ou8_ICIndex, ou8_RegionIndex, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
/*! \brief   Implementation of get_flash_information erase-time sub-service.
   ...
   \param[in]     ou8_ICIndex        index of flash IC on server hardware
   \param[out]    oru32_EraseTime    time it takes to erase the slowest block in
   ms
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFlashInformationEraseTime(const uint8_t ou8_ICIndex,
                                                    uint32_t &oru32_EraseTime) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ERASE_TIME, 1U,
                          ou8_ICIndex, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
/*! \brief   Implementation of get_flash_information erase-time sub-service.
   ...
   \param[in]     ou8_ICIndex        index of flash IC on server hardware
   \param[out]    oru32_WriteTime    time it takes to write up to 255 bytes in
   ms
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFlashInformationWriteTime(const uint8_t ou8_ICIndex,
                                                    uint32_t &oru32_WriteTime) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_PROGRAM_TIME,
                          1U, ou8_ICIndex, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
/*! \brief   Implementation of get_flash_information aliasing sub-services
   ...
   Read information about aliased memory using the corresponding protocol
   services.
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetFlashInformationAliases(C_XFLAliasedRanges &orc_Aliases) {
  int32_t s32_Return;
  uint8_t u8_Region;
  uint8_t au8_Result[4];

  s32_Return =
      GetFlashInformation(mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_NUM_ALIASES, 0U,
                          0U, 0U, au8_Result);
  if (s32_Return == C_NO_ERR) {
    ...
    for (u8_Region = 0U; u8_Region < orc_Aliases.size(); u8_Region++) {
      s32_Return = GetFlashInformation(
          mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ALIAS_PHYSICAL, 1U, u8_Region,
          0U, au8_Result);
      if (s32_Return == C_NO_ERR) {
        orc_Aliases[u8_Region].u32_PhysicalAddress =
            mh_AU8ToU32LE(&au8_Result[0]);

        s32_Return = GetFlashInformation(
            mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ALIAS_SIZE, 1U, u8_Region, 0U,
            au8_Result);
      }
      if (s32_Return == C_NO_ERR) {
        ...
        s32_Return = GetFlashInformation(
            mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_ALIAS_ADDRESS, 1U, u8_Region,
            0U, au8_Result);
      }
      if (s32_Return == C_NO_ERR) {
        orc_Aliases[u8_Region].u32_AliasedAddress =
            mh_AU8ToU32LE(&au8_Result[0]);
      } else {
        break;
      }
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetImplementationInformation(const uint8_t ou8_Index,
                                            uint8_t (&orau8_Result)[4]) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_implementation_information protocol-version
   sub-service.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetImplementationInformationProtocolVersion(
    uint16_t &oru16_Version) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return = GetImplementationInformation(
      mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_PROTOCOL_VERSION, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_implementation_information services
   sub-service.

   get_implementation_information services client implementation

   \param[out]    orc_ImplementedServices    structure containing boolean values
   flagging which services are implemented
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetImplementationInformationServices(
    C_XFLImplementedServices &orc_ImplementedServices) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return = GetImplementationInformation(
      mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_SPECIAL_SERVICES, au8_Result);
  if (s32_Return == C_NO_ERR) {
    orc_ImplementedServices.q_EERead =
        ((au8_Result[0] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_EEWrite =
        ((au8_Result[0] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved1[0] =
        ((au8_Result[0] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved1[1] =
        ((au8_Result[0] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved1[2] =
        ((au8_Result[0] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved1[3] =
        ((au8_Result[0] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved1[4] =
        ((au8_Result[0] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved1[5] =
        ((au8_Result[0] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.q_NetStart =
        ((au8_Result[1] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_NetReset =
        ((au8_Result[1] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_NetSleep =
        ((au8_Result[1] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved2[0] =
        ((au8_Result[1] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved2[1] =
        ((au8_Result[1] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved2[2] =
        ((au8_Result[1] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved2[3] =
        ((au8_Result[1] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved2[4] =
        ((au8_Result[1] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.q_GetSNR =
        ((au8_Result[2] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_NodeWakeupLID =
        ((au8_Result[2] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_NodeWakeupSNR =
        ((au8_Result[2] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved3[0] =
        ((au8_Result[2] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved3[1] =
        ((au8_Result[2] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved3[2] =
        ((au8_Result[2] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved3[3] =
        ((au8_Result[2] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved3[4] =
        ((au8_Result[2] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_Reserved4[0] =
        ((au8_Result[3] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[1] =
        ((au8_Result[3] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[2] =
        ((au8_Result[3] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[3] =
        ((au8_Result[3] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[4] =
        ((au8_Result[3] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[5] =
        ((au8_Result[3] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[6] =
        ((au8_Result[3] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved4[7] =
        ((au8_Result[3] & 0x80U) > 0U) ? true : false;

    s32_Return = GetImplementationInformation(
        mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_GET_SERVICES, au8_Result);
  }
  if (s32_Return == C_NO_ERR) {
    orc_ImplementedServices.q_GetLocalID =
        ((au8_Result[0] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetSectorCount =
        ((au8_Result[0] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetVersionNumber =
        ((au8_Result[0] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetDownloadCount =
        ((au8_Result[0] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetDeviceID =
        ((au8_Result[0] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetControlID =
        ((au8_Result[0] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetSectorChecksum =
        ((au8_Result[0] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetSectorModeCompare =
        ((au8_Result[0] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.q_GetTimeOutFactor =
        ((au8_Result[1] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetLastUser =
        ((au8_Result[1] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetFlashInformation =
        ((au8_Result[1] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetImplementationInformation =
        ((au8_Result[1] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetFingerPrint =
        ((au8_Result[1] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetDeviceInfoAddress =
        ((au8_Result[1] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetBlockStartAddress =
        ((au8_Result[1] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetBlockEndAddress =
        ((au8_Result[1] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.q_GetBlockChecksum =
        ((au8_Result[2] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_GetBlockCompareMode =
        ((au8_Result[2] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[0] =
        ((au8_Result[2] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[1] =
        ((au8_Result[2] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[2] =
        ((au8_Result[2] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[3] =
        ((au8_Result[2] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[4] =
        ((au8_Result[2] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[5] =
        ((au8_Result[2] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_Reserved6[6] =
        ((au8_Result[3] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[7] =
        ((au8_Result[3] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[8] =
        ((au8_Result[3] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[9] =
        ((au8_Result[3] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[10] =
        ((au8_Result[3] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[11] =
        ((au8_Result[3] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[12] =
        ((au8_Result[3] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved6[13] =
        ((au8_Result[3] & 0x80U) > 0U) ? true : false;

    s32_Return = GetImplementationInformation(
        mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_SET_SERVICES, au8_Result);
  }

  if (s32_Return == C_NO_ERR) {
    orc_ImplementedServices.q_SetLocalID =
        ((au8_Result[0] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetCommBitrate =
        ((au8_Result[0] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetCommID =
        ((au8_Result[0] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetCommType =
        ((au8_Result[0] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetControlID =
        ((au8_Result[0] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetSectorChecksum =
        ((au8_Result[0] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetSectorModeCompare =
        ((au8_Result[0] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetTimeoutFactor =
        ((au8_Result[0] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.q_SetGatewayParameter =
        ((au8_Result[1] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetXflExchange =
        ((au8_Result[1] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetFingerPrint =
        ((au8_Result[1] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetTempBitrate =
        ((au8_Result[1] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetBlockStartAddress =
        ((au8_Result[1] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetBlockEndAddress =
        ((au8_Result[1] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetBlockChecksum =
        ((au8_Result[1] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.q_SetBlockCompareMode =
        ((au8_Result[1] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_Reserved7[0] =
        ((au8_Result[2] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[1] =
        ((au8_Result[2] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[2] =
        ((au8_Result[2] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[3] =
        ((au8_Result[2] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[4] =
        ((au8_Result[2] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[5] =
        ((au8_Result[2] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[6] =
        ((au8_Result[2] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[7] =
        ((au8_Result[2] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_Reserved7[8] =
        ((au8_Result[3] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[9] =
        ((au8_Result[3] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[10] =
        ((au8_Result[3] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[11] =
        ((au8_Result[3] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[12] =
        ((au8_Result[3] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[13] =
        ((au8_Result[3] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[14] =
        ((au8_Result[3] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved7[15] =
        ((au8_Result[3] & 0x80U) > 0U) ? true : false;

    s32_Return = GetImplementationInformation(
        mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_FLASH_SERVICES, au8_Result);
  }

  if (s32_Return == C_NO_ERR) {
    orc_ImplementedServices.q_EraseSector =
        ((au8_Result[0] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_ProgFlash =
        ((au8_Result[0] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_NodeSleep =
        ((au8_Result[0] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.q_NodeReset =
        ((au8_Result[0] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.q_NodeReturn =
        ((au8_Result[0] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.q_ReadFlash =
        ((au8_Result[0] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.q_NodeCompID =
        ((au8_Result[0] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.q_DivertStream =
        ((au8_Result[0] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.q_WakeupDivertClient =
        ((au8_Result[1] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.q_Reserved9 =
        ((au8_Result[1] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.q_ReadSeed =
        ((au8_Result[1] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.q_WriteKey =
        ((au8_Result[1] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.q_ResetLocks =
        ((au8_Result[1] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.q_EnterBSL =
        ((au8_Result[1] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[0] =
        ((au8_Result[1] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[1] =
        ((au8_Result[1] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_Reserved8[2] =
        ((au8_Result[2] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[3] =
        ((au8_Result[2] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[4] =
        ((au8_Result[2] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[5] =
        ((au8_Result[2] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[6] =
        ((au8_Result[2] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[7] =
        ((au8_Result[2] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[8] =
        ((au8_Result[2] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[9] =
        ((au8_Result[2] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_Reserved8[10] =
        ((au8_Result[3] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[11] =
        ((au8_Result[3] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[12] =
        ((au8_Result[3] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[13] =
        ((au8_Result[3] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[14] =
        ((au8_Result[3] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[15] =
        ((au8_Result[3] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[16] =
        ((au8_Result[3] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_Reserved8[17] =
        ((au8_Result[3] & 0x80U) > 0U) ? true : false;

    s32_Return = GetImplementationInformation(
        mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_TARGET_SPECIFIC_SERVICES,
        au8_Result);
  }

  if (s32_Return == C_NO_ERR) {
    orc_ImplementedServices.aq_TargetSpecific[0] =
        ((au8_Result[0] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[1] =
        ((au8_Result[0] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[2] =
        ((au8_Result[0] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[3] =
        ((au8_Result[0] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[4] =
        ((au8_Result[0] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[5] =
        ((au8_Result[0] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[6] =
        ((au8_Result[0] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[7] =
        ((au8_Result[0] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_TargetSpecific[8] =
        ((au8_Result[1] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[9] =
        ((au8_Result[1] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[10] =
        ((au8_Result[1] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[11] =
        ((au8_Result[1] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[12] =
        ((au8_Result[1] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[13] =
        ((au8_Result[1] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[14] =
        ((au8_Result[1] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[15] =
        ((au8_Result[1] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_TargetSpecific[16] =
        ((au8_Result[2] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[17] =
        ((au8_Result[2] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[18] =
        ((au8_Result[2] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[19] =
        ((au8_Result[2] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[20] =
        ((au8_Result[2] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[21] =
        ((au8_Result[2] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[22] =
        ((au8_Result[2] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[23] =
        ((au8_Result[2] & 0x80U) > 0U) ? true : false;

    orc_ImplementedServices.aq_TargetSpecific[24] =
        ((au8_Result[3] & 0x01U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[25] =
        ((au8_Result[3] & 0x02U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[26] =
        ((au8_Result[3] & 0x04U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[27] =
        ((au8_Result[3] & 0x08U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[28] =
        ((au8_Result[3] & 0x10U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[29] =
        ((au8_Result[3] & 0x20U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[30] =
        ((au8_Result[3] & 0x40U) > 0U) ? true : false;
    orc_ImplementedServices.aq_TargetSpecific[31] =
        ((au8_Result[3] & 0x80U) > 0U) ? true : false;
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of get_implementation_information hex-records
   sub-service.

   get_implementation_information hex-records client implementation

   \param[out]    oru8_MaxRecordLength   maximum hex file record length
   supported by this server
   \param[out]    oru8_Granularity       required alignment of hex-records
   (1,2,4 bytes)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetImplementationInformationHexRecords(
    uint8_t &oru8_MaxRecordLength, uint8_t &oru8_Granularity) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return = GetImplementationInformation(
      mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_HEX_RECORDS, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFingerPrint(const uint8_t ou8_Index,
                                      uint8_t (&orau8_Result)[4]) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFingerPrintSupportedIndexes(
    C_XFLFingerPrintSupportedIndexes &orc_SupportedIndexes) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_SUPPORTED_INDEXES, au8_Result);
  if (s32_Return == C_NO_ERR) {
    orc_SupportedIndexes.q_ProgrammingTime =
        ((au8_Result[0] & 0x01U) > 0U) ? true : false;
    orc_SupportedIndexes.q_ProgrammingDate =
        ((au8_Result[0] & 0x02U) > 0U) ? true : false;
    orc_SupportedIndexes.q_ApplicationCheckSum =
        ((au8_Result[0] & 0x04U) > 0U) ? true : false;
    orc_SupportedIndexes.q_UsernamePart1 =
        ((au8_Result[0] & 0x08U) > 0U) ? true : false;
    orc_SupportedIndexes.q_UsernamePart2 =
        ((au8_Result[0] & 0x10U) > 0U) ? true : false;
    orc_SupportedIndexes.q_UsernamePart3 =
        ((au8_Result[0] & 0x20U) > 0U) ? true : false;
    orc_SupportedIndexes.q_UsernamePart4 =
        ((au8_Result[0] & 0x40U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[0] =
        ((au8_Result[0] & 0x80U) > 0U) ? true : false;

    orc_SupportedIndexes.aq_Reserved[1] =
        ((au8_Result[1] & 0x01U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[2] =
        ((au8_Result[1] & 0x02U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[3] =
        ((au8_Result[1] & 0x04U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[4] =
        ((au8_Result[1] & 0x08U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[5] =
        ((au8_Result[1] & 0x10U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[6] =
        ((au8_Result[1] & 0x20U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[7] =
        ((au8_Result[1] & 0x40U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[8] =
        ((au8_Result[1] & 0x80U) > 0U) ? true : false;

    orc_SupportedIndexes.aq_Reserved[9] =
        ((au8_Result[2] & 0x01U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[10] =
        ((au8_Result[2] & 0x02U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[11] =
        ((au8_Result[2] & 0x04U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[12] =
        ((au8_Result[2] & 0x08U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[13] =
        ((au8_Result[2] & 0x10U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[14] =
        ((au8_Result[2] & 0x20U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[15] =
        ((au8_Result[2] & 0x40U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[16] =
        ((au8_Result[2] & 0x80U) > 0U) ? true : false;

    orc_SupportedIndexes.aq_Reserved[17] =
        ((au8_Result[3] & 0x01U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[18] =
        ((au8_Result[3] & 0x02U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[19] =
        ((au8_Result[3] & 0x04U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[20] =
        ((au8_Result[3] & 0x08U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[21] =
        ((au8_Result[3] & 0x10U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[22] =
        ((au8_Result[3] & 0x20U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[23] =
        ((au8_Result[3] & 0x40U) > 0U) ? true : false;
    orc_SupportedIndexes.aq_Reserved[24] =
        ((au8_Result[3] & 0x80U) > 0U) ? true : false;
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFingerPrintProgrammingTime(uint8_t &oru8_Hours,
                                                     uint8_t &oru8_Minutes,
                                                     uint8_t &oru8_Seconds) {
  int32_t s32_Return;
  uint8_t au8_Result[4];

  s32_Return =
      GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_PROGRAMMING_TIME, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFingerPrintProgrammingDate(uint8_t &oru8_Years,
                                                     uint8_t &oru8_Months,
                                                     uint8_t &oru8_Days) {
  int32_t s32_Return;
  uint8_t au8_Result[4];
  ...
  s32_Return =
      GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_PROGRAMMING_DATE, au8_Result);
  if (s32_Return == C_NO_ERR) {
...
/*! \brief   Implementation of get_finger_print user-name sub-service.
   ...
   \param[out]    orc_UserName   user name stored in server, zero terminated
   (max. 16 characters)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetFingerPrintUserName(QString &orc_UserName) {
  int32_t s32_Return;
  ...
  // we can be sure to have enough buffer -> provide buffer through casting to
  // avoid needing a local 4 byte buffer ...
  s32_Return = GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_1, au8_Buffer);
  if (s32_Return == C_NO_ERR) {
    (void)memcpy(&acn_UserName[0], &au8_Buffer[0], 4);
    s32_Return =
        GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_2, au8_Buffer);
  }
  if (s32_Return == C_NO_ERR) {
    (void)memcpy(&acn_UserName[4], &au8_Buffer[0], 4);
    s32_Return =
        GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_3, au8_Buffer);
  }
  if (s32_Return == C_NO_ERR) {
    (void)memcpy(&acn_UserName[8], &au8_Buffer[0], 4);
    s32_Return =
        GetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_4, au8_Buffer);
  }
  if (s32_Return == C_NO_ERR) {
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::m_GetDeviceInfoAddress(const uint8_t ou8_Index,
                                              uint8_t (&orau8_Result)[4]) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
...
/*! \brief   Implementation of get_device_info_address service.
   ...
   \param[out]    orc_Addresses            list of offsets of "device info"
   struct in ROM
   \param[in]     ou16_ProtocolVersion     protocol version
                                            (as returned by
   GetImplementationInformationProtocolVersion())
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetDeviceInfoAddresses(QList<uint32_t> &orc_Addresses,
  ...
  if (ou16_ProtocolVersion < 0x3030U) {
    ...
    s32_Return = this->m_GetDeviceInfoAddress(
        mu8_XFL_CMD_GET_DEVICE_INFO_ADDRESS_NUM_BLOCKS, au8_Data);
    if (s32_Return == C_NO_ERR) {
...
/*! \brief   read device information block from server
   ...
   \param[in]     ou32_Address  address of info structure in server's flash
   memory
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetDeviceInfoBlock(const uint32_t ou32_Address,
                                          C_XFLECUInformation &orc_Data) {
  int32_t s32_Return;
  ...
  // check that sizes match; otherwise the following charn / uint8 assignment
  // will fail miserable
  Q_ASSERT(sizeof(char_t) ==
           sizeof(uint8_t)); // lint !e948  //evaluates to contant true if
                             // everything is fine ...

  // first read the first couple of bytes to detect "MAGIC":
  s32_Return = this->ReadFlash(
      XFL_DEVICE_INFO_MAGIC_LENGTH_V1, ou32_Address,
      reinterpret_cast<uint8_t *>(
          orc_Data.acn_Magic)); // lint !e926 //cf. assertion above
  if (s32_Return != C_NO_ERR) {
    return s32_Return;
  }

  s32_Return = memcmp(&orc_Data.acn_Magic[0], XFL_DEVICE_INFO_MAGIC_V1,
                      XFL_DEVICE_INFO_MAGIC_LENGTH_V1);
  if (s32_Return != 0) {
    // try V2:
    s32_Return = this->ReadFlash(
        XFL_DEVICE_INFO_MAGIC_LENGTH_V2, ou32_Address,
        reinterpret_cast<uint8_t *>(
            orc_Data.acn_Magic)); // lint !e926 //cf. assertion above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
    s32_Return = memcmp(&orc_Data.acn_Magic[0], XFL_DEVICE_INFO_MAGIC_V2,
                        XFL_DEVICE_INFO_MAGIC_LENGTH_V2);
    if (s32_Return != 0) {
      return C_CONFIG;
    }
    q_IsV2 = true;
  ...
  u16_Offset =
      static_cast<uint16_t>(u16_Offset + XFL_DEVICE_INFO_MAGIC_LENGTH_V1);
  u16_Offset += static_cast<uint16_t>((q_IsV2 == true) ? 1U : 0U);

  // read header information
  s32_Return = this->ReadFlash(2U, ou32_Address + u16_Offset,
                               &orc_Data.u8_StructVersion);
  if (s32_Return != C_NO_ERR) {
    return s32_Return;
  }
  ...
  u8_VersionIndex =
      (q_IsV2 == true) ? 1U : 0U; // lookup index for array length tables

  if (orc_Data.ContainsDeviceID() == true) {
    u8_Size = C_XFLECUInformation::hau8_LENGTHS_DEVICE_ID[u8_VersionIndex];
    s32_Return = this->ReadFlash(
        u8_Size, ou32_Address + u16_Offset,
        reinterpret_cast<uint8_t *>(
            orc_Data.acn_DeviceID)); // lint !e926 //cf. assertion
                                     //  above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
  ...
  if (orc_Data.ContainsDateAndTime() == true) {
    ...
    s32_Return = this->ReadFlash(
        u8_Size, ou32_Address + u16_Offset,
        reinterpret_cast<uint8_t *>(
            orc_Data.acn_Date)); // lint !e926 //cf. assertion above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
    u16_Offset = static_cast<uint16_t>(u16_Offset + u8_Size);
    ...
    s32_Return = this->ReadFlash(
        u8_Size, ou32_Address + u16_Offset,
        reinterpret_cast<uint8_t *>(
            orc_Data.acn_Time)); // lint !e926 //cf. assertion above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
    u16_Offset = static_cast<uint16_t>(u16_Offset + u8_Size);
  ...
  if (orc_Data.ContainsProjectName() == true) {
    ...
    s32_Return = this->ReadFlash(
        u8_Size, ou32_Address + u16_Offset,
        reinterpret_cast<uint8_t *>(
            orc_Data.acn_ProjectName)); // lint !e926 //cf. assertion
                                        //  above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
  ...
  if (orc_Data.ContainsProjectVersion() == true) {
    u8_Size =
        C_XFLECUInformation::hau8_LENGTHS_PROJECT_VERSION[u8_VersionIndex];
    s32_Return =
        this->ReadFlash(u8_Size, ou32_Address + u16_Offset,
                        reinterpret_cast<uint8_t *>(
                            orc_Data.acn_ProjectVersion)); // lint !e926 //cf.
                                                           //  assertion
                                                           //  above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
  ...
  if (orc_Data.ContainsAdditionalInfo() == true) {
    s32_Return = this->ReadFlash(1U, ou32_Address + u16_Offset,
                                 &orc_Data.u8_LenAdditionalInfo);
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
    ...
    s32_Return = this->ReadFlash(
        orc_Data.u8_LenAdditionalInfo, ou32_Address + u16_Offset,
        reinterpret_cast<uint8_t *>(
            &orc_Data.acn_AdditionalInfo[0])); // lint !e926 //cf.
                                               //  assertion above
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetBlockAddressesAll(
    QList<C_XFLChecksumBlock> &orc_ChecksumBlocks) {
  int32_t s32_Return;
  uint8_t u8_BlockIndex = 0U;
  ...
  while (s32_Return == C_NO_ERR) {
    ...
    case C_NO_ERR:
      ...
      orc_ChecksumBlocks[(
          orc_ChecksumBlocks.size() > 0 ? orc_ChecksumBlocks.size() - 1 : 0)] =
          c_Block;
      break;
    case C_NOACT:
...
/*! \brief   Implementation of get_block_address service.
   ...
   Note:
   The "CHECKSUM_ERR" flags a block that is expected to be there but is invalid
   on the server. A "CHECKSUM_ERR" returned from the server is not reported to
   the application with "C_NOACT". Instead it is reported via
   "orc_ChecksumBlock.q_BlockDefinitionValid" being set to false. In this case
   this function returns C_NO_ERR.
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_XFLProtocol::GetBlockAddresses(const uint8_t ou8_BlockNumber,
                                 C_XFLChecksumBlock &orc_ChecksumBlock) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
...
/*! \brief   Implementation of get_block_checksum service.
   ...
   ou8_ChecksumType parameter is not range-checked: So we are open to future
   expansions of the protocol. If the server can not handle it it will complain
   ...

   \param[in]     ou8_BlockNumber            number of block
   \param[in]     ou8_ChecksumType           type of checksum 0 -> read from
   EEPROM; 1 -> calculate and return
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetBlockChecksum(const uint8_t ou8_BlockNumber,
                                        const uint8_t ou8_ChecksumType,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetFingerPrint(const uint8_t ou8_Index,
                                      const uint8_t (&orau8_Param)[4]) {
  mc_CanWriteMessage.u8_DLC = 8U;
  mc_CanWriteMessage.au8_Data[1] = mu8_XFL_CMD_GRP_SET_COMMAND;
  ...
  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data, 7U, NULL, 8);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetFingerPrintProgrammingTime(const uint8_t ou8_Hour,
                                                     const uint8_t ou8_Minute,
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetFingerPrintProgrammingDate(const uint8_t ou8_Year,
                                                     const uint8_t ou8_Month,
                                                     const uint8_t ou8_Day) {
  uint8_t au8_Data[4];

  if ((ou8_Month < 1U) || (ou8_Month > 12U) || (ou8_Day < 1U) ||
      (ou8_Day > 31U) || (ou8_Year > 99U)) {
    return C_RANGE;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetFingerPrintUserName(const QString &orc_UserName) {
  int32_t s32_Return;
  uint8_t au8_Data[17];
  uint8_t au8_Buffer[4];
  uint32_t u32_Length;

  u32_Length = static_cast<uint32_t>(orc_UserName.length());

  if (u32_Length >= 16U) {
    au8_Data[16] = 0U; // terminate string
    (void)memcpy(&au8_Data[0], orc_UserName.toUtf8().constData(), 16U);
  } else {
    (void)memset(&au8_Data[0], 0, 17U);
    (void)memcpy(&au8_Data[0], orc_UserName.toUtf8().constData(), u32_Length);
  }

  // we can be sure to have enough data -> provide data through casting to avoid
  // needing a local 4 byte buffer ...
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return =
        SetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_2, au8_Buffer);
  }
  if (s32_Return == C_NO_ERR) {
    (void)memcpy(&au8_Buffer[0], &au8_Data[8], 4);
    s32_Return =
        SetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_3, au8_Buffer);
  }
  if (s32_Return == C_NO_ERR) {
    (void)memcpy(&au8_Buffer[0], &au8_Data[12], 4);
    s32_Return =
        SetFingerPrint(mu8_XFL_CMD_FINGER_PRINT_USERNAME_4, au8_Buffer);
  }
  return s32_Return;
}
...
int32_t C_XFLProtocol::SetTempBitrate(const uint32_t ou32_TempBitrate) {
  ...
  mc_CanWriteMessage.au8_Data[5] =
      static_cast<uint8_t>(ou32_TempBitrate >> 16U);
  mc_CanWriteMessage.au8_Data[6] =
      static_cast<uint8_t>(ou32_TempBitrate >> 24U);
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Implementation of set_block_start_address and set_block_end_address
   services.

   set_block_start_address and set_block_end_address client implementation.

   \param[in]     ou8_BlockNumber     block number to set start address and end
   address for
   \param[in]     ou32_StartAddress   block start address
   \param[in]     ou32_EndAddress     block end address
   \param[in]     oq_StartAddressOnly true: only set start address, not end
   address
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetBlockAddresses(const uint8_t ou8_BlockNumber,
                                         const uint32_t ou32_StartAddress,
                                         const uint32_t ou32_EndAddress,
                                         const bool oq_StartAddressOnly) {
  int32_t s32_Return;
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                        mc_CanWriteMessage.au8_Data, 7U, NULL, 8);
  if (s32_Return != C_NO_ERR) {
    return s32_Return;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetBlockChecksum(const uint8_t ou8_BlockNumber,
                                        uint32_t &oru32_CalculatedChecksum) {
  int32_t s32_Return;
  T_STWCAN_Msg_RX t_RXMsg;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::GetBlockCompareMode(const uint8_t ou8_BlockNumber,
                                           bool &orq_CheckAtStartup,
  ...
  s32_Return =
      m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_READ_INFO_MS,
                        mc_CanWriteMessage.au8_Data, 3U, &t_RXMsg, 5);
  if (s32_Return == C_NO_ERR) {
    // we ignore any bits except for 0 and 1 as they are not defined
    orq_CheckAtStartup =
        ((t_RXMsg.au8_Data[4] & 0x01U) == 0x01U) ? true : false;
    orq_CheckAfterFlashing =
        ((t_RXMsg.au8_Data[4] & 0x02U) == 0x02U) ? true : false;
  }
  return s32_Return;
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::SetBlockCompareMode(const uint8_t ou8_BlockNumber,
                                           const bool oq_CheckAtStartup,
  ...
  mc_CanWriteMessage.au8_Data[4] +=
      ((oq_CheckAfterFlashing == true) ? 0x02U : 0x00U);
  m_SendMessageWithIDs();

  return m_WaitForResponse(mc_Config.u8_LocalID, mu16_TIMEOUT_WRITE_CONFIG_MS,
                           mc_CanWriteMessage.au8_Data, 4U, NULL, 5);
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_XFLProtocol::TRG_ReportProgress(const uint16_t ou16_Progress1_1000,
                                          const QString &orc_AdditionalText) {
...
/*! \brief   Report status of activity
   ...
   \param[in]     ou8_Type       type of status (see gu8_DL_REPORT_STATUS_TYPE_*
   constants)
*/
//----------------------------------------------------------------------------------------------------------------------
void C_XFLProtocol::TRG_ReportStatus(const QString &orc_Text,
                                     const uint8_t ou8_Type) {
  (void)orc_Text;
  (void)ou8_Type;
}
...
/*! \brief   Perform operating system message handling
   ...
   Prevents the application from freezing during waiting for replies from the
   server node.
*/
//----------------------------------------------------------------------------------------------------------------------
