...
class C_KFXVariableLists : public QList<C_KFXVariableListBase> {
private:
public:
  // over whole structure, not just the values:
  void CalcCRCOverAllLists(uint16_t &oru16_CRC,
                           const bool oq_SkipValues = false) const;

  void ClearDefaults(void);
  void ClearValues(void);

  bool VariableExists(const uint16_t ou16_Index) const;
  bool VariableExists(const uint16_t ou16_List,
                      const uint16_t ou16_Variable) const;
  bool ListExists(const uint16_t ou16_List) const;

  // Note: put the const functions first here as a workaround for problems in
  // PC-Lint 9.00k in combination with -ctc
  const C_KFXVariableBase *GetVariable(const uint16_t ou16_Index) const;
  const C_KFXVariableBase *GetVariable(const uint16_t ou16_List,
                                       const uint16_t ou16_Variable) const;
  ...
  C_KFXVariableBase *GetVariable(const uint16_t ou16_List,
                                 const uint16_t ou16_Variable);

  static uint16_t PackIndex(const uint16_t ou16_List,
                            const uint16_t ou16_Variable);
  static void UnpackIndex(const uint16_t ou16_Index, uint16_t *const opu16_List,
