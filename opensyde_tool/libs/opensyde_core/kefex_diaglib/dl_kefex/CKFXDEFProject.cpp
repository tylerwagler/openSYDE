...
C_KFXRAMNameIndex::C_KFXRAMNameIndex() { u16_ListIndex = 0; }
...
/*! \brief    Load data of one RAM variable list from corresponding .RAM file
   ...
   Assumptions:
   - it has already been checked that the device name in the RAM file matches
   the project
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_KFXDEFProject::m_LoadRAMListFromFile(const QString &orc_FilePath,
                                               C_KFXVariableListBase &orc_List,
  ...
  // read out RAM file
  try {
    pc_IniFile = new QSettings(orc_FilePath, QSettings::IniFormat);
  } catch (...) {
    Q_ASSERT(false);
    return C_OVERFLOW;
  }
  ...
  u16_NumVars =
      static_cast<uint16_t>(pc_IniFile->value("CONFIG/NUMOFVARS", 0).toUInt());
  orc_List.q_CheckSummed =
      pc_IniFile->value("CONFIG/CHECKSUMMED", false).toBool();
  orc_List.u32_CheckSumAddress = static_cast<uint32_t>(
      pc_IniFile->value("CONFIG/CRCADDRESS", 1024).toUInt());
  orc_List.c_ListName = pc_IniFile->value("CONFIG/LISTNAME", "").toString();
  orc_List.u8_ListType = static_cast<uint8_t>(
      pc_IniFile->value("CONFIG/VARIABLETYPE", KFX_VARIABLE_TYPE_RAM).toUInt());
  if (orc_List.c_ListName == "") {
    orc_ErrorText = orc_FilePath + ":\n" +
                    stw::opensyde_core::C_OscUtils::h_LoadString(
                        STR_DS_ERR_NO_VALID_NAME_IN_LIST);
    delete pc_IniFile;
    return C_NOACT;
  }

  orc_List.SetNumDefaults(static_cast<uint16_t>(
      pc_IniFile->value("CONFIG/NUMDEFAULTS", 1).toUInt()));

  orc_List.VariableList.resize(
      u16_NumVars); // much faster than "++" in a for loop
  for (j = 0; j < u16_NumVars; j++) {
    ...
    c_Temp = pc_IniFile->value(c_Section + "/ADDRESS", "").toString();
    pt_Entry->u32_Address =
        (c_Temp == "") ? 0UL : static_cast<uint32_t>(c_Temp.toLongLong());

    q_Ok = true;
    if (pc_IniFile->contains(c_Section + "/TYPE_INDEX") ==
        false) // TYPE_INDEX exists !! -> type is contained as an index
    {
      q_Ok = false;
    }
    ...
    if (q_Ok == false) {
      q_Ok = true;
      // not numeric -> name of type (following very old conventions) !!! ->
      // stay compatible to old files
      c_Temp = pc_IniFile->value(c_Section + "/TYPE", "")
                   .toString()
                   .toUpper()
                   .trimmed();
      if (c_Temp == "BYTE") {
    ...
    if (q_Ok == false) {
      orc_ErrorText =
          orc_FilePath + ":\n" + c_Temp + " " +
          stw::opensyde_core::C_OscUtils::h_LoadString(STR_DS_INVALID_TYPE);
      delete pc_IniFile;
      return C_CONFIG;
    }
    pt_Entry->u8_Type = static_cast<uint8_t>(k);

    c_Temp = pc_IniFile->value(c_Section + "/SIZE", "").toString();
    if ((c_Temp == "") || (c_Temp == "0")) {
    ...
    c_Temp =
        pc_IniFile->value(c_Section + "/LOCATIONRAM", "").toString().toUpper();
    ...
    m_VarStringsToMinMax(pc_IniFile->value(c_Section + "/MIN", "").toString(),
                         pc_IniFile->value(c_Section + "/MAX", "").toString(),
                         *pt_Entry);

    for (k = 0; k < KFX_NUM_ACCESS_GROUPS; k++) {
      c_Directive = "ACCESS" + QString::number(k);
      c_Temp = pc_IniFile->value(c_Section + "/" + c_Directive, "")
                   .toString()
                   .toUpper();
      if (c_Temp == "RO") {
        ...
        orc_ErrorText = orc_FilePath + ":\n" + c_Temp +
                        stw::opensyde_core::C_OscUtils::h_LoadString(
                            STR_DS_INVALID_ACC_TYPE);
        delete pc_IniFile;
        return C_RANGE;
      }
    }
    pt_Entry->c_Unit = pc_IniFile->value(c_Section + "/UNIT", "").toString();
    pt_Entry->SetNumericValue(0LL);
    pt_Entry->SetNumDefaults(orc_List.GetNumDefaults());

    c_Temp = pc_IniFile->value(c_Section + "/SCALINGFACTOR", "").toString();
    if (c_Temp == "") {
      c_Temp = QString::number(KFX_FACTOR_VALUE_100_PERCENT);
    }
    try {
    ...
    c_Temp = pc_IniFile->value(c_Section + "/SCALINGDIGITS", "").toString();
    if (c_Temp == "") {
      c_Temp = "0";
    }
    try {
    ...
    pt_Entry->e_DefTransmissionType = TransTypeStringToEnum(
        pc_IniFile->value(c_Section + "/TRANSTYPE", "").toString().toUpper());
    try {
      pt_Entry->u16_DefInterval = static_cast<uint16_t>(
          pc_IniFile->value(c_Section + "/TRANSTIME", "").toInt());
    } catch (...) {
      pt_Entry->u16_DefInterval = 200;
    }
    try {
      pt_Entry->u32_DefLowerHysteresis = static_cast<uint32_t>(
          pc_IniFile->value(c_Section + "/LOWERHYST", "").toLongLong());
    } catch (...) {
      pt_Entry->u32_DefLowerHysteresis = 10;
    }
    try {
      pt_Entry->u32_DefUpperHysteresis = static_cast<uint32_t>(
          pc_IniFile->value(c_Section + "/UPPERHYST", "").toLongLong());
    } catch (...) {
      pt_Entry->u32_DefUpperHysteresis = 10;
    }

    try {
      pt_Entry->u8_Class = static_cast<uint8_t>(
          pc_IniFile->value(c_Section + "/VAR_CLASS", "").toInt());
    } catch (...) {
...
void C_KFXDEFProject::m_VarStringsToMinMax(const QString &orc_Min,
                                           const QString &orc_Max,
  ...
  if (c_Max == "") {
    ...
    // compatibility for projects without min/max for arrays: use maximum range
    // of variable type:
    if ((c_Min != "0") && (c_Max != "0")) {
      // parse and set
      c_TokensMin = c_Min.split(";");
      c_TokensMax = c_Max.split(";");
    ...
    try {
      u_Val64.s64_Value = c_Min.toLongLong();
      // keep compatibility: float min/max values are saved in int64
      // representation
      ...
      u_Val64.s64_Value = c_Max.toLongLong();
      if (orc_Variable.IsFloatType() == true) {
...
/*! \brief    Convert transmission type string to enum value
   ...
   \return
   Transmission type enum         if no valid transmission type is detected in
   oc_Text "eSRR" is returned
*/
//----------------------------------------------------------------------------------------------------------------------
E_TransmissionType
C_KFXDEFProject::TransTypeStringToEnum(const QString &orc_Text) {
...
/*! \brief    Append data from .RAM files to variable table
   ...
   Clear VariableLists
   Search of all .ram files in oc_Directory belonging to the device
   oc_DeviceName. Read .RAM files and add the contents at the end of
   "VariableLists".
   ...
   \param[out]    orc_ErrorText    additional text information on error
   \param[out]    orc_Warnings     warning texts (should be displayed by
   application even when returning C_NO_ERR)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_KFXDEFProject::LoadRAMFiles(const QString &orc_Directory,
                                      const QString &orc_DeviceName,
                                      C_KFXVariableLists &orc_Lists,
                                      QString &orc_ErrorText,
  ...
  for (s32_Index = 0; s32_Index < c_Files.size(); s32_Index++) {
    ...
    s32_Return = m_LoadRAMListFromFile(c_Files[s32_Index].c_FileName,
                                       orc_Lists[s32_Index], orc_ErrorText);
    if (s32_Return != C_NO_ERR) {
      return C_RD_WR;
    }
  }
...
//**************************************************************.FE*
int32_t C_KFXDEFProject::m_SortRAMLists(QList<C_KFXRAMNameIndex> &orc_Files,
                                        QString &orc_ErrorText) {
  int32_t s32_File;
  ...
  // Sort
  for (s32_File = 0;
       s32_File < (orc_Files.size() > 0 ? orc_Files.size() - 1 : 0);
       s32_File++) {
    if (orc_Files[s32_File + 1].u16_ListIndex <
        orc_Files[s32_File].u16_ListIndex) {
  ...
  for (s32_File = 0; s32_File < orc_Files.size(); s32_File++) {
    if (orc_Files[s32_File].u16_ListIndex <
        static_cast<uint16_t>(s32_File)) // two lists with the same index !
    {
      ...
      return C_CONFIG;
    } else if (orc_Files[s32_File].u16_ListIndex >
               static_cast<uint16_t>(s32_File)) // probably missing file ?
    {
      orc_ErrorText = orc_Files[s32_File].c_FileName + ":\n" +
                      stw::opensyde_core::C_OscUtils::h_LoadString(
                          STR_DS_ERR_PROJECT_INVALID) +
...
//--------------------------------------------------------------.F1-----------------------------------------------------
//   .DESCRIPTION
//    Get a list of all .ram files in the work directory which refer to the
//    specified device
//----------------------------------------------------------------------------------------------------------------------
//   .PARAMETERS
//    Name             Comment
...
//   .RETURNVALUE
//**************************************************************.FE*
int32_t C_KFXDEFProject::m_FindRelatedFiles(const QString &orc_Directory,
                                            const QString &orc_DeviceName,
  ...
  c_Dir =
      stw::opensyde_core::C_OscUtils::h_IncludeTrailingDelimiter(orc_Directory);
  ...
  for (int s32_Index = 0; s32_Index < c_InfoList.count(); s32_Index++) {
    QString c_FileName = c_InfoList.at(s32_Index).fileName();

    pc_IniFile = new QSettings((c_Dir + c_FileName), QSettings::IniFormat);
    // first check device match
    c_Device = pc_IniFile->value("CONFIG/DEVICE", "").toString();
    if (c_Device == orc_DeviceName) {
      // Collect the available RAM-Path,Name and listindexes
      orc_Files.resize(orc_Files.size() + 1);
      orc_Files[(orc_Files.size() > 0 ? orc_Files.size() - 1 : 0)].c_FileName =
          (c_Dir + c_FileName);
      orc_Files[(orc_Files.size() > 0 ? orc_Files.size() - 1 : 0)]
          .u16_ListIndex = static_cast<uint16_t>(
          pc_IniFile->value("CONFIG/LISTINDEX", 0xFFFFU).toUInt());
    }
    delete pc_IniFile;
  }
  return C_NO_ERR;
...
//**************************************************************.FE*
int32_t C_KFXDEFProject::LoadComments(const QString &orc_FileName,
                                      const QString &orc_DeviceName,
  ...
  pc_IniFile = new QSettings(orc_FileName, QSettings::IniFormat);
  c_Device = pc_IniFile->value("CONFIG/DEVICE", "").toString();
  if (c_Device != orc_DeviceName) {
    delete pc_IniFile;
    return C_CONFIG;
  ...
  u16_NumLanguages =
      static_cast<uint16_t>(pc_IniFile->value("CONFIG/NUMOFLANG", 0).toUInt());
  ...
  for (i = 0; i < u16_NumLanguages; i++) {
    ...
    c_Section = pc_IniFile->value("CONFIG/" + c_Directive, "").toString();
    if ((c_Section == "") ||
        (pc_IniFile->childGroups().contains(c_Section) == false &&
         pc_IniFile->value(c_Section + "/DUMMY").isValid() ==
             false)) // Hacky check for section existence
    {
      // QSettings doesn't support checking for empty groups easily without
      // iterating. But if we assume well formed ini... Actually, let's just
      // proceed. If section doesn't exist, we just won't find values. But the
      // logic below relies on c_Section being valid key. Re-reading code: it
      // reads LANGNAMEx to get the SECTION name for that language. Then it
      // reads parameters from that SECTION.
    }
    // Simplified:
    if (c_Section == "") {
      delete pc_IniFile;
    ...
    for (const QString &c_KeyQS : c_Keys) {
      c_Directive = c_KeyQS;
      c_Text = pc_IniFile->value(c_KeyQS).toString();
      // c_Text is the comment value

      // Split "ListName.VariableName" into list name and variable name
      int s32_DotPos = c_Directive.indexOf('.');
      if (s32_DotPos >= 0) {
        c_ListName = c_Directive.left(s32_DotPos);
        c_VariableName = c_Directive.mid(s32_DotPos + 1);
      } else {
      ...
      if (q_Found == true) {
        ...
        for (l = 0; l < orc_VariableLists[k].VariableList.size(); l++) {
          if (orc_VariableLists[k].VariableList[l].c_Name.toUpper() ==
              c_VariableName.toUpper()) {
            q_Found = true;
            break;
          }
        }
...
/*! \brief    Load default set names from INI file
   ...
   \param[out]    orc_VariableLists variable lists instance to place the names
   into
*/
//----------------------------------------------------------------------------------------------------------------------
void C_KFXDEFProject::LoadDefaultNames(QSettings *const opc_IniFile,
  ...
  orc_VariableLists.ac_DefaultNames.resize(static_cast<uint16_t>(
      opc_IniFile->value("DEFAULT_SETS/DEFAULT_NAMES", 0).toUInt()));
  for (i = 0; i < orc_VariableLists.ac_DefaultNames.size(); i++) {
    orc_VariableLists.ac_DefaultNames[i] =
        opc_IniFile
            ->value("DEFAULT_SETS/NAMEDEFAULT" + QString::number(i),
                    "DEFAULT_" + QString::number(i))
            .toString();
  }
}
