...
//   Caution: Logon,Logoff,ServiceRead,ServiceWrite are not protected by
//   CriticalSection
...
uint16_t
C_KFXProcotolResponse::GetU16FromDataIntel(const uint8_t ou8_StartIndex) const {
  Q_ASSERT(ou8_StartIndex < (sizeof(au8_Data) - 1));
  if (ou8_StartIndex >= (sizeof(au8_Data) - 1)) {
    return 0U;
  }
  return static_cast<uint16_t>(
      static_cast<uint16_t>(au8_Data[ou8_StartIndex]) +
      (static_cast<uint16_t>(
          (static_cast<uint16_t>(au8_Data[ou8_StartIndex + 1U])) << 8U)));
}
...
// Could be named "..Little" as well. But Intel might be quicker to read.
uint32_t
C_KFXProcotolResponse::GetU32FromDataIntel(const uint8_t ou8_StartIndex) const {
...
C_KFXProtocol::C_KFXProtocol(C_KFXCommConfiguration *const opt_CommConfig,
                             const bool oq_SleepBetweenPolling)
    : mpr_ResetCallBack(NULL), mpv_ResetCallBackInstance(NULL),
      mu32_SendIDRQ(0xFFFFFFFFU), mu32_ReceiveIDRSP(0xFFFFFFFFU),
      mpc_Dispatcher(NULL), mu16_CANDispatcherHandle(0U),
      mpt_Config(opt_CommConfig),
      mq_SleepBetweenPolling(oq_SleepBetweenPolling), u8_CreateCommProtocol(0U),
...
void C_KFXProtocol::m_ProtocolCheckInit(void) {
  if (mpt_DebugFileHandle == NULL) {
    mpt_DebugFileHandle = std::fopen("kfxprotocol.txt",
                                     "wb"); // will erase if it already exists !
  }
}

//---------------------------------------------------------------------------

void C_KFXProtocol::m_ProtocolSend(const bool oq_OK,
                                   const T_STWCAN_Msg_TX &orc_Msg) {
  m_ProtocolCheckInit();
  if (mpt_DebugFileHandle != NULL) {
    QString c_Text;
    QString c_Help;
    ...
    if (u8_CreateCommProtocol == 1U) {
      c_Text = QString::asprintf("%s Msg_Sent: MSG: %03x %d ",
                                 c_Time.toUtf8().constData(), orc_Msg.u32_ID,
                                 orc_Msg.u8_DLC);
    ...
    fwrite(c_Text.toUtf8().constData(), 1, static_cast<size_t>(c_Text.length()),
           mpt_DebugFileHandle);
  }
}
  ...
  if (mpt_DebugFileHandle != NULL) {
    ...
    if (u8_CreateCommProtocol == 1U) {
      c_Text = QString::asprintf(
          "%s Msg_Read OK: MSG: %03x %d ",
          c_Time.toUtf8(.constData(), orc_Msg.u32_ID, orc_Msg.u8_DLC));
      for (i = 0; i < orc_Msg.u8_DLC; i++) {
        c_Help = QString::asprintf("%02x ", orc_Msg.au8_Data[i]);
        c_Text += c_Help;
      }
...
int32_t C_KFXProtocol::ConfigChanged(void) {
  ...
  mu32_SendIDRQ = (((static_cast<uint32_t>(u8_BaseID)) << 8U) |
                   ((static_cast<uint32_t>(u8_ClientAddress)) << 1U));
  mu32_ReceiveIDRSP = (((static_cast<uint32_t>(u8_BaseID)) << 8U) |
                       ((static_cast<uint32_t>(u8_ServerAddress)) << 1U)) |
                      1U;
...
int32_t C_KFXProtocol::m_SendMessage(T_STWCAN_Msg_TX &orc_Msg,
                                     const bool oq_SetXtdOff) {
...
//---------------------------------------------------------------------------
// 02.04.09  project index is simply put into the highest nibble without any
// checks
//          -> allows to use the highest nibble for the data version for
//          projects
//             that do not require the project-index
int32_t C_KFXProtocol::SendLogonRequest(const uint16_t ou16_TypeAddressCRC,
                                        const uint8_t ou8_ProjectIndex,
                                        const uint16_t ou16_DataVersion,
                                        const uint16_t ou16_NumOfVariables) {
  ...
  c_MSG.au8_Data[5] =
      static_cast<uint8_t>((ou16_DataVersion >> 8U) |
                           (static_cast<uint8_t>(ou8_ProjectIndex << 4U)));
...
int32_t C_KFXProtocol::SendECRRRequest(const uint16_t ou16_VariableIndex,
                                       const uint16_t ou16_MaxWaitTime,
                                       const uint8_t ou8_UpperHysteresis,
                                       const uint8_t ou8_LowerHysteresis,
  ...
  if ((ou8_UpperHysteresis > 127U) || (ou8_LowerHysteresis > 127U) ||
      (ou8_HysteresisBase > 3U)) {
    return C_RANGE;
  }
  c_MSG.u32_ID = mu32_SendIDRQ;
  c_MSG.u8_DLC = 8U;
  ...
  c_MSG.au8_Data[6] = static_cast<uint8_t>(
      (static_cast<uint8_t>(ou8_UpperHysteresis << 7U)) | ou8_LowerHysteresis);
  c_MSG.au8_Data[7] =
      static_cast<uint8_t>((static_cast<uint8_t>(ou8_HysteresisBase << 6U)) |
                           (ou8_UpperHysteresis >> 1U));
  mc_LastResponse.q_NewValue = false; // we don't have a response yet ...
  return m_SendMessage(c_MSG);
}
...
int32_t C_KFXProtocol::SendTCRRRequest(const uint16_t ou16_VariableIndex,
...
int32_t C_KFXProtocol::SendAbortIndividualResponseRequest(
    const uint16_t ou16_VariableIndex, const bool oq_Handshake) {
  T_STWCAN_Msg_TX c_MSG;
  ...
  c_MSG.u8_DLC = 4U;
  c_MSG.au8_Data[0] = mpt_Config->GetServerAddress();
  c_MSG.au8_Data[1] = static_cast<uint8_t>(
      (oq_Handshake == false) ? KFX_PROTOCOL_ABORT_INDIVIDUAL_RESPONSE
...
int32_t C_KFXProtocol::SendAbortAllResponsesRequest(const bool oq_Handshake) {
  ...
  c_MSG.au8_Data[0] = mpt_Config->GetServerAddress();
  c_MSG.au8_Data[1] = static_cast<uint8_t>(
      (oq_Handshake == false) ? KFX_PROTOCOL_ABORT_ALL_RESPONSES
...
int32_t
C_KFXProtocol::SendImmediateWriteRequest(const uint16_t ou16_VariableIndex,
...
int32_t C_KFXProtocol::SendReadDataWordRequest(const uint8_t ou8_Type,
                                               const uint32_t ou32_Address) {
  T_STWCAN_Msg_TX c_MSG;

  if (mpc_Dispatcher == NULL) {
    return C_NOACT;
  }
  if (((ou8_Type != KFX_PROTOCOL_READ_EEPROM) &&
       (ou8_Type != KFX_PROTOCOL_READ_FLASH_EEPROM) &&
       (ou8_Type != KFX_PROTOCOL_READ_SRAM) &&
       (ou8_Type != KFX_PROTOCOL_READ_EPROM)) ||
...
int32_t C_KFXProtocol::SendReadDataByteRequest(const uint8_t ou8_Type,
                                               const uint32_t ou32_Address) {
  T_STWCAN_Msg_TX c_MSG;
...
int32_t C_KFXProtocol::SendWriteDataWordRequest(const uint8_t ou8_Type,
                                                const uint32_t ou32_Address,
  ...
  if (((ou8_Type != KFX_PROTOCOL_WRITE_EEPROM_HS) &&
       (ou8_Type != KFX_PROTOCOL_WRITE_FLASH_EEPROM_HS) &&
...
int32_t C_KFXProtocol::SendWriteDataByteRequest(const uint8_t ou8_Type,
                                                const uint32_t ou32_Address,
...
int32_t C_KFXProtocol::SendUpdateTaskRequest(const uint16_t ou16_TaskIndex,
  ...
  c_MSG.u8_DLC = 4U;
  c_MSG.au8_Data[0] = mpt_Config->GetServerAddress();
  c_MSG.au8_Data[1] = static_cast<uint8_t>((oq_Handshake == false)
                                               ? KFX_PROTOCOL_UPDATE_TASK
                                               : KFX_PROTOCOL_UPDATE_TASK_HS);
...
/*!
   ...
   \param[in]  ou16_NumFollowingWriteServices   number of following WRITE EEPROM
   services
...
*/
//-----------------------------------------------------------------------------
int32_t C_KFXProtocol::SendWriteEepromSslStartRequest(
    const uint16_t ou16_NumFollowingWriteServices) {
  T_STWCAN_Msg_TX c_MSG;
  uint16_t u16_Crc;
  ...
  c_MSG.au8_Data[5] =
      static_cast<uint8_t>(ou16_NumFollowingWriteServices >> 8U);
...
/*!
   ...
   Send request for confirmation of finished EEPROM write procedure with CRC
   over the sent data.

   \param[in]  ou16_NumRequestedWriteServices   number of WRITE EEPROM services
   sent since calling SendWriteEepromSslStartRequest \param[in]
   ou16_CRCOverSentPayload          CRC over payload sent since calling
...
*/
//-----------------------------------------------------------------------------
int32_t C_KFXProtocol::SendWriteEepromSslEndRequest(
    const uint16_t ou16_NumRequestedWriteServices,
  ...
  c_MSG.au8_Data[5] =
      static_cast<uint8_t>(ou16_NumRequestedWriteServices >> 8U);
...
//                                     in this case the submitted *pbBase will
//                                     be used
//--------------------------------------------------------------
//   .RETURNVALUE
...
//**************************************************************.FE*
int32_t C_KFXProtocol::m_CalcHysteresis(const uint32_t ou32_AbsoluteValue,
                                        uint8_t *const opu8_Base,
                                        uint8_t *const opu8_Value,
  ...
  if (s64_Value <= static_cast<int64_t>(3.0 * f64_Factor)) {
    *opu8_Value = static_cast<uint8_t>(
        (static_cast<float64_t>(s64_Value) * 10.0) / f64_Factor);
  } else {
    *opu8_Value = static_cast<uint8_t>(
        (static_cast<float64_t>(s64_Value) / f64_Factor) + 27.0);
  }
  return C_NO_ERR;
}
...
//                      but have been forced to use the one of the greater
//                      number
//**************************************************************.FE*
int32_t C_KFXProtocol::SendECRRRequestAbsolute(
    const uint16_t ou16_VariableIndex, const uint16_t ou16_MaxWaitTime,
  ...
  s32_Return =
      m_CalcHysteresis(ou32_UpperHysteresis, &u8_BaseHigh, &u8_ValHigh, false);
  if (s32_Return != C_NO_ERR) {
    return s32_Return;
  }
  s32_Return =
      m_CalcHysteresis(ou32_LowerHysteresis, &u8_BaseLow, &u8_ValLow, false);
  if (s32_Return != C_NO_ERR) {
    return s32_Return;
  }
  if (u8_BaseHigh != u8_BaseLow) {
    if (ou32_LowerHysteresis > ou32_UpperHysteresis) {
      ...
      // parameter has not changed and was already checked for range in
      // preceeding call
      (void)m_CalcHysteresis(ou32_UpperHysteresis, &u8_BaseHigh, &u8_ValHigh,
                             true);
    } else {
      u8_BaseLow = u8_BaseHigh;
      // parameter has not changed and was already checked for range in
      // preceeding call
      (void)m_CalcHysteresis(ou32_LowerHysteresis, &u8_BaseLow, &u8_ValLow,
                             true);
    }
    s32_Return = C_WARN;
  }
  ...
  c_MSG.au8_Data[5] = static_cast<uint8_t>(ou16_MaxWaitTime >> 8U);
  c_MSG.au8_Data[6] = static_cast<uint8_t>(
      (static_cast<uint8_t>(u8_ValHigh << 7U)) | u8_ValLow);
  c_MSG.au8_Data[7] = static_cast<uint8_t>(
      (static_cast<uint8_t>(u8_BaseLow << 6U)) | (u8_ValHigh >> 1U));
  mc_LastResponse.q_NewValue = false; // we don't have a response yet ...
  if (s32_Return == C_NO_ERR) {
    return m_SendMessage(c_MSG);
  }
...
void C_KFXProtocol::m_SetLastResponse(const uint8_t ou8_Service,
...
//--------------------------------------------------------------
// return value:
// C_NO_ERR -> expected message received (or: at least one message received if
// oq_CheckExpectedService == false) C_WARN   -> expected message not received
// (or: no new messages if oq_CheckExpectedService == false)
int32_t C_KFXProtocol::m_EvalResponses(const uint16_t ou16_TimeOut,
                                       const uint8_t ou8_ExpectedService,
                                       const bool oq_CheckExpectedService,
                                       C_KFXProcotolResponse &orc_Response) {
  ...
  while (true) {
    ...
    s32_Return = C_NO_ERR;
    while (s32_Return ==
           C_NO_ERR) // make sure to read until there are no more new messages.
    {
      s32_Return =
          mpc_Dispatcher->ReadFromQueue(mu16_CANDispatcherHandle, c_MSG);
      if (s32_Return == C_NO_ERR) {
        q_SomeResponse = this->m_MessageReceived(c_MSG);
        if (q_SomeResponse == true) {
          if (oq_CheckExpectedService == true) {
            if ((ou8_ExpectedService == mc_LastResponse.u8_Service) &&
                (mc_LastResponse.q_NewValue == true)) {
          ...
          // if it's a cyclic transmission it will already have been added to
          // the buffer by "m_MessageReceived"
        }
      }
    }
...
int32_t C_KFXProtocol::EvaluateResponses(const uint8_t ou8_Service,
                                         C_KFXProcotolResponse &orc_Response) {
  return m_EvalResponses(mpt_Config->GetTimeout(), ou8_Service, true,
                         orc_Response);
}
  ...
  if (orc_MSG.u32_ID == mu32_ReceiveIDRSP) {
    ...
    case KFX_PROTOCOL_SERVICE_READ: // no break (same format)
      m_SetLastResponse(
          u8_Service,
          orc_MSG.au8_Data[1] +
              ((static_cast<uint32_t>(orc_MSG.au8_Data[2])) << 8U), // index
    ...
    case KFX_PROTOCOL_UPDATE_TASK_HS:
      m_SetLastResponse(u8_Service, orc_MSG.au8_Data[2],
    ...
    case KFX_PROTOCOL_LOGON_HS:
      m_SetLastResponse(u8_Service,
                        orc_MSG.au8_Data[5] +
                            ((static_cast<uint32_t>(orc_MSG.au8_Data[6]))
                             << 8U), // num of
                                     //  variables
    ...
    case KFX_PROTOCOL_TCRR_TIMESTAMPED:
      ...
      int64_t s64_Value;
      uint16_t u16_Index;
      u16_Index = static_cast<uint16_t>(
          orc_MSG.au8_Data[1] +
          (static_cast<uint16_t>(static_cast<uint16_t>(orc_MSG.au8_Data[2])
                                 << 8U)));
      if (u8_Service == KFX_PROTOCOL_TCRR_TIMESTAMPED) {
        s64_Value = orc_MSG.au8_Data[3] +
                    (static_cast<uint16_t>(
                        static_cast<uint16_t>(orc_MSG.au8_Data[4]) << 8U));
        u32_TimeStamp = orc_MSG.au8_Data[5] +
      ...
      if (mpr_OnNewCyclicTransmissionReceived != NULL) {
        mpr_OnNewCyclicTransmissionReceived(mpv_CyclicCallbackInstance,
                                            u16_Index, s64_Value, u32_TimeStamp,
    ...
    case KFX_PROTOCOL_WRITE_EEPROM_SSL:
      m_SetLastResponse(u8_Service,
                        orc_MSG.au8_Data[4] +
                            ((static_cast<uint32_t>(orc_MSG.au8_Data[5]))
                             << 8U),          // num of services
...
/*!
   ...
   This function will determine which type of segmented transport mechanism is
   configured and use the corresponding functionality.

   For regular segmented transfer the functionality is described here:

   If there is ongoing asynchronuous communication in the background
   (TCRRS/ECRRS) corresponding incoming messages are submitted to the
   corresponding handler functions. This function will actively wait for the
   response data.

   \param[in]     ou16_Index              variable index
   \param[in]     ou32_NumBytesExpected   number of bytes expected in response
   \param[out]    opu8_Data               read data (caller has to provide a
   buffer of ou32_NumBytesExpected bytes) \param[out]    opu16_Error error code
   if receiving an error response (NULL -> don't return error code)
...
*/
//-----------------------------------------------------------------------------
int32_t C_KFXProtocol::SegmentedSRRTransfer(
    const uint16_t ou16_Index, const uint32_t ou32_NumBytesExpected,
  ...
  if ((s32_Return != C_NO_ERR) ||
      (ou16_Index != static_cast<uint16_t>(c_Service.u32_IndexOrAddress))) {
    return C_RD_WR;
  }
  ...
  if (c_Service.q_Error == true) {
    if (opu16_Error != NULL) {
      *opu16_Error = c_Service.au8_Data[0] +
                     (static_cast<uint16_t>(
                         (static_cast<uint16_t>(c_Service.au8_Data[1])) << 8U));
    }
    return C_WARN;
  }

  u32_NumBytesReported =
      c_Service.au8_Data[0] +
  ...
  // now expect all the data frames and send another FC whenever we have
  // received bBSMax messages
  u32_NumMessagesReceived = 0U;
  for (s32_Message = 0;
       s32_Message < static_cast<int32_t>(u32_NumMessagesExpected);
       s32_Message++) {
    ...
    if (c_Service.au8_Data[0] !=
        static_cast<uint8_t>(u32_NumMessagesReceived)) {
      return C_RD_WR; // block number wrong value ...
    }

    (void)memcpy(pu8_Act, &c_Service.au8_Data[1],
                 (u32_NumBytesLeft > 6U) ? 6U : u32_NumBytesLeft);
    u32_NumBytesLeft -= 6U;
    ...
    u8_NextFCRequired--;
    if ((u8_NextFCRequired == 0U) &&
        (s32_Message != static_cast<int32_t>(u32_NumMessagesExpected - 1U))) {
...
int32_t C_KFXProtocol::SegmentedIWRTransfer(const uint16_t ou16_Index,
  ...
  if (ou32_NumBytes >
      0xFFFFFFUL) // maximum possibly with KEFEX protocol: 16MB in a block
  {
    return C_RANGE;
  }
  ...
  for (s32_Message = 0;
       s32_Message < static_cast<int32_t>(u32_NumMessagesTotal);
       s32_Message++) {
    // send CF
    c_MSG.u32_ID = mu32_SendIDRQ;
    c_MSG.u8_DLC = 8U;
    ...
    (void)memcpy(&c_MSG.au8_Data[3], opu8_Data,
                 (u32_NumBytesLeft > 5) ? 5 : u32_NumBytesLeft);
    u32_NumBytesLeft -= 5U;
    ...
    u32_NumMessagesSent++;
    if ((u8_NextFCRequired == 0U) &&
        (s32_Message != static_cast<int32_t>(u32_NumMessagesTotal - 1U))) {
      // get next FC
      s32_Return = this->EvaluateResponses(KFX_PROTOCOL_IWR_FC, c_Service);
      if ((s32_Return != C_NO_ERR) ||
          (ou16_Index != static_cast<uint16_t>(c_Service.u32_IndexOrAddress))) {
...
int32_t
C_KFXProtocol::SendServiceReadRequest(const uint16_t ou16_VariableIndex) {
  T_STWCAN_Msg_TX c_MSG;
...
int32_t
C_KFXProtocol::SendServiceWriteRequest(const uint16_t ou16_VariableIndex,
                                       const uint32_t ou32_Value) {
  T_STWCAN_Msg_TX c_MSG;
...
void C_KFXProtocol::InstallECUResetHandler(
    const PR_KFXProtECUResetCallBack opr_Callback,
...
void C_KFXProtocol::SetKEFEXVariableInfo(
    const C_KFXVariableLists *const opt_KFXLists,
...
void C_KFXProtocol::SetCommDispatcher(
