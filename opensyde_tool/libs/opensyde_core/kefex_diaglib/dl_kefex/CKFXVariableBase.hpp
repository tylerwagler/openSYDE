...
extern const C_KFXVariableTypeDefinition
    gatRAMParameterTypes[KFX_NUM_DATA_TYPES];
...
private:
  uint8_t
      mu8_ActAccess; // actual access rights based on the current user group(s)
  uint32_t mu32_Size;

  static bool mh_IsValueInRange(const float64_t of64_Value,
                                const float64_t of64_Min,
                                const float64_t of64_Max);

  static float64_t mh_GetFloatData(const uint8_t ou8_Type,
                                   const uint8_t *const opu8_Data);
  static int64_t mh_GetNumericData(const uint8_t ou8_Type,
                                   const uint8_t *const opu8_Data);
  static void mh_SetNumericData(const uint32_t ou32_Size,
                                uint8_t *const opu8_Data,
                                const int64_t os64_Value);
  static void mh_SetFloatData(const uint8_t ou8_Type, uint8_t *const opu8_Data,
                              const float64_t of64_Value);

  void m_CheckArrayAccessParams(const uint32_t ou32_ArrayIndex,
                                uint32_t &oru32_ByteOffset) const;
  void m_CheckArrayAccessParamsDefault(const uint16_t ou16_DefaultIndex,
...
public:
  ...
  uint8_t
      u8_Class; ///< matlab/simulink variable class; 0 = signal; 1 = parameter
  ...
  // min/max handling
  void SetMinMaxToMaximum(
      void); // set min/max values to maximum ranges allowed by data type
  ...
  void SetFloatMinInArray(const uint32_t ou32_ArrayIndex,
                          const float64_t of64_Min);
  void SetFloatMaxInArray(const uint32_t ou32_ArrayIndex,
                          const float64_t of64_Max);
  ...
  void SetNumericMinInArray(const uint32_t ou32_ArrayIndex,
                            const int64_t os64_Min);
  void SetNumericMaxInArray(const uint32_t ou32_ArrayIndex,
  ...
  void
  RecalcActAccess(const bool (&orq_AccessLevels)[KFX_NUM_ACCESS_GROUPS + 1]);
  ...
  void SetNumericValueInArray(const uint32_t ou32_ArrayIndex,
                              const int64_t os64_Value);
  void SetFloatValueInArray(const uint32_t ou32_ArrayIndex,
                            const float64_t of64_Value);
  ...
  void SetNumericDefault(const int64_t os64_Value,
                         const uint16_t ou16_DefaultIndex = 0U);
  float64_t GetFloatDefault(const uint16_t ou16_DefaultIndex = 0U) const;
  void SetFloatDefault(const float64_t of64_Value,
                       const uint16_t ou16_DefaultIndex = 0U);
  void ClearDefault(const uint16_t ou16_DefaultIndex = 0U);
  QString GetStringDefault(const uint16_t ou16_DefaultIndex = 0U) const;
  void SetStringDefault(const QString &orc_Value,
                        const uint16_t ou16_DefaultIndex = 0U);
  // array defaults:
  int64_t
  GetNumericDefaultFromArray(const uint32_t ou32_ArrayIndex,
                             const uint16_t ou16_DefaultIndex = 0U) const;
  float64_t
  GetFloatDefaultFromArray(const uint32_t ou32_ArrayIndex,
                           const uint16_t ou16_DefaultIndex = 0U) const;
  void SetNumericDefaultInArray(const uint32_t ou32_ArrayIndex,
                                const int64_t os64_Value,
                                const uint16_t ou16_DefaultIndex = 0U);
  void SetFloatDefaultInArray(const uint32_t ou32_ArrayIndex,
                              const float64_t of64_Value,
                              const uint16_t ou16_DefaultIndex = 0U);
  ...
  QString
  GetTypeDependentValueString(const bool oq_Hex,
                              const bool oq_LeadingZeroes = false) const;
  QString
  GetTypeDependentDefaultString(const bool oq_Hex,
                                const bool oq_LeadingZeroes = false,
                                const uint16_t ou16_DefaultIndex = 0U) const;
};
