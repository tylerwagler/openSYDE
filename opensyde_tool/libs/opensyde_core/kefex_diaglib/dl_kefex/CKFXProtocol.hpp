...
typedef void (*PR_KFXProtNewCyclicTransmissionReceived)(
...
private:
  C_KFXProtocol(
      const C_KFXProtocol &orc_Source); ///< not implemented -> prevent copying
  C_KFXProtocol &
  operator=(const C_KFXProtocol
                &orc_Source); ///< not implemented -> prevent assignment
...
protected:
  ...
  void m_ProtocolSend(const bool oq_OK,
                      const stw::can::T_STWCAN_Msg_TX &orc_Msg);
  ...
  int32_t m_EvalResponses(const uint16_t ou16_TimeOut,
                          const uint8_t ou8_ExpectedService,
                          const bool oq_CheckExpectedService,
                          C_KFXProcotolResponse &orc_Response);

  void m_SetLastResponse(const uint8_t ou8_Service, const uint32_t ou32_Index,
                         const uint8_t *const opu8_Data, const uint8_t ou8_Size,
                         const bool oq_Error);
  void m_GetLastResponse(C_KFXProcotolResponse &orc_Response);

  int32_t m_CalcHysteresis(const uint32_t ou32_AbsoluteValue,
                           uint8_t *const opu8_Base, uint8_t *const opu8_Value,
                           const bool oq_ForceBase) const;

  C_KFXProcotolResponse mc_LastResponse;

  int32_t m_SendMessage(stw::can::T_STWCAN_Msg_TX &orc_Msg,
                        const bool oq_SetXtdOff = true);
...
public:
  C_KFXProtocol(C_KFXCommConfiguration *const opt_CommConfig,
                const bool oq_SleepBetweenPolling = false);
  ~C_KFXProtocol();
  ...
  int32_t EvaluateResponses(const uint8_t ou8_Service,
                            C_KFXProcotolResponse &orc_Response);

  int32_t ConfigChanged(void);

  int32_t SendLogonRequest(const uint16_t ou16_TypeAddressCRC,
  ...
  int32_t SendServiceWriteRequest(const uint16_t ou16_VariableIndex,
                                  const uint32_t ou32_Value);

  int32_t SendSRRRequest(const uint16_t ou16_VariableIndex);
  int32_t SendECRRRequest(const uint16_t ou16_VariableIndex,
  ...
  int32_t SendECRRRequestAbsolute(const uint16_t ou16_VariableIndex,
                                  const uint16_t ou16_MaxWaitTime,
                                  const uint32_t ou32_UpperHysteresis,
                                  const uint32_t ou32_LowerHysteresis);
  int32_t SendTCRRRequest(const uint16_t ou16_VariableIndex,
                          const bool oq_TimeStamped,
                          const uint16_t ou16_Interval);

  int32_t SendAbortIndividualResponseRequest(const uint16_t ou16_VariableIndex,
                                             const bool oq_Handshake);
  int32_t SendAbortAllResponsesRequest(const bool oq_Handshake);
  int32_t SendImmediateWriteRequest(const uint16_t ou16_VariableIndex,
                                    const uint32_t ou32_Value);
  int32_t SendReadDataWordRequest(const uint8_t ou8_Type,
                                  const uint32_t ou32_Address);
  int32_t SendReadDataByteRequest(const uint8_t ou8_Type,
                                  const uint32_t ou32_Address);
  int32_t SendWriteDataWordRequest(const uint8_t ou8_Type,
                                   const uint32_t ou32_Address,
                                   const uint8_t oau8_Values[2]);
  int32_t SendWriteDataByteRequest(const uint8_t ou8_Type,
                                   const uint32_t ou32_Address,
                                   const uint8_t ou8_Value);
  int32_t
  SendWriteEepromSslStartRequest(const uint16_t ou16_NumFollowingWriteServices);
  int32_t
  SendWriteEepromSslEndRequest(const uint16_t ou16_NumRequestedWriteServices,
                               const uint16_t ou16_CRCOverSentPayload);

  int32_t SendUpdateTaskRequest(const uint16_t ou16_TaskIndex,
                                const bool oq_Handshake);

  int32_t SegmentedSRRTransfer(const uint16_t ou16_Index,
                               const uint32_t ou32_NumBytesExpected,
                               uint8_t *const opu8_Data,
                               uint16_t *const opu16_Error = NULL);
  int32_t SegmentedIWRTransfer(const uint16_t ou16_Index,
                               const uint32_t ou32_NumBytes,
                               const uint8_t *opu8_Data);

  // StartupIndication will be directly forwarded to application if polled
  void InstallECUResetHandler(const PR_KFXProtECUResetCallBack opr_Callback,
...
#ifdef CMONPROTOCOL_ALLOW_RAMVIEW_PROJECT_MAPPING
  void SetKEFEXVariableInfo(const C_KFXVariableLists *const opt_KFXLists,
