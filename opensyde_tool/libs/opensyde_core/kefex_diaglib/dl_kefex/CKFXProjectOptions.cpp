...
C_KFXProjectMetaInfo::C_KFXProjectMetaInfo(
    const C_KFXProjectMetaInfo &orc_Source) {
  (*this) = orc_Source;
}

//--------------------------------------------------------------

C_KFXProjectMetaInfo &
...
void C_KFXProjectMetaInfo::LoadFromINI(QSettings &orc_IniFile,
  ...
  c_Text.clear();
  c_Temp =
      orc_IniFile.value(c_Section + "/Line_" + QString::number(s32_Count), "")
          .toString();
...
C_KFXProjectOptions::C_KFXProjectOptions(void)
    : u16_MaxDesignatorLength(32U), u16_ProjectIndex(1U), u16_ListOffset(0U),
      u16_DataVersion(1U), c_DeviceName("Generic Node") {
...
void C_KFXProjectOptions::LoadConfigFromIni(QSettings &orc_IniFile) {
  ...
  u16_MaxDesignatorLength = static_cast<uint16_t>(
      orc_IniFile.value("EDIT/DESIGNATOR_MAX_LENGTH", 200U).toUInt());
  c_DeviceName = orc_IniFile.value("CONFIG/DEVICENAME", "").toString();
  u16_DataVersion = static_cast<uint16_t>(
      orc_IniFile.value("CONFIG/RAMDATAVERSION", 0U).toUInt());
  u16_ListOffset = static_cast<uint16_t>(
      orc_IniFile.value("CONFIG/LISTOFFSET", 0U).toUInt());
  u16_ProjectIndex = static_cast<uint16_t>(
      orc_IniFile.value("CONFIG/PROJECTINDEX", 0U).toUInt());
  for (u8_Index = 0U; u8_Index < KFX_NUM_ACCESS_GROUPS; u8_Index++) {
    aq_UserMayWriteKDX[u8_Index] =
        orc_IniFile
            .value("CONFIG/KDX_WRITE_PERMITTED" + QString::number(u8_Index),
                   false)
            .toBool();
  }
