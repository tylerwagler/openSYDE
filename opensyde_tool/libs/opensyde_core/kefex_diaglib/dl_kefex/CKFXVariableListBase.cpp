...
C_KFXVariableListBase::C_KFXVariableListBase(
    const C_KFXVariableListBase &orc_Source) {
  mu16_NumDefaults =
      0U; // have a clearly defined start value for assignment operator
...
/*! \brief   Calculate CRC over list definition
   ...
   \param[in,out] oru16_CRC              calculated CRC (caller must/may set
   start value) \param[in]     oq_SkipValues          true: include variable
   values; false: don't include values
*/
//----------------------------------------------------------------------------------------------------------------------
void C_KFXVariableListBase::CalcCRCOverList(uint16_t &oru16_CRC,
                                            const bool oq_SkipValues) const {
  int32_t i;
  uint8_t au8_Data[4];

  C_SclChecksums::CalcCRC16STW(this->c_ListName.toUtf8().constData(),
                               static_cast<uint32_t>(this->c_ListName.length()),
                               oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u8_ListType, sizeof(this->u8_ListType),
                               oru16_CRC);
...
const C_KFXVariableBase *C_KFXVariableListBase::GetVariableByName(
    const QString &orc_VariableName) const {
...
C_KFXVariableBase *
C_KFXVariableListBase::GetVariableByName(const QString &orc_VariableName) {
  int32_t s32_VarIndex;
