// Note: for multiple data sets there is no matching of data set names. It is
// the job of the application to care for
//  consistency.
...
//   - number of lists          -> uint16_t (must match information from block
//   0x0100) For Each list:
//   - Number of variables      -> uint16_t (must match information from block
//   0x0100)
//   - number of data sets      -> uint16_t
//   For each variable:
//        -variablesize         -> uint32_t (must match information from block
//        0x0100) -value(s)             -> num-sets * size * uint8_t

// compatibility concept:
//  block 0x0100 MUST be present
//  additional blocks will be parsed if present and understood; else they will
//  be ignored
...
int32_t C_KFXDATFile::LoadDATAllLists(const QString &orc_FileName,
                                      const QString &orc_DeviceName,
                                      C_KFXVariableLists &orc_VariableLists,
                                      QByteArray *const opc_ListsLoaded) {
  return LoadDATList(orc_FileName, orc_DeviceName, orc_VariableLists, -1,
                     opc_ListsLoaded);
}

//--------------------------------------------------------------

void C_KFXDATFile::m_GetWordFromBuffer(uint16_t *const opu16_Value,
                                       const uint8_t **const oppu8_Buffer) {
  // DAT file format litte endian
  (*opu16_Value) = static_cast<uint16_t>(
      (*oppu8_Buffer)[0] +
      static_cast<uint16_t>(
          ((static_cast<uint16_t>((*oppu8_Buffer)[1])) << 8U)));
  *oppu8_Buffer += sizeof(uint16_t);
}

//--------------------------------------------------------------

void C_KFXDATFile::m_GetDWordFromBuffer(uint32_t *const opu32_Value,
                                        const uint8_t **const oppu8_Buffer) {
  // DAT file format litte endian
...
void C_KFXDATFile::m_GetStringFromBuffer(QString &orc_String,
                                         const uint8_t **const oppu8_Buffer) {
  char_t *pcn_Buffer;
  uint16_t u16_Length;
...
/*!
   ...
   - match data from .dat file and place it in the specified VariableList's
   Variable's defaults

   \param[in]     orc_FileName         File path to .dat file
   \param[in]     orc_DeviceName       Device name expected in .dat file
   \param[in,out] orc_VariableLists    Variable lists to fill with the values
   read from the .dat file \param[in]     os32_ListIndex       -1: extract
   values for all variable lists else: extract only values for the specified
   list index \param[out]    opc_ListsLoaded      A flag for each list in the
   KEFEX project 0: Values for list not loaded from file (list is not in file)
   ...
   C_CONFIG   -> .dat file has an unknown file version  \n
                 or: .dat file is for a different project (oc_DeviceName does
   not match) C_WARN     -> .dat read, but at least one variable was not found
   in oc_VariableLists C_OVERFLOW -> could not allocate system memory
*/
//-----------------------------------------------------------------------------
int32_t C_KFXDATFile::LoadDATList(const QString &orc_FileName,
                                  const QString &orc_DeviceName,
                                  C_KFXVariableLists &orc_VariableLists,
                                  const int32_t os32_ListIndex,
                                  QByteArray *const opc_ListsLoaded) {
  ...
  if ((os32_ListIndex != -1) &&
      (os32_ListIndex >
       (orc_VariableLists.size() > 0 ? orc_VariableLists.size() - 1 : 0))) {
    return C_RANGE;
  }
  ...
  try {
    pu8_Zipped =
        new uint8_t[static_cast<uint32_t>(s32_NumBytes) - sizeof(uint32_t)];
  } catch (const std::bad_alloc &) {
    pu8_Zipped = NULL;
    return C_OVERFLOW; // lint !e438 //placate lint ...
  }

  u32_Return = fread(pu8_Zipped, 1UL,
  ...
  s32_Return = C_OscZipData::h_Unzip(pu8_Buffer, u32_UnpackedSize, pu8_Zipped,
                                     static_cast<uint32_t>(s32_NumBytes) -
                                         sizeof(uint32_t));
  delete[] pu8_Zipped;
  ...
  if (os32_ListIndex == -1) {
    s32_Return = m_BufferToLists(pu8_Act, orc_VariableLists, u16_NumLists,
                                 u32_UnpackedSize, false, 0U, opc_ListsLoaded);
  } else {
    s32_Return = m_BufferToLists(
        pu8_Act, orc_VariableLists, u16_NumLists, u32_UnpackedSize, true,
        static_cast<uint16_t>(os32_ListIndex), opc_ListsLoaded);
  }
...
// C_WARN    -> at least one variable in buffer does not exist in VariableLists
// (was ignored) ou16_NumListsInDat: number of lists in .dat file !
int32_t C_KFXDATFile::m_BufferToLists(const uint8_t *const opu8_Buffer,
                                      C_KFXVariableLists &orc_VariableLists,
                                      const uint16_t ou16_NumListsInDat,
                                      const uint32_t ou32_NumBytesTotal,
                                      const bool oq_SingleList,
                                      const uint16_t ou16_SingleListIndex,
                                      QByteArray *const opc_ListsLoaded) {
  QString c_List;
  QString c_Var;
  uint16_t u16_Value;
  ...
  if (oq_SingleList == true) {
    for (s32_Variable = 0;
         s32_Variable <
         orc_VariableLists[ou16_SingleListIndex].VariableList.size();
         s32_Variable++) {
      orc_VariableLists[ou16_SingleListIndex]
          .VariableList[s32_Variable]
          .ClearAllDefaults();
    }
  } else {
    orc_VariableLists.ClearDefaults();
  }
  aapc_IndexMapping.resize(
      ou16_NumListsInDat); // remember mappings, so we do not need to reparse
                           // following blocks
  ...
  for (i = 0; i < ou16_NumListsInDat; i++) {
    ...
    if (oq_SingleList == true) {
      if (orc_VariableLists[ou16_SingleListIndex].c_ListName.toUpper() ==
          c_List.toUpper()) {
        q_Found = true;
      ...
      for (s32_List = 0; s32_List < orc_VariableLists.size(); s32_List++) {
        if (orc_VariableLists[s32_List].c_ListName.toUpper() ==
            c_List.toUpper()) {
          q_Found = true;
    ...
    m_GetWordFromBuffer(&u16_NumVars,
                        &pu8_Data); // number of variables for that list
    ...
    for (j = 0; j < u16_NumVars; j++) {
      ...
      if (q_Found == true) {
        ...
        q_Found = false;
        for (s32_k = 0;
             s32_k < orc_VariableLists[u16_ListIndex].VariableList.size();
             s32_k++) {
          if (orc_VariableLists[u16_ListIndex]
                  .VariableList[s32_k]
                  .c_Name.toUpper() == c_Var.toUpper()) {
            q_Found = true;
            break;
          }
        }
      ...
      if (q_Found == true) {
        ...
        u32_Size = (u32_Value > pc_Variable->GetSize()) ? pc_Variable->GetSize()
                                                        : u32_Value;
        (void)memcpy(&pc_Variable->aau8_Defaults[0][0], pu8_Data,
                     u32_Size); // data
  ...
  u32_NumBytesLeft =
      ou32_NumBytesTotal - static_cast<uint32_t>(pu8_Data - opu8_Buffer);
  while (u32_NumBytesLeft >
         (sizeof(uint32_t) +
          sizeof(uint16_t))) // looks like there is an additional block !
  {
    ...
    case KFX_DAT_FILE_BLOCK_0101:
      ...
      Q_ASSERT(ou16_NumListsInDat ==
               u16_Value); // must match the one from the first block
      u32_NumBytesLeft -= sizeof(uint16_t);
      for (i = 0; i < ou16_NumListsInDat; i++) {
        m_GetWordFromBuffer(&u16_Value, &pu8_Data); // number of variables !
        u32_NumBytesLeft -= sizeof(uint16_t);
        Q_ASSERT(u16_Value ==
                 static_cast<uint16_t>(aapc_IndexMapping[i].size()));

        m_GetWordFromBuffer(&u16_NumDefaults,
                            &pu8_Data); // number of default sets !
        u32_NumBytesLeft -= sizeof(uint16_t);
        ...
        for (j = 0; j < aapc_IndexMapping[i].size(); j++) {
          ...
          if (aapc_IndexMapping[i][j] != NULL) {
            ...
            u32_Size = (u32_Value > pc_Variable->GetSize())
                           ? pc_Variable->GetSize()
                           : u32_Value;
            for (s32_k = 0; s32_k < u16_NumDefaults; s32_k++) {
              if (s32_k < pc_Variable->GetNumDefaults()) {
                (void)memcpy(&pc_Variable->aau8_Defaults[s32_k][0],
                             &pu8_Data[s32_k * u32_Value], u32_Size); // data
              } else {
