...
// The string is used as unique IDs and must never be changed, so applications
// can rely on it ! Notes: Ideally this should be placed as member variables of
// the actual implemented protocol drivers. This is done for other protocols.
// However some existing code (C_KFXSecureProject) requires the KEFEX protocol
// name for compatibility reasons. So if the name would be part of the specific
// protocol class an application would need to drag in
//  that class even if the protocols is not used.
// So we keep the name global here.
// The array must be of POD type instead of a string class so we can rely on it
// being initialized when using it in
//  a constructor.
...
public:
  ...
  // true  -> checking/updating checksums is also expected/supported in indexed
  // mode
  //          only should be used
  ...
  bool q_EEPROMWriteSSL; // true: EEPROM writing with application layer CRC over
                         // data supported
...
// use 32bit index in all functions for future enhancements (e.g. CANopen
// Index/Subindex) ...
typedef void (*PR_KFXCommCyclicTransmissionCallBack)(
...
private:
  C_KFXCommunicationBase(const C_KFXCommunicationBase
                             &orc_Source); // not implemented -> prevent copying
  C_KFXCommunicationBase &operator=(
      const C_KFXCommunicationBase &orc_Source); // not implemented -> prevent
                                                 //  assignment
...
public:
  ...
  virtual void
  InstallECUResetHandler(const PR_KFXCommECUResetCallBack opr_Callback,
                         void *const opv_Instance);
  virtual void InstallCyclicTransmissionCallback(
      const PR_KFXCommCyclicTransmissionCallBack opr_Callback,
      void *const opv_Instance);

  virtual void
  SetCommDispatcher(stw::can::C_CanDispatcher *const opc_Dispatcher) = 0;

  // comm functions:
  virtual int32_t Logon(const uint64_t ou64_ProjectChecksum,
  ...
  virtual int32_t WriteService(const uint32_t ou32_Index,
                               const uint32_t ou32_Value);

  virtual int32_t ReadEEPROM(const uint32_t ou32_Address,
                             const uint32_t ou32_NumBytes,
                             uint8_t *const opu8_Data);
  virtual int32_t WriteEEPROM(const uint32_t ou32_Address,
                              const uint32_t ou32_NumBytes,
  ...
  virtual int32_t ReadROM(const uint32_t ou32_Address,
                          const uint32_t ou32_NumBytes,
                          uint8_t *const opu8_Data);

  // integer + float variables:
  virtual int32_t ReadNumericVariable(const uint32_t ou32_Index,
                                      const uint8_t ou8_NumBytes,
                                      int64_t &ors64_Value);
  virtual int32_t WriteNumericVariable(const uint32_t ou32_Index,
                                       const uint8_t ou8_NumBytes,
                                       const int64_t os64_Value);

  // aggregate types (arrays, structs, whatever ...)
  virtual int32_t ReadAggregateVariable(const uint32_t ou32_Index,
                                        const uint32_t ou32_NumBytes,
                                        uint8_t *const opu8_Data);
  virtual int32_t WriteAggregateVariable(const uint32_t ou32_Index,
                                         const uint32_t ou32_NumBytes,
                                         const uint8_t *const opu8_Data);
  ...
  virtual int32_t
  RequestTimeTriggeredTransmission(const uint32_t ou32_Index,
                                   const bool oq_TimeStamped,
                                   const uint32_t ou32_Interval);
  virtual int32_t RequestChangeTriggeredTransmission(
  ...
  virtual int32_t
  TerminateAllCyclicTransmissions(const bool oq_WaitForHandshake);
  ...
  // lint
  // -sem(stw::diag_lib::C_KFXCommunicationBase::HasFeatureDirectCheckSumHandling,pure)
...
//---------------------------------------------------------------------------
/// Wrapper for IV-style higher level layers (saves us from putting this in a
