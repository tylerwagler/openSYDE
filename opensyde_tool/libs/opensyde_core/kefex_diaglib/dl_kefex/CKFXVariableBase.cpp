...
const C_KFXVariableTypeDefinition
    stw::diag_lib::gatRAMParameterTypes[KFX_NUM_DATA_TYPES] = {
        // name       array  number float  base_type
        {"", false, true, false, KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_NOVAR
        {"UINT8", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_UINT8
        {"SINT8", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_SINT8
        {"SINT16", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_SINT16
        {"UINT16", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_UINT16
        {"SINT32", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_SINT32
        {"UINT32", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_UINT32
        {"STRING", true, false, false,
         KFX_DATA_TYPE_SINT8},                         // KFX_DATA_TYPE_ASINT8
        {"", false, true, false, KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_CRC
        {"AOBYTE", true, true, false,
         KFX_DATA_TYPE_UINT8}, // KFX_DATA_TYPE_AUINT8
        {"FLOAT32", false, true, true,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_FLOAT32
        {"FLOAT64", false, true, true,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_FLOAT64
        {"ASINT16", true, true, false,
         KFX_DATA_TYPE_SINT16}, // KFX_DATA_TYPE_ASINT16
        {"AUINT16", true, true, false,
         KFX_DATA_TYPE_UINT16}, // KFX_DATA_TYPE_AUINT16
        {"ASINT32", true, true, false,
         KFX_DATA_TYPE_SINT32}, // KFX_DATA_TYPE_ASINT32
        {"AUINT32", true, true, false,
         KFX_DATA_TYPE_UINT32}, // KFX_DATA_TYPE_AUINT32
        {"AFLOAT32", true, true, true,
         KFX_DATA_TYPE_FLOAT32}, // KFX_DATA_TYPE_AFLOAT32
        {"AFLOAT64", true, true, true,
         KFX_DATA_TYPE_FLOAT64}, // KFX_DATA_TYPE_AFLOAT64
        {"SINT64", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_SINT64
        {"UINT64", false, true, false,
         KFX_DATA_TYPE_NOVAR}, // KFX_DATA_TYPE_UINT64
        {"ASINT64", true, true, false,
         KFX_DATA_TYPE_SINT64}, // KFX_DATA_TYPE_ASINT64
        {"AUINT64", true, true, false,
         KFX_DATA_TYPE_UINT64} // KFX_DATA_TYPE_AUINT64
};
...
C_KFXVariableBase::C_KFXVariableBase(void) {
  // strings will be automatically initialized to "" in their corresponding
  // constructor !
  this->pu8_Value = NULL;
  this->mu32_Size = 0U;
  ...
  (void)std::memset(&au8_Access[0], KFX_DATA_ACCESS_INVALID,
                    sizeof(au8_Access));
...
/*!
   ...
   \param[in]   orq_AccessLevels   flags showing which groups the user belongs
   to
*/
//-----------------------------------------------------------------------------
void C_KFXVariableBase::RecalcActAccess(
    const bool (&orq_AccessLevels)[KFX_NUM_ACCESS_GROUPS + 1]) {
  uint8_t u8_Index;
...
bool C_KFXVariableBase::IsReadable(void) const {
  return ((this->mu8_ActAccess == KFX_DATA_ACCESS_RO) ||
          (this->mu8_ActAccess == KFX_DATA_ACCESS_RW));
}
...
bool C_KFXVariableBase::IsWriteable(void) const {
  return ((this->mu8_ActAccess == KFX_DATA_ACCESS_WO) ||
          (this->mu8_ActAccess == KFX_DATA_ACCESS_RW));
}
...
*/
//-----------------------------------------------------------------------------
uint32_t C_KFXVariableBase::GetSize(void) const { return this->mu32_Size; }
...
float64_t C_KFXVariableBase::mh_GetFloatData(const uint8_t ou8_Type,
...
void C_KFXVariableBase::SetFloatValue(const float64_t of64_Value) {
  C_KFXVariableBase::mh_SetFloatData(this->u8_Type, this->pu8_Value,
                                     of64_Value);
}

//**************************************************************

int64_t C_KFXVariableBase::mh_GetNumericData(const uint8_t ou8_Type,
                                             const uint8_t *const opu8_Data) {
  int64_t s64_Value;
  ...
  case KFX_DATA_TYPE_UINT16:
    s64_Value = opu8_Data[0] + (static_cast<uint32_t>(opu8_Data[1]) << 8);
    break;
  case KFX_DATA_TYPE_SINT16:
    s64_Value = static_cast<int16_t>(
        opu8_Data[0] + (static_cast<uint32_t>(opu8_Data[1]) << 8));
    break;
  ...
  case KFX_DATA_TYPE_FLOAT32: // if someone really requests data as float32_t we
                              // give him/her the best we can ...
    s64_Value = opu8_Data[0] + (static_cast<uint32_t>(opu8_Data[1]) << 8U) +
                (static_cast<uint32_t>(opu8_Data[2]) << 16U) +
                (static_cast<uint32_t>(opu8_Data[3]) << 24U);
    break;
  case KFX_DATA_TYPE_SINT64:
    s64_Value = static_cast<int64_t>(
        opu8_Data[0] + (static_cast<uint64_t>(opu8_Data[1]) << 8U) +
        (static_cast<uint64_t>(opu8_Data[2]) << 16U) +
        (static_cast<uint64_t>(opu8_Data[3]) << 24U) +
        (static_cast<uint64_t>(opu8_Data[4]) << 32U) +
        (static_cast<uint64_t>(opu8_Data[5]) << 40U) +
  ...
  case KFX_DATA_TYPE_FLOAT64: // if someone really requests data as float64_t we
                              // give him/her the best we can ...
    // note: MSB might be cut off; application has to cast back
    s64_Value = opu8_Data[0] + (static_cast<uint64_t>(opu8_Data[1]) << 8U) +
                (static_cast<uint64_t>(opu8_Data[2]) << 16U) +
                (static_cast<uint64_t>(opu8_Data[3]) << 24U) +
                (static_cast<uint64_t>(opu8_Data[4]) << 32U) +
                (static_cast<uint64_t>(opu8_Data[5]) << 40U) +
...
void C_KFXVariableBase::SetNumericValue(const int64_t os64_Value) {
  C_KFXVariableBase::mh_SetNumericData(this->mu32_Size, this->pu8_Value,
                                       os64_Value);
}
...
void C_KFXVariableBase::SetStringValue(const QString &orc_Value) {
  if (orc_Value.length() < this->mu32_Size) // consider the '\0' -> "<"
  {
    ...
    std::sprintf(reinterpret_cast<char_t *>(this->pu8_Value), "%s",
                 orc_Value.toUtf8().constData());
  } else {
    (void)std::memcpy(this->pu8_Value, orc_Value.toUtf8().constData(),
                      this->mu32_Size);
  }
}
...
void C_KFXVariableBase::CopyDefaultToValue(const uint16_t ou16_DefaultIndex) {
  if (ou16_DefaultIndex < static_cast<uint16_t>(aau8_Defaults.size())) {
    (void)std::memcpy(this->pu8_Value,
                      reinterpret_cast<const uint8_t *>(
                          this->aau8_Defaults[ou16_DefaultIndex].constData()),
                      this->mu32_Size);
  }
}
...
void C_KFXVariableBase::CopyValueToDefault(const uint16_t ou16_DefaultIndex) {
  if (ou16_DefaultIndex < static_cast<uint16_t>(aau8_Defaults.size())) {
    (void)std::memcpy(reinterpret_cast<uint8_t *>(const_cast<char *>(
                          this->aau8_Defaults[ou16_DefaultIndex].data())),
                      this->pu8_Value, this->mu32_Size);
  }
}

//**************************************************************

bool C_KFXVariableBase::DefaultMatchesValue(
    const uint16_t ou16_DefaultIndex) const {
  int32_t s32_Return;

  if (ou16_DefaultIndex < static_cast<uint16_t>(aau8_Defaults.size())) {
    s32_Return =
        std::memcmp(this->pu8_Value,
                    reinterpret_cast<const uint8_t *>(
                        this->aau8_Defaults[ou16_DefaultIndex].constData()),
                    this->mu32_Size);
  } else {
...
void C_KFXVariableBase::ClearDefault(const uint16_t ou16_DefaultIndex) {
  if (ou16_DefaultIndex < static_cast<uint16_t>(aau8_Defaults.size())) {
    (void)std::memset(reinterpret_cast<uint8_t *>(const_cast<char *>(
                          this->aau8_Defaults[ou16_DefaultIndex].data())),
                      0, this->mu32_Size);
  }
}
...
bool C_KFXVariableBase::mh_IsValueInRange(const float64_t of64_Value,
                                          const float64_t of64_Min,
                                          const float64_t of64_Max) {
  volatile float64_t
      f64_Compare; // make volatile for NaN check to be sure to work !

  // some libraries (tested with Borland C++ Builder) behave weird here and
  // throw exception on INF or NAN
  try {
...
/*!
   ...
   Checks whether the current variable value lies within the define minimum and
   maximum values. Can be used for all available variable types.
  ...
  if (gatRAMParameterTypes[this->u8_Type].q_IsArray == true) {
    ...
    for (u32_Element = 0U; u32_Element < u32_NumEntries; u32_Element++) {
      if (gatRAMParameterTypes[this->u8_Type].q_IsFloat == true) {
        q_Return = C_KFXVariableBase::mh_IsValueInRange(
            this->GetFloatValueFromArray(u32_Element),
    ...
    q_Return = C_KFXVariableBase::mh_IsValueInRange(
        this->GetFloatValue(), this->GetFloatMin(), this->GetFloatMax());
  } else {
    s64_Value = this->GetNumericValue();
    q_Return = ((s64_Value >= this->GetNumericMin()) &&
                (s64_Value <= this->GetNumericMax()));
  }
  return q_Return;
}
...
/*!
   ...
   \return
   numeric default value for the specified set (zero if the set does not exist
   or the variable type is invalid)
*/
//-----------------------------------------------------------------------------
int64_t
C_KFXVariableBase::GetNumericDefault(const uint16_t ou16_DefaultIndex) const {
  if (ou16_DefaultIndex >= static_cast<uint16_t>(this->aau8_Defaults.size())) {
    return 0;
  }

  return C_KFXVariableBase::mh_GetNumericData(
      this->u8_Type, reinterpret_cast<const uint8_t *>(
                         this->aau8_Defaults[ou16_DefaultIndex].constData()));
}
...
*/
//-----------------------------------------------------------------------------
void C_KFXVariableBase::SetNumericDefault(const int64_t os64_Value,
                                          const uint16_t ou16_DefaultIndex) {
  if (ou16_DefaultIndex < static_cast<uint16_t>(this->aau8_Defaults.size())) {
    C_KFXVariableBase::mh_SetNumericData(
        this->mu32_Size,
        reinterpret_cast<uint8_t *>(
            const_cast<char *>(this->aau8_Defaults[ou16_DefaultIndex].data())),
        os64_Value);
  }
}
...
//-----------------------------------------------------------------------------
float64_t
C_KFXVariableBase::GetFloatDefault(const uint16_t ou16_DefaultIndex) const {
  if (ou16_DefaultIndex >= static_cast<uint16_t>(this->aau8_Defaults.size())) {
    return 0.0;
  }

  return C_KFXVariableBase::mh_GetFloatData(
      this->u8_Type, reinterpret_cast<const uint8_t *>(
                         this->aau8_Defaults[ou16_DefaultIndex].constData()));
}
...
*/
//-----------------------------------------------------------------------------
void C_KFXVariableBase::SetFloatDefault(const float64_t of64_Value,
                                        const uint16_t ou16_DefaultIndex) {
  if (ou16_DefaultIndex < static_cast<uint16_t>(this->aau8_Defaults.size())) {
    C_KFXVariableBase::mh_SetFloatData(
        this->u8_Type,
        reinterpret_cast<uint8_t *>(
            const_cast<char *>(this->aau8_Defaults[ou16_DefaultIndex].data())),
        of64_Value);
...
QString
C_KFXVariableBase::GetStringDefault(const uint16_t ou16_DefaultIndex) const {
  QString c_Help;
  uint32_t u32_Len;
  ...
  pu8_Data = reinterpret_cast<const uint8_t *>(
      this->aau8_Defaults[ou16_DefaultIndex].constData());
...
void C_KFXVariableBase::SetStringDefault(const QString &orc_Value,
                                         const uint16_t ou16_DefaultIndex) {
  uint8_t *pu8_Data;

  if (ou16_DefaultIndex < static_cast<uint16_t>(this->aau8_Defaults.size())) {
    pu8_Data = reinterpret_cast<uint8_t *>(
        const_cast<char *>(this->aau8_Defaults[ou16_DefaultIndex].data()));

    if (orc_Value.length() < this->mu32_Size) {
      std::sprintf(reinterpret_cast<char_t *>(pu8_Data), "%s",
                   orc_Value.toUtf8().constData());
    } else {
      (void)std::memcpy(pu8_Data, orc_Value.toUtf8().constData(),
                        this->mu32_Size);
    }
  }
}
...
C_KFXVariableBase::operator=(const C_KFXVariableBase &orc_Source) {
  ...
  this->SetSize(
      orc_Source
          .mu32_Size); // will allocate memory for value and default internally
  ...
  (void)std::memcpy(this->pu8_Value, orc_Source.pu8_Value,
                    orc_Source.mu32_Size);
...
*/
//-----------------------------------------------------------------------------
void C_KFXVariableBase::CalcCRCOverEntry(uint16_t &oru16_CRC,
                                         const bool oq_SkipValue) const {
  int32_t s32_Default;
  uint8_t u8_Index;

  C_SclChecksums::CalcCRC16STW(this->c_Name.toUtf8().constData(),
                               static_cast<uint32_t>(this->c_Name.length()),
                               oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u32_Address, sizeof(this->u32_Address),
                               oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->mu32_Size, sizeof(this->mu32_Size),
                               oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u8_Type, sizeof(this->u8_Type),
                               oru16_CRC);
  C_SclChecksums::CalcCRC16STW(
      reinterpret_cast<uint8_t *>(const_cast<char *>(this->mc_MinValue.data())),
      this->mu32_Size, oru16_CRC);
  C_SclChecksums::CalcCRC16STW(
      reinterpret_cast<uint8_t *>(const_cast<char *>(this->mc_MaxValue.data())),
      this->mu32_Size, oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->au8_Access[0], sizeof(this->au8_Access),
                               oru16_CRC);
  for (s32_Default = 0; s32_Default < aau8_Defaults.size(); s32_Default++) {
    C_SclChecksums::CalcCRC16STW(
        reinterpret_cast<const uint8_t *>(
            this->aau8_Defaults[s32_Default].constData()),
        this->mu32_Size, oru16_CRC);
  }
  if (oq_SkipValue == false) {
    C_SclChecksums::CalcCRC16STW(this->pu8_Value, this->mu32_Size, oru16_CRC);
  }
  C_SclChecksums::CalcCRC16STW(&this->s32_ScalingFactor,
                               sizeof(this->s32_ScalingFactor), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u8_ScalingDigits,
                               sizeof(this->u8_ScalingDigits), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(this->c_Unit.toUtf8().constData(),
                               static_cast<uint32_t>(this->c_Unit.length()),
                               oru16_CRC);
  for (u8_Index = 0U; u8_Index < KFX_DATA_MAX_NUM_LANGUAGES; u8_Index++) {
    C_SclChecksums::CalcCRC16STW(
        this->ac_Comments[u8_Index].toUtf8().constData(),
        static_cast<uint32_t>(this->ac_Comments[u8_Index].length()), oru16_CRC);
  }
  C_SclChecksums::CalcCRC16STW(&this->q_LocationRAM,
                               sizeof(this->q_LocationRAM), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->e_DefTransmissionType,
                               sizeof(this->e_DefTransmissionType), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u16_DefInterval,
                               sizeof(this->u16_DefInterval), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u32_DefLowerHysteresis,
                               sizeof(this->u32_DefLowerHysteresis), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u32_DefUpperHysteresis,
                               sizeof(this->u32_DefUpperHysteresis), oru16_CRC);
  C_SclChecksums::CalcCRC16STW(&this->u8_Class, sizeof(this->u8_Class),
                               oru16_CRC);
}
...
bool C_KFXVariableBase::IsNumericValue(void) const {
  if (this->u8_Type < KFX_NUM_DATA_TYPES) {
    return ((gatRAMParameterTypes[this->u8_Type].q_IsNumber) &&
            (!gatRAMParameterTypes[this->u8_Type].q_IsArray));
  }
  return false;
}
...
bool C_KFXVariableBase::IsBinaryArrayType(void) const {
  if (this->u8_Type < KFX_NUM_DATA_TYPES) {
    return ((gatRAMParameterTypes[this->u8_Type].q_IsArray) &&
            (gatRAMParameterTypes[this->u8_Type].q_IsNumber));
  }
  return false;
}
...
float64_t C_KFXVariableBase::GetFloatMin(void) const {
  return C_KFXVariableBase::mh_GetFloatData(
      this->u8_Type, reinterpret_cast<uint8_t *>(
                         const_cast<char *>(this->mc_MinValue.data())));
}
...
float64_t C_KFXVariableBase::GetFloatMax(void) const {
  return C_KFXVariableBase::mh_GetFloatData(
      this->u8_Type, reinterpret_cast<uint8_t *>(
                         const_cast<char *>(this->mc_MaxValue.data())));
}
...
float64_t
C_KFXVariableBase::GetFloatMinFromArray(const uint32_t ou32_ArrayIndex) const {
  ...
  return C_KFXVariableBase::mh_GetFloatData(
      u8_ElementType, reinterpret_cast<uint8_t *>(const_cast<char *>(
                          &mc_MinValue.data()[u32_ByteOffset])));
}

//**************************************************************

float64_t
C_KFXVariableBase::GetFloatMaxFromArray(const uint32_t ou32_ArrayIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
  ...
  return C_KFXVariableBase::mh_GetFloatData(
      u8_ElementType, reinterpret_cast<uint8_t *>(const_cast<char *>(
                          &mc_MaxValue.data()[u32_ByteOffset])));
}

//**************************************************************

void C_KFXVariableBase::SetFloatMinInArray(const uint32_t ou32_ArrayIndex,
                                           const float64_t of64_Min) {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
  ...
  C_KFXVariableBase::mh_SetFloatData(
      u8_ElementType,
      reinterpret_cast<uint8_t *>(&this->mc_MinValue.data()[u32_ByteOffset]),
      of64_Min);
}

//**************************************************************

void C_KFXVariableBase::SetFloatMaxInArray(const uint32_t ou32_ArrayIndex,
                                           const float64_t of64_Max) {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
  ...
  C_KFXVariableBase::mh_SetFloatData(
      u8_ElementType,
      reinterpret_cast<uint8_t *>(&this->mc_MaxValue.data()[u32_ByteOffset]),
      of64_Max);
}

//**************************************************************

int64_t C_KFXVariableBase::GetNumericMinFromArray(
    const uint32_t ou32_ArrayIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
...
int64_t C_KFXVariableBase::GetNumericMaxFromArray(
    const uint32_t ou32_ArrayIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
...
void C_KFXVariableBase::SetNumericMinInArray(const uint32_t ou32_ArrayIndex,
                                             const int64_t os64_Min) {
...
void C_KFXVariableBase::SetNumericMaxInArray(const uint32_t ou32_ArrayIndex,
                                             const int64_t os64_Max) {
  uint8_t u8_SizeOfOneEntry;
  uint32_t u32_ByteOffset;
...
int64_t C_KFXVariableBase::GetNumericDefaultFromArray(
    const uint32_t ou32_ArrayIndex, const uint16_t ou16_DefaultIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;

  this->m_CheckArrayAccessParamsDefault(ou16_DefaultIndex, ou32_ArrayIndex,
                                        u32_ByteOffset);

  u8_ElementType = this->m_GetEntryTypeByArrayType();
  if (u8_ElementType == KFX_DATA_TYPE_NOVAR) {
    throw "C_KFXVariableBase::GetNumericDefaultFromArray: not a valid array "
          "type";
  }
  return C_KFXVariableBase::mh_GetNumericData(
      u8_ElementType,
      reinterpret_cast<uint8_t *>(const_cast<char *>(
          &aau8_Defaults[ou16_DefaultIndex].data()[u32_ByteOffset])));
}

//**************************************************************

float64_t C_KFXVariableBase::GetFloatDefaultFromArray(
    const uint32_t ou32_ArrayIndex, const uint16_t ou16_DefaultIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
...
int64_t C_KFXVariableBase::GetNumericValueFromArray(
    const uint32_t ou32_ArrayIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
  ...
  return C_KFXVariableBase::mh_GetNumericData(u8_ElementType,
                                              &this->pu8_Value[u32_ByteOffset]);
}

//**************************************************************
// Assumption: float endianness is the same as integer endianness
void C_KFXVariableBase::mh_SetFloatData(const uint8_t ou8_Type,
  ...
  case KFX_DATA_TYPE_FLOAT64:
    ...
    break;
  default:
    // don't throw: this might cause some compatibility issues with existing
    // applications
    break;
  }
}
...
void C_KFXVariableBase::mh_SetNumericData(const uint32_t ou32_Size,
                                          uint8_t *const opu8_Data,
  ...
  case 2:
    ...
    opu8_Data[1] =
        static_cast<uint8_t>((static_cast<uint16_t>(os64_Value)) >> 8U);
    break;
  case 4:
    opu8_Data[0] = static_cast<uint8_t>(static_cast<uint32_t>(os64_Value));
    opu8_Data[1] =
        static_cast<uint8_t>((static_cast<uint32_t>(os64_Value)) >> 8U);
    opu8_Data[2] =
        static_cast<uint8_t>((static_cast<uint32_t>(os64_Value)) >> 16U);
    opu8_Data[3] =
        static_cast<uint8_t>((static_cast<uint32_t>(os64_Value)) >> 24U);
    break;
  case 8:
    opu8_Data[0] = static_cast<uint8_t>(static_cast<uint64_t>(os64_Value));
    opu8_Data[1] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 8U);
    opu8_Data[2] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 16U);
    opu8_Data[3] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 24U);
    opu8_Data[4] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 32U);
    opu8_Data[5] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 40U);
    opu8_Data[6] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 48U);
    opu8_Data[7] =
        static_cast<uint8_t>((static_cast<uint64_t>(os64_Value)) >> 56U);
    break;
  default:
    // don't throw: this might cause some compatibility issues with existing
    // applications
...
void C_KFXVariableBase::SetNumericValueInArray(const uint32_t ou32_ArrayIndex,
                                               const int64_t os64_Value) {
  uint8_t u8_SizeOfOneEntry;
  uint32_t u32_ByteOffset;
  ...
  C_KFXVariableBase::mh_SetNumericData(
      u8_SizeOfOneEntry, &this->pu8_Value[u32_ByteOffset], os64_Value);
}

//**************************************************************

void C_KFXVariableBase::SetFloatValueInArray(const uint32_t ou32_ArrayIndex,
                                             const float64_t of64_Value) {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
  ...
  C_KFXVariableBase::mh_SetFloatData(
      u8_ElementType, &this->pu8_Value[u32_ByteOffset], of64_Value);
}

//**************************************************************

float64_t C_KFXVariableBase::GetFloatValueFromArray(
    const uint32_t ou32_ArrayIndex) const {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
  ...
  return C_KFXVariableBase::mh_GetFloatData(u8_ElementType,
                                            &this->pu8_Value[u32_ByteOffset]);
}
...
void C_KFXVariableBase::SetNumericDefaultInArray(
    const uint32_t ou32_ArrayIndex, const int64_t os64_Value,
    const uint16_t ou16_DefaultIndex) {
  uint8_t u8_SizeOfOneEntry;
  uint32_t u32_ByteOffset;
  ...
  C_KFXVariableBase::mh_SetNumericData(
      u8_SizeOfOneEntry,
      reinterpret_cast<uint8_t *>(
          &this->aau8_Defaults[ou16_DefaultIndex].data()[u32_ByteOffset]),
      os64_Value);
}

//**************************************************************

void C_KFXVariableBase::SetFloatDefaultInArray(
    const uint32_t ou32_ArrayIndex, const float64_t of64_Value,
    const uint16_t ou16_DefaultIndex) {
  uint8_t u8_ElementType;
  uint32_t u32_ByteOffset;
...
void C_KFXVariableBase::m_CheckArrayAccessParams(
    const uint32_t ou32_ArrayIndex, uint32_t &oru32_ByteOffset) const {
  uint8_t u8_SizeOfOneEntry;
...
void C_KFXVariableBase::m_CheckArrayAccessParamsDefault(
    const uint16_t ou16_DefaultIndex, const uint32_t ou32_ArrayIndex,
...
/*!
   ...
   Set the minimum and maximum values to the extremes possible based on the
   defined variable type. For array variables this sets the individual elements
   of the min/max arrays to the min/max of the array base type.
*/
//-----------------------------------------------------------------------------
void C_KFXVariableBase::SetMinMaxToMaximum(void) {
  ...
  case KFX_DATA_TYPE_ASINT8:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetNumericMinInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<int8_t>::min()));
      this->SetNumericMaxInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<int8_t>::max()));
    }
    break;
  case KFX_DATA_TYPE_AUINT8:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetNumericMinInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<uint8_t>::min()));
      this->SetNumericMaxInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<uint8_t>::max()));
    }
    break;
  case KFX_DATA_TYPE_FLOAT32:
    this->SetFloatMin(
  ...
  case KFX_DATA_TYPE_ASINT16:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetNumericMinInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<int16_t>::min()));
      this->SetNumericMaxInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<int16_t>::max()));
    }
    break;
  case KFX_DATA_TYPE_AUINT16:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
  ...
  case KFX_DATA_TYPE_ASINT32:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetNumericMinInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<int32_t>::min()));
      this->SetNumericMaxInArray(
          u32_Index, static_cast<int64_t>(std::numeric_limits<int32_t>::max()));
    }
    break;
  ...
  case KFX_DATA_TYPE_AFLOAT32:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetFloatMinInArray(
          u32_Index,
          static_cast<float64_t>(-std::numeric_limits<float32_t>::max()));
      this->SetFloatMaxInArray(
          u32_Index,
          static_cast<float64_t>(std::numeric_limits<float32_t>::max()));
    }
    break;
  case KFX_DATA_TYPE_AFLOAT64:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetFloatMinInArray(u32_Index,
                               -std::numeric_limits<float64_t>::max());
      this->SetFloatMaxInArray(u32_Index,
                               std::numeric_limits<float64_t>::max());
    }
    break;
  case KFX_DATA_TYPE_SINT64:
  ...
  case KFX_DATA_TYPE_ASINT64:
    for (u32_Index = 0U; u32_Index < this->GetNumberOfArrayElements();
         u32_Index++) {
      this->SetNumericMinInArray(u32_Index,
                                 std::numeric_limits<int64_t>::min());
      this->SetNumericMaxInArray(u32_Index,
...
QString C_KFXVariableBase::GetTypeName(void) const {
  if ((this->u8_Type != KFX_DATA_TYPE_NOVAR) &&
      (this->u8_Type != KFX_DATA_TYPE_CRC) &&
      (this->u8_Type < KFX_NUM_DATA_TYPES)) {
    return static_cast<QString>(
        gatRAMParameterTypes[this->u8_Type].pcn_WrittenName);
  }
  return "INVALID";
}
...
QString C_KFXVariableBase::GetTypeDependentValueString(
    const bool oq_Hex, const bool oq_LeadingZeroes) const {
  QString c_Help;
  int64_t s64_Temp;
  float64_t f64_Temp;
  ...
  case KFX_DATA_TYPE_UINT64: // todo: consider MSB
    ...
    if (oq_Hex == true) {
      if (s64_Temp < 0) {
        s64_Temp *= -1;
        c_Help =
            "-0x" + stw::opensyde_core::C_OscUtils::h_IntToHex(s64_Temp, 0U);
        if (oq_LeadingZeroes == true) {
          int s32_PadCount =
              static_cast<int>((3 + (this->GetSize() * 2)) - c_Help.length());
          if (s32_PadCount > 0) {
        ...
        if (oq_LeadingZeroes == true) {
          int s32_PadCount =
              static_cast<int>((2 + (this->GetSize() * 2)) - c_Help.length());
          if (s32_PadCount > 0) {
...
QString C_KFXVariableBase::GetTypeDependentDefaultString(
    const bool oq_Hex, const bool oq_LeadingZeroes,
    const uint16_t ou16_DefaultIndex) const {
  QString c_Help;
  int64_t s64_Temp;
  float64_t f64_Temp;
