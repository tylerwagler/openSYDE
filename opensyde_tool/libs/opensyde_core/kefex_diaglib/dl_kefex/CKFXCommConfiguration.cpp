...
C_KFXCommParameter::C_KFXCommParameter(void)
    : c_Name(""), s64_MinValue(0), s64_MaxValue(0), s64_DefaultValue(0),
...
uint8_t C_KFXCommConfiguration::GetBaseID(void) const {
  ...
  if (s32_Return != C_NO_ERR) {
    ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to bBaseID for a protocol implementation that does not have "
          "this parameter !";
  }
...
uint8_t C_KFXCommConfiguration::GetClientAddress(void) const {
  ...
  if (s32_Return != C_NO_ERR) {
    // complain really hard ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to bClientAddress for a protocol implementation that does "
...
void C_KFXCommConfiguration::SetClientAddress(const uint8_t ou8_Value) {
  ...
  s32_Return =
      SetParameterValue("CLIENTADDRESS", static_cast<int64_t>(ou8_Value));
  if (s32_Return != C_NO_ERR) {
    // complain really hard ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to bClientAddress for a protocol implementation that does "
...
void C_KFXCommConfiguration::SetServerAddress(const uint8_t ou8_Value) {
  ...
  s32_Return =
      SetParameterValue("SERVERADDRESS", static_cast<int64_t>(ou8_Value));
  if (s32_Return != C_NO_ERR) {
    // complain really hard ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to bServerAddress for a protocol implementation that does "
          "not have this parameter !";
  }
}
...
uint16_t C_KFXCommConfiguration::GetTimeout(void) const {
  ...
  if (s32_Return != C_NO_ERR) {
    // complain really hard ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to wTimeout for a protocol implementation that does not have "
          "this parameter !";
  }
...
uint8_t C_KFXCommConfiguration::GetSTMin(void) const {
  ...
  if (s32_Return != C_NO_ERR) {
    // complain really hard ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to bSTMin for a protocol implementation that does not have "
          "this parameter !";
  }
  return static_cast<uint8_t>(s64_Value);
}
...
uint8_t C_KFXCommConfiguration::GetBSMax(void) const {
  ...
  if (s32_Return != C_NO_ERR) {
    // complain really hard ...
    throw "Error in usage of C_KFXCommConfiguration:\nTrying to use deprecated "
          "access to bBSMax for a protocol implementation that does not have "
          "this parameter !";
  }
  return static_cast<uint8_t>(s64_Value);
}
...
C_KFXCommConfigurationBase::C_KFXCommConfigurationBase(void) {
  // default: stay compatible with existing applications -> use standard KEFEX
  // protocol parameters:
  ...
  mc_Parameters[4].c_Comment =
      "Minimum distance between 2 RX messages in milli-seconds";
  ...
  mc_Parameters[5].c_Comment =
      "Maximum number of RX messages without a handshake";
...
C_KFXCommConfigurationBase::C_KFXCommConfigurationBase(
    const C_KFXCommConfigurationBase &orc_Source) {
  this->operator=(orc_Source);
}

//---------------------------------------------------------------------------

C_KFXCommConfigurationBase &C_KFXCommConfigurationBase::operator=(
    const C_KFXCommConfigurationBase &orc_Source) {
...
C_KFXCommConfigurationBase::C_KFXCommConfigurationBase(
    const QList<C_KFXCommParameter> &orc_Parameters) {
  this->SetParameterList(orc_Parameters);
}

//---------------------------------------------------------------------------

void C_KFXCommConfigurationBase::SetParameterList(
    const QList<C_KFXCommParameter> &orc_Parameters) {
  int32_t s32_Index;
...
int32_t
C_KFXCommConfigurationBase::LoadConfigFromINI(QSettings &orc_File,
  ...
  for (s32_Index = 0; s32_Index < mc_Parameters.size(); s32_Index++) {
    mc_Values[s32_Index] = static_cast<int64_t>(
        orc_File
            .value(
                c_Section + "/" +
                    QString::fromUtf8(
...
int32_t C_KFXCommConfigurationBase::SaveConfigToINI(
  ...
  try {
    orc_File.setValue(c_Section + "/COMM_PROTOCOL_TYPE",
                      QString::fromUtf8(mc_ProtocolName.toUtf8().constData()));
    for (s32_Index = 0; s32_Index < mc_Parameters.size(); s32_Index++) {
      orc_File.setValue(
          c_Section + "/" +
              QString::fromUtf8(
