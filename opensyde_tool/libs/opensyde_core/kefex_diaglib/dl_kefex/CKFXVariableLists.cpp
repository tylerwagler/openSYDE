...
void C_KFXVariableLists::ClearDefaults(void) {
  ...
  for (u16_List = 0U; u16_List < this->size(); u16_List++) {
    for (u16_Variable = 0U;
         u16_Variable < (*this)[u16_List].VariableList.size(); u16_Variable++) {
...
//**************************************************************
// 04.02.09  also include default names (strictly speaking not a part of the
// lists,
//                                       but we want to detect any changes)
void C_KFXVariableLists::CalcCRCOverAllLists(uint16_t &oru16_CRC,
                                             const bool oq_SkipValues) const {
  uint16_t u16_Index;
  ...
  for (u16_Index = 0; u16_Index < this->ac_DefaultNames.size(); u16_Index++) {
    C_SclChecksums::CalcCRC16STW(
        this->ac_DefaultNames[u16_Index].toUtf8().constData(),
        static_cast<uint32_t>(this->ac_DefaultNames[u16_Index].length()),
        oru16_CRC);
  }
}
...
bool C_KFXVariableLists::VariableExists(const uint16_t ou16_List,
                                        const uint16_t ou16_Variable) const {
  if (this->size() <= ou16_List) {
    return false;
  } else {
...
uint16_t C_KFXVariableLists::PackIndex(const uint16_t ou16_List,
...
void C_KFXVariableLists::UnpackIndex(const uint16_t ou16_Index,
                                     uint16_t *const opu16_List,
...
C_KFXVariableBase *
C_KFXVariableLists::GetVariable(const uint16_t ou16_List,
                                const uint16_t ou16_Variable) {
...
const C_KFXVariableBase *
C_KFXVariableLists::GetVariable(const uint16_t ou16_Index) const {
  uint16_t u16_List;
  uint16_t u16_Variable;
...
const C_KFXVariableListBase *
C_KFXVariableLists::GetListByName(const QString &orc_ListName) const {
...
C_KFXVariableListBase *
C_KFXVariableLists::GetListByName(const QString &orc_ListName) {
  int32_t s32_ListIndex;
