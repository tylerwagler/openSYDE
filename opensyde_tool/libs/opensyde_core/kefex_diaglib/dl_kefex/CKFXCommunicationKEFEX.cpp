...
C_KFXCommunicationKEFEX::C_KFXCommunicationKEFEX(
  ...
  mpc_Protocol->InstallECUResetHandler(
      &C_KFXCommunicationKEFEX::m_StaticResetHandler, this);
...
void C_KFXCommunicationKEFEX::Cycle(void) {
  ...
  // all incoming cyclic transmissions will be reported in callback in the
  // background: only do this if there is no other communication going on:
  for (u8_Index = 0U; u8_Index < KFX_NUM_LOCKS; u8_Index++) {
...
/*!
   ...
   \param[in]  ou8_ProjectIndex        index of project to log in to (0 for most
...
*/
//-----------------------------------------------------------------------------
int32_t C_KFXCommunicationKEFEX::Logon(const uint64_t ou64_ProjectChecksum,
                                       const uint16_t ou16_DataVersion,
                                       const uint16_t ou16_NumOfVars,
  ...
  this->Cycle(); // clear all orphaned responses (will also trigger event
                 // handlers for incoming cyclic transmissions)

  s32_Return = mpc_Protocol->SendLogonRequest(
      static_cast<uint16_t>(ou64_ProjectChecksum), ou8_ProjectIndex,
  ...
  // Get Response
  s32_Return =
      mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_LOGON_HS, c_Service);
  if (s32_Return != C_NO_ERR) {
    return C_COM;
  }
...
/*!
   ...
   \param[in]  oq_WaitForHandshake     true  -> will acticely wait for error or
...
int32_t C_KFXCommunicationKEFEX::Logoff(const bool oq_WaitForHandshake) {
  ...
  this->Cycle(); // clear all potentially orphaned responses
                 //(will also trigger event handlers for incoming cyclic
                 //transmissions)
  ...
  if (oq_WaitForHandshake == true) {
    // Get Response
    s32_Return =
        mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_LOGOFF_HS, c_Service);
    if (s32_Return != C_NO_ERR) {
      return C_COM;
    }
    if (c_Service.q_Error == true) {
...
int32_t C_KFXCommunicationKEFEX::ReadService(const uint32_t ou32_Index,
  ...
  Q_ASSERT(
      ou32_Index <=
      0xFFFFU); // this protocol implementation can only handle a 16bit index

  this->Cycle(); // clear all orphaned responses (will also trigger event
                 // handlers for incoming cyclic transmissions)

  s32_Return =
      mpc_Protocol->SendServiceReadRequest(static_cast<uint16_t>(ou32_Index));
  if (s32_Return != C_NO_ERR) {
    return C_RD_WR;
  }

  s32_Return =
      mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_SERVICE_READ, c_Service);
  if (s32_Return != C_NO_ERR) {
    return C_NOACT;
  }
...
//---------------------------------------------------------------------------
// note: which index contains which information is protocol dependent !!!
int32_t C_KFXCommunicationKEFEX::WriteService(const uint32_t ou32_Index,
  ...
  s32_Return = mpc_Protocol->SendServiceWriteRequest(
      static_cast<uint16_t>(ou32_Index), ou32_Value);
  if (s32_Return != C_NO_ERR) {
    return C_RD_WR;
  }

  s32_Return =
      mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_SERVICE_WRITE, c_Service);
  if (s32_Return != C_NO_ERR) {
    return C_NOACT;
  }
  if (ou32_Index != c_Service.u32_IndexOrAddress) {
...
int32_t C_KFXCommunicationKEFEX::ReadEEPROM(const uint32_t ou32_Address,
  ...
  while (u32_BytesLeft > 0U) {
    if (u32_BytesLeft >= 2U) {
      ...
      s32_Return = mpc_Protocol->SendReadDataWordRequest(
          KFX_PROTOCOL_READ_EEPROM, ou32_Address + u32_Offset);
    } else {
      s32_Return = mpc_Protocol->SendReadDataByteRequest(
          KFX_PROTOCOL_READ_EEPROM, ou32_Address + u32_Offset);
    }
    if (s32_Return != C_NO_ERR) {
    ...
    s32_Return =
        mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_READ_EEPROM, c_Service);
    if (s32_Return != C_NO_ERR) {
      m_Unlock(KFX_LOCK_ReadEEP);
      return C_COM;
    }
...
int32_t C_KFXCommunicationKEFEX::WriteEEPROM(const uint32_t ou32_Address,
                                             const uint32_t ou32_NumBytes,
  ...
  while (u32_BytesLeft > 0U) {
    ...
    if (u32_BytesLeft >= 2U) {
      ...
      C_SclChecksums::CalcCRC16(&opu8_Data[u32_Offset], 2U,
                                mu16_CRCOverSentServices);
      s32_Return = mpc_Protocol->SendWriteDataWordRequest(
          KFX_PROTOCOL_WRITE_EEPROM_HS, ou32_Address + u32_Offset,
          &opu8_Data[u32_Offset]);
    } else {
      C_SclChecksums::CalcCRC16(&opu8_Data[u32_Offset], 1U,
                                mu16_CRCOverSentServices);
      s32_Return = mpc_Protocol->SendWriteDataByteRequest(
          KFX_PROTOCOL_WRITE_EEPROM_HS, ou32_Address + u32_Offset,
          opu8_Data[u32_Offset]);
    }
    if (s32_Return != C_NO_ERR) {
    ...
    s32_Return = mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_WRITE_EEPROM_HS,
                                                 c_Service);
    if (s32_Return != C_NO_ERR) {
      m_Unlock(KFX_LOCK_WriteEEP);
      return C_COM;
...
*/
//-----------------------------------------------------------------------------
int32_t C_KFXCommunicationKEFEX::WriteEEPROMSSLStart(
    const uint32_t ou32_DesiredWriteSize) {
  int32_t s32_Return;
  ...
  u32_NumServices =
      (ou32_DesiredWriteSize + 1U) / 2U; // 2 bytes fit in one service
  ...
  s32_Return = mpc_Protocol->SendWriteEepromSslStartRequest(
      static_cast<uint16_t>(u32_NumServices));
  if (s32_Return != C_NO_ERR) {
    return C_RD_WR;
  }

  s32_Return =
      mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_WRITE_EEPROM_SSL, c_Service);
  if (s32_Return != C_NO_ERR) {
    return C_COM;
  }
...
/*!
   ...
   over the sent data. Then wait for response and report result.

   The following data is used:
   - the data size set with WriteEEPROMSSLStart()
   - the CRC calculated in WriteEEPROM() since the last call of
   WriteEEPROMSSLStart()
...
int32_t C_KFXCommunicationKEFEX::WriteEEPROMSSLEnd(void) {
  ...
  s32_Return = mpc_Protocol->SendWriteEepromSslEndRequest(
      mu16_NumServicesToSend, mu16_CRCOverSentServices);
  if (s32_Return != C_NO_ERR) {
    return C_RD_WR;
  }
  ...
  if ((c_Service.au8_Data[0] != 2U) || //"END" service ?
      (c_Service.u32_IndexOrAddress !=
       static_cast<uint32_t>(mu16_NumServicesToSend))) {
    mu16_LastErrorCode = c_Service.GetU16FromDataIntel(1U);
    return C_WARN;
  }
...
int32_t C_KFXCommunicationKEFEX::ReadNumericVariable(const uint32_t ou32_Index,
                                                     const uint8_t ou8_NumBytes,
                                                     int64_t &ors64_Value) {
  int32_t s32_Return;
  C_KFXProcotolResponse c_Service;
  ...
  if (ou8_NumBytes > 4U) {
    return this->ReadAggregateVariable(
        ou32_Index, ou8_NumBytes, reinterpret_cast<uint8_t *>(&ors64_Value));
  }
...
int32_t
C_KFXCommunicationKEFEX::WriteNumericVariable(const uint32_t ou32_Index,
                                              const uint8_t ou8_NumBytes,
  ...
  // in KEFEX protocol we can only transfer variables <= 4 bytes expedited !
  if (ou8_NumBytes > 4U) {
    return this->WriteAggregateVariable(
        ou32_Index, ou8_NumBytes,
        reinterpret_cast<const uint8_t *>(&os64_Value));
  }

  m_Lock(KFX_LOCK_IWR);
  s32_Return = mpc_Protocol->SendImmediateWriteRequest(
      static_cast<uint16_t>(ou32_Index), static_cast<uint32_t>(os64_Value));
  if (s32_Return != C_NO_ERR) {
    m_Unlock(KFX_LOCK_IWR);
    return C_RD_WR;
  }

  s32_Return = mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_IMMEDIATE_WRITE_HS,
                                               c_Service);
  m_Unlock(KFX_LOCK_IWR);
  if (s32_Return != C_NO_ERR) {
    return C_COM;
...
int32_t
C_KFXCommunicationKEFEX::ReadAggregateVariable(const uint32_t ou32_Index,
                                               const uint32_t ou32_NumBytes,
                                               uint8_t *const opu8_Data) {
  int32_t s32_Return;
  ...
  s32_Return = mpc_Protocol->SegmentedSRRTransfer(
      static_cast<uint16_t>(ou32_Index), ou32_NumBytes, opu8_Data,
      &mu16_LastErrorCode);
...
int32_t C_KFXCommunicationKEFEX::WriteAggregateVariable(
    const uint32_t ou32_Index, const uint32_t ou32_NumBytes,
    const uint8_t *const opu8_Data) {
  int32_t s32_Return;
  ...
  s32_Return = mpc_Protocol->SegmentedIWRTransfer(
      static_cast<uint16_t>(ou32_Index), ou32_NumBytes, opu8_Data);
...
int32_t
C_KFXCommunicationKEFEX::SendChangeNotification(const uint32_t ou32_Index) {
  int32_t s32_Return;
  C_KFXProcotolResponse c_Service;
  ...
  s32_Return = mpc_Protocol->SendUpdateTaskRequest(
      static_cast<uint16_t>(ou32_Index), true);
  if (s32_Return != C_NO_ERR) {
    m_Unlock(KFX_LOCK_UpdateTask);
    return C_RD_WR;
  }
  s32_Return =
      mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_UPDATE_TASK_HS, c_Service);
  m_Unlock(KFX_LOCK_UpdateTask);
  if (s32_Return != C_NO_ERR) {
    return C_COM;
  }
...
int32_t C_KFXCommunicationKEFEX::RequestTimeTriggeredTransmission(
    const uint32_t ou32_Index, const bool oq_TimeStamped,
  ...
  Q_ASSERT(
      ou32_Index <=
      0xFFFFU); // this protocol implementation can only handle a 16bit index
  s32_Return = mpc_Protocol->SendTCRRRequest(
      static_cast<uint16_t>(ou32_Index), oq_TimeStamped,
...
int32_t C_KFXCommunicationKEFEX::RequestChangeTriggeredTransmission(
    const uint32_t ou32_Index, const uint32_t ou32_MaxTimeout,
  ...
  Q_ASSERT(
      ou32_Index <=
      0xFFFFU); // this protocol implementation can only handle a 16bit index
  s32_Return = mpc_Protocol->SendECRRRequestAbsolute(
      static_cast<uint16_t>(ou32_Index), static_cast<uint16_t>(ou32_MaxTimeout),
      ou32_UpperHysteresis, ou32_LowerHysteresis);
  if (s32_Return == C_WARN) // just a warning that the requested hysteresis
                            // values can not be exactly matched
  {
    s32_Return = C_NO_ERR;
  }
  return s32_Return;
...
int32_t C_KFXCommunicationKEFEX::TerminateCyclicTransmission(
  ...
  m_Lock(KFX_LOCK_AIR);
  s32_Return = mpc_Protocol->SendAbortIndividualResponseRequest(
      static_cast<uint16_t>(ou32_Index), true);
  if (s32_Return != C_NO_ERR) {
  ...
  s32_Return = mpc_Protocol->EvaluateResponses(
      KFX_PROTOCOL_ABORT_INDIVIDUAL_RESPONSE_HS, c_Service);
  m_Unlock(KFX_LOCK_AIR);
  if (s32_Return != C_NO_ERR) {
    return C_COM;
  }
  if ((c_Service.q_Error == true) ||
      (c_Service.u32_IndexOrAddress != ou32_Index)) {
    mu16_LastErrorCode = 0U;
    return C_WARN;
  }
  return C_NO_ERR;
...
int32_t C_KFXCommunicationKEFEX::TerminateAllCyclicTransmissions(
    const bool oq_WaitForHandshake) {
  int32_t s32_Return;
  C_KFXProcotolResponse c_Service;
  ...
  if (oq_WaitForHandshake == true) {
    s32_Return = mpc_Protocol->EvaluateResponses(
        KFX_PROTOCOL_ABORT_ALL_RESPONSES_HS, c_Service);
    m_Unlock(KFX_LOCK_AAR);
    if (s32_Return != C_NO_ERR) {
      return C_COM;
    }
...
int32_t C_KFXCommunicationKEFEX::LoadConfigFromINI(QSettings *const opc_File,
...
int32_t C_KFXCommunicationKEFEX::SetConfig(
    const C_KFXCommConfiguration *const opt_Config) {
  int32_t s32_Return;
...
void C_KFXCommunicationKEFEX::SetCommDispatcher(
...
int32_t C_KFXCommunicationKEFEX::WaitForStartupIndication(
    const uint32_t ou32_MaxWaitTimeMS) {
  int32_t s32_Return;
  ...
  s32_Return = mpc_Protocol->EvaluateResponses(KFX_PROTOCOL_STARTUP_INDICATION,
                                               c_Service);
  if (s32_Return == C_NO_ERR) {
...
void C_KFXCommunicationKEFEX::SetKEFEXVariableInfo(
...
void C_KFXCommunicationKEFEX::InstallCyclicTransmissionCallback(
    const PR_KFXCommCyclicTransmissionCallBack opr_Callback,
    void *const opv_Instance) {
  C_KFXCommunicationBase::InstallCyclicTransmissionCallback(opr_Callback,
                                                            opv_Instance);
