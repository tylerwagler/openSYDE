...
/*!
   ...
   \copyright   Copyright 2000 Sensor-Technik Wiedemann GmbH. All rights
   reserved.
*/
//----------------------------------------------------------------------------------------------------------------------
#ifndef CHEXFILEHPP
...
/// Element for one line of a hex file (will be created in a dynamic ring
/// buffer)
struct T_HexLine {
  ...
  uint8_t
      *pu8_HexLine; ///< binary data contained in hex line (without leading ":")
};
...
private:
  C_HexFile(
      const C_HexFile &orc_Source); ///< not implemented -> prevent copying
  C_HexFile &operator=(
      const C_HexFile &orc_Source); ///< not implemented -> prevent assignment
...
public:
  ...
  // Reformat hex file (uses a memory dump internally); only use if you know
  // there are no bigger gaps in the hex file
  //                    data)
  uint32_t OptimizeLinear(const uint32_t ou32_RecSize,
                          const int32_t os32_FillFlag,
                          const uint8_t ou8_FillPattern);
  // Reformat hex files (using C_HexDataDump internally; so no RAM penalty for
  // gaps within hex file)
  ...
  // Get pointer to data of current hex line as string and advance data pointer
  // to next line. After last line sets data pointer to NULL
  //  Error: NULL pointer
  ...
  uint32_t CreateHexFile(const uint16_t *const opu16_BinImage,
                         const uint32_t ou32_Offset, const uint32_t ou32_Size,
                         const uint32_t ou32_RecSize);
  // create hex file data from image that only contains used data
  uint32_t CreateHexFile(const C_HexDataDump &orc_Dump,
                         const uint32_t ou32_RecSize);

  const C_HexDataDump *GetDataDump(uint32_t &oru32_ErrorResult);

  int32_t GetDataByAddress(const uint32_t ou32_Address,
                           uint16_t &oru16_NumBytes, uint8_t *const opu8_Data);
...
protected:
  ...
  uint8_t mu8_MaxRecordLength; // Maximum detected record length when parsing
                               // hex file; set by LoadFromFile
  const T_HexLine
      *mpt_DataEntry; // Start of dynamic ring buffer containing all hex lines
  ...
  bool mq_DumpIsDirty;   // flags whether dump has been compromised by changing
                       // hex data contents since it was
                       //  last built
  uint32_t mu32_LastOverlayErrorAddress;

  void m_InitHexFile(void);
  uint32_t m_GetFileType(std::FILE *const opt_File,
  ...
  uint32_t m_GetSRecordType(const char_t *const opcn_String,
                            uint8_t &oru8_RecordType) const;
  uint32_t m_GetIntelAddress(const char_t *const opcn_String,
  ...
  static uint32_t mh_GetByte(const char_t *const opcn_String,
  ...
  static uint32_t mh_GetWord(const char_t *const opcn_String,
                             const uint32_t ou32_Index, uint32_t &oru32_Word);
