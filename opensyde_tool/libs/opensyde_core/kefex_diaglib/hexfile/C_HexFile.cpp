...
//                          all records are sorted by address, Intel16 offset
//                          commands are converted into Intel32 offset commands
...
static const uint32_t mu32_HEXBUFFER_SIZE =
    530U; // buffer size for hex line data
static const int32_t ms32_INTELHEX_MAX =
    (522 + 2); // max size of intel-hex line + \r\n
...
C_HexFile::C_HexFile(void) : mu32_LastOverlayErrorAddress(0U) {
  m_InitHexFile();
}

//------------------------------------------------------------------------

C_HexFile::~C_HexFile(void) { this->Clear(); }

//------------------------------------------------------------------------

uint32_t C_HexFile::ByteCount(void) const { return mu32_NumRawBytes; }
...
uint32_t C_HexFile::m_LoadSRecord(std::FILE *const opt_File) {
  ...
  while ((fgets(acn_HexBuffer, ms32_SRECORD_MAX, opt_File)) != NULL) {
    ...
    if (u32_Error == NO_ERR) {
      ...
      if ((u8_RecordType == mu8_SREC_DATA16) ||
          (u8_RecordType == mu8_SREC_DATA24) ||
          (u8_RecordType == mu8_SREC_DATA32)) {
        // get record address
        u32_Error =
            m_GetSRecordAddress(acn_HexBuffer, u8_RecordType, u32_AdrNew);

        if (mu32_AdrOffs !=
            (u32_AdrNew & mu32_XADR32_MASK)) // 32bit address offset changed?
        {
          mu32_AdrOffs =
              (u32_AdrNew & mu32_XADR32_MASK); // yes -> set new address offset
        ...
        if (u32_Error == NO_ERR) // no error?
        {
          ...
          u8_Length -= static_cast<uint8_t>(u8_RecordType +
                                            2U); // calc. intel-hex length
          acn_IntelBuffer[0] = ':';

          mu32_NumRawBytes += u8_Length;

          mh_SetByte(acn_IntelBuffer, mu8_INTEL_LEN, u8_Length);
          mh_SetWord(acn_IntelBuffer, mu8_INTEL_ADRH,
                     (u32_AdrNew & ~mu32_XADR32_MASK));
          mh_SetByte(acn_IntelBuffer, mu8_INTEL_CMD, mu8_CMD_DATA);
          std::memmove(&acn_IntelBuffer[(mu8_INTEL_DAT * 2U) + 1U],
                       &acn_HexBuffer[(u8_RecordType * 2U) + 6U],
...
//              xxxxxxx = error line number of input file
//************************************************************************
uint32_t C_HexFile::m_GetFileType(std::FILE *const opt_File,
                                  int32_t &ors32_FileType) const {
  uint32_t u32_Error;
  ...
  pcn_Ret = fgets(acn_HexBuffer, ms32_INTELHEX_MAX,
                  opt_File); // read first line of file
...
//  - Note: This approach requires to create a memory dump of all the hex file
//  data
//           including all gaps. So if using this function on hex files with big
//           gap (e.g. caused by virtual address offsets) this function can
...
// .RETURNVALUE
//              NO_ERR                  0x00000000
//              WRN_RECORD_OVERLAY      0x2xxxxxxx (xxxxxxx = lowest 7 nibbles
//              of the offending address) ERR_NOT_ENOUGH_MEMORY   0xE0000000
//              ERR_NO_DATA             0xB0000000
//************************************************************************
uint32_t C_HexFile::OptimizeLinear(const uint32_t ou32_RecSize,
                                   const int32_t os32_FillFlag,
  ...
  if (LineInit() != NULL) // HEX Data loaded?
  {
    ...
    {
      (void)std::memset(pu16_BinImage, 0,
                        u32_Size * sizeof(uint16_t)); // clear memory
      // store memory image
      uint32_t u32_Warning = m_CopyHex2Mem(pu16_BinImage, u32_Offset);

      if (os32_FillFlag != 0) // fill gaps?
      {
        for (uint32_t u32_ByteIndex = 0U; u32_ByteIndex < u32_Size;
             u32_ByteIndex++) // yes, fill unused data
        {
          if ((pu16_BinImage[u32_ByteIndex] & 0xFF00U) == 0U) {
            pu16_BinImage[u32_ByteIndex] = static_cast<uint16_t>(
                static_cast<uint16_t>(0x0100U) + ou8_FillPattern);
          }
        }
      }

      u32_Error = CreateHexFile(pu16_BinImage, u32_Offset, u32_Size,
                                ou32_RecSize); // create new HEX file from image
...
/*!
   ...
   Note: does not create a linear memory dump including gaps. Instead
   GetDataDump() is used to create an image only containing the used areas.
   ...
   \return
   NO_ERR                 -> no problems
   WRN_RECORD_OVERLAY     -> address used twice (lowest 7 nibbles contain lowest
   7 nibbles of the offending address) ERR_NOT_ENOUGH_MEMORY  -> PC out of
   memory ERR_NO_DATA            -> no hex file data available
*/
//-----------------------------------------------------------------------------
uint32_t C_HexFile::Optimize(const uint32_t ou32_RecSize) {
  ...
  if (this->LineInit() != NULL) // HEX Data loaded?
  {
    ...
    if (u32_Error == NO_ERR) {
      u32_Error = this->CreateHexFile(
          *pc_Dump, ou32_RecSize); // create new HEX file from image
    }
  }
  ...
  if (LineInit() != NULL) // HEX Data loaded?
  {
    ...
    if (pc_File == NULL) {
      ...
      while ((pu8_HexLine = NextLine()) != NULL) {
        ...
        // we always create CR/LF to avoid different hex-file data on different
        // OS
        fputs("\r\n", pc_File);
      }
...
//************************************************************************
const uint8_t *C_HexFile::NextBinData(uint32_t &oru32_Address,
                                      uint8_t &oru8_Size) {
...
//************************************************************************
uint32_t C_HexFile::m_CopyHex2Mem(uint16_t *const opu16_BinImage,
  ...
  if (LineInit() != NULL) // HEX Data loaded?
  {
    while ((pu8_HexLine = NextLine()) != NULL) {
      ...
      {
        u32_AdrOffset =
            (static_cast<uint32_t>(pu8_HexLine[mu8_INTEL_DAT]) << 24U);
        u32_AdrOffset |=
            (static_cast<uint32_t>(pu8_HexLine[mu8_INTEL_DAT + 1]) << 16U);
      } else if (u8_Command == mu8_CMD_DATA) // data record?
      {
        uint32_t u32_NumBytes = pu8_HexLine[mu8_INTEL_LEN];
        uint32_t u32_Adr =
            (static_cast<uint32_t>(pu8_HexLine[mu8_INTEL_ADRH]) << 8U);
        u32_Adr |= pu8_HexLine[mu8_INTEL_ADRL];
        u32_Adr = (u32_Adr + u32_AdrOffset) - ou32_Offset;

        for (uint32_t u32_ByteIndex = 0; u32_ByteIndex < u32_NumBytes;
             u32_ByteIndex++) // copy data into memory
        {
          // check for data overlay
          if (((opu16_BinImage[u32_Adr + u32_ByteIndex] & 0xFF00U) != 0) &&
              (u32_Error == NO_ERR)) {
            // -> set warning
            u32_Error = (WRN_RECORD_OVERLAY |
                         ((u32_Adr + ou32_Offset + u32_ByteIndex) & ~ERR_MASK));
            mu32_LastOverlayErrorAddress =
                u32_Adr + ou32_Offset + u32_ByteIndex;
          }
          opu16_BinImage[u32_Adr + u32_ByteIndex] =
              static_cast<uint16_t>(static_cast<uint16_t>(0x0100U) +
...
//************************************************************************
uint32_t C_HexFile::CreateHexFile(const uint16_t *const opu16_BinImage,
                                  const uint32_t ou32_Offset,
  ...
  if (u32_Error == NO_ERR) {
    for (uint32_t u32_ByteIndex = 0; u32_ByteIndex < ou32_Size;
         u32_ByteIndex++) {
      while ((opu16_BinImage[u32_ByteIndex] & 0xFF00U) !=
             0U) // do while data valid...
      {
        ...
        if (u32_Error == NO_ERR) {
          if (((u32_Adr - mu32_AdrOffs) % u32_RecSize) ==
              0U) // record size boundary?
          {
            u32_Error = m_CloseRecord(acn_Record, q_RecordOpen);
          }
        }
        ...
        // store data
        mh_SetByte(acn_Record,
                   static_cast<uint32_t>(mu8_INTEL_DAT + s32_Length),
                   static_cast<uint8_t>(opu16_BinImage[u32_ByteIndex]));
        s32_Length++; // inc. data length
        mh_SetByte(acn_Record, mu8_INTEL_LEN,
                   static_cast<uint8_t>(s32_Length)); // set data length
      ...
      if ((u32_ByteIndex < ou32_Size) &&
          ((opu16_BinImage[u32_ByteIndex] & 0xFF00U) == 0)) {
...
*/
//-----------------------------------------------------------------------------
uint32_t C_HexFile::CreateHexFile(const C_HexDataDump &orc_Dump,
                                  const uint32_t ou32_RecSize) {
  uint32_t u32_Error;
    ...
    for (s32_Block = 0U; s32_Block < orc_Dump.at_Blocks.size(); s32_Block++) {
      ...
      for (u32_OffsetInBlock = 0U;
        ...
        if (q_RecordOpen == false) // no record open?
        {
          // -> open data record...
          s32_Length = 0;
          mh_SetWord(acn_Record, mu8_INTEL_ADRH,
                     (u32_AbsoluteAddress & ~mu32_XADR32_MASK));
          mh_SetByte(acn_Record, mu8_INTEL_CMD, mu8_CMD_DATA);
          q_RecordOpen = true;
        }
...
//              xxxxxxx = line number of optimized file
//************************************************************************
uint32_t C_HexFile::m_CloseRecord(char_t *const opcn_Record,
                                  bool &orq_RecordOpen) {
  uint32_t u32_Error = NO_ERR;
...
//************************************************************************
const char_t *
C_HexFile::mh_HexLineString(const uint8_t *const opu8_HexLine) // const
{
  ...
  for (uint32_t u32_ByteIndex = 0U; u32_ByteIndex < u32_Length;
       u32_ByteIndex++) // store ASCII HEX data
  {
...
//************************************************************************
uint32_t C_HexFile::m_GetIHexCommand(const char_t *const opcn_String,
  ...
  if (opcn_String[0] == ':') {
    if (mh_GetByte(opcn_String, mu8_INTEL_LEN, u8_Data) ==
        NO_ERR) // read record data length
    {
      u32_Length = static_cast<uint32_t>(u8_Data) + mu8_INTEL_DAT +
                   1; // calculate complete record length

      for (u32_Loop = 0U; u32_Loop < u32_Length;
           u32_Loop++) // calculate checksum...
      {
...
//              xxxxxxx = line number of input file
//************************************************************************
uint32_t C_HexFile::m_GetSRecordType(const char_t *const opcn_String,
                                     uint8_t &oru8_RecordType) const {
  uint32_t u32_Error = ERR_HEXLINE_SYNTAX;
  uint8_t u8_Data;
  uint8_t u8_Checksum = 0U;

  if (opcn_String[0] == 'S') {
    if (mh_GetByte(opcn_String + 1, mu8_SREC_LEN, u8_Data) ==
        NO_ERR) // read record data length
    {
      int32_t s32_Length =
          static_cast<int32_t>(u8_Data) + 1; // calculate complete record length

      for (int32_t u32_ByteIndex = 0; u32_ByteIndex < s32_Length;
           u32_ByteIndex++) // calculate checksum...
      {
    ...
    if (u32_Error == 0U) {
      oru8_RecordType =
          static_cast<uint8_t>(opcn_String[1]) - 0x30U; // read record type
...
//              xxxxxxx = line number of input file
//************************************************************************
uint32_t C_HexFile::m_GetIntelAddress(const char_t *const opcn_String,
                                      const uint8_t ou8_Command,
...
//              xxxxxxx = line number of input file
//************************************************************************
uint32_t C_HexFile::m_GetSRecordAddress(const char_t *const opcn_String,
                                        const uint8_t ou8_RecordType,
                                        uint32_t &oru32_Adr) const {
  uint32_t u32_Error;
  uint32_t u32_Address;
  uint32_t u32_LSW;
...
uint32_t C_HexFile::m_SetXAdrPtr(const uint32_t ou32_Adr) {
  ...
  if (ou32_Adr != 0U) // address offset > 0x00000000 ?
  {
    ...
    if (pt_Next->pu8_HexLine[mu8_INTEL_CMD] == mu8_CMD_XADR32) {
      u32_Offs =
          (static_cast<uint32_t>(pt_Next->pu8_HexLine[mu8_INTEL_DAT]) << 24U) +
          (static_cast<uint32_t>(pt_Next->pu8_HexLine[mu8_INTEL_DAT + 1U])
           << 16U);
...
//              xxxxxxx = line number of input file
//************************************************************************
uint32_t C_HexFile::m_CopyData(const char_t *const opcn_String,
                               T_HexLine *const opt_HexLine,
  ...
  for (uint32_t u32_ByteIndex = 0U; u32_ByteIndex < ou32_Length;
       u32_ByteIndex++) // copy HEX data into buffer
  {
  ...
  if (u32_Error == 0U) // no error in HEX data ?
  {
    ...
    if (opt_HexLine->pu8_HexLine[mu8_INTEL_CMD] == mu8_CMD_XADR32) {
      opt_HexLine->u32_XAdr =
          ((static_cast<uint32_t>(opt_HexLine->pu8_HexLine[mu8_INTEL_DAT])
            << 24U) +
           (static_cast<uint32_t>(opt_HexLine->pu8_HexLine[mu8_INTEL_DAT + 1])
            << 16U));
    }
    ...
    if (opt_HexLine->pu8_HexLine[mu8_INTEL_CMD] == mu8_CMD_DATA) {
      ...
      u32_StartAdr =
          ((static_cast<uint32_t>(opt_HexLine->pu8_HexLine[mu8_INTEL_ADRH])
            << 8U) +
           opt_HexLine->pu8_HexLine[mu8_INTEL_ADRL]) +
          mu32_AdrOffs;
    ...
    if (u32_Error == NO_ERR) {
      ...
      if (u32_Error == NO_ERR) {
        if ((u32_EndAdr < 0x00010000U) ||
          ...
          mu32_AdrOffs = (mu32_AdrOffs + u32_Adr) &
                         mu32_XADR32_MASK;       // create 32bit Offset
          u32_Error = m_AddHexLine(opcn_String); // add 32bit HEX record
          mu32_AdrOffs = u32_SaveOffs;           // restore address offset
        } else {
          u8_Len1 = static_cast<uint8_t>(0x00010000U -
                                         (u32_StartAdr & ~mu32_XADR32_MASK));
          u8_Len -= u8_Len1;
          ...
          for (uint8_t u32_ByteIndex = 0U; u32_ByteIndex < u8_Len1;
               u32_ByteIndex++) {
            strncpy(&acn_HexLine[(u32_ByteIndex * 2U) + 9U],
                    &opcn_String[(u32_ByteIndex * 2U) + 9U], 2U);
          }
          ...
          if (u32_Error == NO_ERR) {
            // write 32bit offset command
            u32_StartAdr =
                (((mu32_AdrOffs + u32_Adr) & mu32_XADR32_MASK) + 0x10000U);
            u32_Error = m_SetOffset(u32_StartAdr);
            if (u32_Error == NO_ERR) {
              // create 2nd part of 32bit record:
              mh_SetByte(acn_HexLine, mu8_INTEL_LEN, u8_Len);
              mh_SetWord(acn_HexLine, mu8_INTEL_ADRH, u32_StartAdr);
              for (uint32_t i = u8_Len1;
                   i < (static_cast<uint32_t>(u8_Len) + u8_Len1); i++) {
                strncpy(&acn_HexLine[((i - u8_Len1) * 2U) + 9U],
                        &opcn_String[(i * 2U) + 9U], 2U);
              }
              mh_CalcCheck(acn_HexLine);
...
//              ERR_HEXLINE_SYNTAX      0x80000000
//************************************************************************
uint32_t C_HexFile::mh_GetByte(const char_t *const opcn_String,
                               const uint32_t ou32_Index, uint8_t &oru8_Data) {
  uint32_t u32_Error = ERR_HEXLINE_SYNTAX;
  ...
  if ((u8_HiNib >= static_cast<uint8_t>('0')) &&
      (u8_HiNib <= static_cast<uint8_t>('9'))) {
    u8_HiNib =
        static_cast<uint8_t>((static_cast<uint8_t>(u8_HiNib - 0x30U)) << 4);
  } else {
    u8_HiNib |= 0x20U; // A..F to lower case a..f
    if ((u8_HiNib >= 0x61U) && (u8_HiNib <= 0x66U)) {
      u8_HiNib =
          static_cast<uint8_t>((static_cast<uint8_t>(u8_HiNib - 0x57U)) << 4);
    } else {
      return (u32_Error);
    }
  }

  if ((u8_LoNib >= static_cast<uint8_t>('0')) &&
      (u8_LoNib <= static_cast<uint8_t>('9'))) {
...
// .RETURNVALUE void
//************************************************************************
void C_HexFile::mh_SetByte(char_t *const opcn_String, const uint32_t ou32_Index,
  ...
  au8_HexByte[0] =
      static_cast<uint8_t>(au8_HexByte[0] + 0x30U); // create HEX ASCII char...
  ...
  au8_HexByte[1] =
      static_cast<uint8_t>(au8_HexByte[1] + 0x30U); // create HEX ASCII char...
...
//              ERR_HEXLINE_SYNTAX      0x80000000
//************************************************************************
uint32_t C_HexFile::mh_GetWord(const char_t *const opcn_String,
...
// .RETURNVALUE void
//************************************************************************
void C_HexFile::mh_SetWord(char_t *const opcn_String, const uint32_t ou32_Index,
...
uint32_t C_HexFile::LineCount(void) const { return (mu32_LineCountNew); }

//------------------------------------------------------------------------

uint32_t C_HexFile::MinAdr(void) const { return (mu32_MinAdr); }

//------------------------------------------------------------------------

uint32_t C_HexFile::MaxAdr(void) const { return (mu32_MaxAdr); }
...
/*!
   ...
   -2       data read (result in opu8_Data), but not all bytes were available
   before the next memory gap after ou32_Address
*/
//-----------------------------------------------------------------------------
int32_t C_HexFile::GetDataByAddress(const uint32_t ou32_Address,
                                    uint16_t &oru16_NumBytes,
  ...
  for (s32_Block = 0; s32_Block < pc_HexFileData->at_Blocks.size();
       s32_Block++) {
    u32_BlockLength = pc_HexFileData->at_Blocks[s32_Block].au8_Data.size();
    if ((ou32_Address <=
         (pc_HexFileData->at_Blocks[s32_Block].u32_AddressOffset +
          u32_BlockLength)) &&
        (ou32_Address >=
         pc_HexFileData->at_Blocks[s32_Block].u32_AddressOffset)) {
      // it's in this block !
      // it must be completely in this block, as we do not want to read
      // undefined data from any gaps !
      u32_Offset =
          ou32_Address - pc_HexFileData->at_Blocks[s32_Block]
                             .u32_AddressOffset; // offset within the block

      if ((u32_BlockLength - u32_Offset) <
          static_cast<uint32_t>(oru16_NumBytes)) {
      ...
      (void)std::memcpy(opu8_Data,
                        pc_HexFileData->at_Blocks[s32_Block].au8_Data.data() +
                            u32_Offset,
                        u16_NumBytes);
      return s32_Return;
    }
...
/*!
   ...
   Finds a specified data pattern in memory and returns the offset in the
   searched buffer.
...
*/
//-----------------------------------------------------------------------------
int32_t C_HexFile::mh_FindPattern(const uint8_t *const opu8_Buffer,
  ...
  if (ou32_BufSize > 0x7FFFFFFFUL) // as we return an sint32 we can't handle
                                   // block bigger than 2GB
  {
    return -1;
  }
...
*/
//-----------------------------------------------------------------------------
int32_t C_HexFile::FindPattern(uint32_t &oru32_Address,
                               const uint8_t ou8_PatternLength,
                               const uint8_t *const opu8_Pattern) {
  int32_t s32_Return;
  int32_t s32_Block;
  uint32_t u32_Offset;
  ...
  for (s32_Block = 0; s32_Block < pc_HexFileData->at_Blocks.size();
    ...
    if (oru32_Address <
      ...
      s32_Return = mh_FindPattern(
          reinterpret_cast<const uint8_t *>(
              pc_HexFileData->at_Blocks[s32_Block].au8_Data.data() +
              u32_Offset),
          opu8_Pattern, u32_BlockLength - u32_Offset, ou8_PatternLength);
      if (s32_Return >= 0) // found in this block !
      {
        oru32_Address =
            (pc_HexFileData->at_Blocks[s32_Block].u32_AddressOffset +
             u32_Offset + static_cast<uint32_t>(s32_Return));
        return 0;
      }
    }
  }
...
/*!
   ...
   So we use a dynamic structure holding all the individual "blocks" of the
   hex-file. As soon as there is a gap of at least one byte between a byte and
   the next we interpret this as a new block.

   Will also check for addresses in hex file occupied more than once.

   Assumption: hex file data is sorted linear in instance data (done by
   LoadFromFile())

   \param[out]  oru32_ErrorResult   error code:
        NO_ERR                  data dump created
        WRN_RECORD_OVERLAY      address in hex file multiply used (lowest 7
   nibbles contain lowest 7 nibbles of the offending address)
...
const C_HexDataDump *C_HexFile::GetDataDump(uint32_t &oru32_ErrorResult) {
  ...
  // This will improve speed as we do not need to resize the DynamicArray,
  // potentially
  ...
  s64_LastAddress =
      -2; //-2 -> make sure that a block is detected at the first line
  u32_NumBlocks = 0U;
  while (pu8_Data != NULL) {
  ...
  while (pu8_Data != NULL) {
    ...
    if (pu8_Data != NULL) {
      ...
      (void)std::memcpy(&mc_Dump.at_Blocks[s32_BlockIndex].au8_Data[u32_Index],
                        pu8_Data, u8_Size);
...
/*!
   ...
   If this class returns WRN_RECORD_OVERLAY from any function only the lowest 7
   nibbles contain the offending address. Use this function to read the address
   including the most significant nibble.
