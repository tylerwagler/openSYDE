...
C_CanMonProtocolKefex::C_CanMonProtocolKefex(void)
    : C_CanMonProtocolKefexIva(), mu16_KfxBaseID(0x600U) {}

//---------------------------------------------------------------------------

uint16_t C_CanMonProtocolKefex::GetBaseId(void) const { return mu16_KfxBaseID; }
...
*/
//-----------------------------------------------------------------------------
int32_t C_CanMonProtocolKefex::SaveParamsToIni(QSettings &orc_IniFile,
  ...
  try {
    ...
    orc_IniFile.setValue(c_Section + "/PP_KFX_LIST_OFFSET",
...
*/
//-----------------------------------------------------------------------------
int32_t C_CanMonProtocolKefex::LoadParamsFromIni(QSettings &orc_IniFile,
                                                 const QString &orc_Section) {
  QString c_Section = orc_Section;

  this->mu16_KfxBaseID = static_cast<uint16_t>(
      orc_IniFile.value(c_Section + "/PP_KFX_BASE_ID", this->mu16_KfxBaseID)
          .toUInt());
  this->mu16_KfxListOffset = static_cast<uint16_t>(
      orc_IniFile
          .value(c_Section + "/PP_KFX_LIST_OFFSET", this->mu16_KfxListOffset)
          .toUInt());
...
QString C_CanMonProtocolKefex::m_KfxIndexAndErrorToString(
    const char_t *const opcn_Text, const uint16_t ou16_Index,
    const uint16_t ou16_Error, const bool oq_IsKefexVarIndex) const {
  QString c_Help;
  QString c_Help2;

  c_Help = static_cast<QString>(" ") + opcn_Text + "  " +
           m_KfxIndexToString(ou16_Index, oq_IsKefexVarIndex) + "  ERROR ";

  if (mq_Decimal == true) {
    c_Help2 = QString::asprintf("%d", ou16_Error);
  } else {
    c_Help2 = QString::asprintf("%X", ou16_Error);
  }
...
QString C_CanMonProtocolKefex::m_KfxTextAndValueToString(
    const char_t *const opcn_Text, const uint32_t ou32_Value) const {
  QString c_Help;

  if (mq_Decimal == true) {
    c_Help = QString::asprintf("%s %d", opcn_Text, ou32_Value);
  } else {
    c_Help = QString::asprintf("%s %X", opcn_Text, ou32_Value);
  }
  return c_Help;
}
...
//-----------------------------------------------------------------------------
QString
C_CanMonProtocolKefex::MessageToString(const T_STWCAN_Msg_RX &orc_Msg) const {
  QString c_Text;
  QString c_Help;
  QString c_Help2;
  QString c_Help3;
  ...
  if ((orc_Msg.u32_ID >= static_cast<uint32_t>(mu16_KfxBaseID)) &&
    ...
    if (q_Request == true) {
      if (orc_Msg.u8_DLC < 2U) {
        ...
        c_Text = m_GetByteAsStringFormat(u8_Sender) + " -> " +
                 m_GetByteAsStringFormat(u8_Receiver);
        // which service ?
        ...
        case mu8_KFX_PROTOCOL_SERVICE_READ:
          if (orc_Msg.u8_DLC < 4U) {
            ...
            c_Help = m_KfxTextAndValueToString(
                " ServiceRead REQ  INDEX ",
        ...
        case mu8_KFX_PROTOCOL_SERVICE_WRITE:
          if (orc_Msg.u8_DLC < 8U) {
            ...
            c_Help =
                m_KfxTextAndValueToString(
                    " ServiceWrite REQ  INDEX ",
        ...
        case mu8_KFX_PROTOCOL_SRR:
          if (orc_Msg.u8_DLC < 4U) {
            ...
            c_Help = m_KfxTextAndIndexToString(
                " SRR REQ ", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2]));
          }
          c_Text += c_Help;
          break;
        case mu8_KFX_PROTOCOL_SRR_FC:
          if (orc_Msg.u8_DLC < 7U) {
            ...
            if ((orc_Msg.au8_Data[1] & 0x80U) == 0x80U) {
              // error
              c_Help = m_KfxIndexAndErrorToString(
                  "SRR SEG FC REQ", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2]),
        ...
        case mu8_KFX_PROTOCOL_ECRR:
          if (orc_Msg.u8_DLC < 8U) {
            ...
            const uint16_t u16_HystBase =
                static_cast<uint16_t>((u16_Hysteresis >> 14U) & 0x03U);
            const uint16_t u16_UpperHyst =
                static_cast<uint16_t>((u16_Hysteresis >> 7U) & 0x7FU);
            const uint16_t u16_LowerHyst =
                static_cast<uint16_t>(u16_Hysteresis & 0x7FU);
            float64_t f64_Help;
            ...
            c_Help2 += QString::number(f64_Help) + "%";
            ...
            c_Help = " ECRR REQ  " +
                     m_KfxIndexToString(
                         mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2])) +
                     "  MAXTIME " + m_GetValueDecHex(u16_Time) + "ms " +
                     c_Help2;
          }
          c_Text += c_Help;
          break;
        ...
        case mu8_KFX_PROTOCOL_ABORT_INDIVIDUAL_RESPONSE:
          if (orc_Msg.u8_DLC < 4U) {
            ...
            c_Help = m_KfxTextAndIndexToString(
                " AIR REQ  ", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2]));
          }
          c_Text += c_Help;
          break;
        case mu8_KFX_PROTOCOL_ABORT_INDIVIDUAL_RESPONSE_HS:
          if (orc_Msg.u8_DLC < 4U) {
            ...
            c_Help = m_KfxTextAndIndexToString(
                " AIR HS REQ  ", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2]));
          }
          c_Text += c_Help;
          break;
          ...
          if (orc_Msg.u8_DLC < 8U) {
            ...
            if (mq_Decimal == true) {
              c_Help = QString::asprintf(
                  " IMM SEG FF REQ  %s NUM %d PAR %d",
                  m_KfxIndexToString(
                      mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2]))
                      .toUtf8()
                      .constData(),
                  u32_Size, orc_Msg.au8_Data[7]);
            } else {
              c_Help = QString::asprintf(
                  " IMM SEG FF REQ  %s  NUM %X  PAR %X",
                  m_KfxIndexToString(
                      mh_BytesToWordLowHigh(&orc_Msg.au8_Data[2]))
                      .toUtf8()
                      .constData(),
        ...
        case mu8_KFX_PROTOCOL_IWR_CF:
          if (orc_Msg.u8_DLC < 8U) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              // error
              c_Help = " IMM SEG CF REQ  BLK " +
                       m_GetValueDecHex(orc_Msg.au8_Data[2]) + "  ERROR " +
                       m_GetValueDecHex(static_cast<uint32_t>(
                           mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4])));
            } else {
              if (mq_Decimal == true) {
                c_Help = QString::asprintf(
                    " IMM SEG CF REQ  BLK %d  DATA [%d,%d,%d,%d,%d]",
                    orc_Msg.au8_Data[2], orc_Msg.au8_Data[3],
                    orc_Msg.au8_Data[4], orc_Msg.au8_Data[5],
                    orc_Msg.au8_Data[6], orc_Msg.au8_Data[7]);
              } else {
                c_Help =
                    QString::asprintf(" IMM SEG CF REQ  BLK %02X  DATA "
                                      "[%02X,%02X,%02X,%02X,%02X]",
                                      orc_Msg.au8_Data[2], orc_Msg.au8_Data[3],
                                      orc_Msg.au8_Data[4], orc_Msg.au8_Data[5],
                                      orc_Msg.au8_Data[6], orc_Msg.au8_Data[7]);
        ...
        case mu8_KFX_PROTOCOL_READ_EPROM:        // no break
          ...
          if (orc_Msg.u8_DLC < 6U) {
            ...
            u32_Address = orc_Msg.au8_Data[3] +
        ...
        case mu8_KFX_PROTOCOL_WRITE_SRAM_HS:
          ...
          if (orc_Msg.u8_DLC < 6U) {
            ...
            if (orc_Msg.au8_Data[2] == 0U) {
              ...
              c_Help +=
                  m_GetByteAsStringFormat(static_cast<uint8_t>(u16_Value));
            }
          }
          c_Text += c_Help;
          break;
        ...
        case mu8_KFX_PROTOCOL_WRITE_EEPROM_SSL:
          if (orc_Msg.u8_DLC != 8) {
            ...
            if (orc_Msg.au8_Data[2] == 0x1U) // START
            {
              c_Help =
                  m_KfxTextAndValueToString(
                      " WRITE EEPROM SSL REQ  START  NUM_SERVICES ",
                      mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4])) +
                  "  CRC " +
            ...
            } else if (orc_Msg.au8_Data[2] == 0x2U) // END
            {
              c_Help =
                  m_KfxTextAndValueToString(
                      " WRITE EEPROM SSL REQ  END  NUM_SERVICES ",
                      mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4])) +
                  "  CRC " +
                  m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[6]));
            } else {
      ...
      if (orc_Msg.u8_DLC < 2U) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              ...
              c_Help = " " + c_Help3 + "  " +
                       m_KfxIndexToString(
                           mh_BytesToWordLowHigh(&orc_Msg.au8_Data[1]));
              if (mq_Decimal == true) {
                c_Help2 = "  VALUE: " + QString::number(mh_BytesToDwordLowHigh(
                                            &orc_Msg.au8_Data[3]));
              } else {
                c_Help2 = QString::asprintf(
                    "  VALUE: %X",
                    mh_BytesToDwordLowHigh(&orc_Msg.au8_Data[3]));
              }
            }
          }
        ...
        case mu8_KFX_PROTOCOL_SRR_FF:
          if (orc_Msg.u8_DLC < 8U) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              // error
              c_Help = m_KfxIndexAndErrorToString(
                  "SRR SEG FF RES", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[1]),
                  mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
            } else {
              c_Help =
                  " SRR SEG FF RES  " +
        ...
        case mu8_KFX_PROTOCOL_SRR_CF:
          if (orc_Msg.u8_DLC < 8U) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              // error
              c_Help = " SRR SEG CF RES  BLK " +
                       m_GetValueDecHex(orc_Msg.au8_Data[1]) + "  ERROR " +
                       m_GetValueDecHex(static_cast<uint32_t>(
                           mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3])));
            } else {
              if (mq_Decimal == true) {
                c_Help = QString::asprintf(
                    " SRR SEG CF RES  BLK %d  DATA [%d,%d,%d,%d,%d,%d]",
                    orc_Msg.au8_Data[1], orc_Msg.au8_Data[2],
                    orc_Msg.au8_Data[3], orc_Msg.au8_Data[4],
                    orc_Msg.au8_Data[5], orc_Msg.au8_Data[6],
                    orc_Msg.au8_Data[7]);
              } else {
                c_Help =
                    QString::asprintf(" SRR SEG CF RES  BLK %02X  DATA "
        ...
        case mu8_KFX_PROTOCOL_TCRR_TIMESTAMPED:
          if (orc_Msg.u8_DLC < 8U) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              // error
              c_Help = m_KfxIndexAndErrorToString(
                  "TCRR TS RES", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[1]),
                  mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
              c_Help2 = "";
            } else {
              c_Help = m_KfxTextAndIndexToString(
        ...
        case mu8_KFX_PROTOCOL_ABORT_INDIVIDUAL_RESPONSE_HS:
          if (orc_Msg.u8_DLC < 3U) {
            ...
            c_Help = m_KfxTextAndValueToString(
                " AIR HS  ", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[1]));
          }
          c_Text += c_Help;
          break;
        case mu8_KFX_PROTOCOL_ABORT_ALL_RESPONSES_HS:
        ...
        case mu8_KFX_PROTOCOL_SERVICE_WRITE:
          ...
          if (orc_Msg.u8_DLC < 5) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              ...
              c_Help = m_KfxTextAndValueToString(
                  (c_Help3 + "  INDEX").toUtf8().constData(),
                  mh_BytesToWordLowHigh(&orc_Msg.au8_Data[1]));
              c_Help += " OK";
            }
          }
        ...
        case mu8_KFX_PROTOCOL_IWR_FC:
          if (orc_Msg.u8_DLC < 5U) {
            ...
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              // error
              c_Help = m_KfxIndexAndErrorToString(
                  "IMM SEG FC RES", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[1]),
                  mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
            } else {
        ...
        case mu8_KFX_PROTOCOL_LOGON_HS:
          if (orc_Msg.u8_DLC < 5U) {
            ...
            c_Text += " LOGON HS RES";
            if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
              c_Text += m_KfxTextAndValueToString(
                  "  ERROR", mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
            }
          }
          break;
        case mu8_KFX_PROTOCOL_LOGOFF_HS:
          ...
          if (orc_Msg.u8_DLC < 8U) {
            ...
            if (orc_Msg.au8_Data[1] == 0x1U) // START
            {
              ...
              if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
                ...
                c_Help = m_KfxTextAndValueToString(
                    " WRITE EEPROM SSL RES  START  NUM_SERVICES ",
                    mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]));
              }
            } else if (orc_Msg.au8_Data[1] == 0x2U) // END
            {
              if ((orc_Msg.au8_Data[0] & 0x80U) == 0x80U) {
                ...
                c_Help = m_KfxTextAndValueToString(
                             " WRITE EEPROM SSL RES  END  NUM_SERVICES ",
                             mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4])) +
                         "  CRC " +
                         m_GetValueDecHex(
                             mh_BytesToWordLowHigh(&orc_Msg.au8_Data[6]));
...
*/
//-----------------------------------------------------------------------------
QString C_CanMonProtocolKefex::GetProtocolName(void) const {
  return "STW KEFEX";
}
