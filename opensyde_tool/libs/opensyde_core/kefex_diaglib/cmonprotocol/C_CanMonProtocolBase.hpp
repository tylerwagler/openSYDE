...
// Provides basic functions to help converting a CAN message structure into
// text. Also provides an abstract interface for protocol-specific conversions.
...
protected:
  ...
  static QString
  mh_SerialNumberToString(const uint8_t *const opu8_SerialNumber);
...
public:
  ...
  */
  //-----------------------------------------------------------------------------
  virtual QString
  MessageToString(const stw::can::T_STWCAN_Msg_RX &orc_Msg) const = 0;
...
/// Adds functionality to link in an opened KEFEX project in order to decode
/// variable indexes from CAN messages.
class C_CanMonProtocolKefexIva : public C_CanMonProtocolBase {
private:
  C_CanMonProtocolKefexIva(
      const C_CanMonProtocolKefexIva &orc_Source); // not implemented -> prevent
  // copying
  C_CanMonProtocolKefexIva &operator=(
      const C_CanMonProtocolKefexIva &orc_Source); // not implemented -> prevent
  // assignment
...
protected:
  ...
  // used by KFX and IVA:
  QString m_KfxIndexToString(const uint16_t ou16_Index,
                             bool oq_IsKefexVarIndex = true) const;
  QString m_KfxTextAndIndexToString(const char_t *const opcn_Text,
                                    const uint16_t ou16_Index) const;

public:
  C_CanMonProtocolKefexIva(void);
#ifdef CMONPROTOCOL_ALLOW_RAMVIEW_PROJECT_MAPPING
  void SetVariableInfo(const stw::diag_lib::C_KFXVariableLists *const opc_Lists,
