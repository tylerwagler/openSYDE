...
/*!
   \file
   \brief       CAN message to text conversion class for the Generic Driver
   protocol

   Converts a CAN message structure to text. The interpretation is based on the
   Generic Driver CAN protocol specification.
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_CanMonProtocolGd::MessageToString(const T_STWCAN_Msg_RX &orc_Msg) const {
  bool q_IsRequest;
  ...
  c_HelpSender = m_GetByteAsStringFormat(u8_SenderSegment) + "/" +
                 m_GetByteAsStringFormat(u8_SenderNode);

  // get receiver "network segment" and "node number"
  u8_ReceiverSegment =
      static_cast<uint8_t>((orc_Msg.au8_Data[0] & 0xF0U) >> 4U);
  u8_ReceiverNode = static_cast<uint8_t>(orc_Msg.au8_Data[0] & 0x0FU);
  c_HelpReceiver = m_GetByteAsStringFormat(u8_ReceiverSegment) + "/" +
                   m_GetByteAsStringFormat(u8_ReceiverNode);

  // get the variables index:
  u16_VariableIndex = static_cast<uint16_t>(
      orc_Msg.au8_Data[1] +
  ...
  case GD_REQUEST_SRR_WO_HS:
    ...
    if (q_IsRequest == true) {
      ...
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", SRR REQ IDX: " + m_GetValueDecHex(u16_VariableIndex);
    } else {
      // response
      u16_Data = static_cast<uint16_t>(
          orc_Msg.au8_Data[5] +
          (static_cast<uint16_t>(orc_Msg.au8_Data[6]) << 8U));
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", SRR RES IDX: " + m_GetValueDecHex(u16_VariableIndex) +
  ...
  case GD_REQUEST_SRR_HS:
    ...
    if (q_IsRequest == true) {
      // request
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", SRR REQ HS IDX: " + m_GetValueDecHex(u16_VariableIndex);
    } else {
      // response
      u16_Data = static_cast<uint16_t>(
          orc_Msg.au8_Data[5] +
          (static_cast<uint16_t>(orc_Msg.au8_Data[6]) << 8U));
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", SRR RES HS IDX: " + m_GetValueDecHex(u16_VariableIndex) +
               ", DATA: " + m_GetValueDecHex(u16_Data);
    }
    break;
  case GD_REQUEST_ECRR_WO_HS:
    ...
    if (q_IsRequest == true) {
      ...
      if (orc_Msg.au8_Data[4] == 0U) {
        ...
        c_HelpTime =
            m_GetValueDecHex(static_cast<uint16_t>(orc_Msg.au8_Data[4]) * 100) +
            "ms";
      }
      // evaluate upper hysteresis
      if (orc_Msg.au8_Data[5] <= 99U) {
        // 0.0% .. 9.9%
        c_HelpHystHigh = m_GetByteAsStringFormat(
                             static_cast<uint8_t>(orc_Msg.au8_Data[5] / 10)) +
                         "." +
                         m_GetByteAsStringFormat(
                             static_cast<uint8_t>(orc_Msg.au8_Data[5] % 10)) +
                         "%";
      } else if ((orc_Msg.au8_Data[5] >= 110U) &&
                 (orc_Msg.au8_Data[5] <= 200U)) {
        // 10% .. 100%
        c_HelpHystHigh = m_GetByteAsStringFormat(
                             static_cast<uint8_t>(orc_Msg.au8_Data[5] - 100)) +
                         "%";
      } else {
      ...
      if (orc_Msg.au8_Data[6] <= 99U) {
        // 0.0% .. 9.9%
        c_HelpHystLow = m_GetByteAsStringFormat(
                            static_cast<uint8_t>(orc_Msg.au8_Data[6] / 10)) +
                        "." +
                        m_GetByteAsStringFormat(
                            static_cast<uint8_t>(orc_Msg.au8_Data[6] % 10)) +
                        "%";
      } else if ((orc_Msg.au8_Data[6] >= 110U) &&
                 (orc_Msg.au8_Data[6] <= 200U)) {
        // 10% .. 100%
        c_HelpHystLow = m_GetByteAsStringFormat(
                            static_cast<uint8_t>(orc_Msg.au8_Data[6] - 100)) +
                        "%";
      } else {
      ...
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ...
               ", WAIT: " + c_HelpTime + ", HYST HIGH: " + c_HelpHystHigh +
      ...
      u16_Data = static_cast<uint16_t>(
          ...
          (static_cast<uint16_t>(orc_Msg.au8_Data[6]) << 8U));
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", ECRR RES IDX: " + m_GetValueDecHex(u16_VariableIndex) +
               ", DATA: " + m_GetValueDecHex(u16_Data);
    }
    break;
  case GD_REQUEST_ECRR_HS:
    ...
    if (q_IsRequest == true) {
      ...
      if (orc_Msg.au8_Data[5] <= 99U) {
        ...
        // 10% .. 100%
        c_HelpHystHigh = m_GetByteAsStringFormat(
                             static_cast<uint8_t>(orc_Msg.au8_Data[5] - 100U)) +
                         "%";
      } else {
        // invalid upper hysteresis
        c_HelpHystHigh = "invalid";
      ...
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", ECRR REQ HS IDX: " + m_GetValueDecHex(u16_VariableIndex) +
               ", WAIT: " + c_HelpTime + ", HYST HIGH: " + c_HelpHystHigh +
               ", HYST LOW: " + c_HelpHystLow;
    } else {
      // response
      u16_Data = static_cast<uint16_t>(
          ...
          (static_cast<uint16_t>(orc_Msg.au8_Data[6]) << 8U));
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", ECRR RES HS IDX: " + m_GetValueDecHex(u16_VariableIndex) +
               ", DATA: " + m_GetValueDecHex(u16_Data);
    }
    break;
  case GD_REQUEST_TCRR:
    ...
    if (q_IsRequest == true) {
      ...
      if (orc_Msg.au8_Data[4] == 0U) {
        ...
        c_HelpTime =
            m_GetValueDecHex(static_cast<uint16_t>(orc_Msg.au8_Data[4]) * 10U) +
            "ms";
      } else if (orc_Msg.au8_Data[4] >= 101U) {
        c_HelpTime =
            m_GetValueDecHex(
                (static_cast<uint32_t>(orc_Msg.au8_Data[4]) - 100U) * 1000U) +
            "sec";
      } else {
        c_HelpTime = "invalid";
      }
      ...
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", TCRR REQ IDX: " + m_GetValueDecHex(u16_VariableIndex) +
               ", PERIOD: " + c_HelpTime;
    } else {
      // response
      u16_Data = static_cast<uint16_t>(
          ...
          (static_cast<uint16_t>(orc_Msg.au8_Data[6]) << 8U));
      c_Text = c_HelpSender + " -> " + c_HelpReceiver +
               ", TCRR RES IDX: " + m_GetValueDecHex(u16_VariableIndex) +
               ", DATA: " + m_GetValueDecHex(u16_Data);
    }
    break;
  case GD_HS_TO_FIRST_RESPONSE_SRR:
    ...
    c_Text = c_HelpSender + " -> " + c_HelpReceiver +
             ", HS on SRR IDX: " + m_GetValueDecHex(u16_VariableIndex);
    break;
  case GD_HS_TO_FIRST_RESPONSE_ECRR:
    // handshake to first event controlled response request
    c_Text = c_HelpSender + " -> " + c_HelpReceiver +
             ", HS on ECRR IDX: " + m_GetValueDecHex(u16_VariableIndex);
    break;
  case GD_ABORT_GLOBAL_INDIV_RQ:
    ...
    if (orc_Msg.au8_Data[0] == 0xFFU) {
      ...
      c_Text =
          c_HelpSender + " -> " + c_HelpReceiver +
          ", ABORT INDIVIDUAL REQ IDX: " + m_GetValueDecHex(u16_VariableIndex);
    }
    break;
  case GD_IMMEDIATE_WRITE_WO_HS:
    ...
    u16_Data = static_cast<uint16_t>(
        ...
        (static_cast<uint16_t>(orc_Msg.au8_Data[6]) << 8U));
    c_Text = c_HelpSender + " -> " + c_HelpReceiver +
             ", IMMEDIATE WRITE IDX: " + m_GetValueDecHex(u16_VariableIndex) +
             ", DATA: " + m_GetValueDecHex(u16_Data);
    break;
  case GD_IMMEDITAE_WRITE_HS:
    ...
    if (q_IsRequest == true) {
      ...
      c_Text =
          c_HelpSender + " -> " + c_HelpReceiver +
          ", IMMEDIATE WRITE HS IDX: " + m_GetValueDecHex(u16_VariableIndex) +
          ", DATA: " + m_GetValueDecHex(u16_Data);
    } else {
      // response
      c_Text =
          c_HelpSender + " -> " + c_HelpReceiver +
          ", HS on IMMEDIATE WRITE IDX: " + m_GetValueDecHex(u16_VariableIndex);
    }
    break;
  case GD_BT_REQ_CTRL_ACK:
    if (q_IsRequest == true) {
      ...
      case 0x0U:
        ...
        u16_NumberOfBytes = static_cast<uint16_t>(
            orc_Msg.au8_Data[4] +
            (static_cast<uint16_t>(orc_Msg.au8_Data[5]) << 8U));
        c_Text = c_HelpSender + " -> " + c_HelpReceiver +
                 ", BLOCK TRANSFER READ REQ IDX: " +
                 m_GetValueDecHex(u16_VariableIndex) +
                 ", NOB: " + m_GetValueDecHex(u16_NumberOfBytes) +
  ...
  case GD_LOGON:
    ...
    u32_SysTime = static_cast<uint32_t>(orc_Msg.au8_Data[4]) +
                  ...
                  (static_cast<uint32_t>(orc_Msg.au8_Data[7]) << 24U);
    c_Text = c_HelpSender +
             ", LOGON (SYSTIME: " + m_GetValueDecHex(u32_SysTime) + "ms)";
    break;
  case GD_LOGOFF:
    // logon messages (with mapped system time in [ms])
    u32_SysTime = static_cast<uint32_t>(orc_Msg.au8_Data[4]) +
                  ...
                  (static_cast<uint32_t>(orc_Msg.au8_Data[7]) << 24U);
    c_Text = c_HelpSender +
             ", LOGOFF (SYSTIME: " + m_GetValueDecHex(u32_SysTime) + "ms)";
    break;
  default:
    ...
    c_Text += " (ID: " + m_GetValueDecHex(orc_Msg.u32_ID) + ", XTD: " + c_Help +
