...
/* Address Claim */
#define J1939_PGN_ADDRESS_CLAIMED                                              \
  (static_cast<uint16_t>(0xEE00U)) // Address Claim Response

/* Multi-Packet Broadcast/Peer-to-Peer */
#define J1939_PGN_TP_CM                                                        \
  (static_cast<uint16_t>(0xEC00U)) // TP.CM Connection Management
...
#define J1939_TP_CM_EOM_ACK                                                    \
  (static_cast<uint8_t>(19U)) // TP.CM End of message acknowledgement
#define J1939_TP_CM_CONN_ABORT                                                 \
  (static_cast<uint8_t>(0xFFU)) // TP.CM Connection Abort
#define J1939_TP_CM_BAM                                                        \
  (static_cast<uint8_t>(32U)) // TP.CM Broadcast Announce Message
#define J1939_TP_CM_ABORT_AS_BUSY                                              \
  (static_cast<uint8_t>(1U)) // TP.CM Connection Abort Reason 1
#define J1939_TP_CM_ABORT_AS_RESOURCE_LACK                                     \
  (static_cast<uint8_t>(2U)) // TP.CM Connection Abort Reason 2
#define J1939_TP_CM_ABORT_AS_TIMEOUT                                           \
  (static_cast<uint8_t>(3U)) // TP.CM Connection Abort Reason 3
#define J1939_TP_CM_ABORT_AS_UNEXPECTED_CTS                                    \
  (static_cast<uint8_t>(4U)) // TP.CM Connection Abort Reason 4
#define J1939_TP_CM_ABORT_AS_MAX_RETRANSMIT                                    \
  (static_cast<uint8_t>(5U)) // TP.CM Connection Abort Reason 5
#define J1939_TP_CM_ABORT_AS_UNEXPECTED_DT                                     \
  (static_cast<uint8_t>(6U)) // TP.CM Connection Abort Reason 6
#define J1939_TP_CM_ABORT_AS_BAD_SEQ_NR                                        \
  (static_cast<uint8_t>(7U)) // TP.CM Connection Abort Reason 7
#define J1939_TP_CM_ABORT_AS_DUPLICATE_SEQ_NR                                  \
  (static_cast<uint8_t>(8U)) // TP.CM Connection Abort Reason 8
#define J1939_TP_CM_ABORT_AS_MSG_SIZE_TOO_BIG                                  \
  (static_cast<uint8_t>(9U)) // TP.CM Connection Abort Reason 9
#define J1939_TP_CM_ABORT_AS_UNDEFINED                                         \
  (static_cast<uint8_t>(250U)) // TP.CM Connection Abort Reason 250

/* Destination Addresses */
#define J1939_DA_BROADCAST                                                     \
  (static_cast<uint8_t>(0xFFU)) // Destination address for Broadcasts
#define J1939_DA_MASK                                                          \
  (static_cast<uint16_t>(0xFF00U)) // Mask destination address

/* Source Addresses */
#define J1939_SA_NULL                                                          \
  (static_cast<uint8_t>(0xFEU)) // NULL address (used by addressless nodes)
 ...
 * ------------------------------------------------------------------------------------------------
 */
C_CanMonProtocolJ1939::C_CanMonProtocolJ1939(void) : C_CanMonProtocolBase() {}
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_CanMonProtocolJ1939::MessageToString(const T_STWCAN_Msg_RX &orc_Msg) const {
  ...
  if (orc_Msg.u8_XTD == 1) {
    ...
    if (u8_PduFormat <= J1939_PDU1_MAX) {
      // PDU1 (specific, peer to peer), PDU specific (PS) contains destination
      // address (DA) mask out DA
      ...
      case J1939_PGN_TP_CM:
        ...
        case J1939_TP_CM_RTS:
          ...
          t_PGNMBM += m_GetValueDecHex(m_GetPgn(&orc_Msg.au8_Data[5]));
          // Concatenate data
          c_Data = c_MessageSize + " " + c_TotalPackets + " " + c_MaxPackets +
                   " " + t_PGNMBM;
          break;
        ...
        case J1939_TP_CM_CONN_ABORT:
          ...
          if ((orc_Msg.au8_Data[1] > J1939_TP_CM_ABORT_AS_MSG_SIZE_TOO_BIG) &&
            ...
            c_Reason +=
                "SAE_RESERVED (" + m_GetValueDecHex(orc_Msg.au8_Data[1]) + ") ";
          } else if (orc_Msg.au8_Data[1] > J1939_TP_CM_ABORT_AS_UNDEFINED) {
            // abort reasons 251-255
            c_Reason +=
                "J1939_71_DEF (" + m_GetValueDecHex(orc_Msg.au8_Data[1]) + ") ";
          } else {
            // abort reasons 1-9
      ...
      case J1939_PGN_REQUEST:
        if ((orc_Msg.u8_DLC == 3U) && (orc_Msg.au8_Data[0] == 0U) &&
            (orc_Msg.au8_Data[1] == 0xEEU) && (orc_Msg.au8_Data[2] == 0U)) {
          ...
          // Concatenate data
          c_Data = t_PGNMBM + " (" +
                   m_PgnToString(m_GetPgn(&orc_Msg.au8_Data[0])) + ")";
        }
        break;
      ...
      case J1939_PGN_ACK:
        ...
        case J1939_ACK_CANNOT_RESPOND:
          // Cannot respond (PGN supported, but ECU cannot respond; request data
          // later)
      ...
      // PDU2 (global, broadcast), PDU specific (PS) contains group extension
      // (GE) evaluate PGN
      c_Pgn = m_PgnToString(u16_Pgn);
      if (c_Pgn == "PDU2") {
    ...
    // get source address
    c_SourceAddress +=
        m_GetByteAsStringFormat(static_cast<uint8_t>(orc_Msg.u32_ID)) + " ";
...
QString C_CanMonProtocolJ1939::m_PgnToString(const uint32_t ou32_Pgn) const {
  ...
  uint16_t u16_Last =
      static_cast<uint16_t>((sizeof(mat_Pgn) / sizeof(T_J1939_PGN)) - 1);
...
uint32_t C_CanMonProtocolJ1939::m_GetPgn(const uint8_t *const opu8_Data) const {
  return (static_cast<uint32_t>(opu8_Data[2]) << 16U) +
...
QString
C_CanMonProtocolJ1939::m_GetMessageSize(const T_STWCAN_Msg_RX &orc_Msg) const {
  QString c_Size = "SIZE ";
...
QString C_CanMonProtocolJ1939::m_GetData(const T_STWCAN_Msg_RX &orc_Msg,
                                         const uint8_t ou8_StartIdx) const {
  uint8_t u8_Index;
  QString c_Data = "DATA [";
...
QString C_CanMonProtocolJ1939::m_GetName(const T_STWCAN_Msg_RX &orc_Msg) const {
  ...
  for (u8_Index = 0U; u8_Index < orc_Msg.u8_DLC; u8_Index++) {
    u64_Name += static_cast<uint64_t>(orc_Msg.au8_Data[u8_Index])
                << (8 * u8_Index);
  }
  ...
  c_Data += "MC:";
  c_Data +=
      m_GetValueDecHex(static_cast<uint32_t>((u64_Name >> 21U) & 0x07FFU));
  c_Data += " ";
