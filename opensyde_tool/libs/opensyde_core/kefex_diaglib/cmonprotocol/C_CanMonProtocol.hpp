...
public:
  ...
  C_CanMonProtocols(
      const C_CanMonProtocols &orc_Source); // copying will cause compiler error
  C_CanMonProtocols &
  operator=(const C_CanMonProtocols
                &orc_Source); // assignment will cause compiler error

  int32_t SetProtocolMode(const e_CanMonL7Protocols oe_L7Protocol);
  e_CanMonL7Protocols GetProtocolMode(void) const;

  int32_t GetProtocolName(const e_CanMonL7Protocols oe_L7Protocol,
  ...
  static QString FormatTimeStamp(const uint64_t ou64_TimeStampUs,
                                 const bool oq_LeftFillBlanks = false);

  // for displaying on screen (will consider configured L7 protocol and
  // decimal/hex setting):
  ...
  // for protocolling (e.g. to file; individual fields separated by semicolon;
  // L2 AND L7 interpretation; always hex)
  ...
  int32_t
  KFXSetVariableInfo(const stw::diag_lib::C_KFXVariableLists *const opc_Lists,
