...
//-----------------------------------------------------------------------------
QString
C_CanMonProtocolCanOpen::MessageToString(const T_STWCAN_Msg_RX &orc_Msg) const {
  QString c_Text;
  QString c_Help;
  QString c_Help2;
  uint16_t u16_Index;
  ...
  if ((orc_Msg.u32_ID >= GUARD_AREA_LOW) &&
      (orc_Msg.u32_ID < GUARD_AREA_HIGH)) // Nodeguarding / Heartbeat / Bootup
  {
    if ((orc_Msg.u8_DLC >= 1U) && (orc_Msg.u8_RTR == 0U)) {
      c_Text = "HB/Guard " +
               m_GetByteAsStringFormat(static_cast<uint8_t>(
                   orc_Msg.u32_ID - (GUARD_AREA_LOW - 1))) +
               " : TGL:" + QString::number(orc_Msg.au8_Data[0] >> 7) + " : ST:";
      ...
      case 127:
        ...
        break;
      default:
        c_Help = m_GetByteAsStringFormat(
            static_cast<uint8_t>(orc_Msg.au8_Data[0] & 0x7FU));
        break;
      }
      c_Text = c_Text + c_Help;
    } else // probably a guard request from a master
    {
      c_Text = "Guard RQ " + m_GetByteAsStringFormat(static_cast<uint8_t>(
                                 orc_Msg.u32_ID - (GUARD_AREA_LOW - 1)));
    }
  }
  if (orc_Msg.u32_ID == NMT_ZERO_ID) {
    if (orc_Msg.au8_Data[1] == 0) {
      ...
      c_Help =
          "NMT NODE " + m_GetByteAsStringFormat(orc_Msg.au8_Data[1]) + ": ";
    }
  ...
  if ((orc_Msg.u32_ID >= EMCY_AREA_LOW) && (orc_Msg.u32_ID < EMCY_AREA_HIGH)) {
    c_Text = "EMCY NODE " +
             m_GetByteAsStringFormat(
                 static_cast<uint8_t>(orc_Msg.u32_ID - (EMCY_AREA_LOW - 1))) +
             ":";
    if (orc_Msg.u8_DLC == 0) {
  ...
  if ((orc_Msg.u32_ID > SERVER_BASE_ID) &&
      (orc_Msg.u32_ID < SDO_RX_AREA_HIGH)) {
    if (orc_Msg.u32_ID < CLIENT_BASE_ID) {
      c_Help = "SDO SERVER " +
               m_GetByteAsStringFormat(
                   static_cast<uint8_t>(orc_Msg.u32_ID - SERVER_BASE_ID)) +
               ": ";
      ...
      case SCS_INIT_UPLOAD:
        ...
        if ((orc_Msg.au8_Data[0] & 0x02U) == 0x02U) {
          ...
          c_Help2 = QString::asprintf("%X,%02X  ", u16_Index, u16_SubIndex);
          if ((orc_Msg.au8_Data[0] & 0x01U) == 0x01U) // size is indicated
          {
            u32_Size = 4 - ((orc_Msg.au8_Data[0] & 0x0CU) >> 2);
            c_Help2 +=
                ("Size:" +
                 m_GetByteAsStringFormat(static_cast<uint8_t>(u32_Size)) + " ");
          } else {
            u32_Size = 4;
            c_Help2 += ("Size:?\?\? ");
          }
      ...
      case ABORT_DOMAIN:
        ...
        u16_SubIndex = orc_Msg.au8_Data[3];
        c_Help2 =
            QString::asprintf("%X,%02X  Reason:", u16_Index, u16_SubIndex);
        for (j = 0; j < 4; j++) {
          c_Help2 += (m_GetByteAsStringFormat(orc_Msg.au8_Data[j + 4]) + " ");
        }
        c_Text = c_Help + c_Help2;
      ...
      c_Help = "SDO CLIENT " +
               m_GetByteAsStringFormat(
                   static_cast<uint8_t>(orc_Msg.u32_ID - CLIENT_BASE_ID)) +
               ": ";
      switch (orc_Msg.au8_Data[0] & 0xE0U) {
      ...
      case CCS_INIT_DOWNLOAD:
        ...
        if ((orc_Msg.au8_Data[0] & 0x02U) == 0x02U) {
          ...
          if ((orc_Msg.au8_Data[0] & 0x01U) == 0x01U) {
            u32_Size =
                4U - ((orc_Msg.au8_Data[0] & 0x0CU) >> 2); // size is indicated
            c_Help2 +=
                ("Size:" +
                 m_GetByteAsStringFormat(static_cast<uint8_t>(u32_Size)) + " ");
          } else {
...
*/
//-----------------------------------------------------------------------------
QString C_CanMonProtocolCanOpen::GetProtocolName(void) const {
  return "CANopen";
}
