...
public:
  ...
  void Set(const QString &orc_Name, const uint8_t ou8_Size,
           const bool oq_Signed);

  // for more complex stuff we could also add a pointer to a "decode" function
  // returning the value as a string e.g. return "RAM" or "EEPROM" for memory
  // test types instead of bland numbers
...
class C_CanMonProtocolShipIpIva : public C_CanMonProtocolKefexIva {
private:
  ...
  QString
  m_MessageToString11Bit(const stw::can::T_STWCAN_Msg_RX &orc_Msg) const;
  ...
  QString
  m_Sip11MessageToString(const stw::can::T_STWCAN_Msg_RX &orc_Msg) const;
  QString m_SipDecodeSfFfReadWriteHeader(
  ...
  QString m_SipGetMemoryRwTypeAndIndex(const uint8_t ou8_MemType,
                                       const uint8_t ou8_MemIndex) const;
  QString m_SipGetOdRwTypeAndIndex(const uint8_t ou8_OdType,
