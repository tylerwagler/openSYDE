...
#define CAN_ID_R                                                               \
  (0x02000000U) // CAN-ID reserved bit (usually 0, STW: routing 1)
#define CAN_ID_DP                                                              \
  (0x01000000U) // CAN-ID data page bits (usually 0, STW: routing 1)
#define CAN_ID_TOS                                                             \
  (0x00C00000U) // CAN-ID type of service (only used for routing, usually 11b)
#define CAN_ID_PF__N_TA_TYPE_5                                                 \
  (0x00DA0000U) // CAN-ID protocol data unit format (normal fixed address.
                // physical 218)
#define CAN_ID_PF__N_TA_TYPE_6                                                 \
  (0x00DB0000U) // CAN-ID protocol data unit format (normal fixed address.
                // functional 219)

// ISO15765-2 addressing mode
#define OSY_CTP_ADDR_MODE_PHYSICAL                                             \
  (0U) ///< CAN TP mode fixed physical addressing
#define OSY_CTP_ADDR_MODE_FUNCTIONAL                                           \
  (1U) ///< CAN TP mode fixed functional addressing
...
#define OSY_DPD_SI_OS_READ_DATAPOOLDATA_EVENT_DRIVEN                           \
  (0xBAU) ///< system supplier specific
#define OSY_DPD_SI_OS_READ_DATAPOOLDATA_BY_IDENTIFIER                          \
  (0xBBU) ///< system supplier specific
#define OSY_DPD_SI_OS_WRITE_DATAPOOLDATA_BY_IDENTIFIER                         \
  (0xBCU)                                       ///< system supplier specific
...
#define OSY_OS_READ_DATAPOOLDATA_EVENT_DRIVEN                                  \
  (0xF1U) ///< STW-specific single frame specific
#define OSY_OS_POS_STARTUP_MESSAGE                                             \
  (0xF3U) ///< STW-specific single frame specific
#define OSY_OS_FSN_STARTUP_MESSAGE                                             \
  (0xF4U) ///< STW-specific single frame specific
...
#define OSY_UDS_NRC_SUB_FUNCTION_NOT_SUPPORTED                                 \
  (0x12U) ///< sub-functionNotSupported
#define OSY_UDS_NRC_INCORRECT_MESSAGE_LENGTH_OR_INVALID_FORMAT                 \
...
#define OSY_UDS_NRC_EXCEEDED_NUMBER_OF_ATTEMPTS                                \
  (0x36U) ///< exceededNumberOfAttempts
#define OSY_UDS_NRC_REQUIRED_TIME_DELAY_NOT_EXPIRED                            \
  (0x37U) ///< requiredTimeDelayNotExpired
#define OSY_UDS_NRC_UPLOAD_DOWNLOAD_NOT_ACCEPTED                               \
  (0x70U) ///< uploadDownloadNotAccepted
#define OSY_UDS_NRC_TRANSFER_DATA_SUSPENDED (0x71U) ///< transferDataSuspended
#define OSY_UDS_NRC_GENERAL_PROGRAMMING_FAILURE                                \
  (0x72U) ///< generalProgrammingFailure
#define OSY_UDS_NRC_WRONG_BLOCK_SEQUENCE_COUNTER                               \
  (0x73U) ///< wrongBlockSequenceCounter
#define OSY_UDS_NRC_RESPONSE_PENDING                                           \
  (0x78U) ///< requestCorrectlyReceivedResponsePending
#define OSY_UDS_NRC_SUBFUNCTION_NOT_SUPPORTED_IN_ACTIVE_SESSION                \
  (0x7EU) ///< sub-functionNotSupportedInActiveSession
#define OSY_UDS_NRC_SERVICE_NOT_SUPPORTED_IN_ACTIVE_SESSION                    \
  (0x7FU) ///< serviceNotSupportedInActiveSession
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_AddressInformationToText(
    const C_CanMonProtocolOpenSyde::T_CanAddressInformation
        &orc_CanAddressInformation) const {
  QString c_Text;
  ...
  if (orc_CanAddressInformation.u8_RoutingMode == OSY_CTP_ROUTING_ACTIVE) {
    ...
    c_Text = "LOCAL";
    if (orc_CanAddressInformation.u8_AddressMode ==
        OSY_CTP_ADDR_MODE_PHYSICAL) {
      c_Text += "_PHYS ";
    } else {
      c_Text += "_FUNC ";
    }
    ...
    c_Text += ("(" +
               m_GetByteAsStringFormat(
                   orc_CanAddressInformation.t_NodeIdSource.u8_NodeId) +
               " -> ");
    if (orc_CanAddressInformation.t_NodeIdTarget.u8_NodeId ==
        0x7FU) // broadcast address ?
    {
      c_Text += "ALL) ";
    } else {
      c_Text += (m_GetByteAsStringFormat(
                     orc_CanAddressInformation.t_NodeIdTarget.u8_NodeId) +
                 ") ");
    }
  }
...
/*! \brief   Convert 24bit openSYDE data pool identifier code to text
   ...
   \param[in]     oq_IsResponse                Flag if service for accessing the
   datapool element was a response or a request \param[in]
   orc_CanAddressInformation    Information about nodes involved in
   communication
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_DataPoolIdentifierToText(
  ...
  const bool q_ReservedBitSet =
      ((ou32_DataPoolIdentifier & 0x00800000U) != 0U) ? true : false;
  const uint8_t u8_DataPoolIndex =
      static_cast<uint8_t>(ou32_DataPoolIdentifier >> 18) & 0x1FU;
  const uint8_t u8_ListIndex =
      static_cast<uint16_t>(ou32_DataPoolIdentifier >> 11) & 0x7FU;
  const uint16_t u16_ElementIndex =
      static_cast<uint16_t>(ou32_DataPoolIdentifier) & 0x7FFU;
  QString c_Text;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_CanMonProtocolOpenSyde::mh_DisassembleCanId(
    const uint32_t ou32_CanId,
  ...
  /*      |   P   | R  | DP |    PF    |   PS   |   SA   |
     ...
     bit  | 28-26 | 25 | 24 | 23-22 |  21-18 | 17-11  |  10-7  |  6-0   |
          |  110  |  1 |  1 |   11  | SrcBus | SrcAdr | TrgBus | TrgAdr |   //
     routed communication
  */
  // routing
  if (((ou32_CanId & CAN_ID_R) == CAN_ID_R) &&
      ((ou32_CanId & CAN_ID_DP) == CAN_ID_DP)) {
    or_CanAddressInformation.t_NodeIdSource.u8_Subnet =
        static_cast<uint8_t>((ou32_CanId >> 18U) & 0x0F);
    or_CanAddressInformation.t_NodeIdSource.u8_NodeId =
        static_cast<uint8_t>((ou32_CanId >> 11U) & 0x7F);
    or_CanAddressInformation.t_NodeIdTarget.u8_Subnet =
        static_cast<uint8_t>((ou32_CanId >> 7U) & 0x0F);
    or_CanAddressInformation.t_NodeIdTarget.u8_NodeId =
        static_cast<uint8_t>((ou32_CanId >> 0U) & 0x7F);
    or_CanAddressInformation.u8_AddressMode =
        OSY_CTP_ADDR_MODE_PHYSICAL; // unused
  ...
  // no routing
  else if (((ou32_CanId & CAN_ID_R) == 0U) &&
           ((ou32_CanId & CAN_ID_DP) == 0U)) {
    ...
    } else if ((ou32_CanId & CAN_ID_PF__N_TA_TYPE_5) ==
               CAN_ID_PF__N_TA_TYPE_5) {
    ...
    or_CanAddressInformation.t_NodeIdTarget.u8_NodeId =
        static_cast<uint8_t>((ou32_CanId >> 8U) & 0xFFU);
    or_CanAddressInformation.t_NodeIdSource.u8_NodeId =
        static_cast<uint8_t>((ou32_CanId >> 0U) & 0xFFU);
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_ServiceIdToText(
    const uint8_t ou8_ServiceData, const bool oq_IsNegativeResponse) const {
...
//----------------------------------------------------------------------------------------------------------------------
QString
C_CanMonProtocolOpenSyde::m_SessionIdToText(const uint8_t ou8_SessionId) const {
  QString c_Text;
...
/*! \brief   Convert openSYDE service identifier data to text
   ...
   * we handle either a request or a positive response
   ...
   \param[in]     opu8_ServiceData           service data (starting with service
   identifier) \param[in]     ou8_ServiceSize            number of bytes in
   opu8_ServiceData (1..6) \param[in]     orc_CanAddressInformation  information
   about nodes involved in communication

   \return
   Text interpretation of service identifier
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_ServiceDataToText(
    const uint8_t *const opu8_ServiceData, const uint8_t ou8_ServiceSize,
    const T_CanAddressInformation &orc_CanAddressInformation,
    const bool oq_IsSingleFrame) const {
  const bool q_IsResponse =
      ((opu8_ServiceData[0] & 0x40U) == 0x40U) ? true : false;
  const uint8_t u8_Service = (opu8_ServiceData[0] & 0xBFU);
  QString c_Text;
  uint8_t u8_FirstRawByte = 1U; // index of first raw data byte to handle (to be
                                // displayed as dump data)
  ...
  case OSY_UDS_SI_WRITE_DATA_BY_IDENTIFIER:
    if (ou8_ServiceSize < 3U) {
      ...
      c_Text =
          ":" + m_DataIdentifierAndDataToText(
                    u8_Service == OSY_UDS_SI_WRITE_DATA_BY_IDENTIFIER,
                    mh_BytesToWordHighLow(&opu8_ServiceData[1]), q_IsResponse,
                    ou8_ServiceSize - 3U, &opu8_ServiceData[3]);
    }
    u8_FirstRawByte = ou8_ServiceSize; // complete interpretation is in
                                       // subfunction; don't do it here ...
    break;
  ...
  case OSY_DPD_SI_OS_WRITE_DATAPOOLDATA_BY_IDENTIFIER:
    if (ou8_ServiceSize < 4U) {
      ...
      const uint32_t u32_DpIndex =
          (static_cast<uint32_t>(opu8_ServiceData[1]) << 16U) +
          (static_cast<uint32_t>(opu8_ServiceData[2]) << 8U) +
          opu8_ServiceData[3];
      c_Text += m_DataPoolIdentifierToText(u32_DpIndex, q_IsResponse,
      ...
      if (ou8_ServiceSize < 2U) {
        ...
        if (opu8_ServiceData[1] == 0) {
          ...
          if (ou8_ServiceSize < 5U) {
            ...
            const uint32_t u32_DpIndex =
                (static_cast<uint32_t>(opu8_ServiceData[2]) << 16U) +
            ...
            if (q_IsChange == true) {
              if (ou8_ServiceSize < 6U) {
                ...
                // we only have part of the threshold in the first frame; only
                // display raw data ...
                u8_FirstRawByte = 5U;
              }
            } else {
  ...
  case OSY_UDS_SI_ROUTINE_CONTROL:
    if (ou8_ServiceSize < 4U) {
      c_Text =
          " error: DLC too short (no subFunction and/or routineIdentifier)";
    } else {
      switch (opu8_ServiceData[1]) {
      case 1:
        c_Text = ":start";
      ...
      case 3:
        ...
        break;
      default:
        c_Text = ":undefinedsubfunction(" +
                 m_GetValueDecHex(opu8_ServiceData[1]) + ")";
        break;
      }
      c_Text +=
          (":" + m_RoutineDataToText(
                     mh_BytesToWordHighLow(&opu8_ServiceData[2]), q_IsResponse,
  ...
  case OSY_UDS_SI_DIAGNOSTIC_SESSION_CONTROL:
    if (ou8_ServiceSize < 2U) {
      ...
      const bool q_ExpectResponse =
          ((opu8_ServiceData[1] & 0x80U) == 0) ? true : false;
  ...
  case OSY_UDC_B_SI_READ_SERIAL_NUMBER:
    if (q_IsResponse == false) {
      ...
      if (ou8_ServiceSize != 7U) {
        ...
        // For the response it is not possible to tell the difference between
        // standard and extended format
        QString c_Snr;
        const uint8_t u8_BlockNumber = opu8_ServiceData[1] & 0x0FU;
        const uint8_t u8_SubNodeId =
            static_cast<uint8_t>((opu8_ServiceData[1] & 0xF0U) >> 4U);
        ...
        c_Text += "  UniqueId: " +
                  stw::opensyde_core::C_OscUtils::h_IntToHex(u32_UniqueId, 6);

        if (u8_BlockNumber == 0) {
          // First byte reserved
          c_Text +=
              "  Manufacturer Format: " + QString::number(opu8_ServiceData[6]);
          ...
          c_Text2 = QString::asprintf("  SNR POS First signs: %02x",
                                      opu8_ServiceData[6]);
          c_Text += c_Text2;
        } else {
          QString c_Text2;
          ...
          if (u8_BlockNumber <= 3U) {
            c_Text2 =
                QString::asprintf("  SNR POS Part: %02x%02x",
                                  opu8_ServiceData[5], opu8_ServiceData[6]);
            c_Text += c_Text2;
          } else if (u8_BlockNumber == 4U) {
            c_Text2 = QString::asprintf("  SNR POS Last signs: %02x",
                                        opu8_ServiceData[5]);
            c_Text += c_Text2;
          } else {
  ...
  case OSY_UDS_SI_WRITE_MEMORY_BY_ADDRESS:
    if ((q_IsResponse == false) ||
        (u8_Service == OSY_UDS_SI_WRITE_MEMORY_BY_ADDRESS)) {
      ...
      if (ou8_ServiceSize < 2U) {
        ...
        if (ou8_ServiceSize < u8_ExpectedSize) {
          c_Text = " error: DLC too short (missing or incomplete address "
                   "and/or memory size)";
        } else {
          c_Text = "  ADD:";
          for (uint8_t u8_Index = 0U; u8_Index < u8_NumBytesAddress;
               u8_Index++) {
          ...
          for (uint8_t u8_Index = 0U; u8_Index < u8_NumBytesSize; u8_Index++) {
            if (u8_Index <= u8_ExpectedSize) {
              c_Text += m_GetByteAsStringFormat(
                  opu8_ServiceData[2U + u8_NumBytesAddress + u8_Index]);
            }
            if ((u8_Index < (u8_NumBytesSize - 1)) &&
                (u8_Index < u8_ExpectedSize)) {
              c_Text += ",";
            }
          }
        }
    ...
    if (ou8_ServiceSize < 2U) {
      ...
      if (opu8_ServiceData[1] == 0U) {
        ...
        if ((opu8_ServiceData[1] & 0x01U) == 0x01U) {
          c_Text += "  REQUEST_SEED  LEVEL:" +
                    m_GetByteAsStringFormat(opu8_ServiceData[1]);
        } else {
          c_Text += "  SEND_KEY  LEVEL:" +
                    m_GetByteAsStringFormat(opu8_ServiceData[1] - 1U);
        }
        ...
        if (ou8_ServiceSize > 2U) {
          ...
          if (ou8_ServiceSize < 6U) {
            ...
            const uint32_t u32_Value =
                (static_cast<uint32_t>(opu8_ServiceData[2]) << 24U) +
  ...
  if ((ou8_ServiceSize - u8_FirstRawByte) >
      0) // we have data byte(s) left to display in raw format ...
  {
    uint8_t u8_Index;
    c_Text += "  DATA[";
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_NegativeResponseCodeToText(
  ...
  default:
    c_Text =
        "unknownnegativeresponsecode (" + m_GetValueDecHex(ou8_NrCode) + ")";
    break;
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_GetSupposedlyAsciiText(
    const uint8_t ou8_NumBytes, const uint8_t *const opu8_Data) const {
  QString c_Text = "\"";
  QString c_Text2;
  uint8_t u8_Index;
  ...
  for (u8_Index = 0U; u8_Index < ou8_NumBytes; u8_Index++) {
    if (std::isprint(opu8_Data[u8_Index]) == 0) {
      ...
      c_Text2 = QString::asprintf("%c", opu8_Data[u8_Index]);
    }
    c_Text += c_Text2;
  }
  c_Text += "\"";
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::mh_ThreeByteVersionToString(
    const uint8_t *const opu8_Version) {
  QString c_Text;

  c_Text = QString::asprintf("%d.%02dr%d", opu8_Version[0], opu8_Version[1],
                             opu8_Version[2]);
  return c_Text;
}
...
/*! \brief   Convert openSYDE data identifier code and data to text
   ...
   \param[in]     ou8_PayloadSize          number of data bytes (in
   opu8_Payload) \param[in]     opu8_Payload             payload data

   \return
   Text interpretation of data identifier
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_DataIdentifierAndDataToText(
  ...
  case UDC_H_DATAID_PROTOCOL_VERSION:
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize != 3U) {
        ...
        c_Text +=
            ("  VERSION:" + mh_ThreeByteVersionToString(&opu8_Payload[0]));
  ...
  case DPD_DATAID_MAX_NUM_ASYNC:
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize != 2U) {
        ...
        c_Text += ("  MAX_NUMBER:" +
                   m_GetValueDecHex(mh_BytesToWordHighLow(&opu8_Payload[0])));
  ...
  case FL_H_DATAID_FLASH_COUNT:
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize != 4U) {
        ...
        c_Text += ("  FLASH_COUNT:" +
                   m_GetValueDecHex(mh_BytesToDwordHighLow(&opu8_Payload[0])));
        u8_FirstRawByte = ou8_PayloadSize; // finished here ...
      }
    }
    break;
  case UDC_H_DATAID_DEVICE_NAME:
    ...
    if (oq_IsResponse == true) {
      c_Text += ("  DEV_NAME:" +
                 m_GetSupposedlyAsciiText(ou8_PayloadSize, opu8_Payload));
      u8_FirstRawByte = ou8_PayloadSize; // finished here ...
    }
    break;
  ...
  case UDC_H_DATAID_BOOTSOFTWARE_IDENTIFICATION:
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize != 4U) {
        ...
        c_Text += "  MODULES:" + m_GetValueDecHex(opu8_Payload[0]) +
                  "  VERSION:" + mh_ThreeByteVersionToString(&opu8_Payload[1]);
        u8_FirstRawByte = ou8_PayloadSize; // finished here ...
      }
    }
    break;
  ...
  case UDC_H_DATAID_CERTIFICATE_SERIAL_NUMBER:
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize != 3U) {
        ...
        QString c_Text2;
        c_Text2 = QString::asprintf("  CERTIFICATESNR:%02x:%02x:%02x",
                                    opu8_Payload[0], opu8_Payload[1],
                                    opu8_Payload[2]);
  ...
  case UDC_H_DATAID_SECURITY_KEY:
    ...
    if (oq_IsResponse == false) {
      if (ou8_PayloadSize != 3U) {
        ...
        QString c_Text2;
        c_Text2 =
            QString::asprintf("  PUBLICKEY:%02x:%02x:%02x", opu8_Payload[0],
                              opu8_Payload[1], opu8_Payload[2]);
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize != 3U) {
        ...
        if (opu8_Payload[0] < static_cast<uint8_t>(0x20)) {
          ...
          c_Text2 = QString::asprintf("  SNR:%02x.%02x%02x", opu8_Payload[0],
                                      opu8_Payload[1], opu8_Payload[2]);
        } else {
  ...
  case UDC_H_DATAID_ECU_SERIAL_NUMBER_EXT:
    ...
    if (oq_IsResponse == true) {
      if (ou8_PayloadSize > 2U) {
        c_Text +=
            "  SNR Manufacturer Format: " + QString::number(opu8_Payload[0]);
        ...
        if (opu8_Payload[0] != 0U) {
          ...
          c_Text2 =
              QString::asprintf("  SNR First signs: %02x", opu8_Payload[2]);
          c_Text += c_Text2;
        }
        u8_FirstRawByte = ou8_PayloadSize; // finished here ...
      } else {
  ...
  default:
    c_Text =
        "unknowndataidentifier (" + m_GetValueDecHex(ou16_DataIdentifier) + ")";
    break;
  }

  // there is more data we cannot interpret: display as raw
  if ((ou8_PayloadSize - u8_FirstRawByte) >
      0) // do we have data byte(s) left to display in raw format ?
  {
    c_Text += m_RawDataToString(ou8_PayloadSize - u8_FirstRawByte,
                                &opu8_Payload[u8_FirstRawByte]);
  }
...
/*! \brief   Convert openSYDE routine identifier code to text
   ...
   \param[in]     oq_IsResponse                 true: response flag was set
                                                false: response flag was not set
   (we have a request) \param[in]     ou8_DataSize                  number of
   bytes in payload \param[in]     opu8_Data                     payload of
   routine starting with the first byte after the routine identifier (2 bytes in
   a FF, up to 3 bytes in a SF)

   \return
   Text interpretation of routine data
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_RoutineDataToText(
    const uint16_t ou16_RoutineIdentifier, const bool oq_IsResponse,
  ...
  case DPD_ROUTINEID_NOTIFY_NVM_CHANGED:
    ...
    if (ou8_DataSize < 2U) {
      ...
      c_Text += "  DATA_POOL:" + m_GetValueDecHex(opu8_Data[0]) +
                "  LIST:" + m_GetValueDecHex(opu8_Data[1]);
      if (ou8_DataSize >= 3U) {
        switch (opu8_Data[2]) {
        case 0:
          c_Text += "  ACK:OK";
  ...
  case UDS_H_ROUTINE_CTRL_SET_NODEID_BY_SERIALNUMBER_PART1:
    ...
    if (oq_IsResponse == false) {
      if (ou8_DataSize == 3U) {
        QString c_Text2;
        c_Text2 = QString::asprintf("  SNR POS First Part: %02x%02x%02x",
                                    opu8_Data[0], opu8_Data[1], opu8_Data[2]);
        c_Text += c_Text2;
        u8_FirstRawByte = ou8_DataSize; // finished here ...
      } else {
        c_Text += " error: incorrect number of data bytes";
  ...
  case UDS_H_ROUTINE_CTRL_SET_NODEID_BY_SERIALNUMBER_PART2:
    ...
    if (oq_IsResponse == false) {
      if (ou8_DataSize == 3U) {
        QString c_Text2;
        c_Text2 = QString::asprintf("  SNR POS Last Part: %02x%02x%02x",
                                    opu8_Data[0], opu8_Data[1], opu8_Data[2]);
        c_Text += c_Text2;
        u8_FirstRawByte = ou8_DataSize; // finished here ...
      } else {
  ...
  case UDS_H_ROUTINE_CTRL_SET_NODEID_BY_SERIALNUMBER_EXT_12: // Similar service
    // no details reported for now
    c_Text =
        "SetNodeIdBySerialNumberExtPart" +
        QString::number((ou16_RoutineIdentifier -
                         UDS_H_ROUTINE_CTRL_SET_NODEID_BY_SERIALNUMBER_EXT_1) +
                        1U);
    ...
    if (oq_IsResponse == false) {
      if (ou8_DataSize > 0U) {
        ...
        c_Text += "  SNR FSN Part: ";
        for (u32_SnrSignCounter = 0U; u32_SnrSignCounter < ou8_DataSize;
             ++u32_SnrSignCounter) {
          c_Text += static_cast<char_t>(opu8_Data[u32_SnrSignCounter]);
        }

        if ((ou8_DataSize == 3U) &&
            ((ou16_RoutineIdentifier ==
              UDS_H_ROUTINE_CTRL_SET_NODEID_BY_SERIALNUMBER_EXT_3) ||
             (ou16_RoutineIdentifier ==
              UDS_H_ROUTINE_CTRL_SET_NODEID_BY_SERIALNUMBER_EXT_4))) {
          QString c_Text2;
          // POS could be possible, but can not be differed. Show both
          // possibilities
          c_Text2 = QString::asprintf("  SNR POS Part: %02x%02x%02x",
                                      opu8_Data[0], opu8_Data[1], opu8_Data[2]);
          c_Text += c_Text2;
        }
        u8_FirstRawByte = ou8_DataSize; // finished here ...
  ...
  case UDS_H_ROUTINE_CTRL_FACTORY_MODE:
    ...
    if (oq_IsResponse == false) {
      if (ou8_DataSize == 2U) {
        if (opu8_Data[0] == UDC_H_DATAID_MASTER_RESET) {
          ...
          c_Text +=
              "  OPERATION:UNDEFINED (" + m_GetValueDecHex(opu8_Data[0]) + ")";
        }
      } else {
        c_Text += " error: incorrect number of data bytes";
      }
    ...
    break;
  default:
    c_Text = "unknownroutineidentifier (" +
             m_GetValueDecHex(ou16_RoutineIdentifier) + ")";
    break;
  }

  // there is more data we cannot interpret: display as raw
  if ((ou8_DataSize - u8_FirstRawByte) >
      0) // do we have data byte(s) left to display in raw format ?
  {
    c_Text += m_RawDataToString(ou8_DataSize - u8_FirstRawByte,
                                &opu8_Data[u8_FirstRawByte]);
  }
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::MessageToString(
  ...
  if (orc_Msg.u8_XTD == 1U) // 29bit ID ?
  {
    ...
    // decode ID:
    s32_Return = C_CanMonProtocolOpenSyde::mh_DisassembleCanId(orc_Msg.u32_ID,
                                                               t_Address);
    if (s32_Return == C_NO_ERR) {
      ...
      if (orc_Msg.u8_DLC < 1U) {
        ...
        case ISO15765_N_PCI_SF:
          ...
          c_Text +=
              (" SF  NUM " +
               m_GetByteAsStringFormat(static_cast<uint8_t>(u16_NumBytes)) +
               " ");
          ...
          if (orc_Msg.au8_Data[1] == OSY_UDS_NR_SI) // negative response ?
          {
            if ((u16_NumBytes == 3U) && (orc_Msg.u8_DLC >= 4U)) {
              ...
              c_Text += m_ServiceIdToText(orc_Msg.au8_Data[2], true) + " NRC:" +
                       ...
                       (orc_Msg.au8_Data[2] ==
                        OSY_DPD_SI_OS_READ_DATAPOOLDATA_EVENT_DRIVEN)) {
              ...
              const uint32_t u32_DpIndex =
                  ...
                  orc_Msg.au8_Data[6];
              c_Text +=
                  m_ServiceIdToText(orc_Msg.au8_Data[2], true) +
                  " NRC:" + m_NegativeResponseCodeToText(orc_Msg.au8_Data[3]) +
                  " " +
                  m_DataPoolIdentifierToText(u32_DpIndex, true, t_Address);
            } else {
              c_Text += "NEGRES with invalid length (incorrect number of bytes "
                        "in header or not enough data"
            ...
            if (orc_Msg.u8_DLC < (1 + u16_NumBytes)) {
              ...
              if (u16_NumBytes > 0U) {
                ...
                c_Text += m_ServiceDataToText(
                    &orc_Msg.au8_Data[1], static_cast<uint8_t>(u16_NumBytes),
                    t_Address, true);
              }
            }
          }
        ...
        case ISO15765_N_PCI_FF:
          ...
          if (orc_Msg.u8_DLC != 8) // FF only makes sense if all bytes are used
          {
            ...
            c_Text +=
                m_ServiceDataToText(&orc_Msg.au8_Data[2], 6, t_Address, false);
          }
          break;
        ...
        case ISO15765_N_PCI_FC:
          ...
          if (orc_Msg.u8_DLC < 2) // not enough data
          {
            ...
            c_Text +=
                (" FS " + m_GetByteAsStringFormat(orc_Msg.au8_Data[0] & 0x0FU) +
        ...
        case STWOPENSYDE_PCI_SF:
          ...
          if (orc_Msg.au8_Data[0] == OSY_OS_READ_DATAPOOLDATA_EVENT_DRIVEN) {
            ...
            if (orc_Msg.u8_DLC < 3U) // not enough data
            {
              ...
              // we should have data:
              const uint32_t u32_DpIndex =
                  (static_cast<uint32_t>(orc_Msg.au8_Data[1]) << 16U) +
                  (static_cast<uint32_t>(orc_Msg.au8_Data[2]) << 8U) +
                  orc_Msg.au8_Data[3];
              c_Text +=
                  m_DataPoolIdentifierToText(u32_DpIndex, true, t_Address);
              // display data in raw format
              if ((orc_Msg.u8_DLC - 4U) > 0U) // do we have data byte(s) left to
                                              // display in raw format ...
              {
                c_Text += m_RawDataToString(orc_Msg.u8_DLC - 4U,
                                            &orc_Msg.au8_Data[4]);
              }
            }
            ...
            if (orc_Msg.u8_DLC < 8U) // not enough data
            {
              ...
              // we should have data:
              QString c_Snr;
              uint32_t u32_SubNodeId =
                  static_cast<uint32_t>(orc_Msg.au8_Data[1] & 0xF0U) >> 4U;
            ...
            if (orc_Msg.u8_DLC < 2U) // not enough data
            {
              ...
              // we should have data:
              uint32_t u32_BlockNr =
                  static_cast<uint32_t>(orc_Msg.au8_Data[1] & 0x0FU);
              uint32_t u32_SubNodeId =
                  static_cast<uint32_t>(orc_Msg.au8_Data[1] & 0xF0U) >> 4U;
              ...
              {
                c_Text += "  SNR Manufacturer Format: " +
                          QString::number(orc_Msg.au8_Data[2]);
                c_Text +=
                    "  SNR Length: " + QString::number(orc_Msg.au8_Data[3]);
              ...
              if (q_DlcCorrect == true) {
                ...
                for (u32_SnrSignCounter = u32_SnrSignStartIndex;
                     u32_SnrSignCounter < orc_Msg.u8_DLC;
                     ++u32_SnrSignCounter) {
                  c_Text +=
                      static_cast<char_t>(orc_Msg.au8_Data[u32_SnrSignCounter]);
                }
              }
            }
          } else {
            c_Error = "unknown service" +
                      m_GetByteAsStringFormat(orc_Msg.au8_Data[0] & 0x0FU);
          }
          break;
        ...
        case STWOPENSYDE_PCI_MF:
          ...
          if ((orc_Msg.au8_Data[0] & 0x0FU) == 0x00U) {
            ...
            c_Text += "I ";
            if (orc_Msg.u8_DLC !=
                8) // first OMF only makes sense if all bytes are used
            {
              c_Error = "DLC too short (not equals 8)";
            } else {
              c_Text +=
                  ("NUM " + m_GetByteAsStringFormat(orc_Msg.au8_Data[1]) + " ");
              c_Text += m_ServiceIdToText(orc_Msg.au8_Data[2]);
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::m_RawDataToString(
    const uint8_t ou8_NumBytes, const uint8_t *const opu8_Data) const {
  uint8_t u8_Index;
  QString c_Text = "  DATA[";
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_CanMonProtocolOpenSyde::GetProtocolName(void) const {
  return "STW openSYDE";
}
