...
class C_CanMonProtocolKefex : public C_CanMonProtocolKefexIva {
private:
  uint16_t mu16_KfxBaseID;

  QString m_MessageToStringKefex(const stw::can::T_STWCAN_Msg_RX &orc_Msg);
  QString m_KfxIndexAndErrorToString(
      const char_t *const opcn_Text, const uint16_t ou16_Index,
      const uint16_t ou16_Error, const bool oq_IsKefexVarIndex = true) const;
  QString m_KfxTextAndValueToString(const char_t *const opcn_Text,
