...
/*!
   \file
   \brief       CAN message to text conversion class for the STW Flashloader
   protocol
...
static const uint8_t
    mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_PROTOCOL_VERSION = 0x01;
static const uint8_t
    mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_SPECIAL_SERVICES = 0x02;
...
static const uint8_t mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_FLASH_SERVICES =
    0x05;
static const uint8_t
    mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_TARGET_SPECIFIC_SERVICES = 0x06;
...
C_CanMonProtocolXfl::C_CanMonProtocolXfl(void)
    : C_CanMonProtocolBase(), mu32_XFLSendId(0x51U) {}

//---------------------------------------------------------------------------

uint32_t C_CanMonProtocolXfl::GetSendId(void) const { return mu32_XFLSendId; }
...
*/
//-----------------------------------------------------------------------------
int32_t C_CanMonProtocolXfl::SaveParamsToIni(QSettings &orc_IniFile,
                                             const QString &orc_Section) {
  int32_t s32_Error = C_NO_ERR;
  QString c_Section = orc_Section;

  try {
    orc_IniFile.setValue(c_Section + "/PP_XFL_SEND_ID",
...
*/
//-----------------------------------------------------------------------------
int32_t C_CanMonProtocolXfl::LoadParamsFromIni(QSettings &orc_IniFile,
                                               const QString &orc_Section) {
  QString c_Section = orc_Section;
  this->mu32_XFLSendId = static_cast<uint32_t>(
      orc_IniFile
          .value(c_Section + "/PP_XFL_SEND_ID",
                 static_cast<int32_t>(this->mu32_XFLSendId))
          .toUInt());
  return C_NO_ERR;
}
...
//-----------------------------------------------------------------------------
QString
C_CanMonProtocolXfl::MessageToString(const T_STWCAN_Msg_RX &orc_Msg) const {
  int32_t s32_Index;
  QString c_Text;
  QString c_Help;
  QString c_Help2;
  ...
  if (orc_Msg.u32_ID == mu32_XFLSendId) {
    if ((orc_Msg.u8_DLC == 5U) && (orc_Msg.au8_Data[0] == 0x46U) &&
        (orc_Msg.au8_Data[1] == 0x4CU) && (orc_Msg.au8_Data[2] == 0x41U) &&
        (orc_Msg.au8_Data[3] == 0x53U) && (orc_Msg.au8_Data[4] == 0x48U)) {
      c_Text = "REQ \"FLASH\"";
      return c_Text;
    }
    ...
    case mu8_XFL_CMD_GRP_EE_COMMAND:
      ...
      {
        c_Help = "ee_read NUM: " + m_GetValueDecHex(orc_Msg.au8_Data[3]) +
                 " ADD: " +
    ...
    case 0x12U:
      ...
      {
        c_Help = "ee_read_word NUM: " + m_GetValueDecHex(orc_Msg.au8_Data[3]) +
                 " ADD: " +
                 m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]));
      } else {
        c_Help = ""; // no other sub-commands defined yet
    ...
    case mu8_XFL_CMD_GRP_GET_COMMAND:
      ...
      case mu8_XFL_CMD_GET_SEC_MODE_COMPARE:
        switch (orc_Msg.u8_DLC) {
        case 4U:
          c_Help = "get_sec_mode_compare SEC:" +
                   m_GetValueDecHex(orc_Msg.au8_Data[3]);
          break;
        case 5U:
          c_Help =
              "get_sec_mode_compare SEC:" +
              m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
          break;
        default:
          c_Help = "";
      ...
      case mu8_XFL_CMD_GET_TIMEOUT_FACTOR:
        ...
        c_Help =
            "get_timeout_factor TYPE:" + m_GetValueDecHex(orc_Msg.au8_Data[3]);
        break;
      ...
      case mu8_XFL_CMD_GET_FLASH_INFORMATION:
        ...
        default:
          c_Help = c_Help +
                   "undefined_index:" + QString::number(orc_Msg.au8_Data[3]);
          break;
        }
        if (c_Help2 == "") {
          switch (orc_Msg.u8_DLC) {
      ...
      case mu8_XFL_CMD_GET_DEVICE_INFO_ADDRESS:
        ...
        c_Help = "get_device_info_address IDX:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]);
        break;
      case mu8_XFL_CMD_GET_BLOCK_START_ADDRESS:
        ...
        c_Help = "get_block_start_address BLK:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]);
        break;
      case mu8_XFL_CMD_GET_BLOCK_END_ADDRESS:
        ...
        c_Help = "get_block_end_address BLK:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]);
        break;
      case mu8_XFL_CMD_GET_BLOCK_CHECKSUM:
        ...
        c_Help =
            "get_block_checksum BLK:" + m_GetValueDecHex(orc_Msg.au8_Data[3]) +
            " IDX:" + m_GetValueDecHex(orc_Msg.au8_Data[4]);
        break;
      case mu8_XFL_CMD_GET_BLOCK_COMPARE_MODE:
        ...
        c_Help = "get_block_compare_mode BLK:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]);
        break;
      default:
        c_Help = "";
    ...
    case mu8_XFL_CMD_GRP_SET_COMMAND:
      ...
      case mu8_XFL_CMD_SET_BITRATE_CAN:
        ...
        case 5U:
          c_Help =
              "set_bitrate_can BITRATE:" +
              m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
          break;
        case 7U:
          c_Help =
              "set_bitrate_can BITRATE:" +
              m_GetValueDecHex(mh_BytesToDwordLowHigh(&orc_Msg.au8_Data[3]));
          break;
        default:
          c_Help = "";
          break;
      ...
      case mu8_XFL_CMD_SET_SEC_MODE_COMPARE:
        switch (orc_Msg.u8_DLC) {
        case 5U:
          c_Help = "set_sec_mode_compare SEC:" +
                   m_GetValueDecHex(orc_Msg.au8_Data[3]) +
                   " MODE:" + QString::number(orc_Msg.au8_Data[4]);
          break;
        case 6U:
          c_Help =
              "set_sec_mode_compare SEC:" +
              m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3])) +
      ...
      case mu8_XFL_CMD_SET_TIMEOUT_FACTOR:
        ...
        c_Help =
            "set_timeout_factor TYPE:" + QString::number(orc_Msg.au8_Data[3]) +
      ...
      case mu8_XFL_CMD_SET_DIVERT_PARAM:
        ...
        c_Help = "set_gateway_parameter DEV:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]) +
                 " POS:" + m_GetValueDecHex(orc_Msg.au8_Data[4]) +
                 " IDX:" + m_GetValueDecHex(orc_Msg.au8_Data[5]) + " PAR:" +
      ...
      case mu8_XFL_CMD_SET_FINGER_PRINT:
        ...
        c_Help =
            "set_finger_print IDX:" + m_GetValueDecHex(orc_Msg.au8_Data[3]) +
      ...
      case mu8_XFL_CMD_SET_TEMP_BITRATE:
        ...
        c_Help = "set_temp_bitrate BITRATE:" +
                 QString::number(mh_BytesToDwordLowHigh(&orc_Msg.au8_Data[3]));
        break;
      case mu8_XFL_CMD_SET_BLOCK_START_ADDRESS:
        ...
        c_Help = "set_block_start_address BLK:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]) + " ADD:" +
      ...
      case mu8_XFL_CMD_SET_BLOCK_END_ADDRESS:
        ...
        c_Help = "set_block_end_address BLK:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]) + " ADD:" +
                 m_GetValueDecHex(mh_BytesToDwordLowHigh(&orc_Msg.au8_Data[4]));
        break;
      case mu8_XFL_CMD_SET_BLOCK_CHECKSUM:
        ...
        c_Help =
            "set_block_checksum BLK:" + m_GetValueDecHex(orc_Msg.au8_Data[3]);
        break;
      case mu8_XFL_CMD_SET_BLOCK_COMPARE_MODE:
        ...
        c_Help = "set_block_compare_mode BLK:" +
                 m_GetValueDecHex(orc_Msg.au8_Data[3]) +
    ...
    case mu8_XFL_CMD_GRP_FLASH_COMMAND:
      ...
      case mu8_XFL_CMD_NODE_COMPID:
        ...
        case 5: // 2 bytes
        case 8: // 5 bytes
          s32_Return = CID_bytes_to_string(
              &orc_Msg.au8_Data[3], static_cast<uint8_t>(orc_Msg.u8_DLC - 3),
              &acn_CIDBuffer[0]);
          if (s32_Return == C_NO_ERR) {
            c_Help = QString::asprintf("node_compid ID:%s", acn_CIDBuffer);
          } else {
      ...
      case mu8_XFL_CMD_BB_WAKEUP:
        ...
        if (mq_Decimal == true) {
          c_Help = QString::asprintf(
              "wakeup_divert_client POS:%d UID:%d", orc_Msg.au8_Data[3],
              mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]));
        } else {
          c_Help = QString::asprintf(
              "wakeup_divert_client POS:%02X UID:%04X", orc_Msg.au8_Data[3],
              mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]));
        }
        break;
      case 0x09U:
        ...
        c_Help =
            "read_flash_word NUM: " + m_GetValueDecHex(orc_Msg.au8_Data[2]) +
            " ADD: " +
            m_GetValueDecHex(mh_BytesToDwordLowHigh(&orc_Msg.au8_Data[3]));
        break;
      default:
    ...
    case mu8_XFL_CMD_GRP_NODE_WAKEUP:
      if (orc_Msg.u8_DLC == 2U) {
        ...
        c_Help = "node_wakeup SNR:" +
                 this->mh_SerialNumberToString(&orc_Msg.au8_Data[2]);
      }
      break;
    ...
    case 0x12U: // ee_read_word
      ...
      c_Help = "ee_read_word NUM: " + m_GetValueDecHex(orc_Msg.au8_Data[2]) +
               " ADD: " +
               m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
      break;
    ...
    case mu8_XFL_CMD_GRP_GET_COMMAND: // get command
      ...
      case mu8_XFL_CMD_GET_SECTOR_COUNT: // get_sector_count
        ...
        case 4U:
          c_Help =
              "get_sector_count NUM:" + m_GetValueDecHex(orc_Msg.au8_Data[3]);
          break;
        case 5U:
          c_Help =
              "get_sector_count NUM:" +
              m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3]));
          break;
        default:
          c_Help = "";
      ...
      case mu8_XFL_CMD_GET_VERSION_NUMBER: // get_version_number
        if (orc_Msg.u8_DLC == 3U) {
          c_Help =
              "get_version_number VER:" + m_GetValueDecHex(orc_Msg.au8_Data[3]);
        } else {
          if (orc_Msg.u8_DLC != 8U) {
            c_Help = "";
            break;
          ...
          c_Help = QString::asprintf(
              "get_version_number VER: %c.%c%c/%d", orc_Msg.au8_Data[3],
              orc_Msg.au8_Data[4], orc_Msg.au8_Data[5],
              (static_cast<uint16_t>(
                  (static_cast<uint16_t>(orc_Msg.au8_Data[6])) << 8U)) +
                  orc_Msg.au8_Data[7]);
        }
        break;
      case mu8_XFL_CMD_GET_ERASE_COUNT: // get_download_count
        ...
        c_Help = "get_download_count COUNT:" +
                 m_GetValueDecHex(static_cast<uint32_t>(
                     orc_Msg.au8_Data[3] +
      ...
      case mu8_XFL_CMD_GET_DEVICE_ID: // get_device_id
        // Some ECUs have a shorter DLC if (orc_Msg.bDLC != 8) { c_Help = "";
        // break; } There is no way we can know whether this is an indexed or
        ...
        for (s32_Index = 3; s32_Index < orc_Msg.u8_DLC; s32_Index++) {
          if (std::isalnum(orc_Msg.au8_Data[s32_Index]) == 0) {
            c_Help2 =
                "[" + m_GetValueDecHex(orc_Msg.au8_Data[s32_Index]) + "] ";
          } else {
            c_Help2 = QString::asprintf("'%c' ", orc_Msg.au8_Data[s32_Index]);
          }
          c_Help += c_Help2;
        }
        break;
      ...
      case mu8_XFL_CMD_GET_SEC_CRC: // get_sec_crc
        ...
        c_Help = "get_sec_crc CRCCALC:" +
                 m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[3])) +
                 " CRCEE:" +
                 m_GetValueDecHex(mh_BytesToWordLowHigh(&orc_Msg.au8_Data[5]));
        break;
      ...
      case mu8_XFL_CMD_GET_LAST_USER: // get_last_user
        ...
        if (mq_Decimal == true) {
          c_Help =
              QString::asprintf("get_last_user UID:%d",
                                mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]));
        } else {
          c_Help =
              QString::asprintf("get_last_user UID:%04X",
                                mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]));
        }
        break;
      case mu8_XFL_CMD_GET_FLASH_INFORMATION:
        ...
        case mu8_XFL_CMD_GET_FLASH_INFORMATION_IDX_REGION_INFO:
          ...
          if (u16_Help == 0U) {
            ...
            c_Help2 =
                " SIZE: " +
                m_GetValueDecHex((static_cast<uint32_t>(u16_Help)) * 256U) +
                "bytes";
          }
      ...
      case mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION:
        ...
        case mu8_XFL_CMD_GET_IMPLEMENTATION_INFORMATION_PROTOCOL_VERSION:
          c_Help = c_Help + "protocol_version";
          u16_Help = mh_BytesToWordLowHigh(&orc_Msg.au8_Data[4]);
          c_Help2 = QString::asprintf(
              " V%x.%x%xr%x", static_cast<uint8_t>((u16_Help >> 12U) & 0x0FU),
      ...
      case mu8_XFL_CMD_GET_BLOCK_CHECKSUM:
        ...
        c_Help =
            "get_block_checksum IDX:" + m_GetValueDecHex(orc_Msg.au8_Data[3]) +
            " CHK: " +
            m_GetValueDecHex(mh_BytesToDwordLowHigh(&orc_Msg.au8_Data[4]));
        break;
      case mu8_XFL_CMD_GET_BLOCK_COMPARE_MODE:
    ...
    case mu8_XFL_CMD_GRP_GET_SNR: // get_snr
      ...
      c_Help =
          "get_snr SNR:" + this->mh_SerialNumberToString(&orc_Msg.au8_Data[2]);
      break;
    ...
    case mu8_XFL_CMD_GRP_FLASH_COMMAND: // flash commands
      ...
      case mu8_XFL_CMD_NODE_COMPID: // node_compid
        ...
        case 8U: // 5 bytes
          s32_Return = CID_bytes_to_string(
              &orc_Msg.au8_Data[3], static_cast<uint8_t>(orc_Msg.u8_DLC - 3U),
              &acn_CIDBuffer[0]);
          if (s32_Return == C_NO_ERR) {
            c_Help = QString::asprintf("node_compid ID:%s", acn_CIDBuffer);
          } else {
    ...
    case mu8_COMMAND_ERR: // error
      if (orc_Msg.u8_DLC == 2U) {
        ...
        if ((orc_Msg.au8_Data[2] >= 0x10U) && (orc_Msg.au8_Data[2] <= 0x3FU)) {
          c_Help =
              "ERR: devicespecific " + m_GetValueDecHex(orc_Msg.au8_Data[2]);
        } else {
          switch (orc_Msg.au8_Data[2]) {
          ...
          case mu8_XFL_ERR_WRONG_REC_TYPE:
            ...
            break;
          default:
            c_Help = "ERR: unknown error (" +
                     QString::number(orc_Msg.au8_Data[2]) + "d)";
            break;
          }
        }
      }
...
*/
//-----------------------------------------------------------------------------
QString C_CanMonProtocolXfl::GetProtocolName(void) const {
