...
public:
  ...
  void DataTypeToTextAndSize(QString *const opc_Text,
                             uint8_t *const opu8_Size) const;
...
private:
  int32_t m_CheckForExistingObjects(const QString &orc_Blockname,
                                    QSettings &orc_IniFile);
  int32_t m_GetObjectDescription(const uint16_t ou16_Index,
  ...
  QString mc_LastError;
  uint32_t mu32_OriginalFileHash;

  int32_t m_IsSectionRo(const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx,
                        const uint8_t ou8_OdSubIndex, bool &orq_IsRo) const;
  bool m_DoesSectionExist(const uint16_t ou16_PdoIndex,
...
public:
  ...
  int32_t LoadFromFile(const QString &orc_File);
  ...
  bool DoesEventTimerSectionExist(const uint16_t ou16_PdoIndex,
                                  const bool oq_MessageIsTx) const;
  int32_t IsCobIdRo(const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx,
                    bool &orq_IsRo) const;
  int32_t IsInhibitTimeRo(const uint16_t ou16_PdoIndex,
                          const bool oq_MessageIsTx, bool &orq_IsRo) const;
  int32_t IsEventTimerRo(const uint16_t ou16_PdoIndex,
                         const bool oq_MessageIsTx, bool &orq_IsRo) const;
  int32_t IsTransmissionTypeRo(const uint16_t ou16_PdoIndex,
                               const bool oq_MessageIsTx, bool &orq_IsRo) const;
  int32_t IsSyncStartRo(const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx,
                        bool &orq_IsRo) const;
  int32_t IsPdoMappingRo(const uint16_t ou16_PdoIndex,
                         const bool oq_MessageIsTx, bool &orq_IsRo) const;

  // Util
  const C_OscCanOpenObjectData *
  GetCanOpenObject(const uint16_t ou16_OdIndex) const;
  const C_OscCanOpenObjectData *
  GetCanOpenSubIndexObject(const uint16_t ou16_OdIndex,
                           const uint8_t ou8_OdSubIndex) const;
