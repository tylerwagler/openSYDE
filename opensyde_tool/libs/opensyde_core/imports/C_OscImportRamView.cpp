...
/*! \brief  Load RAMView .def project and import lists to openSYDE Data Pool
   ...
   * Basic project options
   ...
   The caller can select which types of RAMView lists to import using
   orc_ProjectPath::e_Type:
   ...
   * c_Lists: will contain the actual list and element definition grabbed from
   the RAMView project
   ...
   * u32_NvmSize: will be set to the number of bytes occupied by the sum of all
   imported NVM elements (only for "eNVM")

   Other elements will not be touched.

   Error reporting:
   When loading the RAMView project we assume we have correct project files that
   load ok in RAMView. Details about errors when loading the project will be
   logged using the OSC logger.

   Reporting interface:
   One String list with zero to many information string which we can use to
   inform the user on compromises made that were needed as RAMView and openSYDE
   data structures are not 100% compatible.

   \param[in]     orc_ProjectPath        Full path to .def file
   \param[in,out] orc_DataPool           Loaded Data Pool definition
   \param[out]    orc_ImportInformation  Textual information about issues when
   importing
...
int32_t C_OscImportRamView::h_ImportDataPoolFromRamViewDefProject(
  ...
  if (s32_Result == C_NO_ERR) {
    s32_Result = mh_LoadRamViewDefProject(orc_ProjectPath, c_ProjectOptions,
                                          c_VariableLists);
  }
  ...
  if (s32_Result == C_NO_ERR) {
    ...
    if (orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) {
      const C_SclString c_Info = "When importing RAMView EEPROM lists the "
    ...
    C_OscImportRamView::mh_AdaptName(orc_DataPool.c_Name,
                                     orc_DataPool.c_Comment, "Datapool",
                                     orc_ImportInformation);

    orc_DataPool.au8_Version[0] = static_cast<uint8_t>(
        (c_ProjectOptions.u16_DataVersion & 0xF000U) >> 12U);
    orc_DataPool.au8_Version[1] = static_cast<uint8_t>(
        (c_ProjectOptions.u16_DataVersion & 0x0FF0U) >> 4U);
    orc_DataPool.au8_Version[2] =
        static_cast<uint8_t>(c_ProjectOptions.u16_DataVersion & 0x000FU);

    // copy over lists contents:
    orc_DataPool.c_Lists.clear();

    for (uint16_t u16_ListRamView = 0U;
         u16_ListRamView < c_VariableLists.size(); u16_ListRamView++) {
      // check for correct list type:
      if (((orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) &&
           (c_VariableLists[u16_ListRamView].u8_ListType ==
            KFX_VARIABLE_TYPE_EEPROM)) ||
          ((orc_DataPool.e_Type == C_OscNodeDataPool::eDIAG) &&
           (c_VariableLists[u16_ListRamView].u8_ListType ==
            KFX_VARIABLE_TYPE_RAM))) {
        ...
        C_OscImportRamView::mh_AdaptName(c_List.c_Name, c_List.c_Comment,
                                         "list", orc_ImportInformation);

        // Dataset information
        c_List.c_DataSets.resize(
            c_VariableLists[u16_ListRamView].GetNumDefaults());
        // In RAMView we do not have names of sets per list but as a basic
        // project setting. Adopt names of named sets:
        for (uint16_t u16_DataSet = 0U; u16_DataSet < c_List.c_DataSets.size();
             u16_DataSet++) {
          ...
          if (u16_DataSet < c_VariableLists.ac_DefaultNames.size()) {
            ...
            rc_DataSet.c_Name =
                "Dataset_" +
                QString::number(static_cast<uint32_t>(u16_DataSet) + 1U);
          }
          rc_DataSet.c_Comment = ""; // no comments supported in RAMView

          // make sure the name is openSYDE compliant:
          C_OscImportRamView::mh_AdaptName(rc_DataSet.c_Name,
                                           rc_DataSet.c_Comment, "Dataset",
                                           orc_ImportInformation);
        }

        // setting specific to the type of list resp. Datapool:
        if (orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) {
          c_List.q_NvmCrcActive =
              c_VariableLists[u16_ListRamView].q_CheckSummed;
        } else {
        ...
        // Elements
        // Note that the type "KFX_DATA_TYPE_CRC" is only used in code generated
        // by RAMView. Such variables are not part of the RAMView project. So
        // there is no need to handle those when loading the project.
        ...
        c_List.c_Elements.resize(
            c_VariableLists[u16_ListRamView].VariableList.size());

        for (uint32_t u32_ElementIndex = 0U;
             u32_ElementIndex <
          ...
          C_OscNodeDataPoolListElement &rc_Element =
              c_List.c_Elements[u32_ElementIndex];
          ...
          // comment for each RAMView comment set
          //  with pattern: <symbolic_name>: <comment>\n
          for (uint8_t u8_CommentIndex = 0U;
               u8_CommentIndex < KFX_DATA_MAX_NUM_LANGUAGES;
               u8_CommentIndex++) {
          ...
          if (u8_NumCommentSets <= 1U) {
            ...
            for (uint8_t u8_CommentIndex = 0U;
                 u8_CommentIndex < u8_NumCommentSets; u8_CommentIndex++) {
              c_Comment +=
                  (c_ProjectOptions.ac_CommentDescriptions[u8_CommentIndex] +
          ...
          // make sure the name is openSYDE compliant:
          C_OscImportRamView::mh_AdaptName(rc_Element.c_Name,
                                           rc_Element.c_Comment, "element",
                                           orc_ImportInformation);
          ...
          // set type:
          s32_TypeSetResult =
              mh_ImportElementType(rc_ElementRamView, rc_Element);
          ...
          if ((rc_ElementRamView.IsArrayType() == true) &&
            ...
            const C_SclString c_Info = "Variable \"" + c_List.c_Name + "." +
                                       rc_Element.c_Name +
          ...
          // RAMView factor is decimal to reduce need for FPU operation (10000 =
          // 1)
          rc_Element.f64_Factor =
          ...
          if (orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) {
            // Apply the openSYDE tool default permissions depending on the DP
            // type.
          ...
          // Location "FUNCTION" in RAMView translates to "Event" flag in
          // openSYDE
          rc_Element.q_DiagEventCall =
              (rc_ElementRamView.q_LocationRAM == false);

          // Special handling if element is of type "array of sint8" and the
          // Dataset value is not zero
          //  "terminated". In RAMView this type is used to describe a visible
          //  string.
          // In openSYDE we always assume additional space for the "\0"
          // terminator is required. So in this case we replace the final
          // character by '\0' and add a report string.
          if (rc_ElementRamView.u8_Type == KFX_DATA_TYPE_ASINT8) {
            for (uint16_t u16_DataSetIndex = 0U;
                 u16_DataSetIndex < rc_ElementRamView.GetNumDefaults();
                 u16_DataSetIndex++) {
              if (rc_ElementRamView.GetStringDefault(u16_DataSetIndex)
                      .Length() == rc_ElementRamView.GetSize()) {
                // filled up to the max -> patch it
                const C_SclString c_Info =
                    "Datapool element \"" + c_List.c_Name + "." +
                    rc_Element.c_Name + "\" is a string array. Dataset \"" +
          ...
          // RAMView projects have a clear logic: arrays of sint8 are generally
          // interpreted as strings
          rc_Element.q_InterpretAsString =
              ((rc_Element.GetType() == C_OscNodeDataPoolContent::eSINT8) &&
    ...
    if (orc_DataPool.e_Type == C_OscNodeDataPool::eNVM) {
      // Set NvmSize to the number of bytes occupied by the sum of all imported
      // NVM elements:
      for (uint16_t u16_List = 0U; u16_List < orc_DataPool.c_Lists.size();
           u16_List++) {
...
/*! \brief  Utility to set openSYDE type based on RAMView type
   ...
   * Number of array elements
   This will also preset the types for the "Content" elements for
   min/max/value/datasets

   \param[in]     orc_RamViewElement          KEFEX RAMView variable element
   \param[in,out] orc_Element                 openSYDE element with updated
   information
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscImportRamView::mh_ImportElementType(
...
/*! \brief  Utility to import min/max values
   ...
   The functions expects that the type of orc_Element is already set to match
   the type of orc_RamViewElement.
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscImportRamView::mh_ImportElementMinMax(
    const C_KFXVariableBase &orc_RamViewElement,
    C_OscNodeDataPoolListElement &orc_Element) {
  if (orc_Element.GetArray() == false) {
    switch (orc_Element.GetType()) {
    case C_OscNodeDataPoolContent::eUINT8:
      orc_Element.c_MinValue.SetValueU8(
          static_cast<uint8_t>(orc_RamViewElement.GetNumericMin()));
      orc_Element.c_MaxValue.SetValueU8(
          static_cast<uint8_t>(orc_RamViewElement.GetNumericMax()));
      break;
    case C_OscNodeDataPoolContent::eUINT16:
      orc_Element.c_MinValue.SetValueU16(
          static_cast<uint16_t>(orc_RamViewElement.GetNumericMin()));
      orc_Element.c_MaxValue.SetValueU16(
          static_cast<uint16_t>(orc_RamViewElement.GetNumericMax()));
      break;
    case C_OscNodeDataPoolContent::eUINT32:
      orc_Element.c_MinValue.SetValueU32(
          static_cast<uint32_t>(orc_RamViewElement.GetNumericMin()));
      orc_Element.c_MaxValue.SetValueU32(
          static_cast<uint32_t>(orc_RamViewElement.GetNumericMax()));
      break;
    case C_OscNodeDataPoolContent::eSINT8:
      orc_Element.c_MinValue.SetValueS8(
          static_cast<int8_t>(orc_RamViewElement.GetNumericMin()));
      orc_Element.c_MaxValue.SetValueS8(
          static_cast<int8_t>(orc_RamViewElement.GetNumericMax()));
      break;
    case C_OscNodeDataPoolContent::eSINT16:
      orc_Element.c_MinValue.SetValueS16(
          static_cast<int16_t>(orc_RamViewElement.GetNumericMin()));
      orc_Element.c_MaxValue.SetValueS16(
          static_cast<int16_t>(orc_RamViewElement.GetNumericMax()));
      break;
    case C_OscNodeDataPoolContent::eSINT32:
      orc_Element.c_MinValue.SetValueS32(
          static_cast<int32_t>(orc_RamViewElement.GetNumericMin()));
      orc_Element.c_MaxValue.SetValueS32(
          static_cast<int32_t>(orc_RamViewElement.GetNumericMax()));
      break;
    ...
    case C_OscNodeDataPoolContent::eFLOAT32:
      orc_Element.c_MinValue.SetValueF32(
          static_cast<float32_t>(orc_RamViewElement.GetFloatMin()));
      orc_Element.c_MaxValue.SetValueF32(
          static_cast<float32_t>(orc_RamViewElement.GetFloatMax()));
      break;
    ...
    case C_OscNodeDataPoolContent::eUINT8:
      for (uint32_t u32_ArrayIndex = 0U;
        ...
        orc_Element.c_MinValue.SetValueArrU8Element(
            static_cast<uint8_t>(
                orc_RamViewElement.GetNumericMinFromArray(u32_ArrayIndex)),
            u32_ArrayIndex);
        orc_Element.c_MaxValue.SetValueArrU8Element(
            static_cast<uint8_t>(
                orc_RamViewElement.GetNumericMaxFromArray(u32_ArrayIndex)),
            u32_ArrayIndex);
      }
      break;
    case C_OscNodeDataPoolContent::eUINT16:
      ...
      for (uint32_t u32_ArrayIndex = 0U;
        ...
        orc_Element.c_MinValue.SetValueArrF32Element(
            static_cast<float32_t>(
                orc_RamViewElement.GetFloatMinFromArray(u32_ArrayIndex)),
            u32_ArrayIndex);
        orc_Element.c_MaxValue.SetValueArrF32Element(
            static_cast<float32_t>(
                orc_RamViewElement.GetFloatMaxFromArray(u32_ArrayIndex)),
            u32_ArrayIndex);
      }
      break;
    case C_OscNodeDataPoolContent::eFLOAT64:
...
/*! \brief  Utility to import Dataset values
   ...
   * number of Datasets in orc_Elements already matches the number in
   orc_RamViewElement
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscImportRamView::mh_ImportElementDataSetValues(
    const C_KFXVariableBase &orc_RamViewElement,
    C_OscNodeDataPoolListElement &orc_Element) {
  for (uint16_t u16_DataSetIndex = 0U;
       u16_DataSetIndex < orc_RamViewElement.GetNumDefaults();
       u16_DataSetIndex++) {
    C_OscNodeDataPoolContent &rc_Content =
        orc_Element.c_DataSetValues[u16_DataSetIndex];

    if (orc_Element.GetArray() == false) {
      switch (orc_Element.GetType()) {
      case C_OscNodeDataPoolContent::eUINT8:
        rc_Content.SetValueU8(static_cast<uint8_t>(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eUINT16:
        rc_Content.SetValueU16(static_cast<uint16_t>(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eUINT32:
        rc_Content.SetValueU32(static_cast<uint32_t>(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eSINT8:
        rc_Content.SetValueS8(static_cast<int8_t>(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eSINT16:
        rc_Content.SetValueS16(static_cast<int16_t>(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eSINT32:
        rc_Content.SetValueS32(static_cast<int32_t>(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eSINT64:
        rc_Content.SetValueS64(
            orc_RamViewElement.GetNumericDefault(u16_DataSetIndex));
        break;
      case C_OscNodeDataPoolContent::eFLOAT32:
        rc_Content.SetValueF32(static_cast<float32_t>(
            orc_RamViewElement.GetFloatDefault(u16_DataSetIndex)));
        break;
      case C_OscNodeDataPoolContent::eFLOAT64:
        rc_Content.SetValueF64(
            orc_RamViewElement.GetFloatDefault(u16_DataSetIndex));
        break;
      case C_OscNodeDataPoolContent::eUINT64:
      default:
      ...
      case C_OscNodeDataPoolContent::eUINT8:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrU8Element(
              static_cast<uint8_t>(
                  orc_RamViewElement.GetNumericDefaultFromArray(
      ...
      case C_OscNodeDataPoolContent::eUINT16:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrU16Element(
              static_cast<uint16_t>(
                  orc_RamViewElement.GetNumericDefaultFromArray(
                      u32_ArrayIndex, u16_DataSetIndex)),
              u32_ArrayIndex);
      ...
      case C_OscNodeDataPoolContent::eUINT32:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrU32Element(
              static_cast<uint32_t>(
                  orc_RamViewElement.GetNumericDefaultFromArray(
                      u32_ArrayIndex, u16_DataSetIndex)),
              u32_ArrayIndex);
      ...
      case C_OscNodeDataPoolContent::eSINT8:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrS8Element(
              static_cast<int8_t>(orc_RamViewElement.GetNumericDefaultFromArray(
                  u32_ArrayIndex, u16_DataSetIndex)),
              u32_ArrayIndex);
        }
        break;
      case C_OscNodeDataPoolContent::eSINT16:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrS16Element(
              static_cast<int16_t>(
                  orc_RamViewElement.GetNumericDefaultFromArray(
                      u32_ArrayIndex, u16_DataSetIndex)),
              u32_ArrayIndex);
      ...
      case C_OscNodeDataPoolContent::eSINT32:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrS32Element(
              static_cast<int32_t>(
                  orc_RamViewElement.GetNumericDefaultFromArray(
                      u32_ArrayIndex, u16_DataSetIndex)),
              u32_ArrayIndex);
      ...
      case C_OscNodeDataPoolContent::eSINT64:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrS64Element(
              orc_RamViewElement.GetNumericDefaultFromArray(u32_ArrayIndex,
                                                            u16_DataSetIndex),
              u32_ArrayIndex);
        }
        break;
      case C_OscNodeDataPoolContent::eFLOAT32:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrF32Element(
              static_cast<float32_t>(
                  orc_RamViewElement.GetFloatDefaultFromArray(
                      u32_ArrayIndex, u16_DataSetIndex)),
              u32_ArrayIndex);
        }
        break;
      case C_OscNodeDataPoolContent::eFLOAT64:
        for (uint32_t u32_ArrayIndex = 0U;
             u32_ArrayIndex < orc_Element.GetArraySize(); u32_ArrayIndex++) {
          rc_Content.SetValueArrF64Element(
              orc_RamViewElement.GetFloatDefaultFromArray(u32_ArrayIndex,
                                                          u16_DataSetIndex),
              u32_ArrayIndex);
        }
        break;
...
/*! \brief  Load RAMView .def project
   ...
   * Basic project options
   ...
   This implementation is based on pre-existing code from KEFEX Diaglib
   (C_KFXProject::OpenProject). The logic was simplified to match the use case
   to import from existing projects in the openSYDE context:
   * no dependency from protocol drivers
   * can only open .def style projects (no .ksp projects)
   * project passwords will be ignored (these are not considered a real
   protection in .def-style projects)

   Error reporting:
   We assume we have correct project files that load ok in RAMView.
   So we do not provide a fancy error detail string reporting engine here. Also
   we only provide one "could not open" error code. But we'll log to the OSC
   logger if there are any problems.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscImportRamView::mh_LoadRamViewDefProject(
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    //.def file format:
    pc_IniFile =
        new QSettings(orc_ProjectPath.ToQString(), QSettings::IniFormat);
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    const C_SclString c_WorkDirectory =
        (QFileInfo(orc_ProjectPath.ToQString()).absolutePath() + "/")
            .toStdString();

    s32_Return = C_KFXDEFProject::LoadRAMFiles(
        c_WorkDirectory, orc_ProjectOptions.c_DeviceName, orc_VariableLists,
    ...
    case C_WARN:
      osc_write_log_error("Loading RAMView project",
                          "No variable lists found in project.");
      s32_Return = C_RD_WR;
      break;
    case C_CONFIG:
      osc_write_log_error("Loading RAMView project",
                          "List index inconsistency.");
      s32_Return = C_RD_WR;
      break;
    case C_RD_WR:
      osc_write_log_error("Loading RAMView project",
    ...
    default:
      osc_write_log_error("Loading RAMView project",
                          "Unexpected error parsing .ram files.");
      s32_Return = C_RD_WR;
      break;
    }
  }
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    case C_NO_ERR:
      break;
    case C_WARN:
      // just a warning; .dat does not exactly match project: continue
      // nevertheless
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    s32_Return = C_KFXDEFProject::LoadComments(
        c_FilePath, orc_ProjectOptions.c_DeviceName, orc_VariableLists,
...
/*! \brief  Utility: Adapt RAMView name to openSYDE conventions
   ...
   Naming rules for project, lists, variables, Datasets in RAMView is more
   relaxed.
   ...
   * Append a note to "orc_Comment" with the original name
   ...
   RAMView prevents entering such names. While it would be technically possible
   to have created project files with invalid characters from another source
   this is considered a minor risk. And even if we have such a scenario the
    names will be flagged in the openSYDE GUI after importing.

   \param[in,out]  orc_Name              in: original name; out: adapted name
   \param[in,out]  orc_Comment           in: original comment; out: comment with
   added "upgrade information" (if needed)
   \param[in]      orc_ElementDesignator Description of the object for which the
   name shall be adapted (for reporting)
   \param[in]      orc_ImportInformation List to which to add reporting
...
void C_OscImportRamView::mh_AdaptName(
  ...
  if ((c_NewName.length() >= 1) && (c_NewName[0].isDigit())) {
    c_NewName = "_" + c_NewName;
  }

  // cut string:
  if (c_NewName.length() > s32_C_ITEM_MAX_CHAR_COUNT) {
    c_NewName.truncate(s32_C_ITEM_MAX_CHAR_COUNT);
  }
