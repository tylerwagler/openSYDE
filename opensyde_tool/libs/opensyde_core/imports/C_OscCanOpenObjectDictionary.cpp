...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanOpenObjectData::operator<(
    const C_OscCanOpenObjectData &orc_Object) const {
  bool q_IsLess = false;
...
void C_OscCanOpenObjectDictionary::m_RememberFileHash() {
  ...
  for (int32_t s32_Line = 0; s32_Line < this->c_TextFileContent.size();
       s32_Line++) {
    stw::scl::C_SclChecksums::CalcCRC32(
        this->c_TextFileContent[s32_Line].toUtf8().constData(),
...
/*! \brief   Load data from CANopen EDS file
   ...
   * go through all sections and extract all object and sub-object descriptions
   into "c_Objects"
...
int32_t C_OscCanOpenObjectDictionary::LoadFromFile(const QString &orc_File) {
  ...
  if (!(QFileInfo(orc_File).exists() && QFileInfo(orc_File).isFile())) {
    ...
    for (int32_t s32_Section = 0U; s32_Section < c_Groups.size();
         s32_Section++) {
      // We are only interested in the sections describing objects or objects
      // with subobjects. All other sections will be ignored.
      ...
      if (c_SectionName.length() == 4) {
        ...
        try {
          const uint16_t u16_Index = static_cast<uint16_t>(
              ("0x" + c_SectionName.mid(0, 4)).toInt(nullptr, 16));
          // create new map entry or use existing depending on sequence of
          // sections in EDS file
          C_OscCanOpenObject &rc_Object = c_OdObjects[u16_Index];

          s32_Return = m_GetObjectDescription(
              u16_Index, 0U, false, c_SectionName, c_IniFile, rc_Object);
        } catch (...) {
          // Do not handle as an error. Malformed entry or valid section with
          // size of 4.
        }
      } else if ((c_SectionName.length() > (4 + 3)) &&
                 (c_SectionName.contains("sub", Qt::CaseInsensitive) &&
                  c_SectionName.indexOf("sub", Qt::CaseInsensitive) == 4)) {
        // Pattern: [<4 hex digits>sub<1 or 2 hex digits>, e.g. [12ABsubCD]
        try {
          const uint16_t u16_Index = static_cast<uint16_t>(
              ("0x" + c_SectionName.mid(0, 4)).toInt(nullptr, 16));
          // 1 or 2 characters:
          const uint8_t u8_SubIndex = static_cast<uint8_t>(
              ("0x" + c_SectionName.mid(7, 2)).toInt(nullptr, 16));

          // create new map entry or use existing
          C_OscCanOpenObject &rc_Object = c_OdObjects[u16_Index];
          // sub-object really should be created here as it should not be
          // present multiple times
          C_OscCanOpenObjectData &rc_SubObject =
              rc_Object.c_SubObjects[u8_SubIndex];

          s32_Return =
              m_GetObjectDescription(u16_Index, u8_SubIndex, true,
                                     c_SectionName, c_IniFile, rc_SubObject);
        } catch (...) {
          // Do not handle as an error. Most probably this is a malformed entry.
          // But there is a very low change this is a kind of valid section like
          // e.g. "Foobsubs" which
...
/*! \brief   Check whether referenced objects exist in file.
   ...
   Loads section "orc_Blockname" and checks whether all referenced objects exist
   in "c_Objects"
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanOpenObjectDictionary::m_CheckForExistingObjects(
    const QString &orc_Blockname, QSettings &orc_IniFile) {
  int32_t s32_Return = C_NO_ERR;

  const QString c_Group = orc_Blockname + "/";
  const uint16_t u16_NumEntries = static_cast<uint16_t>(
      orc_IniFile.value(c_Group + "SupportedObjects", 0).toUInt());

  for (int32_t s32_Loop = 0; s32_Loop < u16_NumEntries; s32_Loop++) {
    const QString c_Directive = QString::number(s32_Loop + 1);
    const QString c_Index =
        orc_IniFile.value(c_Group + c_Directive, "").toString();
    if (c_Index == "") {
      mc_LastError =
          orc_Blockname + ": SupportedObjects inconsistent with object list !";
      s32_Return = C_CONFIG;
    }
    ...
    if (s32_Return == C_NO_ERR) {
      ...
      try {
        ...
        mc_LastError =
            orc_Blockname +
            ": File contains non-numeric Index in SupportedObjects !";
        s32_Return = C_CONFIG;
      }
      ...
      if (s32_Return == C_NO_ERR) {
        ...
        const QHash<uint16_t, C_OscCanOpenObject>::const_iterator c_Object =
            c_OdObjects.find(u16_Index);
        if (c_Object == c_OdObjects.end()) {
          mc_LastError = orc_Blockname + ": References object 0x" +
                         C_OscUtils::h_IntToHex(u16_Index, 4) +
                         "which is not described in the file.";
          s32_Return = C_CONFIG;
        }
      }
    }
...
/*! \brief   Utility: get information from one EDS/DCF section
   ...
   \param[in]     oq_IsSubIndex      true: object is a subindex object; false:
   object is the main thing \param[in]     orc_SectionValues  EDS/DCF section
   contents (in the form of "parameter=value" strings) \param[out]    orc_Object
   read information
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanOpenObjectDictionary::m_GetObjectDescription(
  ...
  // upper case, so we only need to do this call on the side of the file
  // content:
  static const QString ac_StringsToSearchFor[u32_NUM_STRINGS_TO_SEARCH] = {
      "PARAMETERNAME",  "ACCESSTYPE", "SUBNUMBER", "DATATYPE",
      "DEFAULTVALUE",   "LOWLIMIT",   "HIGHLIMIT", "PDOMAPPING",
  ...
  orc_Object.u8_DataType =
      C_OscCanOpenObjectData::hu8_DATA_TYPE_DOMAIN; // optional for "DOMAIN"
                                                    // objects
  ...
  for (int32_t s32_Key = 0; s32_Key < c_Keys.size(); s32_Key++) {
    ...
    for (uint32_t u32_StringIndex = 0U;
         u32_StringIndex < u32_NUM_STRINGS_TO_SEARCH; u32_StringIndex++) {
      if ((aq_StringsAlreadyFound[u32_StringIndex] == false) &&
        ...
        aq_StringsAlreadyFound[u32_StringIndex] =
            true; // no need to string compare this one again
        ...
        case 2:
          try {
            ...
            mc_LastError = QString::asprintf(
                "File contains non-numeric SubNumber for object %04X.%02X !",
        ...
        case 3:
          try {
            ...
            mc_LastError = QString::asprintf(
                "File contains non-numeric DataType for object %04X.%02X !",
                static_cast<uint32_t>(ou16_Index),
                static_cast<uint32_t>(ou8_SubIndex));
            s32_Return = C_CONFIG;
          }
        ...
        case 7:
          try {
            ...
            if (s32_Value == 0) {
              ...
              orc_Object.q_IsMappableIntoPdo = false;
              mc_LastError = QString::asprintf(
                  "Invalid boolean value \"%s\" found in entry \"PDOMapping\" "
                  "for object %04X.%02X !",
            ...
            mc_LastError =
                QString::asprintf("Could not parse entry \"PDOMapping\" value "
                                  "\"%s\" for object %04X.%02X !",
                                  rc_Value.toUtf8().constData(),
                                  static_cast<uint32_t>(ou16_Index),
                                  static_cast<uint32_t>(ou8_SubIndex));
            s32_Return = C_CONFIG;
...
/*! \brief  Is section read-only
   ...
   \param[in]   oq_MessageIsTx   Flag if message is TX (for CANopen manager
   usage this flag is inverse) \param[in]   ou8_OdSubIndex   Object dictionary
   sub index \param[out]  orq_IsRo         Is read-only
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanOpenObjectDictionary::m_IsSectionRo(
    const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx,
  ...
  const C_OscCanOpenObjectData *const pc_Object =
      this->GetCanOpenSubIndexObject(u16_ObjectIndex, ou8_OdSubIndex);
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanOpenObjectDictionary::m_DoesSectionExist(
    const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx,
...
uint16_t C_OscCanOpenObjectData::GetSize(void) const { return this->mu16_Size; }
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanOpenObjectData::DataTypeToTextAndSize(
    QString *const opc_Text, uint8_t *const opu8_Size) const {
  if ((u8_DataType < mu8_NUM_DATA_TYPES) &&
      (u8_DataType > hu8_DATA_TYPE_INVALID)) {
    if (opc_Text != NULL) {
...
C_OscCanOpenObjectData::C_OscCanOpenObjectData()
    : mu16_Size(0), u16_Index(0), u8_SubIndex(0), u8_DataType(0), u8_NumSubs(0),
...
bool C_OscCanOpenObjectData::IsReadable(void) const {
  ...
  return ((c_Help == "RO") || (c_Help == "RW") || (c_Help == "RWW") ||
          (c_Help == "RWR") || (c_Help == "CONST"));
}
...
bool C_OscCanOpenObjectData::IsWriteable(void) const {
  ...
  return ((c_Help == "RW") || (c_Help == "RWW") || (c_Help == "RWR") ||
          (c_Help == "WO"));
}
...
bool C_OscCanOpenObjectData::IsMappableIntoTxPdo() const {
  ...
  return this->IsMappableIntoPdo() &&
         ((c_Help == "RO") || (c_Help == "RW") || (c_Help == "RWR") ||
          (c_Help == "CONST"));
}
...
bool C_OscCanOpenObjectData::IsMappableIntoRxPdo() const {
  ...
  return this->IsMappableIntoPdo() &&
         ((c_Help == "WO") || (c_Help == "RW") || (c_Help == "RWW"));
}
...
void C_OscCanOpenObjectDictionary::CalcHash(uint32_t &oru32_HashValue) const {
  C_SclChecksums::CalcCRC32(&this->mu32_OriginalFileHash,
                            sizeof(this->mu32_OriginalFileHash),
                            oru32_HashValue);

  this->c_InfoBlock.CalcHash(oru32_HashValue);

   for (QHash<uint16_t, C_OscCanOpenObject>::const_iterator c_Element =
            this->c_OdObjects.begin();
        c_Element != this->c_OdObjects.end(); ++c_Element) {
     c_Element.value().CalcHash(oru32_HashValue);
   }
}
...
uint8_t C_OscCanOpenObjectDictionary::GetNumHeartbeatConsumers() const {
  uint8_t u8_Retval = 0;
  const C_OscCanOpenObjectData *const pc_Object =
      this->GetCanOpenObject(hu16_OD_INDEX_HEARTBEAT_CONSUMER);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsHeartbeatConsumerRo(bool &orq_IsRo) const {
  int32_t s32_Retval = C_NO_ERR;
  // Checking the first consumer for read only because this is the entry which
  // will be used for the user specified value when it is not read-only
  const C_OscCanOpenObjectData *const pc_Object =
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsHeartbeatProducerRo(bool &orq_IsRo) const {
  int32_t s32_Retval = C_NO_ERR;
  const C_OscCanOpenObjectData *const pc_Object =
      this->GetCanOpenObject(hu16_OD_INDEX_HEARTBEAT_PRODUCER);
...
//----------------------------------------------------------------------------------------------------------------------
QSet<uint8_t>
C_OscCanOpenObjectDictionary::GetAllAvailableFactorySettingsSubIndices() const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
QSet<uint8_t> C_OscCanOpenObjectDictionary::GetAllAvailableSubIndices(
    const uint16_t ou16_OdIndex) const {
  QSet<uint8_t> c_Retval;

  const QHash<uint16_t, C_OscCanOpenObject>::const_iterator c_Object =
      c_OdObjects.find(ou16_OdIndex);
  if (c_Object != c_OdObjects.end()) {
   for (QHash<uint8_t, C_OscCanOpenObjectData>::const_iterator c_Element =
            c_Object.value().c_SubObjects.begin();
        c_Element != c_Object.value().c_SubObjects.end(); ++c_Element) {
       if (c_Element.value().u8_NumSubs ==
           C_OscCanOpenObjectData::hu8_NUM_SUBS_WE_ARE_A_SUB) {
         c_Retval.insert(c_Element.value().u8_SubIndex);
       }
    }
  }
  return c_Retval;
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanOpenObjectDictionary::DoesInhibitTimeSectionExist(
    const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx) const {
  return this->m_DoesSectionExist(
      ou16_PdoIndex, oq_MessageIsTx,
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanOpenObjectDictionary::DoesEventTimerSectionExist(
    const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx) const {
  return this->m_DoesSectionExist(
      ou16_PdoIndex, oq_MessageIsTx,
...
/*! \brief  Is COB-ID read-only
   ...
   \param[in]   ou16_PdoIndex    PDO index (0 = first PDO)
   \param[in]   oq_MessageIsTx   Flag if message is TX (for CANopen manager
   usage this flag is inverse) \param[out]  orq_IsRo         Is read-only
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscCanOpenObjectDictionary::IsCobIdRo(const uint16_t ou16_PdoIndex,
                                                const bool oq_MessageIsTx,
                                                bool &orq_IsRo) const {
  const int32_t s32_Retval = this->m_IsSectionRo(
      ou16_PdoIndex, oq_MessageIsTx,
      C_OscCanOpenObjectDictionary::hu8_OD_SUB_INDEX_COB_ID, orq_IsRo);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsInhibitTimeRo(const uint16_t ou16_PdoIndex,
                                              const bool oq_MessageIsTx,
                                              bool &orq_IsRo) const {
  const int32_t s32_Retval = this->m_IsSectionRo(
      ou16_PdoIndex, oq_MessageIsTx,
      C_OscCanOpenObjectDictionary::hu8_OD_SUB_INDEX_INHIBIT_TIME, orq_IsRo);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsEventTimerRo(const uint16_t ou16_PdoIndex,
                                             const bool oq_MessageIsTx,
                                             bool &orq_IsRo) const {
  const int32_t s32_Retval = this->m_IsSectionRo(
      ou16_PdoIndex, oq_MessageIsTx,
      C_OscCanOpenObjectDictionary::hu8_OD_SUB_INDEX_EVENT_TIMER, orq_IsRo);
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsTransmissionTypeRo(const uint16_t ou16_PdoIndex,
                                                   const bool oq_MessageIsTx,
                                                   bool &orq_IsRo) const {
  const int32_t s32_Retval = this->m_IsSectionRo(
      ou16_PdoIndex, oq_MessageIsTx,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsSyncStartRo(const uint16_t ou16_PdoIndex,
                                            const bool oq_MessageIsTx,
                                            bool &orq_IsRo) const {
  const int32_t s32_Retval = this->m_IsSectionRo(
      ou16_PdoIndex, oq_MessageIsTx,
...
//----------------------------------------------------------------------------------------------------------------------
int32_t
C_OscCanOpenObjectDictionary::IsPdoMappingRo(const uint16_t ou16_PdoIndex,
                                             const bool oq_MessageIsTx,
                                             bool &orq_IsRo) const {
  int32_t s32_Retval = C_NO_ERR;
  const uint16_t u16_ObjectIndex =
  ...
  const C_OscCanOpenObjectData *const pc_Object =
      this->GetCanOpenSubIndexObject(u16_ObjectIndex, 0U);
...
/*! \brief  Get CANopen object
   ...
   Goes through the list of known object dictionary entries and returns a
   pointer to the object with the specified index.
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscCanOpenObjectData *C_OscCanOpenObjectDictionary::GetCanOpenObject(
    const uint16_t ou16_OdIndex) const {
...
*/
//----------------------------------------------------------------------------------------------------------------------
const C_OscCanOpenObjectData *
C_OscCanOpenObjectDictionary::GetCanOpenSubIndexObject(
    const uint16_t ou16_OdIndex, const uint8_t ou8_OdSubIndex) const {
  const C_OscCanOpenObjectData *pc_SubObject = NULL;

  const QHash<uint16_t, C_OscCanOpenObject>::const_iterator c_Object =
      this->c_OdObjects.find(ou16_OdIndex);

  if (c_Object != this->c_OdObjects.end()) {
   const QHash<uint8_t, C_OscCanOpenObjectData>::const_iterator c_SubObject =
       c_Object.value().c_SubObjects.find(ou8_OdSubIndex);
...
/*! \brief  Check object present by index
   ...
   \param[in]  ou16_OdIndex      Object dictionary index
   \param[in]  ou8_OdSubIndex    Object dictionary sub index (0 = get main
   object)
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscCanOpenObjectDictionary::CheckObjectPresentByIndex(
    const uint16_t ou16_OdIndex, const uint8_t ou8_OdSubIndex) const {
  bool q_Retval = false;
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscCanOpenObjectDictionary::GetMappableObjects(
    QHash<uint32_t, QList<uint32_t>> &orc_SubIndices) const {
   for (QHash<uint16_t, C_OscCanOpenObject>::const_iterator c_Element =
            this->c_OdObjects.begin();
        c_Element != this->c_OdObjects.end(); ++c_Element) {
     if (c_Element.value().IsMappableIntoPdo() == true) {
       orc_SubIndices[c_Element.key()].push_back(0U);
     }

     // check sub objects:
     for (QHash<uint8_t, C_OscCanOpenObjectData>::const_iterator c_SubElement =
              c_Element.value().c_SubObjects.begin();
          c_SubElement != c_Element.value().c_SubObjects.end(); ++c_SubElement) {
       if (c_SubElement.value().IsMappableIntoPdo() == true) {
         orc_SubIndices[c_Element.key()].push_back(c_SubElement.key());
       }
     }
   }
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint16_t C_OscCanOpenObjectDictionary::h_GetCanOpenObjectDictionaryIndexForPdo(
    const uint16_t ou16_PdoIndex, const bool oq_MessageIsTx) {
  uint16_t u16_Retval;

  if (oq_MessageIsTx) {
    u16_Retval = C_OscCanOpenObjectDictionary::hu16_OD_INDEX_FIRST_TX_PDO +
                 ou16_PdoIndex;
  } else {
    u16_Retval = C_OscCanOpenObjectDictionary::hu16_OD_INDEX_FIRST_RX_PDO +
                 ou16_PdoIndex;
  }
  return u16_Retval;
}
