...
/*! \brief  Convert ECDSA signature to DER encoded string
   ...
   * <size_r> bytes     R part (with inserted leading zero padding if MSB of
   first byte is set)
   * one byte <size_s>  Size of the S part
   * <size_s> bytes     S part (with inserted leading zero padding if MSB of
   first byte is set)
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::C_Ecdsa256Signature::GetAsDerString(
    QString &orc_Signature) const {
  int32_t s32_Return = C_CONFIG;

  orc_Signature = "";

  if ((this->u8_NumBytesUsedRpart != 0) && (this->u8_NumBytesUsedSpart != 0)) {
    BIGNUM *const pc_Rpart =
        BN_bin2bn(&this->au8_Rpart[0], this->u8_NumBytesUsedRpart, NULL);
    BIGNUM *const pc_Spart =
        BN_bin2bn(&this->au8_Spart[0], this->u8_NumBytesUsedSpart, NULL);
    ...
    if ((pc_Rpart != NULL) && (pc_Spart != NULL)) {
      ...
      if (pc_Signature != NULL) {
        ...
        int x_Result =
            ECDSA_SIG_set0(pc_Signature, pc_Rpart,
                           pc_Spart); // lint !e970 !e8080 //using type to
                                      //  match library interface
        if (x_Result == 1) {
          // convert to string
          // first dry run to get size of required buffer:
          const int x_BufferSize = i2d_ECDSA_SIG(
              pc_Signature, NULL); // lint !e970 !e8080 //using type to
                                   //  match library interface
          if (x_BufferSize > 0) {
            ...
            // convert to string:
            for (uint32_t u32_Character = 0U;
                 u32_Character < static_cast<uint32_t>(x_BufferSize);
                 u32_Character++) {
              orc_Signature +=
                  C_OscUtils::h_IntToHex(pu8_OriginalBuffer[u32_Character], 2U);
            }
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::C_Ecdsa256Signature::SetFromDerString(
  ...
  if ((orc_Signature.length() > (7U * 2U)) &&
    ...
    const uint8_t u8_NumBytes =
        static_cast<uint8_t>(orc_Signature.length() / 2U);
    for (uint32_t u32_Byte = 0U; u32_Byte < u8_NumBytes; u32_Byte++) {
      try {
        au8_BinarySignature[u32_Byte] = static_cast<uint8_t>(
            ("0x" + orc_Signature.mid(u32_Byte * 2, 2)).toInt(nullptr, 16));
      } catch (...) {
        q_Error = true;
        break;
      }
    ...
    if (q_Error == false) {
      ECDSA_SIG *const pc_Signature =
          d2i_ECDSA_SIG(NULL, &pu8_Data, orc_Signature.length());
      if (pc_Signature != NULL) {
        ...
        if ((pc_SignatureRpart != NULL) || (pc_SignatureSpart != NULL) ||
            (BN_num_bytes(pc_SignatureRpart) > 32) ||
            (BN_num_bytes(pc_SignatureSpart) > 32)) {
          // operation successful -> place signature into output array
          int x_Result = BN_bn2bin(
              pc_SignatureRpart,
              &this->au8_Rpart[0]); // lint !e970 !e8080 //using type to
                                    //  match library interface
          Q_ASSERT(
              x_Result ==
              BN_num_bytes(
                  pc_SignatureRpart)); // we already checked size above; this
                                       //  would
                                       //  be unexpected
          ...
          Q_ASSERT(x_Result ==
                   BN_num_bytes(pc_SignatureSpart)); // we already checked size
                                                     // above; this would
                                                     //  be unexpected
 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscSecurityEcdsa::C_OscSecurityEcdsa() : mpc_MdContext(NULL) {}
...
/*! \brief  Utility to extract ESDSA public key from X509 certificate data
   ...
   ** effectively everything between the "BEGIN CERTIFICATE" and "END
   CERTIFICATE" lines converted from base64 to binary.

   orau8_Binary:
   * binary (concatenated "x" and "y" parts; 32 bytes each)

   The function will report an error if a key of the expected type and size
   cannot be extracted.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::h_ExtractPublicKeyFromX509Certificate(
    const QByteArray &orc_X509,
    uint8_t (
        &orau8_Binary)[C_OscSecurityEcdsa::hu32_SECP256R1_PUBLIC_KEY_LENGTH]) {
  int32_t s32_Result = C_RANGE;

  if (orc_X509.size() != 0) {
    const uint8_t *pu8_KeyData =
        reinterpret_cast<const uint8_t *>(orc_X509.data());
      ...
      if ((pc_EvpKey != NULL) && (EVP_PKEY_base_id(pc_EvpKey) == EVP_PKEY_EC)) {
        ...
        if (pc_EcGroup != NULL) {
          ...
          if (pc_PublicKey != NULL) {
            ...
            // Whether this is compressed or uncompressed is up to the OpenSSL
            // library. So first ask the library about the required buffer size.
            int x_ResultGetPublicKey; // lint !e970 !e8080 //use API type

            x_ResultGetPublicKey = EVP_PKEY_get_octet_string_param(
                pc_EvpKey, OSSL_PKEY_PARAM_PUB_KEY, NULL, 0,
                &x_NumBytesPublicKey);
            if (x_ResultGetPublicKey == 1) {
              c_PublicKeyOctets.resize(x_NumBytesPublicKey);
              x_ResultGetPublicKey = EVP_PKEY_get_octet_string_param(
                  pc_EvpKey, OSSL_PKEY_PARAM_PUB_KEY,
                  reinterpret_cast<uint8_t *>(c_PublicKeyOctets.data()),
                  c_PublicKeyOctets.size(), &x_NumBytesPublicKey);
            }
            if ((x_ResultGetPublicKey == 1) &&
                (x_NumBytesPublicKey == c_PublicKeyOctets.size())) {
              // convert from octet string to EC_POINT
              const int x_ResultOct2Point = // lint !e970 !e8080 //use API type
                  EC_POINT_oct2point(
                      pc_EcGroup, pc_PublicKey,
                      reinterpret_cast<uint8_t *>(c_PublicKeyOctets.data()),
              ...
              if (x_ResultOct2Point == 1) {
                ...
                int x_Result = // lint !e970 !e8080 //use API type
                    EC_POINT_get_affine_coordinates(pc_EcGroup, pc_PublicKey,
                                                    pc_Xpart, pc_Ypart, NULL);
                if ((x_Result == 1) && (BN_num_bytes(pc_Xpart) == 32) &&
                    (BN_num_bytes(pc_Ypart) == 32)) {
                  // put x and y into array:
                  x_Result = BN_bn2bin(pc_Xpart, &orau8_Binary[0]);
                  Q_ASSERT(
                      x_Result ==
                      32); // size already checked; this would be unexpected
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Calculates a ECDSA signature using a secp256r1 curve and a private
   key over a SHA encrypted input.

   The SHA encrypted input is a stream of data (see SHA256 utility functions in
   this class)

   We provide the hashing and signing steps in separate functions on purpose.
   Thus it is possible to create the hash (and therefore signature) over
   non-continuous data by streaming in multiple blocks into the SHA calculation
   one by one before calculating the signature over the final digest.
   ...
   * see structure for documentation
   ...
   \param[in]    orau8_Digest          Result of the SHA256 calculation of the
   data we want to sign \param[in]    orau8_PrivateKey      Private key
   ...
   \retval   C_RANGE    Invalid key; technically this should not happen as we do
   not explicitly check the key content \retval   C_NOACT    Could not generate
   signature
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::h_CalcEcdsaSecp256r1Signature(
    const uint8_t (&orau8_Digest)[hu32_SHA256_FINAL_LENGTH],
    const uint8_t (&orau8_PrivateKey)[hu32_SECP256R1_PRIVATE_KEY_LENGTH],
    C_Ecdsa256Signature &orc_Signature, QString &orc_ErrorMessage) {
  int32_t s32_Return = C_RANGE;

  // converting the binary data of private key back to openSSL internal
  // structures
  BIGNUM *const pc_BigNum =
      BN_bin2bn(&orau8_PrivateKey[0], hu32_SECP256R1_PRIVATE_KEY_LENGTH, NULL);

  if (pc_BigNum != NULL) {
    // the key parameter for EC_KEY_set_private_key shall not be NULL so we
    // first need a "dummy" key the prime256v1 curve is equivalent to secp256r1,
    // that has no specific NID in openssl library. See:
    ...
    if (pc_EcdsaKey != NULL) {
      int x_Result = EC_KEY_set_private_key(
          pc_EcdsaKey, pc_BigNum); // lint !e970 !e8080 //using type to
      if (x_Result == 1) {
        // Checking for the validity of the private key is not so simple.
        // EC_KEY_check_key will fail as we have not explicitly set a public
        // key. Technically the private key must be in a certain range, but this
        // is rather broad. So we will not perform an additional check for
        // validity.
        s32_Return = C_NOACT;
        ...
        // sign the digest
        ECDSA_SIG *const pc_Signature =
            ECDSA_do_sign(orau8_Digest, hu32_SHA256_FINAL_LENGTH, pc_EcdsaKey);
        EC_KEY_set_private_key(pc_EcdsaKey, NULL);
        EC_KEY_free(pc_EcdsaKey);

        if (pc_Signature != NULL) {
          const BIGNUM *const pc_SignatureRpart =
              ECDSA_SIG_get0_r(pc_Signature);
          const BIGNUM *const pc_SignatureSpart =
              ECDSA_SIG_get0_s(pc_Signature);

          if ((pc_SignatureRpart == NULL) || (pc_SignatureSpart == NULL) ||
              (BN_num_bytes(pc_SignatureRpart) > 32) ||
              (BN_num_bytes(pc_SignatureSpart) > 32)) {
            orc_ErrorMessage = "ECDSA signature size is greater than the "
                               "expected maximum size.";
          } else {
            // operation successful -> place signature into output array

            x_Result =
                BN_bn2bin(pc_SignatureRpart, &orc_Signature.au8_Rpart[0]);
            Q_ASSERT(x_Result ==
                     BN_num_bytes(pc_SignatureRpart)); // we already checked
                                                       // size above; this would
                                                       //  be unexpected
            orc_Signature.u8_NumBytesUsedRpart = static_cast<uint8_t>(x_Result);

            x_Result =
                BN_bn2bin(pc_SignatureSpart, &orc_Signature.au8_Spart[0]);
            Q_ASSERT(x_Result ==
                     BN_num_bytes(pc_SignatureSpart)); // we already checked
                                                       // size above; this would
                                                       //  be unexpected
          ...
          orc_ErrorMessage = "openSSL unable to sign the data. Check inputs: "
    ...
    orc_ErrorMessage =
        "Could not convert private key from data. Maybe check pem file.";
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  Checks if an ECDSA signature that was calculated with a secp256r1
   curve by the method h_CalcEcdsaSecp256r1Signature is valid
   ...
   * binary result of SHA256 algorithm
   ...
   \param[in]      orau8_Digest     Result of the SHA256 calculation of the data
   we did sign \param[out]     orq_Valid        true: signature valid false:
   signature not valid
   ...
   \retval   C_NOACT    Signature verification failed
                        (internal error returned by used security library;
   should not happen in regular use cases)
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::h_VerifyEcdsaSecp256r1Signature(
    const uint8_t (&orau8_PublicKey)[hu32_SECP256R1_PUBLIC_KEY_LENGTH],
  ...
  if ((orc_Signature.u8_NumBytesUsedRpart > 0U) &&
      (orc_Signature.u8_NumBytesUsedSpart > 0U)) {
    // convert binary signature data back to openSSL structure
    BIGNUM *const pc_Rpart = BN_bin2bn(
        &orc_Signature.au8_Rpart[0], orc_Signature.u8_NumBytesUsedRpart, NULL);
    BIGNUM *const pc_Spart = BN_bin2bn(
        &orc_Signature.au8_Spart[0], orc_Signature.u8_NumBytesUsedSpart, NULL);
    BIGNUM *const pc_Xpart = BN_bin2bn(&orau8_PublicKey[0], 32, NULL);
    BIGNUM *const pc_Ypart = BN_bin2bn(&orau8_PublicKey[32], 32, NULL);

    if ((pc_Rpart != NULL) && (pc_Spart != NULL) && (pc_Xpart != NULL) &&
        (pc_Ypart != NULL)) {
      ECDSA_SIG *const pc_Signature = ECDSA_SIG_new();
      if (pc_Signature != NULL) {
        int x_Result =
          ...
          if (pc_EcdsaKey != NULL) {
            ...
            if (pc_EcGroup != NULL) {
              ...
              if (x_Result == 1) {
                ...
                x_Result = EC_KEY_set_public_key_affine_coordinates(
                    pc_EcdsaKey, pc_Xpart, pc_Ypart);
              }
              if (x_Result == 1) {
                // verify signature
                x_Result =
                    ECDSA_do_verify(&orau8_Digest[0], hu32_SHA256_FINAL_LENGTH,
                                    pc_Signature, pc_EcdsaKey);
                switch (x_Result) {
        ...
        // only free R and S parts manually if they were not indirectly freed
        // via ECDSA_SIG_free
        BN_clear_free(pc_Rpart);
        BN_clear_free(pc_Spart);
      }
...
int32_t C_OscSecurityEcdsa::Sha256Init() {
  ...
  // initialize context:
  const int x_Result = EVP_DigestInit_ex(
      mpc_MdContext, pc_Algorithm, NULL); // lint !e8080 !e970  //using type to
                                          //  match library interface
  if (x_Result == 1) {
    s32_Return = C_NO_ERR;
  }
...
/*! \brief  Stream data into SHA calculation
   ...
   As a result the context will be updated depending on the supplied data and
   previous context.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::Sha256Update(const uint8_t *const opu8_Data,
  ...
  if (opu8_Data != NULL) {
    if (mpc_MdContext == NULL) {
      ...
      const int x_Result = EVP_DigestUpdate(
          mpc_MdContext, opu8_Data, ou32_NumBytes); // lint !e8080 !e970 //using
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSecurityEcdsa::Sha256GetDigest(
    uint8_t (&orau8_Digest)[hu32_SHA256_FINAL_LENGTH]) {
  ...
  if (mpc_MdContext == NULL) {
    ...
    unsigned int
        x_Size; // lint !e8080 !e970  //using type to match library interface
    // get final digest value:
    const int x_Result =
        EVP_DigestFinal_ex(mpc_MdContext, orau8_Digest,
                           &x_Size); // lint !e8080 !e970  //using type
