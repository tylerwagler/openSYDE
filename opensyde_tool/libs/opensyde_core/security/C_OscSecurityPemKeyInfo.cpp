 ...
 */
//----------------------------------------------------------------------------------------------------------------------
C_OscSecurityPemKeyInfo::C_CertificateKeyUsageInformation::
...
/*! \brief  Get X.509 certificate data
   ...
   Return the binary representation of the whole X.509 "Certificate" section of
   a pem file. i.e. the base64 data converted to binary form.
...
C_OscSecurityPemKeyInfo::GetKeyUsageInformation() const {
...
/*! \brief  Check for presence of key information
   ...
   Check whether the instance contains non-zero length certificate and private
   key information. If information is missing the function will fill a text
   string with details.

   \param[out]     orc_ErrorMessage       Details about what information is
   missing \param[in]      oq_CheckSerialNumber   true: also check for non-zero
   length certificate serial number

   \retval   true    All requested information present
   \retval   false   At least one piece of information (certificate, private
   key, serial number) not present
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_OscSecurityPemKeyInfo::AreKeysAvailable(
    QString &orc_ErrorMessage, const bool oq_CheckSerialNumber) const {
  bool q_Retval = false;
...
/*! \brief  Set X.509 certificate data
   ...
   Set the binary representation of the whole X.509 "Certificate" section of a
   pem file.

   Technically:
   Everything between the "BEGIN CERTIFICATE" and "END CERTIFICATE" lines
   converted from base64 to binary.

   This will typically include the public key, certificate information (e.g.
   serial number, flags).

   \param[in]  orc_Value   Value
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSecurityPemKeyInfo::SetX509CertificateData(
...
/*! \brief  Set certificate serial number
   ...
   Format: binary array describing a large integer (big endian; without leading
   zeroes)

   \param[in]  orc_Value   Value
*/
//----------------------------------------------------------------------------------------------------------------------
void C_OscSecurityPemKeyInfo::SetCertificateSerialNumber(
    const QByteArray &orc_Value) {
