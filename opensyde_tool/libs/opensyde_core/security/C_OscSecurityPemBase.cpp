 ...
 */
//----------------------------------------------------------------------------------------------------------------------
void C_OscSecurityPemBase::Clear() { this->mc_KeyInfo.Clear(); }
    ...
    if (pc_RsaPub != NULL) {
          ...
          C_OscSecurityPemBase::mhu32_DEFAULT_BUFFER_SIZE);
      {
        ...
        if (u32_PubKeyTextDecodedOpensslCount > 0) {
          ...
          const uint32_t u32_ExtensionFlags =
              X509_get_extension_flags(pc_RsaPub);
          if ((u32_ExtensionFlags & static_cast<uint32_t>(EXFLAG_KUSAGE)) ==
              EXFLAG_KUSAGE) {
            // get key usage flags
            const uint32_t u32_KeyUsage = X509_get_key_usage(pc_RsaPub);
            c_Info.q_KeyUsageDefined = true;

            if ((u32_KeyUsage & static_cast<uint32_t>(KU_DIGITAL_SIGNATURE)) ==
                KU_DIGITAL_SIGNATURE) {
              c_Info.q_KeyUsageDigitalSignature = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_NON_REPUDIATION)) ==
                KU_NON_REPUDIATION) {
              c_Info.q_KeyUsageNonRepudiation = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_KEY_ENCIPHERMENT)) ==
                KU_KEY_ENCIPHERMENT) {
              c_Info.q_KeyUsageKeyEncipherment = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_DATA_ENCIPHERMENT)) ==
                KU_DATA_ENCIPHERMENT) {
              c_Info.q_KeyUsageDataEncipherment = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_KEY_AGREEMENT)) ==
                KU_KEY_AGREEMENT) {
              c_Info.q_KeyUsageKeyAgreement = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_KEY_CERT_SIGN)) ==
                KU_KEY_CERT_SIGN) {
              c_Info.q_KeyUsageKeyCertSign = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_CRL_SIGN)) ==
                KU_CRL_SIGN) {
              c_Info.q_KeyUsageCrlSign = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_ENCIPHER_ONLY)) ==
                KU_ENCIPHER_ONLY) {
              c_Info.q_KeyUsageEncipherOnly = true;
            }
            if ((u32_KeyUsage & static_cast<uint32_t>(KU_DECIPHER_ONLY)) ==
                KU_DECIPHER_ONLY) {
              c_Info.q_KeyUsageDecipherOnly = true;
            }
          }
          if ((u32_ExtensionFlags & static_cast<uint32_t>(EXFLAG_XKUSAGE)) ==
              EXFLAG_XKUSAGE) {
            // get extended key usage flags
            const uint32_t u32_ExtendedKeyUsage =
                X509_get_extended_key_usage(pc_RsaPub);
            c_Info.q_ExtendedKeyUsageDefined = true;

            if ((u32_ExtendedKeyUsage &
                 static_cast<uint32_t>(XKU_SSL_SERVER)) == XKU_SSL_SERVER) {
              c_Info.q_ExtendedKeyUsageServerAuth = true;
            }
            if ((u32_ExtendedKeyUsage &
                 static_cast<uint32_t>(XKU_SSL_CLIENT)) == XKU_SSL_CLIENT) {
              c_Info.q_ExtendedKeyUsageClientAuth = true;
            }
            if ((u32_ExtendedKeyUsage & static_cast<uint32_t>(XKU_SMIME)) ==
                XKU_SMIME) {
              c_Info.q_ExtendedKeyUsageEmailProtection = true;
            }
            if ((u32_ExtendedKeyUsage & static_cast<uint32_t>(XKU_CODE_SIGN)) ==
                XKU_CODE_SIGN) {
              c_Info.q_ExtendedKeyUsageCodeSigning = true;
            }
            if ((u32_ExtendedKeyUsage & static_cast<uint32_t>(XKU_OCSP_SIGN)) ==
                XKU_OCSP_SIGN) {
              c_Info.q_ExtendedKeyUsageOcspSigning = true;
            }
            if ((u32_ExtendedKeyUsage & static_cast<uint32_t>(XKU_TIMESTAMP)) ==
                XKU_TIMESTAMP) {
              c_Info.q_ExtendedKeyUsageTimeStamping = true;
            }
            if ((u32_ExtendedKeyUsage & static_cast<uint32_t>(XKU_DVCS)) ==
                XKU_DVCS) {
              c_Info.q_ExtendedKeyUsageDvcs = true;
            }
            if ((u32_ExtendedKeyUsage & static_cast<uint32_t>(XKU_ANYEKU)) ==
                XKU_ANYEKU) {
            ...
            if (pc_SerialNumberOpenssl != NULL) {
                  ...
                  C_OscSecurityPemBase::mhu32_DEFAULT_BUFFER_SIZE);
              {
                ...
                if (u32_SerialNumberCount == 0) {
                  s32_Retval = C_CONFIG;
                  orc_ErrorMessage =
                      "Public key: could not convert serial number into bytes";
                } else {
                  ...
                  if ((c_PubKeySerialNumber.size() > 22UL) ||
                    ...
                    if ((c_PubKeySerialNumber[0UL] == 0x02U) &&
                        (c_PubKeySerialNumber[1UL] ==
                         (c_PubKeySerialNumber.size() - 2UL))) {
                      // Erase first two elements
                      c_PubKeySerialNumber.erase(c_PubKeySerialNumber.begin());
                      c_PubKeySerialNumber.erase(c_PubKeySerialNumber.begin());

                      // Strip leading zeroes for compatibility (see #100795 for
                      // details)
                      while ((c_PubKeySerialNumber.size() > 0) &&
                             (c_PubKeySerialNumber[0] == 0x00U)) {
                        c_PubKeySerialNumber.erase(
                            c_PubKeySerialNumber.begin());
                      }

                      // Add serial number
                      this->mc_KeyInfo.SetCertificateSerialNumber(
                          c_PubKeySerialNumber);
                    } else {
                      s32_Retval = C_CONFIG;
                      orc_ErrorMessage =
                          "Public key: serial number format unknown";
                    }
                  }
                }
              }
          ...
          s32_Retval = C_CONFIG;
          orc_ErrorMessage =
              "Public key: could not convert X509 section into bytes";
        }
      }
      X509_free(pc_RsaPub);
    } else {
    ...
    if (pc_RsaPub != NULL) {
      ...
      if (pc_PrintBuffer != NULL) {
        if (X509_print(pc_PrintBuffer, pc_RsaPub) == 1) {
          ...
          BIO_get_mem_ptr(pc_PrintBuffer,
                          &pc_MemPtr); // lint !e970 OpenSSL interface
          if (pc_MemPtr != NULL) {
            this->mc_MetaInfo = "";
            ...
            s32_Retval = C_CONFIG;
            orc_ErrorMessage =
                "Public key: could not get mem pointer for buffer object";
          }
        } else {
          s32_Retval = C_CONFIG;
