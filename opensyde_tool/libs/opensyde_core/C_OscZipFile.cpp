...
/*! \brief   Create ZIP archive from files
   ...
   * erase a pre-existing zip file
   ...
   (technically the function will probably add to the existing file but this has
   not been tested).

   Assumptions:
   * consistent data
   * moderate file sizes to zip
    (is not zipping files by chunk, with files greater than 0.5 GB there can
   occur problems)

   \param[in]  orc_SourcePath                path of folder containing files to
   add to archive (needs trailing path terminator)
   ...
   \param[out] opc_ErrorText                 if != NULL and return value !=
   C_NO_ERR: will be filled with textual details of problem
...
int32_t C_OscZipFile::h_CreateZipFile(const QString &orc_SourcePath,
  ...
  for (c_Iter = orc_SupFiles.begin(); c_Iter != orc_SupFiles.end(); ++c_Iter) {
    ...
    if (((QFileInfo(c_AbsPath).exists() && QFileInfo(c_AbsPath).isFile()) ==
         false) &&
        (QFileInfo(c_AbsPath).isDir() == false)) {
      if (opc_ErrorText != NULL) {
        (*opc_ErrorText) =
            "Input file/folder \"" + c_AbsPath + "\" does not exist.";
      }
      s32_Return = C_CONFIG;
      break;
    }
  ...
  for (c_Iter = orc_SupFiles.begin();
    ...
    if ((QFileInfo(orc_SourcePath + c_FileName).exists() &&
      ...
      const QString c_AbsPath = orc_SourcePath + c_FileName; // absolute path
      ifstream c_FileStream(c_AbsPath.toLocal8Bit().constData(),
                            ifstream::binary |
                                ifstream::ate); // open file and set pos
                                                // to the end of file
      if (c_FileStream.is_open() == true) {
        ...
        // allocate memory
        char_t *const pcn_FileData =
            new char_t[static_cast<size_t>(c_FileLength)];
        ...
        s32_Return = C_OscZipFile::mh_AddContentToZipFile(
            orc_ZipArchivePath, c_FileName, pcn_FileData,
            static_cast<uint32_t>(c_FileLength), "file", opc_ErrorText);

        // free memory
        delete[] pcn_FileData;
      } else {
        if (opc_ErrorText != NULL) {
          (*opc_ErrorText) = "Could not open file \"" + c_FileName +
                             "\" for zipping into archive.";
        }
        s32_Return = C_RD_WR;
      }
    } else {
      s32_Return = C_OscZipFile::mh_AddContentToZipFile(
          orc_ZipArchivePath, c_FileName, NULL, 0UL, "folder", opc_ErrorText);
    }
  }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Unpacks source zip archive and stores the contents to target unzip
   path.
   ...
   * read/write permissions
   ...
   \param[in]  orc_TargetUnzipPath           target path for unpacking the zip
   archive (with or without trailing path delimiter)
   \param[out] opc_ErrorText                 if != NULL and return value !=
   C_NO_ERR: will be filled with textual details of problem
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscZipFile::h_UnpackZipFile(const QString &orc_SourcePath,
  ...
  x_MzStatus = mz_zip_reader_init_file(
      &c_ZipArchive, orc_SourcePath.toLocal8Bit().constData(), 0);
  if (x_MzStatus == MZ_FALSE) {
    if (opc_ErrorText != NULL) {
      (*opc_ErrorText) =
          "Could not open zip archive \"" + orc_SourcePath + "\". Reason: \"" +
  ...
  if (s32_Return == C_NO_ERR) {
    ...
    for (mz_uint x_Pos = 0;
         (x_Pos < mz_zip_reader_get_num_files(&c_ZipArchive)) &&
         (x_MzStatus == MZ_TRUE);
         x_Pos++) {
      ...
      if (x_MzStatus == MZ_FALSE) {
        ...
        if (opc_ErrorText != NULL) {
          (*opc_ErrorText) = "Could not get information of zip archive \"" +
                             orc_SourcePath + "\" for position \"" +
    ...
    for (c_Iter = c_Files.begin();
      ...
      void *const pv_Data = mz_zip_reader_extract_file_to_heap(
          &c_ZipArchive, &c_Iter->m_filename[0], &x_UncompFileSize, 0);
      if (pv_Data != NULL) {
        // get complete file path of current file
        const QString c_CompleteFilePath =
            QDir(orc_TargetUnzipPath)
                .filePath(QString::fromUtf8(c_Iter->m_filename));

        // check if we have to create a subfolder
        const QString c_Path =
            QFileInfo(c_CompleteFilePath).absolutePath() + "/";
        if (QFileInfo(c_Path).isDir() == false) {
          // create subfolder
        ...
        if (c_Iter->m_is_directory == MZ_FALSE) {
          ...
          if (s32_Return == C_NO_ERR) {
            std::FILE *const pc_File =
                std::fopen(c_CompleteFilePath.toLocal8Bit().constData(), "wb");
            // write data
            if (pc_File != NULL) {
              // lint -e{8080} //using type expected by the library for
              // compatibility
              const size_t x_SIZE_OF_ELEMENT = sizeof(char_t);
              // lint -e{8080} //using type expected by the library for
              // compatibility
              const size_t x_NumOfBytesWritten = std::fwrite(
                  pv_Data, x_SIZE_OF_ELEMENT, x_UncompFileSize, pc_File);
              if (x_NumOfBytesWritten != x_UncompFileSize) {
                // data not written completely
                if (opc_ErrorText != NULL) {
                  (*opc_ErrorText) =
                      "Could not unpack file \"" + c_CompleteFilePath +
                      "\" of zip archive \"" + orc_SourcePath + "\".";
                }
                s32_Return = C_RD_WR;
              }
              std::fclose(pc_File);
              ...
              if (opc_ErrorText != NULL) {
                (*opc_ErrorText) =
                    "Could not create file \"" + c_CompleteFilePath +
                    "\" to unpack from zip archive \"" + orc_SourcePath + "\".";
              }
              s32_Return = C_RD_WR;
            }
          }
        ...
        if (opc_ErrorText != NULL) {
          (*opc_ErrorText) = QString("Could not read file \"") +
                             c_Iter->m_filename + "\" from zip archive \"" +
                             orc_SourcePath + "\".";
        }
        s32_Return = C_RD_WR;
      }
...
void C_OscZipFile::h_AppendFilesRelative(std::set<QString> &orc_Set,
  ...
  for (uint32_t u32_PosFilesToFlash = 0; u32_PosFilesToFlash < orc_Files.size();
       u32_PosFilesToFlash++) {
    QString c_RelativeFilePath = orc_Files[u32_PosFilesToFlash];
    c_RelativeFilePath = c_RelativeFilePath.mid(orc_BasePath.length());
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscZipFile::h_IsZipFile(const QString &orc_FilePath) {
  int32_t s32_Return;
  ...
  if ((QFileInfo(orc_FilePath).exists() && QFileInfo(orc_FilePath).isFile()) ==
    ...
    ifstream c_FileStream(orc_FilePath.toLocal8Bit().constData(),
                          ifstream::binary |
                              ifstream::ate); // open file and set pos
    if (c_FileStream.is_open() == true) {
      ...
      if (s32_FileLength > 4) {
        // using type to match library interface; if char_t had a size different
        // than a "uint8" this spot would be the smallest of our problems
        const char_t acn_ZIP_HEADER[4] = {0x50, 0x4B, 0x03,
                                          0x04}; // lint !e970 !e9128
        char_t acn_Header[4];
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscZipFile::mh_AddContentToZipFile(const QString &orc_ZipArchivePath,
                                             ...
                                             QString *const opc_ErrorText) {
  int32_t s32_Return = C_NO_ERR;
  QString c_FilePathWithSlashes = orc_ItemName;
  const QString c_Comment = "Zipping " + orc_ItemType + ": " +
                            orc_ItemName; // set filename as comment
  ...
  // therefore change to '/'
  c_FilePathWithSlashes.replace("\\", "/");

  // store file content to zip archive
  // mz_zip_add_mem_to_archive_file_in_place is creating, appending and always
  // finalizing the archive after successful operation
  x_MzStatus = mz_zip_add_mem_to_archive_file_in_place(
      orc_ZipArchivePath.toLocal8Bit().constData(),
      c_FilePathWithSlashes.toLocal8Bit().constData(), opcn_Content,
      static_cast<size_t>(ou32_ContentSize),
      c_Comment.toLocal8Bit().constData(),
      static_cast<uint16_t>(c_Comment.length()), MZ_BEST_COMPRESSION);

  if (x_MzStatus == MZ_FALSE) {
    if (opc_ErrorText != NULL) {
      (*opc_ErrorText) =
          "Could not create zip " + orc_ItemType + " \"" + orc_ItemName + "\".";
    }
    s32_Return = C_NOACT;
  }
  return s32_Return;
