...
/*!
   ...
   Header to class implementing a high level interface to communication driver
   DLLs in the STW-CAN-DLL format.
...
public:
  ...
  C_Can &
  operator=(const C_Can &orc_Souce); // not implemented -> prevent assignment
  C_Can(void);
  ...
  virtual int32_t
  CAN_Init(void); // use bitrate currently configured in CAN driver
