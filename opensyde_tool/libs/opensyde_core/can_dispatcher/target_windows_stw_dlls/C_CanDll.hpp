...
/*!
   ...
   Header to class implementing an interface to communication driver DLLs in the
   STW-CAN-DLL format.
...
typedef int32_t(WINAPI *PR_CAN_SEND_EXTMSG)(
    const STW_CAN_EXTMSG *const opt_Msg);
typedef int32_t(WINAPI *PR_CAN_READ_EXTMSG)(STW_CAN_EXTMSG *const opt_Msg);
typedef int32_t(WINAPI *PR_CAN_SEND_EXTRTR)(
    const STW_CAN_EXTMSG *const opt_Msg);
...
typedef int32_t(WINAPI *PR_CANext_Send_Msg)(
    const uint8_t ou8_Channel, const T_STWCAN_Msg_TX *const opt_Message);
...
typedef int32_t(WINAPI *PR_CANext_Get_System_Time)(
    uint64_t *const opu64_TimeUs);
typedef int32_t(WINAPI *PR_CANext_Get_Num_Supported_Bitrates)(
    uint32_t *const opu32_MultiplicationFactor);
typedef int32_t(WINAPI *PR_CANext_Get_Supported_Bitrate)(
    const uint16_t ou16_BitrateIndex, uint32_t *const opu32_Bitrate);
...
public:
  ...
  C_CanDll &
  operator=(const C_CanDll &orc_Souce); // not implemented -> prevent assignment
  ...
  int32_t CAN_Init_One_ID(const int32_t os32_BitrateKBitS,
                          const uint32_t ou32_ID);

  //"ext" functions:
  int32_t CANext_Init(const uint8_t ou8_Channel);
  int32_t CANext_Exit(const uint8_t ou8_Channel);
  int32_t CANext_Bitrate(const uint8_t ou8_Channel,
                         const int32_t os32_BitrateKBitS);
  int32_t CANext_Read_Msg(const uint8_t ou8_Channel,
                          T_STWCAN_Msg_RX &orc_Message);
  int32_t CANext_Send_Msg(const uint8_t ou8_Channel,
  ...
  int32_t CANext_Init_One_ID(const uint8_t ou8_Channel,
                             const int32_t os32_BitrateKBitS,
                             const uint32_t ou32_ID);
  int32_t CANext_DLL_Info(const uint8_t ou8_Channel,
                          T_STWCAN_DLLInfo &orc_Info);
  int32_t CANext_Get_Num_Channels(void);
  int32_t CANext_Get_System_Time(uint64_t &oru64_SystemTimeUs);
  int32_t
  CANext_Get_Num_Supported_Bitrates(uint32_t &oru32_MultiplicationFactor);
  int32_t CANext_Get_Supported_Bitrate(const uint16_t ou16_BitrateIndex,
                                       uint32_t &oru32_Bitrate);

  // utilities to convert between different message structure types
  static void STWCANMSG_to_CANMSGRX(T_STWCAN_Msg_RX &orc_Target,
                                    const STW_CAN_MSG &orc_Source);
  static void STWCANEXTMSG_to_CANMSGRX(T_STWCAN_Msg_RX &orc_Target,
                                       const STW_CAN_EXTMSG &orc_Source);
  static void CANMSGTX_to_STWCANMSG(STW_CAN_MSG &orc_Target,
                                    const T_STWCAN_Msg_TX &orc_Source);
  static void CANMSGTX_to_STWCANEXTMSG(STW_CAN_EXTMSG &orc_Target,
                                       const T_STWCAN_Msg_TX &orc_Source);
