...
C_Can::C_Can(const uint8_t ou8_CommChannel)
    : C_CanDispatcher(ou8_CommChannel), mc_DllName(""), mpc_Can(NULL),
      mq_DLLOpened(false), mu32_RxId(CAN_RX_ID_INVALID) {}

//----------------------------------------------------------------------------------------------------------------------

C_Can::C_Can(void)
    : C_CanDispatcher(), mc_DllName(""), mpc_Can(NULL), mq_DLLOpened(false),
...
/*! \brief   Close CAN bus
   ...
   Depending on availability the "Ext" or regular DLL functions are used ("Ext"
   has priority).
   ...
   C_CONFIG                          channel other than 0 configured but not
   supported by loaded DLL
*/
//----------------------------------------------------------------------------------------------------------------------
...
/*! \brief   Reset CAN bus
   ...
   Depending on availability the "Ext" or regular DLL functions are used ("Ext"
   has priority). The "Ext" set does not provide a specific "Reset" function so
   a sequence of "Exit" and "Init" is called in this case.
...
/*! \brief   Initialize CAN bus
   ...
   Depending on availability the "Ext" or regular DLL functions are used ("Ext"
   has priority). Will use the DLL's "Init" function for initialization. If the
   "limit RX ID" is configured to a valid value the DLL's init function
   "Init_One_ID" will be used (limiting reception to exactly that ID).

   \return
   C_NO_ERR                          DLL function returned OK; bus initialized
   -1                                initialization error returned by DLL
   function CAN_COMP_ERR_DLL_NOT_OPENED       DLL was not yet loaded C_CONFIG
   channel other than 0 configured but not supported by loaded DLL
*/
//----------------------------------------------------------------------------------------------------------------------
...
/*! \brief   Initialize CAN bus to specific bitrate
   ...
   Depending on availability the "Ext" or regular DLL functions are used ("Ext"
  has priority). Will use the DLL's "CAN_Bitrate" function for initialization.
   If the "limit RX ID" is configured to a valid value the DLL's init function
  "Init_One_ID" will be used (limiting reception to exactly that ID).
...
int32_t C_Can::CAN_Init(const int32_t os32_BitrateKBitS) {
  ...
  if (mpc_Can->mq_ExtFunctionsAvailable == false) {
    ...
    if (mu32_RxId == CAN_RX_ID_INVALID) {
      ...
      s32_Return = mpc_Can->CANext_Init_One_ID(mu8_CommChannel,
                                               os32_BitrateKBitS, mu32_RxId);
    }
  }
  if (s32_Return == C_NO_ERR) {
    mu8_CANOpened = 1U;
...
/*! \brief   Show DLL's configuration dialog
   ...
   Depending on availability the "Ext" or regular DLL functions are used ("Ext"
   has priority). Execute's the DLL's blocking configuration dialog.

   Only the "Ext" function type returns the information whether the user clocked
   "OK" or "CANCEL". The this function can only return "1" if the "Ext"
   functions are available.

   \return
   C_NO_ERR                          DLL function returned OK   -> user clicked
   "CANCEL" or: non "Ext" function was executed 1 DLL function returned 1    ->
   user clicked "OK" -1                                DLL function returns
   error CAN_COMP_ERR_DLL_NOT_OPENED       DLL was not yet loaded C_CONFIG
   channel other than 0 configured but not supported by loaded DLL
*/
//----------------------------------------------------------------------------------------------------------------------
...
int32_t C_Can::m_SendMsgToDLL(const T_STWCAN_Msg_TX &orc_Message) const {
  ...
  if (mpc_Can == NULL) {
    return C_CONFIG; // will not happen in RL; defensive measure to pacify
...
/*! \brief   Read CAN message
  ...
  \param[out] orc_Message   read message
   ...
   \return
   C_NO_ERR                          DLL function returned OK; message placed in
  orc_Message -1                                DLL function returns error
   CAN_COMP_ERR_DLL_NOT_OPENED       DLL was not yet loaded
   C_CONFIG                          channel other than 0 configured but not
  supported by loaded DLL
*/
...
int32_t C_Can::CAN_Read_Msg_Timeout(const uint32_t ou32_MaxWaitTimeMs,
  ...
  do {
    s32_Return = this->m_ReadMsgFromDLL(orc_Message);
           ...
           (c_Timer.hasExpired(ou32_MaxWaitTimeMs) == false));
...
/*! \brief   Get DLL's opinion about system time
   ...
   Depending on availability the "Ext" or regular DLL functions are used ("Ext"
   has priority). The "NonExt" set does not provide this functions; so in this
   case the time since system start is returned instead.

   \param[out]   oru64_SystemTimeUs    system time in micro-seconds as reported
   by the DLL
...
int32_t C_Can::CAN_Get_System_Time(uint64_t &oru64_SystemTimeUs) const {
  ...
  oru64_SystemTimeUs =
      std::chrono::duration_cast<std::chrono::microseconds>(duration)
          .count(); // best we can do: we provide the timestamps ourself anyway
...
int32_t
C_Can::CAN_Get_Supported_Bitrates(QString &orc_Unit,
  ...
  s32_Return =
      mpc_Can->CANext_Get_Num_Supported_Bitrates(oru32_MultiplicationFactor);
  if (s32_Return < C_NO_ERR) {
    return s32_Return;
  }
  ...
  case 1000000U:
    ...
    break;
  default:
    orc_Unit = QString::number(oru32_MultiplicationFactor) + "Bit/s";
    break;
  }
  ...
  for (s32_Loop = 0; s32_Loop < s32_NumBitrates; s32_Loop++) {
    s32_Return = mpc_Can->CANext_Get_Supported_Bitrate(
        static_cast<uint16_t>(s32_Loop), u32_Bitrate);
    if (s32_Return != C_NO_ERR) {
      return s32_Return;
    }
...
int32_t C_Can::DLL_Open(void) {
  ...
  try {
    mpc_Can = new C_CanDll(mc_DllName.toUtf8().constData());
  } catch (...) {
    delete mpc_Can;
    mpc_Can = NULL;
...
void C_Can::SetDLLName(const QString &orc_DLLName) { mc_DllName = orc_DLLName; }

//----------------------------------------------------------------------------------------------------------------------

QString C_Can::GetDLLName(void) const { return mc_DllName; }
