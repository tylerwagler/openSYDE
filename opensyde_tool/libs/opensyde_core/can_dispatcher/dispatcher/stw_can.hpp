...
namespace stw {
namespace can {
// we keep the "old" naming conventions and prefixes to signal these are
// deprecated
...
struct T_STWCAN_Msg_RX {
  ...
  uint64_t u64_TimeStamp; ///< message reception time in micro-seconds since
                          ///< system start; 0 -> not available
};
...
const int32_t STATUS_FUNCTION_NOT_SUPPORTED =
    -1; // use old name for compatibility
...
typedef T_STWCAN_Status STW_CAN_STATUS; // we use the same structure in "old"
                                        // and "ext" DLL functions
...
typedef T_STWCAN_DLLInfo STW_CAN_DLL_INFO; // we use the same structure in "old"
                                           // and "ext" DLL functions
} // namespace can
} // namespace stw
#endif
