...
/*!
   ...
   Abstract base class for interfacing with different CAN communication driver
   architectures.
...
void C_CanBase::SetCommChannel(const uint8_t ou8_CommChannel) {
  if (mu8_CANOpened == 1U) {
    throw "Error in C_CAN_Base::SetDLLCommChannel: Channel can not be modified "
...
C_CanBase::C_CanBase(void) { m_InitClassBase(); }
...
/*!
   ...
   Assume there is no driver mechanism to check for OS signals for incoming
   frames. Override this function if there is a mechanism available in the
   target driver.
...
void C_CanBase::WaitForRxFrame(const uint32_t ou32_MaxWaitTimeMs) {
  ...
  // How efficient this approach is in not burning too much CPU time depends on
  // the used OS.
...
bool C_CanBase::GetXTDAvailable(void) const { return mq_XTDAvailable; }

//----------------------------------------------------------------------------------------------------------------------

uint8_t C_CanBase::GetCommChannel(void) const { return mu8_CommChannel; }
