...
/*!
   ...
   Header to an abstract base class for interfacing with different CAN
   communication driver architectures.
...
public:
  ...
  */
  ...
  /*!
     ...
     Can be used by an application to have a time basis for putting the
     timestamps of received messages into a timeframe.
  ...
  */
  ...
  /*!
     ...
     This function shall check whether at least one RX CAN frame is available
     for reading. It shall use operating system functions to wait without
     actively wasting CPU time. Depending on the type of CAN driver this is
     typically done with some kind of "select" or "poll" mechanism. If such a
     passive mechanism is not available then the function shall at least pass a
     minimum of CPU time to other threads and then return. In this scenario the
     function may return before the MaxWaitTime has elapsed.
