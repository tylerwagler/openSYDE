...
public:
   ...
   [[nodiscard]] std::error_code GetApplicationInformationBlocks(std::vector<C_OscApplicationInfoBlock> & orc_InfoBlocks,
                                                   const uint32_t ou32_SearchStartAddress,
                                                   const bool oq_OnlyOneBlock,
                                                   const bool oq_ExactAddressMatch, const bool oq_Block0Only);
   [[nodiscard]] std::error_code CalcFileChecksum(uint32_t & oru32_Checksum);

   [[nodiscard]] std::error_code GetSignatureBlockAddress(uint32_t & oru32_Address);
   [[nodiscard]] std::error_code ScanDeviceIdFromHexFile(std::string & orc_DeviceId);
   [[nodiscard]] std::error_code ScanApplicationInformationBlockFromHexFile(C_OscApplicationInfoBlock & orc_InfoBlock);
};
