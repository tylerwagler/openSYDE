...
/*! \brief  Activate flashloader of device.
   ...
   \return
   Errc::success    flashloader activated
   else             error occured
*/
//----------------------------------------------------------------------------------------------------------------------
std::error_code C_OscBuSequences::ActivateFlashLoader(const uint32_t ou32_FlashloaderResetWaitTime)
{
   std::error_code c_Return = Errc::success;
   ...
   m_ReportProgress(c_Return.value(), "Starting the flashloader activation ...");
   ...
   if (c_Return != Errc::success)
   {
      ...
      c_Text = PrintFormattedCompat("Could not set the \"request programming\" flag: Failed with result %d. "
                            "You still have the chance to reset the device manually ...", c_Return.value());
   ...
   m_ReportProgress(c_Return.value(), "Flashloader activation finished.");
...
*/
//----------------------------------------------------------------------------------------------------------------------
std::error_code C_OscBuSequences::ReadDeviceInformation(void)
{
   std::error_code c_Return = Errc::success;
   ...
   m_ReportProgress(c_Return.value(), "Read device information finished.");
...
*/
//----------------------------------------------------------------------------------------------------------------------
std::error_code C_OscBuSequences::UpdateNode(const std::string & orc_HexFilePath,
                                             ...
                                             const uint32_t ou32_TransferDataTimeout)
{
   ...
   m_ReportProgress(c_Return.value(), "Starting node update...");

   c_Return = h_ReadHexFile(orc_HexFilePath, c_HexFile, u32_SignatureBlockAddress);
   ...
   if (c_Return == Errc::success)
   {
      ...
      pc_HexDump = c_HexFile.GetDataDump(c_HexError);
      if (c_HexError)
      {
         c_Return = Errc::rd_wr;
   ...
   if ((c_Return == Errc::success) && (pc_HexDump != nullptr))
   {
   ...
   if ((c_Return == Errc::success) && (pc_HexDump != nullptr))
   {
      ...
      for (uint16_t u16_Area = 0U; u16_Area < pc_HexDump->at_Blocks.size(); u16_Area++)
      {
         ...
         if (c_Return == Errc::success)
         {
            ...
            m_ReportProgress(c_Return.value(), c_Text);
            m_ReportProgress(c_Return.value(), "Finished writing area, finalizing ...");
   ...
   m_ReportProgress(c_Return.value(), "Node update finished.");
...
/*! \brief  Read HEX file
   ...
   \return
   Errc::success    HEX file read
   Errc::rd_wr      Could not load file
   Errc::noact      No signature block found in HEX file
*/
//----------------------------------------------------------------------------------------------------------------------
std::error_code C_OscBuSequences::h_ReadHexFile(const std::string & orc_HexFilePath, C_OscHexFile & orc_HexFile,
                                        uint32_t & oru32_SignatureBlockAddress)
{
   ...
   c_HexError = orc_HexFile.LoadFromFile(orc_HexFilePath.c_str());
   if (c_HexError)
   {
      c_Return = Errc::rd_wr;
      osc_write_log_error(c_LogActivity, "Could not open the HEX file \"" + orc_HexFilePath + "\"! Details: " +
                          orc_HexFile.ErrorCodeToErrorText(c_HexError));
   }
   else
   {
      ...
      c_Return = orc_HexFile.GetSignatureBlockAddress(oru32_SignatureBlockAddress);
      if (c_Return != Errc::success)
      {
...
uint64_t C_OscBuSequences::h_GetAllHexFilesSize(const std::vector<std::string> & orc_HexFiles)
{
   ...
   {
      const std::error_code c_Return = h_ReadHexFile(orc_HexFiles[u32_Index], c_HexFile, u32_SignatureBlockAddress);

      if (c_Return == Errc::success)
      {
         //C_HexFile reports its own hex_file category, which is not the STW one
         std::error_code c_HexError = stw::hex_file::HexFileErrc::success;
         const stw::hex_file::C_HexDataDump * const pc_HexDump = c_HexFile.GetDataDump(c_HexError);
