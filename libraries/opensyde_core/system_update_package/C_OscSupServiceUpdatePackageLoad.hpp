...
{
public:
   [[nodiscard]] static std::error_code h_ProcessPackageUsingPemFiles(const std::string & orc_PackagePath,
   ...
   [[nodiscard]] static std::error_code h_ProcessPackage(const std::string & orc_PackagePath,
                                                         const std::string & orc_TargetUnzipPath,
...
private:
   [[nodiscard]] static std::error_code mh_CheckSupFiles(const std::string & orc_PackagePath);

   [[nodiscard]] static std::error_code mh_CheckParamsToProcessPackage(const std::string & orc_PackagePath,
                                                                       std::string & orc_TargetUnzipPath, const bool oq_IsZip);

   [[nodiscard]] static std::error_code mh_SetNodesUpdateOrder(const std::map<uint32_t,
                                                               uint32_t> & orc_UpdateOrderByNodes,
                                                               std::vector<uint32_t> & orc_NodesUpdateOrder);
   [[nodiscard]] static std::error_code mh_UnpackAndLoadNodes(const C_OscSystemDefinition & orc_SystemDefinition,
   ...
   [[nodiscard]] static std::error_code mh_UnpackNodes(const std::vector<uint8_t> & orc_DecryptNodes,
                                                       const std::vector<std::string> & orc_DecryptNodesPassword,
                                                       const uint32_t ou32_NodeCount,
                                                       const std::vector<std::string> & orc_PackageFiles,
                                                       const std::string & orc_TargetUnzipPath,
                                                       const std::vector<std::string> & orc_NodeFoldersAbs);
   [[nodiscard]] static std::error_code mh_VerifySignatures(const std::vector<C_OscSuSequences::C_DoFlash> & orc_ApplicationsToWrite,
                                                            const std::vector<uint8_t> & orc_ActiveNodes,
                                                            const std::vector<std::vector<uint8_t> > & orc_NodeSignatureKeys,
                                                            const std::vector<std::string> & orc_Signatures,
                                                            const std::vector<std::string> & orc_AbsSydeSecureDefFileNames);
   [[nodiscard]] static std::error_code mh_VerifySignature(const C_OscSuSequences::C_DoFlash & orc_ApplicationsToWrite,
