...
{
public:
   static std::error_code h_SaveNodes(const std::vector<std::string> & orc_Files,
                                      const std::vector<C_OscSupNodeDefinition> & orc_Nodes,
                                      const bool oq_UseMinorVersion1);
   static std::error_code h_LoadNodes(const std::vector<std::string> & orc_Files,
...
private:
   static std::error_code mh_SaveNode(const std::string & orc_File, const C_OscSupNodeDefinition & orc_Node,
                                      const bool oq_UseMinorVersion1);
   ...
   static std::error_code mh_LoadPemConfigSectionVersion1Minor1(C_OscSuSequences::C_DoFlash & orc_DoFlash,
   ...
   static void mh_DecideNodeRequiresFlashForSecurity(const C_OscSuSequences::C_DoFlash & orc_DoFlash,
                                                     ...
                                                     std::map<uint32_t,
                                                              uint32_t> & orc_PositionMap);
   static std::error_code mh_LoadPemConfigOption(const std::string & orc_NodeName, bool & orq_SendOption,
   ...
   static std::error_code mh_LoadSignatureFile(const std::string & orc_NodeFolderAbs,
                                               std::string & orc_Signature, C_OscXmlParserBase & orc_XmlParser);
   static std::error_code mh_CheckFileVersion(const bool oq_UseMinorVersion1, C_OscXmlParserBase & orc_XmlParser);
};
