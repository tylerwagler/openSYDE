...
{
public:
   [[nodiscard]] static std::error_code h_LoadNodeComProtocolFile(C_OscCanProtocol & orc_NodeComProtocol,
                                                    const std::string & orc_FilePath,
                                                    const std::vector<C_OscNodeDataPool> & orc_NodeDataPools);
   [[nodiscard]] static std::error_code h_LoadNodeComProtocol(C_OscCanProtocol & orc_NodeComProtocol,
                                                C_OscXmlParserBase & orc_XmlParser,
                                                const std::vector<C_OscNodeDataPool> & orc_NodeDataPools);
   [[nodiscard]] static std::error_code h_SaveNodeComProtocolFile(const C_OscCanProtocol & orc_NodeComProtocol,
                                                    const std::string & orc_FilePath,
   ...
   static void h_SaveNodeComProtocol(const C_OscCanProtocol & orc_NodeComProtocol, C_OscXmlParserBase & orc_XmlParser,
                                     const std::string & orc_DatapoolName);
   [[nodiscard]] static std::error_code h_LoadNodeComMessageContainers(
   ...
   [[nodiscard]] static std::error_code h_LoadNodeComMessageContainer(C_OscCanMessageContainer & orc_NodeComMessageContainer,
                                                        C_OscXmlParserBase & orc_XmlParser);
   ...
   [[nodiscard]] static std::error_code h_LoadNodeComMessages(std::vector<C_OscCanMessage> & orc_NodeComMessages,
                                                C_OscXmlParserBase & orc_XmlParser);
   static void h_SaveNodeComMessages(const std::vector<C_OscCanMessage> & orc_NodeComMessages,
                                     C_OscXmlParserBase & orc_XmlParser,
                                     const C_OscCanProtocol::E_Type oe_ProtocolType);
   [[nodiscard]] static std::error_code h_LoadNodeComMessage(C_OscCanMessage & orc_NodeComMessage,
                                               C_OscXmlParserBase & orc_XmlParser);
   static void h_SaveNodeComMessage(const C_OscCanMessage & orc_NodeComMessage, C_OscXmlParserBase & orc_XmlParser,
                                    const C_OscCanProtocol::E_Type oe_ProtocolType);
   [[nodiscard]] static std::error_code h_LoadNodeComSignals(std::vector<C_OscCanSignal> & orc_NodeComSignals,
                                               C_OscXmlParserBase & orc_XmlParser);
   static void h_SaveNodeComSignals(const std::vector<C_OscCanSignal> & orc_NodeComSignals,
                                    C_OscXmlParserBase & orc_XmlParser, const C_OscCanProtocol::E_Type oe_ProtocolType);
   [[nodiscard]] static std::error_code h_LoadNodeComSignal(C_OscCanSignal & orc_NodeComSignal, C_OscXmlParserBase & orc_XmlParser,
   ...
   [[nodiscard]] static std::error_code h_StringToCommunicationProtocol(const std::string & orc_String,
   ...
   [[nodiscard]] static std::error_code h_LoadNodeOwnerIndex(C_OscCanInterfaceId & orc_OwnerNodeIndex,
                                               const C_OscXmlParserBase & orc_XmlParser);

private:
   static void mh_SaveMessageCanOpenPart(const C_OscCanMessage & orc_NodeCommMessage,
                                         C_OscXmlParserBase & orc_XmlParser);
   [[nodiscard]] static std::error_code mh_LoadMessageCanOpenPart(C_OscCanMessage & orc_NodeCommMessage,
                                                    C_OscXmlParserBase & orc_XmlParser);
   static void mh_SaveNodeOwnerIndices(const C_OscCanInterfaceId & orc_OwnerNodeIndex,
                                       C_OscXmlParserBase & orc_XmlParser);
   [[nodiscard]] static std::error_code mh_LoadNodeOwnerIndices(C_OscCanInterfaceId & orc_OwnerNodeIndex,
                                                  C_OscXmlParserBase & orc_XmlParser);
   static void mh_SaveSignalCanOpenPart(const C_OscCanSignal & orc_NodeCommSignal, C_OscXmlParserBase & orc_XmlParser);
   [[nodiscard]] static std::error_code mh_LoadSignalCanOpenPart(C_OscCanSignal & orc_NodeCommSignal,
                                                   C_OscXmlParserBase & orc_XmlParser);
   static void mh_SaveSignalJ1939Part(const C_OscCanSignal & orc_NodeCommSignal, C_OscXmlParserBase & orc_XmlParser);
   [[nodiscard]] static std::error_code mh_LoadSignalJ1939Part(C_OscCanSignal & orc_NodeCommSignal,
                                                 C_OscXmlParserBase & orc_XmlParser);
   static std::string mh_CommunicationByteOrderToString(
      const C_OscCanSignal::E_ByteOrderType & ore_CommunicationByteOrder);
   [[nodiscard]] static std::error_code mh_StringToCommunicationByteOrder(const std::string & orc_String,
   ...
   [[nodiscard]] static std::error_code mh_StringToCommunicationMuxType(const std::string & orc_String,
