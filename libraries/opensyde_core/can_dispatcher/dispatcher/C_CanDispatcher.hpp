...
public:
   ...
   [[nodiscard]] std::error_code Push(const T_STWCAN_Msg_RX & orc_Message);
   [[nodiscard]] std::error_code Pop(T_STWCAN_Msg_RX & orc_Message);
   ...
   [[nodiscard]] std::error_code GetStatus(void);
};
...
protected:
   ...
   */
   //-----------------------------------------------------------------------------
   [[nodiscard]] virtual std::error_code m_CAN_Read_Msg(T_STWCAN_Msg_RX & orc_Message) = 0;
...
public:
   ...
   [[nodiscard]] std::error_code RegisterClient(uint16_t & oru16_Handle, const C_CanRxFilter * const opc_RXFilter = nullptr,
                                                const uint32_t & oru32_BufferSize = mu32_CAN_QUEUE_DEFAULT_MAX_SIZE);
   [[nodiscard]] std::error_code RemoveClient(const uint16_t ou16_Handle);
   [[nodiscard]] std::error_code SetRXFilter(const uint16_t ou16_Handle, const C_CanRxFilter & orc_RXFilter);

   [[nodiscard]] std::error_code ReadFromQueue(const uint16_t ou16_Handle, T_STWCAN_Msg_RX & orc_Message);
   [[nodiscard]] std::error_code ClearQueue(const uint16_t ou16_Handle);

   //we hide the base class function on purpose here
   [[nodiscard]] std::error_code CAN_Read_Msg(const uint16_t ou16_Handle, T_STWCAN_Msg_RX & orc_Message); //lint !e1411
   //try to read from CAN driver and add message to all installed RX queues:
   [[nodiscard]] virtual std::error_code CAN_Read_Msg(T_STWCAN_Msg_RX & orc_Message);
};
