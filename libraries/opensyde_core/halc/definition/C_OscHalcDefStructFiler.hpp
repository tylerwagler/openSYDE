...
{
public:
   [[nodiscard]] static std::error_code h_LoadStructs(std::vector<C_OscHalcDefStruct> & orc_Structs,
   ...
   [[nodiscard]] static std::error_code h_SaveStructs(const std::vector<C_OscHalcDefStruct> & orc_Structs,
                                        const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
   ...
   [[nodiscard]] static std::error_code h_ParseSimplestTypeValue(const std::string & orc_TypeStr,
                                                   C_OscNodeDataPoolContent & orc_Content,
                                                   const C_OscXmlParserBase & orc_XmlParser,
                                                   const std::string & orc_AttributeName);
   [[nodiscard]] static std::error_code h_GetTypeForSimplestTypeString(const std::string & orc_TypeStr,
                                                         C_OscNodeDataPoolContent::E_Type & ore_Type,
                                                         const C_OscXmlParserBase & orc_XmlParser);
   static std::string h_GetTypeString(const C_OscNodeDataPoolContent::E_Type oe_Type);
   [[nodiscard]] static std::error_code h_SaveSimpleValueAsAttribute(const std::string & orc_Node,
                                                       C_OscXmlParserBase & orc_XmlParser,
                                                       const C_OscNodeDataPoolContent & orc_Content);
   [[nodiscard]] static std::error_code h_SetType(C_OscXmlParserBase & orc_XmlParser, C_OscHalcDefContent & orc_Content,
                                    std::string & orc_TypeStr, std::string & orc_BaseTypeStr,
                                    const std::string & orc_CurrentNodeName);
   [[nodiscard]] static std::error_code h_ParseAttributeIntoContent(C_OscHalcDefContent & orc_Content,
                                                      const C_OscXmlParserBase & orc_XmlParser,
                                                      const std::string & orc_AttributeName,
...
private:
   ...
   [[nodiscard]] static std::error_code mh_LoadStruct(C_OscHalcDefStruct & orc_Struct, C_OscXmlParserBase & orc_XmlParser,
                                        const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
                                        const std::string & orc_GroupNodeName,
                                        const std::string & orc_SingleNodeName, const bool oq_RequireId,
                                        const uint32_t ou32_DomainNameLength);
   [[nodiscard]] static std::error_code mh_SaveStruct(const C_OscHalcDefStruct & orc_Struct,
                                        const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
                                        C_OscXmlParserBase & orc_XmlParser, const std::string & orc_GroupNodeName,
                                        const std::string & orc_SingleNodeName);
   [[nodiscard]] static std::error_code mh_LoadDataElement(C_OscHalcDefElement & orc_Element, C_OscXmlParserBase & orc_XmlParser,
                                             const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
                                             const std::vector<uint32_t> & orc_BaseAvailability,
                                             const std::string & orc_SingleNodeName, const bool oq_RequireId,
                                             const uint32_t ou32_DomainNameLength);
   [[nodiscard]] static std::error_code mh_SaveDataElement(const C_OscHalcDefElement & orc_Element,
                                             const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
                                             C_OscXmlParserBase & orc_XmlParser,
                                             const std::string & orc_SingleNodeName);
   [[nodiscard]] static std::error_code mh_ParseAttributeAvailability(std::vector<uint32_t> & orc_Availability,
                                                        const std::string & orc_AttributeContent,
                                                        const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
                                                        const C_OscXmlParserBase & orc_XmlParser);
   [[nodiscard]] static std::error_code mh_GetAvailabilityString(const std::vector<uint32_t> & orc_Availability,
                                                   const std::vector<C_OscHalcDefChannelUseCase> & orc_UseCases,
   ...
   [[nodiscard]] static std::error_code mh_CheckValidUint(const std::string & orc_Item);
   [[nodiscard]] static std::error_code mh_CheckValidSint(const std::string & orc_Item);
   [[nodiscard]] static std::error_code mh_CheckValidDouble(const std::string & orc_Item);
};
