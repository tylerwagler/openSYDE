...
std::error_code C_OscConfFileHandler::LoadSettings(const std::string & orc_Path)
{
   ...
   if (c_Result == Errc::success)
   {
      ...
      for (uint32_t u32_Line = 0U; u32_Line < static_cast<uint32_t>(c_StringListSource.size()); u32_Line++)
      {
         const std::string c_Line = TrimCompat(c_StringListSource[u32_Line]);

         if ((c_Line.empty() == false) && (c_Line.find('#') == std::string::npos))
         {
                                                         ...
                                                         const std::vector<std::pair<std::string, std::string> > & orc_Configs)
{
   ...
   if (c_Result == Errc::success)
   {
      ...
      for (u32_Counter = 0U; u32_Counter < static_cast<uint32_t>(orc_Configs.size()); u32_Counter++)
      {
         ...
         {
            c_StringList[static_cast<uint32_t>(s32_Index)] = c_NewEntry;
         }
      }
