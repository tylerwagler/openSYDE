...
**Next:** **Phase B step 8** — HALC. **Sub-stepped 8a / 8b / 8c / 8d.** This is the load-bearing cluster of ~17 `_Serialization.cpp` zombie files that block the `C_OscNodeDataPoolContent` strip and several other strips. Doing it all in one session is 6–12 hours of focused work, so it is split into four sub-steps (definition tree → configuration tree → standalone tree → coordinated strip). **See the dedicated "Phase B step 8 — HALC sub-stepping plan" section below for the complete walkthrough — that section is self-contained and the agent doing this work should read it in full before touching any HALC file.** Also read canonical plan sections 6 ("HALC vendor XML → JSON converter") and 9 ("Resolved decisions") for the locked decision that the main openSYDE tool reads only JSON HALC files; vendor XML conversion is a separate offline tool, out of scope for Phase B step 8.
