...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvComMessageMonitor::StartAddOsySysDef(const std::string & orc_PathSystemDefinition)
{
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvComMessageMonitor::StartAddDbcFile(const std::string & orc_PathDbc)
{
   int32_t s32_Return = C_NO_ERR;
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvComMessageMonitor::GetDbcFile(const std::string & orc_PathDbc,
                                           C_CieConverter::C_CieCommDefinition & orc_DbcDefinition)
{
   int32_t s32_Return = C_RANGE;

   std::map<std::string, C_CieConverter::C_CieCommDefinition>::iterator c_ItDbc;
...
*/
//----------------------------------------------------------------------------------------------------------------------
std::error_code C_SyvComMessageMonitor::RemoveDatabase(const std::string & orc_Path)
{
   ...
   if (c_Return == Errc::noact)
   {
      ...
      c_Return = C_OscComMessageLogger::RemoveDatabase(orc_Path);
   }
   this->mc_CriticalSectionConfig.unlock();
...
std::error_code C_SyvComMessageMonitor::ActivateDatabase(const std::string & orc_Path, const bool oq_Active)
{
   ...
   //Logger handling
   this->mc_CriticalSectionConfig.lock();
   c_Return = C_OscComMessageLogger::ActivateDatabase(orc_Path, oq_Active);
   this->mc_CriticalSectionConfig.unlock();
...
std::error_code C_SyvComMessageMonitor::HandleCanMessage(const T_STWCAN_Msg_RX & orc_Msg, const bool oq_IsTx)
{
   ...
   std::string c_RawTsAbsStr;
   std::string c_RawTsTodStr;
   std::string c_RawTsRelStr;
   std::string c_RawCounter;
   ...
   if (q_Reassembled)
   {
      ...
      this->mc_CriticalSectionCounter.lock();
      c_Return = C_OscComMessageLogger::HandleCanMessage(orc_Msg, oq_IsTx);
      this->mc_CriticalSectionCounter.unlock();
      ...
      this->mc_CriticalSectionCounter.lock();
      c_Return = C_OscComMessageLogger::HandleCanMessage(c_ReconstMsg, oq_IsTx);
      this->mc_CriticalSectionCounter.unlock();
   }
   else
   {
   ...
   if (c_Return == Errc::success)
   {
      ...
      if (q_Reassembled)
      {
         ...
         c_MessageData.c_TimeStampAbsoluteTimeOfDay = c_RawTsTodStr;
          ...
          c_MessageData.c_Counter = std::to_string(
             this->mc_MessageCounter[orc_Msg.u32_ID]);
...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SyvComMessageMonitor::m_InsertOsySysDef(const std::string & orc_PathSystemDefinition,
...
*/
//----------------------------------------------------------------------------------------------------------------------
std::string C_SyvComMessageMonitor::m_GetProtocolStringHexHook(void) const
{
   // TODO for logging
   return "";
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
std::string C_SyvComMessageMonitor::m_GetProtocolStringDecHook(void) const
{
   // TODO for logging
   return "";
}
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_SyvComMessageMonitor::m_AddDbcFile(const std::string & orc_PathDbc)
{
   int32_t s32_Return = C_RANGE;
   ...
   if (LowerCaseCompat(SubStringCompat(orc_PathDbc, orc_PathDbc.length() - 3U, 4U)) == ".dbc")
   {
      ...
      {
         this->mc_CriticalSectionConfig.lock();
         this->mc_DbcFiles.emplace(std::pair<std::string, C_CieConverter::C_CieCommDefinition>(orc_PathDbc,
                                                                                               c_DbcDefinition));

         // Register the database in the activation flag map
         this->mc_DatabaseActiveFlags.emplace(std::pair<std::string, bool>(orc_PathDbc, true));
         ...
         if (rc_DbcSignal.e_MultiplexerType == C_OscCanSignal::eMUX_MULTIPLEXER_SIGNAL)
         {
            ...
            if (orc_MessageData.c_Signals.size() > 0)
            {
               ...
               if (rc_Signal.q_DlcError == false)
               {
                  try
                  {
                     u16_MultiplexerValue = static_cast<uint16_t>(std::stoi(rc_Signal.c_RawValueDec));
                                                          ...
                                                          const C_CieConverter::C_CieCanSignal & orc_DbcSignal)
{
   ...
   if (orc_DbcSignal.c_ValueDescription.size() > 0)
   {
      ...
      try
      {
         const int64_t s64_Value = std::stoll(c_Signal.c_RawValueDec);
         const std::map<int64_t, std::string>::const_iterator c_ItDescription =
...
void C_SyvComMessageMonitor::m_ThreadFunc(void)
{
   ...
   case eADD_OSY_SYSDEF_WITHOUT_BUSINDEX:
      //boundary: the callee now reports std::error_code
      this->ms32_Result = this->AddOsySysDef(this->mc_Path, this->mc_Busses).value();
      break;
   case eADD_OSY_SYSDEF_WITH_BUSINDEX:
      //boundary: the callee now reports std::error_code
      this->ms32_Result = this->AddOsySysDef(this->mc_Path, this->mu32_BusIndex, this->mc_Busses).value();
      break;
   case eADD_DBC_FILE:
