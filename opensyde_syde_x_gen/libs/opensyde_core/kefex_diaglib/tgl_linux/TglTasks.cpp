...
static void m_SplitCommandLineParameters(const C_SclString & orc_Parameters, std::vector<std::string> & orc_Arguments);
...
static void m_SplitCommandLineParameters(const C_SclString & orc_Parameters, std::vector<std::string> & orc_Arguments)
{
   const std::string c_Parameters = orc_Parameters.c_str();
   ...
   for (uint32_t u32_It = 0U; u32_It < c_Parameters.size(); ++u32_It)
   {
      const char_t cn_Char = c_Parameters[u32_It];
...
/*! \brief   Start process and return after start
   ...
   This ensured that relative paths in the executable and its dependencies work as expected.
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t stw::tgl::TglStartProcessDetached(const C_SclString & orc_BinaryPath, const C_SclString & orc_Parameters)
{
   ...
   c_ArgumentStorage.push_back(orc_BinaryPath.c_str());
   ...
   if (q_Continue == true)
   {
      const pid_t x_ForkResult = //lint !e8080 //using API type
                                 fork();
      if (x_ForkResult == 0)
      {
         int32_t s32_ExecErrno;
         std::string c_BinaryDir;
         std::vector<char_t *> c_Argv;
         ssize_t x_WriteResult; //lint !e8080 //using API type
         ...
         // expected.
         c_BinaryDir = TglExtractFilePath(orc_BinaryPath).c_str();
         if (chdir(c_BinaryDir.c_str()) != 0)
         {
            s32_ExecErrno = errno;
            // Report startup failure back to the parent.
            // assign and then ignore the result; nothing serious can happen; but not using the return value can trigger
            // GCC build warnings
         ...
         for (uint32_t u32_It = 0U; u32_It < c_ArgumentStorage.size(); ++u32_It)
         {
            ...
            //lint -e{9005} //const_cast necessary to match execv parameter type; data is not actually modified
            c_Argv.push_back(const_cast<char_t *>(c_ArgumentStorage[u32_It].c_str()));
         }
      ...
      else if (x_ForkResult > 0)
      {
         ...
         ssize_t x_ReadResult; //lint !e8080 //using API type
